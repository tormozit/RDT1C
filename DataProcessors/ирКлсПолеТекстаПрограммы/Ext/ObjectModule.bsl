...
Функция УстановитьТекущийКонтекстМетода(НовыйТекст) Экспорт
	...
	ПолеТекста.УстановитьГраницыВыделения(мНачальнаяСтрока, мНачальнаяКолонка, мНачальнаяСтрока, мНачальнаяКолонка);
...
//
Функция НайтиПоказатьСловоВТексте(СтрокаПоиска, СловоЦеликом = Истина, ПолеТекста = Неопределено, УстановитьФокус = Ложь) Экспорт 
	
	Если ПолеТекста = Неопределено Тогда
		ПолеТекста = ЭтотОбъект.ПолеТекста;
	КонецЕсли; 
	Результат = ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ФормаВладелец, ПолеТекста, СтрокаПоиска, СловоЦеликом);
...
Процедура ВыделитьВесьТекст(ПолеТекста = Неопределено) Экспорт 
	...
	ПолеТекста.УстановитьГраницыВыделения(1, 1, Макс(1, КоличествоСтрок), СтрДлина(ПолеТекста.ПолучитьСтроку(КоличествоСтрок) + 1));
...
Процедура ПослеУстановкиВыделенногоМногострочногоТекста()
	...
	Если ФормаВладелец = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Если ТипЗнч(ФормаВладелец.ТекущийЭлемент) = Тип("ПолеТекстовогоДокумента") Тогда
		...
		ПолеТекста.УстановитьГраницыВыделения(НачСтрока, НачКолонка, КонСтрока, КонКолонка);
...
//  Нет.
//
Процедура УстановитьФокус(Надежно = Ложь) Экспорт 
	...
	Если ТипЗнч(ФормаВладелец.ТекущийЭлемент) = Тип("ПолеHTMLДокумента") Тогда
		...
		//Если Надежно Тогда
		//	// Для возвращения фокуса после отображения диалога ввода строки в шаблоне \дляиз
		//	//ФормаВладелец.Открыть();
		//	ирОбщий.ОтправитьНажатияКлавишЛкс("%");
		//	ирОбщий.ОтправитьНажатияКлавишЛкс("%");
		//КонецЕсли; 
		ПолеТекста.ВосстановитьФокусВвода();
	КонецЕсли; 
...
Функция ВыделитьТекстТекущегоСтроковогоЛитерала() Экспорт 
	...
	Для Каждого Match Из Результат Цикл
		...
		Тогда
			ПолеТекста.УстановитьГраницыВыделения(Match.FirstIndex + 1, Match.FirstIndex + Match.Length + 1);
			Успех = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Функция ЗаполнитьТаблицуСлов(ТаблицаСтруктурТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, РазрешитьОткрытиеОкон = Истина, ЗаполнятьТипы = Истина) Экспорт
	...
	Если ТаблицаСтруктурТиповКонтекста.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли; 
	ГлубокоВычислятьТипыВСпискеАвтодополнения = Истина
		И ЗаполнятьТипы
		И (Ложь
			Или ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ГлубокоВычислятьТипыВСпискеАвтодополнения") = Истина 
			Или ЯзыкПрограммы <> 0);
	ТаблицаСлов.Очистить();
	Если мЭтоСтроковыйЛитерал Тогда
	...
	Попытка
		ВнутренняяТаблицаСлов = мПлатформа.ПолучитьТаблицуСловСтруктурыТипа(выхСтруктураТипаКонтекста, ЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ГлубокоВычислятьТипыВСпискеАвтодополнения,,, ЛиСерверныйКонтекст);
		#Если Сервер И Не Сервер Тогда
		...
		Тогда
			...
			Если ЗаполнятьТипы Тогда
				ЗагрузитьВТаблицуСловИзВнутреннейТаблицыСлов(ВнутренняяТаблицаСлов, ВыгрузкаТаблицыСлов);
			Иначе
				ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ВнутренняяТаблицаСлов, ВыгрузкаТаблицыСлов);
			КонецЕсли; 
		...
		Иначе
			Если ЗаполнятьТипы Тогда
				ЗагрузитьВТаблицуСловИзВнутреннейТаблицыСлов(ВнутренняяТаблицаСлов, ТаблицаСлов);
			Иначе
				ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ВнутренняяТаблицаСлов, ТаблицаСлов,, Новый Структура("ТипЗначения", "??"));
			КонецЕсли; 
		КонецЕсли; 
	Исключение
		Ошибка = ИнформацияОбОшибке();
...
Процедура ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, КонкретныйТип, СтруктураТипаКонтекста, ТаблицаСтруктурТиповКонтекста = Неопределено, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	#КонецЕсли
	ТаблицаСтатистикиВыбора = мПлатформа.ТаблицаСтатистикиВыбора;
	ГлубокоВычислятьТипыВСпискеАвтодополнения = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ГлубокоВычислятьТипыВСпискеАвтодополнения") = Истина Или ЯзыкПрограммы <> 0;
	...
	Если Не мЭтоСтроковыйЛитерал И СтрокаРезультата.ТипСлова = "Метод" Тогда
		...
		Иначе
			Если Истина
				...
				//И ГлубокоВычислятьТипыВСпискеАвтодополнения
			Тогда
	...
	Если Истина
		И ПараметрЗакрытияПодсказки <> Истина
		И (Ложь
			Или Не ГлубокоВычислятьТипыВСпискеАвтодополнения
...
Функция АвтоматическаяПодсказкаПоВызовуМетода()
	Результат = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".АвтоматическаяПодсказкаПоВызовуМетода") <> Ложь;
	Возврат Результат;
КонецФункции
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(Знач СтруктураТипаКонтекста, Знач ТекущаяСтрока) Экспорт 
	#Если Сервер И Не Сервер Тогда
...
Процедура ВыполнитьШаблонТекста()
	...
	Если КонечныйТекстЗамены <> Неопределено Тогда
		...
		мНачальнаяКолонка = мКонечнаяКолонка;
	КонецЕсли;
	УстановитьФокус(НадежноУстановитьФокус);
...
Процедура РедакторHTML_Инициировать(ПолеHTML) Экспорт 
	...
	РедакторHTML.setOption("disableNativeSuggestions", Истина); // События перестают вызываться
	РедакторHTML.setOption("disableNativeSignatures", Истина); // События перестают вызываться
	РедакторHTML.setOption("disableNativeHovers", Истина); // События перестают вызываться
	// Отключаем вставку точки
...
Процедура РедакторHTML_ПередПоказомСигнатуры(НомерПараметра, ИмяМетодаОтРедактора, НомерСигнатуры) Экспорт 
	...
	Если Не ЛиДоступноОткрытиеСвободнойФормы() Тогда 
		...
		Для ИндексВарианта = 0 По ФормаВызовМетода.ВариантыСинтаксиса.Количество() - 1 Цикл
			...
			ПодсказкаСлова.Вставить("documentation", "Метод: " + ирОбщий.ПоследнийФрагментЛкс(ФормаВызовМетода.ИмяМетода, " ") + ?(ЗначениеЗаполнено(ФормаВызовМетода.ТекущийВариант), ". " + ФормаВызовМетода.ТекущийВариант, ""));
	...
	Тогда 
		...
		Иначе
			...
			Если ПоказатьСписок Тогда
				Если ПоказыватьВсеТипыВСпискеАвтодополненияHTML = Неопределено Тогда
					ПоказыватьВсеТипыВСпискеАвтодополненияHTML = Истина = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ПоказыватьВсеТипыВСпискеАвтодополненияHTML");
				КонецЕсли; 
				ПоказыватьВсеТипыВСпискеАвтодополненияHTML = Истина; // Временно 
				РедакторHTML.enableSuggestActivationEvent(Истина, ПоказыватьВсеТипыВСпискеАвтодополненияHTML);  // Второй параметр включает отображение типов для всех слов списка автодополнения https://github.com/salexdv/bsl_console/issues/119
				УспехЗаполнения = ЗаполнитьТаблицуСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста, Триггер = "space", ПоказыватьВсеТипыВСпискеАвтодополненияHTML);
				Если Истина
					И УспехЗаполнения
					И (Ложь
						Или Триггер = "space"
...
Процедура РедакторHTML_ПриВыбореСтрокиАвтодополнения(Триггер, ПоследнееВыражение, ВыбранноеСлово)  Экспорт 
...
Процедура РедакторHTML_ПриАктивацииСтрокиАвтодополнения(Триггер, ТекущееСлово, НомерСлова)  Экспорт 
	
	// Еще есть событие EVENT_ON_DETAIL_SUGGEST_ROW
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
	#КонецЕсли
	РедакторHTML = ПолеТекста.РедакторHTML();
	...
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, СтрокаТаблицыСлов);
		// Показывать надо обязательно каждый раз, т.к. редактор забывает предыдущие уточнения типа
		Если Триггер = "hover" Тогда
			РедакторHTML.setSuggestItemDetailById(НомерСлова, СтрокаТаблицыСлов.ТипЗначения);
		Иначе
...
Процедура РедакторHTML_ПриКлике() Экспорт 
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
	#КонецЕсли
	РедакторHTML = ПолеТекста.РедакторHTML();
