...
Процедура ВнешнееСобытиеОбъекта(Источник, Событие, Данные) Экспорт 
	...
	Если Ложь
		...
		//Или Не ирОбщий.Форма_ЛиФокусВводаЛкс(ФормаВладелец) // Для поля HTML это затратно
	Тогда
		Возврат;
	КонецЕсли;
	...
	Если Истина
		...
		И ТипЗнч(ПолеТекста.ЭлементФормы) = Тип("ПолеТекстовогоДокумента")
		#КонецЕсли
		И ФормаВладелец.ТекущийЭлемент = ПолеТекста.ЭлементФормы 
	Тогда
		Если Ложь
...
//
Функция ТекущееОбъектноеВыражение(НачальнаяСтрока = 0, НачальнаяКолонка = 0, выхЕстьТочкаСправа = Ложь, КончитьОбработкуКоманды = Истина) Экспорт
	...
	РазобратьТекущийКонтекст(, выхЕстьТочкаСправа,, НачальнаяСтрока, НачальнаяКолонка);
...
Функция ВыполнитьКоманду(Знач Команда)
	...
	Попытка
		...
		ИначеЕсли Команда = "ВставитьИзБуфераОбменаВесьТекст" Тогда
			ирОбщий.УстановитьТекстСОткатомЛкс(ПолеТекста, ирОбщий.ПолучитьТекстИзБуфераОбменаОСЛкс());
		ИначеЕсли Команда = "КопироватьВБуферОбменаВесьТекст" Тогда
			ирОбщий.ПоместитьТекстВБуферОбменаОСЛкс(ПолеТекста.ПолучитьТекст());
...
Процедура АвтоОбновитьСправку() Экспорт 
	...
	Если Ложь
		Или Не ирОбщий.Форма_ЛиФокусВводаЛкс(ФормаВладелец) 
...
Процедура ЗаменитьФрагментТекстаВПоле(Знач ПолеТекста, Знач ВыделенныйФрагмент, Знач ДлинаПоследнейСтроки, Знач ПерваяСтрока, Знач ПоследняяСтрока, Знач ПоследняяКолонка)
	...
	КонецЕсли;
	ПолеТекста.УстановитьГраницыВыделения(ПерваяСтрока, 1, ПоследняяСтрока, 1 + ДлинаПоследнейСтроки);
	...
	Если ЧислоПереносовСтрокНачальное > ПоследняяСтрока - ПерваяСтрока Тогда
		...
		Для Счетчик = 1 По Мин(2, ДлинаПоследнейСтроки - 1) Цикл 
			...
			ПолеТекста.УстановитьГраницыВыделения(ПерваяСтрока, 1, ПоследняяСтрока, 1 + ДлинаПоследнейСтрокиБезПереноса);
...
//
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, КакВызовМетода = Ложь, НачальнаяСтрока = 0, НачальнаяКолонка = 0) Экспорт 
	...
	Если Не ЗначениеЗаполнено(НачальнаяСтрока) Тогда
		ПолеТекста.ПолучитьГраницыВыделения(мНачальнаяСтрока, мНачальнаяКолонка, мКонечнаяСтрока, мКонечнаяКолонка);
	Иначе
		мНачальнаяСтрока = НачальнаяСтрока;
		мКонечнаяСтрока = НачальнаяСтрока;
	    мНачальнаяКолонка = НачальнаяКолонка;
		мКонечнаяКолонка = НачальнаяКолонка;
	КонецЕсли; 
	ВыделитьНачалоИКонецТекущейСтроки();
...
Процедура ПоказатьТекущиеКоординаты(ПолеТекста, СтартоваяСтрокаДляОтображенияОшибок = 0)
	...
	ПолеТекста.УстановитьГраницыВыделения(мПарсер.CurrentLineNumber() + СтартоваяСтрокаДляОтображенияОшибок, 1,
...
Функция РазобратьТекстЗапроса(ТекстЗапроса, СокращенноеДерево = Ложь, ОбновлятьСостояние = Истина, Знач пПолеТекста = Неопределено,
	...
	Пока Не Закончили Цикл 
		...
		Тогда
			...
			Если ТипЗнч(пПолеТекста.ЭлементФормы) = Тип("ПолеТекстовогоДокумента") Тогда
...
Процедура ПерейтиКОпределению()
	...
	Если ПоследнееВхождение <> Неопределено Тогда
		...
		СлужебноеПолеТекста.ПолучитьГраницыВыделения(мНачальнаяСтрока, мНачальнаяКолонка, мКонечнаяСтрока, мКонечнаяКолонка);
		ПолеТекста.УстановитьГраницыВыделения(НачальнаяПозицияОпределения, КонечнаяПозицияОпределения);
		//ПолеТекста.ПолучитьГраницыВыделения(мНачальнаяСтрока, мНачальнаяКолонка, мКонечнаяСтрока, мКонечнаяКолонка);
	Иначе
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	Пока Истина Цикл
		Успешно = ЗаполнитьТаблицСлов(ТаблицаСтруктурТиповКонтекста, КонкретныйТип, СтруктураТипаКонтекста);
...
Функция ЗаполнитьТаблицСлов(ТаблицаСтруктурТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено) Экспорт
	...
	Если ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" И мЭтоСтроковыйЛитерал Тогда 
		...
		Если ВыбраннаяСтруктураТипа.ТипЯзыка = "ИмяМетода" Тогда
			Если ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
				...
				ВнутренняяТаблицаСлов = мПлатформа.ПолучитьВнутреннююТаблицуМетаданныхСлов(выхСтруктураТипаКонтекста,, "Метод",,,,,, Истина);
