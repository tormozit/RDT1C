...
Процедура ЗаменитьФрагментТекстаВПоле(Знач Элемент, Знач ВыделенныйФрагмент, Знач ДлинаПоследнейСтроки, Знач ПерваяСтрока, Знач ПоследняяСтрока, Знач ПоследняяКолонка)
	...
	ДлинаПоследнейСтрокиБезПереноса = ДлинаПоследнейСтроки;
	Пока ДлинаПоследнейСтрокиБезПереноса > 0 И СтрЧислоВхождений(Элемент.ВыделенныйТекст, Символы.ПС) > 0 Цикл 
		// https://www.hostedredmine.com/issues/925662
		ДлинаПоследнейСтрокиБезПереноса = ДлинаПоследнейСтрокиБезПереноса - 1;
