...
Процедура РедакторHTML_ПередПоказомПодсказкиУдержания(Знач ИмяФункцииВычислитьВыражение, Знач НомерКолонки23, Знач НомерСтроки23, Знач ТекущееСлово) Экспорт 
	...
	Тогда
		ИмяТипаЗначения = "";
		Если Ложь
			Или (ЯзыкПрограммы > 0 И Лев(ОбъектноеВыражение, 1) = "&")
...
Процедура РедакторHTML_ПередПоказомСигнатуры(НомерПараметра, ИмяМетодаОтРедактора, НомерСигнатуры, Триггер) Экспорт 
	...
	Если Не ЛиДоступноОткрытиеСвободнойФормы() Тогда 
		...
		ФормаВызовМетода.ПараметрСтруктураТипаКонтекста = Неопределено;
		ФормаВызовМетода.ОбновитьИлиЗакрытьФорму(, Истина);
		Если Не ФормаВызовМетода.ЕстьПолезнаяИнформация И Триггер <> Неопределено Тогда
			Возврат;
		КонецЕсли; 
		...
		Для ИндексВарианта = 0 По ФормаВызовМетода.ВариантыСинтаксиса.Количество() - 1 Цикл
			...
			КонецЦикла;
			ПредставлениеМетода = ирОбщий.ПредставлениеЗначенияСОграничениемДлиныЛкс(ФормаВызовМетода.ИмяМетода, 13) + "(" + ирОбщий.СтрСоединитьЛкс(КраткиеОписанияПараметров, ", ") + ")";
