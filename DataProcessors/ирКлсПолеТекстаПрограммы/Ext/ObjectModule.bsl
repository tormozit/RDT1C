...
Процедура ВнешнееСобытиеОбъекта(Источник, Событие, Данные) Экспорт 
	...
	Иначе
		Символ = "";
	КонецЕсли;
	Если ФормаВладелец.ТекущийЭлемент = ПолеТекста.ПолеТекста Тогда
...
Процедура УстановитьФокус() Экспорт 
	...
	ФормаВладелец.ТекущийЭлемент = ПолеТекста.ПолеТекста;
...
Функция ПроверитьПрограммныйКод(СообщатьОбУспешнойПроверке = Ложь, ТекстДляПроверки = Неопределено, Контекст = "") Экспорт
	...
	Иначе
		Если СообщатьОбУспешнойПроверке Тогда
			ирОбщий.СообщитьСУчетомМодальностиЛкс(ирОбщий.ПредставлениеИзИдентификатораЛкс(ПолеТекста.Имя) 
...
//
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, КакВызовМетода = Ложь) Экспорт 
...
Функция ПолучитьКоординатыВТекстеЗапроса(ПолеТекста, СтартоваяСтрокаДляОтображенияОшибок = 0)
	...
	Результат = "";
	Если ТипЗнч(ПолеТекста) = Тип("ПолеТекстовогоДокумента") Тогда
		Результат = Результат + ирОбщий.ПредставлениеИзИдентификатораЛкс(ПолеТекста.Имя) + ": ";
	КонецЕсли; 
...
Процедура УстановитьПризнакМодифицированностиФормы()
	...
	Если ПолеТекста.ПолеТекста.ИзменяетДанные Тогда
		ФормаВладелец.Модифицированность = Истина;
	КонецЕсли; 
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	Пока Истина Цикл
		...
		Если СтрокаРезультата = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, КонкретныйТип, СтруктураТипаКонтекста, ПараметрЗакрытияПодсказки);
	КонецЦикла;
...
Процедура ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, КонкретныйТип, СтруктураТипаКонтекста, ПараметрЗакрытияПодсказки = Истина) Экспорт 
