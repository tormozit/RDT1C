...
//
Функция НайтиПоказатьСловоВТексте(СтрокаПоиска, СловоЦеликом = Истина, ПолеТекста = Неопределено) Экспорт 
...
Функция ВыполнитьКоманду(Знач Команда)
	...
	Попытка
		...
		ИначеЕсли Команда = "СравнитьТекст" Тогда
			ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ФормаВладелец, ПолеТекста);
		ИначеЕсли Истина
			И ЯзыкПрограммы = 1
...
Процедура АвтоОбновитьСправку() Экспорт 
	...
	Если Ложь
		...
		Или ФормаВладелец.ТекущийЭлемент <> ПолеТекста
	Тогда
		Возврат;
	КонецЕсли; 
...
Функция ЗагрузитьТекстВКонструктор(ТекстЗапроса = Неопределено, Знач КонструкторЗапроса = Неопределено, Знач СокращенноеДерево = Ложь, ДиалектSQL = Неопределено,
	ИменованныеПараметры = Неопределено, СтартоваяСтрокаДляОтображенияОшибок = Неопределено) Экспорт
	...
	Пока Не Закончили Цикл 
		...
		ИначеЕсли Ответ = gpMsgSyntaxError Тогда
			...
			Иначе
				...
				//	Тогда 
				//		//СлужебноеПолеТекста.УстановитьТекст(ТекстЗапроса); // Нужно дл использования функции ПолучитьТекстИзТокена
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, ВключатьКонструкции = Истина,
	...
	Тогда
		Шаблон = "(?:(" + шИмя + ")\()"
			+ "|(?:(" + шПараметрЗапроса + ")"
				+ "|(" + шИмя + ")(?=(?:\.(?:" + шИмя + ")?)+|[^\(?:\._А-ЯA-Z\d]|$))"
			+ "|(?:[_А-ЯA-Z\d\.]+)"; // Эта часть нужна чтобы не находить дочерние свойства и методы
		КоличествоПодгрупп = 3;
	Иначе
		Шаблон = "(?:" + ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")\()"
			+ "|(?:" + ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=[^\(?:_А-ЯA-Z\d]|$))"
			+ "|(?:[_А-ЯA-Z\d\.]+)";
...
Функция ЗаполнитьТаблицуСлов(ТаблицаСтруктурТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, РазрешитьОткрытиеОкон = Истина, ЗаполнятьТипы = Истина) Экспорт
	...
	Если ТаблицаСтруктурТиповКонтекста.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли; 
	ВычислятьТипыВСписке = Истина
		И ЗаполнятьТипы
		И (Ложь
			Или ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ВычислятьТипыВСписке") = Истина 
			Или ЯзыкПрограммы <> 0);
	ТаблицаСлов.Очистить();
	Если мЭтоСтроковыйЛитерал Тогда
	...
	Попытка
		ВнутренняяТаблицаСлов = мПлатформа.ПолучитьТаблицуСловСтруктурыТипа(выхСтруктураТипаКонтекста, ЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипыВСписке,,, ЛиСерверныйКонтекст);
		#Если Сервер И Не Сервер Тогда
		...
		Иначе
			Если ЗаполнятьТипы Тогда
				ЗагрузитьВТаблицуСловИзВнутреннейТаблицыСлов(ВнутренняяТаблицаСлов, ТаблицаСлов);
			Иначе
				ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ВнутренняяТаблицаСлов, ТаблицаСлов);
			КонецЕсли; 
		КонецЕсли; 
	Исключение
		Ошибка = ИнформацияОбОшибке();
...
Процедура РедакторHTML_Инициировать(ПолеHTML) Экспорт 
	...
	РедакторHTML.enableSuggestActivationEvent(Истина, Ложь);  // Второй параметр включает отображение типов для всех слов списка автодополнения
	...
	РедакторHTML.setActiveSuggestionAcceptors("."); // По нажатию точки выполняется вставка активного слова и ввод точки
...
Функция ДобавитьПодсказкуПоЗначениюВыражения(Знач ЗначениеВыражения, Знач ОбъектноеВыражение, Знач ПодсказкаМассив) Экспорт 
	...
	Иначе
		...
		Если ЕстьОписаниеТипа Тогда
			ИмяСсылкиТипа = "_Тип" + XMLСтрока(мИменаТиповПодсказкиУдержания.Количество());
...
Процедура РедакторHTML_ПередПоказомАвтодополнения(Триггер, ПоследнееВыражение, ПоследнееСлово)  Экспорт 
	...
	Если мРазрешенаПодсказкаHTML <> Истина Тогда 
		...
		Если Ложь
			Или Триггер = Неопределено // Вызов явный
		...
		Тогда 
			...
			Если ПоказатьСписок Тогда
				...
				ЗаполнитьТаблицуСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста, Триггер = Неопределено, Ложь);
				Если Ложь
...
Процедура РедакторHTML_ПриВыбореСтрокиАвтодополнения(Триггер, ВыбранноеСлово)  Экспорт 
...
Процедура РедакторHTML_ПриАктивацииСтрокиАвтодополнения(ТекущееСлово)  Экспорт 
...
Процедура РедакторHTML_ОтключитьСочетанияПереключенияСигнатуры() Экспорт 
	...
	РедакторHTML.disableKeyBinding(528); // ALT(512)+ArrowUp(16)
	РедакторHTML.disableKeyBinding(530); // ALT(512)+ArrowDown(18)
