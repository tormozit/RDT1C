	...
	Попытка
		...
		Тогда
			...
			Если выхСтруктураТипаКонтекста.ИмяОбщегоТипа = "ВнешнийИсточникДанных.<Имя внешнего источника>.ВременнаяТаблица" Тогда
				...
				КонецЦикла;
			КонецЕсли;
			Если ВыгрузкаТаблицыСлов <> Неопределено Тогда
				ТаблицаСлов.Загрузить(ВыгрузкаТаблицыСлов);
			КонецЕсли; 
		Иначе
			// Ветка сделана только для ускорения
			Если ЗаполнятьТипы Тогда 
				ЗагрузитьВТаблицуСловИзВнутреннейТаблицыСлов(ВнутренняяТаблицаСлов, ТаблицаСлов);
			Иначе
				ТаблицаСлов.Загрузить(ВнутренняяТаблицаСлов);
			КонецЕсли; 
		КонецЕсли; 
	Исключение
		Ошибка = ИнформацияОбОшибке();
...
Функция ДобавитьПодсказкуПоЗначениюВыражения(Знач ЗначениеВыражения, Знач ОбъектноеВыражение, Знач ПодсказкаМассив) Экспорт 
	...
	Иначе
		...
		КонецЕсли; 
		ИмяСсылкиТипа = "Тип" + XMLСтрока(мИменаТиповПодсказкиУдержания.Количество());
...
Функция ДобавитьПодсказкуПоДопустимымТипам(Знач ИмяТипаЗначения, Знач ПодсказкаМассив, Знач ВВидеГиперссылки = Истина) Экспорт 
...
Процедура ОткрытьОписаниеТипаПоГиперссылке(Знач ИмяОбщегоТипа) Экспорт 
	...
	мИменаТиповПодсказкиУдержания.Свойство(ИмяОбщегоТипа, ИмяОбщегоТипа);
