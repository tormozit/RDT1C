...
Процедура ОбновлениеОтображения()
	...
	Если ВКОбщая <> Неопределено Тогда
		...
		Если ВводДоступен() Тогда
			Если ВКОбщая <> "" Тогда
				ВКОбщая.ПереместитьОкноВПозициюКаретки(Истина);
			КонецЕсли;
...
Процедура ПриОткрытии()
	...
	ПриЛюбомОтрытии();
	Если Открыта() Тогда // Могли уже закрыть программно
...
Процедура ОбновитьИлиЗакрытьФорму(ЭтоВызовЧерезОжидание = Ложь, Необязательное = Ложь) Экспорт 
	...
	Тогда
		...
		Если СтараяСтрокаОписания <> СтрокаОписания Тогда
			...
			Если Ложь
				...
				Или (Истина
					И ПараметрСтруктураТипаКонтекста = Неопределено
					И ирОбщий.Форма_ВводДоступенЛкс(ФормаВладелец))
			Тогда
				ОчиститьДанные();
...
Процедура ЗагрузитьВариантМетода(НовыйТекущийВариант = Неопределено) Экспорт 
	...
	ОтборПараметров.Вставить("ТипКонтекста", ТипКонтекста);
	Если мЭтоКонструктор Тогда
		ОтборПараметров.Вставить("Слово", "<Новый>");
	Иначе
		ОтборПараметров.Вставить("Слово", ИмяМетода);
	КонецЕсли; 
	ОтборПараметров.Вставить("ЯзыкПрограммы", ЯзыкПрограммы);
	...
	//Если Не ЛиТекущийВариантУстановленВручную Тогда
		...
		НомерВарианта = 0;
		ЭтаФорма.ТекущийВариант = мПлатформа.ПодобратьВариантСинтаксисаМетода(ТаблицаВариантов, мФактическиеПараметры.Количество(), ТекущийВариант, ЛиТекущийВариантУстановленВручную, НомерВарианта);
		...
		Если мЭтоКонструктор И ЗначениеЗаполнено(ТекущийВариант) Тогда
	...
	//КонецЕсли; 
	Если Истина
		И СтрокаОписания.Описание = Неопределено
		И ЗначениеЗаполнено(СтрокаОписания.ПутьКОписанию)
	Тогда
		...
		ТекстовыйДокумент.Прочитать(НовыйАдрес);
		ТекстДокумента = ТекстовыйДокумент.ПолучитьТекст();
		СинтаксПомощник.ЗагрузитьОписаниеМетода(ТипКонтекста, СтрокаОписания.Слово, ТекстДокумента, СтрокаОписания.ТипСлова, ЯзыкПрограммы,,, СтрокаОписания);
		СтрокиПараметров = мПлатформа.ТаблицаПараметров.Скопировать(ОтборПараметров);
		#Если Сервер И Не Сервер Тогда
			СтрокиПараметров = Новый ТаблицаЗначений;
		#КонецЕсли
	...
	Если Источник = "KeyboardHook" Тогда
		Если Открыта() Тогда 
			Если ЭлементыФормы.ВариантСледующий.Доступность Тогда
				Если Найти(Данные, "01318") = 1 Тогда  // ALT+Up
					ВариантПредыдущийНажатие();
				ИначеЕсли Найти(Данные, "01320") = 1 Тогда  // ALT+Down
					ВариантСледующийНажатие();
				КонецЕсли;
			КонецЕсли; 
			Если Найти(Данные, "00027") = 1 Тогда  // Esc // Работает только в поле HTML документа. В остальных местах платформа делает полный перехват 
				Закрыть();
			КонецЕсли; 
		КонецЕсли;
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКлсПолеТекстаПрограммы.Форма.ВызовМетода");
