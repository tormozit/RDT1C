			...
			Если Найти(Результат, ".") > 0 Тогда
				...
				Тогда
					...
					Если СтруктураПолей.Режим = "ПоискПолейМетаданных" Тогда 
						ДобавитьПолеВСтруктуруПолей(СтруктураПолей, ДоступноеПоле);
					КонецЕсли; 
				КонецЕсли; 
			Иначе
				АктивироватьЗапросИЧастьОбъединения(ЗапросПакета, ЧастьОбъединения);
		...
		Иначе
			...
			Тогда
				...
				Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
					СтрокаПравогоЗначения = ирОбщий.ЛитеральноеПредставлениеЗначенияВЯзыкеЗапросовЛкс(ЭлементОтбора.ПравоеЗначение);
		...
		Для Каждого СтрокаПеретаскивания Из ЗначениеПеретаскивания Цикл
			Если ТипЗнч(СтрокаПеретаскивания) = Тип("СтрокаТаблицыЗначений") Тогда
				...
				Если Строка <> Неопределено Тогда
					Элемент.Значение.Сдвинуть(НоваяСтрока, Элемент.Значение.Индекс(Строка) - Элемент.Значение.Индекс(НоваяСтрока));
				КонецЕсли; 
			Иначе
...
Функция ПоляДоступнойТаблицы(СтрокаДоступнойТаблицы, ПараметрыТаблицы = Неопределено)
	...
	ИначеЕсли ТипЗнч(Конфигурация) = Тип("ОбъектМетаданныхКонфигурация") Тогда
		...
		Если ПараметрыТаблицы <> Неопределено Тогда
			...
			КонецЦикла;
		КонецЕсли; 
		ПоляТаблицыБД = ирОбщий.ПоляТаблицыМДЛкс(ПолноеИмяТаблицы, Ложь, ИндексПараметраПериодичность, ОпределениеПараметраПериодичность, Ложь);
		#Если Сервер И Не Сервер Тогда
		...
		Если ПоляТаблицыБД <> Неопределено Тогда
			...
			КонецЦикла;
			Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(СтрокаДоступнойТаблицы.Тип) Тогда
				СтрокаПоля = ТаблицаПолей.Добавить();
				СтрокаПоля.Имя = "Представление";
				СтрокаПоля.ТипЗначения = Новый ОписаниеТипов("ХранилищеЗначения");
			КонецЕсли; 
			Если Ложь
				Или ирОбщий.ЛиКорневойТипДокументаЛкс(СтрокаДоступнойТаблицы.Тип) 
				Или (Истина
					И ирОбщий.ЛиКорневойТипРегистраБДЛкс(СтрокаДоступнойТаблицы.Тип)
					И Не ирОбщий.ЛиМетаданныеНезависимогоРегистраЛкс(ирКэш.ОбъектМДПоПолномуИмениЛкс(СтрокаДоступнойТаблицы.ПолноеИмя)))
			Тогда
				СтрокаПоля = ТаблицаПолей.Добавить();
...
Функция СкорректироватьИмяВыбраннойТаблицы(Знач ВыбраннаяТаблица, ПринудительноАвтоимя = Ложь, РазрешитьПереименованиеДублей = Истина)
	...
	Тогда
		...
		Иначе
			Суффикс = "T";
		КонецЕсли; 
		БазовоеИмя = Лев(СтрокаДоступнойТаблицы.Имя, 50) + Суффикс;
	Иначе
	...
	КонецЕсли;
	НовоеИмя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ВыбраннаяТаблица.Владелец(), ВыбраннаяТаблица,,,, 0);
...
Функция ПолучитьТипФункцииCast(ТокенТипа, ЧастьОбъединения)
	...
	Иначе
		...
		ИначеЕсли ИмяПравилаТипа = "<BinaryType>" Тогда
			Результат = Новый ОписаниеТипов("ХранилищеЗначение");
					...
					Для Каждого ДоступноеПолеЛевое Из ГруппаДоступныхПолей.Элементы Цикл
						Для Каждого ДоступноеПолеПравое Из ДоступноеПолеТаблицыРодителя.Элементы Цикл
							...
							Если Истина
								...
								И ирОбщий.ОписанияТиповПересекаютсяЛкс(ДоступноеПолеЛевое.ТипЗначения, ДоступноеПолеПравое.ТипЗначения)
							...
							Тогда 
								...
								Если Истина
									И ОписаниеЛевогоПоля <> Неопределено
									И ОписаниеЛевогоПоля.Метаданные <> Неопределено
									И Найти(ОписаниеЛевогоПоля.Метаданные.ПолноеИмя(), ".Измерение.") > 0
								Тогда 
									ДобавитьСвязь = Истина;
								Иначе
									ОписаниеПравогоПоля = ОписаниеДоступногоПоля(ДоступноеПолеПравое);
									Если Истина
										И ОписаниеПравогоПоля <> Неопределено
										И ОписаниеПравогоПоля.Метаданные <> Неопределено
										И Найти(ОписаниеПравогоПоля.Метаданные.ПолноеИмя(), ".Измерение.") > 0
									Тогда 
										ДобавитьСвязь = Истина;
									КонецЕсли; 
								КонецЕсли; 
...
Процедура ЗагрузитьВыражениеУсловия()
	...
	Если ТипЗнч(мТекущаяСтрокаУсловия) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
		МожноСобиратьВыражение = Ложь
			Или ЗначениеЗаполнено(ирОбщий.ЛитеральноеПредставлениеЗначенияВЯзыкеЗапросовЛкс(мТекущаяСтрокаУсловия.ПравоеЗначение))
...
Процедура ДоступныеТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ОформлениеСтроки.Ячейки.Имя.ИндексКартинки = ирОбщий.ПолучитьИндексКартинкиТипаТаблицыБДЛкс(ДанныеСтроки.Тип);
...
// Результат - Истина - псевдоним был изменен автокоррекцией
Функция ВыбранныеПоляИмяПриИзменении(Элемент = Неопределено, Знач СтрокаВыбранногоПоля = Неопределено, Знач ЧастьОбъединения = Неопределено)
	...
	НовоеИмя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(БолееПриоритетныеПоля, НовоеИмя, "Имя");
	СтрокаВыбранногоПоля.Имя = НовоеИмя;
	ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаВыбранногоПоля);
...
Процедура ОбновитьПсевдонимыАвтополей(Знач ЧастьОбъединения = Неопределено)
	...
	Для каждого СтрокаАвтополя Из СтрокиАвтополей Цикл
		ВыбранныеПоляИмяПриИзменении(, СтрокаАвтополя, ЧастьОбъединения);
	КонецЦикла;
...
Функция ВлияющиеНаВыбранноеПолеПоляМетаданных(Знач ИмяПоля) Экспорт 
	...
	Если ПолеОбъединения = Неопределено Тогда
		Возврат Результат;
	КонецЕсли; 
	Для Каждого Тип Из ПолеОбъединения.ТипЗначения.Типы() Цикл
		...
		Для Каждого ЭлементСписка Из ПоляДляПровеки Цикл
			...
			Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
				...
				Для Каждого СтрокаПоля Из СтруктураОбработкиПолей.СоответствиеПолей Цикл
					ДоступноеПоле = СтрокаПоля.Значение;
					КраткоеИмяПоля = ирОбщий.ПоследнийФрагментЛкс(ДоступноеПоле.Поле);
					ВыбраннаяТаблица = ЧастьОбъединения.ВыбранныеТаблицы.Найти(Нрег(ДоступноеПоле.Родитель.Поле), "НИмя");
					Если ВыбраннаяТаблица.ВложенныйПакет <> Неопределено Тогда
					...
					Иначе
						...
						Тогда 
							//
						Иначе
							Результат.Добавить(ДоступнаяТаблица.ПолноеИмя + "." + КраткоеИмяПоля);
						КонецЕсли; 
					КонецЕсли; 
				КонецЦикла;
			КонецЦикла;
...
Процедура ВыбранныеПоляОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Если Истина
		И ЭтаФорма.ТекущийЭлемент = Элемент
	...
	Тогда
		Для Каждого СтрокаТаблицы Из ПараметрыПеретаскивания.Значение Цикл
			ПеретащеннаяСтрока = Элемент.Значение.Найти(СтрокаТаблицы.ИД, "ИД");
			Если ПеретащеннаяСтрока <> Неопределено Тогда
				Элемент.Значение.Удалить(ПеретащеннаяСтрока);
			КонецЕсли; 
		КонецЦикла;
	КонецЕсли; 
