...
Функция ОбработкаОбщихКлавиш(ПолеВвода, КодСочетанияКлавиш)
	...
	Тогда  
		...
		КонецЕсли;
		Если Найти(КодСочетанияКлавиш, "00013") = 1 Тогда
			Закрыть(Истина);
		Иначе
			Закрыть(" = ");
		КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	//Если ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ГлубокоВычислятьТипыВСпискеАвтодополнения") <> Истина И ЯзыкПрограммы = 0 Тогда
...
Процедура УстановитьГраницыВыделенияФильтра()
	...
	ирОбщий.ПромежуточноеОбновлениеСтроковогоЗначенияПоляВводаЛкс(ЭлементыФормы.ЭлементУправленияTextBox,, Истина);
