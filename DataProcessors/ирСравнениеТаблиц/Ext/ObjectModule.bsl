...
Функция Сравнить2СтрокиПоКолонкам(Знач КолонкиСопоставления, Знач Строка1, Знач Строка2, Знач ТабличныеЧасти1, Знач ТабличныеЧасти2)
	...
	Для Каждого СтрокаСопоставления Из КолонкиСопоставления Цикл    		Если НеУчитыватьРегистр Тогда    			ЛевоеЗначение = Строка1[СтрокаСопоставления.ИмяКолонки];  			ПравоеЗначение = Строка2[СтрокаСопоставления.ИмяКолонки2];  			Если ТипЗнч(ЛевоеЗначение) = Тип("Строка") И ТипЗнч(ПравоеЗначение) = Тип("Строка") Тогда  				Если ВРег(ЛевоеЗначение) <> ВРег(ПравоеЗначение) Тогда  					ЗначенияЯчеекРавны = Ложь;  				КонецЕсли;  			Иначе  				Если ЛевоеЗначение <> ПравоеЗначение Тогда  					ЗначенияЯчеекРавны = Ложь;  				КонецЕсли;  			КонецЕсли;  			Если Не ЗначенияЯчеекРавны Тогда  				Прервать;  			КонецЕсли;  		Иначе  			Если Строка1[СтрокаСопоставления.ИмяКолонки] <> Строка2[СтрокаСопоставления.ИмяКолонки2] Тогда  				ЗначенияЯчеекРавны = Ложь;  				Прервать;  			КонецЕсли;  		КонецЕсли;  	КонецЦикла;  

	Если ЗначенияЯчеекРавны Тогда
...
Функция СравнитьТаблицы(ТабличноеПолеРезультата = Неопределено, РежимСравненияСВыгрузкой = Ложь) Экспорт 
	...
	Пока Истина Цикл
		...
		Если РезультатСравненияКлючейСтрок = ВидСравнения.Равно Тогда 
			Если ЕстьТЧ Тогда
				...
				КонецЦикла;
			КонецЕсли; 
			Если Сравнить2СтрокиПоКолонкам(КолонкиСопоставления, ТекущаяСтрокаЭталона, ТекущаяСтрока, ТабличныеЧастиСтрокиЭталона, ТабличныеЧастиСтроки) Тогда 
...
Процедура ВывестиСтрокуРезультата(выхСтрокаРезультата, Знач СтрокаТаблицы, Знач НомерСтороны, Знач ВыводимыеКолонки, Знач КлючевыеКолонки = Неопределено)
	...
	#КонецЕсли
	// Однострочный код использован для ускорения. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика" (http://devtool1c.ucoz.ru)
	Если НомерСтороны = "2" Тогда  		Для Каждого КлючеваяКолонка Из КлючевыеКолонки Цикл  			выхСтрокаРезультата[КлючеваяКолонка.ИмяКолонки + "1"] = СтрокаТаблицы[КлючеваяКолонка.ИмяКолонки2];  		КонецЦикла;  	КонецЕсли;  	Для Каждого ИмяКолонки Из ВыводимыеКолонки Цикл  		выхСтрокаРезультата[ИмяКолонки + НомерСтороны] = СтрокаТаблицы[ИмяКолонки];  	КонецЦикла;  
...
Функция ВыгрузитьДанныеДляСравнения() Экспорт
...
мИмяКолонкиНомераСтрокиРезультата = "НомерСтрокиРезультата" + ирОбщий.СуффиксСлужебногоСвойстваЛкс();
