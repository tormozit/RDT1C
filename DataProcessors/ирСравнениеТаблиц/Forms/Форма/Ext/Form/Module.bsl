...
Функция СравнитьТаблицыВФорме(Кнопка = Неопределено) Экспорт
	...
	#Если Сервер И Не Сервер Тогда
		СравнитьЗавершение();
	#КонецЕсли
	РезультатЗадания = ирОбщий.СравнениеТаблиц_СравнитьЛкс(ОбщиеПараметрыОбработки(), МоментНачала,, 
		ЭтаФорма, Кнопка, "СравнитьЗавершение");
...
Процедура СравнитьЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		РазницаВремени = ТекущаяДата() - РезультатЗадания.МоментНачала;
		ирОбщий.СообщитьЛкс("Найдено различий: " + Формат(ВсегоРазличий, "ЧН=") + ". Длительность: " + РазницаВремени + " сек");
	КонецЕсли; 
...
Процедура УстановитьТипИсточникаТаблицаЗначений(НомерСтороны)
	...
	ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементыФормы["Файл" + НомерСтороны], "");
	ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементыФормы["ТипИсточника" + НомерСтороны], "ТаблицаЗначений");
