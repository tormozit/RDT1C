...
Функция ПолучитьРезультатВыбораФайла(ПолеВвода)
	...
	СписокФильтров.Добавить("Табличный файл 1С (*.mxl)");
	СписокФильтров.Добавить("Документ MS Excel (*.xls)");
	СписокФильтров.Добавить("Таблица значений (*.VT_)");

	ПолноеИмяФайла = ирОбщий.ВыбратьФайлЛкс(, "mxl,xls,xlsx,ods,vt_");
...
Функция СравнитьТаблицы() Экспорт
	
	ВремяНачала = ТекущаяДата();
	Результат = ВыполнитьСравнение(ЭлементыФормы.ВыбранныйРезультат);
	...
	РазницаВремени = ВремяОкончания - ВремяНачала;
	РазницаВремени = '00010101' + РазницаВремени;
	РазницаВремени = Формат(РазницаВремени, "ДЛФ=T");
	Сообщить("Найдено различий: " + ВсегоРазличий + ". Время выполнения: " + РазницаВремени);
	Возврат Результат;
...
Процедура Таблица1Нажатие(Элемент = Неопределено)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(Таблица1, Истина);
	ОбновитьСопоставлениеКолонокВФорме();
...
Процедура Таблица2Нажатие(Элемент = Неопределено)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(Таблица2, Истина);
	ОбновитьСопоставлениеКолонокВФорме();
...
Процедура Файл1Очистка(Элемент, СтандартнаяОбработка)
...
Процедура КолонкиТаблица1ПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ОформлениеСтроки.Ячейки.Сравнивать.ТолькоПросмотр = ДанныеСтроки.Ключевая;
...
Процедура КоманднаяПанельРезультатОткрытьТаблицу(Кнопка)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ВыбранныйРезультат, Ложь,,,,, ЭлементыФормы.ВыбранныйРезультат);
...
Процедура ДействияФормыСравнитьЧерезТабличныеДокументы(Кнопка)
	...
	СравниваемыйДокумент1 = ирОбщий.ВывестиТаблицуВТабличныйДокументЛкс(Таблица1); 
	СравниваемыйДокумент2 = ирОбщий.ВывестиТаблицуВТабличныйДокументЛкс(Таблица2); 
