...
Функция ОбщиеПараметрыОбработки() 
	...
	Результат.ВыбранныйРезультат = Новый ТаблицаЗначений;
...
// Вызывается из ирОбщий.СравнитьЗначенияВФормеЛкс
Функция СравнитьТаблицыВФорме(Кнопка = Неопределено, РазрешитьАсинхронно = Истина) Экспорт
	...
	Тогда 
		...
		#КонецЕсли
		РезультатЗадания = ирОбщий.СравнениеТаблиц_СравнитьЛкс(ОбщиеПараметрыОбработки(), МоментНачала, РежимСравненияСВыгрузкой,,, ЭтаФорма, Кнопка, "СравнитьТаблицыЗавершение");
	...
	ИначеЕсли ЭлементыФормы.ПанельРежима.ТекущаяСтраница = ЭлементыФормы.ПанельРежима.Страницы.ВыгрузкаИсточника Тогда
		...
		#КонецЕсли
		ирОбщий.СравнениеТаблиц_ВыгрузитьДанныеЛкс(ОбщиеПараметрыОбработки(), МоментНачала,, ЭтаФорма, Кнопка, "ВыгрузитьДанныеЗавершение");
	КонецЕсли; 
...
Процедура СравнитьТаблицыЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		Если РезультатЗадания.ЛиТаблицыРавны = Неопределено Тогда
			Возврат;
		КонецЕсли; 
		СостояниеТабличногоПоля = ирОбщий.ТабличноеПолеСостояниеСтрокЛкс(ЭлементыФормы.ВыбранныйРезультат, мИмяКолонкиНомераСтрокиРезультата);
		СоздатьКолонкиРезультата(ЭлементыФормы.ВыбранныйРезультат);
		...
		ирОбщий.НастроитьДобавленныеКолонкиТабличногоПоляЛкс(ЭлементыФормы.ИтогиРезультата,,,, Истина);
		Для Каждого ИмяКолонки Из СравниваемыеКолонкиРезультата Цикл
		...
		ОтображениеРезультатаУстановитьОтбор(ЭлементыФормы.ВыбранныйРезультат,,, СостояниеТабличногоПоля);
...
Процедура КолонкиТаблица1ПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.ИмяКолонки2) Тогда
		ЛиОписанияТиповПересекаются = ирОбщий.ОписанияТиповПересекаютсяЛкс(Таблица1.Колонки[ДанныеСтроки.ИмяКолонки].ТипЗначения, Таблица2.Колонки[ДанныеСтроки.ИмяКолонки2].ТипЗначения);
	КонецЕсли; 
...
Процедура ДействияФормыСравнитьЧерезТабличныеДокументы(Кнопка)
	...
	ирОбщий.СравнитьЗначенияВФормеЛкс(СравниваемыйДокумент1, СравниваемыйДокумент2);
...
Процедура ВыбранныйРезультатПриАктивизацииСтроки(Элемент)
	...
	ирОбщий.ТабличноеПолеПриАктивизацииСтрокиЛкс(ЭтаФорма, Элемент, Ложь);
...
Процедура ОтображениеРезультатаУстановитьОтбор(ТабличноеПолеРезультата, ОтборПоРезультатуСравненияСтроки = Неопределено, ОтборПоРазличиямВКолонке = Неопределено, СостояниеТабличногоПоля = Неопределено)
	...
	Если ЗначениеЗаполнено(ОтборПоРезультатуСравненияСтроки) Тогда
		ПредставлениеОтбора = ОтборПоРезультатуСравненияСтроки;
	КонецЕсли; 
	СтруктураОтбора = Новый Структура();
	Если ЗначениеЗаполнено(ОтборПоРазличиямВКолонке) Тогда
		СтруктураОтбора.Вставить(ОтборПоРазличиямВКолонке + "Различно", Истина);
		ПредставлениеОтбора = ОтборПоРазличиямВКолонке + " Различно";
	КонецЕсли; 
	...
	Если ЗначениеЗаполнено(ОтборПоРезультатуСравненияСтроки) И ОтборПоРезультатуСравненияСтроки <> "Все" Тогда
		СтруктураОтбора.Вставить(мИмяКолонкиРезультатаСравнения, РезультатыСравненияСтроки.Найти(ОтборПоРезультатуСравненияСтроки, "Имя").Код);
	КонецЕсли; 
	Если СтруктураОтбора.Количество() = 0 Тогда
		ВыбранныйРезультат = РезультатСравнения;
	Иначе
		ВыбранныйРезультат = РезультатСравнения.Скопировать(СтруктураОтбора);
	КонецЕсли; 
	...
	Для Каждого КолонкаТП Из ТабличноеПолеРезультата.Колонки Цикл
		...
		Тогда
			КолонкаТП.ТекстШапки = "Различно - " + ВыбранныйРезультат.Итог(КолонкаТП.Имя);
		КонецЕсли; 
	КонецЦикла;
...
Процедура ИтогиРезультатаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ирОбщий.СтрНачинаетсяСЛкс(Колонка.Имя, мИмяКолонкиРезультатаСравнения) Тогда
		ОтображениеРезультатаУстановитьОтбор(ЭлементыФормы.ВыбранныйРезультат, ирОбщий.ПоследнийФрагментЛкс(Колонка.Имя, мИмяКолонкиРезультатаСравнения), "");
	Иначе
		ОтображениеРезультатаУстановитьОтбор(ЭлементыФормы.ВыбранныйРезультат, "", Колонка.Имя);
	КонецЕсли; 
...
Функция ДанныеСтрокиИсточникаИзСтрокиРезультатаЛкс(СтрокаРезультата, НомерСтороны, выхТекущаяКолонка = "")
	...
	Результат = Новый Структура;
	ТекущаяКолонкаТП = ЭлементыФормы.ВыбранныйРезультат.ТекущаяКолонка;
	Для Каждого Колонка Из ВыбранныйРезультат.Колонки Цикл
		ЧистоеИмяКолонки = "";
...
Процедура КоманднаяПанельРезультатСравнитьСтрокиТаблиц(Кнопка)
	...
	ТабличноеПоле = ЭлементыФормы.ВыбранныйРезультат;
