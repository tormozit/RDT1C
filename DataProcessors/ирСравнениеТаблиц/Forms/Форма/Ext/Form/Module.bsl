...
Функция СравнитьТаблицыВФорме(Кнопка = Неопределено, РазрешитьАсинхронно = Истина) Экспорт
	...
	ИначеЕсли ЭлементыФормы.ПанельРежима.ТекущаяСтраница = ЭлементыФормы.ПанельРежима.Страницы.ВыгрузкаИсточника Тогда
		...
		#КонецЕсли
		ирОбщий.СравнениеТаблиц_ВыгрузитьДанныеЛкс(ОбщиеПараметрыОбработки(), МоментНачала,, 
			ЭтаФорма, Кнопка, "ВыгрузитьДанныеЗавершение");
	КонецЕсли; 
...
Процедура ВыбранныйРезультатПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ТекущаяКолонка <> Неопределено И ДанныеСтроки = Элемент.ТекущаяСтрока Тогда
		...
		Если СтруктураКолонокРезультата.Свойство(ТекущаяКолонка.Имя, ЧистоеИмяКолонки) Тогда 
			Если СравниваемыеКолонкиРезультата.Найти(ЧистоеИмяКолонки) <> Неопределено Тогда 
				ОформитьСравниваемыеЯчейки(ОформлениеСтроки, ЧистоеИмяКолонки, ЦветаСтиля.ЦветФонаВыделенияПоля, ЦветаСтиля.ЦветТекстаВыделенияПоля, Ложь);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ОформитьСравниваемыеЯчейки(Знач ОформлениеСтроки, Знач ЧистоеИмяКолонки, Знач ЦветФона, Знач ЦветТекста, ТолькоЕслиРазличаются = Истина)
	
	ДанныеСтроки = ОформлениеСтроки.ДанныеСтроки;
	Если Ложь
		Или Не ТолькоЕслиРазличаются
		Или Не СравнитьЗначенияЯчеек(ДанныеСтроки[ЧистоеИмяКолонки + "1"], ДанныеСтроки[ЧистоеИмяКолонки + "2"]) 
	Тогда 
		...
		КонецЕсли; 
		ИмяРазличныКолонки = ЧистоеИмяКолонки + "Различно";
