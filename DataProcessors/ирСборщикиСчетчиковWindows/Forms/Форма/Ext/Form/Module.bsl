...
Процедура ЗагрузитьНастройкуВФорме(НастройкаФормы, ДопПараметры) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ИмяСборщика) Тогда
		ЭтаФорма.ИмяСборщика = "Показатели 1С";
	КонецЕсли; 
...
Процедура СоздатьСборщикСчетчиковНажатие(Элемент)
	...
	ИмяФайлаСккрипта = ПолучитьИмяВременногоФайла("ps1");
	ТекстДокумент.Записать(ИмяФайлаСккрипта);
	КомандаСистемы = "powershell -executionpolicy RemoteSigned -file """ + ИмяФайлаСккрипта + """";
...
Процедура ПосмотретьГрафики(Знач ВыбраннаяСтрока, РежимПросмотра = 0)
	...
	Если РежимПросмотра = 1 Тогда
		...
		ТаблицаСтроковыхЗначений = ирОбщий.ПолучитьТаблицуИзСтрокиСРазделителемЛкс(Текст.ПолучитьТекст(), Разделитель, Истина, Истина);
		#Если Сервер И Не Сервер Тогда
			ТаблицаСтроковыхЗначений = Новый ТаблицаЗначений;
		#КонецЕсли
		ТаблицаТипизированая = Новый ТаблицаЗначений;
		ИменаКомпьютеров = Новый Структура;
		Для Каждого Колонка Из ТаблицаСтроковыхЗначений.Колонки Цикл
			...
			Иначе
				...
				ИменаКомпьютеров.Вставить(ирОбщий.СтрокаМеждуМаркерамиЛкс(Колонка.Заголовок, "\\", "\", Ложь));
			КонецЕсли;
			ТаблицаТипизированая.Колонки.Добавить(Колонка.Имя, Тип, Колонка.Заголовок);
		КонецЦикла;
		Если ИменаКомпьютеров.Количество() = 1 Тогда
			ИмяКомпьютера = "";
			Для Каждого КлючИЗначение Из ИменаКомпьютеров Цикл
				ИмяКомпьютера = КлючИЗначение.Ключ;
			КонецЦикла;
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирСборщикиСчетчиковWindows.Форма.Форма");
