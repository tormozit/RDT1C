...
Процедура УстановитьИсточникВФорме(РазрешитьЗаменуДанныхТДиТЗ = Истина)
	...
	Если Не РежимРедактора Тогда
		ЗагрузитьНастройкуПоУмолчанию(РазрешитьЗаменуДанныхТДиТЗ);
	КонецЕсли; 
...
Процедура ЗагрузитьНастройкуПоУмолчанию(РазрешитьЗаменуДанныхТДиТЗ = Истина)
	...
	Если НастройкаПоУмолчанию.Значение <> Неопределено Тогда
		...
		ЗагрузитьНастройкуВФорме(НастройкаПоУмолчанию.Значение,, РазрешитьЗаменуДанныхТДиТЗ);
	Иначе
...
Процедура СохранитьНастройкуПоУмолчанию()
	...
	НастройкаПоУмолчанию.Значение = мПолучитьНастройку();
...
Процедура ОбновлениеОтображения()
	...
	ДоступностьКнопкиВосстановитьЗначения = (СохраненныеНастройки <> Неопределено) И СохраненныеНастройки.Количество() <> 0;
...
Процедура СохранитьНастройкиФормы()
	...
	ирОбщий.СохранитьЗначениеЛкс(Метаданные().Имя + "." + мТекущееПолноеИмяТаблицы, СохраненныеНастройки);
...
Процедура КоманднаяПанельФормыСохранитьЗначения(Кнопка)
    ...
    ТекущиеДанные = ирОбщий.ВыбратьНастройкуДляСохраненияЛкс(СохраненныеНастройки, ПоследняяНастройка);
...
Процедура ЗагрузитьНастройкуВФорме(Настройка, ИзФайла = Ложь, РазрешитьЗаменуДанныхТДиТЗ = Истина)
	
	ЗаменитьДанныеТДиТЗ = Ложь;
	Если РазрешитьЗаменуДанныхТДиТЗ Тогда
		ЗаменитьДанныеТДиТЗ = Истина;
		Если Ложь
			Или ТаблицаЗначений.Колонки.Количество() > 1 
			Или ЗначениеЗаполнено(ТабличныйДокумент.Область(СтрокаЗаголовковТабличногоДокумента, 1).Текст) 
		Тогда
...
Процедура ОбновитьФлагиСохраненияТДиТЗ(Знач ЗаменитьДанныеТДиТЗ, Знач ИзФайла = Ложь)
	...
	ЭтотОбъект.СохранятьДанныеТД = Не ИзФайла И ЗаменитьДанныеТДиТЗ И ТабличныйДокумент.ВысотаТаблицы >= ПерваяСтрокаДанныхТабличногоДокумента;
	ЭтотОбъект.СохранятьДанныеТЗ = Не ИзФайла И ЗаменитьДанныеТДиТЗ И ТаблицаЗначений.Количество() > 0;
