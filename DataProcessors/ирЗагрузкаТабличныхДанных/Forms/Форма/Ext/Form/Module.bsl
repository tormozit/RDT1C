...
Процедура ВыполнитьЗапросДополнения(Знач Интерактивно = Ложь)
	...
	мКонсольЗапросов.мКоллекцияДляЗаполнения = ТаблицаЗначений;
	Если Интерактивно Тогда
		мКонсольЗапросов.ОткрытьМодально();
	Иначе
		Попытка
			мКонсольЗапросов.ВыполнитьЗапрос();
		Исключение
			ирОбщий.СообщитьЛкс("Ошибка дополнения запросом: " + ОписаниеОшибки());
			//Возврат;
		КонецПопытки; 
	КонецЕсли; 
	Если мКонсольЗапросов.ЭлементыФормы.РезультатКоллекция.Значение.Количество() > 0 Тогда
		ТабличноеПоле = ЭлементыФормы.ТаблицаЗначений;
		СтарыйИндекс = ирОбщий.ТабличноеПолеПозицияТекущейСтрокиЛкс(ТабличноеПоле);
		ТаблицаЗначений.Очистить();
		ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(мКонсольЗапросов.ЭлементыФормы.РезультатКоллекция.Значение, ТаблицаЗначений);
