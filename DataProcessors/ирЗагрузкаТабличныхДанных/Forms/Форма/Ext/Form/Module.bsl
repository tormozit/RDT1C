...
Процедура ОсновныеДействияФормыКонвертировать(Кнопка = Неопределено)
	...
	Если СопоставлениеКолонокТЗ.НайтиСтроки(Новый Структура("Пометка", Истина)).Количество() = 0 Тогда
		...
		Возврат;
	КонецЕсли;
	Если Не ЗагрузитьВТаблицуЗначенийИзТабличногоДокумента(ТолькоВыделенныеСтрокиТД, ЭтаФорма) Тогда 
...
Процедура ТаблицаЗагружаемыхРеквизитовПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Для Каждого КлючИЗначение Из ОтключитьЯчейки Цикл
		...
		Если Ложь
			...
			Или КлючИЗначение.Ключ = ЭлементыФормы.СопоставлениеКолонокТЗ.Колонки.СвязьПоВладельцу.Имя
			Или КлючИЗначение.Ключ = ЭлементыФормы.СопоставлениеКолонокТЗ.Колонки.СвязьПоТипу.Имя
			Или КлючИЗначение.Ключ = ЭлементыФормы.СопоставлениеКолонокТЗ.Колонки.ЭлементСвязиПоТипу.Имя
	...
	Если ДанныеСтроки.РежимКонвертации = "Вычислять" Тогда
...
Процедура ОбновитьКолонкиТабличногоПоляТаблицыЗначений() Экспорт 
...
Процедура СопоставлениеКолонокБДПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Пометка Или ДанныеСтроки.ПолеПоиска Тогда
		Если ДанныеСтроки.ИзИсточника Тогда
			...
			ОформлениеСтроки.Ячейки.Значение.ОтображатьКартинку = Ложь;
...
Процедура КоманднаяПанельСопоставлениеКолонокТЗЗаполнитьПоСинонимам(Кнопка)
	...
	Ответ = Вопрос("Хотите очистить пересоздать колонки таблицы-приемника по колонкам источника?", РежимДиалогаВопрос.ДаНет);
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ТаблицаЗначений.Колонки.Очистить();
		КоманднаяПанельТаблицаЗначенийЗаполнитьИзТабличногоДокумента();
	Иначе
...
Процедура ОтобратьТаблицуЗначений(РежимОтладки = Ложь)
	...
	Если ТаблицаЗначенийИспользоватьОтбор Тогда
		...
		//ЭтаФорма.ТаблицаЗначенийОтобранное = ирОбщий.СузитьТипыКолонокТаблицыБезПотериДанныхЛкс(ТаблицаЗначенийОтобранное,,,, Истина);
