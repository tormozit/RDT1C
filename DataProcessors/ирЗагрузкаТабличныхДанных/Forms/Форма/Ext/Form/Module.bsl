...
Процедура ЗагрузитьНастройкуВФорме(Настройка, ДопПараметры = Неопределено, ИзФайла = Ложь) Экспорт 
	...
	ЗаменитьМетаданныеТДиТЗ = ДопПараметры = Неопределено Или ДопПараметры.РазрешитьЗаменуМетаданныхТДиТЗ;
	Если ДопПараметры = Неопределено Или ДопПараметры.РазрешитьЗаменуДанныхТДиТЗ Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	мЗагрузитьНастройку(Настройка, ЗаменитьДанныеТДиТЗ, ЗаменитьМетаданныеТДиТЗ);
...
Процедура УстановитьИсточникВФорме(РазрешитьЗаменуДанныхТДиТЗ = Истина, РазрешитьЗаменуМетаданныхТДиТЗ = Истина)
	...
	ирОбщий.СоздатьМенеджерСохраненияНастроекФормыЛкс(ЭтаФорма, ПолноеИмяТаблицы, Не РежимРедактора, ПараметрыЗагрузкиНастройкиФормы);
...
Процедура СопоставлениеКолонокБДУстановитьСнятьФлажки(НовоеЗначениеПометки)
	...
	Тогда
		ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.СопоставлениеКолонокБД, "", НовоеЗначениеПометки);
	КонецЕсли;
