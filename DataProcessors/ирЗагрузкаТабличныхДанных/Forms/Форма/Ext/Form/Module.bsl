...
Процедура ОбновлениеОтображения()
	...
	Если мСтарыйСнимокНастройкиКомпоновки <> ирОбщий.СохранитьОбъектВВидеСтрокиXMLЛкс(ТаблицаЗначенийКомпоновщик.Настройки) Тогда
		...
		Если ТаблицаЗначенийИспользоватьОтбор И ОтбиратьСразу И ТаблицаЗначений.Количество() > 0 Тогда
			ТаблицаЗначенийКомпоновщик.Восстановить();
			ОтобратьТаблицуЗначений();
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ОсновныеДействияФормыКонвертировать(Кнопка = Неопределено)
	...
	Для Счетчик = 1 По НачальноеКоличество Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ЗагрузитьВТаблицуЗначенийИзТабличногоДокумента(ТолькоВыделенныеСтрокиТД);
	ОбновитьКолонкиТабличногоПоляТаблицыЗначений();
...
Процедура ТаблицаЗагружаемыхРеквизитовПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Пометка Тогда
		...
		Иначе
			ОформлениеСтроки.Ячейки.ИскатьПо.ЦветТекста = ЦветТекстаНеактивности;
			ОформлениеСтроки.Ячейки.СвязьПоВладельцу.ЦветТекста = ЦветТекстаНеактивности;
			ОформлениеСтроки.Ячейки.СвязьПоТипу.ЦветТекста = ЦветТекстаНеактивности;
			ОформлениеСтроки.Ячейки.ЭлементСвязиПоТипу.ЦветТекста = ЦветТекстаНеактивности;
		КонецЕсли;
	Иначе
	...
	ОформлениеСтроки.Ячейки.АмериканскоеПоложениеМесяца.Видимость = Истина
		И ДанныеСтроки.РежимКонвертации = "Искать"
		И ДанныеСтроки.ОписаниеТипов.СодержитТип(Тип("Дата"));
...
Функция ОбщиеПараметрыОбработки(ТолькоВыделенныеСтрокиТЗ = Ложь) 
	...
	Если ТолькоВыделенныеСтрокиТЗ Тогда
		Результат.ТаблицаЗначенийОтобранное = ТаблицаЗначенийОтобранное.Скопировать(ирОбщий.ВыделенныеСтрокиТабличногоПоляЛкс(ЭлементыФормы.ТаблицаЗначений));
	КонецЕсли; 
	Возврат Результат;
...
Процедура ОсновныеДействияФормыЗагрузить(Кнопка)
	...
	ЗапуститьЗагрузку(Кнопка, Истина);
	
КонецПроцедуры

Процедура ЗапуститьЗагрузку(Знач Кнопка, Знач Записывать)
	
	ОбщиеПараметрыОбработки = ОбщиеПараметрыОбработки(ТолькоВыделенныеСтрокиТЗ);
	Если Записывать И Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		ОтобратьТаблицуЗначений();
		КоличествоСтрокДляОбработки = ОбщиеПараметрыОбработки.ТаблицаЗначенийОтобранное.Количество();
	...
	#КонецЕсли
	ирОбщий.ЗагрузкаТабличныхДанных_ЗагрузитьЛкс(ОбщиеПараметрыОбработки, Записывать,, ЭтаФорма, Кнопка, "ЗагрузитьДанныеЗавершение");

КонецПроцедуры

Процедура КоманднаяПанельТаблицаБДЗагрузитьБезЗаписи(Кнопка)
	
	ЗапуститьЗагрузку(Кнопка, Ложь);
...
Процедура ЗагрузитьДанныеЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
...
Процедура ОбновитьСопоставлениеКолонокТЗВФорме()
	...
	СостояниеСтрок = ирОбщий.ТабличноеПолеСостояниеСтрокЛкс(ЭлементыФормы.СопоставлениеКолонокТЗ, "ИмяКолонкиПриемника");
	ОбновитьСопоставлениеКолонокТЗ(,, ТолькоВыделенныеСтрокиТД);
	ирОбщий.ТабличноеПолеВосстановитьСостояниеСтрокЛкс(ЭлементыФормы.СопоставлениеКолонокТЗ, СостояниеСтрок);
	...
	Для Каждого КолонкаТЗ Из ТаблицаЗначений.Колонки Цикл
		Если ирОбщий.СтрКончаетсяНаЛкс(КолонкаТЗ.Имя, мСуффиксСырыхДанных) Тогда
			...
			КолонкиТабличногоПоля.Сдвинуть(КолонкиТабличногоПоля[КолонкаТЗ.Имя], 
				КолонкиТабличногоПоля.Индекс(КолонкиТабличногоПоля[ИмяГотовойКолонки]) - КолонкиТабличногоПоля.Индекс(КолонкиТабличногоПоля[КолонкаТЗ.Имя]) + 1);
		...
		КонецЕсли; 
	КонецЦикла;
	ПослеДобавленияКолонокТаблицыЗначений();
...
Процедура ЗаполнитьСписокВыбораКолонкиИсточникаДляБД(Знач СписокВыбора, ИмяПредставления)
	...
	Для Каждого СтрокаНесопоставленнойКолонки Из КолонкиТЗ Цикл
		Если Не ирОбщий.ОписаниеТипов1ВходитВОписаниеТипов2Лкс(СтрокаНесопоставленнойКолонки.ОписаниеТипов, ЭлементыФормы.СопоставлениеКолонокБД.ТекущаяСтрока.ОписаниеТиповПриемника) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СопоставлениеКолонокБДПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, , "Значение");
	ОформлениеСтроки.Ячейки.Пометка.ОтображатьФлажок = Не ДанныеСтроки.Ключевое И ДанныеСтроки.ДоступноИзменение;
	...
	Если Истина
		...
		И Не ирОбщий.ОписаниеТипов1ВходитВОписаниеТипов2Лкс(ДанныеСтроки.ОписаниеТиповИсточника, ДанныеСтроки.ОписаниеТиповПриемника) 
	Тогда
		ОформлениеСтроки.Ячейки.ОписаниеТиповИсточника.ЦветФона = ЦветФонаОшибки;
	КонецЕсли; 
	Если Истина
		И ДанныеСтроки.ИзИсточника
		И Не ЗначениеЗаполнено(ДанныеСтроки.ИмяКолонкиИсточника) 
	Тогда
...
Процедура СобытияОбработкиОбъектовАлгоритмНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Для Каждого СтрокаПараметра Из ТекущаяСтрока.Параметры Цикл
		...
		ИначеЕсли ирОбщий.СтрокиРавныЛкс(СтрокаПараметра.Имя, "СтрокаТаблицыЗначений") Тогда
			...
			СтрокаПараметра.ТаблицаСтруктурТипов = мПлатформа.ПолучитьТаблицуСтруктурТиповИзЗначения(СтрокаТЗ);
...
Процедура ОтобратьТаблицуЗначений(РежимОтладки = Ложь)

	ТабличноеПоле = ЭлементыФормы.ТаблицаЗначений;
	ТабличноеПоле.ТолькоПросмотр = ТаблицаЗначенийИспользоватьОтбор;
	КлючТекущейСтроки = ирОбщий.ТабличноеПолеКлючСтрокиЛкс(ТабличноеПоле, мИмяКолонкиНомерСтроки);
	ИндексТекущейСтроки = ирОбщий.ТабличноеПолеПозицияТекущейСтрокиЛкс(ТабличноеПоле);
	Если ТаблицаЗначенийИспользоватьОтбор Тогда
		ЭтаФорма.ТаблицаЗначенийОтобранное = Новый ТаблицаЗначений;
		НаборыДанных = Неопределено;
		СхемаКомпоновки = СхемаКомпоновки(НаборыДанных);
		ирОбщий.СкомпоноватьВКоллекциюЗначенийПоСхемеЛкс(СхемаКомпоновки, ТаблицаЗначенийКомпоновщик.Настройки, ТаблицаЗначенийОтобранное, НаборыДанных,,,,, РежимОтладки,,,, Истина);
		...
		Если КлючТекущейСтроки <> Неопределено Тогда 
			...
			Для ИндексПроверяемойСтроки = ТаблицаЗначений.Индекс(ТекущаяСтрока) По Мин(ИндексТекущейСтроки + 1000, ТаблицаЗначений.Количество() - 1)  Цикл
				...
				КлючТекущейСтроки = ирОбщий.ТабличноеПолеКлючСтрокиЛкс(ТабличноеПоле, мИмяКолонкиНомерСтроки, ПроверяемаяСтрока);
	...
	Иначе
		...
		ТаблицаЗначенийОтобранное = ТаблицаЗначений;
		ирОбщий.ТабличноеПолеВосстановитьТекущуюСтрокуЛкс(ТабличноеПоле, КлючТекущейСтроки);
		мСтарыйСнимокНастройкиКомпоновки = Неопределено;
	КонецЕсли; 

КонецПроцедуры 

Процедура ПослеДобавленияКолонокТаблицыЗначений()
	
	СхемаКомпоновки = СхемаКомпоновки();
	ТаблицаЗначенийКомпоновщик.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновки));
...
Функция СхемаКомпоновки(выхНаборыДанных = Неопределено)
	
	выхНаборыДанных = Новый Структура("ТаблицаЗначений", ТаблицаЗначений);
	Схема = ирОбщий.СоздатьСхемуПоТаблицамЗначенийЛкс(выхНаборыДанных,,,, Ложь);
	Возврат Схема;
...
Процедура КоманднаяПанельТаблицаЗначенийОтборПоЗначению(Кнопка)
	...
	ОтборИзменен = ирОбщий.ТабличноеПолеОтборПоЗначениюВТекущейКолонкеЛкс(ЭлементыФормы.ТаблицаЗначений, ТаблицаЗначенийКомпоновщик.Настройки.Отбор,, ЭлементыФормы.Отбор);
...
Процедура КоманднаяПанельТаблицаЗначенийОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ОтборИзменен = ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонкеЛкс(ЭлементыФормы.ТаблицаЗначений, ТаблицаЗначенийКомпоновщик.Настройки.Отбор,, ЭлементыФормы.Отбор);
	Если ОтборИзменен Тогда
		ТаблицаЗначенийИспользоватьОтбор = Истина;
		ОтобратьТаблицуЗначений();
	КонецЕсли; 
...
Процедура КоманднаяПанельТаблицаЗначенийИсполняемаяКомпоновка(Кнопка)
	...
	ОтобратьТаблицуЗначений(Истина);
