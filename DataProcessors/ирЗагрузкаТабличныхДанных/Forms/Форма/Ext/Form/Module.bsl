...
Процедура ОбновлениеОтображения()
	...
	Если ИспользоватьОтбор И ОтбиратьСразу И ТаблицаЗначений.Количество() > 0 Тогда
...
Функция ОбщиеПараметрыОбработки() 
	...
	Для Каждого МетаРеквизит Из Метаданные().Реквизиты Цикл
		Если Ложь
			...
			Или МетаРеквизит.Имя = "Компоновщик"
		Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура ОсновныеДействияФормыЗагрузить(Кнопка)
	...
	Если ТаблицаЗначенийОтобранная.Количество() > 0 Тогда 
		ТекстВопросаИсточника = " строк в таблице БД " + ПолноеИмяТаблицы;
		Если Вопрос("Загрузить " + ТаблицаЗначенийОтобранная.Количество() + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) <> КодВозвратаДиалога.Да Тогда
			Возврат;
		КонецЕсли; 
	КонецЕсли; 
	ОчиститьСообщения();
...
Процедура СинхронизироватьРеквизитыОбъекта(ИзФормыВОбъект = Истина) Экспорт 
	...
	Если ИзФормыВОбъект Тогда
		...
		//ЭтотОбъект.ТаблицаЗначенийОтобранная = ЭтаФорма.ТаблицаЗначенийОтобранная;
	...
	Иначе
		...
		//ЭтаФорма.ТаблицаЗначенийОтобранная = ЭтотОбъект.ТаблицаЗначенийОтобранная;
...
Процедура ОтобратьТаблицуЗначений(РежимОтладки = Ложь)
	...
	ТабличноеПоле = ЭлементыФормы.ТаблицаЗначений;
	ТабличноеПоле.ТолькоПросмотр = ИспользоватьОтбор;
	КлючТекущейСтроки = ирОбщий.ТабличноеПоле_КлючСтрокиЛкс(ТабличноеПоле, мИмяКолонкиНомерСтроки);
	ИндексТекущейСтроки = ирОбщий.ТабличноеПолеПозицияТекущейСтрокиЛкс(ТабличноеПоле);
	Если ИспользоватьОтбор Тогда
		ЭтаФорма.ТаблицаЗначенийОтобранная = Новый ТаблицаЗначений;
		НаборыДанных = Неопределено;
		СхемаКомпоновки = СхемаКомпоновки(НаборыДанных);
		ирОбщий.СкомпоноватьВКоллекциюЗначенийПоСхемеЛкс(СхемаКомпоновки, Компоновщик.Настройки, ТаблицаЗначенийОтобранная, НаборыДанных,,,,, РежимОтладки,,,, Истина);
		...
		Если КлючТекущейСтроки <> Неопределено Тогда 
			...
			Для ИндексПроверяемойСтроки = ТаблицаЗначений.Индекс(ТекущаяСтрока) По Мин(ИндексТекущейСтроки + 1000, ТаблицаЗначений.Количество() - 1)  Цикл
				...
				КлючТекущейСтроки = ирОбщий.ТабличноеПоле_КлючСтрокиЛкс(ТабличноеПоле, мИмяКолонкиНомерСтроки, ПроверяемаяСтрока);
	...
	Иначе
		//ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаЗначений, ТаблицаЗначенийОтобранная);
		ТаблицаЗначенийОтобранная = ТаблицаЗначений;
		ирОбщий.ВосстановитьТекущуюСтрокуТаблицыФормыЛкс(ТабличноеПоле, КлючТекущейСтроки);
...
Процедура ПослеДобавленияКолонокТаблицыЗначений()
	...
	ирОбщий.НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(Компоновщик.Настройки.Порядок, мИмяКолонкиНомерСтроки);
...
Процедура КоманднаяПанельТаблицаЗначенийОтборПоЗначению(Кнопка)
	...
	ОтборИзменен = ирОбщий.ТабличноеПолеОтборПоЗначениюВТекущейКолонкеЛкс(ЭлементыФормы.ТаблицаЗначений, Компоновщик.Настройки.Отбор,, ЭлементыФормы.Отбор);
...
Процедура КоманднаяПанельТаблицаЗначенийОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ОтборИзменен = ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонкеЛкс(ЭлементыФормы.ТаблицаЗначений, Компоновщик.Настройки.Отбор,, ЭлементыФормы.Отбор);
	Если ОтборИзменен Тогда
		ИспользоватьОтбор = Истина;
		ОтобратьТаблицуЗначений();
	КонецЕсли; 
