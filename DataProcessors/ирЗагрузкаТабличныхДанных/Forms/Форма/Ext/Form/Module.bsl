...
Процедура ОсновныеДействияФормыЗагрузить(Кнопка)
	...
	ОтобратьТаблицуЗначений();
	Если ТаблицаЗначенийОтобранное.Количество() > 0 Тогда 
		ТекстВопросаИсточника = " строк в таблице БД " + ПолноеИмяТаблицы;
		Если Вопрос("Загрузить " + ТаблицаЗначенийОтобранное.Количество() + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) <> КодВозвратаДиалога.Да Тогда
			Возврат;
		КонецЕсли; 
	КонецЕсли; 
	ОчиститьСообщения();
