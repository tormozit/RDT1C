...
Функция ВычислитьЗначениеЯчейки(Знач АлгоритмОбъект, Знач ТекущиеДанные, Знач ТекстЯчейки, Знач ТекстыЯчеек, Знач Результат)
	...
	ОписаниеОшибки = "";
	Попытка
		мПлатформа.ВыполнитьМетодАлгоритма(АлгоритмОбъект, 0, ТекущиеДанные, ТекстЯчейки, ТекстыЯчеек, Результат);
	Исключение
...
Функция ЗагрузитьВТаблицуБД(Записывать = Истина) Экспорт
	...
	Тогда
		...
		Для Каждого КлючОбъектаБД Из ТаблицаКлючейОбъектовБД Цикл
			...
			Отказ = Ложь;
			ОписаниеОшибки = "";
			Успех = ирОбщий.ОбработатьСобытиеЛкс(СобытияОбработкиОбъектов, Метаданные().Макеты.АлгоритмПередОбработкойОбъекта.Имя, ОписаниеОшибки, ОбъектБД, Отказ);
			Если Не Успех Тогда
				Отказ = Истина;
			КонецЕсли; 
			Для Каждого СтрокаТаблицыБД Из СтрокиТаблицыБД Цикл
				...
				Успех = ирОбщий.ОбработатьСобытиеЛкс(СобытияОбработкиОбъектов, Метаданные().Макеты.АлгоритмПослеЗагрузкиСтроки.Имя, ОписаниеОшибки, ОбъектБД, СтрокаОбъектаБД, СтрокаТЗ, СтрокаНайдена);
				Если Не Успех Тогда
					Отказ = Истина;
	...
	Для каждого СтрокаСопоставления Из СопоставлениеКолонокТЗ Цикл
		Если СтрокаСопоставления.Пометка Тогда
			...
			ИначеЕсли СтрокаСопоставления.РежимКонвертации = "Вычислять" Тогда
				...
				КонецПопытки;
				Вычисление  = ВычислитьЗначениеЯчейки(АлгоритмОбъект, ТекущаяСтрока, "", ТекстыЯчеек, СтрокаСопоставления.ЗначениеПоУмолчанию);
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаСопоставления.ИмяКолонкиИсточника) Тогда
				ОбластьЯчейки = ТабличныйДокумент.Область("R"+Формат(НомерСтроки,"ЧГ=")+"C"+Формат(СтрокаСопоставления.ИмяКолонкиИсточника,"ЧГ="));
				Если Не ОбработатьОбласть(ОбластьЯчейки, СтрокаСопоставления, ТекущаяСтрока, ТекстыЯчеек) Тогда
					КоличествоОшибок = КоличествоОшибок + 1;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
//  ТекстыЯчеек    - массив текстов ячеек строки
//
Функция ОбработатьОбласть(Область, СтрокаСопоставления, ТекущиеДанные, ТекстыЯчеек)
	...
	ИначеЕсли ПустаяСтрока(Представление) Тогда
		Результат = Неопределено;
	Иначе
		НайденныеЗначения = ПолучитьВозможныеЗначения(СтрокаСопоставления, Представление, Примечание, ТекущиеДанные);
		...
		Иначе
			...
			Если Перечисления.ТипВсеСсылки().СодержитТип(ТипРеквизита) Тогда
				...
				Для каждого ЗначениеПеречисления Из ирОбщий.ПолучитьМенеджерЛкс(ТипРеквизита) Цикл
					Если Колонка.ИскатьПо = "Синоним" Тогда
						КлючЗначенияПеречисления = Строка(ЗначениеПеречисления);
					ИначеЕсли Колонка.ИскатьПо = "Имя" Тогда
						КлючЗначенияПеречисления = XMLСтрока(ЗначениеПеречисления);
					Иначе
						ВызватьИсключение "Не задан режим поиска значений";
					КонецЕсли; 
...
Процедура ЗагрузитьНастройку(Настройка) Экспорт
	...
	Если Настройка <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Настройка,, "СобытияОбработкиОбъектов, СопоставлениеКолонокТЗ, СопоставлениеКолонокБД"); 
		...
		СобытияОбработкиОбъектов.ЗаполнитьЗначения(, "Алгоритм");
...
Процедура ПроверитьДобавитьКолонкуИдентификатораСтроки(Таблица) Экспорт 
	...
	Если Таблица.Колонки.Найти(мИмяКолонкиНомерСтроки) = Неопределено Тогда
		Таблица.Колонки.Вставить(0, мИмяКолонкиНомерСтроки, Новый ОписаниеТипов("Число"), "<Идентификатор строки>");
	КонецЕсли;
...
Процедура СопоставитьКолонкуТЗ(Знач СтрокаСопоставления, Знач Колонка = Неопределено) Экспорт 
