...
Процедура УстановитьИсточник() Экспорт
	...
	КонецЕсли;
	СписокСохраненныхНастроек = ХранилищеОбщихНастроек.Загрузить(ирОбщий.ИмяПродуктаЛкс(), "ирЗагрузкаТабличныхДанных." + ПолноеИмяТаблицы);
	Если Ложь
...
Процедура ОбновитьКолонкиТаблицыЗначений(ОчиститьСуществующие = Ложь, ИзБД = Истина) Экспорт 
	...
	Если ИзБД Тогда
		Если ЗначениеЗаполнено(ПолноеИмяТаблицы) Тогда
			Если Истина
				И СопоставлениеКолонокБД.НайтиСтроки(Новый Структура("Пометка")).Количество() = 0 
				И СопоставлениеКолонокБД.НайтиСтроки(Новый Структура("ПолеПоиска")).Количество() = 0
			Тогда
...
Функция ЗагрузитьВТаблицуЗначений() Экспорт
	...
	Для К = ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		ТекущаяСтрока = КонтрольЗаполненияСтроки(ТабличныйДокумент, К, ТекстыЯчеек, КоличествоОшибок, КэшПоиска);
	...
	Сообщить("Ошибок обработки ячеек - " + КоличествоОшибок, ?(КоличествоОшибок = 0, СтатусСообщения.Информация, СтатусСообщения.Внимание));
		...
		Для Каждого КлючОбъектаБД Из ТаблицаКлючейОбъектовБД Цикл
			...
			Иначе// Если ТекущаяГруппаТипаМетаданных = "Ссылочный" И Не СтрокаНайдена Тогда 
				Если ЗначениеЗаполнено(СтрокаТаблицыБД.Ссылка) Тогда
					ОбъектБД = СтрокаТаблицыБД.Ссылка.ПолучитьОбъект(); // Лишние обращения к БД
				КонецЕсли; 
			...
			Для Каждого СтрокаТаблицыБД Из СтрокиТаблицыБД Цикл
				...
				Иначе
					...
					ИначеЕсли ТекущаяГруппаТипаМетаданных = "Регистр" Тогда
						СтрокаОбъектаБД = ОбъектБД.Добавить();
					Иначе
						СтрокаОбъектаБД = ОбъектБД;
					КонецЕсли; 
				КонецЕсли;
			...
			ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипРеквизита) Тогда
				...
				//Если ИскатьПо = "Идентификатор" Тогда
					...
					Для Каждого ЭлементСпискаДокументов Из СписокДокументов Цикл
						НайденныеЗначения.Добавить(ЭлементСпискаДокументов.Значение);
					КонецЦикла;
...
Процедура ЗаполнитьСопоставлениеКолонокБД() Экспорт 
	...
	Для Каждого ПолеТаблицы Из ПоляТаблицы Цикл
		...
		ЗагружаемыйРеквизит.ДоступноИзменение = Ложь
