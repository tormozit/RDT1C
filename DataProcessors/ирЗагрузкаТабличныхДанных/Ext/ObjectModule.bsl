...
Процедура УстановитьИсточник() Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ПолноеИмяТаблицы) Тогда 
		...
		КонецЕсли; 
	КонецЕсли;
	ЭтотОбъект.СохраненныеНастройки = ирОбщий.ВосстановитьЗначениеЛкс(Метаданные().Имя + "." + ПолноеИмяТаблицы);
	Если Ложь
...
//  табличный документ
//
Функция мПолучитьНастройку() Экспорт
	...
	Если СохранятьЗагружатьДанныеТД Тогда
		ДокументРезультат.Вставить("ШапкаТабличногоДокумента", ТабличныйДокумент.ПолучитьОбласть());
	Иначе
...
//
Процедура мЗагрузитьНастройку(Настройка, ЗаменитьТаблицуЗначений = Ложь) Экспорт
	
	Если Настройка <> Неопределено Тогда
		// Это нужно делать до загрузки свойства СохранятьЗагружатьДанныеТД через ЗаполнитьЗначенияСвойств!
		Если СохранятьЗагружатьДанныеТД Тогда
			ТабличныйДокумент.Очистить();
			ТабличныйДокумент.Вывести(Настройка.ШапкаТабличногоДокумента);
		Иначе
		...
		КонецЕсли; 
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Настройка,, "ТаблицаЗначений, СобытияОбработкиОбъектов, СопоставлениеКолонокТЗ, СопоставлениеКолонокБД, ПолноеИмяТаблицы"); 
...
СохранятьЗагружатьДанныеТД = Истина;
