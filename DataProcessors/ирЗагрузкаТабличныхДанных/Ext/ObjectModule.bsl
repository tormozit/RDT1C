...
Функция ПолучитьВозможныеЗначения(Колонка, Представление, Примечание, ТекущиеДанные, КэшПоиска = Неопределено)
	...
	Для каждого ТипРеквизита Из ОписаниеТипов.Типы() Цикл
		...
		Тогда
			НайденныеЗначения.Добавить(мПривестиКДате(Представление, Колонка.ОписаниеТипов, Примечание, Колонка.АмериканскоеПоложениеМесяца));
...
мИмяКолонкиНомерСтроки = "_НомерСтроки" + ирОбщий.СуффиксСлужебногоСвойстваЛкс(); 
