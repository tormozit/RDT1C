...
Функция ЗагрузитьВТаблицуЗначенийИзТабличногоДокумента() Экспорт
	...
	КоличествоОшибокКонвертации = 0;
	Для К = СтруктураПозицийТД.НомерСтрокиДанных По ТабличныйДокумент.ВысотаТаблицы Цикл
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
...
Функция ПолучитьКолонкиТабличногоДокумента(СтруктураПозицийТД = Неопределено) Экспорт 
	...
	Для Счетчик = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		...
		КонецЕсли; 
		ИмяКолонки = "" + Счетчик;
		ПримерДанных = ТабличныйДокумент.Область(СтруктураПозицийТД.НомерСтрокиДанных, Счетчик).Текст;
...
Функция ЗагрузитьВТаблицуБДИзТаблицыЗначений(Записывать = Истина) Экспорт
	...
	КоличествоЭлементов = ТаблицаЗначенийОтобранная.Количество();
	Если КоличествоЭлементов <= 0 Тогда
	...
	КонецЕсли;
	ПроверитьДобавитьКолонкуИдентификатораСтроки(ТаблицаЗначенийОтобранная);
	...
	КонецЕсли;
	Для Каждого СтрокаТЗ Из ТаблицаЗначенийОтобранная Цикл
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
		Если Не ЗначениеЗаполнено(СтрокаТЗ[мИмяКолонкиНомерСтроки]) Тогда
			СтрокаТЗ[мИмяКолонкиНомерСтроки] = ТаблицаЗначенийОтобранная.Индекс(СтрокаТЗ) + 1;
		КонецЕсли; 
		Отказ = Ложь;
	...
	Для Каждого КлючОбъектаБД Из ТаблицаКлючейОбъектовБД Цикл
		...
		Иначе// Если ТекущаяГруппаТипаМетаданных = "Ссылочный" И Не СтрокаНайдена Тогда 
			...
			Если ОбъектБД.Методы.ЭтоНовый() Тогда
				...
				Если ирОбщий.ЛиМетаданныеОбъектаСГруппамиЛкс(МетаданныеИсточника) Тогда
					...
					КонецЕсли; 
				КонецЕсли; 
				ОбъектБД = ирОбщий.ОбъектБДПоКлючуЛкс(МетаданныеИсточника.ПолноеИмя(), ЭтоГруппаДляНового, Истина,,, ирОбщий.ИдентификаторСсылкиЛкс(СтрокаТаблицыБД.Ссылка));
			КонецЕсли; 
		КонецЕсли; 
		...
		Для Каждого СтрокаТаблицыБД Из СтрокиТаблицыБД Цикл
			...
			КонецЕсли;
			ОписаниеОшибки = "";
			СтрокаТЗ = ТаблицаЗначенийОтобранная.Найти(СтрокаТаблицыБД[мИмяКолонкиНомерСтроки], мИмяКолонкиНомерСтроки);
...
Функция мПолучитьНастройку(ДляФайла = Ложь) Экспорт
	...
	ДокументРезультат.Вставить("НастройкаКомпоновки", Компоновщик.ПолучитьНастройки());
...
Процедура мЗагрузитьНастройку(Настройка, ЗаменитьДанныеТДиТЗ = Истина, ЗаменитьМетаданныеТДиТЗ = Истина) Экспорт
	...
	Если Настройка <> Неопределено Тогда
		...
		Если ЗаменитьМетаданныеТДиТЗ Тогда
			...
			Компоновщик.ЗагрузитьНастройки(Настройка.НастройкаКомпоновки);
		КонецЕсли; 
	Иначе
...
Процедура ОбновитьСопоставлениеКолонокТЗ(УстановитьПометкуДляСопоставленных = Ложь, УстановитьПометкуДляНовых = Истина) Экспорт 
