...
//
Функция ЗагрузитьВТаблицуЗначенийИзТабличногоДокумента(ТолькоВыделенныеСтрокиТД = Ложь) Экспорт
	...
	Для НомерСтроки = НачальнаяСтрока По КонечнаяСтрока Цикл
		...
		Для каждого КлючИЗначение Из ТекущаяСтрока Цикл
			Если Истина
				И ДобавлятьСырыеДанныеВТЗ 
				И ТаблицаЗначений.Колонки.Найти(КлючИЗначение.Ключ) = Неопределено
			Тогда
				ЗаголовокКолонки = ТаблицаЗначений.Колонки[СтрЗаменить(КлючИЗначение.Ключ, мСуффиксСырыхДанных, "")].Заголовок + ". Сырое";
				ТаблицаЗначений.Колонки.Добавить(КлючИЗначение.Ключ, Новый ОписаниеТипов("Строка"), ЗаголовокКолонки);
			КонецЕсли; 
...
Процедура ОбновитьДанныеТабличногоДокумента(БезВопросов = Ложь) Экспорт
	...
	Если ТабличныйДокумент.ВысотаТаблицы > 1 И Не БезВопросов Тогда
		...
		ИначеЕсли Результат = КодВозвратаДиалога.Отмена Тогда
			Возврат;
		Иначе
			СтруктураПозицийТД = СтруктураПозицийТабличногоДокумента(Истина);
	...
	Если ТабличныйДокумент.ВысотаТаблицы = 0 Тогда
		СтрокаЗаголовковТабличногоДокумента = 0;
		ПерваяСтрокаДанныхТабличногоДокумента = 0;
	КонецЕсли; 
...
Процедура ОбновитьКолонкиТаблицыЗначений(ОчиститьСуществующие = Ложь, ИзБД = Истина) Экспорт 
	...
	Иначе
		...
		Для Счетчик = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
			...
			КонецЕсли; 
			ИмяКолонки = мПлатформа.ИдентификаторИзПредставленияЛкс(СинонимКолонки);
...
Функция СтруктураПозицийТабличногоДокумента(ОбязательноЗаголовки = Ложь) Экспорт 
	
	#Если Сервер И Не Сервер Тогда
		ТабличныйДокумент = Новый ТабличныйДокумент;
	#КонецЕсли
	Результат = Новый Структура("НомерСтрокиЗаголовков, НомерСтрокиДанных");
	Результат.НомерСтрокиЗаголовков = СтрокаЗаголовковТабличногоДокумента;
	Результат.НомерСтрокиДанных = ПерваяСтрокаДанныхТабличногоДокумента;
	Если Не ЗначениеЗаполнено(ПерваяСтрокаДанныхТабличногоДокумента) Тогда
		МаксКоличествоЗаполненыхЯчеекДанных = 0;
		КоличествоЗаполненыхЯчеекЗаголовков = 0;
		Для НомерСтроки = Макс(1, СтрокаЗаголовковТабличногоДокумента) По Мин(10, ТабличныйДокумент.ВысотаТаблицы) Цикл
			//КоличествоЯчеек = 0;
			КоличествоЗаполненыхЯчеек = 0;
			КоличествоЯчеекСнизу = 0;
			КоличествоЗаполненыхЯчеекСнизу = 0;
			КоличествоСовпаденийЯчеекСнизу = 0;
			МожетБытьСтрокойЗаголовков = Не ЗначениеЗаполнено(СтрокаЗаголовковТабличногоДокумента);
			...
			Если НомерСтроки = Результат.НомерСтрокиЗаголовков Тогда 
				КоличествоЗаполненыхЯчеекЗаголовков = КоличествоЗаполненыхЯчеек;
			ИначеЕсли Истина
				И МожетБытьСтрокойЗаголовков
				И КоличествоЗаполненыхЯчеекЗаголовков < КоличествоЗаполненыхЯчеек
			Тогда
...
Функция ЗагрузитьВТаблицуБДИзТаблицыЗначений(Записывать = Истина) Экспорт
	...
	Для Каждого КлючОбъектаБД Из ТаблицаКлючейОбъектовБД Цикл
		...
		ДобавитьОбъектВОчередьОбработки(ирОбщий.СтруктураИзСтрокиТаблицыИлиДереваИлиВыборкиЛкс(КлючОбъектаБД), СтруктураКлючаСтроки, ПолноеИмяТаблицы, ПолноеИмяТаблицыВладельца, КопияСтрокОбъекта,
...
Процедура ДобавитьОбъектВОчередьОбработки(КлючОбъектаБД, СтруктураКлючаСтроки, ПолноеИмяТаблицы, ПолноеИмяТаблицыВладельца, КопияСтрокОбъекта, КопияСтрокТЗ, СобытияОбработкиОбъектов,
	...
	ПараметрыОбработкиОбъекта.Вставить("ПолноеИмяТаблицыВладельца", ПолноеИмяТаблицыВладельца);
...
Функция ОбработатьЭлементыОбъекта(ПараметрыОбработкиОбъекта) Экспорт 
	...
	ПолноеИмяТаблицыВладельца = ПараметрыОбработкиОбъекта.ПолноеИмяТаблицыВладельца;
...
Функция ОбработатьОбласть(Знач Область, СтрокаСопоставления, ТекущиеДанные, ТекстыЯчеек, КэшПоиска)
	...
	Если Не ПустаяСтрока(Примечание) И ВыводитьОшибкиКонвертацииСразу Тогда
		ирОбщий.СообщитьЛкс("Ячейка["+Область.Имя+"]("+СтрокаСопоставления.СинонимКолонкиПриемника+"): " + Примечание, СтатусСообщения.Внимание);
	КонецЕсли;
