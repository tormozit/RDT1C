...
Процедура УстановитьОбъектМетаданных(ПолноеИмяТаблицы = Неопределено) Экспорт
	...
	ЭтаФорма.ЕстьОграниченияДоступа = ирОбщий.ЕстьОграниченияДоступаКСтрокамТаблицыЛкс();
	...
	Если Не ирСервер.НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, фОбъект.РежимИмяСиноним) Тогда 
		фОбъект.ОбъектМетаданных = "";
		Возврат;
	КонецЕсли; 
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(фОбъект.ОбъектМетаданных)) Тогда
			Если Копирование Тогда
				СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ОбъектМетаданных, Элемент.ТекущаяСтрока);
			...
			Иначе
				...
				СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ОбъектМетаданных, ЭтоГруппа);
			КонецЕсли; 
		...
		Иначе
			...
			Для Каждого КлючИЗначение Из КлючОбъекта Цикл
				...
				КонецЕсли; 
			КонецЦикла;
			СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ОбъектМетаданных, КлючОбъекта);
...
Процедура ОбновитьКоличествоСтрок()
	...
	фОбъект.КоличествоСтрокВОбластиПоиска = ирОбщий.КоличествоСтрокВТаблицеБДЛкс(фОбъект.ОбъектМетаданных);
...
Процедура ОбъектМетаданныхОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Если ирОбщий.ДинамическийСписок_ОбъектМетаданных_НачалоВыбора(ЭтаФорма, Элемент, СтандартнаяОбработка, Текст) <> Неопределено Тогда 
...
Процедура ПраваДоступаКСтрокамНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Форма = ирОбщий.ПолучитьФормуЛкс("Отчет.ирАнализПравДоступа.Форма",,, фОбъект.ОбъектМетаданных);
	Форма.ОбъектМетаданных = фОбъект.ОбъектМетаданных;
