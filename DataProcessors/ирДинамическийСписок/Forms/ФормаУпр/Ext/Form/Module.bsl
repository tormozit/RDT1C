...
Процедура ДинамическийСписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	КонецЕсли; 
	ПоляПароли = Новый Массив;
	ПоляТаблицы = ирКэш.ПоляТаблицыБДЛкс(Настройки.ДополнительныеСвойства.ИмяТаблицы);
	Для Каждого ПолеТаблицы Из ПоляТаблицы Цикл
		Если Истина
			И ПолеТаблицы.Метаданные <> Неопределено
			И ПолеТаблицы.Метаданные.РежимПароля 
		Тогда
			ПоляПароли.Добавить(ПолеТаблицы.Имя);
		КонецЕсли; 
	КонецЦикла; 
	Для Каждого СтрокаСписка Из Строки Цикл
		...
		Для Каждого Ячейка Из Ячейки Цикл
			...
			Если Истина
				...
				И ПоляПароли.Найти(Ячейка.Ключ) <> Неопределено 
			Тогда
				ПредставлениеЗначения = "*****";
			КонецЕсли; 
...
Процедура НастройкаСписка(Команда = Неопределено, АктивироватьФиксированныйОтбор = Ложь)
	...
	ОткрытьФорму("Обработка.ирДинамическийСписок.Форма.НастройкиСпискаУпр", ПараметрыФормы, Элементы.ДинамическийСписок,,,, Новый ОписаниеОповещения("НастройкаСпискаЗавершение", ЭтотОбъект),
...
Процедура Сравнить2Строки(Команда)
	...
	Если Элементы.ДинамическийСписок.ВыделенныеСтроки.Количество() < 2 Тогда
		Возврат;
	КонецЕсли; 
	Объект1 = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ПолноеИмяТаблицы, Элементы.ДинамическийСписок.ВыделенныеСтроки[0]).Методы;
	Объект2 = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ПолноеИмяТаблицы, Элементы.ДинамическийСписок.ВыделенныеСтроки[1]).Методы;
	ирОбщий.СравнитьЗначенияИнтерактивноЧерезXMLСтрокуЛкс(Объект1, Объект2);
...
Процедура КомпоновщикНастроекПользовательскиеНастройкиОтборПравоеЗначениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	// Платформа почему то не вызывает этот обработчик
