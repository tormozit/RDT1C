...
Процедура УстановитьОбъектМетаданных(ПолноеИмяТаблицы = Неопределено) Экспорт
	...
	ИмяТипаСписка = ирОбщий.ИмяТипаИзПолногоИмениТаблицыБДЛкс(ОсновнойОбъект.ОбъектМетаданных, "Список");
	Если ЗначениеЗаполнено(ИмяТипаСписка) Тогда
		ОбъектМД = Метаданные.НайтиПоПолномуИмени(ОсновнойОбъект.ОбъектМетаданных);
		Если Истина
			И ЭтаФорма.Открыта()
		...
		Иначе
			...
			ирОбщий.НастроитьАвтоТабличноеПолеДинамическогоСпискаЛкс(ОсновнойЭУ, ОсновнойОбъект.РежимИмяСиноним);
			ЭтаФорма.Отбор = ЭлементыФормы.ДинамическийСписок.Значение.Отбор;
		КонецЕсли; 
	Иначе
		ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 100000 РАЗРЕШЕННЫЕ Т.* ИЗ " + ОсновнойОбъект.ОбъектМетаданных + " КАК Т";
		ОсновнойЭУ.ТипЗначения = Новый ОписаниеТипов("ТаблицаЗначений");
	...
	ОсновнойОбъект.ВместоОсновной = ирОбщий.ПолучитьИспользованиеДинамическогоСпискаВместоОсновнойФормыЛкс(ОбъектМетаданных);
	Попытка
	...
	ОсновнойОбъект.СтарыйОбъектМетаданных = ОбъектМетаданных;
...
Процедура ЗагрузитьНастройкиКолонок()
	...
	СохраненныеНастройки = ХранилищеОбщихНастроек.Загрузить(ирОбщий.ИмяПродуктаЛкс(), "ДинамическийСписок." + ОбъектМетаданных + "." + РежимВыбора);
...
Процедура НайтиСсылкуВСписке(КлючСтроки, УстановитьОбъектМетаданных = Истина) Экспорт
	...
	Иначе
		ирОбщий.СкопироватьОтборЛюбойЛкс(ЭлементыФормы.ДинамическийСписок.Значение.Отбор, КлючСтроки.Отбор);
	КонецЕсли; 
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			...
			Для Каждого ЭлементОтбора Из КлючОбъекта.Отбор Цикл
				...
				Если Копирование Тогда
					ЭлементОтбора.Значение = Элемент.ТекущаяСтрока[ЭлементОтбора.Имя];
				КонецЕсли; 
			КонецЦикла;
...
Процедура ВместоОсновнойПриИзменении(Элемент)
	...
	СохранитьЗначение("ирДинамическийСписок.ВместоОсновной." + ОбъектМетаданных, ОсновнойОбъект.ВместоОсновной);
...
Процедура КП_СписокИмяСиноним(Кнопка)
	...
	ОсновнойОбъект.РежимИмяСиноним = Не Кнопка.Пометка;
	Кнопка.Пометка = ОсновнойОбъект.РежимИмяСиноним;
	ирОбщий.НастроитьЗаголовкиАвтоТабличногоПоляДинамическогоСпискаЛкс(ЭлементыФормы.ДинамическийСписок, ОсновнойОбъект.РежимИмяСиноним);
