...
Функция ОбновитьДанные()
	...
	Если Отбор <> Неопределено Тогда
		...
		ОписаниеТиповКолонки = ЭлементОтбора.ТипЗначения;
	...
	Иначе
		ОписаниеТиповКолонки = ЗначениеТабличногоПоля.Колонки[ДанныеКолонки].ТипЗначения;
	КонецЕсли; 
	...
	Если ОписаниеТиповКолонки.Типы().Количество() = 1 Тогда
	...
	Иначе	
		...
		Для Каждого ДоступноеПоле Из КомпоновщикОбъектаМД.Настройки.ДоступныеПоляОтбора.Элементы Цикл
			...
			КонецЕсли;
			Если Истина
				И ирОбщий.СтрокиРавныЛкс("" + ДоступноеПоле.Поле, ЭлементОтбора.Имя)
				И ЭлементОтбора.ТипЗначения.СодержитТип(Тип("Строка"))
				И ЭлементОтбора.ТипЗначения.КвалификаторыСтроки.Длина = 0
			Тогда
				ВыражениеПоля = "ВЫРАЗИТЬ(Т." + ДоступноеПоле.Поле + " КАК СТРОКА(100))";	
			Иначе
				ВыражениеПоля = "Т." + ДоступноеПоле.Поле;
			КонецЕсли;
			Если ирОбщий.СтрокиРавныЛкс("" + ДоступноеПоле.Поле, ЭлементОтбора.Имя) Тогда
				ВыражениеГруппировки = ВыражениеПоля;
			КонецЕсли;
			//ТекстГДЕ = ТекстГДЕ + ВыражениеПоля + ".* КАК " + ДоступноеПоле.Поле; // запрещенные имена например "Соединение" так вызывают ошибку
