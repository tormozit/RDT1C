...
Функция ОбновитьДанные(РежимОтладки = Ложь, ИзменятьФлагУчитыватьОтбор = Ложь)
	...
	Тогда
		...
		Тогда
			...
			Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
				...
				Если ДобавитьЛиСсылка Тогда
					СтрокаРезультата.ЛиСсылка = ирОбщий.ЛиТипСсылкиБДЛкс(СтрокаРезультата.ТипЗначения); 
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли; 
	...
	Иначе
		...
		КонецЦикла;
		ирОбщий.СкопироватьОтборЛюбойЛкс(НастройкаКомпоновки.Отбор, Отбор);
		ЭлементОтбораКолонки = ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(НастройкаКомпоновки.Отбор, ВыражениеПоляКолонки);
...
Функция ЗаполнитьИменаТиповЗначений(Знач ТаблицаРезультата)
	...
	Если РазличныеТипы.Количество() = 1 Тогда
		...
		ирОбщий.ОбновитьТипЗначенияВСтрокеТаблицыЛкс(СтрокаТЧ, "", мОписаниеТиповКолонки,,, КолонкиТЧ, ?(СтрокаТЧ.ТипЗначения = "Null", Null, СтрокаТЧ.Значение));
