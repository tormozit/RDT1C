...
Функция ОбновитьДанные(РежимОтладки = Ложь)
	...
	Если Отбор <> Неопределено Тогда
		...
		Иначе
			...
			#КонецЕсли
			ЭлементОтбора = ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(Отбор, ПутьКДаннымКолонки);
	...
	Тогда
		...
		Пока ВыборкаРезультата.Следующий() Цикл
			...
			СтрокаТЧ.Значение = ВыборкаРезультата[ИмяКолонки];
			СтрокаТЧ.ЗначениеПредставление = ВыборкаРезультата[ИмяПоляЗначениеПредставление];
			Если ОтображениеКартинки <> Неопределено Тогда
				ирОбщий.ОбновитьТипЗначенияВСтрокеТаблицыЛкс(СтрокаТЧ, "",,,, КолонкиТЧ, ВыборкаРезультата[ИмяКолонки]);
			КонецЕсли;
		КонецЦикла;
