...
Функция ОбновитьДанные(РежимОтладки = Ложь)
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	ВыгрузкаРезультата = ирОбщий.ТаблицаЗначенийИзТаблицыФормыСКоллекциейЛкс(ИсточникДействий);
	...
	Если Ложь
		...
		Или ИмяТипаЗначения = "ДеревоЗначений"
	Тогда
		...
		ВыборкаРезультата = Построитель.Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, ИмяКолонки);
		Пока ВыборкаРезультата.Следующий() Цикл
			ЗначениеКолонки = ВыборкаРезультата[ИмяКолонки];
			...
			Если ОтображениеКартинки <> Неопределено Тогда
				...
				Если СтрокаСТипомЗначения = Неопределено Тогда
					ирОбщий.ОбновитьТипЗначенияВСтрокеТаблицыЛкс(СтрокаТЧ, "",,,, КолонкиТЧ, ЗначениеКолонки);
...
Процедура ОсновныеДействияФормыПрименить(Кнопка = Неопределено)
	...
	Если СписокЭлементовДляОтбора.Количество() = 0 Тогда
		...
		Возврат;
	КонецЕсли; 
	Если ЭтоБулеваНеограниченнаяСтроковаяКолонкаКолонка() И СписокЭлементовДляОтбора.Количество() > 0 Тогда
...
Процедура ЗначенияВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Отбор <> Неопределено Тогда
		Если ЭтоБулеваНеограниченнаяСтроковаяКолонкаКолонка() Тогда
...
Функция ЭтоБулеваНеограниченнаяСтроковаяКолонкаКолонка()
	Возврат Ложь
...
Процедура ДействияФормыСортироватьПоВозрастанию(Кнопка)
	...
	ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.Значения, Истина);

КонецПроцедуры

Процедура ДействияФормыСортироватьПоУбыванию(Кнопка)
	
	ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.Значения, Ложь);
...
Процедура ДействияФормыПоказыватьИтоги(Кнопка)
	
	ирОбщий.ТабличноеПолеКнопкаОтображенияИтоговНажатиеЛкс(ЭлементыФормы.Значения, Кнопка);
