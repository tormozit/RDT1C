...
Функция ОбновитьДанные(РежимОтладки = Ложь, ИзменятьФлагУчитыватьОтбор = Ложь)
	...
	Тогда
		...
		Иначе
			...
			КонецЦикла;
			СсылкиБД = ирОбщий.ТаблицаСМинимальнымиТипамиКолонокЛкс(СсылкиБД,,, Значения.ВыгрузитьКолонки().Колонки.ЗначениеПредставление.ТипЗначения.КвалификаторыСтроки.Длина);
			Запрос = Новый Запрос;
			Запрос.Текст = "
