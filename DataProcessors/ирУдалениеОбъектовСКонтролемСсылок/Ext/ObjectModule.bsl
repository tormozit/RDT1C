	...
	Для Каждого СтрокаОбъекта Из ТаблицаОбъектов Цикл
		...
		Если СтрокаКандидата = Неопределено Тогда
			...
			Если ПомечатьНаУдаление = Истина И Не СтрокаОбъекта.ПометкаУдаления Тогда
				...
				СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(СтрокаОбъекта.Ссылка.Метаданные().ПолноеИмя(), СтрокаОбъекта.Ссылка,,, ОбъектыНаСервере);
...
Функция КонтролироватьСсылкиНаКандидаты(ЭтаФорма = Неопределено, Пустышка = Ложь, выхДобавленыКандидаты = Ложь) Экспорт 
	...
	Для каждого ЭлементТаблицыСсылок из ТаблицаСсылок Цикл
		...
		СтрокаСсылки = СоответствиеКандидатов[Ссылка];
		...
		Иначе
			...
			ИначеЕсли Данные = Ссылка Тогда 
				СтрокаДанного = СтрокаСсылки;
			Иначе
				//СтрокаДанного = ТаблицаКандидатов.Найти(Данные, "Ссылка");
				СтрокаДанного = СоответствиеКандидатов[Данные];
			КонецЕсли; 
	...
	Пока НашлиДляИсключения Цикл
		...
		Для каждого ЭлементТаблицыСсылок из ТаблицаСсылок Цикл
			СсылкаДанного = ЭлементТаблицыСсылок.СсылкаДанного;
			Если СсылкаДанного = Неопределено Тогда
				СсылкаДанного = ЭлементТаблицыСсылок.Данные;
			КонецЕсли; 
			СтрокаДанного = СоответствиеКандидатов[СсылкаДанного];
			Если СтрокаДанного <> Неопределено Тогда
				УдаляетсяДанное = СтрокаДанного.Удаляется;
				Удаляется = ЭлементТаблицыСсылок.Удаляется;
				СтрокаСсылки = СоответствиеКандидатов[ЭлементТаблицыСсылок.Ссылка];
				УдаляетсяСсылка = СтрокаСсылки.Удаляется;
...
Процедура УдалитьОбъектыЛкс(ТаблицаСсылок = Неопределено) Экспорт
	...
	Пока Истина Цикл
		...
		Для Каждого СтрокаУдаляемогоОбъекта из МассивУдаляемых Цикл
			...
			СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(ОбъектМД.ПолноеИмя(), Ссылка,,, ОбъектыНаСервере);
			Попытка 
				Если Истина
					И СтруктураОбъекта.Методы <> Неопределено 
					И Не СтруктураОбъекта.Методы.ЭтоНовый()
				Тогда
					ирОбщий.УдалитьОбъектЛкс(СтруктураОбъекта.Методы, ОбъектыНаСервере);
				ИначеЕсли ирОбщий.ЛиКорневойТипДокументаЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаУдаляемогоОбъекта.Метаданные)) Тогда 
					...
					Для Каждого МетаРегистр из ОбъектыМД Цикл
						...
						ИмяТаблицыБДРегистра = ирОбщий.ПолучитьИмяТаблицыИзМетаданныхЛкс(ПолноеИмяМД);
						ИмяПоляОтбора = ирОбщий.ИмяПоляОтбораПодчиненногоНабораЗаписейЛкс(ИмяТаблицыБДРегистра);
						СтруктураНаборЗаписей = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяМД, Новый Структура(ИмяПоляОтбора, Ссылка),, Ложь, ОбъектыНаСервере);
						ирОбщий.ЗаписатьОбъектЛкс(СтруктураНаборЗаписей.Методы, ОбъектыНаСервере);
					КонецЦикла;
				КонецЕсли;
			...
			Исключение
				...
				вСообщитьОбОшибке("Не удален объект: " + СокрЛП(СтрокаУдаляемогоОбъекта.Метаданные) + ": " + СтрокаУдаляемогоОбъекта.Представление, СтатусСообщения.Важное);
