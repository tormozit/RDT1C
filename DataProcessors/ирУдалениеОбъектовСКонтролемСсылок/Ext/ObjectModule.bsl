						...
						Если ЭтоТаблицаОбъектов И Связи.Найти(Выборка.Ссылка,"Ссылка")<>Неопределено Тогда // связанная ссылка находится в таблице кандидатов на удаление, поэтому нельзя пока сказать точно можно удалить текущую или нет
							...
							ИначеЕсли С.Данные<>Выборка.Ссылка Тогда // если текущая найденная связь не с этим кандидатом, поищем нет ли с этим
								...
								Если Связи.НайтиСтроки(Отбор).Количество()=0 Тогда // нет связей с таким кандидатом - надо добавить
									С = Связи.Добавить();
									ЗаполнитьЗначенияСвойств(С, Отбор); 
									С.ОбъектСвязи = ОбъектСвязи;
								КонецЕсли;
							КонецЕсли;
	...
	Для Каждого СтрокаОбъекта Из ТаблицаОбъектов Цикл
		...
		Если СтрокаКандидата = Неопределено Тогда
			...
			Если ПомечатьНаУдаление = Истина И Не СтрокаОбъекта.ПометкаУдаления Тогда
				...
				ирОбщий.УстановитьПометкуУдаленияОбъектаЛкс(ОбъектКандидата, ЗаписьНаСервере, Истина);
...
Процедура УстановитьНеблокирующиеТипы(МассивПолныхИменМД) Экспорт 
	...
	Для Каждого ПолноеИмяМД Из МассивПолныхИменМД Цикл
		...
		Если СтараяСтрока <> Неопределено Тогда
			СтрокаНеблокируюещгоТипа.Игнорировать = СтараяСтрока.Игнорировать;
		КонецЕсли; 
	КонецЦикла;
	...
	Для Каждого ОписаниеТаблицы Из ТаблицаВсехТаблиц Цикл
		...
		Для Каждого ДоступноеПоле Из КомпоновщикТаблицы.Настройки.ДоступныеПоляВыбора.Элементы Цикл
			Если Ложь
				...
				Или ирОбщий.СтрокиРавныЛкс("" + ДоступноеПоле.Поле, КлючевоеПоле)
			Тогда
				Продолжить;
			КонецЕсли; 
			Реквизит = Реквизиты.Добавить();
			Реквизит.Имя = ДоступноеПоле.Поле;
			Реквизит.Тип = ДоступноеПоле.Тип;
		КонецЦикла;
...
Функция ОбновитьДанныеКандидатов(ТаблицаОбъектов = Неопределено, МинимизироватьТипы = Истина, ДополнительноеПоле = "") Экспорт 
	...
	Исключение
		// Такой грязный способ проверить наличие хотя бы одного типа с полем Предопределенный
		...
		Для Каждого СтрокаУдаляемогоОбъекта из МассивУдаляемых Цикл
			...
			Попытка 
				...
				ИначеЕсли ирОбщий.ЛиКорневойТипДокументаЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаУдаляемогоОбъекта.Метаданные)) Тогда 
					...
					КонецЦикла;
				КонецЕсли;
				вСообщитьОбОшибке("Удален объект: "+СокрЛП(СтрокаУдаляемогоОбъекта.Метаданные) + ": " + СтрокаУдаляемогоОбъекта.Представление, СтатусСообщения.Информация);
