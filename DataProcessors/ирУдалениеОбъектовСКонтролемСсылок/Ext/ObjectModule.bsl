...
Функция ОбновитьДанныеКандидатов(ТаблицаОбъектов = Неопределено, ДополнительноеПоле = "") Экспорт 
	...
	КонецЕсли; 
	ТаблицаОбъектов = ирОбщий.ПолучитьТаблицуЗначенийСМинимальнымиТипамиКолонокЛкс(ТаблицаОбъектов); // Так будет меньше неявных соединений в запросе
	Запрос = Новый Запрос;
	ТекстЗапроса = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ ТЗ.* ПОМЕСТИТЬ ТЗ ИЗ &ТЗ КАК ТЗ;
	|ВЫБРАТЬ ТЗ.Удалять, ТЗ.Удаляется, ТЗ.НеудаляемыхСсылок, ТЗ.Ссылок, ТЗ.Метаданные, ТЗ.Ссылка, ТЗ.Ссылка.ПометкаУдаления КАК ПометкаУдаления,
	|Представление(ТЗ.Ссылка) КАК Представление, 
	|ВЫБОР КОГДА ТЗ.Ссылка.ПометкаУдаления ТОГДА 1 ИНАЧЕ 0 КОНЕЦ КАК ИндексКартинки ";
...
Процедура УдалитьОбъектыЛкс(ТаблицаСсылок = Неопределено) Экспорт
	...
	Пока Истина Цикл
		...
		Если Индикатор = Неопределено Тогда
			Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ТаблицаУдаляемых.Количество(), "Удаление");
		КонецЕсли; 
