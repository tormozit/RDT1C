...
Функция ОбновитьДанныеКандидатов(ТаблицаОбъектов = Неопределено, МинимизироватьТипы = Истина, ДополнительноеПоле = "") Экспорт 
	...
	Если МинимизироватьТипы Тогда
		ТаблицаОбъектов = ирОбщий.ТаблицаСМинимальнымиТипамиКолонокЛкс(ТаблицаОбъектов); // Так будет меньше неявных соединений в запросе
	КонецЕсли; 
	Запрос = Новый Запрос;
	ТекстЗапроса = "
...
Функция КонтролироватьСсылкиНаКандидаты(выхДобавленыКандидаты = Ложь) Экспорт 
