	...
	Для Каждого СтрокаОбъекта Из ТаблицаОбъектов Цикл
		...
		Если СтрокаКандидата = Неопределено Тогда
			...
			Если ПомечатьНаУдаление = Истина И Не СтрокаОбъекта.ПометкаУдаления Тогда
				...
				СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(СтрокаОбъекта.Ссылка.Метаданные().ПолноеИмя(), СтрокаОбъекта.Ссылка,,, ЗаписьНаСервере);
...
Процедура УдалитьОбъектыЛкс(ТаблицаСсылок = Неопределено) Экспорт
	...
	Пока Истина Цикл
		...
		Для Каждого СтрокаУдаляемогоОбъекта из МассивУдаляемых Цикл
			...
			СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(ОбъектМД.ПолноеИмя(), Ссылка,,, ЗаписьНаСервере);
			Попытка 
				Если Истина
					И СтруктураОбъекта.Методы <> Неопределено 
					И Не СтруктураОбъекта.Методы.ЭтоНовый()
				Тогда
					ирОбщий.УдалитьОбъектЛкс(СтруктураОбъекта.Методы, ЗаписьНаСервере);
				ИначеЕсли ирОбщий.ЛиКорневойТипДокументаЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаУдаляемогоОбъекта.Метаданные)) Тогда 
					...
					Для Каждого МетаРегистр из ОбъектыМД Цикл
						...
						СтруктураНаборЗаписей = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяМД, Новый Структура("Регистратор", Ссылка),, Ложь, ЗаписьНаСервере);
						ирОбщий.ЗаписатьОбъектЛкс(СтруктураНаборЗаписей.Методы, ЗаписьНаСервере);
					КонецЦикла;
				КонецЕсли;
