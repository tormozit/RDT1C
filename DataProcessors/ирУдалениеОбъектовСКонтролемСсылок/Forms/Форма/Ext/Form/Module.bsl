...
Процедура ЗагрузитьНастройкуВФорме(НастройкаФормы, ДопПараметры) Экспорт 
	...
	Если НастройкаФормы <> Неопределено Тогда
		...
		КонецЕсли; 
		Если НастройкаФормы.Свойство("НеблокирующиеТипы") Тогда
			Если НастройкаФормы.НеблокирующиеТипы.Колонки.Метаданные.ТипЗначения.КвалификаторыСтроки.Длина <= 200 Тогда // Опасно
...
Процедура ЗаполнитьПомеченныеНаУдаление()
	...
	#КонецЕсли
	ирОбщий.УдалениеОбъектовСКонтролемСсылок_ЗаполнитьКандидатыЛкс(ОбщиеПараметрыОбработки, МоментНачала, МассивРазрешенныхТипов, , ЭтаФорма, Кнопка, "ЗаполнитьПомеченныеНаУдалениеЗавершение");
...
Процедура ЗаполнитьПомеченныеНаУдалениеЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		МоментНачала = РезультатЗадания.МоментНачала;
...
Процедура вПодсчитатьИтогУдаляемыеОбъекты()
	...
	ирОбщий.ТабличноеПолеОбновитьТекстыПодваловЛкс(ЭлементыФормы.УдаляемыеОбъекты);
...
Процедура ОсновныеДействияФормыКонтроль(Кнопка)
	...
	#КонецЕсли
	ирОбщий.УдалениеОбъектовСКонтролемСсылок_КонтролироватьЛкс(ОбщиеПараметрыОбработки, ЗапуститьУдалениеПослеКонтроля, , ЭтаФорма, Кнопка, "КонтролироватьЗавершение");
...
Процедура НеблокирующиеТипыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
	ОформлениеСтроки.Ячейки.Метаданные.ИндексКартинки = ирОбщий.ПолучитьИндексКартинкиТипаТаблицыБДЛкс(ирОбщий.ПервыйФрагментЛкс(ДанныеСтроки.Метаданные));
