...
Функция вОбновитьПомеченныеНаУдаление()
	...
	Если МассивРазрешенныхТипов.Количество() > 0 Тогда
		...
		Для Каждого ПолноеИмяМД Из МассивРазрешенныхТипов Цикл
			РазрешенныеТипы[Тип(СтрЗаменить(ПолноеИмяМД, ".", "Ссылка."))] = 1;
		КонецЦикла;
...
Процедура КП_СсылкиНаУдаляемыеОбъектыКонсольОбработки(Кнопка)
	...
	ирОбщий.ОткрытьОбъектыИзВыделенныхЯчеекВПодбореИОбработкеОбъектовЛкс(ЭтаФорма.ЭлементыФормы.СсылкиНаКандидата, "Данные");
...
Процедура КП_СсылкиНаУдаляемыеОбъектыМенеджерТабличногоПоля(Кнопка)
	 ...
	 ирОбщий.ПолучитьФормуЛкс("Обработка.ирМенеджерТабличногоПоля.Форма",, ЭтаФорма, ).УстановитьСвязь(ЭлементыФормы.СсылкиНаКандидата);
