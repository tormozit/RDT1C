	...
	Для каждого ЭлементТаблицыСсылок из ТаблицаСсылок Цикл
		...
		Иначе
			...
			Если НеблокирующиеТипы.Найти(ИмяМетаданных, "Метаданные") <> Неопределено Тогда
				МассивДобавляемыхОбъектов = Новый Массив;
				МассивДобавляемыхОбъектов.Добавить(Данные);
				ДобавитьМассивОбъектовВУдаляемыеОбъекты(МассивДобавляемыхОбъектов, Ложь);
			КонецЕсли; 
...
Функция ДобавитьМассивОбъектовВУдаляемыеОбъекты(Знач МассивОбъектов, УстанавливатьТекущуюСтроку = Истина) Экспорт 
	...
	Для Каждого ЭлементМассива Из МассивОбъектов Цикл
		...
		КонецЕсли; 
	КонецЦикла;
	Если ВМассивеБылиПодходящиеОбъекты Тогда
		ПодключитьОбработчикОжидания("ПослеИзмененияСоставаКонтролируемыхОбъектов", 0.1, Истина);
	КонецЕсли; 
...
Процедура ЗагрузитьНастройкиИзСтруктуры(СтруктураНастроек)
	...
	УдаляемыеОбъекты.Очистить();
	ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(СтруктураНастроек.УдаляемыеОбъекты, УдаляемыеОбъекты);
	ПослеИзмененияСоставаКонтролируемыхОбъектов();
