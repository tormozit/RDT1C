...
Функция ПолучитьПолноеИмяКонфигурационногоФайла(ВариантРасположенияФайлаНастроек = Неопределено) Экспорт
	...
	ИначеЕсли ВариантРасположенияФайлаНастроек = мВариантыРасположенияФайла.АктивныйФайл Тогда 
		Результат = ирОбщий.ПолучитьПолноеИмяФайлаНастройкиПриложения1СЛкс("logcfg.xml", НаСервере);
	Иначе
...
Функция ПолучитьКаталогНастроекПриложения(ВариантРасположенияФайлаНастроек = Неопределено, СоздатьЕслиОтсутствует = Ложь) Экспорт
	...
	ИначеЕсли НаСервере И ирКэш.ЛиПортативныйРежимЛкс() И Не ирКэш.ЭтоФайловаяБазаЛкс() Тогда 
		Результат = "";
	Иначе
		Результат = ирОбщий.ПолучитьКаталогНастроекПриложения1СЛкс(ВариантРасположенияФайлаНастроек, СоздатьЕслиОтсутствует, НаСервере);
	КонецЕсли; 
	Возврат Результат;
...
Процедура ЗаполнитьТаблицуТиповСобытий() Экспорт
	...
	ТаблицаИзМакета = ирОбщий.ПолучитьТаблицуИзТабличногоДокументаЛкс(ПолучитьМакет("ТипыСобытий"));
...
//
Процедура ПрочитатьНастройкиЖурнала(Индекс, выхМестоположение, выхВремяХранения, выхПредставлениеСобытий = Неопределено, выхСобиратьБлокировкиСУБД = Ложь) Экспорт
	...
	выхПредставлениеСобытий = ПолучитьПредставлениеОтбораСобытийЖурнала(Индекс);

	ЭлементыБлокировкиСУБД = УзелЖурнала.ПолучитьЭлементыПоИмени("dbmslocks");
	выхСобиратьБлокировкиСУБД = (ЭлементыБлокировкиСУБД.Количество() > 0);
...
//
Процедура ЗаписатьНастройкиЖурнала(Индекс, Местоположение, ВремяХранения, СобиратьБлокировкиСУБД) Экспорт
	...
	НайденныеЭлементы = ЭлементДомЖурнала.ПолучитьЭлементыПоИмени("*", ИмяЭлемента);
	Для Каждого НайденныйЭлемент Из НайденныеЭлементы Цикл
		НайденныйЭлемент.РодительскийУзел.УдалитьДочерний(НайденныйЭлемент);
	КонецЦикла; 
	Если СобиратьБлокировкиСУБД Тогда
		НайтиДобавитьУзелВКоллекцию(, , ЭлементДомЖурнала, ИмяЭлемента);
	КонецЕсли;
...
Процедура ПрочитатьНастройкиЖурналов(ТабличноеПолеЖурналы) Экспорт
	...
	Для Инд = 0 По УзлыЖурналов.Количество() - 1 Цикл
		...
		ПрочитатьНастройкиЖурнала(Инд, Журнал.Местоположение, Журнал.ВремяХранения, Журнал.События, Журнал.СобиратьБлокировкиСУБД);
