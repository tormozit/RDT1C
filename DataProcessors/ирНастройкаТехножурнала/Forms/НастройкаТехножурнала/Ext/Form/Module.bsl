	...
	Для Индекс = 0 По ИндексТекущегоВарианта - 1 Цикл
		...
		Если Не ирОбщий.СтрокиРавныЛкс(ИмяВыбранногоФайла, ИмяИндивидуальнойНастройки) Тогда
			...
			Если ФайлСуществует Тогда
				...
				ирОбщий.СообщитьЛкс("Файл настройки техножурнала """ + ИмяИндивидуальнойНастройки + """ отключен (переименован в """ + КраткоеИмяФайлаОтката + """)");
			КонецЕсли; 
		КонецЕсли; 
	КонецЦикла;
...
Функция ПереключитьТрассировкуЗапросов(НовыйРежим = Истина) Экспорт 
	...
	КаталогЛогов = ирСервер.СоздатьКаталогТрассировкиЗапросовЛкс();
	...
	ФормаЖурнала.УстановитьЭлементОтбораВВыделенныхГруппахИ("t:applicationName", "1CV8"); // Толстый клиент
	Если Не ирКэш.ЭтоФайловаяБазаЛкс() Тогда
...
Функция ЗагрузитьФайлНастройки(пИмяФайла = Неопределено, УстановитьПризнакИзменения = Ложь, УстанавливатьОсновныеКаталоги = Ложь, пНаСервере = Неопределено, РазрешитьВопросы = Ложь) Экспорт
	...
	Если РазрешитьВопросы Тогда
		Ответ = Вопрос("Загрузить настройку полностью (иначе только в текущий журнал)?", РежимДиалогаВопрос.ДаНет);
	Иначе
	...
	Память = мДокументДОМ.ПолучитьЭлементыПоИмени("mem");
	ЭтотОбъект.СледитьЗаУтечкамиПамятиВРабочихПроцессах = (Память.Количество() > 0);
	
	ПланыЗапросов = мДокументДОМ.ПолучитьЭлементыПоИмени("plansql");
	ЭтотОбъект.ФиксироватьПланыЗапросовSQL = (ПланыЗапросов.Количество() > 0);
	
	ЭлементыЛ = мДокументДОМ.ПолучитьЭлементыПоИмени("scriptcircrefs");
...
Процедура ПриЗакрытии()
	...
	Если ВремяДоСчитывания > 0 Тогда 
		Сообщить("" + ТекущаяДата() + " до считывания всеми процессами последних изменений настройки техножурнала осталось " + ВремяДоСчитывания + "с");
	КонецЕсли; 
