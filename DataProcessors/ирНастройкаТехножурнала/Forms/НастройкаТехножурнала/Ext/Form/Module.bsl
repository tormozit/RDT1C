...
Процедура КоманднаяПанель1ОчиститьКаталогЖурнала(Кнопка)
	...
	КонецЕсли; 
	Если ЗначениеЗаполнено(ТекущаяСтрока.Местоположение) Тогда
		ирОбщий.ОчиститьКаталогЖурналаЛкс(ТекущаяСтрока.Местоположение, НаСервере);
		ТекущаяСтрока.Включен = Не ирОбщий.ЛиКаталогТехножурналаСодержитБлокирующиеФайлыЛкс(ТекущаяСтрока.Местоположение, НаСервере, Ложь);
	КонецЕсли; 
...
Процедура ВычислитьРазмерыКаталогов() Экспорт
	...
	Для Каждого СтрокаКаталога Из ТабличноеПолеЖурналы Цикл
		...
		СтрокаКаталога.Включен = Не ирОбщий.ЛиКаталогТехножурналаСодержитБлокирующиеФайлыЛкс(СтрокаКаталога.Местоположение, НаСервере);
	КонецЦикла;
