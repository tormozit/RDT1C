...
Процедура ПостроитьДеревоОбменаДанных(ДеревоОбмена, МетаданныеПланаОбмена, МетаданныеДляОграничения = Неопределено)
	...
	Если МетаданныеДляОграничения = Неопределено Тогда
		...
		Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(МетаданныеПланаОбмена.Состав.Количество(), "Подготовка данных");
	...
	Если ОтображатьКоличествоОбъектовДляКоторыхЗарегистрированыИзменения Тогда
		ирОбщий.ОбновитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(ДеревоОбмена, "Данные", "КоличествоЗарегистрированных", Истина,
...
Функция ИзменитьРегистрациюИзмененийНаУзлеДляВсегоСоставаПланаОбмена(НовоеЗначение = Истина)
	...
	МетаданныеПланаОбмена = УзелОбмена.Метаданные();
	Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(МетаданныеПланаОбмена.Состав.Количество(), "Регистрация изменени");
...
Процедура КоманднаяПанельДереваОбъектовПоказатьРезультатСтандартнойВыгрузки(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(СтрXML, "XML представление объекта """ + ЭлементыФормы.ДеревоОбмена.ТекущаяСтрока.Данные + "." + Объект + """", "HTML", Истина, ЭлементыФормы.ТаблицаИзменений.ТекущаяСтрока);
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирРедакторИзмененийНаУзле.Форма.Форма");
