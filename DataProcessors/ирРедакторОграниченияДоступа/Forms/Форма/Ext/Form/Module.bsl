...
Функция ДействияФормыПеречитать(Кнопка = Неопределено)
	...
	#КонецЕсли
	ФайлРоли = Новый Файл("" + ирПлатформа.СтруктураПодкаталоговФайловогоКэша.КэшПрав.ПолноеИмя + "\Роль." + Роль + ".Права.xml");
...
Функция ЗаполнитьПараметрыЗапроса(ТекстЗапроса)
	...
	ИмяПараметра = "";
	ДлинаТекстаЗапросаRLS = СтрДлина(ТекстЗапроса);
	ИменаПараметров = Новый Структура;
	ВычислительРегулярок = Неопределено;
	Для ТекущийИндексСимвола = 1 По ДлинаТекстаЗапросаRLS Цикл
...
Функция ОбъектМетаданныхНачалоВыбора(Элемент, СтандартнаяОбработка, Текст = "")
	
	СтандартнаяОбработка = Ложь;
	Форма = ирКэш.Получить().ПолучитьФорму("ВыборОбъектаМетаданных",, ЭтаФорма);
	лСтруктураПараметров = Новый Структура;
	лСтруктураПараметров.Вставить("НачальноеЗначениеВыбора", ТаблицаБД);
	...
	лСтруктураПараметров.Вставить("Фильтр", Текст);
...
Процедура ДействияФормыКэшПрав(Кнопка)
	
	ПолучитьФорму("КэшПрав").Открыть();
...
Процедура ШаблоныПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ЭлементыФормы.Шаблоны.ТекущаяСтрока.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ЭлементыФормы.Шаблоны.Значение, ЭлементыФормы.Шаблоны.ТекущаяСтрока);
...
Процедура ТаблицаБДОкончаниеВводаТекста(Элемент, Текст, Значение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		ИначеЕсли ОбъектМетаданныхНачалоВыбора(Элемент, СтандартнаяОбработка, Текст) <> Неопределено Тогда 
			Значение = Новый СписокЗначений;
