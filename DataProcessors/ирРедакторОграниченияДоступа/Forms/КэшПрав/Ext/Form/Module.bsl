...
Процедура КоманднаяПанель1ОбновитьКэш(Кнопка)
	Ответ = Вопрос("Обновление кэша RLS использует конфигуратор текущей базы и может занять до нескольких минут. Продолжить?", РежимДиалогаВопрос.ОКОтмена);
...
Процедура ОбновитьИнфоПапкиКэшаRLS()
	...
	#КонецЕсли
	ПапкаКэшаПрав = ирПлатформа.СтруктураПодкаталоговФайловогоКэша.КэшПрав;
	#Если Сервер И Не Сервер Тогда
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирРедакторОграниченияДоступа.Форма.КэшПрав");
