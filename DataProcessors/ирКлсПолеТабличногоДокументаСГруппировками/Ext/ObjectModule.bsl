...
Процедура ПерезаполнитьКоманднуюПанельСтатическимиКнопками() Экспорт 
	...
	ЛксДобавитьКнопкиКоманднойПанелиКомпоненты(ЭтотОбъект, ФормаКласса.ЭлементыФормы.КоманднаяПанельОбщая.Кнопки, КоманднаяПанель);
...
Процедура СвернутьДоУровня(СтрокаКоманды) Экспорт
	...
	СтрокаУровня = ЛксПолучитьСтрокуМеждуМаркерами(СтрокаКоманды, МаркерСвернутьДоУровня);
	СтрокаНомераУровняСтрок = ЛксПолучитьСтрокуМеждуМаркерами(СтрокаУровня, "Строк", , Ложь);
	Если СтрокаНомераУровняСтрок <> Неопределено Тогда 
		...
		ПолеТабличногоДокумента.ПоказатьУровеньГруппировокСтрок(Уровень);
	Иначе
		СтрокаНомераУровняКолонок = ЛксПолучитьСтрокуМеждуМаркерами(СтрокаУровня, "Колонок", , Ложь);
...
Процедура СформироватьМенюГруппировок(Источник) Экспорт 
	...
	Для Счетчик = 1 По 2 Цикл
		...
		Если КоличествоГруппировок > 1 Тогда
			...
			КонецЦикла;
			ЛксДобавитьКнопкиКоманднойПанелиКомпоненты(ЭтотОбъект, МассивКнопок, КнопкаУровни);
		КонецЕсли;
	КонецЦикла;
