		...
		Для Каждого МетаОбъект Из КоллекцияМетаОбъектов Цикл
			...
			Если ТипЗнч(МетаОбъект) = Тип("Файл") Тогда
				...
				Исключение
					...
					Если Найти(НРег(ОписаниеОшибки), НРег("не может быть прочитан")) = 0 Тогда
						ДобавитьСтрокуОшибки(ВнешнийОбъект.ИспользуемоеИмяФайла, ИнформацияОбОшибке());
					КонецЕсли; 
					Продолжить;
				КонецПопытки; 
			...
			Если СтрокаТипаМетаданных.Единственное = "Справочник" Тогда
				Если МетаОбъект.Владельцы.Количество() > 0 Тогда
					...
					ПараметрыФормы = Новый Структура("ПараметрВыборПоВладельцу, Владелец, Отбор", Объект.Владелец, Объект.Владелец, Новый Структура("Владелец", Объект.Владелец));
				КонецЕсли; 
			КонецЕсли; 
			Для Каждого МетаФорма Из МетаФормы Цикл
				ИмяОперации = МетаФорма.ПолноеИмя();
...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	ПодключитьОбработчикОжидания("ОбновитьДоступность", 0.1, Истина);
			...
			Пока Истина Цикл
				...
				Исключение
					Если ОбработкаCOM.мТекущееПолноеИмяМД = Неопределено Тогда
						ВызватьИсключение;
					КонецЕсли; 
					ДобавитьСтрокуОшибки(ОбработкаCOM.мТекущееПолноеИмяМД + ".Объект", ИнформацияОбОшибке().Причина, , "ВнешнееСоединение");
				КонецПопытки;
...
Процедура ОбновитьВнешниеМетаданные(ЗаполнитьТолькоЕслиНет = Ложь)
	...
	Для СчетчикОбъектыМетаданных = 1 По НачальноеКоличество Цикл
		...
		Если Ложь
			Или ирОбщий.ПолучитьПервыйФрагментЛкс(ПолноеИмяМД) = "ВнешняяОбработка" 
			Или ирОбщий.ПолучитьПервыйФрагментЛкс(ПолноеИмяМД) = "ВнешнийОтчет"
		Тогда
...
Процедура РезультатыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	
	КорневойТип = ирОбщий.ПолучитьПервыйФрагментЛкс(ДанныеСтроки.Метаданные);
...
Процедура ОбновитьДоступность()
	...
	ЭлементыФормы.ЗаписьНаСервере.Доступность = ПроверятьОбъекты;
