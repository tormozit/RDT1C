...
Функция РедактироватьСтруктуруЗапроса(ВладелецФормы = Неопределено, СтруктураЗапроса) Экспорт
	...
	Если СтруктураЗапроса.Свойство("Параметры") Тогда
		мСтрокаЗапроса.ПараметрыЗапроса = ирОбщий.КопияОбъектаЛкс(СтруктураЗапроса.Параметры);
	КонецЕсли;
	Если СтруктураЗапроса.Свойство("ПараметрыADO") Тогда
		мСтрокаЗапроса.ПараметрыADO = ирОбщий.КопияОбъектаЛкс(СтруктураЗапроса.ПараметрыADO);
	КонецЕсли;
	Если СтруктураЗапроса.Свойство("ПараметрыWMI") Тогда
		мСтрокаЗапроса.ПараметрыWMI = ирОбщий.КопияОбъектаЛкс(СтруктураЗапроса.ПараметрыWMI);
	КонецЕсли;
...
Функция ConnectADO(ИсточникДанных, СоединениеADO = Неопределено, стОпции = Неопределено, Еррорс = Неопределено, выхСтрокаСоединенияДляСообщения = "", Асинхронно = Ложь) Экспорт
	Перем Параметры;
	Перем Результат;
	...
	// макро имена параметров подключения
	Результат = Неопределено;
	СоединениеADO.ConnectionTimeOut = 5; // секунд. 5 - недостаточно 
	СоединениеADO.CommandTimeout = 120; // секунд
	СоединениеADO.CursorLocation = 3; // курсоры на стороне клиента
	ирОбщий.СостояниеЛкс("ADO Подключение " + выхСтрокаСоединенияДляСообщения);
	...
	КонецЕсли; 
	Попытка
		СоединениеADO.Open(Параметры.СтрокаСоединения, Параметры.Пользователь, Параметры.Пароль, КодСинхронности);
		Результат = 1;
	Исключение
		СтрЕррор = 
