...
Функция ОбновитьПараметрыКонтекстнойПодсказкиТекстаЗапроса(ПодключатьсяКМетаданным = Истина)
	...
	Для Каждого ОписаниеТаблицы Из ВременныеТаблицы Цикл
		ПолеТекстовогоДокументаСКонтекстнойПодсказкой.ТекстЗапроса.НайтиДобавитьДоступнуюВременнуюТаблицу(ОписаниеТаблицы.Имя);
	КонецЦикла;
...
Процедура КлсПолеТекстовогоДокументаСКонтекстнойПодсказкойНажатие(Кнопка)
	...
	Если мРежимДереваЗапроса Тогда
		...
		Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если Кнопка = ирОбщий.ПолучитьКнопкуКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "Форматировать") Тогда
...
Процедура СохранитьСтарыеКолонкиТабличногоПоляРезультата()
	...
	Если мСтарыеКолонкиТабличногоПоляРезультата = Неопределено Тогда
	...
	Для Каждого СтараяКолонкаТП Из ЭлементыФормы.РезультатКоллекция.Колонки Цикл
		Если СтараяКолонкаТП.Ширина > мШиринаПустойКолонки И ЗначениеЗаполнено(СтараяКолонкаТП.Данные) Тогда
			мСтарыеКолонкиТабличногоПоляРезультата.Вставить(КлючСохраненияНастроекКолонкиРезультата(СтараяКолонкаТП), СтараяКолонкаТП);
		КонецЕсли; 
	КонецЦикла;

КонецПроцедуры

Функция КлючСохраненияНастроекКолонкиРезультата(Знач СтараяКолонкаТП)
	
	СтараяКолонкаТЗ = ЭлементыФормы.РезультатКоллекция.Значение.Колонки[СтараяКолонкаТП.Имя];
...
Процедура ВыполнитьЗапрос(Кнопка = Неопределено, Знач пЗагружатьЭлементов = Неопределено, РежимВременнойТаблицы = Неопределено, СохранитьДляОтложеннойОтладки = Неопределено,
	...
	Если пЗагружатьЭлементов = Неопределено Тогда
		пЗагружатьЭлементов = ЗагружатьЭлементов;
	КонецЕсли;
	Если Не КлючУникальности = "Расшифровки" Тогда
	...
	Если ЭлементыФормы.РезультатКоллекция.ТекущаяСтрока <> Неопределено Тогда
		...
		Для Каждого Колонка Из ЭлементыФормы.РезультатКоллекция.Значение.Колонки Цикл
			...
			Если Ложь
				Или ТипЗнч(ЗначениеКолонки) = Тип("ТаблицаЗначений")
				Или ТипЗнч(ЗначениеКолонки) = Тип("ХранилищеЗначения")
				Или (Истина
					И ТипЗнч(ЗначениеКолонки) = Тип("Строка")
					И СтрДлина(ЗначениеКолонки) > 1000)
			Тогда
				Продолжить;
			КонецЕсли; 
	...
	Иначе
		Если РежимВременнойТаблицы <> Истина Тогда
			...
			МассивСоздаваемыхВременныхТаблиц = УничтожитьВременныеТаблицыПередВыполнениемЗапроса1С(ТекстЗапроса, ИспользоватьНастройкиРасширений);
		Иначе
...
Функция УничтожитьВременныеТаблицыПередВыполнениемЗапроса1С(Знач ТекстЗапроса, Знач ИспользоватьНастройкиРасширений = Неопределено)
...
Процедура ДеревоЗапросовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель)
	...
	КонецЕсли; 
	НоваяСтрока.Запрос = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ТекущийРодитель.Строки, НоваяСтрока.Запрос, "Запрос", Ложь, "Запрос");
	Если мРежимДереваЗапроса Тогда
		КоманднаяПанельФормыРежимДереваЗапроса();
	КонецЕсли; 
...
Процедура ПараметрыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Истина
		И НоваяСтрока 
	Тогда 
		ТекущиеДанные.ИмяПараметра = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(Параметры, ТекущиеДанные, "ИмяПараметра");
	КонецЕсли;
...
Процедура ПриПолученииДанныхДоступныхПолей(Элемент, ОформленияСтрок)
	...
	ирОбщий.ПриПолученииДанныхДоступныхПолейКомпоновкиЛкс(ОформленияСтрок, ЭлементыФормы.ВыборКомпоновки);
...
Процедура ДеревоЗапросовПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Тогда
		...
		Для Каждого СтрокаПараметра Из ЗначениеПеретаскивания Цикл
			...
			СтрокаНовогоПараметра.ИмяПараметра = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(Строка.ПараметрыЗапроса, СтрокаНовогоПараметра, "ИмяПараметра");
...
Процедура МенюДереваЗапросовДобавитьПапку(Кнопка)
	...
	НоваяСтрока.Запрос = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ТекущийРодитель.Строки, НоваяСтрока.Запрос, "Запрос", Ложь, "Папка");
	ЭлементыФормы.ДеревоЗапросов.ТекущаяСтрока = НоваяСтрока;
...
Процедура НастроитьКолонкиТабличногоПоляРезультата()
	...
	Для Каждого Колонка Из ТабличноеПолеРезультата.Колонки Цикл
		...
		СохраненныеНастройкиКолонки = мСтарыеКолонкиТабличногоПоляРезультата[КлючСохраненияНастроекКолонкиРезультата(Колонка)];
		...
		Если ВыполнятьАнализДанных Тогда
			...
			Если ПустыеСтрокиБезNUll.Количество() + ПустыеСтрокиNull.Количество() = КоличествоСтрокВТаблице Тогда
				Колонка.Ширина = мШиринаПустойКолонки;
			КонецЕсли; 
		КонецЕсли; 
	КонецЦикла; 
	Если СпособВыгрузки = 2 Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	ОбновитьОтображениеИтоговВПодвалеРезультата();
...
Процедура ОбновитьОтображениеИтоговВПодвалеРезультата()
...
Процедура ВыполнитьВсеПодзапросыСтрокиДереваЗапросов(Знач СтрокаДереваЗапроса = Неопределено, ВыполнятьУжеВыполненные = Истина, Индикатор = Неопределено, ВыполнятьТолькоЗапросы = Ложь,
...
Процедура ПоместитьДанныеСтрокиЗапросаВоВременнуюТаблицу(СтрокаЗапроса = Неопределено)
	...
	Если Не ирОбщий.ЛиИмяПеременнойЛкс(ИмяТаблицы) Тогда
		//ИмяТаблицы = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ирОбщий.ПолучитьРодителяСтрокиДереваЛкс(СтрокаЗапроса).Строки,
...
Функция НайтиПозициюТекстаВДеревеЗапроса(СтрокиДерева, НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка, ИгнорироватьМелкиеУзлы = Ложь)
	
	Результат = Неопределено;
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		СтрокаДанных = СтрокаДерева.СтрокаДанных;
		Результат = НайтиПозициюТекстаВДеревеЗапроса(СтрокаДерева.Строки, НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка, ИгнорироватьМелкиеУзлы);
		Если Истина
...
Процедура КоманднаяПанельПараметрыЗаменитьЛитераломЗНАЧЕНИЕ(Кнопка)
	...
	Если ЗначениеЗаполнено(ТекстЗамены) Тогда
		...
		#Если Сервер И Не Сервер Тогда
			КомпонентаТекстаЗапроса = Обработки.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой.Создать();
		#КонецЕсли
		ИскомаяСтрока = "&" + ТекущиеДанные.ИмяПараметра;
		...
		КонецЦикла; 
		Если мРежимДереваЗапроса Тогда
			Сообщить("Обращения к параметру " + ИскомаяСтрока + " заменены на " + ТекстЗамены + " в тексте текущего подзапроса.");
		Иначе
			Если КомпонентаТекстаЗапроса.НайтиПоказатьСловоВТексте("&" + ТекущиеДанные.ИмяПараметра) Тогда
				Сообщить("Обращения к параметру в виде " + ИскомаяСтрока + " заменены на " + ТекстЗамены + ", но остались другие незаменяемые обращения.");
			Иначе
				Сообщить("Все обращения к параметру " + ИскомаяСтрока + " заменены на " + ТекстЗамены + ", а сам параметр удален.");
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаВыполнятьНаСервере(Кнопка)
	...
	ПолеВстроенногоЯзыка = ПолеТекстовогоДокументаСКонтекстнойПодсказкой.КодОбработкиСтрокиРезультата;
	#Если Сервер И Не Сервер Тогда
	    ПолеВстроенногоЯзыка = Обработки.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой.Создать();
	#КонецЕсли
	ПолеВстроенногоЯзыка.ЛиСерверныйКонтекст = НаСервере;
...
Процедура КоманднаяПанельКоллекцияПерейтиКОпределению(Кнопка)
	...
	Тогда
		Возврат;
	КонецЕсли;
	ИмяПоля = ЭлементыФормы.РезультатКоллекция.ТекущаяКолонка.Данные;
	КомпонентаТекстаЗапроса = ПолеТекстовогоДокументаСКонтекстнойПодсказкой.ТекстЗапроса;
	#Если Сервер И Не Сервер Тогда
	    КомпонентаТекстаЗапроса = Обработки.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой.Создать();
	#КонецЕсли
	КомпонентаТекстаЗапроса.НайтиПоказатьСловоВТексте("КАК " + ИмяПоля);
...
Процедура КП_СвойстваСтрокиРезультатаПерейтиКОпределению(Кнопка)
	...
	Если ЭлементыФормы.СвойстваСтрокиРезультата.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ИмяПоля = ЭлементыФормы.СвойстваСтрокиРезультата.ТекущаяСтрока.Имя;
	КомпонентаТекстаЗапроса = ПолеТекстовогоДокументаСКонтекстнойПодсказкой.ТекстЗапроса;
	#Если Сервер И Не Сервер Тогда
	    КомпонентаТекстаЗапроса = Обработки.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой.Создать();
	#КонецЕсли
...
Процедура КоманднаяПанельКоллекцияРасшифроватьСтроку(Кнопка)
	...
	Если Ответ = КодВозвратаДиалога.Нет Тогда
		...
		Если КонструкторЗапроса.ЗагрузитьТекстВКонструктор(ТекстЗапроса, КонструкторЗапроса) Тогда 
			...
			Если Не КонструкторЗапроса.ОпцияПервые Тогда
				...
				КонструкторЗапроса.ПервыеКоличество = 1000000;
			КонецЕсли; 
			...
			УничтожитьВременныеТаблицыПередВыполнениемЗапроса1С(ТекстЗапросаБезОтбора);
	...
	Если КонструкторЗапроса.ЗагрузитьТекстВКонструктор(ТекстЗапроса, КонструкторЗапроса) Тогда
		...
		Если КонструкторЗапроса.ЧастиОбъединения.Количество() > 1 Тогда
			Сообщить("Расшифровка строки результата поддерживает только запросы без объединений");
			Возврат;
		КонецЕсли;
		СохранитьДанныеТекущейСтрокиДереваЗапросов();
		...
		СтрокаЗапросаНовойФормы.Запрос = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(НоваяФорма.ДеревоЗапросов.Строки, СтрокаЗапросаНовойФормы, "Запрос", Ложь);
		...
		ЕстьАгрегаты = Ложь;
		ВыбранныеПоляПослеГруппировки = КонструкторЗапроса.ВыбратьВсеПоляИспользованныеВВыражениях(Ложь, ЕстьАгрегаты);
		...
		НовыйТекстЗапроса = КонструкторЗапроса.СобратьПолныйТекст(, Истина);
		...
		Если НоваяФорма.ЭлементыФормы.РезультатКоллекция.Значение.Количество() > 0 Тогда
			Для Каждого СтрокаДопПоля Из ВыбранныеПоляПослеГруппировки Цикл
				...
				Тогда
					РасшифровкаПоля = КонструкторЗапроса.ПолучитьПредставлениеЗначенияНаЯзыке(ЗначениеПоля);
				Иначе
					...
					КонецЕсли; 
				КонецЕсли;
				СтрокаДопПоля.Расшифровка = РасшифровкаПоля;
			КонецЦикла;
		КонецЕсли;
		СтруктураПолей = Новый Структура;
		СтруктураПолей.Вставить("Режим", "Замена");
		...
		НоваяФорма.КоманднаяПанельФормыРежимДереваЗапроса();
		Если ЕстьАгрегаты Тогда
			НоваяФорма.ИзменитьСвернутостьРезультатПакета(ЕстьАгрегаты);
...
Процедура КПОтборКомпоновкиНайтиВСписке(Кнопка)
	...
	ирОбщий.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭлементыФормы.КомпоновщикДоступныеПоляОтбора, ЭлементыФормы.ОтборКомпоновки);
