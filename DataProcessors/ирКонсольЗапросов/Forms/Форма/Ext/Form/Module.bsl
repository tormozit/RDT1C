...
Функция КодУстановкиПараметра(Знач СтрокаПараметра, ИмяПеременнойЗапроса = "мОбъектЗапроса")
	...
	КонецЕсли; 
	КодУстановкиПараметра = ИмяПеременнойЗапроса + ".Параметры.Вставить(""" + СтрокаПараметра.ИмяПараметра + """, " + ТекстЗначения + "); " + КомментарийЗначения;
...
Функция ЛитеральноеПредставлениеЗначенияВЗапросе(ЗначениеПараметра)
	...
	КонецЕсли;
	Если ТекстЗамены = "" Тогда
		Сообщить("Литеральное представление в запросе для значения """ + ирОбщий.РасширенноеПредставлениеЗначенияЛкс(ЗначениеПараметра) + """ невозможно")
	Иначе 
		Если ОбъектМД <> Неопределено Тогда
