...
Процедура РедакторHTMLДокументСформирован(Элемент)
	...
	добавитьобработчик Элемент.Документ.defaultView.onclick, ЭтаФорма.РедакторHTMLonclick; // https://github.com/salexdv/bsl_console/issues/178
...
Процедура ПанельРедактораПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML Тогда
		...
		Поле2.УстановитьТекст(Поле1.ПолучитьТекст(), Истина, мТекущаяСтрокаДереваЗапросов.ТекстЗапроса);
...
Процедура РедакторHTMLonclick(ДанныеСобытия) Экспорт 
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_SIGNATURE" Тогда
			ПолеТекстаЗапроса.РедакторHTML_ПередПоказомСигнатуры(Событие.params.activeParameter, Событие.params.word, Событие.params.activeSignature, Событие.params.triggerCharacter);
		ИначеЕсли Событие.event = "EVENT_BEFORE_SHOW_SUGGEST" Тогда
			...
			ПолеТекстаЗапроса.РедакторHTML_ПередПоказомАвтодополнения(Событие.params.trigger, Событие.params.last_expression, Событие.params.last_word);
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			ПолеТекстаЗапроса.РедакторHTML_ПриВыбореСтрокиАвтодополнения(Событие.params.trigger, Событие.params.last_expression, Событие.params.selected);
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			ПолеТекстаЗапроса.РедакторHTML_ПриАктивацииСтрокиАвтодополнения(Событие.params.trigger, Событие.params.focused, Событие.params.row_id);
		КонецЕсли;
	КонецЕсли;
