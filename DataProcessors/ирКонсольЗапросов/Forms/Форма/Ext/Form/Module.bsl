...
Процедура КлсПолеТекстовогоДокументаСКонтекстнойПодсказкойНажатие(Кнопка)
	...
	Если Ложь
		Или Кнопка = ирОбщий.ПолучитьКнопкуКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КонструкторЗапросовИР")
	Тогда
		Для Каждого КлючИЗначение Из КомпонентаТекстаЗапроса.КонтекстВыполнения.Параметры Цикл
			СтрокаПараметра = Параметры.Найти(НРег(КлючИЗначение.Ключ), "НИмяПараметра");
			Если СтрокаПараметра <> Неопределено Тогда
				Продолжить;
			КонецЕсли; 
...
Функция ПроверитьКорректностьТекстаЗапроса(ЛиДобавлятьНовые = Истина, ОбновитьНастройки = Истина, СтартоваяСтрока = 0, Знач ТекстЗапроса = "")
...
Функция РазобратьТекстЗапроса(ТекстЗапроса = Неопределено, СтроитьДеревоЗапроса = Истина, ПолеТекстовогоДокумента = Неопределено, СтартоваяСтрокаДляОтображенияОшибок = 0)
	...
	Тогда 
		...
		Если ДеревоЗапроса.Строки.Количество() > 0 Тогда
			...
			ЭлементыФормы.МенюДереваЗапроса.Кнопки.МенюСтруктурыЗапроса.Кнопки.ВыполнитьВсеЗапросыПакета.Доступность = ЭтоПакетЗапросов;
			ЭлементыФормы.МенюДереваЗапроса.Кнопки.МенюСтруктурыЗапроса.Кнопки.ВыполнитьЗапрос.Доступность = ЭтоПакетЗапросов;
...
Процедура ДеревоЗапросаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	КонецЕсли; 
	Если ДанныеСтроки.Имя = мИмяВременнойТаблицыДляОтображенияЗависимых Тогда
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаПолучитьТекст(Кнопка)
	...
	ТекстМодуля = ТекстМодуля + "
	|" + ПеременнаяТаблица + " = РезультатЗапроса";
	Если ТипЗнч(мРезультатЗапроса) = Тип("РезультатЗапроса") Тогда
...
Процедура КоманднаяПанельКоллекцияОткрытьТаблицу(Кнопка)
	...
	Форма = ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ЭлементыФормы.РезультатКоллекция.Значение, Ложь,,,,, ЭлементыФормы.РезультатКоллекция);
...
Процедура МенюДереваЗапросаНайтиРезультат(Кнопка)
...
Процедура КП_СвойстваСтрокиРезультатаСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(мСравнительТабличныхДокументов, ЭлементыФормы.СвойстваСтрокиРезультата);
...
Процедура МенюДереваЗапросаПодкраситьЗависимыеЗапросы(Кнопка)
	...
	Тогда
		...
		ЗависимыеСтрокиДерева = ДеревоЗапроса.Строки.НайтиСтроки(Новый Структура("ТипУзла, Имя",  "<TableWithName>", мИмяВременнойТаблицыДляОтображенияЗависимых), Истина);
