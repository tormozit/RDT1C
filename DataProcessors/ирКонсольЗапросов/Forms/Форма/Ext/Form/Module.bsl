...
Функция ОбновитьПараметрыКонтекстнойПодсказкиТекстаЗапроса(ПодключатьсяКМетаданным = Истина)
	...
	Иначе
		...
		ДиалектSQL = "1С";
	КонецЕсли; 
	ПолеТекстаЗапроса.УстановитьКонфигурациюМетаданных(КонфигурацияМетаданных, КонтекстВыполненияЗапроса, ДиалектSQL,
...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "ПерейтиКОпределению") Тогда
		...
		ТекущееВыражение = КомпонентаТекстаЗапроса.ТекущееОбъектноеВыражение(,, ЕстьТочкаСправа);
...
Процедура СохранитьФайлДляВосстановленияПериодически()
	...
	Если Истина
		И ирОбщий.Форма_ЛиФокусВводаЛкс(ЭтаФорма, Истина)
		И (Ложь
...
Функция ЛиПарсерЗапросаДоступен()
	...
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	Парсер = мПлатформа.мПолучитьПарсер("ГрамматикаЯзыкаЗапросов",, "Будет использовано упрощенное дерево запроса. Для полного дерева необходима регистрация COM-компоненты GoldParser");
...
Функция РедакторHTML()
	Возврат ЭлементыФормы.РедакторHTML.Документ.defaultView;
КонецФункции

Процедура РедакторHTMLДокументСформирован(Элемент = Неопределено)
	...
	РедакторHTML.enableSuggestActivationEvent(Истина);
...
Процедура ПанельРедактораПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ЭлементыФормы.ПанельРедактора.ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML Тогда
		Если Истина
			И ЗначениеЗаполнено(РедакторHTML().version1C) // Чтобы игнорировать ситуацию до вызова события Сформирован()
			И ЭлементыФормы.РедакторHTML.Документ.defaultView <> Неопределено 
		Тогда
...
Процедура РедакторHTMLonclick(Элемент, ДанныеСобытия)
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_HOVER" Тогда
			...
			//	Или Не ирОбщий.ЛиИмяПеременнойЛкс(Лев(ОбъектноеВыражение, 1)) 
			//Тогда
			//	Возврат;
			//КонецЕсли; 
			//Подсказка = "";
			//ИмяТипаЗначения = "";
			//Попытка
			//	ЗначениеВыражения = ВычислитьТекущееВыражение(ОбъектноеВыражение);
			...
			//	ИмяТипаЗначения = СтруктураТипа.ИмяОбщегоТипа;
			//	Если ТипЗнч(ЗначениеВыражения) = Тип("Строка") Тогда
			//		Подсказка = """" + ЗначениеВыражения + """";
			//	Иначе
			//		Подсказка = ирОбщий.РасширенноеПредставлениеЗначенияЛкс(ЗначениеВыражения,, Ложь, Истина);
			//		Если Подсказка = ИмяТипаЗначения Тогда
			//			Подсказка = "[" + ИмяТипаЗначения + "]";
			//		Иначе
			//			Подсказка = Подсказка + " [" + ИмяТипаЗначения + "]";
			//		КонецЕсли; 
			//	КонецЕсли; 
			//КонецЕсли; 
			//ПредставлениеДопустимыхТипов = ПолеТекстаЗапроса.ПредставлениеМассиваСтруктурТипов(ПолеТекстаЗапроса.ТаблицаСтруктурТиповТекущегоВыражения(Истина, Истина));
			//Если Истина
			...
			//		Подсказка = Подсказка + "; ";
			//	КонецЕсли; 
			//	Подсказка = Подсказка + "Допустимые типы: " + ПредставлениеДопустимыхТипов;
			//КонецЕсли; 
			//ПолеТекстаЗапроса.КончитьОбработкуКоманды();
			//ОбновитьПодсказкуHTML(Событие.params.word.word, Подсказка);
			...
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				...
				Тогда 
					...
					Если ПоказатьСписок Тогда
						ПолеТекстаЗапроса.ЗаполнитьТаблицСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста);
						Если Ложь
		...
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			РедакторHTML.hideSuggestionsList();
			СтрокаТаблицыСлов = ПолеТекстаЗапроса.ТаблицаСлов.Найти(Событие.params.selected, "Слово");
		...
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			...
			//Если мРазрешенаПодсказкаHTML <> Истина Тогда 
			//	СтрокаТаблицыСлов = ПолеТекстаЗапроса.ТаблицаСлов.Найти(ирОбщий.СтрокаМеждуМаркерамиЛкс(Событие.params.focused, "Элемент ", ",", Истина) , "Слово");
			//	Если СтрокаТаблицыСлов <> Неопределено Тогда
			//		Если ПолеТекстаЗапроса.УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, СтрокаТаблицыСлов) Тогда 
			//			//РедакторHTML.hideSuggestionsList();
			//			//ПередатьТаблицуСловВРедакторHTML();
			//			ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(Событие.params.focused, , СтрокаТаблицыСлов.ТипЗначения, ", документы: ");
			//		КонецЕсли; 
			//	КонецЕсли; 
			//КонецЕсли; 
		КонецЕсли;
...
Процедура ОбновитьПодсказкуHTML(Выражение = "", Подсказка = "")
	РедакторHTML = АктивноеПолеТекста(ЭлементыФормы.РедакторHTML).РедакторHTML();
