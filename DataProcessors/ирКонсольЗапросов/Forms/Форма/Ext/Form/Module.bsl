...
Процедура КлсПолеТекстовогоДокументаСКонтекстнойПодсказкойНажатие(Кнопка)
	...
	Если Кнопка = ирОбщий.ПолучитьКнопкуКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		...
		Если СтрДлина(КомпонентаТекстаЗапроса.ПолеТекстовогоДокумента.ВыделенныйТекст) = 0 Тогда
			ПараметрыИЗТекстаЗапроса = ПолучитьПараметрыИзТекстаЗапроса(0,, Истина);
			#Если Сервер И Не Сервер Тогда
	...
	Если РежимВременнойТаблицы <> Истина Тогда
		...
		Для Каждого КлючИЗначение Из мОбъектЗапроса.Параметры Цикл
			Если ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") И Найти(НРег(мОбъектЗапроса.Текст), "&" + КлючИЗначение.Ключ) > 0 Тогда
				...
				Исключение
					ТаблицаПараметр = ирОбщий.ПолучитьТаблицуСМинимальнымиТипамиКолонокЛкс(ТаблицаПараметр);
					МассивСтрок = ирОбщий.БыстрыйМассивЛкс(ТаблицаПараметр[0]);
					...
					Если ТаблицаПараметр <> Неопределено Тогда
						Сообщить("Для избежания ошибки ""тип не может быть выбран в запросе"" из описаний типов колонок параметра-таблицы """ + КлючИЗначение.Ключ + """ удалены лишние типы без потери данных");
...
Функция ПроверитьКорректностьТекстаЗапроса(ЛиДобавлятьНовые = Истина, ОбновитьНастройки = Истина, СтартоваяСтрока = 0, Знач ТекстЗапроса = "", ДобавитьВыборкуИзВременнойТаблицы = Истина)
	...
	КонецЕсли; 
	ПараметрыИЗТекстаЗапроса = ПолучитьПараметрыИзТекстаЗапроса(СтартоваяСтрока, ТекстЗапроса);
	Если Ложь
		Или ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "ADO")
		Или ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "WQL")
	Тогда
	...
	Иначе
		РезультатПроверки = ПараметрыИЗТекстаЗапроса <> Неопределено;
	КонецЕсли; 
	...
	Если ЛиДобавлятьНовые Тогда
		Для каждого ПараметрЗапроса Из ПараметрыИЗТекстаЗапроса Цикл
			...
			Иначе
				ИндексПараметра = ПараметрыИЗТекстаЗапроса.Индекс(ПараметрЗапроса);
...
Функция ПолучитьПараметрыИзТекстаЗапроса(Знач СтартоваяСтрока, ТекстЗапроса = "", ПривестиИменаКНижнемуРегистру = Ложь)
