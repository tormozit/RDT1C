...
Функция ОбновитьПараметрыКонтекстнойПодсказкиТекстаЗапроса(ПодключатьсяКМетаданным = Истина)
	...
	ПолеТекстовогоДокументаСКонтекстнойПодсказкой.ТекстЗапроса.РежимКомпоновкиДанных = мТекущаяСтрока.ТипЗапроса;
	Возврат Неопределено;
	...
	ИначеЕсли ТипЗнч(ПолученноеЗначение) = Тип("ДеревоЗначений") Тогда
		...
		Если ПолученноеЗначение.Колонки.Найти("КоординатыТекущейСтроки") <> Неопределено Тогда // Такой несистемный прием нужен для совместимости по формату со старыми консолями
			Если ПолученноеЗначение.Строки.Количество() > 0 Тогда
				...
				Попытка
					НоваяТекущаяСтрока = ирНеглобальный.ПолучитьСтрокуДереваПоКоординатамЛкс(ДеревоЗапросов, КоординатыТекущейСтроки);
				Исключение
...
Функция вСохранитьЗапросыВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, ОповеститьВладельца = Ложь, Знач ИмяФайла = Неопределено, 
	...
	Иначе
		...
		Если КопияДерева.Строки.Количество() > 0 Тогда
			КопияДерева.Строки[0].КоординатыТекущейСтроки = ирНеглобальный.ПолучитьКоординатыСтрокиДереваЛкс(мТекущаяСтрока);
		КонецЕсли; 
		ФайлВыбран = ирНеглобальный.СохранитьФайлВКонсолиСВосстановлениемЛкс(ДиалогВыбораФайла, ИмяФайла, мИмяФайла, КопияДерева, мСтруктураВосстановления, 
			ЗапрашиватьИмяФайла);
	...
	Если мКоллекцияДляЗаполнения <> Неопределено Тогда
		Если мКоллекцияДляЗаполнения.Колонки.Количество() > 0 Тогда
			...
			Если СписокОтсутствующихКолонок <> "" Тогда
				...
				ЛксСообщитьСУчетомМодальности("Обязательные колонки """ + Сред(СписокОтсутствующихКолонок, 2) + """ отсутствуют в результате", МодальныйРежим, СтатусСообщения.Внимание);
				Возврат;
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
				...
				Если СпособВыгрузки = 1 Тогда
					...
					Иначе //Если ТипЗнч(мРезультатЗапроса) = Тип("COMОбъект") Тогда
						Если ирНеглобальный.СтрокиРавныЛкс(мТекущаяСтрока.ТипЗапроса, "WQL") Тогда
							...
							ТекстПорядок = ирНеглобальный.ПолучитьВыражениеПорядкаКомпоновкиНаЯзыкеЛкс(Компоновщик.Настройки.Порядок,,, "WQL"); 
							Если Истина
					...
					Если ТипЗнч(мРезультатЗапроса) = Тип("РезультатЗапроса") Тогда
						...
						Если Не СтандартнаяВыгрузкаВДерево Тогда
							...
							Если мТаблицаВыборок.Количество() = 0 Тогда
								ЛксСообщитьСУчетомМодальности("Не описана ни одна выборка итогов!", МодальныйРежим, СтатусСообщения.Внимание);
								Возврат;
							КонецЕсли; 
						...
						Иначе
							...
							Если ЗагрузитьДерево Тогда
								...
								ирНеглобальный.ОбновитьЧислоЗагруженныхЭлементовВыборкиЛкс(ЭлементыФормы.РезультатКоллекция, ЭлементыФормы.КоманднаяПанельКоллекция,
...
Функция ЗагрузитьРезультатВДеревоРезультатаПоВыборкамИтогов(ПолнаяЗагрузка = Ложь)
	...
	СчетчикРекурсии = 0;
	Индикатор = ЛксПолучитьИндикаторПроцесса(, "Загрузка результата");
...
Процедура ОбновитьЧистуюДлительность(СтрокаДереваЗапроса)
	...
	Если Ложь
		...
		Или ирНеглобальный.СтрокиРавныЛкс(СтрокаДереваЗапроса.ТипУзла, "<TableWithName>")
	Тогда
		Возврат;
	КонецЕсли; 
	...
	Для Каждого ДочерняяСтрока Из СтрокаДереваЗапроса.Строки Цикл
		Если ТипЗнч(ДочерняяСтрока.ДлительностьПолная) <> Тип("Число") Тогда
			Если Не ирНеглобальный.СтрокиРавныЛкс(ДочерняяСтрока.ТипУзла, "<TableWithName>") Тогда
...
Процедура ЗагрузитьЗапросВКонсоль()
	...
	Если Истина
		И мТекущаяСтрока <> Неопределено
		И ирНеглобальный.СтрокиРавныЛкс(мТекущаяСтрока.ТипЗапроса, "ADO") 
	Тогда 
	...
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ИсходнаяТаблицаПараметров <> Неопределено Тогда
			Для каждого СтрокаИсходнойТаблицы из ИсходнаяТаблицаПараметров Цикл
				...
				КонецЕсли;
				ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(НоваяСтрока, "ИмяПараметра");
			КонецЦикла;
		КонецЕсли;
...
Процедура ДеревоЗапросовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель)
	...
	Если ТекущийРодитель.ТипЗапроса <> "Папка" Тогда
		ТекущийРодитель = ирНеглобальный.ПолучитьРодителяСтрокиДереваЛкс(ТекущийРодитель);
	КонецЕсли; 
	НоваяСтрока = ТекущийРодитель.Строки.Добавить();
	Если Копирование Тогда
		ЛксСкопироватьСтрокиДерева(ЭлементыФормы.ДеревоЗапросов.ТекущаяСтрока, НоваяСтрока);
	Иначе
		УстановитьЗначенияПоУмолчаниюСтрокиЗапроса(НоваяСтрока);
	КонецЕсли; 
	НоваяСтрока.Запрос = ирНеглобальный.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(ТекущийРодитель.Строки, НоваяСтрока.Запрос, "Запрос", Ложь, "Запрос");
...
Процедура КоманднаяПанельКоллекцияСвернутьНажатие(Элемент)
	...
	ЛксДеревоЗначенийСвернуть(ЭлементыФормы.РезультатКоллекция, истина);
...
Процедура КоманднаяПанельКоллекцияРазвернутьНажатие(Элемент)
	...
	ЛксДеревоЗначенийСвернуть(ЭлементыФормы.РезультатКоллекция, Ложь);
...
Функция УдалитьВременныеТаблицы()
	...
	Если мТекущаяСтрока <> Неопределено Тогда
		Если ирНеглобальный.СтрокиРавныЛкс(мТекущаяСтрока.ТипЗапроса, "1С") Тогда 
...
Процедура СброситьМенеджерВременныхТаблиц()
	...
	ИначеЕсли ирНеглобальный.СтрокиРавныЛкс(мТекущаяСтрока.ТипЗапроса, "1С") Тогда
		мОбъектЗапроса.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		мВременныеТаблицы1С.Очистить();
	КонецЕсли; 
	мИзмененияМенеджераВременныхТаблиц.Очистить();
...
Процедура ПараметрыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Истина
		И НоваяСтрока 
	Тогда 
		ТекущиеДанные.ИмяПараметра = ирНеглобальный.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(Параметры, ТекущиеДанные, "ИмяПараметра");
	КонецЕсли;
...
Процедура СоздатьВременныеТаблицы(СтрокаДереваЗапроса)
	...
	Для Каждого ДочерняяСтрока Из СтрокаДереваЗапроса.Строки Цикл
		...
		Иначе
			...
			КонецЦикла;
			ВыполнитьЗапросИсточникаДанных(ЛксПолучитьЗапросИмитаторКоллекцииПолей(КоллекцияПолей) + " INTO " + ДочерняяСтрока.Псевдоним);
		КонецЕсли;
	КонецЦикла;
...
Процедура РезультатКоллекцияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирНеглобальный.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.Пустые.Пометка);
...
Процедура КоманднаяПанельКоллекцияШиринаКолонок(Кнопка)
	...
	ЛксВвестиИУстановитьШиринуКолонокТабличногоПоля(ЭлементыФормы.РезультатКоллекция);
	
КонецПроцедуры

Процедура КоманднаяПанельКоллекцияСжатьКолонки(Кнопка)
	
	ЛксСжатьКолонкиТабличногоПоля(ЭлементыФормы.РезультатКоллекция);
...
Процедура КоманднаяПанельКоллекцияСравнить(Кнопка)
	...
	ЛксСравнитьСодержимоеЭлементаУправления(мСравнительТабличныхДокументов, ЭлементыФормы.РезультатКоллекция);
...
Процедура КоманднаяПанельКоллекцияКонсольОбработки(Кнопка)
	...
	ирНеглобальный.ОткрытьОбъектыИзВыделенныхЯчеекВПодбореИОбработкеОбъектов(ЭтаФорма.ЭлементыФормы.РезультатКоллекция);
...
Процедура ДеревоЗапросовПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Тогда
		...
		Для Каждого СтрокаПараметра Из ЗначениеПеретаскивания Цикл
			...
			ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаНовогоПараметра, "ИмяПараметра");
		КонецЦикла;
	КонецЕсли; 
...
Процедура КоманднаяПанельКоллекцияЗаполнитьГруппыДублейДляЗамены(Кнопка)
	...
	ЛксОткрытьФормуЗаменыСсылокИзТабличногоПоля(ЭтаФорма.ЭлементыФормы.РезультатКоллекция);
...
Процедура КоманднаяПанельКоллекцияРедакторОбъектаБД(Кнопка)
	...
	ирНеглобальный.ОткрытьСсылкуЯчейкиВРедактореОбъектаБДЛкс(ЭлементыФормы.РезультатКоллекция);
	
КонецПроцедуры

Процедура КоманднаяПанельКоллекцияКонсольКомпоновки(Кнопка)
	
    Отчеты.ирКонсольКомпоновокДанных.Создать().ОткрытьПоТаблицеЗначений(РезультатКоллекция);
	
КонецПроцедуры

Процедура КоманднаяПанельКоллекцияОткрытьТаблицу(Кнопка)

	ирНеглобальный.ОткрытьФормуПроизвольногоЗначенияЛкс(РезультатКоллекция, Ложь);
...
Процедура КоманднаяПанельПараметрыДобавитьСтандартныйПериод(Кнопка)
	...
	ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаПериода, "ИмяПараметра");
	...
	ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаНачалаПериода, "ИмяПараметра");
	...
	ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаКонцаПериода, "ИмяПараметра");
...
Процедура КоманднаяПанельСсылкаНаОбъектБД(Кнопка)
	...
	ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаПараметра, "ИмяПараметра");
...
Процедура КоманднаяПанельКоллекцияЗагрузитьПолностью(Кнопка)
	...
	Если СпособВыгрузки = 1 Тогда
		ирНеглобальный.ЗагрузитьДинамическуюВыборкуВТабличноеПолеПолностьюЛкс(ЭтаФорма, мВыборкаРезультата, ЭлементыФормы.КоманднаяПанельКоллекция);
	Иначе
...
Процедура МенюДереваЗапросовДобавитьПапку(Кнопка)
	...
	НоваяСтрока.Запрос = ирНеглобальный.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(ТекущийРодитель.Строки, НоваяСтрока.Запрос, "Запрос", Ложь, "Папка");
	ЭлементыФормы.ДеревоЗапросов.ТекущаяСтрока = НоваяСтрока;
...
Процедура НастроитьКолонкиРезультата()
	...
	Если ТипЗнч(ТабличноеПолеРезультата.Значение) = Тип("ДеревоЗначений") Тогда
		// КоличествоСтрокВТаблице = ирНеглобальный.ПолучитьВсеСтрокиДереваЗначенийЛкс(ТабличноеПолеРезультата.Значение); // Долго
...
Функция ЗапросADOРезультат_Выгрузить(РезультатRecordset, Типизировать1С = Ложь, BynaryToHex = Ложь, RecordsAffected = 0) Экспорт
	...
	Иначе
		FieldКолонка = ирНеглобальный.ПолучитьКолонкиRecordsetADOЛкс(РезультатТаблица, РезультатRecordset, Типизировать1С);
		// Открыта выборка
...
Процедура DisconnectADO(ИсточникДанных, выхСоединениеADO, стОпции=Неопределено, Еррорс=Неопределено) Экспорт
	...
	//Если НЕ ПустаяСтрока(Параметры.Disconnect) Тогда
		...
		Если Истина
			...
			И ирНеглобальный.СтрокиРавныЛкс(Лев(ИсточникДанных.Путь, 7), "<Пусто>")
		Тогда
	...
	Пока ТекВыборка.Следующий() Цикл
		...
		Если ДобавлятьСлужебныеКолонкиРезультата Тогда
			...
			Если Истина
				...
				И Не ирНеглобальный.СтрокиРавныЛкс(ИмяГруппировкиЗаписи, "Общие")
			Тогда
...
Процедура ПоместитьВоВременныеТаблицы(Кнопка)
	...
	Если мТекущаяСтрока.ТипЗапроса = "Папка" Тогда
		Для Каждого ДочерняяСтрока Из мТекущаяСтрока.Строки Цикл
			ЛксСообщитьСУчетомМодальности("---Выполняем запрос """ + ДочерняяСтрока.Запрос + """");
...
Процедура ПоместитьДанныеСтрокиЗапросаВоВременнуюТаблицу(СтрокаЗапроса = Неопределено)
	...
	Иначе
		// ?
	КонецЕсли; 
	ТаблицаЗначений = ирНеглобальный.ПолучитьТаблицуЗначенийСМинимальнымиТипамиКолонокЛкс(ТаблицаЗначений, Истина);
	ИмяТаблицы = СтрокаЗапроса.Запрос;
	Если Не ЛксЛиИмяПеременной(ИмяТаблицы) Тогда
		ИмяТаблицы = ирНеглобальный.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(ирНеглобальный.ПолучитьРодителяСтрокиДереваЛкс(СтрокаЗапроса).Строки,
	...
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		ЛксСообщитьСУчетомМодальности("Ошибка помещения данных во временную таблицу 1С с именем запроса (""" + ИмяТаблицы + """): " + ОписаниеОшибки,
			МодальныйРежим, СтатусСообщения.Внимание);
		Возврат;
	КонецПопытки; 
	мВременныеТаблицы1С.Вставить(ИмяТаблицы);
	ЛксСообщитьСУчетомМодальности(XMLСтрока(ТаблицаЗначений.Количество()) 
...
Процедура ПутьКИсточникуДанныхADOОткрытие(Элемент, СтандартнаяОбработка)
	...
	ирНеглобальный.ОткрытьФайлСПредупреждениемЛкс(Элемент.Значение, СтандартнаяОбработка);
...
Процедура ПараметрыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Тогда
			ТекущиеДанные.ИмяПараметра = ирНеглобальный.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(Параметры, ТекущиеДанные, "ИмяПараметра");
			ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(ТекущиеДанные, "ИмяПараметра");
		КонецЕсли; 
	КонецЕсли; 
