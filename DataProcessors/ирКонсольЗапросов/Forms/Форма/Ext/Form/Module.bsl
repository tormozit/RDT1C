...
Функция ОбновитьКонтекстПодсказкиИПолучитьСтруктуруПараметров(ЭкземплярКомпоненты)
	...
	ИначеЕсли ЭкземплярКомпоненты = ПолеТекстовогоДокументаСКонтекстнойПодсказкой.КодПередВыполнениемЗапроса Тогда
	    ДобавитьПараметрыВКомпонентуВыполненияКода(ЭкземплярКомпоненты);
...
//
Процедура ВыполнитьЗапрос(Кнопка = Неопределено, Знач пЗагружатьЭлементов = Неопределено, РежимВременнойТаблицы = Неопределено)
	...
	Иначе
		...
		КонецЕсли; 
		Попытка
			мРезультатПакета = ВыполнитьЗапросИсточникаДанных(ТекстЗапроса);
		Исключение
...
Процедура РезультатКоллекцияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.Пустые.Пометка, ,
		ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.Идентификаторы.Пометка);
...
Функция ПолучитьКомментарийЗначенияДляПрограммногоКода(Знач ЗначениеЭлемента)
	...
	Если Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(ЗначениеЭлемента)) Тогда
		...
		КонецЕсли;
	ИначеЕсли ПланыВидовХарактеристик.ТипВсеСсылки().СодержитТип(ТипЗнч(ЗначениеЭлемента)) Тогда
...
Функция ПолучитьWMIСлужбу()
	...
	Если мWMIService = Неопределено Тогда
		мWMIService = ирКэш.ПолучитьCOMОбъектWMIЛкс(ИмяСервераWMI);
	КонецЕсли; 
	Возврат мWMIService;
...
Процедура СвойстваСтрокиРезультатаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки,
		ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.Пустые.Пометка, "Значение", ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.Идентификаторы.Пометка);
...
Процедура КоманднаяПанельКоллекцияОтображатьИдентфиикаторы(Кнопка)
