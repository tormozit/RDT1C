...
Процедура КоманднаяПанельКоллекцияРасшифроватьСтроку(Кнопка)
	...
	Если КонструкторЗапроса.ЗагрузитьТекстВКонструктор(ТекстЗапроса, КонструкторЗапроса) Тогда
		...
		КонецЦикла; 
		КонструкторЗапроса.ЗагрузитьПоследнийЗапрос();
		//Если КонструкторЗапроса.ЧастиОбъединения.Количество() > 1 Или КонструкторЗапроса.ЗапросыПакета.Количество() > 1 Тогда
		//	Сообщить("Расшифровка строки результата поддерживает только не пакетный запрос без объединений");
		//	Возврат;
		//КонецЕсли;
		...
		Если ЕстьАгрегаты Тогда
			...
			КонструкторЗапроса.УдалитьВыбранныеПоляИПорядок(ВыбранныеПоляПослеГруппировки);
		...
		Если ЕстьАгрегаты Тогда
			...
			ФрагментыПакета = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(НовыйТекстЗапроса, РазделительЗапросов);
			НовыйТекстЗапросаДоГруппировки = ФрагментыПакета[0];
			НовыйТекстЗапроса = ФрагментыПакета[1];
		КонецЕсли; 
		Для Каждого СтрокаДопПоля Из ВыбранныеПоляПослеГруппировки Цикл
			...
			Иначе
				...
				Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеПоля) Тогда
					СтрокаПараметра = ирОбщий.НайтиДобавитьПараметрСсылкуВТаблицуЛкс(Параметры, "ИмяПараметра", "Значение", ЗначениеПоля,, Истина);
					Если СтрокаПараметра <> Неопределено Тогда
						ОбновитьТипЗначенияВСтрокеПараметровВФорме(СтрокаПараметра);
			...
			Для СчетчикСмещения = 1 По 10 Цикл
				НовыйТекстЗапроса = ирОбщий.СтрЗаменитьЛкс(НовыйТекстЗапроса, ЧтоЗаменять, "{" + РасшифровкаПоля + "}", Истина, Ложь);
		...
		КонецЦикла;
		Если ЕстьАгрегаты Тогда
			НовыйТекстЗапроса = НовыйТекстЗапросаДоГруппировки + РазделительЗапросов + НовыйТекстЗапроса;
		КонецЕсли; 
		...
		Если ЕстьАгрегаты Тогда
			НоваяФорма.КоманднаяПанельФормыРежимДереваЗапроса();
			НоваяФорма.ИзменитьСвернутостьРезультатПакета(Истина);
