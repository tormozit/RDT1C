	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_SHOW_SUGGEST" Тогда
			#Если Сервер И Не Сервер Тогда
				ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
			#КонецЕсли
			Если ПолеТекстаЗапроса.мРазрешенаПодсказкаHTML <> Истина Тогда
		...
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			ПолеТекстаЗапроса.РедакторHTML_ПриВыбореСтрокиАвтодополнения(Событие.params.trigger, Событие.params.selected);
