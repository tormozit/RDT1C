...
Процедура ЗапроситьОчисткуМенеджераВременныхТаблиц()
	Открыть();
	...
	Если ВременныеТаблицы.Количество() > 0 Тогда
		...
		КонецЕсли; 
		Ответ = Вопрос("Менеджер временных таблиц удерживает соединение с СУБД. Очистить его для освобождения соединения для избежания снижения производительности сервера СУБД?",
