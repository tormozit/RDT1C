...
Процедура ОбработатьСтрокуДереваЗапроса(СтрокаДерева, НачальнаяСтрока = 1, НачальнаяКолонка = 1)
	...
	Для Каждого ДочерняяСтрока Из СтрокаДерева.Строки Цикл
		...
		Если СтрокаДанных = Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		ОбработатьСтрокуДереваЗапроса(ДочерняяСтрока, СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка + СмещениеКолонки);
	КонецЦикла;
...
Функция ПолучитьПостроительЗапросовДляИтогов(СообщатьОбОшибке = Ложь)
...
Процедура СвойстваСтрокиРезультатаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если ТекущаяСтрокаВладельца <> Неопределено Тогда
		ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельКоллекция.Кнопки.Идентификаторы, "Значение");
...
Функция ЗаполнитьВыборкиИтоговИзЗапроса()
	...
	ВыборкиИтогов.Очистить();
	ПостроительЗапроса = ПолучитьПостроительЗапросовДляИтогов(Истина);
	Если ПостроительЗапроса = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли; 
