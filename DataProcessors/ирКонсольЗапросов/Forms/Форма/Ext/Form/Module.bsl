...
Функция ОбновитьПараметрыКонтекстнойПодсказкиТекстаЗапроса(ПодключатьсяКМетаданным = Истина)
	...
	Иначе
		...
		ДиалектSQL = "1С";
		Для Каждого СтрокаТаблицы Из ВременныеТаблицы Цикл
			ПолеТекстовогоДокументаСКонтекстнойПодсказкой.ТекстЗапроса.НайтиДобавитьДоступнуюВременнуюТаблицу(СтрокаТаблицы.Имя);
		КонецЦикла;
	КонецЕсли; 
...
Процедура КлсПолеТекстовогоДокументаСКонтекстнойПодсказкойНажатие(Кнопка)
	...
	Если Кнопка = ирОбщий.ПолучитьКнопкуКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "ПерейтиКОпределению") Тогда
		...
		КонецЕсли;
		Если мРежимДереваЗапроса Тогда
			ФрагментыВыражения = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(ТекущееВыражение);
			ПервыйФрагмент = ФрагментыВыражения[0];
			СтрокаЗапросаСоздания = НайтиСтрокуДереваСозданияВременнойТаблицы(ПервыйФрагмент);
...
Процедура ЗапомнитьИсточникПереходаВДеревеЗапроса()
	...
	КонецЕсли; 
	мПутьВДеревеЗапросаОткудаПерешли.Вставить(0, ирОбщий.ПолучитьСтрокуПутиВДеревеЛкс(мТекущаяСтрокаДереваЗапроса, ""));
	...
	ИначеЕсли ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "ADO") Тогда
		...
		Для Каждого ИмяСоздаваемойВременнойТаблицы Из МассивСоздаваемыхВременныхТаблиц Цикл
			Если Ложь
				...
				Или мИзмененияМенеджераВременныхТаблиц[НРег(ИмяСоздаваемойВременнойТаблицы)] = Неопределено // Добавил, т.к. есть опасность удалить статическую таблицу
			Тогда
				Продолжить;
			КонецЕсли;
...
Функция УдалитьВременныеТаблицы()
	...
	// Оставляем в менеджере временных таблиц только те таблицы, которые существовали на момент открытия формы
	Для Каждого КлючИЗначение Из мИзмененияМенеджераВременныхТаблиц Цикл
...
Процедура МенюДереваЗапросаВернутьсяИзПерехода(Кнопка)
	...
	НоваяТекущаяСтрокаДереваЗапроса = ирОбщий.НайтиПоСтрокеПутиВДеревеЛкс(ДеревоЗапроса, "", ПутьВозврата);
	Если НоваяТекущаяСтрокаДереваЗапроса <> ДеревоЗапроса Тогда 
		ЭлементыФормы.ДеревоЗапроса.ТекущаяСтрока = НоваяТекущаяСтрокаДереваЗапроса;
	КонецЕсли;
