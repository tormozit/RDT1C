...
Процедура ЗагрузитьРезультатПакета(ОбновитьРазмерРезультата = Ложь)
	...
	Если мРежимДереваЗапроса И Не мРежимРедактированияУзла И ДеревоЗапроса.Строки[0].ТипУзла = "<Packet>" Тогда
		Для Каждого ЭлементРезультата Из ТаблицаРезультатаПакета Цикл
			...
			СтрокаДерева.РазмерРезультата = ЭлементРезультата.КоличествоСтрок;
			Если СтрокаДерева.ТипУзла = "<CreateRoot>" Тогда
				СтрокаДерева.РазмерРезультата = мРезультатПакета[ЭлементРезультата.Индекс].Выгрузить()[0][0];
...
Процедура СохранитьРезультатВТабличныйДокументСГруппировками()
	...
	Если ТипЗнч(мРезультатЗапроса) = Тип("РезультатЗапроса") Тогда
		...
		ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ТабДок,,,, Ложь);
	КонецЕсли;
...
Процедура КоманднаяПанельКоллекцияОткрытьТаблицу(Кнопка)
	...
	Форма = ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ЭлементыФормы.РезультатКоллекция.Значение,,,,,, ЭлементыФормы.РезультатКоллекция);
