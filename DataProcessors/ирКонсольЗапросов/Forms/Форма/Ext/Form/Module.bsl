...
Функция ОбновитьПараметрыКонтекстнойПодсказкиТекстаЗапроса(ПодключатьсяКМетаданным = Истина)
	...
	КомпонентаТекстЗапроса = ПолеТекстаПрограммы.ТекстЗапроса;
	#Если Сервер И Не Сервер Тогда
	    КомпонентаТекстЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
...
Процедура ЗагрузитьДанныеТекущейСтрокиДереваЗапросов()
	...
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ЭлементыФормы.ТекстЗапроса, СтрокаПоискаВДереве, Ложь);
		КонецЕсли; 
...
Функция РазобратьТекстЗапроса(ТекстЗапроса = Неопределено, СтроитьДеревоЗапроса = Истина, ПолеТекста = Неопределено, СтартоваяСтрокаДляОтображенияОшибок = 0)
	...
	Если ПолеТекста = Неопределено Тогда
		ПолеТекста = ЭлементыФормы.ТекстЗапроса;
	КонецЕсли; 
...
Процедура КоманднаяПанельСсылкаНаОбъектБД(Кнопка)
	...
	СтрокаПараметра = ЭкземплярКомпоненты.ВставитьСсылкуНаОбъектБД(ЭлементыФормы.Параметры, "ИмяПараметра", , "Параметры", Истина,,,
		ЭтаФорма.ТекущийЭлемент = ЭкземплярКомпоненты.ПолеТекста);
	Если СтрокаПараметра = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ОбновитьТипЗначенияВСтрокеПараметровВФорме(СтрокаПараметра);
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		...
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ЭлементыФормы.ТекстЗапроса, СтрокаПоискаВДереве, Ложь);
	КонецЕсли; 
