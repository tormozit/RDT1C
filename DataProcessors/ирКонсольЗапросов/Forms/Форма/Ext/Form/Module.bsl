...
Функция СохранитьВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, ОповеститьВладельца = Ложь, Знач ИмяФайла = Неопределено, 
	...
	Иначе
		...
		КонецЕсли; 
		ФайлВыбран = ирОбщий.СохранитьФайлВКонсолиСВосстановлениемЛкс(ДиалогВыбораФайла, ИмяФайла, мИмяФайла, КопияДерева, мСтруктураВосстановления, ЗапрашиватьИмяФайла);
...
Функция КоманднаяПанельТекстЗапросаЗакончитьРедактирование(НовыйРежимСворачивания = Неопределено)
	...
	Если Не мРежимРедактированияУзла И АвтосохранениеФайлаВосстановления Тогда
		СохранитьВФайл(,,, мСтруктураВосстановления.ФайлВосстановления.ПолноеИмя, Ложь);
	КонецЕсли; 
	Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
...
Процедура КоманднаяПанельЗапросаРезультатаСтатистикаMSSQLПоПоследнимЗапросам(Кнопка)
	...
	НастройкиОтчета = ФормаОтчета.КомпоновщикНастроек.Настройки;
		...
		#КонецЕсли
	////УсловноеОформление = ФормаОтчета.КомпоновщикНастроек.Настройки.УсловноеОформление;
	////#Если Сервер И Не Сервер Тогда
	////    УсловноеОформление = Компоновщик.Настройки.УсловноеОформление;
	////#КонецЕсли
	////ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	////ирОбщий.НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ЭлементОформления.Поля, "last_execution_time");
	////ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона", WebЦвета.СветлоКоралловый);
	////ПараметрОтбор = ЭлементОформления.Отбор;
	//ГруппаОтбора = НастройкиОтчета.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	//ГруппаОтбора.Представление = "Время последнего выполнения в диапазоне";
	////ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(ГруппаОтбора, "last_execution_time", ДатаНачалаВыполнения, ВидСравненияКомпоновкиДанных.БольшеИлиРавно,, Ложь);
	////ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(ГруппаОтбора, "last_execution_time", ДатаКонцаВыполнения, ВидСравненияКомпоновкиДанных.МеньшеИлиРавно,, Ложь);
