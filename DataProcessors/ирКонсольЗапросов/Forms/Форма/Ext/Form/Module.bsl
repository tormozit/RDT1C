...
Функция ЗагрузитьЗапросыИзОбъекта(Знач ПолученноеЗначение, Знач РежимОбъединения = Ложь) Экспорт 
...
Процедура ПриОткрытии()
	...
	Если МодальныйРежим Тогда 
		// Антибаг платформы http://www.hostedredmine.com/issues/883933
...
Процедура ПоместитьТаблицуЗначенийВоВременнуюТаблицуИнтерактивно(Знач ТаблицаЗначений, Знач ИзПараметра = Ложь, Знач ИмяТаблицы = "")
	...
	Если Не ИзПараметра И ФормаПараметровПомещения.ОтредактироватьДанныеТаблицы Тогда
		Если ирОбщий.ОткрытьЗначениеЛкс(ТаблицаЗначений, Истина,,,,, ЭлементыФормы.РезультатКоллекция) <> Истина Тогда
			Возврат;
		КонецЕсли; 
	Иначе
