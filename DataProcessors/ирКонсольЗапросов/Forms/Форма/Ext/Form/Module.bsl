...
Функция ПолучитьОбъектЗапроса() 
	...
	ИначеЕсли ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "WQL") Тогда
		ПараметрОбъектЗапроса = ирКэш.ПолучитьCOMОбъектWMIЛкс();
	Иначе
...
//
Процедура ВыполнитьЗапрос(Кнопка = Неопределено, Знач пЗагружатьЭлементов = Неопределено, РежимВременойТаблицы = Неопределено)
	...
	ЗагрузитьРезультатЗапроса(пЗагружатьЭлементов, РежимВременойТаблицы);
