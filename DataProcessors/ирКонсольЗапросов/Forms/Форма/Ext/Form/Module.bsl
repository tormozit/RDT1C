...
Процедура КлсПолеТекстовогоДокументаСКонтекстнойПодсказкойНажатие(Кнопка)
	...
	Если Кнопка = ирОбщий.ПолучитьКнопкуКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		...
		Если СтрДлина(КомпонентаТекстаЗапроса.ПолеТекстовогоДокумента.ВыделенныйТекст) = 0 Тогда
			...
			ПроверитьКорректностьТекстаЗапроса(,,,,, ПараметрыИЗТекстаЗапроса); 
			...
			Для Каждого СтрокаПараметра Из Параметры Цикл
				Если ПараметрыИЗТекстаЗапроса.Найти(СтрокаПараметра.НИмяПараметра, "Имя") = Неопределено Тогда
					Продолжить;
				КонецЕсли; 
				СтрокаТекстаЗапроса = СтрокаТекстаЗапроса + КодУстановкиПараметра(СтрокаПараметра, "Запрос") + Символы.ПС;
			КонецЦикла;
		КонецЕсли; 
	    ирОбщий.ПоместитьТекстВБуферОбменаОСЛкс(СтрокаТекстаЗапроса);
		Возврат;
	КонецЕсли;
...
Функция ПроверитьКорректностьТекстаЗапроса(ЛиДобавлятьНовые = Истина, ОбновитьНастройки = Истина, СтартоваяСтрока = 0, Знач ТекстЗапроса = "", ДобавитьВыборкуИзВременнойТаблицы = Истина,
	выхПараметрыИЗТекстаЗапроса = Неопределено)

	Если Не ЗначениеЗаполнено(ТекстЗапроса) Тогда
		ТекстЗапроса = ПолучитьТекстЗапроса(Истина);
	КонецЕсли; 
	выхПараметрыИЗТекстаЗапроса = ПолучитьПараметрыИзТекстаЗапроса(СтартоваяСтрока, ТекстЗапроса);
	Если Ложь
		Или ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "ADO")
		Или ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "WQL")
	Тогда
	...
	Иначе
		РезультатПроверки = выхПараметрыИЗТекстаЗапроса <> Неопределено;
	КонецЕсли; 
