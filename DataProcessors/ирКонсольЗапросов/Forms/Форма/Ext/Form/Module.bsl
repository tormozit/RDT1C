	...
	Если Открыта() Тогда
		...
		Если ВременныеТаблицы.Количество() > 0 Тогда
			...
			Ответ = Вопрос("Менеджер временных таблиц консоли удерживает соединение с СУБД. 
				|Пересоздать его для переустановки соединения для избежания снижения производительности сервера СУБД?",
...
Процедура ПересоздатьМенеджерВременныхТаблиц(ТолькоУдалить = Ложь) Экспорт 
	...
	Иначе
		...
		Если ТолькоУдалить Тогда
			...
			Возврат;
		КонецЕсли; 
		ФрагментыВыборки = Новый Массив;
		Для Каждого ОписаниеТаблицы Из ВременныеТаблицы Цикл
			ФрагментыВыборки.Добавить("SELECT * FROM " + ОписаниеТаблицы.Имя);
		КонецЦикла;
		мОбъектЗапроса.Текст = ирОбщий.СтрСоединитьЛкс(ФрагментыВыборки, ";" + Символы.ПС);
		РезультатПакета = мОбъектЗапроса.ВыполнитьПакет();
		ФрагментыСоздания = Новый Массив;
		ИндексРезультата = 0;
		Для Каждого ОписаниеТаблицы Из ВременныеТаблицы Цикл
			ФрагментыСоздания.Добавить("SELECT Т.* INTO " + ОписаниеТаблицы.Имя + " FROM &" + ОписаниеТаблицы.Имя + " КАК Т");
			мОбъектЗапроса.Параметры.Вставить(ОписаниеТаблицы.Имя, РезультатПакета[ИндексРезультата]);
			ИндексРезультата = ИндексРезультата + 1;
		КонецЦикла;
		мОбъектЗапроса.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		мОбъектЗапроса.Текст = ирОбщий.СтрСоединитьЛкс(ФрагментыСоздания, ";" + Символы.ПС);
