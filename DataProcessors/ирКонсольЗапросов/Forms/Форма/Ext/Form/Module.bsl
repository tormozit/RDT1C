...
Перем мРежимРедактированияУзла;
	...
	Если Кнопка = ирОбщий.ПолучитьКнопкуКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "ПерейтиКОпределению") Тогда
		...
		Если мРежимДереваЗапроса Тогда
			...
			Если СтрокаЗапросаСоздания.Количество() > 0 Тогда
				Если ЗакончитьРедактированиеУзла() Тогда
...
Процедура УстановитьЗаголовокФормы()
	...
	Если мИмяФайла <> "" Тогда
		НовыйЗаголовок = мЗаголовокФормы;
		Если ИспользоватьАвтосохранение Тогда
			НовыйЗаголовок = НовыйЗаголовок + ", Автосохранение";
		КонецЕсли; 
		НовыйЗаголовок = НовыйЗаголовок + ": " + мИмяФайла;
	Иначе
		НовыйЗаголовок = мЗаголовокФормы;
	КонецЕсли;
...
Функция СохранитьТекущийПодзапрос()
	...
	Если мРежимРедактированияУзла Тогда
...
Процедура ЗагрузитьЗапросыИзФайла(ОчиститьПередЗагрузкой = Истина)
	...
	ИначеЕсли ТипЗнч(ПолученноеЗначение) = Тип("ДеревоЗначений") Тогда
		...
		Если ОчиститьПередЗагрузкой Тогда
			...
			УстановитьЗаголовокФормы();
		КонецЕсли;
		ирОбщий.СкопироватьДеревоЛкс(ПолученноеЗначение, ДеревоЗапросов, ОчиститьПередЗагрузкой);
...
Функция ЗакончитьРедактированиеУзла(ЗапрашиватьСохранение = Истина)
			...
			Иначе
				Если Истина
					И (Ложь
						...
						Или ЭлементыФормы.ТаблицаРезультатаПакета.ТекущаяСтрока.Индекс = ТаблицаРезультатаПакета.Количество() - 1)
					И (Ложь
						...
						ИначеЕсли ирОбщий.СтрокиРавныЛкс(мОписаниеРезультата.ТипЗапроса, "ADO") Тогда
							...
							Если мРезультатЗапроса.State <> 0 Тогда
								...
								Если ЭлементыФормы.РезультатКоллекция.Значение.Колонки.Количество() = 0 Тогда
									ЭлементыФормы.РезультатКоллекция.Значение = ирОбщий.ПреобразоватьРезультатADOВТаблицуЗначенийЛкс(мРезультатЗапроса, Типизировать1С, БинарныеВСтроку,,,
										ЗагружатьЭлементов, СмещениеГодаADO);
								КонецЕсли; 
							КонецЕсли; 
		...
		Если РежимВременнойТаблицы <> Истина Тогда
			Для Каждого ИмяСоздаваемойВременнойТаблицы Из МассивСоздаваемыхВременныхТаблиц Цикл
				Если ИмяСоздаваемойВременнойТаблицы <> Неопределено Тогда
					мВременныеТаблицыМенеджера1С.Вставить(ИмяСоздаваемойВременнойТаблицы);
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ЗагрузитьРезультатПакета(ОбновитьРазмерРезультата = Ложь)
	...
	Если мРежимДереваЗапроса И Не мРежимРедактированияУзла И ДеревоЗапроса.Строки[0].ТипУзла = "<Packet>" Тогда
...
Процедура ОбновитьКоличествоСтрокРезультата(Знач РезультатЗапроса, Знач СтрокаДерева = Неопределено, ПолучатьСозданнуюВременнуюТаблицу = Ложь)
	...
	КонецЕсли; 
	Если СтрокаДерева.ТипУзла = "<CreateRoot>" Тогда
		Если Не ПолучатьСозданнуюВременнуюТаблицу Тогда
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстовогоДокументаЛкс(ЭтаФорма, ЭлементыФормы.ТекстЗапроса, СтрокаПоискаВДереве);
		КонецЕсли; 
		...
		Если ИсходнаяТаблицаПараметров <> Неопределено Тогда
			Для каждого СтрокаИсходнойТаблицы из ИсходнаяТаблицаПараметров Цикл
				...
				ОбновитьТипЗначенияВСтрокеПараметров(НоваяСтрока);
			КонецЦикла;
		КонецЕсли;
...
Процедура ПриОткрытии()
	...
	ЭтаФорма.ПоказыватьРезультатПакета = ВосстановитьЗначение("ирКонсольЗапросов_ПоказыватьРезультатПакета") = Истина;
	...
	КонецЕсли; 
	Если мРежимРедактораЗапроса Тогда
		мЗаголовокФормы = мЗаголовокФормы + " : " + " Редактирование";
		Если ВладелецФормы <> Неопределено Тогда
			ВладелецФормы.Панель.Доступность = Ложь;
		КонецЕсли; 
	ИначеЕсли мКоллекцияДляЗаполнения <> Неопределено Тогда
		мЗаголовокФормы = мЗаголовокФормы + " : " + " Заполнение коллекции";
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если СохранитьВФайл(Истина) Тогда
		...
		СохранитьЗначение("ирКонсольЗапросов_ПоказыватьРезультатПакета", ПоказыватьРезультатПакета);
	Иначе
		СтандартнаяОбработка = Ложь;
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПараметрыЭтоВыражениеПриИзменении(Элемент)
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	ОбновитьТипЗначенияВСтрокеПараметров(ТекущиеДанные);
	УправлениеКолонкамиПараметры();
	ВычислитьПараметрыЗапроса();
...
Процедура ЗаблокироватьРедактированиеТекстаЗапроса(Флаг)
	...
	УстановитьДоступностьКнопокКоманднойПанели(ЭлементыФормы.КоманднаяПанельТекстЗапроса.Кнопки, Не Флаг, ОбъектКомпоненты);
	ЭлементыФормы.КоманднаяПанельТекстЗапроса.Кнопки.ОтменитьРедактирование.Доступность = Не Флаг;
	ЭлементыФормы.КоманднаяПанельТекстЗапроса.Кнопки.НайтиВДеревеЗапроса.Доступность = Флаг;
	Если Флаг Тогда
		УстановитьКнопкуИзменитьПодзапрос();
	КонецЕсли; 
	ЭлементыФормы.ДеревоЗапросов.ТолькоПросмотр = Не Флаг Или мРежимРедактораЗапроса;
	...
	КонецЕсли;
	мРежимРедактированияУзла = Не Флаг;
...
Функция КоманднаяПанельТекстЗапросаЗакончитьРедактирование(НовыйРежимСворачивания = Неопределено)
	...
	Если мРежимРедактированияУзла Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ПутьВДеревеЗапроса = ирОбщий.ПолучитьСтрокуПутиВДеревеЛкс(ТекущийПодзапрос, "");
	...
	Если Истина
		...
		И ТекущийПодзапрос.ТипУзла <> "<TableWithName>"
	Тогда
...
Процедура КоманднаяПанельТекстЗапросаИзменитьИлиСохранитьУзелДерева(Кнопка = Неопределено, ПринудительноИзменить = Неопределено)
	...
	Тогда
		Если КоманднаяПанельТекстЗапросаЗакончитьРедактирование() Тогда
			УстановитьКнопкуИзменитьПодзапрос();
		КонецЕсли; 
	Иначе
...
Функция ПолучитьТекстМеждуТокенами(Знач НачальныйТокен, Знач КонечныйТокен, НачальнаяСтрока = Неопределено, НачальнаяКолонка = Неопределено, КонечнаяСтрока = Неопределено,
	...
	Если НачальныйТокен <> Неопределено И КонечныйТокен <> Неопределено Тогда
		...
		КоличествоСтрокКонечногоТокена = СтрЧислоСтрок(ТекстКонечногоТокена);
		...
		Иначе
			КонечнаяКолонка = СтрДлина(СтрПолучитьСтроку(ТекстКонечногоТокена, КоличествоСтрокКонечногоТокена)) + 1;
		КонецЕсли;
...
Функция ОбработатьСтрокуДереваЗапроса(СтрокаДерева, НачальнаяСтрока = 1, НачальнаяКолонка = 1)
	...
	Если СтрокаДерева.ТипУзла = "<TableWithName>" Тогда
		...
		ИмяВременнойТаблицы = НРег(СтрокаДерева.Текст);
		Если мВременныеТаблицыПакета[ИмяВременнойТаблицы] = Неопределено Тогда
			мВременныеТаблицыПакета[ИмяВременнойТаблицы] = 1;
		Иначе
			мВременныеТаблицыПакета[ИмяВременнойТаблицы] = мВременныеТаблицыПакета[ИмяВременнойТаблицы] + 1;
		КонецЕсли; 
		Если Истина
			И Не ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "ADO")
			И Не ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "WQL")
			И ирОбщий.ЛиИмяПеременнойЛкс(ИмяВременнойТаблицы)
			И мВременныеТаблицыМенеджера1С.Свойство(ИмяВременнойТаблицы)
		Тогда
...
Процедура ОбновитьСтатусВременойТаблицы(Знач ИмяВременнойТаблицы)
	...
	Иначе
		Если мВременныеТаблицыМенеджера1С.Свойство(ИмяВременнойТаблицы) Тогда
			мВременныеТаблицыМенеджера1С.Удалить(ИмяВременнойТаблицы);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ПараметрыЗначениеПараметраОчистка(Элемент, СтандартнаяОбработка)
	...
	Иначе
		Если Не ТекущаяСтрока.НеИспользоватьОграничениеТипа Тогда
			...
			ОбновитьТипЗначенияВСтрокеПараметров(ЭлементыФормы.Параметры.ТекущиеДанные);
		КонецЕсли; 
	КонецЕсли;
...
Функция ПолучитьМассивСоздаваемыхВременныхТаблицПакета(ТекстЗапроса, ТолькоТребующиеУничтоженияНаВходе = Ложь)
...
Функция ПолучитьРезультирующийЗапрос(пЗапрос, Знач ТекстЗапроса = "", ИспользоватьНастройкиРасширений = Ложь)
	...
	Если Не ЗначениеЗаполнено(ТекстЗапроса) Тогда
		ТекстЗапроса = мОбъектЗапроса.Текст;
	КонецЕсли; 
	МассивСоздаваемыхВременныхТаблиц = ПолучитьМассивСоздаваемыхВременныхТаблицПакета(ТекстЗапроса, Истина);
...
Процедура МенюДереваЗапросаПеренестиВНовыйЗапрос(Кнопка)
	...
	Если мВременныеТаблицыПакета[Нрег(ИмяВременнойТаблицы)] <> Неопределено Тогда
		...
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			СтрокиДереваЗапроса = НайтиСтрокуДереваСозданияВременнойТаблицы(ИмяВременнойТаблицы);
			Если СтрокиДереваЗапроса.Количество() > 0 Тогда
				ирОбщий.СравнитьЗначенияИнтерактивноЧерезXMLСтрокуЛкс(мТекущаяСтрокаДереваЗапроса.Текст, СтрокиДереваЗапроса[0].Текст,, "Новый", "Существующий");
			КонецЕсли; 
		КонецЕсли;
		Если Ответ <> КодВозвратаДиалога.Да Тогда
			Возврат;
...
Функция ПолучитьКодПолученияРезультатаЗапроса()
	...
	Иначе
		...
		Для Каждого СтрокаПараметра Из мОписаниеРезультата.ПараметрыЗапроса Цикл
			...
			КонецЕсли; 
			ТекстМодуля = ТекстМодуля + "
			|мОбъектЗапроса.Параметры.Вставить(""" + СтрокаПараметра.ИмяПараметра + """, " + ТекстЗначения + "); " + КомментарийЗначения;
		КонецЦикла;
	КонецЕсли; 
...
Процедура КоманднаяПанельПараметрыДобавитьСтандартныйПериод(Кнопка)
	...
	ОбновитьТипЗначенияВСтрокеПараметров(СтрокаПериода);
...
Процедура ОбновитьТипЗначенияВСтрокеПараметров(СтрокаПараметра = Неопределено)
...
Функция ЗаполнитьВыборкиИтоговИзЗапроса()
	...
	Если RegExp.Test(ТекстЗапроса) Тогда // грязно
		...
		Если Найти(ТекстБезКомментариев, "ОБЩИЕ") Тогда // Грязно
...
Процедура ПоместитьДанныеСтрокиЗапросаВоВременнуюТаблицу(СтрокаЗапроса = Неопределено)
	...
	Исключение
		...
		Возврат;
	КонецПопытки; 
	мВременныеТаблицыМенеджера1С.Вставить(ИмяТаблицы);
	ОбновитьТабличноеПолеВременныхТаблиц();
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		...
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстовогоДокументаЛкс(ЭтаФорма, ЭлементыФормы.ТекстЗапроса, СтрокаПоискаВДереве);
	КонецЕсли; 
...
Процедура КоманднаяПанельПараметрыЗаменитьЛитераломЗНАЧЕНИЕ(Кнопка)
	...
	Если ТекстЗамены = "" Тогда
		Сообщить("Литеральное представление для значения парамета &" + ТекущиеДанные.ИмяПараметра + " невозможно")
	Иначе 
