...
Процедура ДеревоЗапросаПриАктивизацииСтроки(Элемент = Неопределено)
	...
	СтрокаЗапросаСоздания = НайтиСтрокуДереваСозданияВременнойТаблицы(ТекущаяСтрока.Имя);
...
Функция КоманднаяПанельТекстЗапросаЗакончитьРедактирование(НовыйРежимСворачивания = Неопределено)
	...
	Если мРежимРедактирования Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ПутьВДеревеЗапроса = ирОбщий.ПолучитьСтрокуПутиВДеревеЛкс(ТекущийПодзапрос, "Имя", ДеревоЗапроса.Строки[0].ТипУзла = "<Packet>");
...
Процедура МенюДереваЗапросаПерейтиКСозданию(Кнопка)
	...
	СтрокаЗапросаСоздания = НайтиСтрокуДереваСозданияВременнойТаблицы(ЭлементыФормы.ДеревоЗапроса.ТекущаяСтрока.Имя);
	Если СтрокаЗапросаСоздания <> Неопределено Тогда
...
Процедура УстановитьИсточникДанныхADO(ПараметрыADO)
	...
	Если Не ЭлементыФормы.АутентификацияОС.Доступность Тогда
...
Процедура МенюДереваЗапросаНайтиРезультат(Кнопка)
	...
	Если ИндексЗапроса >= 0 Тогда
		Возврат;
	КонецЕсли; 
	Если ТаблицаРезультатаПакета.Количество() > ИндексЗапроса Тогда
