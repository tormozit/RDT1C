...
Функция ВыполнитьЛокальноКодОбработкиРезультата(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Если НаСервере Тогда
		ирСервер.ВыполнитьАлгоритм(ТекстДляВыполнения, , , ПолучитьСтруктуруПараметров(), , мРезультатЗапроса, мРезультатПакета, мРезультатПоИменам);
	Иначе
	...
	Исключение
		...
		#Если Клиент Тогда
			Если Отладка = Истина Тогда
				ирОбщий.СообщитьЛкс("Попытка подключения ВК 'GameWithFire.ADOUtils' ...", СтатусСообщения.Информация);
			КонецЕсли; 
			Попытка
				ПодключитьВнешнююКомпоненту("GameWithFire.ADOUtils");
			Исключение
				Если Отладка = Истина Тогда
					ирОбщий.СообщитьЛкс("	ошибка подключения ВК: "+ОписаниеОшибки(), СтатусСообщения.Важное);
				КонецЕсли; 
				Возврат Неопределено;
			КонецПопытки;
			Если Отладка = Истина Тогда
				ирОбщий.СообщитьЛкс("	ВК успешно подключена.",СтатусСообщения.БезСтатуса);
			КонецЕсли; 
			Попытка
				ADOUtils = Новый("AddIn.ADOUtils");
			Исключение
				Если Отладка = Истина Тогда
					ирОбщий.СообщитьЛкс("	ошибка создания объекта 'AddIn.ADOUtils' из ВК: "+ОписаниеОшибки(), СтатусСообщения.Важное);
				КонецЕсли; 
			КонецПопытки;
		#Иначе
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаЗаписьНаСервере(Кнопка)
	...
	ЭлементыФормы.КодОбработкиСтрокиРезультата.ВыделенныйТекст = Символы.ПС;
...
Процедура КоманднаяПанельОбработкиРезультатаЗаписьНаСервере(Кнопка)
	...
	// Антибаг 8.3.12 https://partners.v8.1c.ru/forum/t/1719342/m/1719342, http://www.hostedredmine.com/issues/882423
	ЭлементыФормы.КодОбработкиРезультата.ВыделенныйТекст = Символы.ПС;
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаСообщитьСразу(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.СообщитьЛкс();
	#КонецЕсли
	ЭлементыФормы.КодОбработкиСтрокиРезультата.ВыделенныйТекст = "ирОбщий.СообщитьЛкс();";
	// Антибаг 8.3.12 https://partners.v8.1c.ru/forum/t/1719342/m/1719342, http://www.hostedredmine.com/issues/882423
	ЭлементыФормы.КодОбработкиСтрокиРезультата.ВыделенныйТекст = Символы.ПС;
...
Процедура КоманднаяПанельОбработкиРезультатаСообщитьСразу(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.СообщитьЛкс();
	#КонецЕсли
	ЭлементыФормы.КодОбработкиРезультата.ВыделенныйТекст = "ирОбщий.СообщитьЛкс();";
	// Антибаг 8.3.12 https://partners.v8.1c.ru/forum/t/1719342/m/1719342, http://www.hostedredmine.com/issues/882423
	ЭлементыФормы.КодОбработкиРезультата.ВыделенныйТекст = Символы.ПС;
