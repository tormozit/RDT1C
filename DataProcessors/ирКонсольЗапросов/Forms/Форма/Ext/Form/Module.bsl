...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		...
		Если СтрДлина(КомпонентаТекстаЗапроса.ПолеТекста.ВыделенныйТекст()) = 0 Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
	    ирОбщий.ТекстВБуферОбменаОСЛкс(СтрокаТекстаЗапроса, "ВстроенныйЯзык");
		Возврат;
	КонецЕсли;
...
Функция ПерейтиКОпределению(Знач НомерСтроки = 0, Знач НомерКолонки = 0) Экспорт 
	КомпонентаТекстаЗапроса = ПолеТекстаПрограммы.ТекстЗапроса;
	#Если Сервер И Не Сервер Тогда
	    КомпонентаТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	...
	Если Лев(ТекущееВыражение, 1) = "&" Тогда
		УспехВычисления = Ложь;
		ЗначениеПараметра = ВычислитьЗначениеПараметра(ТекущееВыражение, УспехВычисления);
...
Функция ВычислитьЗначениеПараметра(Знач ТекущееВыражение, УспехВычисления = Истина)
...
Процедура УстановитьТекстПоляТекстаЗапроса(Текст)
	...
	ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(Текст);
...
Функция СохранитьВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, Знач ИмяФайла = Неопределено, СброситьМодифицированность = Неопределено, Знач ЭтоФайлВосстановления = Ложь)
	...
	Тогда
		...
		//Исключение
		//	ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаЗапроса.ПолеТекста,,,, МодальныйРежим, ИнформацияОбОшибке());
		//	Возврат;
		//КонецПопытки;
...
Процедура ЗапроситьОчисткуМенеджераВременныхТаблиц() Экспорт 
	...
	Если Ответ = КодВозвратаДиалога.ОК Или Ответ = КодВозвратаДиалога.Таймаут Тогда
		ПересоздатьМенеджерВременныхТаблиц(Не Открыта());
	КонецЕсли;
...
Функция ВыполнитьЗапрос(Кнопка = Неопределено, Знач пЗагружатьЭлементов = Неопределено, РежимВременнойТаблицы = Неопределено, СохранитьДляОтложеннойОтладки = Неопределено,
	...
	Иначе
		...
		Исключение
			...
			ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаЗапроса.ПолеТекста, , , Истина, МодальныйРежим, ИнформацияОбОшибке(),,, ЭтаФорма);
			Возврат Ложь;
		КонецПопытки;
...
Процедура ЗагрузитьДанныеТекущейСтрокиДереваЗапросов()
	...
	ЭлементыФормы.КоманднаяПанельФормы.Кнопки.УдалитьВременныеТаблицы.Доступность = мЗапросЗагруженВКонсоль;
...
Процедура ПриОткрытии()
	...
	Если мРежимОтладки Тогда
		...
		Если Истина
			...
			И СтрДлина(ПолеТекстаЗапроса.ПолеТекста.ПолучитьТекст()) < 20000 
		Тогда
			КоманднаяПанельФормыРежимДереваЗапроса();
		КонецЕсли; 
...
Функция ВыполнитьЗапросИсточникаДанных(ТекстЗапроса, пЗагружатьЭлементов = 0, ВыводитьСообщения = Истина, СохранитьДляОтложеннойОтладки = Ложь, выхРезультатПакета = Неопределено,
	...
	Если выхРезультатПакета <> Неопределено Тогда
		выхРезультатПоИменам = мПлатформа.ПолучитьСтруктуруРезультатаПакетногоЗапроса(ТекстЗапроса, ПолеТекстаПрограммы.ТекстЗапроса.мПараметрыДиалектаSQL.СтрочныйКомментарий);
	КонецЕсли; 
	Возврат Результат;
...
Функция ПолучитьПараметрыИзТекстаЗапроса(Знач СтартоваяСтрока, ТекстЗапроса = "", ПривестиИменаКНижнемуРегистру = Ложь, выхИнформацияОбОшибке = Неопределено, СообщитьОшибку = Истина)
	...
	Если ирОбщий.СтрокиРавныЛкс(мТекущаяСтрокаДереваЗапросов.ТипЗапроса, "ADO") Тогда 
		ПрефиксПараметра = ПолеТекстаПрограммы.ТекстЗапроса.мПараметрыДиалектаSQL.ПрефиксПараметра;
	...
	Иначе
		...
		Исключение
			...
			Если СообщитьОшибку Тогда
				ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаЗапроса.ПолеТекста, СтартоваяСтрока, , Истина, МодальныйРежим, выхИнформацияОбОшибке,,, ЭтаФорма);
			КонецЕсли; 
		КонецПопытки;
...
Процедура ДеревоЗапросаПриАктивизацииСтроки(Элемент = Неопределено)
	...
	Если мРедактируемыйУзел = мТекущаяСтрокаДереваЗапроса Тогда
		ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(мРедактируемыйТекстУзла);
	КонецЕсли; 
	ПолеТекстаЗапроса.ПолеТекста.ТолькоПросмотр(мРедактируемыйУзел = Неопределено Или мРедактируемыйУзел <> мТекущаяСтрокаДереваЗапроса);
...
Процедура ЗаблокироватьРедактированиеТекстаЗапроса(ТолькоПросмотрДерева)
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ПолеТекстаЗапроса.ПолеТекста.ТолькоПросмотр(ТолькоПросмотрДерева);
...
Процедура КоманднаяПанельФормыРежимДереваЗапроса(Кнопка = Неопределено) Экспорт 
	...
	Если мТекущаяСтрокаДереваЗапросов.ТипЗапроса <> "Папка" Тогда
		Если НовыйРежимДереваЗапроса Тогда 
			...
			ЗаблокироватьРедактированиеТекстаЗапроса(Истина);
		Иначе
			ПолеТекстаЗапроса.ПолеТекста.ТолькоПросмотр(Ложь);
...
Функция ЛиПарсерЗапросаДоступен(ПринудительноВывестиСообщение = Ложь)
	...
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	СообщениеОпциональное = "Поэтому будет использовано упрощенное дерево запроса, недоступны команды ""Конструктор ИР"" и ""Расшифровать строку результата"".";
	Если ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		Если ПринудительноВывестиСообщение Тогда
			СообщениеЛокальное = СообщениеОпциональное;
			СообщениеОпциональное = Неопределено;
		КонецЕсли; 
		Парсер = мПлатформа.мПолучитьПарсер("ГрамматикаЯзыкаЗапросов",, СообщениеОпциональное);
		Если СообщениеЛокальное <> Неопределено Тогда
			ирОбщий.СообщитьЛкс(СообщениеЛокальное);
		КонецЕсли; 
...
Функция ПолучитьПостроительЗапросовДляИтогов(СообщатьОбОшибке = Ложь, ТекстЗапроса = "")
	...
	Исключение
		...
		Если СообщатьОбОшибке Тогда
		    ...
		    ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаЗапроса.ПолеТекста, , , Истина, МодальныйРежим,
		        ИнформацияОбОшибке);
		КонецЕсли; 
		Возврат Неопределено;
	КонецПопытки; 
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаПолучитьТекст(Кнопка)
    ...
    ирОбщий.ТекстВБуферОбменаОСЛкс(ТекстМодуля, "ВстроенныйЯзык");
...
Процедура КП_ТаблицаПакетаНайтиЗапросВДереве(Кнопка = Неопределено)
	...
	Если мРежимДереваЗапроса Тогда
		...
		КонецЕсли; 
	Иначе
		//ЗагрузитьВременнуюТаблицуРезультатаПакета(Истина); // Это итак выполняется при активации строки пакета
		ирОбщий.СообщитьЛкс("Необходимо включить дерево запроса");
	КонецЕсли; 
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		...
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаЗапроса.ПолеТекста, СтрокаПоискаВДереве, Ложь);
	КонецЕсли; 
...
Процедура РедакторHTMLДокументСформирован(Элемент)
	...
	ПанельРедактораПриСменеСтраницы(ЭлементыФормы.ПанельРедактора, ЭлементыФормы.ПанельРедактора.ТекущаяСтраница);
КонецПроцедуры
...
Процедура ПанельРедактораПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ЭлементыФормы.ПанельРедактора.ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML Тогда
		Если РедакторHTML() <> Неопределено Тогда
			ПолеТекстаЗапроса.ПолеТекста = АктивноеПолеТекста(Поле2);
			Поле1.ПолучитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка);
			Поле2.УстановитьТекст(Поле1.ПолучитьТекст(), Истина);
			Поле2.УстановитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка);
		КонецЕсли;
	Иначе
		Если ЭлементыФормы.РедакторHTML.Документ.defaultView <> Неопределено Тогда
			ПолеТекстаЗапроса.ПолеТекста = АктивноеПолеТекста(Поле1);
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_ON_LINK_CLICK" Тогда
			...
			ИначеЕсли Событие.params.label = "Знач" Тогда
				ирОбщий.ОткрытьЗначениеЛкс(ВычислитьЗначениеПараметра(Событие.params.href));
			ИначеЕсли Ложь
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_HOVER" Тогда
			...
			Если Лев(ОбъектноеВыражение, 1) = "&" Тогда
				УспехЗначения = Ложь;
				ЗначениеВыражения = ВычислитьЗначениеПараметра(ОбъектноеВыражение, УспехЗначения);
				...
				КонецЕсли; 
			КонецЕсли; 
			ПредставлениеДопустимыхТипов = ПолеТекстаЗапроса.ДобавитьПодсказкуПоДопустимымТипам(ИмяТипаЗначения, ПодсказкаМассив, Ложь);
			ПолеТекстаЗапроса.КончитьОбработкуКоманды();
			ОбновитьПодсказкуHTML(Событие.params.word.word, ПодсказкаМассив);
		ИначеЕсли Событие.event = "EVENT_BEFORE_SIGNATURE" Тогда
			...
			#Если ТолстыйКлиентУправляемоеПриложение Тогда
				ПолеТекстаЗапроса.КончитьОбработкуКоманды();
				ФормаВызовМетода = ПолеТекстаЗапроса.ФормаВызовМетода();
				ФормаВызовМетода.ПараметрСтруктураТипаКонтекста = Неопределено;
				ФормаВызовМетода.ОбновитьИлиЗакрытьФорму(, Истина);
				ПередатьОписаниеМетодаВРедакторHTML();
				Событие.params.activeParameter = ПолеТекстаЗапроса.мНомерПараметра - 1;
			#Иначе
				РедакторHTML.hideSignatureList();
			#КонецЕсли
		ИначеЕсли Событие.event = "EVENT_BEFORE_SHOW_SUGGEST" Тогда
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				РедакторHTML.hideSuggestionsList();
				Последние2Символа = Прав(Событие.params.last_expression, 2);
				Если Ложь
					Или Событие.params.trigger = Неопределено // Вызов явный
					Или Событие.params.trigger = "." И Последние2Символа <> ".."
					//Или Событие.params.trigger = """"
					Или ирОбщий.ЛиИмяПеременнойЛкс(Событие.params.trigger) // Иначе слишком часто вызывается затратное обновление списка слов локального контекста
					//Или ирОбщий.СтрокиРавныЛкс(Событие.params.last_word, "новый")
					//Или ирОбщий.СтрокиРавныЛкс(Событие.params.last_word, "new")
					Или Событие.params.trigger = "&" И Последние2Символа <> "&&"
				Тогда 
					#Если Сервер И Не Сервер Тогда
						ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
					...
					КонецЕсли; 
					ПолеТекстаЗапроса.РазобратьТекущийКонтекст();
					//Если Событие.params.trigger = """" Тогда
					//	ПоказатьСписок = ПолеТекстаЗапроса.мЭтоСтроковыйЛитерал;
					//Иначе
						ПоказатьСписок = Истина;
					//КонецЕсли; 
					Если ПоказатьСписок Тогда
						ОбновитьКонтекстПодсказкиИПолучитьСтруктуруПараметров(ПолеТекстаЗапроса);
						ПолеТекстаЗапроса.ЗаполнитьТаблицуСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста, Событие.params.trigger = Неопределено, Ложь);
						Если Ложь
							Или Событие.params.trigger = Неопределено
							Или ПолеТекстаЗапроса.ТаблицаСлов.Количество() > 0
						Тогда
							ПередатьТаблицуСловВРедакторHTML();
						КонецЕсли; 
					КонецЕсли; 
				КонецЕсли; 
			КонецЕсли; 
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			...
			ПолеТекстаЗапроса.КончитьОбработкуКоманды();
			ПолеТекстаЗапроса.РазобратьТекущийКонтекст();
			СтрокаТаблицыСлов = ПолеТекстаЗапроса.ТаблицаСлов.Найти(ПолеТекстаЗапроса.ЧистоеСловоСпискаПодсказкиHTML(Событие.params.selected), "Слово");
			Если СтрокаТаблицыСлов = Неопределено Тогда
				// баг редактора https://github.com/salexdv/bsl_console/issues/78#issuecomment-836447865
				Возврат;
			КонецЕсли; 
			ПолеТекстаЗапроса.ВставитьВыбранноеСловоАвтодополнения(СтрокаТаблицыСлов, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста);
			ПолеТекстаЗапроса.УстановитьГраницыВыделения();
			Если ПолеТекстаЗапроса.мОткрытьСправкуПоПараметру = Истина Тогда
				#Если ТолстыйКлиентОбычноеПриложение Тогда
					ПолеТекстаЗапроса.ОткрытьСправкуПоПараметру();
				#КонецЕсли
			КонецЕсли; 
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			// https://github.com/salexdv/bsl_console/issues/78
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				СтрокаТаблицыСлов = ПолеТекстаЗапроса.ТаблицаСлов.Найти(ПолеТекстаЗапроса.ЧистоеСловоСпискаПодсказкиHTML(Событие.params.focused), "Слово");
				Если СтрокаТаблицыСлов <> Неопределено Тогда
					ПолеТекстаЗапроса.УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, СтрокаТаблицыСлов);
					РедакторHTML.setActiveSuggestDetail(СтрокаТаблицыСлов.ТипЗначения);
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередатьОписаниеМетодаВРедакторHTML()
	...
	Для Каждого ЭлементСписка Из ФормаВызовМетода.ВариантыСинтаксиса Цикл
		Если ФормаВызовМетода.ВариантыСинтаксиса.Количество() > 1 Тогда
			ФормаВызовМетода.ЗагрузитьВариантМетода(ЭлементСписка.Значение);
		КонецЕсли; 
		...
		Для Каждого СтрокаПараметра Из ФормаВызовМетода.ТаблицаПараметров Цикл
			...
			КраткоеПредставлениеПараметра = "" + ?(СтрокаПараметра.Знач = "Знач", "`", СтрокаПараметра.Знач) + СтрокаПараметра.Имя + ?(ЛиНеОбязательный, "?", "");
			...
			ОписаниеПараметра = 
			"Тип: " + СтрокаПараметра.ТипЗначения + "
			|Возвр.: " + ?(Не ЗначениеЗаполнено(СтрокаПараметра.Знач), "?", ?(СтрокаПараметра.Знач = "Знач", "Нет", "Да")) + "
			|Обяз.: " + ?(Не ЛиНеОбязательный, "Да", "Нет, " + СтрокаПараметра.Значение) + "
		...
		КонецЦикла;
		ПодсказкаСлова.Вставить("label", "(" + ирОбщий.СтрСоединитьЛкс(КраткиеОписанияПараметров, ", ") + ")");
		ПодсказкаСлова.Вставить("parameters", ПараметрыМетода);
		ПодсказкаСлова.Вставить("documentation", ЭлементСписка.Представление);
		СтруктураПодсказки.Добавить(ПодсказкаСлова);
	КонецЦикла;
	Если СтруктураПодсказки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	СтруктураПодсказки = Новый Структура(ФормаВызовМетода.ИмяМетода, СтруктураПодсказки);
	РедакторHTML().setCustomSignatures(ирОбщий.ОбъектВСтрокуJSONЛкс(СтруктураПодсказки));

КонецПроцедуры

Процедура ПередатьТаблицуСловВРедакторHTML()
	
	СтруктураПодсказки = Новый Массив;
	Для Каждого СтрокаСлова Из ПолеТекстаЗапроса.ТаблицаСлов Цикл
		ПодсказкаСлова = Новый Структура();
		...
		Если СтрокаСлова.ТипСлова = "Метод" Тогда 
			КодТипаСлова = "Method";
		ИначеЕсли СтрокаСлова.ТипСлова = "Свойство" Тогда 
			КодТипаСлова = "Property";
		ИначеЕсли СтрокаСлова.ТипСлова = "Поле" Тогда 
			КодТипаСлова = "Field";
		ИначеЕсли СтрокаСлова.ТипСлова = "Конструкция" Тогда 
			КодТипаСлова = "Keyword";
		ИначеЕсли СтрокаСлова.ТипСлова = "Таблица" Тогда 
			КодТипаСлова = "Folder";
		Иначе
		...
		ПодсказкаСлова.Вставить("documentation", СтрокаСлова.Определение);
		СтруктураПодсказки.Добавить(ПодсказкаСлова);
	КонецЦикла;
	мРазрешенаПодсказкаHTML = Истина;
	РедакторHTML().showCustomSuggestions(ирОбщий.ОбъектВСтрокуJSONЛкс(СтруктураПодсказки));
	мРазрешенаПодсказкаHTML = Ложь;

КонецПроцедуры

Процедура ОбновитьПодсказкуHTML(Выражение = "", Подсказка = "")
	РедакторHTML = РедакторHTML();
	СтруктураПодсказки = Новый Соответствие;
	Если ЗначениеЗаполнено(Подсказка) Тогда
		СтруктураПодсказки.Вставить(Выражение, Подсказка);
	КонецЕсли; 
...
Процедура КПРедакторHTMLЗаменитьВхождения(Кнопка)
...
Процедура ОткрытьКонструкторФорматнойСтроки(ФорматнаяСтрока)
	...
	Если Конструктор.ОткрытьМодально() Тогда
		...
		Результат = """" + Результат + """";
		ПолеТекстаЗапроса.ВыделенныйТекст(Результат);
	КонецЕсли; 
	
КонецПроцедуры

Процедура КПРедакторHTMLКонструкторФорматнойСтроки(Кнопка)
	
	ФорматнаяСтрока = СтрЗаменить(СтрЗаменить(ПолеТекстаЗапроса.ВыделитьТекстТекущегоСтроковогоЛитерала(), "|", ""), """", "");
