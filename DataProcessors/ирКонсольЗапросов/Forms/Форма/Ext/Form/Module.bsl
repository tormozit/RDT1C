...
Процедура ЗагрузитьЗапросыИзФайла(ОчиститьПередЗагрузкой = Истина)
	...
	ПолученноеЗначение = ирОбщий.ПрочитатьЗначениеИзФайлаСКонтролемПотерьЛкс(мИмяОткрытогоФайла);
...
Процедура ЗаблокироватьРедактированиеТекстаЗапроса(ТолькоПросмотрДерева)
	...
	ЭлементыФормы.ДеревоЗапросов.ТолькоПросмотр = мРежимДереваЗапроса Или мРежимРедактораЗапроса;
...
Функция КоманднаяПанельТекстЗапросаСохранитьУзел(НовыйРежимСворачивания = Неопределено)
	...
	Если Не ПроверитьТекстПодзапроса() Тогда 
		...
		КонецЕсли;
	КонецЕсли;
	ПутьВДеревеЗапроса = ирОбщий.ПолучитьСтрокуПутиВДеревеЛкс(мРедактируемыйУзел, "");
	СвернутыйТекстНовогоПодзапроса = ЭлементыФормы.ТекстЗапроса.ПолучитьТекст();
	РазвернутыйТекстНовогоПодзапроса = РазвернутьВложенныеЗапросы(мРедактируемыйУзел, СвернутыйТекстНовогоПодзапроса);
	Если Истина
		И Прав(РазвернутыйТекстНовогоПодзапроса, 1) <> Символы.ПС 
		И мРедактируемыйУзел.ТипУзла <> "<TableWithName>"
	Тогда
	...
	СтрокаДанных = мРедактируемыйУзел.СтрокаДанных;
	Если СтрокаДанных <> Неопределено Тогда
		...
		Если ЛиСворачиваемыйТипУзла(мРедактируемыйУзел.ТипУзла) Тогда
