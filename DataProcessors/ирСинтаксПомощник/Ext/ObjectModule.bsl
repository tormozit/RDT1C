...
Функция ЗагрузитьОписаниеМетода(Знач ТипКонтекста, Знач ИмяМетода, Знач ОписаниеHTML, Знач ТипСлова = "Метод", Знач ЯзыкПрограммы = 0, Знач ТаблицаПараметров = Неопределено, Знач РабочийКаталог = "",
	...
	Для Каждого ЭлементСписка Из ОписанияВариантов Цикл
		Если ТаблицаПараметров = мПлатформа.ТаблицаПараметров Тогда
			...
			Результат = ВычислительРегВыражений.НайтиВхождения(ЭлементСписка.Значение);
			Если Результат.Количество() = 0 Тогда
				СтрокаОписания.Описание = "";
			Иначе
				СтрокаОписания.Описание = ИзвлечьИзФрагментаHTMLОбычныйТекст(Результат[0].SubMatches(0));
			КонецЕсли; 
		КонецЕсли; 
		// Параметры
		ВычислительРегВыражений.Pattern = шТегКлюч + "+Параметры:?" + шКонецТегаКлюч + "+((?:.|\n)*?)(?:" + шТегКлюч + "+Описание(?: варианта метода)?:?" + шКонецТегаКлюч + "|$)";
		Результат = ВычислительРегВыражений.НайтиВхождения(ЭлементСписка.Значение);
		Если Результат.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли; 
		...
		Иначе
			...
			ВычислительРегВыражений.Pattern = шТегКлюч + "(&lt;[^&]*&gt;)" 
			+ "()?(?:\s*(?:\((необязательный|обязательный)?\)\s*)?" + шКонецТегаКлюч + "+(?:Тип:" + шТегНеКлюч + "?((?:" + шТип + ")+)" + шТегНеКлюч + "?)?)?\.?((?:(?:Значение по умолчанию: " + шТег 
			+ "?([^<>]+)" + шТег + "?\.)|[^p](?=[^p]|$)|[^<]p+)*)";
		КонецЕсли;
		Результат = ВычислительРегВыражений.НайтиВхождения(ОписаниеВарианта);
		...
		Для Каждого Item Из Результат Цикл
			#Если Сервер И Не Сервер Тогда
				Item = Обработки.ирОболочкаРегВхождение.Создать();
			#КонецЕсли
			...
			Если ТипСлова <> "Таблица" Тогда
				...
				КонецЕсли;
				Если ТаблицаПараметров = мПлатформа.ТаблицаПараметров Тогда
					СтруктураСтроки.Описание = ИзвлечьИзФрагментаHTMLОбычныйТекст(Item.SubMatches(6));
				КонецЕсли; 
				ЗначениеПоУмолчанию = Item.SubMatches(7);
...
Функция ИзвлечьИзФрагментаHTMLОбычныйТекст(Знач Текст) Экспорт 
	...
	Текст = СокрЛП(ирОбщий.ДекодироватьТекстИзXMLЛкс(Текст));
	Возврат Текст;
...
шТег =           "(?:\s*<[^>]+>\s*)";
шТегНеКлюч =     "(?:\s*<[^>""]+>\s*)";
шТегКлюч =       "(?:\s*<(?:div|p|b)[^>]*>\s*)";
шКонецТегаКлюч = "(?:\s*</(?:div|p|b)>\s*)";
