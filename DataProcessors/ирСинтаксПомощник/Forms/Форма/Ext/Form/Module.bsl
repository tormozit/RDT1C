...
Функция ПолучитьНовыйАдресИзСсылки(Знач НовыйАдрес) Экспорт
	...
	Иначе
		НовыйАдрес = ирОбщий.ПоследнийФрагментЛкс(лИмяАрхива, "file://");
	КонецЕсли; 
	Возврат НовыйАдрес;
...
Процедура ПолеHTMLДокументаonclick(Элемент, pEvtObj)
	...
	Иначе
		НовыйАдрес = ЗагрузитьНовуюСтраницу(НовыйАдрес);
...
Для Каждого Файл Из Файлы Цикл
	Если Файл.Размер() > 20000 Тогда // отсекаем мелкий мусор
		Если Файл.ИмяБезРасширения = "dcsui_ru" Тогда
			ПредставлениеЭлемента = "Язык выражений компоновки";
		...
		ИначеЕсли Файл.ИмяБезРасширения = "shquery_ru" Тогда
			ПредставлениеЭлемента = "Язык запросов";
		Иначе
			Продолжить;
		КонецЕсли; 
