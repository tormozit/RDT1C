...
Процедура ДействияФормыВыполнить(Кнопка)
	...
	Если ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.Страницы.Выгрузка Тогда
		...
		#КонецЕсли
		ирОбщий.ВыгрузкаЗагрузкаДанныхЧерезФайл_ВыполнитьВыгрузкуЛкс(ОбщиеПараметрыОбработки(),, 
			ЭтаФорма, Кнопка, "ВыполнитьВыгрузкуЗавершение", БлокируемыеЭлементыФормы);
	Иначе
		#Если Сервер И Не Сервер Тогда
		...
		ирОбщий.ВыгрузкаЗагрузкаДанныхЧерезФайл_ВыполнитьЗагрузкуЛкс(ОбщиеПараметрыОбработки(),,, 
			ЭтаФорма, Кнопка, "ВыполнитьЗагрузкуЗавершение", БлокируемыеЭлементыФормы);
	КонецЕсли; 
...
Процедура ВыполнитьВыгрузкуЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		РезультатЗадания.ДвоичныеДанные.Записать(РезультатЗадания.ИмяФайла);
	КонецЕсли; 
