...
Процедура УстановитьЗначениеСловаВСтроке(СтрокаДерева, Успех, НовоеЗначение)
	...
	Если Успех = Истина Тогда
		...
		СтрокаДерева.КоличествоЭлементов = ирНеглобальный.ПолучитьКоличествоЭлементовКоллекцииЛкс(НовоеЗначение);
...
Процедура ОткрытьТекущийЭлемент(ПредпочитатьИсследовательКоллекций = Ложь)
	...
	Если СтрокаОписанияСлова.Владелец().Колонки.Найти("ТипЭлементаКоллекции") = Неопределено Тогда
		...
		Если ПолучитьСтруктуруТипаИзЗначения Тогда
			...
			Если Не ирНеглобальный.СтрокиРавныЛкс(ЛксПолучитьПервыйФрагмент(СтруктураКонкретногоТипа.ИмяОбщегоТипа), "COMОбъект") Тогда
	...
	ЭтоКоллекция = ирНеглобальный.ЭтоКоллекцияЛкс(ТекущаяСтрока.Значение);
...
Процедура КоманднаяПанельДереваКонсольКода(Кнопка)
	...
	Если Ложь
		...
		Или (Истина
			И ирНеглобальный.СтрокиРавныЛкс(ТекущаяСтрока.Слово, "_Значение_")
