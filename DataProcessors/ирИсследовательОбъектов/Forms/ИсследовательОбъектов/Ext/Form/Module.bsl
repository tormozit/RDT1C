...
Процедура мВычислитьВыражение() 
	...
	ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(ЭтаФорма.Заголовок,, КорневаяСтрока.Слово, ": ");
...
Процедура КоманднаяПанельДереваЗапомнитьОбъектДляСравнения(Кнопка)
	...
	Если ирОбщий.ЛиКоллекцияЛкс(ОбъектДляСравнения) Тогда
		ОбъектДляСравнения = Новый Структура("Объект", ОбъектДляСравнения);
	КонецЕсли;
	ирОбщий.ДобавитьДокументВБуферСравненияЛкс(ирОбщий.СтруктураИзОбъектаЛкс(ОбъектДляСравнения), ЭтаФорма, ЭлементыФормы.ДеревоЗначений.ТекущаяСтрока.Слово);
