...
Функция ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта, ДанныеИзБД = Истина, СсылкаИзменилась = Истина) 
	...
	Если фОбъект.ЭтоНаборЗаписей И Не фОбъект.мРежимМенеджераЗаписи Тогда
		...
		Если КлючТекущейСтроки <> Неопределено Тогда
			СтруктураКлючаТекущейСтроки = ирОбщий.СтруктураКлючаТаблицыБДЛкс(фОбъект.ИмяОсновнойТаблицы);
...
Процедура НадписьОтборНажатие(Элемент)
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ПеречитатьДанные();
			СтруктураКлючаСтроки = ирОбщий.СтруктураКлючаТаблицыБДЛкс(фОбъект.ИмяОсновнойТаблицы);
...
Процедура ОбновитьВерсииОбъекта()
	...
	#КонецЕсли
	КлючОбъекта = ПолучитьКлючОбъекта(Истина);
	Если Не ЗначениеЗаполнено(КлючОбъекта) Или Не ПравоДоступа("ПросмотрИсторииДанных", Метаданные.НайтиПоПолномуИмени(фОбъект.мПолноеИмяМД)) Тогда
		Возврат;
	КонецЕсли; 
...
Процедура КоманднаяПанель_НайтиВФормеСпискаВерсий(Кнопка)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьСистемнойФормуСписокВерсийЛкс(ПолучитьКлючОбъекта(Истина), ТекущиеДанные.НомерВерсии);
...
Процедура КоманднаяПанель_ВерсииСравнитьСПредыдущей(Кнопка)
	...
	Если ВыделеннаяВерсия = НомерПервойВерсии Тогда
		...
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьСистемнуюФормуСравненияВерсийЛкс(ПолучитьКлючОбъекта(Истина), ВыделеннаяВерсия, ВыделеннаяВерсия - 1);
...
Процедура КоманднаяПанель_ОткрытьОтчетПоВерсии(Кнопка = Неопределено)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьСистемнойФормуОтчетПоВерсииЛкс(ПолучитьКлючОбъекта(Истина), ТекущиеДанные.НомерВерсии);
...
Процедура КоманднаяПанель_ВерсииСравнитьСПоследней(Кнопка)
	...
	ирОбщий.ОткрытьСистемнуюФормуСравненияВерсийЛкс(ПолучитьКлючОбъекта(Истина), НомерПоследнейВерсии, ТекущаяВерсия);
...
Процедура КоманднаяПанель_ВерсииСравнить2(Кнопка)
	...
	ирОбщий.ОткрытьСистемнуюФормуСравненияВерсийЛкс(ПолучитьКлючОбъекта(Истина), МладшаяВерсия, СтаршаяВерсия);
...
Процедура КоманднаяПанельВерсииНайтиВИсторииДанных(Кнопка)
	...
	ФормаИнструмента.НайтиВерсию(ПолучитьКлючОбъекта(), ЭлементыФормы.ВерсииОбъекта.ТекущаяСтрока.НомерВерсии);
