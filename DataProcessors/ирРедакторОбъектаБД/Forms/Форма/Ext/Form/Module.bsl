...
Функция ЗаполнитьСтрокуТаблицыРеквизиты(ЗначениеРеквизита, ИмяРеквизита, Представление = "", ОписаниеТипов = Неопределено, ТолькоПросмотр = Ложь, ДанныеИзБД = Истина, ПодсказкаРеквизита = "",
	...
	СтрокаРеквизита.Подсказка = ПодсказкаРеквизита;
...
Функция ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта, ДанныеИзБД = Истина, СсылкаИзменилась = Истина) 
	...
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
		...
		Исключение
		КонецПопытки;
		ЗаполнитьСтрокуТаблицыРеквизиты(ЗначениеРеквизита, "ИдентификаторСсылкиЛкс", "Идентификатор ссылки", ОписаниеТиповИД, 
...
Процедура ЗагрузитьОбъектПоКлючу(Знач КлючИлиОбъект) Экспорт 
	...
	Если фОбъект.ПараметрПрочитатьОбъект Тогда
		...
		Иначе
			ПеречитатьДанные();
		КонецЕсли;
		Если ирОбщий.ЛиКлючЗаписиРегистраЛкс(мСтруктураОбъектаБД.Методы) Тогда
			ПоказатьСтрокуНабораЗаписей(мСтруктураОбъектаБД.Методы);
		КонецЕсли; 
	Иначе
...
Процедура ПриЗакрытии()
	...
	Если ЭтаФорма.КлючУникальности = "Связанный" Тогда
		Оповестить("ЗакрытСвязанныйРедакторОбъектаБД");
	КонецЕсли; 
...
Процедура РеквизитыТаблицаЗначениеРеквизитаНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если фОбъект.СвязиИПараметрыВыбора Тогда
		...
		ПоляТаблицыБД = ирКэш.ПоляТаблицыБДЛкс(фОбъект.ИмяОсновнойТаблицы);
		ПолеТаблицыБД = ПоляТаблицыБД.Найти(ИмяПоляТаблицы, "Имя");
		Если ПолеТаблицыБД <> Неопределено Тогда
			МетаРеквизит = ПолеТаблицыБД.Метаданные;
			РеквизитыОбъекта = ПолучитьИзмененныйОбъект().Данные;
			Если фОбъект.ЭтоНаборЗаписей Тогда
				РеквизитыОбъекта = РеквизитыОбъекта[0];
			КонецЕсли; 
			СтруктураОтбора = ирОбщий.ПолучитьСтруктуруОтбораПоСвязямИПараметрамВыбораЛкс(РеквизитыОбъекта, МетаРеквизит);
		КонецЕсли; 
	КонецЕсли; 
	ЗначениеИзменено = ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.РеквизитыТаблица, СтандартнаяОбработка,, Истина, СтруктураОтбора);
...
Процедура НайтиЗначениеВТаблицеОбъекта(ИмяТабличногоПоля, ИмяТаблицыОбъекта, ИскомоеЗначение)
	...
	ТаблицаТЧ = ирОбщий.ТаблицаЗначенийИзТаблицыФормыСКоллекциейЛкс(ТабличноеПоле);
...
Процедура ПоказатьЯчейкуДанныхОбъекта(ПолноеИмяТаблицы = "", ИмяРеквизита = "", КлючСтроки = Неопределено) Экспорт
	...
	Попытка
		ОбъектМетаданных = ирКэш.ОбъектМДПоПолномуИмениЛкс(ПолноеИмяТаблицы);
	Исключение
		// ВидыСубконто
	КонецПопытки;
	Если Ложь
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	КонецЕсли; 
	ирОбщий.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельОсновная.Страницы.СтраницаСвязанныеДанные, Ложь);
	мМетаданныеОбъекта = ирОбщий.ОбъектМДПоПолномуИмениТаблицыБДЛкс(фОбъект.ИмяОсновнойТаблицы, Истина);
	ТипТаблицы = ирОбщий.ТипТаблицыБДЛкс(фОбъект.ИмяОсновнойТаблицы);
	...
	ИначеЕсли мМетаданныеОбъекта = Неопределено Тогда
...
Процедура ПриИзмененииОтбораНабораЗаписей()
	...
	фОбъект.мРежимМенеджераЗаписи = Истина
		И ирОбщий.ЛиКорневойТипРегистраСведенийЛкс(ирОбщий.ПервыйФрагментЛкс(фОбъект.ИмяОсновнойТаблицы))
...
Процедура СвязанныеДанныеОтборБезЗначенияПриИзменении(Кнопка)
	...
	ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонке_КнопкаЛкс(ЭлементыФормы.СвязанныеДанные);
...
Процедура КП_ОтборСнятьФлажки(Кнопка)
	...
	Если Не ЭлементыФормы.ТаблицаОтбора.Колонки.Использование.ТолькоПросмотр Тогда
		ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.ТаблицаОтбора, "Использование", Ложь);
		ПриИзмененииОтбораНабораЗаписей();
	КонецЕсли; 
...
Процедура КП_ОтборУстановитьФлажки(Кнопка)
	...
	Если Не ЭлементыФормы.ТаблицаОтбора.Колонки.Использование.ТолькоПросмотр Тогда
		ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.ТаблицаОтбора, "Использование", Истина);
		ПриИзмененииОтбораНабораЗаписей();
	КонецЕсли; 
...
Процедура ОсновныеДействияФормыСравнить(Кнопка)
	...
	ТабличноеПоле = ТекущееТабличноеПоле(ЭтаФорма);
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭтаФорма, ТабличноеПоле);
...
Процедура КП_СсылкиНаОбъектОбновить(Кнопка)
	...
	ОбработкаОбъект.ОбновитьНайденныеСсылки(МассивИскомыхСсылок, фОбъект.СсылкиНаОбъект, Истина);
...
Процедура КоманднаяПанель_ВерсииОтборБезЗначения(Кнопка)
	...
	ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонке_КнопкаЛкс(ЭлементыФормы.ВерсииОбъекта);
...
Процедура КП_СвязанныеКолонкиУстановитьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.СвязанныеДанные, "Избранное", Истина);

КонецПроцедуры

Процедура КП_СвязанныеКолонкиСнятьПометки(Кнопка)
	
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.СвязанныеДанные, "Избранное", Ложь);
...
Процедура ПоискВОбъектеПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура ВерсииОбъектаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура ОсновныеДействияФормыСверткаТаблицы(Кнопка)
	
	ТабличноеПоле = ТекущееТабличноеПоле(ЭтаФорма);
	ирОбщий.ОткрытьГруппировкуТабличногоПоляЛкс(ТабличноеПоле);
...
Процедура ОсновныеДействияФормыПоказыватьИтоги(Кнопка)
	
	ирОбщий.ТабличноеПолеКнопкаОтображенияИтоговНажатиеЛкс(ТекущееТабличноеПоле(ЭтаФорма), Кнопка);
...
ЭлементыФормы.ВыбОбъект.ТипЗначения = Новый ОписаниеТипов(ирОбщий.ОписаниеТиповВсеСсылкиЛкс(), "Строка"); // Нужно для добавления всех типов внешних источников данных
