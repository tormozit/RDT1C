...
Функция ЗаполнитьСтрокуТзРеквизиты(Значение, ИмяРеквизита, Представление = "", ОписаниеТипов = Неопределено, ТолькоПросмотр = Ложь, ДанныеИзБД = Истина, ПодсказкаРеквизита = "")
	...
	Если ирОбщий.СтрокиРавныЛкс(ИмяРеквизита, "Ссылка") Тогда
		ЗначениеРеквизита = ирОбщий.ПолучитьИдентификаторСсылкиЛкс(Значение);
	Иначе
	...
	ирОбщий.ОбновитьТипЗначенияИзОписанияТиповЛкс(строкаТзРеквизиты, "ОригинальноеЗначениеРеквизита");
...
Функция ПоместитьДанныеОбъектаВФорму(Объект, ДанныеИзБД = Истина) 
	...
	Если мЭтоСсылочныйОбъект Тогда
		...
		Если мЭтоВнешнийОбъект Тогда
			ОписаниеТиповИД = Новый ОписаниеТипов("Строка");
		Иначе
			ОписаниеТиповИД = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(36));
		КонецЕсли; 
		ЗаполнитьСтрокуТзРеквизиты(?(Объект.ЭтоНовый(), Объект.ПолучитьСсылкуНового(), ВыбОбъект), "Ссылка", "Идентификатор", ОписаниеТиповИД, мЭтоВнешнийОбъект Или Не Объект.ЭтоНовый());
		...
		Для Каждого ДоступноеПоле Из мПоляТаблицыБД Цикл
			...
			Иначе
				ПодсказкаРеквизита = "";
			КонецЕсли; 
			ЗаполнитьСтрокуТзРеквизиты(Объект, ИмяПоля, ДоступноеПоле.Заголовок, ДоступноеПоле.ТипЗначения, Ложь
	...
	ИначеЕсли мЭтоКонстанта Тогда
		...
		МассивТипов.Добавить(ТипЗнч(Объект.Значение));
		строкаТзРеквизиты.ТипЗначения = Новый ОписаниеТипов(МассивТипов);
...
Процедура ПоместитьСтрокуНабораВТаблицуРеквизитов(ДанныеИзБД = Ложь)
	...
	Для Каждого ДоступноеПоле Из мПоляТаблицыБД Цикл
		...
		Иначе
			ПодсказкаРеквизита = "";
		КонецЕсли; 
		СтрокаРеквизита = ЗаполнитьСтрокуТзРеквизиты(СтрокаНабора, ИмяПоля, ДоступноеПоле.Заголовок, ДоступноеПоле.ТипЗначения, , ДанныеИзБД, ПодсказкаРеквизита);
...
Процедура ПоместитьТаблицуРеквизитовВОбъект(Знач Объект)
	...
	Для каждого СтрокаРеквизита из РеквизитыТаблица Цикл
		...
		//КонецЕсли;
		Попытка
			Объект[СтрокаРеквизита.ИмяРеквизита] = СтрокаРеквизита.ОригинальноеЗначениеРеквизита;
		Исключение
...
Процедура РеквизитыТаблицаЗначениеРеквизитаПриИзменении(Элемент = Неопределено, ТекущиеДанные = Неопределено)
	...
	ирОбщий.ОбновитьТипЗначенияИзОписанияТиповЛкс(ТекущиеДанные, "ОригинальноеЗначениеРеквизита");
	Если мРежимМенеджераЗаписи Тогда
		...
		Если СтрокаОтбора <> Неопределено Тогда
			...
			СтрокаОтбора.ТипЗначения = ТипЗнч(СтрокаОтбора.Значение);
		КонецЕсли; 
	КонецЕсли;
...
Процедура РеквизитыТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(Элемент, СтандартнаяОбработка, ВыбраннаяСтрока.ОригинальноеЗначениеРеквизита, СтрокуРазрешеноРедактировать) Тогда 
