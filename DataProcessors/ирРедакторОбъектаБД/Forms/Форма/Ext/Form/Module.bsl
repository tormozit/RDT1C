...
Процедура ПоместитьСтрокуНабораВТаблицуРеквизитов(ДанныеИзБД = Ложь)
	...
	Для Каждого ДоступноеПоле Из мПоляТаблицыБД Цикл
		...
		Иначе
			ПодсказкаРеквизита = "";
		КонецЕсли; 
		ЗаполнитьСтрокуТзРеквизиты(СтрокаНабора, ИмяПоля, ДоступноеПоле.Заголовок, ДоступноеПоле.ТипЗначения, , ДанныеИзБД, ПодсказкаРеквизита);
	КонецЦикла;
	ЗаполнитьИндексыКартинокТаблицыРеквизитов(СтрокаНабора);
	РеквизитыТаблица.Сортировать("Порядок, ИмяРеквизита");
...
Процедура ЗагрузитьОбъектПоКлючу(КлючИлиОбъект) Экспорт 
	...
	Если Не мПроверитьМодифицированность() Тогда
		Возврат;
	КонецЕсли; 
	НовоеИмяОсновнойТаблицы = ирОбщий.ПолучитьИмяТаблицыИзМетаданныхЛкс(КлючИлиОбъект.Метаданные());
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.Прочее.Кнопки.ВыгрузитьВXML.Доступность = Не мЭтоВнешнийОбъект И ЗначениеЗаполнено(ИмяОсновнойТаблицы);
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.Прочее.Кнопки.ЗагрузитьИзXML.Доступность = Не мЭтоВнешнийОбъект И ЗначениеЗаполнено(ИмяОсновнойТаблицы);
