...
Функция ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта, ДанныеИзБД = Истина, СсылкаИзменилась = Истина) 
	...
	Иначе // Это набор записей
		...
		НастроитьТабличноеПолеДвиженийИлиТЧ(ЭлементыФормы.НаборЗаписей);
...
Процедура ЗаполнитьСтрокуСвязаннойКолонки(Знач ОписаниеТаблицы, Знач ПолеТаблицыБД, Знач СтрокаСвязи)
	...
	СтрокаСвязи.ТипТаблицыКартинка = ОписаниеТаблицы.ИндексКартинки;
		...
		Если фОбъект.ЭтоСсылочныйОбъект Тогда
			...
			Если СписокСвойств.Количество() > 0 Тогда
				...
				Попытка
					...
					ДопСведения = РегистрыСведений.ДополнительныеСведения.СоздатьНаборЗаписей();
					ДопСведения.Отбор.Объект.Установить(СтруктураОбъекта.Методы.Ссылка);
					ДопСведения.Прочитать();
					ТаблицаДопСведения = ДопСведения.Выгрузить();
					ИмяПоляСсылка = "";
					Для Каждого СтрокаРеквизита Из РеквизитыТаблица Цикл
						Если ЗначениеЗаполнено(СтрокаРеквизита.ДопРеквизит) Тогда
						...
						КонецЕсли; 
					КонецЦикла;
					ДопСведения.Загрузить(ТаблицаДопСведения);
					ирОбщий.ЗаписатьОбъектЛкс(ДопСведения);
...
Процедура РеквизитыТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	КонецЕсли;
	РежимИдентифкаторов = ЭлементыФормы.ОсновныеДействияФормы.Кнопки.ПодменюТекущаяТаблица.Кнопки.Идентификаторы.Пометка;
	Если Истина
...
Функция ЛиРежимПароляСтрокиРеквизита(Знач ДанныеСтроки)
	...
	Если СтрокаПоля <> Неопределено Тогда
		Попытка
			РежимПароля = СтрокаПоля.Метаданные.РежимПароля;
		Исключение
		КонецПопытки;
	КонецЕсли;
	Возврат РежимПароля;
...
Процедура ПриАктивизацииСтрокиСтраницыДвижений()
	...
	Если ЭлементыФормы.ПанельДвижения.Видимость Тогда
		...
		Если ТабличноеПолеНабораДвижений.Колонки.Количество() = 0 Тогда
			...
			ТабличноеПолеНабораДвижений.Доступность = Истина;
			ирОбщий.СоздатьКолонкиТабличногоПоляЛкс(ТабличноеПолеНабораДвижений);
			НастроитьТабличноеПолеДвиженийИлиТЧ(ТабличноеПолеНабораДвижений);
...
Процедура ПриАктивизацииСтрокиСтраницыТабличныхЧастей()
	...
	Если ЭлементыФормы.ПанельТабличныеЧасти.Видимость Тогда
		...
		Если ТабличноеПолеТабличнойЧасти.Колонки.Количество() = 0 Тогда
			...
			НастроитьТабличноеПолеДвиженийИлиТЧ(ТабличноеПолеТабличнойЧасти);
...
Процедура НастроитьТабличноеПолеДвиженийИлиТЧ(Знач ТабличноеПолеНабораДвиженийИлиТЧ)
...
Функция КлючОбъекта(ПолныйКлюч = Истина)
	...
	ИначеЕсли фОбъект.ЭтоКонстанта Тогда
		Результат = ирОбщий.КлючОбъектаКонстантыЛкс(ирОбщий.ПоследнийФрагментЛкс(фОбъект.ИмяОсновнойТаблицы), Ложь);
	Иначе
...
Процедура НаборЗаписейПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
	...
	КонецЕсли; 
	ОформитьЯчейкиПоМетаданным(Элемент, ИмяТаблицыБД, ОформлениеСтроки);
	
КонецПроцедуры

Процедура ОформитьЯчейкиПоМетаданным(Знач ТабличноеПоле, Знач ИмяТаблицыБД, Знач ОформлениеСтроки)
	...
	МаскироватьПароли = Не ЭлементыФормы.ОсновныеДействияФормы.Кнопки.ПодменюТекущаяТаблица.Кнопки.Идентификаторы.Пометка;
	Для Каждого ПолеТаблицы Из ПоляТаблицыБД Цикл
		Попытка
			РежимПароля = ПолеТаблицы.Метаданные.РежимПароля;
		Исключение
			Продолжить;
		КонецПопытки;
...
Процедура КоманднаяПанель_ВерсииОбновитьИсторию(Кнопка)
	...
	#КонецЕсли
	ирОбщий.ОбновитьИсториюДанныхЛкс(Новый Структура, ЭтаФорма, Кнопка, "ОбновитьИсториюЗавершение");
...
Процедура СброситьКолонкиТаблицы(Кнопка)
	...
	ИначеЕсли ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФормы.ПанельОбъект.Страницы.СтраницаСтраницыПанельТабличныеЧасти Тогда
		ПриАктивизацииСтрокиСтраницыТабличныхЧастей();
	Иначе
		// TODO сброс колонок для набора записей
	КонецЕсли; 
КонецПроцедуры
