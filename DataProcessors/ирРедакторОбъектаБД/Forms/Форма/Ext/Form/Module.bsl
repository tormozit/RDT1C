...
Процедура ОбновитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(фОбъект.ВыбОбъект) Тогда
		ЭтаФорма.Заголовок = СтрЗаменить(ЭтаФорма.Заголовок, "Редактор объекта БД ", "РО");
	КонецЕсли; 
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
...
Процедура РеквизитыТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	КонецЕсли;
	Если Истина
		И Не ЭлементыФормы.ОсновныеДействияФормы.Кнопки.ПодменюТекущаяТаблица.Кнопки.Идентификаторы.Пометка 
...
Процедура СвязанныеДанныеОбновитьКоличество(Кнопка = Неопределено)
	...
	Для Каждого СтрокаСвязанныхДанных Из фОбъект.СвязанныеДанные Цикл
		...
		Если ирКэш.ДоступныОбщиеРеквизитыЛкс() Тогда
			...
			КонецЕсли; 
		КонецЕсли;
		Если Не СтрокаСвязанныхДанных.ЕстьДоступ Тогда
			Продолжить;
		КонецЕсли; 
		...
		|ИЗ " + СтрокаСвязанныхДанных.ПолноеИмяТаблицы + " КАК Т
