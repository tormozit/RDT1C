...
Перем мПрефиксИмениТабличногоПоляТЧ Экспорт;
Перем мЭтоГруппа;
Перем мСчитанНепустойНаборЗаписей;
Перем мДоступноИзменение;
...
Процедура ДоступностьКнопок()
	...
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.Удалить.Доступность = Ложь
		Или мЭтоСсылочныйОбъект И ЗначениеЗаполнено(ВыбОбъект) И мДоступноИзменение
		Или мРежимМенеджераЗаписи;
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.ПометитьНаУдаление.Доступность = мЭтоСсылочныйОбъект и ЗначениеЗаполнено(ВыбОбъект) И мДоступноИзменение;
...
Функция ПоместитьДанныеОбъектаВФорму(Объект, ДанныеИзБД = Истина) 
	...
	Если мЭтоСсылочныйОбъект Тогда
		...
		Исключение
		КонецПопытки;
		ЗаполнитьСтрокуТаблицыРеквизиты(ЗначениеРеквизита, "ИдентификаторСсылкиЛкс", "Идентификатор ссылки", ОписаниеТиповИД, мЭтоМетаСсылка Или мЭтоВнешнийОбъект Или Не Объект.ЭтоНовый());
...
Процедура ПодготовитьСтраницыТабличныхЧастей(Знач Объект = Неопределено, Знач СтруктураТЧ = Неопределено)
	...
	Для Каждого ЭлементТЧ Из СтруктураТЧ Цикл
		...
		СтрокаСтраницы.Количество = ТабличноеПолеНабораДвижений.Значение.Количество(); // Сразу заполним количество, чтобы после завершения потока кода не сбросилась текущая строка при отборе по количеству
	КонецЦикла;
...
Функция ПолучитьТекущееТабличноеПоле()
	...
	ИначеЕсли СтраницаОбъекта = ЭлементыФОрмы.ПанельОбъекта.Страницы.СвойстваОбъекта Тогда
		...
		ИначеЕсли ЭлементыФормы.ПанельТабличныеЧасти.ТекущаяСтраница = ЭлементыФОрмы.ПанельТабличныеЧасти.Страницы.НаборЗаписей Тогда
...
Процедура ОсновныеДействияФормыВТаблицу(Кнопка)
	...
	ТекущееТабличноеПоле = ПолучитьТекущееТабличноеПоле();
	...
	Для Каждого СтрТаблицы ИЗ ТЗ Цикл
		...
		Для Каждого КолТаблицы ИЗ ТЗ.Колонки Цикл
			...
			лкУстановитьЗначениеВЯчейку(ТабДок.Область(Стр, Кол), Зн, КолТаблицы.ТипЗначения);
			Кол = Кол+1;
		КонецЦикла;
		Стр = Стр+1;
	КонецЦикла;
...
Процедура РеквизитыТаблицаЗначениеРеквизитаПриИзменении(Элемент = Неопределено, ТекущиеДанные = Неопределено)
	...
	Если ТекущиеДанные.ИмяРеквизита = "ИдентификаторСсылкиЛкс" И мЭтоСсылочныйОбъект Тогда
...
Процедура ОбновитьПометкиДереваУзлов()
	...
	КонецЕсли; 
	ЭлементыФормы.ПанельОбъекта.Страницы.ИзмененияНаУзлах.Значение = Истина;
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	Иначе
		...
		КонецЕсли;
	мДоступноИзменение = Истина
		И ЗначениеЗаполнено(ИмяОсновнойТаблицы)
		И Не мЭтоМетаСсылка
		И (Ложь
			Или Не мЭтоВнешнийОбъект
	...
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.Новый.Доступность = мЭтоСсылочныйОбъект И мДоступноИзменение;
	...
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.Скопировать.Доступность = мЭтоСсылочныйОбъект И мДоступноИзменение;
	...
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.Записать.Доступность = мДоступноИзменение;
	ЭлементыФормы.ОсновныеДействияФормы.Кнопки.ПоказатьРазличияБД.Доступность = мДоступноИзменение;
...
Процедура КП_СсылкиНаОбъектОбновить(Кнопка)
	...
	ЭлементыФормы.ПанельОбъекта.Страницы.СсылкиНаОбъект.Значение = Истина;
...
Процедура НайтиПоказатьПолеПоПредставлению(ПредставлениеПоля, ПредставлениеЗначения = "") Экспорт 
	...
	Иначе
		...
		Для Каждого СтрокаОписанияТЧ Из СтраницыТабличныеЧасти Цикл
			#Если Сервер И Не Сервер Тогда
				СтрокаОписанияТЧ = Обработки.ирРедакторОбъектаБД.Создать().СтраницыТабличныеЧасти.Добавить();
			#КонецЕсли
			ТабличноеПолеТЧ = ЭлементыФормы[мПрефиксИмениТабличногоПоляТЧ + СтрокаОписанияТЧ.Имя];
		...
		КонецЦикла;
		Если НайденнаяСтрокаОписанияТЧ <> Неопределено Тогда
			ЭлементыФормы.СтраницыТабличныеЧасти.ТекущаяСтрока = НайденнаяСтрокаОписанияТЧ;
			ТекущийЭлемент = ЭлементыФормы[мПрефиксИмениТабличногоПоляТЧ + СтрокаОписанияТЧ.Имя];
...
Процедура ОсновныеДействияФормыОткрытьОбъектМетаданных(Кнопка)
	
	Если мМетаданныеОбъекта <> Неопределено Тогда
		ирОбщий.ОткрытьОбъектМетаданныхЛкс(мМетаданныеОбъекта);
	КонецЕсли; 
