...
Процедура КоманднаяПанельНабораДвиженийРедакторОбъектаБДНабораДвижений(Кнопка)
	...
	Если ТекущаяСтрокаНабора <> Неопределено И ирОбщий.ПолучитьТипТаблицыБДЛкс(ИмяТаблицыБДРегистра) <> "Перерасчет" Тогда
		...
		КлючСтроки = ирОбщий.КлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ИмяТаблицыБДРегистра, ТекущаяСтрокаНабора).Структура;
		Если КлючСтроки.Свойство("Регистратор") Тогда
			КлючСтроки.Удалить("Регистратор");
		КонецЕсли; 
	КонецЕсли; 
