...
Функция ЗаполнитьИндексыКартинокТаблицыРеквизитов(Объект, ЗаполнятьПорядок = Истина)
	...
	КонецЕсли; 
	ВнутренняяТаблицаСлов = мПлатформа.ПолучитьТаблицуСловСтруктурыТипа(СтруктураТипаКонтекста,,,,, Ложь, "Свойство");
	...
	Для Каждого ВнутренняяСтрокаСлова Из ВнутренняяТаблицаСлов Цикл
		...
		Если ПереводВключен И СтрокаРеквизита.Количество() = 0 Тогда
			СтрокаРеквизита = РеквизитыТаблица.НайтиСтроки(Новый Структура("ИмяРеквизита", ирОбщий.ПеревестиСтроку(ВнутренняяСтрокаСлова.Слово)));
		КонецЕсли; 
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	Если ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(ТипТаблицы) Тогда
		Сообщить("Таблица " + фОбъект.ИмяОсновнойТаблицы + " является вложенной и не допускает прямого редактирования");
...
Процедура ОбновитьВерсииОбъекта()
	...
	Если Ложь
		...
		Или Не ПравоДоступа("ПросмотрИсторииДанных", Метаданные.НайтиПоПолномуИмени(фОбъект.мПолноеИмяМД)) 
	Тогда
		Возврат;
	КонецЕсли; 
