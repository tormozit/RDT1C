...
Функция мДобавитьСтраницуТЧ(Объект, ИмяТЧ, ПредставлениеТЧ)
	...
	МетаТЧ = мМетаданныеОбъекта.ТабличныеЧасти.Найти(ИмяТЧ);
	Если МетаТЧ <> Неопределено Тогда
		МетаРеквизиты = МетаТЧ.Реквизиты;
	КонецЕсли; 
	ирОбщий.НастроитьДобавленныеКолонкиТабличногоПоляЛкс(тчНаФорме, , МетаРеквизиты);
...
Процедура НайтиНажатие(Элемент = Неопределено)
	...
	Тогда
		...
		Для Индекс = мБазовоеКоличествоСтраницПанелиТЧ По ЭлементыФормы.ПанельТабличныеЧасти.Страницы.Количество() - 1 Цикл
			...
			ОсновноеПолеСтраницы = ЭлементыФормы[Страница.Имя];
			Если ТипЗнч(ОсновноеПолеСтраницы) = Тип("ТабличноеПоле") Тогда
				НайтиЗначениеВТаблицеОбъекта(ОсновноеПолеСтраницы, Страница.Имя, ИскомоеЗначение);
			КонецЕсли; 
		КонецЦикла;
