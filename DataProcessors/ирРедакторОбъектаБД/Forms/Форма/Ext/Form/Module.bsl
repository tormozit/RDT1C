...
Процедура ПеречитатьДанные(Знач СохранятьИдентификаторСсылки = Ложь, ТолькоМакет = Ложь, СсылкаИзменилась = Истина)
	...
	ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта,, СсылкаИзменилась);
...
Процедура ЗагрузитьОбъектПоКлючу(Знач КлючИлиОбъект) Экспорт 
	...
	Если ТипЗнч(КлючИлиОбъект) = Тип("Структура") Тогда
		Если КлючИлиОбъект.Свойство("ПолноеИмяТаблицы") Тогда
			...
			СтруктураОбъекта.Вставить("Методы", КлючИлиОбъект.Структура);
		Иначе
	...
	Если фОбъект.ПараметрПрочитатьОбъект Тогда
		...
		Если ирОбщий.ЛиКлючЗаписиРегистраЛкс(СтруктураОбъекта.Методы) Тогда
			ПоказатьСтрокуНабораЗаписей(СтруктураОбъекта.Методы);
		КонецЕсли; 
	Иначе
...
Процедура ИмяСинонимПриИзменении(Элемент = Неопределено)
	...
	// Табличные части
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
		СтруктураОбъекта = ПолучитьОсновнойОбъектИзБД();
...
Процедура ОсновныеДействияФормыСверткаТаблицы(Кнопка)
	...
	ТабличноеПоле = ТекущееТабличноеПоле(ЭтаФорма);
	ирОбщий.ОткрытьСверткуТабличногоПоляЛкс(ТабличноеПоле);
