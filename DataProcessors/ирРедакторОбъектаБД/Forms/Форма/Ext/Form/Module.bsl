...
Функция мДобавитьСтраницуТЧ(Объект, ИмяТЧ, ПредставлениеТЧ)
	...
	ПодключитьОбработчикиСобытийПолейВводаКолонокТабличногоПоля(тчНаФорме, Истина);
...
Функция ПоместитьДанныеОбъектаВФорму(Объект, ДанныеИзБД = Истина)
	...
	Иначе // Это набор записей
		...
		#КонецЕсли
		ирОбщий.НастроитьДобавленныеКолонкиТабличногоПоляЛкс(ЭлементыФормы.НаборЗаписей,,, КомпоновщикТаблицы.Настройки.ДоступныеПоляВыбора);
...
Процедура ПодключитьОбработчикиСобытийПолейВводаКолонокТабличногоПоля(Знач ТабличноеПолеСДинамическимиКолонками, ПодключитьОбработчикНачалоВыбора = Ложь)
	...
	Для Каждого КолонкаТП Из ТабличноеПолеСДинамическимиКолонками.Колонки Цикл
		Если КолонкаТП.ЭлементУправления <> Неопределено Тогда
			...
			Если ПодключитьОбработчикНачалоВыбора Тогда
...
Процедура ПриОткрытии()
	...
	Если мПараметрКлючИлиОбъект <> Неопределено Тогда
		...
		Если мПараметрПрочитатьОбъект Тогда
			мПеречитатьДанные();
		Иначе
			ПоместитьДанныеОбъектаВФорму(мПараметрКлючИлиОбъект, Ложь);
		КонецЕсли; 
	Иначе
	...
	Если ЭлементыФормы.Движения.Видимость Тогда
		...
		Если ТабличноеПолеНабораДвижений.Колонки.Количество() = 0 Тогда
			...
			#Если _ Тогда
			    КомпоновщикТаблицы = Новый КомпоновщикНастроекКомпоновкиДанных;
			#КонецЕсли
			ирОбщий.НастроитьДобавленныеКолонкиТабличногоПоляЛкс(ТабличноеПолеНабораДвижений,,, КомпоновщикТаблицы.Настройки.ДоступныеПоляВыбора);
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	Тогда
		...
		Если ЗначениеЗаполнено(ИмяОсновнойТаблицы) Тогда
			...
			Иначе
				ИмяТипаСсылки = ирОбщий.ПолучитьИмяТипаСсылкиТаблицыБДЛкс(ИмяОсновнойТаблицы);
			КонецЕсли; 
...
Процедура ОсновныеДействияФормыШиринаКолонок(Кнопка)
	...
	ТабличноеПоле = ПолучитьТекущееТабличноеПоле();
	ирОбщий.ВвестиИУстановитьШиринуКолонокТабличногоПоляЛкс(ТабличноеПоле);
...
Процедура ОсновныеДействияФормыОткрытьВТонкомКлиенте(Кнопка)
	...
	ВнешняяСсылка = COMСеанс.NewObject(ирОбщий.ПолучитьИмяТипаСсылкиТаблицыБДЛкс(ИмяОсновнойТаблицы), COMСеанс.NewObject("УникальныйИдентификатор", "" + ВыбОбъект.УникальныйИдентификатор()));
