...
Процедура ЗаполнитьТаблицыДвижений(Объект)
	...
	Для Каждого МетаРегистр из ОбъектыМД Цикл
		...
		ИмяТипаРегистра = ЛксПолучитьПервыйФрагмент(ПолноеИмяМДНабораЗаписей);
...
Процедура мПоместитьДанныеВТЧ(Объект, ИмяТЧ)
	...
	Если Истина
		...
		И ирНеглобальный.СтрокиРавныЛкс(ИмяТЧ, "ВидыСубконто")
	Тогда
	...
	Если Истина
		...
		И (Ложь
			Или ирНеглобальный.СтрокиРавныЛкс(ИмяТЧ, "БазовыеВидыРасчета")
			Или ирНеглобальный.СтрокиРавныЛкс(ИмяТЧ, "ВедущиеВидыРасчета")
			Или ирНеглобальный.СтрокиРавныЛкс(ИмяТЧ, "ВытесняющиеВидыРасчета"))
	Тогда
...
Функция мДобавитьСтраницуТЧ(ИмяТЧ, ПредставлениеТЧ)
	...
	НоваяСтраница = ЭлементыФормы.панельТабличныеЧасти.Страницы.Добавить(ИмяТаблицы, ПредставлениеТЧ, , БиблиотекаКартинок.ирТабличнаяЧасть);
...
Функция ПоместитьДанныеОбъектаВФорму(Объект, ДанныеИзБД = Истина)
	...
	ЛксОбновитьТекстПослеМаркераВСтроке(ЭтаФорма.Заголовок, , ВыбОбъект, ": ");
	УдалитьРеквизиты();
	...
	мдОбъекта = ЛксПолучитьМетаданные(ВыбОбъект);
	Если мдОбъекта <> Неопределено Тогда
		ТипМетаданных = ЛксПолучитьПервыйФрагмент(мдОбъекта.ПолноеИмя());
	Иначе
...
Процедура ЗаполнитьСвязанныеДанные()
	...
	Для Каждого СтрокаТаблицыМетаОбъектов Из СтрокиМетаОбъектов Цикл
		...
		Если Ложь
			...
			Или ЛксЛиКорневойТипПоследовательности(Единственное)
		Тогда
			КоллекцияКорневыхТипов.Добавить(Единственное);
		КонецЕсли;
	КонецЦикла;
	//КоллекцияКорневыхТипов.Добавить("Константа"); // Константы
	ИндикаторТипов = ЛксПолучитьИндикаторПроцесса(КоллекцияКорневыхТипов.Количество(), "Все типы");
	Для Каждого КорневойТип Из КоллекцияКорневыхТипов Цикл
		ЛксОбработатьИндикатор(ИндикаторТипов);
		...
		Если КоллекцияМетаданных.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		ПредставлениеТипаТаблицы = ЛксПолучитьПредставлениеИзИдентификатора(МножественноеКорневогоТипа);
		ИндикаторТипа = ЛксПолучитьИндикаторПроцесса(КоллекцияМетаданных.Количество(), "Анализ " + КорневойТип);
		Для Каждого МетаИсточник Из КоллекцияМетаданных Цикл
			ЛксОбработатьИндикатор(ИндикаторТипа);
...
Процедура РеквизитыТаблицаПриИзмененииФлажка(Элемент, Колонка)
	...
	ЛксИнтерактивноЗаписатьВКолонкуТабличногоПоля(Элемент, Колонка, Не Элемент.ТекущаяСтрока[Колонка.Данные]);
...
Процедура ОсновныеДействияФормыНайтиСсылкиНаОбъект(Кнопка)
	...
	Если ТипЗнч(ВыбОбъект) = Тип("Строка") Тогда 
		Возврат;
	КонецЕсли; 
	Обработки.ирПоискСсылокНаОбъект.Создать().НайтиИПоказатьСсылки(ВыбОбъект);
...
Процедура ОсновныеДействияФормыМенеджерТабличногоПоля(Кнопка)
	 ...
	 Обработки.ирМенеджерТабличногоПоля.ПолучитьФорму(, ЭтаФорма, ).УстановитьСвязь(ТабличноеПоле);
		...
		//Если МассивУзлов.Количество() > 0 Тогда
			Для каждого Узел Из МассивУзлов Цикл
				...
				Если СтрокаУзла = Неопределено Тогда
					...
					СтрокаУзла.ИндексКартинки = ЛксПолучитьИндексКартинкиСсылки(Узел);
				КонецЕсли;
			КонецЦикла;
		//КонецЕсли;
	КонецЦикла;
...
Процедура ДеревоУзловПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Иначе
		Ячейка.УстановитьКартинку(БиблиотекаКартинок.ирСостояниеСсылки);
		Ячейка.ИндексКартинки = ДанныеСтроки.ИндексКартинки;
...
Процедура ОсновныеДействияФормыЖурналРегистрации(Кнопка)
	...
	Обработки.ирАнализЖурналаРегистрации.Создать().ОткрытьСПараметром("Данные", ВыбОбъект);
...
Процедура СвязанныеДанныеПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ТипТаблицы = ЛксПолучитьПервыйФрагмент(ДанныеСтроки.ТипТаблицы);
...
Процедура СвязанныеДанныеВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ВыбраннаяСтрока.ТипТаблицы = "Константа" Тогда
		...
		ТекущаяСтрока = ЛксПолучитьПоследнийФрагмент(ВыбраннаяСтрока.ИмяТаблицы);
		ФормаСписка.НачальноеЗначениеВыбора = ТекущаяСтрока;
		ФормаСписка.Открыть();
	ИначеЕсли Истина
		И Не ЛксЛиТипВложеннойТаблицыБД(ВыбраннаяСтрока.ТипТаблицы)
...
Процедура ОсновныеДействияФормыНайтиВДинамическомСписке(Кнопка)
	...
	ФормаСписка = Обработки.ирДинамическийСписок.ПолучитьФорму(,, Новый УникальныйИдентификатор);
...
Процедура ПоказатьЯчейкуДанныхОбъекта(ПолноеИмяТаблицы = "", ИмяКолонки = "", КлючСтроки = Неопределено) Экспорт
	...
	ИмяТаблицы = ЛксПолучитьПоследнийФрагмент(ПолноеИмяТаблицы);
	...
	Если ЛксЛиКорневойТипСсылки(ТипТаблицы) Тогда 
		...
		КонецЕсли; 
	Иначе
		Если ЛксЛиТипВложеннойТаблицыБД(ТипТаблицы) Тогда 
			ЭтаФорма.ТекущийЭлемент = ЭтаФорма.ЭлементыФормы["ТабличнаяЧасть_" + ЛксПолучитьПоследнийФрагмент(ИмяТаблицы)];
		ИначеЕсли Истина
			И (Ложь
...
Процедура РеквизитыТаблицаЗначениеРеквизитаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Тогда
		СтандартнаяОбработка = Ложь;
		Исследовать(ЭлементыФормы.РеквизитыТаблица.ТекущаяСтрока.ОригинальноеЗначениеРеквизита);
	КонецЕсли; 
...
Процедура ОсновныеДействияФормыИзXML(Кнопка)
	...
	КонецЕсли;
	Текст = "";
	ирНеглобальный.ОткрытьФормуПроизвольногоЗначенияЛкс(Текст, ,, "Введите XML текст с данными объекта");
	ПрочитанныйОбъект = ирНеглобальный.ВосстановитьОбъектИзСтрокиXMLЛкс(Текст);
...
Процедура ОсновныеДействияФормыВыгрузитьВXML(Кнопка)
	...
	СериализованныйОбъект = ирНеглобальный.СохранитьОбъектВВидеСтрокиXMLЛкс(ПолучитьИзмененныйОбъект(), Истина);
	ирНеглобальный.ОткрытьФормуПроизвольногоЗначенияЛкс(СериализованныйОбъект);
...
РеквизитыТаблица.Колонки.Добавить("ЗначениеРеквизита", ЛксПолучитьОписаниеТиповВсеРедактируемыеТипы()); // Программно нужно добавлять, чтобы все типы можно было редактировать
...
Если ЛксЛиСсылкаНаОбъектБД(КлючУникальности) Тогда
