...
Процедура ТабличноеПолеТабличнойЧастиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Функция мДобавитьСтраницуТЧ(ИмяТЧ, ПредставлениеТЧ)
	...
	Если тчНаФорме.Значение.Колонки.Найти("НомерСтроки") <> Неопределено Тогда
		...
		тчНаФорме.УстановитьДействие("ПриВыводеСтроки", Новый Действие("ТабличноеПолеТабличнойЧастиПриВыводеСтроки"));
	КонецЕсли; 
	Возврат тчНаФорме;
...
Функция КнопкаЗаписатьНажатие(Элемент = Неопределено, РежимЗаписи = "")
	...
	Иначе
		...
		Если РежимЗаписи <> "ПометкаУдаления" Тогда
			НоваяСсылка = ЗаписатьОбъект(?(ТипЗнч(РежимЗаписи) = Тип("Строка"), РежимЗаписиДокумента.Запись, РежимЗаписи));
		КонецЕсли; 
		Если мЭтоДокумент Тогда
			Если ЛиЧитатьПисатьДвижения Тогда
...
Процедура ПослеВосстановленияЗначений()
	...
	КП_СтраницыДвижения1СкрыватьПустые();
...
Процедура СвязанныеДанныеВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ВыбраннаяСтрока.ТипТаблицы = "Константа" Тогда
		...
		ФормаСписка.Открыть();
	Иначе
		ирОбщий.ОткрытьФормуСпискаСОтбором(ВыбраннаяСтрока.ПолноеИмяТаблицы, Новый Структура(ВыбраннаяСтрока.ИмяКолонки, ВыбОбъект));
	КонецЕсли; 
...
Процедура КП_СтраницыДвижения1СкрыватьПустые(Элемент = Неопределено)
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	мЭтоСсылочныйОбъект = Ложь
		Или ирОбщий.ЛиКорневойТипОбъектаБДЛкс(ТипОсновнойТаблицы) 
		Или (Истина
	...
	ЭлементыФормы.ЛиЗаписьБезПроверок.Доступность = Не мЭтоПланОбмена;
	ОбновитьДеревоУзлов();
	ПодготовитьСтраницыДвижений(ВыбОбъект);
	//Попытка
...
Процедура НаборЗаписейПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
	...
	КонецЕсли; 
	Если Элемент.Значение.Колонки.Найти("НомерСтроки") <> Неопределено Тогда 
		ОформлениеСтроки.Ячейки.НомерСтроки.Значение = Элемент.Значение.Индекс(ДанныеСтроки) + 1;
	КонецЕсли; 
