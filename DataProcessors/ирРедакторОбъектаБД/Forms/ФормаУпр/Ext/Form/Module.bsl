...
Функция ПолучитьОсновнойОбъектИзБД(Знач СохранятьИдентификаторСсылки = Ложь, Знач ТолькоМакет = Ложь)
	...
	КонецЕсли; 
	ДанныеОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ИмяОсновнойТаблицы, КлючОбъекта, СохранятьИдентификаторСсылки, Не ТолькоМакет, ЗаписьНаСервере(), фОбъект.ИдентификаторСсылки);
	...
	Для Каждого РеквизитТаблицы Из ТаблицаОбъекта.ВыгрузитьКолонки().Колонки Цикл
		Если РеквизитТаблицы.ТипЗначения.СодержитТип(Тип("ХранилищеЗначения")) Тогда
			Для Каждого СтрокаТаблицыОбъекта Из ТаблицаОбъекта Цикл
				...
				СтрокаТаблицыФормы[РеквизитТаблицы.Имя + "_Хранилище"] = СтрокаТаблицыОбъекта[РеквизитТаблицы.Имя];
			КонецЦикла;
		КонецЕсли; 
	КонецЦикла;
...
Процедура ЗаполнитьДополнительныеКолонкиОбъектаИзТаблицыФормы(ТаблицаФормы, ТаблицаОбъекта)
	...
	//		Для Каждого СтрокаТаблицыОбъекта Из ТаблицаОбъекта Цикл
	//			СтрокаТаблицыФормы = ТаблицаФормы[ТаблицаОбъекта.Индекс(СтрокаТаблицыОбъекта)];
	//			СтрокаТаблицыОбъекта[РеквизитТаблицы.Имя] = СтрокаТаблицыФормы[РеквизитТаблицы.Имя + "_Хранилище"];
	//		КонецЦикла;
	//	КонецЕсли; 
	//КонецЦикла;
	...
	КонецЦикла;
	Для Каждого РеквизитТаблицы Из ТаблицаОбъекта.Колонки Цикл
		Если РеквизитТаблицы.ТипЗначения.СодержитТип(Тип("ХранилищеЗначения")) Тогда
			Для Каждого СтрокаТаблицыОбъекта Из ТаблицаОбъекта Цикл
				СтрокаТаблицыФормы = ТаблицаФормы[ТаблицаОбъекта.Индекс(СтрокаТаблицыОбъекта)];
				СтрокаТаблицыОбъекта[РеквизитТаблицы.Имя] = СтрокаТаблицыФормы[РеквизитТаблицы.Имя + "_Хранилище"];
...
Процедура ИмяОсновнойТаблицыПриИзменении()
			...
			Для Каждого МетаТЧ Из мМетаданныеОбъекта.ТабличныеЧасти Цикл
				Для Каждого МетаРеквизит Из МетаТЧ.Реквизиты Цикл
					Если МетаРеквизит.Тип.СодержитТип(Тип("ХранилищеЗначения")) Тогда
						...
						НовыйРеквизит = Новый РеквизитФормы(МетаРеквизит.Имя + "_Хранилище", Новый ОписаниеТипов, "СсылочныйОбъект." + МетаТЧ.Имя, МетаРеквизит.Представление(), Истина);
	...
	Иначе
		...
		Для Каждого ПолеТаблицы Из ПоляТаблицыБД() Цикл
			Если ПолеТаблицы.ТипЗначения.СодержитТип(Тип("ХранилищеЗначения")) Тогда
				...
				//НовыйРеквизит = Новый РеквизитФормы(ПолеТаблицы.Имя, Новый ОписаниеТипов, "НаборЗаписей", ПолеТаблицы.Заголовок, Истина);
				НовыйРеквизит = Новый РеквизитФормы(МетаРеквизит.Имя + "_Хранилище", Новый ОписаниеТипов, "НаборЗаписей", ПолеТаблицы.Заголовок, Истина);
				ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
			КонецЕсли; 
	...
	ЭлементыФормы.ОсновныеДействияФормы.ПодчиненныеЭлементы.ОткрытьОсновнуюФормуОбъекта.Доступность = фОбъект.ЭтоСсылочныйОбъект;
...
Процедура РеквизитыТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка = ЭлементыФормы.РеквизитыТаблица.ПодчиненныеЭлементы.РеквизитыТаблицаПодсказка И ЗначениеЗаполнено(ВыбраннаяСтрока.Подсказка) Тогда
