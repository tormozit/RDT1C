...
Процедура ЗаполнитьТаблицуОбъектаИзТаблицыФормыСДополнительнымиКолонками(ТаблицаФормы, ТаблицаОбъекта)
	...
	Если ОбъектМД <> Неопределено И ирОбщий.ПолучитьПервыйФрагментЛкс(ОбъектМД.ПолноеИмя()) = "РегистрБухгалтерии" Тогда
		ирОбщий.ЗаполнитьВидыСубконтоВТаблицеНабораЗаписейЛкс(ТаблицаДляЗагрузки, ОбъектМД);
	КонецЕсли; 
...
Функция ПолучитьКлючОбъекта(ПолныйКлюч = Ложь)
	...
	Иначе
		...
		Если ПолныйКлюч Тогда
			...
			КонецЕсли; 
		Иначе
			Результат = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ИмяОсновнойТаблицы, Результат,, Ложь, ОбъектыНаСервере());
		КонецЕсли; 
	КонецЕсли; 
	Возврат Результат;
