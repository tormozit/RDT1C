...
Функция ЗаполнитьСтрокуТаблицыРеквизиты(ЗначениеРеквизита, ИмяРеквизита, Представление = "", ОписаниеТипов = Неопределено, ТолькоПросмотр = Ложь, ДанныеИзБД = Истина, ПодсказкаРеквизита = "",
	...
	Если МетаРеквизит <> Неопределено Тогда
		СписокФункОпций = Новый СписокЗначений;
		ФункциональныеОпцииВключены = Истина;
		Для Каждого ФункциональнаяОпция Из Метаданные.ФункциональныеОпции Цикл
			Если ФункциональнаяОпция.Состав.Содержит(Метареквизит) Тогда
				ОпцияВключена = ПолучитьФункциональнуюОпцию(ФункциональнаяОпция.Имя);
				СписокФункОпций.Добавить(ФункциональнаяОпция.Имя, ФункциональнаяОпция.Имя + " = " + ОпцияВключена);
				Если Не ОпцияВключена Тогда
					ФункциональныеОпцииВключены = Ложь;
				КонецЕсли; 
			КонецЕсли; 
		КонецЦикла;
...
Процедура НастроитьЭлементыФормы(ЭтаФорма)
	...
	ЭлементыФормы.ОсновныеДействияФормы.ПодчиненныеЭлементы.Записать.Доступность = Ложь
...
Процедура ЗагрузитьТаблицыДвижений(Объект)
	...
	Для Каждого МетаРегистр из ОбъектыМД Цикл
		...
		ИмяТипаРегистра = ирОбщий.ПолучитьКорневойТипКонфигурацииЛкс(ПолноеИмяМДНабораЗаписей);
		...
		Если ИмяТипаРегистра = "Последовательность" Тогда
			НаборДвижений = НаборДвижений.Выгрузить();
		КонецЕсли; 
		ЗначениеВРеквизитФормы(НаборДвижений, ИмяТаблицыФормыРегистра);
...
Функция ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта, ДанныеИзБД = Истина) 
	...
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
		...
		Если СтараяТекущаяСтраницаТЧ <> Неопределено Тогда
			НоваяТекущаяСтраница = ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.Найти(СтараяТекущаяСтраницаТЧ);
...
&НаКлиенте
// Упр
Процедура ЗаполнитьДополнительныеКолонкиОбъектаИзТаблицыФормы(ТаблицаФормы, ТаблицаОбъекта)
	...
	Для Каждого СтрокаТаблицыФормы Из ТаблицаФормы Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаОбъекта.Добавить(), СтрокаТаблицыФормы); 
	КонецЦикла;
...
Функция ПолучитьИзмененныйОбъект(выхВерсияДанныхИзменена = Ложь)
	...
	Иначе
		...
		Если фОбъект.ЭтоСсылочныйОбъект Тогда
			...
			Для Каждого ЭлементТЧ Из СтруктураТЧ Цикл
				ЗаполнитьДополнительныеКолонкиОбъектаИзТаблицыФормы(ЭтаФорма.СсылочныйОбъект[ЭлементТЧ.Ключ], СтруктураОбъекта.Данные[ЭлементТЧ.Ключ]);
		...
		Иначе
			...
			//СтруктураОбъекта.Методы = РеквизитФормыВЗначение("НаборЗаписей");
			//СтруктураОбъекта.Данные = СтруктураОбъекта.Методы;
			ЗаполнитьДополнительныеКолонкиОбъектаИзТаблицыФормы(ЭтаФорма.НаборЗаписей, СтруктураОбъекта.Данные);
		КонецЕсли; 
...
Процедура ДеревоУзловПриИзмененииФлажка(Элемент, Колонка)
	...
	Иначе
		//Попытка
			ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(СтрокаУзла.УзелОбмена, КлючОбъекта.Методы, СтрокаУзла.Пометка, фОбъект.ЛиЧитатьПисатьДвижения, фОбъект.ЛиЧитатьПисатьДвижения);
		//Исключение
		//	ОписаниеОшибки = ОписаниеОшибки(); // Для отладки
		//КонецПопытки;
	КонецЕсли;
...
Функция ПолучитьКлючОбъекта(ПолныйКлюч = Ложь)
	...
	Иначе
		...
		//КонецЦикла;
		Результат = СтруктураОтбораНаборЗаписей();
		Если ПолныйКлюч Тогда
			Если ирОбщий.ПолучитьТипТаблицыБДЛкс(фОбъект.ИмяОсновнойТаблицы) = "РегистрСведений" И фОбъект.мРежимМенеджераЗаписи Тогда
				ПараметрыКонструктора = Новый Массив;
				ПараметрыКонструктора.Добавить(Результат);
				Результат = Новый (СтрЗаменить(фОбъект.ИмяОсновнойТаблицы, ".", "КлючЗаписи."), ПараметрыКонструктора);
			Иначе
				Сообщить("Получить ключ записи можно только после включения отбора по всем измерениям регистра");
				Возврат Неопределено;
			КонецЕсли; 
		Иначе
			Результат = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ИмяОсновнойТаблицы, Результат);
		КонецЕсли; 
	КонецЕсли; 
	Возврат Результат;
...
Процедура ОсновныеДействияФормыНайтиВСписке(Кнопка)
	...
	Иначе
		ирОбщий.ОткрытьФормуСпискаЛкс(фОбъект.ИмяОсновнойТаблицы,,,,,,,, СтруктураОтбораНаборЗаписей());
	КонецЕсли; 
...
Процедура ИмяОсновнойТаблицыПриИзменении()
	...
	ЭлементыФормы.ОсновныеДействияФормы.ПодчиненныеЭлементы.Записать.Доступность = фОбъект.мДоступноИзменение;
...
Процедура РеквизитыТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Ложь
		...
		Или Колонка = ЭлементыФормы.РеквизитыТаблица.ПодчиненныеЭлементы.РеквизитыТаблицаФункциональныеОпции
	Тогда 
		ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(Элемент, СтандартнаяОбработка);
	КонецЕсли; 
...
Процедура НадписьОтборНажатие(Элемент)
	...
	Если ВыбранноеЗначение <> Неопределено Тогда
		Объект = ирОбщий.ОбъектБДПоКлючуЛкс(фОбъект.ИмяОсновнойТаблицы, ВыбранноеЗначение);
