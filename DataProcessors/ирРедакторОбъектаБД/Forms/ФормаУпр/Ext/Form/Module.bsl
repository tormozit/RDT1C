...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КП_СтраницыДвиженийСкрыватьПустые(ЭлементыФормы.КП_СтраницыДвижения.ПодчиненныеЭлементы.ДвиженияСкрыватьПустые);
	КП_СтраницыТабличныхЧастейСкрыватьПустые(ЭлементыФормы.КП_СтраницыТабличныхЧастей.ПодчиненныеЭлементы.ТабличныеЧастиСкрыватьПустые);
...
Функция ПоляСИсториейВыбора()
	...
	Возврат Элементы.ИмяОсновнойТаблицы;
...
Процедура ОбновитьДоступность()
	...
	Если ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СтраницыПанельДвижения Тогда
	...
	АктивнаСтраницаИзменяемыхДанных = Ложь
		Или ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвойстваОбъекта
		Или ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СтраницыПанельДвижения;
...
Процедура УстановитьОтображениеЗакладок(Панель)
	...
	Если Панель.ПодчиненныеЭлементы.Количество() < 2 Тогда
		Панель.ОтображениеЗакладок = ОтображениеСтраницФормы.Нет;
	Иначе
		Панель.ПодчиненныеЭлементы.Удалить(0);
	КонецЕсли;
...
Процедура ПодготовитьСтраницыДвижений(Объект)
	...
	Для Каждого МетаРегистр из ОбъектыМД Цикл
		...
		НоваяСтраница = ПанельТипаМетаданных.ПодчиненныеЭлементы.Добавить(ИмяТаблицыРегистра, МетаРегистр.Представление());
...
Функция ПоместитьДанныеОбъектаВФорму(Объект, ДанныеИзБД = Истина) 
	...
	ОбновитьДоступность();
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.ИзмененияНаУзлах.Значение = Null;
	фОбъект.ПоискВОбъекте.Очистить();
	//ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвязанныеДанные.Значение = Null;
	...
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СсылкиНаОбъект.Значение = Null;
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СтраницыПанельДвижения.Видимость = фОбъект.ЭтоДокумент;;
	...
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
		...
		Иначе
			...
			Если БизнесПроцессы.ТипВсеСсылки().СодержитТип(ТипЗнч(фОбъект.ВыбОбъект)) Тогда
				ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.КартаМаршрута.Видимость = Истина;
				ЭлементыФормы.КартаМаршрута.УстановитьСхему(Объект.ПолучитьКартуМаршрута());
			Иначе
				ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.КартаМаршрута.Видимость = Ложь;
			КонецЕсли; 
		КонецЕсли; 
		ЗаполнитьИндексыКартинокТаблицыРеквизитов(Объект);
		РеквизитыТаблица.Сортировать("Порядок, ИмяРеквизита");
		ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица;
...
Процедура ПодготовитьСтраницыТабличныхЧастей(Знач Объект = Неопределено, Знач СтруктураТЧ = Неопределено)
	...
	Для Каждого ЭлементТЧ Из СтруктураТЧ Цикл
		...
		НоваяСтраница = ПанельТипаМетаданных.ПодчиненныеЭлементы.Добавить(СтрокаСтраницы.ИмяСтраницы, ПредставлениеТабличнойЧасти);
		ПанельТипаМетаданных.ТекущаяСтраница = НоваяСтраница;
...
Процедура ПодключитьОбработчикиСобытийПолейВводаКолонокТабличногоПоля(Знач ТабличноеПолеСДинамическимиКолонками)
	...
	Для Каждого КолонкаТП Из ТабличноеПолеСДинамическимиКолонками.Колонки Цикл
		Если КолонкаТП.ЭлементУправления <> Неопределено Тогда
			...
			КолонкаТП.ЭлементУправления.УстановитьДействие("ОкончаниеВводаТекста", "ПолеВвода_ОкончаниеВводаТекста");
			КолонкаТП.ЭлементУправления.УстановитьДействие("НачалоВыбора", "ПолеВводаКолонки_НачалоВыбора");
			КолонкаТП.ЭлементУправления.УстановитьДействие("ПриИзменении", "ПолеВводаКолонки_ПриИзменении");
		КонецЕсли; 
	КонецЦикла;
...
Процедура ЗаполнитьСвязанныеДанные()
	...
	ЭлементыФормы = Элементы;
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвязанныеДанные.Значение = Неопределено;
...
Процедура ПриОткрытии()
	...
	Если фОбъект.ПараметрИскомоеЗначение <> Неопределено Тогда
		...
		ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.ПоискВОбъекте;
		НайтиНажатие();
	КонецЕсли;
...
Функция КнопкаЗаписатьНажатие(Элемент = Неопределено, РежимЗаписи = "", ОбратимаяЗапись = Неопределено)
	...
	Если Истина
		...
		И ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СтраницыПанельДвижения
	Тогда
...
Функция ТекущееТабличноеПоле()
	...
	Если СтраницаОбъекта = ЭлементыФОрмы.ПанельОсновная.ПодчиненныеЭлементы.ТабличныйДокумент Тогда
		Возврат Неопределено;
	ИначеЕсли СтраницаОбъекта = ЭлементыФОрмы.ПанельОсновная.ПодчиненныеЭлементы.СвойстваОбъекта Тогда
		СтраницаТЧ = ЭлементыФормы.ПанельОбъект.ТекущаяСтраница;
		Если СтраницаТЧ = ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица Тогда
			Возврат ЭлементыФормы.РеквизитыТаблица;
		ИначеЕсли ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФОрмы.ПанельОбъект.ПодчиненныеЭлементы.СтраницыПанельТабличныеЧасти Тогда
		...
		ИначеЕсли ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФОрмы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей Тогда
			Возврат ЭлементыФормы.НаборЗаписей;
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	ИначеЕсли СтраницаОбъекта = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СтраницыПанельДвижения Тогда
...
Процедура ОсновныеДействияФормыВТаблицу(Кнопка)
	...
	Для Каждого СтрТаблицы ИЗ ТЗ Цикл
		...
		Стр = Стр+1;
	КонецЦикла;
	ЭлементыФОрмы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.ТабличныйДокумент;
...
Процедура ОсновныеДействияФормыИсследовать(Кнопка)
	...
	ЭлементыФормы = Элементы;
	Объект = ПолучитьИзмененныйОбъект();
	Если ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвойстваОбъекта Тогда 
		Если Объект <> Неопределено Тогда 
...
Процедура КП_СтраницыДвиженийСкрыватьПустые(Кнопка)
	...
	ЭлементыФормы.СтраницыПанельДвижения.ОтборСтрок = Новый ФиксированнаяСтруктура("Непустая", Истина);
...
Процедура ОтобратьСтраницыРегистров(Текст = Неопределено)
	...
	ТабличноеПоле.ОтборСтрок.Непустая.Использование = Не ТабличноеПоле.ОтборСтрок.Непустая.Использование; // Чтобы отбор точно наложился
	ТабличноеПоле.ОтборСтрок.Непустая.Использование = Не ТабличноеПоле.ОтборСтрок.Непустая.Использование; // Чтобы отбор точно наложился
...
Процедура ОбновитьПометкиДереваУзлов()
	...
	КонецЕсли; 
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.ИзмененияНаУзлах.Значение = Истина;
...
Процедура НайтиНажатие(Элемент = Неопределено)
	...
	Тогда
		...
		КонецЦикла; 
	Иначе
		НайтиЗначениеВТаблицеОбъекта(ЭлементыФормы.НаборЗаписей, ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей.Имя, фОбъект.ИскомоеЗначение);
	КонецЕсли; 
...
Процедура ИмяОсновнойТаблицыПриИзменении()
	...
	мПоляТаблицыБД = ПоляТаблицыБД();
	Если Не ЗначениеЗаполнено(фОбъект.ИмяОсновнойТаблицы) Тогда
		ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвойстваОбъекта;
	КонецЕсли; 
	//ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвойстваОбъекта.Видимость = Не фОбъект.ЭтоМетаСсылка;
	ЭлементыФОрмы.ПанельОсновная.ПодчиненныеЭлементы.ТабличныйДокумент.Видимость = Не фОбъект.ЭтоМетаСсылка;
	ЭлементыФОрмы.ПанельОсновная.ПодчиненныеЭлементы.ИзмененияНаУзлах.Видимость = Не фОбъект.ЭтоМетаСсылка;
	ЭлементыФОрмы.ПанельОсновная.ПодчиненныеЭлементы.ПоискВОбъекте.Видимость = Не фОбъект.ЭтоМетаСсылка;
	Если Ложь
		Или Не ЗначениеЗаполнено(фОбъект.ИмяОсновнойТаблицы) 
		Или фОбъект.ЭтоСсылочныйОбъект
	Тогда
		ЭлементыФормы.ПанельКлючОбъекта.ТекущаяСтраница = ЭлементыФормы.ПанельКлючОбъекта.ПодчиненныеЭлементы.Ссылка;
		ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица.Видимость = Истина;
		ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей.Видимость = Ложь;
		ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей.Видимость = Ложь;
		ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица;
		Если ЗначениеЗаполнено(фОбъект.ИмяОсновнойТаблицы) Тогда
	...
	ИначеЕсли фОбъект.ЭтоКонстанта Тогда
		ЭлементыФормы.ПанельКлючОбъекта.ТекущаяСтраница = ЭлементыФормы.ПанельКлючОбъекта.ПодчиненныеЭлементы.Константа;
		ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица.Видимость = Истина;
		ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей.Видимость = Ложь;
		ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица;
	Иначе
		ЭлементыФормы.ПанельКлючОбъекта.ТекущаяСтраница = ЭлементыФормы.ПанельКлючОбъекта.ПодчиненныеЭлементы.Отбор;
		//ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей.Видимость = Истина;
		//ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица.Видимость = Ложь;
		//ЭлементыФормы.ПанельОбъект.ТекущаяСтраница = ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей;
	...
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СвязанныеДанные.Видимость = фОбъект.ЭтоСсылочныйОбъект;
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СсылкиНаОбъект.Видимость = фОбъект.ЭтоСсылочныйОбъект;
...
Функция ПоляТаблицыБД()
	...
	Если Истина
		И фОбъект.мПолноеИмяМД <> Неопределено
		И Не фОбъект.ЭтоМетаСсылка
	Тогда
...
Процедура ПриИзмененииОтбораНабораЗаписей()
	...
	ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.РеквизитыТаблица.Видимость = фОбъект.мРежимМенеджераЗаписи;
	ЭлементыФормы.ПанельОбъект.ПодчиненныеЭлементы.НаборЗаписей.Видимость = Не фОбъект.мРежимМенеджераЗаписи;
	Если фОбъект.мРежимМенеджераЗаписи Тогда
...
Процедура КП_СсылкиНаОбъектОбновить(Кнопка)
	...
	ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СсылкиНаОбъект.Значение = Истина;
...
Процедура НайтиИПоказатьСсылкиВФорме() Экспорт 
	...
	ЭлементыФормы = Элементы;
	ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СсылкиНаОбъект;
