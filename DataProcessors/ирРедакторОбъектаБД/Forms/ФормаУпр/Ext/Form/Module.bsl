...
Процедура ЗаписатьРегистры(СсылкаДокумента, ОбратимаяЗапись = Ложь)
	...
	Для Каждого МетаРегистр из ОбъектыМД Цикл
		...
		НоваяТаблица = ирОбщий.ДанныеЭлементаФормыЛкс(ЭлементыФормы[ИмяТаблицыРегистра]);
		Для Каждого СтрокаНовойТаблицы Из НоваяТаблица Цикл
			ЗаполнитьЗначенияСвойств(СтруктураОбъекта.Данные.Добавить(), СтрокаНовойТаблицы); 
		КонецЦикла;
		ирОбщий.НаборЗаписейПослеЗагрузкиИзТаблицыЗначенийЛкс(СтруктураОбъекта.Методы);
		УстановитьДополнительныеСвойстваОбъекта(СтруктураОбъекта);
