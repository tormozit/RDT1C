...
Процедура ЗаполнитьТаблицуОбъектаИзТаблицыФормыСДополнительнымиКолонками(ТаблицаФормы, ТаблицаОбъекта)
	...
	//КонецЦикла;
		...
		#КонецЕсли
	// Промежуточная таблица нужна для добавления колонок видов субконто у регистров бухгалтерии
	ТаблицаДляЗагрузки = ТаблицаОбъекта.ВыгрузитьКолонки();
	Для Каждого СтрокаТаблицыФормы Из ТаблицаФормы Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаДляЗагрузки.Добавить(), СтрокаТаблицыФормы); 
	КонецЦикла;
	ОбъектМД = Метаданные.НайтиПоТипу(ирОбщий.ТипОбъектаБДЛкс(ТаблицаОбъекта));
	Если ОбъектМД <> Неопределено И ирОбщий.ПолучитьПервыйФрагментЛкс(ОбъектМД.ПолноеИмя()) = "РегистрБухгалтерии" Тогда
		ирОбщий.ЗаполнитьВидыСубконтоВТаблицеЛкс(ТаблицаДляЗагрузки, ОбъектМД);
	КонецЕсли; 
	ТаблицаОбъекта.Загрузить(ТаблицаДляЗагрузки);
...
Функция ПолучитьИзмененныйОбъект(выхВерсияДанныхИзменена = Ложь)
	...
	Иначе
		...
		Иначе
			...
			// Упр
			//ирОбщий.ЗагрузитьКоллекциюВНаборЗаписейЛкс(ЭтаФорма.НаборЗаписей, СтруктураОбъекта.Методы, (ЭтаФорма.НаборЗаписей) = Тип("ДанныеФормыКоллекция"));
			...
			//ирОбщий.НаборЗаписейПослеЗагрузкиИзТаблицыЗначенийЛкс(СтруктураОбъекта.Методы); //Теперь это делается в ирОбщий.ЗаписатьОбъектЛкс()
		КонецЕсли; 
...
Процедура ЗаписатьРегистры(СсылкаДокумента, ОбратимаяЗапись = Ложь)
	...
	Для Каждого МетаРегистр из ОбъектыМД Цикл
		...
		ирОбщий.ЗагрузитьКоллекциюВНаборЗаписейЛкс(ТаблицаНабораДвижений, СтруктураОбъекта.Методы, ТипЗнч(ТаблицаНабораДвижений) = Тип("ДанныеФормыКоллекция"));
		УстановитьДополнительныеСвойстваОбъекта(СтруктураОбъекта);
...
Процедура ОбновитьВерсииОбъекта()
	...
	Если ЭлементыФормы.ПанельОсновная.ПодчиненныеЭлементы.СтраницаВерсииОбъекта.Доступность Тогда
		ИсторияДанныхМоя = Вычислить("ИсторияДанных");
		#Если Сервер И Не Сервер Тогда
			ИсторияДанныхМоя = ИсторияДанных;
		#КонецЕсли
		фОбъект.ВерсииОбъекта.Загрузить(ИсторияДанныхМоя.ВыбратьВерсии(Новый Структура("Данные", ПолучитьКлючОбъекта(Истина)),,, 1000));
	КонецЕсли;
...
Процедура КоманднаяПанель_ВерсииСравнитьСПредыдущей(Кнопка)
	...
	Если ВыделеннаяВерсия = НомерПервойВерсии Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Предыдущая версия отсутствует!'"));
		Возврат;
	КонецЕсли; 
...
Процедура КоманднаяПанельВерсииПереключательВеденияИстории(Кнопка)
	...
	НовыйРежим = Не ЭлементыФормы.КоманднаяПанельВерсии.ПодчиненныеЭлементы.ПереключательВеденияИстории.Пометка;
	ИсторияДанныхМоя = Вычислить("ИсторияДанных");
	#Если Сервер И Не Сервер Тогда
		ИсторияДанныхМоя = ИсторияДанных;
	#КонецЕсли
