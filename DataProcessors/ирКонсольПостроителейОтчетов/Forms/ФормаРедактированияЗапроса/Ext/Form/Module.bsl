...
Процедура ЗаполнитьПредставления()
	...
	Для Каждого ДоступноеПоле Из ПостроительОтчетовВременный.ДоступныеПоля Цикл
		...
		Иначе
			НоваяСтрока.Представление = ирОбщий.ПолучитьПредставлениеИзИдентификатораЛкс(ДоступноеПоле.Имя);
...
Процедура ЗаполнитьПараметры(ЗаменятьТипы = Ложь)
	...
	Для каждого ПараметрЗапроса Из ПараметрыЗапроса Цикл
		...
		Тогда 
			СтрокаПараметров.ПредставлениеПараметра = ирОбщий.ПолучитьПредставлениеИзИдентификатораЛкс(ИмяПараметра);
		КонецЕсли;
...
Процедура ПриОткрытии()
	...
	// +++.КЛАСС.ПолеТекстовогоДокументаСКонтекстнойПодсказкой
	ПолеТекстовогоДокументаСКонтекстнойПодсказкой = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой");
	#Если Сервер И Не Сервер Тогда
	    ПолеТекстовогоДокументаСКонтекстнойПодсказкой = Обработки.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой.Создать();
	#КонецЕсли
...
Процедура ПараметрыИмяПараметраПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ЭлементыФормы.Параметры.ТекущиеДанные.ПредставлениеПараметра) Тогда
		ЭлементыФормы.Параметры.ТекущиеДанные.ПредставлениеПараметра = ирОбщий.ПолучитьПредставлениеИзИдентификатораЛкс(ЭлементыФормы.Параметры.ТекущиеДанные.ИмяПараметра);
	КонецЕсли;
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирКонсольПостроителейОтчетов.Форма.ФормаРедактированияЗапроса");
