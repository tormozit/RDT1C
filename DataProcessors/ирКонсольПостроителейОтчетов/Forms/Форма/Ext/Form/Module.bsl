...
Процедура СкопироватьПараметрыСтроки(ИсходнаяСтрока, КонечнаяСрока)
	...
	КонечнаяСрока.ПредставленияДляИмен = ЛксПолучитьКопиюОбъекта(ИсходнаяСтрока.ПредставленияДляИмен);
	КонечнаяСрока.ФорматыДляИмен = ЛксПолучитьКопиюОбъекта(ИсходнаяСтрока.ФорматыДляИмен);
	...
	КонечнаяСрока.Макет = ЛксПолучитьКопиюОбъекта(ИсходнаяСтрока.Макет);
	КонечнаяСрока.НастройкиПостроителя = ЛксПолучитьКопиюОбъекта(ИсходнаяСтрока.НастройкиПостроителя);
...
Функция ПолучитьТекстНастроекПостроителя()
	...
	Для Каждого Параметр Из ПостроительОтчетов.Параметры Цикл
		...
		Тогда 
			ПредставлениеПараметра = ЛксПолучитьПредставлениеИзИдентификатора(Параметр.Ключ);
		Иначе
...
Функция ВыполнитьЗапрос(Куда)
	...
	Если СтраницаДляВывода = 0 Тогда
		...
		Если ЛиМинимальнаяШирина Тогда
			ЛксРассчитатьИУстановитьШиринуКолонок(ЭлементыФормы.РезультатТабДокСвод, Истина, Ложь, 1);
		КонецЕсли;
	...
	Если ЛиЗамерВремени Тогда
		ЛксСообщитьСУчетомМодальности("Время формирования результата - " + Строка(ирКэш.Получить().ПолучитьТекущееВремяВМиллисекундах() - НачалоВыполнения)
			+ " мс", МодальныйРежим);
	КонецЕсли;
		...
		Если ОтработыватьРасшифровкуПоУмолчанию И ТекущаяСтрока.ОтчетРасшифровки <> Неопределено Тогда
			...
			Если ВыбранныйОтчет <> Неопределено Тогда
				Если ВыбранныйОтчет = ТекущаяСтрока Тогда
					...
					ЛксУстановитьОтборПостроителяПриРасшифровке(ПостроительОтчетов, Расшифровка);
				Иначе
		...
		Если ТипЗнч(ТекущаяСтрока.НастройкиПостроителя) = Тип("ТаблицаЗначений") Тогда
			Для Каждого Тн ИЗ ТекущаяСтрока.НастройкиПостроителя Цикл
				Если ПоследняяОткрытаяНастройка = Тн.Имя ИЛИ 
					...
					Тн.Макет = ЛксПолучитьКопиюОбъекта(ФиксированныйМакет);
			...
			Для Каждого Тн ИЗ ТекущаяСтрока.НастройкиПостроителя Цикл
				...
				Если Найдено Тогда
					...
					Если ФормаСохраненияНастроек.НаименованиеНастройки = Тн.Имя Тогда
						...
						ДС.Макет = ЛксПолучитьКопиюОбъекта(ФиксированныйМакет);
					...
					Иначе
						...
						ДС.НастройкиПостроителя = ЛксПолучитьКопиюОбъекта(Тн.НастройкиПостроителя);
						ДС.Макет = ЛксПолучитьКопиюОбъекта(Тн.Макет);
...
Процедура ДеревоЗапросовПередНачаломИзменения(Элемент, Отказ)
	...
	ФормаРедактирования.ПредставленияДляИмен = ЛксПолучитьКопиюОбъекта(Элемент.ТекущаяСтрока.ПредставленияДляИмен);
	ФормаРедактирования.ФорматыДляИмен = ЛксПолучитьКопиюОбъекта(Элемент.ТекущаяСтрока.ФорматыДляИмен);
	ФормаРедактирования.Параметры = ЛксПолучитьКопиюОбъекта(Элемент.ТекущаяСтрока.ПараметрыЗапроса);
...
Процедура ПриОткрытии()
	...
	Если ПостроительОтчетов.Текст <> "" Тогда
		...
		Для Каждого ЭлементПараметра Из ПостроительОтчетов.Параметры Цикл
			...
			СтрокаПараметра.ПредставлениеПараметра = ЛксПолучитьПредставлениеИзИдентификатора(ЭлементПараметра.Ключ);
			СтрокаПараметра.ТипЗначения = Новый ОписаниеТипов(ЛксБыстрыйМассив(ТипЗнч(ЭлементПараметра.Значение)));
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	...
	ИначеЕсли ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		Если ЗначениеВыбора.Свойство("РедактированиеЗапроса") Тогда
			...
			Элемент.ТекущаяСтрока.ПараметрыЗапроса = ЛксПолучитьКопиюОбъекта(ФормаРедактирования.Параметры);
			Элемент.ТекущаяСтрока.ПредставленияДляИмен = ЛксПолучитьКопиюОбъекта(ФормаРедактирования.ПредставленияДляИмен);
			Элемент.ТекущаяСтрока.ФорматыДляИмен = ЛксПолучитьКопиюОбъекта(ФормаРедактирования.ФорматыДляИмен);
		...
		ИначеЕсли ЗначениеВыбора.Свойство("РедактированиеНовогоЗапроса") Тогда
			...
			НоваяСтрока.ПараметрыЗапроса = ЛксПолучитьКопиюОбъекта(ФормаРедактирования.Параметры);
			НоваяСтрока.ПредставленияДляИмен = ЛксПолучитьКопиюОбъекта(ФормаРедактирования.ПредставленияДляИмен);
			НоваяСтрока.ФорматыДляИмен = ЛксПолучитьКопиюОбъекта(ФормаРедактирования.ФорматыДляИмен);
			...
			Если ЗначениеВыбора.Копирование Тогда
				...
				НоваяСтрока.Макет = ЛксПолучитьКопиюОбъекта(СтрокаОригинал.Макет);
				НоваяСтрока.НастройкиПостроителя = ЛксПолучитьКопиюОбъекта(СтрокаОригинал.НастройкиПостроителя);
...
Процедура КоманднаяПанельФормыИсполняемыйЗапрос(Кнопка)
	...
	Обработки.ирКонсольЗапросов.Создать().ОткрытьПоПостроителю(ПостроительОтчетов);
...
Процедура КоманднаяПанельФормыСравнить(Кнопка)
	...
	ЛксСравнитьСодержимоеЭлементаУправления(СравнительТабличныхДокументов, ЭлементыФормы.РезультатТабДокСвод);
...
Процедура ДеревоЗапросовНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	СохранитьТекущиеНастройкиВСтроку();
	ЛксДеревоКонсолиНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение, "СтрокаДереваПостроителей");
	
КонецПроцедуры

Процедура ДеревоЗапросовПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	
	ЛксДеревоКонсолиПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка, "СтрокаДереваПостроителей");
	
КонецПроцедуры

Процедура ДеревоЗапросовПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	
	ЛксДеревоКонсолиПеретаскивание(ЭтаФорма, Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка, "СтрокаДереваПостроителей");
