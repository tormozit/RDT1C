...
Перем ПараметрыВыполненияВФоне;
...
Процедура ОбновитьТаблицуЖурнала() Экспорт
	...
	ПараметрыВыполненияВФоне = ЗагрузитьДанныеЖурнала(ЭтаФорма, ЭлементыФормы.ДействияФормы.Кнопки.Обновить);
	Если ПараметрыВыполненияВФоне.Загружено Тогда
...
Процедура ВыгрузкаЗавершитьВФорме(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	...
	Если ЭлементыФормы.ТаблицаЖурнала.ТекущаяСтрока <> Неопределено Тогда
		...
		КонецЦикла;
	КонецЕсли;
	ПараметрыВыполненияВФоне.Вставить("Результат", РезультатЗадания);
	ЗавершитьВыгрузкуЖурнала(ПараметрыВыполненияВФоне, СостояниеЗадания);
	Если Ложь
