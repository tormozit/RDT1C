...
Перем ПоследнийЗагруженныйВариантОтбора;
...
Процедура КоманднаяПанельОтборСохранитьВариантОтбора(Кнопка)
	...
	Для Каждого ВариантОтбора Из ПользовательскиеВариантыОтбора Цикл
		НовыйЭлементСписка = СписокВариантов.Добавить(ИндексВарианта, ВариантОтбора.Представление);
		ИндексВарианта = ИндексВарианта + 1;
		Если ЗначениеЗаполнено(ПоследнийЗагруженныйВариантОтбора) И ПоследнийЗагруженныйВариантОтбора = ВариантОтбора.Представление Тогда
			НачальныйВыбор = НовыйЭлементСписка;
		КонецЕсли; 
	КонецЦикла;
	Если НачальныйВыбор = Неопределено Тогда
		НачальныйВыбор = СписокВариантов[0];
	КонецЕсли; 
	СписокВариантов.СортироватьПоПредставлению();
	ВыбранныйВариант = СписокВариантов.ВыбратьЭлемент("Выберите вариант отбора для перезаписи", НачальныйВыбор);
	...
	Если ВыбранныйВариант.Значение = "<Новый>" Тогда
		...
		Для Каждого ЭлементСписка Из СписокВариантов Цикл
			Если ЭлементСписка.Представление = ИмяВарианта Тогда
				Сообщить("Вариант отбора с таким именем уже существует");
				Возврат;
			КонецЕсли; 
		КонецЦикла;
		ВыбранныйВариант = ПользовательскиеВариантыОтбора.Добавить();
		ВыбранныйВариант.Представление = ИмяВарианта;
		ПоследнийЗагруженныйВариантОтбора = ВыбранныйВариант.Представление;
	КонецЕсли; 
...
Процедура КоманднаяПанельОтборЗагрузитьВариантОтбора(Кнопка)
	...
	СписокВариантов = Новый СписокЗначений;
	Для Каждого ВариантОтбора Из ПользовательскиеВариантыОтбора Цикл
		ВариантОтбора.Представление = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ПользовательскиеВариантыОтбора, ВариантОтбора, "Представление", Ложь,, 200);
		НовыйЭлементСписка = СписокВариантов.Добавить(ВариантОтбора.Значение, ВариантОтбора.Представление);
		Если ЗначениеЗаполнено(ПоследнийЗагруженныйВариантОтбора) И ПоследнийЗагруженныйВариантОтбора = ВариантОтбора.Представление Тогда
			НачальныйВыбор = НовыйЭлементСписка;
		КонецЕсли; 
	КонецЦикла;
	...
	КонецЕсли; 
	СписокВариантов.СортироватьПоПредставлению();
	ВыбранныйВариант = СписокВариантов.ВыбратьЭлемент("Выберите вариант отбора для загрузки", НачальныйВыбор);
	Если ВыбранныйВариант = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ПоследнийЗагруженныйВариантОтбора = ВыбранныйВариант.Представление;
