...
Процедура ТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если СтрокаТаблицыЗначений <> Неопределено Тогда
		ЛксОформитьЯчейкуСРасширеннымЗначением(ОформлениеСтроки.Ячейки.Метаданные, СтрокаТаблицыЗначений.Метаданные, Элемент.Колонки.Метаданные);
		ирНеглобальный.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, СтрокаТаблицыЗначений);
	КонецЕсли; 
...
Процедура ОбновитьТаблицуЖурнала() Экспорт
	...
	Если АнализироватьТранзакцииСУчастиемОбъекта Тогда
		...
		Индикатор = ЛксПолучитьИндикаторПроцесса(Транзакции.Количество(), "Выгрузка журнала по транзакциям");
		ФильтрТранзакции = ЛксСкопироватьУниверсальнуюКоллекцию(Фильтр);
		Для Каждого Транзакция Из Транзакции Цикл
			ЛксОбработатьИндикатор(Индикатор);
			//ФильтрТранзакции.Вставить("Транзакция", ЛксПолучитьСтрокуМеждуМаркерами(Транзакция, "(",")"));
			...
			Иначе
				ЛксЗагрузитьВТаблицуЗначений(ТаблицаТранзакции, ТаблицаЗначенийЖурнала);
			КонецЕсли; 
...
Процедура КоманднаяПанельЖурналРегистрацииКонсольКомпоновки(Кнопка)
    ...
    Отчеты.ирКонсольКомпоновокДанных.Создать().ОткрытьПоТаблицеЗначений(ТаблицаЖурнала.Выгрузить());
