...
Функция ЗагрузитьДанныеЖурнала(ЭтаФорма = Неопределено, Кнопка = Неопределено) Экспорт 
	...
	Результат = Новый Структура("НачалоВыполнения, ПредставлениеОтбора, Загружено", НачалоВыполнения, ПредставлениеОтбора, Ложь);
	Если АнализироватьТранзакцииСУчастиемОбъекта Тогда
		...
		ЗавершитьВыгрузкуЖурнала(Результат);
	Иначе
		ПараметрыЗадания = Новый Массив;
		ПараметрыЗадания.Добавить(Фильтр);
		ПараметрыЗадания.Добавить(МаксимальныйРазмерВыгрузкиТекущий);
		Если Ложь
			Или ЭтаФорма = Неопределено
			Или Не ирОбщий.ЛиАсинхронностьДоступнаЛкс()
			Или ЗначениеЗаполнено(ИмяФайла) 
		Тогда
			ЭтотОбъект.ТаблицаЗначенийЖурнала = Новый ТаблицаЗначений;
			#Если Клиент Тогда
			Состояние("Выгрузка из журнала регистрации...");
			#КонецЕсли
		...
		Иначе
			...
			#Если Сервер И Не Сервер Тогда
				ирСервер.ВыгрузитьЖурналРегистрацииЛкс();
			#КонецЕсли
			ОписаниеЗадания = ирОбщий.ОписаниеФоновогоЗаданияФормыЛкс("Выгрузка", "ирСервер.ВыгрузитьЖурналРегистрацииЛкс", ПараметрыЗадания, "Отбор - " + ПредставлениеОтбора, 
				Кнопка, "ВыгрузкаЗавершитьВФорме", АдресРезультата);
...
Процедура ЗавершитьВыгрузкуЖурнала(ПараметрыВыполнения, Знач СостояниеЗадания = Неопределено) Экспорт 
	...
	Тогда
		Если СостояниеЗадания = Неопределено Тогда
			#Если Клиент Тогда
				Состояние("Загрузка выгрузки журнала регистрации в форму...");
			#КонецЕсли
		КонецЕсли; 
		Если ЗначениеЗаполнено(СостояниеЗадания) Тогда
			ЭтотОбъект.ТаблицаЗначенийЖурнала = ПараметрыВыполнения.Результат;
		КонецЕсли; 
		#Если Сервер И Не Сервер Тогда
	...
	Если СостояниеЗадания = Неопределено Тогда 
		...
		ДлительностьСекунд = КонецВыполнения - ПараметрыВыполнения.НачалоВыполнения;
		Если ВыводитьДлительностьВыгрузки Или ДлительностьСекунд > 5 Тогда
			...
			ирОбщий.СообщитьЛкс("Выгрузка данных журнала завершена через " + ДлительностьСтрока + ". Отбор - " + ПараметрыВыполнения.ПредставлениеОтбора);
		КонецЕсли;
	КонецЕсли; 
