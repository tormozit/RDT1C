...
Перем ПредыдущиеСтроки; // Нужна для обработки ожидания при генерации контекстного меню
...
Процедура СгенерироватьКомандыКонтекстногоМеню(ТабличноеПоле) Экспорт 
	...
	Если ТекущаяСтрока = ПредыдущиеСтроки[ТабличноеПоле.Имя] Тогда
		Возврат;
	КонецЕсли;
	ПредыдущиеСтроки[ТабличноеПоле.Имя] = ТекущаяСтрока; 
...
Функция ДобавитьСтрокуВСтатическуюВетку(СтрокаДерева, РежимЗаполненияИзбранного = Ложь) Экспорт 
	...
	Если Открыта() Тогда
		...
		Иначе
			ЗаполнитьСтатическиеВеткиОтложенно();
...
Процедура ЗаполнитьСтатическиеВеткиОтложенно()

	#Если Сервер И Не Сервер Тогда
		ЗаполнитьСтатическиеВетки();
	#КонецЕсли
	ПодключитьОбработчикОжидания("ЗаполнитьСтатическиеВетки", 0.1, Истина);
...
Процедура ЗаполнитьСтатическиеВеткиДереваИнтерфейса(РежимЗаполненияИзбранного = Неопределено, Знач УстановитьТекущуюСтроку = Неопределено) Экспорт
	...
	тзСтатистикаИспользования.Колонки.Добавить("СтрокаДерева");
	...
	Для каждого СтатическаяВетка Из МассивСтатическихВеток Цикл
		...
		КонецЕсли; 
	КонецЦикла;
	тзСтатистикаИспользования.Колонки.Удалить("СтрокаДерева");
...
Процедура ЗаполнитьДеревоВсехОбъектов() Экспорт
	...
	Если ирКэш.НомерВерсииПлатформыЛкс() >= 802014 Тогда
		Для Каждого МетаВнешнийИсточникДанных Из Метаданные.ВнешниеИсточникиДанных Цикл
			...
			ДобавитьВетвьМетаданных(МетаВнешнийИсточникДанных.Таблицы, ПредставлениеВетви, 64, 65, Истина);
		КонецЦикла;
	КонецЕсли; 
...
Процедура ПоискПоМетаданным()
	...
	Для каждого стрКорень из дзВсеОбъекты.Строки цикл
		...
		Для каждого стрОбъект из стрКорень.Строки цикл
			Если НЕ ОбъектПроходитФильтры(стрОбъект,, ВеткиМетаданных.НайтиПоЗначению(стрКорень.Имя) <> Неопределено, ОбъектыВыбранныхПодсистем, СловаФильтра) тогда
				Продолжить;
			КонецЕсли;
			Если СтрокиКорня = Неопределено тогда
				...
				стрКореньИнтерфейса.Имя = стрКорень.Имя;
...
Процедура ИзбранноеПередУдалением(Элемент, Отказ)
	...
	Для Каждого СтрокаДляУдаления Из Элемент.ВыделенныеСтроки Цикл
		...
		Если СтрокаСтатистики.Количество() > 0 Тогда 
			...
			Если Не СтрокаСтатистики[0].Избранное Тогда
