...
//
Процедура ДобавитьКнопку(КП, Имя, Синоним = "", Команда, Действие, Картинка = Неопределено, Подсказка = "")
		...
		Попытка
			...
			Тогда
				...
				ИначеЕсли Кнопка.Имя = "СправкаМетаданного" Тогда
					НоваяФорма = Неопределено;
					ОткрытьСправку(Метаданные.НайтиПоПолномуИмени(СтрокаПолногоИмени));
...
Функция ФункциональныеОпцииОбъекта(ПолноеИмяМД)
	...
	Для Каждого МетаФункциональнаяОпция Из Метаданные.ФункциональныеОпции Цикл
		Если МетаФункциональнаяОпция.Состав.Содержит(ОбъектМД) Тогда
			СписокОпций.Добавить(МетаФункциональнаяОпция.Имя);
		КонецЕсли; 
	КонецЦикла;
	ФормаСписка = ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(СписокОпций, Ложь);
	Возврат ФормаСписка;
