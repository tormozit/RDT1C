...
Процедура СгенерироватьКомандыКонтекстногоМеню()
	...
	ИначеЕсли ЗначениеЗаполнено(ВидОбъекта) И ЗначениеЗаполнено(ИмяОбъекта) Тогда
		...
		КорневойТип = ЛксПолучитьКорневойТипКонфигурации(МДОбъект);
		Если ЛксЛиКорневойТипТаблицыБД(КорневойТип) Тогда
		...
		Попытка
			...
			Тогда
				...
				ИначеЕсли Кнопка.Имя = "ДинамическийСписок" Тогда
					НоваяФорма = Обработки.ирДинамическийСписок.ПолучитьФорму(,, СтрокаПолногоИмени);
				ИначеЕсли Кнопка.Имя = "КонсольЗапросов" Тогда
					НоваяФорма = Обработки.ирКонсольЗапросов.Создать().ОткрытьПоОбъектуМетаданных(МДОбъект.ПолноеИмя());
				ИначеЕсли Кнопка.Имя = "КонсольКомпоновки" Тогда
					НоваяФорма = Отчеты.ирКонсольКомпоновокДанных.Создать().ОткрытьПоОбъектуМетаданных(МДОбъект.ПолноеИмя());
				ИначеЕсли Кнопка.Имя = "ПодборИОбработкаОбъектов" Тогда
					НоваяФорма = Обработки.ирПодборИОбработкаОбъектов.ПолучитьФорму(,, МДОбъект.ПолноеИмя());
				ИначеЕсли Кнопка.Имя = "ЖурналРегистрации" Тогда
					НоваяФорма = Обработки.ирАнализЖурналаРегистрации.Создать().ОткрытьСПараметром("Метаданные", СтрокаПолногоИмени);
				Иначе
...
Процедура КомандыПанелиПечать(Кнопка)
	...
	Обработки.ирИсследовательОбъектов.Создать().ИсследоватьКоллекцию(ДеревоИнтерфейса.Строки);
КонецПроцедуры
...
Функция ирУниверсальныйСписок(ВидОбъекта, ИмяОбъекта)
	...
	Возврат Обработки.ирДинамическийСписок.ПолучитьФорму(,, СоответствиеВидовМетаданныхДляПоискаПолногоИмени[ВидОбъекта]+"."+ИмяОбъекта)
КонецФункции
