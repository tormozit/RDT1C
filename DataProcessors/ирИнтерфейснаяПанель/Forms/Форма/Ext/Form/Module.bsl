	...
	Для каждого стрКорень из дзВсеОбъекты.Строки цикл
		...
		Для каждого стрОбъект из стрКорень.Строки цикл
			...
			Если ДочерниеСтроки = Неопределено тогда
				...
				ЗаполнитьЗначенияСвойств(СтрокаКорня, стрКорень, "Имя, Представление, ИндексКартинки, ЕстьДоступ"); 
				ДочерниеСтроки = СтрокаКорня.Строки;
			КонецЕсли;
			стрОбъектИнтерфейса = ДочерниеСтроки.Добавить();
			ЗаполнитьЗначенияСвойств(стрОбъектИнтерфейса, стрОбъект, "Вид, Имя, Представление, ИндексКартинки, ЕстьДоступ"); 
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
