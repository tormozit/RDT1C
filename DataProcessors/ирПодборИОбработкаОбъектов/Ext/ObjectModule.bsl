...
Процедура ОбработатьЭлементыОбъекта(ФормаОбработки, ТипТаблицы, СтруктураКлючаОбъекта, СтруктураКлючаПолная, СтрокаКлюча, СтрокиДляОбработки)
	...
	КонецЕсли; 
	//ЭтоРегистрБухгалтерии = ирНеглобальный.ЛиПолноеИмяРегистраБухгалтерииЛкс(ПолноеИмяТаблицыСтроки);
	...
	Попытка
		...
		КонецЦикла;
		//Если ЭтоРегистрБухгалтерии Тогда
			Если ТаблицаНабора <> Неопределено Тогда
				ОбъектДляЗаписи.Загрузить(ТаблицаНабора);
			КонецЕсли; 
		//КонецЕсли; 
		Если РежимОбходаДанных <> "КлючиОбъектов" Тогда
			Попытка
