...
Функция ОбработатьЭлементыОбъекта(ФормаОбработки, ТипТаблицы, СтруктураКлючаОбъекта, СтруктураКлючаПолная, СтрокаКлюча, СтрокиДляОбработки, ПараметрыОбработки)
	...
	Если ПообъектныеТранзакции Тогда
		НачатьТранзакцию();
	КонецЕсли; 
	Попытка
		Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(ТипТаблицы) Тогда
			...
			Иначе
				Если ТранзакцияАктивна() Тогда
					ирОбщий.ЗаблокироватьСсылкуВТранзакцииЛкс(СтрокаКлюча.Ссылка);
				КонецЕсли; 
				ОбъектДляОбработки = СтрокаКлюча.Ссылка.ПолучитьОбъект();
			КонецЕсли; 
			МассивОбъектов.Добавить(ОбъектДляОбработки);
		ИначеЕсли ирОбщий.ЛиКорневойТипКонстантыЛкс(ТипТаблицы) Тогда
			...
			Иначе
				Если ТранзакцияАктивна() Тогда
					ирОбщий.ЗаблокироватьКонстантуЛкс(ОбъектДляОбработки);
				КонецЕсли; 
		...
		Тогда
			...
			Если РежимОбходаДанных <> "КлючиОбъектов" Тогда
				Если ТранзакцияАктивна() Тогда
					ирОбщий.ЗаблокироватьНаборЗаписейПоОтборуЛкс(ОбъектДляОбработки);
				КонецЕсли; 
				ОбъектДляОбработки.Прочитать();
			КонецЕсли;
			Если РежимОбходаДанных = "Строки" Тогда
		...
		Если РежимОбходаДанных <> "КлючиОбъектов" Тогда
			Попытка
				лОбменДанными = ОбъектДляОбработки.ОбменДанными;
			Исключение
				лОбменДанными = Неопределено;
			КонецПопытки; 
			Если лОбменДанными <> Неопределено Тогда
				лОбменДанными.Загрузка = ОтключатьКонтрольЗаписи;
				лОбменДанными.Получатели.Автозаполнение = Не БезАвторегистрацииИзменений;
			КонецЕсли;
		КонецЕсли; 
...
Функция ПолучитьОписаниеТиповОбрабатываемогоЭлемента(ИскомыйОбъект) Экспорт
	...
	ИначеЕсли ирОбщий.ЛиКорневойТипКонстантыЛкс(ТипТаблицы) Тогда
		Для Каждого ОбъектМД Из МассивОбъектовМД Цикл
			МассивТипов.Добавить(Тип(СтрЗаменить(ОбъектМД.ПолноеИмя(), ".", "МенеджерЗначения.")));
		КонецЦикла;
	ИначеЕсли ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(ТипТаблицы) Тогда
		Для Каждого ОбъектМД Из МассивОбъектовМД Цикл
