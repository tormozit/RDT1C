...
Функция ВыполнитьГрупповуюОбработку(ПараметрыОбработки) Экспорт
	...
	Попытка
		...
		Если КоличествоСтрок > 0 Тогда
			...
			ирОбщий.СообщитьЛкс("Обработано строк успешно - " + СтрокиУспешных.Количество() + ", неуспешно - " + КоличествоНеуспешно + ". "
				+ "Обработано объектов - " + КоличествоОбъектов + ", записано автоматически - " + мКоличествоЗаписанныхАвтоматически + ".");
		КонецЕсли; 
		Если СтруктураПотоков.ФактическоеКоличествоПотоков = 1 И ОбщаяТранзакция Тогда
	...
	Для каждого СтрокаРеквизита из ПараметрыОбработки.РеквизитыДляСохранения Цикл
		...
		Тогда
			...
			Если ЗначениеЗаполнено(ДопРеквизит) Тогда
				...
				КонецЕсли; 
			Иначе
				Если ирОбщий.ПрисвоитьЕслиНеРавноЛкс(Объект[СтрокаРеквизита.Идентификатор], НовоеЗначение) Тогда
