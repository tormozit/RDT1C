...
Функция ОбработатьЭлементыОбъекта(ФормаОбработки, ТипТаблицы, СтруктураКлючаОбъектаСПометкой, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, СтрокаКлюча, СтрокиДляОбработки, ПараметрыОбработки,
	...
	Попытка
		...
		Тогда
			...
			Иначе
				...
				ОбъектДляОбработки = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицыСтроки, СтрокаКлюча.Ссылка,,, ОбъектыНаСервере);
			КонецЕсли; 
		...
		ИначеЕсли ирОбщий.ЛиКорневойТипКонстантыЛкс(ТипТаблицы) Тогда
			...
			ОбъектДляОбработки = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицыСтроки,,,, ОбъектыНаСервере);
			Если РежимОбходаДанных = "КлючиОбъектов" Тогда
				//
			Иначе
		...
		ИначеЕсли ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(ТипТаблицы) Тогда
			ирОбщий.ЗаблокироватьСсылкуВТранзакцииЛкс(СтрокаКлюча.Ссылка, Истина);
			//ОбъектДляОбработки = СтрокаКлюча.Ссылка.ПолучитьОбъект();
			ОбъектДляОбработки = ирОбщий.ОбъектБДПоКлючуЛкс(ОбъектМДЗаписи.ПолноеИмя(), СтрокаКлюча.Ссылка,,, ОбъектыНаСервере);
			Если РежимОбходаДанных = "Строки" Тогда
		...
		Тогда
			...
			ОбъектДляОбработки = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицыСтроки, СтруктураКлючаОбъекта,,, ОбъектыНаСервере);
			Если РежимОбходаДанных <> "КлючиОбъектов" Тогда
		...
		Иначе
			...
			Тогда
				...
				Если Модифицированность Тогда
					...
					КонецЕсли;
					ирОбщий.ЗаписатьОбъектЛкс(ОбъектДляОбработки.Методы, ОбъектыНаСервере, РежимЗаписи);
				КонецЕсли;
			КонецЕсли; 
	...
	Тогда 
		...
		Если мСоставПланаОбмена.Содержит(ОбъектМД) Тогда
			ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзелОтбораОбъектов, ОбъектДляОбработки, Ложь, ОбъектыНаСервере);
		КонецЕсли; 
	КонецЕсли; 
