	...
	Попытка
		...
		Если ДинамическаяВыборка Тогда
			...
			Если СтруктураКлючаСтроки.Количество() = СтруктураКлючаОбъектаДоп.Количество() Тогда
				...
				Пока ВыборкаКлючей.Следующий() Цикл
					...
					СтрокиОбъекта = СтрокиДляОбработки.СкопироватьКолонки();
					ЗаполнитьЗначенияСвойств(СтрокиОбъекта.Добавить(), ВыборкаКлючей);
					...
					РезультатОбработки = ОбработатьЭлементыОбъекта(ТипТаблицы, СтруктураКлючаОбъектаДоп, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, ВыборкаКлючей,
						СтрокиОбъекта, ПараметрыОбработки, ТранзакцииРазрешены, Индикатор.Счетчик = 1, Индикатор.Счетчик = КоличествоОбъектов, ИмяОбъектаОбработкиОбъекта, ИмяМетодаОбработкиОбъекта);
				КонецЦикла;
			Иначе
				...
				Пока ВыборкаКлючей.Следующий() Цикл
					...
					СтрокиОбъекта = СтрокиДляОбработки.СкопироватьКолонки();
					ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаРезультатаДеталей, СтрокиОбъекта);
					РезультатОбработки = ОбработатьЭлементыОбъекта(ТипТаблицы, СтруктураКлючаОбъектаДоп, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, ВыборкаКлючей,
						СтрокиОбъекта, ПараметрыОбработки, ТранзакцииРазрешены, ИндексКлюча = 0, ИндексКлюча = КоличествоОбъектов - 1, ИмяОбъектаОбработкиОбъекта, ИмяМетодаОбработкиОбъекта);
					КоличествоСтрок = КоличествоСтрок + СтрокиОбъекта.Количество();
		...
		Иначе
			...
			Для ИндексКлюча = 0 По КоличествоОбъектов - 1 Цикл
				...
				СтрокиДанных = СтрокиДляОбработки.НайтиСтроки(СтруктураКлючаОбъектаДоп);
				СтрокиОбъекта = СтрокиДляОбработки.Скопировать(СтрокиДанных);
	...
	Попытка
		...
		Если ОбъектДляОбработки = Неопределено Тогда
			...
			Если ВыводитьСообщения Тогда
				ирОбщий.СообщитьЛкс("Пропущен удаленный """ + СтрокаКлюча[ИмяПоляСсылка] + """");
			КонецЕсли;
		Иначе
