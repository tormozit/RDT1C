...
Процедура ВернутьПараметрыПослеОбработки(ОбщиеПараметрыОбработки, ВладелецФормы) Экспорт
	...
	ирОбщий.ОповеститьФормыПодсистемыЛкс("ЗаписанОбъект", , ВладелецФормы);
...
Функция ОбработатьЭлементыОбъекта(ПараметрыОбработкиОбъекта) Экспорт 
	...
	Если ПрименятьПообъектныеТранзакции Тогда
		Если ТипТаблицы = "Внешняя" Тогда
			ВнешниеИсточникиДанных[ОбъектМДЗаписи.Родитель().Имя].НачатьТранзакцию();
		Иначе
			НачатьТранзакцию();
		КонецЕсли; 
	КонецЕсли; 
	Попытка
		...
		Если ПрименятьПообъектныеТранзакции Тогда
			Если ТипТаблицы = "Внешняя" Тогда
				ВнешниеИсточникиДанных[ОбъектМДЗаписи.Родитель().Имя].ЗафиксироватьТранзакцию();
			Иначе
				ЗафиксироватьТранзакцию();
			КонецЕсли; 
		КонецЕсли; 
	Исключение
		Если ПрименятьПообъектныеТранзакции Тогда
			Если ТипТаблицы = "Внешняя" Тогда
				ВнешниеИсточникиДанных[ОбъектМДЗаписи.Родитель().Имя].ОтменитьТранзакцию();
			Иначе
				ОтменитьТранзакцию();
			КонецЕсли; 
		КонецЕсли; 
...
Функция ПеренумерацияОбъектов(НастройкаОбработки, ТипНомера, ДлинаНомера) Экспорт 
