...
Процедура вОбработатьОбъект(Знач Объект, КоллекцияСтрок = Неопределено, Параметры = Неопределено) Экспорт
	...
	ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(Узел, Объект.Ссылка, НовоеЗначениеРегистрации, ВместеСДвижениями);
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирПодборИОбработкаОбъектов.Форма.ИзменитьРегистрирациюНаУзле");
мИспользоватьНастройки = Истина;

//Реквизиты настройки и значения по умолчанию.
мНастройка = Новый Структура("Узел, НовоеЗначениеРегистрации, ВместеСДвижениями");
