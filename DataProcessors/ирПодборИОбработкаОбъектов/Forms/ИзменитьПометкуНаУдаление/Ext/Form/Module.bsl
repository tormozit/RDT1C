...
Процедура вВыполнитьОбработку() Экспорт 
	...
	ОбработаноОбъектов = ирОбщий.ПодборИОбработкаОбъектов_ВыполнитьОбработкуЛкс("ИзменитьПометкуНаУдаление", ОбщиеПараметрыОбработки(), ПолучитьНастройку(),, ЭтаФорма);
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирПодборИОбработкаОбъектов.Форма.ИзменитьПометкуНаУдаление");
мИспользоватьНастройки = Истина;

//Реквизиты настройки и значения по умолчанию.
мНастройка = Новый Структура("НовоеЗначениеПометки");
мНастройка.НовоеЗначениеПометки = Истина;
