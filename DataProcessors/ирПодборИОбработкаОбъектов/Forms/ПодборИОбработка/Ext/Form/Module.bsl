...
Процедура ЗагрузитьНастройкуВФорме(НастройкаФормы, ДопПараметры) Экспорт 
	...
	ЗагружаемаяНастройкаФормы = НастройкаФормы;
	Если Не Открыта() И ЗначениеЗаполнено(ОбластьПоиска) И ЗагружаемаяНастройкаФормы.Свойство("ОбластьПоиска") Тогда
		ЗагружаемаяНастройкаФормы = ирОбщий.СкопироватьУниверсальнуюКоллекциюЛкс(ЗагружаемаяНастройкаФормы);
		ЗагружаемаяНастройкаФормы.Удалить("ОбластьПоиска");
	КонецЕсли; 
	ирОбщий.ЗагрузитьНастройкуФормыЛкс(ЭтаФорма, ЗагружаемаяНастройкаФормы); 
...
Процедура НастроитьЭлементыУправления()
	...
	//ЭлементыФормы.КПСтрокиДляОбработки.Кнопки.РедакторОбъектаБД.Доступность = Не Метаданные.Последовательности.Содержит(мИскомыйОбъект.МетаОбъект);
...
Процедура НастроитьКомпоновщик(СохранятьТекущиеНастройки = Истина, ЗагружатьТекущиеНастройки = Ложь)
...
Функция УстановитьОбластьПоиска(пОбластьПоиска = Null) Экспорт
	...
	Иначе
		...
		Если ирОбщий.ЛиТаблицаБДСуществуетЛкс(ОбластьПоиска) Тогда
	...
	Если МетаданныеОбъекта <> Неопределено Тогда
		...
		НастроитьКомпоновщик(СохранятьТекущиеНастройки, ЗагружатьТекущиеНастройки);
...
Процедура КПСтрокиДляОбработкиСжатьКолонки(Кнопка)
	
	ирОбщий.СжатьКолонкиТабличногоПоляЛкс(ЭлементыФормы.СтрокиДляОбработки);
	
КонецПроцедуры

Процедура КПСтрокиДляОбработкиШиринаКолонок(Кнопка)
	
	ирОбщий.РасширитьКолонкиТабличногоПоляЛкс(ЭлементыФормы.СтрокиДляОбработки);
		...
		Если Не ЭтоРегистр Тогда
			...
			Для Каждого МетаРегистр Из Метаданные.РегистрыСведений Цикл
				...
				Тогда
					...
					СтрокаРегистра.Представление = МетаРегистр.Представление() + " (" + XMLСтрока(МетаРегистр.Измерения[0].Тип.Типы().Количество()) + " типов) - связанный регистр";
				КонецЕсли; 
			КонецЦикла;
			Если СписокВыбораМД.Количество() > 0 Тогда
				...
				СтрокаРегистра.Представление = ПредставлениеМД + " - Основная таблица";
...
Процедура КПСтрокиДляОбработкиСнятьПометкиУспешноОбработанных(Кнопка)
	...
	СтрокиДляОбработки = СтрокиДляОбработки.НайтиСтроки(Новый Структура(мИмяКолонкиРезультатаОбработки, "Успех"));
...
Процедура КПСтрокиДляОбработкиОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ТекущаяСтрока = ЭлементыФормы.СтрокиДляОбработки.ТекущаяСтрока;
	Если ТекущаяСтрока <> Неопределено Тогда
		ТекущаяКолонка = ЭлементыФормы.СтрокиДляОбработки.ТекущаяКолонка;
		ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(Компоновщик.Настройки.Отбор, ТекущаяКолонка.Данные, ТекущаяСтрока[ТекущаяКолонка.Данные], ВидСравненияКомпоновкиДанных.НеРавно,,,, Истина);
...
Процедура СтрокиДляОбработкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура КПСтрокиДляОбработкиОткрытьТаблицу(Кнопка)
	
	ирОбщий.ОткрытьЗначениеЛкс(СтрокиДляОбработки.Скопировать(),,,, Ложь,, ЭлементыФормы.СтрокиДляОбработки);
...
Функция ПолучитьСтруктуруНастроекОбработки()
	...
	Если Не ДинамическаяВыборка Тогда
		СтруктураНастроек.Вставить("СтрокиДляОбработки", СтрокиДляОбработки);
	Иначе
		СтруктураНастроек.Вставить("СтрокиДляОбработки", СтрокиДляОбработки.СкопироватьКолонки());
	КонецЕсли; 
...
Процедура КПСтрокиДляОбработкиИдентификаторы(Кнопка)
...
Процедура КПСтрокиДляОбработкиМенеджерТабличногоПоля(Кнопка)
	
	ирОбщий.ОткрытьМенеджерТабличногоПоляЛкс(ЭлементыФормы.СтрокиДляОбработки, ЭтаФорма);
	
КонецПроцедуры

Процедура КПСтрокиДляОбработкиВывестиСтроки(Кнопка)
	
	ирОбщий.ВывестиСтрокиТабличногоПоляИПоказатьЛкс(ЭлементыФормы.СтрокиДляОбработки);
