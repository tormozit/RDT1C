...
Процедура КоманднаяПанельНайденныеОбъектыОткрытьТаблицу(Кнопка)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(НайденныеОбъекты, Ложь,,,,, ЭлементыФормы.ОбъектыДляОбработки);
...
Процедура ОбновитьКоллекциюРезультата(ЗадаватьВопросПередПеречитыванием = Ложь) Экспорт 
	...
	Если НайденныеОбъекты.Найти(Истина, мИмяКолонкиПометки) = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	НастройкаКомпоновки = Неопределено;
