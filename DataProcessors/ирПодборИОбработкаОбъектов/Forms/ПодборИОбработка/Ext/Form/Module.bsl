		...
		Если ВыбратьВсеПоля Тогда
			Для Каждого ДоступноеПолеВыбора Из Компоновщик.Настройки.ДоступныеПоляВыбора.Элементы Цикл
				Если ДоступноеПолеВыбора.Папка Тогда
					Продолжить;
				КонецЕсли; 
				ВыбранноеПоле = выхНастройкаКомпоновки.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
...
Процедура вНастроитьПостроитель(СохранятьТекущиеНастройки = Истина)
	...
	Если СохранятьТекущиеНастройки Тогда
		СохранитьНастройкиОбъектаМетаданных();
	КонецЕсли;
	Если МноготабличнаяВыборка Тогда
		...
		Тогда
			ВариантИсточников = 2;
		КонецЕсли; 
		ТекстЗапроса = ирОбщий.ПолучитьТекстЗапросаПоВыбраннымТаблицамЛкс(ИменаОбъектовМД, ВариантИсточников);
...
Процедура СохранитьНастройкиОбъектаМетаданных()
    
    Перем СохраняемыеНастройки;
...
Функция ПолучитьКлючТекущейСтроки(Знач ТекущаяСтрока)
	...
	Иначе
		...
		КонецЕсли; 
		НачальнаяСтрока = Новый (СтрЗаменить(ПолноеИмяТаблицы, ".", "КлючЗаписи."));
		#Если _ Тогда
			НачальнаяСтрока = РегистрыСведений.СостоянияАвтозаданийИис.СоздатьКлючЗаписи();
		#КонецЕсли
		ЗаполнитьЗначенияСвойств(НачальнаяСтрока, ТекущаяСтрока); 
	КонецЕсли;
...
Процедура ОбъектыДляОбработкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельНайденныеОбъекты.Кнопки.Идентификаторы.Пометка);
