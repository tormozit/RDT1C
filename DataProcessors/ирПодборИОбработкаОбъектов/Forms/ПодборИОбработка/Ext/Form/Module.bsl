...
Процедура вВыполнитьОтчет() Экспорт
	...
	СтруктураКлючаОбъекта = ирОбщий.ПолучитьСтруктуруКлючаТаблицыБДЛкс(?(МноготабличнаяВыборка, ОбластьПоиска[0], ОбластьПоиска), Ложь,, Истина);
...
Функция ПолучитьЗапросВыборки(НастройкаКомпоновки =  Неопределено)
	...
	//	КонецЦикла;
	//КонецЦикла;
	Запрос = ирОбщий.ПолучитьЗапросИзКомпоновкиЛкс(мСхемаКомпоновки, НастройкаКомпоновки, Истина,,, мСхемаКолонок, Автоупорядочивание);
	Возврат Запрос;
...
Функция ПолучитьИсполняемуюКомпоновку(выхНастройкаКомпоновки = Неопределено)
	...
	СписокПолейКлюча = ирОбщий.ПолучитьСтруктуруКлючаТаблицыБДЛкс(?(МноготабличнаяВыборка, ОбластьПоиска[0], ОбластьПоиска), , Ложь, Истина);
...
Функция вПолучитьТекстЗапроса()
	...
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ИЗ" + Символы.ПС + ирОбщий.ПолучитьОпределениеТаблицыБДДляИЗЛкс(ПолноеИмяТаблицыБД) + " КАК _Таблица_" + Символы.ПС;
	Если Истина
	...
	Тогда
		Если ирОбщий.ЕстьТаблицаИзмененийОбъектаМетаданных(мИскомыйОбъект.Метаобъект) Тогда
			...
			Для Каждого КлючИЗначение Из СтруктураКлючаИзменений Цикл
				...
				КонецЕсли;
				ТекстУсловияСоединения = ТекстУсловияСоединения + " _Изменения_." + КлючИЗначение.Ключ + " = _Таблица_." + КлючИЗначение.Ключ;
			КонецЦикла;
		...
		КонецЕсли; 
	КонецЕсли; 
	Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(ПолноеИмяМД) Тогда
...
Процедура вНастроитьПостроитель(СохранятьТекущиеНастройки = Истина)
	...
	Если МноготабличнаяВыборка Тогда
		...
		Тогда
			ВариантИсточников = 2;
		КонецЕсли; 
		ТекстЗапроса = ирОбщий.ПолучитьТекстЗапросаПоВыбраннымТаблицамЛкс(ИменаОбъектовМД, ВариантИсточников, ПервыеN);
	...
	Для Каждого ДоступноеПоле Из Компоновщик.Настройки.ДоступныеПоляОтбора.Элементы Цикл
		Для Каждого Тип Из ДоступноеПоле.ТипЗначения.Типы() Цикл
			Если ирОбщий.ЭтоТипСсылкиНаОбъектБДЛкс(Тип) Тогда 
...
Функция ОбработкаРазрешенаДляТаблицы(Знач СтрокаПроверки)
	...
	ИначеЕсли Истина
		...
		И ирОбщий.СтрокиРавныЛкс(СтрокаПроверки.ИмяФормы, "ПроизвольныйАлгоритм")
	Тогда 
		Возврат Истина;
	ИначеЕсли Истина
		И ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(мИскомыйОбъект.ТипТаблицы)
...
Процедура УстановитьОбластьПоиска(пОбластьПоиска = Null) Экспорт
	...
	Иначе
		...
		Если МетаданныеОбъекта <> Неопределено Тогда
			...
			ЕстьТаблицаИзменений = ирОбщий.ЕстьТаблицаИзмененийОбъектаМетаданных(мИскомыйОбъект.Метаобъект);
		Иначе
			мИскомыйОбъект = Неопределено;
			ЕстьТаблицаИзменений = Ложь;
		КонецЕсли;
	КонецЕсли; 
	мДоступнаЗапись = мИскомыйОбъект <> Неопределено И Не ирОбщий.ЛиКорневойТипПеречисленияЛкс(мИскомыйОбъект.ТипТаблицы);
	...
	Если МетаданныеОбъекта <> Неопределено Тогда
		...
		КонецЕсли; 
		мСтруктураКлюча = ирОбщий.ПолучитьСтруктуруКлючаТаблицыБДЛкс(ПолноеИмяТаблицы,,, Истина);
		Для Каждого ЭлементКлюча Из мСтруктураКлюча Цикл
			Если Истина
				И МноготабличнаяВыборка
				И ирОбщий.ЛиКорневойТипСсылкиЛкс(мИскомыйОбъект.ТипТаблицы)
...
Процедура ОбъектыДляОбработкиОбработкаВыбора(Элемент, Знач ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ЭлементМассива Из ВыбранноеЗначение Цикл
		ПолноеИмяМДЭлемента = "";
		ОбъектМДЭлемента = Метаданные.НайтиПоТипу(ТипЗнч(ЭлементМассива));
		Если ОбъектМДЭлемента <> Неопределено Тогда
			ПолноеИмяМДЭлемента = ОбъектМДЭлемента.ПолноеИмя();
		КонецЕсли; 
		Если Истина
			И ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(мИскомыйОбъект.ТипТаблицы)
		...
		Тогда
			...
			СтруктураКлюча = ирОбщий.ПолучитьСтруктуруКлючаТаблицыБДЛкс(ПолноеИмяМДЭлемента);
...
ЭлементыФормы.ПервыеN.СписокВыбора.Добавить(100);
ЭлементыФормы.ПервыеN.СписокВыбора.Добавить(1000);
ЭлементыФормы.ПервыеN.СписокВыбора.Добавить(10000);
ЭлементыФормы.ПервыеN.СписокВыбора.Добавить(100000);
ЭлементыФормы.ПервыеN.СписокВыбора.Добавить(1000000);
