...
Функция ПолучитьИсполняемуюКомпоновку(выхНастройкаКомпоновки = Неопределено)
	...
	Если АвтоВыбранныеПоля Тогда
		Для Каждого ЭлементОтбора Из выхНастройкаКомпоновки.Отбор.Элементы Цикл
			...
			КонецЕсли; 
		КонецЦикла; 
		Если ВыбратьВсеПоля Или ЭлементыВыбора.Количество() = 0 Тогда
...
Функция вПолучитьТекстЗапроса()
	...
	Для Каждого ДоступноеПоле Из КомпоновщикТаблицы.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		Если ДоступноеПоле.Папка Тогда
			Продолжить;
		КонецЕсли;
		//ТекстРеквизитов = ТекстРеквизитов + ", " + Символы.ПС + "Т." + ДоступноеПоле.Поле + " КАК " + ДоступноеПоле.Поле; // запрещенные имена например "Соединение" так вызывают ошибку
		ТекстРеквизитов = ТекстРеквизитов + ", " + Символы.ПС + "Т." + ДоступноеПоле.Поле;
	КонецЦикла; 
	...
	Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(ПолноеИмяМД) Тогда
		Для каждого ТЧ Из мИскомыйОбъект.МетаОбъект.ТабличныеЧасти Цикл
			Для каждого ТЧР Из ТЧ.Реквизиты Цикл
				...
				КонецЕсли;
				Условие = Условие + "Т." + ТЧ.Имя + "." + ТЧР.Имя + ".* КАК " + ТЧ.Имя + "_тч_" + ТЧР.Имя;
			КонецЦикла; 
		КонецЦикла;
	КонецЕсли; 
	Если Условие <> "" Тогда
...
Процедура КоманднаяПанельНайденныеОбъектыЗагрузитьПолностью(Кнопка)
	...
	ирОбщий.ЗагрузитьДинамическуюВыборкуВТабличноеПолеПолностьюЛкс(ЭтаФорма, мВыборкаРезультата, ЭлементыФормы.КоманднаяПанельНайденныеОбъекты,, СсылкаНаБуфернуюТаблицу);
	НайденныеОбъекты.ЗаполнитьЗначения(Истина, мИмяКолонкиПометки);
	ЭтотОбъект.ДинамическаяВыборка = Ложь;
