...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	...
	КонецЕсли;
	выхИменаСвойств = "Реквизит.АвтовВыбранныеПоляИзОтбора, Реквизит.АвтозаполнениеПорядка, Реквизит.БезАвтоупорядочивания, Реквизит.ВыбратьВсеПоля, Реквизит.ВыводитьСообщения, Реквизит.ВыполнятьНаСервере,
	|Реквизит.ИмяСиноним, Реквизит.ИспользоватьОтборПоУзлу, Реквизит.МноготабличнаяВыборка, Реквизит.НастройкиКомпоновки, Реквизит.ОбластьПоиска, Реквизит.ОбщаяТранзакция,
	|Реквизит.ПервыеNКаждойТаблицы, Реквизит.ПодключатьПоляКоличестваДвижений, Реквизит.ПообъектныеТранзакции, Реквизит.ПроводитьПроведенныеДокументыПриЗаписи, Реквизит.ПропускатьОшибки,
	|Реквизит.УзелОтбораОбъектов";
...
Процедура НастроитьСлужебныеКолонкиТабличногоПоляВыборки()
	...
	Если МноготабличнаяВыборка Тогда
		...
		КолонкаТП.Ширина = 20;
		НайденныеОбъекты.Колонки[КолонкаТП.Данные].Заголовок = КолонкаТП.ТекстШапки;
	КонецЕсли; 
	...
	КолонкаТП = ЭлементыФормы.ОбъектыДляОбработки.Колонки[мИмяКолонкиОтсутствияСтрокиВБД];
	КолонкаТП.Данные = "";
	...
	КолонкаТП = ЭлементыФормы.ОбъектыДляОбработки.Колонки[мИмяКолонкиСообщенияОбработки];
...
Процедура ОбновитьРазмерДинамическойТаблицы() Экспорт
	...
	ирОбщий.ПослеЗагрузкиВыборкиВТабличноеПолеЛкс(ЭтаФорма, мВыборкаРезультата, ЭлементыФормы.КоманднаяПанельНайденныеОбъекты, ЭлементыФормы.КоличествоСтрок, СсылкаНаБуфернуюТаблицу);
	...
	Если ЗначениеЗаполнено(ОбластьПоиска) Тогда
		Если ЭлементыФормы.ОбъектыДляОбработки.Колонки.Количество() > 0 Тогда
			...
			ЭлементыФормы.ОбъектыДляОбработки.Колонки[мИмяКолонкиСообщенияОбработки].Видимость = ВсеСчитано И Не ДинамическаяВыборка;
		КонецЕсли; 
	КонецЕсли; 
...
Процедура вВыполнитьОтчет() Экспорт
	...
	НайденныеОбъекты.ЗаполнитьЗначения(Ложь, мИмяКолонкиОтсутствияСтрокиВБД);
...
Функция ПолучитьИсполняемуюКомпоновку(выхНастройкаКомпоновки = Неопределено)
	...
	Для Индекс = 0 По КоличествоПолей - 1 Цикл
		...
		Если ДоступноеПоле = Неопределено Тогда
			Сообщить("Ключевое поле """ + ЭлементСписка.Представление + """ таблицы не найдено в доступных полях компоновки. На него или таблицу нет прав просмотра или не включена функциональная опция");
			Возврат Неопределено;
		КонецЕсли; 
	...
	Если АвтовВыбранныеПоляИзОтбора Тогда
		Для Каждого ЭлементОтбора Из выхНастройкаКомпоновки.Отбор.Элементы Цикл
			...
			Тогда 
				Продолжить;
			КонецЕсли; 
			ДобавитьПолеЭлементаОтбораВВыбранныеПоля(выхНастройкаКомпоновки, ЭлементОтбора.ЛевоеЗначение);
			ДобавитьПолеЭлементаОтбораВВыбранныеПоля(выхНастройкаКомпоновки, ЭлементОтбора.ПравоеЗначение);
		КонецЦикла; 
	КонецЕсли;
...
Процедура ДобавитьДоступныйОтборПоКоличествуСтрокПодчиненнойТаблицы(Знач КраткоеИмяПодчиненнойТаблицы, Знач ПолеСсылкиВПодчиненнойТаблице, Знач ПолноеИмяПодчиненнойТаблицы,
	...
	ТекстОбщееКоличествоДвижений = ВыражениеКоличества + " КАК КоличествоСтрок_" + ПрефиксИмениПоля+ "_" + КраткоеИмяПодчиненнойТаблицы + ",
	|" + ТекстОбщееКоличествоДвижений;
...
Процедура НастроитьКомпоновщик(СохранятьТекущиеНастройки = Истина)
	...
	Если МноготабличнаяВыборка Тогда
		...
		НовыйТекущийОбъект = ирОбщий.СтрСоединитьЛкс(ИменаОбъектовМД, ",");
	Иначе
...
Процедура ПриОткрытии()
	...
	Если ПараметрМассивСсылок <> Неопределено Тогда
		ЗагрузитьОбъектыДляОбработки(ирОбщий.ПолучитьУникальныеЗначенияМассиваЛкс(ПараметрМассивСсылок),, ПараметрВыбранныеПоля);
...
Функция УстановитьОбластьПоиска(пОбластьПоиска = Null) Экспорт
	...
	Если Не ирКэш.ЛиПортативныйРежимЛкс() И Не ирКэш.ЭтоФайловаяБазаЛкс() Тогда
		...
		мАдресХранилищаКоличестваСтрок = ПоместитьВоВременноеХранилище(Неопределено);
	...
	Если МетаданныеОбъекта <> Неопределено Тогда
		...
		Для Каждого ЭлементКлюча Из мСтруктураКлюча Цикл
			...
			КонецЕсли; 
			НайденныеОбъекты.Колонки.Добавить(ЭлементКлюча.Ключ, ОписаниеТипов, ОписаниеПоляТаблицы.Заголовок);
...
Процедура ДобавитьСлужебныеКолонкиВТаблицуВыборки()
	...
	НайденныеОбъекты.Колонки.Добавить(мИмяКолонкиОтсутствияСтрокиВБД, Новый ОписаниеТипов("Булево, Строка"));
	НайденныеОбъекты.Колонки.Добавить(мИмяКолонкиРезультатаОбработки, Новый ОписаниеТипов("Строка"));
	НайденныеОбъекты.Колонки.Добавить(мИмяКолонкиСообщенияОбработки, Новый ОписаниеТипов("Строка"));
		...
		Если Не ЭтоРегистр Тогда
			...
			Для Каждого МетаРегистр Из Метаданные.РегистрыСведений Цикл
				...
				Тогда
					...
					СтрокаРегистра.Представление = МетаРегистр.Представление() + " - связанный регистр";
				КонецЕсли; 
			КонецЦикла;
			Если СписокВыбораМД.Количество() > 0 Тогда
				СписокВыбораМД.СортироватьПоПредставлению();
				НовыйОбъектМД = Метаданные.НайтиПоПолномуИмени(НовоеПолноеИмяМД);
				СтрокаРегистра = СписокВыбораМД.Вставить(0);
				СтрокаРегистра.Значение = НовоеПолноеИмяМД;
				СтрокаРегистра.Представление = НовыйОбъектМД.Представление() + " - Основная таблица";
				РезультатВыбора = СписокВыбораМД.ВыбратьЭлемент("Выберите таблицу для обработки");
				Если РезультатВыбора <> Неопределено И НовоеПолноеИмяМД <> РезультатВыбора.Значение Тогда
					...
					#КонецЕсли
					СписокПолныхИменМД.Очистить();
					СписокПолныхИменМД.Добавить(НовоеПолноеИмяМД);
...
Процедура КоманднаяПанельНайденныеОбъектыРедакторОбъектаБД(Кнопка)
	...
	Иначе
		ПолноеИмяМД = ОбластьПоиска;
	КонецЕсли; 
	ирОбщий.ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ЭлементыФормы.ОбъектыДляОбработки, ПолноеИмяМД, Компоновщик.Настройки.ДоступныеПоляВыбора,,,,, Истина);

КонецПроцедуры

Процедура ОбъектыДляОбработкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельНайденныеОбъекты.Кнопки.Идентификаторы);

КонецПроцедуры

Процедура КоманднаяПанельНайденныеОбъектыОткрытьТаблицу(Кнопка)
	
	ирОбщий.ОткрытьЗначениеЛкс(НайденныеОбъекты.Скопировать(),,,, Ложь,, ЭлементыФормы.ОбъектыДляОбработки);
...
Процедура ОбновитьКоллекциюРезультата(ЗадаватьВопросПередПеречитыванием = Ложь) Экспорт 
	...
	//Если КолонкиДляОбновления <> "" Тогда
		...
		Иначе
			ПомеченныеСтроки = НайденныеОбъекты.НайтиСтроки(Новый Структура(мИмяКолонкиПометки, Истина));
			Ответ = "ПеречитатьВсе";
			ПомеченныхСильноМеньше = НайденныеОбъекты.Количество() - ПомеченныеСтроки.Количество() > 1000;
			Если Ложь
			...
			Иначе
				СтрокиДляПеречитывания = НайденныеОбъекты;
			КонецЕсли; 
			...
			КонецЦикла;
			Если НайденныеОбъекты = СтрокиДляПеречитывания Тогда
				ТаблицаКлючей = НайденныеОбъекты.Скопировать(, КолонкиКлюча);
			Иначе
				ТаблицаКлючей = НайденныеОбъекты.Скопировать(СтрокиДляПеречитывания, КолонкиКлюча);
			КонецЕсли; 
			...
			//Если Найти(мТекстЗапросаБезУсловий, "}") = 0 Тогда
				ТекстГДЕ = "ГДЕ (" + КолонкиКлюча + ") В (ВЫБРАТЬ " + КолонкиКлюча + " ИЗ ТаблицаКлючей)";
			//КонецЕсли; 
...
Процедура КоманднаяПанельНайденныеОбъектыЗаполнитьЗапросом(Кнопка)
	...
	НайденныеОбъекты.ЗаполнитьЗначения("?", мИмяКолонкиОтсутствияСтрокиВБД);
...
Процедура ОбъектыДляОбработкиПриИзмененииФлажка(Элемент, Колонка)
	
	ирОбщий.ТабличноеПолеПриИзмененииФлажкаЛкс(Элемент, Колонка);
	
КонецПроцедуры

Процедура ОбъектыДляОбработкиПриАктивизацииСтроки(Элемент)
...
Процедура ПоляБитыхСсылокПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура КоманднаяПанельНайденныеОбъектыСортироватьПоВозрастанию(Кнопка)
	
	ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.ОбъектыДляОбработки, Истина);

КонецПроцедуры

Процедура КоманднаяПанельНайденныеОбъектыСортироватьПоУбыванию(Кнопка)
	
	ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.ОбъектыДляОбработки, Ложь);
...
Процедура РазвернутьОтбор() Экспорт 
