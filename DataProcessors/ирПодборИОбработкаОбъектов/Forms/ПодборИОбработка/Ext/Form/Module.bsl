...
Процедура НастроитьСлужебныеКолонкиТабличногоПоляВыборки()
	...
	КолонкаТП.ТекстШапки = "<Отсутствует>";
	...
	КолонкаТП.ТекстШапки = "<Сообщения>";
	КолонкаТП.ТолькоПросмотр = Истина;
	КолонкаТП.ЦветФонаПоля = Новый Цвет(250, 250, 255);
	КолонкаТП.ИзменятьВидимость = Ложь;
	КолонкаТП.Видимость = Истина;
...
Процедура КПОтборНайтиВСписке(Кнопка)
	...
	ирОбщий.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭлементыФормы.КомпоновщикДоступныеПоляОтбора, ЭлементыФормы.КомпоновщикОтбор);
	
КонецПроцедуры

Процедура КПВыборНайтиВСписке(Кнопка)
	
	ирОбщий.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭлементыФормы.КомпоновщикДоступныеПоляОтбора, ЭлементыФормы.КомпоновщикВыбор);
