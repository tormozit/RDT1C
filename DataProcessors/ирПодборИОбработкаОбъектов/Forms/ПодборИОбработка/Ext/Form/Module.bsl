	...
	Если КолонкиДляОбновления <> "" Тогда
		...
		Иначе
			Если ЗадаватьВопросПередПеречитыванием Тогда
				Ответ = Вопрос("Хотите перечитать данные в дополнительных (неключевых) колонках помеченных строк для обработки (можеть быть долго)?", РежимДиалогаВопрос.ДаНет);
			...
			КонецЕсли;
			СтрокиДляПеречитывания = НайденныеОбъекты.НайтиСтроки(Новый Структура(мИмяКолонкиПометки, Истина));
