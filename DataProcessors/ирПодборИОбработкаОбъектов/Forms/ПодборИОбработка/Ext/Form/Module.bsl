...
Функция ПолучитьЗапросВыборки(НастройкаКомпоновки =  Неопределено)
	...
	Если НастройкаКомпоновки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Узел")) <> Неопределено Тогда
		НастройкаКомпоновки.ПараметрыДанных.УстановитьЗначениеПараметра("Узел", УзелОтбораОбъектов);
	КонецЕсли; 
	Если АвтоВыбранныеПоля Тогда
...
Процедура вНастроитьПостроитель(СохранятьТекущиеНастройки = Истина)
	...
	Если МноготабличнаяВыборка Тогда
		...
		Тогда
			ВариантИсточников = 2;
		КонецЕсли; 
		ТекстЗапроса = ирОбщий.ПолучитьТекстЗапросаПоВыбраннымМетаданнымЛкс(ИменаОбъектовМД, ВариантИсточников);
