...
Процедура вНастроитьЭУ()
	...
	ЭлементыФормы.КоманднаяПанельНайденныеОбъекты.Кнопки.ОбъединитьСсылки.Доступность = Истина
		...
		И ЛксЛиКорневойТипСсылки(мИскомыйОбъект.ТипТаблицы);
...
Процедура вВыполнитьОтчет() Экспорт
	...
	Для Индекс = 0 По КоличествоПолей - 1 Цикл
		...
		Если Не ПолеВыбора.Использование Тогда
			Продолжить;
		КонецЕсли;
		ИмяПоля = ирНеглобальный.ПолучитьИмяКолонкиРезультатаПоИмениПоляЛкс("" + ПолеВыбора.Поле);
...
Функция ПолучитьЗапросВыборки(НастройкаКомпоновки =  Неопределено)
	...
	//	КонецЦикла;
	//КонецЦикла;
	Запрос = ирНеглобальный.ПолучитьЗапросИзКомпоновкиЛкс(мСхемаКомпоновки, НастройкаКомпоновки, Истина,,, мСхемаКолонок);
	Возврат Запрос;
...
Функция вПолучитьТекстЗапроса()
	...
	Если ЛксЛиКорневойТипОбъектаБД(ПолноеИмяМД) Тогда
...
Процедура вНастроитьПостроитель(СохранятьТекущиеНастройки = Истина)
	...
	Если МноготабличнаяВыборка Тогда
		...
		НовыйТекущийОбъект = ЛксПолучитьСтрокуСРазделителемИзМассива(ИменаОбъектовМД, ",");
	Иначе
		Если мИскомыйОбъект.МетаОбъект = Неопределено Тогда
			Возврат;
		КонецЕсли; 
		НовыйТекущийОбъект = мИскомыйОбъект.МетаОбъект.ПолноеИмя();
		МетаданныеОбъекта = ЛксПолучитьМетаданные(ОбластьПоиска);
	...
	Если ЛксЛиКорневойТипОбъектаБД(мИскомыйОбъект.ТипТаблицы) Тогда
	...
	Если Компоновщик.Настройки.Порядок.Элементы.Количество() = 0 Тогда
		Если ЛксЛиКорневойТипСсылки(мИскомыйОбъект.ТипТаблицы) Тогда
...
Процедура ПриОткрытии()
	...
	Если НЕ ВосстановленныеДоступныеОбработки = Неопределено Тогда
		...
		ЛксЗагрузитьВДеревоЗначений(ВосстановленныеДоступныеОбработки, ДоступныеОбработки);
	КонецЕсли;
...
Процедура НайденныеОбъектыПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	//КонецЕсли; 
	//КорневойТип = ЛксПолучитьПервыйФрагмент(ИмяТаблицы);
	//Если Не ЛксЛиКорневойТипОбъектаБД(КорневойТип) Тогда
	//	Возврат;
	//КонецЕсли; 
...
Процедура КоманднаяПанельВыбранныеОбработкиВыполнить(Кнопка)
	...
	Индикатор = ЛксПолучитьИндикаторПроцесса(ВыбранныеОбработки.Количество(), "Обработки");
...
Функция ОбработкаРазрешенаДляТаблицы(Знач СтрокаПроверки)
	...
	КонецЕсли; 
	Если Истина
		И ЛксЛиКорневойТипОбъектаБД(мИскомыйОбъект.ТипТаблицы)
	...
	ИначеЕсли Истина
		И (Ложь
			Или ЛксЛиКорневойТипОбъектаБД(мИскомыйОбъект.ТипТаблицы)
			Или ЛксЛиТипВложеннойТаблицыБД(мИскомыйОбъект.ТипТаблицы))
		И РежимОбходаДанных = "КлючиОбъектов"
	...
	ИначеЕсли Истина
		...
		И ЛксЛиТипВложеннойТаблицыБД(мИскомыйОбъект.ТипТаблицы)
		И Найти(СтрокаПроверки.ПоддерживаемыеТипыТаблиц + ",", "СсылочныйОбъект" + ",") > 0
	Тогда
		Возврат Истина;
	ИначеЕсли Истина
	...
	ИначеЕсли Истина
		И РежимОбходаДанных = "Объекты"
		И (Ложь
			Или ЛксЛиКорневойТипРегистраБД(мИскомыйОбъект.ТипТаблицы) 
			Или ЛксЛиКорневойТипПоследовательности(мИскомыйОбъект.ТипТаблицы))
...
Процедура УстановитьОбластьПоиска(пОбластьПоиска = Null) Экспорт
	...
	Если ТипЗнч(ОбластьПоиска) = Тип("СписокЗначений") Тогда
		...
		Для Каждого ЭлементСписка Из ОбластьПоиска Цикл
			...
			Если МетаданныеОбъекта <> Неопределено Тогда
				...
				мИскомыйОбъект.Вставить("КорневойТип", ЛксПолучитьПервыйФрагмент(ПолноеИмяМД));
	...
	Иначе
		...
		Если МетаданныеОбъекта <> Неопределено Тогда
			мИскомыйОбъект = Новый Структура;
			мИскомыйОбъект.Вставить("КорневойТип", ЛксПолучитьПервыйФрагмент(ОбластьПоиска));
	...
	Если МетаданныеОбъекта <> Неопределено Тогда
		...
		//	Или ЛксЛиКорневойТипРегистраБД(мИскомыйОбъект.КорневойТип)
		//	Или ЛксЛиКорневойТипПоследовательности(мИскомыйОбъект.КорневойТип)
		//	Или ЛксЛиТипВложеннойТаблицыБД(мИскомыйОбъект.ТипТаблицы);
		...
		КонецЕсли; 
		мСтруктураКлюча = ирНеглобальный.ПолучитьСтруктуруКлючаТаблицыБДЛкс(ПолноеИмяМД);
		Для Каждого ЭлементКлюча Из мСтруктураКлюча Цикл
			Если Истина
				И МноготабличнаяВыборка
				И ЛксЛиКорневойТипСсылки(мИскомыйОбъект.ТипТаблицы)
...
Процедура КоманднаяПанельНайденныеОбъектыСжатьКолонки(Кнопка)
	...
	ЛксСжатьКолонкиТабличногоПоля(ЭлементыФормы.ОбъектыДляОбработки);
	
КонецПроцедуры

Процедура КоманднаяПанельНайденныеОбъектыШиринаКолонок(Кнопка)
	
	ЛксВвестиИУстановитьШиринуКолонокТабличногоПоля(ЭлементыФормы.ОбъектыДляОбработки);
...
Функция ПолучитьТипКлючаТаблицы(ПолноеИмяТаблицы)
	...
	ИначеЕсли ЛксЛиКорневойТипРегистраБД(мИскомыйОбъект.ТипТаблицы) Тогда
...
Процедура ОбъектыДляОбработкиОбработкаВыбора(Элемент, Знач ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ЭлементМассива Из ВыбранноеЗначение Цикл
		...
		Тогда
			СтруктураКлюча = ЛксПолучитьКопиюОбъекта(мСтруктураКлюча);
...
Процедура ОбъектыДляОбработкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирНеглобальный.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельНайденныеОбъекты.Кнопки.Пустые.Пометка);
...
Процедура ПеречитатьДанныеОбъектовДляОбработки(ЗадаватьВопрос = Ложь) Экспорт 
	...
	ЛксСостояние("Обновляем строки для обработки...");
...
Процедура КоманднаяПанельНайденныеОбъектыЗаполнитьЗапросом(Кнопка)
	...
	КонецЦикла;
	РезультатЗапроса = Обработки.ирКонсольЗапросов.Создать().ОткрытьДляЗаполненияКоллекции(НайденныеОбъекты.СкопироватьКолонки(СтрокаИменКолонокКлюча), Запрос, , "Строки для обработки");
	...
	ЛксЗагрузитьВТаблицуЗначений(РезультатЗапроса, НайденныеОбъекты, Новый Структура("Пометка", Истина));
