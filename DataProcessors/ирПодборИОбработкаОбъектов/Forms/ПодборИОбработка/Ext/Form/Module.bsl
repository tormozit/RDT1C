...
Процедура ЗаполнитьКолонкиТабличногоПоляВыборки(Знач НастройкаКомпоновки = Неопределено)
	...
	Для Каждого КолонкаТП Из ЭлементыФормы.ОбъектыДляОбработки.Колонки Цикл
...
Процедура вНастроитьПостроитель(СохранятьТекущиеНастройки = Истина)
	...
	Для Каждого ДоступноеПоле Из Компоновщик.Настройки.ДоступныеПоляОтбора.Элементы Цикл
		Для Каждого Тип Из ДоступноеПоле.ТипЗначения.Типы() Цикл
			Если ирОбщий.ЛиТипСсылкиНаОбъектБДЛкс(Тип) Тогда 
...
Функция ОбработкаРазрешенаДляТаблицы(Знач СтрокаПроверки)
	...
	ИначеЕсли Истина
		...
		И Найти(СтрокаПроверки.ПоддерживаемыеТипыТаблиц + ",", "Метассылка" + ",") > 0
	Тогда 
		Возврат Истина;
	ИначеЕсли Истина
		И МноготабличнаяВыборка
...
Процедура ЗагрузитьОбъектыДляОбработки(Массив, ОчиститьТекущийСписок = Истина) Экспорт
	...
	Для Каждого КлючЗаписи Из Массив Цикл
		ПолноеИмяМД = Метаданные.НайтиПоТипу(ТипЗнч(КлючЗаписи)).ПолноеИмя();
		Если СписокПолныхИменМД.НайтиПоЗначению(ПолноеИмяМД) = Неопределено Тогда
			БылиПропуски = Истина;
			Продолжить;
		КонецЕсли; 
		СтрокаДанных = НайденныеОбъекты.Добавить();
		Если ирОбщий.ЛиСсылкаНаПеречислениеЛкс(КлючЗаписи) Тогда
...
Процедура ОбновитьКоллекциюРезультата(ЗадаватьВопросПередПеречитыванием = Ложь) Экспорт 
	...
	Для Каждого КлючИЗначение Из мСхемаКолонок Цикл
		Если НайденныеОбъекты.Колонки.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			НайденныеОбъекты.Колонки.Добавить(КлючИЗначение.Ключ);
		КонецЕсли; 
	КонецЦикла;
...
Процедура КоманднаяПанель3РедактироватьСписок(Кнопка)
	
	ирОбщий.РедактироватьСписокПравогоЗначенияОтбораКомпоновкиЛкс(ЭлементыФормы.КомпоновщикОтбор);
