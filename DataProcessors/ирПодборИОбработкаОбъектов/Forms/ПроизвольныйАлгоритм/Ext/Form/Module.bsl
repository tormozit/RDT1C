...
Процедура вСохранитьНастройку() Экспорт
    ...
    НоваяНастройка = ПолучитьНастройку();
...
Процедура вВыполнитьОбработку() Экспорт 
	...
	Если Открыта() И Не РедакторАлгоритма.ПроверитьПрограммныйКод() Тогда
		Возврат;
	КонецЕсли; 
	ОбработаноОбъектов = ирОбщий.ПодборИОбработкаОбъектов_ВыполнитьОбработкуЛкс("ПроизвольныйАлгоритм", ОбщиеПараметрыОбработки(), ПолучитьНастройку(),, ЭтаФорма);
...
СтрокаПараметра.НИмя = НСтр(СтрокаПараметра.Имя);
СтрокаПараметра.Комментарий = "Для доступа к объекту данных при обработке его элементов. В режиме ""Объекты на сервере"" в непортативных вариантах содержит только данные объекта.";
СтрокаПараметра.Вход = Истина;
СтрокаПараметра = РедакторАлгоритма.Параметры.Добавить();
СтрокаПараметра.Имя = "ОбъектБДМетоды";
СтрокаПараметра.НИмя = НСтр(СтрокаПараметра.Имя);
СтрокаПараметра.Комментарий = "Для доступа к объекту данных при обработке его элементов. В режиме ""объекты на сервере"" в непортативных вариантах содержит только предопределенные свойства и методы объекта.";
СтрокаПараметра.Вход = Истина;
СтрокаПараметра = РедакторАлгоритма.Параметры.Добавить();
