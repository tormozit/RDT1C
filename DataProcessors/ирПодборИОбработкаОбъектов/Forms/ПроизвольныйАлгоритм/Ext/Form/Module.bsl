...
Процедура вОбработатьОбъект(Знач Объект, КоллекцияСтрок = Неопределено) Экспорт
	...
	Для Каждого СтрокаПараметра Из РедакторАлгоритма.Параметры Цикл
		...
		|" + ТекстАлгоритма;
	КонецЦикла; 
	ирОбщий.ВыполнитьАлгоритм(ТекстАлгоритма, РедакторАлгоритма.Параметры.ВыгрузитьКолонку("Значение"), , Объект);
...
Процедура вЗагрузитьНастройку() Экспорт
	...
	КонецЦикла;
	ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(мНастройка.Параметры, РедакторАлгоритма.Параметры);
	Для Каждого СтрокаПараметра Из РедакторАлгоритма.Параметры Цикл
...
Функция вВыполнитьОбработку() Экспорт
	...
	Если Не ирОбщий.ЛиПараметрыАлгоритмыКорректныЛкс(РедакторАлгоритма.Параметры.Выгрузить()) Тогда
		Возврат Неопределено;
	КонецЕсли; 
...
Процедура ПараметрыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ОбновитьПараметрыВМобильномРежиме();
...
Процедура ОбновитьПараметрыВМобильномРежиме()
