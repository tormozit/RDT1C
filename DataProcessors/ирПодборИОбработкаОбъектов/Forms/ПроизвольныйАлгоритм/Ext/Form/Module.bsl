...
Процедура вОбработатьОбъект(Знач Объект, Знач КоллекцияСтрок = Неопределено, Знач ОбъектБД = Неопределено, Знач ПараметрыОбработки = Неопределено) Экспорт
	...
	ТекстАлгоритма = "Объект = _П0; ОбъектБД = _П1; Параметры = _П2; ЭтоПервыйОбъектБД = _П3; ЭтоПоследнийОбъектБД = _П4; ЭтоПервыйЭлемент = _П5; ЭтоПоследнийЭлемент = _П6; ПринудительнаяЗапись = _П7;
	|" + ТекстАлгоритма;
	...
	ирОбщий.ВыполнитьАлгоритм(ТекстАлгоритма, мТабличноеПолеПараметры.Значение.ВыгрузитьКолонку("Значение"), , Объект, ОбъектБД, ПараметрыОбработки,
		...
		ПараметрыОбработки.ПринудительнаяЗапись);
