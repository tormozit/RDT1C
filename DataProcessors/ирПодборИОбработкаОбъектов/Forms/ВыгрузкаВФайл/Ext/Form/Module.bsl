...
Процедура ПередОбработкойВсех(ПараметрыОбработки, Отказ) Экспорт 
...
//
Процедура ОбработатьОбъект(Знач Объект, Знач КоллекцияСтрок = Неопределено, Знач ОбъектБДДанные = Неопределено, Знач ПараметрыОбработки = Неопределено, Знач ОбъектБДМетоды = Неопределено) Экспорт

	ВыгрузитьОбъектБД(ПараметрыОбработки.ЗаписьXML, ОбъектБДМетоды, ПараметрыОбработки.СчетчикВыгруженныхОбъектов);
	Если ВыгружатьДвиженияВместеСДокументами Тогда
		...
		Если ирОбщий.ЛиКорневойТипДокументаЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ОбъектМД.ПолноеИмя())) Тогда
			...
			Для Каждого МетаРегистр из ОбъектыМД Цикл
				...
				НаборЗаписей = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяМДРегистра, Новый Структура(ИмяПоляОтбора, Объект.Ссылка));
...
Функция вВыполнитьОбработку() Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
		...
		Возврат 0;
	КонецЕсли; 
	ПараметрыОбработки = ОбщиеПараметрыОбработки();
	ПараметрыОбработки.Вставить("ФормаОбработки", ЭтаФорма);
	ОбработаноОбъектов = вВыполнитьГрупповуюОбработку(ПараметрыОбработки);
