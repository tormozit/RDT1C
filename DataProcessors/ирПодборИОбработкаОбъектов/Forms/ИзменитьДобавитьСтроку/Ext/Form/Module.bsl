...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНоваяНастройка() Тогда
		Если Не МноготабличнаяВыборка Тогда
			Если ТабличноеПолеСтрокДляОбработки.ТекущаяСтрока <> Неопределено Тогда
...
Процедура ОбновитьТаблицуРеквизитов()
	...
	Если Истина
		...
		И Метаданные.ОпределяемыеТипы.ВладелецДополнительныхСведений.Тип.СодержитТип(ТипЗнч(СтрокиДляОбработки[0].Ссылка)) 
	Тогда
		СписокСвойств = ирСервер.СписокДопСвойствОбъектаБСПЛкс(СтрокиДляОбработки[0].Ссылка);
...
Процедура ЗначенияРеквизитовЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если СвязиИПараметрыВыбора Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	Если ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.ЗначенияРеквизитов, СтандартнаяОбработка, , Истина, СтруктураОтбора) Тогда 
		ОбновитьТипЗначенияВСтрокеТаблицы();
	КонецЕсли; 
