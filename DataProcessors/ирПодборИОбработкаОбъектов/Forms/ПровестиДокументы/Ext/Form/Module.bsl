...
Процедура вОбработатьОбъект(Знач Объект, Знач КоллекцияСтрок = Неопределено, Знач ОбъектБДДанные = Неопределено, Знач ПараметрыОбработки = Неопределено, Знач ОбъектБДМетоды = Неопределено) Экспорт
	...
	Если ОбъектМД.Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить Тогда
		ирОбщий.ЗаписатьОбъектЛкс(ОбъектБДМетоды, ОбъектыНаСервере, РежимЗаписиДокумента.Проведение, , ОтключатьКонтрольЗаписи, БезАвторегистрацииИзменений);
	КонецЕсли;
...
Процедура вСохранитьНастройку() Экспорт
    ...
    НоваяНастройка = Новый Структура("СниматьПометкиУдаления", СниматьПометкиУдаления);
...
Процедура ПропускатьНепроведенныеПриИзменении(Элемент)
...
Процедура ОбновитьДоступность()
	...
	ЭлементыФормы.СниматьПометкиУдаления.Доступность = Не ПропускатьНепроведенные;
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирПодборИОбработкаОбъектов.Форма.ПровестиДокументы");
мИспользоватьНастройки = Истина;

//Реквизиты настройки и значения по умолчанию.
мНастройка = Новый Структура("ПропускатьНепроведенные, СниматьПометкиУдаления", Истина, Ложь);
