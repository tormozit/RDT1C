...
Функция вВыполнитьОбработку(Кнопка = Неопределено) Экспорт
	...
	КонецЕсли; 
	ирОбщий.ПодборИОбработкаОбъектов_ПеренумерацияОбъектовЛкс(ОбщиеПараметрыОбработки(), ПолучитьНастройкуЛкс(), мТипНомера, мДлинаНомера, , ЭтаФорма, Кнопка, "ВыполнитьОбработкуЗавершение");
