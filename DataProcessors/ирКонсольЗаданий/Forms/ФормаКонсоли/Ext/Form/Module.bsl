...
Функция ОбновитьСписокФоновыхЗаданий(ВызовВнутриОбновленияРегламентныхЗаданий = Ложь, ДляСбораСтатистики = Ложь)
	...
	Если ОтборФоновыхЗаданий.Свойство("Начало") Тогда
		...
		АктивныеФоновыеЗадания = ФоновыеЗадания.ПолучитьФоновыеЗадания(КонечныйОтбор);
		Для Каждого АктивноеФоновоеЗадание Из АктивныеФоновыеЗадания Цикл
	...
	Для Каждого Фоновое из Фоновые Цикл
		...
		Если Не ДляСбораСтатистики Тогда
			...
			Если ПолучатьОшибкиИзЖурнала Тогда
				СтруктураОтбора = ОтборДляЖурналаПоСтрокеЗадания(НоваяСтрока);
...
Процедура ПриОткрытии()
	...
	Попытка
		ЭтаФорма.ОтборФоновыхЗаданий = ирОбщий.ВосстановитьЗначениеЛкс("ФоновыеЗадания.Отбор");
		ОбновитьБыстрыйОтборПоСостояниюФоновыхЗаданий();
		ЭтаФорма.ОтборРегламентныхЗаданий = ПреобразоватьОтборПослеВосстановленияЗначений(ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.Отбор"));
		ЭтаФорма.РегламентныеПодстрока = ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.РегламентныеПодстрока");
		ЭтаФорма.МетодыПодстрока = ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.МетодыПодстрока");
		ЭтаФорма.ОтборРегламентныхЗаданийВключен = ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.ОтборВключен");
		СортировкаРегламентныхЗаданийЛ = ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.СортировкаРегламентныхЗаданий");
		Если ЭлементыФормы.СортировкаРегламентныхЗаданий.СписокВыбора.НайтиПоЗначению(СортировкаРегламентныхЗаданийЛ) <> Неопределено Тогда
			ЭтаФорма.СортировкаРегламентныхЗаданий = СортировкаРегламентныхЗаданийЛ;
		КонецЕсли; 
		СортировкаМетодовЛ = ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.СортировкаМетодов");
		Если ЭлементыФормы.СортировкаМетодов.СписокВыбора.НайтиПоЗначению(СортировкаМетодовЛ) <> Неопределено Тогда
			ЭтаФорма.СортировкаМетодов = СортировкаМетодовЛ;
		КонецЕсли; 
		
		ОтборПоЗаданиюИлиМетоду = ирОбщий.ВосстановитьЗначениеЛкс("ФоновыеЗадания.ОтборПоЗаданиюИлиМетоду");
		Если ОтборПоЗаданиюИлиМетоду = Неопределено Тогда
		...
		ОтборПоМетоду = ирОбщий.ВосстановитьЗначениеЛкс("ФоновыеЗадания.ОтборПоМетоду");
		Если ОтборПоМетоду = Неопределено Тогда
			ОтборПоМетоду = Ложь;
		КонецЕсли; 
		...
		АвтообновлениеСпискаМетодов = ирОбщий.ВосстановитьЗначениеЛкс("Методы.АвтообновлениеСписка");
		Если АвтообновлениеСпискаМетодов = Неопределено Тогда
		...
		ПериодАвтообновленияСпискаМетодов = ирОбщий.ВосстановитьЗначениеЛкс("РегламентныеЗадания.ПериодАвтообновленияСпискаМетодов");
		Если Не ЗначениеЗаполнено(ПериодАвтообновленияСпискаМетодов) Тогда
		...
		ЭтаФорма.ПолучатьСообщенияПользователю = ирОбщий.ВосстановитьЗначениеЛкс("ФоновыеЗадания.ПолучатьСообщенияПользователю");
		ЭлементыФормы.КоманднаяПанельФоновыеЗадания.Кнопки.ПолучатьСообщенияПользователю.Пометка = ПолучатьСообщенияПользователю;
		ЭтаФорма.ПолучатьОшибкиИзЖурнала = ирОбщий.ВосстановитьЗначениеЛкс("ФоновыеЗадания.ПолучатьОшибкиИзЖурнала");
...
Процедура ФоновыеЗаданияУстановитьОтбор(СохранитьОтбор = Истина) Экспорт
	...
	ирОбщий.СохранитьЗначениеЛкс("ФоновыеЗадания.ОтборПоЗаданиюИлиМетоду", ОтборПоЗаданиюИлиМетоду);
	ирОбщий.СохранитьЗначениеЛкс("ФоновыеЗадания.ОтборПоМетоду", ОтборПоМетоду);
...
Процедура КПРегламентныеЗаданияНастройкаОбновления(Кнопка)
	...
	Если Диалог.ОткрытьМодально() = Истина Тогда
		...
		ирОбщий.СохранитьЗначениеЛкс("РегламентныеЗадания.ПериодАвтообновленияСписка", ПериодАвтообновленияСпискаРегламентныхЗаданий);
...
Процедура УстановитьАвтообновлениеСпискаРегламентныхЗаданий(НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение <> Неопределено Тогда
		АвтообновлениеСпискаРегламентныхЗаданий = НовоеЗначение;
	КонецЕсли; 
	ЭлементыФормы.КоманднаяПанельРегламентныеЗадания.Кнопки.НастройкаОбновления.Пометка = АвтообновлениеСпискаРегламентныхЗаданий;
	ирОбщий.СохранитьЗначениеЛкс("РегламентныеЗадания.АвтообновлениеСписка", АвтообновлениеСпискаРегламентныхЗаданий);
	Если АвтообновлениеСпискаРегламентныхЗаданий = Истина Тогда
		ПодключитьОбработчикОжидания("ОбновитьСписокРегламентныхЗаданий", ПериодАвтообновленияСпискаРегламентныхЗаданий, Истина);
	КонецЕсли; 
...
Процедура КоманднаяПанельФоновыеЗаданияЖурналРегистрации(Кнопка = Неопределено)
	...
	СтруктураОтбора = ОтборДляЖурналаПоСтрокеЗадания(ТекущаяСтрока);
	АнализЖурналаРегистрации = ирОбщий.ПолучитьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирАнализЖурналаРегистрации");
	#Если Сервер И Не Сервер Тогда
		АнализЖурналаРегистрации = Обработки.ирАнализЖурналаРегистрации.Создать();
	#КонецЕсли
...
Процедура КоманднаяПанельФоновыеЗаданияОтладчикФоновыхЗаданий(Кнопка)
	...
	ПараметрыЗапуска = ирОбщий.ПараметрыЗапускаПриложения1СТекущейБазыЛкс(,,, Истина,,,,,, СтрокаСоединения);
...
Процедура КоманднаяПанельФоновыеЗаданияПолучатьОшибкиИзЖурнала(Кнопка)
	...
	ПолучатьОшибкиИзЖурнала = Не Кнопка.Пометка;
	Кнопка.Пометка = ПолучатьОшибкиИзЖурнала;
	ирОбщий.СохранитьЗначениеЛкс("ФоновыеЗадания.ПолучатьОшибкиИзЖурнала", ПолучатьОшибкиИзЖурнала);
	Если ПолучатьОшибкиИзЖурнала Тогда
		ОбновитьСписокФоновыхЗаданий();
	КонецЕсли; 
...
Процедура КоманднаяПанельМетодыНастройкаОбновления(Кнопка)
	...
	Если Диалог.ОткрытьМодально() = Истина Тогда	
		АвтообновлениеСпискаМетодов = Диалог.Автообновление;
		ПериодАвтообновленияСпискаМетодов = Диалог.ПериодАвтообновления;
		ирОбщий.СохранитьЗначениеЛкс("Методы.АвтообновлениеСписка", АвтообновлениеСпискаМетодов);
		Кнопка.Пометка = АвтообновлениеСпискаМетодов;
		ирОбщий.СохранитьЗначениеЛкс("Методы.ПериодАвтообновленияСписка", ПериодАвтообновленияСпискаРегламентныхЗаданий);
...
Процедура СортировкаРегламентныхЗаданийОчистка(Элемент, СтандартнаяОбработка)
	...
	Элемент.Значение = "Метаданные";
	СтандартнаяОбработка = Ложь;
	
КонецПроцедуры

Процедура СортировкаРегламентныхЗаданийПриИзменении(Элемент)
	
	ирОбщий.СохранитьЗначениеЛкс("РегламентныеЗадания.СортировкаРегламентныхЗаданий", СортировкаРегламентныхЗаданий);
	ОбновитьСписокРегламентныхЗаданий();
	
КонецПроцедуры

Процедура СортировкаМетодовПриИзменении(Элемент)
	
	ирОбщий.СохранитьЗначениеЛкс("РегламентныеЗадания.СортировкаМетодов", СортировкаМетодов);
	ОбновитьСписокМетодов();
...
Процедура РегламентныеПодстрокаПриИзменении(Элемент)
	...
	ирОбщий.СохранитьЗначениеЛкс("РегламентныеЗадания.РегламентныеПодстрока", РегламентныеПодстрока);
	ОбновитьСписокРегламентныхЗаданий();
...
Процедура МетодыПодстрокаПриИзменении(Элемент)
	...
	ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, ЭтаФорма);
	ирОбщий.СохранитьЗначениеЛкс("РегламентныеЗадания.МетодыПодстрока", МетодыПодстрока);
	ОбновитьСписокМетодов();
...
Процедура КоманднаяПанельРегламентныеЗаданияВключитьПоСпискуИзФайла(Кнопка)
	...
	СписокВключенных = ирОбщий.ЗагрузитьЗначениеИзФайлаИнтерактивноЛкс("urt", "Список включенных регламентных заданий");
