...
Функция ОбновитьСписокФоновыхЗаданий(ВызовВнутриОбновленияРегламентныхЗаданий = Ложь, ДляСбораСтатистики = Ложь, Перезапустить = Ложь)
	...
	#КонецЕсли
	ТаблицаЗаданий = ирОбщий.ФоновыеЗаданияПодробноЛкс(ТаблицаЗаданий, ДляСбораСтатистики, КонечныйОтбор, ОтборФоновыхЗаданий, ОтображатьИндикатор, ВызовВнутриОбновленияРегламентныхЗаданий,
		АвтообновлениеСпискаМетодов, ПолучатьОшибкиИзЖурнала, ПолучатьСообщенияПользователю, ФоновыеЗаданияСоответствие,
		ЭтаФорма, ЭлементыФормы.КоманднаяПанельМетоды.Кнопки.Обновить, "ПолучитьФоновыеЗаданияПодробноЗавершение", ДляСбораСтатистики, Перезапустить);
...
Процедура ПолучитьФоновыеЗаданияПодробноЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	
	Если Ложь
		Или СостояниеЗадания = Неопределено
		Или СостояниеЗадания = СостояниеФоновогоЗадания.Завершено 
	Тогда
		ДляСбораСтатистики = ПараметрыЗадания[1];
		ВызовВнутриОбновленияРегламентныхЗаданий = ПараметрыЗадания[5];
		Если Истина
		...
		Если ДляСбораСтатистики Тогда
			ЗагрузитьСписокМетодов(РезультатЗадания);
		Иначе
			...
			ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(РезультатЗадания, СписокФоновыхЗаданий,,, Истина);
