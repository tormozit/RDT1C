...
Процедура ОбновитьДанные()
	...
	СообщенияПользователюЛ = ФоновоеЗадание.ПолучитьСообщенияПользователю();
	Если СообщенияПользователюЛ <> Неопределено Тогда
		ЗаписьXML = Новый ЗаписьXML;
		ЗаписьXML.УстановитьСтроку("");
		Для Каждого СообщениеПользователю Из СообщенияПользователюЛ Цикл
			#Если Сервер И Не Сервер Тогда
				СообщениеПользователю = Новый СообщениеПользователю;
			#КонецЕсли
			ЗаписьXML.ЗаписатьБезОбработки(СообщениеПользователю.Текст);
			ЗаписьXML.ЗаписатьБезОбработки(Символы.ПС);
		КонецЦикла;
		ЭтаФорма.СообщенияПользователю = ЗаписьXML.Закрыть();
	КонецЕсли;
...
Процедура РазделениеДанныхПредставлениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ОткрытьЗначениеЛкс(РазделениеДанных);
