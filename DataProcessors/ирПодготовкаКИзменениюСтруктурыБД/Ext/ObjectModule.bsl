...
Процедура ЗаполнитьПоРазницеМеждуКонфигурациями(ПолноеИмяФайла = "") Экспорт
	...
	СтрокаКоманды = """" + ирОбщий.ИмяИсполняемогоФайлаПлатформыЛкс() + """ " + "CREATEINFOBASE File=""" + ВременныйКаталог + """;";
		...
		Для Каждого МетаобъектТекущий Из КоллекцияТекущая Цикл
			...
			Для Каждого КолонкаТекущая Из ТаблицаНабораТекущая.Колонки Цикл
				...
				Если КолонкаНовая = Неопределено Тогда
					...
					Продолжить;
				КонецЕсли; 
				Если ирОбщий.ОписаниеТипов1ВходитВОписаниеТипов2Лкс(КолонкаТекущая.ТипЗначения, КолонкаНовая.ТипЗначения) Тогда 
				...
				Если ирОбщий.ОписаниеТипов1ВходитВОписаниеТипов2Лкс(КолонкаТекущая.ТипЗначения, ТипЗначенияНовый) Тогда 
					Продолжить;
				КонецЕсли; 
