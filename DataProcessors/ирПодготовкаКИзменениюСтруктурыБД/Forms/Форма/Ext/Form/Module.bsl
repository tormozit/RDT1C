...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Реквизит.ВыполнятьВТранзакции, Реквизит.УдаляемыеТипы";
...
Процедура ВывестиСоставРегистра(НовыйТекущийРегистр = Неопределено)
	...
	ПоляБД = ирОбщий.ПолучитьПоляТаблицыМДЛкс(МетаРегистр.ПолноеИмя());
...
Процедура КоманднаяПанельЗаполнитьУсекаемыеТипы(Кнопка, Таймаут = Неопределено)
	...
	Если Таймаут = Неопределено Тогда
		Таймаут = 10;
	КонецЕсли; 
	Ответ = Вопрос("Хотите заполнить удаляемые типы и измерения путем сравнения по именам объектов конфигурации БД с выбранным файлом конфигурации (Да),
		|иначе путем сравнения с основной конфигурацией (на 8.3.8+ полностью корректного) и требуется освободить конфигуратор (Нет)?", РежимДиалогаВопрос.ДаНет, Таймаут);
...
Процедура ПриОткрытии()
	...
	ЭтаФорма.УдаляемыеИзмеренияПодсказка = "Удаляемые измерения в виде ""РегистрСведений.<Имя>.Измерение.<Имя>""";
...
Процедура УдаляемыеИзмеренияНачалоВыбора(Элемент, СтандартнаяОбработка)
...
Процедура ЗатрагиваемыеЭлементыТекущегоПланаВидовХарактеристикПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
	
КонецПроцедуры

Процедура ПроблемныеПланыВидовХарактеристикПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
