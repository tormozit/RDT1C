...
Процедура ВывестиСоставРегистра(НовыйТекущийРегистр = Неопределено)
	...
	КонецЕсли; 
	ЛксОбновитьТекстПослеМаркераВСтроке(ЭлементыФормы.РамкаГруппыГруппыТекущегоРегистра.Заголовок, , 
...
Процедура ВывестиСоставПВХ(НовыйТекущийПВХ = Неопределено)
	...
	ЛксОбновитьТекстПослеМаркераВСтроке(ЭлементыФормы.РамкаГруппыЗатрагиваемыеЭлементыТекущегоПланаВидовХарактеристик.Заголовок, ,
...
Процедура ВывестиСоставГруппы(НоваяТекущаяГруппа = Неопределено)
	...
	ЛксОбновитьТекстПослеМаркераВСтроке(ЭлементыФормы.РамкаГруппыЭлементыГруппы.Заголовок, ,
...
Процедура ОбновитьДанные()
	...
	ЛксОбновитьТекстПослеМаркераВСтроке(ЭлементыФормы.ПанельОсновная.Страницы.РегистрыСведений.Заголовок, ,
		Строка(ПроблемныеРегистры.Количество()) + ")", "(");
		
	ЛксОбновитьТекстПослеМаркераВСтроке(ЭлементыФормы.ПанельОсновная.Страницы.ПланыВидовХарактеристик.Заголовок, ,
	...
	//ГруппыТекущегоРегистра.Сортировать(ЛксПолучитьСтрокуПорядка(ПостроительОтчетаОтбора.Порядок));
...
Процедура ЭлементыТекущейГруппыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка.Имя = "ОткрытьЗапись" Тогда 
		...
		ФормаСписка.ПараметрТекущаяСтрока = ЛксПолучитьКлючПоСтруктуреЗаписиРегистра(ВыбраннаяСтрока, Метаданные.РегистрыСведений[мТекущийРегистр.Имя]);
		ФормаСписка.Открыть();
	Иначе
...
Процедура КоманднаяПанельЗаполнитьУсекаемыеТипы(Кнопка, Таймаут = Неопределено)
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ВыборФайла.Фильтр = ирНеглобальный.ПолучитьСтрокуФильтраДляВыбораФайлаЛкс("CF");
