...
Процедура КоманднаяПанель1ЗаписатьКонстанты(Кнопка)
	...
	Для каждого Стр Из ТаблицаКонстант Цикл
		...
		Если ПравоДоступа("Изменение", Метаданные.Константы[Стр.ИдентификаторКонстанты], ПользователиИнформационнойБазы.ТекущийПользователь()) Тогда
			...
			Константы[Стр.ИдентификаторКонстанты].Установить(Стр.Значение);
			Стр.ПризнакМодификации = Ложь;
			ЗаписьЖурналаРегистрации("Редактирование констант", УровеньЖурналаРегистрации.Информация, Метаданные.Константы[Стр.ИдентификаторКонстанты], Стр.Значение);
...
Процедура ПрочитатьКонстантыИзБазы()
	...
	Для каждого Константа Из Метаданные.Константы Цикл
		Если НЕ ПравоДоступа("Чтение", Константа, ПользователиИнформационнойБазы.ТекущийПользователь()) Тогда
			Продолжить;
		КонецЕсли;
		Если Константа.Тип.Типы()[0] = Тип("ХранилищеЗначения") Тогда
			Продолжить;
		КонецЕсли; 
