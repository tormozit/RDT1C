	...
	Для каждого Константа Из Метаданные.Константы Цикл
		...
		Для Каждого МетаФункциональнаяОпция Из Метаданные.ФункциональныеОпции Цикл
			Если МетаФункциональнаяОпция.Хранение = Константа Тогда
				НоваяСтрока.ХранимаяФункциональнаяОпция = МетаФункциональнаяОпция.Имя;
			КонецЕсли; 
		КонецЦикла;
...
Процедура ТаблицаКонстантПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, , "Значение", РасширенныеКолонки, Истина, ТекущиеДанные);
...
Процедура МетодыПоследниеВыбранныеДобавить(Знач ТекущиеДанные = Неопределено)
	...
	КонецЕсли; 
	ирОбщий.ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, );
	ОбновитьПодменюПоследниеВыбранные();
...
Процедура ТаблицаКонстантВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		Если ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(Элемент, СтандартнаяОбработка, ТекущиеДанные.РасширенноеЗначение) Тогда 
			ОбновитьТипЗначенияВСтрокеТаблицы();
		КонецЕсли; 
	КонецЕсли; 
...
Процедура КоманднаяПанель1МенеджерТабличногоПоля(Кнопка)
	...
	ирОбщий.ОткрытьМенеджерТабличногоПоляЛкс(ЭлементыФормы.ТаблицаКонстант, ЭтаФорма);
...
Процедура КоманднаяПанель1ОтборБезЗначенияВТекущейКолонке(Кнопка)
	
	ирОбщий.ТабличноеПоле_ОтборБезЗначенияВТекущейКолонке_КнопкаЛкс(ЭлементыФормы.ТаблицаКонстант);
