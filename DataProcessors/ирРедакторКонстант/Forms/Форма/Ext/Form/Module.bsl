...
Процедура ПрочитатьКонстантыИзБазы()
	...
	Для каждого Константа Из Метаданные.Константы Цикл
		...
		Исключение
			...
			ирОбщий.СообщитьЛкс("Пропущена недоступная константа """ + Константа.Имя + """");
...
Процедура ОбновитьПодменюПоследниеВыбранные()
	...
	ирОбщий.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КоманднаяПанель1.Кнопки.ПоследниеВыбранные);
...
Процедура МетодыПоследниеВыбранныеДобавить(Знач ТекущиеДанные = Неопределено)
	...
	КонецЕсли; 
	ирОбщий.ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, ТекущиеДанные.ИдентификаторКонстанты, ТекущиеДанные.СинонимКонстанты);
	ОбновитьПодменюПоследниеВыбранные();
...
Процедура КоманднаяПанель1ОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонке_КнопкаЛкс(ЭлементыФормы.ТаблицаКонстант);
