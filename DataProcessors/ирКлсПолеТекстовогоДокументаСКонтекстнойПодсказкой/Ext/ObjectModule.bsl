...
Процедура ФорматироватьТекст()
	...
	Если ЗагрузитьТекстВКонструктор(ТекстЗапроса, КонструкторЗапроса) Тогда 
		ПолеТекстовогоДокумента.УстановитьГраницыВыделения(1, 1, СтрЧислоСтрок(ТекстЗапроса), 333);
...
Процедура ПерейтиКОпределению()
	...
	Если ЯзыкПрограммы = 1 Тогда
		RegExp.Global = Ложь;
		RegExp.Pattern = СтрЗаменить(шПоискОписанияТаблицы, "#Идентификатор#", ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(мКонтекст));
		...
		Иначе
			...
			Если Прав(мПредшествующийТекст, СтрДлина(МаркерЗначение)) = МаркерЗначение Тогда
				RegExp.Global = Ложь;
				RegExp.Pattern = шИмя;
				Если RegExp.Test(мКонтекст) Тогда 
					Попытка
						ПредопределенноеЗначение = ПредопределенноеЗначение(мКонтекст);
					Исключение
