...
Процедура УстановитьКонфигурациюМетаданных(пКонфигурация = Неопределено, пКонтекстВыполнения = Неопределено, ДиалектSQL = Неопределено) Экспорт
	...
	Если ЯзыкПрограммы = 1 Тогда
		...
		Если мДиалектыSQL = Неопределено Тогда
			ЭтотОбъект.мДиалектыSQL = ирНеглобальный.ПолучитьТаблицуИзТабличногоДокументаЛкс(ПолучитьМакет("ДиалектыSQL"),,,, Истина);
		КонецЕсли; 
	...
	//ИначеЕсли ирНеглобальный.СтрокиРавныЛкс(мДиалектSQL, "WQL") Тогда
...
Функция ЗаполнитьДоступныеТаблицыWQL() Экспорт
	...
	Индикатор = ЛксПолучитьИндикаторПроцесса(КоллекцияКлассов.Count, "Получаем метаданные");
	Для Каждого ЭлементКоллекции Из КоллекцияКлассов Цикл
		...
		СтрокаТаблицы.Описание = ирНеглобальный.ПолучитьОписаниеЭлементаWMIЛкс(ЭлементКоллекции);
...
Функция НайтиДобавитьДоступнуюВременнуюТаблицу(ИмяДоступнойТаблицы) Экспорт
	...
	Если СтрокаДоступнойТаблицы = Неопределено Тогда
		...
		ирНеглобальный.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаДоступнойТаблицы, "ПолноеИмя");
...
Функция ЗагрузитьТекстВКонструктор(ТекстЗапроса = Неопределено, КонструкторЗапроса = Неопределено, Знач СокращенноеДерево = Ложь, ДиалектSQL = Неопределено) Экспорт
	
	Если ТекстЗапроса = Неопределено Тогда
		ТекстЗапроса = ПолеТекстовогоДокумента.ПолучитьТекст();
	КонецЕсли;
	КонструкторЗапроса.УстановитьДиалектSQL(ДиалектSQL);
	НачальныйТокен = РазобратьТекстЗапроса(ТекстЗапроса, СокращенноеДерево,,, Истина);
	Если Истина
...
Функция РазобратьТекстЗапроса(ТекстЗапроса, СокращенноеДерево = Ложь, ОбновлятьСостояние = Истина, пПолеТекстовогоДокумента = Неопределено,
	СообщатьОПропущенныхТерминалах = Ложь) Экспорт 
	...
	Если ОбновлятьСостояние Тогда
		Индикатор = ЛксПолучитьИндикаторПроцесса(, "Синтаксическое дерево");
	КонецЕсли; 
	gpMsgTokenRead			= 1;
	gpMsgReduction			= 2;
	gpMsgAccept				= 3;
	...
	Пока Не Закончили Цикл 
		...
		Тогда
			...
			Закончили = Истина;
			ЛксСообщитьСУчетомМодальности(ПолучитьКоординатыВТекстеЗапроса(пПолеТекстовогоДокумента) + "Лексическая ошибка!",
		...
		ИначеЕсли Ответ = gpMsgSyntaxError Тогда
			...
			Тогда
				Если СообщатьОПропущенныхТерминалах Тогда
					ЛксСообщитьСУчетомМодальности(ПолучитьКоординатыВТекстеЗапроса(пПолеТекстовогоДокумента) + "Проигнорирован комментарий """ 
			...
			Иначе
				...
				КонецЕсли; 
				ЛксСообщитьСУчетомМодальности(ТекстОшибки, мСообщенияЧерезПредупреждения, СтатусСообщения.Важное);
