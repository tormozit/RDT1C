...
Функция ВставитьСсылкуНаОбъектБД(ТабличноеПолеПараметров, ИмяКолонкиИмени = "Имя", ИмяКолонкиЗначения = "Значение", ИмяСтруктурыПараметров = "",
	ОбновитьКопиюСвойстваВНижнемРегистре = Ложь, НачальноеЗначениеВыбора = Неопределено, РазрешитьВыбор = Истина) Экспорт
...
Процедура Закомментировать() Экспорт
	...
	Для Счетчик = мНачальнаяСтрока По мКонечнаяСтрока Цикл
		ПолеТекстовогоДокумента.УстановитьГраницыВыделения(мНачальнаяСтрока, 1, мНачальнаяСтрока, 1);
...
Процедура Раскомментировать() Экспорт
	...
	Для Счетчик = мНачальнаяСтрока По мКонечнаяСтрока Цикл
		...
		Если Лев(СокрЛ(СтрокаТекста), 2) = "//" Тогда
			...
			ПолеТекстовогоДокумента.УстановитьГраницыВыделения(мНачальнаяСтрока, 1, мНачальнаяСтрока, 10000);
			ПолеТекстовогоДокумента.ВыделенныйТекст = Сред(СтрокаТекста, 1, ПозицияМаркера - 1) + Сред(СтрокаТекста, ПозицияМаркера + 3);
		КонецЕсли; 
	КонецЦикла;
...
Функция НайтиДобавитьДоступнуюВременнуюТаблицу(Знач ИмяДоступнойТаблицы, ПородившийЗапрос = Неопределено) Экспорт
...
Функция РазобратьТекстЗапроса(ТекстЗапроса, СокращенноеДерево = Ложь, ОбновлятьСостояние = Истина, пПолеТекстовогоДокумента = Неопределено,
	...
	Если Ответ = gpMsgAccept Тогда
		НачальныйТокен = Новый Структура("Data, LineNumber, ColumnNumber, Name", мПарсер.CurrentReduction, 0, 0, "");
