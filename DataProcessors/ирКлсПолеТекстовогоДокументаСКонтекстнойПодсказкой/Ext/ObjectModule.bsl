...
Процедура Уничтожить() Экспорт
	...
	КонецЦикла;
	ЭтотОбъект.Автодополнение = Неопределено;
	ОчиститьТаблицуСловЛокальногоКонтекста();
...
Процедура ВнешнееСобытиеОбъекта(Источник, Событие, Данные) Экспорт 
	...
	Если Источник <> "KeyboardHook" Тогда
		Возврат;
	КонецЕсли; 
	Если Не ФормаВладелец.ВводДоступен() Тогда
		Возврат;
	КонецЕсли;
	...
	Если ФормаВладелец.ТекущийЭлемент = ПолеТекстовогоДокумента Тогда
		Если Символ = "(" Или Символ = "," Тогда
			...
			КонецЕсли;
		Иначе
			ФормаПодсказкаПоПараметрам = ФормаПодсказкиПоВызовуМетода();
			ФормаПодсказкаПоПараметрам.ПараметрСтруктураТипаКонтекста = Неопределено;
			Если ФормаПодсказкаПоПараметрам.Открыта() Тогда
				ФормаПодсказкаПоПараметрам.ОбновитьИлиЗакрытьФорму(, Истина);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
...
Функция ФормаПодсказкиПоВызовуМетода()
	
	ФормаПодсказкаПоПараметрам = ПолучитьФорму("ВызовМетода", ФормаВладелец);
	Возврат ФормаПодсказкаПоПараметрам;
...
Функция ЗаменитьОператорыВнешнегоПерехода(Знач Текст = Неопределено, выхТипыВнешнихПереходов, выхЗаглушкаВозврата = "", выхИмяПараметраТипаВыхода = "") Экспорт 
	...
	Пока Истина Цикл
		...
		Если ИмяОператора = "Возврат" Тогда
			КоординатыОшибки.НомерКолонки = КоординатыОшибки.НомерКолонки - 1;
		КонецЕсли;
		ПолеТекстовогоДокумента.УстановитьГраницыВыделения(КоординатыОшибки.НомерСтроки, КоординатыОшибки.НомерКолонки - СтрДлина(ИмяОператора), КоординатыОшибки.НомерСтроки, КоординатыОшибки.НомерКолонки);
...
Функция КоординатыОшибки(Знач ИнформацияОбОшибке, Знач СтартоваяСтрока = 0) Экспорт 
	...
	КоординатыОшибки.НомерСтроки = Число(ирОбщий.СтрокаМеждуМаркерамиЛкс(ИнформацияОбОшибке.Описание, "{(", ",")) + СтартоваяСтрока;
	КоординатыОшибки.НомерКолонки = Число(ирОбщий.СтрокаМеждуМаркерамиЛкс(ИнформацияОбОшибке.Описание, ",", ")}"));
...
//
Функция ОткрытьПоискВСинтаксПомощнике(ПутьКСлову, СтрокаОписания = Неопределено, ВключатьПутьКОписаниюТипаЗначения = Ложь, выхФормаВыбора = Неопределено, РазрешитьАнализИмениТипа = Истина,
	...
	Иначе
		Если ВладелецФормы <> Неопределено И СтруктураТипаКонтекста.СтрокаОписания <> Неопределено Тогда
			...
			Если ТаблицаВладелец.Колонки.Найти("ЛиЭкспорт") <> Неопределено  Тогда
				ФормаВызовМетода = ФормаПодсказкиПоВызовуМетода();
...
Процедура ОткрытьКонтекстнуюСправку(ПутьКСлову = "", ВладелецФормы = Неопределено) Экспорт
	...
	КонецЕсли;
	ОткрытьПоискВСинтаксПомощнике(ПутьКСлову,,,, РазобратьТекущийКонтекст, ВладелецФормы);
...
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, КакВызовМетода = Ложь)
	...
	мПлатформа.ИнициализацияОписанияМетодовИСвойств();
	ВыделитьНачалоИКонецТекущейСтроки();
	мЭтоСтроковыйЛитерал = (?(Лев(СокрЛ(ТекущееНачалоСтроки), 1) = "|", 1, 0) + СтрЧислоВхождений(ТекущееНачалоСтроки, """")) % 2 = 1;
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	Пока ТаблицаСтруктурТиповКонтекста.Количество() > 0 Цикл
		...
		Если Автодополнение = Неопределено Тогда
			Автодополнение = ПолучитьФорму("Автодополнение", ФормаВладелец);
			Автодополнение.КлючСохраненияПоложенияОкна = ЯзыкПрограммы;
		КонецЕсли; 
		Попытка
			Автодополнение.СтруктураТипаКонтекста = СтруктураТипаКонтекста;
		Исключение
		...
		Если ЗначениеЗаполнено(СтруктураТипаКонтекста.ТипЯзыка) Тогда
			Автодополнение.Контекст = СтруктураТипаКонтекста.ТипЯзыка;
		Иначе
			Автодополнение.Контекст = мРодительскийКонтекст;
		КонецЕсли; 
		Автодополнение.ТекущееСлово = НачалоСлова;
		Автодополнение.ЗапомнитьПозициюКаретки();
		ПараметрЗакрытияПодсказки = Автодополнение.ОткрытьМодально();
		ирОбщий.УстановитьФокусВводаФормеЛкс(ФормаВладелец);
		Если ПараметрЗакрытияПодсказки = Неопределено Тогда
			Возврат;
		КонецЕсли; 
		СтрокаРезультата = Автодополнение.СтрокаСловаРезультата;
		...
		Тогда
			...
			Если ПараметрЗакрытияПодсказки = "." Тогда
				...
				Иначе
					ТаблицаСтруктурТиповКонтекста = ОпределитьТипДочернегоКонтекста(ирОбщий.БыстрыйМассивЛкс(Автодополнение.СтруктураТипаКонтекста),
...
Процедура ОбновитьТипЗначенияИзТаблицыСтруктурТипов(Знач НоваяСтрока, Знач ТаблицаСтруктурТипов, ЗаменитьСуществующий = Истина) Экспорт 
	...
	НовыйТипЗначения = ПолучитьПредставлениеМассиваСтруктурТипов(ТаблицаСтруктурТипов);
...
шВызовМетодаПрограммы = "(?:" + шПредИмя + "(Новый|New)\s+)?(" + шИмя + шСкобки + "?" + "(?:(?:\.(?:" + шИмя + ")" + шСкобки + "?)|" + шИндекс + ")*?)"
	+ "\(((?:(?:" + шВыражениеПрограммы + ")?" + шРазделитель + "*,)*)" + "(" + шНачалоВыраженияПрограммы + ")?" + шРазделитель + "*";

//шПрисваивание = "(" + шРазделитель + "*=" + шРазделитель + "*((новый|New)" 
