...
Процедура Закомментировать() Экспорт
	...
	ТекстДоИзменения = ПолеТекстовогоДокумента.ПолучитьТекст();
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	ВычислятьТипыВСписке = ВосстановитьЗначение(ИмяКласса + ".ВычислятьТипыВСписке") = Истина Или ЯзыкПрограммы <> 0;
	Если Ложь
		Или ПолеТекстовогоДокумента.ТолькоПросмотр
		Или ФормаВладелец.ТолькоПросмотр
	Тогда
