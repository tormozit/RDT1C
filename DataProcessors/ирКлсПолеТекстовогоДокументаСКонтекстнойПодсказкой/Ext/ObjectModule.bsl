...
Перем ФормаАвтодополнение Экспорт;
Перем ФормаВызовМетода Экспорт;
...
Процедура Уничтожить() Экспорт
	...
	КонецЦикла;
	Если ФормаВызовМетода <> Неопределено Тогда
		ЭтотОбъект.ФормаВызовМетода.ВладелецФормы = Неопределено;
	КонецЕсли; 
	ЭтотОбъект.ФормаВызовМетода = Неопределено;
	Если ЭтотОбъект.ФормаАвтодополнение <> Неопределено Тогда
		ЭтотОбъект.ФормаАвтодополнение.ВладелецФормы = Неопределено;
	КонецЕсли; 
	ЭтотОбъект.ФормаАвтодополнение = Неопределено;
	ОчиститьТаблицуСловЛокальногоКонтекста();
...
Функция ФормаВызовМетода()
	...
	Если ФормаВызовМетода = Неопределено Тогда
		ФормаВызовМетода = ПолучитьФорму("ВызовМетода", ФормаВладелец);
	КонецЕсли; 
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	Пока ТаблицаСтруктурТиповКонтекста.Количество() > 0 Цикл
		...
		Если ФормаАвтодополнение = Неопределено Тогда
			Если ирОбщий.ЛиПеренаправлениеКлавиатурногоВводаЛкс() Тогда
				ИмяФормы = "Автодополнение";
			Иначе
				ИмяФормы = "АвтодополнениеCOM";
			КонецЕсли; 
			ФормаАвтодополнение = ПолучитьФорму(ИмяФормы, ФормаВладелец);
			ФормаАвтодополнение.КлючСохраненияПоложенияОкна = ЯзыкПрограммы;
		КонецЕсли; 
		Попытка
			ФормаАвтодополнение.СтруктураТипаКонтекста = СтруктураТипаКонтекста;
		Исключение
		...
		Если ЗначениеЗаполнено(СтруктураТипаКонтекста.ТипЯзыка) Тогда
			ФормаАвтодополнение.Контекст = СтруктураТипаКонтекста.ТипЯзыка;
		Иначе
			ФормаАвтодополнение.Контекст = мРодительскийКонтекст;
		КонецЕсли; 
		ФормаАвтодополнение.ТекущееСлово = НачалоСлова;
		...
		Тогда
			...
			Если ПараметрЗакрытияПодсказки = "." Тогда
				...
				Иначе
					ТаблицаСтруктурТиповКонтекста = ОпределитьТипДочернегоКонтекста(ирОбщий.БыстрыйМассивЛкс(ФормаАвтодополнение.СтруктураТипаКонтекста),
