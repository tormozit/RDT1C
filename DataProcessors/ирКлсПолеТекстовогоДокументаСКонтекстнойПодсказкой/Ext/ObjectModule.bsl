...
// Находит первое вхождение слова в тексте. Если слово найдено, устанавливается выделение и фокус.
 ...
 //  СтрокаПоиска – Строка.
 //
Функция НайтиПоказатьСловоВТексте(СтрокаПоиска) Экспорт 
	
	Результат = ирОбщий.НайтиПоказатьСтрокуВПолеТекстовогоДокументаЛкс(ФормаВладелец, ПолеТекстовогоДокумента, СтрокаПоиска);
	Возврат Результат; 
	
КонецФункции // НайтиПоказатьСловоВТексте()
		...
		Если ЯзыкПрограммы = 0 Тогда
			...
			Для Каждого Match Из Результат Цикл
				...
				Если ЗначениеЗаполнено(НайденныйТекст) Тогда
					ЗалитыйТекст = ЗалитыйТекст + Лев(НайденныйТекст, 1) + RegExp.Replace(Сред(НайденныйТекст, 2, СтрДлина(НайденныйТекст) - 2), СимволЗамена)
