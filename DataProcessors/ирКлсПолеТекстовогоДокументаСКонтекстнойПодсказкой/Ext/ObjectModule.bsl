...
Функция Нажатие(Кнопка) Экспорт
	...
	ИначеЕсли Команда = "ВводСкобки" Тогда
		Если ФормаВладелец.ТекущийЭлемент = ПолеТекстовогоДокумента Тогда
			...
			ПодсказкаПоВызовуМетодаПриВводеСкобки = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ПодсказкаПоВызовуМетодаПриВводеСкобки");
			Если ПодсказкаПоВызовуМетодаПриВводеСкобки = Истина Тогда
...
Функция ОткрытьПоискВСинтаксПомощнике(ПутьКСлову, СтрокаОписания = Неопределено, ВключатьПутьКОписаниюТипаЗначения = Ложь, выхФормаВыбора = Неопределено, РазрешитьАнализИмениТипа = Истина) Экспорт
	...
	Иначе
		...
		#Если Сервер И Не Сервер Тогда
			мПлатформа = Обработки.ирПлатформа.Создать();
		#КонецЕсли
		мПлатформа.ВыбратьСтрокуОписанияИзМассиваСтруктурТипов(СтруктураЦикла, ВключатьПутьКОписаниюТипаЗначения, ФормаВладелец, Слово, мНомерПараметра,, выхФормаВыбора, мФактическиеПараметры.Количество());
	КонецЕсли; 
...
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, КакВызовМетода = Ложь)
	...
	//Если Результат.Count > 0 Тогда
	//	Вхождение = Результат.Item(0);
	//	мВызовМетода = Вхождение.SubMatches(1) + "(";
	//	ТекстПараметров = Вхождение.SubMatches(5);
	//	мЭтоКонструктор = ЗначениеЗаполнено(Вхождение.SubMatches(0));
	//	RegExp.Global = Истина;
	//	Если ЯзыкПрограммы = 0 Тогда
