...
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь)
	...
	Если Результат.Count > 0 Тогда
		...
		Если Лев(Результат.Item(0).Value, 1) = """" Тогда
			РазобратьКонтекстПараметра();
			НачалоСлова = НачалоКонтекста;
			Возврат;
		КонецЕсли; 
	КонецЕсли;
...
Процедура РазобратьКонтекстПараметра()
	...
	Если Результат.Count > 0 Тогда
		...
		КонецЕсли;
	КонецЕсли;
	мИмяМетода = ирОбщий.ПервыйФрагментЛкс(ирОбщий.ПоследнийФрагментЛкс(мКонтекст), "(");
	мРазбиратьКонтекст = Ложь;
...
Функция ОпределитьТипЗначенияКонтекста(Контекст = "", ТекстДляПоискаОпределения = "", ПредшествующийТекст = "") Экспорт
	...
	ИначеЕсли Результат.Count > 0 Тогда 
		...
		Если МассивРодительскихСтрутурТипа.Количество() = 0 Тогда
			МассивРодительскихСтрутурТипа = ОпределитьТипЗначенияКонтекста(РодительскийКонтекст, ТекстДляПоискаОпределения, ПредшествующийТекст);
		КонецЕсли;
...
Функция ПолучитьТаблицуСтруктурТиповТекущегоВыражения(ЛиСправаОтРавенства = Ложь) Экспорт
	...
	лКонтекст = ?(ЛиСправаОтРавенства Или мНомерПараметра > 0, мКонтекст, мРодительскийКонтекст);
	МассивЗащитыОтРекурсии.Очистить();
	Попытка
		ТаблицаСтруктурТиповКонтекста = ОпределитьТипЗначенияКонтекста(лКонтекст, " " + мТекстДляПоискаОпределения, мПредшествующийТекст);
	Исключение
...
Процедура ВызватьКонтекстнуюПодсказку()
	...
	Пока ТаблицаСтруктурТиповКонтекста.Количество() > 0 Цикл
		...
		КонецЕсли;
		СтруктураТипаКонтекста = мПлатформа.ПолучитьНовуюСтруктуруТипа();
		ОтображаемыйКонтекст = мРодительскийКонтекст;
		Если мНомерПараметра > 0 Тогда 
			...
			Если СтрокиПараметров.Количество() >= мНомерПараметра Тогда
				...
				ОтображаемыйКонтекст = ПодготовитьИмяПараметраМетода(СтрокаПараметра.Параметр);
			КонецЕсли; 
		...
		КонецПопытки;
		ФормаПодсказки.Контекст = ОтображаемыйКонтекст;
