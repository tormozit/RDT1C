...
Процедура ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, СтароеИмяТаблицыИлиПараметра = "", НовоеИмяТаблицыИлиПараметра = "", пСтароеИмяПоля = "", пНовоеИмяПоля = "",
	...
	Для Каждого ЗапросПакета Из Пакет Цикл
		...
		Для Каждого ЧастьОбъединения Из лЧастиОбъединения Цикл
			СтароеИмя = пСтароеИмяПоля;
			НовоеИмя = пНовоеИмяПоля;
			Если ГлобальныйОбход Тогда
				Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
					...
					КонецЦикла;
					Если ВыбраннаяТаблица.ВложенныйПакет <> Неопределено Тогда
						ПереименоватьТаблицуИлиПараметр(ВыбраннаяТаблица.ВложенныйПакет, СтароеИмяТаблицыИлиПараметра, НовоеИмяТаблицыИлиПараметра, СтароеИмя, НовоеИмя);
					ИначеЕсли ирОбщий.СтрокиРавныЛкс(ВыбраннаяТаблица.ПолноеИмя, СтароеИмя) Тогда
						ВыбраннаяТаблица.ПолноеИмя = НовоеИмя;
					КонецЕсли;
				КонецЦикла;
				СтрокаВыбраннойТаблицы = ЧастьОбъединения.ВыбранныеТаблицы.Найти(СтароеИмяТаблицыИлиПараметра, "ПолноеИмя");
				Если СтрокаВыбраннойТаблицы <> Неопределено Тогда
					СтрокаВыбраннойТаблицы.ПолноеИмя = НовоеИмяТаблицыИлиПараметра;
					Если ЗначениеЗаполнено(СтароеИмя) Тогда
						СтароеИмя = СтароеИмяТаблицыИлиПараметра + "." + СтароеИмя;
						НовоеИмя = НовоеИмяТаблицыИлиПараметра + "." + НовоеИмя;
					КонецЕсли; 
				Иначе
					Продолжить;
				КонецЕсли; 
