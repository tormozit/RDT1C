...
Функция ПолучитьТекущуюКомпонентуКонтекстнойПодсказки(Кнопка = Неопределено)
	...
	Если ТекущийЭлемент <> Неопределено И ПолеТекстовогоДокументаСКонтекстнойПодсказкой.Свойство(ТекущийЭлемент.Имя) Тогда
...
Процедура КПДоступныеТаблицыВключитьКоличествоСтрок(Кнопка)
	...
	Если Кнопка.Пометка Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			...
			#КонецЕсли
			ирОбщий.ВычислитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(,,,,,, ЭтаФорма, ЭлементыФормы.КПДоступныеТаблицы.Кнопки.ВключитьКоличествоСтрок, "ОбновитьСтатистикуЗавершение");
		Иначе
