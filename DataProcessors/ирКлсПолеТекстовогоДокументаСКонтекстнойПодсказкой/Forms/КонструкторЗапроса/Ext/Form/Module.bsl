...
Функция ПолучитьВыражениеЭлементаОтбораЛкс(ЭлементОтбора, Знач Отступ = 0, Знач ГруппаИзОдногоЭлемента = Ложь, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено, СтруктураПолей = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ЭлементОтбора.Представление) Тогда
		...
		Если Истина
			И СтруктураПолей <> Неопределено
			И СтруктураПолей.Режим = "Замена"
			И ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("COMОбъект") // Там может быть строка "ИСТИНА"
		Тогда
		...
		Тогда
			...
			Если ТипЗнч(ПравоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
				...
				Если XMLТип <> Неопределено Тогда
					...
					Тогда
						ИмяПараметра = ДобавитьЗначениеВПараметры(ПравоеЗначение, XMLТип);
				...
				КонецЕсли; 
			КонецЕсли; 
			СтрокаПравогоЗначения = ПолучитьПредставлениеЗначенияНаЯзыке(ПравоеЗначение, СтруктураПолей);
			Если Ложь
...
Функция ДобавитьЗначениеВПараметры(ЗначениеПараметра, XMLТип = Неопределено)
	...
	Если СтрокаПараметра = Неопределено Тогда
		...
		КонецЕсли; 
		ИмяПараметра = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(Параметры, БазовоеИмяПараметра,,, ПолучитьСловоЯзыкаЗапросов("Parameter"));
		СтрокаПараметра = Параметры.Добавить();
...
Функция ДобавитьОтборыПоСтрокеРезультата(СтрокаРезультата, ЗапросДоГруппировки = Ложь) Экспорт 
	...
	Для Каждого КолонкаРезультата Из РезультатЗапроса.Колонки Цикл
		...
		Если ТипЗначения = Тип("Null") Тогда
			ЭлементОтбора.Представление = "(" + ВыбранноеПоле.Определение + ") ЕСТЬ NULL";
		Иначе
			ИмяПараметра = ДобавитьЗначениеВПараметры(ЗначениеКолонки);
			ЭлементОтбора.Представление = "(" + ВыбранноеПоле.Определение + ") = &" + ИмяПараметра;
		КонецЕсли; 
	КонецЦикла;
	Возврат Результат;
...
Функция ВыбратьВсеПоляИспользованныеВВыражениях(ЗапросДоГруппировки = Ложь, выхЕстьАгрегаты = Ложь) Экспорт 
	...
	СтруктураПолей.Вставить("ЗапросДоГруппировки", ЗапросДоГруппировки);
	СтруктураПолей.Вставить("ВнутриОтбора", Ложь);
	СтруктураПолей.Вставить("ЕстьАгрегатыВЧастиОбъединения", ЗапросДоГруппировки);
