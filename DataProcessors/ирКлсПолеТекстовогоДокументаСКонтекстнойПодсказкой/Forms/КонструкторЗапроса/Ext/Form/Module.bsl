...
Перем мИндексПринимающегоПоля; // Используется при разборе текста запроса для INSERT
...
Процедура ЗаполнитьДоступныеТаблицы()
	...
	КонецЕсли;
	ДоступныеТаблицы.Сортировать("Имя");
...
Функция ЗаполнитьПоТокену(Знач Токен, Стек = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено, Индикатор = Неопределено,
	...
	ИначеЕсли ИмяПравила = "<Fetch>" Тогда
		ЗапросПакета.ОпцияПервые = Истина;
		ЗапросПакета.ПервыеКоличество = Число(Данные.Tokens(2).Data);
	...
	ИначеЕсли ИмяПравила = "<InsertExpr>" Тогда
		ТекущийУзел = ЗапросПакета.ПринимающиеПоля[мИндексПринимающегоПоля];
				...
				Если XMLТип <> Неопределено Тогда
					...
					Тогда
						...
						Если СтрокаПараметра = Неопределено Тогда
							ИмяПараметра = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(Параметры, "" + ПравоеЗначение,,, ПолучитьСловоЯзыкаЗапросов("Parameter") + "1");
...
Функция РазобратьВыражениеЗапроса(ВыражениеЗапроса) Экспорт
	...
	НачальныйТокен = НачальныйТокен.Data.Tokens(0).Data.Tokens(0).Data.Tokens(5).Data.Tokens(0).Data.Tokens(1).Data.Tokens(1); // При UPDATE
...
Функция СобратьТекстЗапросаПакета(ЗапросПакета = Неопределено, Смещение = "")
	...
	ИначеЕсли ЗапросПакета.ТипЗапроса = 3 Тогда
		...
		Для Каждого ПринимающееПоле Из ЗапросПакета.ПринимающиеПоля Цикл
			...
			ТекстВыраженияЗначений = ТекстВыраженияЗначений + ОбновитьВыражениеЗапроса(ПринимающееПоле,,,,,,,,, 0);
			ТекстВыраженияЗначений = ТекстВыраженияЗначений + " " + ПолучитьКомментарийДляВставки(ПринимающееПоле.Поля, Смещение + Символы.Таб);
		КонецЦикла;
		Если ТекстПринимающиеПоля <> "" Тогда
			Результат = Результат + "INSERT INTO " + ЗапросПакета.ИмяОсновнойТаблицы + " (" + ТекстПринимающиеПоля + ")" + Символы.ПС
				+ "VALUES (" + ТекстВыраженияЗначений + ")";
		КонецЕсли;
		Возврат Результат;
	ИначеЕсли ЗапросПакета.ТипЗапроса = 4 Тогда
		...
		КонецЦикла;
		Если ТекстПринимающиеПоля <> "" Тогда
			Результат = Результат + "UPDATE " + ЗапросПакета.ИмяОсновнойТаблицы + " SET " + Символы.ПС + Смещение + Символы.Таб + ТекстПринимающиеПоля;
		КонецЕсли; 
	...
	Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
		...
		Если ЧастьОбъединения.РучноеИмя Тогда
			Результат = Результат + ПараметрыДиалектаSQL.СтрочныйКомментарий + мПлатформа.мМаркерИмениЧастиОбъединения + ЧастьОбъединения.Имя + " " 
				+ ирОбщий.ПолучитьСтрокуПовторомЛкс(ПараметрыДиалектаSQL.СтрочныйКомментарий, 20);
			Результат = Результат + Символы.ПС + Смещение;
		КонецЕсли; 
...
Функция ДобавитьДоступнуюТаблицу1С(ПолноеИмя, ТипТаблицы = "", Имя = "", СхемаТаблицы = "", ПородившийЗапрос = 0)
	...
	СтрокаТаблицы = ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(ДоступныеТаблицы, ПолноеИмя,, ТипТаблицы, Имя, , СхемаТаблицы, Истина);
...
Функция ПолучитьПоляДоступнойТаблицы(СтрокаДоступнойТаблицы, ПараметрыТаблицы = Неопределено)
	...
	ИначеЕсли ТипЗнч(Конфигурация) = Тип("ОбъектМетаданныхКонфигурация") Тогда
		...
		Если ПараметрыТаблицы <> Неопределено Тогда
			...
			КонецЦикла;
		КонецЕсли; 
		ПоляТаблицыБД = ирОбщий.ПолучитьПоляТаблицыМДЛкс(ПолноеИмяТаблицы, Ложь, ИндексПараметраПериодичность, ОпределениеПараметраПериодичность);
		#Если _ Тогда
		...
		Иначе // ADO
			...
			Если ADOUtils <> Неопределено Тогда
				...
				Для Каждого СтрокаТЗ Из ОписаниеПолейТЗ Цикл
					...
					FieldADOЛкс = ирОбщий.FieldADOЛкс(СтрокаПоля.Имя, НомерТипа, ДлинаТипа, ЧисловаяТочность, ЧисловаяРазрядность);
					СтрокаПоля.ТипЗначения = ирОбщий.FieldADO_ПолучитьТип1CЛкс(FieldADOЛкс);
				КонецЦикла;
			Иначе
...
Процедура ОбновлениеОтображения()
	...
	Если ПараметрыДиалектаSQL <> Неопределено Тогда
		...
		ЭлементыФормы.ИмяОсновнойТаблицы.Доступность = Ложь
			Или ТипЗапроса <> 0;
		ЭлементыФормы.КомментарийОтбора.Доступность = ТипЗапроса <> 2 И ТипЗапроса <> 3;
		...
		ЭлементыФормы.ПанельОсновная.Страницы.Параметры.Видимость = ТипЗапроса <> 2 И ТипЗапроса <> 3;
		...
		ЭлементыФормы.ПанельЧасти.Страницы.ВыбранныеТаблицы.Видимость = ТипЗапроса <> 2 И ТипЗапроса <> 3;
		ЭлементыФормы.ПанельЧасти.Страницы.ДеревоУсловия.Видимость = ТипЗапроса <> 2 И ТипЗапроса <> 3;
		ЭлементыФормы.ПанельЧасти.Страницы.Группировки.Видимость = ТипЗапроса < 2;
		ЭлементыФормы.ПанельЧасти.Страницы.Текст.Видимость = ТипЗапроса <> 2 И ТипЗапроса <> 3;
...
Процедура ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, Знач СтароеИмя, Знач НовоеИмя)
	...
	ЭтоПереименованиеПараметра = ПараметрыДиалектаSQL.ПрефиксПараметра = Лев(СтароеИмя, 1);
	Если Пакет = Неопределено Тогда
	...
	Если ТипЗнч(Токен) = Тип("COMОбъект") Тогда
		Если Токен.Kind = 0 Тогда
			...
			ИначеЕсли ИмяПравила = "<Meaning>" Тогда
				...
				ИмяСсылочногоТипа = ирОбщий.ПолучитьИмяТипаСсылкиТаблицыБДЛкс(ИмяСсылочногоТипа);
				Попытка
...
Процедура ПараметрыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если мИмяТекущегоПараметра <> НовоеИмяПараметра Тогда
			ПереименоватьТаблицуИлиПараметр(, ПараметрыДиалектаSQL.ПрефиксПараметра + мИмяТекущегоПараметра, ПараметрыДиалектаSQL.ПрефиксПараметра + НовоеИмяПараметра);
		КонецЕсли; 
	КонецЕсли; 
...
Процедура СохранитьЗапросПакета()
	...
	Если ТипЗапроса = 2 Или ТипЗапроса = 3 Тогда
...
Функция СкопироватьЗапрос(Знач ЗапросИсточник, ЗапросПриемник = Неопределено)
	...
	Если ЗапросПриемник <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ЗапросПриемник, ЗапросыПриемника[0]); 
	Иначе
	...
	Если РезультатФормы <> Неопределено Тогда
		Если ВложенныйПакет <> Неопределено Тогда
			...
			Для Каждого СтрокаСтарогоПоля Из ВложенныйПакет[0].ПоляОбъединения Цикл
				...
				Тогда
					ПереименоватьТаблицуИлиПараметр(, СтрокаВыбраннойТаблицы.Имя + "." + СтрокаСтарогоПоля.Имя, СтрокаВыбраннойТаблицы.Имя + "." + СтрокаНовогоПоля.Имя);
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли; 
...
Процедура ПараметрыТаблицыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		...
		Если ЭтоПараметрОтбор1С(ВыбраннаяСтрока) Тогда
			...
			ФормаКонструктораВыражения.КомпоновщикНастроек = ирКэш.ПолучитьКомпоновщикТаблицыМетаданныхЛкс(ИмяОсновнойТаблицы,,,, Истина);
		КонецЕсли; 
...
Процедура ВыбранныеПоляИмяПриИзменении(Элемент)
	...
	Если СтрокаПоляОбъединенияПоИмени <> СтрокаПоляОбъединенияПоИД Тогда // заработало в 3.19, ранее ошибочно никогда не выполнялось
...
Процедура КПВыбранныеТаблицыПреобразоватьВПодзапрос(Кнопка)
	...
	ДоступнаяТаблица = ДоступныеТаблицы.Найти(НРег(ТекущаяСтрока.ПолноеИмя), "НПолноеИмя");
...
Процедура ДоступныеПоляПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Иначе
		...
		Если ДанныеСтроки.Папка Тогда
			...
			Если СтрокаВыбраннойТаблицы <> Неопределено Тогда
				Если СтрокаВыбраннойТаблицы.ВложенныйПакет <> Неопределено Тогда
					КартинкаТипаТаблицы = ирОбщий.ПолучитьОбщуюКартинкуЛкс("ирВложенныйЗапрос");
				Иначе
					...
					Если КартинкаТипаТаблицы.Вид = ВидКартинки.Пустая Тогда
						СтрокаДоступнойТаблицы = ДоступныеТаблицы.Найти(НРег(СтрокаВыбраннойТаблицы.ПолноеИмя), "НПолноеИмя");
						Если СтрокаДоступнойТаблицы <> Неопределено Тогда
			...
			ИначеЕсли Найти("" + ДанныеСтроки.Поле, "._Индексы") > 0 Тогда 
				КартинкаТипаТаблицы = ирОбщий.ПолучитьОбщуюКартинкуЛкс("ирИндексы");
				ОформлениеСтроки.Ячейки[0].УстановитьКартинку(КартинкаТипаТаблицы);
			КонецЕсли; 
		Иначе
...
ЭлементыФормы.ПодстрокаФильтраДоступныхТаблиц.Данные = "ЭлементыФормы.ДоступныеТаблицы.Отбор.Имя.Значение";
ЭлементыФормы.ПодстрокаФильтраДоступныхТаблиц.КнопкаВыбора = Ложь;
ЭлементыФормы.ПодстрокаФильтраДоступныхТаблиц.КнопкаСпискаВыбора = Истина;
