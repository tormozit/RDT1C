				...
				Если ИмяПоля = "*" Тогда
					...
					Иначе
						...
						Если СтрокаВыбраннойТаблицы = Неопределено Тогда
							...
							СтрокаВыбраннойТаблицы.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(ЧастьОбъединения.ВыбранныеТаблицы,  ИмяТаблицы);
...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	Если Истина
		...
		И Найти(СтрокаПоля.Определение, ">") = 0
		И Найти(СтрокаПоля.Определение, "<") = 0
		И СтрЧислоВхождений(СтрокаПоля.Определение, ".") = 1
		И (Ложь
	...
	Иначе 
		...
		КонецЕсли; 
		ИмяПоля = ПолучитьСловоЯзыкаЗапросов("Поле") + XMLСтрока(СчетчикБезымянныхПолей);
...
Процедура ВыбранныеПоляИмяПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(НовоеИмя) Тогда
		НовоеИмя = ПолучитьАвтоПсевдонимПоля(мТекущаяСтрокаВыбранногоПоля);
		мТекущаяСтрокаВыбранногоПоля.Имя = НовоеИмя;
	КонецЕсли; 
