...
Функция ПолучитьПолноеИмяДоступногоПоля(ДоступноеПоле)
	...
	Если ЭтоПолеПараметра(ДоступноеПоле.Поле) Тогда
		Результат = ПараметрыДиалектаSQL.ПрефиксПараметра + ирОбщий.ПоследнийФрагментЛкс(ДоступноеПоле.Поле);
	Иначе
		Результат = "" + ДоступноеПоле.Поле;
	КонецЕсли; 
	Возврат Результат;
	...
	Если Не ЗначениеЗаполнено(ЗначениеПараметра) Тогда
		...
		Если АнглийскийРежим Тогда
			...
			ИначеЕсли Не ЗначениеЗаполнено(БазовоеИмяПараметра) Тогда
				...
				КонецЕсли;
				БазовоеИмяПараметра = БазовоеИмяПараметра + ТРег(ирОбщий.ПоследнийФрагментЛкс(XMLТип.ИмяТипа));
			КонецЕсли; 
		Иначе
