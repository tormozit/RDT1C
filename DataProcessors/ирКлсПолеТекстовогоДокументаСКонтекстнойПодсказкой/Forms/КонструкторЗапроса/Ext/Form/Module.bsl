		...
		Если СтрокаПоляПостроителя = Неопределено Тогда
			...
			Если СтрокаПоляПостроителя.Владелец().Колонки.Найти("Определение") <> Неопределено Тогда
				Если Истина
					...
					И Не ЗначениеЗаполнено(СтрокаПоляПостроителя[ИмяСвойстваИмениПриемника]) 
				Тогда
...
Функция СкорректироватьИмяВыбраннойТаблицы(Знач ВыбраннаяТаблица, ПринудительноАвтоимя = Ложь)
	...
	КонецЕсли;
	ВыбраннаяТаблица.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ВыбраннаяТаблица.Владелец(), ВыбраннаяТаблица,,,, 0);
...
Функция ДобавитьОтборыПоСтрокеРезультата(СтрокаРезультата, ЗапросДоГруппировки = Ложь, ИменаПолей = Неопределено) Экспорт 
	...
	Для Каждого КолонкаРезультата Из РезультатЗапроса.Колонки Цикл
		...
		Если КомпоновщикЧасти.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ВыбранноеПоле.Определение)) = Неопределено Тогда
			// https://www.hostedredmine.com/issues/897767
			Продолжить;
		КонецЕсли; 
