...
Функция СобратьТекстЗапросаПакета(ЗапросПакета = Неопределено, Смещение = "") Экспорт 
...
// Текст FROM
Функция СобратьТекстИЗ(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Знач Смещение = "")
	...
	Для Каждого ГруппаСвязей Из ГруппыСвязей Цикл
		...
		СобратьВеткуСвязейТаблиц(НомерОткрытойГруппы, ГруппаСвязей, ЗапросПакета, Смещение + Символы.Таб, ТекстТаблицы, ЧастьОбъединения);
...
Процедура СобратьВеткуСвязейТаблиц(НомерОткрытойГруппы, Знач ВеткаСвязей, ЗапросПакета, Смещение, ТекстТаблицы, ЧастьОбъединения)
	...
	Для Каждого СтрокаСвязи Из ВеткаСвязей.Строки Цикл
		...
		ТекстУсловия = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(СтрокаСвязи.Условие.Отбор, 1,,, ЧастьОбъединения.Компоновщик.Настройки);
...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	Если ЛиВыражениеЯвляетсяПолем(ОпределениеПоля) Тогда 
...
Функция ЛиЛитералЗначения(Знач ИмяПоля, ПроверятьСкобки = Ложь)
	...
	Возврат Ложь
		Или ирОбщий.СтрокиРавныЛкс(ИмяПоля, СловоNULL)
		Или ирОбщий.СтрокиРавныЛкс(ИмяПоля, СловоНЕОПРЕДЕЛЕНО)
		Или ирОбщий.СтрокиРавныЛкс(ИмяПоля, СловоЛОЖЬ)
		Или ирОбщий.СтрокиРавныЛкс(ИмяПоля, СловоИСТИНА);
...
Функция МенюДоступныхПолейПерейтиКВыбраннойТаблице(Кнопка = Неопределено, ПерейтиКОпределению = Неопределено)
	...
	Если ИсточникДействия = ЭлементыФормы.ВыражениеВыбранногоПоля Тогда
		...
		Если ЛиВыражениеЯвляетсяПолем(ТекущееВыражение) Тогда 
...
Процедура УдалитьВыбранныеПоляИПорядок(УдаляемыеВыбранныеПоля) Экспорт 
	
	Для Каждого СтрокаДопПоля Из УдаляемыеВыбранныеПоля Цикл
...
Функция ДобавитьОтборыПоСтрокеРезультата(СтрокаРезультата, ЗапросДоГруппировки = Ложь) Экспорт 
	...
	Для Каждого КолонкаРезультата Из РезультатЗапроса.Колонки Цикл
		...
		Если Не ЗапросДоГруппировки И ВыбранноеПоле.ЕстьАгрегаты Тогда
			Продолжить;
		КонецЕсли; 
		ВыражениеКолонки = ВыбранноеПоле.Определение;
		Если ЗапросДоГруппировки И Прав(ВыражениеКолонки, 1) = ")" Тогда
			Продолжить;
		КонецЕсли; 
		...
		ЭлементОтбора.Представление = ВыражениеКолонки + " = &" + ИмяПараметра;
	КонецЦикла;
	Возврат Результат;
...
Функция ВыбратьВсеПоляИспользованныеВВыражениях(ЗапросДоГруппировки = Ложь, выхЕстьАгрегаты = Ложь) Экспорт 
	...
	Результат.Колонки.Добавить("КоличествоТочек");
	...
	Если Не СтруктураПолей.ЕстьАгрегатыВЧастиОбъединения Тогда
		// Сначала выявим наличие группировкки
		Для Каждого ВыбранноеПоле Из ЧастьОбъединения.ВыбранныеПоля Цикл
			ОбновитьВыражениеЗапроса(ВыбранноеПоле, , , ЧастьОбъединения, , ,, выхЕстьАгрегаты,, 0);
		КонецЦикла;
		ОбновитьВыраженияОтбора(ЧастьОбъединения.Компоновщик.Настройки.Отбор, , , ЧастьОбъединения,,, 0,, выхЕстьАгрегаты);
	КонецЕсли; 
	СтруктураПолей.ЕстьАгрегатыВЧастиОбъединения = выхЕстьАгрегаты;
	Для Каждого ВыбранноеПоле Из ЧастьОбъединения.ВыбранныеПоля Цикл
		ОбновитьВыражениеЗапроса(ВыбранноеПоле, , , ЧастьОбъединения,,,,,, 0, СтруктураПолей);
	КонецЦикла;
	...
	Для Каждого СвязьТаблиц Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(ЧастьОбъединения.СвязиТаблиц) Цикл
		Если СвязьТаблиц.Условие = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ОбновитьВыраженияОтбора(СвязьТаблиц.Условие.Отбор,,, ЧастьОбъединения,,, 0, СтруктураПолей);
	КонецЦикла;
	ОбновитьВыраженияОтбора(ЧастьОбъединения.Компоновщик.Настройки.Отбор,,, ЧастьОбъединения,,, 0, СтруктураПолей);
	...
	Для Каждого КлючИЗначение Из СтруктураПолей.СоответствиеПолей Цикл
		...
		ЕстьАгрегатыВПоле = Найти(КлючИЗначение.Значение, "(") > 0;
		СтрокаВыбранногоПоля = ЧастьОбъединения.ВыбранныеПоля.Добавить();
		...
		СтрокаРезультата.КоличествоТочек = СтрЧислоВхождений(КлючИЗначение.Значение, ".");
		Если ЗапросДоГруппировки И Не ЕстьАгрегатыВПоле Тогда
			ПолеПорядка = ПоляПорядка.Вставить(0);
			ПолеПорядка.Определение = ИмяПоля;
		КонецЕсли;
	КонецЦикла;
	ОбновитьПоляОбъединенияЗапроса();
	Результат.Сортировать("КоличествоТочек УБЫВ");
	Возврат Результат;
