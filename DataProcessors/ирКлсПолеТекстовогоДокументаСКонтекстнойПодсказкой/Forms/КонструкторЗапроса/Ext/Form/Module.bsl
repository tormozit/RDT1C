...
Функция ЗаполнитьПоТокену(Знач Токен, Стек = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено, Индикатор = Неопределено,
	...
	Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущийУзел.Имя) Тогда
			...
			КонецЕсли; 
			ТекущийУзел.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ЧастьОбъединения.ВыбранныеТаблицы, ТекущийУзел,,, ЗаменаПустойСтроки);
		КонецЕсли; 
...
Функция ПолучитьПредставлениеЗначенияНаЯзыке(Знач Значение, СтруктураПолей = Неопределено) Экспорт
...
Функция ПолучитьПолноеИмяПоляВВыражении(ИмяПоля, ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, СтароеИмяТаблицы = "",
	НовоеИмяТаблицы = "", РазрешитьНормализациюИмен = Истина, выхНужноПересобратьТокены = Ложь, РазрешитьПроверкуСуществованияТаблиц = Ложь) Экспорт
	...
	Тогда // Пока не проверяем другие выражения
		...
		Иначе
			...
			КонецЕсли; 
		КонецЕсли; 
		Если РазрешитьНормализациюИмен И РазрешитьПроверкуСуществованияТаблиц И Не ЛиТерминалЯзыка(Результат) Тогда
			ИмяТаблицы = ирОбщий.ПолучитьПервыйФрагментЛкс(Результат);
			СтрокаВыбраннойТаблицы = ЧастьОбъединения.ВыбранныеТаблицы.Найти(НРег(ИмяТаблицы), "НИмя");
			Если СтрокаВыбраннойТаблицы = Неопределено Тогда
...
Функция СобратьВыражениеЗапроса(Знач Токен, Отступ = 0, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	СтароеИмяТаблицы = "", НовоеИмяТаблицы = "", выхЕстьАгрегаты = Ложь, РазрешитьНормализациюИмен = Истина, выхНужноПересобратьТокены = Неопределено, СтруктураПолей = Неопределено) Экспорт
	...
	Иначе
		...
		Если ИмяПравила = "<Case>" Тогда
			ВыражениеВыбора = СобратьВыражениеЗапроса(Данные.Tokens(1), Отступ, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы,
				выхЕстьАгрегаты, РазрешитьНормализациюИмен, выхНужноПересобратьТокены, СтруктураПолей);
		...
		ИначеЕсли ИмяПравила = "<WhenThen>" Тогда
			...
			|" + ЛевыйОтступ + "	" + ПолучитьСловоЯзыкаЗапросов("WHEN") + " " + СобратьВыражениеЗапроса(Данные.Tokens(1), Отступ + 2, ЗапросПакета, ЧастьОбъединения,
				СтароеИмяТаблицы, НовоеИмяТаблицы, выхЕстьАгрегаты, РазрешитьНормализациюИмен, выхНужноПересобратьТокены, СтруктураПолей) + "
		...
		Тогда
			Результат = СобратьВыражениеЗапроса(Данные.Tokens(0), Отступ, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы, выхЕстьАгрегаты,
				РазрешитьНормализациюИмен, выхНужноПересобратьТокены, СтруктураПолей) + "
		...
		Иначе
			...
			Тогда
				...
				Если Найти(Результат, ПолучитьСловоЯзыкаЗапросов("COUNT") + "(*)") = 1 Тогда
					Результат = "(NULL)";
				ИначеЕсли Найти(Результат, ПолучитьСловоЯзыкаЗапросов("COUNT") + "(" + ПолучитьСловоЯзыкаЗапросов("DISTINCT")) = 1 Тогда 
					Маркер = "(" + ПолучитьСловоЯзыкаЗапросов("DISTINCT") + " ";
		...
		Если ИмяПравила = "<Field>" Тогда
			//Результат = СобратьВыражениеПоля(Результат, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы, РазрешитьНормализациюИмен, СтруктураПолей, выхНужноПересобратьТокены);
			Результат = ПолучитьПолноеИмяПоляВВыражении(Результат, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы, РазрешитьНормализациюИмен, выхНужноПересобратьТокены, СтруктураПолей <> Неопределено);
			Если СтруктураПолей <> Неопределено Тогда 
				Если Истина
					...
					И (Ложь
						...
						Или (Истина
							И Не СтруктураПолей.ВнутриОтбора 
							И Не СтруктураПолей.ВнутриАгрегата))
				Тогда
					ДобавитьПолеВСтруктуруПолей(СтруктураПолей, Результат);
				КонецЕсли; 
				Если Истина
					И СтруктураПолей.Режим = "Замена" 
...
Функция СобратьВыражениеПоля(Знач Результат, Знач ЗапросПакета= Неопределено, Знач ЧастьОбъединения = Неопределено, Знач СтароеИмяТаблицы = Неопределено, Знач НовоеИмяТаблицы = Неопределено,
	Знач РазрешитьНормализациюИмен = Истина, Знач СтруктураПолей = Неопределено, выхНужноПересобратьТокены = Ложь)
...
Процедура ЗаполнитьЭлементОтбораПроизвольногоВыражения(Знач ЭлементОтбора, Знач Токен, Знач Отступ = 0, Знач ЧастьОбъединения = Неопределено, Знач ЗапросПакета = Неопределено, СтруктураПолей = Неопределено)
...
Функция СобратьВыражениеЭлементаОтбора(ЭлементОтбора, Знач Отступ = 0, Знач ГруппаИзОдногоЭлемента = Ложь, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено, СтруктураПолей = Неопределено)
	...
	Иначе
		...
		КонецЕсли; 
		ПредставлениеЛевогоПоля = ПолучитьПредставлениеЗначенияНаЯзыке(ЭлементОтбора.ЛевоеЗначение, СтруктураПолей);
		...
		Тогда
			Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
				...
				КонецЕсли; 
			КонецЕсли; 
			СтрокаПравогоЗначения = ПолучитьПредставлениеЗначенияНаЯзыке(ЭлементОтбора.ПравоеЗначение, СтруктураПолей);
			Если Ложь
			...
			Тогда
				Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("Строка") Тогда
					СтрокаПравогоЗначения = ПолучитьПредставлениеЗначенияНаЯзыке("%" + ЭлементОтбора.ПравоеЗначение + "%");
				КонецЕсли; 
			ИначеЕсли Ложь
...
Функция СобратьВыражениеОтбораКомпоновки(ГруппаОтбора, Отступ = 0, ФильтрацияАгрегатов = 0, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено, СтруктураПолей = Неопределено) Экспорт 
	...
	Для Каждого ЭлементОтбора Из ГруппаОтбора.Элементы Цикл
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			КонецЕсли;
		Иначе
			ПредставлениеЭлемента = СобратьВыражениеЭлементаОтбора(ЭлементОтбора, Отступ + 1, ГруппаИзОдногоЭлемента, ЕстьАгрегаты, НастройкиКомпоновки, СтруктураПолей);
		КонецЕсли; 
...
Функция РазобратьВыражениеЗапроса(ВыражениеЗапроса, ЗаменятьПустоеНаNULL = Истина) Экспорт
	...
	НачальныйТокен = РазобратьТекстЗапроса(ТекстДляРазбора, , Ложь, СлужебноеПолеТекстовогоДокумента);
	Если НачальныйТокен = Неопределено Тогда
...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "", РазвернутьАвтополя = Ложь, Знач СтруктураПолей = Неопределено)
	...
	Результат = "";
	ЕстьАгрегаты = Ложь;
	Если РасширеннаяПроверка Или ЧастьОбъединения.БылиУдаленияТаблиц Тогда
		Если СтруктураПолей = Неопределено Тогда
	...
	Для Каждого СтрокаПоляОбъединения Из ЗапросПакета.ПоляОбъединения Цикл
		...
		ИначеЕсли ВыбранноеПоле.Автополе И Не РазвернутьАвтополя И мПараметрыДиалектаSQL.МногоТаблиц Тогда
			...
			КонецЕсли; 
		Иначе
			ТекстВыражения = ОбновитьВыражениеЗапроса(ВыбранноеПоле, ЭлементыФормы.ВыбранныеПоля, ЗапросПакета, ЧастьОбъединения,,,, ЕстьАгрегаты,, 0, СтруктураПолей);
	...
	// WHERE
	//ТекстУсловия = СобратьВыражениеОтбораКомпоновки(ЧастьОбъединения.Условие.Отбор, 1, 1, ЕстьАгрегаты, ЧастьОбъединения.Компоновщик.Настройки);
	ТекстУсловия = СобратьВыражениеОтбораКомпоновки(ЧастьОбъединения.Компоновщик.Настройки.Отбор, 1, 1, ЕстьАгрегаты, ЧастьОбъединения.Компоновщик.Настройки, СтруктураПолей);
	ДобавитьТекстОтбора(Результат, Смещение, ТекстУсловия, ЧастьОбъединения);
	...
	// HAVING
	//ТекстУсловия = СобратьВыражениеОтбораКомпоновки(ЧастьОбъединения.Условие.Отбор, 1, 2,, ЧастьОбъединения.Компоновщик.Настройки);
	ТекстУсловия = СобратьВыражениеОтбораКомпоновки(ЧастьОбъединения.Компоновщик.Настройки.Отбор, 1, 2,, ЧастьОбъединения.Компоновщик.Настройки);
...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	ИначеЕсли Прав(ОпределениеПоля, 1) = ")" Тогда 
		ИмяФункции = ирОбщий.ПолучитьПервыйФрагментЛкс(ОпределениеПоля, "(");
		Если Ложь
...
Процедура ПриОткрытии()
	...
	Если Не ЛиПакетныйЗапрос Тогда
		// http://www.hostedredmine.com/issues/849101
		ПодключитьОбработчикОжидания("ИзменитьСвернутостиПанелейОтложенно", 0.1, Истина);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяВыбранногоПоляДляАктивации) Тогда
		ПодключитьОбработчикОжидания("АктивизироватьВыбранноеПолеОтложенно", 0.1, Истина);
	КонецЕсли;
	Если ОбновитьВсеВыражения Тогда
		ПодключитьОбработчикОжидания("ОбновитьВсеВыраженияПакетаОтложенно", 0.1, Истина);
	КонецЕсли; 

КонецПроцедуры

Процедура ОбновитьВсеВыраженияПакетаОтложенно()
	
	ОбновитьВсеВыраженияПакета(, Истина);

КонецПроцедуры

Процедура ИзменитьСвернутостиПанелейОтложенно()
...
Процедура АктивизироватьВыбранноеПолеОтложенно()
...
Процедура ОбновитьДоступныеВременныеТаблицы(ИмяВременнойТаблицы = Неопределено) Экспорт
	...
	Если ТабличноеПолеКорневогоПакета.ТекущаяСтрока <> Неопределено Тогда
		ИндексТекущегоЗапроса = ТабличноеПолеКорневогоПакета.Значение.Индекс(ТабличноеПолеКорневогоПакета.ТекущаяСтрока);
	Иначе
...
Процедура ВыбранныеТаблицыИмяПриИзменении(Элемент = Неопределено)
	...
	ЗакончитьРедактированиеСвязанныхПолей();
	Если Не СкорректироватьИмяВыбраннойТаблицы(мТекущаяСтрокаВыбранныеТаблицы) Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, Знач СтароеИмяТаблицыИлиПараметра = "", Знач НовоеИмяТаблицыИлиПараметра = "", Знач пСтароеИмяПоля = "", Знач пНовоеИмяПоля = "",
		...
		Для Каждого ЧастьОбъединения Из лЧастиОбъединения Цикл
			...
			Если ГлобальныйОбход Тогда
				...
				Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
					...
					ИначеЕсли Истина
						И НовоеИмяТаблицыИлиПараметра <> СтароеИмяТаблицыИлиПараметра
						И ирОбщий.СтрокиРавныЛкс(ВыбраннаяТаблица.ПолноеИмя, СтароеИмяТаблицыИлиПараметра) 
					Тогда 
				...
				Если ЗначениеЗаполнено(пСтароеИмяПоля) Тогда
					...
					КонецЕсли; 
				КонецЕсли; 
				Если СоответствиеЗаменПолей.Количество() = 0 И Не ЭтоПереименованиеПараметра Тогда 
					Продолжить;
				КонецЕсли;
			КонецЕсли; 
	...
	Если ТипЗнч(Токен) = Тип("COMОбъект") Тогда
		Если Токен.Kind = 0 Тогда
			...
			ИначеЕсли ИмяПравила = "<Case>" Тогда
				Результат = ПолучитьТипЗначенияВыражения(ДанныеТокена.Tokens(2), ЧастьОбъединения);
			ИначеЕсли ИмяПравила = "<WhenThen>" Тогда
				Результат = ПолучитьТипЗначенияВыражения(ДанныеТокена.Tokens(3), ЧастьОбъединения);
				Если ДанныеТокена.TokenCOunt = 5 Тогда
					Результат2 = ПолучитьТипЗначенияВыражения(ДанныеТокена.Tokens(4), ЧастьОбъединения);
					#Если Сервер И Не Сервер Тогда
...
//		1 - пересобрать
//		2 - очистить токен
Функция ОбновитьВсеВыраженияПакета(Пакет = Неопределено, РазрешитьНормализациюИмен = Ложь, Знач РежимОбновленияВыражений = 1)

	Если Пакет = Неопределено Тогда
		Пакет = ЗапросыПакета;
	КонецЕсли;
	Для Каждого ЗапросПакета Из Пакет Цикл
		ОбновитьВсеВыраженияЗапроса(ЗапросПакета, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
	КонецЦикла;
	Возврат Неопределено;

КонецФункции

Процедура ОбновитьВсеВыраженияЗапроса(Знач ЗапросПакета, РазрешитьНормализациюИмен = Ложь, Знач РежимОбновленияВыражений = 1)
	
	Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
		ОбновитьВсеВыраженияЧастиОбъединения(ЗапросПакета, ЧастьОбъединения, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
	КонецЦикла;
	Для Каждого ПолеПорядка Из ЗапросПакета.ПоляПорядка Цикл
		ОбновитьВыражениеЗапроса(ПолеПорядка, ЭлементыФормы.ПоляПорядка, ЗапросПакета,,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
	КонецЦикла;
	Для Каждого ГруппируемоеПолеИтогов Из ЗапросПакета.ГруппируемыеПоляИтогов Цикл
		ОбновитьВыражениеЗапроса(ГруппируемоеПолеИтогов, ЭлементыФормы.ГруппируемыеПоляИтогов, ЗапросПакета,,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
	КонецЦикла;
	Для Каждого ГруппирующееПолеИтогов Из ЗапросПакета.ГруппирующиеПоляИтогов Цикл
		ОбновитьВыражениеЗапроса(ГруппирующееПолеИтогов, ЭлементыФормы.ГруппирующиеПоляИтогов, ЗапросПакета,,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
	КонецЦикла;
	Если ТипЗапроса <> 3 Или Не ЗначенияИзВыборки Тогда
		Для Каждого ПринимающееПоле Из ЗапросПакета.ПринимающиеПоля Цикл
			ОбновитьВыражениеЗапроса(ПринимающееПоле, ЭлементыФормы.ПринимающиеПоля, ЗапросПакета,,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
		КонецЦикла;
	КонецЕсли; 

КонецПроцедуры

Процедура ОбновитьВсеВыраженияЧастиОбъединения(Знач ЗапросПакета, Знач ЧастьОбъединения, РазрешитьНормализациюИмен = Ложь, Знач РежимОбновленияВыражений = 1)
	...
	Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
		Если ВыбраннаяТаблица.ВложенныйПакет <> Неопределено Тогда
			Попытка
				ОбновитьВсеВыраженияПакета(ВыбраннаяТаблица.ВложенныйПакет, РазрешитьНормализациюИмен, РежимОбновленияВыражений);
			Исключение
		...
		Для каждого ПараметрТаблицы Из ВыбраннаяТаблица.Параметры Цикл
			Если Не ЗначениеЗаполнено(ПараметрТаблицы.Определение) Тогда
				Продолжить;
			КонецЕсли; 
			ОбновитьВыражениеЗапроса(ПараметрТаблицы, ЭлементыФормы.ПараметрыТаблицы, ЗапросПакета, ЧастьОбъединения,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений, СтруктураПолей);
		КонецЦикла;
	КонецЦикла;
	Для Каждого ВыбранноеПоле Из ЧастьОбъединения.ВыбранныеПоля Цикл
		ОбновитьВыражениеЗапроса(ВыбранноеПоле, ЭлементыФормы.ВыбранныеПоля, ЗапросПакета, ЧастьОбъединения,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений, СтруктураПолей);
	КонецЦикла;
	Для Каждого ПолеГруппировки Из ЧастьОбъединения.Группировки Цикл
		ОбновитьВыражениеЗапроса(ПолеГруппировки, ЭлементыФормы.Группировки, ЗапросПакета, ЧастьОбъединения,,,,, РазрешитьНормализациюИмен, РежимОбновленияВыражений, СтруктураПолей);
	КонецЦикла;
	Для Каждого СвязьТаблиц Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(ЧастьОбъединения.СвязиТаблиц) Цикл
		Если СвязьТаблиц.Условие <> Неопределено Тогда
...
Функция ПоказатьОшибкуВложенногоПакета(Знач ВыбраннаяТаблица, Знач ЗапросПакета, Знач ЧастьОбъединения)
	...
	ЭлементыФормы.ЗапросыПакета.ТекущаяСтрока = ЗапросПакета;
	ЭлементыФормы.ЧастиОбъединения.ТекущаяСтрока = ЧастьОбъединения;
	АктивизироватьТабличноеПоле(ЭлементыФормы.ВыбранныеТаблицы);
	ЭлементыФормы.ВыбранныеТаблицы.ТекущаяСтрока = ВыбраннаяТаблица;
...
Процедура ДобавитьПолеВСтруктуруПолей(СтруктураПолей, ИмяПоля)
	
	#Если Сервер И Не Сервер Тогда
		СтруктураПолей = Новый Соответствие;
	#КонецЕсли
	Если ЛиТерминалЯзыка(ИмяПоля) Тогда
		Возврат;
	КонецЕсли; 
	СтруктураПолей.СоответствиеПолей.Вставить(НРег(ИмяПоля), ИмяПоля);
...
Функция ОбновитьВыражениеЗапроса(СтрокаТаблицыВыражений, ТабличноеПоле = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	СтароеИмяТаблицы = "", НовоеИмяТаблицы = "", РазрешитьПустое = Ложь, выхЕстьАгрегаты = Ложь, РазрешитьНормализациюИмен = Истина, РежимОбновленияВыражений = 1, СтруктураПолей = Неопределено)
	...
	Иначе
		...
		Попытка
			...
			Результат = СобратьВыражениеЗапроса(НачальныйТокенВыражения, , ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы, выхЕстьАгрегаты, РазрешитьНормализациюИмен,
				НужноПересобратьТокены, СтруктураПолей);
		Исключение
...
Процедура СохранитьЗапросПакета()
	...
	Если Не ЗначениеЗаполнено(ИмяОсновнойТаблицы) Тогда
		мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблицы = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ЗапросыПакета, мТекущаяСтрокаЗапросыПакета, "ИмяОсновнойТаблицы");
	КонецЕсли; 
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент)
	...
	ОбновитьНаименованиеЗапроса(мТекущаяСтрокаЗапросыПакета);
	ПереименоватьТаблицуИлиПараметр(, мТекущееИмяОсновнойТаблицы, НовоеИмя,,,, ЗапросыПакета.Индекс(мТекущаяСтрокаЗапросыПакета) + 1);
...
Процедура ОбновитьЭлементОтбораПослеИзменения(Элемент)
	...
	Если Истина
		И ТипЗнч(Элемент.ТекущаяСтрока) = Тип("ЭлементОтбораКомпоновкиДанных") 
		И Элемент.ТекущаяСтрока.Представление <> ""
	Тогда
		СобратьВыражениеЭлементаОтбора(Элемент.ТекущаяСтрока);
	КонецЕсли; 
...
Процедура УдалитьВыбраннуюТаблицу(ВыбраннаяТаблица, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено)
	...
	Для СчетчикВыбранныеПоля = 1 По НачальноеКоличество Цикл
		...
		Если Истина
			...
			И ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ВыбранноеПоле.Определение), ВыбраннаяТаблица.Имя)
		Тогда
	...
	Для СчетчикВыбранныеПоля = 1 По НачальноеКоличество Цикл
		...
		Если Истина
			...
			И ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(Группировка.Определение), ВыбраннаяТаблица.Имя)
		Тогда
...
Процедура ВыбранныеПоляПередУдалением(Элемент, Отказ)
	...
	Если Элемент.ТекущаяСтрока.Автополе Тогда 
		Возврат;
	КонецЕсли; 
	Для Каждого ВыделеннаяСтрока Из ЭлементыФормы.ВыбранныеПоля.ВыделенныеСтроки Цикл
		УдалитьВыбранноеПоле(ВыделеннаяСтрока);
	КонецЦикла;
...
Процедура ГруппируемыеПоляИтоговПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	ЗначениеПеретаскивания = ПараметрыПеретаскивания.Значение;
	Если ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") Тогда
		Если ЛиРазрешенноеПолеДляПеретастикиванияВИтоги(ЗначениеПеретаскивания[0]) Тогда 
			СтандартнаяОбработка = Ложь;
			ПараметрыПеретаскивания.ДопустимыеДействия = ДопустимыеДействияПеретаскивания.Копирование;
		КонецЕсли;
	КонецЕсли; 
...
Функция ЛиРазрешенноеПолеДляПеретастикиванияВИтоги(Знач ЗначениеПеретаскивания)
	
	Возврат Истина
		И ТипЗнч(ЗначениеПеретаскивания) = Тип("СтрокаТаблицыЗначений")
		//И ЗначениеПеретаскивания.Владелец() = ПоляОбъединения // Так не работает
		И ЗначениеПеретаскивания.Владелец().Колонки.Найти("РольИтога") <> Неопределено;
	...
	Если ИсточникДействия = ЭлементыФормы.ВыражениеВыбранногоПоля Тогда
		...
		Если ЛиВыражениеЯвляетсяПолемИлиТерминалом(ТекущееВыражение) Тогда 
			...
			Если СтрокаЗапросаСоздания = Неопределено Тогда
				СтрокаЗапросаСоздания = ДоступныеПоля.НайтиПоле(НОвый ПолеКомпоновкиДанных(ирОбщий.ПолучитьПервыйФрагментЛкс(ТекущееВыражение)));
			КонецЕсли; 
			Если СтрокаЗапросаСоздания <> Неопределено Тогда
...
Процедура КПДеревоУсловияНайтиВСписке(Кнопка)
	...
	ДоступноеПоле = ирОбщий.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭлементыФормы.ДоступныеПоляУсловия, ЭлементыФормы.ДеревоУсловия);
	Если ДоступноеПоле <> Неопределено Тогда
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ДоступныеПоляУсловия;
	КонецЕсли; 
...
Процедура ДеревоУсловияЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ДеревоУсловия, СтандартнаяОбработка,, Истина,, "ПравоеЗначение");
	
КонецПроцедуры

Процедура ДеревоУсловияСвязиЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ДеревоУсловияСвязи, СтандартнаяОбработка,, Истина,, "ПравоеЗначение");
...
Процедура ПереключитьЭлементыОтбораВЗапросе(НачальныйИндексДобавленныхЭлементовОтбора = 0, НовоеИспользование = Ложь) Экспорт 
	
	Для ИндексЭлемента = НачальныйИндексДобавленныхЭлементовОтбора По КомпоновщикЧасти.Настройки.Отбор.Элементы.Количество() - 1 Цикл
...
Функция ДобавитьОтборыПоСтрокеРезультата(СтрокаРезультата, ЗапросДоГруппировки = Ложь, ИменаПолей = Неопределено) Экспорт 
	...
	Для Каждого КолонкаРезультата Из РезультатЗапроса.Колонки Цикл
		...
		Если ЗапросДоГруппировки И ВыбранноеПоле.ЕстьАгрегаты Тогда
			Продолжить;
		КонецЕсли; 
		...
		Тогда
			ВыражениеОтбора = "(" + ВыбранноеПоле.Определение + ") = " + ПолучитьПредставлениеЗначенияНаЯзыке(ЗначениеКолонки);
		Иначе
		...
		Если Не ВыражениеОтбораСуществует Тогда
			Если Не РазделительДобавлен Тогда
				...
				ЭлементОтбора.Представление = """" + КомментарийРазделительСлужебныхСекцийЗапроса() + """ > """"";
	...
	Если Не ЗначениеЗаполнено(ЗначениеПараметра) Тогда
		...
		Если АнглийскийРежим Тогда
			Если ЗначениеПараметра = Неопределено Тогда
				БазовоеИмяПараметра = БазовоеИмяПараметра + "Undefined";
			ИначеЕсли Не ЗначениеЗаполнено(БазовоеИмяПараметра) Тогда
				Если XMLТип = Неопределено Тогда
					XMLТип = СериализаторXDTO.XMLТипЗнч(ЗначениеПараметра);
				КонецЕсли;
				БазовоеИмяПараметра = БазовоеИмяПараметра + ТРег(ирОбщий.ПолучитьПоследнийФрагментЛкс(XMLТип.ИмяТипа));
			КонецЕсли; 
		Иначе
...
Функция ВыбратьВсеПоляИспользованныеВВыражениях(ЗапросДоГруппировки = Ложь, выхЕстьАгрегаты = Ложь, выхИндексКолонкиКоличество = Неопределено) Экспорт 
	...
	Если Не СтруктураПолей.ЕстьАгрегатыВЧастиОбъединения Тогда
		// Сначала выявим наличие агрегатов
		Для Каждого ВыбранноеПоле Из ВыбранныеПоля Цикл
			ОбновитьВыражениеЗапроса(ВыбранноеПоле,,,,,,, выхЕстьАгрегаты,, 0);
		КонецЦикла;
		ОбновитьВыраженияОтбора(КомпоновщикЧасти.Настройки.Отбор,,,,,, 0,, выхЕстьАгрегаты);
	КонецЕсли; 
	...
	Для Каждого ПолеГруппировки Из Группировки Цикл
		ОбновитьВыражениеЗапроса(ПолеГруппировки,,,,,,,,, 0, СтруктураПолей);
	КонецЦикла;
	СтруктураПолей.ВнутриОтбора = Истина;
	...
	Если Не ЗапросДоГруппировки И выхЕстьАгрегаты Тогда
		...
		СтруктураПолей.СоответствиеПолей.Вставить(НРег(ОпределениеПоляКоличество), ОпределениеПоляКоличество);
	КонецЕсли; 
	РазделительДобавлен = Ложь;
	Для Каждого КлючИЗначение Из СтруктураПолей.СоответствиеПолей Цикл
		СтрокаРезультата = Результат.Добавить();
		ОпределениеПоля = КлючИЗначение.Значение;
		...
		КонецЕсли; 
		ЗаполнитьЗначенияСвойств(СтрокаРезультата, СтрокаВыбранногоПоля, "ЕстьАгрегаты, Имя, Определение"); 
		Если ЗапросДоГруппировки И Не СтрокаВыбранногоПоля.ЕстьАгрегаты Тогда
...
Процедура ЗапросыПакетаПередУдалением(Элемент, Отказ)
	...
	Если Элемент.Значение.Количество() = 1 Тогда
		Отказ = Истина;
	ИначеЕсли мТекущаяСтрокаЗапросыПакета.ТипЗапроса = 1 И ЗапросыПакета.Количество() > мТекущаяСтрокаЗапросыПакета.Индекс + 1  Тогда 
		Ответ = Вопрос("Удалить все выбранные таблицы, ссылающиеся на эту временную таблицу?", РежимДиалогаВопрос.ДаНет);
		Если Ответ = КодВозвратаДиалога.Да Тогда
