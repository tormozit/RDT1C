		...
		Иначе
			...
			Тогда
				...
				Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
					СтрокаПравогоЗначения = ирОбщий.ЛитеральноеПредставлениеЗначенияВЗапросеЛкс(ЭлементОтбора.ПравоеЗначение);
...
Процедура ЗагрузитьВыражениеУсловия()
	...
	Если ТипЗнч(мТекущаяСтрокаУсловия) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
		МожноСобиратьВыражение = Ложь
			Или ЗначениеЗаполнено(ирОбщий.ЛитеральноеПредставлениеЗначенияВЗапросеЛкс(мТекущаяСтрокаУсловия.ПравоеЗначение))
...
Процедура ЗаменитьПравоеЗначениеЭлементаОтбораПараметром(Знач ВыбраннаяСтрока, XMLТип = Неопределено)
	...
	Иначе
		...
		Если ВыбраннаяСтрока.ПравоеЗначение = Неопределено Или ТипЗнч(ВыбраннаяСтрока.ПравоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
			ТипЗначения = ТипЗнч(КомпоновщикЧасти.Настройки.ДоступныеПоляОтбора.НайтиПоле(ВыбраннаяСтрока.ЛевоеЗначение).ТипЗначения.ПривестиЗначение());
			ИмяПараметра = ПроверитьДобавитьПараметр(БазовоеИмяПараметра, ТипЗначения);
		Иначе
