...
Функция ЗаполнитьПоТокену(Знач Токен, Стек = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено, Индикатор = Неопределено,
	...
	Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущийУзел.Имя) Тогда
			ТекущийУзел.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(ЧастьОбъединения.ВыбранныеТаблицы, ТекущийУзел,,, ПолучитьСловоЯзыкаЗапросов("Table") + "1");
		КонецЕсли; 
...
Функция ПолучитьСловоЯзыкаЗапросов(Слово) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Тогда
			...
			Если Результат = Неопределено Тогда
				ВызватьИсключение "Не найден терминал """ + Слово + """ языка запросов";
			КонецЕсли; 
		Иначе
			Результат = Слово;
		КонецЕсли; 
...
Функция РазобратьВыражениеЗапроса(ВыражениеЗапроса) Экспорт
	...
	//СлужебноеПолеТекстовогоДокумента.УстановитьТекст(ТекстДляРазбора); // Закомментировал 19.04.2015
...
Процедура ОбновитьДоступныеВременныеТаблицы() Экспорт
	...
	Для Индекс = 0 По ИндексТекущегоЗапроса - 1 Цикл
		...
		ИначеЕсли ЗапросПакета.ТипЗапроса = 2 Тогда
			Если СтруктураДобавленныхТаблиц[ЗапросПакета.ИмяВременнойТаблицы] <> Неопределено Тогда
...
Функция ПолучитьПоляДоступнойТаблицы(СтрокаДоступнойТаблицы, ПараметрыТаблицы = Неопределено)
	...
	ИначеЕсли ТипЗнч(Конфигурация) = Тип("ОбъектМетаданныхКонфигурация") Тогда
		...
		Если ПараметрыТаблицы <> Неопределено Тогда
			...
			КонецЦикла;
		КонецЕсли; 
		КомпоновщикТаблицы = ирКэш.ПолучитьКомпоновщикТаблицыМетаданныхЛкс(ПолноеИмяТаблицы, Ложь, ИндексПараметраПериодичность, ОпределениеПараметраПериодичность);
