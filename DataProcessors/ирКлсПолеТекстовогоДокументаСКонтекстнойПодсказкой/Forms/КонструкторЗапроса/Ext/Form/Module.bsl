...
Процедура ОбработатьЗапросПослеУстановкиДанных(ЗапросПакета)
	...
	Тогда
		...
		НеуникальныеКлючи = ирОбщий.НеуникальныеКлючиТаблицыЛкс(ПерваяЧасть.ВыбранныеПоля, "НИмя");
...
Функция СобратьВыражениеЗапроса(Знач Токен, Знач Отступ = 0, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	Знач СтароеИмяТаблицы = "", Знач НовоеИмяТаблицы = "", выхЕстьАгрегаты = Ложь, выхНужноПересобратьТокены = Неопределено, Знач СтруктураПолей = Неопределено) Экспорт
...
Процедура ВыборЛевойТаблицыСвязиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ВыбранныеТаблицыПриВыводеСтроки1(Элемент, ОформлениеСтроки, ДанныеСтроки);
...
Функция ЛиТерминалЯзыка(Знач ИмяПоля)
	...
	ОтборАнглийский = Новый Структура("МожетБытьПсевдонимом, Английский", Ложь, ИмяПоля);
	ОтборРусский = Новый Структура("МожетБытьПсевдонимом, Русский", Ложь, ИмяПоля);
	Результат = Ложь
...
Процедура ЗагрузитьСвязьТаблиц()
	...
	//#Если Сервер И Не Сервер Тогда
		АктивироватьЛюбуюСтрокуУсловияСвязиТаблицОтложенно();
	//#КонецЕсли
	//ПодключитьОбработчикОжидания("АктивироватьЛюбуюСтрокуУсловияСвязиТаблицОтложенно", 0.1, Истина);
	
КонецПроцедуры

Процедура АктивироватьЛюбуюСтрокуУсловияСвязиТаблицОтложенно()
...
Процедура ВосстановитьТекущиеСтрокиТабличныхПолей(Знач ТабличныеПоля, Знач ТекущиеСтроки)
	...
	Для Каждого ИмяТабличногоПоля Из ТабличныеПоля Цикл
		...
		ИначеЕсли ТипЗнч(ЗначениеПоля) = Тип("ДоступныеПоляКомпоновкиДанных") Тогда 
			НоваяТекущаяСтрока = ЗначениеПоля.НайтиПоле(Новый ПолеКомпоновкиДанных(ИндексСтроки));
...
Процедура ВыбранныеТаблицыПриВыводеСтроки1(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
...
Процедура ОбновитьТаблицуПараметровВыбраннойТаблицы(Знач ВыбраннаяТаблица = Неопределено)
	...
	ДоступнаяТаблица = ПолучитьДоступнуюТаблицуПоИмениВыбранной(ПолноеИмяТаблицы);
...
Процедура ОформитьСтрокуДоступногоПоля(ОформлениеСтроки, СтрокаПоля, Знач СтрокаДоступнойТаблицы, ИмяКолонки, Знач ЭтоИндекс = Ложь, Использовано = Ложь)
	...
	Если Использовано И ОформлениеСтроки.Ячейки.Найти("Использовано") <> Неопределено Тогда
		ОформлениеСтроки.Ячейки.Использовано.УстановитьКартинку(ирКэш.КартинкаПоИмениЛкс("ирРеквизит"));
	КонецЕсли;
...
Процедура ОбновитьСтатистикуЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	...
	Тогда
		#Если Сервер И Не Сервер Тогда
			ирСервер.ВыполнитьЗапросСтатистикиПоТаблицамФоновоЛкс();
		#КонецЕсли
...
Функция ДобавитьОтборыПоСтрокеРезультата(СтрокаРезультата, ЗапросДоГруппировки = Ложь, ИменаПолей = Неопределено) Экспорт 
	...
	Для Каждого КолонкаРезультата Из РезультатЗапроса.Колонки Цикл
		...
		Если Истина
			...
			И КомпоновщикЧасти.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ВыбранноеПоле.Определение)) = Неопределено // https://www.hostedredmine.com/issues/897767
		Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура ЗаменитьПравоеЗначениеЭлементаОтбораПараметром(Знач ВыбраннаяСтрока, XMLТип = Неопределено)
	...
	Иначе
		...
		Если ВыбраннаяСтрока.ПравоеЗначение = Неопределено Или ТипЗнч(ВыбраннаяСтрока.ПравоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
			ОписаниеТипов = КомпоновщикЧасти.Настройки.ДоступныеПоляОтбора.НайтиПоле(ВыбраннаяСтрока.ЛевоеЗначение).ТипЗначения;
...
Процедура ПолеВводаЗначенияОтбораОкончаниеВводаТекста(Знач Элемент, Знач Текст, Значение, СтандартнаяОбработка, Знач КомпоновщикОтбора)
	...
	Если ТипЗнч(Элемент.Значение) = Тип("ПолеКомпоновкиДанных") Тогда
		ДоступноеПоле = КомпоновщикОтбора.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(Текст));
		Если ДоступноеПоле <> Неопределено Тогда
		...
		Для Каждого ЭлементСписка Из ПоляДляПровеки Цикл
			...
			Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
				...
				Для Каждого СтрокаПоля Из СтруктураОбработкиПолей.СоответствиеПолей Цикл
					...
					Иначе
						ДоступнаяТаблица = ПолучитьДоступнуюТаблицуПоИмениВыбранной(ВыбраннаяТаблица.ПолноеИмя);
