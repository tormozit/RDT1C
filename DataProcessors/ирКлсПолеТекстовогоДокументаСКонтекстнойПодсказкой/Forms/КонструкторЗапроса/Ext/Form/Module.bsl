...
Функция ПолучитьПолноеИмяПоляВВыражении(ИмяПоля, ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, СтароеИмяТаблицы = "",
	НовоеИмяТаблицы = "", РазрешитьНормализациюИмен = Истина, выхНужноПересобратьТокены = Ложь, РазрешитьПроверкуСуществованияТаблиц = Ложь, СтруктураПолей = Неопределено) Экспорт
	...
	Тогда // Пока не проверяем другие выражения
		...
		КонецЕсли; 
		Если Не ВыполненаЗамена И РазрешитьНормализациюИмен И РазрешитьПроверкуСуществованияТаблиц И Не ЛиТерминалЯзыка(Результат) Тогда
...
Функция СобратьВыражениеЗапроса(Знач Токен, Отступ = 0, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	...
	Иначе
		...
		Если ИмяПравила = "<Field>" Тогда
			...
			Результат = ПолучитьПолноеИмяПоляВВыражении(Результат, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы, РазрешитьНормализациюИмен, выхНужноПересобратьТокены,
				СтруктураПолей <> Неопределено, СтруктураПолей);
...
Процедура ДобавитьДоступноеПолеСДочернимиВВыбранныеПоля(СтрокаДоступногоПоля, Автополе = Ложь, ЗапросПакета = Неопределено,
	...
	Тогда
		...
		КонецЕсли; 
		Для Каждого лСтрокаДоступногоПоля Из СтрокаДоступногоПоля.Элементы Цикл
			Если Автополе И Найти(лСтрокаДоступногоПоля.Поле, МаркерПоляФункцииТипЗначения()) > 0 Тогда
				Продолжить;
			КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	#Если Сервер И Не Сервер Тогда
		ИзменитьСвернутостиПанелейОтложенно();
	#КонецЕсли
...
Процедура ВыбранныеТаблицыИмяПриИзменении(Элемент = Неопределено)
	...
	ЗакончитьРедактированиеСвязанныхПолей();
	СкорректироватьИмяВыбраннойТаблицы(мТекущаяСтрокаВыбранныеТаблицы);
...
Функция ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, Знач СтароеИмяТаблицыИлиПараметра = "", Знач НовоеИмяТаблицыИлиПараметра = "", Знач СтароеИмяПоля = "", Знач НовоеИмяПоля = "",
	...
	СтруктураПолей = Новый Структура("Режим, КоличествоОбнаружено", "ГлобальнаяЗамена", 0);
	...
	Для ИндексВПакете = НачальныйИндексВПакете По Пакет.Количество() - 1 Цикл
		...
		Для Каждого ЧастьОбъединения Из лЧастиОбъединения Цикл
				...
				Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
					Для Каждого ПараметрТаблицы Из ВыбраннаяТаблица.Параметры Цикл
						Если ЗначениеЗаполнено(ПараметрТаблицы.Определение) Тогда
					        ОбновитьВыражениеЗапроса(ПараметрТаблицы, ЭлементыФормы.ПараметрыТаблицы, , ЧастьОбъединения, СтароеИмяТаблицыИлиПараметра, НовоеИмяТаблицыИлиПараметра,,, Ложь,, СтруктураПолей);
			...
			Для Каждого ЗаменаПолей Из ТаблицаЗамен Цикл
				...
				Для Каждого ВыбранноеПоле Из ЧастьОбъединения.ВыбранныеПоля Цикл
					ОбновитьВыражениеЗапроса(ВыбранноеПоле, ЭлементыФормы.ВыбранныеПоля, , ЧастьОбъединения, СтароеИмя, НовоеИмя,,,,, СтруктураПолей);
				...
				Для Каждого ПолеГруппировки Из ЧастьОбъединения.Группировки Цикл
					ОбновитьВыражениеЗапроса(ПолеГруппировки, ЭлементыФормы.Группировки, , ЧастьОбъединения, СтароеИмя, НовоеИмя,,,,, СтруктураПолей);
				КонецЦикла;
				Для Каждого СвязьТаблиц Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(ЧастьОбъединения.СвязиТаблиц) Цикл
					...
					Если СвязьТаблиц.Условие = Неопределено Тогда
						Продолжить;
					КонецЕсли; 
					ОбновитьВыраженияОтбора(СвязьТаблиц.Условие.Отбор, ЭлементыФормы.ДеревоУсловияСвязи, , ЧастьОбъединения, СтароеИмя, НовоеИмя,, СтруктураПолей);
				КонецЦикла;
				...
				ОбновитьВыраженияОтбора(ЧастьОбъединения.Компоновщик.Настройки.Отбор, ЭлементыФормы.ДеревоУсловия, , ЧастьОбъединения, СтароеИмя, НовоеИмя,, СтруктураПолей);
				Для Каждого ПолеПорядка Из мТекущаяСтрокаЗапросыПакета.ПоляПорядка Цикл
					ОбновитьВыражениеЗапроса(ПолеПорядка, ЭлементыФормы.ПоляПорядка,,,,,,, Ложь,, СтруктураПолей);
				КонецЦикла;
				...
				Для Каждого ПолеПостроителя Из ЧастьОбъединения.ПоляВыбораПостроителя Цикл
					ОбновитьВыражениеЗапроса(ПолеПостроителя, ЭлементыФормы.ПоляВыбораПостроителя, , ЧастьОбъединения, СтароеИмя, НовоеИмя,,,,, СтруктураПолей);
				КонецЦикла;
...
Процедура УсловияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		ОбновитьЭлементОтбораПослеИзменения(Элемент, ЭлементыФормы.ВыражениеЭлементаУсловия);
	КонецЕсли; 
...
Процедура ДеревоУсловияСвязиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		ОбновитьЭлементОтбораПослеИзменения(Элемент, ЭлементыФормы.ВыражениеЭлементаУсловияСвязи);
		СохранитьУсловиеСвязи();
	КонецЕсли; 
...
Процедура ОбновитьЭлементОтбораПослеИзменения(Элемент, ПолеТекста)
	...
	Тогда
		...
		ПолеТекста.УстановитьТекст(Элемент.ТекущаяСтрока.Представление);
	КонецЕсли; 
