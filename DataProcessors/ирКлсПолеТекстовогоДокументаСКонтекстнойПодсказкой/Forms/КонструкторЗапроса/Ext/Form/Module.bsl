...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	КонецЕсли; 
	Если Истина
		// Грязно. Правильнее делать рекурсивный обход дерева выражения
		И Найти(СтрокаПоля.Определение, "+") = 0
		И Найти(СтрокаПоля.Определение, "-") = 0
		И Найти(СтрокаПоля.Определение, ",") = 0
		И Найти(СтрокаПоля.Определение, "/") = 0
		И Найти(СтрокаПоля.Определение, "*") = 0
		...
		И Найти(СтрокаПоля.Определение, "<") = 0
		И (Ложь
			Или Прав(СтрокаПоля.Определение, 1) <> ")"
			Или (Истина 
				И Прав(СтрокаПоля.Определение, 1) = ")"
				И (Ложь
					Или ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаПоля.Определение, "("), "Количество")
					Или ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаПоля.Определение, "("), "Сумма")
					Или ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаПоля.Определение, "("), "Максимум")
					Или ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаПоля.Определение, "("), "Минимум")
					Или ирОбщий.СтрокиРавныЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаПоля.Определение, "("), "Среднее"))))
	Тогда
