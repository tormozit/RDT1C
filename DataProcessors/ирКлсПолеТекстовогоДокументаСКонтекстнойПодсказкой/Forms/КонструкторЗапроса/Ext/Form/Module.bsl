...
Функция ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ГруппаОтбора, Отступ = 0, ФильтрацияАгрегатов = 0, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено) Экспорт 
	...
	Для Каждого ЭлементОтбора Из ГруппаОтбора.Элементы Цикл
		...
		Если Истина
			...
			И ПредставлениеЭлемента = НейтральныйЭлементГруппы 
		Тогда
			Продолжить;
		КонецЕсли; 
		Если Ложь
	...
	КонецЦикла;
	Если КоличествоСобранныхЭлементовГруппы = 1 И Лев(ТекстГруппы, 1) = "(" Тогда
		ТекстГруппы = Сред(ТекстГруппы, 2, СтрДлина(ТекстГруппы) - 2);
	КонецЕсли; 
	Если ИерархическийСтильЛогическихВыражений И КоличествоСобранныхЭлементовГруппы > 1 Тогда
...
Процедура ДобавитьТекстОтбора(Результат, Знач Смещение, Знач ТекстУсловия, Знач ЧастьОбъединения)
	...
	Если ТекстУсловия <> "" Тогда
		...
		Иначе
			Результат = Результат + Символы.ПС + Смещение + Символы.Таб;
		КонецЕсли; 
		Результат = Результат + ПолучитьКомментарийДляВставки(ЧастьОбъединения.КомментарийОтбора, Смещение + Символы.Таб);
		ирОбщий.ДобавитьМногострочнуюСтрокуВТекстЛкс(Результат, ТекстУсловия, Смещение);
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	//ЭтаФорма.ПоказыватьИндексы = ХранилищеОбщихНастроек.Загрузить(мКлючХраненияНастроек, "ПоказыватьИндексы"); // Много времени уходит на получение структуры БД
	ЗначениеНастройки = ХранилищеОбщихНастроек.Загрузить(мКлючХраненияНастроек, "ИерархическийСтильЛогическихВыражений");
	...
	Если Не ЛиПакетныйЗапрос Тогда
		ИзменитьСвернутостьПанельПакета(ЛиПакетныйЗапрос);
	КонецЕсли;
	ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ПодстрокаФильтраДоступныхТаблиц;
...
Процедура ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, СтароеИмяТаблицыИлиПараметра = "", НовоеИмяТаблицыИлиПараметра = "", пСтароеИмяПоля = "", пНовоеИмяПоля = "",
	...
	ЭтоПереименованиеПараметра = ПараметрыДиалектаSQL.ПрефиксПараметра = Лев(пСтароеИмяПоля, 1);
...
Процедура ПриЗакрытии()
	...
	//ХранилищеОбщихНастроек.Сохранить(мКлючХраненияНастроек, "ПоказыватьИндексы", ПоказыватьИндексы);
	ХранилищеОбщихНастроек.Сохранить(мКлючХраненияНастроек, "ИерархическийСтильЛогическихВыражений", ИерархическийСтильЛогическихВыражений);
...
Процедура КПДоступныеТаблицыВключитьКоличествоСтрок(Кнопка)
	...
	Если Кнопка.Пометка Тогда
		...
		Иначе
			Ответ = Вопрос("Хотите обносить статистику?", РежимДиалогаВопрос.ДаНет);
		КонецЕсли; 
		Если Ответ = КодВозвратаДиалога.Да Тогда
...
мКлючХраненияНастроек = "ИнструментыРазработчикаTormozit.КонструкторЗапроса";
