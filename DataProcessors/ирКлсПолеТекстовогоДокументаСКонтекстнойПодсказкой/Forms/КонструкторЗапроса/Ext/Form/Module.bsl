...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "", Знач РазвернутьАвтополя = Ложь, Знач СтруктураПолей = Неопределено)
	...
	Если Истина
		И ЧастьОбъединения.Автогруппировки 
		И СтруктураПолей.ЕстьАгрегатыВЧастиОбъединения
		...
		Если СтрокаПоляПостроителя = Неопределено Тогда
			...
			Если СтрокаПоляПостроителя.Владелец().Колонки.Найти("Определение") <> Неопределено Тогда
				...
				Тогда
					...
					КонецЕсли; 
					ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаПоляПостроителя, ИмяСвойстваИмениПриемника);
				Иначе
					ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаПоляПостроителя, ИмяКлючаПриемника);
				КонецЕсли;
...
Функция СкорректироватьИмяВыбраннойТаблицы(Знач ВыбраннаяТаблица, ПринудительноАвтоимя = Ложь, РазрешитьПереименованиеДублей = Истина)
	...
	Если ЗначениеЗаполнено(СтрокаДоступнойТаблицы) Тогда
		БазовоеИмя = Лев(СтрокаДоступнойТаблицы.Имя, 50) + "Т";
	Иначе
...
Процедура ОбновитьСтатистикуЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
...
Функция ВыбратьВсеПоляИспользованныеВВыражениях(ЗапросДоГруппировки = Ложь, выхЕстьАгрегаты = Ложь, выхИндексКолонкиКоличество = Неопределено) Экспорт 
	...
	//СтруктураПолей.СобиратьПоляВнеАгрегатов = Истина; 
	СтруктураПолей.СобиратьПоляВнеАгрегатов = Не СтруктураПолей.ЕстьАгрегатыВЧастиОбъединения;
