...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	КонецЕсли;
	Если Не СтрокаПоля.Автополе Тогда
		ИмяПоля = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(СтрокаПоля.Владелец().Скопировать(Новый Структура("Автополе", Ложь), "Имя"), ИмяПоля);
	КонецЕсли; 
	Возврат ИмяПоля;
...
Процедура ПоляИндексаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Если ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") Тогда
		Если РазрешеноПеретаскиваниеВПоля(ЗначениеПеретаскивания[0]) Тогда
			СтандартнаяОбработка = Ложь;
			Для Каждого СтрокаПеретаскивания Из ЗначениеПеретаскивания Цикл
				ДобавитьДоступноеПолеВТабличноеПолеВыражений(СтрокаПеретаскивания, Элемент,,,,, "Имя");
			КонецЦикла;  
		КонецЕсли; 
	КонецЕсли;
...
мТерминалыЯзыкаЗапросов = ирОбщий.ТаблицаЗначенийИзТабличногоДокументаЛкс(мПлатформа.ПолучитьМакет("ТерминалыЯзыкаЗапросов"));
