...
Функция СобратьВыражениеЗапроса(Знач Токен, Отступ = 0, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	СтароеИмяТаблицы = "", НовоеИмяТаблицы = "", выхЕстьАгрегаты = Ложь, РазрешитьНормализациюИмен = Истина, НужноПересобратьТокены = Неопределено, СтруктураПолей = Неопределено) Экспорт
	...
	Иначе
		...
		Если ИмяПравила = "<Case>" Тогда
			ВыражениеВыбора = СобратьВыражениеЗапроса(Данные.Tokens(1), Отступ, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы,
				выхЕстьАгрегаты, РазрешитьНормализациюИмен, НужноПересобратьТокены, СтруктураПолей);
		...
		ИначеЕсли ИмяПравила = "<WhenThen>" Тогда
			...
			|" + ЛевыйОтступ + "	" + ПолучитьСловоЯзыкаЗапросов("WHEN") + " " + СобратьВыражениеЗапроса(Данные.Tokens(1), Отступ + 2, ЗапросПакета, ЧастьОбъединения,
				СтароеИмяТаблицы, НовоеИмяТаблицы, выхЕстьАгрегаты, РазрешитьНормализациюИмен, НужноПересобратьТокены, СтруктураПолей) + "
		...
		Тогда
			Результат = СобратьВыражениеЗапроса(Данные.Tokens(0), Отступ, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы, выхЕстьАгрегаты,
				РазрешитьНормализациюИмен, НужноПересобратьТокены, СтруктураПолей) + "
		...
		ИначеЕсли ИмяПравила = "<EmbeddedQuery>" Тогда
			...
			Иначе
				Результат = "(" + СобратьВыражениеЗапроса(ВнутреннийТокен,,,,,,,, НужноПересобратьТокены, СтруктураПолей) + ")";
			КонецЕсли; 
		Иначе
		...
		Если ИмяПравила = "<Field>" Тогда
			...
			Если СтруктураПолей <> Неопределено Тогда 
				...
				Если Истина
					И СтруктураПолей.Режим = "Замена" 
					И Не ЛиЛитералЗначения(Результат, Истина)
				Тогда
					ДописатьКВыражениюЕгоЗначение(Результат, СтруктураПолей);
				КонецЕсли; 
			КонецЕсли; 
...
Функция ПолучитьВыражениеЭлементаОтбораЛкс(ЭлементОтбора, Знач Отступ = 0, Знач ГруппаИзОдногоЭлемента = Ложь, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено, СтруктураПолей = Неопределено) Экспорт
...
//
Функция ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ГруппаОтбора, Отступ = 0, ФильтрацияАгрегатов = 0, выхЕстьАгрегаты = Ложь, НастройкиКомпоновки = Неопределено, СтруктураПолей = Неопределено) Экспорт 
	...
	Для Каждого ЭлементОтбора Из ГруппаОтбора.Элементы Цикл
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			КонецЕсли; 
			ПредставлениеЭлемента = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ЭлементОтбора, ОтступВложенный, ФильтрацияАгрегатов, ЕстьАгрегаты, НастройкиКомпоновки, СтруктураПолей);
			Если Истина
				И Не ГруппаИзОдногоЭлемента 
			...
			КонецЕсли;
		Иначе
			ПредставлениеЭлемента = ПолучитьВыражениеЭлементаОтбораЛкс(ЭлементОтбора, Отступ + 1, ГруппаИзОдногоЭлемента, ЕстьАгрегаты, НастройкиКомпоновки, СтруктураПолей);
		КонецЕсли; 
...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "", РазвернутьАвтополя = Ложь, СтруктураПолей = Неопределено)
	...
	ТекстУсловия = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ЧастьОбъединения.Компоновщик.Настройки.Отбор, 1, 1, ЕстьАгрегаты, ЧастьОбъединения.Компоновщик.Настройки, СтруктураПолей);
	ДобавитьТекстОтбора(Результат, Смещение, ТекстУсловия, ЧастьОбъединения);
...
Процедура СобратьВеткуСвязейТаблиц(НомерОткрытойГруппы, Знач ВеткаСвязей, ЗапросПакета, Смещение, ТекстТаблицы, ЧастьОбъединения, СтруктураПолей = Неопределено)
	...
	Для Каждого СтрокаСвязи Из ВеткаСвязей.Строки Цикл
		...
		ТекстУсловия = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(СтрокаСвязи.Условие.Отбор, 1,,, ЧастьОбъединения.Компоновщик.Настройки, СтруктураПолей);
...
Функция ДобавитьДоступнуюТаблицуВВыбранные(СтрокаДоступнойТаблицы, ПроверятьУникальность = Ложь)
	...
	Если ВыбраннаяТаблица = Неопределено Тогда
		...
		ВыбраннаяТаблица.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ВыбранныеТаблицы, СтрокаДоступнойТаблицы.Имя + "Т");
...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	Если ЛиВыражениеЯвляетсяПолемИлиТерминалом(ОпределениеПоля) Тогда 
		ЕстьПодходящееИмя = Не ЛиЛитералЗначения(ОпределениеПоля);
...
Процедура ПриОткрытии()
	...
	Если Не ЛиПакетныйЗапрос Тогда
		...
		ПодключитьОбработчикОжидания("ИзменитьСвернутостьПанельПакетаОтложенно", 0.1, Истина);
	КонецЕсли;
...
Процедура ИзменитьСвернутостьПанельПакетаОтложенно()
...
Процедура ВыбранныеТаблицыИмяПриИзменении(Элемент = Неопределено)
	...
	мТекущаяСтрокаВыбранныеТаблицы.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(мТекущаяСтрокаВыбранныеТаблицы.Владелец(),
		...
		Иначе
			Попытка
				Если ЭлементГруппы.Представление <> "" Тогда
					...
					КонецЕсли; 
					Если СтруктураПолей <> Неопределено И ЕстьАгрегаты Тогда 
...
Процедура ДобавитьПолеВСтруктуруПолей(СтруктураПолей, ИмяПоля)
	...
	#КонецЕсли
	Если ЛиЛитералЗначения(ИмяПоля, Истина) Тогда
		Возврат;
	КонецЕсли; 
...
// долгая
Функция ЛиЛитералЗначения(Знач ИмяПоля, ПроверятьСкобки = Ложь)
	...
	//СловоNULL = ПолучитьСловоЯзыкаЗапросов("NULL");
	//СловоНЕОПРЕДЕЛЕНО = ПолучитьСловоЯзыкаЗапросов("UNDEFINED");
	//СловоИСТИНА = ПолучитьСловоЯзыкаЗапросов("TRUE");
	//СловоЛОЖЬ = ПолучитьСловоЯзыкаЗапросов("FALSE");
	//СловоЧАС = ПолучитьСловоЯзыкаЗапросов("HOUR");
	Возврат Результат;
		...
		Тогда 
			Если Истина
				...
				И (Ложь
					Или СтруктураПолей.ЗапросДоГруппировки
					Или Не СтруктураПолей.ЕстьАгрегатыВЧастиОбъединения
					Или Не СтруктураПолей.ВнутриОтбора) 
			Тогда
				ДобавитьПолеВСтруктуруПолей(СтруктураПолей, Результат);
			КонецЕсли; 
			Если Истина
				И СтруктураПолей.Режим = "Замена" 
				И Не ЛиЛитералЗначения(Результат)
			Тогда 
				ДописатьКВыражениюЕгоЗначение(Результат, СтруктураПолей);
			КонецЕсли; 
		КонецЕсли; 
...
Функция ОткрытьВложенныйПакет(ВложенныйПакет = Неопределено, Знач ИмяЗапроса = "", ТекстЗапроса = "", ИмяВыбранногоПоляДляАктивации = "")
...
Процедура КПВыбранныеТаблицыЗаменитьТаблицу(Кнопка)
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			мТекущаяСтрокаВыбранныеТаблицы.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ВыбранныеТаблицы, ТекущаяДоступнаяТаблица.Имя + "Т");
...
Функция ВыбратьВсеПоляИспользованныеВВыражениях(ЗапросДоГруппировки = Ложь, выхЕстьАгрегаты = Ложь) Экспорт 
	...
	СтрокаВыбранногоПоля.Определение = """--------""";
	СтрокаВыбранногоПоля.ИД = Новый УникальныйИдентификатор();
	...
	Для Каждого КлючИЗначение Из СтруктураПолей.СоответствиеПолей Цикл
		СтрокаРезультата = Результат.Добавить();
		СтрокаВыбранногоПоля = ВыбранныеПоля.Найти(КлючИЗначение.Значение, "Определение");
		Если СтрокаВыбранногоПоля = Неопределено Тогда
			СтрокаВыбранногоПоля = ВыбранныеПоля.Добавить();
			СтрокаВыбранногоПоля.Определение = КлючИЗначение.Значение;
			СтрокаВыбранногоПоля.ИД = Новый УникальныйИдентификатор();
			...
			ИмяПоля = ирОбщий.ПолучитьИдентификаторИзПредставления(КлючИЗначение.Значение);
			СтрокаВыбранногоПоля.Имя = ИмяПоля;
			СтрокаВыбранногоПоля.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ВыбранныеПоля, СтрокаВыбранногоПоля);
			ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаВыбранногоПоля);
		Иначе
...
Функция НоваяСтруктураОбработкиПолей()
	
	СтруктураПолей = Новый Структура;
	СтруктураПолей.Вставить("Режим", "Чтение");
