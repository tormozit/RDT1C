	...
	Если ТаблицаКомментариев <> Неопределено Тогда
		...
		Тогда
			...
			Если НачальныйТерминал <> Неопределено Тогда
				КонечныйТерминал = Токен.GetBorderToken(1);
...
Функция ПроверитьДобавитьПараметр(выхИмяПараметра, ТипЗначения = Неопределено)
	...
	Если Лев(выхИмяПараметра, 1) = ПараметрыДиалектаSQL.ПрефиксПараметра Тогда
...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета, ЧастьОбъединения, Смещение = "", РазвернутьАвтополя = Ложь)
	...
	Для Каждого СтрокаПоляОбъединения Из ЗапросПакета.ПоляОбъединения Цикл
		...
		ИначеЕсли ВыбранноеПоле.Автополе И Не РазвернутьАвтополя Тогда
			ПсевдонимПоля = "";
			ТекстВыражения = "*";
...
Процедура ОбновитьДоступныеВременныеТаблицы(ИмяВременнойТаблицы = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из СтруктураДобавленныхТаблиц Цикл
		НайтиДобавитьДоступнуюВременнуюТаблицу(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
