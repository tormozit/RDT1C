...
Процедура ОбработатьЗапросПослеУстановкиДанных(ЗапросПакета)
	...
	Тогда
		...
		Для Каждого ЧастьОбъединения Из ЗапросПакета.ЧастиОбъединения Цикл
			...
			МассивВыбранныхЗвездочкамиТаблиц = Новый Массив();
				...
				Если ПерваяЧасть = ЧастьОбъединения Тогда
					...
					Если ИмяПоля = "*" Тогда
						...
						Если ИмяТаблицы = "" Тогда
							...
							МассивВыбранныхЗвездочкамиТаблиц = ЧастьОбъединения.ВыбранныеТаблицы;
						Иначе
							...
							КонецЕсли; 
							МассивВыбранныхЗвездочкамиТаблиц.Добавить(СтрокаВыбраннойТаблицы);
						КонецЕсли; 
					Иначе
			...
			КонецЕсли; 
			Для ИндексТаблицыИзЗвездочки = 0 По МассивВыбранныхЗвездочкамиТаблиц.Количество() - 1 Цикл
				ВыбраннаяТаблица = МассивВыбранныхЗвездочкамиТаблиц[ИндексТаблицыИзЗвездочки];
				...
				Если КоличествоВыбранныхПолей > 0 Тогда
					СдвигИндексаПослеДобавления = -(КоличествоВыбранныхПолей - ИндексТаблицыИзЗвездочки - 1);
				КонецЕсли; 
...
Функция ЗаполнитьПоТокену(Знач Токен, Стек = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено, Индикатор = Неопределено,
	РазборРасширений = Ложь, ТаблицаТекстаРасширения = Неопределено) Экспорт
	...
	ИначеЕсли ИмяПравила = "<Join>" Тогда
		ЛевыйУзел = ЗаполнитьПоТокену(Данные.Tokens(0).Data.Tokens(0), Стек, ЗапросПакета, ЧастьОбъединения, Индикатор, РазборРасширений, ТаблицаТекстаРасширения); //<TableOrJoin>.<TableWithName>
		ТекущийУзел = ЧастьОбъединения.СвязиТаблиц.Добавить();
		ПравыйУзел = ЗаполнитьПоТокену(Данные.Tokens(3).Data.Tokens(0), Стек, ЗапросПакета, ЧастьОбъединения, Индикатор, РазборРасширений, ТаблицаТекстаРасширения); //<TableOrJoin>.<TableWithName>
		...
		Если ТипЗнч(ЛевыйУзел) = Тип("СтрокаТаблицыЗначений") Тогда
			ТекущийУзел.ЛеваяТаблица = ЛевыйУзел.ПраваяТаблица;
		Иначе
			ТекущийУзел.ЛеваяТаблица = ЛевыйУзел;
		КонецЕсли; 
		Если ТипЗнч(ПравыйУзел) = Тип("СтрокаТаблицыЗначений") Тогда
			ТекущийУзел.ПраваяТаблица = ПравыйУзел.ЛеваяТаблица;
		Иначе
			ТекущийУзел.ПраваяТаблица = ПравыйУзел;
		КонецЕсли; 
		//ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(ТекущийУзел, "ЛеваяТаблица");
		//ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(ТекущийУзел, "ПраваяТаблица");
		
		//Результат = ТекущийУзел.ПраваяТаблица;
		//Результат = ТекущийУзел.ЛеваяТаблица;
		Результат = ТекущийУзел;
	...
	Иначе
		...
		Для ИндексТокена = 0 По КоличествоДочерних - 1 Цикл
			...
			Если ТокенВниз.Kind = 0 Тогда
				...
				ПсевдонимСнизу = ЗаполнитьПоТокену(ТокенВниз, Стек, ЗапросПакета, ЧастьОбъединения, Индикатор, РазборРасширений, ТаблицаТекстаРасширения);
	...
	Если ТаблицаТекстаРасширения <> Неопределено Тогда
		...
		Тогда
			...
			Если СтрокаСлужебнойТаблицы <> Неопределено Тогда
				СтрокиСвязи = ЧастьОбъединения.СвязиТаблиц.НайтиСтроки(Новый Структура("ЛеваяТаблица", СтрокаСлужебнойТаблицы.Имя));
				Для Каждого СтрокаСвязи Из СтрокиСвязи Цикл
					...
					Если СтрокаОбязательнойТаблицы <> Неопределено Тогда
						СтрокаСвязи.ЛеваяТаблица = СтрокаОбязательнойТаблицы.Имя;
					Иначе
						ЧастьОбъединения.СвязиТаблиц.Удалить(СтрокаСвязи);
					КонецЕсли; 
				КонецЦикла;
...
Функция СобратьВыражениеЗапроса(Токен, Отступ = 0, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	...
	Если ИмяПравила = "<Case>" Тогда
		ВыражениеВыбора = СобратьВыражениеЗапроса(Данные.Tokens(1), Отступ, ЗапросПакета, ЧастьОбъединения, СтароеИмяТаблицы, НовоеИмяТаблицы,
			выхЕстьАгрегаты, РазрешитьНормализациюИмен);
	...
	ИначеЕсли ИмяПравила = "<WhenThen>" Тогда
		...
		|" + ЛевыйОтступ + "	" + ПолучитьСловоЯзыкаЗапросов("WHEN") + " " + СобратьВыражениеЗапроса(Данные.Tokens(1), Отступ + 1, ЗапросПакета, ЧастьОбъединения,
			СтароеИмяТаблицы, НовоеИмяТаблицы, выхЕстьАгрегаты, РазрешитьНормализациюИмен) + "
	...
	ИначеЕсли ИмяПравила = "<EmbededQuery>" Тогда
		...
		Иначе
			Результат = "(" + СобратьВыражениеЗапроса(ВнутреннийТокен) + ")";
		КонецЕсли; 
	Иначе
...
Функция ПолучитьВыражениеЭлементаОтбораЛкс(ЭлементОтбора, Знач Отступ = 0, Знач ГруппаИзОдногоЭлемента = Ложь, выхЕстьАгрегаты = Ложь, ЧастьОбъединения = Неопределено) Экспорт
	...
	Иначе
		Если ТипЗнч(ЭлементОтбора.ЛевоеЗначение) = Тип("ПолеКомпоновкиДанных") Тогда
			ДоступноеПолеИлиОписаниеТипов = ЧастьОбъединения.Компоновщик.Настройки.ДоступныеПоляОтбора.НайтиПоле(ЭлементОтбора.ЛевоеЗначение);
...
//
Функция ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ГруппаОтбора, Отступ = 0, ФильтрацияАгрегатов = 0, выхЕстьАгрегаты = Ложь, ЧастьОбъединения = Неопределено) Экспорт 
	...
	Для Каждого ЭлементОтбора Из ГруппаОтбора.Элементы Цикл
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			ПредставлениеЭлемента = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ЭлементОтбора, Отступ + 1, ФильтрацияАгрегатов, ЕстьАгрегаты, ЧастьОбъединения);
			Если Истина
				И Не ГруппаИзОдногоЭлемента 
			...
			КонецЕсли;
		Иначе
			ПредставлениеЭлемента = ПолучитьВыражениеЭлементаОтбораЛкс(ЭлементОтбора, Отступ + 1, ГруппаИзОдногоЭлемента, ЕстьАгрегаты, ЧастьОбъединения);
		КонецЕсли; 
		Если Ложь
...
Функция СобратьТекстПакета(Пакет = Неопределено, Смещение = "")
	...
	Для Каждого ЗапросПакета Из Пакет Цикл
		...
		Если Результат <> "" Тогда
			...
			Тогда
				Результат = Результат + ирОбщий.ПолучитьСтрокуПовторомЛкс(ПараметрыДиалектаSQL.СтрочныйКомментарий, 20) + "
				|";
			КонецЕсли; 
		КонецЕсли; 
...
Функция ПолучитьОбновитьВыражениеПоляПостроителя(ПолеПостроителя, ТабличноеПоле, Знач ЗапросПакета, Знач ЧастьОбъединения = Неопределено)
	...
	ТекстВыражения = ОбновитьВыражениеЗапроса(ПолеПостроителя, ТабличноеПоле, ЗапросПакета, ЧастьОбъединения,,,,,, Ложь);
...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета, ЧастьОбъединения, Смещение = "")
	...
	// WHERE
	ТекстУсловия = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ЧастьОбъединения.Условие.Отбор, 1, 1,, ЧастьОбъединения);
	...
	// HAVING
	ТекстУсловия = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ЧастьОбъединения.Условие.Отбор, 1, 2,, ЧастьОбъединения);
	Если ТекстУсловия <> "" Тогда
...
Функция СобратьТекстИЗ(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "")
	...
	Для Каждого НомерГруппы Из ГруппыТаблиц Цикл
		ГруппаОткрыта = Ложь;
		Для Индекс = 0 По КоличествоПарТаблиц - 1 Цикл
			...
			Иначе
				...
				КонецЕсли; 
				СтрокаВыбраннойПравойТаблицы = ЧастьОбъединения.ВыбранныеТаблицы.Найти(НРег(СоединяемаяТаблица), "НИмя");
				Если Истина
					И СтрокаВыбраннойПравойТаблицы.НомерГруппы = НомерГруппы
				Тогда
					...
					ТекстУсловия = ПолучитьВыражениеОтбораКомпоновкиНаЯзыкеЛкс(ПараТаблиц.Условие.Отбор, 2,,, ЧастьОбъединения);
		...
		Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
			Если Истина
				И ДобавленныеТаблицыСвязей.Найти(ВыбраннаяТаблица.Имя) = Неопределено 
				И ВыбраннаяТаблица.НомерГруппы = НомерГруппы
			Тогда
				Если Не ГруппаОткрыта Тогда
					Если НомерГруппы > 0 Тогда
						ТекстТаблицы = ТекстТаблицы + "{";
...
Процедура ПереименоватьВыбраннуюТаблицу()
	...
	//Если ирОбщий.СтрокиРавныЛкс(мТекущееИмяТаблицы, НовоеИмяТаблицы)  Тогда
	...
	Для Каждого СвязьТаблиц Из ЧастьОбъединения.СвязиТаблиц Цикл
		...
		Если ирОбщий.СтрокиРавныЛкс(СвязьТаблиц.ЛеваяТаблица, мТекущееИмяТаблицы) Тогда
			СвязьТаблиц.ЛеваяТаблица = НовоеИмяТаблицы;
		КонецЕсли; 
		Если ирОбщий.СтрокиРавныЛкс(СвязьТаблиц.ПраваяТаблица, мТекущееИмяТаблицы) Тогда
...
Процедура ПараметрыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования Тогда
		Элемент.ТекущиеДанные.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(Элемент.Значение, Элемент.ТекущиеДанные,,, ПолучитьСловоЯзыкаЗапросов("Parameter"));
	КонецЕсли; 
...
Процедура ИзменитьСвернутостьПанельЧастей(Видимость)
	...
	ирОбщий.ИзменитьСвернутостьЛкс(Видимость, ЭлементыФормы.ПанельЧастей, ЭлементыФормы.вРазделительПанельЧастей, Панель, "лево");
...
Функция ПолучитьИмяВыбраннойТаблицы(СтрокаВыбраннойТаблицы)
	...
	Если Не ЗначениеЗаполнено(СтрокаВыбраннойТаблицы.Имя) Тогда
		СтрокаВыбраннойТаблицы.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(СтрокаВыбраннойТаблицы.Владелец(), СтрокаВыбраннойТаблицы.ПолноеИмя, "Имя");
		ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаВыбраннойТаблицы);
	КонецЕсли;
...
Функция ОткрытьВложенныйПакет(ВложенныйПакет = Неопределено, Знач ИмяЗапроса = "")
	...
	ФормаКонструктора.Параметры = Параметры;
...
Процедура ВыбранныеТаблицыПередУдалением(Элемент, Отказ)
	...
	Для СчетчикСвязиТаблиц = 1 По НачальноеКоличество Цикл
		...
		Если Ложь
			Или ирОбщий.СтрокиРавныЛкс(СвязьТаблиц.ЛеваяТаблица, мТекущаяСтрокаВыбранныеТаблицы.Имя)
			Или ирОбщий.СтрокиРавныЛкс(СвязьТаблиц.ПраваяТаблица, мТекущаяСтрокаВыбранныеТаблицы.Имя)
		Тогда
