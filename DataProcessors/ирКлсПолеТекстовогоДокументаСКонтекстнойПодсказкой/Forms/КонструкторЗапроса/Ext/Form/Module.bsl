	...
	Если РезультатФормы <> Неопределено Тогда
		Если ЗначениеЗаполнено(ИмяЗапроса) Тогда
			...
			Для Каждого СтрокаСтарогоПоля Из ВложенныйПакет[0].ПоляОбъединения Цикл
				...
				Если СтрокаСтарогоПоля.Имя <> СтрокаНовогоПоля.Имя Тогда
...
Процедура ЗагрузитьПараметрТаблицы()
	...
	ПолеВводаЗначенияПараметра.РежимВыбораИзСписка = ЭтоПараметрПериодичность1С(мТекущаяСтрокаПараметраТаблицы);
	Если ПолеВводаЗначенияПараметра.РежимВыбораИзСписка Тогда
...
Процедура ПараметрыТаблицыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Не ЭтоПараметрПериодичность1С(ВыбраннаяСтрока) Тогда
		СтандартнаяОбработка = Ложь;
