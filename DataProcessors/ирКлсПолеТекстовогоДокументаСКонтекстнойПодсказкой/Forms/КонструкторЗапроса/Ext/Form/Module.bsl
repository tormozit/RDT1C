...
Процедура ОбновлениеОтображения()
	...
	Если ПараметрыДиалектаSQL <> Неопределено Тогда
		...
		ЭлементыФормы.ИмяОсновнойТаблицы.Доступность = Ложь
			Или ТипЗапроса <> 0;
		ЭлементыФормы.КомментарийОтбора.Доступность = ТипЗапроса <> 2 И (ТипЗапроса <> 3 Или ЗначенияИзВыборки);
		...
		ЭлементыФормы.ПанельЧасти.Страницы.ВыбранныеТаблицы.Видимость = ТипЗапроса <> 2 И (ТипЗапроса <> 3 Или ЗначенияИзВыборки);
		ЭлементыФормы.ПанельЧасти.Страницы.ДеревоУсловия.Видимость = ТипЗапроса <> 2 И (ТипЗапроса <> 3 Или ЗначенияИзВыборки);
		ЭлементыФормы.ПанельЧасти.Страницы.Группировки.Видимость = ТипЗапроса < 2 Или (ТипЗапроса = 3 И ЗначенияИзВыборки);
		ЭлементыФормы.ПанельЧасти.Страницы.Текст.Видимость = ТипЗапроса <> 2 И (ТипЗапроса <> 3 Или ЗначенияИзВыборки);
...
Процедура ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, Знач пСтароеИмя, Знач пНовоеИмя, ИмяВременнойТаблицы = "")
    
    Состояние("Обновление выражений...");
	ЭтоПереименованиеПараметра = ПараметрыДиалектаSQL.ПрефиксПараметра = Лев(пСтароеИмя, 1);
	ГлобальныйОбход = ЭтоПереименованиеПараметра Или ЗначениеЗаполнено(ИмяВременнойТаблицы);
	Если Пакет = Неопределено Тогда
		...
		Для Каждого ЧастьОбъединения Из лЧастиОбъединения Цикл
			...
			Если ГлобальныйОбход Тогда
				Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
					...
					КонецЦикла;
					Если ВыбраннаяТаблица.ВложенныйПакет <> Неопределено Тогда
						ПереименоватьТаблицуИлиПараметр(ВыбраннаяТаблица.ВложенныйПакет, СтароеИмя, НовоеИмя, ИмяВременнойТаблицы);
					КонецЕсли; 
				КонецЦикла;
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяВременнойТаблицы) Тогда
				СтрокаВыбраннойТаблицы = ЧастьОбъединения.ВыбранныеТаблицы.Найти(ИмяВременнойТаблицы, "ПолноеИмя");
				Если СтрокаВыбраннойТаблицы <> Неопределено Тогда
					СтароеИмя = СтрокаВыбраннойТаблицы.Имя + "." + СтароеИмя;
					НовоеИмя = СтрокаВыбраннойТаблицы.Имя + "." + НовоеИмя;
				Иначе
					Продолжить;
				КонецЕсли; 
			КонецЕсли; 
	...
	Тогда
		...
		Если НоваяНастройкаКомпоновки.Отбор.Элементы.Количество() = 0 Тогда
			...
			Если ДоступноеПолеТаблицыРодителя <> Неопределено Тогда
				Для Каждого ДоступноеПолеЛевое Из ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных(мТекущаяСтрокаСвязи.Таблица)).Элементы Цикл
	...
	Если СтрокаПоляОбъединенияПоИмени <> СтрокаПоляОбъединенияПоИД Тогда // заработало в 3.19, ранее ошибочно никогда не выполнялось
		Если СтрокаПоляОбъединенияПоИмени = Неопределено Тогда
			...
			Если ТипЗапроса = 1 Тогда
				ПереименоватьТаблицуИлиПараметр(, СтароеИмя, НовоеИмя, ИмяОсновнойТаблицы);
			КонецЕсли; 
		Иначе
			// Склеивание полей
...
Процедура ПоляДоступнойТаблицыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ДоступноеПоле = КомпоновщикЧасти.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(СтрокаВыбраннойТаблицы.Имя + "." + ВыбраннаяСтрока.Имя));
	Если ДоступноеПоле <> Неопределено Тогда
