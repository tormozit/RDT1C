...
Процедура ДобавитьМетодыМодуля(Знач СтруктураТипа, СтрокаМетаТипа)
	...
	КонецЕсли; 
	Для Каждого МетодМодуля Из МетодыМодуля.НайтиСтроки(Новый Структура("ЛиЭкспорт", Истина)) Цикл
...
Процедура ОбщиеМетодыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура ОбщиеМетодыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		ирОбщий.ОткрытьТекстЛкс(СтрокаПолногоОписания.Тело, "Тело метода " + СтрокаПолногоОписания.ПолноеИмя, "ВстроенныйЯзык", Истина, СтрокаПолногоОписания.ПолноеИмя);
	КонецЕсли; 
