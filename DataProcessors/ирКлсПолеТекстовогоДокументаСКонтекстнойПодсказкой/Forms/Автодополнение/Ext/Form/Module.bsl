...
Процедура ТаблицаСловПриАктивизацииСтроки(Элемент)
	...
	Если Истина
		И ТекущаяСтрока.ТипЗначения = "??"
		И ТекущаяСтрока.ТипСлова = "Метод"
	Тогда
		#Если Сервер И Не Сервер Тогда
			мПлатформа = Обработки.ирПлатформа.Создать();
		#КонецЕсли
		ТаблицаМетодов = мПлатформа.ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтруктураТипаКонтекста, ТекущаяСтрока.Слово, "Метод",, ЭтотОбъект);
		Если ТаблицаМетодов.Количество() > 0 Тогда
			ТекущаяСтрока.ТипЗначения = ПолучитьПредставлениеМассиваСтруктурТипов(ТаблицаМетодов[0].ТаблицаСтруктурТипов);
		КонецЕсли; 
	КонецЕсли; 
