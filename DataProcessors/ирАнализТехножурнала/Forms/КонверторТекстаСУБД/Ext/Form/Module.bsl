...
Процедура ОбновитьЗапрос()
	...
	КонецЕсли; 
	Индексы.Очистить();
	СтруктураЗапроса = ПолучитьСтруктуруЗапросаИзТекстаБД(ЭлементыФормы.ТекстБД.ПолучитьТекст(), лТипСУБД, ПересобратьТекст, ПереводитьИндексы, ПереводитьВМета);
	...
	ФормаСтруктурыХранения = ирОбщий.ПолучитьФормуЛкс("Обработка.ирСтруктураХраненияБД.Форма");
	ФормаСтруктурыХранения.ПоказыватьСУБД = Не ЭтоТекстSDBL;
	ФормаСтруктурыХранения.ПоказыватьSDBL = ЭтоТекстSDBL;
	ФормаСтруктурыХранения.ПоказыватьРазмеры = Истина;
	ФормаСтруктурыХранения.ОбновитьТаблицы();
	Для Каждого СтрокаТаблицыБД Из Таблицы Цикл
		...
		КонецЕсли; 
		СтрокаСтруктурыХрания = ФормаСтруктурыХранения.Таблицы.Найти(СтрокаТаблицыБД.ИмяБД, "ИмяТаблицыХранения");
		Если СтрокаСтруктурыХрания <> Неопределено Тогда
...
Процедура ПараметрыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка = Элемент.Колонки.ЗначениеSDBL Тогда
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстовогоДокументаЛкс(ЭтаФорма, ЭлементыФормы.Результат, ВыбраннаяСтрока.ЗначениеSDBL);
	Иначе
...
Процедура КонтекстноеМенюКонстантаНайтиВТексте(Кнопка)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Если Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстовогоДокументаЛкс(ЭтаФорма, ЭлементыФормы[ИмяСтраницы], ТекущаяСтрока.ЗначениеSDBL) Тогда 
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстовогоДокументаЛкс(ЭтаФорма, ЭлементыФормы[ИмяСтраницы], ТекущаяСтрока.Имя);
	КонецЕсли; 
