...
Процедура ОбновитьЗапрос()
	...
	СтруктураХраненияБД = ирКэш.ПолучитьСтруктуруХраненияБДЛкс(Не ЭтоТекстSDBL,, мПлатформа.ЧужаяСхемаБД <> Неопределено);
...
Процедура ТаблицыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяСтрокаТаблиц = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	СтруктуруХраненияБД = ирКэш.ПолучитьСтруктуруХраненияБДЛкс(Не ЭтоТекстSDBL,, мПлатформа.ЧужаяСхемаБД <> Неопределено);
