...
Процедура ДобавитьДочерние(РодительскаяСтрокаДерева, НомераЗапросов, РодительЕстьЖертва = Истина)
	...
	Для Каждого НомерЗапроса Из ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(НомераЗапросов, ",", Истина) Цикл
		...
		Иначе
			ИмяПоляСвязи = "lkpid";
		КонецЕсли; 
		ОтборСвязи = ВременнныйПостроительЗапроса.Отбор.Добавить(ИмяПоляСвязи);
		...
		//ВременнныйПостроительЗапроса.Отбор.Добавить("ИнфобазаСУБД").Установить(Инфобаза);
		ВременнныйПостроительЗапроса.Порядок.Установить("МоментВремени Возр");
		
		СтрокиЖурнала = ВременнныйПостроительЗапроса.Результат.Выгрузить();
...
Процедура ДеревоОжиданийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если ДанныеСтроки.РодительЕстьЖертва Тогда
		Картинка = ирОбщий.ПолучитьОбщуюКартинкуЛкс("ирВыходящий");
		ЦветФона = Новый Цвет(240, 240, 255);
	Иначе
		Картинка = ирОбщий.ПолучитьОбщуюКартинкуЛкс("ирВходящий");
