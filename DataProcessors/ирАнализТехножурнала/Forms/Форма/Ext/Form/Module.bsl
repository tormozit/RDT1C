	...
	Если Истина
		...
		И (Ложь
			Или (Истина
				...
				И ДанныеСтроки.ПроцессОС = мПлатформа.ПолучитьИдентификаторПроцессаОС()
...
Процедура ТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		...
		ФормаБлокировки = ПолучитьФорму("Блокировка", , ВыбраннаяСтрока.МоментВремени);
	...
	Тогда
		...
		Тогда
			...
			Тогда
				...
				Если ТекстПланаЗапроса = "<Текст слишком большой для быстрого перевода>" Тогда
...
Процедура ОбновитьИтоги(Принудительно = Ложь)
	...
	Если мСхемаКомпоновки = Неопределено Тогда
		...
		ирОбщий.СоздатьИлиОбновитьНаборДанныхОбъектПоМетаданнымЛкс(мСхемаКомпоновки, ТаблицаЖурнала.ВыгрузитьКолонки().Колонки);
...
Функция РассчитатьИтоги(Принудительно = Ложь)
	...
	ГлавноеПоле = ирОбщий.ПолучитьПервыйФрагментЛкс(СтрокаКлюча, ",");
	...
	КонецЕсли; 
	КлючевыеПоля = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(СтрокаКлюча, ",", Истина);
	...
	ТаблицаИтогов = ирОбщий.СкомпоноватьВКоллекциюЗначенийПоСхемеЛкс(мСхемаКомпоновки, Компоновщик.Настройки, , мВнешниеНаборыДанных,
...
Процедура УстановитьБесполезныеКолонки(БесполезныеКолонкиТрассы = Неопределено) Экспорт
	...
	Если БесполезныеКолонкиТрассы <> Неопределено Тогда
		Если ТипЗнч(БесполезныеКолонкиТрассы) = Тип("Строка") Тогда
			мБесполезныеКолонкиТрассы = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(БесполезныеКолонкиТрассы, ",", Истина, Ложь);
		Иначе
	...
	Иначе
		...
		Для Каждого ЭлементОтбора Из ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок Цикл
			Если ЭлементОтбора.Использование Тогда
				Попытка
					лВидСравнения = Вычислить("ВидСравненияКомпоновкиДанных." + ЭлементОтбора.ВидСравнения);
				Исключение
					Продолжить;
				КонецПопытки; 
				ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(НастройкаКомпоновки.Отбор, ЭлементОтбора.Имя, ЭлементОтбора.Значение, лВидСравнения);
			КонецЕсли; 
		КонецЦикла;
...
Процедура ДействияФормыОчиститьКаталогЖурнала(Кнопка)
	...
	ирОбщий.ОчиститьКаталогЖурналаЛкс(КаталогЖурнала);
...
Процедура СвойстваВТерминахМетаданныхПриИзменении(Элемент = Неопределено)
	...
	УстановитьВидимостьКолонок(ЭлементыФормы.Блокировки);
...
Процедура ПриАктивизацииСтрокиИтогов(Элемент = Неопределено)
	...
	Если ИтогиВключены Тогда
		...
		Если ТекущаяСтрока <> Неопределено Тогда
			...
			ИначеЕсли ПанельИтоги.ТекущаяСтраница = ПанельИтоги.Страницы.Блокировки Тогда
...
Процедура СкрытьКолонкиПоСобытию(СкрытьКолонкиПоСобытию = "<ALL>", СкрытьКолонкуСобытия = Истина)
	...
	Если ЗначениеЗаполнено(СкрытьКолонкиПоСобытию) Тогда
		...
		Иначе
			мАктуальныеСвойства = ПолучитьСтруктуруСвойствСобытия(СкрытьКолонкиПоСобытию, Не СкрытьКолонкуСобытия);
		КонецЕсли; 
	КонецЕсли; 
...
Процедура УстановитьРежимИтогов(НовыйРежим = Ложь) Экспорт
	
	ЭтаФорма.ИтогиВключены = НовыйРежим;
	ирОбщий.ИзменитьСвернутостьЛкс(ИтогиВключены, ЭлементыФормы.ПанельИтоги, ЭтаФорма.ЭлементыФормы.гРазделительИтогиЖурнал, ЭтаФорма.Панель, "верх");
	Если Не ИтогиВключены Тогда
		мАктуальныеСвойства = Неопределено;
		ОтборТаблицыЖурнала = ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок;
		ОтборТаблицыЖурнала.Сбросить();
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не Отказ Тогда
		ирОбщий.ИзменитьСвернутостьЛкс(Истина, ЭлементыФормы.ПанельИтоги, ЭтаФорма.ЭлементыФормы.гРазделительИтогиЖурнал, ЭтаФорма.Панель, "верх");
		ирОбщий.ИзменитьСвернутостьЛкс(Истина, ЭлементыФормы.ПанельСвойств, ЭтаФорма.ЭлементыФормы.гРазделительЖурналСвойства, ЭтаФорма.Панель, "низ");
...
Процедура КоманднаяПанельЖурналРегистрацииОткрытьСтрокуВОтдельномОкне(Кнопка = Неопределено) Экспорт 
...
Процедура ОбновитьТаблицыМетаданных(ТаблицыМетаданныхСтрокой)
	...
	Массив = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(ТаблицыМетаданныхСтрокой, ",", Истина, Ложь);
...
Процедура КоманднаяПанельЖурналРегистрацииПоказыватьПанельСвойств(Кнопка)
	...
	Кнопка.Пометка = Не Кнопка.Пометка;
	ирОбщий.ИзменитьСвернутостьЛкс(Кнопка.Пометка, ЭлементыФормы.ПанельСвойств, ЭтаФорма.ЭлементыФормы.гРазделительЖурналСвойства, ЭтаФорма.Панель, "низ");
	ОбновитьТекущуюСтраницуПанелиЭлемента();
...
Процедура ДействияФормыСохранитьКак(Кнопка)
	...
	ирОбщий.СохранитьЗначениеВФайлЛкс(СтруктураФайла, "tld", "Данные техножурнала 1С", Истина);
	
КонецПроцедуры

Процедура ДействияФормыОткрытьФайл(Кнопка)
	
	СтруктураФайла = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс("tld", "Данные техножурнала 1С", Истина);
...
Функция ПолучитьСтрокуОтбораТекстаSDBL()
	...
	Иначе
		...
		Если ирОбщий.СтрокиРавныЛкс(ОтборТипВыраженияSDBL, "ИмяТаблицы") Тогда
...
Функция ПолучитьИмяТаблицыХраненияДляОтбора(ОтборТекстSDBL, ЛиИменаБД)
	...
	Фрагменты = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(ОтборТекстSDBL);
...
Процедура ОтборТекстSDBLНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ирОбщий.СтрокиРавныЛкс(ОтборТипВыраженияSDBL, "ИмяТаблицы") Тогда
		СтандартнаяОбработка = Ложь;
		Форма = ирОбщий.ПолучитьФормуВыбораОбъектаМетаданныхЛкс(,, Элемент.Значение,, Истина, Истина, Истина, Истина, Истина, Истина, Истина);
...
Процедура КоманднаяПанельЖурналРегистрацииСжатьКолонки(Кнопка)
	
	ирОбщий.СжатьКолонкиТабличногоПоляЛкс(ЭлементыФормы.ТаблицаЖурнала);
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирАнализТехножурнала.Форма.Форма");
...
ПанельИтоги.Страницы.ТекстыSDBL.Значение = Новый Структура("Группировка, Рассчитаны", "ТекстSDBLШаблон, ТекстSDBLШаблонМета", Ложь);
ПанельИтоги.Страницы.ТекстыСУБД.Значение = Новый Структура("Группировка, Рассчитаны", "ТекстСУБДШаблон, ТекстСУБДШаблонМета", Ложь);
ПанельИтоги.Страницы.Блокировки.Значение = Новый Структура("Группировка, Рассчитаны", "Regions, RegionsМета", Ложь);
...
ПанельИтоги.Страницы.Описания.Значение = Новый Структура("Группировка, Рассчитаны", "ОписаниеШаблон", Ложь);
...
ЭтаФорма.БлокировкиПолеСортировки1 = "Длительность";
...
ОписаниеТехножурнала = ирОбщий.ПолучитьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирНастройкаТехножурнала");
#Если _ Тогда
