...
Процедура ОбновитьТаблицуЖурнала() Экспорт
	...
	Если Не РежимДозагрузки И "" + ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок <> "" Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	МоментНачалаЗагрузки = ТекущаяДата();
	ИменаПередаваемыхСвойств = мИменаВозвращаемыхСвойств + ", мАдресЧужойСхемыБД, мЛиАнализТрассыЗапроса, ТаблицаЖурнала";
	ОбщиеПараметрыОбработки = Новый Структура(ИменаПередаваемыхСвойств);
	ЗаполнитьЗначенияСвойств(ОбщиеПараметрыОбработки, ЭтотОбъект, ИменаПередаваемыхСвойств); 
	Для Каждого МетаРеквизит Из Метаданные().Реквизиты Цикл
		ОбщиеПараметрыОбработки.Вставить(МетаРеквизит.Имя, ЭтотОбъект[МетаРеквизит.Имя]);
	КонецЦикла;

	#Если Сервер И Не Сервер Тогда
		ОбновитьТаблицуЖурналаЗавершение();
	#КонецЕсли
	ирОбщий.ПрочитатьТехножурналЛкс(МоментНачалаЗагрузки, ОбщиеПараметрыОбработки, ФильтрЗагрузки,
		ЭтаФорма, ЭлементыФормы.ДействияФормы.Кнопки.Загрузить, "ОбновитьТаблицуЖурналаЗавершение");
...
Процедура ОбновитьТаблицуЖурналаЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		Если СостояниеЗадания <> Неопределено Тогда
			ТаблицаЖурнала.Загрузить(РезультатЗадания.ТаблицаЖурнала);
		КонецЕсли; 
		МоментНачалаЗагрузки = ПараметрыЗадания[0];
...
Процедура УстановитьБесполезныеКолонки(БесполезныеКолонкиТрассы = Неопределено) Экспорт
	...
	Для Каждого КолонкаТП Из ЭлементыФормы.ТаблицаЖурнала.Колонки Цикл
		...
		Если Ложь
			...
			Или ирОбщий.СтрокиРавныЛкс(ИмяКолонкиДанных, "Вложенность")
			Или ирОбщий.СтрокиРавныЛкс(ИмяКолонкиДанных, "ТекстSDBLШаблон")
	...
	Если Истина
		...
		И (Ложь
			Или (Истина
				...
				И ДанныеСтроки.ПроцессОС = мПлатформа.ИдентификаторПроцессаОС()
