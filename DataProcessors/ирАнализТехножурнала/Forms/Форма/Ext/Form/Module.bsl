...
Процедура ОбновитьТаблицуЖурнала() Экспорт
	...
	Если Не РежимДозагрузки И "" + ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок <> "" Тогда
		Ответ = Вопрос("Применить при загрузке текущий фильтр отображения (" + ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок + ")?", РежимДиалогаВопрос.ДаНетОтмена,, КодВозвратаДиалога.Нет);
		Если Ответ = КодВозвратаДиалога.Да Тогда 
	...
	ЖурналПрочитан = ПрочитатьЖурнал(ФильтрЗагрузки);
...
Процедура ПриАктивизацииСтрокиИтогов(Элемент = Неопределено)
	...
	Если ИтогиВключены Тогда
		...
		//	СтруктуруКлюча = ирОбщий.ПолучитьПервыйФрагментЛкс(СтруктуруКлюча, ",");
		//КонецЕсли; 
		СтруктуруКлюча = ирОбщий.ПолучитьПервыйФрагментЛкс(СтруктуруКлюча, ",");
