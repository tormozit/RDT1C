...
Процедура ТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		...
		ФормаБлокировки = ПолучитьФорму("БлокировкаУпр", , ВыбраннаяСтрока.МоментВремени);
...
Процедура УстановитьБесполезныеКолонки(БесполезныеКолонкиТрассы = Неопределено) Экспорт
	...
	Для Каждого КолонкаТП Из ЭлементыФормы.ТаблицаЖурнала.Колонки Цикл
		...
		Если Ложь
			...
			Или ирОбщий.СтрокиРавныЛкс(ИмяКолонкиДанных, "НеизвестныеСвойства")
...
Процедура СвойстваВТерминахМетаданныхПриИзменении(Элемент = Неопределено)
	...
	УстановитьВидимостьКолонок(ЭлементыФормы.БлокировкиСУБД);
...
Процедура ПриАктивизацииСтрокиИтогов(Элемент = Неопределено)
	...
	Если ИтогиВключены Тогда
		...
		Если ТекущаяСтрока <> Неопределено Тогда
			...
			ИначеЕсли ПанельИтоги.ТекущаяСтраница = ПанельИтоги.Страницы.БлокировкиСУБД Тогда
				СкрытьКолонкиПоСобытию = "DBMSLOCKS";
...
Процедура ОтборТекстSDBLНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ТипЗнч(Элемент.Значение) = Тип("Строка") Тогда
		...
		Если ИмяТаблицыХранения = Неопределено Тогда
			Возврат;
		КонецЕсли; 
		ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(Элемент, ИмяТаблицыХранения);
	КонецЕсли;
...
Процедура ТекстыБДВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если мСвойстваСИменамиБД.Свойство(ИмяСвойстваБезМета) Тогда
		ОткрытьТекстБДВКонверторе(ТекущаяСтрокаТаблицыЖурнала[Колонка.Данные], Не ирОбщий.СтрокиРавныЛкс(ИмяСвойстваБезМета, ИмяРеквизита),
			"" + ТекущаяСтрокаТаблицыЖурнала.МоментВремени + Колонка.Данные, Найти(Колонка.Данные, "SDBL") > 0);
	КонецЕсли; 
...
ПанельИтоги.Страницы.БлокировкиУпр.Значение = Новый Структура("Группировка, Рассчитаны", "Regions, RegionsМета", Ложь);
ПанельИтоги.Страницы.БлокировкиСУБД.Значение = Новый Структура("Группировка, Рассчитаны", "Regions, RegionsМета", Ложь);
...
ЭтаФорма.БлокировкиСУБДПолеСортировки1 = "Длительность";
...
мОписаниеТехножурнала = ирОбщий.ПолучитьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирНастройкаТехножурнала");
#Если _ Тогда
