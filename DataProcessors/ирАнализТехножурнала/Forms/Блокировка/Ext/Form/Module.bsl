...
Процедура ЗагрузитьОбластиБлокировки(ТаблицаОбластейБлокировки, ОписаниеБлокировки, ОписаниеБлокировкиМета)
	...
	Для ИндексЭлемента = 0 По ВхожденияЭлементов.Count - 1 Цикл
		...
		СтрокаОбластиБлокировки.Пространство = ВхождениеЭлемента.SubMatches(0);
		СтрокаОбластиБлокировки.ПространствоМета = ВхождениеЭлементаМета.SubMatches(0);
		СтрокаОбластиБлокировки.ТипБлокировки = ВхождениеЭлемента.SubMatches(1);
...
Процедура ТаблицаЖурналаПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ЗагрузитьОбластиБлокировки(ОбластиБлокировкиБлокиратора, Элемент.ТекущаяСтрока.Locks, Элемент.ТекущаяСтрока.LocksМета);
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирАнализТехножурнала.Форма.Блокировка");
