...
Процедура ПриОткрытии()
	...
	Для Каждого Вхождение Из РезультатПоиска Цикл
		...
		СтрокаДерева.Инструкция = Сред(ТекстИнструкции, ПозицияПалки + СтрДлина(МаркерИнструкции));
		...
		Для Индекс = 0 По 7 Цикл
			СтрокаДерева[КолонкиПланаЗапроса[Индекс].ИмяКолонки] = Вхождение.SubMatches(Индекс);
		КонецЦикла;
...
Процедура КоманднаяПанельТаблицаПланаОткрытьМенеджерТабличногоПоля(Кнопка)
	...
	ирОбщий.ПолучитьФормуЛкс("Обработка.ирМенеджерТабличногоПоля.Форма",, ЭтаФорма, ).УстановитьСвязь(ЭлементыФормы.ТаблицаПлана);
