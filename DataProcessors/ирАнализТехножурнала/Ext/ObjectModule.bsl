...
Процедура ЗаполнитьСвойстваСИменамиМетаданных(ВыбраннаяСтрока, Принудительно = Ложь) Экспорт
	...
	ЭтоГарантированоСобытиеВыбраннойЧужойБазы = Истина
		...
		И мПлатформа.ЧужаяСхемаБД <> Неопределено
		И ирОбщий.СтрокиРавныЛкс(Инфобаза, НСтр(мПлатформа.ЧужаяСхемаБД.СтрокаСоединения, "Ref"));
...
Функция ПреобразоватьЗначениеВSDBL(Ссылка) Экспорт
	...
	СтруктураБД = ирКэш.ПолучитьСтруктуруХраненияБДЛкс(,, мПлатформа.ЧужаяСхемаБД <> Неопределено);
		...
		Если ПереводитьВМета Тогда
			...
			Для Каждого Вхождение Из Вхождения Цикл
				...
				Если ОбработанныеОбращения[ЗначениеСУБД] = Неопределено Тогда
					ПолноеИмяМД = ирОбщий.ПолучитьМетаданныеПоНомеруСсылочнойТаблицыЛкс(ирОбщий.СтрокаHEXtoINTЛкс(Вхождение.SubMatches(0)), мПлатформа.ЧужаяСхемаБД <> Неопределено);
    ...
    Иначе
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Если ТаблицаПараметров.Найти(ЗначениеSDBL, "ЗначениеSDBL") = Неопределено Тогда
				ЗначениеПараметра = ирОбщий.ПреобразоватьЗначениеИзSDBLЛкс(ЗначениеSDBL, мПлатформа.ЧужаяСхемаБД <> Неопределено);
	...
	Если ВстраиватьПараметры Тогда
		...
		Пока ТекстМета <> "" Цикл
			...
			Если Истина
				...
				И ТаблицаПараметров.Количество() > ИндексПараметра
			Тогда
				СтрокаПоиска = СтрокаПоиска + ТаблицаПараметров[ИндексПараметра].Имя;
			КонецЕсли; 
...
Функция ПеревестиТекстБДВТерминыМетаданных(Знач Текст, СтруктураХраненияБазыДанных = Неопределено,
	...
	Если СтруктураХраненияБазыДанных = Неопределено Тогда
		СтруктураХраненияБазыДанных = ирКэш.ПолучитьСтруктуруХраненияБДЛкс(ЗначениеЗаполнено(ТипСУБД),, Не ЭтоГарантированоТекстСобственнойБазы И мПлатформа.ЧужаяСхемаБД <> Неопределено);
	КонецЕсли; 
	Если СловарьШаблоновМетаданных = Неопределено Тогда
		СловарьШаблоновМетаданных = ирКэш.ПолучитьСловарьШаблоновМетаданныхЛкс(ЗначениеЗаполнено(ТипСУБД), Не ЭтоГарантированоТекстСобственнойБазы И мПлатформа.ЧужаяСхемаБД <> Неопределено);
	КонецЕсли; 
