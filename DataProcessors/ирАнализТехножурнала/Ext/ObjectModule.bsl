	...
	Для Каждого ФайлЖурнала Из ФайлыЖурнала Цикл
		...
		Пока Истина Цикл
			...
			Для Каждого Вхождение Из Вхождения Цикл
				...
				ВхожденияСвойств = РегВыражение2.НайтиВхождения(Вхождение.SubMatches(5),, Истина);
...
Функция ПрочитатьСобственныйЖурналДвухСторон(УдалитьДополнительныеСобытия = Истина, ИменаНеполезныхКолонок = "", РежимТрассы = Истина, ЖдатьСекунд = 2, СмещениеБазовогоУровня = Неопределено)
	...
	Если РежимТрассы Тогда
		Если мИдентификаторТрассы = Неопределено Тогда
			Если Не ирКэш.ЛиФайловаяБазаЛкс() Тогда
				ирОбщий.ЛиТехножурналВключенЛкс(Истина, Истина, Истина, мЛиАнализТрассыЗапроса);
			КонецЕсли; 
			ирОбщий.ЛиТехножурналВключенЛкс(, Истина, Истина, мЛиАнализТрассыЗапроса);
			Возврат Ложь;
		КонецЕсли; 
	...
	Тогда
		...
		КонецЕсли; 
		РезультатСервера = ПрочитатьСобственныйЖурналОднойСтороны(НачалоПериодаСервера, КонецПериодаСервера, Истина, СдвигВремени, РежимТрассы, ЖдатьСекунд, СмещениеБазовогоУровня);
	...
	Если Ложь
		Или Не РежимТрассы
		Или мЛиАнализТрассыЗапроса <> Истина
	...
	Тогда
		...
		Если ЗагружатьЖурналКлиента Тогда
			...
			Иначе
				СдвигВремени = 0;
			КонецЕсли;
			РезультатКлиента = ПрочитатьСобственныйЖурналОднойСтороны(НачалоПериодаКлиента, КонецПериодаКлиента, Ложь, СдвигВремени, РежимТрассы, ЖдатьСекунд, СмещениеБазовогоУровня);
...
Функция ПрочитатьСобственныйЖурналОднойСтороны(НачалоПериода = Неопределено, КонецПериода = Неопределено, НаСервере = Истина, СдвигВремени = 0, 
	РежимТрассы = Истина, ЖдатьСекунд = 2, СмещениеБазовогоУровня = Неопределено)
	...
	КонецЕсли; 
	ТехножурналВключен = ирОбщий.ЛиТехножурналВключенЛкс(НаСервере,, Истина, РежимТрассы И мЛиАнализТрассыЗапроса = Истина);
	Если Истина
...
Функция ПолучитьКаталогТекущегоЖурнала(НаСервере) Экспорт
	...
	Результат = ирОбщий.КаталогЧтенияТехножурналаЛкс(НаСервере, Истина, мЛиАнализТрассыЗапроса);
...
Функция НачатьТрассу(ПрефиксТрассы = "", ВыводитьСообщения = Ложь, выхТекущаяДатаСервера = Неопределено) Экспорт
	...
	Если ВыводитьСообщения Тогда
		...
		Если Не ирКэш.ЛиФайловаяБазаЛкс() Тогда
			ТехножурналСервераВключен = ирОбщий.ЛиТехножурналВключенЛкс(Истина, ВыводитьСообщения,, , мЛиАнализТрассыЗапроса);
		КонецЕсли; 
		ТехножурналКлиентаВключен = ирОбщий.ЛиТехножурналВключенЛкс(, ВыводитьСообщения,, мЛиАнализТрассыЗапроса);
		Если Истина
...
Процедура ЗагрузитьТрассуСУБД()
	...
	НачальныйИндексСтроки = ТаблицаЖурнала.Количество();
	Пока УзелСобытие <> Неопределено Цикл
		...
		Если ИмяСобытия = "rpc_completed" ИЛИ ИмяСобытия = "sql_batch_completed" Тогда
			...
			ЗаписьДанныеЗапроса = ТаблицаЖурнала.Добавить();
			ЗаписьДанныеЗапроса.МоментВремени = МоментВремени;
			ЗаписьДанныеЗапроса.Дата = ДатаСобытия;
			ЗаписьДанныеЗапроса.ДатаНачала = ДатаНачала;
			ЗаписьДанныеЗапроса.Длительность = ДлительностьМикросекунд / 1000;
			ЗаписьДанныеЗапроса.НаСервере = Истина;
			ЗаписьДанныеЗапроса.ТипПроцессаОС = "sqlservr";
			ЗаписьДанныеЗапроса.МоментВремениНачала = МоментВремениНачала;
			ЗаписьДанныеЗапроса.CPUTime = СвойстваСобытия["cpu_time"] / 1000;
			ЗаписьДанныеЗапроса.ЛогическиПрочитано = СвойстваСобытия["logical_reads"];
			ЗаписьДанныеЗапроса.ФизическиПрочитано = СвойстваСобытия["physical_reads"];
			ЗаписьДанныеЗапроса.ЛогическиЗаписано = СвойстваСобытия["writes"];
			//ЗаписьДанныеЗапроса.ИмяОбъекта = СвойстваСобытия["object_name"];
			//ЗаписьДанныеЗапроса.Приложение = СвойстваСобытия["client_app_name"];
			//ЗаписьДанныеЗапроса.ИдентификаторЯдраПроцессора = СвойстваСобытия["cpu_id"];
			//ЗаписьДанныеЗапроса.Соединение_ = СвойстваСобытия["client_pid"];
			//ЗаписьДанныеЗапроса.Системный = ПреобразоватьКБулево(СвойстваСобытия["is_system"]);
			ЗаписьДанныеЗапроса.ЧислоСтрок = СвойстваСобытия["row_count"];
			ЗаписьДанныеЗапроса.ТекстСУБД = ТекстЗапроса;
			Если МассивОператоров.Количество() = 1 Тогда
				...
				КонецЕсли;
				ЗаписьДанныеЗапроса.ПланСУБД = "0x" + ВРег(ИдентификаторПланаЗапроса);
				МассивИДПланов.Добавить(ЗаписьДанныеЗапроса.ПланСУБД);
			КонецЕсли;
			ЗаписьДанныеЗапроса.Событие = ИмяСобытия;
			ЗаписьДанныеЗапроса.Дата = ДатаСобытия;
	...
	Если МассивИДПланов.Количество() > 0 Тогда
		...
		Для Индекс = НачальныйИндексСтроки По ТаблицаЖурнала.Количество() - 1 Цикл
			СтрокаРезультата = ТаблицаПлановЗапросов.Найти(ТаблицаЖурнала[Индекс].ПланСУБД, "plan_handle");
			Если СтрокаРезультата <> Неопределено Тогда
				ТаблицаЖурнала[Индекс].ПланСУБД = СтрокаРезультата.query_plan;
			Иначе
				ТаблицаЖурнала[Индекс].ПланСУБД = МаркерПланЗапросаНеНайден();
			КонецЕсли; 
		КонецЦикла;
	КонецЕсли;
	ЗапросУдалениеСессии = ПолучитьМакет("SQLServer_СкриптУдаленияСессииСбораДанных").ПолучитьТекст();
...
Функция ПоказатьТрассу(УдалитьДополнительныеСобытия = Истина, ИменаНеполезныхКолонок = "", ЖдатьСекунд = 2, СмещениеБазовогоУровня = Неопределено) Экспорт
	...
	ЖурналПрочитан = ПрочитатьСобственныйЖурналДвухСторон(УдалитьДополнительныеСобытия, ИменаНеполезныхКолонок, Истина, ЖдатьСекунд, СмещениеБазовогоУровня);
	ВыраниватьДатуПоСерверу = ирОбщий.ВосстановитьЗначениеЛкс("ирАнализТехножурнала.ВыраниватьДатуПоСерверу") = Истина;
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		Если ТипСУБД = "DBMSSQL" Тогда
			...
			ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ЛитералЗначения, "'") Тогда
				...
				КонецПопытки;
			// ТипСобытия = DBMSSQL
			ИначеЕсли Прав(ЛитералЗначения, 1) = "N" Тогда
