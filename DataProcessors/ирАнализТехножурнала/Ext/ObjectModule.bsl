	...
	Для Каждого ФайлЖурнала Из ФайлыЖурнала Цикл
		...
		Если АбсолютнаяПозицияВФайле > 0 Тогда
			Если ЗначениеЗаполнено(КартаФайла.ДатаИзмененияФайла) Тогда
				Если КартаФайла.ДатаИзмененияФайла = ФайлЖурнала.ПолучитьВремяИзменения() Тогда
					Продолжить;
				КонецЕсли; 
			КонецЕсли; 
		...
		Пока Истина Цикл
			КартаФайла.ДатаИзмененияФайла = ФайлЖурнала.ПолучитьВремяИзменения();
...
Процедура ЗаполнитьСвойстваСИменамиМетаданных(ВыбраннаяСтрока) Экспорт
	...
	Для Каждого КлючИЗначение Из мСвойстваСИменамиБД Цикл
		...
		Тогда
			...
			КонецЕсли; 
			//Попытка
				ТекстМета = ПеревестиТекстБДВТерминыМетаданных(ЗначениеСвойства, , , ТипСУБД, НайденныеТаблицы);
			//Исключение
			//	ТекстМета = "";
			//КонецПопытки; 
	...
	Если Не ЗначениеЗаполнено(ТекстSDBLШаблонМета) Тогда
		...
		Тогда
			...
			Иначе
				ВыбраннаяСтрока.ТекстSDBLШаблонМета = ПеревестиТекстБДВТерминыМетаданных(ВыбраннаяСтрока.ТекстSDBLШаблон);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Попытка
	...
	Если Не ЗначениеЗаполнено(ТекстСУБДШаблонМета) Тогда
		...
		Тогда
			...
			Иначе
				ВыбраннаяСтрока.ТекстСУБДШаблонМета = ПеревестиТекстБДВТерминыМетаданных(ВыбраннаяСтрока.ТекстСУБДШаблон,,, мТипСУБД);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОткрытьТекстБДВКонверторе(ТекстБД, ОткрытьСтраницуЗапроса = Истина, КлючУникальности = Неопределено, ЭтоТекстSDBL = Истина) Экспорт
...
Функция ПолучитьСтруктуруСвойствСобытия(Событие, ВключатьКолонкуСобытия = Истина) Экспорт 
	...
	СтрокаСобытия = мСоставСвойствСобытий.Найти(Событие, "Event");
...
Функция ПолучитьСтруктуруЗапросаИзТекстаБД(ТекстБД, ТипСУБД = "", ПересобратьТекст = Ложь) Экспорт
	...
	ТекстМета = ПеревестиТекстБДВТерминыМетаданных(Текст,,, ТипСУБД, ТаблицаТаблиц, 999999);
...
Функция ПеревестиТекстБДВТерминыМетаданных(Знач Текст, СтруктураХраненияБазыДанных = Неопределено,
    СловарьШаблоновМетаданных = Неопределено, Знач ТипСУБД = "", выхТаблицы = Неопределено, ДопустимыйРазмер = 20000) Экспорт
...
Для Каждого МетаРеквизит Из МетаРеквизиты Цикл
	Если мСоответствиеКолонок[МетаРеквизит.Имя] = Неопределено Тогда
		мСоответствиеКолонок.Вставить(МетаРеквизит.Имя, МетаРеквизит.Имя);
	КонецЕсли; 
КонецЦикла; 
