...
Процедура ЗаполнитьСписокИнструментовСтандартныеНастройки() Экспорт
	...
	ТаблицаСписка = ирКэш.Получить().ПолучитьСписокИнструментов();
	#Если _ Тогда
...
Процедура ОткрытьИнструмент(СтрокаИнструмента) Экспорт
	...
	Иначе
		...
		Если Найти(ИмяВыбраннойФормы, "ОбщаяФорма.") <> 1 И Найти(ИмяВыбраннойФормы, ".Форма.") = 0 Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
		Форма = ирОбщий.ПолучитьФормуЛкс(ИмяВыбраннойФормы);
...
Процедура ПриОткрытии()
	...
	Иначе
		ЭтаФорма.Заголовок = Метаданные.Подсистемы.ИнструментыРазработчика.Синоним;
	КонецЕсли; 
