...
Процедура ПриОткрытии()
	...
	ПолеВстроенногоЯзыка = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
	#Если Сервер И Не Сервер Тогда
	    ПолеВстроенногоЯзыка = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	...
	ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML.Доступность = ирКэш.НомерВерсииПлатформыЛкс() >= 803018;
...
Функция ПолучитьТелоМетода(ТекстДляВыполнения = Неопределено, СтруктураПараметров =  Неопределено, ЛиСинтаксическийКонтроль = Ложь, РежимВнешнейОбработки = Ложь)
	...
	Если ТекстДляВыполнения = Неопределено Тогда
		ТекстДляВыполнения = ПолеВстроенногоЯзыка.ПолеТекста.ПолучитьТекст();
	КонецЕсли;
	Если Не ирОбщий.ПроверитьПлатформаНеWindowsЛкс(, "Замена возвратов") Тогда 
...
Функция ВыполнитьЛокально(Знач ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПолеВстроенногоЯзыка, "Выполнить");
...
Процедура ОбработатьИсключениеАлгоритма(Знач ВремяНачала, ИнформацияОбОшибке, Знач ЛиСинтаксическийКонтроль, Знач СтруктураПараметров = Неопределено)
	...
	Если Не ЛиСинтаксическийКонтроль Тогда 
		...
		КонецЕсли;
		ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеВстроенногоЯзыка.ПолеТекста, -СтартоваяСтрока,,, МодальныйРежим,
...
Процедура ОсновныеДействияФормыПрименить(Кнопка)
	...
	Если ТипЗнч(мСписокВнешнихПараметров) = Тип("ТаблицаЗначений") Тогда
		...
		РезультатФормы.Вставить("Текст", АктивноеПолеТекста().ПолучитьТекст());
...
Функция ПроверитьПрограммныйКод()
	...
	Результат = ПолеВстроенногоЯзыка.ПроверитьПрограммныйКод();
	Возврат Результат
...
Процедура КоманднаяПанельВстроенныйЯзыкСсылкаНаОбъектБД(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
	    ПолеВстроенногоЯзыка = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	СтрокаПараметра = ПолеВстроенногоЯзыка.ВставитьСсылкуНаОбъектБД(ЭлементыФормы.Параметры,,,, Истина,,, ЭтаФорма.ТекущийЭлемент = АктивноеПолеТекста().ЭлементФормы);
	Если СтрокаПараметра = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	//СтрокаПараметра.НИмя = НРег(СтрокаПараметра.Имя);
...
Процедура ОсновныеДействияФормыСохранить(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
	    АлгоритмОбъект = Справочники.ирАлгоритмы.СоздатьЭлемент();
	#КонецЕсли
	ФормаАлгоритма.ТекстАлгоритма = ПолеВстроенногоЯзыка.ПолеТекста.ПолучитьТекст();
...
Процедура ОсновныеДействияФормыЗагрузить(Кнопка)
	...
	#КонецЕсли
	ПолеВстроенногоЯзыка.ПолеТекста.УстановитьТекст(ТекущийАлгоритм.ТекстАлгоритма);
...
Процедура КоманднаяПанельВстроенныйЯзыкВозврат(Кнопка)
	...
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(), "Перейти ~Конец;");
...
Процедура КоманднаяПанельВстроенныйЯзыкЗамераЦикла(Кнопка)
	...
	#КонецЕсли
		ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(),
	"КоличествоПроходов = 10;
	|ирОбщий.НачатьЗамерЛкс(КоличествоПроходов, """ + НовыйКлючЗамераДляВставкиВАлгоритм() + """);
	|Для Счетчик = 1 По КоличествоПроходов Цикл" + ирОбщий.ДобавитьМногострочнуюСтрокуВТекстЛкс("", Символы.ПС + АктивноеПолеТекста().ВыделенныйТекст(), Символы.Таб) + "
	|КонецЦикла;
...
Процедура КоманднаяПанельВстроенныйЯзыкНачалоЗамера(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.НачатьЗамерЛкс();
	#КонецЕсли
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(), "ирОбщий.НачатьЗамерЛкс(, """ + НовыйКлючЗамераДляВставкиВАлгоритм() + """);");
...
Процедура КоманднаяПанельВстроенныйЯзыкКонецЗамера(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.КончитьЗамерЛкс();
	#КонецЕсли
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(), "ирОбщий.КончитьЗамерЛкс();");
...
Процедура КоманднаяПанельВстроенныйЯзыкАдаптироватьТекстИзМетода(Кнопка)
	...
	Если ирОбщий.ПроверитьПлатформаНеWindowsЛкс() Тогда 
		Возврат;
	КонецЕсли; 
	ТекстАлгоритма = АктивноеПолеТекста().ПолучитьТекст();
...
Процедура УстановитьТекстАлгоритмаСВозможностьюОтмены(Знач ТекстАлгоритма)
	...
	АктивноеПолеТекста = АктивноеПолеТекста();
	#Если Сервер И Не Сервер Тогда
...
Процедура КоманднаяПанельВстроенныйЯзыкЗаписьНаСервере(Кнопка)
	...
	#КонецЕсли
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(), "ирОбщий.ЗаписатьОбъектЛкс(Объект, Истина);");
...
Процедура НайтиПараметрВТексте(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
	    ПолеВстроенногоЯзыка = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ПолеВстроенногоЯзыка.НайтиПоказатьСловоВТексте(ТекущаяСтрока.Имя,, АктивноеПолеТекста());
...
Процедура СохранитьДанныеТекущейСтроки()
	...
	Тогда
		...
		АктивноеПолеТекста().ПолучитьГраницыВыделения(мТекущаяСтрокаДереваАлгоритмов.НачальнаяСтрока, мТекущаяСтрокаДереваАлгоритмов.НачальнаяКолонка,
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	АктивноеПолеТекста().УстановитьТекст("");
	Параметры.Очистить();
	Если мАлгоритмЗагруженВКонсоль Тогда
		АктивноеПолеТекста().УстановитьТекст(мТекущаяСтрокаДереваАлгоритмов.ТекстАлгоритма);
		ВосстановитьТекущуюСтрокуВТекстеЗапроса = Истина;
		Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, АктивноеПолеТекста(), СтрокаПоискаВДереве, Ложь);
		КонецЕсли; 
		Если ВосстановитьТекущуюСтрокуВТекстеЗапроса И мТекущаяСтрокаДереваАлгоритмов.НачальнаяСтрока > 0 Тогда
			АктивноеПолеТекста().УстановитьГраницыВыделения(мТекущаяСтрокаДереваАлгоритмов.НачальнаяСтрока, мТекущаяСтрокаДереваАлгоритмов.НачальнаяКолонка,
				мТекущаяСтрокаДереваАлгоритмов.КонечнаяСтрока, мТекущаяСтрокаДереваАлгоритмов.КонечнаяКолонка);
		КонецЕсли; 
...
Процедура КоманднаяПанельВстроенныйЯзыкЗначениеИзБуфера(Кнопка)
	...
	Если ЭтаФорма.ТекущийЭлемент <> АктивноеПолеТекста() Тогда
		Возврат;
	КонецЕсли; 
	...
	#Если Сервер И Не Сервер Тогда
	    ПолеВстроенногоЯзыка = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	СтрокаПараметра = ПолеВстроенногоЯзыка.ВставитьСсылкуНаОбъектБД(ЭлементыФормы.Параметры,,,, Истина, ЗначениеИзБуфера, Ложь, ЭтаФорма.ТекущийЭлемент = АктивноеПолеТекста().ЭлементФормы);
	Если СтрокаПараметра = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	//СтрокаПараметра.НИмя = НРег(СтрокаПараметра.Имя);
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, АктивизироватьПервуюСтроку = Неопределено)
	...
	ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, АктивноеПолеТекста(), СтрокаПоискаВДереве, Ложь);
...
Процедура КоманднаяПанельВстроенныйЯзыкВычислитьВыражение(Кнопка)
	...
	ТекущееВыражение = АктивноеПолеТекста().ВыделенныйТекст();
...
Процедура КоманднаяПанельВстроенныйЯзыкВставитьУказаниеТипаПеременной(Кнопка)
	...
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(), "//: <ИмяПеременной> = Новый (""ТаблицаЗначений"");");
...
Процедура КоманднаяПанельВстроенныйЯзыкВОднострочныйВид(Кнопка)
	...
	Если ВесьТекст Тогда
		АктивноеПолеТекста.УстановитьГраницыВыделения(1, СтрДлина(ЭлементыФормы.ВстроенныйЯзык.ПолучитьТекст()) + 1);
...
Процедура КоманднаяПанельВстроенныйЯзыкЦиклСИндикатором(Кнопка)
	...
	|");
	АктивноеПолеТекста().ВыделенныйТекст(Символы.ПС);
...
Функция СравнитьЗагрузитьИзмененияИзВнешнейОбработки(ПоказыватьСравнение = Истина)
	...
	Если мТекущаяСтрокаДереваАлгоритмов.Наименование = ИнфоФайла.ИдентификаторАлгоритма Тогда
		...
		ТекстИзПоля = АктивноеПолеТекста().ПолучитьТекст();
		Если ТекстИзОбработки <> ТекстИзПоля Тогда
			...
			Если ЗагрузитьТекст Тогда
				...
				АктивноеПолеТекста().ПолучитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка);
				АктивноеПолеТекста().УстановитьТекст(ТекстИзОбработки);
				АктивноеПолеТекста().УстановитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка);
				ЭтаФорма.Модифицированность = Истина;
...
Процедура КоманднаяПанельВстроенныйЯзыкСообщитьСразу(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.СообщитьЛкс();
	#КонецЕсли
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(), "ирОбщий.СообщитьЛкс();");
...
Процедура КоманднаяПанельВстроенныйЯзыкВыделитьМетод(Кнопка)
	...
	Если Не ЗначениеЗаполнено(ПолеВстроенногоЯзыка.ВыделенныйТекст()) Тогда
	...
	НевидимоеПолеТекста.ПолеТекста.УстановитьТекст(ПолеВстроенногоЯзыка.ПолеТекста.ПолучитьТекст());
	ПолеВстроенногоЯзыка.ПолеТекста.ПолучитьГраницыВыделения(НачСтрока, НачКолонка, КонСтрока, КонКолонка);
	...
	ПолеВстроенногоЯзыка.ВыделенныйТекст(ТекстВставки);
		...
		Если Не ЗначениеЗаполнено(НеопределеннаяПеременная) Тогда
			...
			Если Не выхКорректныйФрагмент Тогда
				...
				Если ПоказыватьСтрокуОшибки Тогда
					ЭтаФорма.ТекущийЭлемент = ПолеВстроенногоЯзыка.ПолеТекста;
					ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеВстроенногоЯзыка.ПолеТекста, СтартоваяСтрока,,,, ИнформацияОбОшибке);
				Иначе
...
Процедура КоманднаяПанельВстроенныйЯзыкЕсли(Кнопка)
	...
	ирОбщий.ПолеТекстовогоДокументаУстановитьВставитьТекстИПереносСтрокиЛкс(АктивноеПолеТекста(),
	"Если  Тогда
	|	" + ирОбщий.ДобавитьМногострочнуюСтрокуВТекстЛкс("", АктивноеПолеТекста().ВыделенныйТекст(), Символы.Таб) + "
	|КонецЕсли;");
...
Функция АктивноеПолеТекста(ЭлементФормы = Неопределено)
	Если ЭлементФормы = Неопределено Тогда
		...
		Иначе
			Результат = ЭлементыФормы.ВстроенныйЯзык;
		КонецЕсли;
	Иначе
...
Процедура РедакторHTMLДокументСформирован(Элемент = Неопределено)
	...
	//РедакторHTML.editor.updateOptions("{fontSize: '10px', fontFamily: 'Tahoma'}");
	//РедакторHTML.addContextMenuItem("Конструктор форматной строки...", "EVENT_FORMAT_CONSTRUCT");
...
Процедура ПанельРедактораПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Поле1 = АктивноеПолеТекста(ЭлементыФормы.ВстроенныйЯзык);
	...
	Если ЭлементыФормы.ПанельРедактора.ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML Тогда
		...
		Тогда
			ПолеВстроенногоЯзыка.ПолеТекста = АктивноеПолеТекста(Поле2);
	...
	Иначе
		Если ЭлементыФормы.РедакторHTML.Документ.defaultView <> Неопределено Тогда
			ПолеВстроенногоЯзыка.ПолеТекста = АктивноеПолеТекста(Поле1);
...
Процедура РедакторHTMLonclick(Элемент, ДанныеСобытия)
	...
	Если Событие <> Неопределено Тогда
			...
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				...
				Тогда 
					#Если Сервер И Не Сервер Тогда
						ПолеВстроенногоЯзыка = Обработки.ирКлсПолеТекстаПрограммы.Создать();
					#КонецЕсли
					ПолеВстроенногоЯзыка.КончитьОбработкуКоманды();
					ПолеВстроенногоЯзыка.РазобратьТекущийКонтекст();
					Если Событие.params.trigger = """" Тогда
						ПоказатьСписок = ПолеВстроенногоЯзыка.мЭтоСтроковыйЛитерал;
					Иначе
						ПоказатьСписок = Истина;
					КонецЕсли; 
					Если ПоказатьСписок Тогда
						ПолеВстроенногоЯзыка.ЗаполнитьТаблицСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста);
						Если Ложь
							Или Событие.params.trigger = Неопределено
							Или ПолеВстроенногоЯзыка.ТаблицаСлов.Количество() > 0
						Тогда
		...
		//	ФорматнаяСтрока = СтрЗаменить(СтрЗаменить(Событие.params.text, "|", ""), """", "");
		//	ОткрытьКонструкторФорматнойСтроки(ФорматнаяСтрока, Событие.params.range);
		...
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			// Пока это событие не вызывается https://github.com/salexdv/bsl_console/issues/79
			СтрокаТаблицыСлов = ПолеВстроенногоЯзыка.ТаблицаСлов.Найти(ирОбщий.СтрокаМеждуМаркерамиЛкс(Событие.params.focused, "Элемент ", ",") , "Слово");
			ПолеВстроенногоЯзыка.ВставитьВыбранноеСловоАвтодополнения(СтрокаТаблицыСлов, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста);
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			// https://github.com/salexdv/bsl_console/issues/78
			//Если мРазрешенаПодсказкаHTML <> Истина Тогда 
			//	СтрокаТаблицыСлов = ПолеВстроенногоЯзыка.ТаблицаСлов.Найти(ирОбщий.СтрокаМеждуМаркерамиЛкс(Событие.params.focused, "Элемент ", ",", Истина) , "Слово");
			//	Если СтрокаТаблицыСлов <> Неопределено Тогда
			//		Если ПолеВстроенногоЯзыка.УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, СтрокаТаблицыСлов) Тогда 
			//			//РедакторHTML.hideSuggestionsList();
			//			//ПередатьТаблицуСловВРедакторHTML();
...
Процедура ПередатьТаблицуСловВРедакторHTML()
	...
	Для Каждого СтрокаСлова Из ПолеВстроенногоЯзыка.ТаблицаСлов Цикл
...
Процедура ОбновитьПодсказкиПараметров()
	РедакторHTML = АктивноеПолеТекста(ЭлементыФормы.РедакторHTML).РедакторHTML();
	Если РедакторHTML = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	// https://github.com/salexdv/bsl_console/issues/77
	СтруктураПодсказки = Новый Структура;
	Для Каждого СтрокаПараметра Из Параметры Цикл
		//ПодсказкаСлова = Новый Структура();
		//ПодсказкаСлова.Вставить("name", СтрокаПараметра.Имя);
		//ПодсказкаСлова.Вставить("description", СтрокаПараметра.ИмяТипаЗначения);
		//ПодсказкаСлова.Вставить("returns", ЭлементыФормы.Параметры.ОформлениеСтроки(СтрокаПараметра).Ячейки.ПредставлениеЗначения.Текст);
		СтруктураПодсказки.Вставить(СтрокаПараметра.Имя, СтрокаПараметра.ИмяТипаЗначения + ": " + ЭлементыФормы.Параметры.ОформлениеСтроки(СтрокаПараметра).Ячейки.ПредставлениеЗначения.Текст);
	КонецЦикла;
...
Процедура ОткрытьКонструкторФорматнойСтроки(ФорматнаяСтрока)
	...
	Если Конструктор.ОткрытьМодально() Тогда
		...
		Результат = """" + Результат + """";
		ПолеВстроенногоЯзыка.ВыделенныйТекст(Результат);
	КонецЕсли; 
...
Процедура КПРедакторHTMLКонструкторФорматнойСтроки(Кнопка)
	...
	ФорматнаяСтрока = СтрЗаменить(СтрЗаменить(ПолеВстроенногоЯзыка.ВыделитьТекстТекущегоСтроковогоЛитерала(), "|", ""), """", "");
