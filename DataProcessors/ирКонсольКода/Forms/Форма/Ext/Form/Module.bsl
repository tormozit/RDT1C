...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		ТекстАлгоритма = мТекущаяСтрокаДереваАлгоритмов.ТекстАлгоритма;
		ПолеТекстаАлгоритма.ПолеТекста.УстановитьТекст(ТекстАлгоритма,, ТекстАлгоритма);
...
Процедура РедакторHTMLonclick(Элемент, ДанныеСобытия)
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_SIGNATURE" Тогда
			ПолеТекстаАлгоритма.РедакторHTML_ПередПоказомСигнатуры(Событие.params.activeParameter);
		ИначеЕсли Событие.event = "EVENT_BEFORE_SHOW_SUGGEST" Тогда
			#Если Сервер И Не Сервер Тогда
				ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
			#КонецЕсли
			Если ПолеТекстаАлгоритма.мРазрешенаПодсказкаHTML <> Истина Тогда
				ОбновитьКонтекстПоляВстроенногоЯзыка();
		...
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			ПолеТекстаАлгоритма.РедакторHTML_ПриВыбореСтрокиАвтодополнения(Событие.params.trigger, Событие.params.selected);
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			ПолеТекстаАлгоритма.РедакторHTML_ПриАктивацииСтрокиАвтодополнения(Событие.params.focused);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры

Процедура РедакторHTMLonfocus(Элемент, pEvtObj)
	Пустышка = 1;
КонецПроцедуры
