...
Перем мИдентификаторФоновогоЗадания;
...
Процедура ПриОткрытии()
	...
	ВыбранСпособВыполнения(ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.СпособВыполнения.Кнопки.НаКлиенте);
...
Функция ВыполнитьЛокально(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	ЧерезФоновоеЗадание = СпособВыполнения = "ВФоновомЗадании";
	Если Не СпособВыполнения = "НаКлиенте" Тогда
	...
	Если Не ЛиСинтаксическийКонтроль И ЧерезФоновоеЗадание Тогда
		ФормаРезультатФоновогоЗадания = ирОбщий.ПолучитьФормуЛкс("Обработка.ирКонсольКода.Форма.РезультатФоновогоЗадания");
		...
		мИдентификаторФоновогоЗадания = ФоновоеЗадание.УникальныйИдентификатор;
		ПодключитьОбработчикОжидания("ОтменитьФоновоеЗадание", 0.1, Истина);
		Состояние("Выполняю фоновое задание... CTRL+Break для прерывания");
		Пока Истина Цикл
			ОбработкаПрерыванияПользователя();
			мПлатформа.Sleep(1);
			ФоновоеЗадание = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(мИдентификаторФоновогоЗадания);
			ОбработатьСообщенияФоновогоЗадания(ФоновоеЗадание);
	...
	Иначе
		Попытка
			ирОбщий.ВыполнитьАлгоритмВКонтекстеЛкс(ТекстДляВыполнения, СтруктураПараметров, СпособВыполнения = "НаСервере", РежимВнешнейОбработки, ИмяФайлаВнешнейОбработки, ЛиСинтаксическийКонтроль, ВремяНачала);
		Исключение
...
Процедура ОбработатьСообщенияФоновогоЗадания(Знач ФоновоеЗадание)
	...
	Если СообщенияПользователю <> Неопределено Тогда
		...
		Для Каждого СообщениеПользователю Из СообщенияПользователю Цикл
			...
			Если Найти(СообщениеПользователю.Текст, "#Индикатор-") = 1 Тогда
				СтруктураИндикатора = ирОбщий.ВосстановитьОбъектИзСтрокиXMLЛкс(ирОбщий.ПолучитьСтрокуМеждуМаркерамиЛкс(СообщениеПользователю.Текст, "#Индикатор-"));
			КонецЕсли; 
...
Процедура ЗаполнитьПараметры(ПараметрыВхода = Истина, ПараметрыВыхода = Истина)
	...
	Если ПараметрыВхода Тогда
		Пока Истина Цикл
			...
			Если Не ЗначениеЗаполнено(НеопределеннаяПеременная) Тогда
				...
				Прервать;
			КонецЕсли;
			Если СпособВыполнения <> "НаКлиенте" И МетаОбщиеМодули.Найти(НеопределеннаяПеременная) <> Неопределено Тогда
				Продолжить;
			КонецЕсли; 
	...
	Если ПараметрыВыхода Тогда
		...
		Для Каждого СтрокаПеременной Из СтрокиЛокальныхПеременных Цикл
			Если СпособВыполнения <> "НаКлиенте" И МетаОбщиеМодули.Найти(СтрокаПеременной.НСлово) <> Неопределено Тогда
				Продолжить;
			КонецЕсли; 
...
Функция ПолучитьОбновитьФайлВнешнейОбработки(СтруктураПараметров = Неопределено) Экспорт
	...
	ирОбщий.ОбновитьМодульВнешнейОбработкиДляОтладкиЛкс(ИмяФайлаВнешнейОбработки, ИмяВнешнейОбработки, ТекстМодуля, ТекстМодуляТекущейВнешнейОбработки, ДатаИзмененияВнешнейОбработки, СпособВыполнения <> "НаКлиенте");
...
Процедура КоманднаяПанельВстроенныйЯзыкОткрытьВОтладчике(Кнопка)
	...
	//	Сообщить("Изменения файла будут игнорироваться, т.к. в настройках алгоритмов не включено разрешение редактирования файлового кэша", СтатусСообщения.Информация);
	//КонецЕсли;
	Сообщить("Изменения файла будут игнорироваться", СтатусСообщения.Информация);
	ИмяФайлаВнешнейОбработки = ПолучитьОбновитьФайлВнешнейОбработки();
...
Процедура ОбновитьДоступностьКнопкиВыполнятьНаСервере()
	...
	ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.СпособВыполнения.Кнопки.НаСервере.Доступность = Истина
	...
	ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.СпособВыполнения.Кнопки.ВФоновомЗадании.Доступность = Истина
...
Процедура ВыбранСпособВыполнения(Кнопка)
	...
	Для Каждого КнопкаСпособа Из ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.СпособВыполнения.Кнопки Цикл
		КнопкаСпособа.Пометка = Кнопка = КнопкаСпособа;
	КонецЦикла;
	ЭтотОбъект.СпособВыполнения = Кнопка.Имя;
	ДоступностьВнешнейОбработкиНаСервре = СпособВыполнения = "НаКлиенте" Или Не ирКэш.ЛиПортативныйРежимЛкс();
