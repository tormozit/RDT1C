...
Функция ВыполнитьЛокально(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Попытка
		...
		Если Истина
			...
			И РежимВнешнейОбработки 
		Тогда
			КонтекстВыполнения.ВыполнитьАлгоритмЧерезВнешнююОбработкуЛкс(ИмяФайлаВнешнейОбработки, СтруктураПараметров, ВремяНачала, ПеременнаяСтрока);
		Иначе
