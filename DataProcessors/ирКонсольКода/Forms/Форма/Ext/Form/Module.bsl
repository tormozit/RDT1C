...
Функция ВыполнитьЛокально(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Если Не ЛиСинтаксическийКонтроль И ЧерезФоновоеЗадание Тогда
		...
		АдресПараметровВыхода = ПоместитьВоВременноеХранилище(Null, ФормаРезультатФоновогоЗадания.УникальныйИдентификатор);
...
Процедура КоманднаяПанельВстроенныйЯзыкВыделитьМетод(Кнопка)
	...
	КонецЕсли; 
	НевидимоеПолеТекста = ирОбщий.ПолучитьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой");
	#Если Сервер И Не Сервер Тогда
	...
	НевидимоеПолеТекста.ПолеТекстовогоДокумента.УстановитьГраницыВыделения(КонСтрока, КонКолонка, НевидимоеПолеТекста.ПолеТекстовогоДокумента.КоличествоСтрок(), 300);
	...
	ПараметрыВыходаАлгоритма = ПараметрыИзАлгоритма(ТекстДоПереименованный + ТекстПосле, КонтекстВыполнения = "Клиент", НевидимоеПолеТекста,, ИмяПеременнойЗаглушки);
	#Если Сервер И Не Сервер Тогда
	...
	Если ЗначениеЗаполнено(ТекстПосле) Тогда
	...
	Для Каждого СтрокаПараметра Из ФормаНовогоМетода.ТаблицаПараметров Цикл
		СтрокаПараметра.Обязательный = Истина;
		ЗаполнитьСтрокуВынесенногоПараметра(СтрокаПараметра);
	КонецЦикла;
	//ЗаменаЗаглушки = Символы.ПС + ТекстСмещения + "Возврат";
	ЗаменаЗаглушки = "Возврат";
	ТекстНовогоМетода = СтрЗаменить(ТекстНовогоМетода, ЗаглушкаВозврата + "=", ЗаменаЗаглушки);
	ЗаменаЗаглушки = ЗаменаЗаглушки + " Неопределено";
	ТекстНовогоМетода = СтрЗаменить(ТекстНовогоМетода, ЗаглушкаВозврата, ЗаменаЗаглушки);
...
Процедура ЗаполнитьСтрокуВынесенногоПараметра(Знач СтрокаПараметра)
	...
	Если СтрокаЭтогоАлгоритмаПараметры <> Неопределено Тогда
		...
		СтрокаПараметра.ТипЗначения = СтрокаЭтогоАлгоритмаПараметры.ТипЗначения;
