...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ЭлементыФормы.ВстроенныйЯзык, СтрокаПоискаВДереве, Ложь);
		КонецЕсли; 
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, АктивизироватьПервуюСтроку = Неопределено)
	...
	ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ЭлементыФормы.ВстроенныйЯзык, СтрокаПоискаВДереве, Ложь);
