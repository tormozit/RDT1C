...
Процедура ОткрытьФайлПоПолномуИмени(ПолноеИмяФайла, ОчиститьПередЗагрузкой = Истина)
	...
	Файл = Новый Файл(мИмяФайла);
	ЗагрузитьИзФайла(ОчиститьПередЗагрузкой);
	СохранитьИмяФайла();
	
КонецПроцедуры

Процедура ЗагрузитьИзФайла(ОчиститьПередЗагрузкой = Истина)
	...
	ИначеЕсли ТипЗнч(ПолученноеЗначение) = Тип("ДеревоЗначений") Тогда
		...
		Если ОчиститьПередЗагрузкой Тогда
			...
			УстановитьЗаголовокФормы();
		КонецЕсли;
		ирОбщий.СкопироватьДеревоЛкс(ПолученноеЗначение, ДеревоАлгоритмов, ОчиститьПередЗагрузкой);
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		...
		Если мТекущаяСтрокаДереваАлгоритмов.НачальнаяСтрока > 0 Тогда
