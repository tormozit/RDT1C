...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если ЭтоВыполнениеКода Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	РезультатНажатия = ПолеТекстаАлгоритма.Нажатие(Кнопка);
...
Функция ОбновитьКонтекстПоляВстроенногоЯзыка(Знач Кнопка = Неопределено)
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ирОбщий.ИнициализироватьГлобальныйКонтекстПодсказкиЛкс(ПолеТекстаАлгоритма);
	//Попытка
	//	Выполнить("Ядро2iS.ДобавитьГлобальныйКонтекстВКонтекстнуюПодсказку2iS(ПолеТекстаАлгоритма)");
	//Исключение
	//КонецПопытки;
...
Функция ВыполнитьЛокально(Знач ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Попытка
		ирОбщий.ВыполнитьЗаданиеФормыЛкс("ВыполнитьАлгоритмВКонтексте", ПараметрыЗадания, ЭтаФорма, "ВыполнитьАлгоритм",, Кнопка, "ВыполнитьАлгоритмВФонеЗавершение",
			Не ЛиСинтаксическийКонтроль И ЧерезФоновоеЗадание, ЭлементыФормы);
	Исключение
...
Процедура ОбработатьИсключениеАлгоритма(Знач ВремяНачала, ИнформацияОбОшибке, Знач ЛиСинтаксическийКонтроль, Знач СтруктураПараметров = Неопределено)
	...
	Если Не ЛиСинтаксическийКонтроль Тогда 
		...
		КонецЕсли;
		ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаАлгоритма.ПолеТекста, -СтартоваяСтрока,,, МодальныйРежим,
...
Процедура УстановитьФильтрВыбораФайла(Знач ДиалогВыбораФайла)
	...
	#КонецЕсли
	ДиалогВыбораФайла.Фильтр = ирОбщий.ПолучитьСтрокуФильтраДляВыбораФайлаЛкс(мРасширениеФайла + ", f1c", "Файлы консоли кода");
...
Функция ЗаменитьВозвратыНаПерейтиВТекстеМетода1С(Знач Текст, ИмяПеременнойРезультата = "Результат") Экспорт
	...
	Текст = ОбработкаРегулярныхВыражений.Replace(Текст, "$1" + ИмяПеременнойРезультата + " = $2;" + Символы.ПС + "$1Перейти ~Конец$3");
	ОбработкаРегулярныхВыражений.Pattern = "((?:^|\n|\r)(?:\t| )*)Возврат(?:\t| )*(\n|\r|;)";
...
Процедура НайтиПараметрВТексте(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
	    ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ПолеТекстаАлгоритма.НайтиПоказатьСловоВТексте(ТекущаяСтрока.Имя,, ПолеТекстаАлгоритма.ПолеТекста);
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	ПолеТекстаАлгоритма.ПолеТекста.УстановитьТекст("");
	Параметры.Очистить();
	Если мАлгоритмЗагруженВКонсоль Тогда
		ПолеТекстаАлгоритма.ПолеТекста.УстановитьТекст(мТекущаяСтрокаДереваАлгоритмов.ТекстАлгоритма);
...
Процедура КоманднаяПанельВстроенныйЯзыкЗначениеИзБуфера(Кнопка)
	...
	Если ЭтаФорма.ТекущийЭлемент <> ПолеТекстаАлгоритма.ПолеТекста Тогда
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, АктивизироватьПервуюСтроку = Неопределено)
	...
	ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаАлгоритма.ПолеТекста, СтрокаПоискаВДереве, Ложь);
...
Функция ВычислитьТекущееВыражение(ТекущееВыражение = "")
	...
	ПараметрыЗапуска.Вставить("ВыполнятьНаСервере", КонтекстВыполнения <> "Клиент");
...
Функция ПараметрыИзАлгоритма(Знач ТекстДляПроверки, Знач ЭтоКлиент, Знач НевидимоеПоле, выхКорректныйФрагмент = Истина, ИгнорироватьПеременную = "", ПоказыватьСтрокуОшибки = Ложь) Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
		НевидимоеПоле = Обработки.ирКлсПолеТекстаПрограммы.Создать();
		ПолеТекстаАлгоритма = НевидимоеПоле;
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	СтруктураПараметровКода = Новый Структура;
	МетаОбщиеМодули = Метаданные.ОбщиеМодули;
	СтартоваяСтрока = 0;
	НевидимоеПоле.ПолеТекста.УстановитьТекст(ТекстДляПроверки);
	Пока Истина Цикл
		...
		Если НеопределенныйМетод <> Неопределено Тогда
			...
			НевидимоеПоле.ПолеТекста.УстановитьГраницыВыделения(КоординатыОшибки.НомерСтроки, НачальнаяКолонка, КоординатыОшибки.НомерСтроки, НачальнаяКолонка);
			НевидимоеПоле.ПолеТекста.ВыделенныйТекст("ирОбщий.");
			Продолжить;
		КонецЕсли; 
		НеопределеннаяПеременная = мПлатформа.ИмяНеопределеннойПеременнойИзИнформацииОбОшибке(ИнформацияОбОшибке);
		Если Не ЗначениеЗаполнено(НеопределеннаяПеременная) Тогда
			...
			Если Не выхКорректныйФрагмент Тогда
				...
				Если ПоказыватьСтрокуОшибки Тогда
					ЭтаФорма.ТекущийЭлемент = ПолеТекстаАлгоритма.ПолеТекста;
					ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаАлгоритма.ПолеТекста, СтартоваяСтрока,,,, ИнформацияОбОшибке);
				Иначе
		...
		КонецЕсли; 
		НевидимоеПоле.ПолеТекста.ВставитьСтроку(1, НеопределеннаяПеременная + "=0;");
		СтартоваяСтрока = СтартоваяСтрока + 1;
	КонецЦикла;
...
Процедура ПанельРедактораПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ЭлементыФормы.ПанельРедактора.ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML Тогда
		...
		Тогда
			ПолеТекстаАлгоритма.ПолеТекста = АктивноеПолеТекста(Поле2);
	...
	Иначе
		Если ЭлементыФормы.РедакторHTML.Документ.defaultView <> Неопределено Тогда
			ПолеТекстаАлгоритма.ПолеТекста = АктивноеПолеТекста(Поле1);
...
Процедура РедакторHTMLonclick(Элемент, ДанныеСобытия)
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_HOVER" Тогда
			...
			ОбъектноеВыражение = ПолеТекстаАлгоритма.ТекущееОбъектноеВыражение(Событие.params.line, Событие.params.column,, Ложь);
			Если Ложь
			...
			Если УспехЗначения Тогда
				...
				КонецЕсли; 
			КонецЕсли; 
			ПредставлениеДопустимыхТипов = ПолеТекстаАлгоритма.ПредставлениеМассиваСтруктурТипов(ПолеТекстаАлгоритма.ТаблицаСтруктурТиповТекущегоВыражения(Истина, Истина));
			Если Истина
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_SIGNATURE" Тогда
			...
			#Если ТолстыйКлиентУправляемоеПриложение Тогда
				ПолеТекстаАлгоритма.КончитьОбработкуКоманды();
				ФормаВызовМетода = ПолеТекстаАлгоритма.ФормаВызовМетода();
				...
				Событие.params.activeParameter = ПолеТекстаАлгоритма.мНомерПараметра - 1;
			#Иначе
			...
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				...
				Тогда 
					...
					Если Событие.params.trigger = """" Тогда
						ПоказатьСписок = ПолеТекстаАлгоритма.мЭтоСтроковыйЛитерал;
					Иначе
						ПоказатьСписок = Истина;
					КонецЕсли; 
					Если ПоказатьСписок Тогда
						ПолеТекстаАлгоритма.ЗаполнитьТаблицСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста);
						Если Ложь
							Или Событие.params.trigger = Неопределено
							Или ПолеТекстаАлгоритма.ТаблицаСлов.Количество() > 0
						Тогда
		...
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			РедакторHTML.hideSuggestionsList();
			СтрокаТаблицыСлов = ПолеТекстаАлгоритма.ТаблицаСлов.Найти(Событие.params.selected, "Слово");
			Если СтрокаТаблицыСлов = Неопределено Тогда
				...
				Возврат;
			КонецЕсли; 
			ПолеТекстаАлгоритма.ВставитьВыбранноеСловоАвтодополнения(СтрокаТаблицыСлов, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста);
			ПолеТекстаАлгоритма.УстановитьГраницыВыделения();
			Если ПолеТекстаАлгоритма.мОткрытьСправкуПоПараметру = Истина Тогда
				#Если ТолстыйКлиентОбычноеПриложение Тогда
					ПолеТекстаАлгоритма.ОткрытьСправкуПоПараметру(, Ложь);
				#КонецЕсли
			КонецЕсли; 
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			// https://github.com/salexdv/bsl_console/issues/78
			//Если мРазрешенаПодсказкаHTML <> Истина Тогда 
			//	СтрокаТаблицыСлов = ПолеТекстаАлгоритма.ТаблицаСлов.Найти(ирОбщий.СтрокаМеждуМаркерамиЛкс(Событие.params.focused, "Элемент ", ",", Истина) , "Слово");
			//	Если СтрокаТаблицыСлов <> Неопределено Тогда
			//		Если ПолеТекстаАлгоритма.УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, СтрокаТаблицыСлов) Тогда 
			//			//РедакторHTML.hideSuggestionsList();
			//			//ПередатьТаблицуСловВРедакторHTML();
...
Процедура ПередатьТаблицуСловВРедакторHTML()
	...
	СтруктураПодсказки = Новый Массив;
	Для Каждого СтрокаСлова Из ПолеТекстаАлгоритма.ТаблицаСлов Цикл
		ПодсказкаСлова = Новый Структура();
...
Процедура ОткрытьКонструкторФорматнойСтроки(ФорматнаяСтрока)
	...
	Если Конструктор.ОткрытьМодально() Тогда
		...
		Результат = """" + Результат + """";
		ПолеТекстаАлгоритма.ВыделенныйТекст(Результат);
	КонецЕсли; 
...
Процедура КПРедакторHTMLКонструкторФорматнойСтроки(Кнопка)
	...
	ФорматнаяСтрока = СтрЗаменить(СтрЗаменить(ПолеТекстаАлгоритма.ВыделитьТекстТекущегоСтроковогоЛитерала(), "|", ""), """", "");
