...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если ЭтоПереходКОпределению Тогда
		ПерейтиКОпределению(РезультатНажатия);
	КонецЕсли; 
	
КонецПроцедуры

Процедура ПерейтиКОпределению(Знач РезультатНажатия)
...
Процедура РедакторHTMLonclick(Элемент, ДанныеСобытия)
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_HOVER" Тогда
			...
			Если Ложь
				Или Найти(ОбъектноеВыражение, "(") И Не ирОбщий.СтрКончаетсяНаЛкс(ОбъектноеВыражение, "(") 
			...
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				...
				Тогда 
					...
					Если ПоказатьСписок Тогда
						ОбновитьКонтекстПоляВстроенногоЯзыка();
						ПолеТекстаАлгоритма.ЗаполнитьТаблицуСлов(, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста, Событие.params.trigger = Неопределено, Ложь);
						Если Ложь
		...
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			...
			ПолеТекстаАлгоритма.КончитьОбработкуКоманды();
			ПолеТекстаАлгоритма.РазобратьТекущийКонтекст();
			СтрокаТаблицыСлов = ПолеТекстаАлгоритма.ТаблицаСлов.Найти(ПолеТекстаАлгоритма.ЧистоеСловоСпискаПодсказкиHTML(Событие.params.selected), "Слово");
			Если СтрокаТаблицыСлов = Неопределено Тогда
				...
				Возврат;
			КонецЕсли; 
			Если ирОбщий.СтрНачинаетсяСЛкс(Событие.params.trigger, "force-") Тогда
				ПараметрЗакрытия = Прав(Событие.params.trigger, 1);
			Иначе
				ПараметрЗакрытия = Истина;
			КонецЕсли; 
			ПолеТекстаАлгоритма.ВставитьВыбранноеСловоАвтодополнения(СтрокаТаблицыСлов, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста,, ПараметрЗакрытия);
			ПолеТекстаАлгоритма.УстановитьГраницыВыделения();
			Если ПараметрЗакрытия = "." Тогда
				//РедакторHTML.triggerSuggestions(); // Так список не появляется на экране
			КонецЕсли; 
		...
		ИначеЕсли Событие.event = "EVENT_ON_ACTIVATE_SUGGEST_ROW" Тогда
			...
			Если мРазрешенаПодсказкаHTML <> Истина Тогда 
				СтрокаТаблицыСлов = ПолеТекстаАлгоритма.ТаблицаСлов.Найти(ПолеТекстаАлгоритма.ЧистоеСловоСпискаПодсказкиHTML(Событие.params.focused), "Слово");
				Если СтрокаТаблицыСлов <> Неопределено Тогда
					ПолеТекстаАлгоритма.УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, СтрокаТаблицыСлов);
