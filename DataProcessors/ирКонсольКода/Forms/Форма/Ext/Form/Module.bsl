...
Процедура ПанельРедактораПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторHTML Тогда
		...
		Поле2.УстановитьТекст(Поле1.ПолучитьТекст(), Истина, мТекущаяСтрокаДереваАлгоритмов.ТекстАлгоритма);
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_ON_LINK_CLICK" Тогда
			...
			ИначеЕсли ЗаголовокГиперссылки = "Знач" Тогда
				ирОбщий.ОткрытьЗначениеЛкс(ВычислитьВыражение(ЗначениеГиперссылки));
			ИначеЕсли Ложь
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_HOVER" Тогда
			#Если Сервер И Не Сервер Тогда
				ВычислитьВыражение();
			#КонецЕсли
			ПолеТекстаАлгоритма.РедакторHTML_ПередПоказомПодсказкиУдержания("ВычислитьВыражение", Событие.params.column, Событие.params.line, Событие.params.word);
		ИначеЕсли Событие.event = "EVENT_BEFORE_SIGNATURE" Тогда
			ПолеТекстаАлгоритма.РедакторHTML_ПередПоказомСигнатуры(Событие.params.activeParameter, Событие.params.word, Событие.params.activeSignature, Событие.params.triggerCharacter);
		ИначеЕсли Событие.event = "EVENT_BEFORE_SHOW_SUGGEST" Тогда
			#Если Сервер И Не Сервер Тогда
				ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
			#КонецЕсли
			ОбновитьКонтекстПоляВстроенногоЯзыка();
			ПолеТекстаАлгоритма.РедакторHTML_ПередПоказомАвтодополнения(Событие.params.trigger, Событие.params.last_expression, Событие.params.last_word);
		ИначеЕсли Событие.event = "EVENT_ON_SELECT_SUGGEST_ROW" Тогда
			ПолеТекстаАлгоритма.РедакторHTML_ПриВыбореСтрокиАвтодополнения(Событие.params.trigger, Событие.params.last_expression, Событие.params.selected);
