...
Процедура ПриОткрытии()
	...
	ИмяФайлаВнешнейОбработки = ирОбщий.ФайлВнешнейОбработкиДляОтладкиЛкс(ИмяВнешнейОбработки);
...
Функция ВыполнитьЛокально(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	ЧерезФоновоеЗадание = КонтекстВыполнения = "ФоновоеЗадание";
...
Процедура ОбновитьФайлВнешнейОбработки(Знач СтруктураПараметров = Неопределено)
	...
	Если Не ирОбщий.ОбновитьМодульВнешнейОбработкиДляОтладкиЛкс(ИмяВнешнейОбработки, ТекстМодуля, ТекстМодуляВКонсолиНеМенялся, ДатаИзмененияВнешнейОбработки, КонтекстВыполнения <> "Клиент") Тогда 
		Возврат;
	КонецЕсли; 
...
Процедура ОбновитьДоступностьКнопкиВыполнятьНаСервере()
	...
	ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.КонтекстВыполнения.Кнопки.ФоновоеЗадание.Доступность = Истина
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		...
		Если ИсходнаяТаблицаПараметров <> Неопределено Тогда
			...
			КонецЦикла;
		КонецЕсли; 
		ВыбранСпособВыполнения(ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.КонтекстВыполнения.Кнопки.Найти(мТекущаяСтрокаДереваАлгоритмов.КонтекстВыполнения));
	КонецЕсли;
