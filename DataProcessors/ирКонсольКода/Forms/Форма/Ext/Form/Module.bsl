...
Функция ВыполнитьЛокально(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Попытка
		...
		Если Истина
			...
			И РежимВнешнейОбработки 
		Тогда
			КонтекстВыполнения.ВыполнитьАлгоритмЧерезВнешнююОбработкуЛкс(ИмяФайлаВнешнейОбработки, СтруктураПараметров, ВремяНачала);
		Иначе
...
Процедура КоманднаяПанельВстроенныйЯзыкВыполнятьНаСервере(Кнопка)
	...
	ДоступностьВнешнейОбработкиНаСервре = Не НаСервере Или Не ирКэш.ЛиПортативныйРежимЛкс();
...
Процедура ЗаполнитьПараметры(ПараметрыВхода = Истина, ПараметрыВыхода = Истина)
	...
	Если ПараметрыВхода Тогда
		Пока Истина Цикл
			...
			Если Не ЗначениеЗаполнено(НеопределеннаяПеременная) Тогда
				...
				Прервать;
			КонецЕсли;
			Если НаСервере И МетаОбщиеМодули.Найти(НеопределеннаяПеременная) <> Неопределено Тогда
				Продолжить;
			КонецЕсли; 
	...
	Если ПараметрыВыхода Тогда
		...
		Для Каждого СтрокаПеременной Из СтрокиЛокальныхПеременных Цикл
			Если НаСервере И МетаОбщиеМодули.Найти(СтрокаПеременной.НСлово) <> Неопределено Тогда
				Продолжить;
			КонецЕсли; 
...
Функция ПолучитьОбновитьФайлВнешнейОбработки(СтруктураПараметров = Неопределено) Экспорт
	...
	ирОбщий.ОбновитьМодульВнешнейОбработкиДляОтладкиЛкс(ИмяФайлаВнешнейОбработки, ИмяВнешнейОбработки, ТекстМодуля, ТекстМодуляТекущейВнешнейОбработки, ДатаИзмененияВнешнейОбработки, НаСервере);
