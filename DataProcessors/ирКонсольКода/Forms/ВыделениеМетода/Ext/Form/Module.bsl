...
Процедура ТаблицаПараметровПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	//ОформлениеСтроки.Ячейки.Вход.ТолькоПросмотр = ДанныеСтроки.ВходОбязательно;
...
Процедура ТаблицаПараметровТипЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если РезультатФормы <> Неопределено Тогда
		...
		Для Каждого Тип Из мПлатформа.ПолучитьОписаниеТиповИзДопустимыхТипов(РезультатФормы).Типы() Цикл
			НеопознанныеТипы.Добавить(мПлатформа.ПолучитьСтрокуКонкретногоТипа(мПлатформа.ПолучитьСтруктуруТипаИзКонкретногоТипа(Тип)));
		КонецЦикла;
		ирОбщий.ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ЭлементыФормы.ТаблицаПараметров, ЭлементыФормы.ТаблицаПараметров.Колонки.ТипЗначения, ирОбщий.СтрСоединитьЛкс(НеопознанныеТипы, ", "));
	КонецЕсли; 
