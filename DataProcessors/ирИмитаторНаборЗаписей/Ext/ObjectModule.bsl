...
Процедура Конструктор(Объект) Экспорт 
	...
	Если Не Объект.Модифицированность() И Объект.Выбран() Тогда
...
Процедура КонструкторПоКлючу(ИмяОсновнойТаблицы, КлючОбъекта = Неопределено) Экспорт 
	...
	ДоступныеПоляОтбора = Отбор.ПолучитьДоступныеПоля();
	СтруктураКлюча = ирОбщий.ПолучитьСтруктуруКлючаТаблицыБДЛкс(ИмяОсновнойТаблицы, Ложь);
	Для Каждого КлючИЗначение Из СтруктураКлюча Цикл
		Поле = ДоступныеПоляОтбора.Добавить(КлючИЗначение.Ключ, КлючИЗначение.Ключ, КлючИЗначение.Значение);
		Поле.Отбор = Истина;
	КонецЦикла;
	Отбор.УстановитьДоступныеПоля(ДоступныеПоляОтбора);
	Для Каждого КлючИЗначение Из СтруктураКлюча Цикл
		Отбор.Добавить(КлючИЗначение.Ключ, КлючИЗначение.Ключ);
	КонецЦикла;
