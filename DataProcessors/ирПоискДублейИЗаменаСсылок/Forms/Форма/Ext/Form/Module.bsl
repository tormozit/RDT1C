...
Процедура ОткрытьСЗаполнениемГруппДублейПоТаблицеПар(Знач ТаблицаПар, Знач ИмяКолонкиТаблицыПарПравильный, Знач ИмяКолонкиТаблицыПарНеправильный) Экспорт
	...
	Если ТаблицаПар.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	ТаблицаПар = ирОбщий.ПолучитьТаблицуСМинимальнымиТипамиКолонокЛкс(ТаблицаПар.Скопировать(, ИмяКолонкиТаблицыПарНеправильный + "," + ИмяКолонкиТаблицыПарПравильный));
	Типы0 = ТаблицаПар.Колонки[0].ТипЗначения.Типы();
	Типы1 = ТаблицаПар.Колонки[1].ТипЗначения.Типы();
	Если Ложь
