...
Функция ПолучитьЗапросПоискаДублейПоРавенствуРеквизитов(ЗапросСОтборомОбщим, выхТекстИдексов = "")
	...
	Если ЗапросСОтборомПар <> Неопределено Тогда
		...
		|	" + ТекстИсточника  + " КАК Правильный,
		|	" + ТекстИсточника  + " КАК Неправильный
...
Процедура ОбновитьСхемуКомпоновки()
	...
	ЗапросПар = Новый Запрос("ВЫБРАТЬ Правильный.Ссылка КАК " + мИмяПоляПравильныйЭлемент + ", Неправильный.Ссылка КАК " + мИмяПоляНеправильныйЭлемент + "
		|ИЗ " + ТекстИсточника + " КАК Неправильный, " + ТекстИсточника + " КАК Правильный, " + ТекстИсточника + " КАК " + мПсевдонимТаблицы);
	мСхемаКомпоновкиПар = ирОбщий.ПолучитьСхемуКомпоновкиПоЗапросуЛкс(ЗапросПар,, мПредставления);
	#Если _ Тогда
