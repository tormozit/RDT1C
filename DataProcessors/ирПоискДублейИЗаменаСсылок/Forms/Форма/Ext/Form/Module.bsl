...
Процедура КоманднаяПанельНайденныеСсылкиКонсольКомпоновки(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		КонсольКомпоновокДанных = Отчеты.ирКонсольКомпоновокДанных.Создать();
	#КонецЕсли
	КонсольКомпоновокДанных.ОткрытьПоТаблицеЗначений(ЭтотОбъект.НайденныеСсылки.Выгрузить());
...
Функция ПолучитьСтруктуруНастроекОбработки()
	...
	СтруктураНастроек.Вставить("ПроизвольныйЗапрос", СтруктураПроизвольногоЗапроса());
...
Процедура ЗагрузитьНастройкиИзСтруктуры(СтруктураНастроек)
	...
	ПриУстановкеПолногоИмениОбъекта();
	Если СтруктураНастроек.Свойство("ПроизвольныйЗапрос") Тогда
		УстановитьПроизвольныйЗапросИзСтруктуры(СтруктураНастроек.ПроизвольныйЗапрос);
	КонецЕсли; 
	ОчиститьРезультатыПоиска();
