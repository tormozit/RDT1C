...
Процедура АвтозаменаЭлементов(ОбработатьНеправильные, НепосредственноеУдаление = Истина) Экспорт
	...
	Для Каждого СтрокаПравильного Из ПравильныеСтроки Цикл
		...
		#КонецЕсли
		РезультатОбработки = ирОбщий.ПоискДублейИЗаменаСсылок_ВыполнитьЗаменуЛкс(ОбщиеПараметрыОбработки(), СоответствиеЗамен, РезультатПоиска.ТаблицаСсылок, , ЭтаФорма);
...
Процедура ОбновитьСчетчикиПравилЗамены()
	...
	Для Каждого СтрокаЗамены Из ПравилаЗаменыСсылок Цикл
		...
		КонецЕсли; 
	КонецЦикла;
	ирОбщий.ТабличноеПолеОбновитьТекстыПодваловЛкс(ЭлементыФормы.ПравилаЗаменыСсылок);
