...
Функция УдалитьНеправильнуюСсылку(УдаляемаяСсылка, НепосредственноеУдаление, СтрокаНеправильного = Неопределено, СтрокаГруппы = Неопределено,
	...
	Если Объект <> Неопределено Тогда
		...
		КонецЕсли; 
		Если ОтключатьКонтрольЗаписи Тогда
			Если Не ПланыОбмена.ТипВсеСсылки().СодержитТип(ТипЗнч(УдаляемаяСсылка)) Тогда
				Объект.ОбменДанными.Загрузка = Истина;
			КонецЕсли; 
		КонецЕсли;
...
Процедура ЗаполнитьПризнакПравильного(ИмяКолонкиПризнака, ИмяКолонкиЗначения)
	...
	Для каждого СтрокЭлемента Из мРезультатыПоиска Цикл
		...
		Если ЗначениеЗаполнено(СтрокаГруппы[ИмяКолонкиЗначения]) Тогда
			Продолжить;
		КонецЕсли; 
		СтрокаГруппы[ИмяКолонкиЗначения] = СтрокЭлемента.Ссылка;
		СтрокЭлемента[ИмяКолонкиПризнака] = Истина;
	КонецЦикла;
