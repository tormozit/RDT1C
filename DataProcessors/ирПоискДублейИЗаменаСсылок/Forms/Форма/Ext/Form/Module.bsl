...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	ирОбщий.ИнициироватьГлобальныйКонтекстПодсказкиЛкс(мПолеТекстаПрограммы);
	...
	#КонецЕсли
	мПолеТекстаПрограммы.ДобавитьСловоЛокальногоКонтекста("ПравильныйОбъект", , Новый ОписаниеТипов(МассивТиповПравильных));
	мПолеТекстаПрограммы.ДобавитьСловоЛокальногоКонтекста("НеправильныйОбъект", , Новый ОписаниеТипов(МассивТиповНеправильных));
	мПолеТекстаПрограммы.ДобавитьСловоЛокальногоКонтекста("СтандартнаяОбработка", , Новый ОписаниеТипов("Булево"));
	мПолеТекстаПрограммы.ЛиСерверныйКонтекст = ВыполнятьНаСервере;
...
Процедура ЗагрузитьНастройкуВФорме(НастройкаФормы, ДопПараметры) Экспорт 
	...
	КонецЕсли; 
	Если СтарыеНайденныеГруппы <> Неопределено Тогда
		...
		мПредставления = ирОбщий.СкопироватьУниверсальнуюКоллекциюЛкс(НастройкаФормы.ПредставленияПолей);
...
Функция ПолучитьЗапросПоискаДублей(ЗапросСОтборомОбщим, выхТекстИдексов = "", выхПоискПоПохожимСловам = Неопределено)
	...
	ТекстУпорядочитьГруппы = ирОбщий.ВыражениеПорядкаКомпоновкиНаЯзыкеЛкс(КомпоновщикСравнения.Настройки.Порядок, ?(выхПоискПоПохожимСловам, мПутьКДаннымПоляНечеткогоСравнения, "") , "_");
...
Функция ЗаполнитьПравилаПоГруппеДублей(НомерГруппы)
	...
	Если СтрокаПравильного = Неопределено Тогда
		ирОбщий.СообщитьЛкс("Не определен правильный элемент в группе " + НомерГруппы);
		Возврат Ложь;
	КонецЕсли;
	Если СоставТекущейГруппы.Количество() = 1 Тогда
		ирОбщий.СообщитьЛкс("Отсутствуют неправильные элементы в группе " + НомерГруппы);
		Возврат Ложь;
	КонецЕсли; 
...
Процедура ОбновитьПодменюПоследнихОтборовПоиска()
	...
	#КонецЕсли
	ирОбщий.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КоманднаяПанельОтбор.Кнопки.ПоследниеОтборы, ПолноеИмяОбъекта, Новый Действие("ПоследниеОтборыПоискаНажатие"), "Отборы");

КонецПроцедуры

Функция ПоследниеОтборыПоискаНажатие(Кнопка) Экспорт
	
	НастройкаКомпоновки = ирОбщий.ВыбранныйЭлементПоследнихЗначенийЛкс(ЭтаФорма, ПолноеИмяОбъекта, Кнопка, "Отборы", Истина);
	#Если Сервер И Не Сервер Тогда
		НастройкаКомпоновки = Новый НастройкиКомпоновкиДанных;
	#КонецЕсли
	ирОбщий.СкопироватьОтборЛюбойЛкс(ЭлементыФормы.Отбор.Значение, НастройкаКомпоновки.Отбор);
	ирОбщий.ОбновитьТабличноеПолеДереваПослеУстановкиДанныхЛкс(ЭтаФорма, ЭлементыФормы.Отбор);
...
Процедура КоманднаяПанельНастройкиОтчетаПоиск(Кнопка)
	...
	Тогда
		Если ПустаяСтрока(ПолноеИмяОбъекта) Тогда
			...
			Возврат;
		КонецЕсли;
		ДобавленВСписок = ирОбщий.ДобавитьОтборКомпоновкиВИсториюТаблицыБДЛкс(ЭтаФорма, ПолноеИмяОбъекта, КомпоновщикСравнения.Настройки, СтарыйКомпоновщик.Настройки);
	...
	КонецЕсли; 
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("СтруктураЗапроса", ирОбщий.ЗапросВСтруктуруЛкс(ЗапросПоискаДублей));
...
Процедура ЭлементыТекущейГруппыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельЭлементыТекущейГруппы.Кнопки.Идентификаторы);
...
Функция ПроверитьНаличиеЭлементаПередДобавлением(ВыбранноеЗначение)
	...
	Если НоваяСтрокаНайденных <> Неопределено Тогда
		ирОбщий.СообщитьЛкс("Элемент """ + ВыбранноеЗначение + """ нельзя добавить в группу " + мТекущаяГруппа[мИмяПоляНомерГруппы] 
...
Процедура КоманднаяПанельНайденныеСсылкиВыполнитьЗамену(Кнопка)
	...
	ПараметрыЗадания.Вставить("СоответствиеЗамен", СоответствиеЗамен);
	ПараметрыЗадания.Вставить("НайденныеСсылки", ЭтотОбъект.НайденныеСсылки.Выгрузить(Новый Структура("Пометка", Истина)));
	#Если Сервер И Не Сервер Тогда
		ВыполнитьЗаменуЭлементов();
		ВыполнитьЗаменуЗавершение();
	#КонецЕсли
	ирОбщий.ВыполнитьЗаданиеФормыЛкс("ВыполнитьЗаменуЭлементов", ПараметрыЗадания, ЭтаФорма, "ЗаменаСсылок",, Кнопка, "ВыполнитьЗаменуЗавершение",,, Истина);
...
Функция ОпределитьКоличествоСсылок(РазрешитьАсинхронно = Ложь) Экспорт
	...
	Иначе
		Ответ = Вопрос("Поиск ссылок может длится долго. Продолжить?",
...
Процедура ОпределитьКоличествоСсылокЗавершить(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Если Ложь
		...
		Или СостояниеЗадания = СостояниеФоновогоЗадания.Завершено 
	Тогда
		ТаблицаСсылок = РезультатЗадания.ТаблицаСсылок;
		#Если Сервер И Не Сервер Тогда
			РезультатЗадания = Новый ТаблицаЗначений;
		#КонецЕсли
...
Процедура ОбновитьСчетчикиПравилЗамены()
	...
	Для Каждого СтрокаЗамены Из ПравилаЗаменыСсылок Цикл
		...
		КонецЕсли; 
	КонецЦикла;
	ирОбщий.ТабличноеПолеОбновитьТекстыПодваловЛкс(ЭтаФорма, ЭлементыФормы.ПравилаЗаменыСсылок);
...
Процедура ВыполнитьПоискЗаменяемыхСсылок(ЗадаватьВопрос = Истина, РазрешитьАсинхронно = Истина, Перезапустить = Истина)
	...
	Для Каждого НеуникальноеЗначение Из МассивНеуникальных Цикл
		ирОбщий.СообщитьЛкс("Заменяемая ссылка """ + НеуникальноеЗначение + """ встречается более одного раза в выбранных правилах", СтатусСообщения.Внимание);
	КонецЦикла; 
	Если МассивНеуникальных.Количество() > 0 Тогда
		Возврат;
	КонецЕсли; 
	Если МассивЗаменяемых.Количество() = 0 Тогда
		Предупреждение("Не выбрано правил замены!");
		Возврат;
	КонецЕсли;
	#Если Сервер И Не Сервер Тогда
	...
	#КонецЕсли
	мОбработкаПоискаСсылок.НайтиСсылкиНаОбъекты(МассивЗаменяемых,, ЭтаФорма, ЭлементыФормы.КоманднаяПанельЗаменаСсылок.Кнопки.ПоискСсылок, "ПоискЗаменяемыхСсылокЗавершить", РазрешитьАсинхронно, Перезапустить);
...
Процедура ПоискЗаменяемыхСсылокЗавершить(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		#Если Сервер И Не Сервер Тогда
			мОбработкаПоискаСсылок = Обработки.ирРедакторОбъектаБД.Создать();
		#КонецЕсли
		мОбработкаПоискаСсылок.ОбработатьТаблицуНайденныхСсылок(РезультатЗадания.ТаблицаСсылок);
...
Процедура ЗаменыСсылокЧтоЗаменятьПриИзменении(Элемент = Неопределено)
	...
	Если ТипЗнч(Элемент.Значение) = Тип("Строка") Тогда
		ирОбщий.ПолеВвода_ОкончаниеВводаТекстаЛкс(Элемент, Элемент.Значение, Элемент.Значение, Истина,, Истина);
	КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	ирОбщий.СоздатьМенеджерСохраненияНастроекФормыЛкс(ЭтаФорма,,,, "fdr", СоставНастройкиФормы);
	мПолеТекстаПрограммы = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
	#Если Сервер И Не Сервер Тогда
		мПолеТекстаПрограммы = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
...
Процедура ПриИзмененииАвтовидимостьКолонокСоставаГруппы()
	...
	ирОбщий.СкрытьПоказатьОднозначныеКолонкиТабличногоПоляЛкс(ЭлементыФормы.ЭлементыТекущейГруппы, АвтовидимостьКолонокСоставаГруппы, ИгнорироватьКолонки);
...
Процедура КоманднаяПанельЗаменаСсылокВключитьВсе(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.ПравилаЗаменыСсылок, , Истина,,, Ложь);
	//ОбновитьКоличествоПомеченныхПравил();
...
Процедура КоманднаяПанельЗаменаСсылокВыключитьВсе(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.ПравилаЗаменыСсылок, , Ложь,,, Ложь);
	//ОбновитьКоличествоПомеченныхПравил();
...
Процедура ОбновитьПодменюПоследнихОтборовНайденных()
	...
	ирОбщий.ОбновитьПодменюПоследнихОтборовЛкс(ЭтаФорма, ЭлементыФормы.КоманднаяПанельНайденныеСсылки, ЭлементыФормы.НайденныеСсылки);
...
Процедура ПроверитьИзменениеОтбораНайдененныхДляИсторииОтложенно()
	...
	ДобавленВСписок = ирОбщий.ДобавитьОтборВИсториюТабличногоПоляЛкс(ЭтаФорма, ТабличноеПоле, ТабличноеПоле.ОтборСтрок, СтарыйОтборНайденных);
	Если ДобавленВСписок Тогда
		ОбновитьПодменюПоследнихОтборовНайденных();
	КонецЕсли;
...
Процедура КоманднаяПанельЭлементыТекущейГруппыСравнить2Строки(Кнопка)
	...
	ДанныеСтроки1 = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяОбъекта, ВыделенныеСтроки[0].Ссылка).Методы;
	ДанныеСтроки2 = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяОбъекта, ВыделенныеСтроки[1].Ссылка).Методы;
	ирОбщий.СравнитьЗначенияВФормеЛкс(ДанныеСтроки1, ДанныеСтроки2,,,,, ТекущаяКолонка);
...
Процедура КоманднаяПанельЗаменаСсылокСравнитьОбъекты(Кнопка)
	...
	ирОбщий.СравнитьЗначенияВФормеЛкс(ДанныеСтроки1, ДанныеСтроки2);
	
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПоискДублейИЗаменаСсылок.Форма.Форма");
...
мОбработкаПоискаСсылок = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирРедакторОбъектаБД");
