...
Функция ЗагрузитьРеквизитыЭлементовТекущейГруппы(ВыборочныйМассив = Неопределено)
	...
	Иначе
		...
		Для Каждого СтрокаСоставаГруппы Из ЭлементыТекущейГруппы Цикл
			...
			КонецЕсли; 
			СтрокаСоставаГруппы[мИмяПоляИдентификатор] = XMLСтрока(СтрокаСоставаГруппы.Ссылка);
		КонецЦикла;
...
Процедура НайтиПоРавенствуРеквизитов(ЗапросПоиска, ТекстИдексов)
	...
	//мРезультатыПоиска = РезультатПакета[РезультатПакета.ВГраница()].Выгрузить();
	...
	Для Каждого СтрокаПоиска ИЗ мРезультатыПоиска Цикл
		...
		КонецЕсли; 
		Для Каждого СтроковоеПоле Из МассивСтроковыхПолейСравнения Цикл
			ЗначениеПоля = КлючПоиска[СтроковоеПоле];
			// Заменим неразрывный пробел на нормальный
			ЗначениеПоля = СтрЗаменить(ЗначениеПоля, Символ(10 * 16), " ");
			// Удалим служебный символ "_" в конце строки
			КлючПоиска[СтроковоеПоле] = Сред(Нрег(ЗначениеПоля), 1, СтрДлина(ЗначениеПоля) - 1);
		КонецЦикла;
...
Функция ПолучитьЗапросПоискаДублейПоРавенствуРеквизитов(ЗапросСОтборомОбщим, выхТекстИдексов = "")
	...
	Для Каждого ПолеСравнения Из мПоляСравнения Цикл
		...
		Если ДоступноеПоле.ТипЗначения.СодержитТип(Тип("Строка")) Тогда
			ВыражениеПоля = "ПОДСТРОКА(ВЫРАЗИТЬ(" + мПсевдонимТаблицы + "." + ПолноеИмяПоля + " КАК СТРОКА(300)), 1, 300) + ""_""";
		Иначе
			ВыражениеПоля = мПсевдонимТаблицы + "." + ПолноеИмяПоля;
		КонецЕсли;
		...
		Если ДоступноеПоле.ТипЗначения.СодержитТип(Тип("Строка")) Тогда
			ВыражениеПоляПравильный = "ПОДСТРОКА(ВЫРАЗИТЬ(Правильный." + ПолноеИмяПоля + " КАК СТРОКА(300)), 1, 300) + ""_""";
		Иначе
			ВыражениеПоляПравильный = "Правильный." + ПолноеИмяПоля;
		КонецЕсли;
		ТекстСоединенияПравильный = ТекстСоединенияПравильный + Символы.ПС + "И " + ВыражениеПоляПравильный + " = ПервичныеГруппыДублей." + ИмяПоля;
		Если ДоступноеПоле.ТипЗначения.СодержитТип(Тип("Строка")) Тогда
			ВыражениеПоляНеправильный = "ПОДСТРОКА(ВЫРАЗИТЬ(Неправильный." + ПолноеИмяПоля + " КАК СТРОКА(300)), 1, 300) + ""_""";
		Иначе
			ВыражениеПоляНеправильный = "Неправильный." + ПолноеИмяПоля;
		КонецЕсли;
	...
	ТекстУпорядочитьГруппы = ирОбщий.ПолучитьСтрокуПорядкаКомпоновкиЛкс(КомпоновщикСравнения.Настройки.Порядок, , "_");
	Если ЗапросСОтборомПар <> Неопределено Тогда
		...
		ТекстОтбораПар = ирОбщий.ПолучитьТекстОтбораЗапросаКомпоновкиЛкс(ЗапросСОтборомПар, мПсевдонимТаблицы);
		ТекстЗапросаПоиска = ТекстЗапросаПоиска + "// Пары Правильный-Неправильный
		|ВЫБРАТЬ
		|	Правильный.Ссылка КАК Правильный,
		|	Неправильный.Ссылка КАК Неправильный
		|ПОМЕСТИТЬ Пары
		|ИЗ ПервичныеГруппыДублей КАК ПервичныеГруппыДублей,
		|	" + ПолноеИмяОбъекта  + " КАК Правильный,
		|	" + ПолноеИмяОбъекта  + " КАК Неправильный
		|ГДЕ Правильный.Ссылка <> Неправильный.Ссылка 
		|	И " + СокрЛ(ТекстОтбораПар) + ТекстСоединенияПравильный + ТекстСоединенияНеправильный + "
		|; 
		...
		|ИЗ " + ПолноеИмяОбъекта  + " КАК " + мПсевдонимТаблицы + " ВНУТРЕННЕЕ СОЕДИНЕНИЕ (
	...
	Иначе
		...
		|ИЗ " + ПолноеИмяОбъекта  + " КАК " + мПсевдонимТаблицы + " ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПервичныеГруппыДублей КАК ПервичныеГруппыДублей
		|ПО " + ТекстСоединения + "
...
Процедура НайтиПоПохожимСловам(ЗапросСОтбором)
	...
	Если НечеткоеСравнениеСлов <> Неопределено Тогда
		Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ТаблицаОбъекта.Количество(), "Нечеткое сравнение строк");
		СчетчикОбработанныхСтрок = 0;
		Пока ТаблицаОбъекта.Количество() > 1 Цикл
			ирОбщий.ОбработатьИндикаторЛкс(Индикатор, СчетчикОбработанныхСтрок);
...
Процедура ПриУстановкеПолногоИмениОбъекта()
	...
	Если мМетаданныеОбъекта <> Неопределено Тогда
		мИмяТипаСсылки = ирОбщий.ПолучитьИмяТипаСсылкиТаблицыБДЛкс(ПолноеИмяОбъекта);
...
Процедура ОбновитьСхемуКомпоновки()
	...
	мСхемаКомпоновкиОбщая = ирОбщий.ПолучитьСхемуКомпоновкиПоЗапросуЛкс("ВЫБРАТЬ " + мПсевдонимТаблицы + ".* ИЗ " + ПолноеИмяОбъекта + " КАК " + мПсевдонимТаблицы);
	мСхемаКомпоновкиПар = ирОбщий.ПолучитьСхемуКомпоновкиПоЗапросуЛкс(
		"ВЫБРАТЬ Правильный.Ссылка КАК " + мИмяПоляПравильныйЭлемент + ", Неправильный.Ссылка КАК " + мИмяПоляНеправильныйЭлемент + "
		|ИЗ " + ПолноеИмяОбъекта + " КАК Неправильный, " + ПолноеИмяОбъекта + " КАК Правильный, " + ПолноеИмяОбъекта + " КАК " + мПсевдонимТаблицы,, мПредставления);
	#Если _ Тогда
...
Процедура КоманднаяПанельНастройкиОтчетаПоиск(Кнопка)
	...
	ПоискПоПохожимСловам = ЭтоПоискПоПохожимСловам();
	Если ПоискПоПохожимСловам Тогда
		НайтиПоПохожимСловам(ЗапросСОтборомОбщим);
	Иначе
		ТекстИдексов = "";
		ЗапросПоискаДублей = ПолучитьЗапросПоискаДублейПоРавенствуРеквизитов(ЗапросСОтборомОбщим, ТекстИдексов);
...
Процедура ОбновитьСписокВыбораПутиКДаннымВлияющего()
	...
	Если ТекущееПравило <> Неопределено Тогда
		...
		Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ОсновноеЗначение) Тогда
			ПоляТаблицы = ирОбщий.ПолучитьПоляТаблицыМДЛкс(ОсновноеЗначение.Метаданные());
	...
	Если ПанельСсылок.ТекущаяСтраница = ПанельСсылок.Страницы.ДополнительныеПоля Тогда
		Если КомпоновщикДопПолей.Настройки.ДоступныеПоляПорядка.Элементы.Количество() = 0 Тогда
			...
			Для Каждого ПолноеИмяМД Из ПолныеИменаМД Цикл
				Попытка
					Тип = Тип(ирОбщий.ПолучитьИмяТипаСсылкиТаблицыБДЛкс(ПолноеИмяМД));
				Исключение
					Продолжить;
				КонецПопытки;
...
Процедура КоманднаяПанельЭлементыТекущейГруппыПодбор(Кнопка)
	...
	КонецЕсли; 
	ОписаниеТипов = Новый ОписаниеТипов(ирОбщий.ПолучитьИмяТипаСсылкиТаблицыБДЛкс(ПолноеИмяОбъекта));
...
Процедура КоманднаяПанельНастройкиОтчетаИсполняемыйЗапрос(Кнопка)
	...
	ПоискПоПохожимСловам = ЭтоПоискПоПохожимСловам();
	Если ПоискПоПохожимСловам Тогда
		Сообщить("Для поиска по похожим словам функция не реализована");
	Иначе
		ЗапросПоискаДублей = ПолучитьЗапросПоискаДублейПоРавенствуРеквизитов(ЗапросСОтбором);
...
Процедура КоманднаяПанельОтборРедактироватьСписок(Кнопка)
	
	ирОбщий.РедактироватьСписокПравогоЗначенияОтбораКомпоновкиЛкс(ЭлементыФормы.Отбор);
...
мИмяПоляНеправильныйЭлемент = "НеправильныйЭлемент3463";
мИмяПоляПравильныйЭлемент = "ПравильныйЭлемент3463";
мИмяПоляПравильныеДанные = "ПравильныеДанные3463";
мИмяФлагаПравильныйЭлемент = "ЭтоПравильныйЭлемент3463";
мИмяФлагаПравильныеДанные = "ЭтоПравильныеДанные3463";
УстановитьПредставленияСлужебныхПолей();
