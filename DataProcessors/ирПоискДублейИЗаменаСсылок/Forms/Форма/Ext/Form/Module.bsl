...
Процедура АвтозаменаЭлементов(ОбработатьНеправильные, НепосредственноеУдаление = Истина) Экспорт
	...
	НомерГруппы = 1;
	ЗамещениеВсегда = 0;
	ИндикаторОбщий = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ПравильныеСтроки.Количество(), "Обработка групп");
	Для Каждого СтрокаПравильного Из ПравильныеСтроки Цикл
		...
		Тогда
			...
			Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(МассивНеправильныхСтрок.Количество(), "Удаление неправильных");
	...
	Если мМетаданныеОбъекта <> Неопределено Тогда
		...
		Если НастройкиВосстановления <> Неопределено Тогда
			...
			Если НастройкиВосстановления.Свойство("НастройкиКомпоновки") Тогда
				НастройкиКомпоновкиДублей = ирОбщий.ВосстановитьНастройкуКомпоновкиИзСтрокиXMLЛкс(НастройкиВосстановления.НастройкиКомпоновки);
			КонецЕсли;
...
Процедура ВыполнитьПоискЗаменяемыхСсылок()
	...
	//КоличествоНайденныхСсылок = ЭтотОбъект.НайденныеСсылки.Количество();
...
Процедура КоманднаяПанельНайденныеСсылкиВключитьВсе(Кнопка)
	...
	Для Каждого СтрокаТЧ Из ЭтотОбъект.НайденныеСсылки Цикл
...
Процедура КоманднаяПанельНайденныеСсылкиРедакторОбъектаБД(Кнопка)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Если Не ирОбщий.ЛиКорневойТипОбъектаБДЛкс(ТекущаяСтрока.ТипМетаданных) Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьСсылкуВРедактореОбъектаБДЛкс(ТекущаяСтрока.Данные, ТекущаяСтрока.Ссылка);
