	...
	Для Каждого ПервичнаяГруппа Из ПервичныеГруппы Цикл
		...
		//Если НечеткоеСравнениеСлов <> Неопределено Тогда
			...
			Пока ТаблицаОбъекта.Количество() > 1 Цикл
				...
				РазделительСтрок = "~";
				СтрокаИзМассива = ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(МассивСтрок, РазделительСтрок);
				// Параметры 4-5-6:
...
Процедура ОбновитьСхемуКомпоновки()
	...
	мСхемаКомпоновкиОбщая = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросу(ЗапросОбщий);
	...
	мСхемаКомпоновкиПар = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросу(ЗапросПар);
	#Если Сервер И Не Сервер Тогда
		...
		Для каждого Строка1 Из Строка0.Строки Цикл
			Для каждого Строка2 Из Строка1.Строки Цикл
				Если Строка2.ВключатьВПоиск Тогда
					...
					Попытка
						Если НепосредственноеУдаление Тогда
							ирОбщий.УдалитьОбъектЛкс(Объект, ОбъектыНаСервере, ОтключатьКонтрольЗаписи, БезАвторегистрацииИзменений);
						Иначе
							ирОбщий.УстановитьПометкуУдаленияОбъектаЛкс(Объект, ОбъектыНаСервере, Истина, БезАвторегистрацииИзменений);
						КонецЕсли; 
					Исключение
...
Процедура КоманднаяПанельНайденныеСсылкиВключитьВсе(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхСтрокЛкс(ЭлементыФормы.НайденныеСсылки, , Истина,,, Ложь);
	ОбновитьСчетчикиПравилЗамены();
...
Процедура КоманднаяПанельНайденныеСсылкиВыключитьВсе(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхСтрокЛкс(ЭлементыФормы.НайденныеСсылки, , Ложь,,, Ложь);
	ОбновитьСчетчикиПравилЗамены();
