...
Процедура АвтозаменаЭлементов(ОбработатьНеправильные, НепосредственноеУдаление = Истина) Экспорт
	...
	Для Каждого СтрокаПравильного Из ПравильныеСтроки Цикл
		...
		КонецЦикла;
		НайденныеСсылкиТекущейГруппы = ирОбщий.НайтиПоСсылкамЛкс(МассивСсылок);
		#Если Сервер И Не Сервер Тогда
			ВыполнитьЗаменуЭлементов();
		#КонецЕсли
		РезультатОбработки = ирОбщий.ПоискДублейИЗаменаСсылок_ВыполнитьЗаменуЛкс(ОбщиеПараметрыОбработки(), СоответствиеЗамен, НайденныеСсылкиТекущейГруппы, , ЭтаФорма);
...
Процедура ОпределитьКоличествоСсылокЗавершить(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		#КонецЕсли
		ТаблицаСсылок = РезультатЗадания;
...
Процедура ВыполнитьПоискЗаменяемыхСсылок(ЗаватьВопрос = Истина, РазрешитьАсинхронно = Истина, Перезапустить = Истина)
...
Процедура ПоискЗаменяемыхСсылокЗавершить(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	...
	Тогда
		#Если Сервер И Не Сервер Тогда
			мОбработкаПоискаСсылок = Обработки.ирРедакторОбъектаБД.Создать();
		#КонецЕсли
		мОбработкаПоискаСсылок.ОбработатьТаблицуНайденныхСсылок(РезультатЗадания);
