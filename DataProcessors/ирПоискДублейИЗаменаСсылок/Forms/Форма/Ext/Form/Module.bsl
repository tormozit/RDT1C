...
Процедура НайтиПоПохожимСловам(ЗапросПоиска, ТекстИдексов)
	...
	ПорогПохожести = Макс(0, (70 - ДопустимоеРазличиеСлов) / 70);
	Для Каждого ПервичнаяГруппа Из ПервичныеГруппы Цикл
		...
		Если НечеткоеСравнениеСлов <> Неопределено Тогда
			...
			Пока ТаблицаОбъекта.Количество() > 1 Цикл
				...
				СтрокаИзМассива = ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(МассивСтрок, "~");
				// Параметры 4-5-6:
				...
				ИндексыНайденныхСовпадений = НечеткоеСравнениеСлов.StringSearch(ЛевоеЗначение, СтрокаИзМассива, "~", 10, Макс(0, Цел(ПорогПохожести * 100) - 10), Цел(ПорогПохожести * 100));
				ИндексыПохожихСлов = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(ИндексыНайденныхСовпадений, ",",, Ложь);
