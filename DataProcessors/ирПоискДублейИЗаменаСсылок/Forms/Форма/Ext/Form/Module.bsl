...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	ирОбщий.ИнициализироватьГлобальныйКонтекстПодсказкиЛкс(мПолеТекстаПрограммы);
...
Функция ПолучитьЗапросПоискаДублей(ЗапросСОтборомОбщим, выхТекстИдексов = "", выхПоискПоПохожимСловам = Неопределено)
	...
	Для Каждого ПолеСравнения Из мПоляСравнения Цикл
		...
		Тогда
			Продолжить;
		КонецЕсли;
		ИмяПоля = мПлатформа.ИдентификаторИзПредставленияЛкс(ПолноеИмяПоля);
...
Процедура ОткрытьСЗаполнениемГруппДублейПоТаблицеПар(Знач ТаблицаПар, Знач ИмяКолонкиТаблицыПарНеправильный, Знач ИмяКолонкиТаблицыПарПравильный) Экспорт
	...
	Если Ложь
		...
		Или Типы0[0] <> Типы1[0] 
	Тогда
		ирОбщий.СообщитьЛкс("Описания типов текущей и следующей колонок не совпадают или имеют более одного типа", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли; 
...
Процедура АвтозаменаЭлементов(ОбработатьНеправильные, НепосредственноеУдаление = Истина) Экспорт
	...
	ОбщиеПараметрыОбработки.Вставить("ОбработкаПоГруппам", Истина);
	...
	Для Каждого СтрокаПравильного Из ПравильныеСтроки Цикл
		...
		#КонецЕсли
		РезультатОбработки = ирОбщий.ПоискДублейИЗаменаСсылок_ВыполнитьЗаменуЛкс(ОбщиеПараметрыОбработки, СоответствиеЗамен, РезультатПоиска.ТаблицаСсылок, , ЭтаФорма,, "ВыполнитьЗаменуЗавершение");
...
Процедура КоманднаяПанельНастройкиОтчетаПоиск(Кнопка)
	...
	#КонецЕсли
	ирОбщий.ПоискДублейИЗаменаСсылок_НайтиДублиЛкс(ОбщиеПараметрыОбработки(), ирОбщий.ЗапросВСтруктуруЛкс(ЗапросПоискаДублей), ПоискПоПохожимСловам, ТекстИндексов, СтрокаПорядкаЦенности, ТекстУпорядочить, МоментНачала,,
		ЭтаФорма, Кнопка, "НайтиДублиЗавершение");
...
Процедура НайтиДублиЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		ЭтаФорма.ДлительностьПоследнегоПоиска = ТекущаяДата() - РезультатЗадания.МоментНачала;
		НастроитьЭлементыФормыПоРезультатуПоиска();
		ЭлементыФормы.Панель.ТекущаяСтраница = ЭлементыФормы.Панель.Страницы.НайденныеГруппы;
		ДобавитьПервуюГруппуДублей();
	КонецЕсли; 
...
Процедура КоманднаяПанельНайденныеСсылкиВыполнитьЗамену(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ВыполнитьЗаменуЗавершение();
	#КонецЕсли
	ирОбщий.ПоискДублейИЗаменаСсылок_ВыполнитьЗаменуЛкс(ОбщиеПараметрыОбработки(), СоответствиеЗамен, ЭтотОбъект.НайденныеСсылки.Выгрузить(Новый Структура("Пометка", Истина)),, ЭтаФорма,
		Кнопка, "ВыполнитьЗаменуЗавершение");
