...
Процедура НайтиДубли(Знач ЗапросПоискаДублей, Знач ПоискПоПохожимСловам = Ложь, Знач ТекстИндексов, Знач СтрокаПорядкаЦенности = "", Знач ТекстУпорядочить = "") Экспорт 
	
	Если ПоискПоПохожимСловам Тогда
		НайтиПоПохожимСловам(ЗапросПоискаДублей, ТекстИндексов, ТекстУпорядочить);
	Иначе
		НайтиПоРавенствуРеквизитов(ЗапросПоискаДублей, ТекстИндексов, СтрокаПорядкаЦенности);
	КонецЕсли;
...
Процедура НайтиПоПохожимСловам(ЗапросПоиска, ТекстИндексов, ТекстУпорядочить) Экспорт 
	...
	ИмяПоляНечеткогоСравнения = мПлатформа.ИдентификаторИзПредставленияЛкс(мПутьКДаннымПоляНечеткогоСравнения);
