...
Процедура КоманднаяПанельНайденныеОбъектыОбновить(Кнопка = Неопределено)
	...
	Для Каждого СтрокаТаблицыМетаОбъектов Из СтрокиМетаОбъектов Цикл
		...
		Если ирОбщий.ЛиКорневойТипОбъектаСПредопределеннымЛкс(Единственное) Тогда 
			ТекстЗапроса = "";
...
Процедура ОбновитьТипы()
	...
	Для Каждого СтрокаТипа Из Типы Цикл
		...
		Если Истина
			И (Ложь
				Или МенеджерМД.ПолучитьОбновлениеПредопределенныхДанных() = лОбновлениеПредопределенныхДанных.ОбновлятьАвтоматически
				Или ПланыОбмена.ГлавныйУзел() = Неопределено И МенеджерМД.ПолучитьОбновлениеПредопределенныхДанных() = лОбновлениеПредопределенныхДанных.Авто)
			И (Ложь
				Или ОбъектМД.ОбновлениеПредопределенныхДанных = Метаданные.СвойстваОбъектов.ОбновлениеПредопределенныхДанных.ОбновлятьАвтоматически
				Или ПланыОбмена.ГлавныйУзел() = Неопределено И ОбъектМД.ОбновлениеПредопределенныхДанных = Метаданные.СвойстваОбъектов.ОбновлениеПредопределенныхДанных.Авто)
		Тогда 
			СтрокаТипа.РежимОбновленияРезультирующий = "ОбновлятьАвтоматически";
		Иначе
			СтрокаТипа.РежимОбновленияРезультирующий = "НеОбновлятьАвтоматически";
		КонецЕсли; 
	КонецЦикла;
...
Процедура КП_СвязанныеДанныеМенеджерТабличногоПоля(Кнопка)
	 ...
	 ирОбщий.ПолучитьФормуЛкс("Обработка.ирМенеджерТабличногоПоля.Форма",, ЭтаФорма, ).УстановитьСвязь(ЭлементыФормы.Данные);
...
Процедура ТипыПриАктивизацииСтроки(Элемент)
	...
	ЭлементыФормы.Данные.Колонки.Ссылка.ЭлементУправления.ОграничениеТипа = Новый ОписаниеТипов(СтрЗаменить(ИмяТипаСсылки, ".", "Ссылка."));
	//Если ТекущийПредопределеный <> Неопределено Тогда
	//	ЭлементыФормы.Данные.ТекущаяСтрока = Данные.Найти(ТекущийПредопределеный, "ИмяПредопределенного");
	//КонецЕсли; 
...
Процедура КП_ДанныеОткрытьТаблицу(Кнопка)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(Данные.Выгрузить(), Ложь);
...
Процедура КП_ДанныеРедакторОбъектаБД(Кнопка)
	...
	Если ЭлементыФормы.Данные.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьСсылкуВРедактореОбъектаБДЛкс(ЭлементыФормы.Данные.ТекущаяСтрока.Ссылка);
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если Ложь
		Или мПлатформа.ИДВерсииПлатформы < "83" 
