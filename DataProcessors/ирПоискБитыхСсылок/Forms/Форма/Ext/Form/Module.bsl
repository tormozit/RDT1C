...
Процедура КоманднаяПанельБитыеСсылкиВыполнитьПоиск(Кнопка)
	...
	#КонецЕсли
	ирОбщий.ПоискБитыхСсылок_ВыполнитьПоискЛкс(ОбщиеПараметрыОбработки(), Истина, НачалаПоиска, СвязанныеДанные,,
		ЭтаФорма, Кнопка, "ВыполнитьПоискЗавершение");
...
Процедура ВыполнитьПоискЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		Сообщить("Поиск выполнен за " + XMLСтрока(ТекущаяДата() - РезультатЗадания.НачалаПоиска) + " сек");
