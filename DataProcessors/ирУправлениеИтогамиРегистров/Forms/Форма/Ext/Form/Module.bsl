...
Процедура ОбновитьДанныеПоРегистру(Знач СтрокаРегистра)
	...
	Иначе
		Если Не ирОбщий.РежимСовместимостиМеньше8_3_4Лкс() Тогда
			СтрокаРегистра.Агрегаты = МенеджерРегистра.ПолучитьИспользованиеАгрегатов();
		КонецЕсли; 
...
Процедура ВыполнитьКоманду(ВыделенныеСтроки, Команда, ПериодИтогов = Неопределено) 
...
Процедура КоманднаяПанельТаблицаРегистровВключитьИспользованиеИтогов(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ВключитьИспользованиеИтогов");
...
Процедура КоманднаяПанельТаблицаРегистровВыключитьИспользованиеИтогов(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ВыключитьИспользованиеИтогов");
...
Процедура КоманднаяПанельТаблицаРегистровВключитьТекущиеИтоги(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ВключитьТекущиеИтоги");
...
Процедура КоманднаяПанельТаблицаРегистровВыключитьТекущиеИтоги(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ВыключитьТекущиеИтоги");
...
Процедура КоманднаяПанельТаблицаРегистровВключитьРазделениеИтогов(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ВключитьРазделениеИтогов");
...
Процедура КоманднаяПанельТаблицаРегистровВыключитьРазделениеИтогов(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ВыключитьРазделениеИтогов");
...
Процедура КоманднаяПанельТаблицаРегистровУстановитьПериодИтогов(Кнопка)
	...
	Если Результат <> Неопределено Тогда
		ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "УстановитьГраницыИтогов", Результат);
	КонецЕсли;
...
Процедура КоманднаяПанельТаблицаРегистровПересчитатьИтоги(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ПересчитатьИтоги");
...
Процедура КоманднаяПанельТаблицаРегистровПересчитатьТекущиеИтоги(Кнопка)
	...
	ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ПересчитатьТекущиеИтоги");
...
Процедура КоманднаяПанельТаблицаРегистровПересчитатьИтогиЗаПериод(Кнопка)
	...
	Если НастройкаПериода.Редактировать() Тогда
		...
		ВыполнитьКоманду(ЭлементыФормы.ТаблицаРегистров.ВыделенныеСтроки, "ПересчитатьИтогиЗаПериод", СтруктураПериода);
	КонецЕсли;
...
Функция ЗапросСтатистикиПоПериодам(Знач ПолноеИмяРегистра = "", Знач НазначениеТаблицыИтогов = "")
	...
	ВсегоЗаписей = 0;
	Запрос = "Select Period, sum(TotalsRows) as TotalsRows, sum(DetailRows) as DetailRows 
	...
	Если РасчитатьСтатистику Тогда
		...
		Если мСоединениеADO <> Неопределено Тогда
			...
			Для Каждого СтрокаТаблицыИтогов Из ТаблицыИтоговРегистра Цикл
				...
				РезультатЗапроса = ВыполнитьЗапросADO(Запрос);
...
Функция ЗапросУдаленияНулевыхИтогов(Знач ПолноеИмяРегистра = "", Знач НазначениеТаблицыИтогов = "", выхБудетУдалено = Неопределено)
	...
	Если ЭлементыФормы.ПериодыИтогов.ТекущаяСтрока <> Неопределено Тогда
		...
		Запрос = Запрос + "
		|	AND _Period <= convert(DateTime, '" + СтрокаДаты + "', 101)";
	Иначе
