...
Процедура ТаблицаПараметровСеансаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ЛксОформитьЯчейкуСРасширеннымЗначением(ОформлениеСтроки.Ячейки.ЗначениеПараметраСеанса, ДанныеСтроки.Значение, Элемент.Колонки.ЗначениеПараметраСеанса);
	Если Ложь
...
Процедура КоманднаяПанель1Исследовать(Кнопка)
	...
	Обработки.ирИсследовательОбъектов.Создать().ИсследоватьКоллекцию(ТаблицаПараметровСеанса);
...
Процедура ТаблицаПараметровСеансаЗначениеПараметраСеансаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если МассивНередактируемыхТипов.Найти(ТипЗнч(ВыбраннаяСтрока.Значение)) <> Неопределено Тогда
		Исследовать(ВыбраннаяСтрока.Значение);
...
Процедура КоманднаяПанель1ЖурналРегистрации(Кнопка)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Обработки.ирАнализЖурналаРегистрации.Создать().ОткрытьСПараметром("Метаданные", "ПараметрСеанса." + ТекущаяСтрока.ИдентификаторПараметраСеанса);

КонецПроцедуры

Процедура ТаблицаПараметровСеансаЗначениеПараметраСеансаНачалоВыбора(Элемент, СтандартнаяОбработка)
	
	ЗначениеИзменено = ирНеглобальный.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ТаблицаПараметровСеанса, СтандартнаяОбработка,
