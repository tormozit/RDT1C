...
Процедура ПолучитьАктуальнуюВерсиюОтложенно()
	...
	Если ОтветHttp <> Неопределено Тогда
		...
		Для Каждого УзелВерсии Из УзлыВерсий Цикл
			...
			КонецЕсли; 
			Если ВерсияИзУзла = ирОбщий.ПолучитьПоследнийФрагментЛкс(ИспользуемаяВерсия, " ") И ЗначениеЗаполнено(ДатаВыпуска) Тогда 
				ЭтаФорма.ИспользуемаяВерсияДатаВыпуска = ДатаВыпуска;
			КонецЕсли; 
			Если ВерсияИзУзла = ирОбщий.ПолучитьПоследнийФрагментЛкс(АктуальнаяВерсия, " ") Тогда 
