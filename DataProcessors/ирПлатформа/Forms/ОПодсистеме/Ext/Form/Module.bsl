...
Процедура ОбновитьНажатие(Элемент)
	...
	ИначеЕсли ЭтотРасширение <> Неопределено Тогда 
		Каталог = ирОбщий.ВыбратьКаталогВФормеЛкс(,, "Выберите каталог для сохранения файла старой подсистемы");
		Если Не ЗначениеЗаполнено(Каталог) Тогда
			Возврат;
		КонецЕсли; 
		СтрокаОтвета = Соединение.Получить(Запрос);
		ДвоичныеДанные = СтрокаОтвета.ПолучитьТелоКакДвоичныеДанные();
		ИмяФайла = Каталог + "\Инструменты разработчика " + ИспользуемаяВерсия + ".cfe";
		#Если Сервер И Не Сервер Тогда
		    ЭтотРасширение = РасширенияКонфигурации.Создать();
		#КонецЕсли
		ЭтотРасширение.ПолучитьДанные().Записать(ИмяФайла);
		ЭтотРасширение.Записать(ДвоичныеДанные);
		Сообщить("Теперь выполните обновление конфигурации базы данных");
	Иначе
