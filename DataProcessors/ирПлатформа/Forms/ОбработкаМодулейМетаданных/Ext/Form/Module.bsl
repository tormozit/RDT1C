...
Процедура ВыполнитьЗамену(Знач РежимТеста)
	...
	Для Каждого ФайлМодуля Из ФайлыПапки Цикл
		...
		Если РежимТеста Тогда
			...
			ирОбщий.СравнитьЗначенияИнтерактивноЧерезXMLСтрокуЛкс(ТекстовыйДокумент.ПолучитьТекст(), ТекстМодуля,, "Оригинал", "Замена");
			Возврат;
		КонецЕсли; 
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирПлатформа.Форма.ОбработкаМодулейМетаданных");
Маска = "*";