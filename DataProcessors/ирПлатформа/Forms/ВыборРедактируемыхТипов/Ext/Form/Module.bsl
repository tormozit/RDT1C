...
Функция ОтобранныеСтрокиТаблицы() Экспорт
	...
	ВременныйПостроительЗапроса.ИсточникДанных = Новый ОписаниеИсточникаДанных(ТаблицаРедактируемыхТипов);
	ирОбщий.СкопироватьОтборПостроителяЛкс(ВременныйПостроительЗапроса.Отбор, ЭлементыФормы.ТаблицаРедактируемыхТипов.ОтборСтрок);
...
Функция ПомеченныеСтрокиТаблицы() Экспорт
	...
	Результат = ТаблицаРедактируемыхТипов.Выгрузить(Новый Структура("Пометка", Истина), "Имя");
	Возврат Результат;
...
Процедура КоманднаяПанельТиповУстановитьФлажки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.ТаблицаРедактируемыхТипов,, Истина,,, Ложь);
	
КонецПроцедуры

Процедура КоманднаяПанельТиповСнятьФлажки(Кнопка)
	
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.ТаблицаРедактируемыхТипов,, Ложь,,, Ложь);
...
Процедура ПриОткрытии()
	...
	Если МножественныйВыбор Тогда
		ЭлементыФормы.ТаблицаРедактируемыхТипов.РежимВыделения = РежимВыделенияТабличногоПоля.Множественный;
...
Процедура СброситьПометкиУПомеченных(ВременнаяТаблица = Неопределено, КромеТекущейСтроки = Ложь)
	...
	Для каждого ВременнаяСтрока Из ВременнаяТаблица Цикл
		...
		Если КромеТекущейСтроки И СтрокаТипа = ЭлементыФормы.ТаблицаРедактируемыхТипов.ТекущаяСтрока Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура УстановитьПометкуВТекущейСтроке(НоваяПометка = Истина)
	...
	ирОбщий.ТабличноеПоле_ИнтерактивноУстановитьПометкуТекущейСтрокиЛкс(ЭлементыФормы.ТаблицаРедактируемыхТипов,, НоваяПометка);
...
Процедура КоманднаяПанельТиповНавигационнаяСсылка(Кнопка)
	...
	Текст = "";
	ТекстИзБуфера = ирОбщий.ПолучитьТекстИзБуфераОбменаОСЛкс();
...
Процедура ТаблицаРедактируемыхТиповПриИзмененииФлажка(Элемент, Колонка)
	...
	// https://www.hostedredmine.com/issues/923113 Отключаем стандартный механизм симуляции интерактивной установки флажков,
	// чтобы при обработке изменения флажка отбор "В списке" с большим списком не съедал по 100мс 2 раза какие то непонятные действия при окончании редактирования строки и при оповещении о выборе значения
	//ирОбщий.ТабличноеПолеПриИзмененииФлажкаЛкс(Элемент, Колонка);
	ТекущаяСтрока = ЭлементыФормы.ТаблицаРедактируемыхТипов.ТекущаяСтрока;
	ТекущаяСтрока.Пометка = Не ТекущаяСтрока.Пометка;
	ТаблицаРедактируемыхТиповПометкаПриИзменении();
