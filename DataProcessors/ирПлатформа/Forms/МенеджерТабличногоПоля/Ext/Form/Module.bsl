...
Процедура НастроитьПостроительОтчета()
	...
	КолонкиТП = КолонкиТП();
	Для Каждого Колонка Из КолонкиТП Цикл
...
Процедура ЗначенияКолонокКолонкаПриИзменении(Элемент)
	...
	КолонкиТП = КолонкиТП();
	Колонка = КолонкиТП.Найти(ЭлементыФормы.ЗначенияКолонок.ТекущиеДанные.Колонка);
...
Процедура ОбработкаВыполнить(Кнопка)
	...
	ирОбщий.УстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗЛкс(СвязанноеТабличноеПоле, лЗначениеОбработки, ТипИсточника, Колонка, ТолькоВыделенныеСтроки, ИнтерактивноеУстановка, НаСервере);
