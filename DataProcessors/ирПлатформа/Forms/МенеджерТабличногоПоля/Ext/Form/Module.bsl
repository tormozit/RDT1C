...
Процедура КП_УстановитьЗначениеПолучитьИзТекущейЯчейки(Кнопка = Неопределено)
	...
	Если ЗначениеЗаполнено(ПолеВыбораКолонки) Тогда
		Если СвязанноеТабличноеПоле.ТекущиеДанные <> Неопределено Тогда
			ДанныеКолонки = СвязанноеТабличноеПоле.Колонки[ПолеВыбораКолонки].Данные;
			Если ЗначениеЗаполнено(ДанныеКолонки) Тогда
				ЗначениеОбработки = СвязанноеТабличноеПоле.ТекущиеДанные[ДанныеКолонки];
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ДействияФормыОткрытьДанные(Кнопка)
	...
	ирОбщий.ОткрытьЗначениеЛкс(СвязанноеТабличноеПоле.Значение, Ложь,,, Ложь,, СвязанноеТабличноеПоле);
