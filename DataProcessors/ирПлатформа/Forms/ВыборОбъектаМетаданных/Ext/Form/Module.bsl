...
Процедура мВыбрать()
	...
	Если Результат <> Неопределено Тогда
		ирНеглобальный.ПрименитьИзмененияИЗакрытьФормуЛкс(ЭтаФорма, Результат);
	КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	КонецЕсли; 
	ирНеглобальный.ТабличноеПоле_ОбновитьКолонкиИмяСинонимЛкс(ЭлементыФормы.ДеревоИсточников, РежимИмяСиноним);
	РежимВыбора = Истина;
	Если Истина
		И Не МодальныйРежим
...
Функция ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, ПолноеИмяТаблицы, Имя, Представление, ИмяТекущейКолонки, Подстроки) 
	...
	СтруктураСвойств = ЛксПолучитьСтруктуруСвойствОбъекта(ГлавнаяСтрока);
	СтруктураСвойств.Имя = Имя;
	СтруктураСвойств.Представление = Представление;
	Если Ложь
		Или Не ирНеглобальный.ЛиСтрокаСодержитВсеПодстрокиЛкс(СтруктураСвойств[ИмяТекущейКолонки], Подстроки) 
		Или (Истина
...
Процедура ЗаполнитьСтрокуДерева(СтрокаДерева)
	...
	Тогда
		...
		ирНеглобальный.УстановитьПометкиРодителейЛкс(СтрокаДерева); // Неоптимально
	КонецЕсли; 
...
Процедура ЗаполнитьДеревоИсточников(Знач Фильтр = Неопределено, МассивДоступныхОбъектов = Неопределено) Экспорт
	...
	КонецЕсли; 
	ПодстрокиФильтра = ЛксПолучитьМассивИзСтрокиСРазделителем(НРег(Фильтр), " ", Истина);
	ТекущаяКолонкаТП = ирНеглобальный.ОпределитьВедущуюСтроковуюКолонкуТабличногоПоляЛкс(ТабличноеПолеДерева);
	...
	Для Каждого КорневойТип Из КоллекцияКорневыхТипов Цикл
		...
		Для Каждого МетаИсточник Из КоллекцияМетаданных Цикл
			...
			Если Ложь
				...
				Или (Истина
					И мОтображатьРегистры <> Истина
					И ирНеглобальный.ЛиРегистровыйОбъектМетаданных(МетаИсточник))
			Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ГлавнаяСтрока.Строки.Количество() = 0 Тогда
				Если Ложь
					Или Не ирНеглобальный.ЛиСтрокаСодержитВсеПодстрокиЛкс(ГлавнаяСтрока[ИмяТекущейКолонки], ПодстрокиФильтра) 
					Или (Истина
						И мДоступныеОбъекты <> Неопределено
		...
		Если Истина
			...
			//И Не ирНеглобальный.ЛиСтрокаСодержитВсеПодстрокиЛкс(НовыйИсточник[ИмяТекущейКолонки], ПодстрокиФильтра) 
		Тогда
			ДеревоИсточников.Строки.Удалить(НовыйИсточник);
		КонецЕсли; 
	КонецЦикла;
	Если мОтображатьКоличество = Истина Тогда
		ирНеглобальный.ОбновитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(ДеревоИсточников);
	КонецЕсли;
	...
	ирНеглобальный.ТабличноеПолеДеревоЗначений_АвтоРазвернутьВсеСтрокиЛкс(ТабличноеПолеДерева, , ТекущаяСтрокаУстановлена);
	Если мРезультатПакетаКоличестваСтрок <> Неопределено Тогда
		ирНеглобальный.ЗаполнитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(ДеревоИсточников, мРезультатПакетаКоличестваСтрок);
	КонецЕсли; 
...
Процедура УстановитьФлажокСтроки(ТекущаяСтрока, НовоеЗначениеПометки)
	...
	Если КлючСовпадает Тогда
		...
		ирНеглобальный.УстановитьПометкиРодителейЛкс(ТекущаяСтрока);
...
Функция ПроверитьУстановитьФильтрПоТипуТаблицы(ТекущаяСтрока)
	...
	Если мТипТаблицы = Неопределено Тогда
		мТипТаблицы = ирНеглобальный.ПолучитьТипТаблицыБДЛкс(ТекущаяСтрока.ПолноеИмяОбъекта);
		мСтруктураКлючаТаблицы = ирНеглобальный.ПолучитьСтруктуруКлючаТаблицыБДЛкс(ТекущаяСтрока.ПолноеИмяОбъекта);
	КонецЕсли; 
...
Функция ЛиКлючТаблицыПодходит(СтрокаДерева)
	...
	Если мТипТаблицы <> Неопределено Тогда
		КлючСовпадает = Ложь;
		Если мТипТаблицы = ирНеглобальный.ПолучитьТипТаблицыБДЛкс(СтрокаДерева.ПолноеИмяОбъекта) Тогда
			КлючСовпадает = Истина;
			СтруктураКлючаТаблицы = ирНеглобальный.ПолучитьСтруктуруКлючаТаблицыБДЛкс(СтрокаДерева.ПолноеИмяОбъекта);
...
Процедура ДеревоИсточниковПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирНеглобальный.ТабличноеПоле_ОформитьЯчейкиИмяСинонимЛкс(Элемент, ОформлениеСтроки,,,, ?(мМножественныйВыбор, "Пометка", ""));
...
Процедура ДействияФормыОбновить(Кнопка)
	...
	мРезультатПакетаКоличестваСтрок = ирНеглобальный.ВычислитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(ДеревоИсточников);
	ирНеглобальный.ЗаполнитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(ДеревоИсточников, мРезультатПакетаКоличестваСтрок);
...
Процедура ДействияФормыФормаСписка(Кнопка)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		ТипТаблицы = ирНеглобальный.ПолучитьТипТаблицыБДЛкс(ТекущаяСтрока.ПолноеИмяОбъекта);
		Если Ложь
...
Процедура ДействияФормыСнятьФлажки(Кнопка)
	...
	ирНеглобальный.УстановитьЗначениеКолонкиДереваЛкс(ДеревоИсточников,, Ложь);
	мТипТаблицы = Неопределено;
...
Процедура ДействияФормыОтборПоПодсистеме(Кнопка)
	...
	//ФормаВыбора = ирНеглобальный.ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ВыборПодсистемы");
...
Процедура ФильтрИменПриИзменении(Элемент)
	...
	СтандартнаяОбработка = Ложь;
	//ирНеглобальный.НайтиСтрокуТабличногоПоляДереваЗначенийСоСложнымФильтромЛкс(ЭлементыФормы.ДеревоИсточников, ЭлементыФормы.ФильтрИмен);
	ЗаполнитьДеревоИсточников();
	ирНеглобальный.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, "ВыборОбъектаМетаданных");

КонецПроцедуры

Процедура ФильтрИменНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	
	ирНеглобальный.ПолеВводаСИсториейВыбора_НачалоВыбораИзСпискаЛкс(Элемент, "ВыборОбъектаМетаданных");
...
Процедура ФильтрИменАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	//ирНеглобальный.НайтиСтрокуТабличногоПоляДереваЗначенийСоСложнымФильтромЛкс(ЭлементыФормы.ДеревоИсточников, ЭлементыФормы.ФильтрИмен, Текст);
