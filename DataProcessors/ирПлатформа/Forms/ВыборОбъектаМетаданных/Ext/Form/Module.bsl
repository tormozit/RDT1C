...
Процедура УстановитьГраницыВыделенияФильтра()
	...
	//ЭлементыФормы.ФильтрИмен.УстановитьГраницыВыделения(СтрДлина(ФильтрИмен), СтрДлина(ФильтрИмен)); // Так ставится на предпоследний символ почему то
...
Процедура ЗаполнитьДеревоИсточников(Знач Фильтр = Неопределено, МассивДоступныхОбъектов = Неопределено, Принудительно = Ложь) Экспорт
	...
	Если Истина 
		...
		И ирКэш.НомерРежимаСовместимостиЛкс() >= 802014 
	Тогда
		Если Ложь
		...
		Для Каждого МетаИсточник Из КоллекцияМетаданных Цикл
			...
			Если Ложь
				...
				Или (Истина
					...
					И ОбъектыВыбранныхПодсистем[МетаИсточник] = Неопределено)
			Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ВычислятьФункциональныеОпции Тогда
				...
				ирОбщий.ФункциональныеОпцииОбъектаМДЛкс(МетаИсточник, ЗначенияФункОпций, СписокФункОпций, ФункциональныеОпцииВключены);
			...
			КонецЕсли;
			Попытка
				ГлавнаяСтрока.ПравоПросмотр = ПравоДоступа("Просмотр", МетаИсточник);
			Исключение
				...
				Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(КорневойТип) Тогда
					...
					Для Каждого КлючИЗначение Из СтруктураТЧ Цикл
						...
						Если СтрокаТЧВДереве = Неопределено Тогда
							Продолжить;
						КонецЕсли; 
						МетаТЧ = МетаИсточник.ТабличныеЧасти.Найти(КлючИЗначение.Ключ);
			...
			Если мОтображатьТаблицыИзменений = Истина Тогда
				Если ирОбщий.ЕстьТаблицаИзмененийОбъектаМетаданных(МетаИсточник) Тогда
					ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, ирОбщий.ПолучитьИмяТаблицыИзМетаданныхЛкс(ГлавнаяСтрока.ПолноеИмяОбъекта, Истина, Ложь), "Изменения", "Изменения", ИмяТекущейКолонки, ПодстрокиФильтра);
				КонецЕсли;
			КонецЕсли;
			Если мОтображатьВиртуальныеТаблицы = Истина Тогда
				Если КорневойТип = "РегистрСведений" Тогда 
					Если МетаИсточник.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
						ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяСрезПоследних, МетаИсточник.Имя + ".СрезПоследних",
							МетаИсточник.Представление() + ": срез последних", ИмяТекущейКолонки, ПодстрокиФильтра); 
						ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяСрезПервых, МетаИсточник.Имя + ".СрезПервых",
							МетаИсточник.Представление() + ": срез первых", ИмяТекущейКолонки, ПодстрокиФильтра); 
					КонецЕсли;
				ИначеЕсли КорневойТип = "РегистрНакопления" Тогда 
					ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяОбороты, МетаИсточник.Имя + ".Обороты",
					...
					Если МетаИсточник.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Остатки Тогда
						ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяОстатки, МетаИсточник.Имя + ".Остатки",
							МетаИсточник.Представление() + ": Остатки", ИмяТекущейКолонки, ПодстрокиФильтра);  
						ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяОстаткиИОбороты, МетаИсточник.Имя + ".ОстаткиИОбороты",
				...
				ИначеЕсли КорневойТип = "РегистрБухгалтерии" Тогда 
					...
					Если МетаИсточник.Корреспонденция Тогда
						ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяОборотыДтКт, МетаИсточник.Имя + ".ОборотыДтКт",
							МетаИсточник.Представление() + ": Обороты Дт Кт", ИмяТекущейКолонки, ПодстрокиФильтра);  
					КонецЕсли;
					ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяДвиженияССубконто, МетаИсточник.Имя + ".ДвиженияССубконто",
						МетаИсточник.Представление() + ": Движения с субконто", ИмяТекущейКолонки, ПодстрокиФильтра);  
					ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяСубконто, МетаИсточник.Имя + ".Субконто",
				...
				ИначеЕсли КорневойТип = "Последовательность" Тогда 
					ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, МетаИсточник.ПолноеИмя() + "." + ИмяГраницы, МетаИсточник.Имя + ".Границы",
...
Процедура ДеревоИсточниковПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если ДанныеСтроки.Уровень() > 0 Тогда
		...
		Если ДанныеСтроки.ФункциональныеОпцииНеВключены Или Не ДанныеСтроки.ПравоПросмотр Тогда 
			ОформлениеСтроки.Ячейки.Имя.ЦветТекста = WebЦвета.Красный;
			ОформлениеСтроки.Ячейки.Представление.ЦветТекста = WebЦвета.Красный;
		КонецЕсли; 
	КонецЕсли; 
