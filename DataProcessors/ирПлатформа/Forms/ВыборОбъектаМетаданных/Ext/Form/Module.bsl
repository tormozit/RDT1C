...
Процедура ЗаполнитьДеревоИсточников(Знач Фильтр = Неопределено, МассивДоступныхОбъектов = Неопределено, Принудительно = Ложь) Экспорт
	...
	Для Каждого КорневойТип Из КоллекцияКорневыхТипов Цикл
		...
		Если СтрокаКорневогоТипа = Неопределено Тогда
			...
			ПредставлениеКатегории = ирОбщий.ПредставлениеИзИдентификатораЛкс(СтрокаКорневогоТипа.Множественное) + "." + ОбъектМДКорневогоТипа.Представление();
		...
		НовыйИсточник.ЕстьДоступ = ЕстьДоступ;
		Для Каждого ОбъектМД Из КоллекцияМетаОбъектов Цикл
			...
			ГлавнаяСтрока.ПолноеИмяОбъекта = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ПолноеИмяМД,, Ложь);
			...
			Если мОтображатьТаблицыИзменений = Истина Тогда
				Если ирОбщий.ЕстьТаблицаИзмененийОбъектаМетаданных(ОбъектМД) Тогда
					ДобавитьСтрокуТабличнойЧасти(ГлавнаяСтрока, ирКэш.ИмяТаблицыИзМетаданныхЛкс(ГлавнаяСтрока.ПолноеИмяОбъекта, Истина, Ложь), "Изменения", "Изменения", ИмяТекущейКолонки, ПодстрокиФильтра);
				КонецЕсли;
			КонецЕсли;
...
мСтрокаТипаТабличнойЧасти = ПолучитьСтрокуТипаМетаОбъектов("ТабличнаяЧасть");
мСтрокаТипаВнешнегоИсточникаДанных = ПолучитьСтрокуТипаМетаОбъектов("ВнешнийИсточникДанных");
