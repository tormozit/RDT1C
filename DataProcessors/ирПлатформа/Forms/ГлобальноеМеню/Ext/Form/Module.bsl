...
Процедура ОбновитьДоступныеКоманды()
	...
	ОбщийТипДанныхТаблицы = ирОбщий.ОбщийТипДанныхТабличногоПоляЛкс(ТекущийЭлементАктивнойФормы);
	Если ОбщийТипДанныхТаблицы = "Список" Тогда
		ИмяОсновнойТаблицы = ирОбщий.ИмяТаблицыБДДинамическогоСпискаЛкс(ТекущийЭлементАктивнойФормы);
	КонецЕсли; 
	...
	Тогда
		Если Истина
			И (Ложь
				...
				Или ИмяОсновнойТаблицы <> Неопределено)
			И (Ложь
				Или ТипЗнч(ТекущийЭлементАктивнойФормы) = Тип("ТаблицаФормы") 
				Или ТипЗнч(ТекущийЭлементАктивнойФормы) = Тип("ТабличноеПоле"))
		Тогда
			...
			НоваяКоманда.Картинка = ирКэш.КартинкаПоИмениЛкс("ирСумма");
			НоваяКоманда.Параметр = "таблица";
...
Процедура ОсновныеДействияФормыОК(Кнопка = Неопределено)
	...
	ирОбщий.ПодключитьГлобальныйОбработчикОжиданияСПараметрамиЛкс(ЭлементыФормы.Команды.ТекущаяСтрока.Имя, Новый Структура("Форма", АктивнаяФорма));
...
Процедура ОсновныеДействияФормыПереоткрытьОтложенно(Кнопка)
