...
Процедура ОчиститьТекстОтложенно() Экспорт 
	...
	Если УстановитьПрикрепление Тогда 
		ЭтаФорма.Открыть();
		ирОбщий.УстановитьПрикреплениеФормыВУправляемомПриложенииЛкс(ЭтаФорма);
		УстановитьПрикрепление = Ложь;
		//ЭтаФорма.Высота = 1; // Не работает
	Иначе
		// Активируем предыдущую активную форму
		ОткрытыеФормы = ирКэш.ОчередьОбновленияФормЛкс().ОткрытыеФормы;
		#Если Сервер И Не Сервер Тогда
			ОткрытыеФормы = Новый Массив;
		#КонецЕсли
		Если ОткрытыеФормы.Количество() > 0 Тогда
			ОткрытыеФормы[ОткрытыеФормы.ВГраница()].Открыть();
		КонецЕсли; 
	КонецЕсли; 
