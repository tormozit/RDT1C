...
Функция НайтиСтрокуИзИсточника(Знач СтрокаИсточника, Знач ТабличноеПоле = Неопределено, КоллекцияИсточник = Неопределено) Экспорт 
...
Процедура ПриОткрытии()
	...
	КонецЕсли; 
	ЭтаФорма.Заголовок = "" + ТипЗнч(ЭлементыФормы.ПолеТаблицы.Значение) + ": " + НовыйЗаголовок;
...
Процедура КоманднаяПанельТаблицаПередатьВТаблицуБД(Кнопка)
	...
	#КонецЕсли
	Форма = ЗагрузкаТабличныхДанных.ПолучитьФорму();
	Форма.ТаблицаЗначений = ирОбщий.СузитьТипыКолонокТаблицыБезПотериДанныхЛкс(ЭлементыФормы.ПолеТаблицы.Значение.Скопировать());
...
Процедура КоманднаяПанельТаблицаАлгоритмСоздания(Кнопка)
	...
	Для Каждого СтрокаКолонки Из ТаблицаКолонок Цикл
		...
		Если ТипЗначения.СодержитТип(Тип("Строка")) Тогда
			ТекстКвалификаторыСтроки = "Новый КвалификаторыСтроки(" + ТипЗначения.КвалификаторыСтроки.Длина + ", " + ПолучитьПолноеИмяПредопределенногоЗначения(ТипЗначения.КвалификаторыСтроки.ДопустимаяДлина) + ")";
		КонецЕсли; 
