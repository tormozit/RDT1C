...
Процедура ОсновныеДействияФормыРедактироватьКопию(Кнопка)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ПолучитьРезультат().Скопировать(),,,, Ложь);
...
Функция МакетВТаблицуЗначений(ИмяФайла) Экспорт
	...
	Если Макет.ВысотаТаблицы > 0 Тогда
		...
		Если МассивИменКолонок.Количество() = Макет.ШиринаТаблицы Тогда
			Для Каждого ИмяКолонки Из МассивИменКолонок Цикл
				ТЗ.Колонки.Добавить(ирКэш.Получить().ПолучитьИдентификаторИзПредставления(ИмяКолонки),, ИмяКолонки);
			КонецЦикла;
		КонецЕсли; 
	КонецЕсли; 
	Если ТЗ.Колонки.Количество() = 0 Тогда
		Для Кол=1 По Макет.ШиринаТаблицы Цикл
			ТЗ.Колонки.Добавить("Колонка"+ЧислоВСтрокуПодробно(Кол));
		КонецЦикла;
	КонецЕсли; 
	Для Стр=1 По Макет.ВысотаТаблицы Цикл
		...
		ПустаяСтрока = Истина;
		Для Кол=1 По Макет.ШиринаТаблицы Цикл
			Ячейка=Макет.Область(Стр, Кол);
			Если Ячейка.СодержитЗначение Тогда
				Зн=Ячейка.Значение;
			Иначе
				Зн=СокрЛП(Ячейка.Текст);
			КонецЕсли;
	...
	ЭлементыФормы.ПолеТаблицы.Значение.Колонки.Очистить();
	ЭлементыФормы.ПолеТаблицы.Колонки.Очистить();
	ТаблицаКолонок.Очистить();
	Для каждого Колонка из ТЗ.Колонки Цикл
		...
		ТипЗначения = Новый ОписаниеТипов(МассивТипов,,,, Квалификаторы);
