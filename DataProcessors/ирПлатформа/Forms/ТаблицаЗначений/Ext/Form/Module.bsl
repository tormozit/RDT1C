...
Процедура УстановитьРедактируемоеЗначение(НовоеЗначение, ТабличноеПоле = Неопределено) Экспорт 
	...
	Если ТабличноеПоле <> Неопределено Тогда
		...
		Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда
			НоваяТекущаяСтрока = НайтиСтрокуИзИсточника(ТабличноеПоле.ТекущаяСтрока, ТабличноеПоле);
			Если НоваяТекущаяСтрока <> Неопределено Тогда
		...
		Для Каждого ВыделеннаяСтрока Из ТабличноеПоле.ВыделенныеСтроки  Цикл
			НоваяВыделеннаяСтрока = НайтиСтрокуИзИсточника(ВыделеннаяСтрока, ТабличноеПоле);
			Если НоваяВыделеннаяСтрока <> Неопределено Тогда
				ЭлементыФормы.ПолеТаблицы.ВыделенныеСтроки.Добавить(НоваяВыделеннаяСтрока);
			КонецЕсли;
		КонецЦикла;
...
Функция НайтиСтрокуИзИсточника(СтрокаИсточника, Знач ТабличноеПоле)
	...
	Если КоллекцияИсточник = Неопределено Тогда
		...
		КонецЕсли; 
	ИначеЕсли ТипЗнч(КоллекцияИсточник) = Тип("ДанныеФормыКоллекция") Тогда
	...
	ИначеЕсли ТипЗнч(КоллекцияИсточник) = Тип("ДанныеФормыДерево") Тогда
		//
	ИначеЕсли ТипЗнч(КоллекцияИсточник) = Тип("ДеревоЗначений") Тогда
		КоординатыВДереве = ирОбщий.ПолучитьКоординатыСтрокиДереваЛкс(СтрокаИсточника);
		НоваяСтрока = ирОбщий.ПолучитьСтрокуДереваПоКоординатамЛкс(КоллекцияИсточник, КоординатыВДереве);
...
Процедура КоманднаяПанельТаблицаГруппировкаТаблицы(Кнопка)
	...
	МассивВыделенныхСтрок = ирОбщий.МассивВыделенныхСтрокТабличногоПоляЛкс(ЭлементыФормы.ТаблицаКолонок);
