...
Процедура ПриОткрытии()
	...
	КонецЕсли; 
	ЭлементыФормы.ПанельОсновная.Страницы.XML.Доступность = ирКэш.НомерВерсииПлатформыЛкс() >= 803018;
...
Функция РедакторXML()
	Возврат ЭлементыФормы.XML.Документ.defaultView;
КонецФункции

Процедура XMLДокументСформирован(Элемент)
	
	Инфо = Новый СистемнаяИнформация();
	РедакторXML().init(Инфо.ВерсияПриложения);
	РедакторXML().minimap(Ложь);
	РедакторXML().switchXMLMode();
	РедакторXML().enableModificationEvent(Истина);
	ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.ТекущаяСтраница; // Вызовет ПриСменеСтраницы
	//РедакторXML().updateText(ПолучитьТекст());
