...
Процедура ИндикацияТекущегоЭлемента()
	...
	ИначеЕсли ТекущаяСтрока.Тип = Тип("КолонкаТабличногоПоля") Тогда
		...
		ЭлементФормыСтрокиДерева(СтрокаДереваТабличногоПоля).ТекущаяКолонка = ЭлементФормы;
...
Процедура ИндикацияЭлементаФормыСЗаголовком(Знач ТекущаяСтрока, СтруктураСвойств = Неопределено, ИмяСвойстваЗаголовок = "Заголовок")
	...
	Иначе
		...
		КонецЦикла; 
		СохранитьСтарыеСвойстваЭлементаФормы(ТекущаяСтрока, ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(ИменаСвойств));
...
Процедура ОбновитьДерево()
	...
	Иначе
		...
		#КонецЕсли
		СписокИнструментов = ирПлатформа.ПолучитьСписокИнструментов();
	...
	Иначе
		...
		Пока Истина Цикл
			...
			Если УзелЭлемента = Неопределено Тогда
				Прервать;
			КонецЕсли; 
			ИмяГрафическогоЭлемента = Вычислить(УзелЭлемента.ТекстовоеСодержимое);
			Если Истина
			...
			СтрокаЭлемента.Индекс = УзелЭлемента.ПредыдущийСоседний.ТекстовоеСодержимое;
			СтрокаЭлемента.Имя = ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяГрафическогоЭлемента, "#");
		КонецЦикла; 
		...
		Пока Истина Цикл
			...
			Для Каждого СтрокаЭлемента Из ПодходящиеСтрокиЭлементов Цикл
				...
				Если БазовыйУзел.ДочерниеУзлы.Количество() > 4 Тогда
					ЗаголовокЭлемента = СобратьМногострочнуюСтроку(БазовыйУзел, 4);
				КонецЕсли; 
				Если Не ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
					...
					Если УзелЗаголовка <> Неопределено Тогда
						ЗаголовокЭлемента = СобратьМногострочнуюСтроку(УзелЗаголовка, 1);
					КонецЕсли; 
				КонецЕсли;
				...
				Если УзелПодсказки <> Неопределено Тогда
					ОписаниеЭлемента.Подсказка = СобратьМногострочнуюСтроку(УзелПодсказки, 1);
				КонецЕсли; 
				Если Ложь
		...
		Для Каждого ЭлементФормы Из Форма.Элементы Цикл
			...
			Если ТипЗнч(ЭлементФормы) = Тип("ТаблицаФормы") Тогда
				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					Пока Истина Цикл
						...
						Если УзелЗаголовка <> Неопределено Тогда
							ЗаголовокКолонки = СобратьМногострочнуюСтроку(УзелЗаголовка, 1);
						Иначе
							ЗаголовокКолонки = "";
						КонецЕсли; 
						...
						УзелПодсказки = ИтераторЗаголовка.ПолучитьСледующий();
						Если УзелПодсказки <> Неопределено Тогда
							ПодсказкаКолонки = СобратьМногострочнуюСтроку(УзелПодсказки, 1);
						Иначе
							ПодсказкаКолонки = "";
						КонецЕсли; 
			...
			ИначеЕсли ТипЗнч(ЭлементФормы) = Тип("ГруппаФормы") И ЭлементФормы.Вид = ВидГруппыФормы.КоманднаяПанель Тогда
				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					ИндексПодменюЕще = 0;
					//ИндексКорняПанели = 4; // Индекс - 4 - отображаемый набор кнопок командной панели
					...
					КонецЦикла;
					ТочкаОстанова = 0;
					Для Каждого ОписаниеКнопки Из ГруппыКнопок.Найти(ИндексПодменюЕще, "Индекс").Кнопки Цикл
...
Функция СобратьМногострочнуюСтроку(Знач БазовыйУзел, ИндексУзла)
	
	Результат = "";
	ТекстУзла = БазовыйУзел.ДочерниеУзлы[ИндексУзла].ТекстовоеСодержимое;
	Если Лев(ТекстУзла, 1) = """" Тогда 
		Результат = ТекстУзла;
		Пока Истина Цикл
			Если Прав(Результат, 1) = """" И СтрЧислоВхождений(Результат, """") % 2 = 0 Тогда
				Прервать;
			КонецЕсли; 
			ИндексУзла = ИндексУзла + 1;
			Результат = Результат + БазовыйУзел.ДочерниеУзлы[ИндексУзла].ТекстовоеСодержимое;
		КонецЦикла; 
...
Процедура ДобавитьЗависимуюКоманднуюПанель(Знач СтрокаДЗ, Знач ЭлементФормы, Знач НаборыКнопок, ИмяСвойства = "КоманднаяПанель")
	...
	СтрокаДочерняяСтрока.Текст = ирОбщий.ПолучитьПредставлениеИзИдентификатораЛкс(ИмяСвойства);
...
Процедура ДобавитьГруппуКолонокФормы(СтрокаДЗ, ГруппаТаблицыФормы, СвойстваКолонок, КоличествоВидимыхДобавлено = 0, Группировка = Неопределено)
	...
	Для Каждого ПодчиненныйЭлемент Из ГруппаТаблицыФормы.ПодчиненныеЭлементы Цикл
		...
		Если ПодчиненныйЭлемент.Доступность И ПодчиненныйЭлемент.ОтображатьВШапке Тогда
			...
			Если ТаблицаФормы.ТекущийЭлемент <> ПодчиненныйЭлемент Тогда
				ТаблицаФормы.ТекущийЭлемент = СтарыйТекущийЭлемент; // Антибаг платформы 8.3.15 После присвоения Неопределено свойству ТекущийЭлемент таблицы формы можно присвоить колонку с отключенной пользовательской видимостью
	...
	Для Каждого ПодчиненныйЭлемент Из ГруппаТаблицыФормы.ПодчиненныеЭлементы Цикл
		...
		Если ТипЗнч(ПодчиненныйЭлемент) = Тип("ГруппаФормы") Тогда
			...
			Если ПодчиненныйЭлемент.Вид = ВидГруппыФормы.Подменю Тогда
				...
				ПервоеСловоЗаголовка = СтрДлина(ирОбщий.ПолучитьПервыйФрагментЛкс(ЭффективныеСвойстваКнопки.Заголовок, " "));
...
Процедура ЗаполнитьСтрокуЭлементаУправляемойФормы(СтрокаДерева, Роль = "", ЭффективнаяВидимость = Неопределено)
	...
	Если ЭффективнаяВидимость <> Неопределено Тогда
		СтрокаДерева.Видимость = ЭффективнаяВидимость;
	КонецЕсли; 
	...
	Если Не ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
		...
		Если Не ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
			...
			КонецЕсли; 
			ЗаголовокЭлемента = ирОбщий.ПолучитьПредставлениеИзИдентификатораЛкс(ЗаголовокЭлемента);
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ЗаполнитьСтрокиИзКнопок(Кнопки, СтрокиДерева)
	...
	Для Каждого Кнопка Из Кнопки Цикл
		...
		//	//}
		//	СтрокаВнутр = ирОбщий.ПолучитьПоследнийФрагментЛкс(СтрокаВнутр, "},");
		//	Фрагменты = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(СтрокаВнутр, ",");
...
Процедура ДеревоПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если РезультирующиеПризнаки.ТолькоВЕще Тогда
