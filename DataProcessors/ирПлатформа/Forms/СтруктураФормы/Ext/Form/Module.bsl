				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					Пока Истина Цикл
						...
						Пока Истина Цикл
							...
							КонецЕсли;
							ИндексУзла = 7;
							ЗаголовокКнопки = СобратьМногострочнуюСтроку(УзелКнопки, ИндексУзла);
							ИндексУзла = ИндексУзла + 1;
							ПодсказкиКнопки = СобратьМногострочнуюСтроку(УзелКнопки, ИндексУзла);
...
Процедура ДеревоПриАктивизацииСтроки(Элемент)
	...
	ВосстановитьСвойстваЭлементовФормы();
	ЭтаФорма.ПутьВДереве = ирОбщий.Дерево_ПутьСтрокойЛкс(Элемент.ТекущаяСтрока, "Текст",, " \ ");
...
Процедура ДеревоВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.Дерево.Колонки.Подсказка Тогда
		ОткрытьТекстПодсказки(ВыбраннаяСтрока);
	КонецЕсли; 
	
КонецПроцедуры

Процедура ОткрытьТекстПодсказки(Знач ВыбраннаяСтрока)
	
	Если ЗначениеЗаполнено(ВыбраннаяСтрока.Подсказка) Тогда
		ирОбщий.ОткрытьТекстЛкс(ВыбраннаяСтрока.Подсказка,,, Истина, ВыбраннаяСтрока);
	КонецЕсли;
...
Процедура КоманднаяПанельДереваСсылкаНаМодуль(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(Ссылка,,,,,, Истина);
...
Процедура КоманднаяПанельДереваХранилищеНастроекФормы(Кнопка)
	...
	Если ТипЗнч(Форма) = Тип("Форма") Тогда
		ФормаРедактора.ЭлементыФормы.ОписаниеНастроек.ОтборСтрок.КлючНастроек.Значение = Дерево.Строки[0].Имя;
	Иначе
...
Процедура КоманднаяПанельДереваВывестиСтроки(Кнопка)
	
	ирОбщий.ВывестиСтрокиТабличногоПоляИПоказатьЛкс(ЭлементыФормы.Дерево);
...
Процедура ПодсказкаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЭлементыФормы.Дерево.ТекущаяСтрока = Неопределено Тогда
		Возврат
	КонецЕсли; 
	ОткрытьТекстПодсказки(ЭлементыФормы.Дерево.ТекущаяСтрока);
