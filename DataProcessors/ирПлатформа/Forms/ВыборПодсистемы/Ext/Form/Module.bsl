...
Процедура ФильтроватьПодсистемыПоОбъектуМетаданных(СтрокиДереваПодсистем)
	...
	Для сч = 1 По КоличествоСтрокДереваПодсистем Цикл
		...
		МДПодсистема = ирКэш.ОбъектМДПоПолномуИмениЛкс(ПодсистемаПеревод + "." + СтрЗаменить(ДочерняяПодсистема.ПолноеИмя, ".", "." + ПодсистемаПеревод + "."));
...
Процедура ДеревоПодсистемПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирОбщий.ТабличноеПолеОформитьЯчейкиИмяСинонимЛкс(Элемент, ОформлениеСтроки,,, "", ?(РежимВыбора И МножественныйВыбор, "Пометка", ""));
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПлатформа.Форма.ВыборПодсистемы");
