...
Функция СохранитьВФайл(ПолноеИмяФайла, Знач СразуОткрыть = Ложь)
	...
	Если ирОбщий.СтрокиРавныЛкс(Файл.Расширение, ".xls") Тогда
		...
		Если УстанавливатьПризнакСодержитЗначение Тогда
			УстановитьПризнакСодержитЗначение();
		КонецЕсли; 
	...
	КонецЕсли; 
	Попытка
		ЭлементыФормы.ПолеТабличногоДокумента.Записать(ПолноеИмяФайла, ТипФайла);
	Исключение
		Сообщить(ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
...
Процедура УстановитьПризнакСодержитЗначение()
	...
	Для ТекущаяКолонка = 1 По ЭлементыФормы.ПолеТабличногоДокумента.ШиринаТаблицы Цикл
		// Переберем строки, которые будут использованы для расчета ширин колонок
		Для ТекущаяСтрока = 1 По ЭлементыФормы.ПолеТабличногоДокумента.ВысотаТаблицы Цикл
			ОбластьЯчейки = ЭлементыФормы.ПолеТабличногоДокумента.Область(ТекущаяСтрока, ТекущаяКолонка);
...
Процедура УстанавливатьПризнакСодержитЗначениеПриИзменении(Элемент)
