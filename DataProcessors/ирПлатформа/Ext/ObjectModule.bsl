...
Функция ЗарегистрироватьПолучитьCOMОбъект(КлассКомпоненты, ПолноеИмяDll, Административная = Ложь, ИмяТипаВК = Ложь, ИмяМакетаКомпоненты = "") Экспорт
	...
	//Иначе
		ТекстСообщения = "Не удалось зарегистривать COM" + ТипCOMКомпоненты + "-компоненту" + КлассКомпоненты;
		Если Ложь
			Или ирКэш.ВКОбщая().IsAdmin() 
			Или (Истина
				И ЕстьАдминистративныеПраваУУчетнойЗаписиОС() 
				И Административная) 
		Тогда
			ТекстСообщения = ТекстСообщения + " от имени администратора!"
		Иначе
			ТекстСообщения = ТекстСообщения + ". Запустите приложение от имени администратора или используйте инструмент ""Административная регистрация COM компонент"" для ее регистрации"
		КонецЕсли; 
	//КонецЕсли; 
