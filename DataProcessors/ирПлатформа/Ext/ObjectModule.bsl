		...
		Тогда
			...
			Если ТипМетаданныхРодителя = Тип("ОбъектМетаданных") Тогда
				...
				Иначе
					...
					ПостроительЗапроса = Новый Структура("ДоступныеПоля", ТаблицаРезультата.Колонки);
				КонецЕсли; 
			Иначе
...
Функция СвойстваТипаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Знач ТаблицаСвойств = Неопределено) Экспорт 
	...
	Если Ложь
		...
		Или ЯзыкПрограммы = 2
	Тогда
		ВычислятьТипыВСписке = Неопределено;
		Если Истина
		...
		Тогда
			...
			Для Каждого МетаОбщийМодуль Из Метаданные.ОбщиеМодули Цикл
				...
				ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСвойств, МодульМетаданных, ВычислятьТипыВСписке);
			КонецЦикла;
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
	Знач МодульМетаданных = Неопределено, ВычислятьТипыВСписке = Неопределено) Экспорт 
	...
	Если МодульМетаданных <> Неопределено Тогда
		...
		Тогда
			...
			Если Не ЗначениеЗаполнено(Слово) Тогда
				...
				Если ВычислятьТипыВСписке = Неопределено Тогда
...
Функция СвойстваТипаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			КонецЕсли; 
			ТаблицаСтруктурТипов = Неопределено;
			Если ВычислятьТипы Или ЯзыкПрограммы <> 0 Тогда
				ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
...
Функция ПолучитьТаблицуСловСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, ВычислятьТипы = Истина,
	...
	ИнициализацияОписанияМетодовИСвойств();
	ВнутренняяТаблицаСлов = СвойстваТипаПредопределенные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ЛиСерверныйКонтекст);
	СвойстваТипаМетаданные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы,, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ЛиСерверныйКонтекст,
		МодульМетаданных, ВнутренняяТаблицаСлов);
	Возврат ВнутренняяТаблицаСлов;
...
Функция ПрочитатьИндексАрхиваСинтаксПомощника(ИмяАрхива) Экспорт 
	...
	#КонецЕсли
	Результат = Неопределено;
	ФайлИндекса = Новый Файл(ПапкаКэшаСинтаксПомощника.ПолноеИмя + ирОбщий.РазделительПутиКФайлуЛкс() + ИмяАрхива + ".idx");
...
Процедура ПодготовитьПакер(Знач Каталог)
	...
	Иначе
		...
		КонецЕсли; 
	КонецЕсли;
	ПолноеИмяФайла = Каталог + ?(Прав(Разделитель, 1) = Разделитель, "", Разделитель) + ИмяФайлаПакера;
