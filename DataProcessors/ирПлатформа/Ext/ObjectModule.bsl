	...
	Тогда
		...
		Если ТипМодуля <> Неопределено Тогда
			...
			Если ФайлМетодовМодуляСуществует Тогда
				...
				Если МетодыМодуля.Колонки.Найти("Параметры") = Неопределено Тогда
					ФайлМетодовМодуляСуществует = Ложь;
				КонецЕсли; 
			КонецЕсли;
			Если Ложь
				...
				Если Не ФайлМодуля.Существует() Тогда
					Если СообщениеОПустомКэшеМодулейВыводилось <> Истина Тогда
						...
						Если ФайлыМодулей.Количество() = 0 Тогда
							ирОбщий.СообщитьЛкс("Для отображения экспортных методов, определенных в конфигурации, нужно обновлять папку кэша модулей через форму настроек контекстной подсказки.");
			...
			Если МетодыМодуля <> Неопределено Тогда
				...
				Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
					...
					ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
					СтруктураТипа = ПолучитьНовуюСтруктуруТипа();
					СтруктураТипа.СтрокаОписания = Новый Структура("СтрокаСозданияРодителя, МетодМодуля", СтрокаСозданияРодителя, МетодМодуля);
					ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипа);
		...
		Если Вхождение.SubMatches(0) <> Неопределено Тогда
			...
			Если не ПустаяСтрока(СтрокаПараметров) Тогда
				...
				Если РезультатПараметров.Count > 0 Тогда
					...
					ТаблицаПараметры.Колонки.Добавить("Знач");
					СтрокаМетода.Параметры = ТаблицаПараметры;
				КонецЕсли;	
				Для Каждого ВхождениеПараметра Из РезультатПараметров Цикл
					Если Вхождение.SubMatches(1) <> Неопределено Тогда
						СтрокаПраметра = ТаблицаПараметры.Добавить();
						СтрокаПраметра.Параметр = ВхождениеПараметра.SubMatches(1);
