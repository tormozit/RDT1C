...
Перем мПроверкаКомпиляцииОбщихМодулейВыполнялась Экспорт;
...
Функция НайтиВозможныеИменаВременныхТаблиц(ТекстЗапроса) Экспорт
	...
	RegExp.Global = Истина;
	RegExp.pattern = "(?:ИЗ|СОЕДИНЕНИЕ|УНИЧТОЖИТЬ|,)\s+(" + шИмя + ")(?:\s|$|[^\(\._\d" + шБуква + "])";
...
Функция ПолучитьМассивСоздаваемыхВременныхТаблицПакета(ТекстПакета, ТолькоТребующиеУничтоженияНаВходе = Ложь) Экспорт 
	...
	ШаблонПОМЕСТИТЬИЗ = "(?:" + шСтрокаЗапроса + ")|(?:" + шРазделитель + "+" + "(?:(ПОМЕСТИТЬ|INTO\s+TABLE|INTO)|(УНИЧТОЖИТЬ|DROP|DROP\s+TABLE))" + шРазделитель 
		+ "+(" + шИмя + ")(" + шРазделитель + "|;)|(?:" + шРазделитель + "))";
...
//
Функция ПолучитьТаблицуСловСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, ВычислятьТипы = Истина) Экспорт 
...
Процедура СохранитьНастройкиАлгоритмов() Экспорт 
	...
	ХранилищеОбщихНастроек.Сохранить("ИнструментыРазработчикаTormozit", "ФиксироватьВнешниеИсключенияАлгоритмов", ФиксироватьВнешниеИсключенияАлгоритмов);
	//СохранитьЗначение("ирПлатформа.СинтаксическийКонтрольПередЗаписью", СинтаксическийКонтрольПередЗаписью);
	ХранилищеОбщихНастроек.Сохранить("ИнструментыРазработчикаTormozit", "СинтаксическийКонтрольПередЗаписью", СинтаксическийКонтрольПередЗаписью);
	//СохранитьЗначение("ирПлатформа.ВыполнятьАлгоритмыЧерезВнешниеОбработки", ВыполнятьАлгоритмыЧерезВнешниеОбработки);
	ХранилищеОбщихНастроек.Сохранить("ИнструментыРазработчикаTormozit", "ВыполнятьАлгоритмыЧерезВнешниеОбработки", ВыполнятьАлгоритмыЧерезВнешниеОбработки);
	ХранилищеОбщихНастроек.Сохранить("ИнструментыРазработчикаTormozit", "ФайловыйКэшАлгоритмовДопускаетРедактирование", ФайловыйКэшАлгоритмовДопускаетРедактирование);
	...
	ХранилищеОбщихНастроек.Сохранить("ИнструментыРазработчикаTormozit", "ПоказыватьВнешниеИсключенияПриВыполненииАлгоритмов", ПоказыватьВнешниеИсключенияПриВыполненииАлгоритмов);
	...
	Если ЛиКомпонентаFormsTextBoxДоступна = Неопределено Тогда
		...
		Исключение
			...
			Если Пустышка = Неопределено Тогда
				...
				|2. Зарегистрировать библиотеки FM20.dll и FM20ENU.dll из состава MS Office 97-2007. Это можно сделать с помощью формы ""Административная регистрация COM компонент"" из состава подсистемы");
...
Функция ЗарегистрироватьПолучитьCOMОбъект(КлассКомпоненты, ПолноеИмяDll, Административная = Ложь, ИмяТипаВК = Ложь, ИмяМакетаКомпоненты = "") Экспорт
	...
	Иначе
		Если ЕстьАдминистративныеПраваУУчетнойЗаписиОС() Тогда
			Сообщить("Не удалось зарегистривать COM" + ТипCOMКомпоненты + "-компоненту " + КлассКомпоненты + ". Используйте инструмент ""Административная регистрация COM компонент"" для ее регистрации", СтатусСообщения.Внимание);
		Иначе
			Сообщить("Не удалось зарегистривать COM" + ТипCOMКомпоненты + "-компоненту " + КлассКомпоненты + ". Запустите приложение от имени администратора", СтатусСообщения.Внимание);
		КонецЕсли; 
	КонецЕсли; 
	Возврат Неопределено;
			...
			Если КлассКомпоненты = ИМЯ_КЛАССА_DynamicWrapperX Тогда
				...
				Попытка
					//Если ирКэш.Это64битныйПроцессЛкс() Тогда
						...
						Пустышка = Компонента.MemRead(0,0,0); // версия 2.1
					//Иначе
...
Функция ПолучитьТекущееВремяВМиллисекундах() Экспорт
	...
	КонецЕсли;
	Если Результат = Неопределено Тогда
		Если ИДВерсииПлатформы >= "83" Тогда
...
Процедура ОбработатьВыводОшибкиИнформатора()
	...
	Сообщить("Отключите опцию ""Низкоуровневое получение описаний объектов"" в общих настройках, либо выполните административную регистрацию COM компонент");
...
КонецЕсли; 
ФиксироватьВнешниеИсключенияАлгоритмов = ХранилищеОбщихНастроек.Загрузить("ИнструментыРазработчикаTormozit", "ФиксироватьВнешниеИсключенияАлгоритмов");
Если ФиксироватьВнешниеИсключенияАлгоритмов = Неопределено Тогда
	ФиксироватьВнешниеИсключенияАлгоритмов = Истина;
КонецЕсли;
ФайловыйКэшАлгоритмовДопускаетРедактирование = ХранилищеОбщихНастроек.Загрузить("ИнструментыРазработчикаTormozit", "ФайловыйКэшАлгоритмовДопускаетРедактирование");
Если ФайловыйКэшАлгоритмовДопускаетРедактирование = Неопределено Тогда
	ФайловыйКэшАлгоритмовДопускаетРедактирование = Ложь;
КонецЕсли;
ВыполнятьАлгоритмыЧерезВнешниеОбработки = ХранилищеОбщихНастроек.Загрузить("ИнструментыРазработчикаTormozit", "ВыполнятьАлгоритмыЧерезВнешниеОбработки");
Если ВыполнятьАлгоритмыЧерезВнешниеОбработки = Неопределено Тогда
	...
	ВыполнятьАлгоритмыЧерезВнешниеОбработки = Ложь;
КонецЕсли;
ЛиНизкоуровневоеПолучениеОписанийОбъектов = ХранилищеОбщихНастроек.Загрузить("ИнструментыРазработчикаTormozit", "ЛиНизкоуровневоеПолучениеОписанийОбъектов");
Если ЛиНизкоуровневоеПолучениеОписанийОбъектов = Неопределено Тогда
	ЛиНизкоуровневоеПолучениеОписанийОбъектов = Истина;
КонецЕсли;
АвторегистрацияComКомпонент = ХранилищеОбщихНастроек.Загрузить("ИнструментыРазработчикаTormozit", "АвторегистрацияComКомпонент");
