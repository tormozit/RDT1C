...
Функция ЗарегистрироватьПолучитьCOMОбъект(КлассКомпоненты, ПолноеИмяDll, Административная = Ложь, ИмяТипаВК = Ложь) Экспорт
	...
	Если Не Административная Тогда
		...
		ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(КоманднаяСтрока, ФайлКомпоненты.Путь, Истина,,, Истина);
	...
	ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(КоманднаяСтрока, , Истина,,, Истина);
	Компонента = ПолучитьПроверитьCOMОбъект(КлассКомпоненты, ИмяТипаВК);
	Если Компонента <> Неопределено Тогда 
...
//
Процедура ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Знач СтрокаЗапуска, Знач ТекущийКаталог = "", ИспользоватьWSH = Истина,
	...
	//	Иначе
			...
			Если Элевация Тогда
				КонечнаяСтрока = КонечнаяСтрока + " /elevate";
			КонецЕсли; 
			//КонечнаяСтрока = КонечнаяСтрока + " /TEST"; // для отладки
			КонечнаяСтрока = КонечнаяСтрока + " /nowindow /wait /silent /D=""" + ТекущийКаталог + """ """ + СтрокаЗапуска + """";
...
Функция ПолучитьТаблицуСвойствОбъектаИнформатором(Объект, ТипСлов = "Свойство", НеопределеноСчитатьГлобальнымКонтекстом = Истина) Экспорт
	...
	Перем ТЗ;
	Если Ложь
		Или ВерсияПлатформы >= 803009 // Почти для каждого нового релиза платформы информатор должен 
