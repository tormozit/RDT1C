...
Функция ПолучитьПроверитьCOMОбъект(Знач КлассКомпоненты, ИмяТипаВК = "") Экспорт
	...
	Иначе
		...
		Если Компонента = Неопределено Тогда
			Если КлассКомпоненты = "DynamicWrapperX" Тогда
				ИМЯ_КЛАССА_DynamicWrapperX = "DynamicWrapperX.2";
...
Процедура ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Знач СтрокаЗапуска, Знач ТекущийКаталог = "", Знач ИспользоватьWSH = Истина,
	...
	Если Не ФайлИсполнителя.Существует() Тогда
		Сообщить("Не удалось запустить утилиту hstart для скрытого запуска приложений. Попробуйте отключить антивирусное ПО");
	...
	Иначе
		...
		Если Результат = Неопределено Тогда
			Если ПустоеЗначение = Null Тогда
				Результат = "<Null>";
			ИначеЕсли ПустоеЗначение = Ложь Тогда
...
Процедура ОбработатьВыводОшибкиИнформатора()
	...
	Сообщить("Отключите опцию ""Низкоуровневое получение описаний объектов"" в настройках компоненты ""Контекстная подсказка"", либо выполните административную регистрацию COM компонент");
...
#Если Клиент Тогда
АвторегистрацияComКомпонент = ВосстановитьЗначение("ирПлатформа.АвторегистрацияComКомпонент");
#КонецЕсли 
