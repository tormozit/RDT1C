...
Функция ПолучитьШаблоныДляАнализаВстроенногоЯзыка() Экспорт
	...
	//шБлокКомментариев = "(?:(?:\t| )*//[^\n]*\n)+(?:(?:\t| )*(?:\r|$))+";
	//шБлокКомментариев =   "(?:(?:\t| )*//[^\n]*[\n$])+";
	шБлокКомментариев =   "(?:(?:\t| )*//[^\n]*)+"; // 18.11.2020 Не поглощаем последний конец строки, чтобы корректно парсился модуль с последней строкой-комментарием
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Иначе
			...
			Если Слово <> Неопределено Тогда
				...
				Если Не СтруктураКлюча.Свойство("ТипСлова") Тогда
