...
Функция ЗарегистрироватьПолучитьCOMОбъект(КлассКомпоненты, ПолноеИмяDll, Административная = Ложь, ИмяТипаВК = Ложь) Экспорт
	...
	Если ирКэш.Это64битныйПроцессЛкс() Тогда
		Сообщить("Не удалось загрузить COM-компоненту " + КлассКомпоненты + ", т.к. в 64-битном процессе нельзя использовать 32-битную компоненту", СтатусСообщения.Внимание);
	Иначе
		Если ЕстьАдминистративныеПраваУУчетнойЗаписиОС() Тогда
			Сообщить("Не удалось зарегистривать COM-компоненту " + КлассКомпоненты + ". Используйте инструмент ""Административная регистрация COM компонент"" для ее регистрации", СтатусСообщения.Внимание);
		Иначе
			Сообщить("Не удалось зарегистривать COM-компоненту " + КлассКомпоненты + ". Запустите приложение от имени администратора", СтатусСообщения.Внимание);
		КонецЕсли; 
	КонецЕсли; 
	Возврат Неопределено;
...
Функция ЕстьАдминистративныеПраваУУчетнойЗаписиОС() Экспорт 
	...
	РезультатКоманды = ирКэш.Получить().ПолучитьТекстРезультатаКомандыСистемы(КомандаСистемы);
