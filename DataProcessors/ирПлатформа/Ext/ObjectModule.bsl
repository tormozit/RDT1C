				...
				Если Не ФайлМодуля.Существует() Тогда
					Если МодулиМетаданных.Количество() = 0 И СообщениеОНеобходимостиОбновитьКэшМодулейВыводилось <> Истина Тогда
						...
						Если ФайлыМодулей.Количество() = 0 Тогда
							ирОбщий.СообщитьЛкс("Для получения экспортных методов и переменных модулей нужно обновлять кэш модулей через форму настроек контекстной подсказки.");
							Форма = ирОбщий.ПолучитьФормуЛкс("Обработка.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой.Форма.ФормаНастройки");
							Форма.Открыть();
						КонецЕсли; 
...
// Перем шИмя, шРазделитель, шКомментарий, шСтруктураКомментарияПарам, шСтруктураКомментарияВозвр, шДирективаПрепроцессора, шВозврат, шСтруктураКомментарияОписаниеПараметров, шКомментарийМетода, шОписаниеПеременной, шСтрокаПеременныхБезКомментов, шСтрокаПеременных, шСтрокаПрограммы, шПараметрыМетода, шБлокКомментариев, шМетодЧистый, шМетод1, шМетод, шСтруктураМодуля, шЭлементОбластиМетодов;
// {Шаблон.Начало}
...
// + <шБлокКомментариев> = (?:(?:\t| )*<шКомментарий>)+
// + <шМетодЧистый> = ((Процедура|Функция)<шРазделитель>*(<шИмя>)<шРазделитель>*\(([^\)]*)\)(<шРазделитель>*Экспорт)?)((?:(?:<шСтрокаПрограммы>|<шБлокКомментариев>|\.Конец(?:Процедуры|Функции)|\r|\n|.)*?))[^а-яё_a-z0-9\."]Конец(?:Процедуры|Функции)(?:<шКомментарий>|[^а-яё_a-z0-9\."]|$)
// - <шМетодЧистый> = ((Процедура|Функция)<шРазделитель>*(<шИмя>)<шРазделитель>*\(([^\)]*)\)(<шРазделитель>*Экспорт)?)((?:(?:<шСтрокаПрограммы>|<шБлокКомментариев>|\.Конец(?:Процедуры|Функции)|\r|\n|.)*?))(?:[^а-яё_a-z0-9\."]Конец(?:Процедуры|Функции)|(?:\t| )*<шКомментарий>Конец(?:Процедуры|Функции))(?:<шКомментарий>|[^а-яё_a-z0-9\."]|$)
// + <шМетод1> = (?:^|\n\s*)<шМетодЧистый>
// + <шМетод> = (?:(<шБлокКомментариев>)|\n|^)(?:\t| )*<шМетодЧистый>
// + <шСтруктураМодуля> = ^((?:<шРазделитель><шДирективаПрепроцессора>|<шСтрокаПеременныхБезКомментов>|<шРазделитель>)*\n)?((?:<шРазделитель><шДирективаПрепроцессора>|<шМетод1>|<шРазделитель>)*)((?:\r|\n|.)*)$
// + <шЭлементОбластиМетодов> = <шМетод>|(<шБлокКомментариев>)|(<шДирективаПрепроцессора>)
// {Шаблон.Конец}
шИмя = "[_ЁА-ЯA-Z][_ЁА-ЯA-Z\d]*";
шРазделитель = "(?://[^\n]*\n|\s|^|$)";
шКомментарий = "//[^\n]*[\n$]";
...
шПараметрыМетода = "" + шРазделитель + "*(Знач\s)?" + шРазделитель + "*(" + шИмя + ")" + шРазделитель + "*=?((?:" + шРазделитель + "*|(" + шСтрокаПрограммы + ")|(?:[^,\n$]*))+)(?:,|$)";
шБлокКомментариев = "(?:(?:\t| )*" + шКомментарий + ")+";
// Быстрый вариант, но не находит конец у метода с комментарием в последней строке
шМетодЧистый = "((Процедура|Функция)" + шРазделитель + "*(" + шИмя + ")" + шРазделитель + "*\(([^\)]*)\)(" + шРазделитель + "*Экспорт)?)((?:(?:" + шСтрокаПрограммы + "|" + шБлокКомментариев + "|\.Конец(?:Процедуры|Функции)|\r|\n|.)*?))[^а-яё_a-z0-9\.""]Конец(?:Процедуры|Функции)(?:" + шКомментарий + "|[^а-яё_a-z0-9\.""]|$)";
шМетод1 = "(?:^|\n\s*)" + шМетодЧистый + "";
шМетод = "(?:(" + шБлокКомментариев + ")|\n|^)(?:\t| )*" + шМетодЧистый + "";
// Если после последнего слова КонецПроцедуры или КонецФункции сразу расположен конец, то этот блок попадает в раздел ОсновнаяПрограмма
...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Результат.Вставить("ОписаниеФункцииПроцедуры", шМетод);
	...
	Результат.Вставить("КомментарийОписаниеМетода", шКомментарийМетода);
...
Функция ПодготовитьТипРезультатаМетода(СтрокаМетода, МодульМетаданных, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено) Экспорт 
	...
	Если СтрокаМетода.Тип = "Функция" И СтрокаМетода.ТаблицаСтруктурТипов = Неопределено Тогда
		...
		Если кэшКомпонентаПодсказки = Неопределено Тогда
			кэшКомпонентаПодсказки = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой");
			#Если Сервер И Не Сервер Тогда
...
КонецФункции // СтруктураТипаИзЗначения()
...
Функция БазовыйФайлРедактораJSON() Экспорт 
	Если БазовыйФайлРедактораJSON = Неопределено Тогда
		...
		БазовыйФайлРедактораJSON = БазовыйКаталог + КаталогКомпоненты + "\index.html";
	КонецЕсли; 
	...
	Тогда
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Тогда
				...
				Если Истина
					И ирОбщий.СтрокиРавныЛкс(ПроцессОС.Name, "1cv8.exe")
