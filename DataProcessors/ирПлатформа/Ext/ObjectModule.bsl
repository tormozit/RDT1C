...
Функция ДоступныеЗначенияТипа(Знач ИмяТипаЗначения) Экспорт 
	...
	СтрокиЗначений = ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", "ПеречислениеМетаданных: " + ИмяТипаЗначения, "Свойство"));
	Если СтрокиЗначений.Количество() = 0 Тогда
		СтрокиЗначений = ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", "Перечисление" + ИмяТипаЗначения, "Свойство"));
	КонецЕсли; 
...
Функция ДеревоОбъектовМД() Экспорт
	...
	КонецЕсли; 
	мДеревоОбъектовМД = Новый ДеревоЗначений;
	мДеревоОбъектовМД.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка"));
	мДеревоОбъектовМД.Колонки.Добавить("Представление", Новый ОписаниеТипов("Строка"));
	мДеревоОбъектовМД.Колонки.Добавить("ПолноеИмяОбъекта", Новый ОписаниеТипов("Строка"));
	мДеревоОбъектовМД.Колонки.Добавить("ИндексКартинки", Новый ОписаниеТипов("Число"));
	мДеревоОбъектовМД.Колонки.Добавить("ЕстьДоступ", Новый ОписаниеТипов("Булево"));
	мДеревоОбъектовМД.Колонки.Добавить("ПравоПросмотр", Новый ОписаниеТипов("Булево"));
	мДеревоОбъектовМД.Колонки.Добавить("ТипТаблицы", Новый ОписаниеТипов("Строка"));
	...
	Для Каждого КорневойТип Из КоллекцияКорневыхТипов Цикл
		...
		Если КоллекцияМетаОбъектов.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		СтрокаТипаМетаданных = мДеревоОбъектовМД.Строки.Добавить();
		...
		Если СтрокаТипаМетаданных.Строки.Количество() = 0 Тогда
			мДеревоОбъектовМД.Строки.Удалить(СтрокаТипаМетаданных);
		КонецЕсли; 
	КонецЦикла;
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
...
Функция ТаблицаВсехТаблицБД() Экспорт 
	...
	//		ПолноеИмяТаблицыБД = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ПолноеИмяМД,, Ложь);
	//		Если ЕстьТаблицаБД Тогда
	//			СтрокаОсновнойТаблицы = ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяТаблицыБД, ПолноеИмяМД, ТипТаблицы, ОбъектМД.Имя,
	//				ПредставлениеМД, СхемаТаблиц, , ОбъектМД);
	...
	//				Для Каждого КлючИЗначение Из СтруктураТЧ Цикл
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + КлючИЗначение.Ключ, ПолноеИмяМД + "." + ИмяТабличнаяЧасть + "." + КлючИЗначение.Ключ, "ТабличнаяЧасть", ,
	//						ПредставлениеМД + "." + КлючИЗначение.Значение,,, ОбъектМД);
	//				КонецЦикла;
	//				Если КорневойТип = "БизнесПроцесс" Тогда 
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяТочки,, "Точки", , ПредставлениеМД + "." + "Точки",,, ОбъектМД);
	//				КонецЕсли; 
	//				Если КорневойТип = "Задача" Тогда 
	//					Если ОбъектМД.Адресация <> Неопределено Тогда
	//						ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяЗадачиПоИсполнителю,, "ВиртуальнаяТаблица", ,
	//							ПредставлениеМД + "." + "Задачи по исполнителю",,, ОбъектМД, 1);
	//					КонецЕсли; 
	//				КонецЕсли; 
	//			КонецЕсли; 
	//			Если КорневойТип = "РегистрСведений" Тогда 
	//				Если ОбъектМД.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяСрезПоследних,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Срез последних",,, ОбъектМД, 1);
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяСрезПервых,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Срез первых",,, ОбъектМД, 1);
	//				КонецЕсли;
	//			ИначеЕсли КорневойТип = "РегистрНакопления" Тогда 
	//				ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОбороты,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Обороты",,, ОбъектМД, 3);
	//				Если ОбъектМД.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Остатки Тогда
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОстатки,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Остатки",,, ОбъектМД, 1);
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОстаткиИОбороты,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Остатки и обороты",,, ОбъектМД, 4);
	//				КонецЕсли;
	//			ИначеЕсли КорневойТип = "РегистрБухгалтерии" Тогда 
	//				ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОбороты,,           "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Обороты",,, ОбъектМД, 5);
	//				ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОстатки,,           "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Остатки",,, ОбъектМД, 3);
	//				ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОстаткиИОбороты,,   "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Остатки и обороты",,, ОбъектМД, 6);
	//				ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяДвиженияССубконто,, "ДвиженияССубконто", , ПредставлениеМД + "." + "Движения с субконто",,, ОбъектМД, 2);
	//				Если ОбъектМД.ПланСчетов.МаксКоличествоСубконто > 0 Тогда
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяСубконто,, "Субконто", , ПредставлениеМД + "." + "Субконто",,, ОбъектМД);
	//				КонецЕсли; 
	//				Если ОбъектМД.Корреспонденция Тогда
	//					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяОборотыДтКт,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Обороты Дт Кт",,, ОбъектМД, 7);
	//				КонецЕсли; 
	...
	//				//            КонецЦикла;
	//			ИначеЕсли КорневойТип = "Последовательность" Тогда 
	//				ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ПолноеИмяМД + "." + ИмяГраницы,, "ВиртуальнаяТаблица", , ПредставлениеМД + "." + "Границы",,, ОбъектМД);
	//			КонецЕсли;
	//		КонецЕсли; 
	//		Если ирОбщий.ЕстьТаблицаИзмененийОбъектаМетаданных(ОбъектМД) Тогда
	//			ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(мТаблицаВсехТаблицБД, ирКэш.ИмяТаблицыИзМетаданныхЛкс(ПолноеИмяМД, Истина, Ложь), , "Изменения", ОбъектМД.Имя, ПредставлениеМД + "." + "Изменения",,, ОбъектМД);
	//		КонецЕсли;
	//	КонецЦикла;
	//	ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
	//КонецЦикла;
	//ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
	//мТаблицаВсехТаблицБД.Индексы.Добавить("НПолноеИмя");
	//мТаблицаВсехТаблицБД.Сортировать("НПолноеИмя");
	//Возврат мТаблицаВсехТаблицБД;
