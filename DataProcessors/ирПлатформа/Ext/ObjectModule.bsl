...
Функция СвойстваТипаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Если Ложь
		...
		Или ЯзыкПрограммы = 2
	Тогда
		ГлубокоВычислятьТипыВСпискеАвтодополнения = Неопределено;
		Если Истина
		...
		Тогда
			...
			Для Каждого МетаОбщийМодуль Из Метаданные.ОбщиеМодули Цикл
				...
				ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСвойств, МодульМетаданных, ГлубокоВычислятьТипыВСпискеАвтодополнения);
			КонецЦикла;
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
	Знач МодульМетаданных = Неопределено, ГлубокоВычислятьТипыВСпискеАвтодополнения = Неопределено) Экспорт 
	...
	Если МодульМетаданных <> Неопределено Тогда
		...
		Тогда
			...
			Если Не ЗначениеЗаполнено(Слово) Тогда
				...
				Если ГлубокоВычислятьТипыВСпискеАвтодополнения = Неопределено Тогда
					ГлубокоВычислятьТипыВСпискеАвтодополнения = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".ГлубокоВычислятьТипыВСпискеАвтодополнения") = Истина;
				КонецЕсли;
			Иначе
				ГлубокоВычислятьТипыВСпискеАвтодополнения = Ложь;
			КонецЕсли; 
			Если Истина
			...
			Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
				...
				Если Ложь
					...
					Или ГлубокоВычислятьТипыВСпискеАвтодополнения
				Тогда
...
Функция БазовыйФайлРедактораКода() Экспорт 
	Если БазовыйФайлРедактораКода = Неопределено Тогда
		...
		ТекстФайла = ирОбщий.СтрЗаменитьЛкс(ТекстФайла, "customOptions: true",
			//", automaticLayout: true" // https://github.com/salexdv/bsl_console/issues/185
			, Истина);
...
//
Функция ПолучитьТаблицуШаблоновТекста(ИмяКласса, _мСообщенияЧерезПредупреждения, ПринудительноОбновить = Ложь) Экспорт 
