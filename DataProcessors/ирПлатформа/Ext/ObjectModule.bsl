	...
	Если СтрокаКорневогоТипа <> Неопределено Тогда
		...
		#Если Клиент Тогда
			Если ЛиДобавлятьКартинки Тогда 
				...
				Картинка = ирНеглобальный.ПолучитьКартинкуКорневогоТипаЛкс(КорневойТип);
			КонецЕсли;
		#КонецЕсли
	Иначе
...
Функция ЗамаскироватьВременныеТаблицы(ОбъектЗапроса, Знач ТекстЗапроса = Неопределено,
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ИмяВременнойТаблицы = ЛксПолучитьСтрокуМеждуМаркерами(ИнформацияОбОшибке.Причина.Описание, "Таблица не найдена """, """", Ложь);
			...
			//	ИмяВременнойТаблицы = ЛксПолучитьСтрокуМеждуМаркерами(ИнформацияОбОшибке.Причина.Описание, "Неверные параметры """, """", Ложь);
			//	Если ЗначениеЗаполнено(ИмяВременнойТаблицы) Тогда
			//		Попытка
...
Функция ДобавитьПредложениеПоместитьВЗапрос(ТекстЗапроса, ИмяВременнойТаблицы) Экспорт 
	...
	Для Каждого Match Из Результат Цикл
		Если Ложь
			...
			Или Не ирНеглобальный.ЛиПустаяПодгруппаRegExpЛкс(Match.SubMatches(0))
			Или Не ирНеглобальный.ЛиПустаяПодгруппаRegExpЛкс(Match.SubMatches(3))
		Тогда
...
Функция ПреобразоватьЗапросВПодзапрос(Текст, ТекстВыбранныхПолей = "*", ПсевдонимВложенногоЗапроса = "ВложенныйЗапрос",
	...
	Результат = "ВЫБРАТЬ";
	Если Не ирНеглобальный.ЛиПустаяПодгруппаRegExpЛкс(РезультатПоиска.SubMatches(1)) Тогда
	...
	Если ЗначениеЗаполнено(ТекстДоПоследнегоУПОРЯДОЧИТЬ) Тогда
		ТекстДоПоследнегоУПОРЯДОЧИТЬ = ЛксПолучитьСтрокуБезКонца(ТекстДоПоследнегоУПОРЯДОЧИТЬ, СтрДлина(РезультатПоиска.SubMatches(3)));
...
Функция ПолучитьГрубоКоличествоСтрокВРезультатеЗапроса(ЗапросИлиПостроитель, ЛиЗамерВремени = Ложь, МодальныйРежим = Ложь, ИмяЗапроса = "") Экспорт
	...
	#Если Клиент Тогда
		...
		Тогда
			ЛксСообщитьСУчетомМодальности("Время формирования предварительного результата """ + ИмяЗапроса + """ - " 
...
Функция ПолучитьМассивСоздаваемыхВременныхТаблицПакета(ТекстПакета) Экспорт 
	...
	Для Каждого ТекстЗапроса Из МассивТекстовЗапросов Цикл
		...
		Для Каждого Match2 Из Результат2 Цикл
			Если Не ирНеглобальный.ЛиПустаяПодгруппаRegExpЛкс(Match2.SubMatches(0)) Тогда
...
Функция ВыбратьСтрокуОписанияИзМассиваСтруктурТипов(СтруктураЦикла, ВключатьПутьКОписаниюТипаЗначения = Ложь, ФормаВладелец = Неопределено, Слово = "",
	...
	Если СчетчикСтрокВторогоУровня > 0 Тогда
		ФормаВыбора = ПолучитьФорму("Обработка.ирСинтаксПомощник.Форма");
...
Функция ПолучитьПостроительЗапросаПоСтруктуреТипа(СтрокаОбщегоТипа, ВиртуальнаяТаблица = Неопределено)
	...
	Иначе
		ПолноеИмя = ЛксПолучитьКорневойТипКонфигурации(КэшОбъект(МетаданныеРодителя).ПолноеИмя);
	КонецЕсли; 
		...
		Тогда
			Если ТипМетаданныхРодителя = Тип("ОбъектМетаданных") Тогда
				...
				Иначе
					...
					//КонецЕсли;
					ИмяТаблицы = ирНеглобальный.ПолучитьИмяТаблицыИзМетаданныхЛкс(ПолноеИмя, Ложь, ВиртуальнаяТаблица = Неопределено); // Добавлено 14.04.2012
...
Функция ПолучитьВнутреннююТаблицуМетаданныхСлов(РодительскаяСтруктураТипа, Слово = Неопределено, ТипСлова = Неопределено,
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицуОбщихТиповСтруктурыТипа Цикл
		...
		Если ирНеглобальный.СтрокиРавныЛкс(СтрокаОбщегоТипа.ИмяОбщегоТипа, "COMОбъект.{WbemScripting.SwbemLocator}.ISWbemObject") Тогда
			Если ТипЗнч(СтрокаОбщегоТипа.Метаданные) = Тип("COMОбъект") Тогда
				...
				Для Каждого КлючИЗначение Из СтруктураЦикла Цикл
					...
					Если Истина
						...
						И Не ирНеглобальный.СтрокиРавныЛкс(лТипСлова, ТипСлова)
					Тогда
						Продолжить;
					КонецЕсли; 
					Для Каждого Свойство Из КоллекцияСвойств Цикл
						...
						Если Истина
							И Слово <> Неопределено 
							И Не ирНеглобальный.СтрокиРавныЛкс(ИмяСвойства, Слово)
						Тогда
							Продолжить;
						КонецЕсли;
						ТаблицаСтруктурТипов = ПолучитьНовуюТаблицуСтруктурТипа();
						...
						СтруктураТипа.ИмяОбщегоТипа  = ирНеглобальный.ПолучитьИмяТипаИзКвалификаторовWMIЛкс(Свойство);
						СтруктураТипа.Метаданные = РодительскаяСтруктураТипа.Метаданные;
		...
		Если РодительскаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда 
			...
			Тогда
				...
				Иначе
					КорневойТипРодителя = ЛксПолучитьКорневойТипКонфигурации(КэшОбъект(МетаданныеРодителя).ПолноеИмя);
				КонецЕсли; 
			...
			Для Каждого СтрокаВида Из НайденныеСтроки Цикл
				...
				Тогда
					...
					Для Каждого ОбщийРеквизит Из ОбщиеРеквизиты Цикл
						Если ирНеглобальный.ЛиОбщийРеквизитИспользуетсяВОбъектеМетаданныхЛкс(ОбщийРеквизит, МетаданныеРодителя) Тогда
				...
				ИначеЕсли СтрокаОписанияВида.ИмяКоллекции = "Движения" Тогда
					...
					Тогда 
						...
						КонецЕсли;  
						Если МетаданныеРегистра <> Неопределено Тогда
							МассивФрагментов = ЛксПолучитьМассивИзСтрокиСРазделителем(МетаданныеРегистра.ПолноеИмя());
				...
				Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
					...
					Если СхемаКоллекции.Количество() > 0 Тогда
						...
						КонецЕсли;
					Иначе
						МассивТипов = ЛксБыстрыйМассив(ТипЗнч(ЗначениеСвойства));
					КонецЕсли;
					ТаблицаСтруктурТипов = ПолучитьНовуюТаблицуСтруктурТипа();
					Если Истина
						//И ОписаниеТиповЗначения <> Неопределено
		...
		Если Найти(ИмяОбщегоТипа, МаркерCOMОбъекта) = 1 Тогда
			...
			Для Каждого ИнфоТипа Из МассивИнфоТипа Цикл
				Для Каждого Член Из ИнфоТипа.Members Цикл
					...
					КонецЕсли;
					Если Слово <> Неопределено Тогда
						Если Не ирНеглобальный.СтрокиРавныЛкс(Слово, ИмяЧлена) Тогда
							Продолжить;
						КонецЕсли; 
						Если Не ирНеглобальный.СтрокиРавныЛкс(ТипСлова, лТипСлова) Тогда
							Продолжить;
						КонецЕсли; 
					КонецЕсли; 
			...
			Для Каждого ИмяОбщегоТипа Из МассивТиповЗначения Цикл
				...
				Если КорневойТипРодителя <> Неопределено Тогда 
					...
					Тогда 
						...
						Тогда
							МассивФрагментов = ЛксПолучитьМассивИзСтрокиСРазделителем(СтрокаОбщегоТипа.Метаданные.ПолноеИмя());
...
Функция ПолучитьИнфоТипаCOMОбъекта(выхCOMОбъект, Знач ИмяОбщегоТипа = Неопределено, выхПолноеИмяОсновногоКласса = "") Экспорт
	...
	Если ТипЗнч(выхCOMОбъект) = Тип("COMОбъект") Тогда 
		COMНавигатор.ResolveAliases = Ложь;
		Попытка
	...
	Если ЗначениеЗаполнено(ИмяОбщегоТипа) Тогда
		выхПолноеИмяОсновногоКласса = ЛксПолучитьСтрокуМеждуМаркерами(ИмяОбщегоТипа, "{", "}", Ложь);
	...
	Если ИнфоИнтерфейса = Неопределено Тогда
		...
		Исключение
			...
			Возврат Неопределено;
		КонецПопытки; 
		МассивФрагментов = ЛксПолучитьМассивИзСтрокиСРазделителем(ИмяОбщегоТипа);
		...
		Для Каждого ИнфоТипа Из МассивИнфоТипа Цикл
			Для Каждого Член Из ИнфоТипа.Members Цикл
				Если Истина
					...
					И ирНеглобальный.СтрокиРавныЛкс(Член.Name, "Item") 
				Тогда
	...
	Иначе
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Если НайденныеСтроки[0].ТипЭлементаКоллекции <> "" Тогда
				ТипыЭлементовКоллекции = ЛксПолучитьМассивИзСтрокиСРазделителем(НайденныеСтроки[0].ТипЭлементаКоллекции, ",", Истина);
			КонецЕсли;
		КонецЕсли; 
	КонецЕсли; 
	...
	Если Истина
		...
		И (Ложь
			...
			Или (Истина
				И ЯзыкПрограммы = 2
				И ВнешниеФункцииКомпоновкиДанных
				И ЛксПолучитьПервыйФрагмент(СтруктураТипа.ИмяОбщегоТипа) = "ОбщийМодуль"))
	Тогда
		...
		Если Ложь
			...
			Или ЛксПолучитьПервыйФрагмент(СтруктураТипа.ИмяОбщегоТипа) = "ОбщийМодуль"
		Тогда
			ЗначениеДляИнформатора = Неопределено;
			Если ЛксПолучитьПервыйФрагмент(СтруктураТипа.ИмяОбщегоТипа) = "ОбщийМодуль" Тогда
...
Функция ПолучитьУникальныйИдентификаторИзСтроки(Текст) Экспорт
	...
	Цифра = "[a-fA-F0-9]";
	RegExp.Pattern = "(" + ЛксПолучитьСтрокуПовтором(Цифра, 32) + ")|(" 
		+ ЛксПолучитьСтрокуПовтором(Цифра, 8) + "-" 
		+ ЛксПолучитьСтрокуПовтором(Цифра, 4) + "-"
		+ ЛксПолучитьСтрокуПовтором(Цифра, 4) + "-"
		+ ЛксПолучитьСтрокуПовтором(Цифра, 4) + "-"
		+ ЛксПолучитьСтрокуПовтором(Цифра, 12) + ")";
	Вхождения = RegExp.Execute(Текст);
	Если Вхождения.Count > 0 Тогда
		Вхождение = Вхождения.Item(0);
		Если Не ирНеглобальный.ЛиПустаяПодгруппаRegExpЛкс(Вхождение.SubMatches(1)) Тогда
			УникальныйИдентификатор = Новый УникальныйИдентификатор(Вхождение.SubMatches(1));
		Иначе
			УникальныйИдентификатор = Новый УникальныйИдентификатор(ЛксПолучитьГУИДПрямойИзИнверсного(Вхождение.SubMatches(0)));
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		Если МетаданныеТипа = Неопределено Тогда
			...
			Если Найти(ПредставлениеТипа, ":") = 0 Тогда
				ИдентификаторТипа = ирНеглобальный.ПолучитьИдентификаторТипаЛкс(КонкрентыйТип);
			Иначе
				// Внешние метаданные
				ИдентификаторТипа = "";
				ПредставлениеТипа = ЛксПолучитьПервыйФрагмент(ПредставлениеТипа, ":");
			...
			Иначе
				КлючПоиска = Новый Структура("Представление, ТипТипа", ЛксПолучитьПервыйФрагмент(ПредставлениеТипа, ":"), "Основной");
			КонецЕсли; 
		Иначе
			ЛиНеУчитыватьПодтип = (ЯзыкПрограммы <> 0) И СтруктураТипа.ТипЯзыка <> "ЗначениеВЗапросе";
	...
	Иначе
		Для Каждого Тип Из пОграничениеТипа.Типы() Цикл
			...
			Если Объект <> Неопределено Тогда 
				#Если Клиент Тогда
					Картинка = ирНеглобальный.ПолучитьКартинкуКорневогоТипаЛкс(ЛксПолучитьКорневойТипКонфигурации(Объект));
	...
	Для Каждого СтрокаТипа Из Строки Цикл
		...
		Если Уровень = 0 Тогда
			Если Найти(ТекущееИмяТипа, ".") > 0 Тогда
				ФрагментыОбщегоТипа = ЛксПолучитьМассивИзСтрокиСРазделителем(ТекущееИмяТипа);
...
Функция ПолучитьТаблицуШаблоновТекста(ИмяКласса, мСообщенияЧерезПредупреждения, ПринудительноОбновить = Ложь) Экспорт 
	...
	Если Не Файл.Существует() Тогда
		Если ЗначениеЗаполнено(ФайлШаблонов) Тогда
			ЛксСообщитьСУчетомМодальности("Не обнаружен файл шаблонов текста """ + ФайлШаблонов + """, указанный в настройках компоненты ""Контекстная подсказка""",
...
Функция ВыполнитьЛокально(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт 
	...
	Если ЛиСинтаксическийКонтроль Тогда
		ирНеглобальный.ВыполнитьАлгоритмБезРезультата(ТекстДляВыполнения);
	Иначе
...
Функция ПолучитьАрхивСинтаксПомощникаПоПутиКЭлементу(ПутьКЭлементу, ЭлементСтруктуры = 0, ЛиОбрезатьПутьДоОтносительного = Ложь, ИмяАрхива = "") Экспорт
	...
	Если Найти(ПутьКЭлементу, МаркерДопАрхива) = 1 Тогда
		ИмяАрхива = ЛксПолучитьПервыйФрагмент(Сред(ПутьКЭлементу, СтрДлина(МаркерДопАрхива) + 1), "/");
...
Функция РаспаковатьФайлАрхиваСинтаксПомощника(Знач ПутьКЭлементу, ПрефиксСсылки = "") Экспорт
	...
	КонецЕсли;
	МассивФрагментов = ЛксПолучитьМассивИзСтрокиСРазделителем(ПутьКЭлементу, "#");
	...
	ПолноеИмяРаспакованногоФайла = КаталогВременныхФайлов() + ЛксПолучитьПоследнийФрагмент(ПутьКЭлементу, "/");
	Попытка
...
Функция ПолучитьТекстМодуляВнешнейОбработки(ФайлВнешнейОбработки) Экспорт 
	...
	//КонецЦикла;
	//ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(СтрокаЗапуска, КаталогРаспаковки);
	РаспаковатьФайлВнешнейОбработки(КаталогРаспаковки, ИмяКаталогаРаспаковки);
...
Процедура ЗаполнитьТаблицуРедактируемыхТипов() Экспорт
	...
	Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
		...
		Тогда
			...
			ИмяТипаМетаданных = ЛксПолучитьПервыйФрагмент(ТекущееИмяТипа);
...
Функция ПолучитьТаблицуТиповМетаОбъектов(ЛиВключатьНехранимые = Истина, ЛиВключатьДочерние = Истина, ЛиВключатьВнешние = Истина) Экспорт
	...
	Если ЛиВключатьНехранимые Тогда
		...
		Если ВерсияПлатформы >= 803003 Тогда
			...
			ДобавитьВТаблицуТиповМетаОбъектов(Таблица, 1, "ТаблицаИзмерения", ,   "ТаблицыИзмерений", );
		КонецЕсли; 
...
Функция ЗарегистрироватьПолучитьCOMОбъект(КлассКомпоненты, ПолноеИмяDll, Административная = Ложь, ИмяТипаВК = Ложь) Экспорт
	...
	//Если Не Административная Тогда
	//	 HKCU через сторонний регистратор
	...
	// HKLM через сторонний регистратор
...
Функция ПолучитьCOMОбъектИзМакета(ИмяМакетаКомпоненты, КлассКомпоненты, КаталогУстановки = "авто", ИмяМакетаДополнительнойБиблиотеки = "",
	...
	Если Результат = Неопределено Тогда
		...
		КонецЕсли;
		ФайлКомпоненты = ПроверитьЗаписатьМакетВФайл(ИмяМакетаКомпоненты, КаталогУстановки);
		Если ЗначениеЗаполнено(ИмяМакетаДополнительнойБиблиотеки) Тогда
			ФайлДополнительнойБиблиотеки = ПроверитьЗаписатьМакетВФайл(ИмяМакетаДополнительнойБиблиотеки, КаталогУстановки);
		КонецЕсли; 
...
Функция ПолучитьОбъектВнешнейКомпонентыИзМакета(ИмяМакетаКомпоненты, ИмяОбъекта, ИмяКомпоненты = Неопределено, Знач ТипКомпоненты = Неопределено,
	...
	Исключение
		...
		КонецЕсли;
		ФайлКомпоненты = ПроверитьЗаписатьМакетВФайл(ИмяМакетаКомпоненты);
		Если ЗначениеЗаполнено(ИмяМакетаДополнительнойБиблиотеки) Тогда
			ФайлДополнительнойБиблиотеки = ПроверитьЗаписатьМакетВФайл(ИмяМакетаДополнительнойБиблиотеки);
		КонецЕсли; 
...
Функция ПроверитьЗаписатьМакетВФайл(Знач ИмяМакетаКомпоненты, КаталогУстановки = "")
	...
	ФайлКомпоненты = Новый Файл(КаталогУстановки + "\" + ИмяМакетаКомпоненты + ".dll");
	...
	Пока ФайлКомпоненты.Существует() Цикл
		...
		#Иначе
			...
			КонецЕсли; 
		#КонецЕсли
		ФайлКомпоненты = Новый Файл(КаталогУстановки + "\" + ИмяМакетаКомпоненты +  "_" + Формат(Счетчик, "ЧГ=") + ".dll");
		Счетчик = Счетчик + 1;
	КонецЦикла; 
...
Функция ПолучитьИсполнительСкрытыхКомандСистемы() Экспорт
	...
	Если ИсполнительСкрытыхКомандСистемы = Неопределено Тогда
		ДвоичныеДанные = ПолучитьОбщийМакет("ирHiddenStart");
...
Функция ПолучитьИДВерсииПлатформы(СистемнаяИнформация = Неопределено) Экспорт
	...
	КонецЕсли;
	МассивФрагментов = ЛксПолучитьМассивИзСтрокиСРазделителем(СистемнаяИнформация.ВерсияПриложения);
...
Функция ПредставлениеИнформацииОбОшибке(Знач ИнформацияОбОшибке) Экспорт 
	...
	Если ИнформацияОбОшибке.Причина <> Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		Фрагмент = ЛксПолучитьПоследнийФрагмент(ОписаниеОшибки, "по причине:" + Символы.ПС);
...
Функция ВыполнитьМетодАлгоритма(ДескрипторСервиса, Режим, П0 = Null, П1 = Null, П2 = Null, П3 = Null, П4 = Null,
	...
	//#КонецЕсли
		...
		Если ФиксироватьВнешниеИсключенияАлгоритмов Тогда
			Попытка
				Результат = ирНеглобальный.ВыполнитьАлгоритм(ТекстАлгоритмаСПараметрами, СервисОбъект, Режим, П0, П1, П2, П3, П4, П5, П6, П7, П8, П9);
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
...
Процедура ИнициализацияСкриптаИнформатора() //Экспорт
	...
	//WinAPI.RegisterAddr(buf, "ImplBase_call", "i=pllp", "r=l");	//ID 20
