...
Функция ПолучитьВнутреннююТаблицуМетаданныхСлов(РодительскаяСтруктураТипа, Слово = Неопределено, ТипСлова = Неопределено,
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицуОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			Тогда
				ИмяКоллекцииОбъектовМетаданных = ирОбщий.ПолучитьСтрокуМеждуМаркерамиЛкс(ИмяОбщегоТипа, МаркерКоллекцииОбъектовМетаданных + ": ", , Ложь);
					...
					Тогда
						...
						Для Каждого Тип Из МассивТипов Цикл
							...
							Если Истина
								...
								//	Или мМассивТиповЭлементовУправления.Найти(Тип) <> Неопределено)
							Тогда 
								МетаданныеЭлемента = МетаданныеЭлементов[Слово];
	...
	Если ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных") Тогда
		...
		Если ирОбщий.ЛиКорневойТипСсылкиЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "Объект")) Тогда
			ТипМодуля = "МодульОбъекта";
			КорневойТип = ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "Объект");
			...
			КонецЕсли; 
		ИначеЕсли ирОбщий.ЛиКорневойТипРегистраБДЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "НаборЗаписей")) Тогда
			ТипМодуля = "МодульНабораЗаписей";
			КорневойТип = ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "НаборЗаписей");
			СтрокаТипаМетаОбъекта = ТипыМетаОбъектов.Найти(КорневойТип, "Единственное");
			Если СтрокаТипаМетаОбъекта <> Неопределено Тогда
				Множественное = СтрокаТипаМетаОбъекта.Множественное;
				СтрокаСозданияРодителя = Множественное + "." + МетаданныеРодителя.Имя + ".СоздатьНаборЗаписей()";
			КонецЕсли; 
		ИначеЕсли ирОбщий.ЛиКорневойТипКонстантыЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "МенеджерЗначения")) Тогда
			ТипМодуля = "МодульМенеджераЗначения";
			СтрокаСозданияРодителя = "Константы." + МетаданныеРодителя.Имя + ".СоздатьМенеджерЗначения()";
		ИначеЕсли Ложь
			Или ирОбщий.ЛиКорневойТипСсылкиЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "Менеджер"))
			Или ирОбщий.ЛиКорневойТипРегистраБДЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "Менеджер")) 
			Или ирОбщий.ЛиКорневойТипКонстантыЛкс(ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "Менеджер"))
		Тогда
			ТипМодуля = "МодульМенеджера";
			КорневойТип = ирОбщий.ПолучитьПервыйФрагментЛкс(ИмяОбщегоТипа, "Менеджер");
			СтрокаТипаМетаОбъекта = ТипыМетаОбъектов.Найти(КорневойТип, "Единственное");
			Если СтрокаТипаМетаОбъекта <> Неопределено Тогда
				Множественное = СтрокаТипаМетаОбъекта.Множественное;
...
Функция ПолучитьШаблоныДляАнализаВстроенногоЯзыка() Экспорт
	...
	// Если после последнего слова КонецПроцедуры или КонецФункции сразу расположен конец, то этот блок попадает в раздел ОсновнаяПрограмма
	// Чтобы этого избежать нужно добавлять в конце текста пробел!
	шСтруктураМодуля = "^((?:" + шРазделитель + шДирективаПрепроцессора + "|" + шОписаниеСтрокиПеременныхБезКомментов + "|" + шРазделитель + ")*\n)?"
		+ "((?:" + шРазделитель + шДирективаПрепроцессора + "|" + шОписаниеФункцииПроцедуры1 + "|" + шРазделитель + ")*)((?:\r|\n|.)*)$";
