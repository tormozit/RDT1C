...
Процедура ИнициализацияОписанияМетодовИСвойств() Экспорт
	...
	ТаблицаКонтекстов.Индексы.Добавить("ТипКонтекста, ЯзыкПрограммы, ТипЯзыка, ТипСлова, НСлово");
...
Функция ДоступныеЗначенияТипа(Знач ИмяТипаЗначения) Экспорт 
	...
	СтрокиЗначений = мПлатформа.ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", ИмяТипаЗначения, "Свойство"));
	Если СтрокиЗначений.Количество() > 0 Тогда
		...
		КонецПопытки;
	Иначе
		СтрокиЗначений = мПлатформа.ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", "Перечисление" + ИмяТипаЗначения, "Свойство"));
		Если СтрокиЗначений.Количество() = 0 Тогда
			//Продолжить;
		КонецЕсли; 
	КонецЕсли; 
	СтрокиЗначений = мПлатформа.ТаблицаКонтекстов.Скопировать(СтрокиЗначений);
		...
		Тогда
			Если ТипМетаданныхРодителя = Тип("ОбъектМетаданных") Тогда
				...
				Иначе
					...
					//КонецЕсли;
					ИмяТаблицы = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ПолноеИмя, Ложь, ВиртуальнаяТаблица = Неопределено); // Добавлено 14.04.2012
...
Функция ПолучитьВнутреннююТаблицуМетаданныхСлов(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	ТаблицаОбщихТиповСтруктурыТипа = ПолучитьТаблицуОбщихТиповСтруктурыТипа(РодительскаяСтруктураТипа);
	Если Истина
			...
			Тогда
				...
				Если Найти(ИмяОбщегоТипа, "Колон") = 0 Тогда
					Если СтрокаОписанияВида.ИмяЭлементаКоллекции = "<Имя колонки списка>" Тогда
						...
						ПостроительСписка = ПолучитьПостроительЗапросаПоСтруктуреТипа(СтруктураТипаИзЗначения(ДинамическийСписок));
				...
				Если ИнфоТипа <> Неопределено Тогда
					...
					Для Каждого ИнфоТипа Из МассивИнфоТипа Цикл
						Для Каждого Член Из ИнфоТипа.Members Цикл
							...
							ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
							ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипа);
							ДобавитьВТаблицуСлов(ВнутренняяТаблицаСлов, ИмяЧлена, лТипСлова, ТаблицаСтруктурТипов);
						КонецЦикла;
					КонецЦикла;
				КонецЕсли; 
			КонецЕсли; 
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, ТипМодуля = "") Экспорт 
	...
	Если ТипМодуля <> Неопределено Тогда
		...
		Если МодульМетаданных = Неопределено Тогда
			...
			#КонецЕсли
			ФайлОписанияМодуля = Новый Файл(ПапкаКэшаМодулей.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяФайлаМодуляБезРасширения + ".ixd");
			ФайлОписанияМодуляСуществует = ФайлОписанияМодуля.Существует();
			ФайлМодуля = Новый Файл(ПапкаКэшаМодулей.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяФайлаМодуляБезРасширения + ".txt");
...
// Перем шИмя, шКоммент, шРазделитель, шСтруктураКомментаПарам, шСтруктураКомментаВозвр, шВозврат, шСтруктураКомментаОписаниеПараметров, шКомментМетода, шОписаниеПеременной, шСтрокаПеременныхБезКомментов, шСтрокаПеременных, шСтрокаПрограммы, шПараметрыМетода, ПустоеНачалоСтроки, шДирективаПрепроцессора, шБлокКомментов, шМетодЧистый, шМетод1, шМетодСПредКомментом, шСтруктураМодуля, шЭлементОбластиМетодов;
// {Шаблон.Начало}
// + <шИмя> = [_ЁА-ЯA-Z][_ЁА-ЯA-Z\d]*
// + <шКоммент> = //[^\n]*(?=(?:\n|$))
// + <шРазделитель> = (?:<шКоммент>|\s|^|$)
...
// + <ПустоеНачалоСтроки> = (?:\n|^)(?:\t| )*
// + <шДирективаПрепроцессора> = <ПустоеНачалоСтроки>[#&][^\n]*(?=\n|$)
// + <шБлокКомментов> = <шКоммент>(?:<ПустоеНачалоСтроки><шКоммент>)*(?=(?:\n|$))
// + <шМетодЧистый> = ((Процедура|Функция)<шРазделитель>*(<шИмя>)<шРазделитель>*\(([^\)]*)\)(<шРазделитель>*Экспорт)?)((?:<шСтрокаПрограммы>|<шБлокКомментов>|\.Конец(?:Процедуры|Функции)|(?:\r|\n|.)(?!Конец(?:Процедуры|Функции)))*)[^а-яё_a-z0-9\."]Конец(?:Процедуры|Функции)(?=(?:<шКоммент>|[^а-яё_a-z0-9\."]|$))
// + <шМетод1> = <ПустоеНачалоСтроки><шМетодЧистый>
...
//  ТаблицаЗначений – с колонками "Слово", "ТипСлова", "ТаблицаСтруктурТипов".
//
Функция ПолучитьВнутреннююТаблицуПредопределенныхСлов(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	Знач Конфигурация = Неопределено, Знач ВычислятьТипы = Истина, Знач ЛиСерверныйКонтекст = Ложь) Экспорт 
					...
					Если КорневойТипРодителя <> Неопределено Тогда 
						...
						Тогда
							...
							Для Каждого ТипЗначенияКонстанты Из ТипыЗначенияКонстанты Цикл
								...
								ШаблонСтруктурыТипа.СтрокаОписания = СтрокаСлова;
								ЗаполнитьЗначенияСвойств(ШаблонСтруктурыТипа, РодительскаяСтруктураТипа, "ТипЯзыка");
								СтруктураТипа = СтруктураТипаИзКонкретногоТипа(ТипЗначенияКонстанты, ЯзыкПрограммы, ШаблонСтруктурыТипа);
								ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипа);
							КонецЦикла;
						ИначеЕсли Истина
							И СтрокаСлова.ТипСлова = "Метод"
...
Функция ПолучитьТаблицуСловСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, ВычислятьТипы = Истина,
	Знач РазрешитьСозданиеОбъектаИзТипа = Истина, ТипСлова = Неопределено, ЛиСерверныйКонтекст = Ложь, Знач Слово = Неопределено, Знач МодульМетаданных = Неопределено) Экспорт 

	ИнициализацияОписанияМетодовИСвойств();
	ВнутренняяТаблицаСлов = ПолучитьВнутреннююТаблицуПредопределенныхСлов(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ЛиСерверныйКонтекст);
	ВнутренняяТаблицаСлов.Колонки.Добавить("Определение");
	ВнутренняяТаблицаСлов.ЗаполнитьЗначения("Предопределенный", "Определение");
	ВнутренняяТаблицаМетаданныхСлов = ПолучитьВнутреннююТаблицуМетаданныхСлов(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы,, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ЛиСерверныйКонтекст, МодульМетаданных);
...
Функция ТаблицаСтруктурТиповИзОписанияТипов(ОписаниеТипов, ТаблицаСтруктурТипов = Неопределено) Экспорт
	...
	Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
		СтруктураТипа = СтруктураТипаИзКонкретногоТипа(Тип, Ложь);
...
Функция БазовыйФайлРедактораJSON() Экспорт 
	Если БазовыйФайлРедактораJSON = Неопределено Тогда
		...
		БазовыйФайлРедактораJSON = БазовыйКаталог + КаталогКомпоненты + ирОбщий.РазделительФайловойСистемыЛкс() + "index.html";
	КонецЕсли; 
...
Функция БазовыйФайлРедактораКода() Экспорт 
	Если БазовыйФайлРедактораКода = Неопределено Тогда
		...
		Файл = Новый ЧтениеZipФайла(Чтение.ИсходныйПоток());
		Файл.ИзвлечьВсе(БазовыйКаталог + КаталогКомпоненты);
		БазовыйФайлРедактораКода = БазовыйКаталог + КаталогКомпоненты + ирОбщий.РазделительФайловойСистемыЛкс() + "index.html";
	КонецЕсли; 
...
Функция РедактироватьЗапрос(Текст, Параметры, ТипЗапроса = "Построитель", ПараметрыADO = Неопределено, ПараметрыWMI = Неопределено) Экспорт
	...
	ОбработкаКонсольЗапросов = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКонсольЗапросов");
...
Функция ПортДляПодключенияОтладчика(выхИдентификаторыОтладчиков = Неопределено) Экспорт
	...
	Тогда
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Тогда
				...
				Если Истина
					И ирОбщий.СтрокиРавныЛкс(ПроцессОС.Name, ирОбщий.ИмяИсполняемогоФайлаПлатформыЛкс(Ложь))
...
Функция ПрочитатьИндексАрхиваСинтаксПомощника(ИмяАрхива) Экспорт 
	...
	#КонецЕсли
	Результат = Неопределено;
	ФайлИндекса = Новый Файл(ПапкаКэшаСинтаксПомощника.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяАрхива + ".idx");
...
Функция РаспаковатьФайлАрхиваСинтаксПомощника(Знач ПутьКЭлементу, ПрефиксСсылки = "") Экспорт
	...
	ирОбщий.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершенияЛкс(ФайлРаспаковщикаZIP.Имя + " -j " + ФайлАрхива.Имя 
...
Функция ПолучитьМакетКомпоненты(Компонента) Экспорт
	...
	Если Не МакетыКомпонент.Свойство(Компонента.ИмяКласса, Макет) Тогда
		...
		КонецПопытки;
		Если Успех Тогда
			ирОбщий.СообщитьЛкс("Образована циклическая ссылка на ирПлатформа");
		КонецЕсли; 
...
Функция ТекстМодуляСгенерированнойВнешнейОбработки(ФайлВнешнейОбработки) Экспорт 
	...
	Иначе
		...
		ФайлТекстаМодуляОбработки = Новый Файл(КаталогРаспаковки + ирОбщий.РазделительФайловойСистемыЛкс() + СубПутьКФайлуПотокаМодуляВнешнейОбработки);
...
//  *Переоткрыть   - Булево, *Истина - переоткрыть, если уже открыт
//
Процедура ОткрытьМодульВнешнейОбработкиВКонфигураторе(ПолноеИмяФайла, НомерСтрокиВМодуле = 0, ИдентификаторПроцесса = 0, Переоткрыть = Истина, ВнутреннееИмя = "") Экспорт 
...
Функция ТаблицаРедактируемыхТиповИзОписанияТипов(ОписаниеТипов = Неопределено) Экспорт
	...
	Иначе
		...
		Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
			...
			СтрокиДляКопирования.Добавить(мТаблицаРедактируемыхТипов.НайтиСтроки(КлючПоиска)[0]);
		КонецЦикла;
...
Функция ПроверитьЗаписатьКомпонентуИзМакетаВФайл(Знач ИмяМакетаКомпоненты, КаталогУстановки = "", Расширение = "dll") Экспорт 
	...
	ФайлКомпоненты = Новый Файл(КаталогУстановки + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяМакетаКомпоненты + "." + Расширение);
	...
	Пока ФайлКомпоненты.Существует() Цикл
		...
		#Иначе
			...
			КонецЕсли; 
		#КонецЕсли
		ФайлКомпоненты = Новый Файл(КаталогУстановки + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяМакетаКомпоненты +  "_" + Формат(Счетчик, "ЧГ=") + "." + Расширение);
		Счетчик = Счетчик + 1;
	КонецЦикла; 
...
Функция РазвернутьНовыйШаблонВнешнейОбработки(СМакетом = Ложь, ГлобальныйКлюч = Неопределено,
	...
	ФайлСИменем = Новый Файл(лПутьКШаблонуВнешнейОбработки + ирОбщий.РазделительФайловойСистемыЛкс() + СубПутьКФайлуПотокаЗаголовкаВнешнейОбработки);
...
Процедура СоздатьКомандныйФайлДляУпаковкиФайлаВнешнейОбработки(Каталог) Экспорт
	...
	Разделитель = ирОбщий.РазделительФайловойСистемыЛкс();
	ПолучитьМакет("v8unpack").Записать(Каталог + Разделитель + ИмяФайлаПакера);
	...
	ТекстовыйДокумент.Записать(Каталог + Разделитель + "pack.bat", КодировкаТекста.ANSI);
...
Процедура УпаковатьФайлВнешнейОбработки(КаталогРаспаковки, ПолноеИмяФайлаВнешнейОбработки) Экспорт
	...
	ирОбщий.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершенияЛкс("pack.bat """ + ПолноеИмяФайлаВнешнейОбработки + """", КаталогРаспаковки);
...
Процедура РаспаковатьФайлВнешнейОбработки(ПолноеИмяФайла, ПутьРаспаковки) Экспорт 
	...
	Разделитель = ирОбщий.РазделительФайловойСистемыЛкс();
	ПолучитьМакет("v8unpack").Записать(ПутьРаспаковки + Разделитель + ИмяФайлаПакера);
	ПолучитьМакет("Zlib1").Записать(ПутьРаспаковки + Разделитель + "Zlib1.dll");
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.УстановитьТекст("
	...
	|");
	ТекстовыйДокумент.Записать(ПутьРаспаковки + Разделитель + "unpack.bat", КодировкаТекста.ANSI);
	ИмяФайлаЛога = "log.txt";
	ирОбщий.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершенияЛкс("unpack.bat """ + ПолноеИмяФайла + """", ПутьРаспаковки,, ИмяФайлаЛога);
	ЧитатьЛог = Ложь; // Для отладки
	Если ЧитатьЛог Тогда
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(ПутьРаспаковки + Разделитель + ИмяФайлаЛога);
...
Функция СформироватьВнешнююОбработку(Знач ФайлОбработки, Знач ИмяОбработки, Знач ТекстМодуля, Знач ТекстМакета = Неопределено, Знач ДляИДВерсииПлатформы = "") Экспорт
	...
	Если Не ИспользоватьБыструюРаспаковку Тогда
		...
		ТекстЛога = Неопределено;
		ИмяКорневогоФайла = ШаблонВнешнейОбработки + ирОбщий.РазделительФайловойСистемыЛкс() + "ИмяВнешнейОбработки.xml";
	...
	Иначе
		...
		ПутьКШаблону = ПутьКШаблону + ирОбщий.РазделительФайловойСистемыЛкс();
		Если ЗначениеЗаполнено(ИмяОбработки) Тогда
...
Функция УпаковатьВнешнююОбработку(ПутьКШаблонуВнешнейОбработки, ИмяВыходногоФайла, СоздатьФайлыУпаковщика = Ложь) Экспорт
	...
	ирОбщий.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершенияЛкс("pack.bat """ + ИмяВыходногоФайла + """", ПутьКШаблонуВнешнейОбработки);
...
Функция ПолучитьФайлВнешнейОбработкиАлгоритма(АлгоритмОбъект) Экспорт 
	...
	ФайлВнешнейОбработки = Новый Файл(ПапкаКешаВнешнихОбработокАлгоритмов.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяФайла);
...
Функция ПроверитьКаталогФайловогоКэша() Экспорт
	...
	КаталогФайловогоКэша = КаталогВерсииПлатформыВПрофиле + ирОбщий.РазделительФайловойСистемыЛкс() + ирОбщий.ИдентификаторИзПредставленияЛкс(СтрокаСоединения);
	...
	Если Не ПапкаФайловогоКэша.Существует() Тогда
		Попытка
			...
			ПробныйФайл = Новый ТекстовыйДокумент;
			ПробныйФайл.Записать(ПапкаФайловогоКэша.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + "1.txt");
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
...
Процедура ОчиститьКешАлгоритмов(ОчиститьКэшНаДиске = Ложь) Экспорт 
	...
	Если ОчиститьКэшНаДиске Тогда 
		...
		УдалитьФайлы(ФайлПодкаталога.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс(), "*.*");
		ПроверитьСтруктуруФайловогоКэша();
	КонецЕсли;
...
Процедура _ПроверитьСоздатьФайлЗапаковщика1С(БыстраяПроверка = Ложь) Экспорт 
	...
	Тогда
		...
		ФайлЗапаковщика1С = Новый Файл(ПапкаКешаВнешнихОбработокАлгоритмов.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + ИмяФайла);
	...
	Тогда
		ФайлБиблиотекиЗапаковщика = Новый Файл(ПапкаКешаВнешнихОбработокАлгоритмов.ПолноеИмя + ирОбщий.РазделительФайловойСистемыЛкс() + "Zlib1.dll");
...
Функция ПолучитьНовоеУникальноеИмя() Экспорт
	...
	Результат = "_" + ирОбщий.ИдентификаторИзПредставленияЛкс(Новый УникальныйИдентификатор);
	Возврат Результат;
...
Функция НовыйВычислительРегулярныхВыражений() Экспорт
...
//#КонецЕсли
RegExp = НовыйВычислительРегулярныхВыражений();
Если RegExp <> Неопределено Тогда
	RegExp.IgnoreCase = Истина;
	RegExp.MultiLine = Ложь;
	RegExp2 = НовыйВычислительРегулярныхВыражений();
