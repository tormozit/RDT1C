	...
	Для Каждого ТекстЗапроса Из МассивТекстовЗапросов Цикл
		...
		Для Каждого Match2 Из Результат2 Цикл
			Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(Match2.SubMatches(0)) Тогда
				...
				Тогда
					СозданныеТаблицы.Добавить(ИмяВременнойТаблицы);
					СозданныеТаблицыНрег.Добавить(НРег(ИмяВременнойТаблицы));
				КонецЕсли; 
				Прервать;
			ИначеЕсли Ложь
				Или Не ТолькоТребующиеУничтоженияНаВходе
				Или Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(Match2.SubMatches(1)) 
			Тогда
	...
	Иначе
		Попытка
			...
			ИначеЕсли КлассКомпоненты = "GoldParserForNet.Parser" Тогда
				Попытка
					Если Компонента.Version = "5.07" Тогда 
						Возврат Компонента;
					КонецЕсли; 
				Исключение
				КонецПопытки;
