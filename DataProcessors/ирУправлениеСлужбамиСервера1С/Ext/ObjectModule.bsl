...
Процедура ОбновитьСлужбуПоСтроке(СтрокаСлужбы)
	...
	Тогда
		...
		СкриптУстановкиПрав = ПолучитьМакет("УстановкаПравNTFS");
		ФайлСкрипта = Новый Файл(ПолучитьИмяВременногоФайла("ps1"));
		СкриптУстановкиПрав.Записать(ФайлСкрипта.ПолноеИмя);
		КомандаСистемы = "powershell -executionpolicy RemoteSigned -file " + ФайлСкрипта.Имя + " -FolderListFile " + ФайлПараметров.Имя;
		РезультатУстановкиПрав = ирОбщий.ВыполнитьКомандуОСЛкс(КомандаСистемы,,, Истина);
