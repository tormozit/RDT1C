...
Функция ПрименитьИзменения() Экспорт
	...
	Для Каждого СтрокаСлужбы Из СлужбыАгентовСерверов1С Цикл
		...
		Тогда
			Порт = ПортАгентаСтрокиСлужбы(СтрокаСлужбы);
		...
		Иначе
			...
			ОбновитьСтрокуЗапускаНовую(СтрокаСлужбы, КаталогКонфигурации);
...
Процедура ОбновитьСтрокуЗапускаНовую(Знач СтрокаСлужбы, выхКаталогКонфигурации = "") Экспорт 
	
	#Если Сервер И Не Сервер Тогда
	    СтрокаСлужбы = Обработки.ирУправлениеСлужбамиСерверов1С.Создать().СлужбыАгентовСерверов1С.Добавить();
	#КонецЕсли
	Порт = ПортАгентаСтрокиСлужбы(СтрокаСлужбы);
	...
	КонецЕсли; 
	ирОбщий.ДобавитьЕслиНужноПравыйСлешВФайловыйПутьЛкс(КаталогИсполняемыхФайлов);
	выхКаталогКонфигурации = СтрокаСлужбы.КаталогКонфигурации;
	Если Ложь
		Или Не ЗначениеЗаполнено(выхКаталогКонфигурации)
		Или ирОбщий.СтрокиРавныЛкс("<Авто>", выхКаталогКонфигурации)
	Тогда
		ФайлКаталогаВерсии = Новый Файл(КаталогИсполняемыхФайлов);
		выхКаталогКонфигурации = ФайлКаталогаВерсии.Путь + "srvinfo" + XMLСтрока(ПортАгента);
	КонецЕсли;
	//СтрокаЗапускаСлужбы = БазовыйКаталог + "\" + СборкаПлатформы + "\bin\ragent.exe -srvc -agent -regport " + ПортКластера + " -port " + ПортАгента + " -range "
	СтрокаЗапускаНовая = """" + КаталогИсполняемыхФайлов + "bin\ragent.exe"" -srvc -agent -regport " + ПортКластера + " -port " 
	+ ПортАгента + " -range " + ДиапазонПортов + " -d """ + выхКаталогКонфигурации + """";
...
Функция ПортАгентаСтрокиСлужбы(Знач СтрокаСлужбы)
...
Процедура Заполнить() Экспорт 
	...
	Для Каждого АктуальнаяСлужба Из АктуальныеСлужбы Цикл
		...
		СтрокаСлужбыАгента.СтрокаЗапускаСлужбы = СтрокаЗапускаСлужбы;
		...
		Если ЗначениеЗаполнено(АктуальнаяСлужба.ProcessId) Тогда
			...
			Если ТипЗнч(АктивныйПроцесс) <> Тип("Строка") Тогда
				...
				СтрокаСлужбыАгента.ПараметрыИзменены = Не ирОбщий.СтрокиРавныЛкс(СтрокаСлужбыАгента.СтрокаЗапускаАктивная, СтрокаСлужбыАгента.СтрокаЗапускаСлужбы);
			КонецЕсли; 
		КонецЕсли; 
		...
		КонецЕсли; 
		СтрокаСлужбыАгента.РежимОтладки = ирОбщий.РежимОтладкиИзКоманднойСтрокиЛкс(СтрокаЗапускаСлужбы);
