...
Функция ПолучитьСхемуКомпоновки(Копия = Ложь)
	...
	Если мТекущаяСтрока <> Неопределено Тогда
		Если Копия Тогда
			Результат = ирОбщий.ПолучитьКопиюОбъектаЛкс(мТекущаяСтрока.СхемаКомпоновкиДанных, Истина);
		Иначе
...
Функция СохранитьКомпоновкиВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь)
	...
	Иначе
		...
		Если НЕ ЗначениеВФайл(мИмяОткрытогоФайла, КопияДерева) Тогда
			ирОбщий.СообщитьЛкс("Ошибка записи файла """ + мИмяОткрытогоФайла + """", СтатусСообщения.Внимание);
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнициализироватьИсточникКомпоновщикаНастроек(Строка)
	...
	Если Строка.СхемаКомпоновкиДанных <> Неопределено Тогда
		КопияСхемы = ирОбщий.ПолучитьКопиюОбъектаЛкс(Строка.СхемаКомпоновкиДанных, Истина);
		...
		Если Не Предпросмотр И ЗначениеЗаполнено(ПолеИзОшибки) Тогда
			...
			Если НайденныйЭлемент <> Неопределено Тогда
				...
				Если ТипЗнч(НайденныйЭлемент.ЭлементНастройки) = Тип("ПолеГруппировкиКомпоновкиДанных") Тогда
					АктивизироватьТабличноеПоле(ЭлементыФормы.ПоляГруппировки, НайденныйЭлемент.ЭлементНастройки);
				ИначеЕсли ТипЗнч(НайденныйЭлемент.ЭлементНастройки) = Тип("ВыбранноеПолеКомпоновкиДанных") Тогда
					АктивизироватьТабличноеПоле(ЭлементыФормы.Выбор, НайденныйЭлемент.ЭлементНастройки);
				ИначеЕсли ТипЗнч(НайденныйЭлемент.ЭлементНастройки) = Тип("ЭлементПорядкаКомпоновкиДанных") Тогда
					АктивизироватьТабличноеПоле(ЭлементыФормы.Порядок, НайденныйЭлемент.ЭлементНастройки);
				ИначеЕсли ТипЗнч(НайденныйЭлемент.ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
					АктивизироватьТабличноеПоле(ЭлементыФормы.Отбор, НайденныйЭлемент.ЭлементНастройки);
				ИначеЕсли ТипЗнч(НайденныйЭлемент.ЭлементНастройки) = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
					АктивизироватьТабличноеПоле(ЭлементыФормы.УсловноеОформление, НайденныйЭлемент.ЭлементНастройки);
...
Процедура АктивизироватьТабличноеПоле(Знач ТабличноеПоле, ТекущаяСтрока = Неопределено)
...
Функция СкомпоноватьРезультатМой(Знач МакетКомпоновкиДанных = Неопределено, Знач СтруктураВнешниеНаборыДанных = Неопределено, Предпросмотр = Ложь)
	...
	Если ВыводитьОтладочнуюРазметку Тогда
		МакетКомпоновкиДанных = ирОбщий.ПолучитьКопиюОбъектаЛкс(МакетКомпоновкиДанных, Истина);
	...
	Если Не Предпросмотр И ЭлементыФормы.ПанельРезультатов.ТекущаяСтраница = ЭлементыФормы.ПанельРезультатов.Страницы.РезультатТабличныйДокумент Тогда
		ДанныеРасшифровки.Настройки = ирОбщий.ПолучитьКопиюОбъектаЛкс(ДанныеРасшифровки.Настройки, Истина); 
...
Процедура УстановитьМакетКомпоновкиДанных(Знач МакетКомпоновкиДанных = Неопределено)
	...
	КонецЕсли; 
	КопияМакета = ирОбщий.ПолучитьКопиюОбъектаЛкс(мМакетКомпоновкиДанных, Истина);
...
Процедура СкопироватьПараметрыСтроки(ИсходнаяСтрока, КонечнаяСрока)
	...
	КонечнаяСрока.СхемаКомпоновкиДанных = ирОбщий.ПолучитьКопиюОбъектаЛкс(ИсходнаяСтрока.СхемаКомпоновкиДанных, Истина);
	КонечнаяСрока.Настройки = ирОбщий.ПолучитьКопиюОбъектаЛкс(ИсходнаяСтрока.Настройки, Истина);
...
Процедура ОткрытьФайлОтчетов(РежимОбъединения = Ложь)
	...
	Если СохранитьКомпоновкиВФайл(Не РежимОбъединения) Тогда
		РезультатВыбора = ирОбщий.ВыбратьФайлЛкс(, "dcf", "Файл консоли компоновки", мИмяОткрытогоФайла, мИмяПути,, "Выберите файл со списком отчетов");
...
Процедура СохранитьТекущиеНастройкиВСтроку()
	...
	Тогда
		...
		мТекущаяСтрока.ВнешниеНаборыДанных = ирОбщий.ПолучитьКопиюОбъектаЛкс(ВнешниеНаборыДанных, Истина);
		Для Каждого Тн ИЗ мТекущаяСтрока.Настройки Цикл
			Если Ложь
...
Процедура ОткрытьКонструкторСхемы(СтрокаОтчета, НоваяСхема = Неопределено)
	...
	Если НоваяСхема <> Неопределено Тогда
		...
		КопияСхемы = ирОбщий.ПолучитьКопиюОбъектаЛкс(НоваяСхема, Истина);
...
Процедура ПриОткрытии()
	...
	Тогда
		...
		Если мСхемаКомпоновкиДанных <> Неопределено Тогда
			ГлавнаяСтрока.СхемаКомпоновкиДанных = ирОбщий.ПолучитьКопиюОбъектаЛкс(мСхемаКомпоновкиДанных, Истина);
		КонецЕсли; 
		...
		КонецЕсли;
		#Если Сервер И Не Сервер Тогда
			ЗапроситьОткрытиеКонсолиЗапросовПриОткрытии();
		#КонецЕсли
		ПодключитьОбработчикОжидания("ЗапроситьОткрытиеКонсолиЗапросовПриОткрытии", 0.1, Истина);
	Иначе
...
Процедура ЗапроситьОткрытиеКонсолиЗапросовПриОткрытии()
	...
	Если мМакетКомпоновкиДанных <> Неопределено Тогда
		#Если Сервер И Не Сервер Тогда
			мМакетКомпоновкиДанных = Новый МакетКомпоновкиДанных;
		#КонецЕсли
		Для Каждого НаборДанныхМакета Из мМакетКомпоновкиДанных.НаборыДанных Цикл
			Если ТипЗнч(НаборДанныхМакета) <> Тип("НаборДанныхОбъектМакетаКомпоновкиДанных") Тогда
		...
		КонецЕсли; 
	Иначе
		Для Каждого НаборДанныхМакета Из мСхемаКомпоновкиДанных.НаборыДанных Цикл
			Если ТипЗнч(НаборДанныхМакета) <> Тип("НаборДанныхОбъектМакетаКомпоновкиДанных") Тогда
				ЕстьПодходящийНаборДанных = Истина;
				Прервать;
			КонецЕсли; 
...
Процедура РедактрироватьЗапросСхемы(Знач ВыбраннаяСтрока)
	...
	#КонецЕсли
	КонсольЗапросов.НаСервере = НаСервере;
...
Процедура ПриПолученииДанныхДоступныхПолей(Элемент, ОформленияСтрок)
	...
	ирОбщий.ПриПолученииДанныхДоступныхПолейКомпоновкиЛкс(ОформленияСтрок, ЭлементыФормы.Выбор);
...
Процедура КоманднаяПанельТаблицаСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭтаФорма, ЭлементыФормы.ТаблицаЗначений);
...
Процедура КоманднаяПанельСтруктураПроверитьНастройки(Кнопка)
	...
	ЕстьНекорректныеЭлементы = ирОбщий.ЕстьНекорректныеЭлементВНастройкеКомпоновкиЛкс(ПроверочныйКомпоновщик, СтрокаТекущихНастроек, СтрокаИсправленныхНастроек);
...
Функция ПолучитьНастройкиКомпоновки()
	...
	Иначе
		ТекущиеНастройки = ирОбщий.ПолучитьКопиюОбъектаЛкс(КомпоновщикНастроек.Настройки, Истина);
	КонецЕсли; 
...
Процедура ПараметрыСхемыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Форма = ПолучитьФорму("КонструкторВыраженияКомпоновки", ЭтаФорма);
	СхемаКомпоновкиКопия = ПолучитьСхемуКомпоновки(Истина);
	#Если Сервер И Не Сервер Тогда
...
Процедура СтруктураПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Для Каждого ЭлементЗначения Из ЗначениеПеретаскивания Цикл
		Если Ложь
			Или ТипЗнч(ЭлементЗначения) = Тип("ДоступноеПолеКомпоновкиДанных") 
			Или ТипЗнч(ЭлементЗначения) = Тип("ДоступноеПолеОтбораКомпоновкиДанных")
		Тогда
...
Процедура КоманднаяПанель1СравнитьСРезультирующим(Кнопка)
	...
	Если МакетКомпоновкиДанных <> Неопределено Тогда
		...
		Для Каждого ИмяНабора Из КоординатыСтроки Цикл
			...
			КонецЕсли; 
		КонецЦикла;
		ирОбщий.СравнитьЗначенияИнтерактивноЧерезXMLСтрокуЛкс(ЭлементыФормы.ДеревоНаборовДанныхЗапросов.ТекущаяСтрока.НаборДанных.Запрос, НаборМакета.Запрос,, "Запрос схемы", "Запрос макета");
	КонецЕсли;
...
Процедура СтруктураВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ТекущаяКолонка <> ЭлементыФормы.Структура.Колонки.Заголовок Тогда
		...
		АктивизироватьТабличноеПоле(ЭлементыФормы[ТекущаяКолонка.Имя]);
	КонецЕсли; 
...
Процедура КП_ЗапросыСхемыДобавитьОбъект(Кнопка)
	...
	ирОбщий.СоздатьИлиОбновитьНаборДанныхОбъектПоМетаданнымЛкс(СхемаКомпоновкиДанных, ТаблицаЗначенийНабора.Колонки, НовыйНаборДанных.Имя);
...
Функция ПроверитьПреобразоватьСтруктуруВПростую(Знач ЭлементЦикла, Преобразовать = Ложь)
	...
	Пока ЭлементЦикла.Структура.Количество() > 0 Цикл
		...
		КонецЦикла;
		ЭлементЦикла = ЭлементЦикла.Структура[0];
		Если ТипЗнч(ЭлементЦикла) = Тип("ТаблицаКомпоновкиДанных") Тогда
			ПростаяСтруктураДоступна = Ложь;
		КонецЕсли; 
		Если Не ПростаяСтруктураДоступна Тогда
			Прервать;
...
Процедура СтруктураОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Для Каждого ЭлементПеретаскивания Из МассивЭлементов Цикл
		...
		Если ТипЭлемента = Тип("ГруппировкаКомпоновкиДанных") Тогда
			Если ПростаяСтруктура = Истина Тогда
				СтандартнаяОбработка = Ложь;
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирКонсольКомпоновокДанных.Форма.Форма");
