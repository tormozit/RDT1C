...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено, Предпросмотр = Ложь)
	...
	Попытка
		Если НеЗаполнятьРасшифровки Тогда
			МакетКомпоновкиДанных = КомпоновщикМакета.Выполнить(ВременнаяСхемаКомпоновки, ВременныеНастройкиКомпоновки,, СвойМакетОформления, Тип(ИмяТипаГенератора),
...
Функция СкомпоноватьРезультатМой(Знач МакетКомпоновкиДанных = Неопределено, Знач СтруктураВнешниеНаборыДанных = Неопределено, Предпросмотр = Ложь)
	...
	Результат = ирОбщий.СкомпоноватьОтчетВКонсолиЛкс(КоллекцияВывода, МакетКомпоновкиДанных, СтруктураВнешниеНаборыДанных, Автофиксация, МодальныйРежим, ЛиОтладка, АдресДанныхРасшифровки,
		НаСервере И Не Предпросмотр, Не Предпросмотр, мЭлементыРезультата);
...
Процедура СохранитьТекущиеНастройкиВСтроку()
	...
	Тогда
		...
		Для Каждого СтрокаСНабором Из ВнешниеНаборыДанных Цикл
			...
			КонецЕсли; 
		КонецЦикла;
		мТекущаяСтрока.ВнешниеНаборыДанных = ирОбщий.КопияОбъектаЛкс(ВнешниеНаборыДанных, Истина);
		Для Каждого Тн ИЗ мТекущаяСтрока.Настройки Цикл
			Если Ложь
...
Процедура СобытияАлгоритмНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Для Каждого СтрокаПараметра Из ТекущаяСтрокаСобытия.Параметры Цикл
		Если ирОбщий.СтрокиРавныЛкс(СтрокаПараметра.Имя, "СхемаКомпоновки") Тогда
			СтрокаПараметра.Значение = СхемаКомпоновки(Истина);
...
Процедура ОткрытьАнализПравДоступа(Знач ИмяПоля)
	...
	#КонецЕсли
	ПолеТекста = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой");
...
Функция ПроверитьПреобразоватьСтруктуруВПростую(Знач ЭлементЦикла, Преобразовать = Ложь)
	...
	Пока ЭлементЦикла.Структура.Количество() > 0 Цикл
		...
		ЭлементЦикла = ЭлементЦикла.Структура[0];
		Если Ложь
			Или ТипЗнч(ЭлементЦикла) = Тип("ТаблицаКомпоновкиДанных") 
			Или ТипЗнч(ЭлементЦикла) = Тип("ДиаграммаКомпоновкиДанных")
		Тогда
			ПростаяСтруктураДоступна = Ложь;
		КонецЕсли; 
		Если Не ПростаяСтруктураДоступна Тогда
...
Процедура КоманднаяПанельТаблицаЗначенийОтборПоЗначению(Кнопка)
	...
	ирОбщий.ТабличноеПолеОтборПоЗначениюВТекущейКолонкеЛкс(ТекущееТабличноеПолеРезультата(), КомпоновщикНастроек.Настройки.Отбор,, ЭлементыФормы.Отбор);
	
КонецПроцедуры

Процедура КоманднаяПанельТаблицаЗначенийОтборБезЗначенияВТекущейКолонке(Кнопка)
	
	ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонкеЛкс(ТекущееТабличноеПолеРезультата(), КомпоновщикНастроек.Настройки.Отбор,, ЭлементыФормы.Отбор);
