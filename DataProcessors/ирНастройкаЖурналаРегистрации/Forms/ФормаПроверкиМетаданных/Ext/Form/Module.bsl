...
Функция ПолучитьСсылочныеТипыПоля(Поле, ПолноеИмяМД)
	...
	Если ЭтоПолеТЧ Тогда
		ИмяТЧ = ирОбщий.ПолучитьПервыйФрагментЛкс(Поле);
		КоллекцияПолей = ирКэш.ПоляТаблицыБДЛкс(ПолноеИмяМД + "." + ИмяТЧ);
	Иначе
		КоллекцияПолей = ирКэш.ПоляТаблицыБДЛкс(ПолноеИмяМД);
	КонецЕсли; 
...
ирОбщий.ИнициализироватьФормуЛкс(ЭтаФорма, "Обработка.ирНастройкаЖурналаРегистрации.Форма.ФормаПроверкиМетаданных");
