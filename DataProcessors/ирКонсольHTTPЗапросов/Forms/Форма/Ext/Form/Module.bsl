...
Процедура ОбновитьКонечныйURL()
	...
	ДописатьПараметрыЗапросаВURL(РезультатДекодированный, Результат);
	ЭтаФорма.КонечныйURLДекодированный = РезультатДекодированный;
	ЭтаФорма.КонечныйURL = Результат;
	
КонецПроцедуры

Процедура ДописатьПараметрыЗапросаВURL(РезультатДекодированный, Результат = "")
...
Функция АдресРесурсаСПараметрами()
	...
	РезультатДекодированный = АдресРесурса;
	ДописатьПараметрыЗапросаВURL(РезультатДекодированный);
	Возврат РезультатДекодированный;
...
Процедура КПТелоОтветаФорматироватьJSON(Кнопка)
	...
	ЭтаФорма.ТелоОтветаСтрока = ирОбщий.ФорматироватьТекстJsonЛкс(ТелоОтветаСтрока);

КонецПроцедуры

Процедура КПТелоЗапросаФорматироватьJSON(Кнопка)
	
	ЭтаФорма.ТелоЗапросаСтрока = ирОбщий.ФорматироватьТекстJsonЛкс(ТелоЗапросаСтрока);
...
|СтатусПоиска, ИсточникСтрокиДляПоиска, ИсточникТелаЗапроса, OAuth2_Сервис, OAuth2_КлючКлиента, OAuth2_СекретКлиента, OAuth2_Scope, ИспользоватьДляВсехМетодов";
