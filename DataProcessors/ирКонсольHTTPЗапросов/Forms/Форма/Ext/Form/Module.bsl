...
Функция УстановитьТелоОтветаСтрока(НовыйТекст) Экспорт
	...
	ЭлементыФормы.ТелоОтветаHtml.УстановитьТекст(?(ПустаяСтрока(НовыйТекст), "<html></html>", НовыйТекст));
...
Функция ОбновитьКонечныйURL(Полный = Истина)
	...
	Если Автонаименование Тогда
		мТекущаяСтрокаДереваЗапросов.Наименование = СерверЗапроса + "/" + АдресРесурса; 
	КонецЕсли; 
	Возврат Результат;
