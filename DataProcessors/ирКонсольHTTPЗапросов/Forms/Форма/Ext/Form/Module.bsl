...
Процедура ОбновитьКонечныйURL()
	...
	КонецЕсли;
	ФрагментыАдреса = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(АдресРесурса, "/");
	#Если Сервер И Не Сервер Тогда
	...
	КонецЦикла;
	АдресРесурсаЗакодированный = ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(ФрагментыАдреса, "/");
...
Процедура ПрочитатьКуки(Знач ОтветHTTP)
	...
	Перем ЗначениеКуки, ЗначенияКуки, КукиСтрока, СтрокаКуки;
	
	КукиСтрока = ОтветHTTP.Заголовки["Set-Cookie"];
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	ЗначенияКуки = ирОбщий.НайтиРегулярноеВыражениеЛкс(КукиСтрока, "\s*(" + мПлатформа.шИмя + ")\s*=\s*([^;]*)\s*(?:;|$)", "Имя, Значение");
	КукиОтвета.Очистить();
	Для Каждого ЗначениеКуки Из ЗначенияКуки Цикл
		СтрокаКуки = КукиЗапроса.Найти(НРег(ЗначениеКуки.Имя), "НИмя");
		Если СтрокаКуки = Неопределено Тогда
			СтрокаКуки = КукиЗапроса.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаКуки, ЗначениеКуки); 
			ирОбщий.ОбновитьКопиюСвойстваВНижнемРегистреЛкс(СтрокаКуки);
		КонецЕсли;
		СтрокаКуки = КукиОтвета.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКуки, ЗначениеКуки); 
	КонецЦикла;
...
Процедура ЗагрузитьКонечныйЗапрос(Знач СтрокаURL, ДекодироватьURL = Истина)
	...
	Если Фрагменты.Количество() > 1 Тогда
		...
		СтрокаПараметров = ирОбщий.ПолучитьПервыйФрагментЛкс(Фрагменты[1], "#"); // Отбрасываем якорь
		Фрагменты = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(СтрокаПараметров, "?");
		Для Каждого Фрагмент Из Фрагменты Цикл
			СтрокаПараметра = ПараметрыЗапроса.Добавить();
			ИмяПараметра = ирОбщий.ПолучитьПервыйФрагментЛкс(Фрагмент, "=");
			ЗначениеПараметра =  ирОбщий.ПолучитьПоследнийФрагментЛкс(Фрагмент, "=");
	...
	Если ДекодироватьURL Тогда
		...
		КонецЦикла;
		ЭтаФорма.АдресРесурса = ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(ФрагментыАдреса, "/");
	КонецЕсли; 
	ОбновитьКонечныйURL();
	ЭтаФорма.Модифицированность = Истина;
...
Процедура ЗаголовкиЗапросаЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ЗаголовкиЗапроса, СтандартнаяОбработка,, Истина);
...
Процедура ДанныеФормыЗапросаОписаниеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ДанныеФормыЗапроса, СтандартнаяОбработка,, Истина);
...
Процедура ПараметрыЗапросаОписаниеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ПараметрыЗапроса, СтандартнаяОбработка,, Истина);
...
Процедура КП_ДанныеФормыЗапросаЗаполнитьИзСтрокиТела(Кнопка)
	...
	Если ЗаголовокТипаСодержимого <> Неопределено Тогда
		...
		Если НРег(ирОбщий.ПолучитьПервыйФрагментЛкс(ЗаголовокТипаСодержимого.Значение, ";")) = "multipart/form-data" Тогда 
			Разделитель = "--" + ирОбщий.ПолучитьСтрокуМеждуМаркерамиЛкс(ЗаголовокТипаСодержимого.Значение, """", """", Ложь, Ложь);
				...
				Для Счетчик = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
					...
					ИначеЕсли ВнутриПараметра Тогда 
						...
						ИначеЕсли Найти(СтрокаТела, "Content-Disposition:") = 1 Тогда
							СтрокаПараметра.Имя = ирОбщий.ПолучитьСтрокуМеждуМаркерамиЛкс(СтрокаТела, "'", "'", Ложь, Ложь);
...
Процедура КукиЗапросаЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.КукиЗапроса, СтандартнаяОбработка,, Истина);
...
Процедура КП_ЗаголовкиОтветаСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭлементыФормы.ЗаголовкиОтвета);
	
КонецПроцедуры

Процедура КП_ЗаголовкиЗапросаСравнить(Кнопка)
	
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭлементыФормы.ЗаголовкиЗапроса);
	
КонецПроцедуры

Процедура КП_КукиОтветаСравнить(Кнопка)
	
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭлементыФормы.КукиОтвета);
...
Процедура КП_КукиЗапросаСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭлементыФормы.КукиЗапроса);
...
Процедура СформироватьЗапросOAuth2Нажатие(Элемент)
	...
	ЭлементыФормы.ДеревоЗапросов.ДобавитьСтроку();
	мТекущаяСтрокаДереваЗапросов.Наименование = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(мТекущаяСтрокаДереваЗапросов.Родитель.Строки, "Авторизация OAuth2 ", "Наименование", Ложь);
