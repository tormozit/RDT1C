...
Процедура КоманднаяПанельВыполнениеВыполнитьЗапрос(Кнопка = Неопределено)
	...
	Если Не ЗначениеЗаполнено(СерверЗапроса) Тогда
		...
		Возврат;
	КонецЕсли;
	Если Не ирОбщий.ПроверитьТаблицуПараметровЛкс(КукиЗапроса, "Проверка таблицы ""КукиЗапроса"". ") Тогда
...
Процедура НастроитьЭлементыФормы()
	...
	ЭлементыФормы.ПанельЗапрос.Страницы.ПараметрыЗапроса.Видимость = ирОбщий.СтрокиРавныЛкс(МетодЗапроса, "GET");
...
|ДекодироватьURL, ХранитьПароль, Автоперенаправление, МаксимальныйРазмерОтвета, КодировкаЗапроса, ИспользоватьBOM, ТелоЗапросаСтрока, ТелоЗапросаДвоичныеДанные, РегулярноеВыражение, РезультатПоиска,
