...
Процедура СформироватьЗапросOAuth2Нажатие(Элемент)
	...
	ЭлементыФормы.ДеревоЗапросов.ДобавитьСтроку();
	мТекущаяСтрокаДереваЗапросов.Наименование = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(ирОбщий.ПолучитьРодителяСтрокиДереваЛкс(мТекущаяСтрокаДереваЗапросов).Строки, "Авторизация OAuth2 ", "Наименование", Ложь);
...
Процедура КП_ПараметрыЗапросаСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭлементыФормы.ПараметрыЗапроса);
