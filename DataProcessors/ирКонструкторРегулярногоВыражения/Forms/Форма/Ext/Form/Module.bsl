...
Процедура КПВыражениеСгенерироватьПрограммныйКод(Кнопка)
	...
	Для ИндексПараметра = 0 По Параметры.Количество() - 1 Цикл
		...
		Если ТекущаяСтрока = СтрокаПараметра Тогда
			Прервать;
		КонецЕсли; 
	КонецЦикла;
	...
	Для Счетчик = СтрокиПараметров.Найти(ТекущаяСтрока) + 1 По СтрокиПараметров.Количество() Цикл
		ИндексРодителя = СтрокиПараметров.Найти(ТекущаяСтрока) + СтрокиПараметров.Количество() - Счетчик;
	...
	|RegExp.Pattern = " + ТекущаяСтрока.Имя + ";
	...
	|Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстГдеИскать, " + ТекущаяСтрока.Имя + ", """ + ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(ИменаПодгрупп) + """);
	|	#Если Сервер И Не Сервер Тогда
	|	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
	|	#КонецЕсли
	|Для каждого Вхождение Из Вхождения Цикл
