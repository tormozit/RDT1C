...
Процедура ОбновитьПроверочныйТекст(РазрешитьАвтовыделение = Истина)
	...
	Если ЗначениеЗаполнено(РазвернутоеВыражение) Тогда
		...
		Если ЭлементыФормы.Подгруппы.ТекущаяСтрока.Индекс = -1 Тогда
			Для ИндексКолонки = мПлатформа.Метаданные().ТабличныеЧасти.ВхожденияРегулярногоВыражения.Реквизиты.Количество() По Вхождения.Колонки.Количество() - 1 Цикл
...
Функция ВычислитьВхождения()
	...
	Если ИменаПодгруппВместоИндексов Тогда
		СтрокаИменаПодгрупп = ирОбщий.СтрСоединитьЛкс(ИменаПодгрупп());
	КонецЕсли; 
	Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, РазвернутоеВыражениеПараметра, СтрокаИменаПодгрупп,, ИгнорироватьРегистр, Многострочный, Ложь);
	#Если Сервер И Не Сервер Тогда
		Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
	#КонецЕсли
	...
	Для Каждого Вхождение Из Вхождения Цикл
		Если ПараметрыСоСписками.Количество() > 0 Тогда 
			...
			Если Вхождения.Колонки.Найти(СтрокаПараметраСоСписком.Имя) = Неопределено Тогда
				Вхождения.Колонки.Добавить(СтрокаПараметраСоСписком.Имя);
			КонецЕсли; 
			ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.ТекстВхождения, РазвернутоеВыражениеВложенное,,, ИгнорироватьРегистр, Многострочный, Ложь);
			#Если Сервер И Не Сервер Тогда
				ВложенныеВхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
			#КонецЕсли
