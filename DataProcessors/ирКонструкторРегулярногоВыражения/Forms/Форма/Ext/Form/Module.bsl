...
Функция ЗаполнитьРаскраску(Текст)
	...
	Для каждого Группа Из мНайденныеГруппы Цикл
		...
		КонецЕсли;
		ЗначениеПодгруппы = ирОбщий.КодироватьТекстВXMLЛкс(ТекстПодгруппы(Группа));
	...
	//ТекстовыйДокумент = Новый ТекстовыйДокумент;
	//ТекстовыйДокумент.УстановитьТекст(Код);
	...
	//КонецЦикла;
	РаскрашенныйТекст = РаскрашенныйТекст + Результат.Закрыть();
...
Функция ПроверочныйТекст()
	...
	ДокументHtml = ирОбщий.ПолучитьHtmlFileИзТекстаHtmlЛкс(Текст);
...
Процедура ПараметрыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		ЭлементыФормы.Параметры.ТекущаяСтрока.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(Параметры, "Параметр");
	КонецЕсли; 
...
Процедура ПараметрыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		НовоеИмяПараметра = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииСтрокЛкс(Параметры, ЭлементыФормы.Параметры.ТекущаяСтрока);
...
Процедура ПредыдущееВхождениеНажатие(Элемент = Неопределено, ОбновитьПроверочныйТекст = Ложь)
	...
	Для Каждого Группа Из мНайденныеГруппы Цикл
		...
		Если Группа.ПозицияПодгруппы < ВыделениеВТексте.Начало Тогда
			ВыбранннаяГруппа = Группа;
		Иначе
			ИнвертироватьПоиск = Группа.ПозицияПодгруппы > ВыделениеВТексте.Начало + 2; // 2 - Крайний перенос строки 
			Прервать;
		КонецЕсли; 
	КонецЦикла;
	Если ВыбранннаяГруппа <> Неопределено Тогда
		ВыделитьГруппуВПроверочномТексте(ВыбранннаяГруппа);
...
Функция УстановитьВыделениеВПроверочномТексте(ВыделениеВТексте)
	
	ДокументHtml = ЭлементыФормы.ПроверочныйТекст.Документ;
	ТелоДокумента = ДокументHtml.body;
	Диапазон = ТелоДокумента.createTextRange();
	Диапазон.Collapse();
	Диапазон.moveEnd("character", ВыделениеВТексте.КонецБезПереносов);
    Диапазон.moveStart("character", ВыделениеВТексте.НачалоБезПереносов);
    Диапазон.select();

КонецФункции

Процедура ВыделитьГруппуВПроверочномТексте(Знач ВыбранннаяГруппа)
	
	ДокументHtml = ЭлементыФормы.ПроверочныйТекст.Документ;
	ТелоДокумента = ДокументHtml.body;
	Диапазон = ТелоДокумента.createTextRange();
	ТегГруппы = ДокументHtml.getElementByID(ИДПодгруппыВПроверочномТексте(ВыбранннаяГруппа.Номер));
