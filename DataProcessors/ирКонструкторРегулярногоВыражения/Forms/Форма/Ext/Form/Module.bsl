...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Форма.АвтообновлениеПроверочногоТекста, Форма.Автовыделение, Форма.ПереносСлов";
...
Процедура КПВыражениеСгенерироватьПрограммныйКод(Кнопка)
	...
	Для ИндексПараметра = 0 По Параметры.Количество() - 1 Цикл
		...
		Если мТекущаяСтрокаПараметра = СтрокаПараметра Тогда
			Прервать;
		КонецЕсли; 
	КонецЦикла;
	...
	Для Счетчик = СтрокиПараметров.Найти(мТекущаяСтрокаПараметра) + 1 По СтрокиПараметров.Количество() Цикл
		ИндексРодителя = СтрокиПараметров.Найти(мТекущаяСтрокаПараметра) + СтрокиПараметров.Количество() - Счетчик;
	...
	|RegExp.Pattern = " + мТекущаяСтрокаПараметра.Имя + ";";
	...
	|Многострочный = " + ?(Многострочный, "ИСТИНА", "ЛОЖЬ") + ";
	|ИгнорироватьРегистр = " + ?(ИгнорироватьРегистр, "ИСТИНА", "ЛОЖЬ") + ";
	|Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстГдеИскать, " + мТекущаяСтрокаПараметра.Имя + ", """ + ирОбщий.СтрСоединитьЛкс(ИменаПодгрупп) + """,, ИгнорироватьРегистр, Многострочный, Ложь);
	|	#Если Сервер И Не Сервер Тогда
	|	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
	|	#КонецЕсли";
...
Процедура ПараметрыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		ЭлементыФормы.Параметры.ТекущаяСтрока.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(Параметры, "Параметр");
	КонецЕсли; 
...
Процедура КП_ПроверочныйТекстОткрытьТаблицуРезультатов(Кнопка)
	...
	Если Не ЗначениеЗаполнено(РазвернутоеВыражениеПараметра) Тогда
		Возврат;
	КонецЕсли; 
	Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, РазвернутоеВыражениеПараметра,,, ИгнорироватьРегистр, Многострочный, Ложь);
	#Если Сервер И Не Сервер Тогда
		Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
	#КонецЕсли
	Для Каждого Вхождение Из Вхождения Цикл
...
Процедура КПВыражениеОткрытьВыражениеНаСайтеRegex101(Кнопка)
	...
	СтрокаURL = "https://regex101.com/?regex=" + КодироватьСтроку(РазвернутоеВыражение(), СпособКодированияСтроки.КодировкаURL);
...
Процедура КП_ВхожденияРазличныеЗначенияКолонки(Кнопка)
	
	ирОбщий.ОткрытьРазличныеЗначенияКолонкиЛкс(ЭлементыФормы.ВхожденияПодгруппы);
	
КонецПроцедуры

Процедура КП_ВхожденияВывестиСтроки(Кнопка)
	
	ирОбщий.ВывестиСтрокиТабличногоПоляИПоказатьЛкс(ЭлементыФормы.ВхожденияПодгруппы);
...
ВхожденияПодгруппы.Колонки.Добавить("Подстрока", Новый ОписаниеТипов("СписокЗначений"));
