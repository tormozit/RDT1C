...
Процедура ПриОткрытии()
	...
	Если мРежимРедактора Тогда
		...
		СоздатьФайл();
		Если ПараметрВыражение <> Неопределено Тогда
...
Процедура ОткрытьВыражениеВRegexBuddy(НачатьОтладку = Ложь)
	...
	Иначе
		...
		Если НачатьОтладку Тогда
			мОлеСервер.DebugAtPos(ВыделениеВТексте.Начало); // Не работает почему то
		КонецЕсли; 
	КонецЕсли;
...
Функция ТекстПодгруппы(Знач Группа)
	...
	Иначе
		ЗначениеПодгруппы = Группа.Подгруппы[ИндексПодгруппы].Значение;
	КонецЕсли;
...
Функция УстановитьПроверочныйТекст(Текст)
	...
	ДокументHtml = ЭлементыФормы.ПроверочныйТекст.Документ;
	ДокументHtml.body.innerText = Текст;
	Возврат Текст;
...
Процедура ЗагрузитьИзФайла(ИмяФайла = "")
	...
	Если Параметры.Количество() > 0 Тогда
		...
		#КонецЕсли
		ПодключитьОбработчикОжидания("АвтообновлениеПроверочногоТекста", 0.1, Истина);
	КонецЕсли; 
...
Процедура КПВыражениеСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭтаФорма, ЭлементыФормы.ПолеТекстаВыражения);
...
Процедура ПараметрыСписокПриИзменении(Элемент)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		Если СтрокаПараметра <> мТекущаяСтрокаПараметра Тогда
			СтрокаПараметра.Список = Ложь;
		КонецЕсли; 
	КонецЦикла;
