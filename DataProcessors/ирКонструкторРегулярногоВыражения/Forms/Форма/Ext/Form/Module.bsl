...
Процедура КПВыражениеСгенерироватьПрограммныйКод(Кнопка)
	...
	|Многострочный = " + ?(Многострочный, "ИСТИНА", "ЛОЖЬ") + ";
	|ИгнорироватьРегистр = " + ?(ИгнорироватьРегистр, "ИСТИНА", "ЛОЖЬ") + ";
	|Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстГдеИскать, " + мТекущаяСтрокаПараметра.Имя + ", """ + ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(ИменаПодгрупп) + """,, ИгнорироватьРегистр, Многострочный, Ложь);
	|	#Если Сервер И Не Сервер Тогда
	|	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
	|	#КонецЕсли";
	...
	Если СтрокаПараметраСоСписком <> Неопределено Тогда
		ПрограммныйКод = ПрограммныйКод + 
		"	ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.ТекстВхождения, " + СтрокаПараметраСоСписком.Имя + ", """ + ирОбщий.ПолучитьСтрокуСРазделителемИзМассиваЛкс(ИменаВложенныхПодгрупп) + """,, ИгнорироватьРегистр, Многострочный, Ложь);
		|		#Если Сервер И Не Сервер Тогда
		|		    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегулярногоВыражения;
		|		#КонецЕсли
