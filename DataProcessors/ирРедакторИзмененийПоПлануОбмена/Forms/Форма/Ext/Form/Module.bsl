...
Процедура ЗарегистрироватьИзмененияПоОдномуОбъектуНажатие(Элемент)
	...
	Для Каждого СтрокаДереваМетаданных Из СоставПланаОбмена.Строки Цикл
		...
		Если СтрокаДереваМетаданных.ПолноеИмя = "Константы" Тогда
			...
			КонецЦикла;
		ИначеЕсли ирОбщий.ЛиКорневойТипОбъектаБДЛкс(СтрокаДереваМетаданных.Имя) Тогда
...
Процедура КоманднаяПанельДереваОбъектовОткрытьОбработкуОбъектов(Кнопка)
	...
	КонецЕсли; 
	ФормаОбработки.УстановитьОбластьПоиска(ЭлементыФормы.СоставПланаОбмена.ТекущиеДанные.ПолноеИмя);
...
Процедура КоманднаяПанельДереваОбъектовКопироватьРегистрациюОбъектов(Кнопка)
	...
	КонецЕсли; 
	ВыбранныеСтрокиДерева = ПолучитьВыбранныеСтрокиДерева();
	Если ВыбранныеСтрокиДерева.Количество() = 0 Тогда
	...
	Для Каждого ВыбраннаяСтрока Из ВыбранныеСтрокиДерева Цикл
		ОбъектыМД.Добавить(ирОбщий.НайтиОбъектМетаДанныхПоПолномуИмениТаблицыБДЛкс(ВыбраннаяСтрока.ПолноеИмя));
	КонецЦикла;
