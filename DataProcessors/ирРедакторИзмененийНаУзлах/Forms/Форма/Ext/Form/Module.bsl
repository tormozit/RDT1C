		...
		Для Каждого МетаданныеОбъектаПредка Из КоллекцияПредков Цикл
			...
			Для Каждого МетаданныеОбъекта Из МетаданныеОбъектаПредка[ТипМетаОбъектов.Множественное] Цикл
				...
				Если ЗначениеЗаполнено(ИмяПланОбмена) Тогда
					...
					КонецЕсли; 
				Иначе
					ИмяТаблицыИзменений = ирОбщий.ПолучитьИмяТаблицыИзМетаданныхЛкс(ПолноеИмяМД, Истина, Ложь);
				...
				Для Каждого лИмяПланаОбмена Из мИменаПлановОбмена Цикл
					...
					Если ЭлементСостава <> Неопределено Тогда
						СтрокаДобавления[ИмяКолонки] = (ЭлементСостава.АвтоРегистрация = АвтоРегистрацияИзменений.Разрешить);
					КонецЕсли; 
				КонецЦикла; 
			КонецЦикла;
	...
	Для Каждого СтрокаДерева Из ВыбранныеСтрокиДерева Цикл
		...
		Иначе
			...
			Для Каждого ЗначенияСписка Из СписокРедактирования Цикл
				Успех = ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, ЗначенияСписка.Значение, НовоеЗначение, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				//Если Истина
				//	И Успех 
...
Функция ИзменитьРегистрациюЦелойТаблицы(СтрокаДерева, НовоеЗначениеРегистрации)
	...
	Успех = ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(мВыбранныеУзлы, ОбъектМеданных, НовоеЗначениеРегистрации, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями,, Истина);
...
Функция ПолучитьОбъектДанныхПоСтрокеИзменений(СтрокаДанных = Неопределено)
	...
	Если мТекущаяГруппаТипаМетаданных = "Ссылочный" Тогда
		...
		СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(СтрокаДанных.Ссылка.Метаданные().ПолноеИмя(), СтрокаДанных.Ссылка,,, ОбъектыНаСервере);
...
Процедура ВыполнитьРегистрациюПервыхОбъектовДляУзла(ОбъектМД, УзлыДляРегистрации, КоличествоОбъектовКаждогоТипа = 1)
	...
	Пока Выборка.Следующий() Цикл
		...
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, Выборка.Ссылка, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
	КонецЦикла;
...
Процедура ВыполнитьРегистрациюРегистра(Знач ТипТаблицы, Знач ПолноеИмяТаблицы, УзлыДляРегистрации, КоличествоОбъектовКаждогоТипа = 1,
	...
	ИначеЕсли ТипТаблицы = "РегистрСведений" Тогда
		МетаданныеРегистра = ирОбщий.НайтиОбъектМетаданныхПоПолномуИмениТаблицыБДЛкс(ПолноеИмяТаблицы);
	...
	Если ПустаяСтрока(СтрокаКолонокВыборкиДанных) Тогда
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, МетаданныеРегистра, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
		Возврат;
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл 
		СтруктураНаборЗаписей = ирОбщий.СоздатьНаборЗаписейПоИмениТаблицыБДЛкс(ПолноеИмяТаблицы, ОбъектыНаСервере);
		Если КолонкаОтбора <> "" Тогда 
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, СтруктураНаборЗаписей.Методы, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
	КонецЦикла;
	...
	Для Каждого СтрокаДереваМетаданных Из ДеревоТаблиц.Строки Цикл
		...
		Если СтрокаДереваМетаданных.ПолноеИмя = "Константа" Тогда
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				...
				Тогда
					ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, Константы[СтрокаОбъектаМД.Имя], Истина, ОбъектыНаСервере);
				ИначеЕсли Действие = "УдалитьРегистрациюВсех" Тогда
					ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, Константы[СтрокаОбъектаМД.Имя], Ложь, ОбъектыНаСервере);
				КонецЕсли; 
			КонецЦикла;
		ИначеЕсли ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(СтрокаДереваМетаданных.Имя) Тогда
			...
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				...
				ИначеЕсли Действие = "ЗарегистрироватьВсе" Тогда
					ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, ОбъектМД, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				ИначеЕсли Действие = "УдалитьРегистрациюВсех" Тогда
					ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, ОбъектМД, Ложь, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				КонецЕсли; 
			КонецЦикла;
			ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
		ИначеЕсли ирОбщий.ЛиКорневойТипРегистраБДЛкс(СтрокаДереваМетаданных.Имя) Тогда
			...
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				ирОбщий.ОбработатьИндикаторЛкс(ИндикаторМД);
				ОбъектМД = ирОбщий.НайтиОбъектМетаданныхПоПолномуИмениТаблицыБДЛкс(СтрокаОбъектаМД.ПолноеИмя);
				УзлыДляРегистрации = ирОбщий.ПолучитьРазрешенныеУзлыДляОбъектаМДЛкс(ОбъектМД, мВыбранныеУзлы, ТолькоДляОбъектовСАвтоматическойРегистрацией);
				Если УзлыДляРегистрации.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли; 
...
Процедура КП_ТаблицаИзмененийОбновить(Кнопка = Неопределено)
	...
	Если УровеньВыбраннойСтроки = 1 Тогда
		ОбновитьИтогиПоТаблице();
		ирОбщий.ПолучитьМакетныйОбъектДанныхТаблицыБДЛкс(ВыбраннаяСтрока.ПолноеИмя, мМакетныйОбъект, мТекущаяГруппаТипаМетаданных,, ОбъектыНаСервере);
...
Процедура ПолеВводаТаблицыИзменений_НачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.ТаблицаИзменений, СтандартнаяОбработка,, Истина);
...
Процедура ТаблицаИзмененийПередУдалением(Элемент, Отказ)
	...
	Для СчетчикВыделенныеСтроки = 1 По НачальноеКоличество Цикл
		...
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(ВыделеннаяСтрока.Узел, КлючОбъекта, Ложь, ОбъектыНаСервере);
...
Процедура ТаблицаИзмененийПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	КонецЕсли; 
	Объект = ПолучитьОбъектДанныхПоСтрокеИзменений();
	ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(лУзел, Объект,, ОбъектыНаСервере);
	//ОбновитьИтогиПоТаблице();
	КП_ТаблицаИзмененийОбновить();
...
Процедура КП_ТаблицаИзмененийСброситьНомер(Кнопка)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
		КлючОбъекта = ПолучитьКлючОбъектаСтроки(ВыделеннаяСтрока);
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(ВыделеннаяСтрока.Узел, КлючОбъекта,, ОбъектыНаСервере);
...
Процедура КПУзлыУстановитьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхСтрокЛкс(ЭлементыФормы.Узлы, , Истина, "Ссылка", Новый Структура("СобственныйУзел", Ложь));
	ПриИзмененииВыбранныхУзлов();
...
Процедура ТаблицаИзмененийОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ВыбранныйКлюч Из Массив Цикл
		...
		УзлыДляРегистрации = ирОбщий.ПолучитьРазрешенныеУзлыДляОбъектаМДЛкс(ОбъектРегистрации.Метаданные(), мВыбранныеУзлы);
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, ОбъектРегистрации,, ОбъектыНаСервере);
	КонецЦикла;
	КП_ТаблицаИзмененийОбновить();
