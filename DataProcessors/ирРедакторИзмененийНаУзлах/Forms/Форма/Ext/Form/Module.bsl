...
Процедура КоманднаяПанельДереваОбъектовПоказатьРезультатСтандартнойВыгрузки(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(СтрXML, "XML представление объекта """ + ЭлементыФормы.ДеревоТаблиц.ТекущаяСтрока.ПолноеИмя + """", "XML", Истина, ЭлементыФормы.ТаблицаИзменений.ТекущаяСтрока);
