...
Процедура ПостроитьДеревоОбменаДанных(ДеревоТаблиц, МетаданныеДляОграничения = Неопределено)
	...
	Если МетаданныеДляОграничения = Неопределено Тогда
		...
		КонецЦикла;
	Иначе
		ЭлементыФормы.ДеревоТаблиц.ТекущиеДанные.Строки.Очистить();
		Если мМетаданныеПланаОбмена = Неопределено Тогда
			Возврат;
		КонецЕсли; 
	КонецЕсли;
...
Процедура ПостроитьДеревоСоставаПланаОбмена(Знач МетаданныеДляОграничения = Неопределено)
	...
	ПостроитьДеревоОбменаДанных(ДеревоТаблиц, МетаданныеДляОграничения);
	...
	Для Каждого СтрокаДерева Из ВыбранныеСтрокиДерева Цикл
		...
		Иначе
			...
			Для Каждого ЗначенияСписка Из СписокРедактирования Цикл
				Успех = ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, ЗначенияСписка.Значение, НовоеЗначение, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				//Если Истина
				//	И Успех 
...
Функция ИзменитьРегистрациюЦелойТаблицы(СтрокаДерева, НовоеЗначениеРегистрации)
	...
	Успех = ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(мВыбранныеУзлы, ОбъектМеданных, НовоеЗначениеРегистрации, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями,, Истина);
...
Процедура ВыполнитьРегистрациюПервыхОбъектовДляУзла(ОбъектМД, УзлыДляРегистрации, КоличествоОбъектовКаждогоТипа = 1)
	...
	Пока Выборка.Следующий() Цикл
		...
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, Выборка.Ссылка, Истина, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
	КонецЦикла;
...
Процедура ВыполнитьРегистрациюРегистра(Знач ТипТаблицы, Знач ПолноеИмяТаблицы, УзлыДляРегистрации, КоличествоОбъектовКаждогоТипа = 1,
	...
	Если ПустаяСтрока(СтрокаКолонокВыборкиДанных) Тогда
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, МетаданныеРегистра, Истина, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
		Возврат;
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл 
		НаборЗаписей = ирОбщий.СоздатьНаборЗаписейПоИмениТаблицыБДЛкс(ПолноеИмяТаблицы);
		Если КолонкаОтбора <> "" Тогда 
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
		ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, НаборЗаписей, Истина, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
	КонецЦикла;
	...
	Для Каждого СтрокаДереваМетаданных Из ДеревоТаблиц.Строки Цикл
		...
		Если СтрокаДереваМетаданных.ПолноеИмя = "Константы" Тогда
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				...
				Тогда
					ПланыОбмена.ЗарегистрироватьИзменения(УзлыДляРегистрации, Константы[СтрокаОбъектаМД.Имя]);
				ИначеЕсли Действие = "УдалитьРегистрациюВсех" Тогда
					ПланыОбмена.УдалитьРегистрациюИзменений(УзлыДляРегистрации, Константы[СтрокаОбъектаМД.Имя]);
				КонецЕсли; 
			КонецЦикла;
		ИначеЕсли ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(СтрокаДереваМетаданных.Имя) Тогда
			...
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				...
				ИначеЕсли Действие = "ЗарегистрироватьВсе" Тогда
					ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, ОбъектМД, Истина, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				ИначеЕсли Действие = "УдалитьРегистрациюВсех" Тогда
					ирОбщий.ИзменитьРегистрациюДляУзлаЛкс(УзлыДляРегистрации, ОбъектМД, Ложь, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				КонецЕсли; 
			КонецЦикла;
			ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
...
Процедура ТаблицаИзмененийПередУдалением(Элемент, Отказ)
	...
	Для СчетчикВыделенныеСтроки = 1 По НачальноеКоличество Цикл
		...
		ПланыОбмена.УдалитьРегистрациюИзменений(ВыделеннаяСтрока.Узел, КлючОбъекта);
...
Процедура КП_ТаблицаИзмененийСброситьНомер(Кнопка)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
		КлючОбъекта = ПолучитьКлючОбъектаСтроки(ВыделеннаяСтрока);
		ПланыОбмена.ЗарегистрироватьИзменения(ВыделеннаяСтрока.Узел, КлючОбъекта);
...
Процедура КП_ТаблицаИзмененийПодборВТаблицуИзменений(Кнопка)
	...
	Иначе
		...
		Если ЭлементыФормы.ТаблицаИзменений.ТекущаяСтрока <> Неопределено Тогда
			...
			Иначе
				КлючОбъекта = ирОбщий.ПолучитьКлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ЭлементыФормы.ДеревоТаблиц.ТекущаяСтрока.ПолноеИмя, ЭлементыФормы.ТаблицаИзменений.ТекущаяСтрока, Истина);
			КонецЕсли; 
		КонецЕсли; 
...
Процедура ТаблицаИзмененийОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ВыбранныйКлюч Из Массив Цикл
		...
		ПланыОбмена.ЗарегистрироватьИзменения(УзлыДляРегистрации, ОбъектРегистрации);
	КонецЦикла;
	КП_ТаблицаИзмененийОбновить();
