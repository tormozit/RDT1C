		...
		Для Каждого МетаданныеОбъектаПредка Из КоллекцияПредков Цикл
			...
			Для Каждого ОбъектМД Из МетаданныеОбъектаПредка[ТипМетаОбъектов.Множественное] Цикл
				...
				Если ИмяТаблицыБД = "Константы" Тогда
					...
					ОписаниеТаблицы = Новый Структура("Имя, Представление, Тип, ЕстьДоступ", ОбъектМД.Имя, ОбъектМД.Представление(), "Константа", Истина);
				КонецЕсли; 
...
Процедура ПостроитьДеревоСоставаПланаОбмена(ПринудительноВыполнитьЗапрос = Ложь, Перезапустить = Истина)
	...
	ТабличноеПоле = ЭлементыФормы.ДеревоТаблиц;
	ИмяКлючевойКолонки = "ПолноеИмя";
	КлючТекущейСтрокиЗадания = ирОбщий.ТабличноеПолеКлючСтрокиЛкс(ТабличноеПоле, ИмяКлючевойКолонки);
	КлючиВыделенныхСтрок = ирОбщий.ТабличноеПолеКлючиСтрокЛкс(ТабличноеПоле, ИмяКлючевойКолонки);
	ДеревоТаблиц.Строки.Очистить();
	...
	ирОбщий.ТабличноеПолеВосстановитьТекущуюСтрокуЛкс(ТабличноеПоле, КлючТекущейСтрокиЗадания);
	ирОбщий.ТабличноеПолеВосстановитьВыделенныеСтрокиЛкс(ТабличноеПоле, КлючиВыделенныхСтрок);
...
Функция ПолучитьПостроительДляТаблицыИзменений(ВыбраннаяСтрока = Неопределено, МаксимальнаяПорция = 0)
	...
	Если Истина
		И ирКэш.НомерВерсииПлатформыЛкс() >= 802014
