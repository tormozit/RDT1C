...
Процедура ВыполнитьРегистрациюПервыхОбъектовДляУзла(ОбъектМД, УзлыДляРегистрации, КоличествоОбъектовКаждогоТипа = 1)
	...
	Пока Выборка.Следующий() Цикл
		...
		ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, Выборка.Ссылка, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
	КонецЦикла;
...
Процедура ВыполнитьРегистрациюРегистра(Знач ТипТаблицы, Знач ПолноеИмяТаблицы, УзлыДляРегистрации, КоличествоОбъектовКаждогоТипа = 1,
	...
	Если ПустаяСтрока(СтрокаКолонокВыборкиДанных) Тогда
		ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, МетаданныеРегистра, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл 
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
		ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, СтруктураНаборЗаписей.Методы, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
	КонецЦикла;
	...
	Для Каждого СтрокаДереваМетаданных Из ДеревоТаблиц.Строки Цикл
		...
		Если СтрокаДереваМетаданных.ПолноеИмя = "Константа" Тогда
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				...
				Тогда
					ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, Константы[СтрокаОбъектаМД.Имя], Истина, ОбъектыНаСервере);
				ИначеЕсли Действие = "УдалитьРегистрациюВсех" Тогда
					ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, Константы[СтрокаОбъектаМД.Имя], Ложь, ОбъектыНаСервере);
				КонецЕсли; 
			КонецЦикла;
		ИначеЕсли ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(СтрокаДереваМетаданных.Имя) Тогда
			...
			Для Каждого СтрокаОбъектаМД Из СтрокаДереваМетаданных.Строки Цикл
				...
				ИначеЕсли Действие = "ЗарегистрироватьВсе" Тогда
					ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, ОбъектМД, Истина, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				ИначеЕсли Действие = "УдалитьРегистрациюВсех" Тогда
					ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, ОбъектМД, Ложь, ОбъектыНаСервере, ИзменятьРегистрациюСДвижениями, ДвиженияВместеСПоследовательностями);
				КонецЕсли; 
			КонецЦикла;
			ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
...
Процедура ТаблицаИзмененийПередУдалением(Элемент, Отказ)
	...
	Для СчетчикВыделенныеСтроки = 1 По НачальноеКоличество Цикл
		...
		ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(ВыделеннаяСтрока.Узел, КлючОбъекта, Ложь, ОбъектыНаСервере);
...
Процедура ТаблицаИзмененийПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	КонецЕсли; 
	Объект = ПолучитьОбъектДанныхПоСтрокеИзменений();
	ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(лУзел, Объект,, ОбъектыНаСервере);
	//ОбновитьИтогиПоТаблице();
	КП_ТаблицаИзмененийОбновить();
...
Процедура КП_ТаблицаИзмененийСброситьНомер(Кнопка)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
		КлючОбъекта = ПолучитьКлючОбъектаСтроки(ВыделеннаяСтрока);
		ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(ВыделеннаяСтрока.Узел, КлючОбъекта,, ОбъектыНаСервере);
...
Процедура ТаблицаИзмененийОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Для Каждого ВыбранныйКлюч Из Массив Цикл
		...
		ирОбщий.ПланыОбменаИзменитьРегистрациюЛкс(УзлыДляРегистрации, ОбъектРегистрации,, ОбъектыНаСервере);
	КонецЦикла;
	КП_ТаблицаИзмененийОбновить();
