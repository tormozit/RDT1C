...
Процедура УстановитьПараметрыВыбраннойБазы(Знач НастройкиБазыНаКлиенте = Неопределено)
	...
	#Если Сервер И Не Сервер Тогда
		НастройкиБазыНаКлиенте = Обработки.ирПлатформа.Создать().СписокИнфобазПользователя.Добавить();
	#КонецЕсли
...
Процедура УстановитьПараметрыПользователя1С()
	
	Если ирОбщий.СтрокиРавныЛкс(База, СтрокаСоединенияИнформационнойБазы()) Тогда
...
Процедура ОбновитьТаблицуФайлов()
	...
	СтрокаФайла.Описание = "Настройки конфигуратора. Примеры. Настройки сравнения файлов. Настройки глобального поиска по текстам конфигурации.";
	СтрокаФайла = Файлы.Добавить();
	СтрокаФайла.ЛогическийПуть = "$Roaming\$1Cv\$База";
	...
	СтрокаФайла.Описание = "Настройки конфигуратора. Примеры. Расположение окна синтакс-помощника. Настройки окна табло. Список последних вычисленных выражений.";
	СтрокаФайла = Файлы.Добавить();
	СтрокаФайла.ЛогическийПуть = "$Roaming\$1Cv\$База\$Пользователь1С";
...
Процедура ИдентификаторПользователя1СНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если НРег(База) <> НРег(СтрокаСоединенияИнформационнойБазы()) Тогда
		Возврат;
	КонецЕсли; 
