...
Процедура КПФайлыУдалитьФайлы(Кнопка)
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если Ответ = КодВозвратаДиалога.ОК Тогда
			ПрекратитьРаботуСистемы(Истина, ирОбщий.ПараметрыЗапускаСеансаДляПодключенияКТекущемуОтладчикуЛкс());
		КонецЕсли; 
	КонецЕсли;
...
Функция НайтиЛогическиеФайлыЭлементаАрхива(ЭлементАрхива)
	...
	#КонецЕсли
	ЛогическийПуть = СтрЗаменить(ирОбщий.ПолучитьСтрокуБезКонцаЛкс(ЭлементАрхива.Путь, 1), "/", "\");
...
Процедура КПФайлыОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ирОбщий.ТабличноеПолеОтборБезЗначенияВТекущейКолонке_КнопкаЛкс(ЭлементыФормы.Файлы);
...
Процедура КПФайлыСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(ЭтаФорма, ЭлементыФормы.Файлы);
