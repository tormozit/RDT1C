...
Функция ПрочитатьЗначениеИзФайлаСКонтролемПотерьЛкс(ПолноеИмяФайла) Экспорт
	...
	Попытка
		// Антибаг платформы. ЗначениеВФайл на некоторых ошибках вызывает безусловное завершение работы http://devtool1c.ucoz.ru/forum/2-746-1
...
Функция ПолучитьКлючНабораЗаписейИзКлючаЗаписиЛкс(КлючЗаписи, ПолноеИмяМДЭлемента = "") Экспорт 
	...
	КонецЕсли; 
	СтруктураКлюча = ПолучитьСтруктуруКлючаТаблицыБДЛкс(ПолноеИмяМДЭлемента, Ложь,, Ложь);
...
Функция ОбъектБДПоКлючуЛкс(Знач ИмяОсновнойТаблицы, Знач КлючОбъекта, Знач СохранятьИдентификаторСсылки = Ложь, Знач ЧитатьДанные = Истина, Знач НаСервере = Ложь,
	...
	Иначе
		...
		Иначе
			...
			Иначе
				...
				#КонецЕсли
				КлючОбъекта = ПолучитьКлючНабораЗаписейИзКлючаЗаписиЛкс(КлючОбъекта, ИмяОсновнойТаблицы);
...
Функция КопияОбъектаБДЛкс(СтруктураОбъекта) Экспорт 
	...
	Иначе
		...
		СтруктураОбъекта.Вставить("Методы", СтруктураОбъекта);
		СтруктураОбъекта.Вставить("Данные", СтруктураОбъекта);
	КонецЕсли; 
...
Процедура ОбновитьДоступныеПоляДляДополнительныхПолейЛкс(Знач ТаблицаДанных, Знач КомпоновщикДопПолей, Знач ТабличноеПолеДоступныхПолей) Экспорт 
	...
	//Если КомпоновщикДопПолей.Настройки.ДоступныеПоляПорядка.Элементы.Количество() = 0 Тогда
		...
		КонецЕсли; 
		Если ТипЗнч(ТабличноеПолеДоступныхПолей) = Тип("ТабличноеПоле") Тогда
...
Функция ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ТабличноеПоле, ПолноеИмяМД = Неопределено, ДоступныеПоляВыбора = Неопределено, Связанный = Ложь,
	ФормаРедактора = Неопределено, НаСервере = Неопределено) Экспорт
	...
	Если НаСервере = Неопределено Тогда
		НаСервере = ПолучитьРежимЗаписиНаСервереПоУмолчаниюЛкс();
	КонецЕсли; 
	Если ТипЗнч(ТабличноеПоле) = Тип("ТаблицаФормы") Тогда
		Если ПолноеИмяМД = Неопределено Тогда
	...
	Если ТабличноеПоле.ТекущиеДанные <> Неопределено Тогда
		КлючОбъекта = ПолучитьКлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, ТабличноеПоле.ТекущиеДанные, Истина, СтруктураКлючаСтроки, НаСервере);
	КонецЕсли;
	Если КлючОбъекта = Неопределено Тогда
		...
		Иначе
			КлючОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицы,,, Ложь, НаСервере);
		КонецЕсли;
	КонецЕсли; 
...
Функция ПолучитьКлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, Знач ТекущаяСтрока, ДляРегистровСоздатьНаборЗаписей = Ложь, выхСтруктураКлючаСтроки = Неопределено,
	НаСервере = Неопределено) Экспорт 
	...
	Тогда 
		...
		Тогда
			// Времено заменил вызов, чтобы не переводить на события на сервере сразу все
			//КлючОбъекта = ПолучитьНаборЗаписейПоКлючуЛкс(ПолноеИмяТаблицы, ТекущаяСтрока); 
			КлючОбъекта = ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицы, ТекущаяСтрока,, Ложь, НаСервере);
		Иначе
...
Процедура КоманднаяПанельВставитьКнопкиБуфераОбменаЛкс(Знач КоманднаяПанельПолеВвода)
	...
	КнопкаКопировать.Картинка = ПолучитьОбщуюКартинкуЛкс("ирКопировать"); // Здесь в расширении будет неявное обращение к серверу https://partners.v8.1c.ru/forum/topic/1635523
	...
	КнопкаВставить.Картинка = ПолучитьОбщуюКартинкуЛкс("ирВставить"); // Здесь в расширении будет неявное обращение к серверу https://partners.v8.1c.ru/forum/topic/1635523
