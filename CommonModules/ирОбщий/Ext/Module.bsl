...
Функция ИзменитьРегистрациюДляУзлаЛкс(УзелОбмена, КлючОбъекта, НовоеЗначение, РегистрироватьДвиженияВместеСДокументом = Ложь, ПропускатьДвиженияВнеПланаОбмена = Истина) Экспорт
	...
	Если РегистрироватьДвиженияВместеСДокументом Тогда
		...
		Если ЛиКорневойТипДокументаЛкс(ПолучитьПервыйФрагментЛкс(ОбъектМД.ПолноеИмя())) Тогда
			...
			Для Каждого МетаРегистр из ОбъектыМД Цикл
				...
				Если Не УзелОбмена.Метаданные().Состав.Содержит(МетаРегистр) Тогда
					Если ПропускатьДвиженияВнеПланаОбмена Тогда
						Продолжить;
					Иначе
						ВызватьИсключение "Движение документа по регистру " + ПолноеИмяМД +  " не может быть зарегистрировано в плане обмена " + УзелОбмена.Метаданные().Имя;
					КонецЕсли; 
				КонецЕсли; 
			...
			Если КоличествоЭлементов > 0 Тогда 
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Распределять Тогда
						УзелПриемник = УзлыПриемники[ИндексУзла];
						...
						Если ИндексУзла = УзлыПриемники.Количество() Тогда
							ИндексУзла = 0;
						КонецЕсли; 
					Иначе
						ПланыОбмена.ЗарегистрироватьИзменения(УзлыПриемники, Объект);
...
Процедура ИзменитьОтборКлиентаПоМетаданнымЛкс(ТабличноеПоле, Знач ИмяКолонкиМетаданных = "Метаданные") Экспорт 
	
	ДоступныеОбъекты = ТабличноеПоле.Значение.Выгрузить();
	ДоступныеОбъекты.Свернуть(ИмяКолонкиМетаданных);
	ДоступныеОбъекты = ДоступныеОбъекты.ВыгрузитьКолонку(ИмяКолонкиМетаданных);
	ЭлементОтбора = ТабличноеПоле.ОтборСтрок[ИмяКолонкиМетаданных];
	Если Истина
		И ЭлементОтбора.Использование
		И ЭлементОтбора.ВидСравнения = ВидСравнения.ВСписке 
	Тогда
		НачальноеЗначениеВыбора = ЭлементОтбора.Значение.ВыгрузитьЗначения();
	Иначе
	...
	Если ЗначениеВыбора <> Неопределено Тогда
		...
		СписокЗначений.ЗагрузитьЗначения(ЗначениеВыбора);
...
Функция ПолучитьФайлЗначенияДляИнтерактивногоСравненияЛкс(Значение, Название, ПолучатьXMLПредставлениеДляНеизвестныхТипов = Истина) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("ТабличныйДокумент") Тогда
		Документ = Значение;
	Иначе
		...
		КонецЕсли;
		Если ПолучатьXMLПредставлениеДляНеизвестныхТипов Тогда
...
Процедура ИзменитьПометкиВыделенныхСтрокЛкс(Знач ТабличноеПоле, ИмяКолонкиПометки = "Пометка", НовоеЗначениеПометки = Истина) Экспорт 
	...
	Если ВыделенныеСтроки.Количество() < 2 Тогда
		...
		Если ОтборСтрок <> Неопределено Тогда
			Построитель = ПолучитьПостроительТабличногоПоляСОтборомКлиентаЛкс(ТабличноеПоле);
			#Если _ Тогда
	...
	Для каждого Строка из ВыделенныеСтроки Цикл
		Если Истина
			...
			И НомераОтобранныхСтрок.Найти(Строка.НомерСтроки, "НомерСтроки") = Неопределено
		Тогда
			Продолжить;
		КонецЕсли; 
...
Функция ВыбратьСсылкуЛкс(ПолноИмяИлиОбъектМД, НачальноеЗначениеВыбора = Неопределено, ИспользоватьДинамическийСписокИР = Неопределено) Экспорт
	...
	КонецЕсли; 
	Результат = ОткрытьФормуСпискаСОтборомЛкс(ПолноИмяМД,, ИспользоватьДинамическийСписокИР,, Истина,, НачальноеЗначениеВыбора, Истина);
	Возврат Результат;
...
Функция ПолучитьТаблицуИзТабличногоДокументаЛкс(Знач ТабличныйДокумент, ЛиПерваяСтрокаСодержитИменаКолонок = Истина, ДлинаСтрокиТипаКолонки = 150,
	...
	//Индикатор = ПолучитьИндикаторПроцессаИис(ТабличныйДокумент.ВысотаТаблицы);
...
Процедура ДобавитьПоляНабораДанныхЛкс(ПолноеИмяТаблицыБД, СхемаКомпоновкиДанных) Экспорт
	...
	ИначеЕсли ЛиКорневойТипСсылкиЛкс(ТипТаблицы) Тогда
...
Процедура ДобавитьВыбранныеПоляКомпоновкиПоТаблицеБДЛкс(ПолноеИмяТаблицыБД, НастройкаКомпоновки) Экспорт
	...
	Если КорневойТип = "РегистрНакопления" Тогда
		Для каждого Ресурс Из ОбъектМД.Ресурсы Цикл
			...
			ИначеЕсли ИмяВиртуальнойТаблицы = "ОстаткиИОбороты" Тогда
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляНачальныйОстаток, Ресурс.Имя + "НачальныйОстаток", Ресурс.Синоним);
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляПриход, Ресурс.Имя + "Приход", Ресурс.Синоним);
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляРасход, Ресурс.Имя + "Расход", Ресурс.Синоним);
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляКонечныйОстаток, Ресурс.Имя + "КонечныйОстаток", Ресурс.Синоним);
			ИначеЕсли ИмяВиртуальнойТаблицы = "" Тогда
	...
	ИначеЕсли КорневойТип = "РегистрБухгалтерии" Тогда
		Для каждого Ресурс Из ОбъектМД.Ресурсы Цикл
			ВыбранныеПоля = НастройкаКомпоновки.Выбор;
			Если ИмяВиртуальнойТаблицы = "Обороты" Тогда
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " оборот Дт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " оборот Кт");
			ИначеЕсли ИмяВиртуальнойТаблицы = "ОборотыДтКт" Тогда
				Если Ресурс.Балансовый Тогда
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "Оборот", Ресурс.Синоним + " оборот");
				Иначе
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " оборот Дт");
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " оборот Кт");
				КонецЕсли;
			ИначеЕсли ИмяВиртуальнойТаблицы = "Остатки" Тогда
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "ОстатокДт", Ресурс.Синоним + " ост. Дт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "ОстатокКт", Ресурс.Синоним + " ост. Кт");
			ИначеЕсли ИмяВиртуальнойТаблицы = "ОстаткиИОбороты" Тогда
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляНачальныйОстаток, Ресурс.Имя + "НачальныйОстатокДт", Ресурс.Синоним + " нач. ост. Дт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляНачальныйОстаток, Ресурс.Имя + "НачальныйОстатокКт", Ресурс.Синоним + " нач. ост. Кт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляОбороты, Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " оборот Дт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляОбороты, Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " оборот Кт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляКонечныйОстаток, Ресурс.Имя + "КонечныйОстатокДт", Ресурс.Синоним + " кон. ост. Дт");
				НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоляКонечныйОстаток, Ресурс.Имя + "КонечныйОстатокКт", Ресурс.Синоним + " кон. ост. Кт");
			ИначеЕсли ИмяВиртуальнойТаблицы = "ДвиженияССубконто" Тогда
				Если Ресурс.Балансовый Тогда
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя, Ресурс.Синоним);
				Иначе
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "Дт", Ресурс.Синоним + " Дт");
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя + "Кт", Ресурс.Синоним + " Кт");
				КонецЕсли;
			ИначеЕсли ИмяВиртуальнойТаблицы = "" Тогда
				Если Ресурс.Балансовый Тогда
					НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Ресурс.Имя, Ресурс.Синоним);
			...
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ЛиКорневойТипСсылкиЛкс(КорневойТип) Тогда
		Если ИмяВиртуальнойТаблицы = "" Тогда
			ОбъектМетаданных = ОбъектМД;
		Иначе
			ОбъектМетаданных = ОбъектМД.ТабличныеЧасти[ИмяВиртуальнойТаблицы];
		КонецЕсли;
		Для каждого Реквизит Из ОбъектМетаданных.Реквизиты Цикл
			ВыбранныеПоля = НастройкаКомпоновки.Выбор;
			НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ВыбранныеПоля, Реквизит.Имя);
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьСхемуКомпоновкиПоВсемТаблицамБДЛкс(ТаблицаВсехТаблицБД, ИмяНабораДанных = "НаборДанных1",
...
Функция ДобавитьДоступнуюТаблицуБДЛкс(ДоступныеТаблицыБД, ПолноеИмя, ТипТаблицы = "", Имя = "", Представление = "", СхемаТаблицы = "", ПроверятьУникальность = Ложь,
	ОбъектМД = Неопределено) Экспорт 
...
Функция ПолучитьСтруктуруКлючаТаблицыБДЛкс(Знач ПолноеИмяТаблицыБД, ВключатьНомерСтроки = Истина, ВернутьСтруктуру = Истина, ДляПодчиненногоРегистраДобавлятьРегистраторИНомерСтроки = Ложь) Экспорт
	...
	Если Ложь
		Или ЛиКорневойТипПеречисленияЛкс(ТипТаблицы) 
...
Функция ЗаменитьЗначениеВОбъектеБДЛкс(Объект, ЧтоЗаменять, НаЧтоЗаменять) Экспорт
	...
	//Компоновщик = ирКэш.ПолучитьКомпоновщикТаблицыБДПоМетаданнымЛкс(ОбъектМД.ПолноеИмя()); // Слишком длительное ожидание дает на больших конфигурациях
	#Если _ Тогда
	...
	Тогда
		...
		Иначе
			...
			ИначеЕсли Фрагменты[0] = "ВнешнийИсточникДанных" Тогда 
				...
				КонецЕсли; 
			//ИначеЕсли Фрагменты[0] = "РегистрБухгалтерии" Тогда 
			ИначеЕсли ЛиКорневойТипРегистраБДЛкс(Фрагменты[0]) Тогда  
				Результат = ОбразующийМД;
			Иначе
...
Функция ПолучитьТекстЗапросаПоВыбраннымТаблицамЛкс(МассивИменТаблиц, ВариантИсточников = 0, ПервыеN = 0) Экспорт
	...
	Для Каждого ИмяТаблицы Из МассивИменТаблиц Цикл
		...
		//КомпоновщикТаблицы = ирКэш.ПолучитьКомпоновщикТаблицыБДПоПолномуИмениЛкс(ИмяТаблицы); // Долгое первое выполнение на больших конфигурациях
		КомпоновщикТаблицы = ирКэш.ПолучитьКомпоновщикТаблицыМетаданныхЛкс(ИмяТаблицы);
	...
	Для Каждого ИмяПоля Из ОбщиеМетаПоля Цикл
		Если Истина
			И ЛиКорневойТипСсылкиЛкс(МассивИменТаблиц[0]) 
	...
	#Если Клиент Тогда
		Индикатор = ПолучитьИндикаторПроцессаЛкс(МассивИменТаблиц.Количество(), "Генерация текста запроса");
	#КонецЕсли
	Для Каждого ИмяТаблицы Из МассивИменТаблиц Цикл
		#Если Клиент Тогда
			ОбработатьИндикаторЛкс(Индикатор);
		...
		Если Текст <> "" Тогда
			...
			|";
		КонецЕсли;
		Текст = Текст + "ВЫБРАТЬ """ + ИмяТаблицы + """ КАК _ПолноеИмяТаблицы" + ТекстНеобязательныхПолей + ТекстОбщихМетаПолей + " ИЗ " + ИмяТаблицы + " КАК Т"
			+ ТекстУсловияСоединения;
	КонецЦикла;
	Если ЗначениеЗаполнено(ПервыеN) Тогда
		Текст = "ВЫБРАТЬ ПЕРВЫЕ " + XMLСтрока(ПервыеN) + " * ИЗ (" + Текст + ") КАК Т";
	КонецЕсли;
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
	Возврат Текст;
...
Функция СериализоватьДополнительныеСвойстваОбъектаЛкс(Объект) Экспорт 
	...
	Если лОбменДанными <> Неопределено Тогда
		Узлы = Новый Массив;
		Для Каждого Получатель Из лОбменДанными.Получатели Цикл
			Узлы.Добавить(Получатель);
		КонецЦикла; 
		Получатели = Новый Структура;
		Получатели.Вставить("Автозаполнение", лОбменДанными.Получатели.Автозаполнение);
		Получатели.Вставить("Узлы", Узлы);
		СтруктураОбменаДанными = Новый Структура;
		СтруктураОбменаДанными.Вставить("Загрузка", лОбменДанными.Загрузка); 
		СтруктураОбменаДанными.Вставить("Отправитель", лОбменДанными.Отправитель); 
...
Процедура ЗаписатьОбъектЛкс(Объект, НаСервере = Ложь, РежимЗаписи = Неопределено, РежимПроведения = Неопределено, ОтключатьКонтрольЗаписи = Неопределено,
	...
	Если НаСервере Тогда
		...
		ирСервер.ЗаписатьОбъектXMLЛкс(СтрокаXML, ДополнительныеСвойства, РежимЗаписи, РежимПроведения, ОтключатьКонтрольЗаписи, БезАвторегистрацииИзменений);
		...
		Для СчетчикПопыток = 1 По ПопытокЗаписиОбъекта Цикл
			...
			Исключение
				...
				Тогда
					...
					ПеречитатьОбъектЗапросомЛкс(Объект);
					Если Объект <> Неопределено Тогда
...
Функция ПеречитатьОбъектЗапросомЛкс(Объект) Экспорт 
...
Функция ЗапуститьСеансПодПользователемЛкс(ИмяПользователяИнфобазы, ПарольПользователяИнфобазы, ТипCOMОбъекта = "", РежимЗапуска = "Авто", КодРазрешения = "",
	...
	Если ЗначениеЗаполнено(ТипCOMОбъекта) Тогда
		Соединение = ирОбщий.СоздатьСеансИнфобазы1С8Лкс(, ИмяПользователяИнфобазы, ПарольПользователяИнфобазы, ТипCOMОбъекта, Истина,,,, КодРазрешения);
	Иначе
		#Если Клиент Тогда
		...
		КонецЕсли; 
		ПараметрыЗапуска = ирОбщий.ПолучитьПараметрыЗапускаПриложения1СТекущейБазыЛкс(ИмяПользователяИнфобазы, ПарольПользователяИнфобазы, КодРазрешения, Ложь, КонечныйРежимЗапуска,
...
Функция ПолучитьТаблицуЗначенийСМинимальнымиТипамиКолонокЛкс(Знач ТаблицаДанных, СужатьТолькоПроизвольныеКолонки = Ложь) Экспорт
	...
	Если МетаданныеТаблицыИзменены Тогда
		Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ТаблицаДанных.Количество(), "Оптимизация таблицы");
...
Процедура ПолеВвода_ОкончаниеВводаТекстаЛкс(Элемент, Текст, Значение, СтандартнаяОбработка, РасширенноеЗначение = Null, 
	...
	КонецЕсли;
	Менеджер = Неопределено;
	Если ЭтоТипСсылкиНаОбъектБДЛкс(ТипЗнч(Элемент.Значение)) Тогда 
...
Функция ОткрытьФормуПроизвольногоЗначенияЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина,
	...
	Тогда
		Если ЭтоТипСсылкиНаОбъектБДЛкс(ТипРасширенногоЗначения) Тогда
			Если Истина
			...
			Если ХмлТип <> Неопределено Тогда
				Если Найти(ХмлТип.ИмяТипа, "Ref.") > 0 Тогда 
					Если Истина
						И ЗначениеЗаполнено(РасширенноеЗначение) 
						И (Ложь
							Или Не ЭтоТипСсылкиНаОбъектБДЛкс(ТипРасширенногоЗначения)
...
Функция ОткрытьПодборСВыборомТипаЛкс(ВладелецФормы, ОписаниеТипов = Неопределено, Знач НачальноеЗначениеВыбора = Неопределено, ИспользоватьДинамическийСписокИР = Неопределено) Экспорт
	...
	Тогда
		...
		Форма.НачальноеЗначениеВыбора = Новый Структура("ДоступныеОбъекты, ОтображатьСсылочныеОбъекты, НачальноеЗначениеВыбора", ДоступныеОбъекты, Истина, ПолноеИмяНачальногоТипаВыбора);
	...
	Если НачальноеЗначениеВыбора <> Неопределено Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	ФормаВыбора = ОткрытьФормуСпискаСОтборомЛкс(ПолноеИмяМД, Отбор, ИспользоватьДинамическийСписокИР, ВладелецФормы, Истина, Истина, ТекущаяСтрока);
	Возврат ФормаВыбора;
...
Функция ОткрытьФормуСпискаСОтборомЛкс(ПолноеИмяМД, Отбор, ИспользоватьДинамическийСписокИР = Неопределено, ВладелецФормы = Неопределено, РежимВыбора = Ложь,
...
Функция ПолучитьПостроительВыборкиСвязанныхСтрокЛкс(ПолноеИмяТаблицы, ИмяКолонки, ЗначениеОтбора, МаксимальнаяПорция = 0) Экспорт 
	...
	//Компоновщик = ирКэш.ПолучитьКомпоновщикТаблицыБДПоПолномуИмениЛкс(ПолноеИмяТаблицы); // Долгое первое выполнение на больших конфигурациях
	Компоновщик = ирКэш.ПолучитьКомпоновщикТаблицыМетаданныхЛкс(ПолноеИмяТаблицы);
	#Если _ Тогда
	    Компоновщик = Новый КомпоновщикНастроекКомпоновкиДанных;
	#КонецЕсли
	ТекстПоля = "";
	...
	Для Каждого ДоступноеПоле Из Компоновщик.Настройки.ДоступныеПоляОтбора.Элементы Цикл
		...
		Если ЗначениеЗаполнено(ТекстПоля) Тогда
			ТекстПоля = ТекстПоля + ", ";
		КонецЕсли; 
		ТекстПоля = ТекстПоля + ПсевдонимТаблицы + "." +ДоступноеПоле.Поле;
	КонецЦикла;
	ТекстПервые = "";
...
Функция ЗагрузитьСвязанныеСтрокиТаблицыБДЛкс(ЭтаФорма, ТабличноеПолеСвязанныхКолонок, ТабличноеПолеСвязанныхСтрок, КоманднаяПанельСвязанныхСтрок, мВыборкаРезультатаСтрокиТаблицы,
	...
	Исключение
		...
		Возврат Неопределено;
	КонецПопытки; 
	ирОбщий.ЗагрузитьДинамическуюВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, РезультатСтрокиТаблицы, мВыборкаРезультатаСтрокиТаблицы, КоманднаяПанельСвязанныхСтрок);
...
Функция ЭтоТипСсылкиНаОбъектБДЛкс(Тип) Экспорт 
...
Процедура ЗаблокироватьНаборЗаписейПоОтборуЛкс(НаборЗаписей) Экспорт 
...
Процедура ЗаблокироватьСсылкуВТранзакцииЛкс(СсылочныйОбъект) Экспорт 
...
Процедура ЗаблокироватьКонстантуЛкс(КонстантаМенеджерЗначения) Экспорт 
...
Функция ПолучитьПредставлениеПоляБДЛкс(СтрокаПоля, ЛиИменаБД, ЭтоТабличнаяЧасть = Ложь, ИспользоватьИмяПоляВместоПустогоПредставления = Ложь) Экспорт 
...
Функция ПолучитьПредставлениеИндексаХраненияЛкс(СтрокаИндексаСтруктурыБД, ЛиИменаБД, СтрокаТаблицыХранения, ИспользоватьИмяПоляВместоПустогоПредставления = Ложь) Экспорт 
	...
	Для каждого СтрокаПоля Из СтрокаИндексаСтруктурыБД.Поля Цикл
		ПредставлениеПоля = ПолучитьПредставлениеПоляБДЛкс(СтрокаПоля, ЛиИменаБД, ЭтоТабличнаяЧасть, Истина);
		ПредставлениеИндекса = ПредставлениеИндекса + Разделитель + ПредставлениеПоля;
		Разделитель = ", ";
	КонецЦикла;
	ПредставлениеИндекса = "Индекс(" + ПредставлениеИндекса + ")";
...
Процедура НастроитьАвтоТабличноеПолеДинамическогоСписка(ОсновнойЭУ) Экспорт
	...
	Если ирОбщий.ПолучитьПервыйФрагментЛкс(Метаданные.НайтиПоТипу(ТипЗнч(ОсновнойЭУ.Значение)).ПолноеИмя()) <> "РегистрБухгалтерии" Тогда 
...
Функция УстановитьВременныеСвойстваПользователюИБЛкс(ПользовательИБ, ПодменитьПарольНаВремяЗапуска = Истина, ВременноПредоставитьПравоРазработчикИР = Истина) Экспорт
	...
	Если ПодменитьПарольНаВремяЗапуска Тогда
		...
		КонецЕсли;
		Пароль = Формат(ТекущаяДата(), "ДФ=HHmmss") + XMLСтрока(НомерСеансаИнформационнойБазы());
