...
Функция ВыбратьКаталогВФормеЛкс(Каталог, ЭтаФорма = Неопределено) Экспорт
...
Функция ЗапуститьСеансПодПользователемЛкс(ИмяПользователяИнфобазы, ПарольПользователяИнфобазы, ТипCOMОбъекта = "", РежимЗапуска = "Авто", КодРазрешения = "",
	...
	РежимИнтерфейсаТакси = Ложь) Экспорт 
...
Функция ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ТабличноеПоле, СтандартнаяОбработка = Ложь, РасширенноеЗначение = Null,
	...
	Если Результат Тогда
		ВыбраннаяСтрока[Данные] = РасширенноеЗначение;
	КонецЕсли; 
	Возврат Результат;
...
Функция ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ТабличноеПоле, СтандартнаяОбработка, РасширенноеЗначение = Null, ИспользоватьОграничениеТипа = Ложь,
	...
	КонецЕсли; 
	Если РасширенноеЗначение = Null Тогда
		РасширенноеЗначение = ТабличноеПоле.ТекущаяСтрока[Данные];
	КонецЕсли; 
	ЗначениеИзменено = Ложь;
	Если РасширенноеЗначение = Неопределено Тогда
		...
		Если НовыйТип <> Неопределено Тогда
			...
			НовоеЗначение = НовоеОписаниеТипов.ПривестиЗначение(Неопределено);
			РасширенноеЗначение = НовоеЗначение;
			ТабличноеПоле.ТекущаяСтрока[Данные] = РасширенноеЗначение;
			ЗначениеИзменено = Истина;
...
Функция ОткрытьФормуСпискаЛкс(ПолноеИмяМД, Отбор = Неопределено, ИспользоватьДинамическийСписокИР = Неопределено, ВладелецФормы = Неопределено, РежимВыбора = Ложь,
	...
	ФормаСписка = ПолучитьФормуСпискаЛкс(ПолноеИмяМД, Отбор, ИспользоватьДинамическийСписокИР, ВладелецФормы, РежимВыбора, МножественныйВыбор, ТекущаяСтрока);
...
// Параметры:
//   Отбор - Структура, Отбор, *Неопределено
Функция ПолучитьФормуСпискаЛкс(ПолноеИмяМД, Отбор = Неопределено, ИспользоватьДинамическийСписокИР = Неопределено, ВладелецФормы = Неопределено, РежимВыбора = Ложь,
	...
	Если ИспользоватьДинамическийСписокИР = Неопределено Тогда
		ИспользоватьДинамическийСписокИР = ПолучитьИспользованиеДинамическогоСпискаВместоОсновнойФормыЛкс(ПолноеИмяМД);
	КонецЕсли; 
	Если Истина
...
Функция ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ТабличноеПоле, ПолноеИмяМД = Неопределено, ДоступныеПоляВыбора = Неопределено, Связанный = Ложь,
	...
	Если ФормаРедактора = Неопределено Тогда
		...
		Если Связанный Тогда
			ФормаРедактора.СоединяемоеОкно = Истина;
			ФормаРедактора.КлючСохраненияПоложенияОкна = "Прикрепленное";
			ФормаРедактора.СостояниеОкна = ВариантСостоянияОкна.Прикрепленное;
			ФормаРедактора.ПоложениеПрикрепленногоОкна = ВариантПрикрепленияОкна.Низ;
		КонецЕсли; 
	КонецЕсли; 
	...
	Иначе
		...
		Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				...
				Иначе
					ПоказатьПредупреждение(, "При запуске сеанса из конфигуратора автоматически включается контроль модальности. Запущен новый сеанс без контроля модальности.");
				КонецЕсли; 
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Функция ОткрытьФормуСоединенияСУБДЛкс(Автоподключение = Ложь) Экспорт
	...
	ФормаПодключения = ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ПараметрыСоединенияСУБД");
...
Функция ПолучитьТекстРезультатаКомандыОСЛкс(Знач СтрокаКоманды = "", ОжидатьЗавершения = Истина, Знач ИмяКомпьютера = "") Экспорт
	
	ФайлРезультата = Новый Файл(ПолучитьИмяВременногоФайла("txt"));
	ирКэш.Получить().ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(СтрокаКоманды, ФайлРезультата.Путь,, ФайлРезультата.Имя, ОжидатьЗавершения);
