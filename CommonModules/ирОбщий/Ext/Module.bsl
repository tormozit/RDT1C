...
Функция ВывестиСтрокиТабличногоПоляЛкс(Знач ТабличноеПоле, Знач ПараметрыВывода, Отладка = Ложь, выхТекущаяСтрока = Неопределено) Экспорт
	...
	Если ТипИсточника = "Список" Тогда
		КлючТекущейСтроки = ПолучитьСтруктуруКлючаТаблицыБДЛкс(ПолучитьИмяТаблицыИзМетаданныхЛкс(СтруктураТипа.Метаданные));
	...
	Если ПараметрыВывода.БезОформления Тогда
		...
		Если ТипИсточника = "Список" Тогда
			...
			ТрансформироватьОтборВОтборКомпоновкиЛкс(НастройкаКомпоновки.Отбор, ТабличноеПоле.Значение.Отбор,, Ложь,, Истина);
			ТрансформироватьПорядокВПорядокКомпоновкиЛкс(НастройкаКомпоновки.Порядок, ТабличноеПоле.Значение.Порядок);
			СхемаКомпоновки = ПолучитьСхемуКомпоновкиПоОбъектуМетаданныхЛкс(ОбъектМД);
			Если ПараметрыВывода.ТолькоВыделенныеСтроки Тогда
...
Функция ПреобразоватьПредставлениеВСсылкуЛкс(Знач МенеджерИлиОбъектМД, Знач Текст, КэшПоиска = Неопределено) Экспорт
	...
	Иначе
		...
		//			Менеджер = ПолучитьМенеджерЛкс(ТипРеквизита);
...
Функция ВывестиТаблицуВТабличныйДокументИлиТаблицуЗначенийЛкс(ТаблицаЗначений, Знач Приемник = Неопределено, ДанныеРасшифровки = Неопределено, ИтогиЧисловыхКолонок = Истина,
	...
	Иначе
		Приемник = СкомпоноватьВТабличныйДокументЛкс(СхемаКомпоновки, НастройкаКомпоновки, Приемник, ВнешниеНаборыДанных, ДанныеРасшифровки, АвтофиксацияШапки,, Истина);
		#Если Сервер И Не Сервер Тогда
