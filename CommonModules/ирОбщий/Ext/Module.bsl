	...
	Если ТипЗнч(ТабличноеПоле.Значение) = Тип("ТаблицаЗначений") Тогда
		...
		ИначеЕсли ВыбранныйВариант.Значение = 0 Тогда
			...
			Если СледующаяКолонка.Данные = "" Тогда
				Возврат;
			КонецЕсли; 
			ФормаОбработки.ОткрытьСЗаполнениемГруппДублейПоТаблицеПар(ТабличноеПоле.Значение, СледующаяКолонка.Данные, ТекущаяКолонка.Данные);
		КонецЕсли;
...
Функция ВыполнитьКомандуКонфигуратораЛкс(Знач КомандаКонфигуратора, Знач СтрокаСоединенияБазы = "", выхТекстЛога = "", ПодавлятьДиалоги = Ложь) Экспорт 
...
Функция ПреобразоватьЗначениеИзSDBLЛкс(ЗначениеSDBL, ЧужаяСхемаБД = Ложь) Экспорт
	...
	ПолноеИмяМД = ПолучитьМетаданныеПоНомеруСсылочнойТаблицыЛкс(СтрокаНомераТаблицы, ЧужаяСхемаБД);
...
Функция ПолучитьМетаданныеПоНомеруСсылочнойТаблицыЛкс(СтрокаНомерТаблицы, ЧужаяСхемаБД = Ложь) Экспорт
	
	СтруктураБД = ирКэш.ПолучитьСтруктуруХраненияБДЛкс(,, ЧужаяСхемаБД);
	#Если Сервер И Не Сервер Тогда
	    СтруктураБД = Новый ТаблицаЗначений;
	#КонецЕсли
	СловарьШаблоновМетаданных = ирКэш.ПолучитьСловарьШаблоновМетаданныхЛкс(, ЧужаяСхемаБД);
...
Функция ПрочитатьДополнительныеПоляСсылающихсяОбъектовЛкс(Знач ТабличноеПоле, Знач КомпоновщикДопПолей, Знач ТаблицаДанных = Неопределено, ИмяПоляСсылки = "Данные") Экспорт 
	...
	Для СчетчикПорций = 1 По КоличествоПорций Цикл
		...
		Для Счетчик = 1 По РазмерПорции Цикл
			...
			Если ТипЗнч(СтрокаОбъекта[ИмяПоляСсылки]) = Тип("Строка") Или СтрокаОбъекта[ИмяПоляСсылки] = Неопределено Тогда
...
Функция ПолучитьСтруктуруХраненияБДЛкс(Знач ОтборПоМетаданным = Неопределено, ЛиИменаБД = Ложь, ВычислитьИменаИндексов = Истина, ЧужаяСхемаБД = Ложь) Экспорт
	...
	КонецЕсли; 
	Если ЧужаяСхемаБД Тогда
		СтруктураСхемы = ирКэш.Получить().ЧужаяСхемаБД;
		Если ЛиИменаБД Тогда
