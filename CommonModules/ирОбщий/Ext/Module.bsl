...
Функция ИзменитьРегистрациюДляУзлаЛкс(УзелОбмена, КлючОбъекта, НовоеЗначение, РегистрироватьДвиженияВместеСДокументом = Ложь, ПропускатьДвиженияВнеПланаОбмена = Истина) Экспорт
	...
	Если РегистрироватьДвиженияВместеСДокументом Тогда
		...
		Если ЛиКорневойТипДокументаЛкс(ПолучитьПервыйФрагментЛкс(ОбъектМД.ПолноеИмя())) Тогда
			ОбъектыМД = ирОбщий.ПолучитьМетаданныеНаборовЗаписейПоРегистраторуЛкс(ОбъектМД, , Истина);
			Для Каждого МетаРегистр из ОбъектыМД Цикл
				Если Не УзелОбмена.Метаданные().Состав.Содержит(МетаРегистр) Тогда
					Если ПропускатьДвиженияВнеПланаОбмена Тогда
						Продолжить;
					Иначе
						ВызватьИсключение "Движение документа по регистру " + МетаРегистр.ПолноеИмя() +  " не может быть зарегистрировано в плане обмена " + УзелОбмена.Метаданные().Имя;
					КонецЕсли; 
				КонецЕсли; 
				...
				НаборЗаписей.Отбор[0].Установить(КлючОбъекта.Ссылка);
				Если НовоеЗначение Тогда
...
Процедура ВыбратьИЗаполнитьТабличнуюЧастьОбъектаБДЛкс(ТаблицаИсточник, НачальноеПолноеИмяОбъекта = "") Экспорт 
	...
	Иначе
		ВыбраннаяСтрока = ОткрытьФормуСпискаСОтборомЛкс(ПолноеИмяМДСписка,,,, Истина,,, Истина);
...
Функция ПолучитьТекстЗапросаПоВыбраннымТаблицамЛкс(МассивИменОбъектовМД, ВариантИсточников = 0, ПервыеNКаждойТаблицы = 0, ПодключатьПоляКоличестваДвижений = Ложь) Экспорт
	...
	Для Каждого ИмяОбъектаМД Из МассивИменОбъектовМД Цикл
		...
		КонецЕсли;
		ТекстЗапроса = ТекстЗапроса + """" + ИмяТаблицы + """ КАК _ПолноеИмяТаблицы" + ТекстНеобязательныхПолей + ТекстОбщихМетаПолей + " ИЗ " + ИмяТаблицы + " КАК Т"
			+ ТекстУсловияСоединения;
	КонецЦикла;
...
Процедура УстановитьПараметрыЗаписиОбъектаЛкс(Знач Объект, ОтключатьКонтрольЗаписи = Неопределено, БезАвторегистрацииИзменений = Неопределено) Экспорт
	...
	Если лОбменДанными <> Неопределено Тогда
		...
		Если БезАвторегистрацииИзменений <> Неопределено Тогда
			Попытка
				Получатели = Объект.ОбменДанными.Получатели;
			Исключение
				// Элемент плана обмена в 8.3
...
Функция НайтиДобавитьПараметрСсылкуВТаблицуЛкс(ТаблицаПараметров, ИмяКолонкиИмени = "Имя", ИмяКолонкиЗначения = "Значение", ЗначениеПараметра,
	...
	Иначе
		...
		Иначе
			...
			ДопустимаяДлинаСтроки = ТаблицаПараметров.Колонки[ИмяКолонкиИмени].ТипЗначения.КвалификаторыСтроки.Длина;
...
// Результат - Булево - Истина если значение было изменено
Функция ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ТабличноеПоле, СтандартнаяОбработка, РасширенноеЗначение = Null, ИспользоватьОграничениеТипа = Ложь) Экспорт
...
Функция ПолучитьФормуВыбораОбъектаМетаданныхЛкс(ВладелецФормы, КлючУникальности, НачальноеЗначениеВыбора, МножественныйВыбор = Ложь, 
	ОтображатьСсылочныеОбъекты = Ложь, ОтображатьВыборочныеТаблицы = Ложь, ОтображатьРегистры = Ложь, ОтображатьПоследовательности = Ложь,
	ОтображатьКонстанты = Ложь, ОтображатьТабличныеЧасти = Ложь, ОтображатьТаблицыИзменений = Ложь, ОтображатьВнешниеИсточникиДанных = Ложь, ЗапретитьВыбиратьСсылочныеОбъекты = Ложь) Экспорт 
...
Процедура СчитатьПорциюДинамическойВыборкиВТаблицуЛкс(Выборка, ТаблицаПриемник, Знач РазмерПорции = 9999, СчитыватьЧерезКопиюТаблицы = Истина,
...
Процедура ЗагрузитьДинамическуюВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, РезультатЗапроса, ВыборкаРезультата, КоманднаяПанель,
	...
	Иначе
		...
		СчитатьПорциюДинамическойВыборкиВТаблицуЛкс(ВыборкаРезультата, ТабличноеПоле.Значение, БезопасныйПорогКоличестваСтрок, , СсылкаНаБуфернуюТаблицу);
	КонецЕсли; 
...
Процедура ЗагрузитьДинамическуюВыборкуВТабличноеПолеПолностьюЛкс(ЭтаФорма, мВыборкаРезультата, КоманднаяПанель,
	...
	ЭтаФорма.ПодключитьОбработчикОжидания(ИмяОбработчикаОбновления, 0.1, Истина);
	ТабличноеПоле = КоманднаяПанель.ИсточникДействий;
	СчитатьПорциюДинамическойВыборкиВТаблицуЛкс(мВыборкаРезультата, ТабличноеПоле.Значение, 0, , СсылкаНаБуфернуюТаблицу);

КонецПроцедуры

// Параметру КоличествоРезультата внутри присваивается значение!
Процедура ПослеЗагрузкиДинамическойВыборкиВТабличноеПолеЛкс(ЭтаФорма, мВыборкаРезультата, КоманднаяПанель, ПолеСтрокиКоличестваРезультата, СсылкаНаБуфернуюТаблицу = Неопределено) Экспорт
...
Процедура НастроитьДобавленныеКолонкиТабличногоПоляЛкс(Знач ТабличноеПоле, МетаданныеКолонок = Неопределено) Экспорт 
	
	Если МетаданныеКолонок = Неопределено Тогда
		МетаданныеКолонок = ТабличноеПоле.Значение.Колонки;
	КонецЕсли; 
	Для Каждого КолонкаТаблицы Из МетаданныеКолонок Цикл
