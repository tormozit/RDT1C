...
Процедура СравнитьСодержимоеЭлементаУправленияЛкс(ЭлементУправления) Экспорт
	...
	ИначеЕсли ТипЗнч(ЭлементУправления) = Тип("ТабличноеПоле") Тогда
		...
		Если Ответ = КодВозвратаДиалога.Нет Тогда 
			СравниваемыйДокумент = ВывестиТабличноеПолеКоллекцииВТабличныйДокументСНастройкойЛкс(ЭлементУправления, Ложь);
		Иначе
			СравниваемыйДокумент = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(ЭлементУправления.Значение)); // копируем данные
		КонецЕсли; 
...
Функция ВывестиТабличноеПолеКоллекцииВТабличныйДокументСНастройкойЛкс(Знач ТабличноеПоле, ВыводБезОформления = Ложь) Экспорт 
	
	ТипИсточника = ОбщийТипДанныхТабличногоПоляЛкс(ТабличноеПоле);
	Если ТипИсточника = "Список" Тогда
		Сообщить("Вывод в табличный документ для табличного поля динамического списка не реализован");
		Возврат Неопределено;
	КонецЕсли; 
	...
	//КонецЕсли; 
	ФормаНастройки = ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ПараметрыВыводаВТабличныйДокумент");
	...
	Если РезультатФормы = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
	Результат = ВывестиТабличноеПолеКоллекцииВТабличныйДокументЛкс(ТабличноеПоле, ФормаНастройки);
	Возврат Результат;

КонецФункции

Функция ВывестиТабличноеПолеКоллекцииВТабличныйДокументЛкс(Знач ТабличноеПоле, Знач ПараметрыВывода, Отладка = Ложь) Экспорт
	...
	Если ТипЗнч(ТабличноеПоле.Значение) = Тип("ДеревоЗначений") Тогда
		...
		Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда
			ИндексСтроки = КоллекцияСтрок.Индекс(ТабличноеПоле.ТекущаяСтрока);
		КонецЕсли;
	Иначе
		Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда
			ИндексСтроки = ТабличноеПоле.Значение.Индекс(ТабличноеПоле.ТекущаяСтрока);
		КонецЕсли; 
		Если ТипЗнч(ТабличноеПоле.Значение) = Тип("ТаблицаЗначений") Тогда
	...
	Если ПараметрыВывода.БезОформления Тогда
		...
		Если ТабличноеПоле.ТекущаяКолонка <> Неопределено Тогда
			ИмяТекущейКолонки = ПолучитьПутьКДаннымКолонкиТабличногоПоляЛкс(ТабличноеПоле, ТабличноеПоле.ТекущаяКолонка);
		КонецЕсли; 
		Результат = ВывестиТаблицуВТабличныйДокументЛкс(КоллекцияСтрок,,, ПараметрыВывода.ИтогиЧисловыхКолонок,, ПараметрыВывода.ВстроитьЗначенияВРасшифровки, 
	...
	Иначе
		...
		Для Каждого ВыбраннаяКолонка Из ВыбранныеКолонки Цикл
			...
			КонецЕсли; 
		КонецЦикла;
		ВывестиСтрокиТабличногоПоляКоллекцииВТабличныйДокументЛкс(КоллекцияСтрок, НомерСтроки, Результат, ТабличноеПоле,, ПараметрыВывода.ВстроитьЗначенияВРасшифровки);
...
Процедура ВывестиТабличноеПолеВТабличныйДокументИПоказатьЛкс(Знач ТабличноеПоле, Знач ВыводБезОформления = Неопределено) Экспорт 
	
	ТабличныйДокумент = ВывестиТабличноеПолеКоллекцииВТабличныйДокументСНастройкойЛкс(ТабличноеПоле, ВыводБезОформления);
	ОткрытьЗначениеЛкс(ТабличныйДокумент,,,, Ложь);
...
Процедура ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ТабличноеПоле, Знач Колонка, Знач Значение, Знач ФормаИнициатор = Неопределено,
	...
	КонецЕсли;
	Если Не ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементУправления, Значение, ФормаИнициатор) Тогда 
		Возврат;
	КонецЕсли; 
	Если Истина
...
Функция ЗаписатьОбъектДляОтладкиЛкс(Объект, АдресРезультата = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Объект) = Тип("Структура") Тогда
			...
			ХранилищеОбщихНастроек.Сохранить(ирОбщий.ИмяПродуктаЛкс(), Результат, Объект);
		Иначе
			Объект.Записать();
	...
	Если СсылкаИлиИмяФайла = Неопределено Тогда
		...
		Если ТипЗнч(СсылкаИлиИмяФайла) = Тип("Строка") Тогда
			...
			ИначеЕсли Результат.Item(0).SubMatches(2) <> Неопределено Тогда 
				...
				СтруктураПараметров = ХранилищеОбщихНастроек.Загрузить(ирОбщий.ИмяПродуктаЛкс(), "Объект для отладки", , );
				Если СтруктураПараметров = Неопределено Тогда
...
Функция ПреобразоватьПредставлениеВСсылкуЛкс(Знач МенеджерИлиОбъектМД, Знач Текст) Экспорт
...
Функция ОткрытьПодборИОбработкуОбъектовИзТабличногоПоляДинамическогоСпискаЛкс(ТабличноеПоле) Экспорт 
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		КонецЦикла;
		Форма = ОткрытьМассивОбъектовВПодбореИОбработкеОбъектовЛкс(ПараметрКоманды, ВыбранныеПоля);
	Иначе
		...
		ТрансформироватьПорядокВПорядокКомпоновкиЛкс(Форма.Компоновщик.Настройки.Порядок, ТабличноеПоле.Значение.Порядок);
	КонецЕсли;
	Возврат Форма;
...
Функция ВывестиТаблицуВТабличныйДокументЛкс(ТаблицаЗначений, Знач ТабличныйДокумент = Неопределено, ДанныеРасшифровки = Неопределено, ИтогиЧисловыхКолонок = Истина,
	...
	ДобавлятьКолонкиПредставлений = Ложь, Знач ВыбранныеКолонки = Неопределено, ИмяТекущейКолонки = "", Отладка = Ложь) Экспорт
	...
	Для Каждого ВыбраннаяКолонка Из ВыбранныеКолонки Цикл
		...
		Если ДобавлятьКолонкиПредставлений Тогда
			Если ЗначениеЗаполнено(ИмяТекущейКолонки) И СтрокиРавныЛкс(ВыбраннаяКолонка, ИмяТекущейКолонки) Тогда
		...
		КонецЕсли; 
		Колонка = ТаблицаЗначений.Колонки[ВыбраннаяКолонка];
		Если Истина
		...
		Если ДобавлятьКолонкиИдентификаторов Тогда
			Если ИндексТекущейКолонки = 0 И ЗначениеЗаполнено(ИмяТекущейКолонки) И СтрокиРавныЛкс(ВыбраннаяКолонка, ИмяТекущейКолонки) Тогда
				ИндексТекущейКолонки = НастройкаКомпоновки.Выбор.Элементы.Количество();
			КонецЕсли; 
			НужноДобавлятьКолонку = Истина;
			Если ДобавлятьКолонкиПредставлений Тогда
				Для Каждого ТипКолонки Из Колонка.ТипЗначения.Типы() Цикл
					Если ПримитивныеТипы.Найти(ТипКолонки) <> Неопределено Тогда
						НужноДобавлятьКолонку = Ложь;
						Прервать;
					КонецЕсли; 
				КонецЦикла;
			КонецЕсли; 
			Если НужноДобавлятьКолонку Тогда
		...
		Если ОтображатьПустые Тогда
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Формат", "ЧН=; ДП=; БЛ=");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		Для Каждого ИмяКолонки Из КолонкиТипов Цикл
			СтрокаТаблицы[ИмяКолонки + "_ИмяТипаЗначения_"] = ПредставлениеТипаЛкс(ТипЗнч(СтрокаТаблицы[ИмяКолонки]), , Истина);
		КонецЦикла;
	...
	Если Отладка Тогда
		...
		Возврат Неопределено;
	КонецЕсли; 
	ТабличныйДокумент = СкомпоноватьВТабличныйДокументЛкс(СхемаКомпоновки, НастройкаКомпоновки, ТабличныйДокумент, ВнешниеНаборыДанных, ДанныеРасшифровки, АвтофиксацияШапки,, Истина);
	#Если Сервер И Не Сервер Тогда
	    ТабличныйДокумент = Новый ТабличныйДокумент;
	#КонецЕсли
	Если ИндексТекущейКолонки > 0 Тогда
		ТабличныйДокумент.ТекущаяОбласть = ТабличныйДокумент.Область(2, ИндексТекущейКолонки + 1);
	КонецЕсли; 
...
Функция ПолучитьКаталогОбъектовДляОтладкиЛкс() Экспорт 
	...
	Иначе
		...
		КаталогОбъектовДляОтладки = ХранилищеОбщихНастроек.Загрузить(ирОбщий.ИмяПродуктаЛкс(), "КаталогОбъектовДляОтладки");
	КонецЕсли; 
...
Процедура ОткрытьОднократноАдаптациюРасширенияЛкс() Экспорт 
	...
	ПометкиКоманд = ХранилищеОбщихНастроек.Загрузить(ирОбщий.ИмяПродуктаЛкс(), "ирАдаптацияРасширения.ПометкиКоманд");
	Если ПометкиКоманд = Неопределено Тогда
		ПометкиКоманд = Новый Структура;
		ХранилищеОбщихНастроек.Сохранить(ирОбщий.ИмяПродуктаЛкс(), "ирАдаптацияРасширения.ПометкиКоманд", ПометкиКоманд);
