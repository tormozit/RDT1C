...
Функция ПолучитьОбъектДанныхИзСтрокиРезультатаЗапросаЛкс(Знач Выборка, Знач МакетныйОбъект, Знач ТекущаяГруппаТипаМетаданных, СчитатьДанныеОбъекта = Истина, Знач ИгнорироватьКолонки = "") Экспорт
...
Функция ПолучитьСхемуКомпоновкиТаблицыБДЛкс(ПолноеИмяТаблицыБД, ВыражениеПараметраПериодичность, ДобавитьАвтополеКоличествоСтрок, Знач ИндексПараметраПериодичность,
	Знач ПсевдонимТаблицы) Экспорт 
	...
	Тогда
		...
		Иначе
			...
			ИначеЕсли Фрагменты[0] = "РегистрБухгалтерии" Тогда 
				Результат = ОбразующийМД;
			Иначе 
				ВызватьИсключение "Неизвестный корневой тип метаданных(" + Фрагменты[0] + ")";
			КонецЕсли; 
...
Функция ПолучитьТекстЗапросаПоВыбраннымТаблицамЛкс(МассивИменТаблиц, ВариантИсточников = 0) Экспорт
	...
	Для Каждого ИмяТаблицы Из МассивИменТаблиц Цикл
		...
		КомпоновщикТаблицы = ирКэш.ПолучитьКомпоновщикТаблицыБДПоПолномуИмениЛкс(ИмяТаблицы);
		...
		Для СчетчикПопыток = 1 По ПопытокЗаписиОбъекта Цикл
			...
			Исключение
				...
				Если Истина
					...
					И Найти(ОписаниеОшибки, "несоответствия версии или отсутствия записи базы данных") > 0
					И Найти(ОписаниеОшибки, "version mismatch or lack of database record") > 0
				Тогда
...
Процедура ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ОтображатьИдентификаторы = Ложь, Знач ИменаКолонокСПиктограммамиТипов = "") Экспорт 

	ЛиОтбражатьПустые = ОтображатьИдентификаторы;
	ирПлатформа = ирКэш.Получить();
...
Функция ОткрытьПодборСВыборомТипаЛкс(ВладелецФормы, ОписаниеТипов = Неопределено, Знач НачальноеЗначениеВыбора = Неопределено, ИспользоватьДинамическийСписокИР = Неопределено) Экспорт
	...
	Если ИспользоватьДинамическийСписокИР Тогда
		ФормаВыбора = ирОбщий.ПолучитьФормуЛкс("Обработка.ирДинамическийСписок.Форма",, ВладелецФормы, Новый УникальныйИдентификатор);
		ФормаВыбора.УстановитьОбъектМетаданных(ПолноеИмяМД);
		Если НачальноеЗначениеВыбора <> Неопределено Тогда
			ФормаВыбора.НайтиСсылкуВСписке(НачальноеЗначениеВыбора, Ложь);
		КонецЕсли; 
	Иначе
		Попытка
...
Функция ОткрытьФормуСпискаСОтбором(ИмяТаблицы, Отбор, ИспользоватьДинамическийСписокИР = Неопределено) Экспорт
	...
	Иначе
		ФормаСписка = ПолучитьФормуЛкс(ИмяТаблицы + ".ФормаСписка", ПараметрыФормы);
		Если ТипЗнч(ФормаСписка) = Тип("Форма") Тогда
...
Функция ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ТабличноеПоле, ПолноеИмяМД = Неопределено, ДоступныеПоляВыбора = Неопределено) Экспорт
	...
	КлючОбъекта = ПолучитьКлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, ТекущаяСтрока, , СтруктураКлючаСтроки);
...
Функция ПолучитьКлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, Знач ТекущаяСтрока, ДляРегистровСоздатьНаборЗаписей = Истина, выхСтруктураКлючаСтроки = Неопределено) Экспорт 
	...
	Тогда 
		Если ДляРегистровСоздатьНаборЗаписей Тогда
...
Процедура ОткрытьОбъектыИзВыделенныхСтрокВПодбореИОбработкеОбъектовЛкс(ТабличноеПоле, ПолноеИмяТаблицы = "") Экспорт 
	...
	Для Каждого ВыделеннаяСтрока Из ТабличноеПоле.ВыделенныеСтроки Цикл
		КлючОбъекта = ПолучитьКлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, ВыделеннаяСтрока, Ложь);
...
Функция ПолучитьПостроительВыборкиСвязанныхСтрокЛкс(ПолноеИмяТаблицы, ИмяКолонки, ЗначениеОтбора, МаксимальнаяПорция = 0) Экспорт 
	...
	Компоновщик = ирКэш.ПолучитьКомпоновщикТаблицыБДПоПолномуИмениЛкс(ПолноеИмяТаблицы);
	#Если _ Тогда
	    Компоновщик = Новый КомпоновщикНастроекКомпоновкиДанных;
	#КонецЕсли
	ТекстПоля = "";
...
Функция ОтладитьЛкс(Объект, Модально = Ложь, НастройкаКомпоновкиИлиТекстЗапросаИлиИменаВременныхТаблиц = Неопределено, ВнешниеНаборыДанных = Неопределено,
	...
	Иначе
		...
		Если ТипЗнч(Объект) = Тип("Запрос") Тогда
			...
			СтруктураЗапроса.Параметры = ПреобразоватьПараметрыЗапросаДляСериализацииИис(Объект.Параметры);
		...
		ИначеЕсли ТипЗнч(Объект) = Тип("ПостроительЗапроса") Тогда
			...
			СтруктураЗапроса.Параметры = ПреобразоватьПараметрыЗапросаДляСериализацииИис(СтруктураЗапроса.Параметры);
			СтруктураПараметров.Объект = СтруктураЗапроса;
		ИначеЕсли ТипЗнч(Объект) = Тип("МакетКомпоновкиДанных") Тогда
...
Функция ПреобразоватьПараметрыЗапросаДляСериализацииИис(Параметры) Экспорт 
