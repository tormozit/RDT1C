...
Функция ПроверитьСериализациюXMLПередВызовомЗначениеВФайлЛкс(Знач ПолученноеЗначение) Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
	    мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	Если мПлатформа.ВерсияПлатформы >= 803012 Тогда
...
// Параметры:
//   КлючОбъекта - Произвольный - для нового ссылочного объекта Истина=Папка, для регистров передается Структура
Функция ОбъектБДПоКлючуЛкс(Знач ИмяОсновнойТаблицы, Знач КлючОбъекта, Знач СохранятьИдентификаторСсылки = Ложь, Знач ЧитатьДанныеРегистраИлиКонстанты = Истина, Знач НаСервере = Ложь,
	...
	Если Не ирКэш.ЛиПортативныйРежимЛкс() И НаСервере И Не ЭтоМетаСсылка И РазрешитьВложенныйВызов Тогда
		СтруктураСнимка = ирСервер.ПолучитьСнимокОбъектаБДПоКлючуЛкс(ИмяОсновнойТаблицы, КлючОбъекта, СохранятьИдентификаторСсылки, ЧитатьДанныеРегистраИлиКонстанты, выхИдентификаторСсылки);
...
Функция ПолучитьНаборЗаписейПоКлючуЛкс(ПолноеИмяТаблицыИлиНаборЗаписей, Знач КлючНабора, ДобавитьИЗаполнитьСтрокуНабора = Ложь) Экспорт
...
Функция ПредставлениеКлючаСтрокиБДЛкс(Знач КлючСтроки) Экспорт
	...
	Если ЛиКлючЗаписиРегистраЛкс(КлючСтроки) Тогда
		...
		Для Каждого ЭлементСписка Из СтруктураКлюча Цикл
			...
			КонецЕсли; 
			ПредставленияКлюча = ПредставленияКлюча + ЭлементСписка.Представление + " = " + КлючСтроки[ЭлементСписка.Представление];
		КонецЦикла; 
	Иначе
		ПредставленияКлюча = "" + КлючСтроки;
		Попытка
			Владелец = КлючСтроки.Владелец;
		Исключение
			// Нет доступа
		КонецПопытки;
...
Функция ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(ТабличноеПоле, ПолноеИмяМД = Неопределено, ДоступныеПоляВыбора = Неопределено, Связанный = Ложь,
	ФормаРедактора = Неопределено, НаСервере = Неопределено, ДляРегистровСоздатьНаборЗаписей = Истина) Экспорт
	...
	Если ТабличноеПоле.ТекущиеДанные <> Неопределено Тогда
		КлючОбъекта = КлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, ТабличноеПоле.ТекущиеДанные, ДляРегистровСоздатьНаборЗаписей, СтруктураКлючаСтроки, НаСервере);
	КонецЕсли;
	Если КлючОбъекта = Неопределено Тогда
...
Функция КлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицы, Знач ТекущаяСтрока, ДляРегистровСоздатьНаборЗаписей = Ложь, выхСтруктураКлючаСтроки = Неопределено,
	...
	Тогда 
		...
		Тогда
			...
			КонецЦикла;
			КлючОбъекта = ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицы, СтруктураКлюча,, Ложь, НаСервере);
		Иначе
