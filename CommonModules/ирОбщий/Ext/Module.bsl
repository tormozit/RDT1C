...
Процедура ВывестиТабличноеПолеВТабличныйДокументЛкс(Знач ТабличноеПоле) Экспорт 
	
	ТабличныйДокумент = ВывестиТабличноеПолеКоллекцииВТабличныйДокументЛкс(ТабличноеПоле);
...
Функция ПолучитьПутьКДаннымТекущейКолонкиТабличногоПоляЛкс(Знач ТабличноеПоле, Колонка = Неопределено) Экспорт
...
Функция АдаптироватьРасширениеЛкс(ТолькоПриНаличииПодключенныхКоманд = Ложь) Экспорт 
	...
	//	СтрокаСоединенияИнформационнойБазы(), ТекстЛога);
	...
	Для Каждого Тип Из СписокДобавляемыхТипов Цикл
		ФайлИсточник = Новый Файл(КаталогВыгрузкиКонфигурации + "\" + СтрЗаменить(XMLТип(Тип).ИмяТипа, "Ref.", ".") + ".xml");
		...
		ЧтоЗаменять = ирОбщий.ПолучитьСтрокуМеждуМаркерамиЛкс(ТекстФайла, "<Properties>", "</Catalog>",, Истина);
		НаЧтоЗаменять = 
		"		<Properties>
		...
		ЧтоЗаменять = ирОбщий.ПолучитьСтрокуМеждуМаркерамиЛкс(ТекстФайла, "<Catalog uuid=", ">",, Истина);
		НаЧтоЗаменять = "<Catalog uuid=""" + Новый УникальныйИдентификатор + """>";
		ТекстФайла = СтрЗаменить(ТекстФайла, ЧтоЗаменять, НаЧтоЗаменять);
	...
	//КонецЦикла;
	//ТекстовыйДокумент.Записать(ИмяФайлаСпискаВыгрузкиРасширения);
	//Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/LoadConfigFromFiles """ + КаталогВыгрузкиРасширения + """ -Extension """ + ИмяРасширения + """ -listFile """ + ИмяФайлаСпискаВыгрузкиРасширения + """ -Format Plain", , ТекстЛога);
	Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/LoadConfigFromFiles """ + КаталогВыгрузкиРасширения + """ -Extension """ + ИмяРасширения + """ -Format Plain", , ТекстЛога,,
...
Процедура ОткрытьСписокИнструментовЛкс() Экспорт
	...
	#Иначе
		ФормаСпискаИнструментов = ПолучитьФорму("Обработка.ирПортативный.Форма.Форма");
