	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		Если РегистрироватьДвиженияВместеСДокументом И ЭтоДокумент Тогда
			...
			Для Каждого МетаРегистр из ОбъектыМД Цикл
				...
				СтруктураНабораЗаписей = ОбъектБДПоКлючуЛкс(ИмяТаблицыБДРегистра, Новый Структура("Регистратор", Объект.Ссылка),, Ложь, ОбъектыНаСервере);
...
Функция ПолучитьСтруктуруКлючаТаблицыБДЛкс(Знач ПолноеИмяТаблицыБД, ВключатьНомерСтроки = Истина, ВернутьСтруктуру = Истина, ДляНабораЗаписейРегистраСведений = Истина) Экспорт
	...
	ИначеЕсли ЛиКорневойТипРегистраБДЛкс(ТипТаблицы) Тогда
		ОбъектМД = НайтиОбъектМетаданныхПоПолномуИмениТаблицыБДЛкс(ПолноеИмяТаблицыБД);
		НаборЗаписей = СоздатьНаборЗаписейПоИмениТаблицыБДЛкс(ПолноеИмяТаблицыБД).Методы;
		#Если Сервер И Не Сервер Тогда
...
Процедура ЗагрузитьКоллекциюВНаборЗаписейЛкс(КоллекцияСтрокДвижений, НаборЗаписейКуда, ЗаполнитьВидыСубконто = Ложь) Экспорт 
	...
	Если ЗаполнитьВидыСубконто Тогда
		...
		Если КорневойТип = "РегистрБухгалтерии" Тогда
			ЗаполнитьВидыСубконтоВТаблицеНабораЗаписейЛкс(ТаблицаДляЗагрузки, ОбъектМД);
		КонецЕсли; 
	КонецЕсли; 
	НаборЗаписейКуда.Загрузить(ТаблицаДляЗагрузки);
	ирОбщий.НаборЗаписейПослеЗагрузкиИзТаблицыЗначенийЛкс(НаборЗаписейКуда);
	
КонецПроцедуры

Процедура ЗаполнитьВидыСубконтоВТаблицеНабораЗаписейЛкс(ТаблицаДляЗагрузки, ОбъектМД) Экспорт 
	...
	Для Каждого Проводка Из ТаблицаДляЗагрузки Цикл
		СтрокиВидовСубконто = ТаблицаВидовСубконто.НайтиСтроки(Новый Структура("Ссылка", Проводка.СчетДт));
		Для Каждого СтрокаВидаСубконто Из СтрокиВидовСубконто Цикл
			Проводка["ВидСубконтоДт" + СтрокаВидаСубконто.НомерСтроки] = СтрокаВидаСубконто.ВидСубконто;
		КонецЦикла;
		СтрокиВидовСубконто = ТаблицаВидовСубконто.НайтиСтроки(Новый Структура("Ссылка", Проводка.СчетКт));
		Для Каждого СтрокаВидаСубконто Из СтрокиВидовСубконто Цикл
			Проводка["ВидСубконтоКт" + СтрокаВидаСубконто.НомерСтроки] = СтрокаВидаСубконто.ВидСубконто;
		КонецЦикла;
	КонецЦикла;
...
Функция ОбъектБДПоКлючуЛкс(Знач ИмяОсновнойТаблицы, Знач КлючОбъекта = Неопределено, Знач СохранятьИдентификаторСсылки = Ложь, Знач ЧитатьДанныеРегистраИлиКонстанты = Истина,
	Знач ОбъектыНаСервере = Неопределено, выхИдентификаторСсылки = Неопределено, РазрешитьВложенныйВызов = Истина) Экспорт 
	...
	Если Не ирКэш.ЛиПортативныйРежимЛкс() И ОбъектыНаСервере И Не ЭтоМетаСсылка И РазрешитьВложенныйВызов Тогда
	...
	Иначе
		...
		Если ЭтоСсылочныйОбъект Тогда
			...
			Иначе
				...
				Если ОбъектыНаСервере И Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
...
Функция ИДВерсииПлатформы(СистемнаяИнформация = Неопределено) Экспорт
	...
	КонецЕсли;
	Фрагменты = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(СистемнаяИнформация.ВерсияПриложения);
...
Функция ПолучитьПервыйФрагментЛкс(пСтрока, пРазделитель = ".", пЛиИспользоватьГраницуЕслиМаркерНеНайден = Истина) Экспорт
	...
	#КонецЕсли
	// Однострочный код использован для ускорения. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика" (http://devtool1c.ucoz.ru)
	Позиция = Найти(пСтрока, пРазделитель);  	Если Позиция > 0 Тогда  		Возврат Лев(пСтрока, Позиция - 1);  	Иначе  		Если пЛиИспользоватьГраницуЕслиМаркерНеНайден Тогда  			Возврат пСтрока;  		Иначе  			Возврат "";  		КонецЕсли;  	КонецЕсли;  
