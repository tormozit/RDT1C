...
Функция ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ТабличноеПоле, СтандартнаяОбработка, РасширенноеЗначение = Null, ИспользоватьОграничениеТипа = Ложь,
	...
	Если РасширенноеЗначение = Неопределено Тогда
		...
		Если НовыйТип <> Неопределено Тогда
			...
			РасширенноеЗначение = НовоеЗначение;
			//ТабличноеПоле.ТекущиеДанные[Данные] = РасширенноеЗначение;
			ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ТабличноеПоле, Колонка, РасширенноеЗначение);
			РасширенноеЗначение = НовоеЗначение; // Почему то запрещенные для поля ввода значения здесь превращаются в строку (например МоментВремени)
			ЗначениеИзменено = Истина;
...
Функция ПрочитатьДополнительныеПоляСсылающихсяОбъектовЛкс(Знач ТабличноеПоле, Знач КомпоновщикДопПолей, Знач ТаблицаДанных = Неопределено) Экспорт 
	
	КолонкаДатаДокумента = ТабличноеПоле.Колонки.ДатаОбъекта;
	...
	Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ТаблицаДанных.Количество(), "Чтение дополнительных полей");
	Для Каждого СтрокаОбъекта Из ТаблицаДанных Цикл
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
		...
		КонецЕсли; 
		СтрокаОбъекта.ИндексКартинки = ирОбщий.ПолучитьИндексКартинкиСсылкиЛкс(ОбъектСодержащийСсылку, Истина);
		Для Каждого КлючИЗначение Из МассивДопПолей Цикл
			ПутьКДанным = Сред(КлючИЗначение.Значение, СтрДлина("Объект.") + 1);
			Попытка
				ЗначениеПоля = Вычислить("ОбъектСодержащийСсылку." + ПутьКДанным);
			Исключение
				Продолжить;
			КонецПопытки;
		...
		КонецЦикла;
		Попытка
			ДатаОбъекта = ОбъектСодержащийСсылку.Дата;
		Исключение
			ДатаОбъекта = Неопределено;
		КонецПопытки;
		СтрокаОбъекта.ДатаОбъекта = ДатаОбъекта;
	КонецЦикла;
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
	...
	#Если (ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение) И Не Сервер Тогда
		...
		Если мПлатформа.мПроверкаКомпиляцииОбщихМодулейВыполнялась <> Истина Тогда
			...
			Если СписокМодулей.Количество() > 0 Тогда
				...
				КонецЕсли; 
				Сообщить("Необходимо установить флажок ""Клиент (" + ИмяКлиента + " приложение)"" или ""Вызова сервера"" у общих модулей:");
	...
	Тогда
		...
		Тогда
			Ответ = Вопрос("У текущего пользователя базы включена защита от опасных действий.
				|Для корректной работы с инструментами ее рекомендуется отключить. Сделать это (сеанс будет перезапущен)?", РежимДиалогаВопрос.ДаНет);
			Если Ответ = КодВозвратаДиалога.Да Тогда
