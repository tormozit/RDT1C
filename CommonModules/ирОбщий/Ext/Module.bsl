...
Функция ПолучитьПоляТаблицыМДЛкс(ПолноеИмяМД, ВызыватьИсключениеПриОтсутствииПрав = Истина, ИндексПараметраПериодичность = Неопределено, ВыражениеПараметраПериодичность = "",
...
Функция ОбъектБДПоКлючуЛкс(Знач ИмяОсновнойТаблицы, Знач КлючОбъекта, Знач СохранятьИдентификаторСсылки = Ложь, Знач ЧитатьДанные = Истина, Знач НаСервере = Ложь,
	...
	Если Не ирКэш.ЛиПортативныйРежимЛкс() И НаСервере И Не ЭтоМетаСсылка И РазрешитьВложенныйВызов Тогда
			...
			Иначе
				...
				Для Каждого ЭлементОтбора Из Объект.Отбор Цикл
					...
					Если СтруктураЧтенияЗначения[ЭлементОтбора.Имя] <> Null Тогда
						Объект.Отбор[ЭлементОтбора.Имя].Установить(КлючОбъекта[ЭлементОтбора.Имя]);
					КонецЕсли; 
				КонецЦикла;
			КонецЕсли; 
			Данные = Объект;
...
Функция РезультатЗапросаADOВТаблицуЗначенийОбщийЛкс(РезультатRecordset, Типизировать1С = Истина, БинарныеВСтроку = Ложь, ЗагружатьЭлементов = 0, СмещениеГодаADO = 2000, ИспользованиеGWF = Истина,
	Знач ADOUtils = Неопределено, МодальныйРежим = Ложь) Экспорт 
	
	Если ADOUtils = Неопределено Тогда
		мПлатформа = ирКэш.Получить();
		ADOUtils = мПлатформа.ПолучитьADOUtils(Истина, 2000, Истина);
	КонецЕсли; 
	...
	Если ИспользованиеGWF Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	Если ТаблицаИзADO.Колонки.Количество() = 0 Тогда
		ТаблицаИзADO = ПреобразоватьРезультатADOВТаблицуЗначенийЛкс(РезультатRecordset, Типизировать1С, БинарныеВСтроку,,, ЗагружатьЭлементов, СмещениеГодаADO);
	КонецЕсли;
...
Функция СохранитьФайлВКонсолиСВосстановлениемЛкс(ДиалогВыбораФайла, Знач ИмяСохраняемогоФайла, ИмяОткрытогоФайла = "", ДанныеДляФайла,
	...
	Если СохранитьФайл Тогда
		...
		Попытка
			...
			// ЗначениеВФайл на некоторых ошибках вызывает безусловное завершение работы http://devtool1c.ucoz.ru/forum/2-746-1
			// http://devtool1c.ucoz.ru/forum/2-752-1
			ирОбщий.СохранитьОбъектВВидеСтрокиXMLЛкс(ДанныеДляФайла);
		Исключение
		КонецПопытки;
...
Процедура ИнициализироватьФормуЛкс(ЭтаФорма, ПолноеИмяФормы) Экспорт
	...
	Если мПлатформа.мПроверкаСовместимостиКонфигурацииВыполнялась <> Истина Тогда
		...
		#Если ТолстыйКлиентУправляемоеПриложение Тогда
			Если Не Метаданные.ИспользоватьОбычныеФормыВУправляемомПриложении Тогда
				Сообщить("Рекомендуется включить в свойствах конфигурации флажок ""Использовать обычные формы в управляемом приложении""");
			КонецЕсли; 
		#КонецЕсли
	КонецЕсли; 
...
Функция ИменаИспользуемыхВЗапросеВременныхТаблицЛкс(Знач ЗапросИлиМенеджерВременныхТаблиц, Знач ОбязательныеДляПроверкиИмена = """") Экспорт 
	...
	Если ТипЗнч(ЗапросИлиМенеджерВременныхТаблиц) = Тип("Запрос") Тогда
		...
		КонецЦикла;
	ИначеЕсли Платформа.ВерсияПлатформы >= 803008 Тогда 
		#Если Сервер И Не Сервер Тогда
		    Пустышка = Новый Запрос;
...
Функция ПутьКДаннымЭлементаУправляемойФормыЛкс(Знач Поле, ОтносительноРодителя = Ложь, Знач ЭтаФорма = Неопределено) Экспорт
	...
	Если Поле <> ЭтаФорма Тогда
		Если Не ЭтаФорма.мСлужебныеДанные.ПутиКДанным.Свойство(Поле.Имя, ПутьКДаннымПоля) Тогда
			ПутьКДаннымПоля = "";
		КонецЕсли; 
