...
Функция ВывестиТаблицуВТабличныйДокументИлиТаблицуЗначенийЛкс(ТаблицаЗначений, Знач Приемник = Неопределено, ДанныеРасшифровки = Неопределено, ИтогиЧисловыхКолонок = Истина,
	...
	Иначе
		...
		Если ИндексТекущейКолонки > 0 Тогда
			Приемник.ТекущаяОбласть = Приемник.Область(2, ИндексТекущейКолонки + 1);
		КонецЕсли; 
	КонецЕсли; 
	Возврат Приемник;
...
Функция ИмяМетаданныхИзФормыЛкс(ИмяФормы) Экспорт 
	Результат = ИмяФормы;
	Фрагменты = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(ИмяФормы);
	Если Фрагменты.Количество() > 2  Тогда 
		Фрагменты.Удалить(Фрагменты.ВГраница());
		Если Фрагменты[Фрагменты.ВГраница()] = "Форма" Тогда
			Фрагменты.Удалить(Фрагменты.ВГраница());
		КонецЕсли; 
	КонецЕсли; 
	Возврат Результат;
