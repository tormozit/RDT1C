...
Функция НастройкиБазыНаКлиентеЛкс() Экспорт 
	...
	ТаблицаБазКлиента = ирОбщий.ПолучитьСписокИнфобазПользователяОСЛкс();
	#Если Сервер И Не Сервер Тогда
		ТаблицаБазКлиента = Обработки.ирПлатформа.Создать().СписокИнфобазПользователя;
	#КонецЕсли
	СтрокаТекущейБазы = ТаблицаБазКлиента.Найти(НРег(СтрокаСоединенияИнформационнойБазы()), "КлючСтроки");
