...
// Кэширование в 100 раз ускоряет поиск
...
Функция ОчередьОбновленияФормЛкс() Экспорт 
	
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("ОчередьОбновленияФормЛкс");
		КлючНаборПараметров = "";
		Результат = КэшФункции[КлючНаборПараметров];
		Если Результат <> Неопределено Тогда
			Возврат Результат;
	...
	Результат = Новый Структура;
	Результат.Вставить("ОткрытыеФормы", Новый Массив);
...
Функция ВременноеОкноСообщенийЛкс() Экспорт 
	...
	//Форма = ПолучитьФорму("Обработка.ирПлатформа.Форма.ВременноеОкноСообщений2"); // Крадет фокус только при открытии
	Возврат Форма;
