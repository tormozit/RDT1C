...
Функция ЛиТолстыйКлиентЛкс() Экспорт 
	
	Результат = ирОбщий.СтрокиРавныЛкс(ирКэш.ТекущийСеансЛкс().ИмяПриложения, "1CV8");
	Возврат Результат;
	
КонецФункции

Функция ЛиТонкийКлиентЛкс() Экспорт 
	
	Результат = ирОбщий.СтрокиРавныЛкс(ирКэш.ТекущийСеансЛкс().ИмяПриложения, "1CV8C");
	Возврат Результат;
...
Функция ПолучитьCOMОбъектWMIЛкс(Знач ИмяСервера = Неопределено, Знач ИмяСервераИсполнителя = Неопределено, Знач ТочкаПодключения = Неопределено) Экспорт
	...
	Если Не ирОбщий.ЭтоИмяЛокальногоКомпьютераЛкс(ИмяСервера) Тогда
		...
		ИмяФайлаРезультата = ирОбщий.ПолучитьТекстРезультатаКомандыОСЛкс("rpcping -s " + ИмяСервера, Ложь);
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстРезультата = "";
...
Функция СостояниеПодготовкиКэшаЛкс() Экспорт 
	...
	АдресРезультата = ПоместитьВоВременноеХранилище(Null, ФормаРезультата.УникальныйИдентификатор);
...
Функция ВременноеОкноСообщенийЛкс() Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	//Форма = мПлатформа.ПолучитьФорму("ВременноеОкноСообщений"); // Крадет фокус при каждом выводе сообщения
	Форма = ПолучитьФорму("Обработка.ирПлатформа.Форма.ВременноеОкноСообщенийУпр"); // Крадет фокус только при открытии
	Возврат Форма;
