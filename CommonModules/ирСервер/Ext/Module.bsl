...
Функция ВыгрузитьЖурналРегистрацииЛкс(Отбор, МаксимальныйРазмер, АдресРезультата = "") Экспорт 
	Результат = Новый ТаблицаЗначений;
	ВыгрузитьЖурналРегистрации(Результат, Отбор,,, МаксимальныйРазмер);
...
Функция НайтиПоСсылкамЛкс(Ссылки, АдресРезультата = "") Экспорт 
...
Функция ВыполнитьЗапросСтатистикиПоТаблицамЛкс(ТекстЗапроса, Параметры, ИмяКолонкиКоличества, АдресРезультата = "") Экспорт 
...
Функция ФоновыеЗаданияПодробноЛкс(ТаблицаЗаданий, ДляСбораСтатистики, КонечныйОтбор, ОтборФоновыхЗаданий, ОтображатьИндикатор, ВызовВнутриОбновленияРегламентныхЗаданий, АдресРезультата = "") Экспорт 
...
Функция ОбонвитьИтогиИсторииДанныхПоТипамЛкс(Типы, ВычислятьПоля, ВычислятьКоличествоВерсий, ОтборВерсий, МаксКоличествоВерсий, АдресРезультата = "") Экспорт 
	Результат = ирОбщий.ОбонвитьИтогиИсторииДанныхПоТипамЛкс(Типы, ВычислятьПоля, ВычислятьКоличествоВерсий, ОтборВерсий, МаксКоличествоВерсий);
	Если ЗначениеЗаполнено(АдресРезультата) Тогда
		ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
	КонецЕсли; 
	Возврат Результат;
...
Функция ПрочитатьТехножурналЛкс(МоментНачалаЗагрузки, ОбщиеПараметрыОбработки, ФильтрЗагрузки, АдресРезультата = "") Экспорт 
	Результат = ирОбщий.ПрочитатьТехножурналЛкс(МоментНачалаЗагрузки, ОбщиеПараметрыОбработки, ФильтрЗагрузки);
	Если ЗначениеЗаполнено(АдресРезультата) Тогда
		ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
	КонецЕсли; 
	Возврат Результат;
...
Функция НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, ПолноеИмяТаблицы, РежимИмяСиноним = Ложь, ПредельноеКоличествоВидимыхКолонок = 10, СоединенныеРегистры = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из ИсточникиДанных Цикл
		...
		Для Каждого ПолеТаблицы Из ПоляТаблицы Цикл
			...
			КонецЕсли;
			Если Истина
				И КлючИЗначение.Значение = ПолноеИмяТаблицы
				И ПолеТаблицы.Метаданные <> Неопределено
				И ПолеТаблицы.Метаданные.РежимПароля 
