...
Функция СкомпоноватьОтчетВКонсолиЛкс(Знач АдресКоллекцииВывода, Знач АдресМакетаКомпоновки, ВнешниеНаборыДанных, Автофиксация, МодальныйРежим, ЛиОтладка, выхАдресДанныхРасшифровки) Экспорт 
	...
	Результат = ирОбщий.СкомпоноватьОтчетВКонсолиЛкс(КоллекцияВывода, МакетКомпоновкиДанных, ВнешниеНаборыДанных, Автофиксация, МодальныйРежим, ЛиОтладка, выхАдресДанныхРасшифровки);
...
Процедура УдалитьОбъектXMLЛкс(Знач ОбъектXML, Знач СтруктураДополнительныхСвойств, Знач ТипОбъекта, ПривилегированныйРежим = Неопределено) Экспорт 
	...
	КонецЕсли; 
	Объект.Прочитать();
	ирОбщий.УдалитьОбъектЛкс(Объект, Ложь,,, ПривилегированныйРежим);
	//Если ЭтоИмитатор Тогда
	//	#Если Сервер И Не Сервер Тогда
	//	    Объект = Обработки.ирИмитаторСсылочныйОбъект.Создать();
	//	#КонецЕсли
...
Процедура УстановитьПометкуУдаленияОбъектаЛкс(ОбъектXML, СтруктураДополнительныхСвойств, ЗначениеПометки = Истина, БезАвторегистрацииИзменений = Неопределено,
	ПривилегированныйРежим = Неопределено) Экспорт 
	
	Объект = ирОбщий.ВосстановитьОбъектИзСтрокиXMLЛкс(ОбъектXML);
	Объект.Прочитать(); // Иначе объект будет модифицирован и возникнет ошибка
	ирОбщий.ВосстановитьДополнительныеСвойстваОбъектаЛкс(Объект, СтруктураДополнительныхСвойств);
	ирОбщий.УстановитьПометкуУдаленияОбъектаЛкс(Объект,, ЗначениеПометки, БезАвторегистрацииИзменений, ПривилегированныйРежим);
	ДополнительныеСвойства = ирОбщий.СериализоватьДополнительныеСвойстваОбъектаЛкс(Объект);
	ОбъектXML = ирОбщий.СохранитьОбъектВВидеСтрокиXMLЛкс(Объект);
...
Функция ПриНачалеРаботыСистемыРасширениеЛкс() Экспорт 
	...
	Если Истина
		...
		И ирОбщий.ВосстановитьЗначениеЛкс("ДобавлятьРольИРВсемАдминистраторам") <> Ложь
	Тогда
...
Функция НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, РежимИмяСиноним = Ложь, ПредельноеКоличествоВидимыхКолонок = 10) Экспорт
	...
	Для Каждого ИмяВажнойКолонки Из ИменаВажныхКолонок Цикл
		...
		КонецЕсли; 
	КонецЦикла;
	ирОбщий.НастроитьЗаголовкиАвтоТаблицыФормыДинамическогоСпискаЛкс(ОсновнойЭУ, РежимИмяСиноним);
	Возврат Истина;
