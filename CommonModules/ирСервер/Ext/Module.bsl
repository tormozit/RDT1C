...
Функция ВыполнитьЗапросЛкс(ТекстЗапроса, Параметры = Неопределено) Экспорт 
...
Функция ПолучитьПараметрыПроцессаАгентаСервера(выхИдентификаторПроцесса = Неопределено, выхКомманднаяСтрока = Неопределено, выхИмяСлужбы = Неопределено) Экспорт 
	...
	РабочийПроцесс = ирОбщий.ПолучитьПроцессОСЛкс("текущий");
...
Функция ИнфоСервераПриложений() Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда 
		...
		КонецЕсли; 
	Иначе
		ИнфоСервера = "Сервер. ОС: " + ирОбщий.ОписаниеОСЛкс();
...
Процедура ВыгрузитьЖурналРегистрацииФоновоЛкс(АдресРезультата, Отбор, МаксимальныйРазмер) Экспорт 
	
	ТаблицаЗначений = Новый ТаблицаЗначений;
	ВыгрузитьЖурналРегистрации(ТаблицаЗначений, Отбор,,, МаксимальныйРазмер);
	ПоместитьВоВременноеХранилище(ТаблицаЗначений, АдресРезультата);
	
КонецПроцедуры

Процедура НайтиПоСсылкамФоновоЛкс(Ссылки, АдресРезультата) Экспорт 
	Результат = ирОбщий.НайтиПоСсылкамЛкс(Ссылки);
	ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
КонецПроцедуры

Процедура ВыполнитьЗапросСтатистикиПоТаблицамФоновоЛкс(ТекстЗапроса, Параметры, ИмяКолонкиКоличества, АдресРезультата = "") Экспорт 
	
	Результат = ирОбщий.ВыполнитьЗапросСтатистикиПоТаблицамЛкс(ТекстЗапроса, Параметры, ИмяКолонкиКоличества);
	ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
	
КонецПроцедуры

Процедура ФоновыеЗаданияПодробноФоновоЛкс(ТаблицаЗаданий, ДляСбораСтатистики, КонечныйОтбор, ОтборФоновыхЗаданий, ОтображатьИндикатор, ВызовВнутриОбновленияРегламентныхЗаданий, АдресРезультата) Экспорт 
	Результат = ирОбщий.ФоновыеЗаданияПодробноЛкс(ТаблицаЗаданий, ДляСбораСтатистики, КонечныйОтбор, ОтборФоновыхЗаданий, ОтображатьИндикатор, ВызовВнутриОбновленияРегламентныхЗаданий);
	ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
КонецПроцедуры

Функция ИтогиИсторииДанныхПоТипамФоновоЛкс(Типы, ВычислятьПоля, ВычислятьКоличествоВерсий, ОтборВерсий, МаксКоличествоВерсий, АдресРезультата = "") Экспорт 
	Результат = ирОбщий.ИтогиИсторииДанныхПоТипамЛкс(Типы, ВычислятьПоля, ВычислятьКоличествоВерсий, ОтборВерсий, МаксКоличествоВерсий);
	ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
КонецФункции

Функция ВычислитьПраваДоступаФоновоЛкс(ВычислятьФункциональныеОпции, ИменаРолей, ИспользоватьНаборПолей, НаборПолейТаблица, ОбъектМетаданных, ПолеОбъекта, Пользователь, АдресРезультата = "") Экспорт 
	Результат = ирОбщий.ВычислитьПраваДоступаЛкс(ВычислятьФункциональныеОпции, ИменаРолей, ИспользоватьНаборПолей, НаборПолейТаблица, ОбъектМетаданных, ПолеОбъекта, Пользователь);
	ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
КонецФункции
