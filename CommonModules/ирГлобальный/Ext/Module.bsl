...
Процедура ОткрытьОднократноАдаптациюРасширенияЛкс(выхПерехватКлавиатуры = Неопределено) Экспорт 
	...
	#Если Не ТолстыйКлиентОбычноеПриложение Тогда
		...
		Иначе
			Разрядность = "32";
		КонецЕсли; 
		//Успех = ПодключитьВнешнююКомпоненту("C:\VC\KeyboardHook_1C\Debug\KHook1C_x32.dll", "ПерехватКлавиатуры", ТипВнешнейКомпоненты.Native); // Отладка
		Успех = ПодключитьВнешнююКомпоненту("Обработка.ирПлатформа.Макет.KeyboardHook" + Разрядность, "ПерехватКлавиатуры", ТипВнешнейКомпоненты.Native); // 0.12сек
		Попытка
	...
	Попытка
		...
		Тогда
			...
			Если ПередатьОбработку Тогда
				КодыКлавиш["РазрешитьОбработкуДопКлавиш"] = 1;
				ирОбщий.УдалитьСсылкиНаЗакрытыеФормыЛкс();
				ОчередьОбновленияФорм = ирКэш.ОчередьОбновленияФормЛкс();
				СобытиеОбработано = Ложь;
				Для Каждого Форма Из ОчередьОбновленияФорм.ОткрытыеФормы Цикл
					Если Форма.ВводДоступен() Тогда
						...
						Иначе
							ирОбщий.СообщитьЛкс("У формы """ + Форма.Заголовок + """ отсутствует обработчик ""ВнешнееСобытие""");
						КонецЕсли; 
					КонецЕсли; 
				КонецЦикла;
...
Процедура КопироватьСсылкуАктивнойФормыЛкс(Параметры) Экспорт 
	...
	Ссылка = ирОбщий.КлючОсновногоОбъектаУправляемойФормыЛкс(Форма);
	Если Ссылка <> Неопределено Тогда
		ирОбщий.БуферОбмена_УстановитьЗначениеЛкс(Ссылка,, Ложь, Истина);
	КонецЕсли; 
...
Процедура КопироватьСсылкуАктивнойСтрокиАктивнойФормыЛкс(Параметры) Экспорт 
	...
	ирОбщий.КлючиСтрокБДИзТаблицыФормыЛкс(Форма, КлючТекущейСтроки);
	Если КлючТекущейСтроки <> Неопределено Тогда
		ирОбщий.БуферОбмена_УстановитьЗначениеЛкс(КлючТекущейСтроки,, Ложь, Истина);
	КонецЕсли; 
...
Функция Изм(Значение, ИмяЗначения = "п") Экспорт 
	Результат = ирОбщий.ИзмЛкс(Значение, ИмяЗначения);
	Возврат Результат;
КонецФункции
