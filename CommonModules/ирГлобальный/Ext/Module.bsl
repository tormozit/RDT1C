...
Процедура РедактироватьОбъектИзАктивнойФормыЛкс() Экспорт 

	ИмяПоляФормы = "";
	ИмяТаблицыФормы = "";
	Ссылки = ирОбщий.СсылкиИлиКлючиЗаписейИзАктивнойУправляемойФормыЛкс(ИмяТаблицыФормы, ИмяПоляФормы);
	Если Ссылки.Количество() > 0 Тогда
		ПоляТаблицы = ирОбщий.ПоляТаблицыБДЛкс(Ссылки[0].Метаданные().ПолноеИмя());
		ИмяПоляТаблицыБД = "";
		Для Каждого ПолеТаблицы Из ПоляТаблицы Цикл
			...
			КонецЕсли; 
		КонецЦикла;
		ирОбщий.ОткрытьСсылкуВРедактореОбъектаБДЛкс(Ссылки[0],,, ИмяПоляТаблицыБД);
	КонецЕсли; 

КонецПроцедуры

Процедура РедактироватьОбъектИзТекущейЯчейкиАктивнойФормыЛкс() Экспорт 
	
	ПараметрКоманды = ирОбщий.ПолучитьСсылкуИзТекущейЯчейкиАктивнойУправляемойФормыЛкс();
...
Процедура ОбработатьОбъектыИзАктивнойФормыЛкс() Экспорт 
