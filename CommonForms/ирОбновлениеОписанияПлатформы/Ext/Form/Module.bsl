...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Для Каждого КлючИЗначение Из ИменаФайлов Цикл
		...
		Для Каждого ТегСсылки Из ТегиСсылок Цикл
			...
			КонецЕсли; 
		КонецЦикла;  
		Вхождения = RegExp.Execute(ТекстовыйДокумент.ПолучитьТекст());
		Для каждого Вхождение Из Вхождения Цикл
			ИмяФункции = Вхождение.SubMatches(0);
			ВложенныеВхождения = RegExpВложенный.Execute(Вхождение.Value);
			НомерПараметра = 1;
	...
	Для Каждого Item Из Результат Цикл
		...
		ВхожденияВариантов = RegExp.Execute(Описание);
		ОписанияВариантов = Новый СписокЗначений;
		Если ВхожденияВариантов.Count = 0 Тогда
		...
		Для Каждого ЭлементСписка Из ОписанияВариантов Цикл
			...
			Результат = RegExp.Execute(ЭлементСписка.Значение);
			Если Результат.Count = 0 Тогда
				Продолжить;
			КонецЕсли; 
			ОписаниеВарианта = Результат.Item(0).SubMatches(0);
			...
			КонецЕсли;
			Результат = RegExp.Execute(ОписаниеВарианта);
