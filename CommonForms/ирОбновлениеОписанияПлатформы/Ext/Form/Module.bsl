...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Для Каждого Item Из Результат Цикл
		...
		шТегНеКлюч =     "(?:\s*<[^>""]+>\s*)";
		шТегКлюч =       "(?:\s*<(?:div|p)[^>]+>\s*)";
		шКонецТегаКлюч = "(?:\s*</(?:div|p)>\s*)";
		Если ТекстПрочитан Тогда
			Описание = Текст.ПолучитьТекст();
			Если Найти(НРегПуть, "/tables/") > 0 Тогда 
				RegExp.Global = Ложь;
				RegExp.Pattern = "Синтаксис" + шТегНеКлюч + "(([А-Яа-я_A-Za-z0-9\-\s\:]+)(\.(&lt;[^&]+&gt;|[А-Яа-я_A-Za-z0-9\-\s\:]+))*)";
				Результат = RegExp.НайтиВхождения(Описание);
				Если Результат.Количество() > 0 Тогда
					НовоеСлово = ЗаменитьТегиУгловыхСкобок(Результат[0].SubMatches(0));
				КонецЕсли;
			КонецЕсли;
		...
		Если ТипКонтекста = Неопределено Тогда
			...
			Тогда
				...
				Если Результат.Количество() > 0 Тогда
					ТипКонтекста = ЗаменитьТегиУгловыхСкобок(Результат[0].SubMatches(1));
				КонецЕсли;
			КонецЕсли;
		...
		RegExp.Global = Истина;
		RegExp.Pattern = шТегКлюч + "Вариант синтаксиса:\s*([^<""]+)" + шКонецТегаКлюч;
		ВхожденияВариантов = RegExp.НайтиВхождения(Описание);
		ОписанияВариантов = Новый СписокЗначений;
		Если ВхожденияВариантов.Количество() = 0 Тогда
			ОписанияВариантов.Добавить(Описание);
		Иначе
			ТекущаяПозиция = 1;
			ТекущийВариант = "";
			Для Каждого Вхождение Из ВхожденияВариантов Цикл
				Если ТекущаяПозиция > 1 Тогда
					ОписанияВариантов.Добавить(Сред(Описание, ТекущаяПозиция, Вхождение.FirstIndex - ТекущаяПозиция), ТекущийВариант);
				КонецЕсли; 
				ТекущаяПозиция = Вхождение.FirstIndex;
				ТекущийВариант = Вхождение.SubMatches(0);
			КонецЦикла;
			ОписанияВариантов.Добавить(Сред(Описание, ТекущаяПозиция), Вхождение.SubMatches(0));
		КонецЕсли; 
		Для Каждого ЭлементСписка Из ОписанияВариантов Цикл
			...
			//RegExp.Pattern = "<(?:div|p) class=""V8SH_chapter"">\s*(?:<p[^>]*>)?Параметры:?</(?:div|p)>(.*)<(?:div|p) class=""V8SH_chapter"">\s*(?:<p[^>]*>)?Описание(?: варианта метода)?:?</(?:div|p)>";
			RegExp.Pattern = шТегКлюч + "(?:<p[^>]*>)?Параметры:?" + шКонецТегаКлюч + "(.*?)(?:" + шТегКлюч + "(?:<p[^>]*>)?Описание(?: варианта метода)?:?" + шКонецТегаКлюч + "|$)";
			Результат = RegExp.НайтиВхождения(ЭлементСписка.Значение);
			Если Результат.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли; 
			ОписаниеВарианта = Результат[0].SubMatches(0);
			Если ТипСлова = "Таблица" Тогда
				RegExp.Pattern = "(<a href=""v8help://SyntaxHelperContext(/[А-Яа-я_A-Za-z0-9/]+params/[А-Яа-я_A-Za-z0-9/]+.html)"">[А-Яа-я_A-Za-z0-9\-\s\(\)]+</a>)";
			Иначе
				// Грязный прием поиска до первой английском буквы "p"
				RegExp.Pattern = шТегКлюч + "(&lt;[^&]*&gt;)" 
					+ "()?(?:\s*(?:\((необязательный|обязательный)?\)\s*)?" + шКонецТегаКлюч + "(?:Тип:" + шТегНеКлюч + "?((?:" + шТип + ")+))?)?(?:[^p]*?Значение по умолчанию: " + шТег + "?([^<>]+)" + шТег + "?\.)?";
			КонецЕсли;
			Результат = RegExp.НайтиВхождения(ОписаниеВарианта);
			...
			Для Каждого Item Из Результат Цикл
				...
				Если ТипСлова = "Конструктор" Тогда
					СтруктураСтроки.Слово = "<Новый>";
					СтруктураСтроки.ВариантСинтаксиса = НовоеСлово;
				Иначе
					СтруктураСтроки.Слово = НовоеСлово;
					СтруктураСтроки.ВариантСинтаксиса = ЭлементСписка.Представление;
				КонецЕсли; 
				...
				Если ТипСлова <> "Таблица" Тогда
					...
					КонецЕсли;
					СтруктураСтроки.Значение = Item.SubMatches(6);
				...
				Если Item.SubMatches(1) <> "" Тогда
					...
					Тогда
						...
						RegExp2.Pattern = "Тип параметра:" + шТегНеКлюч + "?((?:" + шТип + ")+)";
						Результат2 = RegExp2.НайтиВхождения(Текст.ПолучитьТекст());
				...
				Если ТекстПоискаТипов <> Неопределено Тогда
					RegExp2.Global = Истина;
					RegExp2.Pattern = шТип;
					Результат2 = RegExp2.НайтиВхождения(ТекстПоискаТипов);
					СтрокаТипаЗначения = "";
					Для Каждого Вхождение2 Из Результат2 Цикл
						СтрокаТипаЗначения = СтрокаТипаЗначения + ", " + СокрЛП(Вхождение2.SubMatches(0));
					КонецЦикла;
	...
	КонецЦикла;
	Если ВключитьАнализСрабатыванияЗамен Тогда
		ирОбщий.ИсследоватьЛкс(СрабатываниеЗамен);
	КонецЕсли; 
...
СрабатываниеЗамен = Новый Соответствие;
Для Каждого КлючИЗначение Из СоответствиеЗамены Цикл
	СрабатываниеЗамен.Вставить(КлючИЗначение.Ключ, 0);
КонецЦикла;
