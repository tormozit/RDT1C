		...
		Если Лев(НоваяСтрока.Слово, СтрДлина(МаркерПеречисления)) = МаркерПеречисления Тогда 
			...
			Попытка
				Если Строка(Вычислить(ИмяСистемногоПеречисления)) = "Перечисление" + ИмяСистемногоПеречисления Тогда
					...
					СтруктураКлюча = Новый Структура("Слово, ЯзыкПрограммы", ИмяСистемногоПеречисления, НоваяСтрока.ЯзыкПрограммы);
					...
					Если НайденныеСтроки.Количество() = 0 Тогда 
						...
						// Закомментировал 16.09.2011. Раскомментировал 22.12.2011
					...
					НоваяСтрокаКонтекста.Слово = ИмяСистемногоПеречисления;
	...
	Для Каждого Item Из Результат Цикл
		...
		Если Результат.Count > 0 Тогда
			...
			Иначе
				...
				RegExp.Pattern = "(&lt;[А-Яа-я_A-Za-z0-9\-\s]*&gt;)" 
					+ "()?(?:\s*(?:\((необязательный|обязательный)?\)\s*)?</div>Тип:(?:<[^>""]+>)?((?:" + шТип + ")+))?";
			КонецЕсли;
...
Процедура ПриОткрытии()
	...
	ЗагрузитьТаблицыИзСтруктурыФайлов(СтруктураФайлов);
	Каталог = ХранилищеОбщихНастроек.Загрузить("ИнструментыРазработчикаTormozit", "ирОбновлениеОписанияПлатформы_Каталог");
...
Процедура КоманднаяПанель1ОткрытьТаблицу(Кнопка)
	...
	Если ТипЗнч(ТекущийЭлемент) <> Тип("ТабличноеПоле") Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ТекущийЭлемент.Значение, Ложь,,,,, ТекущийЭлемент);
	
КонецПроцедуры

Процедура ПриЗакрытии()
	
	ХранилищеОбщихНастроек.Сохранить("ИнструментыРазработчикаTormozit", "ирОбновлениеОписанияПлатформы_Каталог", Каталог);
