...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	КонецЕсли; 
	Попытка
		ирОбщий.ВыполнитьЗапросЭтойСУБДЛкс("SET DATEFORMAT ymd;");
	Исключение
		ОписаниеОшибки = ОписаниеОшибки(); // для отладки
	КонецПопытки; 
	// http://devtrainingforum.v8.1c.ru/forum/thread.jsp?id=542796
	ТекущаяДата = ТекущаяДата();
	СмещениеВремени = ТекущаяДата - ирОбщий.ВыполнитьЗапросЭтойСУБДЛкс("select CURRENT_TIMESTAMP",,, 0, Ложь)[0][0] - 1;
	...
	Если ГруппаОтбора <> Неопределено И ГруппаОтбора.Использование Тогда
		ОбработкаСтруктураХраненияБД = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирСтруктураХраненияБД");
		#Если Сервер И Не Сервер Тогда
	...
	Запросы = ирОбщий.ВыполнитьЗапросЭтойСУБДЛкс(ТекстЗапроса, РежимОтладки = 1, "Статистика MSSQL", 0, Ложь);
	Если Запросы = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Запросы.Колонки.Добавить("ТаблицыМетаданных");
	Запросы.Колонки.Добавить("ТекстЗапросаМета");
	//! Запросы.Колонки.Добавить("query_text");
