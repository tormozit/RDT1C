...
Процедура ОбработкаРасшифровки(ДанныеРасшифровки, ЭлементРасшифровки, ТабличныйДокумент, ДоступныеДействия, СписокДополнительныхДействий, РазрешитьАвтовыборДействия, ЗначенияВсехПолей) Экспорт 
	...
	Тогда 
		СписокДополнительныхДействий.Вставить(0, "ОткрытьСтрокуМодуля", "Открыть строку модуля",, ирКэш.КартинкаПоИмениЛкс("ирКонфигуратор1С8"));
	КонецЕсли;
		...
		Для Счетчик = 1 По 2 Цикл
			...
			Для Каждого СтрокаТаблицы Из СтрокиДляПровеки Цикл
				...
				Если Лев(ИскомаяСтрока, 1) <> "." И Найти(ШаблонСимволаИдентификатора, Сред(СтрокаТаблицы.Текст, ПозицияВхождения - 1, 1)) > 0 Тогда
					Продолжить;
				КонецЕсли;
				КлючВызова = "Вызов." + Формат(СтрокаТаблицы.Количество, "ЧЦ=9; ЧВН=; ЧГ=") + ":" + ирОбщий.СсылкаСтрокиМодуляЛкс(СтрокаТаблицы.Модуль, СтрокаТаблицы.НомерСтроки, СтрокаТаблицы.Метод);
				Если ПодменюВызовов.НайтиПоЗначению(КлючВызова) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ПодменюВызовов.Добавить(КлючВызова,
						"" + XMLСтрока(СтрокаТаблицы.Количество) + "; " + СтрокаТаблицы.ВремяЧистое + ": " + СтрокаТаблицы.Модуль + ":" + XMLСтрока(СтрокаТаблицы.НомерСтроки) + "." + СтрокаТаблицы.Метод 
						+ "():: " + ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(СтрокаТаблицы.Текст));
			КонецЦикла;
		КонецЦикла;
		ПодменюВызовов.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
...
Процедура ДействиеРасшифровки(ВыбранноеДействие, ПараметрВыбранногоДействия, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ВыбранноеДействие = "ОткрытьСтрокуМодуля" Тогда
		ирКлиент.ОткрытьСсылкуСтрокиМодуляЛкс(ПараметрВыбранногоДействия["СсылкаСтроки"]);
	ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ВыбранноеДействие, "Вызов.") Тогда 
		СсылкаСтроки = ирОбщий.ТекстБезПервогоФрагментаЛкс(ВыбранноеДействие, ":");
