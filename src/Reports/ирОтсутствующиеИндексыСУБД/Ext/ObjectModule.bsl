...
Функция РассчитатьСелективностьПоля(ИмяТаблицы, ИмяПоля)
	...
	|GROUP BY RowType"; //@skip-check unknown-method-property
	ТекстЗапроса = ирОбщий.СтрЗаменитьЛкс(ТекстЗапроса, "<<ИмяПоля>>", ИмяПоля);
	ТекстЗапроса = ирОбщий.СтрЗаменитьЛкс(ТекстЗапроса, "<<ИмяТаблицы>>", ИмяТаблицы); // тут возможно ошибка 
	Попытка
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ТаблицаИзСУБД = ирОбщий.ВыполнитьЗапросЭтойСУБДЛкс(ТекстЗапроса, РежимОтладки = 1,, 0, Ложь);
	Если ТаблицаИзСУБД = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ТаблицаРезультата = ирОбщий.ПустаяТаблицаИзНабораДанныхСхемыЛкс(СхемаКомпоновкиДанных.НаборыДанных.Таблица);
	ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаИзСУБД, ТаблицаРезультата);
