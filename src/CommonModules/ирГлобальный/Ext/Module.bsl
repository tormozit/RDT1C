...
Процедура ОткрытьОднократноАдаптациюРасширенияЛкс(выхПерехватКлавиатуры = Неопределено) Экспорт 
	...
	Если Истина
		И Найти(ПараметрЗапуска, "ОтключитьПерехватКлавиатурыИР") = 0 
	...
	Если Найти(НРег(ПараметрЗапуска), НРег("НеДобавлятьРолиИР")) Тогда
		Если Истина
			#Если Не ТонкийКлиент Тогда
			И ирКэш.НомерВерсииБСПЛкс() >= 300
			И ирОбщий.ОтобратьКоллекциюЛкс(ирКэш.РасширенияКонфигурацииЛкс(), "Э.Ключ.Имя=П1",,"ЗащитаРолейРасширенийДляАдминовБСП3").Количество() = 0
			#КонецЕсли
		Тогда
			ОткрытьФорму("Обработка.ирУстановщикРасширения.Форма");
			Сообщить("Рекомендуется нажать кнопку ""Установить защиту"", чтобы БСП не мог удалять роли ИР.");
		КонецЕсли;
	КонецЕсли;
	#КонецЕсли
...
Процедура ВнешнееСобытиеЛкс(Источник, Событие, Данные) Экспорт
	...
	Если Источник = "KeyboardHook" Тогда 
		Если ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+~") Тогда
			...
			Если ПередатьОбработку Тогда
				...
				Если АктивнаяУправляемаяФорма = Неопределено Тогда
					...
					Для Каждого ПроверятьПолеHTML Из ирОбщий.ЗначенияВМассивЛкс(Ложь, Истина) Цикл
						Для Каждого Форма Из ирКэш.ОткрытыеФормыПодсистемыЛкс() Цикл // см. Обработка.ирПлатформа.Форма.Картинка
					...
					#Если ТолстыйКлиентОбычноеПриложение Тогда
						Если Не СобытиеОбработано И ирОбщий.ПроверитьНажатиеКлавишЛкс(Данные, "CTRL+~") Тогда
...
Процедура ПередЗавершениемРаботыСистемыЛкс(Отказ) Экспорт
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		Если Не ПравоДоступа("Просмотр", Метаданные.Подсистемы.ИнструментыРазработчикаTormozit) Тогда
			Возврат;
		КонецЕсли;
		// Закрываем явно все инструменты, чтобы у них сохранились настройки окон https://www.hostedredmine.com/issues/1005050
		...
		КонецЦикла; 
	#Иначе
		// Иначе будут ошибки. Например, Открытие окон при завершении работы запрещено
	#КонецЕсли
КонецПроцедуры

Процедура ПриЗавершенииРаботыСистемыЛкс() Экспорт
	#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		Если Не ПравоДоступа("Просмотр", Метаданные.Подсистемы.ИнструментыРазработчикаTormozit) Тогда
			Возврат;
		КонецЕсли;
		мПлатформа = ирКэш.Получить();
		мПлатформа.ОчиститьКэшАнализатораЯзыка(); // Чтобы записались кэши модулей
	#КонецЕсли
КонецПроцедуры
...
Процедура ОбработатьПараметрЗапускаДляВсехРежимовЛкс() Экспорт 
	...
	Если Найти(ПараметрЗапуска, "ТестироватьМетаданныеИР") Тогда
		...
		Текст = ирОбщий.ТаблицаЗначенийВЖСОНЛкс(Результат);
...
Процедура ВыполнитьПроверкиПриНачалеСеансаЛкс() Экспорт 
	...
	#Если ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение Тогда
		...
		КонецЕсли;
		ПингТекст="";
		ЧислоМС = ирКлиент.ЗамеритьПингДоСервераЛкс(ПингТекст);
		Если ЧислоМС > 10 Тогда
			ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("Обнаружена большая задержка канала до сервера %2 - %1. Толстый клиент лучше запускать на близком к серверу компьютере.",
				ПингТекст, ирОбщий.ИмяКомпьютераКластераЛкс()));
		КонецЕсли;
		#Если ТолстыйКлиентОбычноеПриложение Тогда
			Если Истина
			...
			Тогда
				Сообщить("Инструменты разработчика: Рекомендуется включить в свойствах конфигурации флажок ""Использовать управляемые формы в обычном приложении"" https://devtool1c.ucoz.ru/forum/2-842-1");
			КонецЕсли; 
		#КонецЕсли 
	#КонецЕсли
...
Процедура РедактироватьОбъектАктивнойФормыЛкс(Параметры) Экспорт 
	...
	Если Ссылка <> Неопределено Тогда
		...
		Если ТипЗнч(Форма.ТекущийЭлемент) = Тип("ТаблицаФормы") Тогда
			...
			Если Форма.ТекущийЭлемент.ТекущаяСтрока <> Неопределено И ирОбщий.ЛиДанныеФормыСПоискомЛкс(ДанныеТаблицыФормы) Тогда
...
Процедура ОткрытьТекстАктивнойФормыЛкс(Параметры) Экспорт 
	...
	Текст = ирКлиент.Форма_ЗначениеТекущегоПоляЛкс(АктивнаяФорма,, ПолеТекста);
	Если ПолеТекста <> Неопределено Тогда
		ВыделениеДвумерное = ПолеТекста.ВыделениеДвумерное(Истина);
	КонецЕсли;
	ирКлиент.ОткрытьТекстЛкс(Текст,, "",,, АктивнаяФорма,,, ВыделениеДвумерное);
