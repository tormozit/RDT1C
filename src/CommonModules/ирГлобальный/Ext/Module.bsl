				...
				Если АктивнаяУправляемаяФорма = Неопределено Тогда
					...
					#Если ТолстыйКлиентОбычноеПриложение Тогда
						Если Не СобытиеОбработано И Найти(Данные, КодыКлавиш["CTRL+~"]) = 1 Тогда
							...
							ВводДоступен = Ложь;
							Для Каждого КлючИЗначение Из ирКэш.ОткрытыеФормыЧужиеЛкс() Цикл
								...
								Если КлючТекущейСтроки <> Неопределено И ирОбщий.ЛиСсылкаНаОбъектБДЛкс(КлючТекущейСтроки) Тогда
									...
									Если Форма.Открыта() Тогда
										ирКлиент.НачатьОтслеживаниеЧужойФормыЛкс(Форма);
										ВводДоступен = ирКлиент.Форма_ВводДоступенЛкс(Форма);
										Если ВводДоступен Тогда
											Прервать;
										КонецЕсли;
...
Процедура КопироватьСсылкуАктивнойФормыЛкс(Параметры) Экспорт 
	...
	Ссылка = ирКлиент.КлючОсновногоОбъектаФормыЛкс(Форма);
	Если Ссылка <> Неопределено Тогда
		ирКлиент.БуферОбменаПриложения_УстановитьЗначениеЛкс(Ссылка);
	КонецЕсли; 
...
Процедура ПроверитьФормыСсылокЛкс() Экспорт 
	...
	Для Каждого КлючИЗначение Из ОткрытыеФормыЧужие Цикл
		...
		Тогда 
			...
			Если ЗначениеЗаполнено(Ссылка) Тогда 
				...
				ИначеЕсли ПараметрыСлежения.Ссылка <> Ссылка Тогда
					...
					ирКлиент.НачатьОтслеживаниеЧужойФормыЛкс(ФормаСсылки);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого Ключ Из КлючиУдалить Цикл
		ОткрытыеФормыЧужие.Удалить(Ключ);
	КонецЦикла;
	...
	Если АктивнаяФорма <> Неопределено Тогда
		...
		Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(Ссылка) Тогда
			ирКлиент.НачатьОтслеживаниеЧужойФормыЛкс(АктивнаяФорма);
		КонецЕсли;
	КонецЕсли;
	Если Ложь
		Или ОткрытыеФормыЧужие.Количество() > 0 
...
//  Неопределено.
//
Функция От(Знач Объект, Знач Объект2 = Неопределено, Знач ВнешниеНаборыДанных = Неопределено, ОтложеннаяОтладка = Ложь, ПорогОбрезкиВременнойТаблицы = 500000, Наименование = "",
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		КонецЕсли; 
	#КонецЕсли 
	Результат = ирОбщий.ОтЛкс(Объект, Объект2, ВнешниеНаборыДанных, ОтложеннаяОтладка, ПорогОбрезкиВременнойТаблицы, Наименование, МенеджерВременныхТаблиц);
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
