...
Функция НомерВерсииПодсистемыИРЛкс() Экспорт 
	...
	КонецЦикла;
	Результат = Число(Фрагменты[0]) * 100 + Число(Фрагменты[1]); // 4 цифр
	Возврат Результат;
...
//   Структура - 
//
Функция ДоступныеОбщиеМодулиЛкс(Знач Сервер, Знач КлиентОбычный = Неопределено, Знач КлиентУправляемый = Неопределено) Экспорт 
...
// Возвращаемое значение:
//  Форма -
Функция ПассивнаяФормаПоИмениЛкс(ИмяФормы) Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("ПассивнаяФормаПоИмениЛкс");
		КлючНаборПараметров = "";
		Результат = КэшФункции[КлючНаборПараметров];
		Если Результат <> Неопределено Тогда
			Возврат Результат;
	...
	Попытка
		#Если Клиент Тогда
			Результат = ирКлиент.ПолучитьФормуЛкс(ИмяФормы, ирКлиент.ПараметрыБыстрогоСозданияФормыЛкс());
		#КонецЕсли
	Исключение
...
Функция ПолеТекстаМодуляЛкс(ИмяМодуля) Экспорт
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции = ирПортативный.КэшФункцииЛкс("ПолеТекстаМодуляЛкс");
	...
	ФайлМодуля = Получить().ФайлМодуляИзКраткогоИмени(ИмяМодуля);
	Если ФайлМодуля.Существует() Тогда
		ПолеТекстаМодуля.ПолеТекста.Прочитать(ФайлМодуля.ПолноеИмя);
	КонецЕсли;
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		КэшФункции[КлючНаборПараметров] = Результат;
	КонецЕсли; 
