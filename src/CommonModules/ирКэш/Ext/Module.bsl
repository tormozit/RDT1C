...
Функция СобытияПоИмениОбщегоТипаЛкс(Знач ИмяОбщегоТипа, Знач ИмяТипаРасширения = Неопределено) Экспорт 
	...
	СобытияЭлементаФормы = мПлатформа.СловаКонтекстаПредопределенные(мПлатформа.НоваяСтруктураТипа(ИмяОбщегоТипа),, "Событие");
	Если ИмяТипаРасширения <> Неопределено Тогда
		мПлатформа.СловаКонтекстаПредопределенные(мПлатформа.НоваяСтруктураТипа(ИмяТипаРасширения),, "Событие",,,,,, СобытияЭлементаФормы);
	КонецЕсли;
	КолонкаНРег = СобытияЭлементаФормы.Колонки.Добавить("НСлово", Новый ОписаниеТипов("Строка"));
	ирОбщий.ОбновитьКопиюКолонкиВНижнемРегистреЛкс(СобытияЭлементаФормы, "Слово", КолонкаНРег.Имя);
