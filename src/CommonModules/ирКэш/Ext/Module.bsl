...
Функция ИмяТаблицыИзМетаданныхЛкс(Знач ОбъектИлиИмяМД, ЛиТаблицаИзменений = Ложь, ЛиДвиженияССубконтоДляРегистраБухгалтерии = Истина, ТолькоРазрешенные = Ложь) Экспорт 
	...
	Если ЛиТаблицаИзменений Тогда 
		ДублиТаблицыБД = ирКэш.ТаблицаВсехТаблицБДЛкс().НайтиСтроки(Новый Структура("НПолноеИмя", Нрег(ПолноеИмя + ".Изменения")));
		Если ДублиТаблицыБД.Количество() > 1 Тогда 
			...
			ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонИменЛкс("Таблица изменений %1 перекрыта табличной частью ""Изменения"". Поэтому невозможно получить количество изменений в ней.",, ПолноеИмя),
	...
	Тогда
		Если Не ирКэш.ЛиДоступныТаблицыКонстантЛкс() Тогда
...
Функция ЛиДоступенJSONЛкс() Экспорт
...
Функция ЛиДоступныТаблицыКонстантЛкс() Экспорт 
