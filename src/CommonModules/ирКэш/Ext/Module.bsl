...
//
// Параметры:
//  Таблица      - ТаблицаЗначений - тест
//  Категория   - Число - 0 - хранимый и корневой, 1 - нехранимый и корневой, 2 - дочерний, 3 - внешний;
//  Единственное - Строка - 
//  ИндексКартинкиЕдинственное - Число - см. БиблиотекаКартинок.ирДеревоМетаданных 
//  Множественное - Строка;
//  ИндексКартинкиМножественное - Число - см. БиблиотекаКартинок.ирДеревоМетаданных
//
...
Функция ТипыМетаОбъектов(ЛиВключатьНехранимые = Истина, ЛиВключатьДочерние = Истина, ЛиВключатьВнешние = Истина) Экспорт
	...
	Таблица.Колонки.Добавить("Категория", Новый ОписаниеТипов("Число")); //  см. ДобавитьВТаблицуТиповМетаОбъектов.Категория
	...
	Если ЛиВключатьНехранимые Тогда
		...
		ДобавитьВТаблицуТиповМетаОбъектов(Таблица, 1, "ОбщаяФорма", 7,        "ОбщиеФормы", 6);
		...
		КонецЕсли;
		Если ирКэш.НомерВерсииПлатформыЛкс() >= 805001 Тогда
			ДобавитьВТаблицуТиповМетаОбъектов(Таблица, 1, "ЦветПалитры", , "ЦветаПалитры", );
		КонецЕсли;
		Если ирКэш.НомерВерсииПлатформыЛкс() >= 805004 Тогда
			ДобавитьВТаблицуТиповМетаОбъектов(Таблица, 0, "Очередь", 62, "Очереди", 62);
		КонецЕсли;
	...
	ДобавитьВТаблицуТиповМетаОбъектов(Таблица, 0, "Константа", 1,             "Константы", 0);
...
Функция ЛиДоступенVBScriptЛкс() Экспорт
	Попытка 
...
Функция ТаблицаИменМетаТиповЛкс(Знач ЛиСсылочные = Истина, Знач ЛиОпределяемые = Истина) Экспорт
	...
	Если ЛиСсылочные Тогда
		...
		КонецЦикла;
	КонецЕсли;
	Если ЛиОпределяемые И ирКэш.НомерРежимаСовместимостиЛкс() > 803001 Тогда
