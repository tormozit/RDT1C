...
Функция ОбновитьКэшМодулейЛкс(Знач КаталогКэша = "", Знач Асинхронно = Ложь, Знач РазрешитьДиалоги = Истина) Экспорт 
	...
	Для Каждого РасширенияЭлемент Из ирКэш.РасширенияКонфигурацииЛкс() Цикл
		ВсеГруппыМетаданных.Добавить(РасширенияЭлемент.Ключ, РасширенияЭлемент.Ключ.Имя, Не ОтключенныеРасширения.Свойство(РасширенияЭлемент.Ключ.Имя));
	КонецЦикла;
	...
	Если РазрешитьДиалоги Тогда
		...
		Если Не Асинхронно Тогда
			ВыбранныеГруппыМетаданных = ирКлиент.ВыбратьЭлементСпискаЗначенийЛкс(ВсеГруппыМетаданных,,, "Обновление " + ТекстВопроса + " конфигуратора", Истина);
		...
		КонецЕсли;
	Иначе
		ВыбранныеГруппыМетаданных = ВсеГруппыМетаданных;
	КонецЕсли;
	КаталогЗеркалаБазы = "";
	...
	Для Каждого ЭлементСписка Из ВсеГруппыМетаданных Цикл 
		...
		Если ВыбранныеГруппыМетаданных.Найти(РасширениеКонфигурации) = Неопределено Тогда
			Если РасширениеКонфигурации <> Неопределено Тогда
				ОтключенныеРасширения.Вставить(РасширениеКонфигурации.Имя);
			КонецЕсли;
			Продолжить;
		КонецЕсли;
