...
Функция ПоказатьОшибкуВТекстеПрограммыЛкс(Знач ПолеТекста = Неопределено, Знач СтартоваяСтрока = 0, Знач СтартоваяКолонка = 0, Знач ЯзыкПрограммы = 0, Знач ЛиМодально = Ложь,
	Знач ИнформацияОбОшибке = Неопределено, Знач ИмяМодуля = Неопределено, ПредставлениеКонтекста = "", ЭтаФорма = Неопределено) Экспорт
...
Функция ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина, ЗаголовокФормы = "", РедактироватьМодально = Истина,
	...
	Если Не РедактированиеРазрешено Тогда 
		// Опасно. Изменяем параметр РасширенноеЗначение
		Если Ложь
			Или ТипЗнч(РасширенноеЗначение) = Тип("ТекстовыйДокумент") 
			Или ТипЗнч(РасширенноеЗначение) = ирОбщий.ТипОболочкаHTMLДокументаЛкс()
		Тогда
			РасширенноеЗначение = РасширенноеЗначение.ПолучитьТекст(); 
		КонецЕсли;
	КонецЕсли;
	...
	Тогда 
		Если Не ЗначениеЗаполнено(КлючУникальности) Тогда
			КлючУникальности = РасширенноеЗначение;
		КонецЕсли;
		СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли ТипРасширенногоЗначения = Тип("МакетОформленияКомпоновкиДанных") Тогда
		...
		КонецЕсли;
		Форма = Новый КонструкторМакетаОформленияКомпоновкиДанных(РасширенноеЗначение);
		Форма.Редактировать(ирКэш.ФормаПустышкаЛкс());
	ИначеЕсли ТипРасширенногоЗначения = Тип("ГрафическаяСхема") Тогда
		#Если Сервер И Не Сервер Тогда
			РасширенноеЗначение = Новый ГрафическаяСхема;
		#КонецЕсли
		СтандартнаяОбработка = Ложь;
		РасширенноеЗначение.Показать(ЗаголовокФормы);
		...
		Тогда
			...
			Если СтандартнаяОбработка Тогда
				Если ПримитивныеТипыЛкс().Найти(ТипЗнч(РасширенноеЗначение)) <> Неопределено Тогда
					ирОбщий.СообщитьЛкс("" + ТипЗнч(РасширенноеЗначение) + ": " + РасширенноеЗначение);
				Иначе
					ОткрытьЗначение(РасширенноеЗначение);
				КонецЕсли;
				СтандартнаяОбработка = Ложь;
...
//
Функция НайтиПоказатьФрагментВПолеТекстаЛкс(Форма, Знач ПолеТекста, Знач СтрокаПоиска, Знач СловоЦеликом = Ложь, Знач ИскатьСНачала = Ложь, Знач РазрешитьАктивациюПоля = Истина, Знач РазбиватьНаСлова = Ложь) Экспорт
	...
	Если СтрокаПоиска <> "" Тогда
		Вхождение = ирОбщий.СтрНайтиСловоЛкс(ПолеТекста.ПолучитьТекст(), СтрокаПоиска, КонечнаяПозиция, СловоЦеликом, РазбиватьНаСлова);
		#Если Сервер И Не Сервер Тогда
