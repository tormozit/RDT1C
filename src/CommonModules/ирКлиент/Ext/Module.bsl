...
//
Функция ОткрытьМетодМодуляПоОпределениюЛкс(Знач СтрокаМетода, Знач РазрешитьДиалоги = Истина, Знач ИмяПараметра = "", Знач ПолеТекстаНовое = Неопределено, Знач СмещениеСтрокиМетода = 0) Экспорт
	мПлатформа = ирКэш.Получить();
	...
	Если Не РазрешитьДиалоги Тогда
		Возврат ирКлиент.СсылкаМетодаМодуляИзСтрокиМетодовЛкс(СтрокаМетода);
	КонецЕсли;
	Если ВариантОткрытьВКонфигураторе <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если РезультатВыбора <> Неопределено И РезультатВыбора = ВариантОткрытьВКонфигураторе Тогда 
		ирКлиент.ПерейтиКОпределениюМетодаВКонфигуратореЛкс(СтрокаСозданияРодителя + "." + ИмяМетода);
	Иначе 
		Если ЗначениеЗаполнено(ИмяМодуля) Тогда
			ПолеТекстаНовое = ирКлиент.ОткрытьПолеТекстаМодуляКонфигурацииЛкс(ИмяМодуля);
		КонецЕсли;
