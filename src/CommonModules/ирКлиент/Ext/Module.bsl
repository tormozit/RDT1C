...
// Авторское право (с) 2007-2025, Старых С.А.
// Лицензия MIT
			...
			ИначеЕсли Заголовок = "Результаты поиска" Тогда
				...
				Для Каждого Строка Из Таблица Цикл     
					...
					Если СтруктураСсылки <> Неопределено Тогда
						Строка.Метод = мПлатформа.НайтиИмяМетодаСтрокиМодуля(СтруктураСсылки.МодульСРасширением, СтруктураСсылки.НомерСтроки);
					КонецЕсли;
				КонецЦикла;
			Иначе
				ПодготовитьТаблицуИзКонфигуратора(Таблица);
...
//  КнопкиПриемника - КнопкиКоманднойПанели - приемник;
...
//.
// Параметры:
//   ОбработкаОбъект - ВнешняяОбработкаОбъект - 
Функция ЛиНедоступныйПутьПортативногоРежимаЛкс(Знач ОбработкаОбъект) Экспорт
	Результат = Истина
		...
		И Лев(ОбработкаОбъект.ИспользуемоеИмяФайла, 2) <> "\\";;
	Возврат Результат;
КонецФункции
...
Функция СравнитьТаблицыИнтерактивноЛкс(Знач Значение1, Знач Значение2, Знач Модально = Ложь, Знач БезКлючевыхКолонок = Ложь, Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "") Экспорт 
...
Функция СравнитьЗначенияВФормеЧерезXMLЛкс(Знач Значение1, Знач Значение2, Знач Модально = Ложь, Знач Название1 = "", Знач Название2 = "", Знач XMLПредставлениеДляНеизвестныхТипов = Истина, ВариантСинтаксиса = "",
	Знач ОбщееНазвание = "", Знач РазрешитьСобственныйСравнитель = Истина, Знач КлючУникальностиФормы = "", Знач НомерСтроки1 = 0, Знач НомерСтроки2 = 0) Экспорт 
...
Функция ОткрытьИсториюГитаЛкс(Знач ЧтоИскать = "", Знач ИмяМодуля = "") Экспорт
...
Функция ИмяФайлаМодуляВПапкеГитаЛкс(Знач ИмяМодуля, ПапкаГитаКэш = "", Знач ДобавитьSrc = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПапкаГитаКэш) Тогда
		ирКлиент.ОткрытьНастройкиПоляПрограммыНаЗаднемПланеЛкс("Укажите ""Гит репозиторий"" в открывшейся форме настроек",, "ГитРепозиторий");
		Возврат "";
	КонецЕсли;
	...
	Если Фрагменты.Количество() % 2 = 1 Или ЭтоМодульФормы Тогда
		...
		Если Не ЭтоЕДТ Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ИмяФайлаМодуляВГите = ПапкаГитаКэш + ИмяФайлаМодуляВГите + ИмяСвойстваАнг + ".bsl";
	Иначе
...
Функция СоздатьМенеджерСохраненияНастроекФормыЛкс(Знач ЭтаФорма, Знач КлючНазначенияИспользования = "", Знач ЗагружатьСохранятьНастройкуПоУмолчанию = Истина, Знач ПараметрыЗагрузкиНастройкиПоУмолчанию = Неопределено,
	Знач РасширениеФайла = "", Знач СоставНастройкиФормы = Неопределено) Экспорт 
...
Процедура СохранитьНастройкуФормыЛкс(Знач ЭтаФорма, Знач СохранитьГлобальные = Истина) Экспорт 
	...
	Если МенеджерСохраненияНастроек <> Неопределено И МенеджерСохраненияНастроек.ЗагружатьСохранятьНастройкуПоУмолчанию Тогда
		...
		Тогда
			...
			ДополнититьСохраняемуюНастройкуФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы, ЭтаФорма.СвойстваГлобализуемые());
...
Функция СохраняемаяНастройкаФормыЛкс(Знач ЭтаФорма, выхНовоеНаименование = "", ПорогТаблицыЗначений = 100000) Экспорт 
	...
	Если ЗначениеЗаполнено(ИменаСвойств) Тогда
		Если СохраняемаяНастройкаФормы = Неопределено Тогда
			СохраняемаяНастройкаФормы = Новый Структура;
		КонецЕсли;
		ДополнититьСохраняемуюНастройкуФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы, ИменаСвойств, ПорогТаблицыЗначений);
	КонецЕсли;
	мПлатформа = ирКэш.Получить();
	Если Истина
	...
	Тогда
		...
		ДополнититьСохраняемуюНастройкуФормыЛкс(ЭтаФорма, СохраняемаяНастройкаФормы, ЭтаФорма.СвойстваГлобализуемые(), ПорогТаблицыЗначений);
...
Процедура ДополнититьСохраняемуюНастройкуФормыЛкс(Знач ЭтаФорма, Знач СохраняемаяНастройкаФормы, Знач ИменаСвойствФормы, Знач ПорогТаблицыЗначений = 100000) Экспорт
	...
	Если СохранитьФайл Тогда
		...
		Если ДляВосстановления Тогда 
			...
			Если Истина
				...
				И ТекущаяДата() - ТаблицаФайлов[ЧислоФайлов - 1].ДатаИзменения < МинРасстояниеМеждуТочкамиСекунд
				И ТаблицаФайлов[ЧислоФайлов - 1].ДатаИзменения - ТаблицаФайлов[ЧислоФайлов - 1 - ЧислоФайловВТочке].ДатаИзменения < МинРасстояниеМеждуТочкамиСекунд
			Тогда
				Для Счетчик = 1 По 2 Цикл
...
Процедура ДобавитьДействияРасшифровкиПоПолюЛкс(ЛиПрямоеПоле = Истина, Знач СписокДействийОткрыть, Знач ЗначениеПоля, Знач СписокДействийОтборать, Знач СписокДействийОформить,
	...
	ПредставлениеЗначения = ирОбщий.ПредставлениеСОграничениемДлиныЛкс(ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеПоля.Значение,,,,, Истина), ПорогДлиныПредставления);
	Если Ложь
	...
	Если мПлатформа.мПроверкаСовместимостиКонфигурацииВыполнялась <> Истина Тогда
		...
		Если Метаданные.ХранилищеОбщихНастроек <> Неопределено Тогда 
			...
			Если ирОбщий.ВосстановитьЗначениеЛкс(ИмяПроверочнойНастройки) = Неопределено Тогда 
				ирОбщий.СообщитьЛкс("В конфигурации переопределено хранилище общих настроек и оно не восстанавливает сохраненные значения. Корректная работа подсистемы ""Инструменты разработчика"" невозможна", СтатусСообщения.Важное,, Истина, Ложь);
			КонецЕсли; 
		КонецЕсли; 
		Если Не ирОбщий.ЛиСовместимыйЯзыкСистемыЛкс() Тогда 
			ирОбщий.СообщитьЛкс("Язык системы сеанса 1С (параметр /L) не является русским или английским. Корректная работа подсистемы ""Инструменты разработчика"" не гарантируется", СтатусСообщения.Важное,, Истина, Ложь);
		КонецЕсли; 
...
Процедура ОткрытьПараметрыИИЛкс() Экспорт
	ОткрытьНастройкиПоляПрограммыНаЗаднемПланеЛкс("", Истина, "МодельИИ");
КонецПроцедуры
...
Функция ПолучитьФормуЛкс(ПолноеИмяФормы, Параметры = Неопределено, Владелец = Неопределено, Уникальность = Неопределено, Окно = Неопределено, ТолькоВнешниеФормы = Ложь, Знач НачатьОтслеживание = Истина) Экспорт
	...
	Если Фрагменты.Количество() > 2 Тогда
		...
		КонецЕсли;
		Если ирКэш.ЛиСеансТолстогоКлиентаУПЛкс() Тогда
			ИмяОбработки = ПодключитьВнешнююОбработкуЛкс(Менеджер.ИспользуемоеИмяФайла);
			//ирПортативный #Если Сервер И Не Сервер Тогда
				...
				Результат = ПолучитьФорму("ВнешняяОбработка." + ИмяОбработки + ".Форма." + ИмяФормы, Параметры, Владелец, Уникальность);
...
Функция ПодключитьВнешнююОбработкуЛкс(ИмяФайла) Экспорт
	// https://partners.v8.1c.ru/forum/t/870763/m/870763
	ИмяОбработки = ирСервер.ПодключитьВнешнююОбработкуЛкс(ИмяФайла); // - 20мс+
	Если ПустаяСтрока(ИмяОбработки) Тогда
		ИмяОбработки = ирСервер.ПодключитьВнешнююОбработкуЛкс(ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайла))); // - 20мс+
	КонецЕсли;
...
Функция БуферОбмена_КопироватьЛкс(Знач ЭтаФорма = Неопределено, Знач УстановитьПредставление = Истина) Экспорт
	...
	Если Значение <> Неопределено Тогда
		...
		Тогда 
			...
			Представление = ирОбщий.СтрСоединитьЛкс(Значение.ВыгрузитьЗначения(), Символы.ПС);
		КонецЕсли;
...
Процедура Форма_ПриОткрытииЛкс(Знач ЭтаФорма, Знач КлючНазначенияИспользования = "", Знач ЗагружатьСохранятьНастройкуПоУмолчанию = Истина, Знач ПараметрыЗагрузкиНастройкиПоУмолчанию = Неопределено,
	...
	КонецЕсли;
	Если ирОбщий.МетодРеализованЛкс(ЭтаФорма, "СохраняемаяНастройкаФормы") Тогда
		СоздатьМенеджерСохраненияНастроекФормыЛкс(ЭтаФорма, КлючНазначенияИспользования, ЗагружатьСохранятьНастройкуПоУмолчанию, ПараметрыЗагрузкиНастройкиПоУмолчанию, РасширениеФайла, СоставНастройкиФормы);
	КонецЕсли;
...
Функция ДобавитьОтборВИсториюТабличногоПоляЛкс(Знач ЭтаФорма, Знач ТабличноеПолеИлиКлюч, Знач Отбор, СтарыйОтбор, Знач ИгнорироватьЭлементы = "") Экспорт 
	...
	Если Строка(Отбор) <> Строка(СтарыйОтбор) Тогда 
		Если "" + Отбор <> "" Тогда 
			...
			ПредставлениеОтбора = ирОбщий.ПредставлениеСОграничениемДлиныЛкс(НастройкаКомпоновки.Отбор, 150);
...
Функция ДобавитьОтборКомпоновкиВИсториюТаблицыБДЛкс(Знач ЭтаФорма, Знач ПолноеИмяТаблицы, Знач АктивнаяНастройка, Знач СтараяНастройка, выхОтборИзменен = Ложь) Экспорт 
	...
	Если выхОтборИзменен Тогда 
		Если ПредставлениеОтбора <> "" Тогда 
			...
			ПредставлениеЗначения = ирОбщий.ПредставлениеСОграничениемДлиныЛкс(НастройкаКомпоновки.Отбор, 150);
...
Функция ОткрытьНастройкиПоляПрограммыНаЗаднемПланеЛкс(Знач Причина = "Для анализа содержимого модулей нужно обновить кэш модулей через открывшуюся форму настроек.", Знач ВзятьФокус = Ложь,
...
Функция ОткрытьПолеТекстаМодуляКонфигурацииЛкс(Знач ИмяМодуля, выхФормаТекста = Неопределено) Экспорт
	...
	Если Не ФайлМодуля.Существует() Тогда
		ОткрытьНастройкиПоляПрограммыНаЗаднемПланеЛкс();
		Возврат Неопределено;
	КонецЕсли;
...
Функция ОбновитьКэшМодулейЛкс(Знач КаталогКэша = "", Знач Асинхронно = Ложь, Знач РазрешитьДиалоги = Истина) Экспорт 
	...
	Если Асинхронно Тогда
		ФормаНастроек = ОткрытьНастройкиПоляПрограммыНаЗаднемПланеЛкс("");
	КонецЕсли;
	ГруппыМетаданных = Новый СписокЗначений;
	Для Каждого РасширенияЭлемент Из ирКэш.РасширенияКонфигурацииЛкс() Цикл
		ГруппыМетаданных.Добавить(РасширенияЭлемент.Ключ, РасширенияЭлемент.Ключ.Имя, Истина);
	КонецЦикла;
	ГруппыМетаданных.Добавить(, "<Конфигурация> (до нескольких минут)", Истина);
	Если РазрешитьДиалоги Тогда
		...
		Если Не Асинхронно Тогда
			ГруппыМетаданных = ирКлиент.ВыбратьЭлементСпискаЗначенийЛкс(ГруппыМетаданных,,, "Обновление " + ТекстВопроса + " конфигуратора", Истина);
			Если ГруппыМетаданных = Неопределено Тогда
				Возврат Ложь;
			КонецЕсли;
	...
	КонецЕсли;
	ТекстЛога = ""; 
	ирОбщий.СообщитьЛкс("Будут выгружены модули расширений, кроме патчей EF, и затем основной конфигурации");
	Для Каждого ЭлементСписка Из ГруппыМетаданных Цикл 
		Если Не ЭлементСписка.Пометка Тогда
			Продолжить;
		КонецЕсли;
		РасширениеКонфигурации = ЭлементСписка.Значение;
