	...
	Если ЭлементУправления <> Неопределено Тогда
		...
		Тогда
			...
			Если ТипЗнч(ЭлементУправления) = Тип("ПолеВвода") Тогда
			    ...
			    Тогда
					//! ЭлементУправления=0; // Расширение поля ввода справочника
					Если Ложь
						Или (Истина
...
// Параметры:
//  Форма - Форма
//  ТабличноеПоле - ТабличноеПоле, Расширение табличного поля табличной части -;
	...
	Тогда
		...
		Иначе
			...
			Иначе
				...
				Тогда
					//! ВыгрузкаРезультата = 0; // Табличная часть
...
//
// Параметры:
//  Элемент						 - Расширение поля ввода справочника, ПолеВвода - 
...
//
// Параметры:
//  ОсновнойЭУ			 - Расширение табличного поля списка документов - 
//  РежимИмяСиноним		 - 	 - 
//  РазрешитьСортировку	 - 	 - 
//
Процедура НастроитьАвтоТабличноеПолеДинамическогоСпискаЛкс(ОсновнойЭУ, РежимИмяСиноним = Ложь, РазрешитьСортировку = Истина) Экспорт
	...
	Если КолонкаТП = Неопределено Тогда
		КолонкаКартинки = КолонкиТП.Добавить(ИмяКартинка); // Расширение колонки табличного поля списка документов
...
Процедура РазобратьПозициюМодуляВСтрокеТаблицыЛкс(Знач ТекущаяСтрока, Знач ИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = Неопределено,
	Знач ЧислоПредСтрок = 1, Знач ИмяМетода = "", Знач ЗаполнитьТекстСтроки = Ложь) Экспорт 
...
Функция ДобавитьВФайлТочекОстановаЛкс(Знач ИмяФайла, Знач СсылкаСтрокиМодуля, Знач ДляПерехода = Истина) Экспорт
	...
	Если ЭлементТочки = null Тогда
		...
		ДокументХМЛ.ДочерниеУзлы[0].ДобавитьДочерний(ЭлементТочки);
		ИДТочки = ДокументХМЛ.СоздатьЭлемент("id");
		ЭлементТочки.ДобавитьДочерний(ИДТочки);
	...
	ЭлементТочкаОстанова.ДобавитьДочерний(ЭлементНомерСтроки);
	Если ДляПерехода Тогда
		...
		ЭлементТочкаОстанова.ДобавитьДочерний(ЭлементУсловие);
		Если ирКэш.НомерВерсииПлатформыЛкс() >= 803024 Тогда 
			...
			ЭлементТочкаОстанова.ДобавитьДочерний(ЭлементАктивностьУсловия);
		КонецЕсли;
	КонецЕсли;
