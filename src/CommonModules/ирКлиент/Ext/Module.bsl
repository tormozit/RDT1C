...
//   - 
//
Функция ОткрытьСсылкуСтрокиМодуляЛкс(Знач Ссылка) Экспорт
	мПлатформа = ирКэш.Получить();
	СписокВыбора = Новый СписокЗначений;
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда 
		ВариантОткрытьВКонфигураторе = ДобавитьВариантОткрытияМодуляВКонфигураторе(СписокВыбора);
...
//
// Параметры:
//  ТабличноеПоле			 - ТабличноеПоле, Расширение табличного поля табличной части, Расширение табличного поля таблицы значений - ;
...
Процедура УстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗЛкс(ЭтаФорма, ТабличноеПоле, ЗначениеОбработки, Знач ТипИсточника = Неопределено, Знач Колонка = Неопределено, Знач ТолькоВыделенныеСтроки = Истина,
	...
	ИначеЕсли ЕстьОтборСтрок Тогда
		//! ТабличноеПоле = 0; // Расширение табличного поля табличной части
...
// Возвращаемое значение:
//    Число - 
Функция НомерСтрокиМодуляИзПозицииЛкс(Знач ИмяФайлаМодуля, Знач Позиция, Знач ПеречитатьФайл = Ложь, Знач ТекстФайла = Неопределено, Знач ТекстФрагмента = Неопределено, выхПолеТекста = Неопределено,
...
Функция ПолеТекстаМодуляБезСтруктурыТипаЛкс(Знач ИмяФайлаМодуля, ТекстФайла = Неопределено, Знач ПеречитатьФайл = Ложь, выхФайлМодуля = Неопределено) Экспорт
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	Если Найти(ИмяФайлаМодуля, ":") > 0 Тогда
		...
		Иначе
			ПолеТекстаМодуля.ПрочитатьТекстИзФайлаБезМетаданных(ИмяФайлаМодуля);
	...
	Иначе
		...
		Иначе
			Если ПеречитатьФайл И выхФайлМодуля.Существует() Тогда
				ПолеТекстаМодуля.ПрочитатьТекстИзФайлаБезМетаданных(выхФайлМодуля.ПолноеИмя);
			КонецЕсли;
			ТекстФайла = ПолеТекстаМодуля.мОригинальныйТекст;
