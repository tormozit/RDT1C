...
Процедура ПерейтиКОбъектуМетаданныхВКонфигуратореЛкс(Знач ПолноеИмя) Экспорт 
	...
	#КонецЕсли                                                                        
	ТекстМодуля = ирОбщий.ПолноеИмяМДЕдинственноеВМножественноеЛкс(ПолноеИмя);
	Если Не ЗначениеЗаполнено(ТекстМодуля) Тогда
		Возврат;
	КонецЕсли; 
	ИмяВнешнейОбработки = ирОбщий.ИдентификаторИзПредставленияЛкс("ПереходВКонфигураторе." + ПолноеИмя);
...
Процедура ПерейтиКОпределениюМетодаВКонфигуратореЛкс(Знач СтрокаВызоваМетодаБезСкобок) Экспорт  
	...
	ИмяВнешнейОбработки = ирОбщий.ИдентификаторИзПредставленияЛкс("ПереходВКонфигураторе." + СтрокаВызоваМетодаБезСкобок);
	ИмяФайла = КаталогВременныхФайлов() + ИмяВнешнейОбработки + ".epf";
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
...
Процедура ОткрытьМетодМодуляПоИмениЛкс(Знач ИмяМетода, Знач СтруктураТипаИлиИмяМодуля = Неопределено) Экспорт
	мПлатформа = ирКэш.Получить();
	...
	Если МодульМетаданных <> Неопределено Тогда
		...
		Если МетодМодуля <> Неопределено Тогда
			...
			ОткрытьМетодМодуляПоОпределениюЛкс(МетодМодуля);
			Возврат;
		КонецЕсли;
...
//
Процедура ОткрытьМетодМодуляПоОпределениюЛкс(Знач СтрокаМетода, Знач ИмяПараметра = "", Знач ПолеТекстаНовое = Неопределено, Знач СмещениеСтрокиМетода = 0) Экспорт
	мПлатформа = ирКэш.Получить();
	...
	Если СписокВыбора.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	РезультатВыбора = ВыбратьВариантОткрытияСсылкиМодуляЛкс(ВариантОткрытьВКонфигураторе, СписокВыбора);
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если РезультатВыбора = ВариантОткрытьВКонфигураторе Тогда 
		Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда
			Ссылка = СсылкаМетодаМодуляИзСтрокиМетодовЛкс(СтрокаМетода);
			ОткрытьСсылкуСтрокиМодуляВКонфигуратореЛкс(Ссылка);
		Иначе 
	...
	Иначе 
		...
		Если ПолеТекстаНовое <> Неопределено Тогда
			ПолеТекстаНовое.ПерейтиКОпределениюЛокальногоМетода(СтрокаМетода, ИмяПараметра, СмещениеСтрокиМетода);
		КонецЕсли;
	КонецЕсли; 
КонецПроцедуры
...
Функция ВыбратьВариантОткрытияСсылкиМодуляЛкс(Знач ВариантОткрытьВКонфигураторе, Знач СписокВыбора)
	Если ВариантОткрытьВКонфигураторе <> Неопределено Тогда
...
//   - 
//
Функция ОткрытьСсылкуСтрокиМодуляЛкс(Знач Ссылка, Знач АнализаторКода) Экспорт
	мПлатформа = ирКэш.Получить();
	СписокВыбора = Новый СписокЗначений;
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда 
		ВариантОткрытьВКонфигураторе = ДобавитьВариантОткрытияМодуляВКонфигураторе(СписокВыбора);
...
// Параметры:
//    Ссылка - Неопределено - 
Процедура ОткрытьСсылкуСтрокиМодуляВКонфигуратореЛкс(Знач Ссылка) Экспорт
	мПлатформа = ирКэш.Получить();
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда
...
// Возвращаемое значение:
//    Строка - 
Функция СсылкаМетодаМодуляИзСтрокиМетодовЛкс(СтрокаМетода) Экспорт
	Если СтрокаМетода.ПозицияОпределения <> Неопределено Тогда
		НомерСтрокиМодуля = НомерСтрокиМодуляИзПозицииЛкс(СтрокаМетода.ИмяМодуля, СтрокаМетода.ПозицияОпределения);
	КонецЕсли;
	ИмяМодуля = СтрокаМетода.ИмяМодуля;
	ИмяМетода = СтрокаМетода.Имя;
...
Функция ПоказатьОшибкуВТекстеПрограммыЛкс(Знач ПолеТекста = Неопределено, Знач СтартоваяСтрока = 0, Знач СтартоваяКолонка = 0, Знач ЯзыкПрограммы = 0, Знач ЛиМодально = Ложь,
	...
	Если Вхождения.Количество() > 0 Тогда
		...
		ФигурноеОписаниеОшибки = ПоследнееВхождение.Подгруппы[0];
...
Процедура СравнитьЗначенияВФормеЧерезXMLЛкс(Значение1, Значение2, Знач Модально = Ложь, Название1 = "", Название2 = "", Знач XMLПредставлениеДляНеизвестныхТипов = Истина, ВариантСинтаксиса = "",
	Знач ОбщееНазвание = "", РазрешитьСобственныйСравнитель = Истина) Экспорт 
...
// Параметры:
//  ВариантСинтаксиса - Строка - "ВстроенныйЯзык", "ЯзыкЗапросов", "ЯзыкКомпоновки", "XML"
Процедура ДобавитьОбъектВБуферИПредложитьСравнениеЛкс(Знач ЗначениеДляСравнения, Знач ЭтаФорма = Неопределено, Знач ТекущееСвойство = "", Знач ЗаголовокЭлементаФормы = "", ВариантСинтаксиса = "") Экспорт 
...
Процедура ОткрытьНовоеОкноКонсолиИзЕеОкнаЛкс(Знач ЭтаФорма, Знач ДеревоОбъектов) Экспорт 
	...
	НоваяФорма = ирКлиент.ОткрытьНовоеОкноФормыЛкс(ЭтаФорма);
...
Процедура ОтладитьОбъектПоСтруктуреЛкс(Знач СтруктураПараметров, Знач Модально = Ложь) Экспорт 
	...
	Если ТипОперации = "Отладить" Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли; 
		ирОбщий.ОтладитьЛкс(Объект, Модально, Объект2, СтруктураПараметров.ВнешниеНаборыДанных,,,,, МенеджерВременныхТаблиц);
...
Процедура ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Знач ЭлементФормыИлиЭлементОтбора, Знач КлючИстории, Знач ЗапоминатьПоследние = 20, Знач _НеЗапоминатьПустыеТипизированные = Истина,
	ДополнительныйКлючИстории = "", Знач ЗначениеПоля = Неопределено, Знач МаксДлинаТекста = 1000) Экспорт
	...
	Если ЗначениеПоля = Неопределено Тогда
		...
		Иначе
			...
			ЗначениеПоля = ирОбщий.ДанныеЭлементаФормыЛкс(ПолеВвода);
		КонецЕсли; 
		Если ТипЗнч(ЗначениеПоля) = Тип("СтандартнаяДатаНачала") Тогда
			ЗначениеПоля = ЗначениеПоля.Дата;
		КонецЕсли;
	КонецЕсли; 
	Если ЗначениеЗаполнено(ЗначениеПоля) Тогда
		Если Истина
			И ТипЗнч(ЗначениеПоля) = Тип("СписокЗначений") 
			И ЗначениеПоля.Количество() > 0
		Тогда
			Если Ложь
				Или ЗначениеЗаполнено(ЗначениеПоля[0].Представление) 
				Или ЗначениеЗаполнено(ЗначениеПоля[0].Картинка) 
			Тогда
				ЗначениеПоля = ирОбщий.КопияОбъектаЛкс(ЗначениеПоля);
			КонецЕсли;
			Если ЗначениеЗаполнено(ЗначениеПоля[0].Представление) Тогда
				ирОбщий.УстановитьСвойствоВКоллекцииЛкс(ЗначениеПоля, "Представление");
			КонецЕсли;
			Если ЗначениеЗаполнено(ЗначениеПоля[0].Картинка) Тогда
				ирОбщий.УстановитьСвойствоВКоллекцииЛкс(ЗначениеПоля, "Картинка");
			КонецЕсли;
		КонецЕсли;
...
Функция ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина, ЗаголовокФормы = "", РедактироватьМодально = Истина,
	...
	Если Ложь
		...
		Или (Истина
			И ирКэш.ЛиДоступноФорматированнаяСтрокаЛкс()
			И (Ложь
	...
	Тогда 
		...
		ИначеЕсли ТипРасширенногоЗначения = Тип("ТабличныйДокумент") Тогда
			...
			Иначе
				Форма.ПолеТабличногоДокумента = РедактируемоеЗначение;
			КонецЕсли; 
		...
		Иначе
			...
			Тогда
				Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
					...
					Возврат Неопределено;
				КонецЕсли;
				Форма = ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ФорматированныйДокумент", Новый Структура("Текст", РедактируемоеЗначение),, КлючУникальности);
		...
		Если ТипЗнч(Форма) = Тип("Форма") Тогда
			Если РедактированиеРазрешено И РедактироватьМодально Тогда
				Если Ложь
					...
					Или ТипРасширенногоЗначения = Тип("ДеревоЗначений") 
				Тогда 
					Форма.НачальноеЗначениеВыбора = РедактируемоеЗначение.Скопировать(); // Неглубокая копия, зато с полным сохранением содержимого, включая объекты метаданных
				Иначе
					ИспользоватьXDTO = ТипРасширенногоЗначения = Тип("ТабличныйДокумент"); // Чтобы не терялось свойство "КодЯзыка"
					Форма.НачальноеЗначениеВыбора = ирОбщий.КопияОбъектаЛкс(РедактируемоеЗначение, ИспользоватьXDTO); // Универсально, но с потерей несериализуемых типов
				КонецЕсли; 
			Иначе
				Форма.НачальноеЗначениеВыбора = РедактируемоеЗначение;
			КонецЕсли;
		КонецЕсли;
		Если Ложь
	...
	ИначеЕсли ТипРасширенногоЗначения = Тип("МакетОформленияКомпоновкиДанных") Тогда
		...
		КонецЕсли;
		Форма = Новый КонструкторМакетаОформленияКомпоновкиДанных(РедактируемоеЗначение);
		Форма.Редактировать(ирКэш.ФормаПустышкаЛкс());
...
Функция ОткрытьТаблицуЗначенийЛкс(Знач ТаблицаЗначений, Знач ТекущаяСтрока = Неопределено, Модально = Истина, Заголовок = "", РежимВыбора = Ложь, ТолькоПросмотр = Истина,
	ОтключитьОформление = Ложь, КлючУникальности = Неопределено, Знач ТекущаяКолонка = Неопределено) Экспорт 
...
//   - 
//
Функция ВыбратьЭлементСпискаЗначенийЛкс(Знач СписокЗначений, Знач ТекущийЭлемент = Неопределено, Модально = Истина, Заголовок = "", МножественныйВыбор = Ложь, Знач Сортировать = Ложь) Экспорт 
...
// Результат - Булево - Истина если значение было изменено
Функция ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(Знач Элемент, СтандартнаяОбработка, РасширенноеЗначение = Null, Знач СтруктураОтбора = Неопределено, Знач ЗапретитьБольшиеТипы = Ложь) Экспорт
...
Процедура ТабличноеПолеПриВыводеСтрокиЛкс(Знач ЭтаФорма, Знач Элемент, Знач ОформлениеСтроки, Знач ДанныеСтроки, Знач КнопкаРежимаОтображения = Неопределено, Знач КолонкиСПиктограммамиТипов = "",
			...
			Если ДопСвойства.Отбор <> Неопределено Тогда
				...
				Если КолонкиПоиска.Количество() > 0 Тогда
					...
					Для Каждого КлючИЗначение Из КолонкиПоиска Цикл
						КлючИЗначение.Значение[0] = "(" + ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(КлючИЗначение.Значение[0]) + ")";
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
		...
		Для Каждого Колонка Из КолонкиТаблицы Цикл
			...
			Тогда
				ПредставлениеЗначения = ирОбщий.РасширенноеПредставлениеЗначенияЛкс(ЗначениеЯчейки, Колонка,, РасширенноеПредставлениеХранилищЗначений, Ложь, ОтбражатьПустые);
			КонецЕсли; 
	...
	Кнопка.Доступность = ЛОжь
		...
		Или (Истина
			И (Ложь
				...
				Или ТипЗнч(ЭтаФорма.ВладелецФормы) = ирОбщий.ТипУправляемаяФормаЛкс())
...
Функция ЗагрузитьСтрокиВТабличноеПолеЛкс(Знач Форма, Знач ТабличноеПоле, Знач РазрешитьИнтерактивнуюУстановку = Истина, Знач МетаданныеКолонок = Неопределено, ЛиТЧ = Истина,
	...
	Если ИнтерактивныеКолонки.Количество() = 0 Тогда
		...
		Возврат Ложь;
	КонецЕсли;
	ИменаКолонокДанных = ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(ИнтерактивныеКолонки, "Представление"));
...
Процедура ТабличноеПолеСОтборомПросмотраОтобратьЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ИмяКолонкиНомерСтроки = "", Знач РежимОтладки = Ложь, Знач НовоеИспользованиеОтбора = Неопределено) Экспорт 
	...
	ИначеЕсли Не ирКэш.ЛиДоступнаКомпоновкаТаблицыЗначенийКлс() Тогда
		СтрокаПоиска = "";
	...
	Иначе
		...
		КонецЕсли;                                
		ДопСвойства = ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ТабличноеПоле);
		Если ДопСвойства.Свойство("НачальноеИзменятьСоставСтрок") И ДопСвойства.НачальноеИзменятьСоставСтрок Тогда
...
Функция ПрименитьСтрокуПоискаКТабличномуПолюЛкс(ЭтаФорма, ТабличноеПоле, СтрокаПоиска, Знач ИменаКолонокДанныхДляПоиска = "", Знач АктивироватьНайденнуюСтроку = Истина,
	...
	Если Не ЗначениеЗаполнено(ИменаКолонокДанныхДляПоиска) Тогда
		ИменаКолонокДанныхДляПоиска = ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(ДеревоЗначений.Колонки));
	КонецЕсли;
...
Процедура ОбновитьЧислоЗагруженныхЭлементовВыборкиЛкс(ТабличноеПоле, КоманднаяПанель, ПолеСтрокиКоличестваРезультата, КоличествоЗагружено, КоличествоРезультата, ВсеСчитано) Экспорт
	...
	#Если Сервер И Не Сервер Тогда
		ТабличноеПоле = Новый ТабличноеПоле;
	#КонецЕсли
	КолонкаНомераСтроки = ТабличноеПоле.Колонки.Найти(ирКэш.ИмяКолонкиНомерСтрокиЛкс());
...
//
Функция НайтиПоказатьФрагментВПолеТекстаЛкс(Форма, Знач ПолеТекста, Знач СтрокаПоиска, Знач СловоЦеликомИлиШаблон = Ложь, Знач ИскатьСНачала = Ложь, Знач РазрешитьАктивациюПоля = Истина,
			...
			Если Вхождение <> Неопределено Тогда
				...
				Иначе
					Если СловоЦеликомИлиШаблон <> Ложь Тогда
						ПозицияНачала = ПозицияНачала + СтрДлина(Вхождение.Подгруппа0); 
						ДлинаВхождения = СтрДлина(Вхождение.Подгруппа1);
					Иначе
...
Процедура ПодготовитьЭлементыФормыЛкс(ЭтаФорма) Экспорт
	...
	Для Каждого ЭлементФормы Из ЭтаФорма.ЭлементыФормы Цикл
		...
		Если Истина
			...
			И КоманднаяПанель.Кнопки.Найти(ИмяКнопкиСтруктурыКоманднойПанели) = Неопределено
		Тогда
			НужноВстроить = Ложь;
			КоличествоКнопок = 0;
...
Функция ЗагрузитьТабличныйДокументИнтерактивноЛкс(ТабличныйДокумент = Неопределено, выхПолноеИмяФайла = "", Знач СпособЧтенияЗначений = Неопределено) Экспорт 
	...
	Если ДиалогВыбораФайла.Выбрать() Тогда
		...
		Иначе
			...
			ирОбщий.ПрочитатьТабличныйДокументExcelЛкс(ТабличныйДокументЧтение, выхПолноеИмяФайла, СпособЧтенияЗначений);
...
Функция ОткрытьНовоеОкноФормыЛкс(ЭтотОбъект) Экспорт 
	...
	Если Ложь
		...
		Или ТипЗнч(ЭтотОбъект) = ирОбщий.ТипУправляемаяФормаЛкс() 
	Тогда
	...
	Иначе
		...
		Если ОбъектМД = Неопределено Тогда
			...
			Иначе
				...
				ПолноеИмяФайла = ирПортативный.ПолноеИмяФайлаПортативногоОбъектаМетаданныхЛкс(Фрагменты[1], ТипМетаданных); // На ОС Linux сюда приходит имя в нижнем регистре и потому файл не находится http://devtool1c.ucoz.ru/forum/2-2473-1
...
Процедура КопироватьСсылкуАктивнойСтрокиФормыЛкс(Форма) Экспорт 
	...
	Если КлючиСтрок.Количество() > 1 Тогда
		БуферОбменаПриложения_УстановитьЗначениеЛкс(ирОбщий.МассивВСписокЗначенийЛкс(КлючиСтрок),, ирОбщий.СтрСоединитьЛкс(ПредставлениеКлючей, Символы.ПС));
	ИначеЕсли КлючТекущейСтроки <> Неопределено Тогда
		БуферОбменаПриложения_УстановитьЗначениеЛкс(КлючТекущейСтроки,, ПредставлениеКлючей[0]);
	КонецЕсли;

КонецПроцедуры

Процедура КопироватьСсылкиЯчеекФормыЛкс(Форма) Экспорт 
	
	КлючТекущейСтроки = Неопределено;
	Ссылки = ирКлиент.ЗначенияВыделенныхЯчеекТаблицыЛкс(Форма, КлючТекущейСтроки);
    ПредставлениеКлючей = СсылкиВТекстВВидеНавигационныхСсылокЛкс(Ссылки);
	Если Ссылки.Количество() > 1 Тогда
		БуферОбменаПриложения_УстановитьЗначениеЛкс(ирОбщий.МассивВСписокЗначенийЛкс(Ссылки),, ПредставлениеКлючей);
	ИначеЕсли КлючТекущейСтроки <> Неопределено Тогда
		БуферОбменаПриложения_УстановитьЗначениеЛкс(КлючТекущейСтроки,, ПредставлениеКлючей);
	КонецЕсли;
...
Процедура РазобратьПозициюМодуляВСтрокеТаблицыЛкс(Знач ТекущаяСтрока, Знач ИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = "",
	Знач ЧислоПредСтрок = 1, Знач ИмяМетода = "") Экспорт 
	...
	//Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСтрокиМодуля) Тогда
		...
		Если ПолеТекстаМодуля = Неопределено Тогда 
			СсылкаСтрокиМодуля = "<Файл модуля не найден в кэше модулей>";
			ТекущаяСтрока.Ссылка = СсылкаСтрокиМодуля; 
			Возврат;
		КонецЕсли;
		#Если Сервер И Не Сервер Тогда
		...
		//
		ИмяМетода = мПлатформа.НайтиИмяМетодаСтрокиФайлаМодуля(ПолеТекстаМодуля, ТекущаяСтрока.НомерСтрокиМодуля,,,,, СмещениеСтрокиМетода);
		ТекстСтроки = ПолеТекстаМодуля.ПолеТекста.ПолучитьСтроку(НомерСтрокиМодуля);
		//ИмяМодуля = ФайлМодуля.ИмяБезРасширения;
		ИмяМодуля = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(СтрЗаменить(ФайлМодуля.ПолноеИмя, мПлатформа.ПапкаКэшаМодулей.ПолноеИмя + "\", ""));
		ТекущаяСтрока.Ссылка = ирОбщий.СсылкаСтрокиМодуляЛкс(ИмяМодуля, ПолеТекстаМодуля.мНачальнаяСтрока, ИмяМетода, СмещениеСтрокиМетода, ТекстСтроки);
...
//.
// Параметры:
//    ИмяФайлаМодуля - Строка - если полное имя файла, то чтение текста кэшируется через ТекстФайла
...
//    выхПолеТекста - ОбработкаОбъект.ирОболочкаПолеТекста - 
//    ФайлМодуля - Файл - 
// Возвращаемое значение:
//    Число - 
Функция НомерСтрокиМодуляИзПозицииЛкс(Знач ИмяФайлаМодуля, Знач Позиция, Знач ПеречитатьФайл = Ложь, Знач ТекстФайла = "", Знач ТекстФрагмента = Неопределено, выхПолеТекста = Неопределено, выхФайлМодуля = Неопределено) Экспорт
	Если выхПолеТекста = Неопределено Тогда
		выхПолеТекста = ПолеТекстаМодуляБезСтруктурыТипаЛкс(ИмяФайлаМодуля, ТекстФайла, ПеречитатьФайл, выхФайлМодуля);
	КонецЕсли;
	Если выхПолеТекста <> Неопределено Тогда
		выхПолеТекста.ПолеТекста.УстановитьГраницыВыделения(Позиция + 1, Позиция + 1 + СтрДлина(ТекстФрагмента),,,,, Ложь);
...
//    ТекстФайла - Строка - кэш текста файла
// Возвращаемое значение:
//    ОбработкаОбъект.ирОболочкаПолеТекста - 
Функция ПолеТекстаМодуляБезСтруктурыТипаЛкс(ИмяФайлаМодуля, ТекстФайла = Неопределено, Знач ПеречитатьФайл = Ложь, выхФайлМодуля = Неопределено) Экспорт
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	Если Найти(ИмяФайлаМодуля, ":") > 0 Тогда
		выхФайлМодуля = Новый Файл(ИмяФайлаМодуля);
		...
		Если ТекстФайла = Неопределено Тогда  
			ПолеТекстаМодуля.ПолеТекста.УстановитьТекст(ТекстФайла);
		Иначе
			ПолеТекстаМодуля.ПолеТекста.Прочитать(ИмяФайлаМодуля);
			ТекстФайла = ПолеТекстаМодуля.ПолеТекста.ПолучитьТекст();
		КонецЕсли;
	Иначе
		ПолеТекстаМодуля = ирКэш.ПолеТекстаМодуляБезСтруктурыТипаЛкс(ИмяФайлаМодуля);
		выхФайлМодуля = мПлатформа.ФайлМодуляИзКраткогоИмени(ИмяФайлаМодуля);
		Если ПеречитатьФайл И выхФайлМодуля.Существует() Тогда
			ПолеТекстаМодуля.Прочитать(выхФайлМодуля.ПолноеИмя);
		КонецЕсли;
		Если СтрДлина(ПолеТекстаМодуля.ПолеТекста.ПолучитьТекст()) = 0 Тогда
			// Отсутствует файл
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция ДобавитьВФайлТочекОстановаРезультатыПоискаКонфигуратораЛкс(Знач ИмяФайла, Знач ТекстРезультатовПоиска) Экспорт 
	...
	Для Каждого Вхождение Из Вхождения Цикл
		ИмяМодуля = СтрЗаменить(Вхождение.Подгруппы[2], " ", "");
		ИмяРасширения = Вхождение.Подгруппы[0];
		Если Ложь
		...
		КонецЕсли;
		НомерСтроки = + Вхождение.Подгруппы[3];
		СсылкаСтроки = ирОбщий.СсылкаСтрокиМодуляЛкс(ИмяМодуля, НомерСтроки);
		ДобавитьВФайлТочекОстановаЛкс(ИмяФайла, СсылкаСтроки, Ложь);
	КонецЦикла;
КонецФункции
...
// Возвращаемое значение:
//    Структура - 
Функция ДобавитьВФайлТочекОстановаЛкс(Знач ИмяФайла, Знач СсылкаСтрокиМодуля, Знач ДляПерехода = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураСсылки.Файл) Тогда 
		...
		Фрагменты = ирОбщий.СтрРазделитьЛкс(СтруктураСсылки.Внутренняя);
	Иначе
		Фрагменты = ирОбщий.СтрРазделитьЛкс(СтруктураСсылки.Модуль);
	КонецЕсли;
	ИмяМодуляАнг = "";
	Если Фрагменты.Количество() = 1 Тогда
	...
	Иначе
		...
		КонецЦикла;
	КонецЕсли;
	ИмяСвойстваАнг = Фрагменты[Фрагменты.Количество() - 1];
	...
	ЭлементТочки = null;
	Корень = ДокументХМЛ.ДочерниеУзлы[0]; // Было 1
	Для Каждого ТочкаМодуляЦикл Из Корень.ДочерниеУзлы Цикл
		...
		Тогда
			ЭлементТочки = ТочкаМодуляЦикл;
			Прервать
		КонецЕсли;
	КонецЦикла;
	Если ЭлементТочки = null Тогда
		ЭлементТочки = ДокументХМЛ.СоздатьЭлемент("moduleBPInfo");
		ДокументХМЛ.ДочерниеУзлы[0].AppendChild(ЭлементТочки);
		ИДТочки = ДокументХМЛ.СоздатьЭлемент("id");
		ЭлементТочки.AppendChild(ИДТочки);
		ЭлементТипаМодуля = ДокументХМЛ.СоздатьЭлемент("debugBaseData:type");
		ИДТочки.AppendChild(ЭлементТипаМодуля);
		Если Не ПустаяСтрока(СтруктураСсылки.Расширение) Тогда
			...
			ИДТочки.AppendChild(ЭлементРасширения);
	...
	ЭлементТочкаОстанова.AppendChild(ЭлементНомерСтроки);
	Если ДляПерехода Тогда
		...
		ЭлементТочкаОстанова.AppendChild(ЭлементУсловие);
		Если ирКэш.НомерВерсииПлатформыЛкс() >= 803024 Тогда 
			...
			ЭлементТочкаОстанова.AppendChild(ЭлементАктивностьУсловия);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьАнализПравДоступаПоСпискуОбъектовЛкс(Знач СписокПолныхИменМД, КлючУникальности = "") Экспорт 
...
Процедура ОткрытьАнализПравДоступаНаТипыКолонкиБДЛкс(Знач ИмяКолонки) Экспорт 
	...
	Для Каждого Тип Из КолонкаБД.ТипЗначения.Типы() Цикл
		...
		КонецЕсли;
	КонецЦикла; 
	ОткрытьАнализПравДоступаПоСпискуОбъектовЛкс(СписокПолныхИменМД, ИмяКолонки);
...
Функция КлючОбъектаСтрокиВерсииДанныхЛкс(Знач ДанныеСтроки, Знач ПолноеИмяМД) Экспорт 
	...
	Если НадоСериализоватьКлючДанныхДляОтображенияЛкс(КорневойТип) Тогда
		Попытка
			КлючОбъекта = ирОбщий.ЗначениеИзСтрокиВнутрЛкс(ДанныеСтроки.ДанныеСтрока);
		Исключение
...
Функция ОткрытьНастройкиПоляТекстаПрограммыНаЗаднемПланеЛкс(Знач ВывестиПричинуОткрытия = Истина) Экспорт 
	...
	Если ВывестиПричинуОткрытия Тогда
		ирОбщий.СообщитьЛкс("Для получения методов и переменных модулей нужно обновить кэш модулей через открывшуюся форму настроек поля текста программы.",,, Истина);
	КонецЕсли;
	Возврат Форма;
...
Функция ДобавитьКнопкиКомандБСПЛкс(Знач КоманднаяПанельДляБСП, Знач ИмяТаблицы, Знач ЭтоСсылочныйОбъект) Экспорт 
...
Функция ОткрытьПолеТекстаМодуляКонфигурацииЛкс(Знач ИмяМодуля, выхФормаТекста = Неопределено) Экспорт
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ФайлМодуля = ирКэш.Получить().ФайлМодуляИзКраткогоИмени(ИмяМодуля);
	...
	выхФормаТекста = ирКлиент.ОткрытьТекстЛкс("", ИмяМодуля + " " + ФайлМодуля.ПолучитьВремяИзменения(), "ВстроенныйЯзык", Истина, ИмяМодуля);
...
Функция ОбновитьКэшМодулейЛкс(Знач КаталогКэша = "", Знач ЗапуститьКонфигураторПослеЗавершения = Ложь) Экспорт 
	мПлатформа = ирКэш.Получить();
	...
	ТекстЛога = ""; 
	Если ЗапуститьКонфигураторПослеЗавершения Тогда
		ФормаНастроек = ОткрытьНастройкиПоляТекстаПрограммыНаЗаднемПланеЛкс(Ложь);
		Предупреждение("Ждите", 1); // Чтобы отрисовка окна началась
	КонецЕсли;
	ирОбщий.СообщитьЛкс("Будут выгружены модули всех расширений и затем основной конфигурации");
	ОтображатьОкно = Истина;
	Для Каждого КлючИЗначение Из ирКэш.РасширенияКонфигурацииЛкс() Цикл
		...
		Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigFiles """ + КаталогКэша + "\" + РасширениеКонфигурации.Имя + """ -Extension """ + РасширениеКонфигурации.Имя + """ -Module",
			СтрокаСоединенияИнформационнойБазы(), ТекстЛога, Истина, "Выгрузка модулей расширения " + РасширениеКонфигурации.Имя,,,,,,,,,, ОтображатьОкно);
		Если Не Успех Тогда 
	...
	//	Успех = ирОбщий.ВыполнитьКомандуАвтоСервераЭтойБазыЛкс("infobase config export """ + КаталогКэша + """", ТекстЛога, "Выгрузка модулей конфигурации");
	//Иначе
		Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigFiles """ + КаталогКэша + """ -Module",
			СтрокаСоединенияИнформационнойБазы(), ТекстЛога, Истина, "Выгрузка модулей конфигурации",,,,,,,,,, ОтображатьОкно);
	//КонецЕсли;
	Если Не Успех Тогда 
		ирОбщий.СообщитьЛкс(ТекстЛога);
		Если ЛиТексСодержитБлокировкуКонфигуратораЛкс(ТекстЛога) Тогда
	...
	ирОбщий.СообщитьЛкс("Выгрузка модулей успешно завершена");
	Если ЗапуститьКонфигураторПослеЗавершения Тогда
...
Процедура ОткрытьСправкуПоПодсистемеЛкс(Объект = Неопределено) Экспорт
	...
	#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		...
		ИначеЕсли ТипЗнч(Объект) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
