...
Процедура ПодключитьОбработчикиСобытийДоступныхПолейКомпоновкиЛкс(ТабличноеПоле) Экспорт 
	...
	Если Истина
		...
		И Не ирОбщий.СтрокиРавныЛкс(Обработчик, "ТабличноеПолеПриПолученииДанных") 
	Тогда 
		ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонИменЛкс("Обнаружено переопределение обработчика ПриПолученииДанных табличного поля %1",, ТабличноеПоле.Имя));
	КонецЕсли; 
...
Процедура ВыбратьИЗаполнитьТабличнуюЧастьОбъектаБДЛкс(ТаблицаИсточник, НачальноеПолноеИмяОбъекта = "") Экспорт 
	...
	ФормаВыбораОбъектаБД = ирКлиент.ФормаВыбораОбъектаМетаданныхЛкс(,, НачальноеПолноеИмяОбъекта,, Истина,, Истина, Истина,, Истина,,, Истина);
...
Функция ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ТабличноеПоле, Знач Колонка, Знач Значение, Знач ФормаИнициатор = Неопределено, Знач ВосстанавитьТекущуюКолонку = Истина,
	...
	Если ЭлементУправления <> Неопределено Тогда
		Если ВосстанавитьТекущуюКолонку Тогда
			СтараяТекущаяКолонка = ирКлиент.ТабличноеПоле_ТекущаяКолонкаЛкс(ТабличноеПоле);
		КонецЕсли;
		ирКлиент.ТабличноеПоле_УстановитьТекущуюКолонкуЛкс(ТабличноеПоле, Колонка);
		...
		Тогда
			...
			Если МетаданныеКолонки <> Неопределено Тогда
				СтруктураОтбора = ирКлиент.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(МетаданныеКолонки, ОбъектВладелец, СтруктураТЧ);
		...
		Если ВосстанавитьТекущуюКолонку Тогда
			ирКлиент.ТабличноеПоле_УстановитьТекущуюКолонкуЛкс(ТабличноеПоле, СтараяТекущаяКолонка);                                                                                
		КонецЕсли;
...
Процедура ТабличноеПолеОбновитьТекстыПодваловЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Принудительно = Истина) Экспорт 
	...
	Если ОбщийТип <> "Список" И КоличествоВыделенных > 1 Тогда
		ВыделенныеСтроки = ирКлиент.ВыделенныеСтрокиТабличногоПоляЛкс(ТабличноеПоле, Ложь);
...
Процедура ПолеВвода_ОкончаниеВводаТекстаЛкс(Элемент, Текст, Значение, СтандартнаяОбработка, РасширенноеЗначение = Null, ЛиТипСтрокаСлужебный = Ложь) Экспорт
			...
			Если Не ЗначениеЗаполнено(ЗначениеСсылки) Тогда
				...
				Если Фрагменты.Количество() > 1 Тогда
					...
					Если ОписаниеТипов <> Неопределено Тогда
						...
						Менеджер = ирОбщий.ПолучитьМенеджерЛкс(Значение); 
						СтандартнаяОбработка = Ложь;
					КонецЕсли; 
				КонецЕсли; 
			КонецЕсли; 
	...
	Если Менеджер <> Неопределено Тогда
		Значение = ирОбщий.ПреобразоватьПредставлениеВСсылкуЛкс(Менеджер, Текст);
		Если Значение <> Неопределено Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли; 
...
Функция ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина, ЗаголовокФормы = "", РедактироватьМодально = Истина,
	...
	ИначеЕсли ТипРасширенногоЗначения = Тип("СхемаКомпоновкиДанных") Тогда
		СтандартнаяОбработка = Ложь;
		Редактор = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирКонсольКомпоновокДанных");
		#Если Сервер И Не Сервер Тогда
		...
		КонецЕсли;
		Результат = Редактор.РедактироватьСтруктуруСхемы(, ДанныеРедактора, Истина);
		Если Результат <> Неопределено Тогда
			...
			Тогда
				...
				Тогда
					...
					Иначе
						...
						КонецЕсли;
					КонецЕсли; 
					НачатьОтслеживаниеЧужойФормыЛкс(ФормаСсылки);
				КонецЕсли; 
				СтандартнаяОбработка = Ложь;
			КонецЕсли; 
...
Процедура НачатьОтслеживаниеЧужойФормыЛкс(Форма, РазрешитьСвоюФорму = Ложь) Экспорт 
	Если Не РазрешитьСвоюФорму И ирКэш.ОткрытыеФормыПодсистемыЛкс().Найти(Форма) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	ОткрытыеФормыЧужие = ирКэш.ОткрытыеФормыЧужиеЛкс();
	#Если Сервер И Не Сервер Тогда
		ОткрытыеФормыЧужие = Новый Соответствие;
	#КонецЕсли
	ПараметрыСлежения = ОткрытыеФормыЧужие[Форма];
	Если ПараметрыСлежения = Неопределено Тогда
		...
		ПараметрыСлежения.Вставить("Ссылка", КлючОсновногоОбъектаФормыЛкс(Форма));
	Иначе
		НоваяСсылка = КлючОсновногоОбъектаФормыЛкс(Форма);
		...
		КонецЕсли;
	КонецЕсли;
	ОткрытыеФормыЧужие[Форма] = ПараметрыСлежения;
	Если ОткрытыеФормыЧужие.Количество() = 1 Тогда
...
Процедура ПодключитьПроверкуЧужихФормЛкс() Экспорт 
	...
	#КонецЕсли
	ПодключитьГлобальныйОбработчикОжиданияЛкс("ПроверитьФормыСсылокЛкс", МинДлительностьАктивностиФормыСсылкиЛкс(), Истина);
...
Функция ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, ТабличноеПоле, СтандартнаяОбработка = Ложь, РасширенноеЗначение = Null,
	...
	Если Результат = Неопределено Тогда
		Если Не ЗначениеЗаполнено(СтрокаПоиска) Тогда
			СтрокаПоиска = ирКлиент.ТабличноеПолеСтрокаПоискаЛкс(ЭтаФорма, ТабличноеПоле);
		КонецЕсли;
...
Функция ОткрытьСписокЗначенийЛкс(Список, СтруктураОтбора = Неопределено, Заголовок = "") Экспорт 
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
...
Процедура ТабличноеПолеПриВыводеСтрокиЛкс(Знач ЭтаФорма, Знач Элемент, Знач ОформлениеСтроки, Знач ДанныеСтроки, Знач КнопкаРежимаОтображения = Неопределено, Знач КолонкиСПиктограммамиТипов = "",
	...
	Знач РасширенноеПредставлениеДат = Ложь) Экспорт 
	...
	Если ДопСвойства.ЗапретРазметкиВхождений <> Истина Тогда
		...
		Если МенеджерПоиска <> Неопределено Тогда
			...
			Если МенеджерПоиска.НайденныеСтроки.Найти(ДанныеСтроки) <> Неопределено Тогда
				ОформлениеСтроки.ЦветФона = ирОбщий.ЦветФонаЯчеекТекущегоЗначенияЛкс();
			КонецЕсли; 
		КонецЕсли;
		СтрокаПоиска = ирКлиент.ТабличноеПолеСтрокаПоискаЛкс(ЭтаФорма, Элемент, ПрефиксСтрокиПоиска);
...
Процедура ИнтерактивноЗаписатьВКолонкуФлажкаЛкс(Знач Элемент, Знач Колонка, Знач НовоеЗначение = Неопределено, ВосстанавитьТекущуюКолонку = Истина) Экспорт 
	...
	КонецЕсли; 
	ирКлиент.ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(Элемент, Колонка, НовоеЗначение,, ВосстанавитьТекущуюКолонку,,, Истина); 
...
Процедура _РасширенныйВыборПравогоЗначенияОтбораКомпоновкиЛкс(ТабличноеПолеОтбора) Экспорт 
	...
	Если ТипЗнч(ЗначениеОтбора) <> Тип("СписокЗначений") Тогда
		Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеОтбора, Ложь) Тогда
			...
			ирКлиент.ОткрытьФормуСпискаЛкс(ирОбщий.ПолучитьПолноеИмяМДТипаЛкс(ТипЗнч(ЗначениеОтбора)),,, КолонкаТП.ЭлементУправления, Истина,, ЗначениеОтбора);
		КонецЕсли;
	Иначе
		Результат = ирКлиент.ОткрытьЗначениеЛкс(ЗначениеОтбора, Истина);
		Если Результат Тогда
...
Функция ОткрытьОписаниеТипаПоИмениТипаЛкс(Знач ИмяОбщегоТипа, Знач ЭтаФорма = Неопределено) Экспорт 
	...
	Если СтрокаОбщегоТипа = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Результат = ирКлиент.ОткрытьСтраницуСинтаксПомощникаЛкс(СтрокаОбщегоТипа.ПутьКОписанию, , ЭтаФорма);
	Возврат Результат;
...
Процедура УстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗЛкс(ЭтаФорма, ТабличноеПоле, ЗначениеОбработки, Знач ТипИсточника = Неопределено, Знач Колонка = Неопределено, Знач ТолькоВыделенныеСтроки = Истина,
	...
	Тогда
		...
		Возврат;
	КонецЕсли; 
	ДопСвойства = ирКлиент.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ТабличноеПоле);
	ЗначениеТабличногоПоля = ирОбщий.ДанныеЭлементаФормыЛкс(ТабличноеПоле);
...
Функция ЗагрузитьТаблицуВТабличноеПолеЛкс(Знач Форма, Знач ТабличноеПоле, Знач ЗагружаемаяТаблица, Знач КоллекцияСтрокПоля = Неопределено, Знач РазрешитьИнтерактивнуюУстановку = Истина,
	...
	Для Каждого СтрокаТаблицы Из ЗагружаемаяТаблица Цикл
		...
		КонецЦикла;
		Если ЕстьОбработчикИзменения Тогда
			ирКлиент.ТабличноеПоле_УстановитьТекущуюКолонкуЛкс(ТабличноеПоле, ИнтерактивныеКолонки[0].Значение);
		КонецЕсли;
	КонецЦикла; 
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс(Индикатор);
	Если КоличествоПропущенных > 0 Тогда
	...
	Для Каждого КолонкаТП Из КолонкиТП Цикл
		...
		Если Истина
			...
			И (Ложь 
				...
				Или ирКлиент.ТабличноеПоле_УстановитьТекущуюКолонкуЛкс(ТабличноеПоле, КолонкаТП) // Только так можно проверить видимость по функциональным опциям у таблицы формы
				)
		Тогда
...
Процедура ОбновитьПодменюПоследнихОтборовЛкс(ЭтаФорма, Знач КоманднаяПанель, Знач ТабличноеПоле) Экспорт 
	...
	ирКлиент.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, КоманднаяПанель.Кнопки.ПоследниеОтборы, ТабличноеПоле, Новый Действие("КлсКомандаНажатие"), "Отборы");
...
Процедура ОткрытьСсылкуЯчейкиВРедактореОбъектаБДЛкс(ТабличноеПоле, ИмяКолонки = "") Экспорт
	...
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ИмяКолонки) Тогда
		ТекущаяКолонка = ирКлиент.ТабличноеПоле_ТекущаяКолонкаЛкс(ТабличноеПоле);
...
Функция ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(Знач ТабличноеПоле, Знач ОтборПоЗначению = Ложь, Знач Отбор = Неопределено, Знач СообщитьОДобавлении = Ложь,
	...
	Если Не ЗначениеЗаполнено(ДанныеКолонки) Тогда
		Возврат Ложь;
	КонецЕсли;
	КомпоновкаТП = ирКлиент.КомпоновкаТабличногоПоляЛкс(ЭтаФорма, ТабличноеПоле);
	Если Отбор = Неопределено Тогда
	...
	КонецЕсли; 
	Если КомпоновкаТП <> Неопределено Тогда
		ирКлиент.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ТабличноеПоле,,, Истина);
	КонецЕсли;
	Возврат Истина;
КонецФункции
...
Процедура ТабличноеПолеСОтборомПросмотраОтобратьЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ИмяКолонкиНомерСтроки = "", Знач РежимОтладки = Ложь, Знач НовоеИспользованиеОтбора = Неопределено) Экспорт 
	...
	Если ИспользоватьОтбор Тогда
		ирОбщий.НайтиДобавитьКолонкуНомераСтрокиЛкс(ТаблицаЗначений, ИмяКолонкиНомерСтроки, ТаблицаБезДанныхСУзкимиТипами);
		КлючТекущейСтроки = ирКлиент.ТабличноеПолеКлючСтрокиЛкс(ТабличноеПоле, ИмяКолонкиНомерСтроки);
		...
		Если КлючТекущейСтроки <> Неопределено Тогда 
			...
			Для ИндексПроверяемойСтроки = ТаблицаЗначений.Индекс(ТекущаяСтрока) По Мин(ИндексТекущейСтроки + 1000, ТаблицаЗначений.Количество() - 1)  Цикл
				...
				КлючТекущейСтроки = ирКлиент.ТабличноеПолеКлючСтрокиЛкс(ТабличноеПоле, ИмяКолонкиНомерСтроки, ПроверяемаяСтрока);
	...
	Иначе
		...
		ирКлиент.ТабличноеПолеВосстановитьСостояниеСтрокЛкс(ТабличноеПоле, СостояниеСтрок);
	КонецЕсли;
	ТабличноеПолеОбновитьТекстыПодваловЛкс(ЭтаФорма, ТабличноеПоле);
...
Функция ТабличноеПолеСтрокаПоискаЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ПрефиксСтрокиПоиска = "", выхПутьКДанным = "") Экспорт 
...
Процедура ОбъектМетаданныхОкончаниеВводаТекстаЛкс(Знач Элемент, Знач ПараметрыВыбораОбъектаМетаданных, Текст, Значение, СтандартнаяОбработка) Экспорт 
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Иначе
			ФормаВыбора = ирКлиент.ПолучитьФормуВыбораОбъектаМетаданныхСтруктуройЛкс(Элемент,, ирОбщий.ДанныеЭлементаФормыЛкс(Элемент), ПараметрыВыбораОбъектаМетаданных, Текст);
			РезультатВыбора = ФормаВыбора.ОткрытьМодально();
			Если РезультатВыбора <> Неопределено Тогда 
...
Функция ПолучитьФормуВыбораТаблицыСтруктурыБДЛкс(ЛиИменаБД, ИмяТаблицыХранения = "") Экспорт 
	...
	Форма = ирКлиент.ПолучитьФормуЛкс("Обработка.ирСтруктураХраненияБД.Форма",,, Истина);
	Форма.РежимВыбора = Истина;
...
Функция ПолучитьФормуСпискаЛкс(Знач ИмяТаблицыИлиМДИлиТип, Знач ФиксОтбор = Неопределено, Знач ИспользоватьДинамическийСписокИР = Неопределено, Знач ВладелецФормы = Неопределено, Знач РежимВыбора = Ложь,
	...
	Тогда
		...
		ФормаСписка = ирКлиент.ПолучитьФормуЛкс("Обработка.ирПодборИОбработкаОбъектов.Форма",,, ИмяТаблицы);
		Возврат ФормаСписка;
	КонецЕсли;
...
Процедура ПолеВводаРоли_НачалоВыбораЛкс(Элемент, СтандартнаяОбработка) Экспорт 
	...
	КонецЕсли; 
	//РезультатВыбора = ирКлиент.ВыбратьЭлементСпискаЗначенийЛкс(СписокВыбора, НачальныйВыбор, Истина, Заголовок, МножественныйВыбор);
	ФормаВыбораОбъектаБД = ирКлиент.ФормаВыбораОбъектаМетаданныхЛкс(,, НачальныйВыбор, МножественныйВыбор,,,,,,,,,,,,,,,,,, Истина);
	РезультатВыбора = ФормаВыбораОбъектаБД.ОткрытьМодально();
...
Процедура ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ТабличноеПоле, СтрокаПоиска, ИменаКолонокДанныхДляПоиска, АктивироватьНайденнуюСтроку = Истина,
	Знач ТабличноеПолеНайденных = Неопределено, Знач Кнопки = Неопределено, Знач ИскатьСНачала = Ложь) Экспорт 
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда 
		...
		Если ИскатьСНачала Тогда
			НачальнаяСтрока = ТабличноеПоле.Значение;
			#Если Сервер И Не Сервер Тогда
				НачальнаяСтрока = Новый ДеревоЗначений;
			#КонецЕсли
			Если НачальнаяСтрока.Строки.Количество() = 1 Тогда
				НачальнаяСтрока = НачальнаяСтрока.Строки[0];
				Если НачальнаяСтрока.Строки.Количество() > 0 Тогда
					НачальнаяСтрока = НачальнаяСтрока.Строки[0];
				КонецЕсли;
...
Функция ЗагрузитьСвязанныеСтрокиТаблицыБДЛкс(ЭтаФорма, ТабличноеПолеСвязанныхКолонок, ТабличноеПолеСвязанныхСтрок, КоманднаяПанельСвязанныхСтрок, мВыборкаРезультатаСтрокиТаблицы,
	...
	Исключение
		...
		Возврат Неопределено;
	КонецПопытки; 
	ирКлиент.ЗагрузитьВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, РезультатСтрокиТаблицы, мВыборкаРезультатаСтрокиТаблицы, КоманднаяПанельСвязанныхСтрок);
...
Процедура ДобавитьДействияРасшифровкиПоПолюЛкс(ЛиПрямоеПоле = Истина, Знач СписокДействийОткрыть, Знач ЗначениеПоля, Знач СписокДействийОтборать, Знач СписокДействийОформить,
	...
	Тогда
		...
		, ирКлиент.КартинкаТипаЛкс(ТипЗнч(ЗначениеПоля.Значение)));
	Иначе
...
Процедура ИнициироватьФормуЛкс(ЭтаФорма, ПолноеИмяФормы, РазрешитьОткрытиеДополнительныхФорм = Истина) Экспорт
	...
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		...
		ирКлиент.ОповеститьФормыПодсистемыЛкс("ирПолучитьБазовуюФорму", Контейнер); // Скорость https://www.hostedredmine.com/issues/891484
...
Функция НоваяФормаРезультатаФоновогоЗаданияЛкс() Экспорт 
	...
	Если ЛОжь И ирКэш.НомерВерсииПлатформыЛкс() >= 803013 Тогда
		...
		Если ОписаниеЗадания.БлокируемыеЭлементыФормы <> Неопределено Тогда
			Для Каждого КлючИЗначение Из ОписаниеЗадания.БлокируемыеЭлементыФормы Цикл
				...
				Если ТипЗнч(БлокируемыйЭлемент) = Тип("ПолеHTMLДокумента") Тогда
					ПолеТекста = ирКлиент.ОболочкаПоляТекстаЛкс(БлокируемыйЭлемент);
					#Если Сервер И Не Сервер Тогда
						ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
					#КонецЕсли
...
Функция ФормаРезультатовЗаданийФормыЛкс(Знач ЭтаФорма) Экспорт 
	...
	Если Не СлужебныеДанные.Свойство("ФормаРезультатовЗаданий", Результат) Тогда
		Результат = ирКлиент.НоваяФормаРезультатаФоновогоЗаданияЛкс();
	...
	Если ОписаниеЗадания.БлокируемыеЭлементыФормы <> Неопределено Тогда
		Для Каждого КлючИЗначение Из ОписаниеЗадания.БлокируемыеЭлементыФормы Цикл
			...
			Если ТипЗнч(КлючИЗначение.Ключ) = Тип("ПолеHTMLДокумента") Тогда
				ПолеТекста = ирКлиент.ОболочкаПоляТекстаЛкс(КлючИЗначение.Ключ);
				#Если Сервер И Не Сервер Тогда
					ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
				#КонецЕсли
...
Процедура НастроитьАвтоТабличноеПолеДинамическогоСпискаЛкс(ОсновнойЭУ, РежимИмяСиноним = Ложь, РазрешитьСортировку = Истина, ПредельноеКоличествоВидимыхКолонок = 30) Экспорт
	...
	Для Каждого ЭлементОтбора Из ОсновнойЭУ.Значение.Отбор Цикл
		...
		Если Ложь 
			...
			Или ирОбщий.ЛиОписаниеТиповНеограниченнойСтрокиЛкс(ЭлементОтбора.ТипЗначения)
			Или КоличествоВидимыхКолонок > ПредельноеКоличествоВидимыхКолонок 
		Тогда
			КолонкаТП.Видимость = Ложь;
		Иначе
...
Функция ОткрытьНовоеОкноФормыЛкс(ЭтотОбъект) Экспорт 
	...
	Тогда
		...
		Если СлужебныеДанныеФормы.Свойство("МенеджерСохраненияНастроек") Тогда
			ирКлиент.СохранитьНастройкуФормыЛкс(ЭтотОбъект);
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			НовыйОбъект = ирОбщий.СоздатьОбъектПоПолномуИмениМетаданныхЛкс(ПолноеИмяОбъекта);
		КонецЕсли; 
...
Функция ПолучитьФормуЛкс(ПолноеИмяФормы, Параметры = Неопределено, Владелец = Неопределено, Уникальность = Неопределено, Окно = Неопределено, ТолькоВнешниеФормы = Ложь) Экспорт
	...
	КонецЕсли;
	Если Результат <> Неопределено Тогда
		НачатьОтслеживаниеЧужойФормыЛкс(Результат);
	КонецЕсли;
	Возврат Результат;
...
Функция БуферОбмена_ВставитьЛкс(Знач ЭтаФорма = Неопределено, Знач ЛиОсновнаяВставка = Истина) Экспорт
	...
	Если ЭтаФорма = Неопределено Тогда
		ЭтаФорма = ирКлиент.АктивнаяУправляемаяФормаЛкс();
	КонецЕсли; 
	Если ЭтаФорма = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
...
Процедура ВставитьЗначениеВФормуЛкс(Знач ЭтаФорма, Знач Значение, Знач ЛиОсновнаяВставка = Истина) Экспорт 
	...
	Тогда
		...
		// Если табличное поле находится в режиме редактирования строки, то делам ничего
		Если Не ДобавитьСтроку Или ТекущийЭлементФормы.ТекущаяСтрока <> Неопределено Тогда
			КолонкаТП = ирКлиент.ТабличноеПоле_ТекущаяКолонкаЛкс(ТекущийЭлементФормы, ЭлементУправления, ДоступноРедактирование);
			Если Не ЛиОсновнаяВставка И ТипЗнч(ЭлементУправления) = Тип("ПолеВвода") Тогда
		...
		Тогда
			...
			Если ДобавитьСтроку Тогда
				...
				Если КолонкаТП = Неопределено Тогда
					КолонкаТП = ирКлиент.ТабличноеПоле_ТекущаяКолонкаЛкс(ТекущийЭлементФормы, ЭлементУправления, ДоступноРедактирование); // Текущая колонка могла измениться в ПриНачалеРедактированияСтроки
				Иначе
					ирКлиент.ТабличноеПоле_УстановитьТекущуюКолонкуЛкс(ТекущийЭлементФормы, КолонкаТП);
				КонецЕсли; 
			КонецЕсли; 
			Если ЭлементУправления <> Неопределено Тогда
				...
				Если ТекущийЭлементФормы.ТекущиеДанные <> Неопределено Тогда
					...
					Если ПродолжитьОбработку И ЛиОписаниеТиповСодержитТипЛкс(ОграничениеТипа, ТипЗначения) Тогда
						...
						ПродолжитьОбработку = Не ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ТекущийЭлементФормы, КолонкаТП, Значение,,,, ТипЗнч(ТекущиеДанные) = Тип("ЭлементОтбораКомпоновкиДанных"));
					КонецЕсли; 
...
Функция Форма_ЗначениеТекущегоПоляЛкс(ЭтаФорма, ДляКопирования = Ложь) Экспорт 
	...
	Значение = ирКлиент.ЗначенияВыделенныхЯчеекТаблицыЛкс(ЭтаФорма,, ДляКопирования, Ложь);
			...
			Если ТекущийЭлементФормы.ТекущаяСтрока <> Неопределено Тогда 
				...
				ИначеЕсли ТипЗнч(Значение) <> Тип("Массив") Тогда 
					Если Истина
						...
						И ЭтоУпрДинСписокИРЛкс(ЭтаФорма)
						И ЗначениеЗаполнено(ИмяКолонкиДанных) 
					Тогда
...
Функция ОткрытьРазличныеЗначенияКолонкиЛкс(Знач ТабличноеПоле, Знач НастройкиСписка = Неопределено, Знач _АдресСхемыКомпоновки = Неопределено, ЭтаФорма = Неопределено) Экспорт 
	...
	#КонецЕсли
	Форма = ирКлиент.ПолучитьФормуЛкс("Обработка.ирРазличныеЗначенияКолонки.Форма",, ТабличноеПоле);
...
Процедура Форма_ПриОткрытииЛкс(ЭтаФорма) Экспорт
	...
	Если Не ЭтаФорма.МодальныйРежим И МодальныеГруппы.Количество() > 0 Тогда
		...
		#КонецЕсли
		ирКлиент.ПодключитьГлобальныйОбработчикОжиданияЛкс("АктивироватьАктивнуюФормуЛкс");
	КонецЕсли;
	Если ирКэш.ЛиСеансТолстогоКлиентаУПЛкс() Тогда
		ПодключитьПроверкуЧужихФормЛкс();
	КонецЕсли;
...
Процедура ОповеститьИсследователиОбъектовЛкс(Знач ЭтаФорма, Знач Значение, Знач Выражение = "") Экспорт 
	...
	КонецЕсли;
	ирКлиент.ОповеститьФормыПодсистемыЛкс("Исследовать", Структура, ЭтаФорма);
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(ИсточникДействий) = Тип("ТабличноеПоле") Тогда
			...
			ИначеЕсли Кнопка.Имя = "ГруппировкаТаблицы" Тогда 
				КомпоновкаТП = ирКлиент.КомпоновкаТабличногоПоляЛкс(ЭтаФорма, ИсточникДействий);
				Если КомпоновкаТП <> Неопределено Тогда 
					НастройкаКомпоновки = КомпоновкаТП.Компоновщик.Настройки;
				КонецЕсли;
				ирКлиент.ОткрытьГруппировкуТабличногоПоляЛкс(ИсточникДействий, НастройкаКомпоновки);
			ИначеЕсли Кнопка.Имя = "АнализДанных" Тогда 
				ФормаАнализа = ирКлиент.ПолучитьФормуЛкс("Обработка.ирАнализДанных.Форма",, ЭтаФорма);
...
Процедура ОповеститьОЗаписиОбъектаЛкс(Знач ТипИлиМассив, Знач Источник = Неопределено, Знач ВсеТипыДляПодсистемы = Ложь) Экспорт
	...
	Если ТипИлиМассив.Количество() > 1 Или ВсеТипыДляПодсистемы Тогда
		ирКлиент.ОповеститьФормыПодсистемыЛкс("ЗаписанОбъект",, Источник);
	...
	Для Каждого ТипИлиОбъект Из ТипИлиМассив Цикл
		Если Не ВсеТипыДляПодсистемыОповещены Тогда
			ирКлиент.ОповеститьФормыПодсистемыЛкс("ЗаписанОбъект", ТипИлиОбъект, Источник);
		КонецЕсли; 
...
Процедура ОповеститьОбИзмененииОбъектаВПамятиЛкс(Объект, Источник = Неопределено) Экспорт
	...
	ирКлиент.ОповеститьФормыПодсистемыЛкс("ИзмененОбъектВПамяти", Объект, Источник);
...
Функция ОткрытьПользователяИБЛкс(Пользователь) Экспорт 
	...
	ФормаПользователя = ирКлиент.ПолучитьФормуЛкс("Обработка.ирРедакторПользователей.Форма.ПользовательИнфобазы",,, Пользователь);
...
Функция КлючиСтрокБДИзТаблицыФормыЛкс(Форма = Неопределено, выхКлючТекущейСтроки = Неопределено, выхТаблицаФормыДинамическогоСписка = Неопределено, Знач НуженВидимыйПорядок = Истина) Экспорт 
	...
	Тогда
		...
		Если ирОбщий.ЛиКлючСсылкиИлиРегистраЛкс(ТаблицаФормы.ТекущаяСтрока) Тогда
		...
		Если ирОбщий.ЛиКлючСсылкиИлиРегистраЛкс(Ссылка) Тогда
...
Функция КлючОсновногоОбъектаФормыЛкс(Форма = Неопределено) Экспорт 
	...
	Если ТипЗнч(Форма) = Тип("УправляемаяФорма") Тогда
		...
		Если Не ирОбщий.ЛиКлючСсылкиИлиРегистраЛкс(Ссылка) Тогда
			Попытка
...
Функция ЗначенияВыделенныхЯчеекТаблицыЛкс(Форма = Неопределено, выхКлючТекущейСтроки = Неопределено, НуженВидимыйПорядок = Истина, Знач ТолькоСсылки = Истина) Экспорт 
	...
	Если ТипЗнч(Форма.ТекущийЭлемент) = Тип("ТаблицаФормы") Тогда
		...
		Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл 
			...
			КонецЕсли; 
			Если Не ТолькоСсылки Или ирОбщий.ЛиКлючСсылкиИлиРегистраЛкс(ЗначениеПоля) Тогда
		...
		ИначеЕсли ПолеФормы.Вид = ВидПоляФормы.ПолеТабличногоДокумента Тогда
			...
			Если ТаблицаЗначений.Колонки.Количество() > 0 Тогда
				...
				Если Истина
					...
					И (Ложь
						Или Не ТолькоСсылки 
						Или ирОбщий.ЛиКлючСсылкиИлиРегистраЛкс(КлючТекущейСтроки[0])) 
				Тогда
...
Процедура ЗапомнитьСодержимоеЭлементаАктивнойФормыДляСравненияЛкс(Форма = Неопределено) Экспорт 
	...
	Тогда
		Возврат;
	КонецЕсли; 
	ирКлиент.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(Форма, ТекущийЭлемент);
...
Процедура ОткрытьТаблицуЗначенийИзАктивнойУправляемойФормыЛкс(Форма = Неопределено) Экспорт 
	...
	Иначе // ДинамическийСписок
		ирКлиент.ВывестиСтрокиТабличногоПоляИПоказатьЛкс(Форма, ТекущийЭлемент);
	КонецЕсли; 
...
Функция ОткрытьТабличныйДокументИзАктивнойФормыЛкс(Форма = Неопределено) Экспорт 
	...
	Если Результат <> Неопределено Тогда
		Если ТипЗнч(ТекущийЭлемент) = Тип("ПолеТабличногоДокумента") Тогда 
			...
			ирКлиент.ПолеТабличногоДокументаВосстановитьОформлениеТекущихСтрокЛкс(Форма, ТекущийЭлемент);
		Иначе
...
Процедура ОткрытьРазличныеЗначенияКолонкиАктивнойУправляемойФормыЛкс(Форма = Неопределено) Экспорт 
	...
	Тогда
		Возврат;
	КонецЕсли; 
	ирКлиент.ОткрытьРазличныеЗначенияКолонкиЛкс(ТекущийЭлемент,,, Форма);
...
Процедура РедактироватьАктивныйСписокЗначенийУправляемыйЛкс(Форма = Неопределено) Экспорт 
	...
	КонецЕсли;
	Если ПолеЗначения <> Неопределено Тогда
		СтруктураОтбора = ирКлиент.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(ПолеЗначения);
...
Функция ИсполняемыеСхемаИНастройкиСпискаУправляемойФормаЛкс(Знач Форма, Знач ИмяТаблицыФормы = "") Экспорт 
	...
	Если ЭтоУпрДинСписокИРЛкс(Форма) Тогда
		СхемаИНастройки = Форма.ИсполняемыеСхемаИНастройки();
		Схема = СхемаИНастройки.Схема;
		#Если Сервер И Не Сервер Тогда
			Схема = Новый СхемаКомпоновкиДанных;
		#КонецЕсли
		Схема.НаборыДанных[0].Запрос = СтрЗаменить(Схема.НаборыДанных[0].Запрос, ирОбщий.ОкончаниеВыраженияПодстрокиДинСпискаЛкс() + " КАК", ирОбщий.ОкончаниеВыраженияПодстрокиДинСпискаЛкс(1000000000) + " КАК");
	Иначе
		#Если Сервер И Не Сервер Тогда
...
Функция ЭтоУпрДинСписокИРЛкс(Знач Форма) Экспорт 
...
Функция ДобавитьОтборКомпоновкиВИсториюТаблицыБДЛкс(Знач ЭтаФорма, Знач ПолноеИмяТаблицы, Знач АктивнаяНастройка, Знач СтараяНастройка, выхОтборИзменен = Ложь) Экспорт 
	...
	Если выхОтборИзменен Тогда 
		Если ПредставлениеОтбора <> "" Тогда 
			...
			ДобавленВСписок = ирКлиент.ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, НастройкаКомпоновки, ирОбщий.ПредставлениеЗначенияСОграничениемДлиныЛкс(НастройкаКомпоновки.Отбор, 150), ПолноеИмяТаблицы, "Отборы", Истина);
		КонецЕсли;
...
Процедура ОткрытьПанельИнструментовЛкс(ТолькоОткрытьФормуСпискаИнструментов = Ложь, ОткрытьСтраницуНастроек = Ложь) Экспорт
	...
	#Если ТолстыйКлиентОбычноеПриложение Или ТолстыйКлиентУправляемоеПриложение Тогда
		...
		Иначе
			//ирКлиент.ОткрытьПанельИнструментовЛкс(Истина); // Так будут считаны настройки
			ФормаНастроек = ирКлиент.ОткрытьФормуЛкс("Обработка.ирПортативный.Форма.ФормаНастроек"); 
