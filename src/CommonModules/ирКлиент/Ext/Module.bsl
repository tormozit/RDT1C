...
Функция ДобавитьКнопкиКомандБСПЛкс(Знач КоманднаяПанельДляБСП, Знач ИмяТаблицы, Знач ЭтоСсылочныйОбъект) Экспорт 
	...
	Если Истина
		...
		И (Ложь
			Или ирКэш.НомерВерсииБСПЛкс() < 204 
	...
	Если ПодменюБСП <> Неопределено Тогда
		...
		Если ЭтоСсылочныйОбъект Тогда
			КомандыБСП = ирСервер.КомандыБСППоИмениТаблицыЛкс(ИмяТаблицы);
...
Процедура ВыполнитьКомандуБСПЛкс(Знач Кнопка, Знач ФормаУпрДляПодключаемыхКоманд, Знач Ссылки) Экспорт
	Если Ссылки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	КомандыБСП = ирСервер.КомандыБСППоИмениТаблицыЛкс(Ссылки[0].Метаданные().ПолноеИмя());
	#Если Сервер И Не Сервер Тогда
