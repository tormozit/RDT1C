...
Функция ОткрытьМетодМодуляПоОпределениюЛкс(Знач СтрокаМетода, Знач РазрешитьДиалоги = Истина) Экспорт
	мПлатформа = ирКэш.Получить();
	...
	Иначе
		...
		Если ПолеТекстаНовое <> Неопределено Тогда
			ПолеТекстаНовое.ПерейтиКОпределениюЛокальногоМетода(СтрокаМетода);
		КонецЕсли;
	КонецЕсли; 
	Возврат Истина;
КонецФункции
...
Функция ДанныеСтрокДинамическогоСпискаЛкс(Знач ТабличноеПоле, Знач КлючиСтрок = Неопределено, Знач ВыбранныеКолонки = Неопределено, НастройкиСписка = Неопределено,
	...
	Иначе
		...
		Иначе
			...
			Если Истина
				И КлючиСтрок <> Неопределено 
				И ирКэш.ДоступнаСхемаЗапросаЛкс(Истина)
...
Функция ПолучитьФормуЛкс(ПолноеИмяФормы, Параметры = Неопределено, Владелец = Неопределено, Уникальность = Неопределено, Окно = Неопределено, ТолькоВнешниеФормы = Ложь) Экспорт
	...
	Если Фрагменты.Количество() > 1 Тогда
		...
		Менеджер = ирОбщий.МенеджерВнешнегоОбъектаЛкс(Фрагменты[0], Фрагменты[1]);
