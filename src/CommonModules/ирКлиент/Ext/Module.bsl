...
// Параметры:
//   Браузер - ПолеHTMLДокумента - см. Обработка.ирПлатформа.Макет.ФоновыйHTTPЗапрос
//   Адрес - Строка - 
...
Функция ОткрытьФормуЧатаИИЛкс(Знач ПараметрыЗапроса = Неопределено, Знач РежимНапарника = "", Знач ТекстВопроса = "", Знач ПолеПрограммы = Неопределено, Знач ЛиМодально = Ложь) Экспорт
	Форма = ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ЧатИИ",,, Новый УникальныйИдентификатор);
	Форма.ПараметрыЗапроса = ПараметрыЗапроса;
	Форма.РежимНапарника = РежимНапарника;
	Форма.мПолеТекстаПрограммы = ПолеПрограммы;
	Форма.ТекстВопроса = ТекстВопроса;
	Если ЛиМодально Тогда
		Форма.ОткрытьМодально();
	Иначе
		Форма.Открыть();
