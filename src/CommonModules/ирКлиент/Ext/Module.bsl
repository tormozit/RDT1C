...
// Вызывается из ИР адаптера
Функция ОткрытьТаблицуИзМассиваИзСтрокиВнутрЛкс(Знач ТекстЯчеек, Знач ТекстТекущейЯчейки = "", Знач ЛиТаблица = Ложь, Знач ЛиМодально = Ложь) Экспорт 
	мПлатформа = ирКэш.Получить();
	Массив = ирОбщий.ОбъектИзСтрокиJSONЛкс(ТекстЯчеек); // Массив
	ФормаСписка = мПлатформа.ПолучитьФорму("СписокЗначений",, Новый УникальныйИдентификатор);
	...
	Если ЛиМодально Тогда      
		...
		Если Результат = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		Возврат Массив.Найти(Результат.ПредставлениеЗначения);
	Иначе
		ФормаСписка.Открыть();
	КонецЕсли;
