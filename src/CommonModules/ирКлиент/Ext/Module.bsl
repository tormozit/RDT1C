...
// Вызывается из ИР адаптера
Функция ОткрытьТаблицуИзМассиваИзСтрокиВнутрЛкс(Знач ТекстЯчеек, Знач ТекстТекущейЯчейки = "", Знач ЛиТаблица = Ложь, Знач ЛиМодально = Ложь, Знач ЧислоКолонок = Неопределено, Знач Заголовок = "") Экспорт 
	мПлатформа = ирКэш.Получить();
	...
	Иначе
		...
		//Если ЧислоКолонок > 1 Тогда
			...
			Для Каждого ТекстЯчейки Из Массив Цикл 
				...
				КонецЕсли;
			КонецЦикла;
			Результат = ОткрытьТаблицуЗначенийЛкс(Таблица, ТекущаяСтрока, ЛиМодально, Заголовок, ЛиМодально, ЛиМодально,,, ТекущаяКолонка);
