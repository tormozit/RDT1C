...
Функция ПрочитатьДопПоляСсылающихсяОбъектовЛкс(Знач ТабличноеПоле, Знач КомпоновщикДопПолей, Знач ТаблицаДанных = Неопределено, Знач ТолькоТекущаяСтрока = Ложь, 
	ЭтаФорма = Неопределено, Кнопка = Неопределено, ОбработчикЗавершения = Неопределено) Экспорт 
...
Функция ПрочитатьДопПоляОбъектовЛкс(Знач ТабличноеПоле, Знач КомпоновщикДопПолей, Знач ТаблицаДанных = Неопределено, Знач ИмяПоляСсылки = "Данные", Знач ЧитатьСтандартныеРеквизиты = Ложь, 
	...
	ирОбщий.ПрочитатьДопПоляОбъектовЛкс(ИмяПоляСсылки, ЧитатьСтандартныеРеквизиты, ТаблицаДанных, МассивДопПолей, ОписаниеТиповОбъекта, СтрокаВыбора,,, 
		ЭтаФорма, ?(ТаблицаДанных.Количество() > 1000, Кнопка, Неопределено), ОбработчикЗавершения);
	Возврат СтрокаПорядка;
КонецФункции
