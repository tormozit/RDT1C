	...
	Для каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		...
		Если ТипСтроки = Тип("ТекущиеДанныеСтруктурыНастроекКомпоновкиДанных") Тогда 
			Если ОтображатьВВидеИмен Тогда 
				...
				Тогда
					Представление = ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(ДанныеСтроки.Строка.ПоляГруппировки.Элементы, "Поле", Новый Структура("Использование", Истина)));
				КонецЕсли;
...
Процедура ОткрытьОбъектыИзВыделенныхЯчеекВПодбореИОбработкеОбъектовЛкс(ТабличноеПоле, Знач ИмяКолонки = "", Знач ЭтаФорма = Неопределено, Знач ПолноеИмяТаблицыБД = "", Знач СПараметрами = Ложь) Экспорт 
	...
	Иначе
		...
		Если СПараметрами Тогда
			...
			СписокВыбораКолонок.ЗагрузитьЗначения(ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(ТаблицаЗначений.Колонки));
			...
			Если СписокВыбораКолонок.Количество() > 0 Тогда
				...
				Если СписокВыбораКолонок <> Неопределено Тогда 
					ирОбщий.ДополнитьМассивЛкс(ВыбранныеКолонки, ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(СписокВыбораКолонок, "Значение"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура РазобратьПозициюМодуляВСтрокеТаблицыЛкс(Знач ТекущаяСтрока, Знач ИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = "",
	...
	//Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСтрокиМодуля) Тогда
		...
		ТекущаяСтрока.Ссылка = ирОбщий.СсылкаСтрокиМодуляЛкс(ФайлМодуля.ИмяБезРасширения, ПолеТекстаМодуля.мНачальнаяСтрока, ИмяМетода, СмещениеСтрокиМетода, ТекстСтроки);
	...
	Если ирКэш.НомерРежимаСовместимостиЛкс() >= 803010 Тогда
		...
		Для Каждого РасширениеКонфигурации Из РасширенияКонфигурацииМои.Получить() Цикл
			...
			Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigFiles """ + КаталогКэша + """ -Extension """ + РасширениеКонфигурации.Имя + """ -Module", СтрокаСоединенияИнформационнойБазы(),
				ТекстЛога, Истина, "Выгрузка модулей расширения " + РасширениеКонфигурации.Имя,,,,,,,,,, ОтображатьОкно);
			Если Не Успех Тогда 
				Сообщить(ТекстЛога);
				Возврат Ложь;
			КонецЕсли;
