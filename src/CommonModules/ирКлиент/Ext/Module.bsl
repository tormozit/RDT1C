...
Процедура ОткрытьТаблицуИзМассиваИзСтрокиВнутрЛкс(Знач ТекстЯчеек, Знач ТекстТекущейЯчейки = "") Экспорт 
	мПлатформа = ирКэш.Получить();
	Массив = ирОбщий.ОбъектИзСтрокиJSONЛкс(ТекстЯчеек); // Массив
	ФормаМассива = мПлатформа.ПолучитьФорму("Массив",, Новый УникальныйИдентификатор);
	ФормаМассива.НачальноеЗначениеВыбора = Массив;
	ИндексТекущегоЭлемента = Массив.Найти(ТекстТекущейЯчейки);
	Если ИндексТекущегоЭлемента <> Неопределено Тогда
		ФормаМассива.ПараметрИндексЭлемента = ИндексТекущегоЭлемента;
	КонецЕсли;
	ФормаМассива.Открыть();
	КоличествоКолонок = 1;
	Если Не ВвестиЧисло(КоличествоКолонок, "Введите количество колонок списка") Тогда
		Возврат;
	КонецЕсли;
	Если КоличествоКолонок > 1 Тогда
		ФормаМассива.Закрыть();
		Таблица = Новый ТаблицаЗначений;
		Для Счетчик = 1 По КоличествоКолонок Цикл
			Таблица.Колонки.Добавить("Колонка" + Счетчик);
		КонецЦикла;
		Счетчик = 0;
		ТекущаяСтрока = Неопределено;
		ТекущаяКолонка = Неопределено;
		Для Каждого ТекстЯчейки Из Массив Цикл 
			ИндексЯчейки = Счетчик % КоличествоКолонок;
			Если ИндексЯчейки = 0 Тогда
				СтрокаТаблицы = Таблица.Добавить();
			КонецЕсли;
			НомерЯчейки = ИндексЯчейки + 1;
			ИмяКолонки = "Колонка" + НомерЯчейки; 
			СтрокаТаблицы[ИмяКолонки] = ТекстЯчейки;
			Счетчик = Счетчик + 1;
			Если Истина
			...
			Тогда
				...
				ТекущаяКолонка = ИмяКолонки;
			КонецЕсли;
		КонецЦикла;
		ОткрытьТаблицуЗначенийЛкс(Таблица, ТекущаяСтрока, Ложь,,,,,, ТекущаяКолонка);
	КонецЕсли;
КонецПроцедуры
