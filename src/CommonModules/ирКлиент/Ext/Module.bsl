...
Функция ОткрытьКолонкуБДДоступногоПоляКомпоновкиЛкс(Знач ТабличноеПоле, Знач ИмяТаблицы = "") Экспорт 
	...
	Иначе
		...
		Для Каждого Тип Из ТипЗначения.Типы() Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если ПодходящиеТипы.Количество() = 1 Тогда
...
Процедура ОткрытьАнализПравДоступаПоСпискуОбъектовЛкс(Знач СписокПолныхИменМД) Экспорт 
	
	Если СписокПолныхИменМД.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	Форма = ПолучитьФормуЛкс("Отчет.ирАнализПравДоступа.Форма",,, Новый УникальныйИдентификатор);
