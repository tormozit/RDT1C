...
Процедура ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, СтрокаПоля, Знач ТипТаблицы = Неопределено, ИмяКолонки = "Заголовок", Знач ЭтоИндекс = Ложь, Использовано = Ложь, Знач Измерения = "") Экспорт 
	...
	Если Ложь
		...
		Или (Истина
			...
			И НуженСдвигПоляСсылкаВНачалоСпискаЛкс(ТипТаблицы))
	Тогда
...
Функция ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(Знач ТабличноеПоле, Знач МассивСтрок = Неопределено, Знач СУчетомОтбора = Ложь, Знач ПреобразоватьДеревоВТаблицу = Истина,
	...
	Знач ВернутьКопию = Ложь, ДанныеПоля = Неопределено) Экспорт 
	...
	КонецЕсли;
	ПолноеИмяТаблицыБД = "";
	ТипИсточника = ирОбщий.ОбщийТипДанныхТабличногоПоляЛкс(ТабличноеПоле,,, ПолноеИмяТаблицыБД, ДанныеПоля);
	Если ТипИсточника = "Список" И ЗначениеЗаполнено(ПолноеИмяТаблицыБД) Тогда
		Возврат Неопределено;
	КонецЕсли; 
	...
	Иначе // ТипЗнч(ИсточникДействий) = Тип("ТаблицаФормы") 
		...
		Если ДанныеПоля = Неопределено Тогда
			ДанныеПоля = ирОбщий.ДанныеЭлементаФормыЛкс(ТаблицаФормы);
		КонецЕсли;
...
Функция Форма_ЗначениеТекущегоПоляЛкс(Знач ЭтаФорма, Знач ДляКопирования = Ложь, выхПолеТекста = Неопределено) Экспорт 
...
Функция ОткрытьНастройкиПоляТекстаПрограммыНаЗаднемПланеЛкс(Знач Причина = "Для анализа содержимого модулей нужно обновить кэш модулей через открывшуюся форму настроек.", Знач ВзятьФокус = Ложь) Экспорт 
