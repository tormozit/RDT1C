...
Функция ОткрытьСписокЗначенийЛкс(Знач Список, Знач СтруктураОтбора = Неопределено, Знач Заголовок = "") Экспорт 
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
...
Функция ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ТабличноеПоле, СтрокаПоиска, Знач ИменаКолонокДанныхДляПоиска = "", Знач АктивироватьНайденнуюСтроку = Истина,
	...
	Если ДеревоЗначений.Строки.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если ЗначениеЗаполнено(СтрокаПоиска) И ДеревоЗначений.Строки.Количество() = 1 Тогда
		КорневаяСтрока = ДеревоЗначений.Строки[0];
