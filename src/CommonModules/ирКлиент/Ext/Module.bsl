...
Процедура ОформитьЯчейкуСРасширеннымЗначениемЛкс(ОформлениеЯчейки, Знач Значение = Неопределено, КолонкаТабличногоПоля = Неопределено, ВыводитьПиктограммуТипа = Истина) Экспорт 
	...
	//ОформлениеЯчейки.ЦветФона = ирОбщий.ПолучитьЦветСтиляЛкс("ирЦветФонаРасширенногоПредставленияЗначения");
...
Функция ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина, ЗаголовокФормы = "", РедактироватьМодально = Истина,
	...
	Тогда
		Если ирОбщий.ЛиТипСсылкиБДЛкс(ТипРасширенногоЗначения, Ложь) Тогда
			Если ЗначениеЗаполнено(РасширенноеЗначение) Тогда
				ОбъектСуществует = ирОбщий.ЛиСуществуетОбъектПоСсылкеЛкс(РасширенноеЗначение);
			...
			Если ирОбщий.ЛиТипСсылкиБДЛкс(ТипРасширенногоЗначения, Ложь) Тогда
				...
				Тогда
					...
					Иначе
						...
						КонецЕсли;
					КонецЕсли; 
					НачатьОтслеживаниеФормыСсылкиЛкс(ФормаСсылки);
				КонецЕсли; 
				СтандартнаяОбработка = Ложь;
			КонецЕсли; 
...
Процедура НачатьОтслеживаниеФормыСсылкиЛкс(Форма) Экспорт 
	ОткрытыеФормыСсылок = ирКэш.ОткрытыеФормыСсылокЛкс();
	#Если Сервер И Не Сервер Тогда
		ОткрытыеФормыСсылок = Новый Соответствие;
	#КонецЕсли
	ПараметрыСлежения = ОткрытыеФормыСсылок[Форма];
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	ОткрытыеФормыСсылок[Форма] = ПараметрыСлежения;
	Если ОткрытыеФормыСсылок.Количество() = 1 Тогда
		ПодключитьПроверкуФормСсылокЛкс();
	КонецЕсли;
КонецПроцедуры

Процедура ПодключитьПроверкуФормСсылокЛкс() Экспорт 
...
Функция ТабличноеПолеРегистраОтобразитьОстаткиЛкс(Знач ОформленияСтрок, Знач ОбъектМД, Знач Кнопка, Знач ИмяТаблицыБДРегистра = Неопределено) Экспорт 
	...
	Для Каждого ПолеТаблицы Из ирОбщий.ПоляТаблицыБДЛкс(ИмяТаблицыБДРегистра + ".Остатки") Цикл
		Если ирОбщий.СтрКончаетсяНаЛкс(ПолеТаблицы.Имя, "Остаток") Тогда // Грязно
			Продолжить;
		КонецЕсли;
	...
	Для Каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		...
		Если СтрокаРезультата.Количество() > 0 Тогда
			...
			Для Каждого Ресурс Из ОбъектМД.Ресурсы Цикл
				...
				ОформлениеСтроки.Ячейки[ИмяРесурса + "Остаток"].Значение = СтрокаРезультата[ИмяРесурса + "Остаток"];
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
КонецФункции
...
Процедура ТабличноеПолеПриАктивизацииСтрокиЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ОбновлятьПодвал = Истина) Экспорт 
	...
	Если Не ВсеСтрокиОбновлены Тогда
		...
		Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда
			НовыеВыделенныеСтроки.Удалить(НовыеВыделенныеСтроки.Найти(ТабличноеПоле.ТекущаяСтрока));
...
Функция ВыбранныйЭлементПоследнихЗначенийЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач Кнопка, Знач ТипКлюча = "Выбранные", Знач Десериализовать = Ложь, ДопКлючХранения = "") Экспорт 
	...
	Если ТипЗнч(ПоследниеВыбранные) <> Тип("СписокЗначений") Тогда
		Возврат Неопределено;
	КонецЕсли;
	ИндексЭлементаСписка = Число(Сред(Кнопка.Имя, СтрДлина(ирОбщий.НачалоИмениКнопкиПодменюПоследнихВыбранных(ТипКлюча)) + 1));
...
Функция _НайтиСтрокуТабличногоПоляДереваЗначенийСоСложнымФильтромЛкс(ТабличноеПолеДерева, ПолеВводаФильтра, Подстроки = "") Экспорт
	...
	Иначе
		ТекущаяСтрока = Неопределено;
		ПолеВводаФильтра.ЦветФонаПоля = ирОбщий.ПолучитьЦветСтиляЛкс("ирЦветФонаОшибки");
	КонецЕсли;
...
Процедура ОбработатьЗавершениеЗаданияФормыЛкс(Знач ОписаниеЗадания, Знач ЭтаФорма, Знач ФоновоеЗадание = Неопределено, Знач ПоЗапросуПользователя = Ложь, Знач ЗакрытьФормуЗадания = Ложь,
	...
	Если ФормаЗадания <> Неопределено Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	Выполнить("ЭтаФорма." + ОписаниеЗадания.ОбработчикЗавершения + "(ФоновоеЗадание.Состояние, Результат)");
...
Процедура НастроитьАвтоТабличноеПолеДинамическогоСпискаЛкс(ОсновнойЭУ, РежимИмяСиноним = Ложь, РазрешитьСортировку = Истина, ПредельноеКоличествоВидимыхКолонок = 30) Экспорт
	...
	Для Каждого ЭлементОтбора Из ОсновнойЭУ.Значение.Отбор Цикл
		...
		Если КолонкаТП.Ширина = -1 Тогда
			КолонкаТП.Ширина = ирОбщий.ШиринаТипаЗначенияЛкс(ЭлементОтбора.ТипЗначения);
		КонецЕсли;
		Если Истина
		...
		Тогда
			...
			Если ТекущийЭлементФормы.ТекущаяСтрока <> Неопределено Тогда 
				...
				ИначеЕсли ТипЗнч(Значение) <> Тип("Массив") Тогда 
					Если ТипЗнч(ТекущийЭлементФормы) = Тип("ТабличноеПоле") И ТекущийЭлементФормы.ТекущаяКолонка <> Неопределено Тогда
...
Процедура УниверсальнаяКомандаФормыЛкс(Знач ЭтаФорма, Знач Кнопка, Знач ИсточникДействий = Неопределено, Знач МетаданныеВыбора = Неопределено) Экспорт 
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(ИсточникДействий) = Тип("ТабличноеПоле") Тогда
			...
			ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(Кнопка.Имя, ирОбщий.НачалоИмениКнопкиПодменюПоследнихВыбранных("Отборы")) Тогда 
