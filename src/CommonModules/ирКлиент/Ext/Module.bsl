...
//
Процедура ПриПолученииДанныхТабличногоПоляКомпоновкиЛкс(ЭтаФорма, Элемент, ОформленияСтрок, ТабличноеПолеВыбранныхПолей = Неопределено) Экспорт
	...
	Для каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		...
		ИначеЕсли Истина
			...
			И ТипСтроки <> Тип("ДоступноеПолеКомпоновкиДанных") 
			И ТипСтроки <> Тип("ДоступноеПолеОтбораКомпоновкиДанных") 
		Тогда
			Попытка
		...
		Если ИндексКартинки <> Неопределено Тогда
			...
			Если Истина
				И ТипСтроки <> Тип("СтрокаТаблицыЗначений")
				И Не ЭтоПапка
				И Найти(ЯчейкаПоля.Текст, "_") > 0
			Тогда 
				Если СтрЗаменить(ЯчейкаПоля.Текст, "._", ".") = "" + ДоступноеПоле.Поле Тогда
					ЯчейкаПоля.УстановитьТекст(ДоступноеПоле.Поле);
				Иначе
					КраткоеИмя = ирОбщий.ПоследнийФрагментЛкс(ДоступноеПоле.Поле);
...
Процедура ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, СтрокаПоля, Знач СтрокаДоступнойТаблицы = Неопределено, ИмяКолонки = "Заголовок", Знач ЭтоИндекс = Ложь, Использовано = Ложь) Экспорт 
	...
	Если Ложь
		...
		Или (Истина
			...
			И СтрокаПоля.Имя = "Ссылка" 
			И (Ложь
				Или ирОбщий.ЛиКорневойТипСсылкиЛкс(СтрокаДоступнойТаблицы.Тип)
				Или ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(СтрокаДоступнойТаблицы.Тип)))
	Тогда
...
Процедура ПодобратьВариантыПоляКомпоновкиИзГруппы(Знач Варианты, Знач ГруппаДоступныхПолей, Знач СловаПоиска, Знач СловоРодителя = "", Знач ВычислительРегВыражений = Неопределено)
	...
	Для Каждого ДоступноеПоле Из ГруппаДоступныхПолей.Элементы Цикл
		Если ДоступноеПоле.Папка Тогда 
			...
			КонецЕсли;
		ИначеЕсли ирОбщий.ЛиСтрокаСодержитВсеПодстрокиЛкс(НРег(ДоступноеПоле.Заголовок), СловаПоиска) Тогда 
			...
			Иначе
				ПредставлениеТипа = "";
			КонецЕсли;
			Варианты.Добавить(ДоступноеПоле.Поле, ПредставлениеЗначения + ПредставлениеТипа,, КартинкаТипаЛкс(ТипЗначения));
		КонецЕсли;
	КонецЦикла;
...
Процедура НачатьОтслеживаниеФормыЛкс(Форма, РазрешитьСвоюФорму = Ложь) Экспорт 
	Если Не РазрешитьСвоюФорму И ирКэш.ОткрытыеФормыПодсистемыЛкс().Найти(Форма) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	ОткрытыеФормыЧужие = ирКэш.ОткрытыеФормыВсеЛкс();
	#Если Сервер И Не Сервер Тогда
...
Функция ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ТабличноеПоле, СтрокаПоиска, Знач ИменаКолонокДанныхДляПоиска = "", Знач АктивироватьНайденнуюСтроку = Истина,
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда 
		...
		Если ИскатьСНачала Тогда
			...
			#КонецЕсли
			Если Дочерние.Количество() = 1 Тогда
