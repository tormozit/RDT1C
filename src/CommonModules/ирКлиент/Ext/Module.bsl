...
Процедура ПерейтиКОбъектуМетаданныхВКонфигуратореЛкс(Знач ПолноеИмяОбъекта, Знач ПутьВОбъекте = "") Экспорт 
	...
	Если Найти(ПолноеИмяОбъекта, ".Форма.") = 0 Тогда
		...
		Если Не ЗначениеЗаполнено(МножественноеИмя) Тогда
			Возврат;
		КонецЕсли; 
		ТекстМодуля = ТекстМодуля + Символы.ПС + МножественноеИмя;
	КонецЕсли;
	ИмяВнешнейОбработки = ирОбщий.ИдентификаторИзПредставленияЛкс("ПереходВКонфигураторе." + ПолноеИмяОбъекта);
...
//
Процедура ПодключитьОбработчикОжиданияОповещенияЛкс(ОписаниеОповещения) Экспорт
	Параметры = Новый Структура;
	Параметры.Вставить("Модуль", ОписаниеОповещения.Модуль);
	Параметры.Вставить("ИмяПроцедуры", ОписаниеОповещения.ИмяПроцедуры);
	ПодключитьОбработчикОжиданияСПараметрамиЛкс("ирКлиент.ВыполнитьМетодМодуляЛкс", Параметры);
КонецПроцедуры
...
Процедура ОтладитьОтложенныйОбъектЛкс(Знач СсылкаИлиИмяФайла = Неопределено, УдалитьОбъектПослеУспешногоОткрытия = Ложь) Экспорт
	...
	#КонецЕсли
	ВычислительРегВыражений = ирПлатформа.RegExp;
	ВычислительРегВыражений.Pattern = "Объект ""([^""]+)""|Файл ""([^""]+)""|Пользователь ""([^""]+)""";
	...
	Если СсылкаИлиИмяФайла = Неопределено Тогда
		...
		Результат = ВычислительРегВыражений.НайтиВхождения(СсылкаИлиИмяФайла);
	...
	Если ТипЗнч(СсылкаИлиИмяФайла) = Тип("Строка") Тогда
		...
		Если Результат[0].SubMatches(0) <> Неопределено Тогда 
			Если ЕстьСправочник Тогда
				ВычислительРегВыражений.Pattern = "\b(" + ирПлатформа.шGUID + ")\b";
				Результат = ВычислительРегВыражений.НайтиВхождения(СсылкаИлиИмяФайла);
				Если Результат.Количество() > 0 Тогда
...
Функция ПолеВвода_ОкончаниеВводаТекстаЛкс(Элемент, Текст, выхСписокПодобранных, СтандартнаяОбработка, РасширенноеЗначение = Null, ЛиТипСтрокаСлужебный = Ложь, ЭтаФорма = Неопределено,
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(выхСписокПодобранных) Тогда
			...
			Если ЗначениеЗаполнено(СписокПодобранных) Тогда
...
// Функция - Прочитать дополнительные поля ссылающихся объектов лкс
//
// Параметры:
...
Функция СохранитьФайлВКонсолиСВосстановлениемЛкс(Знач ДиалогВыбораФайла, Знач ИмяСохраняемогоФайла, ИмяОткрытогоФайла = "", Знач ДанныеДляФайла, Знач СтруктураВосстановления,
	...
	Если СохранитьФайл Тогда
		...
		Если НРег(ИмяСохраняемогоФайла) = НРег(ФайлВосстановления.ПолноеИмя) Тогда 
			...
			ЗначениеВФайл(ФайлВосстановления.Путь + ФайлВосстановления.ИмяБезРасширения + ".inf", НоваяСтруктураФайлаВосстановленияЛкс(ИмяОткрытогоФайла, ФайлВосстановленияОбрезан));
...
// Возвращаемое значение:
//     - 
Функция НоваяСтруктураФайлаВосстановленияЛкс(Знач ИмяОткрытогоФайла, Знач ФайлВосстановленияОбрезан) Экспорт
...
Функция ПроверитьВыбратьФайлВосстановленияКонсолиЛкс(СтруктураВосстановления, выхОписаниеФайла = Ложь) Экспорт
	...
	Для Каждого ЭлементСписка Из СписокВосстановления Цикл
		...
		Попытка
			ОписаниеФайла = ЗначениеИзФайла(ИмяФайлаИнформации); // см. НоваяСтруктураФайлаВосстановленияЛкс
		Исключение
...
// .
// Параметры:
//   ОтчетОбъект - Форма, ОтчетОбъект, см. Отчет.ирАнализПравДоступа.Форма.ФормаОтчета -
