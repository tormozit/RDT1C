...
Процедура ТабличноеПолеОбновитьТекстыПодваловЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Принудительно = Истина) Экспорт 
	...
	Если ОбщийТип = "Список" Тогда
		ИменаВидимыхКолонок = ирОбщий.ОтобратьКоллекциюУсловиемЛкс(ТабличноеПоле.Колонки, "Э.Видимость И Не ПустаяСтрока(Э.Данные)", "Данные");
...
Процедура УстановитьРежимПриемаОбъектаФормеЛкс(Знач ЭтаФорма, Знач Кнопка, Знач НовыйПриемОбъекта = Неопределено, Знач ОсновнойКлючУникальности = Неопределено) Экспорт 
	...
	Если Кнопка.Доступность Тогда
		...
		Иначе
			ирОбщий.ОбновитьТекстПослеМаркераЛкс(Кнопка.Подсказка,, "Отправитель - """ + ЭтаФорма.ВладелецФормы.Заголовок + """", Символы.ПС); // Достаточно одного раза
		КонецЕсли;
	Иначе
		Если ТипЗнч(Кнопка) = Тип("КнопкаФормы") Тогда
...
Процедура ТабличноеПолеСОтборомПросмотраОтобратьЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач ИмяКолонкиНомерСтроки = "", Знач РежимОтладки = Ложь, Знач НовоеИспользованиеОтбора = Неопределено) Экспорт 
	...
	ИначеЕсли ирКэш.НомерРежимаСовместимостиЛкс() < 803006 Тогда
		СтрокаПоиска = "";
		Компоновщик.Настройки.Отбор.Элементы.Очистить();
		ирОбщий.СообщитьЛкс("Корректная работа отбора просмотра доступна только в режиме совместимости 8.3.6 и выше");
		ЭтаФорма[ИмяТП + "ИспользоватьОтбор"] = Ложь;
	КонецЕсли; 
...
Процедура СчитатьПорциюВыборкиВТаблицуЛкс(Выборка, ТаблицаПриемник, Знач РазмерПорции = 9999, СчитыватьЧерезКопиюТаблицы = Истина, СсылкаНаБуфернуюТаблицу = Неопределено) Экспорт
	...
	#Если Сервер И Не Сервер Тогда
		Э = Колонки[0];
	#КонецЕсли
	КолонкиВложенныхТаблиц = ирОбщий.ОтобратьКоллекциюУсловиемЛкс(Колонки, "Э.ТипЗначения.СодержитТип(П1)", "Э.Имя", Тип("РезультатЗапроса"));
...
Процедура ПоказатьПозициюМодуляЛкс(Знач ТекущаяСтрока, Знач ПолноеИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = "", Знач ЧислоПредСтрок = 1) Экспорт 
	...
	//Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСтрокиМодуля) Тогда
		...
		Если Не ФайлМодуля.Существует() Тогда
			ТекущаяСтрока.Ссылка = "<Файл модуля формы не найден в кэше модулей>";
			Возврат;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ТекстФайла) Тогда  
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.Прочитать(ПолноеИмяФайлаМодуля);
			ТекстФайла = ТекстовыйДокумент.ПолучитьТекст();
		КонецЕсли;
		ТекстФрагмента = "";
		...
		КонецЕсли;
		ОболочкаСлужебногоПоляТекста = ирКлиент.ОболочкаПоляТекстаЛкс(мПлатформа.СлужебноеПолеТекста);
		#Если Сервер И Не Сервер Тогда
			ОболочкаСлужебногоПоляТекста = Обработки.ирОболочкаПолеТекста.Создать();
		#КонецЕсли
		ОболочкаСлужебногоПоляТекста.УстановитьТекст(ТекстФайла);
		ОболочкаСлужебногоПоляТекста.УстановитьГраницыВыделения(ТекущаяСтрока.Позиция + 1, ТекущаяСтрока.Позиция + 1 + СтрДлина(ТекстФрагмента));
		ТекущаяСтрока.НомерСтрокиМодуля = ОболочкаСлужебногоПоляТекста.ВыделениеДвумерное().НачальнаяСтрока;
		ВыделениеДвумерное = ОболочкаСлужебногоПоляТекста.ВыделениеДвумерное();
		НачальнаяСтрокаВнутр = 2;
		НачальнаяСтрока = Макс(1, ВыделениеДвумерное.НачальнаяСтрока - ЧислоПредСтрок);
		Если НачальнаяСтрока = ВыделениеДвумерное.НачальнаяСтрока Тогда
			НачальнаяСтрокаВнутр = 1;
		КонецЕсли;
		ОболочкаСлужебногоПоляТекста.УстановитьГраницыВыделения(НачальнаяСтрока, 1, ВыделениеДвумерное.КонечнаяСтрока, 100000);
		Если ПолеТекстаФрагмента <> Неопределено Тогда
			ПолеТекстаФрагмента.УстановитьТекст(ОболочкаСлужебногоПоляТекста.ВыделенныйТекст()); 
		...
		Если ЕстьВызывающиеСвойства Тогда
			ТекущаяСтрока.ВызывающийМетод = мПлатформа.НайтиИмяМетодаСтрокиФайлаМодуля(ФайлМодуля.ИмяБезРасширения, ТекущаяСтрока.НомерСтрокиМодуля);
		КонецЕсли;
		Файл = Новый файл(СтрЗаменить(ПолноеИмяФайлаМодуля, ".Форма.Модуль." + РасширениеФайлаМодуля, ".Форма." + РасширениеФайлаМодуля));
		ТекущаяСтрока.Ссылка = "{" + Файл.ИмяБезРасширения + "(" + XMLСтрока(ТекущаяСтрока.НомерСтрокиМодуля) + ")}";
	//КонецЕсли;
