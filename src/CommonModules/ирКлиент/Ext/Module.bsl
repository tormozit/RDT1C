...
Функция ЗагрузитьТабличныйДокументИнтерактивноЛкс(Знач ТабличныйДокумент = Неопределено, выхПолноеИмяФайла = "", Знач СпособЧтенияЗначений = Неопределено) Экспорт 
...
Процедура КопироватьСсылкиЯчеекФормыЛкс(Форма) Экспорт 
	...
	Если Ссылки.Количество() > 1 Тогда
		БуферОбменаПриложения_УстановитьЗначениеЛкс(ирОбщий.МассивВСписокЗначенийЛкс(Ссылки),, ирОбщий.СтрСоединитьЛкс(ПредставлениеКлючей, Символы.ПС));
	ИначеЕсли КлючТекущейСтроки <> Неопределено Тогда
		БуферОбменаПриложения_УстановитьЗначениеЛкс(КлючТекущейСтроки,, ПредставлениеКлючей[0]);
	КонецЕсли;
