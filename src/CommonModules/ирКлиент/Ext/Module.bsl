...
Функция ГитРепозиторий() Экспорт
	Возврат ирОбщий.ВосстановитьЗначениеЛкс("ГитРепозиторий");
КонецФункции

Функция ИмяФайлаМодуляВПапкеГитаЛкс(Знач ИмяМодуля, ПапкаГитаКэш = "") Экспорт
	Если ПустаяСтрока(ПапкаГитаКэш) Тогда
		ПапкаГитаКэш = ГитРепозиторий();  
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ПапкаГитаКэш) Тогда
		ирКлиент.ОткрытьНастройкиПоляТекстаПрограммыНаЗаднемПланеЛкс("Укажите ""Гит репозиторий"" в открывшейся форме настроек",, "ГитРепозиторий");
		Возврат "";
	КонецЕсли;
	...
	ПроверочныйФайл = Новый Файл(ПапкаГитаКэш + Разделитель + ИмяФайлаМодуляВГите + Разделитель + "Configuration.xml");
	...
	Если Фрагменты.Количество() % 2 = 1 Или ЭтоМодульФормы Тогда
		...
		Если Не ЭтоЕДТ Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ИмяФайлаМодуляВГите = ПапкаГитаКэш + Разделитель + ИмяФайлаМодуляВГите + Разделитель + ИмяСвойстваАнг + ".bsl";
	Иначе
		ИмяФайлаМодуляВГите = ПапкаГитаКэш + Разделитель + ИмяФайлаМодуляВГите; /// папка
	КонецЕсли;
...
Функция ОткрытьНастройкиПоляТекстаПрограммыНаЗаднемПланеЛкс(Знач Причина = "Для анализа содержимого модулей нужно обновить кэш модулей через открывшуюся форму настроек.", Знач ВзятьФокус = Ложь,
