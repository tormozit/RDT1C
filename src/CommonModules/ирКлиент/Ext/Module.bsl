			...
			Если ДопСвойства.Отбор <> Неопределено Тогда
				...
				Если КолонкиПоиска.Количество() > 0 Тогда
					...
					Для Каждого КлючИЗначение Из КолонкиПоиска Цикл
						КлючИЗначение.Значение[0] = "(" + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(КлючИЗначение.Значение[0]) + ")";
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ОткрытьПолеТекстаМодуляКонфигурацииЛкс(Знач ИмяМодуля, выхФормаТекста = Неопределено) Экспорт
	...
	ПолеТекстаНовое = выхФормаТекста.ПолеТекстаПрограммы.ВстроенныйЯзык;
	#Если Сервер И Не Сервер Тогда
...
Функция ОбновитьКэшМодулейЛкс(Знач КаталогКэша = "", Знач ЗапуститьКонфигураторПослеЗавершение = Ложь) Экспорт 
	мПлатформа = ирКэш.Получить();
	...
	Если ЗапуститьКонфигураторПослеЗавершение Тогда
