...
//
Процедура ОткрытьМетодМодуляПоОпределениюЛкс(Знач СтрокаМетода, Знач ИмяПараметра = "", Знач ПолеТекстаНовое = Неопределено, Знач СмещениеСтрокиМетода = 0, Знач НомерКолонки = 1) Экспорт
	мПлатформа = ирКэш.Получить();
...
Функция ПолеТекстаМодуляБезСтруктурыТипаЛкс(Знач ИмяФайлаИлиМодуля, ТекстФайла = Неопределено, выхФайлМодуля = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТекстФайла <> Неопределено Тогда  
			...
			ПолеТекстаМодуля.УстановитьТекстБезМетаданных(ТекстФайла);
		Иначе
			ПолеТекстаМодуля = мПлатформа.ПолеТекстаМодуля(ИмяФайлаИлиМодуля); // Актуализируемый в памяти кэш
			ТекстФайла = ПолеТекстаМодуля.мОригинальныйТекст;
...
Функция ДобавитьВФайлТочекОстановаЛкс(Знач ИмяФайла, Знач СсылкаСтрокиМодуля, Знач ДляПерехода = Истина) Экспорт
	...
	Иначе
		Фрагменты = ирОбщий.СтрРазделитьЛкс(ирОбщий.ПоследнийФрагментЛкс(СтруктураСсылки.Модуль, " "));
	КонецЕсли;
	ИмяМодуляАнг = "";
	Если Фрагменты.Количество() = 1 Тогда
...
Функция ОбновитьКэшМодулейЛкс(Знач КаталогКэша = "", Знач Асинхронно = Ложь, Знач РазрешитьДиалоги = Истина) Экспорт 
	...
	Если РазрешитьДиалоги Тогда 
		...
		Иначе
			Ответ = Вопрос("Выполнять выгрузку через автономный сервер (ibcmd)?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
