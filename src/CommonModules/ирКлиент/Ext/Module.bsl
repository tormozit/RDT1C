...
Процедура ПерейтиКОбъектуМетаданныхВКонфигуратореЛкс(Знач ПолноеИмя) Экспорт 
	...
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	Если Найти(ПолноеИмя, ".Форма.") > 0 Тогда
		ТекстМодуля = ПолноеИмя;
	Иначе 
...
//   - 
//
Функция ОткрытьСсылкуСтрокиМодуляЛкс(Знач Ссылка, Знач ДляИзменения = Ложь) Экспорт
	мПлатформа = ирКэш.Получить();
	СписокВыбора = Новый СписокЗначений;
	Если мПлатформа.мСвязанныйКонфигуратор <> Неопределено Тогда 
		ВариантОткрытьВКонфигураторе = ДобавитьВариантОткрытияМодуляВКонфигураторе(СписокВыбора);
...
Процедура ОткрытьНовоеОкноКонсолиИзЕеОкнаЛкс(Знач ЭтаФорма, Знач ДеревоОбъектов) Экспорт 
	...
	НоваяФорма = ирКлиент.ОткрытьНовоеОкноФормыЛкс(ЭтаФорма); // см. Обработка.ирКонсольКода.Форма.Форма
...
Процедура ОтчетКомпоновкиОбработкаРасшифровкиЛкс(Знач ОтчетОбъект, Знач Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры, ПолеТабличногоДокумента, ДанныеРасшифровки,
	...
	Если ирОбщий.МетодРеализованЛкс(ОтчетОбъект, "ОбработкаРасшифровки") Тогда
		ОтчетОбъект.ОбработкаРасшифровки(ДанныеРасшифровки, ЭлементРасшифровки, ПолеТабличногоДокумента, ДоступныеДействия, СписокДополнительныхДействий, РазрешитьАвтовыборДействия, ЗначенияВсехПолей); //@skip-check unknown-method-property
	КонецЕсли; 
	...
	Если ирОбщий.МетодРеализованЛкс(ОтчетОбъект, "ДействиеРасшифровки") Тогда
		ОтчетОбъект.ДействиеРасшифровки(ВыбранноеДействие, ПараметрВыбранногоДействия, СтандартнаяОбработка); //@skip-check unknown-method-property
	КонецЕсли; 
	Если СтандартнаяОбработка Тогда
...
Процедура РазобратьПозициюМодуляВСтрокеТаблицыЛкс(Знач ТекущаяСтрока, Знач ИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = Неопределено,
	Знач ЧислоПредСтрок = 2, Знач ИмяМетода = "", Знач ЗаполнитьТекстСтроки = Ложь) Экспорт 
