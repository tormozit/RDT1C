...
Процедура ОткрытьМетодМодуляПоИмениЛкс(Знач ИмяМетода, Знач СтруктураТипаИлиИмяМодуля = Неопределено) Экспорт
	...
	Иначе
		...
		Если Фрагменты.Количество() > 1 Тогда
			ИмяМетода = Фрагменты[0]; 
			СтруктураТипаМодуля = мПлатформа.СтруктураТипаИзЗначения(Метаданные.ОбщиеМодули.Найти(Фрагменты[1]));
		Иначе
	...
	Если МодульМетаданных <> Неопределено Тогда
		МетодМодуля = МодульМетаданных.Методы.Найти(НРег(ИмяМетода), "НИмя");
...
//
Функция ОткрытьМетодМодуляПоОпределениюЛкс(Знач СтрокаМетода, Знач РазрешитьДиалоги = Истина, Знач ИмяПараметра = "", Знач ПолеТекстаНовое = Неопределено) Экспорт
	мПлатформа = ирКэш.Получить();
	...
	Иначе 
		...
		Если ПолеТекстаНовое <> Неопределено Тогда
			ПолеТекстаНовое.ПерейтиКОпределениюЛокальногоМетода(СтрокаМетода, ИмяПараметра);
		КонецЕсли;
	КонецЕсли; 
	Возврат Истина;
КонецФункции
...
Функция ОткрытьЗначениеЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина, ЗаголовокФормы = "", РедактироватьМодально = Истина,
	...
	ИначеЕсли ТипРасширенногоЗначения = Тип("СхемаКомпоновкиДанных") Тогда
		...
		#КонецЕсли
		ДанныеРедактора = Новый Структура("СхемаКомпоновки", РедактируемоеЗначение);
		Если ЗначениеЗаполнено(ЗаголовокФормы) Тогда
...
Функция ОткрытьТаблицуЗначенийЛкс(Знач ТаблицаЗначений, Знач ТекущаяСтрока = Неопределено, Модально = Истина, Заголовок = "", РежимВыбора = Ложь, ТолькоПросмотр = Истина,
	...
	ИмитаторТабличногоПоля.Вставить("ТекущаяКолонка", Новый Структура("Имя, Данные", ТекущаяКолонка, ТекущаяКолонка));
		...
		Для Каждого Колонка Из КолонкиТаблицы Цикл
			...
			Если Не ОтображатьИдентификаторы И РегВыражениеНачальныхПустыхСтрок <> Неопределено Тогда
				...
				Если ПозицияПС > 0 И ПустаяСтрока(Лев(ТекстЯчейки, ПозицияПС)) Тогда
					ПредставлениеЗначения = "¶" + Сред(ТекстЯчейки, РегВыражениеНачальныхПустыхСтрок.НайтиВхождения(ТекстЯчейки)[0].Length + 1);
				КонецЕсли;
			КонецЕсли;
...
Функция НайтиПоказатьФрагментВПолеТекстаЛкс(Форма, Знач ПолеТекста, Знач СтрокаПоиска, Знач СловоЦеликомИлиШаблон = Ложь, Знач ИскатьСНачала = Ложь, Знач РазрешитьАктивациюПоля = Истина,
	...
	Пока Истина Цикл
		...
		Если СтрокаПоиска <> "" Тогда
			...
			КонецЕсли;
		КонецЕсли; 
		Если ИскатьСНачала Или Не ИскатьСНачалаЕслиНеНайдено Тогда
			Прервать;
		КонецЕсли;
...
Функция ПассивнаяФормаПоИмениЛкс(Знач ИмяФормы) Экспорт
	Результат = ПолучитьФормуЛкс(ИмяФормы, ирКлиент.ПараметрыБыстрогоСозданияФормыЛкс());
	Возврат Результат;
КонецФункции
...
Функция НомерСтрокиМодуляИзПозицииЛкс(Знач ИмяФайлаМодуля, Знач Позиция, Знач ПеречитатьФайл = Ложь, Знач ТекстФайла = "", Знач ТекстФрагмента = Неопределено, выхПолеТекста = Неопределено, выхФайлМодуля = Неопределено) Экспорт
	Если выхПолеТекста = Неопределено Тогда
		выхПолеТекста = ПолеТекстаМодуляЛкс(ИмяФайлаМодуля, ТекстФайла, ПеречитатьФайл, выхФайлМодуля);
	КонецЕсли;
	Если выхПолеТекста <> Неопределено Тогда
...
Функция ПолеТекстаМодуляЛкс(ИмяФайлаМодуля, ТекстФайла = Неопределено, Знач ПеречитатьФайл = Ложь, выхФайлМодуля = Неопределено) Экспорт
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	Если Найти(ИмяФайлаМодуля, ":") > 0 Тогда
		...
		КонецЕсли;
	Иначе
		ПолеТекстаМодуля = ирКэш.ПолеТекстаМодуляЛкс(ИмяФайлаМодуля);
...
// Возвращаемое значение:
//    Структура - 
Функция ДобавитьВФайлТочекОстановаЛкс(Знач ИмяФайла, Знач СсылкаСтрокиМодуля) Экспорт
	...
	ЭлементАктивность.ТекстовоеСодержимое = XMLСтрока(Ложь);
...
Процедура ОткрытьНастройкиПоляТекстаПрограммыНаЗаднемПланеЛкс() Экспорт 
