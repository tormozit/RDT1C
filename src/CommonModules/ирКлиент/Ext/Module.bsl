...
Функция ЧистоеОписаниеОшибкиЛкс(Знач ИнформацияОбОшибке, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрока = 0, Знач СтартоваяКолонка = 0, Знач СтартоваяСтрокаДоп = 0, Знач СтартоваяСтрокаДопПорог = 0,
	...
	Если Вхождения.Количество() > 0 Тогда
		Если ирОбщий.СтрНачинаетсяСЛкс(Вхождения[0].ТекстВхождения, "{(") Тогда
			//{(11, 41)}: Поле не найдено "Т.Валюта"
			//{ГДЕ ТИПЗНАЧЕНИЯ(Т.Регистратор) КАК Регистратор_ТипЗначения_
			ПоследнееВхождение = Вхождения[0]; // У этого варианта некорректная работа на примере выше
		Иначе
			// {ОбщийМодуль.ирОбщий.Модуль(1910)}: Ошибка при вызове метода контекста (Создать): Ошибка инициализации модуля: ВнешняяОбработка.Dynamic.МодульОбъекта: {ВнешняяОбработка.Dynamic.МодульОбъекта(7,5)}: Неопознанный оператор
...
Функция ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, ТабличноеПоле, СтандартнаяОбработка = Ложь, РасширенноеЗначение = Null,
	...
	Если РедактированиеРазрешено И ПринудительноВОтдельнойФорме Тогда
		Результат = ПолеВводаСистемногоПеречисления_НачалоВыбораЛкс(ЭтаФорма, ЭлементУправления, СтандартнаяОбработка, РасширенноеЗначение);
	КонецЕсли;
	Если Результат = Неопределено Тогда
...
//.
// Параметры:
//   ЭтаФорма - ГруппаФормы, ТаблицаФормы, Форма, ФормаКлиентскогоПриложения - 
//   ЭлементУправления - ПолеВвода, ПолеВыбора, ПолеФормы, Флажок, Неопределено - 
//   СтандартнаяОбработка - Булево - 
//   РасширенноеЗначение - Null, Произвольный, Тип, Неопределено - 
// Возвращаемое значение:
//   Булево, Неопределено - 
Функция ПолеВводаСистемногоПеречисления_НачалоВыбораЛкс(Знач ЭтаФорма, Знач ЭлементУправления, СтандартнаяОбработка, РасширенноеЗначение) Экспорт
	мПлатформа = ирКэш.Получить();
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
...
// Возвращаемое значение:
//    - 
Функция ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(Знач ЭтаФорма, Знач Элемент, СтандартнаяОбработка, РасширенноеЗначение = Null, Знач СтруктураОтбора = Неопределено, Знач ЗапретитьБольшиеТипы = Ложь, 
	...
	Иначе
		...
		Иначе
			ЗначениеИзменено = ПолеВводаСистемногоПеречисления_НачалоВыбораЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка, РасширенноеЗначение);
		КонецЕсли;
		Если Истина
			И СтандартнаяОбработка
...
Функция ТабличноеПолеСОтборомПросмотраОбновитьПредставлениеЛкс(Знач ЭтаФорма, Знач ТабличноеПоле, Знач РазрешитьРучноеВключение = Ложь) Экспорт 
	...
	Иначе
		...
		КонецЕсли; 
	КонецЕсли;
	Если НадписьОтборПросмотра <> Неопределено Тогда
...
Процедура ПолеВводаПользователяОСНачалоВыбораЛкс(Знач Элемент, СтандартнаяОбработка, ДобавлятьЛидирующиеСлеши = Ложь) Экспорт 
	...
	КонецЕсли; 
	ФормаВыбора = ирКэш.Получить().ПолучитьФорму("ВыборПользователяОС",, Элемент);
	ФормаВыбора.ВыбранныйПользователь = НачальноеЗначение;
	Результат = ФормаВыбора.ОткрытьМодально();
	Если Результат <> Неопределено Тогда
...
// Выдает индекс типа таблицы в коллекции см. ОбщаяКартинка.ирТаблицыЗапроса
Функция ИндексКартинкиТипаТаблицыБДЛкс(Знач ТипТаблицы) Экспорт
	...
	ИначеЕсли ТипТаблицы = "КритерийОтбора" Тогда
		ИндексКартинки = 31;
	ИначеЕсли ТипТаблицы = "Очередь" Тогда
		ИндексКартинки = 32;
...
//   ИмяЭлемента -  - 
//   ПутьКРеквизиту -  - 
Процедура ОткрытьСтруктуруФормыЛкс(Знач ЭтаФорма, Знач КлючУникальности = Неопределено, Знач ИмяЭлемента = "", Знач ПутьКРеквизиту = "", Знач Модально = Истина) Экспорт
	...
	Если Модально Тогда
		ФормаСтруктуры.ОткрытьМодально();
	Иначе
		ФормаСтруктуры.Открыть();
	КонецЕсли;
КонецПроцедуры

// Вызывается из адаптера.
Процедура ОткрытьСтруктуруФормыПоИмениЛкс(Знач ИмяФормы) Экспорт
	Форма = ПассивнаяФормаПоИмениЛкс(ИмяФормы);
	ОткрытьСтруктуруФормыЛкс(Форма,,,, Ложь);
КонецПроцедуры
...
Процедура ОтладитьКомпоновкуДанныхАктивнойУправляемойФормыЛкс(Форма = Неопределено) Экспорт 
	...
	Если Форма = Неопределено Тогда
		Форма = АктивнаяУправляемаяФормаЛкс();
	КонецЕсли; 
	Наименование = Форма.КлючСохраненияПоложенияОкна;
	Если ТипЗнч(Форма) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
		...
		Если Фрагменты[0] = ирОбщий.ПеревестиСтроку("ОбщаяФорма") Тогда
			Фрагменты = ирОбщий.СтрРазделитьЛкс(ирОбщий.ПервыйФрагментЛкс(Форма.КлючСохраненияПоложенияОкна, "/"));
		КонецЕсли;
		Если Истина
			И НастройкиОтчета <> Неопределено 
			//И НастройкиОтчета.СхемаМодифицирована 
			И ЗначениеЗаполнено(НастройкиОтчета.АдресСхемы)
		Тогда
		...
		Иначе
			Если Фрагменты[0] = ирОбщий.ПеревестиСтроку("Отчет") Тогда 
				...
				Если ПустаяСтрока(Наименование) Тогда
					Наименование = ОтчетОбъект.Метаданные().ПолноеИмя();
				КонецЕсли;
			...
			Иначе
				ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонИменЛкс("Не поддерживаемый тип метаданных отчета - %1",, Фрагменты[0]),,, Истина);
				Возврат;
			КонецЕсли; 
		КонецЕсли;
	...
	КонецЕсли;
	ирОбщий.ОтладитьЛкс(СхемаКомпоновки,, КомпоновщикНастроек.ПолучитьНастройки(),,,,, Наименование);
...
//.
// Параметры:
//   ТабличноеПоле - ТабличноеПоле - с таблицей
