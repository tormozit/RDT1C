	...
	Для Каждого ПолеСписка Из ПоляСписка Цикл
		...
		Для Каждого СоединенныйРегистр Из СоединенныеРегистры Цикл
			...
			Если ИмяКолонки = ИмяПоляЧислоСтрок + "_" + ПсевдонимРегистра Тогда
				ЗаголовокПоля = ЗаголовокПоляЧислоСтрокЛкс(СоединенныйРегистр, РежимИмяСиноним);
				ЗаголовокПоля = ЗаголовокПоляТаблицыФормыЛксФрагмент(ЗаголовокПоля, ОсновнойЭУ, ПолеСписка.ТипЗначения);
				РодительПоляФормы = ДобавитьГруппуКолонокБД("Число строк", ОсновнойЭУ, ИмяПоляЧислоСтрок, ЭтаФорма);
				Прервать;
			КонецЕсли;
...
Функция ЗаголовокПоляЧислоСтрокЛкс(Знач СоединенныйРегистр, РежимИмяСиноним = Ложь) Экспорт 
	...
	Иначе
		ЗаголовокРегистра = ПодчиненныйМД.Представление();
	КонецЕсли;
	Если ирОбщий.СтрНачинаетсяСЛкс(СоединенныйРегистр, "Регистр") Тогда
		ЗаголовокПоля = "Строки: " + ЗаголовокРегистра;    
	Иначе
		ЗаголовокПоля = "Строки ТЧ: " + ЗаголовокРегистра;
	КонецЕсли;
...
Функция ЗаголовокПоляТаблицыФормыЛкс(Знач ОсновнойЭУ, Знач ПолеТаблицыБД, Знач РежимИмяСиноним, Подсказка = "", Знач РазрешитьОбрезку = Истина, Знач МодификаторПоля = "")  Экспорт 
	...
	Если Не ЗначениеЗаполнено(Подсказка) Тогда
		Подсказка = Заголовок;
	КонецЕсли;
	Заголовок = ЗаголовокПоляТаблицыФормыЛксФрагмент(Заголовок, ОсновнойЭУ, ТипЗначения, РазрешитьОбрезку);
	Возврат Заголовок;

КонецФункции

Функция ЗаголовокПоляТаблицыФормыЛксФрагмент(Знач Заголовок, Знач ОсновнойЭУ, Знач ТипЗначения, Знач РазрешитьОбрезку = Истина)
...
Функция ТекстЗапросаДинамическогоСпискаЛкс(Знач ОбъектМД, Знач ПолноеИмяТаблицыБД, Знач СоединенныеРегистры, Знач ДинамическийСписок, СкрытыеПоля = Неопределено, ПсевдонимыТаблиц = Неопределено) Экспорт 
	...
	Для Каждого СоединенныйРегистр Из СоединенныеРегистры Цикл
		ПсевдонимРегистра = ПсевдонимРегистраЛкс(СоединенныйРегистр);
		//ПсевдонимРегистра = СтрЗаменить(СоединенныйРегистр, ".", "_");
		ПодчиненныйМД = ирОбщий.ОбъектМДПоПолномуИмениТаблицыБДЛкс(СоединенныйРегистр);
