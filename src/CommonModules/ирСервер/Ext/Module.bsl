...
Функция НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, ПолноеИмяТаблицыБД, РежимИмяСиноним = Ложь, ПредельноеКоличествоВидимыхКолонок = 10, СоединенныеРегистры = Неопределено) Экспорт
	...
	ТекстЗапроса = ТекстЗапросаДинамическогоСпискаЛкс(ОбъектМД, ПолноеИмяТаблицыБД, СоединенныеРегистры,, ДинамическийСписок, ТекстДопПоля, СкрытыеПоля);
...
Функция ТекстЗапросаДинамическогоСпискаЛкс(ОбъектМД, Знач ПолноеИмяТаблицыБД, Знач СоединенныеРегистры, ПсевдонимОсновнойТаблицы = "_Т", Знач ДинамическийСписок = Неопределено, ТекстДопПоля = "",
	...
	Для Каждого СоединенныйРегистр Из СоединенныеРегистры Цикл
		...
		ИмяРегистра = ФрагментыИмени[1] + ПсевдонимОсновнойТаблицы;
		...
		|ЛЕВОЕ СОЕДИНЕНИЕ " + СоединенныйРегистр + " КАК " + ИмяРегистра + "
		|ПО _Т.Ссылка = " + ИмяРегистра + "." + РегистрМД.Измерения[0].Имя;
		ИсточникиДанных.Вставить(ИмяРегистра, СоединенныйРегистр);
	КонецЦикла;
