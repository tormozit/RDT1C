...
Функция НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, ПолноеИмяТаблицыБД, РежимИмяСиноним = Ложь, ПредельноеКоличествоВидимыхКолонок = 10, СоединенныеРегистры = Неопределено,
	...
	Для Каждого ПолеСписка Из ПоляСписка Цикл
			...
			Если ирОбщий.СтрНачинаетсяСЛкс(ИмяКолонки, ПрефиксИмениПолей) Тогда
				...
				Если РодительПоляФормы = Неопределено Тогда
					...
					Если РежимИмяСиноним Тогда
						ЗаголовокГруппы = ОбъектМД.Имя;
					Иначе
						ЗаголовокГруппы = ОбъектМД.Представление();
					КонецЕсли;
					Если ирОбщий.СтрКончаетсяНаЛкс(СоединенныйРегистр, ".СрезПоследних") Тогда
						ЗаголовокГруппы = ЗаголовокГруппы + " (срез)";
					ИначеЕсли ирОбщий.СтрКончаетсяНаЛкс(СоединенныйРегистр, ".Остатки") Тогда
						ЗаголовокГруппы = ЗаголовокГруппы + " (остатки)";
					КонецЕсли;
		...
		Если КолонкаТП = Неопределено Тогда 
			...
			КонецЕсли;
			КолонкаТП.Ширина = ирОбщий.ШиринаТипаЗначенияЛкс(ПолеСписка.ТипЗначения, КолонкаТП.РастягиватьПоГоризонтали);
...
Процедура НастроитьСиситемноеПолеФормыТипаБулевоЛкс(Знач ОсновнойЭУ, Знач ИмяКолонки, ПоляБезОбрезкиЗаголовков = Неопределено)
	...
	ИначеЕсли ИмяКолонки = "ПометкаУдаления" Тогда
		ИмяКартинки = "ПометитьНаУдаление";
	ИначеЕсли ИмяКолонки = "Проведен" Тогда
		ИмяКартинки = "Провести";
	ИначеЕсли ИмяКолонки = "ЭтоГруппа" Тогда
		ИмяКартинки = "ирПапка";
	Иначе 
...
Функция ТекстЗапросаДинамическогоСпискаЛкс(Знач ОбъектМД, Знач ПолноеИмяТаблицыБД, Знач СоединенныеРегистры, Знач ДинамическийСписок, СкрытыеПоля) Экспорт 
	...
	Для Каждого СоединенныйРегистр Из СоединенныеРегистры Цикл
		...
		#Если Сервер И Не Сервер Тогда
			РегистрМД = Метаданные.РегистрыСведений.ABCКлассификацияПокупателей;
		#КонецЕсли
		...
		Если ирОбщий.ЛиКорневойТипСсылкиЛкс(ТипТаблицы) Тогда
			МассивИзмерений.Добавить(Символы.ПС + "И _Т.Ссылка = " + ПсевдонимРегистра + "." + РегистрМД.Измерения[0].Имя);
		Иначе
			Для Каждого МетаИзмерение Из РегистрМД.Измерения Цикл
				#Если Сервер И Не Сервер Тогда
					МетаИзмерение = РегистрМД.Измерения.Контрагент;
				#КонецЕсли
				МассивИзмерений.Добавить(Символы.ПС + "И _Т." + МетаИзмерение.Имя + " = " + ПсевдонимРегистра + "." + МетаИзмерение.Имя);
			КонецЦикла;
		КонецЕсли;
		ТекстИсточников = ТекстИсточников + ирОбщий.СтрСоединитьЛкс(МассивИзмерений, "");
	...
	Если ТипТаблицы <> "Внешняя" Тогда
		Для Каждого МетаКонстанта Из Метаданные.Константы Цикл
			...
			Если ОпцииВключены Тогда
				ИмяЛюбойКонстанты = Метаданные.Константы[0].Имя;
				ТекстИсточников = ТекстИсточников + "
	...
	Для Каждого КлючИЗначение Из ИсточникиДанных Цикл
		...
		Для Каждого ПолеТаблицы Из ПоляТаблицыБД Цикл
			Если Истина
				...
				И ОбъектМДСоединяемый.Измерения[0].Имя = ПолеТаблицы.Имя 
			Тогда
				Продолжить;
			КонецЕсли;
			...
			Тогда
				ВыбранныеПоля.Добавить("ТИПЗНАЧЕНИЯ(" + ВыражениеПоля + ") КАК " + ПсевдонимПоля + "_ТипЗначения_");
			КонецЕсли;
			Если Истина
...
Функция ЗначениеНедоступногоПоляЛкс() Экспорт 
	...
	Возврат "<Недоступно из-за отсутствия констант>";
