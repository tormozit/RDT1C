...
Функция НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, ПолноеИмяТаблицыБД, РежимИмяСиноним = Ложь, ПредельноеКоличествоВидимыхКолонок = 10, СоединенныеРегистры,
	...
	Для Каждого ПолеСписка Из ПоляСписка Цикл
		...
		Для Каждого ОписаниеСоединения Из СоединенныеРегистры Цикл
			...
			Если ИмяКолонки = ИмяПоляЧислоСтрок + "_" + ПсевдонимРегистра Тогда
				...
				КонецЕсли;
				ЗаголовокПоля = РазбитьЗаголовокПоляТаблицыФормыЛкс(ЗаголовокПоля, ОсновнойЭУ, ПолеСписка.ТипЗначения);
