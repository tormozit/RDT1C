...
Функция НастроитьАвтоТаблицуФормыДинамическогоСпискаЛкс(ЭтаФорма, ОсновнойЭУ, ПолноеИмяТаблицыБД, РежимИмяСиноним = Ложь, СоединенныеРегистры,
	...
	Для Каждого ПолеСписка Из ПоляСписка Цикл
		...
		Если КолонкаТП = Неопределено Тогда 
			...
			Если ЗначениеЗаполнено(Подсказка) Тогда
				...
				ирОбщий.ДобавитьОписаниеТиповВПодсказкуШапкиКолонкиЛкс(КолонкаТП, ПолеСписка.ТипЗначения);
			КонецЕсли;
