...
Функция СоединениеХТТП(ИмяСервера, Таймаут = 0, ИспользоватьЗащищенноеСоединение = Истина) Экспорт 
...
Функция СкачатьФайл(Знач Ссылка) Экспорт
	...
	Соединение = СоединениеХТТП(ИмяСервера);
	...
	КонецЕсли;
	Соединение = СоединениеХТТП(СтруктураАдреса.ИмяСервера);
	Запрос = Новый ("HTTPЗапрос");
...
    ВнешняяОбработкаОбъект = РеквизитФормыВЗначение("Объект"); // ВнешняяОбработкаОбъект      
    Возврат ВнешняяОбработкаОбъект.ИспользуемоеИмяФайла;
КонецФункции
...
Функция НайтиФайлНаСайте(Знач НазваниеНаСайте, Знач АдресСпискаФайлов = "") Экспорт
	...
	КонецЕсли;
	ИмяСервера = АдресСайтаЛкс();
	Соединение = СоединениеХТТП(ИмяСервера, 5);
	#Если Сервер И Не Сервер Тогда
	Соединение = Новый HTTPСоединение;
	#КонецЕсли
	Запрос = Новый ("HTTPЗапрос");
