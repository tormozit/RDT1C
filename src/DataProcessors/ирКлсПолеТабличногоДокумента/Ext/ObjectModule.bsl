...
Процедура СвернутьДоУровня(СтрокаКоманды) Экспорт
	...
	СтрокаУровня = ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаКоманды, МаркерСвернутьДоУровня);
	СтрокаНомераУровняСтрок = ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаУровня, "Строк", , Ложь);
	Если СтрокаНомераУровняСтрок <> Неопределено Тогда 
		...
		ПолеТабличногоДокумента.ПоказатьУровеньГруппировокСтрок(Уровень);
	Иначе
		СтрокаНомераУровняКолонок = ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаУровня, "Колонок", , Ложь);
	...
	Для Счетчик = 1 По 2 Цикл
		Если Счетчик = 1 Тогда 
			...
			Если ТипЗнч(Источник) = Тип("ПостроительОтчета") Тогда
				КоллекцияИзмерений = ИзмеренияПостроителяПоУровням(Источник.ИзмеренияСтроки);
			ИначеЕсли  ТипЗнч(Источник) = Тип("НастройкиКомпоновкиДанных") Тогда 
				КоллекцияИзмерений = ИзмеренияСтрокНастройкиКомпоновкиПоУровням(Источник.Структура);
			КонецЕсли;
		...
		Иначе
			...
			Если ТипЗнч(Источник) = Тип("ПостроительОтчета") Тогда
				КоллекцияИзмерений = ИзмеренияПостроителяПоУровням(Источник.ИзмеренияКолонки);
			ИначеЕсли ТипЗнч(Источник) = Тип("НастройкиКомпоновкиДанных") Тогда
				КоллекцияИзмерений = ИзмеренияКолонокНастройкиКомпоновкиПоУровням(Источник.Структура);
			КонецЕсли;
		КонецЕсли;
...
//
Функция ИзмеренияСтрокНастройкиКомпоновкиПоУровням(СтруктураНастройкиКомпоновки)
	...
	Для Каждого ЭлементСтруктуры Из СтруктураНастройкиКомпоновки Цикл
		...
		Тогда
			...
			Если НетИерархии Тогда
				МассивИзмеренийПотомков = ИзмеренияСтрокНастройкиКомпоновкиПоУровням(ЭлементСтруктуры.Структура);
			КонецЕсли;
			СмещениеГлубины = 1;
		ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда
			МассивИзмеренийПотомков = ИзмеренияСтрокНастройкиКомпоновкиПоУровням(ЭлементСтруктуры.Строки);
			СмещениеГлубины = 0;
		Иначе 
			Продолжить;
		КонецЕсли;
...
//               перечисленные через запятую.
//
Функция ИзмеренияКолонокНастройкиКомпоновкиПоУровням(СтруктураНастройкиКомпоновки)
	...
	Для Каждого ЭлементСтруктуры Из СтруктураНастройкиКомпоновки Цикл  
		...
		Если ТипЭлемента = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			Для Каждого ПолеГруппировки Из ЭлементСтруктуры.ПоляГруппировки.Элементы Цикл
				Если Ложь
					Или ТипЗнч(ПолеГруппировки) <> Тип("ПолеГруппировкиКомпоновкиДанных")
					Или Не ПолеГруппировки.Использование 
				Тогда
					Продолжить;
				КонецЕсли;
				ДоступноеПоле = ЭлементСтруктуры.ПоляГруппировки.ДоступныеПоляПолейГруппировок.НайтиПоле(ПолеГруппировки.Поле);
			...
			КонецЦикла;
			Если НетИерархии Тогда
				МассивИзмеренийПотомков = ИзмеренияКолонокНастройкиКомпоновкиПоУровням(ЭлементСтруктуры.Структура);
			КонецЕсли;
			СмещениеГлубины = 1;
		ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда
			МассивИзмеренийПотомков = ИзмеренияКолонокНастройкиКомпоновкиПоУровням(ЭлементСтруктуры.Колонки);
			СмещениеГлубины = 0;
		Иначе 
			Продолжить;
		КонецЕсли;
...
//
Функция ИзмеренияПостроителяПоУровням(Измерения)
