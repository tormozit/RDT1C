...
Функция ОтправитьЗапросИИ(Знач ПараметрыЗапроса, Знач Форма = Неопределено, Знач ОбработчикОтвета = "", Знач БлокируемыеЭлементы = Неопределено, Знач Кнопка = Неопределено,
	...
	Иначе
		...
		Если ЛиПотоковый Тогда
			//СтруктураЗапроса.Вставить("reasoning", Новый Структура("enabled", Ложь)); /// Блокируем рассуждения. Отключение не все модели поддерживают
			СтруктураЗапроса.Вставить("include_reasoning", Ложь); /// Блокируем передачу рассуждений. Отключение не все модели поддерживают
		КонецЕсли;
