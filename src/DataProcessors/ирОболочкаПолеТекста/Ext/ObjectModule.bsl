...
Функция РазметитьТекстРезультатамиПоискаИУстановитьВПоле(Знач Текст = "", Знач РезультатыПоиска, Знач ИндексГруппы = -1, Знач ПереносСлов = Истина, Знач Стили = "") Экспорт
	...
	КонецЕсли;
	Результат = РазметитьТекстРезультатамиПоиска(Текст, РезультатыПоиска, ИндексГруппы);
	УстановитьТелоHTML(Результат, ПереносСлов, Стили);
КонецФункции
...
// Возвращаемое значение:
//   Строка - 
Функция РазметитьТекстРезультатамиПоиска(Знач Текст, Знач РезультатыПоиска, Знач ИндексГруппы = -1) Экспорт
...
Процедура УстановитьТелоHTML(Знач ТекстHTML, Знач ПереносСлов = Истина, Знач Стили = "", Знач ЧерезСвойство = Истина) Экспорт
	...
	|	<style>" + Стили + "</style>
	|</HEAD>
	|<BODY>"+ ОформлениеТекста + РаскрашенныйТекст + ТекстHTML + "</BODY></HTML>";
	ДокументHtml = ЭлементФормы.Документ; 
