...
// Функция - Позиция в поле текста по номеру строки и колонки
//
// Параметры:
//  КонечнаяСтрока	 - 	 - 
//  КонечнаяКолонка	 - 	 - 
...
Функция ПозицияВПолеТекстаПоНомеруСтрокиИКолонки(Знач КонечнаяСтрока, Знач КонечнаяКолонка) Экспорт 
	...
	ВыделениеДвумерное = ВыделениеДвумерное();
	УстановитьГраницыВыделения(1, 1, КонечнаяСтрока, КонечнаяКолонка);
	Результат = СтрДлина(ВыделенныйТекст()) + 1;
	УстановитьВыделениеДвумерное(ВыделениеДвумерное);
	Возврат Результат;
