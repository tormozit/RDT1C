	...
	Для каждого Группа Из РезультатыПоиска Цикл
		...
		Если Старт Тогда
			Если Группа.ПозицияГруппы <> 0 Тогда
				Результат.ЗаписатьБезОбработки(ирОбщий.КодироватьТекстВXMLЛкс(Сред(Текст, 1, ПозицияГруппы)));
			КонецЕсли;
			Старт = Ложь;
		ИначеЕсли Финиш <> 0 Тогда
			Результат.ЗаписатьБезОбработки(ирОбщий.КодироватьТекстВXMLЛкс(Сред(Текст, Финиш, ПозицияГруппы - Финиш + 1)));
		КонецЕсли;
		ЗначениеГруппы = ирОбщий.КодироватьТекстВXMLЛкс(ТекстГруппы);
