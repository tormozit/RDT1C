...
Функция РазметитьТекстРезультатамиПоиска(Знач Текст = "", Знач РезультатыПоиска, Знач ИндексГруппы = -1, Знач ПереносСлов = Истина) Экспорт 
	...
	Для каждого Группа Из РезультатыПоиска Цикл
		Если Группа.ПозицияГруппы = Неопределено Или РаскрашеноВхождений > МаксРаскашиваемыхВхождений Тогда
			Продолжить;
		КонецЕсли; 
		Если ИндексГруппы = -1 Тогда
			ТекстГруппы = Группа.Значение;
		Иначе
			ТекстГруппы = Группа.Группы[ИндексГруппы].Значение;
		КонецЕсли;
		Если ТекстГруппы = Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		ПозицияГруппы = Группа.ПозицияГруппы;
		ДлинаГруппы = Группа.ДлинаГруппы;
		Если Старт Тогда
			Если Группа.ПозицияГруппы <> 0 Тогда
				ДобавитьРазмеченныйФрагмент(Результат, Сред(Текст, 1, ПозицияГруппы));
			КонецЕсли;
			Старт = Ложь;
		ИначеЕсли Финиш <> 0 Тогда
			ДобавитьРазмеченныйФрагмент(Результат, Сред(Текст, Финиш, ПозицияГруппы - Финиш + 1));
		КонецЕсли;
		ЗначениеГруппы = ТекстГруппы;
		//ЗначениеГруппы = ирОбщий.КодироватьТекстВXMLЛкс(ЗначениеГруппы); // Мультиметка250726_113722 Зачем проверяли запрещенные для XML символы?
		...
		Иначе
			ТегВхождения = "<SPAN id=""" + ИдентификаторГруппыВРазмеченномТексте(Группа.Номер, ИндексГруппы) + """ style=""background-color: " + ИмяЦвета + """>" + ЗначениеГруппы + "</SPAN>";
		КонецЕсли;
	...
	КонецЦикла;
	ДобавитьРазмеченныйФрагмент(Результат, Сред(Текст, Финиш));
	Результат = Результат.Закрыть();
	УстановитьТелоHTML(Результат, ПереносСлов);
	
КонецФункции

Процедура ДобавитьРазмеченныйФрагмент(Результат, Знач Текст) Экспорт          
	//Результат.ЗаписатьБезОбработки(ирОбщий.КодироватьТекстВXMLЛкс(Текст));  // Мультиметка250726_113722 Зачем проверяли запрещенные для XML символы?
...
Функция ИдентификаторГруппыВРазмеченномТексте(Знач НомерГруппы, Знач ИндексГруппы = Неопределено) Экспорт 
	...
	КонецЕсли;
	Возврат "Match" + НомерГруппы + "_SubMatch" + ИндексГруппы;

КонецФункции

Процедура ВыделитьРезультатПоиска(Знач РезультатПоиска, Знач ИндексГруппы = -1) Экспорт 
