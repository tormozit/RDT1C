...
Функция ДобавитьМассивОбъектовВУдаляемыеОбъекты(Знач МассивОбъектов, Знач ЭтаФорма = Неопределено, Знач УстанавливатьТекущуюСтроку = Ложь, Знач ПредлагатьПомечатьНаУдаление = Ложь) Экспорт 
	...
	Если БылиПропуски Тогда
		ирОбщий.СообщитьЛкс("Некоторые типы ссылок были пропущены, т.к. принадлежат расширениям конфигурации. Для их поддержки требуется режим совместимости конфигурации 8.3.20+");
	КонецЕсли;
...
// Возвращаемое значение:
//   ТаблицаЗначений - 
Функция ОбновитьДанныеКандидатов(Знач ТаблицаОбъектов = Неопределено, Знач МинимизироватьТипы = Истина, Знач ДополнительноеПоле = "") Экспорт 
	...
	//! ТаблицаОбъектов.Колонки.Добавить(ДополнительноеПоле);
...
Функция КонтролироватьСсылкиНаКандидаты(Знач Параметры = Неопределено) Экспорт 
	...
	Для Каждого МетаРеквизит Из Метаданные().ТабличныеЧасти.СсылкиНаКандидата.Реквизиты Цикл
		Если ТаблицаСсылок.Колонки.Найти(МетаРеквизит.Имя) = Неопределено Тогда
			ТаблицаСсылок.Колонки.Добавить(МетаРеквизит.Имя, МетаРеквизит.Тип);
		КонецЕсли; 
	КонецЦикла;
	//!ТаблицаСсылок.Колонки.Добавить("ИндексКартинки", Новый ОписаниеТипов("Число"));
	//!ТаблицаСсылок.Колонки.Добавить("ДанныеСтрока", Новый ОписаниеТипов("Строка"));
	...
	Для каждого ЭлементТаблицыСсылок из ТаблицаСсылок Цикл
		...
		Если ЭтоРегистрСведений Тогда
			ЭлементТаблицыСсылок.ДанныеСтрока = ЗначениеВСтрокуВнутр(ЭлементТаблицыСсылок.Данные);
