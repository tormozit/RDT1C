...
Процедура ПриОткрытии()
	...
	Тогда
		...
		ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПолеТекстаАлгоритма, "Выполнить").КнопкаПоУмолчанию = Истина;
	Иначе
...
Процедура ПараметрыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		Если ВыбраннаяСтрока.Имя = мИмяПеременнойЗамераСтрок Тогда
			ирОбщий.ОткрытьТаблицуЗначенийЛкс(ВыбраннаяСтрока.Значение, ВыбраннаяСтрока.Значение.Найти(ПолеТекстаАлгоритма.ПолеТекста.СтруктураДвумерногоВыделения().НачальнаяСтрока, "НомерСтроки"), Ложь,
	...
	ИначеЕсли Колонка.Имя = "ВариантыЗначения" Тогда 
		ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка, ВариантыЗначенияПараметра()); 
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(ДеревоАлгоритмовСтрокаПоиска) Тогда
			ВосстановитьТекущуюСтрокуВТексте = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(, ПолеТекстаАлгоритма.ПолеТекста, ДеревоАлгоритмовСтрокаПоиска, Ложь);
		КонецЕсли; 
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, АктивизироватьПервуюСтроку = Неопределено, Текст = Неопределено)
	...
	ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоАлгоритмов, Текст, "ТекстАлгоритма, Наименование", АктивизироватьПервуюСтроку <> Ложь,, Кнопки);
...
Процедура ВпередНажатие(Элемент)
	...
	ирОбщий.СледующееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.ДеревоАлгоритмов);
	
КонецПроцедуры

Процедура НазадНажатие(Элемент)
	
	ирОбщий.ПредыдущееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.ДеревоАлгоритмов);
...
Процедура ВыбранСпособВыполнения(Кнопка = Неопределено)
	...
	ирОбщий.УстановитьДоступностьПодменюЛкс(ЭлементыФормы.КоманднаяПанельПараметры.Кнопки.НаборыПараметров, Не ирОбщий.СтрНайтиЛкс(КонтекстВыполнения, "Фоновый"));
	#Если Сервер И Не Сервер Тогда
	    ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
...
Процедура ПараметрыВариантыЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка) 
	ирОбщий.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(Элемент, СтандартнаяОбработка, ВариантыЗначенияПараметра());
КонецПроцедуры
...
Процедура КоманднаяПанельПараметрыВыполнитьВарианты(Кнопка)
	...
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,, Ложь, "Алгоритм """ + мТекущаяСтрокаДереваАлгоритмов.Наименование + """ " + ТекущаяДата());
...
Функция ПодготовитьВыполнениеАлгоритмаПоНаборамПараметров(НаборыПараметров)
	...
	ФормаТаблицы = ирОбщий.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,, Ложь, "Выполнение наборов параметров",,, Истина);
...
Процедура КоманднаяПанельПараметрыВыполнитьТаблицуНаборов(Кнопка)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		КонецЕсли;
	КонецЦикла; 
	НаборыПараметров = ирОбщий.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,,, "Заполните наборы параметров",, Ложь);
...
Процедура КоманднаяПанельПараметрыЗагрузитьНабор(Кнопка)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		КонецЕсли;
	КонецЦикла; 
	СтрокаНабораПараметров = ирОбщий.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,,, "Выберите набор параметров", Истина, Ложь);
...
Процедура КоманднаяПанельПараметрыВыгрузитьНабор(Кнопка)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		КонецЕсли;
	КонецЦикла; 
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,,, "Набор параметров """ + мТекущаяСтрокаДереваАлгоритмов.Наименование + """",, Ложь);
...
Процедура КоманднаяПанельВстроенныйЯзыкВыполнитьСЗамером(Кнопка)
	...
	Для НомерСтроки = 1 По ТелоМетода.КоличествоСтрок() Цикл
		...
		Иначе
			СтрокаКодаСледующая = "";
		КонецЕсли;
		МассивСтрок.Добавить(СтрокаКода);
		Если Ложь
	...
	КонецЕсли;
	СтрокаПараметра.Выход = Истина;
	ТелоМетода = ирОбщий.СтрСоединитьЛкс(МассивСтрок, Символы.ПС);
	СтруктураПараметров = Новый Структура;
	...
	Если ЭлементыФормы.ПанельРедактора.ТекущаяСтраница = ЭлементыФормы.ПанельРедактора.Страницы.РедакторВстроенный Тогда
		...
		ирОбщий.ОткрытьТекстЛкс(ТелоМетодаСЗамером, "Замер - " + мТекущаяСтрокаДереваАлгоритмов.Наименование, "ВстроенныйЯзык", Истина);
	Иначе
