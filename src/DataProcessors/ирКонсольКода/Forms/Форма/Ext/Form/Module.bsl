...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если ЭтоВыполнениеКода Тогда
		Если Истина
			...
			И ПолеТекстаАлгоритма.ЧислоМетодов() > 0 
			И ЭлементыФормы.ДеревоАлгоритмов.Колонки.ЭтоМодуль.Видимость
		Тогда
			Ответ = Вопрос("В тексте алгоритма обнаружены методы. Включить режим ""Модуль"" в строке списка алгоритмов?
			|Не забудь прочитать подсказку к этой колонке.", РежимДиалогаВопрос.ДаНет);
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ЭлементыФормы.ДеревоАлгоритмов.ТекущаяКолонка = ЭлементыФормы.ДеревоАлгоритмов.Колонки.ЭтоМодуль;
				ЭлементыФормы.ДеревоАлгоритмов.ТекущиеДанные.ЭтоМодуль = Истина;
				ДеревоАлгоритмовЭтоМодульПриИзменении();
			КонецЕсли;
		КонецЕсли;
	...
	Если ЭтоПереходКОпределению Тогда
		ПерейтиКОпределениюВФорме(,, Истина, ТекущееВыражение);
	КонецЕсли;
КонецПроцедуры

Функция ПерейтиКОпределениюВФорме(Знач НомерСтроки = 0, Знач НомерКолонки = 0, Знач ЭтоПродолжениеОбработки = Ложь, Знач ТекущееВыражение = "") Экспорт
	Если ПустаяСтрока(ТекущееВыражение) Тогда
	...
	Если СтрокаПараметра <> Неопределено Тогда
		...
		Если Истина
			И ПолеТекстаАлгоритма.мНачальнаяСтрока = 1
			И ирОбщий.ЛиСсылкаНаОбъектБДЛкс(СтрокаПараметра.Значение) 
		Тогда 
			ПолеТекстаАлгоритма.ВернутьсяИзПерехода();
			ирКлиент.ОткрытьЗначениеЛкс(СтрокаПараметра.Значение);
		КонецЕсли;
	КонецЕсли;
...
Функция КлсПолеТекстаПрограммыОбновитьКонтекст(Знач Компонента = Неопределено, Знач Кнопка = Неопределено) Экспорт
	...
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		ОбщиеМодули = ирОбщий.СтруктураОсновныхОбщихМодулейЛкс();
	...
	КонецЕсли;
	Если Компонента.ЭтоМодуль Тогда
		ГлавныйМетод = Компонента.мМодульМетаданных.Методы.Найти(НРег(ИмяМетодаГлавный()), "НИмя");
		Если ГлавныйМетод <> Неопределено Тогда
			ГлавныйМетод.Параметры = мПлатформа.НоваяТаблицаПараметровМетодаМодуля();
		КонецЕсли;
	КонецЕсли;
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		Если Ложь
			...
			Или Компонента.мМетодМодуля = ГлавныйМетод
		Тогда
			Компонента.ДобавитьСловоЛокальногоКонтекста(СтрокаПараметра.Имя, , , ЗначениеПараметра, , ЗначениеПараметра, ТаблицаТипов);
		ИначеЕсли Истина
			И Компонента.ЭтоМодуль 
			И ГлавныйМетод <> Неопределено
		Тогда
			ПараметрМетода = ГлавныйМетод.Параметры.Добавить();
			ПараметрМетода.Вычислено = Истина;
			ПараметрМетода.Имя = СтрокаПараметра.Имя;
			ПараметрМетода.НИмя = СтрокаПараметра.НИмя;
			ПараметрМетода.ТипЗначения = СтрокаПараметра.ИмяТипаЗначения;
			Если ТаблицаТипов = Неопределено Тогда
				ТаблицаТипов = мПлатформа.ТаблицаТиповИзЗначения(ЗначениеПараметра);
			КонецЕсли;
			ПараметрМетода.ТаблицаТипов = ТаблицаТипов;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбеспечитьНаличиеГлавногоМетодаМодуля()
	ГлавныйМетод = ГлавныйМетод();
	Если Ложь
		Или ГлавныйМетод = Неопределено
		Или ирОбщий.СтрНачинаетсяСЛкс(СокрЛ(ПолеТекстаАлгоритма.ТекстМетодаСОписанием(ГлавныйМетод)), "//")
	Тогда
		ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("При включенном режиме ""Модуль"" текст должен начинаться с чистого стартового метода ""%1"". Поэтому он добавлен автоматически.", ИмяМетодаГлавный()));
...
Функция ПеречКонтекстВыполнения()
	Возврат ирОбщий.НовоеПеречислениеЛкс("Сервер, Фоновый, КлиентТолстый, ФормаТолстый, КлиентТонкий, ФормаТонкий");
КонецФункции

Процедура ПриОткрытии()
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма); 
	ЭтаФорма.КонтекстВыполнения = "КлиентТолстый"; //  Строка: см. ПеречКонтекстВыполнения
	ПолеТекстаАлгоритма = ирОбщий.НовыйАнализаторКодаЛкс();
	ПолеТекстаАлгоритма.Инициализировать(, ЭтаФорма, ЭлементыФормы.РедакторВстроенный, ЭлементыФормы.КоманднаяПанельВстроенныйЯзык,, "ВыполнитьЛокально", ЭтаФорма,,, Истина);
	...
	Если Ложь
		Или мРежимРедактора
		Или ЗначениеЗаполнено(мСписокВнешнихПараметров)
	...
	Тогда
		...
		Иначе
			...
			Для Каждого СтрокаВнешнегоПараметра Из мСписокВнешнихПараметров Цикл
				Если СтрокаВнешнегоПараметра.Фиксированный = Истина Тогда
		...
		ЭлементыФормы.ДеревоАлгоритмов.Колонки.ЭтоМодуль.Видимость = Не мРежимРедактора;
		//КонецЕсли;
	...
	КонецЕсли;
	ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.РедакторВстроенный; /// в 7.99 почему то активация статическим методом перестала выполняться
...
Процедура ПроверитьВнешнююОбработку()
	...
	Если Не ВнешняяОбработкаОтредактирована Или Не ЗначениеЗаполнено(ИмяФайлаВнешнейОбработки) Тогда
		ИмяФайлаВнешнейОбработки = ирОбщий.ФайлВнешнейОбработкиДляОтладкиЛкс(БазовоеИмяВнешнейОбработки(), Не ЛиКонтекстКлиент(), Ложь);
	КонецЕсли;
...
Функция СформироватьТелоМетода(ТекстДляВыполнения = Неопределено, Знач СтруктураПараметров = Неопределено, Знач ЛиСинтаксическийКонтроль = Ложь, Знач РежимВнешнейОбработки = Ложь)
	...
	ТелоМетода = ТелоМетода + "
		...
		|	// Двойная сериализация нужна для полного удаления неудобных типов. Иначе возможны сложные проблемы https://github.com/tormozit/RDT1C/issues/707
		|	ХранилищеОбщихНастроек.Сохранить(""" + ирКэш.ИмяПродукта() + """, ""КонсольКода_ЗначенияПараметровИсключения"", ЗначениеВСтрокуВнутр(ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(_АлгоритмОбъект)))); 
		|	ВызватьИсключение;
		|КонецПопытки;
...
Функция ВыполнитьЛокально(Знач ТекстДляВыполнения, Знач ЛиСинтаксическийКонтроль = Ложь, выхИнформацияОбОшибке = Неопределено) Экспорт
	...
	Тогда
		...
		КонецЕсли;
	Иначе
		ТекстДляВыполнения = СформироватьТелоМетода(ТекстДляВыполнения, СтруктураПараметров, ЛиСинтаксическийКонтроль);
	КонецЕсли;
	Если Не ЛиСинтаксическийКонтроль Тогда
...
Функция ВыполнитьЛокальноПоСтруктуреПараметров(Знач ТекстДляВыполнения, Знач СтруктураПараметров, Знач ЛиСинтаксическийКонтроль = Ложь, Знач ВерсияАлгоритма = Неопределено,
	...
	ПараметрыЗадания = НовыеПараметрыВыполненияАлгоритмаВКонтексте();
	ПараметрыЗадания.ТекстДляВыполнения = ТекстДляВыполнения;
	ПараметрыЗадания.СтруктураПараметров = СтруктураПараметров;
	ПараметрыЗадания.ВыполнятьНаСервере = Не ЛиКонтекстКлиент();
	ПараметрыЗадания.ЧерезВнешнююОбработку  = Ложь
			Или ПолеТекстаАлгоритма.ЭтоМодуль
			Или РежимВнешнейОбработки И Не ЛиСинтаксическийКонтроль;
	ПараметрыЗадания.ИмяФайлаВнешнейОбработки = ИмяФайлаВнешнейОбработки;
	ПараметрыЗадания.ЛиСинтаксическийКонтроль = ЛиСинтаксическийКонтроль;
	ПараметрыЗадания.ВерсияАлгоритма = ВерсияАлгоритма;
	ПараметрыЗадания.ЛиМодульОтФормы = ЛиМодульОтФормы();
...
Функция ЛиКонтекстКлиент() Экспорт
	Возврат ирОбщий.СтрНачинаетсяСЛкс(КонтекстВыполнения, "Клиент") Или ирОбщий.СтрНачинаетсяСЛкс(КонтекстВыполнения, "Форма");
КонецФункции
...
Процедура ОсновныеДействияФормыПрименить(Кнопка = Неопределено)
	...
	Если Открыта() Тогда
		ЭтаФорма.Модифицированность = СтараяМодифицированность;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПараметры(Знач ПараметрыВхода = Истина, Знач ПараметрыВыхода = Истина, Знач РазобратьТекущийКонтекст = Истина)
	ПолеТекстаАлгоритма.мЯзыкПрограммы = 0;
	...
	Если ПараметрыВхода Тогда
		...
		Пока Истина Цикл
			...
			Если СтрокаПараметра = Неопределено Тогда
				...
				СтрокаПараметра.НИмя = НРег(СтрокаПараметра.Имя);
				ТаблицаТипов = ПолеТекстаАлгоритма.РассчитатьОжидаемыеТипыПеременной(НеопределеннаяПеременная);
				Если ЗначениеЗаполнено(ТаблицаТипов) Тогда
					СтрокаПараметра.Значение = мПлатформа.ОписаниеТиповИзТаблицыТипов(ТаблицаТипов).ПривестиЗначение();
				КонецЕсли;
	...
	Если ПараметрыВыхода Тогда
		ТелоГлавногоМетода = ТелоГлавногоМетода();
...
Процедура ПараметрыИмяПриИзменении(Элемент)
	Элемент.Значение = ПолеТекстаАлгоритма.АнализаторСПолнойТаблицейСловМетода().НовоеИмяПеременной(Элемент.Значение);
	СтрокаПараметра = ЭлементыФормы.Параметры.ТекущиеДанные;
	СтрокаПараметра.НИмя = Нрег(Элемент.Значение);
	Если Ложь
		Или СтрокаПараметра.Значение = Неопределено 
		Или СтрокаПараметра.Значение = ""
	Тогда
		ТипЗначения = ирОбщий.ОписаниеТиповИзИмениПеременнойЛкс(СтрокаПараметра.Имя);
		Если ТипЗначения = Неопределено Тогда
			ТипЗначения = Новый ОписаниеТипов("Строка");
		КонецЕсли;
		СтрокаПараметра.Значение = ТипЗначения.ПривестиЗначение();
		ОбновитьПредставлениеИТипЗначенияВСтроке(СтрокаПараметра);
	КонецЕсли;
	ПодключитьОбновлениеКонтекстаОтложенно();
КонецПроцедуры
...
Процедура ОбновитьФайлВнешнейОбработки(Знач СтруктураПараметров = Неопределено)
	...
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		...
		Если АвтоПараметрыВыхода Тогда
			ЗаполнитьПараметры(Ложь, Истина, Ложь);
		КонецЕсли;
		ТелоМетода = Сред(ПолеТекстаАлгоритма.ТелоМетода(ГлавныйМетод,, Ложь), 2); // - отрезаем начальный разделитель строки
	КонецЕсли;
	ОбщиеМодули = ирОбщий.СтруктураОсновныхОбщихМодулейЛкс();
	...
	ТекстМодуля = ТекстМодуля + "
		|" + СформироватьТелоМетода(ТелоМетода, СтруктураПараметров,, Истина) + "
		|КонецПроцедуры
		|";
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		Если ЛиМодульОтФормы() Тогда
			ТекстМодуля = "&НаКлиенте" + Символы.ПС + ТекстМодуля;
			мСмещениеНомеровСтрок = мСмещениеНомеровСтрок + 1;
		КонецЕсли;
		...
		ПозицияКонцаГлавногоМетода = ГлавныйМетод.ПозицияСОписанием + ГлавныйМетод.ДлинаСОписанием;
		// Вычитаем высоту заголовка главного метода
		мСмещениеНомеровСтрок = мСмещениеНомеровСтрок - Макс(0, ирОбщий.СтрЧислоСтрокЛкс(Лев(Текст, ГлавныйМетод.ПозицияТела - 1)) - 1);
		//мСмещениеНомеровСтрокДоп = мСмещениеНомеровСтрокДопПорог + 1 - ирОбщий.СтрЧислоСтрокЛкс(Лев(Текст, ПозицияКонцаГлавногоМетода));
		мСмещениеНомеровСтрокДоп = мСмещениеНомеровСтрокДопПорог - ирОбщий.СтрЧислоСтрокЛкс(Лев(Текст, ПозицияКонцаГлавногоМетода));
		ТекстМодуля = ТекстМодуля + Сред(Текст, ПозицияКонцаГлавногоМетода + 1);
	Иначе
		мСмещениеНомеровСтрок = мСмещениеНомеровСтрок + 1;
	КонецЕсли;
	...
	ТекстМодуля = ТекстМодуля + "
		...
		|КонецФункции
		|";
	Если Не ирОбщий.ОбновитьМодульВнешнейОбработкиДляОтладкиЛкс(БазовоеИмяВнешнейОбработки(), ИмяВнешнейОбработки(), ТекстМодуля, ТекстМодуляВКонсолиНеМенялся, ДатаИзмененияВнешнейОбработки,, ЛиМодульОтФормы()) Тогда
		Возврат;
	КонецЕсли;
	ТекстМодуляТекущейВнешнейОбработки = ТекстМодуля;
	ИмяФайлаВнешнейОбработки = ирОбщий.ФайлВнешнейОбработкиДляОтладкиЛкс(БазовоеИмяВнешнейОбработки(), Не ЛиКонтекстКлиент(), Истина);
	Если ПустаяСтрока(ИмяФайлаВнешнейОбработки) Тогда
		Возврат;
	КонецЕсли;
...
Процедура КоманднаяПанельВстроенныйЯзыкОткрытьВОтладчике(Кнопка)
	...
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		ЧислоСтрокГлавного = ирОбщий.СтрЧислоСтрокЛкс(ПолеТекстаАлгоритма.ТекстМетодаСОписанием(ГлавныйМетод()));
	...
	Если ИмяФайлаВнешнейОбработки <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторОтладчика) Тогда
			мПлатформа.ОткрытьМодульВнешнейОбработкиВКонфигураторе(ИмяФайлаВнешнейОбработки, НомерСтрокиВМодуле, ИдентификаторОтладчика,, БазовоеИмяВнешнейОбработки(), ЛиМодульОтФормы());
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьИзФайла(ИмяФайла = "", НовыйФормат = Истина)
	...
	ЭтаФорма.Модифицированность = РежимОбъединения Или мРежимРедактора;
	...
	Если Не ДанныеЗагружены Тогда
		СоздатьФайл();
		ирОбщий.СообщитьЛкс("Невозможно загрузить список алгоритмов из указанного файла! Создан новый файл", СтатусСообщения.Важное,, Истина);
	КонецЕсли;
...
Функция СохранитьВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, ИмяФайла = Неопределено, СброситьМодифицированность = Неопределено)
	...
	Для Каждого СтрокаКопииДерева Из ВсеСтрокиКопииДерева Цикл
		...
		КонецЕсли;
		Для Каждого СтрокаПараметра Из СтрокаКопииДерева.Параметры Цикл
			...
			Если ЦиклическиеСсылки.Количество() > 0 Тогда
				ирОбщий.СообщитьЛкс("Сохранение файла не выполнено из-за наличия циклической ссылки в алгоритме """ + СтрокаКопииДерева.Наименование + """ параметре """ + СтрокаПараметра.Имя + """ - "
						+ ЦиклическиеСсылки[0].ЭлементыЦиклическихСсылок[0], СтатусСообщения.Внимание);
				Возврат Ложь;
			КонецЕсли;
			Если Ложь
				Или ТипЗнч(СтрокаПараметра.Значение) = Тип("РезультатЗапроса") 
				Или (Истина
					И ТипЗнч(СтрокаПараметра.Значение) = Тип("Массив") 
					И СтрокаПараметра.Значение.Количество()
					И ТипЗнч(СтрокаПараметра.Значение[0]) = Тип("РезультатЗапроса")) 
			Тогда
				// Внутренняя сериализация полностью сохраняет результат запроса (все полученные данные)
				СтрокаПараметра.Значение = Неопределено;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура КоманднаяПанельВстроенныйЯзыкЗамераЦикла(Кнопка)
	...
	ПолеТекста.ВставитьТекст(
			...
			|Для Счетчик = 1 По КоличествоПроходов Цикл" + ирОбщий.ДобавитьТекстВТекстЛкс("", Символы.ПС + ПолеТекста.ВыделенныйТекст(), Символы.Таб) + "
			|КонецЦикла;
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		...
		Если ИсходнаяТаблицаПараметров <> Неопределено Тогда
			Для Каждого ПараметрыЭлемент Из Параметры Цикл
				СтрокаИсточник = ИсходнаяТаблицаПараметров.Найти(ПараметрыЭлемент.НИмя, "НИмя");
				Если СтрокаИсточник <> Неопределено Тогда
					//ЗаполнитьЗначенияСвойств(ПараметрыЭлемент, СтрокаИсточник, "Значение");
					ИсходнаяТаблицаПараметров.Удалить(СтрокаИсточник);
				КонецЕсли;
			КонецЦикла;
			Если ИсходнаяТаблицаПараметров.Колонки.Найти("Фиксированный") <> Неопределено Тогда
				ИсходнаяТаблицаПараметров.ЗаполнитьЗначения(Ложь, "Фиксированный");
			КонецЕсли;
...
Процедура ОчиститьПараметры() Экспорт
	ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюЛкс(Параметры, "Фиксированный", Ложь);
КонецПроцедуры

Процедура УстановитьТекстПоляАлгоритма(Текст = Неопределено)
	Если ЛиМодульОтФормы() Тогда
		ИмяМодуля = мПлатформа.ИмяДинамическойФормы() + ".Форма.Модуль";
	КонецЕсли;
	Если Текст <> Неопределено Тогда
		НепечатныйКонец = Прав(Текст, СтрДлина(Текст) - СтрДлина(СокрП(Текст)));
		Текст = Текст + ирОбщий.СтрокаПовторомЛкс(Символы.ПС, 10 - ирОбщий.СтрЧислоСтрокЛкс(НепечатныйКонец)); /// Добавляем вспомогательные пустые строки для удобства отображения окон подсказок
	КонецЕсли;
	ПолеТекстаАлгоритма.УстановитьТекст(Текст,, Текст,,, ИмяМодуля);
КонецПроцедуры
...
Функция ВычислитьВыражение(ТекущееВыражение = "", УспехЗначения = Истина, ДляПодсказки = Истина) Экспорт
	...
	КонецЦикла;
	ВычисляемоеВыражение = ТекущееВыражение;
	ПерваяСтрока = СокрЛП(НРег(СтрПолучитьСтроку(ТекущееВыражение, 1)));
	Если Лев(ПерваяСтрока, 1) = "(" Тогда
		ПерваяСтрока = СокрЛ(Сред(ПерваяСтрока, 2));
	КонецЕсли;
	ЛиИерархическоеЛогическоеВыражение = Истина
		И Найти(ТекущееВыражение, Символы.ПС) > 0
		И (Ложь
			Или ПерваяСтрока = "истина"
			Или ПерваяСтрока = "ложь");
	Если ЛиИерархическоеЛогическоеВыражение Тогда
		ВычисляемоеВыражение = "Вычислить(ирОбщий.РасшифровкаИерархическогоЛогическогоВыраженияЛкс("+ ирОбщий.ТекстВВыражениеВстроенногоЯзыкаЛкс(ВычисляемоеВыражение)+"))"; 
	КонецЕсли;	
	АлгоритмВычисления = АлгоритмВычисления + " _АлгоритмОбъект.Результат = " + ВычисляемоеВыражение + ";";
	ПараметрыЗапуска = НовыеПараметрыВыполненияАлгоритмаВКонтексте();
	ПараметрыЗапуска.ТекстДляВыполнения = АлгоритмВычисления;
	ПараметрыЗапуска.СтруктураПараметров = СтруктураПараметров;
	ПараметрыЗапуска.ВыполнятьНаСервере = Не ДляПодсказки И Не ЛиКонтекстКлиент;
	ПараметрыЗапуска.ЛиМодульОтФормы = ЛиМодульОтФормы();
	Попытка
...
Процедура КоманднаяПанельВстроенныйЯзыкВОднострочныйВид(Кнопка)
	...
	Для Счетчик = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Лев(СокрЛ(СтрокаТекста), 1) = "|" Тогда
			ирОбщий.СообщитьЛкс("В однострочном коде не допускаются многострочные текстовые литералы", СтатусСообщения.Внимание);
...
Функция КомментарийОднострочногоВида()
	Возврат "// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах."
		+ " Преобразовано консолью кода ИР.";
КонецФункции
...
Процедура ВыбранСпособВыполнения(Кнопка = Неопределено)
	...
	УстановитьТекстПоляАлгоритма();
	ФлагиКомпиляции = мПлатформа.НовыеФлагиКомпиляции(Не ЛиКонтекстКлиент() Или ирОбщий.СтрНачинаетсяСЛкс(КонтекстВыполнения, "Форма"),
		КонтекстВыполнения = "КлиентТолстый" Или КонтекстВыполнения = "ФормаТолстый",
		КонтекстВыполнения = "КлиентТонкий" Или КонтекстВыполнения = "ФормаТонкий");
	ПолеТекстаАлгоритма.УстановитьФлагиКомпиляцииМодуля(ФлагиКомпиляции);
...
Процедура ОбновитьДоступностьКнопок()
	...
	ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.КонтекстВыполнения.Кнопки.ФормаТолстый.Доступность = ПолеТекстаАлгоритма.ЭтоМодуль И ирКэш.ЛиСеансТолстогоКлиентаУПЛкс();
	ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.КонтекстВыполнения.Кнопки.ФормаТонкий.Доступность = ПолеТекстаАлгоритма.ЭтоМодуль И ирКэш.ЛиСеансТолстогоКлиентаУПЛкс();
...
Функция СравнитьЗагрузитьИзмененияИзВнешнейОбработки(ПоказыватьСравнение = Истина)
	...
	Если мТекущаяСтрокаДереваАлгоритмов.Наименование = ИнфоФайла.ИдентификаторАлгоритма Тогда
		ТекстМодуля = мПлатформа.ТекстМодуляСгенерированнойВнешнейОбработки(Новый Файл(ИмяФайлаВнешнейОбработки), ЛиМодульОтФормы());
		АнализаторКодаФайла = ирОбщий.НовыйАнализаторКодаЛкс(0);
		...
		ГлавныйМетод = ГлавныйМетод();
		Если ГлавныйМетод <> Неопределено Тогда
			ТекстИзОбработки = ирОбщий.ТекстМеждуМаркерамиЛкс(АнализаторКодаФайла.ТелоМетода(ГлавныйМетод), мМаркерНачалаАлгоритма + Символы.ПС, Символы.ПС + мМаркерКонцаАлгоритма, Ложь);
			Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
...
Функция ЛиМодульОтФормы() Экспорт
	Возврат ирОбщий.СтрНачинаетсяСЛкс(КонтекстВыполнения, "Форма");
КонецФункции
...
Процедура КоманднаяПанельВстроенныйЯзыкАдаптироватьТекстИзМетода(Кнопка)
	...
	ГлавныйМетод = ГлавныйМетод();
	Если ГлавныйМетод <> Неопределено Тогда
		мТекущаяСтрокаДереваАлгоритмов.Наименование = ГлавныйМетод.Имя;
		Если ГлавныйМетод.Параметры <> Неопределено Тогда
			Для Каждого СтрокаПараметраМетода Из ГлавныйМетод.Параметры Цикл
			...
			КонецЦикла;
		КонецЕсли;
		УстановитьТекстАлгоритмаСВозможностьюОтмены(ПолеТекстаАлгоритма.ТелоМетода(ГлавныйМетод));
	КонецЕсли;
КонецПроцедуры
...
Процедура КоманднаяПанельВстроенныйЯзыкЗаполнитьОбъект(Кнопка)
	...
	СтрокиТекста.Добавить(ИмяПеременной + " = " + ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(ФрагментыИмени[0]) + "." + ФрагментыИмени[1] + "." + ИмяМетода + "();");
	ПолноеИмяТаблицы = ПолноеИмяМД;
	СоздатьПрисвоенияРеквизитамТЧ(ИмяПеременной, ЗапрещенныеИмена, ПолноеИмяТаблицы, СтрокиТекста);
	Для Каждого ОписаниеТЧ Из СтруктураТЧ Цикл
		...
		СоздатьПрисвоенияРеквизитамТЧ(ИмяПеременнойТЧ, ЗапрещенныеИмена, ПолноеИмяТаблицы + "." + ИмяТЧ, СтрокиТекста);
	КонецЦикла;
...
Процедура СоздатьПрисвоенияРеквизитамТЧ(Знач ИмяПеременнойТЧ, Знач ЗапрещенныеИмена, Знач ПолноеИмяТаблицы, Знач СтрокиТекста)  
...
Процедура ПараметрыВариантыЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	ирКлиент.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка, ВариантыЗначенияПараметра());
КонецПроцедуры
...
Процедура КоманднаяПанельПараметрыВыполнитьВарианты(Кнопка)
	...
	НаборыПараметров = ФормаТаблицы.Коллекция; // см. ПодготовитьТаблицуРезультатовНаборовПараметров
...
//.
// Параметры:
//   НаборыПараметров - ТаблицаЗначений - 
// Возвращаемое значение:
//   Форма - :
//    *Коллекция - см. ПодготовитьТаблицуРезультатовНаборовПараметров - анализатор кода пока не поддерживает это для обычных форм
Функция ПодготовитьВыполнениеАлгоритмаПоНаборамПараметров(НаборыПараметров)
	НаборыПараметров = ПодготовитьТаблицуРезультатовНаборовПараметров(НаборыПараметров);
...
// Возвращаемое значение:
//   ТаблицаЗначений - 
Функция ПодготовитьТаблицуРезультатовНаборовПараметров(НаборыПараметров) Экспорт
	Если АвтоПараметрыВыхода Тогда
...
Процедура ВыполнитьАлгоритмПоНаборуПараметров(Знач СтрокаНабора, Знач СтруктураПараметров = Неопределено, Знач ТекстДляВыполнения = "", Знач ФормаТаблицы = Неопределено)  Экспорт
	Если СтруктураПараметров = Неопределено Или ТекстДляВыполнения = "" Тогда
		СтруктураПараметров = Новый Структура;
		ТекстДляВыполнения = СформироватьТелоМетода(, СтруктураПараметров);
	КонецЕсли;
...
Процедура КоманднаяПанельПараметрыВыполнитьТаблицуНаборов(Кнопка)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		КонецЕсли;
	КонецЦикла;
	НаборыПараметров = ирКлиент.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,,, "Заполните наборы параметров",, Ложь); // ТаблицаЗначений
...
Процедура КоманднаяПанельПараметрыЗагрузитьНабор(Кнопка)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		КонецЕсли;
	КонецЦикла;
	СтрокаНабораПараметров = ирКлиент.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,,, "Выберите набор параметров", Истина, Ложь); // СтрокаТаблицыЗначений
...
Процедура КоманднаяПанельВстроенныйЯзыкВыполнитьСЗамером(Кнопка)
	...
	СтруктураПараметров = Новый Структура;
	ТелоМетода = СформироватьТелоМетода(ТелоМетода, СтруктураПараметров);
...
Функция ТелоГлавногоМетода()
	ИмяГлавногоМетода = ИмяМетодаГлавный();
	ТекстАлгоритма = ПолеТекстаАлгоритма.ПолеТекста.ПолучитьТекст();
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		ГлавныйМетод = ГлавныйМетод();
		Если ГлавныйМетод <> Неопределено Тогда
			ТекстАлгоритма = ПолеТекстаАлгоритма.ТелоМетода(ГлавныйМетод,, Ложь);
		КонецЕсли;
	КонецЕсли;
	Возврат ТекстАлгоритма;
КонецФункции

Процедура ДеревоАлгоритмовЭтоМодульПриИзменении(Элемент = Неопределено)
	ОбновитьСтруктуруАлгоритма();
	ОбновитьДоступностьКнопок();
КонецПроцедуры
...
Процедура ОбновитьСтруктуруАлгоритма()
	...
	#КонецЕсли
	ПолеТекстаАлгоритма.РазобратьТекущийКонтекст();
	ЧислоМетодов = ПолеТекстаАлгоритма.ЧислоМетодов();
	ВыделениеДвумерное = ПолеТекста.ВыделениеДвумерное();
	СдвигНомераСтроки = 0;
	ПолеТекстаАлгоритма.ЭтоМодуль = мТекущаяСтрокаДереваАлгоритмов.ЭтоМодуль;
	ГлавныйМетод = ГлавныйМетод();
	Если мТекущаяСтрокаДереваАлгоритмов.ЭтоМодуль Тогда
		ТелоИнициации = ПолеТекстаАлгоритма.ТелоМетода(Неопределено);
		Если ЧислоМетодов = 0 Тогда
			НовыйТекст = ОпределениеГлавногоМетодаИзТела();
			СдвигНомераСтроки = +1;
		ИначеЕсли ЗначениеЗаполнено(ТелоИнициации) Тогда
			...
			ПолеТекста.ВыделенныйТекст(ОпределениеГлавногоМетодаИзТела(СокрП(ТелоИнициации)) + Символы.ПС);
		...
		Тогда
			ПолеТекста.УстановитьГраницыВыделения(1, 1);
			ПолеТекста.ВыделенныйТекст(ОпределениеГлавногоМетодаИзТела("	ВызватьИсключение ""Укажите тело главного метода"";") + Символы.ПС);
			ПолеТекста.УстановитьГраницыВыделения(2, 1, 2, 20);
		Иначе
			Возврат;
		КонецЕсли;
	Иначе
		Если ЧислоМетодов > 2 Тогда
			Ответ = Вопрос("Модуль содержит другие методы кроме " + ИмяМетодаГлавный() + "(). Они будут удалены. Продолжить?", РежимДиалогаВопрос.ОКОтмена);
			Если Ответ <> КодВозвратаДиалога.ОК Тогда
		...
		КонецЕсли;
		Если ГлавныйМетод <> Неопределено Тогда
			НовыйТекст = СокрЛП(ПолеТекстаАлгоритма.ТелоМетода(ГлавныйМетод,, Ложь));
			СдвигНомераСтроки = -1;
		Иначе
...
//.
// Параметры:
//   Методы - Произвольный, ТаблицаЗначений - 
// Возвращаемое значение:
//   СтрокаТаблицыЗначений, Неопределено - 
Функция ГлавныйМетод() Экспорт
	ГлавныйМетод = ПолеТекстаАлгоритма.мМодульМетаданных.Методы.Найти(НРег(ИмяМетодаГлавный()), "НИмя");
	Возврат ГлавныйМетод;
КонецФункции
	...
	Тогда
		...
		Для Каждого ВнешнийПараметр Из мСписокВнешнихПараметров Цикл
			...
			Если Истина
				...
				И СтрокаПараметра.Значение <> ВнешнийПараметр.Значение
			Тогда
				СтрокаПараметра.Значение = ВнешнийПараметр.Значение;
				ОбновитьПредставлениеИТипЗначенияВСтроке(СтрокаПараметра);
...
ДеревоАлгоритмов.Колонки.Добавить("Параметры"); // см. ЭлементыФормы.Параметры.Значение
