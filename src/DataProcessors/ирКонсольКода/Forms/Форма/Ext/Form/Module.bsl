...
Процедура ПараметрыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		Если ВыбраннаяСтрока.Имя = мИмяПеременнойЗамераСтрок Тогда
			ирКлиент.ОткрытьТаблицуЗначенийЛкс(ВыбраннаяСтрока.Значение, ВыбраннаяСтрока.Значение.Найти(ПолеТекстаАлгоритма.ПолеТекста.ДвухмерноеВыделение().НачальнаяСтрока, "НомерСтроки"), Ложь,
...
Процедура КоманднаяПанельВстроенныйЯзыкПереименоватьСлово(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
	    ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ПолеТекстаАлгоритма.Нажатие(ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПолеТекстаАлгоритма, Кнопка.Имя));
