...
Процедура ОбновитьЗначенияПараметровВыхода(СтруктураПараметров, РазрешитьДесериализацию = Истина)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		Если СтрокаПараметра.Выход Тогда
			...
			Если РазрешитьДесериализацию И КонтекстВыполнения <> "Клиент" Тогда
				Если ТипЗнч(СтрокаПараметра.Значение) = Тип("Тип") Тогда
					ирОбщий.ОбновитьТипЗначенияВСтрокеТаблицыЛкс(СтрокаПараметра,,,,,,, СтрокаПараметра.Значение);
			...
			Если СтрокаПараметра.Выход Тогда
				Если Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
					ТекстВыхода = ТекстВыхода + "
						...
						|		_АлгоритмОбъект." + СтрокаПараметра.Имя + " = ТипЗнч(" + СтрокаПараметра.Имя + ");
