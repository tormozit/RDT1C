...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если ЭтоВыполнениеКода Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	ПолеТекстаАлгоритма.Нажатие(Кнопка, ПолеТекстаАлгоритма.ЭтоМодуль);
...
Функция КлсПолеТекстаПрограммыОбновитьКонтекст(Знач Компонента = Неопределено, Знач Кнопка = Неопределено) Экспорт 
	...
	Если Компонента.ЭтоМодуль Тогда
		Компонента.РазобратьТекущийКонтекст();
	КонецЕсли;
	ЭтоВыполнениеКода = ЭтоКнопкаВыполненияКода(Кнопка);
	Для Каждого СтрокаПараметра Из Параметры Цикл
		Если Ложь
			Или Не Компонента.ЭтоМодуль
			Или ирОбщий.СтрокиРавныЛкс(Компонента.мМетодМодуля.Имя, ИмяГлавногоМетода())
...
Процедура ПриОткрытии()
	...
	ПолеТекстаАлгоритма = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
	#Если Сервер И Не Сервер Тогда
	    ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
...
Функция ВыполнитьЛокальноПоСтруктуреПараметров(Знач ТекстДляВыполнения, Знач СтруктураПараметров, Знач ЛиСинтаксическийКонтроль = Ложь, Знач ВерсияАлгоритма = Неопределено,
	...
	Исключение
		...
		ИнформацияОбОшибке = ОбработатьИсключениеАлгоритма(ВремяНачала, ИнформацияОбОшибке, ЛиСинтаксическийКонтроль, СтруктураПараметров);
		Если Истина
			//И Не ЛиСинтаксическийКонтроль
			И РежимВнешнейОбработки() 
		Тогда
...
Функция ОбработатьИсключениеАлгоритма(Знач ВремяНачала, ИнформацияОбОшибке, Знач ЛиСинтаксическийКонтроль, Знач СтруктураПараметров = Неопределено)
	...
	Если ИнформацияОбОшибке.Причина <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли; 
	Если Не ЛиСинтаксическийКонтроль Или ПолеТекстаАлгоритма.ЭтоМодуль Тогда 
		Если РежимВнешнейОбработки() Тогда 
...
Процедура ЗаполнитьПараметры(Знач ПараметрыВхода = Истина, Знач ПараметрыВыхода = Истина, Знач ОбновитьМодульМетаданных = Истина)
	...
	Если ОбновитьМодульМетаданных Тогда
		ПолеТекстаАлгоритма.ОбновитьМодульМетаданных();
	КонецЕсли;
	...
	Если ПараметрыВыхода Тогда
		...
		ПолеТекстаАлгоритма.ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(,,,, Истина, ТелоГлавногоМетода, Истина);
...
Функция ОбновитьФайлВнешнейОбработки(Знач СтруктураПараметров = Неопределено)
	...
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		Пока ПолеТекста.КоличествоСтрок() > 1 И ПустаяСтрока(ПолеТекста.ПолучитьСтроку(1)) Цикл
			...
			Исключение
				// HTML редактор пока не поддерживает
				Прервать;
			КонецПопытки;
		КонецЦикла;  
		...
		Если Ложь
			...
			Или ирОбщий.СтрНачинаетсяСЛкс(СокрЛ(Методы[0].ОпределениеСОписанием), "//")
		Тогда
			ВызватьИсключение ирОбщий.СтрШаблонЛкс("При включенном флаге ""Это модуль"" текст должен начинаться с чистого определения метода ""%1""", ИмяГлавногоМетода);
		КонецЕсли;
		ТелоМетода = Методы[0].Тело;
	КонецЕсли;
	ОбщиеМодули = ирОбщий.ПолучитьСтруктуруОсновныхОбщихМодулейЛкс();
	...
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		...
		Для Каждого Метод Из Методы Цикл
			Если ирОбщий.СтрокиРавныЛкс(ИмяГлавногоМетода, Метод.Имя) Тогда
				// Вычитаем высоту заголовка главного метода
				мСмещениеНомеровСтрок = мСмещениеНомеровСтрок - Макс(0, ирОбщий.СтрЧислоСтрокЛкс(Лев(ПолеТекста.ПолучитьТекст(), Метод.ПозицияТела - 1)) - 1);
				Продолжить;
			КонецЕсли;     
			Если мСмещениеНомеровСтрокДоп = 0 Тогда
				мСмещениеНомеровСтрокДоп = мСмещениеНомеровСтрокДопПорог + 1 - (ирОбщий.СтрЧислоСтрокЛкс(Лев(ПолеТекста.ПолучитьТекст(), Метод.ПозицияОпределения)) - ирОбщий.СтрЧислоСтрокЛкс(Метод.Описание));
			КонецЕсли;
			ТекстМодуля = ТекстМодуля + Символы.ПС + Метод.ОпределениеСОписанием + Символы.ПС;
		КонецЦикла;
	КонецЕсли;
	...
	|КонецФункции
	|";
	мСмещениеНомеровСтрок = мСмещениеНомеровСтрок + 2;
...
Функция ПеременныеМодуляОтПараметров()
	...
	Возврат ирОбщий.ОтобратьКоллекциюУсловиемЛкс(Параметры, "Э.Модуль И Э.Вход", "Имя");
...
Процедура КоманднаяПанельВстроенныйЯзыкОткрытьВОтладчике(Кнопка)
	...
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		ЧислоСтрокГлавного = ирОбщий.СтрЧислоСтрокЛкс(ПолеТекстаАлгоритма.мМодульМетаданных.Методы[0].ОпределениеСОписанием);
...
Функция ЗаменитьВозвратыНаПерейтиВТекстеМетода1С(Знач Текст, ИмяПеременнойРезультата = "Результат") Экспорт
	...
	ОбработкаРегулярныхВыражений.MultiLine = Ложь;
...
Процедура КоманднаяПанельВстроенныйЯзыкВОднострочныйВид(Кнопка)
	...
	Если СтрДлина(АктивноеПолеТекста.ВыделенныйТекст()) = 0 Тогда
		ВесьТекст = Истина;
		Текст = ПолеТекстаАлгоритма.ПолучитьТекст();
...
Процедура ОбновитьДоступностьКнопок()
	...
	ЭлементыФормы.КоманднаяПанельВстроенныйЯзык.Кнопки.СписокМетодов.Доступность = ПолеТекстаАлгоритма.ЭтоМодуль;
...
Функция СравнитьЗагрузитьИзмененияИзВнешнейОбработки(ПоказыватьСравнение = Истина)
	...
	СлужебныйФайл = Новый Файл(ИмяСлужебногоФайла());
	ИнфоФайла = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(СлужебныйФайл.ПолноеИмя);
	ПолеТекстаАлгоритма.мМодульМетаданных = Неопределено;
	...
	Если мТекущаяСтрокаДереваАлгоритмов.Наименование = ИнфоФайла.ИдентификаторАлгоритма Тогда
		...
		МодульМетаданных = мПлатформа.МодульМетаданных(ТекстМодуля, Ложь, Истина);
		ТекстИзОбработки = ирОбщий.СтрокаМеждуМаркерамиЛкс(МодульМетаданных.Методы[0].Тело, мМаркерНачалаАлгоритма + Символы.ПС, Символы.ПС + мМаркерКонцаАлгоритма, Ложь);
		Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		...
		ТекстИзОбработки = ТекстИзОбработки + ирОбщий.СтрСоединитьЛкс(МодульМетаданных.Методы.ВыгрузитьКолонку("ОпределениеСОписанием"), Символы.ПС + Символы.ПС) + Символы.ПС;
	...
	Результат = Истина;
~Выход: 
	Если ПолеТекстаАлгоритма.мМодульМетаданных = Неопределено Тогда
		ПолеТекстаАлгоритма.ОбновитьМодульМетаданных();
	КонецЕсли;
	Возврат Результат;
...
Процедура КоманднаяПанельВстроенныйЯзыкВыделитьМетод(Кнопка)
	...
	НевидимоеПолеТекста.УстановитьТекст(ПолеТекстаАлгоритма.ТелоАктивногоМетода(НомераПервойСтрокиТела));
	СмещениеНомеровСтрок = НомераПервойСтрокиТела - 1;
	Если Не ЗначениеЗаполнено(ПолеТекстаАлгоритма.ВыделенныйТекст()) Тогда
		Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
			ПолеТекстаАлгоритма.ПолеТекста.УстановитьГраницыВыделения(ПолеТекстаАлгоритма.мПерваяСтрокаТелаМетодаМодуля, 1, ПолеТекстаАлгоритма.мПоследняяСтрокаТелаМетодаМодуля + 1, 1);
		Иначе
	...
	Для Каждого СтрокаПараметра Из ФормаНовогоМетода.ТаблицаПараметров Цикл
		...
		Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
			ПараметрыМетода = ПолеТекстаАлгоритма.мМетодМодуля.Параметры;
			Если ПараметрыМетода <> Неопределено Тогда
				СтрокаПараметраМетода = ПараметрыМетода.Найти(НРег(СтрокаПараметра.Имя), "НИмя");
				Если СтрокаПараметраМетода <> Неопределено Тогда
					СтрокаПараметра.Комментарий = СтрокаПараметраМетода.Описание;
					СтрокаПараметра.ТипЗначения = СтрокаПараметраМетода.ТипЗначения;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Истина
			...
			И (Ложь
				Или Не ПолеТекстаАлгоритма.ЭтоМодуль
		...
		Тогда
			...
			СтрокаПараметра.Модуль = СтрокаЭтогоАлгоритмаПараметры.Модуль;
		...
		Если Не ЗначениеЗаполнено(СтрокаПараметра.ТипЗначения) Тогда
			СтрокаПараметра.ТипЗначения = мПлатформа.ПредставлениеМассиваСтруктурТипов(ПолеТекстаАлгоритма.ОпределитьТипЗначенияКонтекста(СтрокаПараметра.Имя, ТекстДо + ТекстНовогоМетода));
		КонецЕсли; 
	КонецЦикла;
	...
	ТекстНовогоМетода = СтрЗаменить(ТекстНовогоМетода, ЗаглушкаВозврата, ЗаменаЗаглушкиВозврата);
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		ФормаНовогоМетода.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ПолеТекстаАлгоритма.мМодульМетаданных.Методы, ПолеТекстаАлгоритма.мМетодМодуля.Имя);
	Иначе
	...
	ПолеТекста.ВставитьСтроку(ПолеТекстаАлгоритма.мПоследняяСтрокаТелаМетодаМодуля + 2, Символы.ПС + ФормаНовогоМетода.ПолноеОпределение);
		...
		Если Не ЗначениеЗаполнено(НеопределеннаяПеременная) Тогда
			...
			Если Не выхКорректныйФрагмент Тогда
				...
				Если ПоказыватьСтрокуОшибки Тогда
					ЭтаФорма.ТекущийЭлемент = ПолеТекстаАлгоритма.ПолеТекста.ЭлементФормы;
					ирКлиент.ПоказатьОшибкуВТекстеПрограммыЛкс(ПолеТекстаАлгоритма.ПолеТекста, мСмещениеНомеровСтрок,,,, ИнформацияОбОшибке);
				Иначе
...
Процедура КоманднаяПанельВстроенныйЯзыкПереименоватьСлово(Кнопка)
	...
	Если ФормаПереименования <> Неопределено Тогда
...
Функция ТелоГлавногоМетода()
	...
	ТекстАлгоритма = ПолеТекстаАлгоритма.ПолеТекста.ПолучитьТекст();
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		ТекстАлгоритма = ПолеТекстаАлгоритма.мМодульМетаданных.Методы[0].Тело;
	КонецЕсли;
...
Процедура ДеревоАлгоритмовЭтоМодульПриИзменении(Элемент)
	...
	Если мТекущаяСтрокаДереваАлгоритмов.ЭтоМодуль Тогда
		ТекстАлгоритма = ОпределениеГлавногоМетодаИзТела();
		СдвигНомераСтроки = +1;
	Иначе
		ПолеТекстаАлгоритма.ОбновитьМодульМетаданных();
		КоличествоМетодов = ПолеТекстаАлгоритма.мМодульМетаданных.Методы.Количество();
		...
		Если КоличествоМетодов > 0 Тогда
			ТекстАлгоритма = СокрЛП(ПолеТекстаАлгоритма.мМодульМетаданных.Методы[0].Тело);
		Иначе
			ПолеТекстаАлгоритма.ПолеТекста.ПолучитьТекст();
		КонецЕсли;
		СдвигНомераСтроки = -1;
	КонецЕсли;
	ПолеТекстаАлгоритма.ЭтоМодуль = мТекущаяСтрокаДереваАлгоритмов.ЭтоМодуль;
	ПолеТекстаАлгоритма.УстановитьТекст(ТекстАлгоритма);
...
Функция ОпределениеГлавногоМетодаИзТела(Знач ТелоМетода = "")

	#Если Сервер И Не Сервер Тогда
	    ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	Если Не ЗначениеЗаполнено(ТелоМетода) Тогда
...
Процедура КоманднаяПанельВстроенныйЯзыкСписокМетодов(Кнопка)
	// Нужна чтобы кнопка горела
КонецПроцедуры
