...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	КлючТекущейСтроки = ирОбщий.ТабличноеПолеКлючСтрокиЛкс(ЭлементыФормы.Параметры, "Имя");
	Если Ложь
	...
	КонецЕсли; 
	ирОбщий.ТабличноеПолеВосстановитьТекущуюСтрокуЛкс(ЭлементыФормы.Параметры, КлючТекущейСтроки);
...
Функция КлсПолеТекстаПрограммыОбновитьКонтекст(Знач Компонента = Неопределено, Знач Кнопка = Неопределено) Экспорт 
	...
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		...
		КонецЦикла;
	КонецЕсли;
	ЭтоВыполнениеКода = (Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(Компонента, "Выполнить"));
...
Процедура ПриОткрытии()
	...
	//	ЭлементыФормы.СтрокаПоискаВДереве.Видимость = Ложь;
	//	ЭлементыФормы.Назад.Видимость = Ложь;
	//	ЭлементыФормы.Вперед.Видимость = Ложь;
	//	ирОбщий.ИзменитьСвернутостьЛкс(ЭтаФорма, Ложь, ЭлементыФормы.ДеревоАлгоритмов, ЭлементыФормы.вРазделительДерево, Панель, "лево");
	//КонецЕсли;
...
Процедура ОбработатьИсключениеАлгоритма(Знач ВремяНачала, ИнформацияОбОшибке, Знач ЛиСинтаксическийКонтроль, Знач СтруктураПараметров = Неопределено)
	...
	Если Не ЛиСинтаксическийКонтроль Тогда 
		...
		ирОбщий.ПоказатьОшибкуВТекстеПрограммыЛкс(ПолеТекстаАлгоритма.ПолеТекста, -СтартоваяСтрока,,, МодальныйРежим, ИнформацияОбОшибке, ИмяМодуля); 
	КонецЕсли;
...
Процедура ЗагрузитьДанныеТекущейСтроки()
	...
	Если мАлгоритмЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
			ВосстановитьТекущуюСтрокуВТексте = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаАлгоритма.ПолеТекста, СтрокаПоискаВДереве, Ложь);
		КонецЕсли; 
...
Процедура ВыбранСпособВыполнения(Кнопка = Неопределено)
	...
	ЭлементыФормы.КоманднаяПанельПараметры.Кнопки.ВыполнитьВарианты.Доступность = Не ирОбщий.СтрНайтиЛкс(КонтекстВыполнения, "Фоновый");
	#Если Сервер И Не Сервер Тогда
	    ПолеТекстаАлгоритма = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	...
	Если мТекущаяСтрокаДереваАлгоритмов.Наименование = ИнфоФайла.ИдентификаторАлгоритма Тогда
		...
		Если ТекстИзОбработки <> ТекстИзПоля Тогда
			...
			Если ПоказыватьСравнение Тогда
				ирОбщий.Сравнить2ЗначенияВФормеЛкс(ТекстИзПоля, ТекстИзОбработки, Истина, "Консоль", "Файл");
...
Процедура КоманднаяПанельВстроенныйЯзыкЗаполнитьОбъект(Кнопка)
	...
	ФормаВыбора = ирОбщий.ПолучитьФормуВыбораОбъектаМетаданныхСтруктуройЛкс(ВладелецФормы,,, ПараметрыВыбораМетаданных);
...
Процедура ПараметрыВариантыЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.Параметры, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура КоманднаяПанельПараметрыВыполнитьВарианты(Кнопка)
	...
	Для ИндексПараметра = 0 По Координаты.ВГраница() Цикл
		...
		ТаблицаРезультатов.Колонки.Добавить(ИмяКолонки,, "Вход " + ИмяКолонки);
	КонецЦикла; 
	Если АвтоПараметрыВыхода Тогда
		ЗаполнитьПараметры(Ложь, Истина);
	КонецЕсли;
	ПараметрыВыхода = Параметры.НайтиСтроки(Новый Структура("Вход, Выход", Ложь, Истина));
	Для Каждого СтрокаПараметра Из ПараметрыВыхода Цикл
		ТаблицаРезультатов.Колонки.Добавить(СтрокаПараметра.Имя,, "Выход " + СтрокаПараметра.Имя);
	КонецЦикла;
	ТаблицаРезультатов.Колонки.Добавить("Ошибка", Новый ОписаниеТипов("Строка"));
	ТаблицаРезультатов.Колонки.Добавить("Длительность", Новый ОписаниеТипов("Число"));
	ФормаТаблицы = ирОбщий.ОткрытьТаблицуЗначенийЛкс(ТаблицаРезультатов,, Ложь,,,, Истина);
	ТаблицаРезультатов = ФормаТаблицы.Коллекция;
	СтруктураПараметров = Новый Структура;
	...
	Пока Истина Цикл
		...
		Для ИндексПараметра = 0 По Координаты.ВГраница() Цикл
			...
			СтрокаРезультата[ИндексПараметра] = СтрокаПараметра.Значение;
			ПередаваемоеЗначение = СтрокаПараметра.Значение;
			Если КонтекстВыполнения <> "Клиент" Тогда
				ПередаваемоеЗначение = ЗначениеВСтрокуВнутр(ПередаваемоеЗначение);
			КонецЕсли; 
		...
		СтрокаРезультата.Длительность = мТекущаяСтрокаДереваАлгоритмов.Длительность;
		Для Каждого СтрокаПараметра Из ПараметрыВыхода Цикл
			СтрокаРезультата[СтрокаПараметра.Имя] = СтрокаПараметра.Значение;
		КонецЦикла;
		Если ИнформацияОбОшибке <> Неопределено Тогда
			СтрокаРезультата.Ошибка = ирОбщий.ПодробноеПредставлениеОшибкиЛкс(ИнформацияОбОшибке);
		КонецЕсли;
		ФормаТаблицы.ЭлементыФормы.Коллекция.ТекущаяСтрока = СтрокаРезультата;
		КомбинацияЕсть = Ложь;
		Для ИндексПараметра = 0 По Координаты.ВГраница() Цикл
			...
			ПараметрыСВариантами[ИндексПараметра].Значение = ПараметрыСВариантами[ИндексПараметра].ВариантыЗначения[Координата].Значение;
			ОбновитьПредставлениеИТипЗначенияВСтроке(ПараметрыСВариантами[ИндексПараметра]);
	...
	КонецЦикла;
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
	ТаблицаРезультатов = ирОбщий.СузитьТипыКолонокТаблицыБезПотериДанныхЛкс(ТаблицаРезультатов);
	ФормаТаблицы.Закрыть();
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(ТаблицаРезультатов,, Ложь, "Алгоритм """ + мТекущаяСтрокаДереваАлгоритмов.Наименование + """");
