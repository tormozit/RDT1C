	...
	Если ПолеТекстаАлгоритма.ЭтоМодуль Тогда
		...
		Для Каждого Метод Из Методы Цикл
			Если ирОбщий.СтрокиРавныЛкс(ИмяГлавногоМетода, Метод.Имя) Тогда
				...
				мСмещениеНомеровСтрок = мСмещениеНомеровСтрок - Макс(0, ирОбщий.СтрЧислоСтрокЛкс(Лев(Текст, Метод.ПозицияТела - 1)) - 1);
				Продолжить;
			КонецЕсли;     
			Если мСмещениеНомеровСтрокДоп = 0 Тогда
				ВысотаОписанияМетода = ?(Метод.Описание = Неопределено, 0, ирОбщий.СтрЧислоСтрокЛкс(Метод.Описание) - 1);
				мСмещениеНомеровСтрокДоп = мСмещениеНомеровСтрокДопПорог + 1 - (ирОбщий.СтрЧислоСтрокЛкс(Лев(Текст, Метод.ПозицияОпределения)) - ВысотаОписанияМетода);
			КонецЕсли;
