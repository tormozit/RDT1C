...
Процедура ПараметрыПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Массив") Тогда
		...
		Если ЗначениеПараметра.Количество() > 0 Тогда
			ЗначениеПараметра = ЗначениеПараметра[0];
		КонецЕсли;
		Если ТипЗнч(ЗначениеПараметра) = Тип("СтрокаТаблицыЗначений") Тогда
			Для Каждого Колонка Из ЗначениеПараметра.Владелец().Колонки Цикл
				Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеПараметра[Колонка.Имя], Ложь) Тогда
					ЗначениеПараметра = ЗначениеПараметра[Колонка.Имя];
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеПараметра, Ложь) Тогда
			СтандартнаяОбработка = Ложь;
			СтрокаПараметра = ирОбщий.НайтиДобавитьПараметрСсылкуВТаблицуЛкс(Параметры,,, ЗначениеПараметра,, Истина);
			ЗаполнитьСтрокуНовогоПараметра(СтрокаПараметра);
		КонецЕсли;
	КонецЕсли;
