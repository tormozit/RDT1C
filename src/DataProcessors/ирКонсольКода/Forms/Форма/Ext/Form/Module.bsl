...
Процедура ПараметрыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		...
		КонецЕсли; 
	ИначеЕсли Ложь
		Или Колонка.Имя = "ВариантыЗначения" 
...
Процедура ВыполнитьАлгоритмПоНаборуПараметров(Знач СтрокаНабора, Знач СтруктураПараметров = Неопределено, Знач ТекстДляВыполнения = "", Знач ФормаТаблицы = Неопределено)  Экспорт 
	...
	Для Каждого КолонкаНабора Из СтрокаНабора.Владелец().Колонки Цикл
		...
		Тогда
			Продолжить;
		КонецЕсли;
		СтрокаПараметра = Параметры.Найти(КолонкаНабора.Имя, "Имя");
		Если СтрокаПараметра = Неопределено Тогда 
			ВызватьИсключение ирОбщий.СтрШаблонЛкс("Для колонки ""%1"" набора параметров не найден параметр в алгоритме", КолонкаНабора.Имя);
		КонецЕсли;
		Если Не СтрокаПараметра.Вход Тогда
			Продолжить;
		КонецЕсли;
...
Процедура КоманднаяПанельПараметрыТаблицаНаборов(Кнопка)
	...
	Для Каждого СтрокаПараметра Из Параметры Цикл
		...
		КонецЕсли;
	КонецЦикла; 
	НаборыПараметров = ирОбщий.ОткрытьТаблицуЗначенийЛкс(НаборыПараметров,,, "Заполните наборы входных параметров",, Ложь);
