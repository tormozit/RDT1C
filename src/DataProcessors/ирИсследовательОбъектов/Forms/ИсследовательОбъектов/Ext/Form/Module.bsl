		...
		Если Лев(СтрокаДерева.Слово, 1) = "[" И Не ирОбщий.ЛиСсылкаНаОбъектБДЛкс(СтрокаДерева.Значение) Тогда
			...
			Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
				...
				Исключение
					Продолжить;
				КонецПопытки;
				ПредставлениеЭлемента.Добавить(СтрокаСлова.Слово + " = " + ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(ЗначениеСвойства, 20));
			КонецЦикла;
...
Функция ПредставлениеОбъектаДанных() Экспорт 
	...
	КонецЕсли; 
	Результат = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(Результат);
	Возврат Результат;
