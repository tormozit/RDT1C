...
Функция ПредставлениеОбъектаДанных() Экспорт 
	...
	ИначеЕсли КорневаяСтрока.Слово = МаркерСловаЗначения Тогда
		Результат = ирОбщий.ПредставлениеЗначенияЛкс(КорневаяСтрока.Значение);
	Иначе
...
Процедура ДеревоЗначенийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельДерева.Кнопки.Идентификаторы,,
...
Процедура ЗаполнитьДочерниеСтроки(Знач СтрокаДерева)
	...
	Если Истина
		...
		И СтрокаДерева.КоличествоЭлементов <> Неопределено
...
Процедура ДеревоЗначенийВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбраннаяСтрока.Размер) И Колонка = ЭлементыФормы.ДеревоЗначений.Колонки.Размер Тогда
		ирОбщий.ОткрытьТекстЛкс(ирОбщий.ОбъектВСтрокуДляСравненияВнутрЛкс(ВыбраннаяСтрока.Значение));
...
Процедура ОткрытьТекущийЭлемент(ПредпочитатьИсследовательКоллекций = Ложь, СтандартнаяОбработка = Ложь)
	...
	Тогда
		Если ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗначений, СтандартнаяОбработка, ЗначениеСтроки,, Истина,, ТекущаяСтрока.Слово) Тогда 
...
Процедура КоманднаяПанельДереваСправка(Кнопка = Неопределено)
	...
	Если Не ТекущаяСтрока.Успех Тогда
		Если ТекущаяСтрока.ТаблицаСтруктурТипов = Неопределено Тогда
			Возврат;
		КонецЕсли;
...
Процедура КоманднаяПанельДереваОтображениеXML(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(Текст, "XML", "XML", Истина);
...
Процедура КоманднаяПанельДереваОтображениеJSON(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(Текст, "JSON", "JSON", Истина);
...
Процедура ДеревоЗначенийПредставлениеЗначенияПриИзменении(Элемент)
	...
	Иначе 
		...
		Если Не БылаОшибка Тогда
			ирОбщий.ОповеститьОбИзмененииОбъектаВПамятиЛкс(Родитель.Значение);
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ДеревоЗначенийПредставлениеЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ТекущиеДанные = ЭлементыФормы.ДеревоЗначений.ТекущиеДанные;
	Попытка
		ЗначениеИзменено = ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗначений, СтандартнаяОбработка, ТекущиеДанные.Значение,,,,, ТекущиеДанные.Слово);
	Исключение
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если КлючУникальности = "Связанный" И ИмяСобытия = "Исследовать" И Источник = ВладелецФормы Тогда
		...
		мВычислитьВыражение();
		ирОбщий.ДеревоЗначенийСвернутьРазвернутьЛкс(ЭлементыФормы.ДеревоЗначений,,, Ложь);
	КонецЕсли; 
...
Процедура КоманднаяПанельДереваЗапомнитьОбъектДляСравнения(Кнопка)
	...
	//КонецЕсли;
	ИмяСвойства = ЭлементыФормы.ДеревоЗначений.ТекущаяСтрока.Слово;
	ОбъектДляСравнения = ТекущаяСтрока.Значение;
	Если Истина
	...
	Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	ирОбщий.ДобавитьОбъектВБуферИПредложитьСравнениеЛкс(ирОбщий.СтруктураИзОбъектаЛкс(ОбъектДляСравнения), ЭтаФорма);
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, АктивизироватьПервуюСтроку = Неопределено, Текст = Неопределено)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если Не ЭлементыФормы.ДеревоЗначений.Развернут(КорневаяСтрока) Тогда
			ЭлементыФормы.ДеревоЗначений.Развернуть(КорневаяСтрока);
		КонецЕсли;
	КонецЕсли;
	Кнопки = Новый Массив;
	...
	ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗначений, Текст, "Слово", АктивизироватьПервуюСтроку <> Ложь,, Кнопки);
...
Процедура ВпередНажатие(Элемент)
	...
	ирОбщий.СледующееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗначений);
	
КонецПроцедуры

Процедура НазадНажатие(Элемент)
	
	ирОбщий.ПредыдущееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗначений);
...
Процедура КоманднаяПанельДереваИсследоватьВОтдельномОкне(Кнопка)
	...
	Если ТекущаяСтрока = Неопределено Или Не ТекущаяСтрока.Успех Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ИсследоватьЛкс(ТекущаяСтрока.Значение,,,, ПолучитьПолныйПуть(ТекущаяСтрока));

КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирИсследовательОбъектов.Форма.ИсследовательОбъектов");
