...
Процедура мВычислитьВыражение() 
	...
	Если КорневаяСтрока.Слово = МаркерСловаЗначения Тогда
		ПредставлениеКорневогоЗначения = "" + КорневаяСтрока.Значение;
	Иначе
		ПредставлениеКорневогоЗначения = "" + КорневаяСтрока.Слово;
	КонецЕсли; 
	ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(ЭтаФорма.Заголовок,, ПредставлениеКорневогоЗначения, ": ");
...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.КоманднаяПанельДерева.Кнопки.ОтображениеJSON.Доступность = ирКэш.ЛиДоступенJSONЛкс();
	ЭлементыФормы.КоманднаяПанельДерева.Кнопки.ОтображениеJSONПрямое.Доступность = ирКэш.ЛиДоступенJSONЛкс();
	ЭлементыФормы.КоманднаяПанельДерева.Кнопки.ИзСтроки.Кнопки.ИзJSON.Доступность = ирКэш.ЛиДоступенJSONЛкс();
	ЭлементыФормы.КоманднаяПанельДерева.Кнопки.ИзСтроки.Кнопки.ИзJSONВСтруктуру.Доступность = ирКэш.ЛиДоступенJSONЛкс();
	ЭлементыФормы.КоманднаяПанельДерева.Кнопки.ИзСтроки.Кнопки.ИзJSONВСоответствие.Доступность = ирКэш.ЛиДоступенJSONЛкс();
	...
	Иначе
		Попытка
			Если Родитель.Слово = "<ГлобальныйКонтекст>" Тогда
				Выполнить(ТекущиеДанные.Слово + " = Элемент.Значение");
			Иначе
...
Процедура КоманднаяПанельДереваКонсольКода(Кнопка)
	...
	Если Ложь
		...
		Или (Истина
			И ирОбщий.СтрокиРавныЛкс(ТекущаяСтрока.Слово, МаркерСловаЗначения)
			И ТекущаяСтрока.Родитель = Неопределено)
	Тогда
		СтруктураПараметров.Вставить(МаркерСловаЗначения, _Значение_); 
	КонецЕсли; 
...
Процедура КоманднаяПанельДереваЗапомнитьОбъектДляСравнения(Кнопка)
	...
	Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	ирОбщий.ДобавитьДокументВБуферСравненияЛкс(ирОбщий.СтруктураИзОбъектаЛкс(ОбъектДляСравнения), ЭтаФорма);
