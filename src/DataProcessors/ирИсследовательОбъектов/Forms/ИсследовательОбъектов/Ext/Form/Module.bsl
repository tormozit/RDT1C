...
Процедура ЗаполнитьСтрокуСлова(СтрокаДерева)
	...
	Если СтрокаДерева.НеУспех Тогда 
		Если СтрокаДерева.ТипСлова <> "Метод" Тогда
			ТекстРодителя = "";
	...
	Если ЭтоАгрегатноеЗначение Тогда 
		СтрокаДерева.Строки.Добавить().НеУспех = Истина;
		Если Лев(СтрокаДерева.Слово, 1) = "[" И Не ирОбщий.ЛиСсылкаНаОбъектБДЛкс(СтрокаДерева.Значение) Тогда
			...
			Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
				...
				Исключение
					Продолжить;
				КонецПопытки;
				ПредставлениеЭлемента.Добавить(СтрокаСлова.Слово + " = " + ирОбщий.ПредставлениеСОграничениемДлиныЛкс(ЗначениеСвойства, 20));
			КонецЦикла;
...
Функция ПредставлениеОбъектаДанных() Экспорт 
	...
	КонецЕсли; 
	Результат = ирОбщий.ПредставлениеСОграничениемДлиныЛкс(Результат);
	Возврат Результат;
...
Процедура ДеревоЗначенийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если Не ДанныеСтроки.НеУспех Тогда
...
Процедура ЗаполнитьДочерниеСтроки(Знач СтрокаДерева)
	...
	Если СтрокаДерева.Строки.Количество() = 1 И СтрокаДерева.Строки[0].НеУспех Тогда 
		СтрокаДерева.Строки.Очистить();    
	...
	Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
		...
		Если СтрокаСлова.Слово = "ТабличныеПространстваБазыДанных" Тогда
			// Антибаг платформы 8.3.23 https://www.hostedredmine.com/issues/963850
			Продолжить;
		КонецЕсли;
		Если Истина
			И Выражение = ""
			И СтрокаСлова.Определение = "Метаданные"
			И Найти(СтрокаСлова.ТипЗначения, "ОбщийМодуль")
			И Метаданные.ОбщиеМодули.Найти(СтрокаСлова.Слово) = Неопределено
		Тогда
			// Маска общих модулей
			Продолжить;
		КонецЕсли;
		//ЗаписьЖурналаРегистрации("Отладка",,,, СтрокаСлова.Слово); // Отладка
		...
		КонецЕсли; 
		Если СтрокаСлова = СтрокаСловаЗначение Тогда
			НоваяСтрока.Значение = ирОбщий.ДанныеЭлементаФормыЛкс(СтрокаДерева.Значение);
			НоваяСтрока.НеУспех = Ложь;
		КонецЕсли; 
		ЗаполнитьСтрокуСлова(НоваяСтрока);
		Если СтрокаСлова.Определение = "Метаданные" Тогда // Грязно
...
Процедура УстановитьЗначениеСловаВСтроке(СтрокаДерева, Успех, НовоеЗначение)
	...
	СтрокаДерева.НеУспех = Не Успех = Истина;
	Если СтрокаДерева.ТипСлова = "Группа" Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДеревоЗначенийВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Не ВыбраннаяСтрока.НеУспех Тогда
...
Процедура ОткрытьТекущийЭлемент(ПредпочитатьИсследовательКоллекций = Ложь, СтандартнаяОбработка = Ложь)
	...
	Тогда 
		...
		СтандартнаяОбработка = Ложь;
		ирКлиент.ОткрытьЗначениеЛкс(ЗначениеСтроки, Ложь,, ТекущаяСтрока.Слово);
	ИначеЕсли Ложь
...
Процедура КоманднаяПанельДереваСправка(Кнопка = Неопределено)
	...
	Тогда
		Возврат;
	КонецЕсли;
	Если ТекущаяСтрока.НеУспех Тогда
...
Процедура КоманднаяПанельДереваОтображениеXML(Кнопка)
	...
	Если ТекущаяСтрока = Неопределено Или ТекущаяСтрока.НеУспех Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ДеревоЗначенийПредставлениеЗначенияПриИзменении(Элемент)
	...
	Иначе 
		...
		Иначе
			...
			Исключение
				// Виртуальное свойство "Значение" поля формы
...
Процедура КоманднаяПанельДереваЗапомнитьОбъектДляСравнения(Кнопка)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		Если ВыделеннаяСтрока.НеУспех Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура ДобавитьВДеревоЭлементКоллекции(Знач ЗначениеЭлемента, Знач ПутьКЭлементу, Знач УстановитьТекущуюСтроку = Истина, Знач ИмяТекущегоСвойства = "") Экспорт 
	...
	Если Не ирОбщий.СтрНачинаетсяСЛкс(ПутьКЭлементу, ТекущийУзел.Слово) Тогда 
		Возврат;
	КонецЕсли;
	РегВычислитель = ирКэш.РегВыражениеЛкс();
	#Если Сервер И Не Сервер Тогда
