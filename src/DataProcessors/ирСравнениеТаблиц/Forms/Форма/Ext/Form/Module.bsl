...
Процедура ОткрытьТаблицуИсточника(Знач НомерСтроны, Знач НомерСтроки = Неопределено)
	...
	КонецЕсли;
	КопияСсылкиТаблицы = ирКлиент.ОткрытьТаблицуЗначенийЛкс(КопияСсылкиТаблицы, ТекущаяСтрока, Истина, "Таблица " + НомерСтроны,, Ложь);
...
Процедура КолонкиТаблица1Выбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Ложь
		Или Колонка = ЭлементыФормы.КолонкиТаблица1.Колонки.ОписаниеТипов
		Или Колонка = ЭлементыФормы.КолонкиТаблица1.Колонки.ОписаниеТипов2
	Тогда
		ирКлиент.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка);
	КонецЕсли;
