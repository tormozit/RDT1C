...
Процедура ПриОткрытии()
	...
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма,,,, "ctb");
	...
	Если ПараметрТаблица2.Колонки.Количество() > 0 Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(ПараметрИсключитьКолонки) Тогда
			Для Каждого ИмяКолонки Из ирОбщий.СтрРазделитьЛкс(ПараметрИсключитьКолонки, ",", Истина) Цикл
				СтрокаКолонки = КолонкиТаблица1.Найти(ИмяКолонки, "ИмяКолонки");
				Если СтрокаКолонки <> Неопределено Тогда
					СтрокаКолонки.Ключевая = Ложь;
					СтрокаКолонки.Сравнивать = Ложь;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли; 
...
Процедура ОткрытьТаблицуИсточника(Знач НомерСтроны, Знач НомерСтроки = Неопределено)
	...
	КонецЕсли;
	Диаграмма = ЭлементыФормы["КолонкиТаблица" + НомерСтроны]; // ТабличноеПоле
	ТекущаяКолонка = Диаграмма.ТекущаяСтрока.ИмяКолонки;
