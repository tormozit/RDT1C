...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	...
	Если Таблица1.Количество() > МаксЧислоСтрокДляСохранения Тогда
		СохраняемаяТаблица1 = Таблица1.СкопироватьКолонки();
	Иначе
		СохраняемаяТаблица1 = Таблица1.Скопировать();
	КонецЕсли;
	Если Таблица2.Количество() > МаксЧислоСтрокДляСохранения Тогда
...
Функция ПолучитьРезультатВыбораФайла(ПолеВвода, НомерСтороны)
	...
	Если ЗначениеЗаполнено(ПолноеИмяФайла) Тогда
		ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(ПолеВвода, ПолноеИмяФайла);
	КонецЕсли;
	...
	Тогда 
		...
		Если ЯчейкаРазностнойКолонки <> Неопределено Тогда
			...
			Если ТолькоЕслиРазличаются Тогда
				ирОбщий.ОформитьЯчейкуРазностиЛкс(ОформлениеСтроки, ИмяРазностнойКолонки);
			КонецЕсли;
		КонецЕсли; 
...
Процедура КаталогВыгрузкиНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ПолноеИмяФайла) Тогда
		ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(Элемент, ПолноеИмяФайла);
	КонецЕсли;
...
Процедура КоманднаяПанельРезультатСравнитьСтрокиТаблиц(Кнопка)
	...
	ирОбщий.СравнитьОбъектыВФормеЛкс(Объекты,, ТекущаяКолонка,,, 1);
