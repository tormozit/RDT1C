...
Процедура ПодобратьКлючевыеИСравниваемыеКолонки(Знач КлючевыеКолонки = "", Знач СравниваемыеКолонки = "") Экспорт 
	
	СтрокаКлюча1 = "";
	СтрокаКлюча2 = "";                    
	Уникальность1Достигнута = Ложь;
	Уникальность2Достигнута = Ложь;
	Если ЗначениеЗаполнено(КлючевыеКолонки) Тогда
		Для Каждого ИмяКолонки Из ирОбщий.СтрРазделитьЛкс(КлючевыеКолонки, ",", Истина) Цикл
			СтрокаСопоставления = КолонкиТаблица1.Найти(ИмяКолонки, "ИмяКолонки2");
		...
		КонецЦикла;
		Если ПроверитьУникальностьКлючаТаблицы(Таблица1, СтрокаКлюча1) Тогда
			Уникальность1Достигнута = Истина;
		КонецЕсли; 
		Если ПроверитьУникальностьКлючаТаблицы(Таблица2, СтрокаКлюча2) Тогда
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(СравниваемыеКолонки) Тогда
		Для Каждого ИмяКолонки Из ирОбщий.СтрРазделитьЛкс(СравниваемыеКолонки, ",", Истина) Цикл
			СтрокаСопоставления = КолонкиТаблица1.Найти(ИмяКолонки, "ИмяКолонки2");
			Если СтрокаСопоставления = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
//    - 
Функция ПолучитьШаблоныВложенныхТаблиц(Знач ОбъектМД)
...
ЗапросВыгрузки = Новый Структура; // см. ОбработкаОбъект.ирКонсольЗапросов.РедактироватьСтруктуруЗапроса
