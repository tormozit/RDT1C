...
Процедура ПриОткрытии()
	...
	Если ПараметрОбъект <> Неопределено Тогда
		ПараметрТаблицаБД = Метаданные.НайтиПоТипу(ирОбщий.ТипОбъектаБДЛкс(ПараметрОбъект)).ПолноеИмя();
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрТаблицаБД) Тогда
		СтрокаТипа = ДеревоТаблиц.Строки.Найти(ПараметрТаблицаБД, "ПолноеИмя", Истина);
		Если СтрокаТипа <> Неопределено Тогда
			ЭлементыФормы.ДеревоТаблиц.ТекущаяСтрока = СтрокаТипа;
			Если ПараметрОбъект <> Неопределено И ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(СтрокаТипа.Родитель.Имя) Тогда
...
Функция ПостроительЗапросаДляТаблицыИзменений(ВыбраннаяСтрока = Неопределено, МаксимальнаяПорция = 0)
	...
	Тогда
		...
		Для Каждого ОбщийРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
			...
			Тогда
				Если ирОбщий.ЛиОбщийРеквизитПодключенВОбъектеМетаданныхЛкс(ОбщийРеквизит, ОбъектМетаданных) Тогда
