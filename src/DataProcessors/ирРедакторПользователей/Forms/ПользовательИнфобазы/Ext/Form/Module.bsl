...
Процедура ОбновитьДанныеПользователяБД(ПользовательНастроек, Знач ОтображатьИмя = Истина)
	...
	ЗаполнитьЗначенияСвойств(СтруктураФлаговАутентификации, ПользовательНастроек);
	ЗаполнитьЗначенияСвойств(ЭтаФорма, ПользовательНастроек);
	...
	Если НомерВерсииПлатформы < 803027 Тогда
		ЭлементыФормы.КлючТребованияНазначения.Доступность = Ложь;
	Иначе
		ЭтаФорма.КлючТребованияНазначения = ПользовательНастроек.КлючТребованияНазначения;
	КонецЕсли;
	Если НомерВерсииПлатформы < 803016 Тогда
		ЭлементыФормы.ДатаБлокировки.Доступность = Ложь;
	ИначеЕсли ЗначениеЗаполнено(Имя) Тогда
		БлокировкаПользователя = БлокировкаПользователя(Имя);
		Если БлокировкаПользователя <> Неопределено Тогда
			ЭтаФорма.ДатаБлокировки = БлокировкаПользователя.ВремяНачалаБлокировки;
		КонецЕсли;
	КонецЕсли;
	ОбновитьДоступность();
...
Функция _ЗначениеРазделителяИзСтроки(ИмяРазделителя, СтрокаЗначения)
	...
	Если Ложь
		Или МетаРазделитель.Тип.СодержитТип(Тип("Булево"))
		Или МетаРазделитель.Тип.СодержитТип(Тип("Число"))
		Или МетаРазделитель.Тип.СодержитТип(Тип("Дата"))
		Или МетаРазделитель.Тип.СодержитТип(Тип("Строка"))
	Тогда
		Результат = МетаРазделитель.Тип.ПривестиЗначение(СтрокаЗначения);
	Иначе
		ОбъектМД = Метаданные.НайтиПоТипу(МетаРазделитель.Тип.Типы()[0]);
...
Функция ЗаписатьПользователя()
	...
	Тогда
		мСообщитьОбОшибке("Укажите пользователя ОС или запретите аутентификацию ОС!");
		Возврат Ложь;
	КонецЕсли;
	...
	СтруктураФлаговАутентификации = ирОбщий.СтруктураФлаговАутентификацииЛкс();
	ЗаполнитьЗначенияСвойств(СтруктураФлаговАутентификации, ЭтаФорма);
	ЗаполнитьЗначенияСвойств(ПользовательИБ, СтруктураФлаговАутентификации);
	...
	Иначе
		ПользовательИБ.КлючТребованияНазначения = ЭтаФорма.КлючТребованияНазначения;
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	КонецЕсли; 
	НомерВерсииПлатформы = ирКэш.НомерВерсииПлатформыЛкс();
	ЭлементыФормы.АутентификацияOpenID.Доступность = НомерВерсииПлатформы >= 803002;
	ЭлементыФормы.АутентификацияOpenIDConnect.Доступность = НомерВерсииПлатформы >= 803019;
	ЭлементыФормы.АутентификацияQRКодом.Доступность = НомерВерсииПлатформы >= 803026;
	ЭлементыФормы.АутентификацияТокеномДоступа.Доступность = НомерВерсииПлатформы >= 803021;
	ЭлементыФормы.АутентификацияЧерезЭлектроннуюПочту.Доступность = НомерВерсииПлатформы >= 803027;
...
Процедура ДатаБлокировкиОчистка(Элемент, СтандартнаяОбработка)
	ирСервер.РазблокироватьАутентифкациюЛкс(Имя);
КонецПроцедуры

Процедура КлючТребованияНазначенияНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	ирОбщий.ПолеВводаСИсториейВыбора_ОбновитьСписокЛкс(Элемент, ЭтаФорма);
КонецПроцедуры

Процедура КлючТребованияНазначенияПриИзменении(Элемент)
	ирКлиент.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, ЭтаФорма);
КонецПроцедуры
