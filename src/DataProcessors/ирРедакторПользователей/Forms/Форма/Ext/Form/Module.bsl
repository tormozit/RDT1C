...
Функция УправлениеСпискомПользователей_ОбновитьСписокПользователей(Знач УникальныйИдентификатор = "") Экспорт
	...
	СтруктураСвойствПользователя = Неопределено;
	...
	Для каждого Пользователь Из ПользователиИнфобазы Цикл
		...
		УправлениеСпискомПользователей_ОбновитьСтрокуПользователя(Пользователь, ТекущиеДанные, СсылкиПользователей, СтруктураСвойствПользователя);
		Счетчик = Счетчик + 1;
	КонецЦикла;
...
Функция УправлениеСпискомПользователей_ОбновитьСтрокуПользователя(Знач ПользовательИБ = Неопределено, Знач ТекущиеДанные, Знач СсылкиПользователей = Неопределено, СтруктураСвойствПользователя = Неопределено)
	
	#Если Сервер И Не Сервер Тогда
		ТекущиеДанные = Пользователи.Добавить();
	#КонецЕсли
	Если ПользовательИБ = Неопределено Тогда
		ПользовательИБ = ПользовательИБИзСтрокиСписка(ТекущиеДанные);
	Иначе
		ТекущиеДанные.Имя = ПользовательИБ.Имя;
	КонецЕсли; 
	ТекущиеДанные.ПолноеИмя = ПользовательИБ.ПолноеИмя;
	ТекущиеДанные.ПарольУстановлен = ПользовательИБ.ПарольУстановлен;
	ТекущиеДанные.ПоказыватьВСпискеВыбора = ПользовательИБ.ПоказыватьВСпискеВыбора;
	СтруктураФлаговАутентификации = ирОбщий.СтруктураФлаговАутентификацииЛкс();
	ЗаполнитьЗначенияСвойств(СтруктураФлаговАутентификации, ПользовательИБ);
	ЗаполнитьЗначенияСвойств(ТекущиеДанные, СтруктураФлаговАутентификации);
	Попытка
		ТекущиеДанные.ПользовательОС = ПользовательИБ.ПользовательОС;
	Исключение
	...
	Иначе
		ТекущиеДанные.Язык = ПользовательИБ.Язык.Имя;
		ТекущиеДанные.ЯзыкПредставление = ПользовательИБ.Язык;
	КонецЕсли; 
	...
	Иначе
		ТекущиеДанные.ОсновнойИнтерфейс = ПользовательИБ.ОсновнойИнтерфейс.Имя;
		ТекущиеДанные.ОсновнойИнтерфейсПредставление = ПользовательИБ.ОсновнойИнтерфейс;
	КонецЕсли; 
	...
	ТекущиеДанные.УникальныйИдентификатор = ПользовательИБ.УникальныйИдентификатор;
	ТекущиеДанные.СохраняемоеЗначениеПароля = ПользовательИБ.СохраняемоеЗначениеПароля;
	ТекущиеДанные.РежимЗапуска = ПользовательИБ.РежимЗапуска;
	Если ирКэш.ДоступноЗащитаОтОпасныхДействийЛкс() Тогда
		ТекущиеДанные.ЗащитаОтОпасныхДействий = ПользовательИБ.ЗащитаОтОпасныхДействий.ПредупреждатьОбОпасныхДействиях;
	КонецЕсли;
	Если ирКэш.ДоступноВосстановлениеПароляПользователямЛкс() Тогда
		ТекущиеДанные.АдресЭлектроннойПочты = ПользовательИБ.АдресЭлектроннойПочты;
		ТекущиеДанные.ЗапрещеноВосстанавливатьПароль = ПользовательИБ.ЗапрещеноВосстанавливатьПароль;
	КонецЕсли; 
	Если РазделениеДанных.Количество() > 0 Тогда
		ТекущиеДанные.РазделениеДанныхПредставление = ирОбщий.РасширенноеПредставлениеЛкс(ПользовательИБ.РазделениеДанных);
	КонецЕсли;  
	ТекущиеДанные.Ссылка = Неопределено;
	Если СсылкиПользователей <> Неопределено Тогда
		СтрокаСсылки = СсылкиПользователей.Найти(ПользовательИБ.УникальныйИдентификатор, "ИдентификаторПользователяИБ");
		Если СтрокаСсылки <> Неопределено Тогда
			ТекущиеДанные.Ссылка = СтрокаСсылки.Ссылка;
		КонецЕсли; 
	КонецЕсли;
	НастройкиКлиентскогоПриложения = ХранилищеСистемныхНастроек.Загрузить("Общее/НастройкиКлиентскогоПриложения",,, ПользовательИБ.Имя);
	...
	Если ЭлементыФормы.Пользователи.Колонки.ДатаБлокировки.Видимость Тогда
		...
		ОбновитьДатуАвтоблокировкиПользователя(ТекущиеДанные, ПользовательИБ);
		...
		КонецЕсли;
	КонецЕсли;
	ТекущиеДанные.ЕстьАутентификация = ирОбщий.ЛиЕстьАутентификацияПользователяИБЛкс(ПользовательИБ, СтруктураСвойствПользователя);
