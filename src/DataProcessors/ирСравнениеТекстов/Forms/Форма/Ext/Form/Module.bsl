...
Процедура ОбновитьТекущиеПути() Экспорт
	...
	Если КодЯзыкаПрограммы() <> Неопределено Тогда
		ОбновитьПутьСтороны(ЭлементыФормы.ПутьВТексте1, мПолеТекстаПрограммы1, РедакторHTML.editor.getOriginalEditor().getSelection().positionLineNumber);
		ОбновитьПутьСтороны(ЭлементыФормы.ПутьВТексте2, мПолеТекстаПрограммы2, РедакторHTML.editor.getModifiedEditor().getSelection().positionLineNumber, Истина);
	КонецЕсли;
КонецПроцедуры
