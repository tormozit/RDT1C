...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.ДействияФормы.Кнопки.ОткрытьВКонфигураторе.Доступность = Истина
		И мПлатформа.мСвязанныйКонфигуратор <> Неопределено
		И Заголовок2 = мПлатформа.ЗаголовокСтороныМодуляКонфигуратора();
...
Процедура ОбновитьСравнениеТекущихСтрок() Экспорт
	...
	Если РедакторHTML = Неопределено Тогда
		Возврат;
	КонецЕсли;
	НомерСтроки1 = РедакторHTML.editor.getOriginalEditor().getSelection().positionLineNumber;
	НомерСтроки2 = РедакторHTML.editor.getModifiedEditor().getSelection().positionLineNumber;
	ЭлементыФормы.СравнениеТекущейСтроки.Документ.defaultView.setText(РедакторHTML.editor.getModel().original.getLineContent(НомерСтроки1),
			РедакторHTML.editor.getModel().modified.getLineContent(НомерСтроки2), Не УчитыватьКрайнююПустоту);
КонецПроцедуры
...
Процедура ОбновитьСравнение(Знач СохранятьПозицию = Истина)
	...
	//РедакторHTML.compare(Текст2, Истина, ПодсветкаСинтаксиса, Истина, Не УчитыватьКрайнююПустоту);
	РедакторHTML.compare(Текст2, Истина, ПодсветкаСинтаксиса, Истина, Не УчитыватьКрайнююПустоту, Текст1); // https://github.com/salexdv/bsl_console/blob/develop/docs/compare.md
...
Процедура ДействияФормыОткрытьВКонфигураторе(Кнопка)
	НомерСтрокиМодуля = РедакторHTML().editor.getModifiedEditor().getSelection().positionLineNumber;
	Ссылка = ирОбщий.СсылкаСтрокиМодуляЛкс(ОбщееНазвание, НомерСтрокиМодуля);
