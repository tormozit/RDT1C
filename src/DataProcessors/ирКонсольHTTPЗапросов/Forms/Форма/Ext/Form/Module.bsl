...
Процедура ПриОткрытии()
	...
	РазрешенJSON = ирКэш.НомерВерсииПлатформыЛкс() >= 803014;
...
Функция СохранитьВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, Знач ИмяФайла = Неопределено, Знач ЭтоФайлВосстановления = Ложь)
	...
	ФайлВыбран = ирОбщий.СохранитьФайлВКонсолиСВосстановлениемЛкс(ДиалогВыбораФайла, ИмяФайла, мИмяОткрытогоФайла, ДанныеДляФайла, мСтруктураВосстановления, ЗапрашиватьИмяФайла, ФайлВосстановленияОбрезан);
...
Функция УстановитьТелоОтветаСтрока(НовыйТекст = "") Экспорт
	...
	ирОбщий.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельОтвет.Страницы.ТелоОтветаHtml, Ложь);
...
Процедура ВыбранСпособВыполнения(Кнопка)
	...
	ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(ЭлементыФормы.КоманднаяПанельВыполнение.Кнопки.КонтекстВыполнения.Текст,, " " + Кнопка.Текст,  ":");
...
Функция ОбновитьКонечныйURL(Полный = Истина)
	...
	Если ирОбщий.СтрокиРавныЛкс(МетодЗапроса, "GET") Или ИспользоватьДляВсехМетодов Тогда
		СтрокаПараметров = "";
...
Процедура ДобавитьЗаголовокАвторизацииBearer(Токен, ТипАвторизации = "Basic")
...
Процедура КоманднаяПанельВыполнениеВыполнитьЗапрос(Кнопка = Неопределено)
	...
	СоединениеHTTP = ирОбщий.СоединениеHTTPЛкс(СерверЗапроса, ПортВременный, ПользовательЗапроса, ПарольЗапроса, СобственныйПрокси, ТаймаутЗапроса, ИспользоватьЗащищенноеСоединение, АутентификацияОС,
		"Основное соединение", КонтекстВыполнения = "Сервер");
	...
	Если ТипАвторизации = "OAuth2" Тогда
		...
		Если ЗначениеЗаполнено(Токен) Тогда
			ДобавитьЗаголовокАвторизацииBearer(Токен, "Bearer"); 
		КонецЕсли;
	ИначеЕсли ТипАвторизации = "Basic" Тогда
		...
		ДобавитьЗаголовокАвторизацииBearer(СтрокаАутентификации);
	ИначеЕсли ТипАвторизации = "Bearer" Тогда
		ДобавитьЗаголовокАвторизацииBearer(ПользовательЗапроса, "Bearer");
	КонецЕсли;
...
Процедура ПрочитатьКуки(Знач ОтветHTTP)
	...
	Для каждого ВхождениеЭлемент Из ВхожденияЭлементы Цикл
		...
		ВхожденияАтрибуты = ирОбщий.НайтиРегулярноеВыражениеЛкс(ВхождениеЭлемент.ТекстВхождения, Атрибут, "Имя, Значение, Имя1, Значение1, Флаг");
		#Если Сервер И Не Сервер Тогда
...
Процедура ГруппаКодовСостоянийНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(мГруппыСостояний, мГруппыСостояний.Найти(КодСостояния, "Код"), Ложь, "Группы состояний", Истина);
...
Процедура ЗаголовкиЗапросаИмяНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	КонецЕсли; 
	ВыбраннаяСтрока = ирОбщий.ОткрытьТаблицуЗначенийЛкс(ДоступныеЗаголовкиЗапроса, НачальнаяСтрока, Истина, "Выберите заголовок запроса", Истина);
...
Процедура ПоказатьСтрокуОписанияЗаголовка(Знач ВыбраннаяСтрока)
	...
	Если СтрокаОписания <> Неопределено Тогда
		ирОбщий.ОткрытьТаблицуЗначенийЛкс(мЗаголовкиHttp, СтрокаОписания, Ложь, "Описание заголовков", Истина);
	КонецЕсли;
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если ирКэш.НомерРежимаСовместимостиЛкс() < 803006 Тогда 
		ирОбщий.СообщитьЛкс("Инструмент доступен только в режиме совместимости 8.3.6 и выше",,, Истина);
		Отказ = Истина;
	КонецЕсли; 
...
Процедура НастроитьЭлементыФормы()
	...
	КонецЦикла;
	ЭлементыФормы.ПанельЗапрос.Страницы.ТелоЗапроса.Доступность = МетодЗапроса <> "GET";
...
Процедура УстановитьТелоОтветаHtml()
	...
	ирОбщий.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельОтвет.Страницы.ТелоОтветаHtml, Истина);
...
Процедура КП_ЗаголовкиЗапросаСравнить(Кнопка)
	...
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.ЗаголовкиЗапроса);
...
Процедура ДействияФормыСгенерироватьПрограммныйКод(Кнопка)
	...
	Если ЗначениеЗаполнено(РегулярноеВыражение) Тогда
		...
		|Вхождения = RegExp.НайтиВхождения(ТекстГдеИскать);
		|Если Вхождения.Количество() > 0 Тогда
...
Процедура ИспользоватьДляВсехМетодовПриИзменении(Элемент)
...
Процедура ТелоЗапросаJSONДокументСформирован(Элемент)
	...
	ирОбщий.РедакторJSON_ИнициироватьЛкс(мРедакторЗапросаJSON);
...
Процедура ТелоОтветаJSONДокументСформирован(Элемент)
	...
	ирОбщий.РедакторJSON_ИнициироватьЛкс(мРедакторОтветаJSON);
...
Процедура РежимДереваТелаЗапросаJSONПриИзменении(Элемент = Неопределено)
	...
	ирОбщий.ПереключитьРежимДереваРедактораJSONЛкс(мРедакторЗапросаJSON, РежимДереваТелаЗапросаJSON);

КонецПроцедуры

Процедура РежимДереваТелаОтветаJSONПриИзменении(Элемент = Неопределено)
	
	ирОбщий.ПереключитьРежимДереваРедактораJSONЛкс(мРедакторОтветаJSON, РежимДереваТелаОтветаJSON);
...
Процедура ДействияФормыИмпортироватьИзPostman(Кнопка)
	...
	Файл = ирОбщий.ВыбратьФайлЛкс(, "json");
...
Процедура СохранитьФайлДляВосстановления()
	...
	СохранитьВФайл(,, мСтруктураВосстановления.ФайлВосстановления.ПолноеИмя, Истина);
	
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКонсольHTTPЗапросов.Форма.Форма");
...
|СтатусПоиска, ИсточникСтрокиДляПоиска, ИсточникТелаЗапроса, OAuth2_Сервис, OAuth2_КлючКлиента, OAuth2_СекретКлиента, OAuth2_Scope, ИспользоватьДляВсехМетодов, Автонаименование";