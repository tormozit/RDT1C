...
Процедура НастроитьЭлементыФормы()
	...
	Если ирОбщий.СтрокиРавныЛкс(МетодЗапроса, "GET") Тогда 
		ЭлементыФормы.ИспользоватьДляВсехМетодов.ЦветТекста = Новый Цвет;
	Иначе
		ЭлементыФормы.ИспользоватьДляВсехМетодов.ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
	Для Каждого СтрокаЗаголовка Из ЗаголовкиЗапроса Цикл
...
Процедура СерверЗапросаНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если РезультатВыбора <> Неопределено Тогда
		ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(Элемент, РезультатВыбора.Значение);
	КонецЕсли; 
