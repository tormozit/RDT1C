			...
			Для Каждого Тип Из ТипЗначенияТекущий.Типы() Цикл
				...
				Иначе
					Если ирКэш.НомерВерсииПлатформыЛкс() > 803001 Тогда
						ИмяТипа = мПлатформа.ИмяОбщегоТипаИзТипа(Тип);
...
Процедура ЗаполнитьТаблицуПВХ()
	...
	Для Каждого МетаПВХ Из Метаданные.ПланыВидовХарактеристик Цикл
		Менеджер = ирОбщий.ПолучитьМенеджерЛкс(МетаПВХ);
...
Процедура ЗаполнитьПоРазницеМеждуКонфигурациями(ПолноеИмяФайла = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(ПолноеИмяФайла) Тогда
		...
		Если ирКэш.ЛиФайловаяБазаЛкс() Тогда
			// Так вроде бы нужно пароль вводить, но в зеркале базы почему то его не справшивает.
			ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpCfg """ + ПолноеИмяФайла + """", СтрокаСоединенияИнформационнойБазы(),,, "Выгружаем конфигурацию",,,,,,,,,,, Истина);
		Иначе
...
Функция ВычислитьРазницуЧерезCOM(Знач СтрокаСоединенияВременнойБазы, Знач ИмяФайлаЛога)
	...
	Если КомСоединение.КонфигурацияИзменена() Тогда
		...
		ирОбщий.СообщитьЛкс(Текст,,,, Ложь);
		ирОбщий.СообщитьЛкс(НСтр("ru='Ошибка создания временной базы. Проверьте ручное создание файловой базы с этой конфигурацией.'"),,,, Ложь);
	Иначе
		...
		Для Каждого МетаобъектТекущий Из КоллекцияТекущая Цикл
			...
			ТаблицаНабораТекущая = ирОбщий.ПустаяТаблицаИзТаблицыБДЛкс(ПолноеИмяРегистра);
			#Если Сервер И Не Сервер Тогда
...
Процедура ЗаполнитьПоРазницеМеждуКонфигурациямиЧерезОтчетСравнения(ПолноеИмяФайла = "") Экспорт
	...
	Если Не ФайлОтчета.Существует() И ирКэш.ЛиФайловаяБазаЛкс() Тогда
