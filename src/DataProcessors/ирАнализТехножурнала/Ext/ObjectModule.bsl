...
Функция ПрочитатьФайлЖурнала(Параметры) Экспорт 
	...
	РегВыражение1.Pattern = ШаблонСобытия; 
	//
...
Функция СтруктураЗапросаИзТекстаБД(ТекстБД, ТипСУБД = "", ПересобратьТекст = Ложь, ПеревестиИндексы = Истина, ПереводитьВМета = Истина, ЗаменитьHexЛитералы = Ложь,
    ...
    Если ЗначениеЗаполнено(ТипСУБД) Тогда
		Если ПереводитьВМета Тогда
			...
			ВхожденияТипов = RegExp.НайтиВхождения(Текст);
			...
			ВхожденияПараметра = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, шКорень, "ТекстЗапроса, ТипыПараметров", Ложь,,, Ложь);
			#Если Сервер И Не Сервер Тогда
			...
			Для каждого Вхождение Из ВхожденияПараметра Цикл
				...
				ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.ТипыПараметров, шТипПараметра, "Имя, Тип",,,, Ложь,,,, ВложенныеВхождения);
				#Если Сервер И Не Сервер Тогда
				...
				КонецЦикла;
				ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.Подгруппы[6], шЗначениеПараметра,,,,, Ложь,,,, ВложенныеВхождения);
				#Если Сервер И Не Сервер Тогда
					Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
				#КонецЕсли
				ИндексПараметра = 0;
...
Функция ИменаМетаданныхЗапросаСУБД(Знач ТекстЗапроса, СловарьШаблоновМетаданных, ТипСУБД = "") Экспорт
	...
	Для Каждого СтрокаСловаряМетаданных Из СловарьШаблоновМетаданных Цикл
		Если Найти(ТекстЗапроса, СтрокаСловаряМетаданных.БыстрыйМаркер) = 0 Тогда 
			Продолжить;
		КонецЕсли;
		ТаблицаВхождений = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстЗапроса,,,,,,, Истина, СтрокаСловаряМетаданных.Вычислитель,, ТаблицаВхождений);
		#Если Сервер И Не Сервер Тогда
