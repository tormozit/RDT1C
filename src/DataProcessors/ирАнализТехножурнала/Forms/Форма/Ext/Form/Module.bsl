...
Процедура ОбновитьТаблицуЖурнала(УдалитьДополнительныеСобытия = Истина, ИменаНеполезныхКолонок = "", РежимТрассы = Ложь, ЖдатьСекунд = 2, СмещениеБазовогоУровня = Неопределено) Экспорт
	...
	Тогда
		...
		Если Не РежимДозагрузки И "" + ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок <> "" Тогда
			Ответ = Вопрос("Применить при загрузке текущий фильтр отображения (" + ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(ЭлементыФормы.ТаблицаЖурнала.ОтборСтрок, 200) + ")?",
...
Процедура ДействияФормыНастройкаЖурнала(Кнопка)
	...
	Если ЗагружатьЖурналСервера Тогда
		...
		ФормаНастройки.ВариантРасположенияФайлаНастроек = ирОбщий.ВариантыРазмещенияКонфигФайлаПлатформыЛкс().АктивныйФайл;
...
Процедура ОбновитьТаблицуКонтекста(ЗначениеКонтекста)
	...
	КонецЕсли; 
	ТекущаяСтрока = ЭлементыФормы.ТаблицаЖурнала.ТекущаяСтрока;
	РезультатПоиска = ирОбщий.НайтиРегВыражениеЛкс(ЗначениеКонтекста, "(?:\n|^)\s*(?:...|([^:\n]*)\:\s+(\d+)\s+:\s+([^:\n]*))(?=\n|$)");
	Уровень = 1;
	...
	Для Каждого Вхождение Из РезультатПоиска Цикл
		...
		Если Вхождение.Группы[1] = Неопределено Тогда
			...
			СтрокаСтека.Метод = "...";
		Иначе
			СтрокаСтека.Модуль = СокрЛП(Вхождение.Группы[0]);
			СтрокаСтека.НомерСтроки = Число(Вхождение.Группы[1]);
			СтрокаСтека.СтрокаМодуля = Вхождение.Группы[2];
			СтрокаСтека.Метод = мПлатформа.НайтиИмяМетодаСтрокиМодуля(СтрокаСтека.Модуль, СтрокаСтека.НомерСтроки, СтрокаСтека.СтрокаМодуля, ТекстыМодулейКэш);
			СтрокаСтека.Уровень = Уровень;
		КонецЕсли;
		СтрокаСтека.Ссылка = ирОбщий.СсылкаСтрокиМодуляЛкс(СтрокаСтека.Модуль, СтрокаСтека.НомерСтроки);
		Уровень = Уровень + 1;
	КонецЦикла;
