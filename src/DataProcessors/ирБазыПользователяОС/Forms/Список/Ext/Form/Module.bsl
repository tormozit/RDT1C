...
Процедура ЗапуститьПриложение1С(РежимКонфигуратора)
	...
	Если РежимКонфигуратора Тогда
		ПараметрЗапуска = ирОбщий.ПараметрЗапускаПриложенияДляЭлементаСпискаБазЛкс(ИмяВСпискеБазПользователя);
		ПроцессОС = ирОбщий.ПолучитьПроцессОСЛкс(,,,,, "CONFIG" + ПараметрЗапуска);
