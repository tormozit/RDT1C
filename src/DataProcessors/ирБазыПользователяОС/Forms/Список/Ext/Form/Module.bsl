...
Процедура ОбновитьПапкиПоБазам() Экспорт 
	...
	СтрокаПапкиВсе = Папки.Строки.Найти("<Все>", "Имя");
...
Функция РодительИзИмениПапки(Знач ИмяВСписке) Экспорт
	...
	Для Каждого ИмяРодителя Из Родители Цикл
		...
		Если НовыйРезультат = Неопределено Тогда
			...
			ОбновитьИмяПапки(НовыйРезультат);
		КонецЕсли;
...
//    - 
Функция ОбновитьИмяПапки(Знач ТекущиеДанные = Неопределено) Экспорт
	Если ТекущиеДанные = Неопределено Тогда
	...
	Если ТекущиеДанные = Папки Тогда
		Возврат "";
	КонецЕсли;
	Результат = ОбновитьИмяПапки(ирОбщий.РодительСтрокиДереваЛкс(ТекущиеДанные)) + "/" + ТекущиеДанные.Имя;
...
Процедура ЗаписатьСписокБаз()  
	...
	Для Каждого ПапкиЭлемент Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(Папки) Цикл
		...
		ПапкиЭлемент.Folder = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(ПапкиЭлемент.ИмяВСписке, "/"); 
	КонецЦикла;
...
Процедура ОбновлениеОтображения()
	...
	Если Не ЭлементыФормы.БазыПользователя.ОтборСтрок.Папка.Использование Тогда
		ЭлементыФормы.Папки.ТекущаяСтрока = Папки.Строки.Найти("<Все>", "ИмяВСписке");
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзменитьСвернутостьПанельОсобыйЗапуск(Видимость)
