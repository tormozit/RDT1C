...
Функция СвойстваГлобализуемые() Экспорт
	...
	Результат.Вставить("РежимИнтерфейса");
...
Процедура ПриОткрытии()
	...
	Если НачальноеЗначениеВыбора <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	СписокРежимовИнтерфейса = ЭлементыФормы.РежимИнтерфейса.СписокВыбора;
	СписокРежимовИнтерфейса.Добавить("Taxi");
	СписокРежимовИнтерфейса.Добавить("85");
	НастроитьЭлементыФормы();
...
Процедура ЗапуститьСПараметрамиНажатие(Элемент)
	...
	КонецЕсли; 
	ПользовательЗапуска = Пользователь;
	Если ПустаяСтрока(ПользовательЗапуска) Тогда
		ПользовательЗапуска = ТекущаяСтрока.Пользователь;
	КонецЕсли;
	ПараметрыЗапуска = ирОбщий.ПараметрыЗапускаПриложения1СЛкс(ПользовательЗапуска, ТекущаяСтрока.Пароль, КодРазрешения, РежимКонфигуратора, РежимЗапуска, Ложь, ОчисткаКэшаКлиентСерверныхВызовов, 
		ДополнительныеПараметры, Ложь,, ОткрытьПортативныеИнструменты, РежимИнтерфейса, РазделениеДанных, ОтключитьАутентификациюОС, КодЯзыкаСистемы,
...
Функция ИменаПараметровРазовогоЗапуска()
	Возврат "ОткрытьОбработку, СборкаПлатформы, РежимЗапуска, ОчисткаКэшаКлиентСерверныхВызовов, КодРазрешения, ПодключитьОтладчик, СообщитьКоманднуюСтроку, ОткрытьПортативныеИнструменты, РежимИнтерфейса, РазделениеДанных, ЗапускОтИмениАдминистратора, КодЯзыкаСистемы, ПараметрЗапуска, ДополнительныеПараметры, ОтключитьАутентификациюОС, Пользователь, ЗапретРегламентныхЗаданий";
КонецФункции
