...
Функция ПодобратьСборкуПлатформы(СтрокаБазы, Знач ТолькоЕстьВРеестре = Ложь) Экспорт
	...
	Если Истина
		...
		И ирОбщий.СтрНачинаетсяСЛкс(СтрокаБазы.DefaultVersion, СтрокаБазы.Version)
	Тогда
	...
	КонецЕсли;
	Если ПодходящаяСборка = Неопределено Тогда
		Если Не ЗначениеЗаполнено(СтрокаБазы.Version) Тогда
...
Процедура ИзменитьСвернутостьПанельОсобыйЗапуск(Знач Видимость, Знач ПередЗакрытием = Ложь)
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	ИзменитьСвернутостьПанельОсобыйЗапуск(Истина, Истина);
КонецПроцедуры
