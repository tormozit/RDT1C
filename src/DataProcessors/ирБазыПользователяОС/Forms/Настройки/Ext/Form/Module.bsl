...
Процедура ПриОткрытии()
	...
	ВариантРасположенияФайлаНастроек = ирОбщий.ВариантыРазмещенияКонфигФайлаПлатформыЛкс();
	ИмяФайлаНастроекПлатформы = "\conf.cfg";
	ЭтотОбъект.УказательОбщийПлатформы = ирОбщий.КаталогНастроекПриложения1СЛкс(ВариантРасположенияФайлаНастроек.ДляТекущейВерсии) + ИмяФайлаНастроекПлатформы;
	ЭтотОбъект.УказательОбщийВсехПлатформ = ирОбщий.КаталогНастроекПриложения1СЛкс(ВариантРасположенияФайлаНастроек.ПеренаправлениеТекущейВерсии) + ИмяФайлаНастроекПлатформы;
	...
	Если Файл.Существует() Тогда
		...
		Пока СтрокаИзФайла <> Неопределено Цикл
			...
			Если ИмяПараметра = "DefaultVersion" Тогда
				Фрагменты = ирОбщий.СтрРазделитьЛкс(ирОбщий.ТекстБезПервогоФрагментаЛкс(ТекущаяСтрока, "="), "-");
