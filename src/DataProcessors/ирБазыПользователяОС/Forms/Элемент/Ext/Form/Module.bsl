...
Процедура КоманднаяПанельОбщаяОК(Кнопка = Неопределено)
	Если ПустаяСтрока(СтрокаСоединения) Тогда
		...
		Возврат;
	КонецЕсли;
	Результат = ирОбщий.СтруктураИзСтрокиТаблицыИлиДереваИлиВыборкиЛкс(БазыПользователя.ВыгрузитьКолонки("Идентификатор, СтрокаСоединения, Пользователь, ИмяВСписке, Папка, РежимЗапуска, ВерсияПлатформы, App, AppArch, WA").Добавить());
	...
	Результат.ВерсияПлатформы = ВерсияПлатформы;
