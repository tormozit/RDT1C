...
Процедура РегистраторПриИзменении(Элемент = Неопределено)
	...
	Если Не ЗначениеЗаполнено(Регистратор) Тогда
		Возврат;
	КонецЕсли;
	СтруктураОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(ИмяТаблицыРегистра, Новый Структура("Регистратор", Регистратор)); // см. ирОбщий.докОбъектБДПоКлючуРегистр
...
Процедура РегистраторНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка,, Новый Структура("Дата", Дата));
...
Процедура КПНабораЗаписейЗаписать(Кнопка)
	...
	//! КопияНабораЗаписей = Новый ТаблицаЗначений; // Чтобы проверка не ругалась
