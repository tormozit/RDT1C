	...
	Если ЗначениеЗаполнено(ИмяТаблицыРегистра) Тогда
		...
		Для Каждого ДоступноеПоле Из Компоновщик.Настройки.Выбор.ДоступныеПоляВыбора.Элементы Цикл
			...
			Если ЛиПолеОстатка(ДоступноеПоле.Поле) Тогда
				...
				ирОбщий.НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ГруппаРесурсы, ДоступноеПоле.Поле);
			Иначе
				мИменаИзмерений.Добавить("" + ДоступноеПоле.Поле);
				ирОбщий.НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(ГруппаИзмерения, ДоступноеПоле.Поле);
				ирОбщий.НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(Компоновщик.Настройки.Порядок, ДоступноеПоле.Поле);
			КонецЕсли;
		КонецЦикла;
...
Функция ЛиПолеОстатка(ИмяПоля)
	...
	ЛиПолеОстатка = Ложь;
	Для Каждого Ресурс Из мОбъектМДРегистра.Ресурсы Цикл
		Если Истина
			И Найти(ИмяПоля, "Остаток") > 0
			И Найти(ИмяПоля, Ресурс.Имя) > 0
		Тогда
			ЛиПолеОстатка = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура ТипРегистратораНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ВыбранныйТип <> Неопределено Тогда
		ОписаниеТипов = Новый ОписаниеТипов(ирОбщий.БыстрыйМассивЛкс(ВыбранныйТип));
		ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементыФормы.Регистратор, ОписаниеТипов.ПривестиЗначение(Регистратор));
	КонецЕсли;
...
Процедура РегистраторПриИзменении(Элемент = Неопределено)
	...
	Иначе
		ИмяДокумента = "";
	КонецЕсли;
	ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементыФормы.ТипРегистратора, ИмяДокумента);
	...
	Если ЭлементыФормы.КорСчет.Доступность Тогда
		ОтборОстатковРанееУстановленнных = Новый Структура("СчетКт", КорСчет);
	Иначе
		ОтборОстатковРанееУстановленнных = Новый Структура("ВидДвижения", ВидДвиженияНакопления.Приход);
	КонецЕсли;
	ОтборОстатковРанееУстановленнных.Вставить("Период", Дата);
	...
	Для Каждого РанееУстановленныйОстаток Из ТаблицаДвижений.НайтиСтроки(ОтборОстатковРанееУстановленнных) Цикл
		Для Каждого ИмяИзмерения Из мИменаИзмерений Цикл
			...
			СтруктураКлюча[ИмяИзмерения] = РанееУстановленныйОстаток[ИмяКолонкиДвижений];
		КонецЦикла;
		НайденныеСтроки = НаборЗаписей.НайтиСтроки(СтруктураКлюча);
		Если НайденныеСтроки.Количество() > 0 Тогда
			СтрокаОстатка = НайденныеСтроки[0];
		Иначе
			СтрокаОстатка = НаборЗаписей.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаОстатка, СтруктураКлюча);
		КонецЕсли;
...
Процедура ТипРегистратораПриИзменении(Элемент)
	...
	Если ТипЗнч(Регистратор) <> НовыйТип Тогда
		ОписаниеТипов = Новый ОписаниеТипов(ирОбщий.БыстрыйМассивЛкс(НовыйТип));
		ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементыФормы.Регистратор, ОписаниеТипов.ПривестиЗначение(Регистратор));
	КонецЕсли;
	ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(Элемент, ЭтаФорма);
...
Процедура КПНабораЗаписейЗаписать(Кнопка)
	...
	КопияНабораЗаписей = СтруктураОбъекта.Данные.СкопироватьКолонки();
...
Процедура ПриОткрытии()  
	...
	Если КлючУникальности <> Неопределено И ТипЗнч(КлючУникальности) = Тип("Строка") Тогда
		...
		Если ОбъектМД <> Неопределено Тогда
			ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементыФормы.ИмяТаблицыРегистра, ОбъектМД.ПолноеИмя());
		КонецЕсли; 
	КонецЕсли; 
КонецПроцедуры
