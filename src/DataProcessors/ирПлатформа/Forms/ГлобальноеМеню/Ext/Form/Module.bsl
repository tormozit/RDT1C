...
Процедура ОбновитьДоступныеКоманды()
	...
	ирКлиент.КлючиСтрокБДИзТаблицыФормыЛкс(мАктивнаяФорма, КлючТекущейСтрокиТаблицы,, Ложь,, мСерверныеМетаданныеФормы);
	ДанныеТекущегоЭлемента = ирОбщий.ДанныеЭлементаФормыЛкс(ТекущийЭлементФормы,,,, мСерверныеМетаданныеФормы);
	Если Ложь
	...
	Тогда
		...
		ОбщийТипДанныхТаблицы = ирОбщий.ОбщийТипДанныхТабличногоПоляЛкс(ТекущийЭлементФормы,,, ПолноеИмяТаблицыБД, ДанныеТекущегоЭлемента, мСерверныеМетаданныеФормы);
		Если ОбщийТипДанныхТаблицы <> "Список" Тогда
			ТекущаяКолонкаДанных = ирОбщий.ПутьКДаннымКолонкиТабличногоПоляЛкс(ТекущийЭлементФормы,, мСерверныеМетаданныеФормы);
			Если ЗначениеЗаполнено(ТекущаяКолонкаДанных) Тогда
				ТаблицаЗначений = ирКлиент.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ТекущийЭлементФормы, Новый Массив,,,,,,,,, ДанныеТекущегоЭлемента);
	...
	ЗначениеТекущегоПоля = ирКлиент.Форма_ЗначениеТекущегоПоляЛкс(мАктивнаяФорма,,, мСерверныеМетаданныеФормы);
	...
	Тогда
		...
		Тогда
			...
			НоваяКоманда.Представление = НаименованиеКоманды;
			НоваяКоманда.Пояснение = "Вставить (Alt+Shift+V) - " + ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеИзБуфера,, Истина);
		...
		Если ЗначениеИзБуфера <> Неопределено И ТипЗнч(ЗначениеИзБуфера) = ТипЗнч(КлючТекущегоПоляЯчейки) Тогда
			...
			НоваяКоманда.Представление = НаименованиеКоманды;
			НоваяКоманда.Пояснение = "Сравнить с - " + ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеИзБуфера,, Истина);
	...
	Тогда
		...
		Тогда
			...
			Тогда
				...
				Тогда
					...
					НоваяКоманда.Представление = НаименованиеКоманды;
					НоваяКоманда.Пояснение = "Найти - " + ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеИзБуфера,, Истина);
		...
		Тогда
			...
			НоваяКоманда.Пояснение = "Редактировать/отлаживать настройки списка";
		...
		Если Истина
			И ОбщийТипДанныхТаблицы = "Список"
			И ирОбщий.ЛиКорневойТипСсылкиЛкс(ПолноеИмяТаблицыБД)
		Тогда
			Если ТипЗнч(ЗначениеИзБуфера) = ТипЗнч(КлючТекущейСтрокиТаблицы) Тогда
				НаименованиеКоманды = "Найти скопированное значение";
				#Если Сервер И Не Сервер Тогда
	...
	Тогда
		...
		НоваяКоманда.Пояснение = "Отладить схему и настройки компоновки (текущее состояние) отчета в консоли компоновки данных (ИР)";
