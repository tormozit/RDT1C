...
Процедура СвойстваОбъектовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ТипЗнч(НомерСтороны) = Тип("Число") Тогда
		ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка, ВыбраннаяСтрока["Значение" + НомерСтороны],,,, ВыбраннаяСтрока.Представление + " " + НомерСтороны);
	ИначеЕсли Истина
