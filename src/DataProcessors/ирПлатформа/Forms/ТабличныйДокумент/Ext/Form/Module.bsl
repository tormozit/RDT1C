...
Процедура ПриЛюбомОткрытии() Экспорт
	Если ЗначениеЗаполнено(ПараметрИмяТекущейОбласти) Тогда
		ЭлементыФормы.ПолеТабличногоДокумента.ТекущаяОбласть = ЭлементыФормы.ПолеТабличногоДокумента.Область(ПараметрИмяТекущейОбласти);
		ЭтаФорма.ПараметрИмяТекущейОбласти = "";
	КонецЕсли;
	Если ПараметрВставитьВсеИзБуфера Тогда
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ПолеТабличногоДокумента;
		ЭлементыФормы.ПолеТабличногоДокумента.ТекущаяОбласть = ЭлементыФормы.ПолеТабличногоДокумента.Область();
		ирКлиент.ОтправитьНажатияКлавишВОкноЛкс(ирКэш.ИдентификаторПроцессаОСЛкс(), "^v", 0);
		ПараметрВставитьВсеИзБуфера = Ложь;
	КонецЕсли;
КонецПроцедуры

Процедура ОсновныеДействияФормыОК(Кнопка = Неопределено)
	
	Если мКлючИсточника <> Неопределено Тогда
		ирКлиент.ПолеТабличногоДокументаВосстановитьОформлениеТекущихСтрокЛкс(ЭтаФорма, ЭлементыФормы.ПолеТабличногоДокумента);
		ЭтаФорма.Модифицированность = Ложь;    
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ПолеТабличногоДокумента;
		ЭлементыФормы.ПолеТабличногоДокумента.ТекущаяОбласть = ЭлементыФормы.ПолеТабличногоДокумента.Область();
		ирКлиент.ОтправитьНажатияКлавишВОкноЛкс(ирКэш.ИдентификаторПроцессаОСЛкс(), "^c", 0);
		ПодключитьОбработчикОжидания("ЗакрытьИСообщитьКонфигуратору", 1, Истина);
		Возврат;
	КонецЕсли;
	Если ЛиОсновнойПриемникФайл() Тогда 
...
Процедура ЗакрытьИСообщитьКонфигуратору() Экспорт
	Закрыть();
	ИмяВремФайла = ПолучитьИмяВременногоФайла("mxl");
	ПолучитьРезультат().Записать(ИмяВремФайла);
	мПлатформа.АктивизироватьОкноКонфигуратора(мКлючИсточника, Новый Файл(ИмяВремФайла));
КонецПроцедуры
...
Процедура КоманднаяПанельТабличныйДокументИсследоватьОбласть(Кнопка)
	ирОбщий.ИсследоватьЛкс(ЭлементыФормы.ПолеТабличногоДокумента.ТекущаяОбласть);
КонецПроцедуры
...
Функция ОбработатьЯзыкиДокумента(Знач ТабДок, Знач КодЯзыкаУдалить = "", КодыУдаленныхЯзыков = Неопределено, ЯзыкиДокумента = Неопределено)
	...
	Если КодыУдаленныхЯзыков.Количество() > 0 Тогда
		Результат = ирОбщий.ОбъектИзСтрокиXMLЛкс(ирОбщий.ДокументDOMВТекстЛкс(ДокументДом));
