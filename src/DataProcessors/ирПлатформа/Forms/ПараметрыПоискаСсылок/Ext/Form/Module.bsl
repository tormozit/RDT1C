...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.ДополнятьИсключенияИзБСП.Доступность = ирКэш.ДоступнаПодсистемаПоискаСсылокБСПЛкс();
	ТекущиеПараметры = ирОбщий.ПараметрыПоискаСсылокЛкс(ИсключенияБСП);
...
Процедура ОсновныеДействияФормыПрименить(Кнопка)
	ТекущиеПараметры = ирОбщий.НовыеПараметрыПоискаСсылокЛкс(СтандартныйПоиск, ИсключаемыеТаблицы, ОтображатьЗамеры, ИсключаемыеКолонки, ДополнятьИсключенияИзБСП);
...
Процедура КПИсключаемыеТаблицыИзменить(Кнопка = Неопределено)
	...
	Если ТипЗнч(РезультатФормы) = Тип("Массив") Тогда
		...
		ДополнятьИсключенияИзБСППриИзменении();
	КонецЕсли;
КонецПроцедуры
...
Процедура ИсключаемыеТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если Истина
		И ДополнятьИсключенияИзБСП 
		И ИсключенияБСП.ИменаТаблицБСП.Найти(ДанныеСтроки.Значение) <> Неопределено
	Тогда
		ОформлениеСтроки.Ячейки.Значение.Текст = ОформлениеСтроки.Ячейки.Значение.Текст + " (БСП)";
	КонецЕсли;
КонецПроцедуры
...
Процедура ДополнятьИсключенияИзБСППриИзменении(Элемент = Неопределено)
	Если ДополнятьИсключенияИзБСП Тогда
		ИсключенияБСП = ирОбщий.ОбновитьИсключенияПоискаСсылокЛкс(ИсключаемыеТаблицы, ИсключаемыеКолонки, Истина);
	КонецЕсли;
КонецПроцедуры

Процедура ИсключаемыеКолонкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	Если Истина
		И ДополнятьИсключенияИзБСП 
		И ИсключенияБСП.ИменаКолонокБСП.Найти(ДанныеСтроки.Значение) <> Неопределено
	Тогда
		ОформлениеСтроки.Ячейки.Значение.Текст = ОформлениеСтроки.Ячейки.Значение.Текст + " (БСП)";
	КонецЕсли;
КонецПроцедуры
