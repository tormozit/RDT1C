...
Процедура КоманднаяПанельДереваОткрытьВКонфигураторе(Кнопка)
	...
	ПолноеИмяЭлементаФормы = Дерево.Строки[0].Имя;  
	Если ЭлементыФормы.Дерево.ТекущаяСтрока <> Дерево.Строки[0] Тогда
		ПолноеИмяЭлементаФормы = ПолноеИмяЭлементаФормы + ".ЭлементыФормы." + ЭлементыФормы.Дерево.ТекущаяСтрока.Имя;
	КонецЕсли;
	ирКлиент.ПерейтиКОбъектуМетаданныхВКонфигуратореЛкс(ПолноеИмяЭлементаФормы);
