...
Перем мСерверныеМетаданныеФормы; // см. ирОбщий.УправляемаяФормаБСП_СерверныеМетаданныеЛкс
Перем мВсеОбработчикиСобытий;
Перем мСочетанияКлавишОбщие;
Перем мТаблицаДляСхемыЭлементов; 
Перем мЭталонФормы;
Перем мИзмененныеЭлементы; // Массив из СтрокаДереваЗначений: см. Дерево - может содержать дубли
Перем мИзмененныеТекущиеСтраницы; // Массив из СтрокаДереваЗначений: см. Дерево - 

Процедура ПриОткрытии()
	
	ирКлиент.Форма_ПриОткрытииЛкс(ЭтаФорма,,,,,, Истина);
	Если РежимПодсистемы Тогда
		ЭтаФорма.Заголовок = "Структура форм (ИР): " + КлючУникальности;
	КонецЕсли; 
	...
	Если Истина
		И ПараметрЭлементФормы = Неопределено 
		И Форма <> Неопределено 
		И Не РежимПодсистемы
	Тогда
	...
	Если ПараметрЭлементФормы <> Неопределено Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрПутьКРеквизиту) Тогда
		СтрокаРеквизита = ирОбщий.Дерево_НайтиПоПутиСтрокойЛкс(РеквизитыФормы,, ПараметрПутьКРеквизиту);
		Если СтрокаРеквизита <> Неопределено Тогда
			ЭлементыФормы.РеквизитыФормы.ТекущаяСтрока = СтрокаРеквизита;
			ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.РеквизитыФормы;
		КонецЕсли;
	КонецЕсли;
	ЭлементыФормы.КоманднаяПанельДерева.Кнопки.ОткрытьВКонфигураторе.Доступность = ЗначениеЗаполнено(Дерево.Строки[0].Имя);
...
Процедура ИндикацияТекущегоЭлемента()
	...
	КонецЕсли; 
	Если РежимПодсистемы Тогда
		ЭтаФорма.Форма = ФормаИзСтрокиЭлемента(ТекущаяСтрока);
	КонецЕсли; 
	Если Ложь
		Или Форма = Неопределено 
	...
	ИначеЕсли ТекущаяСтрока.Тип = Тип("КнопкаКоманднойПанели") Тогда
		АктивироватьСтраницуРодителя(ТекущаяСтрока);
...
Процедура АктивироватьСтраницуРодителя(ТекущаяСтрока) Экспорт
	СтрокаСтраницыКоманднойПанели = ПолучитьПервогоРодителяПоТипу(ТекущаяСтрока, Тип("СтраницаПанели"));
	Если Истина
		И СтрокаСтраницыКоманднойПанели <> Неопределено 
		И СтрокаСтраницыКоманднойПанели.Родитель <> Неопределено
	Тогда
		ПанельСтроки = СтрокаСтраницыКоманднойПанели.Родитель.ЭлементФормы; // Панель
		ПанельСтроки.ТекущаяСтраница = СтрокаСтраницыКоманднойПанели.ЭлементФормы;
		АктивироватьСтраницуРодителя(СтрокаСтраницыКоманднойПанели.Родитель);
	КонецЕсли;
КонецПроцедуры
...
Процедура СохранитьСтарыеСвойстваЭлементаФормы(Знач ТекущаяСтрока, СтрокаСвойств, ДопСвойства = Неопределено)
	...
	Если ТекущаяСтрока.СтарыеСвойства = Неопределено Тогда
		...
		мИзмененныеЭлементы.Добавить(ТекущаяСтрока);
	КонецЕсли; 
...
Процедура ОбновитьДерево()
	...
	ИначеЕсли КлючУникальности = "ВсеИнструменты" Тогда
		УстановитьВидимостьКолонок(Ложь);
		...
		Для Каждого СтрокаИнструмента Из СписокИнструментов Цикл
			...
			Для Каждого МетаФорма Из МетаФормы Цикл
				...
				Если Найти(МетаФорма.Имя, "Кнт") Тогда
					Продолжить;
				КонецЕсли;
				ПолноеИмяФормы = МетаФорма.ПолноеИмя();
				Попытка
					ФормаЦикл = Объект.ПолучитьФорму(МетаФорма.Имя,, Новый УникальныйИдентификатор());
				Исключение
					...
					Продолжить;
				КонецПопытки;
				Если ТипЗнч(ФормаЦикл) = Тип("Форма") Тогда
					ирКлиент.ПодготовитьЭлементыФормыЛкс(ФормаЦикл);
					...
					СтрокаДереваФормы.Картинка = ФормаЦикл.КартинкаЗаголовка;
					СтрокаДереваФормы.ЭлементФормы = ФормаЦикл;
					ДобавитьФорму(ФормаЦикл, СтрокаДереваФормы);
					...
					Иначе
						СтрокаДереваФормы.Текст = СтрокаДереваФормы.Текст + " (" + МетаФорма.Имя + ")";
					КонецЕсли; 
				КонецЕсли; 
			КонецЦикла;
	...
	Иначе
		УстановитьВидимостьКолонок(Ложь);
		Индикатор2 = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ИменаОбъектовМД.Количество());
		Для Каждого ИмяОбъектаМД Из ИменаОбъектовМД Цикл
			ирОбщий.ОбработатьИндикаторЛкс(Индикатор2);
			МетаОбъект = Метаданные.НайтиПоПолномуИмени(ИмяОбъектаМД);
			КорневойТип = ирОбщий.ПервыйФрагментЛкс(ИмяОбъектаМД);
			Если КорневойТип = "ОбщаяФорма" Тогда
				МетаФормы = ирОбщий.ЗначенияВМассивЛкс(МетаОбъект);
			Иначе
				Попытка
					МетаФормы = МетаОбъект.Формы;
				Исключение
			...
			Для Каждого МетаФорма Из МетаФормы Цикл
				...
				ПолноеИмяФормы = МетаФорма.ПолноеИмя();
				Попытка
					ФормаЦикл = мПлатформа.ПассивнаяФормаПоИмениКэш(ПолноеИмяФормы);
				Исключение
					Если МетаФорма.Имя <> "ОписаниеПлатформы" Тогда
						ирОбщий.СообщитьЛкс("Ошибка при получении формы " + ПолноеИмяФормы + ": " + ОписаниеОшибки(), СтатусСообщения.ОченьВажное);
					КонецЕсли;
				...
				КонецПопытки;
				Если Ложь
					Или ФормаЦикл = Неопределено 
					Или ТипЗнч(ФормаЦикл) = Тип("Структура")
				Тогда
					Продолжить;
				КонецЕсли;
				Если СтрокаДереваОбъекта = Неопределено Тогда
					...
					СтрокаДереваОбъекта.Имя = МетаОбъект.ПолноеИмя();
					СтрокаДереваОбъекта.Видимость = Истина;
					СтрокаДереваОбъекта.Картинка = ирКлиент.КартинкаКорневогоТипаМДЛкс(КорневойТип);
					СтрокаДереваОбъекта.Доступность = Истина;
					СтрокаДереваОбъекта.ПредставлениеТипа = "Объект";
				КонецЕсли; 
				СтрокаДереваФормы = СтрокаДереваОбъекта.Строки.Добавить();
				...
				СтрокаДереваФормы.ЭлементФормы = ФормаЦикл;
				ДобавитьФорму(ФормаЦикл, СтрокаДереваФормы);
				СтрокаДереваФормы.Текст = МетаФорма.Представление() + " (" + МетаФорма.Имя + ")";
			КонецЦикла;
		КонецЦикла;
		ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
	КонецЕсли;
	Дерево.Строки.Сортировать("Текст");
	
КонецПроцедуры

Процедура УстановитьВидимостьКолонок(Знач НоваяВидимость) Экспорт
	ЭлементыФормы.Дерево.Колонки.Динамический.Видимость = НоваяВидимость;
	ЭлементыФормы.Дерево.Колонки.ПутьКДаннымЭлемента.Видимость = НоваяВидимость;
	ЭлементыФормы.Дерево.Колонки.Имя.Видимость = НоваяВидимость;
	ЭлементыФормы.Дерево.Колонки.ЕстьОбработчики.Видимость = НоваяВидимость;
	ЭлементыФормы.Дерево.Колонки.Верх.Видимость = НоваяВидимость;
	ЭлементыФормы.Найденные.Колонки.Динамический.Видимость = НоваяВидимость;
	ЭлементыФормы.Найденные.Колонки.ПутьКДаннымЭлемента.Видимость = НоваяВидимость;
	ЭлементыФормы.Найденные.Колонки.Имя.Видимость = НоваяВидимость;
КонецПроцедуры

Процедура ДобавитьФорму(Знач Форма, Знач КорневаяСтрокаФормы = Неопределено)
	...
	ДобавитьФормуИзСнимка(СтрокаФормы, ТипФормы, Форма, КорневаяСтрокаФормы);
	Если Не РежимПодсистемы Тогда // ускорение
...
//   СнимокФормы - Строка - 
//   ТипФормы - Тип - 
//   Форма - Произвольный, Структура, Форма, ФормаКлиентскогоПриложения, Неопределено - 
//   КорневаяСтрокаФормы - СтрокаДереваЗначений, Неопределено - 
Процедура ДобавитьФормуИзСнимка(Знач СнимокФормы, Знач ТипФормы, Знач Форма = Неопределено, Знач КорневаяСтрокаФормы = Неопределено) Экспорт
	XMLСтрокаФормы = ирОбщий.СтрокаВнутрВХМЛТелоЛкс(СнимокФормы);
	//ирОбщий.Ис(XMLСтрокаФормы)
	Попытка
		ДокументDOM = ирОбщий.ТекстВДокументDOMЛкс(XMLСтрокаФормы);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		Если мПлатформа.ПроверитьТекстОшибкиРазбораСнимкаФормы(ОписаниеОшибки) Тогда
	...
	#КонецЕсли
	//РазыменовательПИ = Новый РазыменовательПространствИменDOM(ДокументDOM); /// Долго!
	РазыменовательПИ = Новый РазыменовательПространствИменDOM(Новый Соответствие);
	мЭталонФормы = Неопределено;
	мСерверныеМетаданныеФормы = Неопределено;
	Если ТипФормы = Тип("Форма") Тогда
		СлужебныеДанныеФормы = ирОбщий.СлужебныеДанныеФормыЛкс(Форма);
		Если Не РежимПодсистемы И Форма <> Неопределено Тогда // ускорение
			Если ЗначениеЗаполнено(СлужебныеДанныеФормы.ИмяФормы) Тогда
				мЭталонФормы = мПлатформа.ПассивнаяФормаПоИмениКэш(СлужебныеДанныеФормы.ИмяФормы);
			КонецЕсли;
		КонецЕсли;
		Если КорневаяСтрокаФормы = Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
		КорневаяСтрокаФормы.Тип = ТипФормы;
		...
		КонецЕсли;
		Если ирОбщий.СвойствоСтруктурыЛкс(СлужебныеДанныеФормы, "ИмяФормы") <> Неопределено Тогда
			КорневаяСтрокаФормы.Имя = СлужебныеДанныеФормы.ИмяФормы;
			СтарыйЗаголовок = Форма.Заголовок;
			Если СлужебныеДанныеФормы.Свойство("ОригинальныйЗаголовок") Тогда
				ирКлиент.ФормаОбъекта_ОбновитьЗаголовокЛкс(Форма,, Истина, Ложь);
			КонецЕсли;
			КорневаяСтрокаФормы.Текст = Форма.Заголовок;
			Форма.Заголовок = СтарыйЗаголовок;
		КонецЕсли;
	...
	Иначе
		#Если Сервер И Не Сервер Тогда
			...
			Форма = 0; // ФормаКлиентскогоПриложения
		#КонецЕсли
		...
		Если Не РежимПодсистемы Тогда // ускорение
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(Форма.ИмяФормы) Тогда
				мЭталонФормы = мПлатформа.ПассивнаяФормаПоИмениКэш(Форма.ИмяФормы);
			КонецЕсли;
		КонецЕсли;
		...
		Пока Истина Цикл
			...
			Если УзелЭлемента = Неопределено Тогда
				Прервать;
			КонецЕсли;
			//ИмяГрафическогоЭлемента = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелЭлемента);
			ИмяГрафическогоЭлемента = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(УзелЭлемента.ТекстовоеСодержимое);
			Если Истина
		...
		Пока Истина Цикл
			...
			Для Каждого СтрокаЭлемента Из ПодходящиеСтрокиЭлементов Цикл
				...
				Если БазовыйУзел.ДочерниеУзлы.Количество() > 4 Тогда
					ЗаголовокЭлемента = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(БазовыйУзел.ДочерниеУзлы[4]);
				КонецЕсли;
				Если Не ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
					...
					Если УзелЗаголовка <> Неопределено Тогда
						ЗаголовокЭлемента = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелЗаголовка.ДочерниеУзлы[1]);
					КонецЕсли;
				КонецЕсли;
				...
				Если УзелПодсказки <> Неопределено Тогда
					ОписаниеЭлемента.Подсказка = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелПодсказки.ДочерниеУзлы[1]);
				КонецЕсли;
				Если Ложь
		...
		//	ОписанияЭлементов.Вставить(СтрокаЭлемента.Имя, ОписаниеЭлемента);
		//КонецЦикла;
		НомерИзданияПлатформы = ирКэш.НомерИзданияПлатформыЛкс();
		ТаблицаИндексов = Неопределено;
		мТаблицаДляСхемыЭлементов = Новый ТаблицаЗначений;
		Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(Форма.Элементы.Количество(), "Элементы формы");
		Для Каждого ЭлементФормы Из Форма.Элементы Цикл
			...
			КонецПопытки;
			Попытка
				мТаблицаДляСхемыЭлементов.Колонки.Добавить(ИмяЭлемента, Новый ОписаниеТипов("Строка"));
			Исключение
				// Ошибка платформы - задублированы имена https://devtool1c.ucoz.ru/forum/2-2943-1
			КонецПопытки;
			Если ТипЗнч(ЭлементФормы) = Тип("ТаблицаФормы") Тогда
				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					Пока Истина Цикл
						...
						Если УзелЗаголовка <> Неопределено Тогда
							ЗаголовокКолонки = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелЗаголовка.ДочерниеУзлы[1]);
						Иначе
							ЗаголовокКолонки = "";
						КонецЕсли;
						...
						УзелПодсказки = ИтераторЗаголовка.ПолучитьСледующий();
						Если УзелПодсказки <> Неопределено Тогда
							ПодсказкаКолонки = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелПодсказки.ДочерниеУзлы[1]);
						Иначе
							ПодсказкаКолонки = "";
						КонецЕсли;
				...
				Если НастройкиФормыDOM <> Неопределено Тогда
					...
					Если УзелНастроек <> Неопределено Тогда
						...
						Для Счетчик = 2 По ИзмененияСоставаКолонок.ДочерниеУзлы.Количество() Цикл
							СвойстваКолонки = НовыйСвойстваКолонки();
							...
							Если Форма.Элементы.Найти(ИмяКолонки) = Неопределено Тогда
								...
								Иначе
									ПутьКДаннымКолонки = "";
								КонецЕсли;
								СвойстваКолонки.ПутьКДанным = ПутьКДаннымКолонки;
							КонецЕсли;
							СвойстваКолонки.Имя = ИмяКолонки;
			...
			Тогда
				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					ГруппыКнопок.Колонки.Добавить("Кнопки"); // Массив из см. ОписаниеКнопкиУправляемойФормы
					...
					Пока Истина Цикл
						...
						Пока Истина Цикл
							...
							Если УзелКнопки = Неопределено Тогда
								Прервать;
							КонецЕсли;
							Если НомерИзданияПлатформы = "85" Тогда
								УзелЗаголовка = УзелКнопки.ДочерниеУзлы[1].ДочерниеУзлы[0];
							Иначе
								УзелЗаголовка = УзелКнопки.ДочерниеУзлы[7]; 
							КонецЕсли;
							ЗаголовокКнопки = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелЗаголовка, УзелЗаголовка);
							ПодсказкиКнопки = мПлатформа.ТекстИзМногострочногоЛитералаУзлаДОМ(УзелЗаголовка, УзелЗаголовка);
							...
							Кнопки.Добавить(ОписаниеКнопкиУправляемойФормы(ЗаголовокКнопки, ИндексПодменю, Картинка, ПодсказкиКнопки));
							Если Истина
	...
	Если Не Открыта() И Форма = Неопределено Тогда
		Открыть();
	КонецЕсли;
КонецПроцедуры

Функция ОписаниеКнопкиУправляемойФормы(Знач ЗаголовокКнопки, Знач ИндексПодменю, Знач Картинка, Знач ПодсказкиКнопки) Экспорт
	Возврат Новый Структура("Заголовок, Подсказка, ИндексПодменю, ЧислоПодчиненных, Видимость, Картинка", ЗаголовокКнопки, ПодсказкиКнопки, ИндексПодменю, 0, Истина, Картинка);
КонецФункции

Функция НовыйСвойстваКолонки() Экспорт
	Возврат Новый Структура("Имя, ПутьКДанным");
КонецФункции
...
Процедура ДобавитьРеквизитыУправляемойФормы(РеквизитыФормы, Знач Форма, Знач ПутьКРодителю = "", ДокументДОМ = Неопределено) Экспорт
	Для Каждого ЛиДляКоллекции Из ирОбщий.ЗначенияВМассивЛкс(Ложь, Истина) Цикл
		Для Каждого КлючИЗначение Из ИменаРеквизитовФормы(Форма, ПутьКРодителю, ЛиДляКоллекции,, ДокументДОМ).Все Цикл
			...
			ЗначениеВычислено = Истина;
			Попытка
				ЗначениеРеквизита = Вычислить("Форма." + ПутьКРеквизиту);
			Исключение
				ЗначениеВычислено = Ложь;
			КонецПопытки;
			Если ЗначениеВычислено Тогда
				СтрокаРеквизита.Значение = ирОбщий.РасширенноеПредставлениеЛкс(ЗначениеРеквизита,,,,, Истина);
			КонецЕсли;
			ДобавитьРеквизитыУправляемойФормы(СтрокаРеквизита, Форма, ПутьКРеквизиту, ДокументДОМ);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗапроситьСерверныеМетаданные()
	...
	Если ТипЗнч(Форма) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
		...
		Если мСерверныеМетаданныеФормы <> Неопределено Тогда
			...
			ПутиКДанным = мСерверныеМетаданныеФормы.ПутиКДанным;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаДерева Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(Дерево) Цикл
		...
		КонецЕсли;
		Если Истина
			И ПутиКДанным <> Неопределено 
			И СтрокаДерева <> Дерево.Строки[0]
			И ПутиКДанным.Свойство(СтрокаДерева.Имя)
		Тогда
			СтрокаДерева.ПутьКДаннымЭлемента = ПутиКДанным[СтрокаДерева.Имя];
		КонецЕсли;
		Если мВсеОбработчикиСобытий <> Неопределено Тогда
...
Процедура ДобавитьОписаниеКнопкиВПолныйСписокКнопок(Знач ОписаниеКнопки, Знач ПолныеКнопки, Знач СоответствиеКнопокКорня, Знач ВиртуальнаяГруппа = Неопределено, Знач ТолькоВЕще = Неопределено)
...
Процедура ДобавитьГруппуКолонокФормы(СтрокаДЗ, ГруппаТаблицыФормы, СвойстваКолонок, КоличествоВидимыхДобавлено = 0, Группировка = Неопределено)
	...
	Для Позиция = 0 По ПодчиненныеЭлементы.Количество() - 1 Цикл
		...
		Если СвойстваКолонок.Свойство(ИмяПозиции) Тогда 
			НастройкиКолонки = СвойстваКолонок[ИмяПозиции]; // см. НовыйСвойстваКолонки
...
Процедура ЗаполнитьСтрокуЭлементаУправляемойФормы(СтрокаДерева, Роль = "", ЭффективнаяВидимость = Неопределено)
	...
	Если Истина
		...
		И Не РежимПодсистемы // ускорение
	Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ЭлементФормы.Имя) Тогда
			ирОбщий.ДанныеЭлементаФормыЛкс(ЭлементФормы, СтрокаДерева.ПутьКДаннымЭлемента, Форма,, мСерверныеМетаданныеФормы);
		КонецЕсли;
	КонецЕсли;
	...
	КонецЕсли; 
	Если Истина
		И ТипЗнч(мЭталонФормы) = ирОбщий.ТипУправляемаяФормаЛкс()
		И СтрокаДерева.Тип <> ирОбщий.ТипУправляемаяФормаЛкс()
	Тогда     
		//! мЭталонФормы = 0 // ФормаКлиентскогоПриложения
		СтрокаДерева.Динамический = мЭталонФормы.Элементы.Найти(ЭлементФормы.Имя) = Неопределено;
	КонецЕсли;
	...
	Тогда
		...
		мИзмененныеТекущиеСтраницы.Добавить(СтрокаДерева);
	КонецЕсли; 
...
Процедура ЗаполнитьСтрокуЭлементаОбычнойФормы(СтрокаДерева, Роль = "")
	...
	КонецЕсли; 
	Если ТипЗнч(мЭталонФормы) = Тип("Форма") Тогда
		//! мЭталонФормы = 0 // Форма
		КоллекцияПроверки = мЭталонФормы.ЭлементыФормы;
		Если СтрокаДерева.Тип = Тип("КнопкаКоманднойПанели") Тогда
			СтрокаБлижайшегоЭлементаФормы = ПолучитьПоследнегоРодителяПоТипу(СтрокаДерева.Родитель, Тип("КнопкаКоманднойПанели"));
			Если СтрокаБлижайшегоЭлементаФормы.Тип = Тип("КоманднаяПанель") Тогда
				БлижайшийЭлементЭталона = КоллекцияПроверки.Найти(СтрокаБлижайшегоЭлементаФормы.Имя); // КоманднаяПанель
			Иначе
				СтрокаКоманднойПанели = ПолучитьПервогоРодителяПоТипу(СтрокаДерева, Тип("КоманднаяПанель"));
				БлижайшийЭлементЭталона = КоллекцияПроверки.Найти(СтрокаКоманднойПанели.Имя); // КоманднаяПанель
				Если БлижайшийЭлементЭталона <> Неопределено Тогда
					БлижайшийЭлементЭталона = БлижайшийЭлементЭталона.Кнопки.Найти(СтрокаБлижайшегоЭлементаФормы.Имя);
				КонецЕсли;
				Если Ложь
					Или ТипЗнч(БлижайшийЭлементЭталона) <> Тип("КнопкаКоманднойПанели") 
					Или БлижайшийЭлементЭталона.ТипКнопки <> ТипКнопкиКоманднойПанели.Подменю
				Тогда
					БлижайшийЭлементЭталона = Неопределено;
				КонецЕсли;
			КонецЕсли;
			Если БлижайшийЭлементЭталона = Неопределено Тогда
				КоллекцияПроверки = Новый Массив;
			Иначе
				КоллекцияПроверки = БлижайшийЭлементЭталона.Кнопки;
			КонецЕсли;
		ИначеЕсли СтрокаДерева.Тип = Тип("КолонкаТабличногоПоля") Тогда
			СтрокаБлижайшегоЭлементаФормы = ПолучитьПервогоРодителяПоТипу(СтрокаДерева, Тип("ТабличноеПоле"));
			БлижайшийЭлементЭталона = КоллекцияПроверки.Найти(СтрокаБлижайшегоЭлементаФормы.Имя); // ТабличноеПоле
			Если БлижайшийЭлементЭталона = Неопределено Тогда
				КоллекцияПроверки = Новый Массив;
			Иначе
				КоллекцияПроверки = БлижайшийЭлементЭталона.Колонки;
			КонецЕсли;
		Иначе
			СтрокаБлижайшегоЭлементаФормы = СтрокаДерева;
		КонецЕсли;
		Попытка
			ИмяЭлемента = ЭлементФормы.Имя;
		Исключение
		...
		Если ИмяЭлемента <> Неопределено Тогда
			СтрокаДерева.Динамический = Истина
				И Не СтрокаБлижайшегоЭлементаФормы.Динамический 
				И КоллекцияПроверки.Найти(ИмяЭлемента) = Неопределено;
		КонецЕсли;
	КонецЕсли;
	РезультирующиеПризнаки = РезультирующийПризнакСтрокиДерева(СтрокаДерева);
	ЗаполнитьЗначенияСвойств(СтрокаДерева, РезультирующиеПризнаки); 
...
Процедура ОбойтиУзелДереваЭлементовОбычнойФормы(Знач КорневаяСтрокаФормы, Знач Форма = Неопределено, Знач Узел, Знач СтрокаДЗ, Знач КорневойЭлемент = Неопределено)
	...
	Для каждого УзелЭлементаФормы Из Узел.ДочерниеУзлы Цикл
		...
		Иначе
			...
			КонецЕсли; 
			ПоследнийЭлементDOM = СвойстваЭлФормы[СвойстваЭлФормы.Количество() - 1];
			ЭтоПанель = Ложь
				Или ПоследнийЭлементDOM.ДочерниеУзлы.Количество() > 1 // ускорение
				Или ПоследнийЭлементDOM.ТекстовоеСодержимое <> "0";
			Если НомерВерсииПлатформы > 803001 И НомерВерсииПлатформы <= 803008 Тогда
				// Видимо тупоковое изменение формата. Позже вернулись к старому
				...
				ИмяЭлементаФормы = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(СвойстваЭлФормыСИменем[2].ТекстовоеСодержимое);
			Иначе
				...
				ИмяЭлементаФормы = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(СвойстваЭлФормыСИменем[1].ТекстовоеСодержимое);
			КонецЕсли; 
			Если Форма <> Неопределено Тогда
				ЭлементФормы = Форма.ЭлементыФормы[ИмяЭлементаФормы];
				Если Ложь
			...
			КонецЕсли;
			Если КорневойЭлемент = Неопределено Тогда
				Пока СтрокаДЗ.Строки.Количество() <= ИндексСтраницы Цикл
					СтрокаСтраницы = СтрокаДЗ.Строки.Добавить();
					СтрокаСтраницы.Имя = "<Страница" + СтрокаДЗ.Строки.Количество() + ">";
					ЗаполнитьСтрокуЭлементаФормы(СтрокаСтраницы);
				КонецЦикла;
			...
			Иначе
				НовСтрокаДЗ.Имя = ИмяЭлементаФормы;
				//НовСтрокаДЗ.Тип = ЗначениеИзСтрокиВнутр("{""T""," + "e69bf21d-97b2-4f37-86db-675aea9ec2cb" + "}");
				ЗаполнитьСтрокуЭлементаФормы(НовСтрокаДЗ);
			КонецЕсли;
			Если ЭтоПанель Тогда
				ОбойтиУзелДереваЭлементовОбычнойФормы(КорневаяСтрокаФормы, Форма, ПоследнийЭлементDOM, НовСтрокаДЗ);
			КонецЕсли;
			Если НовСтрокаДЗ <> СтрокаДЗ И Форма <> Неопределено Тогда
...
Процедура ЗаполнитьСтрокиИзКнопокОбычнойФормы(Кнопки, СтрокиДерева)
	...
	Для Каждого Кнопка Из Кнопки Цикл
		...
		Если Истина
			И Окончание <> ""
			И Не ирОбщий.СтрКончаетсяНаЛкс(ПредставлениеСочетаниеКлавиш, Окончание) 
		Тогда
...
Процедура ДеревоПриАктивизацииСтроки(Элемент)
	...
	Если мТекущаяСтрока.Родитель = Неопределено Тогда
		...
		Если Форма <> Неопределено Тогда
			Если ТипЗнч(Форма) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
				ЭтаФорма.ОписаниеТекущейСтроки = ОписаниеТекущейСтроки + Символы.ПС + "ИмяФормы = " + Форма.ИмяФормы;
			КонецЕсли;
			Если ЗначениеЗаполнено(Форма.КлючСохраненияПоложенияОкна) Тогда
				ЭтаФорма.ОписаниеТекущейСтроки = ОписаниеТекущейСтроки + Символы.ПС + "КлючСохраненияПоложенияОкна = " + Форма.КлючСохраненияПоложенияОкна;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ИндикацияТекущегоЭлемента();
	Если РежимПодсистемы Тогда
		ЭтаФорма.Форма = ФормаИзСтрокиЭлемента();
		Если Форма <> Неопределено Тогда
	...
	Если мСерверныеМетаданныеФормы <> Неопределено Тогда
		Если ТипЗнч(ЭлементФормы) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
			КомпоновщикУсловий.ЗагрузитьНастройки(мСерверныеМетаданныеФормы.Компоновка);
		Иначе
			СсылающиесяЭлементыОформления = Новый Соответствие;
			...
			ирОбщий.СкопироватьЭлементыКомпоновкиЛкс(КомпоновщикУсловий.Настройки.УсловноеОформление, мСерверныеМетаданныеФормы.Компоновка.УсловноеОформление,,,, ЭлементДляКопирования);
		КонецЕсли; 
	КонецЕсли; 
	...
	Иначе
		Если ТипЗнч(мТекущаяСтрока.ЭлементФормы) = Тип("КнопкаФормы") Тогда
			Если ПустаяСтрока(мТекущаяСтрока.ЭлементФормы.ИмяКоманды) Тогда
				МаркерОбщаяКоманда = "ОбщаяКоманда";
				ИмяРодителяФормы = ирОбщий.ПервыеФрагментыЛкс(Форма.ИмяФормы);
				МаркерКомандаОбъекта = СтрЗаменить(ИмяРодителяФормы, ".", "");
				Если Найти(мТекущаяСтрока.ЭлементФормы.Имя, МаркерОбщаяКоманда) Тогда
					СтрокаОбработчика = ОбработчикиСобытий.Добавить();
					СтрокаОбработчика.Событие = "Команда";
					СтрокаОбработчика.Обработчик = МаркерОбщаяКоманда + "." + ирОбщий.ПоследнийФрагментЛкс(мТекущаяСтрока.ЭлементФормы.Имя, МаркерОбщаяКоманда);
				ИначеЕсли Найти(мТекущаяСтрока.ЭлементФормы.Имя, МаркерКомандаОбъекта) Тогда
					ОбъектМД = Метаданные.НайтиПоПолномуИмени(ИмяРодителяФормы);
					Если ОбъектМД <> Неопределено Тогда
						СтрокаОбработчика = ОбработчикиСобытий.Добавить();
						СтрокаОбработчика.Событие = "Команда";
						СтрокаОбработчика.Обработчик = ОбъектМД.ПолноеИмя() + ".Команда." + ирОбщий.ПоследнийФрагментЛкс(мТекущаяСтрока.ЭлементФормы.Имя, МаркерКомандаОбъекта);
					КонецЕсли;
				КонецЕсли;
			Иначе
				СтрокаОбработчика = ОбработчикиСобытий.Добавить();
				...
				Если мСерверныеМетаданныеФормы <> Неопределено Тогда
					//! Форма = 0 // см. Обработка.ирПлатформа.Форма.ПодключаемыеКомадыБСПКнт
					ТаблицаПодключаемыхКоманд = ПолучитьИзВременногоХранилища(Форма.ПараметрыПодключаемыхКоманд.АдресТаблицыКоманд);
					СтрокаКоманды = ТаблицаПодключаемыхКоманд.Найти(мТекущаяСтрока.Имя, "ИмяВФорме");
					Если СтрокаКоманды <> Неопределено Тогда
						СтрокаОбработчика = ОбработчикиСобытий.Добавить();
						СтрокаОбработчика.Событие = "ОбработчикКомандыБСП";
						Если Найти(СтрокаКоманды.Обработчик, ".") Тогда
							СтрокаОбработчика.Обработчик = СтрокаКоманды.Обработчик;
							// АФВ
							Если СтрокаОбработчика.Обработчик = "СлужебныйМодульКлиент.ВыполнитьПодключаемуюКомандуОбработчикаОбъекта" Тогда
								МодульАФВ = ирОбщий.ОбщийМодульЛкс("СлужебныйВызовСервераПовтИсп");
								СтрокаОбработчика = ОбработчикиСобытий.Добавить();
								СтрокаОбработчика.Событие = "ОбработчикОбъектаАФВ";
								ПустаяСсылка = Справочники.ОбработчикиОбъектов.ПустаяСсылка(); //@skip-check unknown-method-property
								СтрокаОбработчика.Обработчик = МодульАФВ.СсылкаИзИдентификатора(ПустаяСсылка, СтрокаКоманды.Идентификатор);
							ИначеЕсли СтрокаОбработчика.Обработчик = "СлужебныйМодульКлиент.ОткрытьДинамическийСписокСвязанныхОбъектовИзКнопки" Тогда
								МодульАФВ = ирОбщий.ОбщийМодульЛкс("СлужебныйВызовСервераПовтИсп");
								СтрокаОбработчика = ОбработчикиСобытий.Добавить();
								СтрокаОбработчика.Событие = "СвязьОбъектовАФВ";
								ПустаяСсылка = Справочники.СвязиОбъектов.ПустаяСсылка(); //@skip-check unknown-method-property
								СвязьОбъектов = МодульАФВ.СсылкаИзИдентификатора(ПустаяСсылка, СтрокаКоманды.Идентификатор);
								СтрокаОбработчика.Обработчик = СвязьОбъектов;
							КонецЕсли;
						Иначе
							Модуль = СтрокаКоманды.Менеджер;
							Если ЗначениеЗаполнено(СтрокаКоманды.ИмяФормы) Тогда
								Модуль = Модуль + ".Форма." + СтрокаКоманды.ИмяФормы;
							КонецЕсли;
							СтрокаОбработчика.Обработчик = Модуль;
							Если ЗначениеЗаполнено(СтрокаКоманды.Обработчик) Тогда
								СтрокаОбработчика.Обработчик = СтрокаОбработчика.Обработчик + "." + СтрокаКоманды.Обработчик;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
...
Процедура ВосстановитьСвойстваЭлементовФормы(ВосстанавливатьТекущиеСтраницы = Ложь)
	...
	//	КонецПопытки;
	//КонецЕсли;
	Для Каждого СтрокаДерева Из мИзмененныеЭлементы Цикл
		ВосстановитьСтарыеСвойстваЭлементаФормы(СтрокаДерева, Истина);
	КонецЦикла;
	мИзмененныеЭлементы.Очистить();
	Если ВосстанавливатьТекущиеСтраницы Тогда
		Для Каждого СтрокаДерева Из мИзмененныеТекущиеСтраницы Цикл
			Если СтрокаДерева.СтараяТекущаяСтраница <> Неопределено Тогда
				СтрокаДерева.ЭлементФормы.ТекущаяСтраница = СтрокаДерева.СтараяТекущаяСтраница;
			КонецЕсли;
		КонецЦикла;
		мИзмененныеТекущиеСтраницы.Очистить();
	КонецЕсли;
КонецПроцедуры
...
Процедура ДеревоВыбор(Элемент = Неопределено, ВыбраннаяСтрока = Неопределено, Колонка = Неопределено, СтандартнаяОбработка = Истина)
	Элемент = ЭлементыФормы.Дерево;
	...
	Если Колонка = ЭлементыФормы.Дерево.Колонки.Текст Тогда
		Если РежимПодсистемы Тогда
			ЭтаФорма.Форма = ФормаИзСтрокиЭлемента(ВыбраннаяСтрока);
			Если Истина
				И Форма <> Неопределено
				И Не Форма.Открыта()
			Тогда
	...
	ИначеЕсли Колонка = ЭлементыФормы.Дерево.Колонки.ПутьКДаннымЭлемента Тогда
		Фрагменты = ирОбщий.СтрРазделитьЛкс(ВыбраннаяСтрока.ПутьКДаннымЭлемента);
		ИмяРеквизита = ирОбщий.ПервыйФрагментЛкс(Фрагменты[0], "[");
		ТекущаяСтрока = РеквизитыФормы.Строки.Найти(ИмяРеквизита, "Имя");
		Если ТекущаяСтрока <> Неопределено И Фрагменты.Количество() > 1 Тогда
			ИмяРеквизита = ирОбщий.ПервыйФрагментЛкс(Фрагменты[1], "[");
			ДочерняяСтрока = ТекущаяСтрока.Строки.Найти(ИмяРеквизита, "Имя");
			Если ДочерняяСтрока <> Неопределено Тогда
				ТекущаяСтрока = ДочерняяСтрока;
			КонецЕсли;
		КонецЕсли;
		Если ТекущаяСтрока <> Неопределено Тогда
			ЭлементыФормы.РеквизитыФормы.ТекущаяСтрока = ТекущаяСтрока;
			ЭлементыФормы.ПанельЭлемента.ТекущаяСтраница = ЭлементыФормы.ПанельЭлемента.Страницы.РеквизитыФормы;
		КонецЕсли;
	ИначеЕсли Колонка = ЭлементыФормы.Дерево.Колонки.ЕстьУсловноеОформление Тогда
		ЭлементыФормы.ПанельЭлемента.ТекущаяСтраница = ЭлементыФормы.ПанельЭлемента.Страницы.УсловноеОформление;
	ИначеЕсли Колонка = ЭлементыФормы.Дерево.Колонки.ЕстьОбработчики Тогда
		ЭлементыФормы.ПанельЭлемента.ТекущаяСтраница = ЭлементыФормы.ПанельЭлемента.Страницы.ОбработчикиСобытий;
	КонецЕсли;
	Если Форма = Неопределено Тогда
...
//    ? - 
Функция ФормаИзСтрокиЭлемента(Знач СтрокаОбъекта = Неопределено) Экспорт
	Если СтрокаОбъекта = Неопределено Тогда
		СтрокаОбъекта = ЭлементыФормы.Дерево.ТекущаяСтрока;
	КонецЕсли;
	Если СтрокаОбъекта = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Возврат Форма; // Форма, ФормаКлиентскогоПриложения
КонецФункции
...
Процедура СтрокаПоискаАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
	...
	Если Истина
		И Не РежимПодсистемы
		И ирКлиент.ПромежуточноеОбновлениеСтроковогоЗначенияПоляВводаЛкс(ЭтаФорма, Элемент, Текст) 
	Тогда 
		НайтиСтрокиДерева();
	КонецЕсли;
...
Процедура НайтиСтрокиДерева(Знач ИскомаяСтрока = Неопределено)
	...
	ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюЛкс(Найденные, "Тип", );
...
Процедура УсловноеОформлениеПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирКлиент.УсловноеОформлениеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, КомпоновщикУсловий, КомпоновщикПолей);
...
Процедура ОбработчикиСобытийВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		...
		Тогда
			Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ВыбраннаяСтрока.Обработчик) Тогда
				ОткрытьЗначение(ВыбраннаяСтрока.Обработчик);
			ИначеЕсли Найти(ТекущаяСтрока.Обработчик, "<") > 0 Тогда
...
Процедура РеквизитыФормыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.РеквизитыФормы.Колонки.Значение Тогда
		Если ЗначениеЗаполнено(ВыбраннаяСтрока[Колонка.Данные]) Тогда
			ирКлиент.ОткрытьЗначениеЛкс(Форма[ВыбраннаяСтрока.Имя]);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПанельЭлементаПриСменеСтраницы(Элемент, ТекущаяСтраница)
	Если Истина
		И Элемент.Страницы[ТекущаяСтраница] = Элемент.Страницы.УсловноеОформление
		И Дерево.Строки[0].ЕстьУсловноеОформление 
		И КомпоновщикПолей.Настройки.ДоступныеПоляВыбора.Элементы.Количество() = 0 
	Тогда
		СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоДеревуПолейЛкс(РеквизитыФормы);
		КомпоновщикУсловий.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновки));
		СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицеЗначенийЛкс(мТаблицаДляСхемыЭлементов);
		КомпоновщикПолей.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновки));
		мТаблицаДляСхемыЭлементов = Неопределено;
	КонецЕсли;
КонецПроцедуры

Процедура НайденныеВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	ЭлементыФормы.Дерево.ТекущаяСтрока = ЭлементыФормы.Найденные.ТекущаяСтрока.СтрокаДанных; 
	ДеревоВыбор();
КонецПроцедуры

Процедура КоманднаяПанельДереваПоискПоТекстамФорм(Кнопка)
	ТекущаяСтрока = ЭлементыФормы.Дерево.ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ПолноеИмяМД = ирОбщий.ПервыеФрагментыЛкс(Дерево.Строки[0].Имя);
	...
	Форма.ИменаОбъектовМД = ирОбщий.ЗначенияВМассивЛкс(ПолноеИмяМД);
	Форма.Открыть();
	ирКлиент.УстановитьПрикреплениеФормыВУправляемомПриложенииЛкс(Форма);
КонецПроцедуры

Процедура КоманднаяПанельДереваОткрытьОбъектМетаданных(Кнопка)
	ОбъектМД = Метаданные.НайтиПоПолномуИмени(Дерево.Строки[0].Имя);
	Если ОбъектМД = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
ЭтаФорма.РежимПодсистемы = Ложь
	Или КлючУникальности = "ВсеИнструменты"
	Или КлючУникальности = "ПоСпискуОбъектов";
Если РежимПодсистемы Тогда
	ЭтаФорма.КлючСохраненияПоложенияОкна = КлючУникальности;
	ЭтаФорма.ИмяСохраненияПоложенияОкна = КлючУникальности;
...
КонецЕсли;
мИзмененныеЭлементы = Новый Массив;
мИзмененныеТекущиеСтраницы = Новый Массив;
