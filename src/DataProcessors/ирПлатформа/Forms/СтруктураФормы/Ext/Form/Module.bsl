...
Процедура ДобавитьРеквизитыУправляемойФормы(РеквизитыФормы, Знач Форма, Знач ПутьКРодителю = "") Экспорт
	...
	Для Каждого ЛиКоллекция Из ирОбщий.ЗначенияВМассивЛкс(Ложь, Истина) Цикл
		Для Каждого КлючИЗначение Из ИменаРеквизитовФормы(Форма, ПутьКРодителю, ЛиКоллекция,, ДокументДОМ).Все Цикл
			СтрокаРеквизита = РеквизитыФормы.Строки.Добавить();
			СтрокаРеквизита.Имя = КлючИЗначение.Ключ;  
...
Процедура ЗапроситьСерверныеМетаданные()
	...
	Если мВсеОбработчикиСобытий <> Неопределено И ФайлМодуля.Существует() Тогда
		...
		Для Каждого СтрокаМетода Из мВсеОбработчикиСобытий Цикл
			МетодМодуля = СтрокаМетодаМодуляПоИмени(МодульМетаданных, СтрокаМетода.Обработчик);
