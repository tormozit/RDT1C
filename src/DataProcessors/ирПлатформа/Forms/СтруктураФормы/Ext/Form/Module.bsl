...
Процедура ДобавитьФорму(Форма, КорневаяСтрокаФормы = Неопределено)
	...
	Иначе
		...
		Если КорневаяСтрокаФормы = Неопределено Тогда
			...
			КорневаяСтрокаФормы.Доступность = Истина;
			КорневаяСтрокаФормы.ЭлементФормы = Форма;
			ЗаголовокФормы = ДокументDOM.ВычислитьВыражениеXPath(ПутьЭлементаЗаголовка, ДокументDOM, РазыменовательПИ, ТипРезультатаDOMXPath.ЛюбойНеупорядоченныйУзел).ОдиночныйУзелЗначение.ТекстовоеСодержимое;
...
Процедура ДеревоПриАктивизацииСтроки(Элемент)
	...
	Если ТипЗнч(Форма) = Тип("Форма") Тогда
		...
		Для Каждого СтрокаСобытия Из мПлатформа.ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", СтруктураТипа.ИмяОбщегоТипа, "Событие")) Цикл
			ИмяСобытия = СтрокаСобытия.Слово;
			Попытка
				ОбработчикСобытия = мТекущаяСтрока.ЭлементФормы.ПолучитьДействие(ИмяСобытия);
			Исключение
				Продолжить;
			КонецПопытки;
			Если ОбработчикСобытия <> Неопределено Тогда
				СтрокаОбработчика = ОбработчикиСобытий.Добавить();
				СтрокаОбработчика.Событие = СтрокаСобытия.Слово;
	...
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	ОбработчикиСобытий.Сортировать("Событие");
...
Процедура НайтиСтрокиДерева(Знач ИскомаяСтрока = Неопределено)
	...
	КонецЕсли; 
	ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.Дерево, ИскомаяСтрока, "Текст, Подсказка",, ЭлементыФормы.Найденные);
