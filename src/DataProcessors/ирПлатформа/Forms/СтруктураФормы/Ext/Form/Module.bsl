...
Процедура ДобавитьРеквизитыУправляемойФормы(РеквизитыФормы, Знач Форма, Знач ПутьКРодителю = "") Экспорт
	Для Каждого ЛиКоллекция Из ирОбщий.ЗначенияВМассивЛкс(Ложь, Истина) Цикл
		Для Каждого КлючИЗначение Из ИменаРеквизитовФормы(Форма, ПутьКРодителю, ЛиКоллекция).Все Цикл
			СтрокаРеквизита = РеквизитыФормы.Строки.Добавить();
			СтрокаРеквизита.Имя = КлючИЗначение.Ключ;  
			СтрокаРеквизита.ЭлементКоллекции = ЛиКоллекция;
...
Процедура ОбработчикиСобытийВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.ОбработчикиСобытий.Колонки.Обработчик Тогда
...
Функция ФайлМодуля()
	...
	Возврат мПлатформа.ФайлМодуляИзКраткогоИмени(Дерево.Строки[0].Имя + ".Форма.Модуль");
...
Процедура КоманднаяПанельДереваОткрытьМодульИзКэша(Кнопка)
	ирКлиент.ОткрытьПолеТекстаМодуляКонфигурацииЛкс(Дерево.Строки[0].Имя + ".Форма.Модуль");
КонецПроцедуры
