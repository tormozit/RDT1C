...
Функция ЭлементФормыСтрокиДерева(Знач ТекущаяСтрока, РазрешитьФорму = Ложь)
	...
	Если Истина
		...
		И (Ложь
			...
			Или ТипЗнч(ЭлементФормы) = ирОбщий.ТипУправляемаяФормаЛкс())
	Тогда
...
Процедура ДобавитьФорму(Форма, КорневаяСтрокаФормы = Неопределено)
	...
	Если ТипЗнч(Форма) = Тип("Форма") Тогда
		Если КорневаяСтрокаФормы = Неопределено Тогда
			...
			ЗаголовокФормы = "";
			РезультатXPath = ДокументDOM.ВычислитьВыражениеXPath("/elem/elem[1]/elem[1]/elem[1]/elem[1]/elem/data[1]", ДокументDOM, РазыменовательПИ, ТипРезультатаDOMXPath.НеупорядоченныйИтераторУзлов);
			Пока Истина Цикл
				Узел = РезультатXPath.ПолучитьСледующий();
				Если Узел = Неопределено Тогда
					Прервать;
				КонецЕсли; 
				Если ирОбщий.СтрокиРавныЛкс(ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(Узел.ТекстовоеСодержимое), ТекущийЯзык().КодЯзыка) Тогда
					ЗаголовокФормы = ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(Узел.СледующийСоседний.ТекстовоеСодержимое);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		КонецЕсли; 
		ПутьЭлемента = "/elem/elem/elem[1]/elem[2]/elem[2]";
		...
		Если Узел <> Неопределено Тогда
			...
			//КонецЕсли; 
		КонецЕсли;
		Для Каждого ИмяРеквизита Из ИменаРеквизитовФормы(ДокументDOM).Все Цикл
			СтрокаРеквизита = РеквизитыФормы.Добавить();
			СтрокаРеквизита.Имя = ИмяРеквизита;  
			Попытка
				ЗначениеРеквизита = ТипЗнч(Форма[ИмяРеквизита]);
			Исключение
				ЗначениеРеквизита = Неопределено;
			КонецПопытки;
			СтрокаРеквизита.ТипЗначения = ЗначениеРеквизита;
		КонецЦикла;
		РеквизитыФормы.Сортировать("Имя");
	Иначе
		#Если Сервер И Не Сервер Тогда
			ирОбщий.УправляемаяФормаБСП_СерверныеМетаданныеЛкс();
		#КонецЕсли
		ПутьЭлементаЗаголовка = "/elem/elem[1]/elem[1]/elem[1]/elem[1]/data[2]";
		Если КорневаяСтрокаФормы = Неопределено Тогда
			КорневаяСтрокаФормы = Дерево.Строки.Добавить();
			КорневаяСтрокаФормы.Видимость = Истина;
			КорневаяСтрокаФормы.Доступность = Истина;
			...
			Если ЗаголовокФормы <> Неопределено Тогда
				...
				ЗаголовокФормы = ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(ЗаголовокФормы);
				Если ЗначениеЗаполнено(ЗаголовокФормы) Тогда
					КорневаяСтрокаФормы.Текст = ЗаголовокФормы;
				КонецЕсли;
			КонецЕсли;
		...
		КонецЕсли; 
		ПутьЭлемента = "//data[not(text()='0' or text()='4294967295' or contains(text(),'""'))]/following-sibling::data[1][contains(text(),'""') and not(text()='""""' or contains(text(),' ') or contains(text(),'$') or contains(text(),'/'))]";
		РезультатXPath = ДокументDOM.ВычислитьВыражениеXPath(ПутьЭлемента, ДокументDOM, РазыменовательПИ, ТипРезультатаDOMXPath.НеупорядоченныйИтераторУзлов);
		Пока Истина Цикл
		...
		ПутьЭлемента = "//data[contains(text(),'-') and not(starts-with(text(),'-'))]/following-sibling::data[1][not(text()='0' or text()='4294967295' or contains(text(),'""'))]";
		РезультатXPath = ДокументDOM.ВычислитьВыражениеXPath(ПутьЭлемента, ДокументDOM, РазыменовательПИ, ТипРезультатаDOMXPath.НеупорядоченныйИтераторУзлов);
		Пока Истина Цикл
			...
			Для Каждого СтрокаЭлемента Из ПодходящиеСтрокиЭлементов Цикл
				...
				Если Не ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
					ПутьЭлемента = "elem[2]/elem[1]/data[2][contains(text(),'""')]/..";
				...
				ПутьЭлемента = "elem[1]/elem[10]/elem[1]/data[2][contains(text(),'""')]/.."; // 10-й узел вероятно подойдет только для ПолеФормы
		...
		Для Каждого ЭлементФормы Из Форма.Элементы Цикл
			...
			Если ТипЗнч(ЭлементФормы) = Тип("ТаблицаФормы") Тогда
				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					ПутьЭлемента = "//data[text()='6d38aadb-3b55-46c6-9b24-abddca02bec9']/following-sibling::data[1][text()=" + ИндексПоля + "]/..//data[contains(text(),'""Колонка')]";
				...
				Если НастройкиФормыDOM <> Неопределено Тогда
					ПутьЭлемента = "//elem/data[1][text()='""" + ИмяЭлемента + """']/../elem/elem[2]";
			...
			Тогда
				...
				Если ОписаниеЭлемента <> Неопределено Тогда
					...
					ПутьЭлемента = "//data[text()='18a60f47-8dfd-4f2e-a693-026b98f58b2e']/following-sibling::data[1][text()=" + ИндексПоля + "]/following-sibling::elem[2]/elem[1]/elem";
		...
		//КонецЕсли; 
		Для Каждого ИмяРеквизита Из ИменаРеквизитовФормы(Форма).Все Цикл
			СтрокаРеквизита = РеквизитыФормы.Добавить();
			СтрокаРеквизита.Имя = ИмяРеквизита;  
			СтрокаРеквизита.ТипЗначения = ТипЗнч(Форма[ИмяРеквизита]);
		КонецЦикла;
		РеквизитыФормы.Сортировать("Имя");
	КонецЕсли;
...
Процедура ЗапроситьСерверныеМетаданные()
	...
	Если мВсеОбработчикиСобытий <> Неопределено И ФайлМодуля.Существует() Тогда
		...
		МодульМетаданных = мПлатформа.МодульМетаданных(ТекстФайла);
		мВсеОбработчикиСобытий.Колонки.Добавить("Позиция");
		Для Каждого СтрокаМетода Из мВсеОбработчикиСобытий Цикл
			МетодМодуля = МодульМетаданных.Методы.Найти(НРег(СтрокаМетода.Обработчик), "НИмя");
...
Функция СобратьМногострочнуюСтроку(Знач ТекущийУзел, выхСледующийУзел = Неопределено)
	...
	Если Лев(ТекстУзла, 1) = """" Тогда 
		...
		Результат = ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(Результат);
	КонецЕсли; 
	...
	Для каждого УзелЭлементаФормы Из Узел.ДочерниеУзлы Цикл
		...
		Иначе
			...
			Если ирКэш.НомерВерсииПлатформыЛкс() <= 803008 Тогда
				...
				ЭлементФормы = Форма.ЭлементыФормы[ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(СвойстваЭлФормыСИменем[2].ТекстовоеСодержимое)];
			Иначе
				...
				ЭлементФормы = Форма.ЭлементыФормы[ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(СвойстваЭлФормыСИменем[1].ТекстовоеСодержимое)];
			КонецЕсли; 
			Если Ложь
...
Процедура ЗагрузитьОбработчикиСобытий()
	...
	Если мВсеОбработчикиСобытий <> Неопределено Тогда
		...
		Иначе
			ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(мВсеОбработчикиСобытий.Скопировать(Новый Структура("Имя", мТекущаяСтрока.Имя)), ОбработчикиСобытий);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ОбработчикиСобытийВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка = ЭлементыФормы.ОбработчикиСобытий.Колонки.Имя Тогда
		ЭлементыФормы.Дерево.ТекущаяСтрока = Дерево.Строки.Найти(ВыбраннаяСтрока.Имя, "Имя", Истина);
...
Функция ФайлМодуля()
	...
	Файл = Новый Файл(мПлатформа.ПапкаКэшаМодулей.ПолноеИмя + "\" + Дерево.Строки[0].Имя + ".Форма.Модуль.txt");
	Возврат Файл;
...
Процедура ОбработчикиСобытийПриАктивизацииСтроки(Элемент)
	...
	КонецЕсли;
	ирКлиент.ТабличноеПолеПриАктивизацииСтрокиЛкс(ЭтаФорма, Элемент);
	ирКлиент.ПоказатьПозициюМодуляЛкс(ТекущаяСтрока, ФайлМодуля().ПолноеИмя);
КонецПроцедуры

Процедура КПОбработчикиСобытийКонтекстнаяСправка(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
	//СтруктураТипа = мПлатформа.СтруктураТипаИзЗначения(ЭлементыФормы.Дерево.ТекущаяСтрока.ЭлементФормы);
	СтруктураТипа = мПлатформа.СтруктураТипаИзКонкретногоТипа(ЭлементыФормы.Дерево.ТекущаяСтрока.Тип);
