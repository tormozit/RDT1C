...
Процедура КоманднаяПанельДереваАнализКонтекстныхВызовов(Кнопка)
	...
	ИмяФайла = мПлатформа.ПапкаКэшаМодулей.ПолноеИмя + "\" + Дерево.Строки[0].Имя + ".Форма.Модуль.txt";
	ФормаАнализа = ПолеТекстаПрограммы.ПолучитьФорму("КонтекстныеВызовы",, ИмяФайла);
