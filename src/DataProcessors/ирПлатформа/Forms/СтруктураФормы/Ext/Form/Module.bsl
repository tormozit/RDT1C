...
Функция СобратьМногострочнуюСтроку(Знач ТекущийУзел, выхСледующийУзел = Неопределено)
	...
	Если Лев(ТекстУзла, 1) = """" Тогда 
		...
		Результат = ирОбщий.СтрокаИзВыраженияВстроенногоЯзыкаЛкс(Результат);
	КонецЕсли; 
...
Процедура ЗаполнитьСтрокуЭлементаУправляемойФормы(СтрокаДерева, Роль = "", ЭффективнаяВидимость = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	ЗаполнитьСтрокуЭлементаОбычнойФормыФрагмент(СтрокаДерева, ЗаголовокЭлемента, Роль);
	Если мУсловноеОформлениеФормы <> Неопределено Тогда
		#Если Сервер И Не Сервер Тогда
...
Процедура ЗаполнитьСтрокуЭлементаОбычнойФормыФрагмент(Знач СтрокаДерева, Знач ЗаголовокЭлемента = "", Знач Роль = "")
...
Процедура ДеревоПриАктивизацииСтроки(Элемент)
	...
	Тогда
		ЭтаФорма.ОписаниеТекущейСтроки = ЭтаФорма.ОписаниеТекущейСтроки + Символы.ПС + ЭлементФормы.ТекстПодвала;
	КонецЕсли;
	ИндикацияТекущегоЭлемента();
	Если мУсловноеОформлениеФормы <> Неопределено Тогда
		Если ТипЗнч(ЭлементФормы) = Тип("УправляемаяФорма") Тогда
...
Процедура ВпередНажатие(Элемент)
	...
	ирОбщий.СледующееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.Дерево);
	
КонецПроцедуры

Процедура НазадНажатие(Элемент)
	
	ирОбщий.ПредыдущееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.Дерево);
...
Процедура КоманднаяПанельДереваИсследоватьЭлементФормы(Кнопка)
	...
	ТекущаяСтрока = ЭлементыФормы.Дерево.ТекущаяСтрока;
	ВосстановитьСтарыеСвойстваЭлементаФормы(ТекущаяСтрока);
	ЭлементФормы = ЭлементФормыСтрокиДерева(ТекущаяСтрока, Истина);
	ИмяТекущегоСвойства = "";
	Если ТипЗнч(ЭлементФормы) = Тип("КолонкаТабличногоПоля") Тогда
		ИмяТекущегоСвойства = "ЭлементУправления";
	КонецЕсли;
	ирОбщий.ИсследоватьЛкс(ЭлементФормы,,,,, ИмяТекущегоСвойства);
...
Процедура НайтиСтрокиДерева(Знач ИскомаяСтрока = Неопределено)
	...
	КонецЕсли; 
	ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.Дерево, ИскомаяСтрока, "Текст, Подсказка",, ЭлементыФормы.Найденные);
...
Процедура ОбновитьОтбор()
	ирОбщий.ТабличноеПолеУстановитьОтборПоПодстрокеЛкс(ЭтаФорма, ЭлементыФормы.СистемныеСочетанияКлавиш, ПодстрокаПоиска);
КонецПроцедуры
...
Процедура КоманднаяПанельДереваСравнитьЭлементы(Кнопка)
	Если ЭлементыФормы.Дерево.ВыделенныеСтроки.Количество() < 2 Тогда
		Возврат;
	КонецЕсли; 
	...
	КонецЦикла;
	ирОбщий.СравнитьОбъектыВФормеЛкс(Объекты,,, ЭлементыФормы.Дерево.ТекущаяСтрока.ЭлементФормы);
КонецПроцедуры
