...
~Конец:
	Если Истина
		И Открыта() 
		И ТекущийЭлемент = ЭлементыФормы.СтрокаПоиска
	Тогда
		Открыть(); // !!! Закрывает все контекстные меню и сбрасывает таймер события АвтоподборТекста
		Если ЭлементыФормы.СтрокаПоиска.Значение <> СтрокаПоиска Тогда
			СтрокаПоискаАвтоПодборТекста(ЭлементыФормы.СтрокаПоиска, ЭлементыФормы.СтрокаПоиска.Значение, "", Истина);
		КонецЕсли;
	КонецЕсли;
	Интервал = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоИндикации;
	Если Ложь
		Или ТекущаяСтрока.СтарыеСвойства <> Неопределено
		Или Интервал < 200 И АктивнаяПодсветка 
	Тогда
...
Процедура ДеревоПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ТипЗнч(ДанныеСтроки) = Тип("СтрокаТаблицыЗначений") Тогда
		ДанныеСтроки = ДанныеСтроки.СтрокаДерева;
	КонецЕсли; 
	...
	КонецЕсли;
	Если Элемент = ЭлементыФормы.Дерево Тогда
...
Процедура НайтиСтрокиДерева(Знач ИскомаяСтрока = Неопределено)
	...
	КонецЦикла; 
	ЭлементыФормы.Найденные.ОбновитьСтроки(); // первый раз поле в своей жизни табличное поле не отображает строки
