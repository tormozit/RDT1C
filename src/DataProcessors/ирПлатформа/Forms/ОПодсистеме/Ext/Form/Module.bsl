...
Функция ТехническаяИнформация()
	...
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		Текст = Текст + "
		|Инструменты разработчика. Серверный модуль: " + БулевоВСтроку(ирПортативный.ЛиСерверныйМодульДоступенЛкс());
	Иначе
		Текст = Текст + "
	...
	Если ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		...
		КонецЕсли;
		Текст = Текст + 
		"Клиент. ОС выполнение BAT: " + БулевоВСтроку(ирОбщий.ЛиДоступноВыполнениеКомандныхФайловЛкс()) + "
