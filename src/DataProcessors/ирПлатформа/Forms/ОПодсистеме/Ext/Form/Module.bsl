...
Функция ТехническаяИнформация()
	...
	Текст = Текст + "
	|Конфигурация. Использовать обычные формы в управляемом приложении: " + Метаданные.ИспользоватьОбычныеФормыВУправляемомПриложении;
	#КонецЕсли
	Текст = Текст + "
	|Инструменты разработчика. Версия: " + ИспользуемаяВерсия + "
	|Инструменты разработчика. Инструмент: " + НазваниеИнструмента + "
	|Инструменты разработчика. Перехват клавиатурного ввода: " + ирКлиент.ЛиПерехватКлавиатурногоВводаЛкс() + "
	...
	|Клиент. Отладка разрешена: " + ирКэш.ЛиОтладкаРазрешенаЛкс() + "
	...
	Если ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		...
		Если ВК <> Неопределено Тогда
			Текст = Текст + "Клиент. ОС от имени администратора: " + ВК.IsAdmin() + "
			|";
		КонецЕсли;
		Текст = Текст + 
		"Клиент. ОС " + ирОбщий.ВыполнитьКомандуОСЛкс("Chcp") + "
		|Клиент. ОС выполнение BAT: " + ирОбщий.ЛиДоступноВыполнениеКомандныхФайловЛкс() + "
		|";
	КонецЕсли;
	Если Не ирКэш.ЛиФайловаяБазаЛкс() Тогда
		...
		Найденные = ирОбщий.НайтиРегулярноеВыражениеЛкс(РезультатКоманды, "=\s*(\d*)");
		Пинг = Найденные[5].Подгруппа0;
		Текст = Текст + 
		"Клиент. ОС пинг сервера: " + Пинг + "мс
		|";
	КонецЕсли;
	Если Не ирКэш.ЛиФайловаяБазаЛкс() Тогда
