...
Функция УстановитьСвязь(Знач ТабличноеПоле = Неопределено, Знач ЛиТолькоПроверить = Ложь, Знач АктивизироватьСтраницу = "", Знач МетаданныеВыбора = Неопределено) Экспорт
	...
	Если Не ЛиТолькоПроверить Тогда  
		...
		ДопСвойства = ирОбщий.ДопСвойстваЭлементаФормыЛкс(ВладелецФормы, ТабличноеПоле);
...
Процедура ОбработкаВыполнить(Кнопка)
	...
	ирОбщий.УстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗЛкс(ВладелецФормы, СвязанноеТабличноеПоле, лЗначениеОбработки, ТипИсточника, Колонка, ТолькоВыделенныеСтроки, ИнтерактивноеУстановка, НаСервере,, МетаданныеВыбора);
