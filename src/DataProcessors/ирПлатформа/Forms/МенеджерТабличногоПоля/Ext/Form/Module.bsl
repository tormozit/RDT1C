...
Процедура НастроитьПостроительОтчета()
	...
	ИначеЕсли ТипЗнч(СвязанноеТабличноеПоле) = Тип("ТаблицаФормы") Тогда 
		СписокСоВсемиКолонками = ирОбщий.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(СвязанноеТабличноеПоле, Новый Массив);
	КонецЕсли; 
...
//
Функция УстановитьСвязь(Знач ТабличноеПоле = Неопределено, Знач пЛиТолькоПроверить = Ложь, Знач АктивизироватьСтраницу = "", Знач МетаданныеВыбора = Неопределено) Экспорт
	...
	Если Не пЛиТолькоПроверить Тогда  
		...
		ЭтаФорма.ПолеВыбораКолонки = ТекущаяКолонкаДанных;
		...
		Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ирОбщий.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ОсновнаяПанель.Страницы.Обработка, Ложь);
		...
		Если ТипИсточника = "ТаблицаЗначений" Тогда
			КомпоновкаТабличногоПоля = ирОбщий.КомпоновкаТабличногоПоляЛкс(ВладелецФормы, СвязанноеТабличноеПоле);
...
Процедура ОтборЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ОписаниеТаблицыБД <> Неопределено Тогда
		...
		Если ПолеТаблицыБД <> Неопределено Тогда
			МетаданныеРеквизита = ирОбщий.СтруктураСвязейИПараметровВыбораЛкс();
			МетаданныеРеквизита.ПараметрыВыбора = ПолеТаблицыБД.Метаданные.ПараметрыВыбора;
			СтруктураОтбора = ирОбщий.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(МетаданныеРеквизита);
		КонецЕсли;
	КонецЕсли;
	Если Не ЭтаФорма.ЭлементыФормы.КоманднаяПанельОтбор.Кнопки.Режимы.Кнопки.НеЗакрыватьФормыПриВыборе.Пометка Тогда
		ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.Отбор, СтандартнаяОбработка, ЭлементыФормы.Отбор.ТекущаяСтрока.Значение, Истина, СтруктураОтбора);
		Возврат;
	КонецЕсли;
	...
	Если МенеджерТипаЗначения = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	ФормаВыбораЗначения = ирОбщий.ПолучитьФормуСпискаЛкс(Тип(ЭлементыФормы.Отбор.ТекущаяСтрока.Значение), СтруктураОтбора, Истина, ЭтаФорма, Истина,, ЭлементыФормы.Отбор.ТекущиеДанные.Значение,,
	...
	Иначе
		ирОбщий.Форма_АктивироватьОткрытьЛкс(ФормаВыбораЗначения);
	КонецЕсли;
		...
		ИначеЕсли Колонка.Имя = "ПолучитьИзТекущейСтроки" Тогда
			Если СвязанноеТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда
				...
				Если ТипЗнч(ВыбраннаяСтрока.Значение) = Тип("СписокЗначений") Тогда
					...
					КонецЕсли;
				Иначе
					ирОбщий.ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(Элемент, ЭлементыФормы.Отбор.Колонки.ЗначениеНезависимое, ЗначениеЯчейки);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаВыполнить(Кнопка)
	...
	ирОбщий.УстановитьЗначениеВКолонкеТабличногоПоляТЧИлиТЗЛкс(СвязанноеТабличноеПоле, лЗначениеОбработки, ТипИсточника, Колонка, ТолькоВыделенныеСтроки, ИнтерактивноеУстановка, НаСервере,, МетаданныеВыбора);
...
Процедура ОтборПриИзмененииФлажка(Элемент, Колонка)
	...
	Если Колонка = ЭлементыФормы.Отбор.Колонки.ЗначениеНезависимое Тогда 
		ирОбщий.ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(Элемент, Колонка, Не ТекущаяСтрока.Значение, ЭтаФорма);
	КонецЕсли;
...
Процедура КоманднаяПанельФормыПереключитьсяВФорму(Кнопка)
	...
	ирОбщий.Форма_АктивироватьОткрытьЛкс(РодительскаяФорма);
...
Процедура ОтборЗначениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаСИсториейВыбора_НачалоВыбораИзСпискаЛкс(Элемент, ирОбщий.КлючИсторииВыбораЗначенияОтбораЛкс(ПолноеИмяМД, ЭлементыФормы.Отбор.ТекущаяСтрока));
...
Процедура КлсКомандаСвязанногоПоляНажатие(Кнопка)
	Если Не УстановитьСвязь() Тогда 
		Возврат;
	КонецЕсли;
	ирОбщий.УниверсальнаяКомандаФормыЛкс(ЭтаФорма, Кнопка, СвязанноеТабличноеПоле);
КонецПроцедуры
...
Процедура ОтборПриАктивизацииСтроки(Элемент)
	...
	ЭлементыФормы.Отбор.Колонки.ЗначениеНезависимое.ЭлементУправления.ОграничениеТипа = Элемент.ТекущиеДанные.ТипЗначения;
	
КонецПроцедуры

Процедура КоманднаяПанельОтборОтборБезЗначенияВТекущейКолонке(Кнопка)
	
	ирОбщий.ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(СвязанноеТабличноеПоле);
...
Процедура ДоступныеПоляФормулыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ИндексКартинки = ирОбщий.ИндексКартинкиТипаЗначенияБДЛкс(ДанныеСтроки.ТипЗначения);
...
Процедура ОтборЗначениеАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
...
Процедура ОтборЗначениеПриИзменении(Элемент)
	...
	ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(ЭлементыФормы.Отбор.ТекущаяСтрока, ПолноеИмяМД);
...
Процедура ОтборЗначениеНезависимоеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	УстановитьЗначениеОтбора(ВыбранноеЗначение);
	
КонецПроцедуры

Процедура ОтборЗначениеНезависимоеОчистка(Элемент, СтандартнаяОбработка)
...
Процедура ОсновнаяПанельПриСменеСтраницы(Элемент, ТекущаяСтраница)
	Если ЭлементыФормы.ОсновнаяПанель.ТекущаяСтраница = ЭлементыФормы.ОсновнаяПанель.Страницы.Обработка Тогда
		Если ирОбщий.ПолучитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ОсновнаяПанель.ТекущаяСтраница) Тогда
			Возврат;
		КонецЕсли; 
		ЭлементыФормы.Выполнить.КнопкаПоУмолчанию = Истина;
		СоответствиеКолонокДанным = Новый Структура;
		СписокВыбораКолонки = ЭлементыФормы.ПолеВыбораКолонки.СписокВыбора;
		ИнтерактивныеКолонки = ирОбщий.ИнтерактивныеКолонкиТабличногоПоляЛкс(СвязанноеТабличноеПоле);
		Для Каждого ЭлементСписка Из ИнтерактивныеКолонки Цикл
			...
			Если ДанныеКолонки <> "" Тогда
				...
				КонецЕсли;
				Если Не ирОбщий.ЛиВКолонкеДоступнаЭмуляцияИнтерактивногоИзмененияЛкс(Колонка) Тогда
					Продолжить;
				КонецЕсли; 
		...
		ирОбщий.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ОсновнаяПанель.ТекущаяСтраница, Истина);
...
Процедура КПОтборПросмотраПрименить(Кнопка = Неопределено)
	...
	ирОбщий.СкопироватьОтборЛюбойЛкс(ирОбщий.КомпоновкаТабличногоПоляЛкс(ВладелецФормы, СвязанноеТабличноеПоле).Компоновщик.Настройки.Отбор, Компоновщик.Настройки.Отбор);
	ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ВладелецФормы, СвязанноеТабличноеПоле,,, Истина);
	
КонецПроцедуры

Процедура КПОтборПросмотраИсполняемаяКомпоновка(Кнопка)
	
	ирОбщий.ОткрытьДанныеТабличногоПоляВКонсолиКомпоновкиЛкс(ВладелецФормы, СвязанноеТабличноеПоле, Компоновщик.Настройки);
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПлатформа.Форма.МенеджерТабличногоПоля");
#Если Сервер И Не Сервер Тогда
