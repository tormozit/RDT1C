...
Процедура НастроитьПостроительОтчета()
	...
	Если ПолноеИмяТаблицы <> Неопределено Тогда
		...
		Если ЕстьКолонки <> Неопределено Тогда
			...
			Если МассивФрагментов.Количество() = 2 Тогда
				...
				ПоляТаблицыБД = ирОбщий.ПоляТаблицыМДЛкс(ПолноеИмяТаблицы, Истина,,, Ложь);
	...
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоПолямНастройкиЛкс(ПостроительОтчета.ДоступныеПоля);
	...
	Если ЗначениеЗаполнено(ПолеВыбораКолонки) Тогда
		НоваяСтрока = ДоступныеПоляФормулы.Найти(ПолеВыбораКолонки, "Имя");
		Если НоваяСтрока <> Неопределено Тогда
			ЭлементыФормы.ДоступныеПоляФормулы.ТекущаяСтрока = НоваяСтрока;
		КонецЕсли;
	КонецЕсли; 
...
Функция УстановитьСвязь(Знач ТабличноеПоле = Неопределено, Знач ЛиТолькоПроверить = Ложь, Знач АктивизироватьСтраницу = "", Знач МетаданныеВыбора = Неопределено) Экспорт
	...
	КонецЕсли;
	Если Истина
		И СтруктураТипаМетаданных <> Неопределено 
		И СтруктураТипаМетаданных.Метаданные <> Неопределено
	Тогда
	...
	Если Не ЛиТолькоПроверить Тогда  
		...
		ЭтаФорма.ИнтерактивноеУстановка = ирОбщий.СвойствоСтруктурыЛкс(ДопСвойства, "НеинтерактивноеИзменение") <> Истина;
		...
		Если ТипИсточника = "ТаблицаЗначений" Тогда
			...
			Если КомпоновкаТабличногоПоля <> Неопределено Тогда
				СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицеЗначенийЛкс(СвязанноеТабличноеПоле.Значение);
				Компоновщик.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновки));
		...
		ирОбщий.ОбновитьТекстПослеМаркераЛкс(ЭтаФорма.Заголовок, , СвязанноеТабличноеПоле.Имя, ": ");
...
Процедура ЗначениеОбработкиНачалоВыбора(Элемент, СтандартнаяОбработка)
	ирКлиент.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка, ЗначениеОбработки);
КонецПроцедуры
