	...
	Для Каждого Item Из Результат Цикл
		...
		Если ТекстПрочитан Тогда
			...
			Иначе
				...
				КонецЕсли;
				РегВыражение.Global = Ложь;
				РегВыражение.Pattern = "Тип:" + шТегНеКлюч + "?((?:" + шТип + ")+)(?:<[^>""]+>\.\s+<br>Результат выполнения обещания\s+[–-]\s+(?:значение типа )?((?:" + шТипСтрогий + ")+<[^>""]+>))?";
				СмещениеИндекса = 0;
				...
				Если Результат.Количество() > 0 Тогда
					...
					Если Результат[0].SubMatches(3) <> Неопределено Тогда
						ТипЗначенияВложенный = СинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(Результат[0].SubMatches(3));
						ТипЗначенияВложенный = ИмяТипаИзПредставления(ТипЗначенияВложенный);
