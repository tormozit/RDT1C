	...
	Для Каждого Item Из Результат Цикл
		...
		Если ТипКонтекста = "" Тогда
			...
			Если Найти(Описание, "<p>Определяет набор ") > 0 Тогда 
				Если Истина 
					И Найти(Описание, ">Значения<") > 0 // Отбрасываем ПараметрыЗаписиJSON https://www.hostedredmine.com/issues/946885
					...
					И Не Найти(Описание, "ПеречислимыеСвойстваОбъектовМетаданных") > 0
				Тогда
...
Процедура ТаблицаПараметровВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбраннаяСтрока.ПутьКОписанию) Тогда
		ирОбщий.ОткрытьСтраницуСинтаксПомощникаЛкс(ВыбраннаяСтрока.ПутьКОписанию, СтрокаДляПодсветкиПараметра);
	Иначе
		...
		ирОбщий.ОткрытьСтраницуСинтаксПомощникаЛкс(СтрокаТаблицыКонтекстов.ПутьКОписанию, СтрокаДляПодсветкиПараметра);
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	ЗагрузитьТаблицыИзСтруктурыФайлов(СтруктураФайлов);
	ЭтаФорма.Каталог = ирОбщий.ВосстановитьЗначениеЛкс("ирОбновлениеОписанияПлатформы_Каталог");
	...
	ЭлементыФормы.Каталог.Видимость = ПолнаяВидимость;
...
Процедура КоманднаяПанель1Сравнить(Кнопка)
	...
	Если ТипЗнч(ТекущийЭлемент) <> Тип("ТабличноеПоле") Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ТекущийЭлемент);
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПлатформа.Форма.ОписаниеПлатформы");
