...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	ЗаполнитьСтрокуТипа(НоваяСтрока);
	СтруктураКлюча = Новый Структура("ТипКонтекста, ЯзыкПрограммы, Слово", "КоллекцияЗначенийПараметровКомпоновкиДанных", 0);
	ИменаМетодов = Новый Структура("Найти, Получить, Вставить, Добавить");
	Для Каждого КлючИЗначение Из ИменаМетодов Цикл
		СтруктураКлюча.Слово = КлючИЗначение.Ключ;
		НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
		НайденныеСтроки[0].ТипЗначения = "ЗначениеПараметраКомпоновкиДанных, ЗначениеПараметраНастроекКомпоновкиДанных"; // - против разделителя ИЛИ
	КонецЦикла;
