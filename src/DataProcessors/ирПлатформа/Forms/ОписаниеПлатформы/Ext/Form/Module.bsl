...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Для Каждого Item Из Результат Цикл
		...
		Тогда 
			...
			КонецЕсли;
		КонецЕсли;
		СинтаксПомощник.ЗагрузитьОписаниеМетода(ТипКонтекста, НовоеСлово, ОписаниеХтмл, ТипСлова, ЯзыкПрограммы, ТаблицаПараметров, РабочийКаталог);
...
Процедура ПриОткрытии()
	...
	Если ЗначениеЗаполнено(ПоискСсылокНаТип) Тогда
		ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(ЭтаФорма.Заголовок,, "ссылки на " + ПоискСсылокНаТип, ": ");
		ТаблицаОбщихТипов.Очистить();
