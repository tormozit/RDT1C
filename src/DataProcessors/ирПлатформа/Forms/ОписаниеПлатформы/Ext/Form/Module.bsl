...
Функция ИмяТипаИзПредставления(Знач Представление, Знач ЛиИмяОдногоТипа = Ложь)
	...
	ИначеЕсли Представление = "" Или Найти(Представление, ".") > 0 Тогда
...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	РегВыражениеВложенное = ирОбщий.НовоеРегВыражениеЛкс();
	РегВыражениеВложенное.Global = Истина;
	РегВыражениеВложенное.Multiline = Ложь;
	РегВыражениеВложенное.IgnoreCase = Истина;
	РегВыражениеВложенное.Pattern = ПараметрФункции;
	Для Каждого КлючИЗначение Из ИменаФайлов Цикл
		...
		Для каждого Вхождение Из Вхождения Цикл
			...
			ВложенныеВхождения = РегВыражениеВложенное.НайтиВхождения(Вхождение.Value);
			НомерПараметра = 1;
	...
	Для Каждого Item Из Результат Цикл
		...
		Если ТекстПрочитан Тогда
			...
			Иначе
				...
				КонецЕсли;
				РегВыражение.Global = Ложь;
				РегВыражение.Pattern = "Тип:" + шТегНеКлюч + "?((?:" + шТип + ")+)(?:<[^>""]+>\.\s+<br>(Результат выполнения обещания\s+[–-]\s+)?(Массив)?[^.<]*((?:" + шТипСтрогий + ")+<[^>""]+>))?";
				СмещениеИндекса = 0;
				...
				Если Результат.Количество() > 0 Тогда
					...
					Если Результат[0].SubMatches(5) <> Неопределено Тогда
						ТипЗначенияВложенный = ИмяТипаИзПредставления(СинтаксПомощник.ИзвлечьИзФрагментаHTMLОбычныйТекст(Результат[0].SubMatches(5)));
					...
					//	И Результат[0].SubMatches(4) <> Неопределено
					//Тогда
	...
	СтруктураКлюча = Новый Структура("ТипКонтекста, Слово, ТипСлова, ЯзыкПрограммы", "ПланОбмена.<Имя плана обмена>", "ВерсияДанных", "Поле", 1);
	НайденныеСтроки = ТаблицаКонтекстов.НайтиСтроки(СтруктураКлюча);
	Если НайденныеСтроки.Количество() = 0 Тогда
		// 8.0-8.3.26
		НоваяСтрока = ТаблицаКонтекстов.Добавить();
...
Процедура ЗаполнитьДоступность(Знач НоваяСтрока, Знач ДоступностьСервер, Знач ДоступностьТолстыйКлиент, Знач ДоступностьТонкийКлиент) Экспорт
