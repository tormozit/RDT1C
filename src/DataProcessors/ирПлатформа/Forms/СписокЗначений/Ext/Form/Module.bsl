...
Процедура ПриОткрытии()
	...
	ирОбщий.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.Таблица).МенеджерПоиска = ирОбщий.СоздатьМенеджерПоискаВТабличномПолеЛкс(); // Для отключения раскраски
	ЭтаФорма.Модифицированность = Ложь; 
...
Функция КоманднаяПанель1СохранитьВФайл(Кнопка = Неопределено)
	...
	Результат = ирОбщий.СохранитьЗначениеВФайлИнтерактивноЛкс(ПолучитьРезультат(), РасширениеФайла, "Список значений");
	Возврат Результат;
...
Процедура КоманднаяПанель1ВМассив(Кнопка)
	...
	ирОбщий.ОткрытьЗначениеЛкс(ПолучитьРезультат().ВыгрузитьЗначения(),,, ирОбщий.ЗаголовокДляКопииОбъектаЛкс(ЭтаФорма), Ложь);
...
Процедура КоманднаяПанель1УстановитьФлажки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭтаФорма, ЭлементыФормы.Таблица,, Истина);

КонецПроцедуры

Процедура КоманднаяПанель1СнятьФлажки(Кнопка)
	
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭтаФорма, ЭлементыФормы.Таблица,, Ложь);
...
Процедура ОтобратьСтроки()
	...
	ирОбщий.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ЭлементыФормы.Таблица,, ТаблицаСУзкимиТипами);
	ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.Таблица,,, Истина);
КонецПроцедуры
...
Процедура ТаблицаПредставлениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.Таблица, СтандартнаяОбработка);
