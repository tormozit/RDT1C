...
Процедура ЗначениеНачалоВыбора(Элемент = Неопределено, СтандартнаяОбработка = Неопределено)
	
	Если ирКлиент.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ЭлементыФормы.Значение, СтандартнаяОбработка, Значение) Тогда 
		ЗначениеПриИзменении();
	КонецЕсли; 
