...
Процедура ПриОткрытии()
	...
	ирОбщий.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.ДеревоИсточников).МенеджерПоиска = ирОбщий.СоздатьМенеджерПоискаВТабличномПолеЛкс(); // Для отключения раскраски
...
Процедура ЗаполнитьДеревоИсточников(Знач Фильтр = Неопределено, МассивДоступныхОбъектов = Неопределено, Принудительно = Ложь) Экспорт
	...
	Иначе
		ИскатьПоПолномуИмени = Найти(Фильтр, ".") > 0 И Фильтр <> "." И Найти(Фильтр, " ") = 0 И Найти(Фильтр, """") = 0;
		СловаПоиска = ирОбщий.РазделитьСтрокуПоискаНаСловаПоискаЛкс(НРег(Фильтр));
	КонецЕсли;
						...
						Если Не УдалитьМетаТЧ Тогда
							...
							УдалитьМетаТЧ = Ложь
								Или (Истина
									...
									И Не УзелПодходитФильтру(СтрокаМетаТЧ, Фильтр, СловаПоиска, ИскатьПоПолномуИмени)) 
								Или (Истина
...
Функция УзелПодходитФильтру(Знач СтрокаОбъектаМД, Знач Фильтр, Знач СловаПоиска, Знач ИскатьПоПолномуИмени)
	
	Если ИскатьПоПолномуИмени Тогда
		УзелПодходитФильтру = ирОбщий.ЛиСтрокаСодержитВсеПодстрокиЛкс(СтрокаОбъектаМД.ПолноеИмяОбъекта, Фильтр);
	Иначе
