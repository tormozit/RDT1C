...
Процедура ПриОткрытии()
	...
	Если Не ЗначениеЗаполнено(ВариантПросмотра) Тогда
		...
		ИначеЕсли ирОбщий.ЛиТекстMarkdownЛкс(НачальноеЗначениеВыбора) Тогда
...
Процедура ОсновныеДействияФормыИзJSONВСтруктуру(Кнопка)
	...
	Попытка
		Объект = ирОбщий.ОбъектИзСтрокиJSONЛкс(СтрокаJSON);
	Исключение
		Попытка
			Объект = ирОбщий.ОбъектИзСтрокиJSONЛкс(СтрокаJSON, Истина);
		Исключение
