...
Функция ТаблицаИзДерева(выхСтрокиДерева = Неопределено)
	...
	КонецЕсли;
	выхСтрокиДерева = ирОбщий.ОтобратьКоллекциюСтруктуройЛкс(выхСтрокиДерева, ОтборСтрок);
...
Процедура ОбновитьПоиск(ИспользоватьВнешнийТекст = Ложь, СохранитьПозицию = Ложь)
	...
	ВхожденияГиперссылок = ирОбщий.НайтиРегВыражениеЛкс(Текст, "(\b(?:[" + шБукваЦифра + "]+)://[" + шБукваЦифра + "-+&@#\\/%?=~_|!:,.;()]+)",,, Истина,, Ложь);
