...
Процедура ЗакрытьССохранением()
	...
	Тогда
		...
		Закрыть();
		мПлатформа.АктивизироватьОкноКонфигуратора(мКлючИсточника, НовыйТекст, мСтарыйТекст);
		Возврат;
	КонецЕсли;
...
Процедура ОсновныеДействияФормыОбсудитьИИ(Кнопка)
	ПолеТекста = ПолеТекста();
	ВыделенныйТекст = ПолеТекста.ВыделенныйТекст();
	Если ПустаяСтрока(ВыделенныйТекст) Тогда
		ФрагментКода = ПолучитьТекст();
	Иначе
		ФрагментКода = ВыделенныйТекст;
	КонецЕсли;
	НовыйТекст = ирКлиент.ОткрытьФормуЧатаИИЛкс(ФрагментКода,,, Истина);
	Если НовыйТекст <> Неопределено Тогда
		Если ПустаяСтрока(ВыделенныйТекст) Тогда
			УстановитьТекст(НовыйТекст);
		Иначе
			ВыделениеДвумерное = ПолеТекста().ВыделениеДвумерное();
			Если ПолеТекста.ВыделенныйТекст() = "" Тогда
				ПолеТекста.ВыделитьВесьТекст();
			КонецЕсли;
