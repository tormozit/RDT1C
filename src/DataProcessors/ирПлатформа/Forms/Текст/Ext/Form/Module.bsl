...
Процедура ПриОткрытии()
	...
	мПолеТекстаПоиска = ирОбщий.ОболочкаПоляТекстаЛкс(ЭлементыФормы.РезультатыПоиска);
...
Процедура ОсновныеДействияФормыРедактироватьКопию(Кнопка)
	...
	Текст = ПолучитьТекст();
	ирОбщий.ПредложитьЗакрытьМодальнуюФормуЛкс(ЭтаФорма); // Там текст очистится
	ирОбщий.ОткрытьЗначениеЛкс(Текст,,, ирОбщий.ЗаголовокДляКопииОбъектаЛкс(ЭтаФорма), Ложь);
...
Процедура КМ_РезультатыПоискаОткрытьГиперссылку(Кнопка) 
	...
	ирОбщий.ОткрытьГиперссылкуИзПоляHTMLЛкс(ВыделениеПоля.focusNode);
КонецПроцедуры
