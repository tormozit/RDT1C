...
Процедура ОбновитьСоставСвойств(ПриОткрытии = Ложь)
	...
	Для Каждого Колонка Из Колонки Цикл
		...
		Если ПоляТаблицыБД <> Неопределено Тогда
			СтрокаСвойства.СвязиПараметровВыбора = ирОбщий.ПредставлениеСвязейПараметровВыбораЛкс(ПоляТаблицыБД.Найти(ИмяКолонкиДанных, "Имя").Метаданные);
		КонецЕсли;
	КонецЦикла;
...
Процедура СвойстваСтрокиВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.СвойстваСтроки.Колонки.ПредставлениеЗначения Тогда 
		ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка,,,,,, СтрокаПоиска);
	ИначеЕсли Ложь
