...
Процедура ОбновитьСоставСвойств(ПриОткрытии = Ложь)
	...
	Для Каждого Колонка Из Колонки Цикл
		...
		Если ПоляТаблицыБД <> Неопределено Тогда
			МетаРеквизит = ПоляТаблицыБД.Найти(ИмяКолонкиДанных, "Имя").Метаданные;
			СтрокаСвойства.СвязиПараметровВыбора = ирОбщий.ПредставлениеСвязейПараметровВыбораЛкс(МетаРеквизит.СвязиПараметровВыбора);
		КонецЕсли;
	КонецЦикла;
...
Процедура СвойстваСтрокиПредставлениеЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если СвязиИПараметрыВыбора Тогда
		Если ЗначениеЗаполнено(ИмяТаблицыБД) Тогда
			...
			СтруктураОтбора = ирОбщий.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(ТабличноеПоле.ТекущаяСтрока, МетаРеквизит);
		КонецЕсли; 
	КонецЕсли; 
