Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Форма.Каталог, Форма.ИсполняемыйФайлПлатформы, Форма.ИмяПользователя, Форма.ПарольПользователя, Форма.СтрокаСоединенияБазыПодсистемы, Форма.СобратьРасширение, Форма.СобратьПортативный, Форма.Токен, Форма.Автор, Форма.Репозиторий, Форма.БезРусскогоЯзыка, Форма.ФайлСпискаЗадачCSV";
КонецФункции
...
Функция СгенерироватьМодульИнициализацииФормПодсистемыДляПортативногоРежима() Экспорт
	...
	Для Каждого СтрокаТипаМетаданных Из ТипыМетаданных Цикл
		...
		Для Каждого МетаОбъект Из КоллекцияМетаОбъектов Цикл // ОбъектМетаданныхСправочник
...
Процедура ОсновныеДействияФормыВыполнить(Кнопка)
	...
	Если ЭлементыФормы.Панель1.ТекущаяСтраница = ЭлементыФормы.Панель1.Страницы.Сборка Тогда
		...
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(ФайлСпискаЗадачCSV) Тогда
			//Ответ = ирОбщий.ЗапроситьХТТПЛкс("https://www.hostedredmine.com/projects/devtool1c/issues.csv?query_id=10523"); // Нужна авторизация и не везде доступен сайт
			//ТекстСРазделителем = Ответ.ПолучитьТелоКакСтроку();
			ТекстСРазделителем = ирОбщий.ПрочитатьТекстИзФайлаЛкс(ФайлСпискаЗадачCSV); 
			//! ТекстСРазделителем = """#"";""Описание"";""Веха"";""Категория"";""Трекер"";""Обновлено"";""Приоритет"";""Тема""";
			СыраяТаблица = ирОбщий.ТаблицаИзТекстаСРазделителемЛкс(ТекстСРазделителем, ";",, Истина);
			СыраяТаблица.Колонки[0].Имя = "Номер";
			СыраяТаблица.Колонки.Веха.Имя = "ВерсияПодсистемы";
			СыраяТаблица.Колонки.Тема.Имя = "Заголовок";
			СыраяТаблица.Колонки.Трекер.Имя = "Тип"; 
			СыраяТаблица.Колонки.Добавить("ВерсияЧисло", Новый ОписаниеТипов("Число"));
			Для Каждого СыраяЭлемент Из СыраяТаблица Цикл
				СыраяЭлемент.Обновлено = ирОбщий.СтрокаВДатуЛкс(СыраяЭлемент.Обновлено);
				ВерсияПодсистемы = СыраяЭлемент.ВерсияПодсистемы;
				Если СтрЧислоВхождений(ВерсияПодсистемы, ".") = 1 Тогда
					ВерсияПодсистемы = ВерсияПодсистемы + ".1";
				КонецЕсли;
				Попытка
					ВерсияЧисло = Число(СтрЗаменить(ВерсияПодсистемы, ".", ""));
				Исключение
					Продолжить;
				КонецПопытки;
				СыраяЭлемент.ВерсияЧисло = ВерсияЧисло;
			КонецЦикла;
			ФайлСпискаЗадачГотовый = Каталог + "\IR-TaskList.zip";
			ирОбщий.СохранитьЗначениеВФайлЛкс(СыраяТаблица, ФайлСпискаЗадачГотовый, Истина,, "Внутр");
			ирОбщий.СообщитьЛкс(ФайлСпискаЗадачГотовый); 
		КонецЕсли;
		Публиковать();  
	КонецЕсли;
...
Функция СоздатьРасширение(Знач СуффиксВерсии, Знач КаталогВыгрузкиКонфигурации)
	...
	//Предупреждение(1);
	мРегВыражение = ирКэш.РегВыражениеЛкс();
	мРегВыражение.Global = Истина;
	...
	КонечныйФайл = Каталог + "\Расширение\IR-" + ВерсияРасширения + ".cfe";
...
Процедура ФайлСпискаЗадачCSVНачалоВыбора(Элемент, СтандартнаяОбработка)
	РезультатВыбора = ирКлиент.ВыбратьФайлЛкс(, "csv", "", Элемент.Значение, Каталог, , Заголовок);
	Если РезультатВыбора <> Неопределено Тогда
		ирКлиент.ИнтерактивноЗаписатьВПолеВводаЛкс(Элемент, РезультатВыбора);
	КонецЕсли; 
КонецПроцедуры

Процедура ФайлСпискаЗадачCSVНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	ирОбщий.ПолеВводаСИсториейВыбора_ОбновитьСписокЛкс(Элемент, ЭтаФорма);
КонецПроцедуры

Процедура ФайлСпискаЗадачCSVПриИзменении(Элемент)
