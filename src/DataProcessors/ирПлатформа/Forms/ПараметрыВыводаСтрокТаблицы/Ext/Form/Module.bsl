...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	...
	Результат = Новый Структура("ИмяТабличногоПоля");
	Если ТабличноеПоле <> Неопределено Тогда
		выхНаименование = "" + ТабличноеПоле.Имя;
		Результат.ИмяТабличногоПоля = ТабличноеПоле.Имя;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ДобавитьКолонки(Знач КолонкиТП = Неопределено, Знач ТекущаяКолонка = Неопределено, Знач ОчиститьКолонки = Истина) Экспорт
	...
	КонецЕсли;
	Если ОчиститьКолонки Тогда
		КолонкиТабличногоПоля.Очистить();
	КонецЕсли;
	Если КолонкиТП = Неопределено Тогда
		КолонкиТП = ирКлиент.КолонкиТаблицыФормыИлиТабличногоПоляЛкс(ТабличноеПоле);
	КонецЕсли;
	Для Каждого КолонкаТП Из КолонкиТП Цикл
		...
		Если ТипЗнч(КолонкаТП) = Тип("ГруппаФормы") Тогда
			ДобавитьКолонки(КолонкаТП.ПодчиненныеЭлементы, ТекущаяКолонка, Ложь);
			Продолжить;
		КонецЕсли; 
...
Процедура ДобавитьСтрокуКолонки(КолонкаТП, Знач ТекущаяКолонка, Знач ПоляТаблицыБД = Неопределено)
	...
	Иначе //! Если ТипЗнч(КолонкаТП) = Тип("КолонкаТабличногоПоля") Тогда
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ИмяТаблицыДинамСписка = "";
	КонецЕсли;
	ДобавитьКолонки(, ТекущаяКолонка);
	Если ТипЗнч(ТабличноеПоле) = Тип("ТаблицаФормы") Тогда
