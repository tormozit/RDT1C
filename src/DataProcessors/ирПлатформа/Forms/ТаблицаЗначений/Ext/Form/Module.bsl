...
Процедура УстановитьРедактируемоеЗначение(НовоеЗначение, ТабличноеПоле = Неопределено, ПредлагатьОбъединение = Истина) Экспорт 
	...
	Если Истина
		...
		И (Ложь
			Или (Истина
				И НЕ ЭтоДерево
				И Коллекция.Количество() > 0)
			Или (Истина
				И ЭтоДерево
				И Коллекция.Строки.Количество() > 0))
	Тогда
		...
		Если Не ЕстьНеСовместимыеКолонки Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				Если ЭтоДерево Тогда
					ИндексПервойСтроки = Коллекция.Строки.Количество();
					ирОбщий.ЗагрузитьВДеревоЗначенийЛкс(НовоеЗначение, Коллекция);
				Иначе
					ИндексПервойСтроки = Коллекция.Количество();
...
Функция НайтиСтрокуИзИсточника(Знач СтрокаИсточника, Знач ТабличноеПоле = Неопределено, КоллекцияИсточник = Неопределено, ИндексПервойСтроки = 0) 
	...
	ИначеЕсли ирОбщий.ЛиДанныеФормыСВозможностьюПоискаЛкс(КоллекцияИсточник) Тогда
		НоваяСтрока = НачальноеЗначениеВыбора.Получить(КоллекцияИсточник.Индекс(КоллекцияИсточник.НайтиПоИдентификатору(СтрокаИсточника)) + ИндексПервойСтроки);
	ИначеЕсли Ложь
		Или ТипЗнч(КоллекцияИсточник) = Тип("ДанныеФормыДерево") 
		Или ТипЗнч(КоллекцияИсточник) = Тип("ДеревоЗначений")
	Тогда
