...
Процедура КоллекцияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ирОбщий.ТабличноеПолеСОтборомПросмотраПриОкончанииРедактированияЛкс(ЭтаФорма, Элемент);
	#Если Сервер И Не Сервер Тогда
...
Процедура КоманднаяПанельТаблицаВСтрокуJSON(Кнопка)
	...
	#КонецЕсли
	Массив = Новый Массив;
	ИменаКолонок = ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(Таблица.Колонки, "Имя");
	...
	КонецЦикла;
	Результат = ирОбщий.ОбъектВСтрокуJSONЛкс(Массив, Истина);
	ирОбщий.ОткрытьТекстЛкс(Результат);
