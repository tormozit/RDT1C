...
Функция КоманднаяПанельТаблицаСохранитьВФайл(Кнопка = Неопределено)
	...
	Результат = ирКлиент.СохранитьЗначениеВФайлИнтерактивноЛкс(ПолучитьРезультат(), РасширениеФайла, "Таблица или дерево значений");
	Возврат Результат;
...
Процедура КоллекцияИспользоватьОтборПриИзменении(Элемент = Неопределено)
	Если ТипЗнч(ЭлементыФормы.Коллекция.Значение) = Тип("ТаблицаЗначений") Тогда
		ирКлиент.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.Коллекция,,, Элемент = Неопределено);
	Иначе
		ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.Коллекция, КоллекцияСтрокаПоиска);
	КонецЕсли;
КонецПроцедуры
