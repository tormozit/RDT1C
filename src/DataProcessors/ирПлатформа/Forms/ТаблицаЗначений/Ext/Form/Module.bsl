...
Перем мЧислоСтатичныхКолонокКолонок;
...
Процедура ТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если РежимВыбора Тогда
		Если КоллекцияИспользоватьОтбор Тогда
			КоллекцияИспользоватьОтборПриИзменении(0);
		КонецЕсли;
		ирКлиент.ПрименитьИзмененияИЗакрытьФормуЛкс(ЭтаФорма, ЭлементыФормы.Коллекция.ТекущаяСтрока);
	Иначе
...
Функция ПолучитьРезультат()
	...
	ЭтаФорма.ПараметрАтрибутыКолонок = Новый Структура;
	Для Каждого СтрокаКолонки Из ТаблицаКолонок Цикл
		АтрибутыКолонки = Новый Структура;
		Для Индекс = мЧислоСтатичныхКолонокКолонок По ТаблицаКолонок.Колонки.Количество()-1 Цикл
			ИмяАтрибута = ТаблицаКолонок.Колонки[Индекс].Имя;
			АтрибутыКолонки.Вставить(ИмяАтрибута, СтрокаКолонки[ИмяАтрибута]);
		КонецЦикла;
		Если АтрибутыКолонки.Количество() = 0 Тогда
			Прервать;
		КонецЕсли;
		ПараметрАтрибутыКолонок.Вставить(СтрокаКолонки.Имя, АтрибутыКолонки);
	КонецЦикла;
	Возврат Коллекция;
...
//   НовоеЗначение -  - 
//   ТабличноеПоле -  - 
//   ПредлагатьОбъединение -  - 
//   АтрибутыКолонок - Структура Из КлючИЗначение - :
//    *Значение - Структура
Процедура УстановитьРедактируемоеЗначение(Знач НовоеЗначение, Знач ТабличноеПоле = Неопределено, Знач ПредлагатьОбъединение = Истина, Знач АтрибутыКолонок = Неопределено) Экспорт 
	...
	Если ИндексПервойСтроки = 0 Тогда 
		...
		Если ЭлементыФормы.Коллекция.ИзменятьСоставСтрок Тогда
	...
	КонецЕсли; 
	Пока ТаблицаКолонок.Колонки.Количество() > мЧислоСтатичныхКолонокКолонок Цикл
		ТаблицаКолонок.Колонки.Удалить(мЧислоСтатичныхКолонокКолонок);
	КонецЦикла;
	Если АтрибутыКолонок <> Неопределено Тогда
		Для Каждого АтрибутыКолонки Из АтрибутыКолонок Цикл
			Если ТаблицаКолонок.Колонки.Количество() = мЧислоСтатичныхКолонокКолонок Тогда
				Для Каждого Атрибут Из АтрибутыКолонки.Значение Цикл
					ТаблицаКолонок.Колонки.Добавить(Атрибут.Ключ, Новый ОписаниеТипов("Строка")); 
					КолонкаТП = ЭлементыФормы.ТаблицаКолонок.Колонки.Добавить(Атрибут.Ключ, "<" + Атрибут.Ключ + ">");
					КолонкаТП.Данные = Атрибут.Ключ;
					КолонкаТП.УстановитьЭлементУправления(Тип("ПолеВвода"));
				КонецЦикла;
			КонецЕсли;
			СтрокаКолонки = ТаблицаКолонок.Найти(АтрибутыКолонки.Ключ, "Имя");
			Для Каждого Атрибут Из АтрибутыКолонки.Значение Цикл
				СтрокаКолонки[Атрибут.Ключ] = Атрибут.Значение;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	ЭтаФорма.КоллекцияИспользоватьОтбор = Ложь;
	ЭлементыФормы.Вперед.Доступность = ЭтоДерево;
	ЭлементыФормы.Назад.Доступность = ЭтоДерево; 
...
Функция НайтиСтрокуИзИсточника(Знач СтрокаИсточника, Знач ТабличноеПоле = Неопределено, КоллекцияИсточник = Неопределено, ИндексПервойСтроки = 0) 
	...
	ИначеЕсли ирОбщий.ЛиКоллекцияКомпоновкиЛкс(КоллекцияИсточник) Тогда
		Возврат Неопределено;
	ИначеЕсли ирОбщий.ЛиДанныеФормыСПоискомЛкс(КоллекцияИсточник) Тогда
		Если НоваяСтрока = Неопределено Тогда
			Если ТипЗнч(СтрокаИсточника) = Тип("Число") Тогда
				СтрокаИсточника = КоллекцияИсточник.НайтиПоИдентификатору(СтрокаИсточника);
			КонецЕсли;
			ИндексВИсточнике = КоллекцияИсточник.Индекс(СтрокаИсточника);
	...
	Тогда
		...
		КонецЕсли; 
	Иначе
		//! ЭлементыФормы.Коллекция.Значение = 0 // ТаблицаЗначений
		Если ЭлементыФормы.Коллекция.Значение.Количество() = КоллекцияИсточник.Количество() Тогда 
...
Процедура ПриОткрытии()
	...
	КонецЕсли;
	ЭтаФорма.рТолькоПросмотр = ТолькоПросмотр И Не ПараметрИзменятьПорядокСтрок;
	ЭлементыФормы.ТолькоПросмотр.Доступность = Не ТолькоПросмотр;
	Если ТолькоПросмотр И ПараметрИзменятьПорядокСтрок Тогда
		ЭлементыФормы.Коллекция.ИзменятьСоставСтрок = Ложь;
		ЭлементыФормы.Коллекция.ИзменятьПорядокСтрок = Истина;
		ЭлементыФормы.ТаблицаКолонок.ИзменятьСоставСтрок = Ложь;
		ЭлементыФормы.ТаблицаКолонок.ИзменятьПорядокСтрок = Истина;
		ирОбщий.УстановитьСвойствоВКоллекцииЛкс(ЭлементыФормы.ТаблицаКолонок.Колонки, "ТолькоПросмотр", Истина);
		Для Каждого КоманднаяПанель Из ирОбщий.ВыгрузитьСвойствоЛкс(ЭлементыФормы, "Э",, Тип("КоманднаяПанель")) Цикл
			Для Каждого Кнопка Из ирОбщий.ВсеЭлементыДереваЛкс(КоманднаяПанель) Цикл
				Если Истина
					И Кнопка.ИзменяетДанные 
					И Найти(Кнопка.Имя, "Сортировать") = 0
					И Найти(Кнопка.Имя, "Переместить") = 0
				Тогда
		...
		КонецЦикла;
	КонецЕсли;
	мЧислоСтатичныхКолонокКолонок = ТаблицаКолонок.Колонки.Количество();
	Если НачальноеЗначениеВыбора <> Коллекция Тогда
		УстановитьРедактируемоеЗначение(НачальноеЗначениеВыбора, ПараметрТабличноеПоле,, ПараметрАтрибутыКолонок);
	КонецЕсли; 
	Если ТаблицаКолонок.Количество() = 0 Тогда
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ТаблицаКолонок;
	ИначеЕсли ПараметрАктивироватьПоиск Тогда
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.КоллекцияСтрокаПоиска;
	КонецЕсли; 
	...
	Если РежимВыбора Или Не ЭлементыФормы.Коллекция.ИзменятьСоставСтрок Тогда
		ИзменитьСвернутостьПанелиКолонок(Ложь);
	КонецЕсли;
	Если Истина
		И Не ЭлементыФормы.Коллекция.ИзменятьСоставСтрок
		И ТипЗнч(Коллекция) = Тип("ТаблицаЗначений")
		И Коллекция.Количество()
	Тогда 
		ИменаКолонокДляСкрытия = Новый Массив;
		Для Каждого Колонка Из Коллекция.Колонки Цикл
			Если Истина
				И Коллекция[0][Колонка.Имя] = Колонка.ТипЗначения.ПривестиЗначение()
				И Коллекция.НайтиСтроки(Новый Структура(Колонка.Имя, Коллекция[0][Колонка.Имя])).Количество() = Коллекция.Количество()
			Тогда
				ЭлементыФормы.Коллекция.Колонки[Колонка.Имя].Видимость = Ложь;
				ИменаКолонокДляСкрытия.Добавить(Колонка.Имя);
			КонецЕсли;
		КонецЦикла;
		Если ИменаКолонокДляСкрытия.Количество() Тогда
			//ирОбщий.СообщитьЛкс("Скрыты пустые колонки: " + ирОбщий.СтрСоединитьЛкс(ИменаКолонокДляСкрытия, ", "));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПереустановитьТекущуюКолонку()
	...
	Если ЭлементыФормы.Коллекция.ТекущаяКолонка.Видимость Тогда
...
Процедура ОсновныеДействияФормыОК(Кнопка = Неопределено)
	...
	Если ЛиОсновнойПриемникФайл() Тогда 
		Если Не Модифицированность Или СохранитьВФайл() Тогда 
			Закрыть();
		Иначе
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьКолонкиТаблицы(РазрешитьВопросы = Ложь)
	...
	Для Каждого КолонкаДанных Из Коллекция.Колонки Цикл
		...
		КолонкаФормы.ТолькоПросмотр = Не ЭлементыФормы.Коллекция.ИзменятьСоставСтрок;
...
Процедура ТаблицаКолонокТипЗначенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.ТаблицаКолонок, СтандартнаяОбработка,,,,,, ЭлементыФормы.ТаблицаКолонок.ТекущаяСтрока.Имя);
...
Процедура ТаблицаКолонокПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		Если Не Копирование Тогда
			Элемент.ТекущиеДанные.ТипЗначения = ирОбщий.ОписаниеТиповВсеРедактируемыеТипыЛкс(); /// Чтобы стандартный поиск работал
		...
		Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.Заголовок) Тогда
...
Процедура КоманднаяПанельТаблицаЗагрузитьИзФайла(Кнопка)
	...
	ИмяФайла = мИмяОткрытогоФайла;
	ДанныеИзФайла = ирКлиент.ЗагрузитьЗначениеИзФайлаИнтерактивноЛкс(РасширениеФайла,,, ИмяФайла);
	ЗагрузитьДанныеИзФайла(ДанныеИзФайла, ИмяФайла);
	
КонецПроцедуры

Процедура ЗагрузитьДанныеИзФайла(Знач ДанныеИзФайла, Знач ИмяФайла) Экспорт
	Если Ложь
		Или ТипЗнч(ДанныеИзФайла) = Тип("ТаблицаЗначений")
		Или ТипЗнч(ДанныеИзФайла) = Тип("ДеревоЗначений")
	Тогда
		мИмяОткрытогоФайла = ИмяФайла;
		ЭтаФорма.Модифицированность = Ложь;
		СохранитьИмяФайла();
		ОбновитьЗаголовокОтФайла();
		УстановитьРедактируемоеЗначение(ДанныеИзФайла);
	КонецЕсли;
КонецПроцедуры

Процедура ТаблицаКолонокВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	
	ирКлиент.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка,,,,, ЭлементыФормы.ТаблицаКолонок.ТекущаяСтрока.Имя);
...
Функция ТабличныйДокументВТаблицуЗначений(Знач ИмяФайла = "", Знач ТабличныйДокумент = Неопределено) Экспорт
	...
	Иначе
		...
		КонецЕсли;
		Ответ = Вопрос("Сохранить текущие колонки таблицы?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			Форма.ТаблицаЗначений = ПолучитьРезультат();
		КонецЕсли;
...
Процедура КоманднаяПанельТаблицаВСписок(Кнопка)
	...
	ВыделенныеСтроки = ирКлиент.ВыделенныеИлиВсеСтрокиТабличногоПоляЛкс(ЭлементыФормы.Коллекция, Истина);
...
Процедура КоманднаяПанельТаблицаВТекстJSON(Кнопка)
	...
	Результат = ирОбщий.ТаблицаЗначенийВЖСОНЛкс(Таблица);
...
Процедура КоманднаяПанельТаблицаВТекстCSV(Кнопка)
	...
	КонецЕсли;
	Таблица = ПолучитьРезультат();
	Текст = ирОбщий.ТаблицаИлиРезультатЗапросаВТекстCSVЛкс(Таблица, Разделитель);
...
Процедура КоллекцияПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	Если ТипЗнч(Коллекция) = Тип("ДеревоЗначений") И ЭлементыФормы.Коллекция.ИзменятьСоставСтрок Тогда
...
Процедура ОбновитьПодменюИсторииФайлов()
	...
	Кнопки = ЭлементыФормы.КоманднаяПанельТаблица.Кнопки.Получить.Кнопки.ОткрытьПоследние.Кнопки;
...
Процедура ОткрытьФайлИзИстории(Кнопка) 
	...
	Если Модифицированность И ЗначениеЗаполнено(мИмяОткрытогоФайла) Тогда
	...
	ДанныеИзФайла = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(СтрокаИстории.Значение, Истина);
	ЗагрузитьДанныеИзФайла(ДанныеИзФайла, СтрокаИстории.Значение);
	
КонецПроцедуры

Процедура ТаблицаКолонокИмяПриИзменении(Элемент)
	ирОбщий.ЗаполнитьТипЗначенияПоИмениЛкс(ЭлементыФормы.ТаблицаКолонок.ТекущаяСтрока);
КонецПроцедуры

Процедура ВпередНажатие(Элемент)
	ирКлиент.СледующееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.Коллекция);
КонецПроцедуры

Процедура НазадНажатие(Элемент)
	ирКлиент.ПредыдущееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.Коллекция);
КонецПроцедуры
