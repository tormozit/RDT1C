...
Процедура КоманднаяПанельТаблицаЗагрузитьИзФайлаJSON(Кнопка)
	...
	СтрокаДерева = ирОбщий.ОткрытьПросмотрДереваJSONЛкс(Текст, "Выберите массив",, ЭтаФорма, Истина);
	Если СтрокаДерева = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТаблицаИзДерева = ирОбщий.ТаблицаИзДереваСвойствоЗначениеЛкс(СтрокаДерева);
...
Процедура КоманднаяПанельТаблицаСброситьКвалификаторы(Кнопка)
	...
	Для Каждого ВыбраннаяКолонка Из ЭлементыФормы.ТаблицаКолонок.ВыделенныеСтроки Цикл
		ВыбраннаяКолонка.ТипЗначения = Новый ОписаниеТипов(ВыбраннаяКолонка.ТипЗначения,,, Новый КвалификаторыЧисла, Новый КвалификаторыСтроки, Новый КвалификаторыДаты, Новый КвалификаторыДвоичныхДанных);
	КонецЦикла;
