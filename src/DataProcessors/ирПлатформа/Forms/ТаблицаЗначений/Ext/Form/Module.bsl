...
Процедура КоллекцияИспользоватьОтборПриИзменении(Элемент = Неопределено)
	...
	Иначе
		ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюЛкс(ЭтаФорма, ЭлементыФормы.Коллекция, КоллекцияСтрокаПоиска);
	КонецЕсли;
КонецПроцедуры
...
Процедура КоманднаяПанельТаблицаКластеризация(Кнопка)
	...
	КонецЕсли;
	ФормаАнализа.ПараметрВыделенныеКолонки = ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(ЭлементыФормы.ТаблицаКолонок.ВыделенныеСтроки, "Имя");
...
Процедура КоманднаяПанельТаблицаВСтрокуCSV(Кнопка)
	...
	ЗаписьТекста.ЗаписатьБезОбработки(ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(Таблица.Колонки), Разделитель) + Символы.ПС);
	_РежимОтладки = ирКэш.РежимОтладкиЛкс();
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.ПрЛкс(_РежимОтладки, 1, 1)
		// Пассивный оригинал расположенного ниже однострочного кода. Выполняйте изменения синхронно в обоих вариантах.
		Для Каждого СтрокаТаблицы Из Таблица Цикл
