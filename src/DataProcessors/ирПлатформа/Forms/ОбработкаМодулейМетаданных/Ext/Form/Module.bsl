...
Процедура ВыполнитьЗамену(Знач РежимТеста)
	...
	Если ЛиЗаменаБлока Тогда
		...
		Иначе
			...
			ШаблонОтступа = "\s*";
		КонецЕсли;
		РегВычислитель.Pattern = "(" + ГраницаСтроки + ШаблонОтступа + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ЧтоЗаменять) + "\s*" + ГраницаСтроки + ")"
			+ "[\S\s]+?" + "(" + ГраницаСтроки + ШаблонОтступа + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ЧтоЗаменятьКонец) + "\s*" + ГраницаСтроки + ")";
	КонецЕсли;
