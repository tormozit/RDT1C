...
Процедура ВыполнитьЗамену(Знач РежимТеста)
	...
	Если ЛиЗаменаБлока Тогда
		...
		Иначе
			...
			ШаблонОтступа = "\s*";
		КонецЕсли;
		РегВычислитель.Pattern = "(" + ГраницаСтроки + ШаблонОтступа + ирОбщий.ТекстДляРегВыраженияЛкс(ЧтоЗаменять) + "\s*" + ГраницаСтроки + ")"
			+ "[\S\s]+?" + "(" + ГраницаСтроки + ШаблонОтступа + ирОбщий.ТекстДляРегВыраженияЛкс(ЧтоЗаменятьКонец) + "\s*" + ГраницаСтроки + ")";
	КонецЕсли;
	ФайлыПапки = НайтиФайлы(Папка, Маска, Истина);
	СчетчикИзменных = 0;
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
