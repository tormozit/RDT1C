		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Тогда
						...
						Иначе
							...
							Иначе
								...
								Попытка
									ЗначениеСвойства = ПроверяемыеМетаданные[СтрокаСлова.Слово];
								Исключение
									ЗначениеСвойства = Неопределено;
								КонецПопытки;
							КонецЕсли; 
					...
					ИначеЕсли Истина
						И СтрокаСлова.ТипСлова = "Метод"
						И Лев(ИмяОбщегоТипаСлова, СтрДлина(МаркерКоллекцииМетаданных)) = МаркерКоллекцииМетаданных
						И (Ложь
					...
					ИначеЕсли Истина
						И СтрокаСлова.ТипКонтекста = "ОтчетОбъект.<Имя отчета>"
						И СтрокаСлова.Слово = "КомпоновщикНастроек"
						И СтрокаСлова.ТипСлова = "Свойство"
						И ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданных")
					Тогда
						СхемаКомпоновкиДанных = ирКэш.ОсновнаяСхемаКомпоновкиОтчетаЛкс(МетаданныеРодителя.ПолноеИмя());
					...
					ИначеЕсли Истина      
							И СтрокаСлова.ТипСлова = "Свойство"
							И СтрокаСлова.Слово = "ТекущаяСтрока"
							И (Ложь
							...
							Иначе
								...
								Тогда
									...
									Если МодульФормы.Переменные.Найти(НРег(МетаданныеРодителя.Данные), "НИмя") <> Неопределено Тогда
										...
										НоваяСтруктураТипа = ТаблицаТиповТаблицыФормы[0];
									КонецЕсли;
								КонецЕсли;
								...
								Если НоваяТаблицаТипов.Количество() = 0 Тогда 
									...
									//ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("ТабличноеПоле") Тогда
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			Если Ложь
				...
				Или СтрокаВида.НомерВерсииПлатформы > мНомерВерсииПлатформы
		...
		Тогда
			...
			Если МодульМетаданных <> Неопределено И СтарыйМодуль <> МодульМетаданных Тогда
				...
				Тогда
					...
					ТаблицаСлов = СловаКонтекстаМетаданные(СтруктураТипа,, "Свойство",,,,,,, ПустойМодульМетаданных());
						...
						Для каждого ВхождениеОписания Из ВхожденияОписаний Цикл
							...
							Если СтрокаПараметра <> Неопределено Тогда
								...
								Если Не ЗначениеЗаполнено(СтрокаПараметра.ТипЗначения) Тогда
									СтрокаПараметра.ТипЗначения = СокрЛП(ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаПараметра.Описание, "-", Символы.ПС));
								КонецЕсли;
								...
								Тогда
									...
									Если ВхожденияТипов.Количество() > 0 Тогда
										Типы = ВхожденияТипов.ВыгрузитьКолонку("Подгруппа0");
										Если ЗначениеЗаполнено(СтрокаПараметра.ТипЗначения) Тогда
...
//
Функция ТаблицаСловИзСтруктурыТипа(Знач СтруктураТипа, Знач ЯзыкПрограммы = 0, Знач Конфигурация = Неопределено, Знач ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	Для Каждого СтрокаИсточника Из ТаблицаИсточник Цикл
		...
		Если НужноВытеснять Тогда 
			Если Найти(СтрокаИсточника.ИмяОбщегоТипа, "Массив") = 1 Тогда
				...
				Для Каждого СтрокаПриемника Из ТаблицаПриемник Цикл
					Если Найти(СтрокаПриемника.ИмяОбщегоТипа, "Массив") = 1 Тогда
						...
						СлитьМетаданныеСтруктурТипов(ТаблицаПриемник, СтрокаИсточника, СтрокаПриемника);
						Прервать;
					КонецЕсли;
				КонецЦикла;
			...
			Иначе
				...
				КонецЕсли;
				Если Найденные.Количество() > 0 Тогда 
					Если СлитьМетаданныеСтруктурТипов(ТаблицаПриемник, СтрокаИсточника, Найденные[0]) Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
//
Функция СлитьМетаданныеСтруктурТипов(Знач ТаблицаПриемник, Знач СтрокаИсточника, Знач СтрокаПриемника) Экспорт
