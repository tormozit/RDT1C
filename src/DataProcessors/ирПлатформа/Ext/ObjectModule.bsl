...
Функция СоздаваемыеВременныеТаблицыПакетаЗапросов(Знач ТекстПакетаИлиМассивТекстовЗапросов, Знач ТолькоТребующиеУничтоженияНаВходе = Ложь, Знач ВместеСУничтожениями = Ложь) Экспорт 
	...
	Для Каждого ТекстЗапроса Из ТекстыЗапросовПакета Цикл
		...
		Если ирОбщий.СтрокиРавныЛкс(СозданиеВременнойТаблицы, "добавить") Тогда
			Если Не ТолькоТребующиеУничтоженияНаВходе Тогда
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Тогда
						...
						#КонецЕсли
					ИначеЕсли Истина
						И СтрокаСлова.Слово = "ВыгрузитьЗначения"
						И СтрокаСлова.ТипСлова = "Метод"
						И ТипЗнч(МетаданныеРодителя) = Тип("Структура")
					Тогда 
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
	...
	Если Слово <> Неопределено И ТекущийИндекс <> Неопределено Тогда 
		...
		Иначе
			...
			РезультатСтроковойКонстанты = мРегВыражение.НайтиВхождения(Слово);
							...
							Если СтрокаПараметра <> Неопределено Тогда
								...
								//Иначе 
									...
									НуженРазборТипа = Истина
										...
										И СокрЛП(СтрокаПараметра.Описание) <> "-";
									...
									Если НуженРазборТипа Тогда
										// Дорого. TODO Проанализировать оправданность
...
Функция ДобавитьВТаблицуСлов(Знач ТаблицаСлов, Знач Слово, Знач ТипСлова = "Свойство", Знач ТаблицаТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "", Знач ВытеснятьСвойство = Истина,
	Знач СтарыеТипы = Неопределено, Знач УстановитьНСлово = Ложь) Экспорт
