...
Функция СтруктураРезультатаПакетногоЗапроса(ТекстПакета, ПрефиксКомментария = "//") Экспорт 
	...
	RegExp.Global = Ложь;
	RegExp.Multiline = Ложь;
	RegExp.Pattern = ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ПрефиксКомментария + мМаркерИмениЗапросаПакета) + "(" + шИмя + ")(?: |\n|\r)";
	Индекс = -1;
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
	Знач МодульМетаданных, Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ВычислятьТипы = Истина, Знач ТолькоЭкспорт = Истина) Экспорт 
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено) Экспорт 
	...
	ВерсияФормата = 1; // Каждый раз при изменении структуры увеличиваем на 1
	...
	Если ЗначениеЗаполнено(выхИмяФайлаМодуляБезРасширения) Тогда
		...
		Тогда
			...
			Если НовыйТекст <> Неопределено Тогда
				...
				КонецЕсли;     
			КонецЕсли;
			ФайлОписанияМодуля = ФайлМодуляИзКраткогоИмени(выхИмяФайлаМодуляБезРасширения, Истина);
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Ложь) Экспорт 
	Если МодульМетаданных <> мПустойМодуль Тогда
		ФайлОписанияМодуля = ФайлМодуляИзКраткогоИмени(МодульМетаданных.Имя, Истина);
...
// Возвращаемое значение:
//    Файл - 
Функция ФайлМодуляИзКраткогоИмени(Знач ИмяФайлаМодуляБезРасширения, Знач ФайлОписания = Ложь) Экспорт
	Если ФайлОписания = Ложь Тогда
		Расширение = "txt";
	Иначе
		Расширение = "ixd";
	КонецЕсли;
...
Функция ИмяФайлаМодуляБезРасширения(Знач СтруктураТипа, ТипМодуля = "", СтрокаСозданияРодителя = "", ФлагиКомпиляции = Неопределено) Экспорт 
	...
	Иначе
		ФлагиКомпиляции = НовыеФлагиКомпиляцииМодуля();
	КонецЕсли;
	...
	Тогда
		...
		Если ИмяОбщегоТипа = "ОбщийМодуль" Тогда
			...
			Если ТипЗнч(МетаданныеРодителя) = Тип("ОбщийМодуль") Тогда
	...
	Тогда 
		Если ТипЗнч(МетаданныеРодителя) = Тип("Форма") Тогда
			...
			Если СлужебныеДанныеФормы.Свойство("ИмяФормы") Тогда
				МетаданныеРодителя = СлужебныеДанныеФормы.ИмяФормы + ".Форма";
			КонецЕсли;
		ИначеЕсли ТипЗнч(МетаданныеРодителя) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
			МетаданныеРодителя = МетаданныеРодителя.ИмяФормы + ".Форма";
		Иначе
			МетаданныеРодителя = МетаданныеРодителя.ПолноеИмя() + ".Форма.!.Форма";
		КонецЕсли;
		ТипМодуля = "Модуль";
	ИначеЕсли Ложь
...
// Возвращаемое значение:
//    Структура - 
Функция НовыеФлагиКомпиляцииМодуля() Экспорт
	ФлагиКомпиляции = Новый Структура;
	ФлагиКомпиляции.Вставить("Сервер", Истина);
	ФлагиКомпиляции.Вставить("КлиентОбычноеПриложение", Истина);
	ФлагиКомпиляции.Вставить("КлиентУправляемоеПриложение", Истина);
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина) Экспорт
	...
	Если СтруктураТипа <> Неопределено Тогда 
		...
		Тогда
			...
			Для Каждого СтрокаСлова Из ТаблицаСловИзСтруктурыТипа(СтруктураТипа,,,, Ложь,, "Свойство",,, МодульМетаданных("")) Цикл
	...
	Вхождения = Модуль.ВхожденияМетодов;
	
	// При добавлении колонок нужно добавлять проверку их в кэше в Метка91237256881041
...
Процедура ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач МодульМетаданных, Знач СтрокаОпределения, Знач ОписаниеТипа = "", Знач ТекстДляПоискаОпределения = "")
	...
	кэшКомпонентаПодсказки.мМетодМодуля = МетодМодуля;
...
Функция СтруктураТипаИзЗначения(Значение, ЯзыкПрограммы = 0, ШаблонСтруктурыТипа = Неопределено) Экспорт
	...
	Тогда
		...
		Тогда
			...
			КлючОсновногоОбъекта = ирОбщий.КлючОсновногоОбъектаФормыЛкс(Значение);
		...
		ИначеЕсли ИмяОбщегоТипа = "Форма" Тогда
			#Если Клиент Тогда
			ОсновнойРеквизитФормы = ИмяОсновногоРеквизитаФормы(Значение);
...
Процедура ДобавитьВТаблицуСтруктурТипов(ТаблицаПриемник, Знач ОписаниеТипов, Знач ПередаватьДополнительныеТипы = Ложь) Экспорт 
	...
	Для Каждого СтрокаИсточника Из ТаблицаИсточник Цикл
		...
		Если НужноВытеснять Тогда 
			...
			Если Истина
				И СтрокаИсточника.Детальность > 0
				И ирОбщий.СтрНачинаетсяСЛкс(СтрокаИсточника.ИмяОбщегоТипа, "Расширение формы", Истина)  // Учитывам регистр для ускорения
			Тогда
...
Функция НайтиИмяМетодаСтрокиМодуля(ИмяМодуля, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулей = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "", Знач ЧислоСтрокДляПоискаИмениМетода = 1000) Экспорт 
...
Функция ПортДляПодключенияОтладчика(выхИдентификаторыОтладчиков = Неопределено, выхПротокол = "") Экспорт
	...
	Иначе
		...
		ЛокальныйАдрес = ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс("127.0.0.1");
...
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM, Знач ПутьКРодителю = Неопределено, Знач ЛиКоллекция = Ложь) Экспорт
	...
	Если ПутьКРодителю <> Неопределено Тогда
		...
		Если ТипЗнч(ФормаИлиДокументDOM) = ТипУправляемаяФорма Тогда
			СтруктураТипа = НоваяСтруктураТипа("УправляемаяФорма");
			СтруктураТипа.Метаданные = ФормаИлиДокументDOM;
			...
			Если ПутьКРодителю = "" И ИменаРеквизитов = Неопределено Тогда 
				...
				Пока Истина Цикл
					Для Каждого ИмяРеквизита Из КандидатыРеквизитов Цикл
						Попытка
							ЗначениеРеквизита = ФормаИлиДокументDOM[ИмяРеквизита];
						Исключение
							Продолжить;
						КонецПопытки;
					...
					Если МодульПроанализирован Тогда
						Прервать;
					КонецЕсли;
					Для Каждого ЭлементФормы Из ФормаИлиДокументDOM.Элементы Цикл
						ИмяРеквизита = ирОбщий.ПервыйФрагментЛкс(ирОбщий.ПутьКДаннымЭлементаУправляемойФормыЛкс(ЭлементФормы,, ФормаИлиДокументDOM));
						Если Истина
					...
					Если ЛюбыеДанныеФормы = Неопределено Тогда
						...
						ПолеТекстаМодуля.мРодительскийКонтекст = "ЭтаФорма";
						ПолеТекстаМодуля.ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(Ложь, Ложь, Истина, Ложь,,,, Истина);
						КандидатыРеквизитов = ПолеТекстаМодуля.ТаблицаСлов.Выгрузить(Новый Структура("Определение", "Статистический")).ВыгрузитьКолонку("Слово");
					КонецЕсли;
					МодульПроанализирован = Истина;
				КонецЦикла;
				~Конец:
			КонецЕсли;
		ИначеЕсли ТипЗнч(ФормаИлиДокументDOM) = Тип("ДокументDOM") Тогда
			ДокументДОМ = ФормаИлиДокументDOM;
		Иначе
			ДокументДОМ = ирОбщий.ТекстВДокументDOMЛкс(ирОбщий.ОбъектВСтрокуXMLЛкс(ФормаИлиДокументDOM));
		КонецЕсли;
		Если ЛюбыеДанныеФормы <> Неопределено Тогда
...
Функция ПолеТекстаПрограммы(Знач ЯзыкПрограммы = 0, Знач ТекстМодуля = "") Экспорт
	Если мПоляТекстаПрограммы = Неопределено Тогда
		мПоляТекстаПрограммы = Новый Соответствие;
	КонецЕсли;
	Если ЯзыкПрограммы = 0 И ирОбщий.ЛиТекстЯзыкаЗапросовЛкс(ТекстМодуля) Тогда
		ЯзыкПрограммы = 1;
	КонецЕсли;
