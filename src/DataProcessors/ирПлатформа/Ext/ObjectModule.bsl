...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено, ТекущийМетод = Неопределено) Экспорт 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	ТекущийМетод = Неопределено) Экспорт
...
Процедура ДобавитьВТаблицуТипов(ТаблицаПриемник, Знач ОписаниеТипов, Знач ПередаватьДополнительныеТипы = Ложь) Экспорт 
	...
	КонецЕсли;
	Если ТаблицаИсточник.Количество() > МаксЧислоТиповДляАнализа() Тогда
