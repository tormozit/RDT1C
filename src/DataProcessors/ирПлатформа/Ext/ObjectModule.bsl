		...
		Тогда
			...
			Если ФайлОписанияМодуля.Существует() Тогда
				...
				Иначе
					...
					МодульМетаданных.Вставить("ТекстПоискаОпределенияСловМодуля");
				КонецЕсли; 
			КонецЕсли; 
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	ВерсияФормата = 14; // Метка91237881041 Каждый раз при изменении структуры увеличиваем на 1
...
Функция НепрямыеВызовыСловаВМодуле(Знач ИмяМодуля, Знач ТекстовыйДокумент, Знач Файл, Знач ПолеТекстаПрограммы, Знач ИскатьСлово, Знач ЛиМетод = Истина) Экспорт
	...
	Если ИндексВызововМодуля = Неопределено Тогда
		ИменаСвойствИндекса = "ВерсияФормата, Вызовы, СтруктураСлов"; 
...
Процедура СброситьКэшТиповВыраженийМодуля(Знач Модуль) Экспорт
	...
	Методы.ЗаполнитьЗначения(, "ТаблицаТипов, ТипыВыражений, Вычислено"); // Чтобы не отслеживать зависимости. ТипыВыражений - самые тяжелые   
	_РежимОтладки = Ложь;
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1) 
		// Пассивный оригинал расположенного ниже однострочного кода. Выполняйте изменения синхронно в обоих вариантах. 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			...
			Иначе 
				...
				Для ИндексМетодаЦикл = ИндексТекущегоМетода + 1 По Методы.Количество() - 1 Цикл
					...
					Если Ошибки <> Неопределено И СтрокаМетода.РассчитаныОшибки Тогда
	...
	Иначе 
		...
		Если СтруктураТипа <> Неопределено Тогда
			...
			СтрокаМетода.РассчитаныОшибки = Истина;
			СтрокаМетода.Имя = "<>";
...
Процедура УдалитьОшибкиМетода(Знач СтрокаМетода, Знач Ошибки) Экспорт
	Если СтрокаМетода = Неопределено Тогда
		ИмяМетода = "<Инициация>";
	Иначе 
		Если Не СтрокаМетода.РассчитаныОшибки Тогда
			Возврат;
		Иначе 
			СтрокаМетода.РассчитаныОшибки = Ложь;
...
Функция НоваяТаблицаМетодовМодуля() Экспорт 
	...
	Методы.Колонки.Добавить("РассчитаныОшибки", Новый ОписаниеТипов("Булево"));
