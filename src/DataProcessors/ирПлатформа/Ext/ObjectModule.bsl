			...
			Тогда
				Если ТипЗнч(МетаданныеРодителя) = Тип("Структура") Тогда 
					...
					Тогда 
						...
						СхемаКоллекции = Новый Структура;
						Для Каждого КлючИЗначение Из МетаданныеРодителя Цикл
							СхемаКоллекции.Вставить(КлючИЗначение.Ключ);
						КонецЦикла;
					Иначе
						КоллекцияЗначений = МетаданныеРодителя;
						Если ОписаниеСвойства <> Неопределено Тогда
			...
			Иначе
				...
				Если МодульМетаданныхКомпонента <> Неопределено Тогда
					...
					Тогда
						...
						ТаблицаСлов = ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, ТолькоЭкспорт,
...
Функция ДобавитьВременныеТаблицыПараметровВТекстЗапроса(ТекстЗапроса, Знач Параметры) Экспорт
	...
	Если Найти(ТекстЗапроса, "&") > 0 Тогда
		Найденное = ирОбщий.НайтиРегВыражениеЛкс(ТекстЗапроса, "\s(?:ИЗ|FROM)\s+&("+ шИмя + ")",, Ложь);
		Если Найденное.Количество() > 0 Тогда
			ИмяТаблицы = Найденное[0].Группа0;
			Если Не Параметры.Свойство(ИмяТаблицы) Тогда
				Параметры.Вставить(ИмяТаблицы, Новый ТаблицаЗначений);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из Параметры Цикл
		...
		Если ТипЗнч(ЗначениеПараметра) = Тип("ТаблицаЗначений") Тогда
			Если ЗначениеПараметра.Колонки.Количество() = 0 Тогда
				ИменаПолей = ирОбщий.НайтиРегВыражениеЛкс(ирОбщий.ПервыйФрагментЛкс(ТекстЗапроса, "ИЗ"), "[\s\.]("+ шИмя +")\s*(?:,|$)");
