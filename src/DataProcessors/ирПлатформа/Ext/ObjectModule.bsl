...
Функция РазвернутаяТаблицаТиповИзСтруктурыТипа(Знач СтруктураТипа, Знач ЭтоЛокальныйКонтекст, Знач ДляФиксированныхСлов = Истина, Знач ДоступноКэширование = Ложь) Экспорт 
	...
	Тогда
		...
		Если СтрокаОбщегоТипа <> Неопределено Тогда
			НоваяСтрока = МассивОбщихТипов.Добавить();
			НоваяСтрока.ИмяОбщегоТипа = СтрокаОбщегоТипа.Слово; 
		КонецЕсли;
				...
				Если Истина
					...
					И (Ложь
						...
						Или (Истина 
							...
							И МодульМетаданныхКомпонента.Имя = "<Пустой>"
							И ЗначениеЗаполнено(СловоФильтр)
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	Возврат 28; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
КонецФункции
...
// Функция - Модуль метаданных. Для пустого текста и НЕ пустого имени возвращает единый модуль с именем "<Пустой>"!
// Параметры:
//  ТекстМодуля								 - Строка	 - 
//  СтруктураТипа							 - см. НоваяСтруктураТипа() -
//  ИмяМодуля								 - Строка	 - 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Если Модуль.ПозицияМетодов > 30 Тогда
		...
		МаркерЗапретаПроверки = "@skip-check unknown-method-property";
...
Функция МетодИнициацииМодуля(Знач Модуль, Знач ОбновитьПозицию = Ложь) Экспорт
	...
	Если ОбновитьПозицию Тогда
		...
		//Если Сред(Модуль.Текст, НачальнаяПозиция, 1) = Символы.ПС Тогда
	...
	Иначе 
		...
		Иначе  
			...
			Попытка
				...
				Тогда
					СтрокаОпределения.ТипЗначения = ПредставлениеМассиваСтруктурТипов(ТаблицаТиповИзКомментария,,,, Истина);
				КонецЕсли;
				Если Ложь
...
// Возвращаемое значение:
//   Строка, Неопределено - 
Функция НайтиМетодПоНомеруСтроки(Знач ПолеТекстаМодуля, Знач НомерСтроки, Знач ИмяМодуля = "", КэшиРазметкиМодуля = Неопределено, Знач ЧислоСтрокДляПоискаИмениМетода = 2000, НужноеОписаниеМетода = Неопределено) Экспорт
