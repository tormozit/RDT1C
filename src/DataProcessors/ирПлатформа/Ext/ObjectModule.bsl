...
Перем мСвязанныйКонфигуратор Экспорт; // см. ПодключитьСвязанныйКонфигуратор
...
Функция ПреобразоватьЗапросВПодзапрос(Текст, ТекстВыбранныхПолей = "*", ПсевдонимВложенногоЗапроса = "ВложенныйЗапрос",
	...
	Если ЗначениеЗаполнено(ТекстДоПоследнегоУПОРЯДОЧИТЬ) Тогда
		...
		ирОбщий.ДобавитьТекстВТекстЛкс(Результат, ТекстДоПоследнегоУПОРЯДОЧИТЬ, Смещение);
	Иначе
		ирОбщий.ДобавитьТекстВТекстЛкс(Результат, РезультатПоиска.SubMatches(5), Смещение);
	КонецЕсли; 
	Если ЭтоРусскийЯзык Тогда
...
//
Функция ПодобратьВариантСинтаксисаМетодаПоЧислуПараметров(Знач ТаблицаВариантов, Знач КоличествоФактПараметровМетода = 0, Знач ТекущийВариант = "", Знач ТекущийВариантУстановленВручную = Ложь, выхНомер = 0) Экспорт
...
Функция ЛиЕстьСловоТипаКонтекстаЛкс(Знач ТипКонтекста, Знач Слово, Знач ТипСлова = "Свойство") Экспорт
	ИнициацияОписанияМетодовИСвойств();
	Результат = ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ЯзыкПрограммы, ТипЯзыка, НСлово, ТипСлова", ТипКонтекста, 0, "", НРег(Слово), ТипСлова)).Количество() > 0;
	Возврат Результат;
КонецФункции
	...
	Тогда 
		...
		Для Каждого ТипЗначения Из МассивТиповЗначения Цикл
			...
			ИначеЕсли Ложь 
				...
				Или ТипЗначения = "ЭлементОтбора" И МетаданныеРодителя <> Неопределено
			Тогда
				...
				Тогда
					...
					Если МассивТипов.Количество() > МаксЧислоТиповДляАнализа Тогда
						...
						Тогда
							...
							КонецЕсли;
						КонецЕсли;
						//СтруктураТипаСвойства = НоваяСтруктураТипа(СтруктураТипаСвойства.ИмяОбщегоТипа);
						СтруктураТипаСвойства.Метаданные = Неопределено;
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ТаблицаСловПриемник = Неопределено,
	...
	КонецЕсли;
	ТаблицаСловКонтрактовМодуля(МодульМетаданных, СловоФильтр, ТипСловаФильтр, ВычислятьТипыМетодовМодулей, ВычислятьТипы, ВнутриГруппыОбщихМодулей, ДляСвойства, ЛиВместеСЛокальнымКонтекстом,
		ТолькоСоЗначениями, СтрогийФильтр, ТаблицаСлов);
	...
	Тогда
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			...
			Если ВычислятьТипы Тогда
				...
				КонецЕсли;
				ТаблицаТипов = ТаблицаТиповСловаМодуля(ПеременнаяМодуля, МодульМетаданных.Имя, ТаблицаТипов);
			Иначе
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если ВычислятьТипы Тогда
				...
				КонецЕсли;
				ТаблицаТипов = ТаблицаТиповСловаМодуля(МетодМодуля, МодульМетаданных.Имя, ТаблицаТипов);
			Иначе
...
//.
// Параметры:
//   МодульМетаданных - Структура, Неопределено - 
//   СловоФильтр - Произвольный, Неопределено - 
//   ТипСловаФильтр - Строка, Неопределено - 
//   ВычислятьТипыМетодовМодулей - Булево - 
//   ВычислятьТипы - Булево - 
//   ВнутриГруппыОбщихМодулей - Булево - 
//   ДляСвойства - Строка - 
//   ЛиВместеСЛокальнымКонтекстом - Булево - 
//   ТолькоСоЗначениями - Булево - 
//   СтрогийФильтр - Булево - 
//   ТаблицаСлов - ТаблицаЗначений, Неопределено - 
// Возвращаемое значение:
//   ТаблицаЗначений, Неопределено - 
Функция ТаблицаСловКонтрактовМодуля(Знач МодульМетаданных, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = "", Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ВычислятьТипы = Ложь, Знач ВнутриГруппыОбщихМодулей = Ложь
, Знач ДляСвойства = "", Знач ЛиВместеСЛокальнымКонтекстом = Ложь, Знач ТолькоСоЗначениями = Ложь, Знач СтрогийФильтр = Ложь, Знач ТаблицаСлов = Неопределено) Экспорт
	Если ТаблицаСлов = Неопределено Тогда
		ТаблицаСлов = НоваяТаблицаСлов();
	КонецЕсли;
	Если МодульМетаданных.Свойство("Родители") Тогда
		...
		Для Каждого ТипРодителя Из МодульМетаданных.Родители Цикл
			ТаблицаТиповРодителей = ПолеТекстаПрограммы.ТаблицаТиповИзТекста(ТипРодителя);
...
// Возвращаемое значение:
//    ТаблицаЗначений - 
Функция ТаблицаТиповСловаМодуля(Знач СловоМодуля, Знач ИмяМодуля, Знач ТаблицаТипов = Неопределено) Экспорт
	Если ТаблицаТипов = Неопределено Тогда
	...
	СловоМодуля.ИмяМодуля = ИмяМодуля;
	Возврат ТаблицаТипов; 
КонецФункции
	...
	Если ЗначениеЗаполнено(выхИмяМодуля) Тогда
		...
		Тогда
			...
			Если Истина
				...
				И ФайлОписанияМодуля <> Неопределено
			...
			Если ОбновитьФайлОписания Тогда
				Если Истина
					И ФайлМодуля <> Неопределено
					И Не ФайлМодуля.Существует() 
					И НовыйТекст = Неопределено 
				Тогда
				...
				Иначе 
					...
					Иначе
						...
						Если ФайлОписанияМодуля <> Неопределено Тогда
			...
			Если Не МодульМетаданных.Свойство("ДатаСоздания") Тогда
				МодульМетаданных.Вставить("ДатаСоздания", Дата(1,1,1));
			КонецЕсли;
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Истина, Знач ТолькоЕслиОтсутствует = Ложь) Экспорт 
	...
	Если ФайлОписанияМодуля = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ТолькоЕслиОтсутствует Тогда
		Если ФайлОписанияМодуля.Существует() Тогда
			Возврат;
		КонецЕсли;
		ФайлТекстаМодуля = ФайлМодуляИзИмениМодуля(МодульМетаданных.Имя);
		Если Ложь
			Или Не ФайлТекстаМодуля.Существует()
			Или ФайлТекстаМодуля.ПолучитьВремяИзменения() > МодульМетаданных.ДатаСоздания
		Тогда                                            
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			...
			Если ТекущийМетод <> Неопределено Тогда
				...
				Если Истина
					...
					И (Ложь
						Или Лев(ТекущийМетод.Имя, 1) = "<"
						Или ИменаМетодовИнициацииБольшогоМодуля().Свойство(ТекущийМетод.Имя))
				Тогда
	...
	Модуль.Вставить("ДатаСоздания", ТекущаяДата());
...
Функция ПодготовитьТипРезультатаМетода(МетодМодуля, ИмяИлиМодуль, ПолеТекстаМодуляКэш = Неопределено, Знач СФактическимиПараметрами = Ложь, Знач ДляСвойства = "", Знач ЛиФинальный = Ложь) Экспорт 
	Если Ложь
	...
	Если ЛиФинальный Тогда
		ТаблицаТипов = ТаблицаТиповСловаМодуля(МетодМодуля, ПолеТекстаМодуляКэш.мИмяМодуля, ТаблицаТипов);
	КонецЕсли;
	Возврат ТаблицаТипов;  
КонецФункции
	...
	Для Каждого СтрокаЭлементаЦикла Из ДеревоВыбора.Строки Цикл
		...
		Тогда
			Если СтрокаДереваВыбора.ТипСлова = "Конструктор" Тогда
				...
				КонецЕсли;
				НовыйТекущийВариант = ПодобратьВариантСинтаксисаМетодаПоЧислуПараметров(ТаблицаВариантов, КоличествоФактПараметровМетода, ТекущийВариант, ЗначениеЗаполнено(ТекущийВариант));
				//НовыйТекущийВариант = ПодобратьВариантСинтаксисаМетодаПоЧислуПараметров(ТаблицаВариантов, КоличествоФактПараметровМетода);
...
Функция ПассивнаяФормаПоИмениКэш(Знач ИмяФормы, Знач ИмяРасширения = "") Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если ФайлФормы <> Неопределено И ФайлФормы.Существует() Тогда 
		...
		Если Сообщение <> "" Тогда
			...
			ирОбщий.СообщитьЛкс(Сообщение, СтатусСообщения.Информация,,, Ложь);
		КонецЕсли;
...
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM, Знач ПутьКРодителю = Неопределено, Знач ЛиДляКоллекции = Ложь, выхИдентификаторыРеквизитов = Неопределено, ДокументДОМ = Неопределено,
	...
	КонецЕсли;
	Если Истина
		И ПустаяСтрока(ИмяОсновногоРеквизита)
		И ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения")
		И ИменаРеквизитов.Свойство("Отчет")
		И ирОбщий.СтрНачинаетсяСЛкс(Форма.ИмяФормы, "Отчет.")
	Тогда
		// Для полностью динамической формы отчета. Точно приходим сюда на платформе 8.5
		ИмяОсновногоРеквизита = "Отчет";
		Если ИменаРеквизитов.Отчет = Новый ОписаниеТипов Тогда
			ИменаРеквизитов.Отчет = Новый ОписаниеТипов("ОтчетОбъект");
		КонецЕсли;
	КонецЕсли;
...
// Активизирует главное окно процесса 1cv8.exe по PID.
//
// Параметры:
//  *ИдентификаторПроцесса - Число.
//
Процедура АктивизироватьОкноПроцесса1С8(Знач ИдентификаторПроцесса = 0) Экспорт 
	ПолучитьФайлОткрывателя1С();
	СтрокаЗапуска = """" + ФайлОткрывателя1С.ПолноеИмя + """";
...
Функция ПодключитьСвязанныйКонфигуратор(Знач ИД, Знач ПапкаПередачиФайлов) Экспорт 
	Если мСвязанныйКонфигуратор = Неопределено Тогда
		мСвязанныйКонфигуратор = Новый Структура("ИД, ПапкаПередачиФайлов");
	КонецЕсли;
	мСвязанныйКонфигуратор.ИД = ИД;
	мСвязанныйКонфигуратор.ПапкаПередачиФайлов = ПапкаПередачиФайлов;
	Возврат мСвязанныйКонфигуратор;
КонецФункции

Процедура АктивизироватьОкноКонфигуратора(Знач Команда = "", Знач НовыйТекстИлиФайл = Неопределено, Знач СтарыйТекст = Неопределено) Экспорт 
	Если ЗначениеЗаполнено(Команда) Тогда
		УдалитьФайлы(мСвязанныйКонфигуратор.ПапкаПередачиФайлов);
		СоздатьКаталог(мСвязанныйКонфигуратор.ПапкаПередачиФайлов);
		СтруктураКоманды = Новый Структура("Команда, ИДПроцесса, ЕстьФайл", СокрЛП(Команда), ирКэш.ИдентификаторПроцессаОСЛкс(), Ложь);
		ТекстДок = Новый ТекстовыйДокумент; 
		Разделитель = ирОбщий.РазделительПутиКФайлуЛкс();
		Если ТипЗнч(НовыйТекстИлиФайл) = Тип("Строка") Тогда
			СтруктураКоманды.ЕстьФайл = Истина;
			ТекстДок.УстановитьТекст(НовыйТекстИлиФайл);
			ИмяФайла = мСвязанныйКонфигуратор.ПапкаПередачиФайлов + Разделитель + "НовыйТекст.txt";
			ирОбщий.СообщитьЛкс("Изменный объект отправлен через файл " + ИмяФайла,,,, Ложь, Истина);
			ТекстДок.Записать(ИмяФайла,, Символы.ПС); // - если оставить разделитель по умолчанию, то вставляется пустая строка на каждом ПС в запросе схемы компоновки
			ТекстДок.УстановитьТекст(СтарыйТекст);
			ТекстДок.Записать(мСвязанныйКонфигуратор.ПапкаПередачиФайлов + Разделитель + "СтарыйТекст.txt");
		ИначеЕсли ТипЗнч(НовыйТекстИлиФайл) = Тип("Файл") Тогда
			СтруктураКоманды.ЕстьФайл = Истина;
			ИмяФайла = мСвязанныйКонфигуратор.ПапкаПередачиФайлов + Разделитель + "НовыйФайл" + НовыйТекстИлиФайл.Расширение;
			ПереместитьФайл(НовыйТекстИлиФайл.ПолноеИмя, ИмяФайла);
			ирОбщий.СообщитьЛкс("Изменный объект отправлен через файл " + ИмяФайла,,,, Ложь, Истина);
		КонецЕсли;
		ТекстДок.УстановитьТекст(ирОбщий.ОбъектВТекстЖСОНЛкс(СтруктураКоманды));
		ТекстДок.Записать(мСвязанныйКонфигуратор.ПапкаПередачиФайлов + Разделитель + "Команда.txt");
		ирКлиент.ОтправитьНажатияКлавишВОкноЛкс(мСвязанныйКонфигуратор.ИД, "^{F12}");
	Иначе
		ирКлиент.ОтправитьНажатияКлавишВОкноЛкс(мСвязанныйКонфигуратор.ИД);
	КонецЕсли;
КонецПроцедуры
