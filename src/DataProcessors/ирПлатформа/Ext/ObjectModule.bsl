...
Перем мВызовыВсехМетодовПоМодулям;
				...
				Если МодульМетаданныхКомпонента <> Неопределено Тогда
					...
					Тогда
						...
						Если МодульМетаданныхБазоый <> Неопределено Тогда
							ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхБазоый, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст,
								ФлагиКомпиляции, ВнутриГруппыОбщихМодулей);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если ВнутриГруппыОбщихМодулей Тогда
				ИмяМетода = МетодМодуля.Имя + " - " + МетаданныеРодителя.Имя;
			КонецЕсли;
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено, Знач ТекущаяПозиция = Неопределено) Экспорт 
	...
	ВерсияФормата = 10; // Метка91237881041 Каждый раз при изменении структуры увеличиваем на 1
			...
			Если ОбновитьФайлОписания Тогда
				...
				Иначе 
					...
					Иначе
						...
						УдалитьФайлы(ФайлМодуляИзКраткогоИмени(выхИмяФайлаМодуляБезРасширения, "cin").ПолноеИмя);
						Если мВызовыВсехМетодовПоМодулям <> Неопределено Тогда
							мВызовыВсехМетодовПоМодулям[НРег(выхИмяФайлаМодуляБезРасширения)] = Неопределено;
						КонецЕсли;  
					КонецЕсли;
...
//    Файл - Файл - 
//    СтрокаВызова - СтрокаТаблицыЗначений - 
//    ПолеТекстаПрограммы - ОбработкаОбъект.ирКлсПолеТекстаПрограммы
// Возвращаемое значение:
//    ТаблицаЗначений - 
Функция ВызовыВсехМетодовПоМодулю(Знач Модуль, Знач ТекстовыйДокумент, Знач Файл, Знач ПолеТекстаПрограммы) Экспорт
	Если мВызовыВсехМетодовПоМодулям = Неопределено Тогда
		мВызовыВсехМетодовПоМодулям = Новый Соответствие;
	КонецЕсли;
	КлючКэша = НРег(Модуль);
	ВызовыВсехМетодов = мВызовыВсехМетодовПоМодулям[КлючКэша];
	Если ВызовыВсехМетодов = Неопределено Тогда
		ВерсияФорматаИндекса = 4;
		ШаблонВнешнегоВызова = "(?:[^"+шБуква+"\d\.]|^)(?:"+ПолеТекстаПрограммы.шИмяСТочками+"\.)("+шИмя+")\s*\((?:(?:"+ПолеТекстаПрограммы.шВыражениеПрограммы+")?"+шРазделитель+"*(?=[,\)])|,)*\)";
		ФайлИндекса = ФайлМодуляИзКраткогоИмени(Модуль, "cin");
		ОбновитьИндекс = Истина;
		
		// Теперь просто удаляем файл индекса при изменении текста модуля, поэтому нет смысла сравнивать даты изменения
		//Попытка   
		//	// Для ускорения НЕ проверяем существование файла
		//	ДатаИзмененияИндекса = ФайлИндекса.ПолучитьВремяИзменения();
		//Исключение
		//	ДатаИзмененияИндекса = Неопределено;
		//КонецПопытки;
		//ОбновитьИндекс = Ложь
		//	Или ДатаИзмененияИндекса = Неопределено
		//	Или Файл.ПолучитьВремяИзменения() > ДатаИзмененияИндекса;
		//
		...
		Если ОбновитьИндекс Тогда
			ВызовыВсехМетодов = ирОбщий.НайтиРегВыражениеЛкс(ТекстовыйДокумент.ПолучитьТекст(), ШаблонВнешнегоВызова,,,,,, Истина);
			ВызовыВсехМетодов.Колонки.Добавить("НИмяМетода", Новый ОписаниеТипов("Строка"));
			Для Каждого СтрокаВызова Из ВызовыВсехМетодов Цикл
				СтрокаВызова.НИмяМетода = НРег(СтрокаВызова.Подгруппы.SubMatches(0));
			КонецЦикла;
			ВызовыВсехМетодов.Колонки.Удалить("Подгруппы");
			ВызовыВсехМетодов.Колонки.Удалить("ТекстВхождения");
			ИндексВызововМодуля = Новый Структура;
			ИндексВызововМодуля.Вставить("ВерсияФормата", ВерсияФорматаИндекса);
			ИндексВызововМодуля.Вставить("Вызовы", ВызовыВсехМетодов); 
			ирОбщий.СохранитьЗначениеВФайлЛкс(ИндексВызововМодуля, ФайлИндекса.ПолноеИмя);
		КонецЕсли;
		ВызовыВсехМетодов = ИндексВызововМодуля.Вызовы;
		ирОбщий.ДобавитьИндексВТаблицуЛкс(ВызовыВсехМетодов, "НИмяМетода");
		мВызовыВсехМетодовПоМодулям[КлючКэша] = ВызовыВсехМетодов;
	КонецЕсли;
	Возврат ВызовыВсехМетодов;
КонецФункции
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Ложь) Экспорт 
	Если МодульМетаданных <> мПустойМодуль Тогда
		...
		Если ЧерезКопию Тогда
			...
			КопияМодуля.Методы.ЗаполнитьЗначения(Неопределено, "ТелоБезВозвратов"); 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Для Каждого Вхождение Из ВхожденияМетодов Цикл
		...
		Если Заголовок <> Неопределено Тогда
			...
			Тело = Сред(Тело, 2); // Сдвиг добавлен после вынужденного переноса символа "переноса строки" из заголовка в тело
			...
			СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияОпределения + Найти(ОпределениеБезОписания, МаркерНачалаТела) - 1; 
			...
			Если ЗначениеЗаполнено(ПоследняяАннотация) Тогда
				СтрокаМетода.РасширяемыйМетод = ирОбщий.ТекстМеждуМаркерамиЛкс(ПоследняяАннотация, "(""", """)");
			КонецЕсли;
...
Функция НоваяТаблицаМетодовМодуля() Экспорт 
	...
	Методы.Колонки.Добавить("РасширяемыйМетод");
	...
	Методы.Колонки.Добавить("ДлинаТела");
...
Функция ПодготовитьТипРезультатаМетода(МетодМодуля, ИмяИлиМодуль, ПолеТекстаМодуляКэш = Неопределено, Знач СФактическимиПараметрами = Ложь, Знач ДляСвойства = "") Экспорт 
	...
	ПолеТекстаМодуляКэш = ПодготовитьПолеТекстаДляСловаМодуля(ИмяИлиМодуль, ПолеТекстаМодуляКэш);
...
Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, Знач ПолеТекстаМодуляКэш, Знач МетодМодуля, Знач ИмяИлиМодуль, Знач СтрокаОпределения, Знач ОписаниеТипа = "",
	...
	Если СФактическимиПараметрами Тогда
		...
		ТаблицаТипов = ПолеТекстаМодуляКэш.ВычислитьТипЗначенияВыражения(ИмяПеременной,,,,,,, МетодМодуля = Неопределено,, Истина,,, ДопКлючКэша, ДляСвойства);
	Иначе 
		...
		Иначе
			...
			//Тогда
				ТаблицаТипов = ПолеТекстаМодуляКэш.ВычислитьТипЗначенияВыражения(ИмяПеременной,,,,,,, МетодМодуля = Неопределено,, Истина,,,, ДляСвойства);
				Если ТаблицаТипов.Количество() > 0 Тогда
					ДобавитьВТаблицуТипов(СтрокаОпределения.ТаблицаТипов, ТаблицаТипов); 
				Иначе
					СтруктураТипа = НоваяСтруктураТипа();
...
Функция ПодготовитьПолеТекстаДляСловаМодуля(Знач ИмяИлиМодуль, ПолеТекстаМодуляКэш = Неопределено) Экспорт
	Если ПолеТекстаМодуляКэш = Неопределено Тогда
		...
		Если Истина
			//И Не ЗначениеЗаполнено(ИмяМодуля) 
			И ТипЗнч(ИмяИлиМодуль) = Тип("Структура")
		Тогда
			ПолеТекстаМодуляКэш.мМодульМетаданных = ИмяИлиМодуль;
		КонецЕсли;
	КонецЕсли;
...
// Возвращаемое значение:
//    Булево - 
Функция ЛиДетальностьТиповДостаточна(ТаблицаТипов, Знач МинДетальность = 3, Знач ВыбратьЛучшийТип = Ложь, выхЛучшийТип = Неопределено) Экспорт
	Если ТаблицаТипов.Количество() > 0 Тогда
		Счетчик = 1;        
