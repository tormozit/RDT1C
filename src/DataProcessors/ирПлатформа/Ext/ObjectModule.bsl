...
Функция РазвернутаяТаблицаТиповИзСтруктурыТипа(Знач СтруктураТипа, Знач ЭтоЛокальныйКонтекст, Знач ДляФиксированныхСлов = Истина, Знач ДоступноКэширование = Ложь) Экспорт 
	...
	Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("БазовыйТип", СтруктураТипа.ИмяОбщегоТипа);
		Для Каждого СтрокаСлова Из ТаблицаОбщихТипов.НайтиСтроки(Отбор) Цикл
			НоваяСтрока = МассивОбщихТипов.Добавить();
			НоваяСтрока.ИмяОбщегоТипа = СтрокаСлова.Слово;
		КонецЦикла;
...
Процедура ОчиститьКэшАнализатораЯзыка() Экспорт 
	...
	мВызовыВсехСловПоМодулям = Неопределено;
					...
					Иначе
						...
						Тогда
							...
							Если МассивТипов.Количество() > МаксЧислоТиповДляАнализа Тогда 
								...
								Если Найти(СтруктураТипаСвойства.ИмяОбщегоТипа, "Ссылка.") Тогда
		...
		Тогда
			...
			Иначе
				...
				Если Истина
					...
					И МетаданныеРодителя <> Неопределено
					//И ТипЗнч(МетаданныеРодителя) <> Тип("Строка") // Если включено, то блокирует доступ к модулю свеже добавленного объекта в конфигураторе
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	Возврат 27; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
КонецФункции
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Истина) Экспорт 
...
// {ОписаниеРегулярногоВыражения.Конец}
	...
	Результат.Вставить("ОписаниеПараметра", ".*(?:\n(?://)?(?:[\t ]{" + МинДлинаОтступаВнутриОписанияПараметра() + ",}|[\t ]+[*-]).*)*");
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			...
			Иначе
				...
				Если Истина
					...
					И Сред(ТекстМодуля, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) = Сред(СтарыйМодуль.Текст, ПозицияНачалаСтарая, ДлинаСверяемойЧасти) 
				Тогда
				...
				Иначе
					...
					Модуль.ТаблицыСлов.Очистить(); // Могла измениться экспортность метода
					Модуль.ХешСигнатур = "";
...
Функция МинДлинаОтступаВнутриОписанияПараметра() Экспорт
	Возврат 6; /// 10 не хватает для ПодключаемыеКомандыПереопределяемый.ПриОпределенииКомандПодключенныхКОбъекту
КонецФункции
...
Функция ПараметрыМетодаИзКомментария(Знач ОписаниеМетода, Шаблоны = Неопределено) Экспорт
	...
	МаксДлинаОтступа = 8; /// Возможно стоит заменить на МинДлинаОтступаВнутриОписанияПараметра()
...
Функция ПредставлениеМассиваСтруктурТипов(ТаблицаТипов, БезДублей = Ложь, МаксКоличествоТипов = 10, Знач Подробное = Истина) Экспорт 
	...
	Если ТаблицаТипов <> Неопределено Тогда
		...
		Иначе
			...
			СписокИменТипов = ТаблицаТиповДляПользователя(ТаблицаТипов, БезДублей, МаксКоличествоТипов, СписокОбрезан, Подробное);
...
Функция ТаблицаТиповДляПользователя(Знач ТаблицаТипов, Знач БезДублей = Истина, Знач МаксКоличествоТипов = 0, выхСписокОбрезан = Ложь, Знач Подробное = Истина) Экспорт 
...
Функция ИмяТипаИзСтруктурыТипа(Знач СтруктураТипа, Знач Подробное = Истина) Экспорт
	...
	Тогда 
		КонкретныйТип = КонкретныйТип + " Из " + ИмяТипаИзСтруктурыТипа(ОбъектМД[0]); // Нужно для описания результата метода https://turboconf.ru/Tasks/9668
	//ИначеЕсли Истина
	//	И (Ложь
...
Функция НайтиИмяМетодаСтрокиМодуля(Знач ИмяМодуля, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулейКэш = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "",
	Знач ЧислоСтрокДляПоискаИмениМетода = 2000, выхСодержаниеСтроки = "") Экспорт 
	...
	ИмяМетода = НайтиИмяМетодаСтрокиФайлаМодуля(ОписаниеМодуля.ИмяМодуля, НомерСтроки, СодержаниеСтроки, ТекстыМодулейКэш, СтароеИмяФайлаМодуляБезРасширения, ЧислоСтрокДляПоискаИмениМетода,,, выхСодержаниеСтроки);
	Возврат ИмяМетода;
...
Функция НайтиМетодПоНомеруСтроки(Знач ПолеТекстаМодуля, Знач НомерСтроки, Знач ИмяМодуля = "", ТекстыМодулейКэш = Неопределено, Знач ЧислоСтрокДляПоискаИмениМетода = 2000, НужноеОписаниеМетода = Неопределено) Экспорт
...
Функция КлючСервисаИИ() Экспорт 
	Если ЛиНапарникВРолиОбщегоИИ() Тогда
...
Функция ЛиНапарникВРолиОбщегоИИ() Экспорт
	Возврат СерверИИ() = "https://code.1c.ai";
КонецФункции 
