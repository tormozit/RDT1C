					...
					Если КорневойТипРодителя <> Неопределено Тогда 
						...
						Тогда 
							...
							Если НРег(КонкретныйТип) = Нрег("ТабличнаяЧасть") Тогда
								...
								Если МассивФрагментов[1] = Метаданные().Имя Тогда 
									Если Метаданные().ТабличныеЧасти.Найти(МассивФрагментов[3]) <> Неопределено Тогда
										Образователь = ЭтотОбъект[МассивФрагментов[3]];
									КонецЕсли;
								ИначеЕсли Ложь
									Или МассивФрагментов[0] = "Обработка"
									Или МассивФрагментов[0] = "Отчет"
								Тогда
									Образователь = Новый (МассивФрагментов[0] + "Объект." + МассивФрагментов[1]); // Опасно. Можно быть долго. Могут быть ошибки на толстом клиенте.
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			ИначеЕсли Истина
				...
				И ТипЗнч(МетаданныеРодителя) = Тип("Структура")
			Тогда
				ИмитаторыЭлементов = ирОбщий.ОтобратьКоллекциюЛкс(РодительскаяСтруктураТипа.ДержательМетаданных.Элементы, "Э.Значение.ИмяРодителя = П1", "Э.Значение", МетаданныеРодителя.Имя);
				КоллекцияЗначений = Новый Структура;
					...
					Для Каждого Член Из ИнфоТипа.Members Цикл
						...
						Тогда 
							...
							Если ОписаниеЭлементаКоллекции <> Неопределено Тогда 
								...
								СтруктураТипа = НоваяСтруктураТипа();
								СтруктураТипа.ИмяОбщегоТипа = ПолучитьТипЗначенияЧленаИнтерфейса(ОписаниеЭлементаКоллекции.ReturnType, ИмяБиблиотеки, ИнфоТипа.Parent);
								СтруктураТипа.СтрокаОписания = Член;
								СтруктураТипа.Метаданные = МетаданныеРодителя;
								ТаблицаТипов = ДобавитьВТаблицуТипов(, СтруктураТипа);
								ДобавитьВТаблицуСлов(ТаблицаСловОтдельная, ИмяЧлена, ТипСловаЧленаКласса, ТаблицаТипов,, "Метаданные");
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ТаблицаСловПриемник = Неопределено,
	...
	Если Истина
		И Не ТолькоЭкспорт 
		И ФлагиКомпиляции <> Неопределено 
		И ЛиИмяМодуляФормы(МодульМетаданных.Имя)
	Тогда 
...
Функция ДобавитьВТаблицуСловЭлементМодуляМетаданных(Знач ТаблицаСлов, Знач Слово, Знач ВычислятьТипы, Знач ТаблицаИзКэша, Знач ТаблицаТипов, Знач ТипЗначения, Знач ТипСлова,
	...
	Если Слово <> "<>" Тогда
		...
		Тогда
			Если Найти(ТипЗначения, "см.") > 0 Тогда
			...
			Если МодульМетаданных <> Неопределено И СтарыйМодуль <> МодульМетаданных Тогда
				...
				Тогда
					...
					Для Каждого СтрокаСлова Из ТаблицаСлов Цикл // TODO добавить фильтр по типу 
						...
						//СтрокаПеременной.ТаблицаТипов = СтрокаСлова.ТаблицаТипов; // Нельзя опираться на эти типы, т.к. они очищаются чаще чем сюда приходим 
					КонецЦикла;
				КонецЕсли;
			КонецЕсли; 
		Иначе 
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	ВерсияФормата = 18; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
...
Функция ИмитаторУправляемойФормыИзДокументаДОМ(Знач ДокументDOM, Знач Форма = Неопределено) Экспорт
	...
	Для Счетчик = 0 По 8 Цикл
		...
		Пока УзелDOM <> Неопределено Цикл 
			...
			КонецЕсли;
			Попытка
				ИмяРодителя = УзелDOM.РодительскийУзел.РодительскийУзел.РодительскийУзел.ДочерниеУзлы[6].ПервыйДочерний.ЗначениеУзла;
			Исключение
	...
	Иначе 
		...
		Иначе  
			...
			Тогда
				...
				КонецЕсли;
			КонецЕсли;
			УказанКонструктор = Найти(НРег(СтрокаОпределения.ТипЗначения), "см.") > 0;
			Если Истина
