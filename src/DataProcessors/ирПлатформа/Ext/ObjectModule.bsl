...
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Если Найти(ИмяОбщегоТипаРодителяБезДеталей, " {") > 0 Тогда
		...
		КонецЕсли;
	КонецЕсли; 
	ЭтоНоваяТаблицаСлов = Неопределено;
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Тогда
			Продолжить;
		КонецЕсли;
		Если ЭтоНоваяТаблицаСлов = Неопределено Тогда
			ЭтоНоваяТаблицаСлов = ТаблицаСлов.Количество() = 0;
		КонецЕсли;
		МетаданныеРодителя = СтрокаОбщегоТипа.Метаданные;
		Если Истина
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если ВычислятьТипыЦикл Тогда
				...
				ИначеЕсли Истина                                    
					...
					И Не ирОбщий.СтрНачинаетсяСЛкс(ИмяОбщегоТипаРодителяБезДеталей, "КоллекцияКолонок")
					И Не (Истина
			...
			КонецЕсли;
			ДобавитьВТаблицуСлов(ТаблицаСлов, СловоЦикл, ТипСловаЦикл, ТаблицаТипов, ТипЗначения, "Предопределенный",,,, Не ЭтоНоваяТаблицаСлов);
		КонецЦикла;
...
Функция ДобавитьВТаблицуСлов(Знач ТаблицаСлов, Знач Слово, Знач ТипСлова = "Свойство", Знач ТаблицаТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "", Знач Вытеснять = Истина,
	Знач СтарыеТипы = Неопределено, Знач УстановитьНСлово = Ложь, Знач ЛиПолныйКонтрольУникальности = Ложь) Экспорт
...
Функция ОписаниеРасширенногоТипа(Знач РасширенныйТип) Экспорт
	Возврат ТаблицаРасширенийТипов.Найти(РасширенныйТип, "Расширение");
КонецФункции
