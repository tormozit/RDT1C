		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					ЗаполнитьЗначенияСвойств(СтруктураТипа, РодительскаяСтруктураТипа, , "ИмяОбщегоТипа, СтрокаОписания, Метаданные, ВиртуальнаяТаблица");
					Если Истина
					...
					Если КорневойТипРодителя <> Неопределено Тогда
						...
						Тогда
							...
							Прервать;
						ИначеЕсли Истина
							И СтрокаСлова.Слово = "ОборотыДтКт"
						...
						Тогда
							...
							Прервать;
						ИначеЕсли Истина
							И СтрокаСлова.Слово = "ФактическийПериодДействия"
						...
						Тогда
							...
							Прервать;
						ИначеЕсли Истина
							И СтрокаСлова.Слово = "ДанныеГрафика"
							И КорневойТипРодителя = "РегистрРасчета"
			...
			Тогда
				Если ТипЗнч(МетаданныеРодителя) = Тип("Структура") Тогда 
					...
					Если Истина
						И ЗначениеЗаполнено(Слово) 
						И МетаданныеРодителя.Свойство(Слово)
						И ЗначениеЗаполнено(МетаданныеРодителя[Слово])
						И ТипЗнч(МетаданныеРодителя[Слово][0].СтрокаОписания) = Тип("Структура")
					Тогда
						ОписаниеСвойства = МетаданныеРодителя[Слово][0].СтрокаОписания; // см. ОбработкаОбъект.ирКлсПолеТекстаПрограммы.НовоеОписаниеДинамическогоСвойства
						ОписаниеСвойства = ирОбщий.СкопироватьКоллекциюЛкс(ОписаниеСвойства);
					...
					Иначе
						...
						Тогда 
							...
							Если Истина
								...
								И СтрокаВида.Слово <> "<Имя перерасчета>"
								И СтрокаВида.Слово <> "База<Имя базового регистра расчета>"
								И ПостроительЗапросаРодителя <> Неопределено
							Тогда 
	...
	Тогда 
		...
		Для Каждого ТипЗначения Из МассивТиповЗначения Цикл
			...
			Если Истина
				...
				И ИмяОбщегоТипаРодителя = "Структура"
				И ТипЗнч(МетаданныеРодителя) = Тип("ТаблицаЗначений")
			Тогда
...
//    ТекстЗапроса - Строка - 
//    Параметры - Структура -  
Функция ДобавитьВременныеТаблицыПараметровВТекстЗапроса(Знач ТекстЗапроса, Знач Параметры, Знач ТекстыЗапросов = Неопределено) Экспорт
