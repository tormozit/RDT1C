...
Функция СвойстваТипаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	Знач Конфигурация = Неопределено, Знач ВычислятьТипы = Истина, Знач ЛиСерверныйКонтекст = Ложь, Знач ТаблицаСлов = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь) Экспорт 
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		ИмяОбщегоТипа = ирОбщий.ПервыйФрагментЛкс(СтрокаОбщегоТипа.ИмяОбщегоТипа, "["); 
		ИспользоватьКэширование = Истина
			И (Ложь
				Или ИмяОбщегоТипа = "Глобальный"
				Или ИмяОбщегоТипа = "")
			И Слово = Неопределено;
		Если ИспользоватьКэширование Тогда
			КлючКэширования = "" + ЯзыкПрограммы + ";" + Конфигурация + ";" + ВычислятьТипы + ";" + ЛиСерверныйКонтекст + ";" + РодительскаяСтруктураТипа.ТипЯзыка;
...
Функция СвойстваТипаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач _ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	Знач ТекущийИндекс = Неопределено, Знач ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина, Знач ЛиСерверныйКонтекст = Ложь, Знач МодульМетаданныхБазовый = Неопределено,
	Знач ТаблицаСлов = Неопределено, Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ЭтоЛокальныйКонтекст = Ложь) Экспорт 
	
	Если ТаблицаСлов = Неопределено Тогда
		ТаблицаСлов = НоваяТаблицаСвойствТипа();
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РодительскаяСтруктураТипа.ИмяОбщегоТипа) Тогда
		// родительский контекст является Процедурой
		Возврат ТаблицаСлов;
	КонецЕсли;
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			Тогда
				МассивИмен = ИменаРеквизитовФормы(МетаданныеРодителя).Все;
				КоллекцияЗначений = Новый Структура;
			...
			Тогда
				...
				Если ТипЗнч(МетаданныеРодителя) = Тип("КоллекцияОбъектовМетаданных") Тогда
					//
				Иначе
					ИмяКоллекцииОбъектовМетаданных = ирОбщий.ТекстМеждуМаркерамиЛкс(ИмяОбщегоТипа, МаркерКоллекцииОбъектовМетаданных + ": ", , Ложь);
		...
		Если Ложь
			...
			Или ЯзыкПрограммы = 2
		Тогда
			МодульМетаданныхКомпонента = МодульМетаданныхБазовый;
			...
			Если МодульМетаданныхКомпонента = Неопределено Тогда
				СтруктураТипа = НоваяСтруктураТипа(ИмяОбщегоТипа);
...
Функция ТаблицаТиповЭлементовКоллекции(Знач РодительскаяСтруктураТипа, Знач Слово = "", Знач ЯзыкПрограммы = 0) Экспорт 
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено) Экспорт 
	...
	выхИмяФайлаМодуляБезРасширения = ИмяФайлаМодуляБезРасширения(СтруктураТипа, ТипМодуля, СтрокаСозданияРодителя);
	Если ЗначениеЗаполнено(выхИмяФайлаМодуляБезРасширения) Тогда
		...
		КонецЕсли; 
		МодульМетаданных = МодулиМетаданных[НРег(выхИмяФайлаМодуляБезРасширения)];
		Если Ложь
		...
		Тогда
			...
			Если НовыйТекст <> Неопределено Тогда
				...
				КонецЕсли;     
			КонецЕсли;
			ФайлОписанияМодуля = Новый Файл(ПапкаКэшаМодулей.ПолноеИмя + ирОбщий.РазделительПутиКФайлуЛкс() + выхИмяФайлаМодуляБезРасширения + ".ixd");
			...
			Если СтарыйТекст = НовыйТекст И ФайлОписанияМодуля.Существует() Тогда
				...
				Если Не ОбновитьФайлОписания Тогда 
					...
					Если Ложь
						...
						Или МодульМетаданных.Методы.Колонки.Найти("Вычислено") = Неопределено // Проверка версии формата кэша
					Тогда
						// Старый формат кэша
						ОбновитьФайлОписания = Истина;
			...
			Если ОбновитьФайлОписания Тогда
				...
				Иначе 
					...
					Иначе
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, ФайлМодуля.ИмяБезРасширения);
			...
			Если МодульМетаданных <> Неопределено Тогда
				...
				КонецЕсли;
			КонецЕсли; 
			МодулиМетаданных[НРег(выхИмяФайлаМодуляБезРасширения)] = МодульМетаданных;
		КонецЕсли; 
	КонецЕсли;
...
// Возвращаемое значение:
//    Файл - 
Функция ФайлМодуляИзКраткогоИмени(выхИмяФайлаМодуляБезРасширения) Экспорт
	ФайлМодуля = Новый Файл(ПапкаКэшаМодулей.ПолноеИмя + ирОбщий.РазделительПутиКФайлуЛкс() + выхИмяФайлаМодуляБезРасширения + ".txt");
	Возврат ФайлМодуля;
КонецФункции

Функция ИмяФайлаМодуляБезРасширения(Знач СтруктураТипа, ТипМодуля = "", СтрокаСозданияРодителя = "") Экспорт 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "") Экспорт
	...
	Если СтруктураТипа <> Неопределено И СтруктураТипа.ИмяОбщегоТипа <> "Глобальный" Тогда
		// TODO Возможно уже не нужно
		// Добавим реквизиты объекта в переменные
		...
		Если Вхождение.SubMatches(0) <> Неопределено Тогда
			...
			Для Каждого Вхождение2 Из Результат2 Цикл
				Если Вхождение2.SubMatches(0) <> Неопределено Тогда
					...
					СтрокаПеременной.ПозицияСОписанием = Вхождение.FirstIndex + 2;
	...
	Методы.Колонки.Добавить("ПозицияОпределения"); // Начиная с 1. Может начинаться с пустой строки и содержать непечатное смещение от начала строки
	...
	Для Каждого Вхождение Из Вхождения Цикл
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			...
			//СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияОпределения + СтрДлина(Вхождение.SubMatches(1)) + 1;  // +1 добавлено после вынужденного переноса первого символа "переноса строки" из заголовка в тело
			////ПервыйСимволОпределения = Лев(ОпределениеБезОписания, 1);
			//Если Ложь
			//	Или ЗначениеЗаполнено(ОписаниеМетода) 
			//	Или СтрокаМетода.ПозицияОпределения = 1
			//Тогда
			//	// Грязно
			//	СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияТела - 1; 
			//КонецЕсли;
			////ПустышкаФФФФФФФФ = Сред(ТекстМодуля, СтрокаМетода.ПозицияТела - 10, 10 +1);
			//
			...
			Если ТипМетода = "функция" Тогда
				...
				Если ЗначениеЗаполнено(СтрокаМетода.Описание) Тогда
					ВхожденияОписаний = ирОбщий.НайтиРегВыражениеЛкс(ОписаниеМетода, ШаблонРезультатаФункции, "ТипЗначения, Описание", Ложь,, Ложь, Ложь);
					Если ВхожденияОписаний.Количество() > 0 Тогда
						...
						Если ЗначениеЗаполнено(ВхождениеОписания.ТипЗначения) Тогда
							СтрокаМетода.ТипЗначения = СтрокаМетода.ТипЗначения + ", " + СокрЛ(СтрЗаменить(ВхождениеОписания.ТипЗначения, "//", "")); 
						КонецЕсли;
...
// Возвращаемое значение:
//  ТаблицаЗначений - 
Функция ПараметрыМетодаМодуля(СтрокаМетода) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		...
		Иначе
			...
			СтрокаМетода.ОписаниеРезультата = ОчиститьТекстМодуляОтПрефиксаКомментария(СтрокаМетода.ОписаниеРезультата);
			Шаблоны = ШаблоныДляАнализаВстроенногоЯзыка();
			RegExp2.Global = Истина;
...
Функция ОчиститьТекстМодуляОтПрефиксаКомментария(Знач ТекстМодуля)
	...
	Если ТекстМодуля = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	RegExp.Pattern = шПустоеНачалоСтроки + "//"; 
	Результат = Сред(RegExp.Заменить(ТекстМодуля, Символы.ПС), 2); // Отрезаем первый перенос строки
	Возврат Результат;

КонецФункции

Функция ЗаменитьВозвратыНаПерейтиВТекстеМетода(Знач Текст, Знач ИмяПеременнойРезультата = "Результат", Знач ДобавлятьПереносыСтрок = Ложь) Экспорт
	...
	Иначе
		Разделитель = " ";
	КонецЕсли;
	Текст = РегВыражение.Заменить(Текст, "$1" + ИмяПеременнойРезультата + " = $2;" + Разделитель + "Перейти ~Конец$3");
	РегВыражение.Pattern = "((?:^|\n|\r)(?:\t| )*)Возврат(?:\t| )*(\n|\r|;)";
	Текст = РегВыражение.Заменить(Текст, "$1Перейти ~Конец$2");
	//РегВыражение.Pattern = "((?:^|\n|\r)(?:\t| )*)Перейти\s(?:\t| )*~[^\r;]+(\n|\r|;)";
	//Текст = РегВыражение.Заменить(Текст, "$1Перейти ~Конец$2");
...
Процедура ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач МодульМетаданных, СтрокаОпределения, Знач ТекстДляПоискаОпределения)
	...
	Если кэшКомпонентаПодсказки = Неопределено Тогда
		кэшКомпонентаПодсказки = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
		#Если Сервер И Не Сервер Тогда
...
Функция ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	Знач _РазрешитьСозданиеОбъектаИзТипа = Истина, ТипСлова = Неопределено, ЛиСерверныйКонтекст = Ложь, Знач Слово = Неопределено, Знач МодульМетаданных = Неопределено,
	Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ТаблицаСвойств = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь) Экспорт 

	ИнициацияОписанияМетодовИСвойств();
	ТаблицаСвойств = СвойстваТипаПредопределенные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ЛиСерверныйКонтекст, ТаблицаСвойств, ЭтоЛокальныйКонтекст);
...
Функция ДеревоОбъектовМД() Экспорт
	...
	мТаблицаВсехТаблицБД = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
	#Если Сервер И Не Сервер Тогда
...
Функция НайтиИмяМетодаСтрокиФайлаМодуля(Знач ИмяФайлаМодуляБезРасширения, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулей = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "", Знач ЧислоСтрокДляПоискаИмениМетода = 1000) Экспорт
	
	Если ТекстыМодулей = Неопределено Тогда
		ТекстыМодулей = Новый Соответствие;
	КонецЕсли;
	КэшМодуля = ТекстыМодулей[ИмяФайлаМодуляБезРасширения];
	Если СтароеИмяФайлаМодуляБезРасширения <> ИмяФайлаМодуляБезРасширения Тогда
		...
		Если КэшМодуля = Неопределено Тогда
			...
			ТекстыМодулей[ИмяФайлаМодуляБезРасширения] = КэшМодуля;
			Методы = Неопределено;
		КонецЕсли;
		СлужебноеПолеТекста.УстановитьТекст(ТекстМодуля.ПолучитьТекст());
		СтароеИмяФайлаМодуляБезРасширения = ИмяФайлаМодуляБезРасширения;
	КонецЕсли;
	Если Истина
		И КэшМодуля.Текст <> Неопределено
		И (Ложь
			Или СодержаниеСтроки = Неопределено
			Или СокрЛП(КэшМодуля.Текст.ПолучитьСтроку(НомерСтроки)) = СодержаниеСтроки) 
	Тогда 
...
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM) Экспорт
	
	Массив = Новый Массив;
	Если ТипЗнч(ФормаИлиДокументDOM) = Тип("УправляемаяФорма") Тогда
		ИмяОсновногоРеквизита = "";
		ЧастыеИменаРеквизитов = ирОбщий.ЧастыеИменаРеквизитовОбъектаВФормеЛкс();  
		Для Каждого ИмяРеквизита Из ЧастыеИменаРеквизитов Цикл
			Попытка
				Пустышка = ФормаИлиДокументDOM[ИмяРеквизита];
			Исключение
				Продолжить;
			КонецПопытки;
			Массив.Добавить(ИмяРеквизита);
		КонецЦикла;
		// Только ненадежный перебор элементов
		Для Каждого ЭлементФормы Из ФормаИлиДокументDOM.Элементы Цикл
			...
			Если Истина
				...
				И Массив.Найти(ИмяРеквизита) = Неопределено 
			Тогда
				Массив.Добавить(ИмяРеквизита);
			КонецЕсли;
		КонецЦикла;
...
//шСтрокаПрограммы = """(?:""""|[^""\n])*(?:" + шРазделитель + "*\|(?:""""|[^""\n])*)*(?:""|$)"; // 
шСтрокаПрограммы = """(?:""""|[^""\n])*(?:(?:\n|^)(?:\t| )*(?:" + шКомментарий + "|\|(?:""""|[^""\n])*)|\s|\xa0)*(?:""|\n|$)"; // Так быстрее (меньше откатов) 14.08.2023
