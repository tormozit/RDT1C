...
Процедура ИнициализацияОписанияОбщихТипов() Экспорт
	...
	ТаблицаТиповМетаданных = МассивТаблиц.ТаблицаТиповМетаданных;
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если Не ЛиКэшАгрегатный Тогда
			...
			Если ИспользоватьКэширование Тогда
				Если ИмяОбщегоТипаРодителяБезДеталей = "ОбъектМетаданных" Тогда
					ЛиКэшАгрегатный = Истина;
					КлючКэширования = ИмяОбщегоТипаРодителяБезДеталей;
				Иначе
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Тогда
						...
						Прервать;
					ИначеЕсли Истина
						И РодительскаяСтруктураТипа.ИмяОбщегоТипа = "ЭлементыФормы"
						И СтрокаСлова.ТипСлова = "Метод"
						И (Ложь
		...
		КонецЦикла;
		Если ИспользоватьКэширование И Не ЛиКэшАгрегатный Тогда
		...
		КонецЕсли; 
	КонецЦикла;
	Если ИспользоватьКэширование И ЛиКэшАгрегатный Тогда
		ДобавитьТаблицуСловВГлобальныйКэш(ТаблицаСлов, КлючКэширования, НаборыСлов);
	КонецЕсли;
	Возврат ТаблицаСлов;
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			Тогда
				Если ТипЗнч(МетаданныеРодителя) = Тип("ДанныеФормыСтруктура") Тогда
					...
					ТаблицаПараметровФормы = ТаблицаСловИзСтруктурыТипа(СтруктураТипаФормы,,,,,, "Параметр");
					...
					Если СлужебныеДанные <> Неопределено И СлужебныеДанные.Свойство("Параметры") Тогда
						ирОбщий.СкопироватьКоллекциюЛкс(СлужебныеДанные.Параметры, СхемаКоллекции);
				...
				ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("Строка") Тогда
					ТаблицаПараметровФормы = ТаблицаСловИзСтруктурыТипа(НоваяСтруктураТипа(МетаданныеРодителя),,,,,, "Параметр");
					Для Каждого СтрокаПараметра Из ТаблицаПараметровФормы Цикл
		...
		Тогда
			...
			КонецЕсли;
			#Если Клиент Тогда
				СловаКонтекстаМетаданныеCOM(Слово, ТипСлова, ИмяОбщегоТипаРодителя, МетаданныеРодителя, ТаблицаСлов);
			#КонецЕсли
		КонецЕсли; 
	КонецЦикла;
	Если Истина
...
Процедура СловаКонтекстаМетаданныеCOM(Знач Слово, Знач ТипСлова, Знач ИмяОбщегоТипаРодителя, Знач МетаданныеРодителя, Знач ТаблицаСлов) Экспорт
	Если Истина
		И (Ложь
		...
		Если ИнфоТипа <> Неопределено Тогда
			...
			Для Каждого ИнфоТипа Из МассивИнфоТипа Цикл
				Для Каждого Член Из ИнфоТипа.Members Цикл
					...
					КонецЕсли;
					Если Ложь
						Или Слово = Неопределено 
						Или Истина
						И ирОбщий.СтрокиРавныЛкс(Слово, ИмяЧлена)
						И ирОбщий.СтрокиРавныЛкс(ТипСлова, ТипСловаЧленаКласса) 
					Тогда
						СтруктураТипа = НоваяСтруктураТипа();
						...
						СтруктураТипа.Метаданные = МетаданныеРодителя;
						ТаблицаТипов = ДобавитьВТаблицуТипов(, СтруктураТипа);
						ДобавитьВТаблицуСлов(ТаблицаСлов, ИмяЧлена, ТипСловаЧленаКласса, ТаблицаТипов,, "Метаданные");
					КонецЕсли; 
					Если Истина    
			...
			Для Каждого БазовыйТипЭлемента Из ТипыЭлементовКоллекции Цикл
				...
				Если ТипЗнч(МетаданныеКоллекции) = Тип("ПостроительЗапроса") Тогда 
					...
					Если ИндексВПакете <> Неопределено И ЗначениеЗаполнено(МетаданныеКоллекции.Текст) Тогда
						...
						#КонецЕсли
						ПостроительЗапроса = ПостроительПакетаЗапросовДоИндекса(ТекстыЗапросов, ИндексВПакете, МетаданныеКоллекции.Параметры);
...
//   - 
//
Функция ПостроительПакетаЗапросовДоИндекса(Знач ТекстыЗапросов, Знач ИндексЭлемента, Знач Параметры, Знач ИмяРождаемойТаблицы = "") Экспорт 
	ТекстЗапроса = "";
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ТаблицаСловПриемник = Неопределено,
	...
	Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ДляСвойства = "", Знач ЛиВместеСЛокальнымКонтекстом = Ложь) Экспорт 
	...
	Тогда
		...
		Тогда
			Если ТаблицаИзКэша <> Неопределено Тогда
				Если ТаблицаИзКэша.Количество() > ТаблицаСловПриемник.Количество() Тогда
					ТаблицаИзКэша = ТаблицаИзКэша.Скопировать();
					ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаСловПриемник, ТаблицаИзКэша);
					ТаблицаСловПриемник = ТаблицаИзКэша;
				Иначе
					ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаИзКэша, ТаблицаСловПриемник);
				КонецЕсли;
	...
	Тогда
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			...
			КонецЕсли;
			ДобавитьВТаблицуСловЭлементМодуляМетаданных(ТаблицаСлов, ПеременнаяМодуля.Имя, ВычислятьТипы, ТаблицаИзКэша, ТаблицаТипов, ПеременнаяМодуля.ТипЗначения, ТипСловаЦикл);
		КонецЦикла;
	КонецЕсли; 
	Если Истина
		И Не ТолькоЭкспорт 
	...
	Если КлючКэша <> Неопределено Тогда
		...
		Если ТаблицаСловПриемник.Количество() > ТаблицаСлов.Количество() Тогда
			ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаСлов, ТаблицаСловПриемник);
		Иначе
			ТаблицаСлов = ТаблицаСлов.Скопировать();
			ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаСловПриемник, ТаблицаСлов);
...
Функция ДобавитьВТаблицуСловЭлементМодуляМетаданных(Знач ТаблицаСлов, Знач Слово, Знач ВычислятьТипы, Знач ТаблицаИзКэша, Знач ТаблицаТипов, Знач ТипЗначения, Знач ТипСлова) Экспорт
	Если Слово <> "<>" Тогда
		Если Истина
			И ВычислятьТипы 
		...
		КонецЕсли;
	КонецЕсли;
	ДобавитьВТаблицуСлов(ТаблицаСлов, Слово, ТипСлова, ТаблицаТипов, ТипЗначения, "Метаданные");
	Возврат ТипЗначения;
КонецФункции
...
Функция НовыйИндексПоискаВызововСловаВМодуле(Знач Модуль = Неопределено, Знач ВерсияФорматаИндекса = 0) Экспорт
	...
	ЛиИскатьОповещения = Истина
		...
		И ирОбщий.ЛиИмяТипаФормыЛкс(Модуль.СтруктураТипа.ИмяОбщегоТипа);
...
Функция НоваяТаблицаМетодовМодуля() Экспорт 
	...
	// При добавлении колонок нужно увеличивать версию формата Мультиметка91237881041
			...
			Иначе
				...
				Если ТаблицаТипов.Колонки.Метаданные.Заголовок = "" Тогда
					...
					Тогда
						...
						ТаблицаТиповИзКомментария.Колонки.Метаданные.Заголовок = "";
						ДобавитьВТаблицуТипов(СтрокаОпределения.ТаблицаТипов, ТаблицаТиповИзКомментария);
					КонецЕсли;
					ТаблицаТипов = СтрокаОпределения.ТаблицаТипов; 
				КонецЕсли;
...
Функция ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	Знач ВычислятьТипыМетодовМодулей = Неопределено, Знач ТаблицаСвойств = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь, Знач НаборыСлов = Неопределено, Знач БазовоеРасширениеКонфигурации = "") Экспорт 
	...
	Если Не ВнутриГруппыОбщихМодулей Тогда
		ТаблицаСвойств = СловаКонтекстаПредопределенные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ФлагиКомпиляции, ТаблицаСвойств, ЭтоЛокальныйКонтекст, НаборыСлов,
...
Функция ИмяТипаРасширенияЭлементаФормы(БазовоеИмяОбщегоТипа, ЭлементИлиФорма = Неопределено) Экспорт
	...
	Тогда 
		Если ирОбщий.ЛиФормаИлиИмитаторЛкс(ЭлементИлиФорма) Тогда
			Пустышка = 0; // TODO Приходим союда при вводе имени события в ДинамическоеПолеФормы.Установитьдействие(" 
		Иначе
...
Функция СтруктураТипаОбъектаОбычнойФормы(Знач Форма) Экспорт
	...
	Если Не СлужебныеДанные.Свойство("СтруктураТипаОбъекта", СтруктураТипаОбъекта) Тогда 
		ОсновнойРеквизитФормы = ИмяОсновногоРеквизитаФормы(Форма);
		Если ЗначениеЗаполнено(ОсновнойРеквизитФормы) Тогда
			СтруктураТипаОбъекта = СтруктураТипаИзЗначения(Форма[ОсновнойРеквизитФормы]);
...
шИндекс = "(\[[^\]\[]+?(?:(?:\[[^\]]+?\][^\]\[]*?)*)*\])";
шЛитералПростой = """[^""]*""";
//шЛитералПростой = шЛитералПрограммы; // Так будут зависания при анализе Мультиметка59234971
шМеждуСкобок = "[^\(\);""]";
