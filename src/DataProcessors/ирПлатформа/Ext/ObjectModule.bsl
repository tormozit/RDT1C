...
Функция СоздаваемыеВременныеТаблицыПакетаЗапросов(Знач ТекстПакетаИлиМассивТекстовЗапросов, Знач ТолькоТребующиеУничтоженияНаВходе = Ложь, Знач ВместеСУничтожениями = Ложь,
	Знач ВместеСДополнениями = Ложь) Экспорт 
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Тогда         
						...
						ТаблицаТиповИзТекста[0].Метаданные.ЗаполнитьЗначения(МетаданныеРодителя, "Метаданные");
...
Функция ОбновитьКэшСтруктурыФормыИзБуфера(Знач СлужебныеДанные, Знач ТекстБуфераОбмена, Знач ТекстМодуляФормы = "", Знач Форма = Неопределено) Экспорт 
	...
	Если Найти(СлужебныеДанные.ИмяФормы, ":") > 0 Тогда
		...
		Если Не ЗначениеЗаполнено(СтрокаРеквизитаОбъект.ОписаниеТипов) Тогда
			ВнешнийОбъект = ирОбщий.МенеджерВнешнегоОбъектаЛкс(ирОбщий.СтрРазделитьЛкс(СлужебныеДанные.ИмяФормы, "::")[1]);
			...
			Если РезультатПараметров.Количество() > 0 Тогда
				...
				Для Каждого ВхождениеПараметра Из РезультатПараметров Цикл
					...
					Если ПозицияТире > 0 Тогда
						СтрокаПраметра.ТипЗначения = Лев(ОписаниеПараметра, ПозицияТире - 1);
						СтрокаПраметра.Описание = Сред(ОписаниеПараметра, ПозицияТире + 1);
					Иначе
						СтрокаПраметра.ТипЗначения = ОписаниеПараметра;
					КонецЕсли;
				КонецЦикла;
				Если ЗначениеЗаполнено(СтрокаМетода.Описание) Тогда
					...
					Если Не ОписанияПараметровЗагружены Тогда
						...
						ВхожденияОписаний = ирОбщий.НайтиРегВыражениеЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаМетода.Описание + Символы.ПС, Анализатор.ЗаголовокРазделаВызывающиеМодули(), ":", Ложь,,, Истина),
...
Процедура ПодготовитьОписаниеРезультатаМетода(СтрокаОпределения) Экспорт
	...
	Если СыроеОписание <> Неопределено Тогда
		...
		Если ВхожденияОписаний.Количество() > 0 Тогда
			...
			Если Не ЗначениеЗаполнено(ТекстТипаЗначения) Тогда
				ТекстТипаЗначения = СокрЛП(ирОбщий.ТекстМеждуМаркерамиЛкс(ОписаниеРезультата, "-", Символы.ПС));
			КонецЕсли; 
...
Функция ИмяТипаИзСтруктурыТипа(Знач СтруктураТипа, Знач Подробное = Истина) Экспорт
	...
	ИначеЕсли Истина
		...
		И СтруктураТипа.Метаданные.Колонки.Найти(ИмяКолонкиФлагаВнутриТаблицы()) <> Неопределено
