...
Функция ОбновитьКэшСтруктурыФормыИзБуфера(Знач СлужебныеДанные, Знач ТекстБуфераОбмена, Знач Форма = Неопределено) Экспорт 
	...
	Попытка                          
		...
		КонецЕсли;
		ТекстМодуля = ПолеТекстаМодуля(Форма.ИмяФормы + ".Форма.Модуль").мОригинальныйТекст;
		Если СтрДлина(СокрЛП(ТекстМодуля)) > 300 Тогда
			ТекстФайла = СтрЗаменить(ТекстФайла, ирОбщий.ТекстВВыражениеВстроенногоЯзыкаЛкс(ТекстМодуля, Ложь, Ложь), "");
		КонецЕсли;
	...
	Иначе 
		...
		Иначе  
			...
			Если Истина
				...
				И Не ЛиНестрогаяТаблицаТипов(ТаблицаТипов, Ложь, МетодМодуля.ТипЗначения) 
			Тогда
