...
Перем WScriptShell;
Перем RegExp Экспорт; // ОбработкаОбъект.ирОболочкаРегВыражение 
Перем RegExp2 Экспорт; // ОбработкаОбъект.ирОболочкаРегВыражение
...
Функция РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(Знач ТекстПакета, Знач ПозицияКурсора = Неопределено, выхПозиции0 = Неопределено) Экспорт 
	...
	RegExp.Global = Истина;
	RegExp.Multiline = Ложь;
	//RegExp.Pattern = "(?:" + шЛитералВЗапросе + ")|" + шКомментарий + "|;(?:\s*\n)*|$";
	RegExp.Pattern = "[^^](?:" + шЛитералВЗапросе + ")|" + шКомментарий + "|;|$";
...
Функция СтруктураРезультатаПакетногоЗапроса(Знач ТекстПакетаИлиМассивТекстовЗапросов, Знач ПрефиксКомментария = "//") Экспорт 
	...
	RegExp.Global = Ложь;
	RegExp.Multiline = Ложь;
	RegExp.Pattern = ирОбщий.ТекстДляРегВыраженияЛкс(ПрефиксКомментария + мМаркерИмениЗапросаПакета) + "(" + шИмя + ")(?: |\n|\r)";
	Индекс = -1;
...
Функция СоздаваемыеВременныеТаблицыПакетаЗапросов(Знач ТекстПакетаИлиМассивТекстовЗапросов, Знач ТолькоТребующиеУничтоженияНаВходе = Ложь, Знач ВместеСУничтожениями = Ложь) Экспорт 
	...
	ШаблонПОМЕСТИТЬИЗ = "(?:(?:" + шРазделитель + "|\|)(?:(ПОМЕСТИТЬ|INTO\s+TABLE|INTO)|(УНИЧТОЖИТЬ|DROP|DROP\s+TABLE))(?:" + шРазделитель + "|\|)" 
...
// Возвращаемое значение:
//
Функция ТаблицаОбщихТиповИзСтруктурыТипа(Знач СтруктураТипа, Знач ЭтоЛокальныйКонтекст, Знач ДляФиксированныхСлов = Истина) Экспорт 
...
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	Знач Конфигурация = Неопределено, Знач ВычислятьТипы = Истина, Знач ФлагиКомпиляции = Неопределено, Знач ТаблицаСлов = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь, Знач НаборыСлов = Неопределено,
	Знач БазовоеРасширениеКонфигурации = "", КлючКэширования = Неопределено) Экспорт 
	...
	Если Найти(ИмяОбщегоТипаРодителяБезДеталей, " {") > 0 Тогда
		...
		КонецЕсли;
	КонецЕсли; 
	МассивОбщихТипов = ТаблицаОбщихТиповИзСтруктурыТипа(РодительскаяСтруктураТипа, ЭтоЛокальныйКонтекст, Истина);
	#Если Сервер И Не Сервер Тогда
	...
	ПроверятьДоступностьВКонтекстеКомпиляции = ПроверятьДоступностьВКонтекстеКомпиляции(ФлагиКомпиляции, ИмяОбщегоТипаРодителяБезДеталей, ЭтоЛокальныйКонтекст);
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если Ложь
			...
			Или ИмяОбщегоТипаРодителяБезДеталей = "ОбъектМетаданных" И РодительскаяСтруктураТипа.ТипЯзыка = ""
		...
		Тогда
			...
			ИспользоватьКэширование = Слово = Неопределено;
			Если ИспользоватьКэширование И ТаблицаСлов <> Неопределено Тогда
				...
				Иначе
					...
					ВызватьИсключение "Для кэшируемого контекста нельзя передавать таблицу-приемник";
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если ИспользоватьКэширование Тогда
			...
			Тогда
				...
				КлючКэширования = ИмяОбщегоТипаРодителяБезДеталей + ";" + ЯзыкПрограммы + ";" + Конфигурация + ";" + ВычислятьТипы + ";" + ТипСлова + ";" + НаСервере + ";" + КлиентОбычноеПриложение + ";" 
					+ КлиентУправляемоеПриложение + ";" + РодительскаяСтруктураТипа.ТипЯзыка + ";" + БазовоеРасширениеКонфигурации + ";" + РодительскаяСтруктураТипа.Конструктор;
			Иначе
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			Если Ложь
				...
				Или СтрокаСлова.НомерВерсииПлатформы > НомерВерсииПлатформы
				Или (Истина
			...
			Если РодительскаяСтруктураТипа.Конструктор Тогда 
				...
				КонецЕсли;
			КонецЕсли; 
			МожноУточнитьТип = СтрокаСлова.Слово = "ВладелецФормы";
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Если Истина
						...
						И ТипЗнч(МетаданныеРодителя) <> Тип("Структура")
					Тогда
						СтруктураТипа.Метаданные = Новый Структура("Тип, ПутьКДанным, Значение");
						СтруктураТипа.Метаданные.Тип = Тип(ИмяОбщегоТипаРодителяБезДеталей);
						Если ИмяОбщегоТипаРодителяБезДеталей <> РодительскаяСтруктураТипа.ИмяОбщегоТипа Тогда
							СтруктураТипа.Метаданные.Вставить("ИмяТипаРасширения", РодительскаяСтруктураТипа.ИмяОбщегоТипа);
						КонецЕсли;
					ИначеЕсли Истина
						И ТипСлова = "Метод"
					...
					Тогда
						...
						ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданныхКонфигурация") Тогда
							СоздатьИмитатор = Истина;
						КонецЕсли;
						Если СоздатьИмитатор Тогда
							СтруктураТипа.Метаданные = Новый Структура("Тип, ПутьКДанным, Значение", РодительскаяСтруктураТипа.ИмяОбщегоТипа);
						Иначе
					...
					ИначеЕсли Истина                                    
						...
						И СтрокаСлова.Слово <> "ВладелецФормы"
						...
						И ИмяОбщегоТипаРодителяБезДеталей <> "Структура" // Исключаем общее описание значений элементов структуры (Метаданные = ТаблицаЗначений)
					Тогда
					...
					ИначеЕсли ИмяОбщегоТипаСлова = "Отбор" Тогда 
						...
						ИначеЕсли СтрокаОбщегоТипа.ИмяОбщегоТипа = "РасширениеТабличногоПоляТабличнойЧасти" Тогда
					...
					Тогда
						...
						Если ЗначениеСвойства <> Неопределено Тогда
							Если Истина 
								...
								И ЗначениеСвойства.Колонки.Найти(ИмяКолонкиФлагаВнутриТаблицы()) <> Неопределено
							Тогда 
								// Например КлючИЗначение, ЭлементСпискаЗначений
					...
					ИначеЕсли Истина
						...
						И СтрокаСлова.Слово = "СхемаКомпоновкиДанных"
						И СтрокаСлова.ТипСлова = "Свойство"
					...
					ИначеЕсли Истина
						...
						И СтрокаСлова.Слово = "ЭкспортМоделиXDTO"
						И СтрокаСлова.ТипСлова = "Метод"
					Тогда
					...
					ИначеЕсли Истина
						...
						И СтрокаСлова.ТипСлова = "Метод"
						И (Ложь
							Или СтрокаСлова.Слово = "Добавить"
							Или СтрокаСлова.Слово = "Получить"
							Или СтрокаСлова.Слово = "Найти")
					Тогда
					...
					Тогда
						...
						Прервать;
					ИначеЕсли Ложь
						Или СтрокаСлова.Слово = "ЭтотОбъект"
						Или СтрокаСлова.Слово = "ЭтаФорма"
					Тогда
						ЛиДобавляемСлово = Истина;
						Если ирОбщий.ЛиИмяТипаФормыЛкс(ИмяОбщегоТипаРодителяБезДеталей, СтрокаСлова.Слово = "ЭтаФорма", Истина) Тогда
					...
					Тогда 
						...
						Если ДанныеЭлементаФормы <> Неопределено Тогда
							...
							Если ТипЗнч(ДанныеЭлементаФормы) = Тип("НастройкиКомпоновкиДанных") Тогда
								Если СтрокаСлова.Слово = "ТекущиеДанные" Тогда
							...
							ИначеЕсли ТипЗнч(ДанныеЭлементаФормы) = Тип("ДинамическийСписок") Тогда
								Если СтрокаСлова.ТипЗначения <> "Произвольный" Тогда
									...
									СтруктураТипа.Метаданные = ИменаРеквизитовФормы(Форма, ИмяРеквизитаФормы, Истина).Все; 
								КонецЕсли;
							Иначе
								НоваяТаблицаТипов = ТаблицаТиповЭлементовКоллекции(СтруктураТипаИзЗначения(ДанныеЭлементаФормы));
						...
						Если НоваяТаблицаТипов <> Неопределено Тогда
							НоваяТаблицаТипов.ЗаполнитьЗначения(СтрокаСлова, "СтрокаОписания");
							Если Форма <> Неопределено Тогда
					...
					Если КорневойТипРодителя <> Неопределено Тогда 
						...
						Тогда 
							...
							Если НРег(КонкретныйТип) = Нрег("ТабличнаяЧасть") Тогда
								Образователь = ирОбщий.УстановитьМетаданныеКоллекцииЛкс(СтрокаОбщегоТипа.Метаданные);
...
Функция ПроверятьДоступностьВКонтекстеКомпиляции(Знач ФлагиКомпиляции, Знач ИмяОбщегоТипаРодителя, Знач ЭтоЛокальныйКонтекст) Экспорт
	Результат = ФлагиКомпиляции <> Неопределено
		И Не (Истина
			...
			И Не (Истина
				И ирОбщий.ЛиИмяТипаФормыЛкс(ИмяОбщегоТипаРодителя) 
				И ЭтоЛокальныйКонтекст))
		;
	Возврат Результат;
КонецФункции
...
//  ТаблицаСлов						 - см. НоваяТаблицаСлов() - 
//  ВычислятьТипыМетодовМодулей		 - Булево, Неопределено - 
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
	...
	Знач ТаблицаСлов = Неопределено, Знач ВычислятьТипыМетодовМодулей = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь, Знач ЛиВместеСЛокальнымКонтекстом = Ложь, Знач БазовоеРасширениеКонфигурации = "",
	Знач ДляСвойства = "", Знач НаборыСлов = Неопределено, КлючКэширования = Неопределено) Экспорт 
	...
	Если ВычислятьТипыМетодовМодулей = Неопределено Тогда
		ВычислятьТипыМетодовМодулей = Слово <> Неопределено;
	КонецЕсли;
	...
	ТаблицаОбщихТиповСтруктурыТипа = ТаблицаОбщихТиповИзСтруктурыТипа(РодительскаяСтруктураТипа, ЭтоЛокальныйКонтекст, Ложь);
	НомерВерсииПлатформы = ирКэш.НомерВерсииПлатформыЛкс();    
	ЕдинственнаяСтрокаВида = Неопределено;
	ПроверятьДоступностьВКонтекстеКомпиляции = ПроверятьДоступностьВКонтекстеКомпиляции(ФлагиКомпиляции, РодительскаяСтруктураТипа.ИмяОбщегоТипа, ЭтоЛокальныйКонтекст);
		...
		Если ирОбщий.СтрокиРавныЛкс(ИмяОбщегоТипаРодителя, "ISWbemObject {WbemScripting.SwbemLocator}") Тогда
			Если ТипЗнч(СтрокаОбщегоТипа.Метаданные) = Тип("COMОбъект") Тогда
				...
				Для Каждого КлючИЗначение Из СтруктураЦикла Цикл
					...
					Если Истина
						И ТипСлова <> Неопределено
						И Не ирОбщий.СтрокиРавныЛкс(ТипСловаЧленаКласса, ТипСлова)
					Тогда
						Продолжить;
					КонецЕсли; 
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			Если Ложь
				...
				Или (Истина
					И ПроверятьДоступностьВКонтекстеКомпиляции
					И (Ложь
						Или ФлагиКомпиляции.Сервер И СтрокаВида.НеСервер
						Или ФлагиКомпиляции.КлиентОбычноеПриложение И СтрокаВида.НеТолстыйКлиент))
			Тогда
				Продолжить;
			КонецЕсли;
			...
			//Если Истина
			//	И ЗначениеЗаполнено(Слово)
			//	И СтрокаВида.Слово = "<Имя реквизита>" 
			...
			Если ЕдинственнаяСтрокаВида = Неопределено Тогда
				ЕдинственнаяСтрокаВида = СтрокаВида;
			Иначе 
			...
			Тогда
				Если ТипЗнч(МетаданныеРодителя) = Тип("Структура") Тогда 
					Если Ложь
						Или СтрокаВида.ТипЗначения = "КолонкаТаблицыЗначений" 
						Или СтрокаВида.ТипЗначения = "КолонкаДереваЗначений" 
					Тогда 
						СхемаКоллекции = Новый Структура;
						ОписаниеТиповКолонки = Новый ОписаниеТипов(СтрокаВида.ТипЗначения);
			...
			ИначеЕсли Истина
				...
				И ЗначениеЗаполнено(Слово)
			Тогда
				СхемаКоллекции.Вставить(Слово, МетаданныеРодителя);
			ИначеЕсли Истина
				И СтрокаОписанияВида.ИмяКоллекции = "Колонки" 
				И (Ложь
				...
				Если ЗначениеЗаполнено(Слово) Тогда
					...
					Иначе
						...
						Если Истина
							...
							И ТипЗнч(МетаданныеРодителя) = Тип("Форма") 
							И Не ИменаРеквизитовФормы(ИмитаторФормы, "").Все.Свойство(Слово)
						Тогда 
			...
			ИначеЕсли Истина
				...
				И МетаданныеРодителя.Реквизиты.Найти(Слово) <> Неопределено
			Тогда
				// Нужно для подготовки реквизита обработки, видимой в ее форме (Обработка.ирКонсольЗапросов.Реквизит.ДеревоЗапросов)
				Попытка
					ЗначениеРеквизита = РодительскаяСтруктураТипа.Метаданные[Слово];
				Исключение
					ЗначениеРеквизита = Неопределено;
				КонецПопытки;
				Если ЗначениеРеквизита <> Неопределено Тогда
					КоллекцияЗначений = Новый Структура(Слово, ЗначениеРеквизита);
				Иначе
			...
			Тогда
				СхемаКоллекции = СхемаКоллекцииПараметровФормы(РодительскаяСтруктураТипа, Слово);
			ИначеЕсли Ложь
			...
			Тогда
				СловаКонтекстаМетаданныеФормы(Слово, ИмяОбщегоТипаРодителя, МетаданныеРодителя, СтрокаОписанияВида, КоллекцияЗначений, СхемаКоллекции);
					...
					Если СлужебныеДанные.Свойство("Элементы") Тогда
						...
						Для Каждого СтруктураЭлемента Из ИмитаторыЭлементов Цикл
							...
							Если ДочернийЭлемент = Неопределено Тогда
								...
								ДочернийЭлемент = ТаблицаТиповИзИмитатораЭлементаФормы(СтруктураЭлемента, ИмитаторФормы, Слово);
							КонецЕсли;
					...
					Иначе
						...
						Если Истина
							...
							И (Ложь
								...
								Или Найти(СтрокаВида.ТипЗначения + ",", "Произвольный,") > 0 
						...
						Тогда 
							...
							Тогда
								...
								Если Истина
									...
									И ТипЗнч(ОписаниеТиповЗначения) = Тип("ОписаниеТипов")
						...
						Тогда
							...
							Иначе
								...
								Для Каждого Тип Из МассивТипов Цикл
									...
									КонецЕсли;  
									Если СтруктураТипаСвойства.Метаданные = Неопределено Тогда
										СтруктураТипаСвойства.Метаданные = мМетаданные; // Чтобы проверка модуля не считала слово неизвестным, например колонку таблицы значений без указанного типа
									КонецЕсли;
		...
		Тогда
			...
			Иначе
				...
				Если МодульМетаданныхКомпонента <> Неопределено Тогда
					...
					Тогда
						ТаблицаСлов = ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст,
							ФлагиКомпиляции, ВнутриГруппыОбщихМодулей, ДляСвойства, ЛиВместеСЛокальнымКонтекстом);
					КонецЕсли;
					...
					Для Каждого ВидимоеРасширение Из ВидимыеРасширения Цикл
						...
						Если МодульМетаданныхРасширение <> Неопределено Тогда
							ТаблицаСлов = ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхРасширение, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст,
									ФлагиКомпиляции, ВнутриГруппыОбщихМодулей, ДляСвойства, ЛиВместеСЛокальнымКонтекстом);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			#Если Клиент Тогда
				ТаблицаСлов = СловаКонтекстаМетаданныеCOM(Слово, ТипСлова, ИмяОбщегоТипаРодителя, МетаданныеРодителя, ТаблицаСлов, НаборыСлов, КлючКэширования);
			#КонецЕсли
		КонецЕсли; 
	КонецЦикла;
	Если Истина
	...
	Тогда 
		...
		Для Каждого ТипЗначения Из МассивТиповЗначения Цикл
			...
			Если ТипЗнч(МетаданныеРодителя) <> Тип("ОбъектМетаданных") Тогда
				СтруктураТипа.Метаданные = МетаданныеРодителя;
			Иначе
				// Мультиметка29332671 Не передаем метаданные, иначе проверка модуля не найдет обращение к несуществующему реквизиту ТЧ в языке запросов
			КонецЕсли;
...
Функция СхемаКоллекцииПараметровФормы(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено) Экспорт
	...
	Если ТипЗнч(МетаданныеРодителя) = Тип("ДанныеФормыСтруктура") Тогда
		...
		ТаблицаПараметровФормы = ТаблицаСловИзСтруктурыТипа(СтруктураТипаФормы,,,,,, "Параметр",, Слово);
	...
	ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("Строка") Тогда
		ТаблицаПараметровФормы = ТаблицаСловИзСтруктурыТипа(НоваяСтруктураТипа(МетаданныеРодителя),,,,,, "Параметр",, Слово);
		Для Каждого СтрокаПараметра Из ТаблицаПараметровФормы Цикл
...
Функция ДобавитьВТаблицуСловЭлементыГлобальныхМодулей(Знач СтрокаОбщегоТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВычислятьТипы = Истина, Знач ВычислятьТипыМетодовМодулей = Ложь,
	Знач ТолькоЭкспорт = Истина, Знач КлиентОбычноеПриложение = Истина, Знач КлиентУправляемоеПриложение = Истина, Знач НаСервере = Ложь, Знач ТаблицаСлов = Неопределено, Знач БезПараметров = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТипСлова) Или ТипСлова = "Метод" Тогда
		...
		Для Каждого ОбщийМодульМД Из ирКэш.ГлобальныеОбщиеМодулиЛкс(НаСервере, КлиентОбычноеПриложение, КлиентУправляемоеПриложение) Цикл
			...
			МодульМетаданныхКомпонента = ПодготовитьМодульМетаданных(СтруктураТипаМодуля);
			ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, "Метод", ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, ТолькоЭкспорт,,,,, БезПараметров);
		КонецЦикла;
	КонецЕсли;
	...
	Если КлиентОбычноеПриложение Тогда
		...
		ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, ТолькоЭкспорт,,,,, БезПараметров);
	КонецЕсли;
...
Функция ТаблицаТиповЭлементовКоллекции(Знач РодительскаяСтруктураТипа, Знач ТекстИндексаЭлемента = "", Знач ЯзыкПрограммы = 0, Знач ТаблицаТипов = Неопределено) Экспорт 
	...
	ИначеЕсли Истина
		...
		И МетаданныеКоллекции.Количество() > 0
		И (Ложь
			Или ирОбщий.ПервыйФрагментЛкс(РодительскаяСтруктураТипа.ИмяОбщегоТипа, "[") = "Структура" 
			Или ирОбщий.ПервыйФрагментЛкс(РодительскаяСтруктураТипа.ИмяОбщегоТипа, "[") = "ФиксированнаяСтруктура")
	Тогда
	...
	Иначе 
		...
		Если ТипыЭлементовКоллекции.Количество() > 0 Тогда
			...
			Для Каждого БазовыйТипЭлемента Из ТипыЭлементовКоллекции Цикл
				...
				КонецЕсли;
				Если Истина
					И ТипЗнч(СтруктураТипаЭлемента.Метаданные) = Тип("ОбъектМетаданныхКонфигурация") 
				...
				Если ТипЗнч(МетаданныеКоллекции) = Тип("ПостроительЗапроса") Тогда 
					...
					КонецПопытки;
					Если ИндексВПакете <> Неопределено И ЗначениеЗаполнено(МетаданныеКоллекции.Текст) Тогда
						ТекстыЗапросов = РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(МетаданныеКоллекции.Текст);   
						#Если Сервер И Не Сервер Тогда
							ТекстыЗапросов = Новый Массив;
						#КонецЕсли
						ПостроительЗапроса = ПостроительПакетаЗапросовДоИндекса(ТекстыЗапросов, ИндексВПакете, МетаданныеКоллекции.Параметры,, РодительскаяСтруктураТипа.ДержательМетаданных);
						Если ПостроительЗапроса <> Неопределено Тогда
							СтруктураТипаЭлемента.Метаданные = ПостроительЗапроса;
							МенеджерВременныхТаблиц = РодительскаяСтруктураТипа.ДержательМетаданных; // см. ОбработкаОбъект.ирКлсПолеТекстаПрограммы.НовыйМенеджерВременныхТаблиц()
...
Функция ПостроительПакетаЗапросовДоИндекса(Знач ТекстыЗапросов, Знач ИндексЭлемента, Знач Параметры, Знач ИмяРождаемойТаблицы = "", Знач ДержательМетаданных = Неопределено) Экспорт 
	...
	Исключение
	КонецПопытки;
	ПодготовитьМетаданныеПостроителяЗапроса(ПостроительЗапроса, ДержательМетаданных);
	Возврат ПостроительЗапроса;
КонецФункции
...
//    ТекстЗапроса - Строка - 
//    Параметры - Структура -  
Функция ДобавитьВременныеТаблицыПараметровВТекстЗапроса(ТекстЗапроса, Знач Параметры) Экспорт
	ПрефиксСлужебнойТаблицы = "ъ_";
...
Функция ПодготовитьЗначениеПараметраЗапроса(Знач ЗначениеПараметра, выхОписаниеТипов = Неопределено) Экспорт
	...
	Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
		...
		Для Каждого ОписаниеКолонки Из ЗначениеПараметра Цикл
			ТаблицаЗначений.Колонки.Добавить(ОписаниеКолонки.Ключ, ОписаниеТиповИзТаблицыТипов(ОписаниеКолонки.Значение));
		КонецЦикла;
...
Процедура ПодготовитьМетаданныеПостроителяЗапроса(Знач ПостроительЗапроса, Знач ДержательМетаданных = Неопределено) Экспорт  
	...
	Если ПостроительЗапроса.ВыбранныеПоля.Количество() = 0 И ирКэш.ДоступноСхемаЗапросаЛкс() Тогда
		...
		НовыйТекст = ДобавитьВременныеТаблицыПараметровВТекстЗапроса(Текст, ПостроительЗапроса.Параметры);
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ТаблицаСловПриемник = Неопределено,
	...
	Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ДляСвойства = "", Знач ЛиВместеСЛокальнымКонтекстом = Ложь, Знач БезПараметров = Ложь) Экспорт 
	...
	Тогда
		...
		КлиентОбычноеПриложение = ирОбщий.СвойствоСтруктурыЛкс(ФлагиКомпиляции, "КлиентОбычноеПриложение", Не ЛиСеансТолстогоКлиентаУП);
		КлиентУправляемоеПриложение = ирОбщий.СвойствоСтруктурыЛкс(ФлагиКомпиляции, "КлиентУправляемоеПриложение", ЛиСеансТолстогоКлиентаУП);
		КлючКэша = "" + ТолькоЭкспорт + ";" + НаСервере + ";" + БезКонтекста + ";" + КлиентОбычноеПриложение + ";" + КлиентУправляемоеПриложение;
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если Ложь
				...
				Или ИмяМетода = "<>" И Не ЗначениеЗаполнено(Слово) 
			Тогда
				Продолжить;
			КонецЕсли;
...
Функция ДобавитьВТаблицуСловЭлементМодуляМетаданных(Знач ТаблицаСлов, Знач Слово, Знач ВычислятьТипы, Знач ТаблицаИзКэша, Знач ТаблицаТипов, Знач ТипЗначения, Знач ТипСлова,
	...
	Если Слово <> "<>" Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ДобавитьВТаблицуСлов(ТаблицаСлов, Слово, ТипСлова, ТаблицаТипов, ТипЗначения, "Метаданные", ВытеснятьСвойство);
	Возврат ТипЗначения;
КонецФункции
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяМодуля = "", Знач НовыйТекст = Неопределено, Знач ТекущаяПозиция = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(выхИмяМодуля) Тогда
		...
		КонецЕсли; 
		МодульМетаданных = МодульМетаданныхИзКэша(выхИмяМодуля);
		Если Ложь
		...
		Тогда
			...
			Если ФайлОписанияМодуля.Существует() Тогда
			...
			Если МодульМетаданных <> Неопределено И СтарыйМодуль <> МодульМетаданных Тогда
				...
				Тогда
					...
					Для Каждого СтрокаСлова Из ТаблицаСлов Цикл // TODO добавить фильтр по типу 
						...
						// Раскомментировал, чтобы добавляемые в модуле формы колонки реквизита-таблицы учитывались
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	ВерсияФормата = 19; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
...
Функция НепрямыеВызовыСловаВМодуле(Знач ИмяМодуля, Знач ТекстовыйДокумент, Знач Файл, Знач ПолеТекстаПрограммы, Знач ИскатьСлово, Знач ЛиМетод = Истина) Экспорт
	...
	Если ИндексВызововМодуля = Неопределено Тогда
		...
		Если Не ОбновитьИндекс Тогда 
			ИндексВызововМодуля = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлИндекса.ПолноеИмя);
...
Функция НовыйИндексПоискаВызововСловаВМодуле(Знач Модуль = Неопределено, Знач ВерсияФорматаИндекса = 0) Экспорт
...
Процедура ДобавитьВхожденияСловаВИндекс(Знач Вхождения, Знач РегВыражение, Знач ТаблицаВызовов, Знач СВложенными = Ложь, Знач Смещение = 0) Экспорт
			...
			Если СВложенными Тогда
				Если Вхождение.SubMatches.Count > 1 Тогда
					...
					Если Найти(ПараметрыВызова, "(") > 0 Тогда
						...
						ДобавитьВхожденияСловаВИндекс(ВхожденияВложенные, РегВыражение, ТаблицаВызовов, Истина, Вхождение.FirstIndex + Найти(Вхождение.Value, ПараметрыВызова));
					КонецЕсли;
				Иначе 
	...
	Иначе
		...
		Для Каждого Вхождение Из Вхождения Цикл  			СтрокаРезультата = ТаблицаВызовов.Добавить();  			СтрокаРезультата.ПозицияВхождения = Смещение + Вхождение.FirstIndex;  			СтрокаРезультата.ДлинаВхождения = Вхождение.Length;  			СтрокаРезультата.Слово = Вхождение.SubMatches(0);  			Если СВложенными Тогда  				Если Вхождение.SubMatches.Count > 1 Тогда  					ПараметрыВызова = Вхождение.SubMatches(1);  					Если Найти(ПараметрыВызова, "(") > 0 Тогда  						ВхожденияВложенные = РегВыражение.НайтиВхождения(ПараметрыВызова,, Истина);  						ДобавитьВхожденияСловаВИндекс(ВхожденияВложенные, РегВыражение, ТаблицаВызовов, Истина, Вхождение.FirstIndex + Найти(Вхождение.Value, ПараметрыВызова));  					КонецЕсли;  				Иначе  					Если Вхождение.FirstIndex > 0 Тогда  						Текст = Сред(Вхождение.Value, 2);  						СмещениеДлины = 1;  					Иначе  						Текст = Вхождение.Value;  						СмещениеДлины = 0;  					КонецЕсли;  					ЧислоТочек = СтрЧислоВхождений(Текст, ".");  					Пока ЧислоТочек > 0 Цикл  						Текст = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(Текст);  						СтрокаВложенная = ТаблицаВызовов.Добавить();  						СтрокаВложенная.ПозицияВхождения = СтрокаРезультата.ПозицияВхождения;  						СтрокаВложенная.ДлинаВхождения = СтрДлина(Текст) + СмещениеДлины;  						СтрокаВложенная.Слово = ирОбщий.ПоследнийФрагментЛкс(Текст);  						ЧислоТочек = ЧислоТочек - 1;  					КонецЦикла;  				КонецЕсли;  			КонецЕсли;  		КонецЦикла;  
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Ложь) Экспорт 
	...
	Если ЧерезКопию Тогда
		...
		КопияМодуля.ПрямыеВызовыМетодов = Неопределено;  
...
Функция МодульМетаданныхИзКэша(Знач ИмяМодуля) Экспорт
	МодульМетаданных = мМодулиМетаданных[НРег(ИмяМодуля)];
	Возврат МодульМетаданных;
КонецФункции 
...
Процедура СброситьКэшТиповВыраженийМодуля(Знач Модуль) Экспорт
	Если Модуль.Имя <> ИмяДинамическогоМодуля() Тогда
		...
		Переменные.ЗаполнитьЗначения(, "ТаблицаТипов, Вычислено"); // Чтобы не отслеживать зависимости 
	КонецЕсли;
	Модуль.ТипыВыраженийПрограммы.Очистить();
	Модуль.КэшПоискаПрограммы.Очистить();
	Модуль.ТаблицыСлов.Очистить();
	Методы = Модуль.Методы;
	Методы.ЗаполнитьЗначения(, "ТаблицаТипов, Вычислено, ТипыВыражений"); // Чтобы не отслеживать зависимости. ТипыВыражений - самые тяжелые  
	_РежимОтладки = Ложь;
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1) 
		// Пассивный оригинал расположенного ниже однострочного кода. Выполняйте изменения синхронно в обоих вариантах. 
		Для Каждого СтрокаМетода Из Методы Цикл
			Если ТипЗнч(СтрокаМетода.Параметры) = Тип("ТаблицаЗначений") Тогда    
				СтрокаМетода.Параметры.ЗаполнитьЗначения(, "ТаблицаТипов, Вычислено");
			КонецЕсли;
		КонецЦикла;
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
		Для Каждого СтрокаМетода Из Методы Цикл  		Если ТипЗнч(СтрокаМетода.Параметры) = Тип("ТаблицаЗначений") Тогда  			СтрокаМетода.Параметры.ЗаполнитьЗначения(, "ТаблицаТипов, Вычислено");  		КонецЕсли;  	КонецЦикла;  
	КонецЕсли; 
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Если ЗначениеЗаполнено(ТипМодуля) Тогда
			...
			Если Найти(Результат, ":") > 0 Тогда
				Результат = ирОбщий.ЗаменитьНедопустимыеСимволыВИмениФайлаЛкс(Результат, "_", "."); // Заменяем точки, чтобы по такому имени модуля не делалось попытки создать форму
			КонецЕсли;
		ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("Строка") Тогда
...
Функция ОбновитьКэшСтруктурыФормыИзБуфера(Знач СлужебныеДанные, Знач ТекстБуфераОбмена, Знач ТекстМодуляФормы = "", Знач Форма = Неопределено) Экспорт 
	Если СлужебныеДанные.Свойство("КорневыеИменаРеквизитов") Тогда
		//! СлужебныеДанные.Вставить("КорневыеИменаРеквизитов")
...
Функция ИмитаторУправляемойФормыИзДокументаДОМ(Знач ДокументDOM, Знач Форма = Неопределено) Экспорт
	...
	//ВидыЭлементовГруппы.Вставить("?", ПолучитьПолноеИмяПредопределенногоЗначения(ВидГруппыФормы.ГруппаКнопок));
	//ВидыЭлементовГруппы.Вставить("?", ПолучитьПолноеИмяПредопределенногоЗначения(ВидГруппыФормы.Подменю));
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Если ЛиНачальныйДиапазонСовпал Тогда
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			Если ТекущийМетод = Неопределено Тогда
				//СтароеНачалоБлока = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(Сред(СтарыйМодуль.Текст, ПозицияНачалаСтарая, ТекущаяПозиция - ПозицияНачалаСтарая), Символы.ПС);
				//СтароеКонецБлока = ирОбщий.СтрокаБезПервогоФрагментаЛкс(Сред(СтарыйМодуль.Текст, ТекущаяПозиция), Символы.ПС);
				//НовоеНачалоБлока = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(Сред(ТекстМодуля, ПозицияНачалаСтарая, ТекущаяПозиция - ПозицияНачалаСтарая), Символы.ПС);
				//НовоеКонецБлока = ирОбщий.СтрокаБезПервогоФрагментаЛкс(Сред(ТекстМодуля, ТекущаяПозиция), Символы.ПС);
			...
			Иначе 
				//СтароеНачалоБлока = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(Сред(СтарыйМодуль.Текст, ПозицияНачалаСтарая, ТекущаяПозиция - ПозицияНачалаСтарая), Символы.ПС);
				//СтароеКонецБлока = ирОбщий.СтрокаБезПервогоФрагментаЛкс(Сред(СтарыйМодуль.Текст, ТекущаяПозиция, ТекущийМетод.ДлинаСОписанием - (ТекущаяПозиция - ПозицияНачалаСтарая)), Символы.ПС); 
				//НовоеНачалоБлока = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(Сред(ТекстМодуля, ПозицияНачалаСтарая, ТекущаяПозиция - ПозицияНачалаСтарая), Символы.ПС);
				//НовоеКонецБлока = ирОбщий.СтрокаБезПервогоФрагментаЛкс(Сред(ТекстМодуля, ТекущаяПозиция, ТекущийМетод.ДлинаСОписанием + СмещениеКонечногоДиапазона - (ТекущаяПозиция - ПозицияНачалаСтарая)), Символы.ПС); 
			...
			КонецЕсли;
			Если Модуль.ПрямыеВызовыМетодов <> Неопределено Тогда
				ВыполнитьОтложенноеОбновлениеВызововСловМодуля(Модуль, ПозицияНачалаСтарая);
				...
				Модуль.ПрямыеВызовыМетодов.Вставить("ОтложенныйСдвиг", ОтложенныйСдвиг);     
	...
	Иначе 
		...
		Модуль.Вставить("ПрямыеВызовыМетодов"); // см. НовыйИндексПоискаВызововСловаВМодуле()
		Шаблоны = ШаблоныДляАнализаВстроенногоЯзыка();
		RegExp.IgnoreCase = Истина;
		RegExp.Multiline = Ложь; 
	...
	Для Каждого Вхождение Из ВхожденияМетодов Цикл
		...
		ИначеЕсли Вхождение.SubMatches(8) <> Неопределено Тогда
			...
			Если Лев(НоваяАннотация, 1) = "&" Тогда
				ПоследняяАннотация = НоваяАннотация;
				ПоследнееНачалоАннотации = Вхождение.FirstIndex + НачальнаяПозицияДиапазона;
			КонецЕсли;
			//Если Лев(НоваяАннотация, 1) = "#" Тогда
...
// Параметры:
//    Модуль - Структура - 
Процедура ВыполнитьОтложенноеОбновлениеВызововСловМодуля(Модуль, Знач ПозицияНачалаСтарая = Неопределено) Экспорт
	ПрямыеВызовыМетодов = Модуль.ПрямыеВызовыМетодов; 
	Если Не ПрямыеВызовыМетодов.Свойство("ОтложенныйСдвиг") Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОтложенныйСдвиг.ПозицияНачалаСтарая = ПозицияНачалаСтарая Тогда
		Возврат;
	КонецЕсли;
	ПолеТекстаМодуля = ПолеТекстаМодуля(Модуль);
	ПрямыеВызовыМетодовВМетоде = НовыйИндексПоискаВызововСловаВМодуле(Модуль);
	ПрямыеВызовыМетодовВМетоде.СтруктураСлов = ПрямыеВызовыМетодов.СтруктураСлов; 
	ПолеТекстаМодуля.ЗаполнитьИндексПоискаВызововМетодовВМодуле(ПрямыеВызовыМетодовВМетоде, ОтложенныйСдвиг.ОпределениеСОписанием, ОтложенныйСдвиг.ПозицияНачалаСтарая);
...
Функция НоваяТаблицаМетодовМодуля() Экспорт 
	...
	Методы.Колонки.Добавить("ТаблицаТипов"); // см. НоваяТаблицаТипов
	...
	Методы.Колонки.Добавить("КлючевыеПараметры"); // Структура
	Методы.Колонки.Добавить("КэшПоиска");     // Соответствие
	Методы.Колонки.Добавить("ТипыВыражений"); // Соответствие
	Методы.Колонки.Добавить("СтарыеТипы"); // Соответствие
	Методы.Колонки.Добавить("ВызывающиеМодули"); // Массив
	Возврат Методы;             
		...
		Иначе
			...
			Если РезультатПараметров.Количество() > 0 Тогда
				...
				Для Каждого ВхождениеПараметра Из РезультатПараметров Цикл
					...
					СтрокаПраметра.Значение = СокрЛП(ВхождениеПараметра.SubMatches(2));
					Если СтрокаПраметра.Значение = "" Тогда
						МинЧислоПараметров = Результат.Количество();
					КонецЕсли;
				КонецЦикла;
				Если ЗначениеЗаполнено(СтрокаМетода.Описание) Тогда
					ОписаниеМетода = ирОбщий.ПервыйФрагментЛкс(СтрокаМетода.Описание, "Возвращаемое значение:"); // Иначе можно найти имя типа результата функции в качестве параметра
					ОписанияПараметровЗагружены = Ложь;
					Если СтрЧислоСтрок(ОписаниеМетода) = 1 Тогда 
						СсылкаНаОписание = ОписаниеМетода;
					...
					Если Не ОписанияПараметровЗагружены Тогда
						ВхожденияОписаний = ПараметрыМетодаИзКомментария(ОписаниеМетода, Шаблоны);
						Анализатор = ПолеТекстаМодуля(СтрокаМетода.ИмяМодуля);
						Для каждого ВхождениеОписания Из ВхожденияОписаний Цикл
							...
							Если СтрокаПараметра <> Неопределено Тогда
								...
								//Иначе 
									...
									НуженРазборТипа = Не ЗначениеЗаполнено(СтрокаПараметра.ТипЗначения);
									Если Не НуженРазборТипа Тогда
										ЧастьСоВторойСтроки = ирОбщий.СтрокаБезПервогоФрагментаЛкс(СтрокаПараметра.Описание, Символы.ПС);
										НуженРазборТипа = Истина
											И ЗначениеЗаполнено(ЧастьСоВторойСтроки)
											И Найти(ЧастьСоВторойСтроки, "-"); // хорошо бы и второй вариант тире проверять
									КонецЕсли;
									Если НуженРазборТипа Тогда
										ТаблицаТипов = Анализатор.ТаблицаТиповИзТекста(СтрокаПараметра.ТипЗначения, СтрокаПараметра.Описание,, Анализатор.ИмяФиктивногоСвойства());
										Если Ложь
											Или Не ЗначениеЗаполнено(СтрокаПараметра.ТипЗначения)
											Или ТаблицаТипов.Количество() > 1 
										Тогда
											СтрокаПараметра.ТипЗначения = ПредставлениеМассиваСтруктурТипов(ТаблицаТипов);
										КонецЕсли;
									КонецЕсли;
								//КонецЕсли;
							КонецЕсли; 
						...
						ВхожденияОписаний = ирОбщий.НайтиРегВыражениеЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(ОписаниеМетода + Символы.ПС, "Вызывающие модули:", ":", Ложь,,, Истина), шИмяМодуля,,,, Ложь, Ложь);
						Если ВхожденияОписаний.Количество() > 0 Тогда
							СтрокаМетода.ВызывающиеМодули = Новый Массив;
							Для каждого ВхождениеОписания Из ВхожденияОписаний Цикл
								СтрокаМетода.ВызывающиеМодули.Добавить(ВхождениеОписания.Подгруппа0);
							КонецЦикла;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли; 
...
Функция НоваяТаблицаПараметровМетодаМодуля() Экспорт
	...
	Результат.Колонки.Добавить("ТаблицаТипов"); // см. НоваяТаблицаТипов()
	Возврат Результат;
КонецФункции
...
Функция ПараметрыМетодаИзКомментария(Знач ОписаниеМетода, Шаблоны = Неопределено) Экспорт
	...
	шОписаниеПараметра = "\n[\t ]{0,8}(" + шИмя + ")\*?[\t ]*(?:" + шРазделительВнутриСтроки + "[\t ]*([^-–\n]*?))?([\t ]*" + шРазделительВнутриСтроки + "\s*?"  + Шаблоны.ОписаниеПараметра + ")[\.;]?";
...
Функция ПодготовитьТипЗначенияПеременнойМодуля(СтрокаОпределения, ИмяИлиМодуль, Знач ПолеТекстаМодуляКэш = Неопределено, Знач ДляСвойства = "") Экспорт 
	...
	Если ТаблицаТипов.Количество() > 1 Тогда
		ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюВКолонкеЛкс(ТаблицаТипов, "ИмяОбщегоТипа", "Произвольный"); // Нужно для проверок выражений
	КонецЕсли;
	Возврат ТаблицаТипов;  
КонецФункции
	...
	Иначе 
		...
		Иначе  
			...
			Если Истина
				...
				И СтрокаОпределения.ТипЗначения <> "?"
			Тогда
				ТаблицаТиповИзКомментария = ПолеТекстаМодуляКэш.ТаблицаТиповИзТекста(СтрокаОпределения.ТипЗначения, ОписаниеТипа,, ДляСвойства);  
...
Функция ПараметрыМетодаПлатформы(Знач СтрокаОписания) Экспорт 
	...
	Если Не ЗначениеЗаполнено(СтрокаОписания.Описание) Тогда // Мультиметка443985642
		...
		Если СтрокаОписания.ТипСлова = "Конструктор" Тогда
			//Отбор = Новый Структура("ТипКонтекста, ЯзыкПрограммы, ТипЯзыка, НСлово"); // Так будет только для одного варианта синтаксиса
...
//
// Параметры:
//  ТаблицаСлов			 - ТаблицаЗначений - НоваяТаблицаСлов();
//  Слово				 - Строка;
//  ТипСлова			 - Строка;
//  ТаблицаТипов		 - ТаблицаЗначений.
//  ТипЗначения			 - 					 - 
...
//  ТаблицаЗначений - 
//
Функция ДобавитьВТаблицуСлов(Знач ТаблицаСлов, Знач Слово, Знач ТипСлова = "Свойство", Знач ТаблицаТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "", Знач ВытеснятьСвойство = Истина,
	Знач СтарыеТипы = Неопределено) Экспорт
...
Функция ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	Знач ВычислятьТипыМетодовМодулей = Неопределено, Знач ТаблицаСлов = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь, Знач НаборыСлов = Неопределено, Знач БазовоеРасширениеКонфигурации = "") Экспорт 
	...
	Если Не ВнутриГруппыОбщихМодулей Тогда
		ТаблицаСлов = СловаКонтекстаПредопределенные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ФлагиКомпиляции, ТаблицаСлов, ЭтоЛокальныйКонтекст, НаборыСлов,
			БазовоеРасширениеКонфигурации, КлючКэширования);
	КонецЕсли;
	Если КлючКэширования <> Истина Тогда
		ТаблицаСлов = СловаКонтекстаМетаданные(СтруктураТипа, Слово, ТипСлова, ВнутриГруппыОбщихМодулей, ЯзыкПрограммы,, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ФлагиКомпиляции, 
			МодульМетаданных, ТаблицаСлов, ВычислятьТипыМетодовМодулей, ЭтоЛокальныйКонтекст,, БазовоеРасширениеКонфигурации, НаборыСлов,, КлючКэширования);
		Если Истина
			И Слово = Неопределено
...
//
Функция ДобавитьВТаблицуТипов(ТаблицаПриемник = Неопределено, Знач ОписаниеТипов, Знач ПередаватьДополнительныеТипы = Ложь, Знач ВытеснятьБезУчетаОписания = Истина, Знач КопироватьСтруктуры = Ложь,
	...
	КонецЕсли;
	ИмяКолонкиФлагаВнутриТаблицы = ИмяКолонкиФлагаВнутриТаблицы();
	Для Каждого СтрокаИсточника Из ТаблицаИсточник Цикл
		Если Истина
			//И НужноВытеснять 
			...
			Иначе
				...
				Если Найденные.Количество() > 0 Тогда 
					...
					Если Ложь
						...
						Или Истина
							И ТипМетаданныхПриемника = Тип("ОбъектМетаданныхКонфигурация") 
							И ТипМетаданныхИсточника = Тип("ОбъектМетаданныхКонфигурация") // Почему то часто равенство значений не выполняется
					...
					Иначе
						...
						Тогда
							Для Каждого Колонка Из СтрокаИсточника.Метаданные.Колонки Цикл
								Если ирОбщий.СвойствоСтруктурыЛкс(СтрокаПриемника.Метаданные, Колонка.Имя) = Неопределено Тогда
									...
									КонецЕсли;
									СтрокаПриемника.Метаданные.Вставить(Колонка.Имя, ТаблицаТиповКолонки);
								КонецЕсли;
							КонецЦикла;
							Продолжить;
						ИначеЕсли Истина
						...
						Тогда
							...
							Продолжить;
						ИначеЕсли Истина
							И ТипМетаданныхИсточника = Тип("ОбъектМетаданныхКонфигурация") 
							И ТипМетаданныхПриемника = Тип("Структура")
						Тогда
							Продолжить;
						ИначеЕсли Истина
						...
						Тогда
							Для Каждого КлючИЗначение Из СтрокаИсточника.Метаданные Цикл
								...
								Если Истина
									...
									И ЗначениеИзПриемника.Колонки.Найти(ИмяКолонкиФлагаВнутриТаблицы) <> Неопределено
									И ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") 
									И КлючИЗначение.Значение.Колонки.Найти(ИмяКолонкиФлагаВнутриТаблицы) <> Неопределено
								Тогда
...
Функция НайтиИмяМетодаСтрокиМодуля(Знач ИмяМодуля, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулей = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "",
	...
	ИмяМетода = НайтиИмяМетодаСтрокиФайлаМодуля(ОписаниеМодуля.ИмяМодуля, НомерСтроки, СодержаниеСтроки, ТекстыМодулей, СтароеИмяФайлаМодуляБезРасширения, ЧислоСтрокДляПоискаИмениМетода);
	Возврат ИмяМетода;
...
Функция ИмяМодуляИзИмениФайла(Знач ПолноеИмя) Экспорт
	ИмяМодуля = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(СтрЗаменить(ПолноеИмя, ПапкаКэшаМодулей.ПолноеИмя + "\", ""),, Ложь);
	Возврат ИмяМодуля;
КонецФункции
...
Функция ПассивнаяФормаПоИмениКэш(Знач ИмяФормы) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если ФайлФормы.Существует() Тогда 
			ИмитаторФормы = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлФормы.ПолноеИмя); // см. ИмитаторУправляемойФормыИзДокументаДОМ()
...
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM, Знач ПутьКРодителю = Неопределено, Знач ЛиДляКоллекции = Ложь, выхИдентификаторыРеквизитов = Неопределено, ДокументДОМ = Неопределено,
	...
	Если Ложь
		Или ТипЗнч(ФормаИлиДокументDOM) = ирОбщий.ТипУправляемаяФормаЛкс()
	...
	Тогда
		...
		Если ЗначениеЗаполнено(ОсновнойРеквизитФормы) Тогда
			...
			Если ТипЗнч(ОбъектОсновнойРеквизит) = Тип("ДанныеФормыСтруктура") Тогда
				//СтруктураТипаОбъекта = ТаблицаТиповИзОписанияТипов(ИменаРеквизитовФормы(Форма).Все[ОсновнойРеквизитФормы])[0]; // Там обычно пустое описание типов
				Попытка
				...
				Если Ключ <> Неопределено Тогда
					СтруктураТипаОбъекта = СтруктураТипаИзКонкретногоТипа(Тип(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(Метаданные.НайтиПоТипу(ТипЗнч(Ключ)), "Объект")));
				КонецЕсли;
			Иначе
