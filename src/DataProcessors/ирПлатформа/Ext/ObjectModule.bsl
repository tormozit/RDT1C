...
Функция СвойстваТипаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
				...
				Для Каждого ИмяОбщегоТипа Из МассивТиповЗначения Цикл
					...
					Если КорневойТипРодителя <> Неопределено Тогда 
						...
						Если Ложь
							...
							// Для уточнения квалификаторов
							Или СтрокаСлова.ТипЗначения = "Дата"
							Или СтрокаСлова.ТипЗначения = "Строка"
							Или СтрокаСлова.ТипЗначения = "Число"
						Тогда
			...
			ДобавитьВТаблицуСлов(ТаблицаСвойств, СтрокаСлова.Слово, СтрокаСлова.ТипСлова, ТаблицаСтруктурТипов,, "Предопределенный"); // Убрал передачу готового типа значения, чтобы квалификаторы позже применились
		КонецЦикла;
		Если ИспользоватьКэширование Тогда
					...
					Иначе
						...
						Если СхемаКоллекции.Количество() > 0 Тогда
							Если Ложь
								...
								// Для уточнения квалификаторов
								Или СтрокаВида.ТипЗначения = "Дата"
								Или СтрокаВида.ТипЗначения = "Строка"
								Или СтрокаВида.ТипЗначения = "Число"
							Тогда 
