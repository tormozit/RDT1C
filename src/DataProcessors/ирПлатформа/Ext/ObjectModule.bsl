					...
					Тогда 
						...
						Если ДанныеЭлементаФормы <> Неопределено Тогда
							...
							ИначеЕсли ирОбщий.ЛиКоллекцияКомпоновкиЛкс(ДанныеЭлементаФормы) Тогда 
								ТаблицаТипов = ТаблицаТиповЭлементовКоллекции(СтруктураТипаИзЗначения(ДанныеЭлементаФормы.Элементы));
				...
				Если МодульМетаданныхКомпонента <> Неопределено Тогда
					...
					Тогда
						ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст,
							ФлагиКомпиляции, ВнутриГруппыОбщихМодулей, ДляСвойства);
					КонецЕсли;
	...
	Для Каждого КлючИЗначение Из Параметры Цикл
		...
		Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
			...
			Для Каждого ОписаниеКолонки Из ЗначениеПараметра Цикл
				ТаблицаЗначений.Колонки.Добавить(ОписаниеКолонки.Ключ, ОписаниеТиповИзТаблицыСтруктурТипов(ОписаниеКолонки.Значение));
			КонецЦикла;
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
	...
	Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ДляСвойства = "") Экспорт 
	...
	Тогда
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			...
			КонецЕсли;
			ТипЗначения = "??";
			Если ЗначениеЗаполнено(ПеременнаяМодуля.ТипЗначения) Тогда
				 ТипЗначения = ТипЗначения + "," + ПеременнаяМодуля.ТипЗначения;
			КонецЕсли;
			ДобавитьВТаблицуСлов(ВнутренняяТаблицаСлов, ПеременнаяМодуля.Имя, "Свойство", ТаблицаТипов, ТипЗначения, "Метаданные");
		КонецЦикла;
	КонецЕсли; 
	Если Истина
			...
			Если ОбновитьФайлОписания Тогда
				...
				Иначе 
					...
					Иначе
						...
						УдалитьФайлы(ПапкаКэшаМодулей.ПолноеИмя, выхИмяМодуля + ".i*"); // Все расширения файлов индексов начинаются с "i"
...
Функция ИндексПоискаВызововСловаВМодуле(Знач ТекстМодуля, Знач ШаблонВнешнегоВызова, Знач ВерсияФорматаИндекса = 0) Экспорт
	_РежимОтладки = Ложь;
	...
	ИндексВызововМодуля = Новый Структура("ВерсияФормата, Вызовы, СтруктураСлов");
