...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено) Экспорт 
	...
	ВерсияФормата = 4; // Метка91237256881041 Каждый раз при изменении структуры увеличиваем на 1
		...
		Тогда
			...
			Если ОбновитьФайлОписания Тогда
				...
				Иначе 
					...
					Иначе
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, выхИмяФайлаМодуляБезРасширения, Ложь);
...
Функция ПодготовитьТипРезультатаМетода(СтрокаОпределения, ИмяМодуля, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено, Знач СФактическимиПараметрами = Ложь) Экспорт 
	...
	Если Не СтрокаОпределения.Вычислено Или СФактическимиПараметрами Тогда
