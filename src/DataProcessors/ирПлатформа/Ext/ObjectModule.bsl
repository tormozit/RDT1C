...
Функция РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(Знач ТекстПакета, Знач ПозицияКурсора = Неопределено, выхПозиции = Неопределено) Экспорт 
	...
	Если Не ирКэш.ДоступныРегВыраженияЛкс() Тогда
		Если ирКэш.ДоступнаСхемаЗапросаЛкс() Тогда
...
Функция СтруктураРезультатаПакетногоЗапроса(ТекстПакета, ПрефиксКомментария = "//") Экспорт 
	...
	RegExp.Global = Ложь;
	RegExp.Multiline = Ложь;
	RegExp.Pattern = ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(ПрефиксКомментария + мМаркерИмениЗапросаПакета) + "(" + шИмя + ")(?: |\n|\r)";
	Индекс = -1;
...
Функция СвойстваТипаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если ИспользоватьКэширование Тогда
			...
			СвойстваТипаМетаданные(СтруктураТипаГлобальный,, ТипСлова,, ЯзыкПрограммы,,, ВычислятьТипы, ФлагиКомпиляции,, ТаблицаСлов);
...
//  ТаблицаЗначений - с колонками "Слово", "ТипСлова", "ТаблицаСтруктурТипов".
//
Функция СвойстваТипаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач _ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
		...
		Тогда
			Если СтрокаОбщегоТипа.ИмяОбщегоТипа = "Глобальный" Тогда
				Если ТипСлова = "Метод" Тогда
					...
					Для Каждого ОбщийМодульМД Из Метаданные.ОбщиеМодули Цикл
						...
						Тогда
							Продолжить;
						КонецЕсли; 
						СтруктураТипа.Метаданные = ОбщийМодульМД;
						МодульМетаданныхКомпонента = ПодготовитьМодульМетаданных(СтруктураТипа);
				...
				Если КлиентОбычноеПриложение Тогда
					...
					МодульМетаданныхКомпонента = ПодготовитьМодульМетаданных(СтруктураТипа, "МодульОбычногоПриложения");
					ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст);
				КонецЕсли;
				Если КлиентУправляемоеПриложение Тогда
					СтруктураТипа = СтруктураТипаИзЗначения(Метаданные);
					МодульМетаданныхКомпонента = ПодготовитьМодульМетаданных(СтруктураТипа, "МодульУправляемогоПриложения");
					ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхКомпонента, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст);
				КонецЕсли;
			Иначе
				МодульМетаданныхКомпонента = МодульМетаданныхДинамический;
				Если МодульМетаданныхКомпонента = Неопределено Тогда
					СтруктураТипа = НоваяСтруктураТипа(ИмяОбщегоТипаРодителя);
			...
			Если ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса") Тогда 
				...
				Если ИндексЭлемента <> Неопределено Тогда
					...
					Если ТекстыЗапросов.Количество() > ИндексЭлемента Тогда
						...
						КонецЦикла;
						ПостроительЗапроса = Новый ПостроительЗапроса;
						ПостроительЗапроса.Текст = ирОбщий.СтрСоединитьЛкс(ТекстыЗапросов, Символы.ПС);
			...
			Если СтарыйТекст = НовыйТекст И ФайлОписанияМодуля.Существует() Тогда
				...
				Если Не ОбновитьФайлОписания Тогда 
					...
					Если Ложь
						...
						Или Не МодульМетаданных.Свойство("ФлагиКомпиляции") // Проверка версии формата кэша
					Тогда
						// Старый формат кэша
						ОбновитьФайлОписания = Истина;
			...
			Если ОбновитьФайлОписания Тогда
				Если Не ФайлМодуля.Существует() Тогда
					Если СообщениеОНеобходимостиОбновитьКэшМодулейВыводилось <> Истина Тогда
						// Может выводиться неоправданно, если модуль пуст. В этом случае конфигуратор не выгружает файл. Поэтому уведомляем только о полной пустоте кэшаы
		...
		Тогда 
			...
			Если ТипЗнч(МетаданныеРодителя) = Тип("Форма") Тогда
				...
				Если СлужебныеДанныеФормы.Свойство("ИмяФормы") Тогда
					МетаданныеРодителя = ирОбщий.ЗаменитьНедопустимыеСимволыВИмениФайлаЛкс(СлужебныеДанныеФормы.ИмяФормы, "_") + ".Форма";
				КонецЕсли;
			ИначеЕсли ТипЗнч(МетаданныеРодителя) = Тип("УправляемаяФорма") Тогда
...
Функция ПолучитьРазделыМодуля1С(Знач ТекстМодуля = "") Экспорт
	...
	Если ВхожденияМетодов.Количество() > 0 Тогда
		ПозицияКонцаПоследнегоМетода = ПозицияКонцаПоследнегоМетода + 1;
	КонецЕсли;
	Результат.Вставить("ПозицияПрограммы", ПозицияКонцаПоследнегоМетода);
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина) Экспорт
	...
	Для Каждого Вхождение Из Вхождения Цикл
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			...
			КонецЕсли;
			СтрокаМетода.ПозицияСОписанием = Вхождение.FirstIndex + 1;
			...
			СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияОпределения + Найти(ОпределениеБезОписания, Лев(СтрокаМетода.Тело, 20)) - 2; 
...
Функция ОчиститьТекстМодуляОтПрефиксаКомментария(Знач ТекстМодуля)
	...
	Результат = Сред(RegExp.Заменить(ТекстМодуля, Символы.ПС), 2); // Отрезаем первый перенос строки
	Возврат Результат;
...
Функция ПодготовитьТипРезультатаМетода(СтрокаОпределения, МодульМетаданных, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено) Экспорт 
	...
	Если Не СтрокаОпределения.Вычислено Тогда
		...
		ПодготовитьТипЗначенияСловаМодуля(ИмяПеременной, кэшКомпонентаПодсказки, СтрокаОпределения, МодульМетаданных, СтрокаОпределения, ТекстДляПоискаОпределения);
	КонецЕсли;
...
Процедура ПодготовитьОписаниеРезультатаМетода(СтрокаОпределения) Экспорт
	...
	Тогда
		ШаблонРезультатаФункции = "(?:Возвращаемое\s+значение|Результат)[:-]\s*(?:([^-]+)\s*-)?\s*(.*)\s*";
		ВхожденияОписаний = ирОбщий.НайтиРегВыражениеЛкс(СтрокаОпределения.Описание, ШаблонРезультатаФункции, "ТипЗначения, Описание", Ложь,, Ложь, Ложь);
		Если ВхожденияОписаний.Количество() > 0 Тогда
			ВхождениеОписания = ВхожденияОписаний[ВхожденияОписаний.Количество() - 1];
			Если ЗначениеЗаполнено(ВхождениеОписания.ТипЗначения) Тогда
				СтрокаОпределения.ТипЗначения = СтрокаОпределения.ТипЗначения + ", " + СокрЛ(СтрЗаменить(ВхождениеОписания.ТипЗначения, "//", "")); 
			КонецЕсли;            
...
Функция ПодготовитьТипЗначенияПеременнойМодуля(СтрокаОпределения, МодульМетаданных, кэшКомпонентаПодсказки = Неопределено) Экспорт 
	...
	Если Не СтрокаОпределения.Вычислено Тогда
		...
		КонецЕсли;
		СтрокаОпределения.Вычислено = Истина; // ставим сразу для защиты от бесконечной рекурсии
		ТекстДляПоискаОпределения = МодульМетаданных.Программа;
		МетодМодуля = Неопределено;
		ПодготовитьТипЗначенияСловаМодуля(СтрокаОпределения.Имя, кэшКомпонентаПодсказки, МетодМодуля, МодульМетаданных, СтрокаОпределения, ТекстДляПоискаОпределения);
	КонецЕсли;

КонецФункции

Процедура ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач МодульМетаданных, СтрокаОпределения, Знач ТекстДляПоискаОпределения)
	...
	ТекстДляПоискаОпределения = кэшКомпонентаПодсказки.ЗалитьКомментарииИСтроковыеЛитералы(ТекстДляПоискаОпределения); // Вырезаем условия. Возможно не нужно
	ТаблицаСтруктурТипов = кэшКомпонентаПодсказки.ВычислитьТипЗначенияВыражения(ИмяПеременной, ТекстДляПоискаОпределения, ТекстДляПоискаОпределения);
	Если ТаблицаСтруктурТипов.Количество() > 0 Тогда
	...
	Для Каждого ИмяТипа Из ирОбщий.СтрРазделитьЛкс(СтрокаОпределения.ТипЗначения, ",", Истина) Цикл
		Попытка
			Тип = Тип(ИмяТипа);
		Исключение
			Продолжить;
		...
		ДобавитьВТаблицуСтруктурТипов(СтрокаОпределения.ТаблицаСтруктурТипов, СтруктураТипа);
	КонецЦикла;
	// Обогащаем текстовое описание типа из таблицу структур типов 
...
Функция ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	ИнициацияОписанияМетодовИСвойств();
	ТаблицаСвойств = СвойстваТипаПредопределенные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ФлагиКомпиляции, ТаблицаСвойств, ЭтоЛокальныйКонтекст);
	СвойстваТипаМетаданные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы,, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ФлагиКомпиляции, 
...
Функция ОбновитьДетальностьСтруктурыТипа(СтруктураТипа) Экспорт 
	...
	Если СтруктураТипа.ИмяОбщегоТипа = "Произвольный" Или Тип = Тип("ОбъектМетаданныхКонфигурация") Тогда
		Результат = 0;
	ИначеЕсли Ложь
		Или Тип = Тип("Форма") 
...
Функция РедактироватьЗапрос(Текст, Параметры = Неопределено, Знач ТипЗапроса = "Построитель", ПараметрыADO = Неопределено, ПараметрыWMI = Неопределено,
	Знач ВыделениеДвумерное = Неопределено) Экспорт
