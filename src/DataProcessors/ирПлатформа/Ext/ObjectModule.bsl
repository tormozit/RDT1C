			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					ИначеЕсли Истина
						...
						И СтрокаСлова.ТипСлова = "Метод"
						И РодительскаяСтруктураТипа.ИмяОбщегоТипа = "ТекстовыйДокумент"
...
Функция ТаблицаТиповЭлементовКоллекции(Знач РодительскаяСтруктураТипа, Знач ТекстИндексаЭлемента = "", Знач ЯзыкПрограммы = 0) Экспорт 
	...
	МетаданныеКоллекции = РодительскаяСтруктураТипа.Метаданные;
	Если Истина
		И ТипЗнч(МетаданныеКоллекции) = Тип("Массив")
		И МетаданныеКоллекции.Количество() > 0
		И (Ложь
			Или РодительскаяСтруктураТипа.ИмяОбщегоТипа = "Массив" 
			Или РодительскаяСтруктураТипа.ИмяОбщегоТипа = "ФиксированныйМассив")
	Тогда
		ТаблицаТипов = ДобавитьВТаблицуТипов(, СтруктураТипаИзЗначения(МетаданныеКоллекции[0]));
	Иначе 
		...
		Если ТипыЭлементовКоллекции.Количество() > 0 Тогда
			...
			Для Каждого БазовыйТипЭлемента Из ТипыЭлементовКоллекции Цикл
				...
				Если ТипЗнч(МетаданныеКоллекции) = Тип("ПостроительЗапроса") Тогда 
					...
					КонецЕсли;
				КонецЕсли;
				ТаблицаТипов = ДобавитьВТаблицуТипов(, СтруктураТипаЭлемента);
			КонецЦикла;
		Иначе
			ТаблицаТипов = ДобавитьВТаблицуТипов(, НоваяСтруктураТипа());
		КонецЕсли; 
	КонецЕсли;
			...
			Если МодульМетаданных <> Неопределено И СтарыйМодуль <> МодульМетаданных Тогда
				...
				Тогда
					...
					Для Каждого СтрокаСлова Из ТаблицаСлов Цикл // TODO добавить фильтр по типу 
						Если СтрокаСлова.ТаблицаТипов[0].Детальность > 2 Тогда
							Продолжить;
						КонецЕсли;
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Тогда
		...
		Иначе
			...
			ПозицияКонцаСтарая = ирОбщий.СтрНайтиЛкс(СтарыйМодуль.Текст, Символы.ПС,, Мин(ТекущаяПозиция, СтрДлина(СтарыйМодуль.Текст)));
		КонецЕсли;
		...
		Иначе
			...
			Пока УзелDOM <> Неопределено Цикл
				...
				Если ИмяРеквизита <> Неопределено Тогда
					...
					Если УзелDOM.ПервыйДочерний <> Неопределено Тогда // Иначе нашли почему то "root" + ПутьКРодителю
						Типы = Новый Массив;
...
мМассивТиповВключающихМетаданные.Добавить(Тип("ТекстовыйДокумент"));
