...
Перем мОчередьСообщенийВнешнемуКлиенту Экспорт; // Массив, Неопределено - для накопления важных сообщений в режиме Automation-сервера
...
Функция ДобавитьВТаблицуСлов(Знач ТаблицаСлов, Знач Слово, Знач ТипСлова = "Свойство", Знач ТаблицаТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "", Знач Вытеснять = Истина,
	...
	Если Ложь
		Или (Истина
			И ТипСлова <> "Метод" // Для ускорения, т.к. платформа не позволяет перегружать методы
			И ТипСлова <> "Конструкция")
	...
	Тогда
		...
		Иначе
			Если выхФормаВыбора.Открыта() Тогда
				выхФормаВыбора.ОбновитьРезультатыПоиска();
			Иначе
				выхФормаВыбора.Открыть();
			КонецЕсли;
		КонецЕсли; 
...
Функция ПассивнаяФормаПоИмениКэш(Знач ИмяФормы) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если Форма = Неопределено Тогда
			Сообщение = "Анализатор кода не видит полное содержимое формы из-за недоступности ее создания " + ИмяФормы + ": " + ОписаниеОшибки;
			ДобавитьСообщениеВнешнемуКлиенту(Сообщение);
...
Процедура ДобавитьСообщениеВнешнемуКлиенту(Текст) Экспорт
	Если мОчередьСообщенийВнешнемуКлиенту <> Неопределено Тогда
		мОчередьСообщенийВнешнемуКлиенту.Добавить(Текст);
	КонецЕсли;
КонецПроцедуры
