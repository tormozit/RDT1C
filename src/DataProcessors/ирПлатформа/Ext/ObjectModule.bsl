...
Функция МодульМетаданных(Знач ТекстМодуля, Знач ПодробныйРазборТолькоЭкспортных = Истина, Знач СохранятьОпределения = Ложь) Экспорт
	...
	ДлинаРазделаПеременных = СтрДлина(Результат.Переменные);
	RegExp.Multiline = Ложь;
	...
	RegExp.MultiLine = Истина; // Умышленно
	RegExp.Global = Истина;
	...
	Вхождения = RegExp.НайтиВхождения(" " + ТекстМетоды);
	//СмещениеПозиции = 1 - 1; // -1 за вставку пробела в начало текста и +1 для отсчета с 1 вместо 0
	#Если Сервер И Не Сервер Тогда
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			...
			СтрокаМетода.ПозицияОпределения = ДлинаРазделаПеременных + Макс(1, Вхождение.FirstIndex) + СтрДлина(ОписаниеМетода);
