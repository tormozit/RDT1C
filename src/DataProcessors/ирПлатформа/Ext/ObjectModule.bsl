...
Процедура СброситьКэшТиповВыраженийМодуля(Знач Модуль, Знач СбросВМетодах = Истина) Экспорт
	...
	Если СбросВМетодах Тогда
		...
		Методы.ЗаполнитьЗначения(, "Вычислено, ТаблицаТипов, ТипыВыражений"); // Чтобы не отслеживать зависимости. ТипыВыражений - самые тяжелые 
		_РежимОтладки = Ложь;
		Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1) 
			// Пассивный оригинал расположенного ниже однострочного кода. Выполняйте изменения синхронно в обоих вариантах. 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Если ЛиНачальныйДиапазонСовпал Тогда
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			Если ТекущийМетод <> Неопределено Тогда
				ТипыВыраженийТекущегоМетода = ТекущийМетод.ТипыВыражений;
			КонецЕсли;
			...
			Иначе
				...
				//ТекущийМетод.Вычислено = Неопределено;
				//ТекущийМетод.ТаблицаТипов = Неопределено;
				//Если ТипЗнч(ТекущийМетод.Параметры) = Тип("ТаблицаЗначений") Тогда    
				//	ТекущийМетод.Параметры.ЗаполнитьЗначения(, "Вычислено, ТаблицаТипов");
				//КонецЕсли;
				Если ТипыВыраженийТекущегоМетода <> Неопределено Тогда
					// Узкая оптимизация для ускорения расчета ожидаемого типа при вводе "=" после имени переменной. Может влиять на свежесть расчета в рекурсивных функциях
					ПозицияВТелеМетода = ирОбщий.СтрНайтиЛкс(ТекстМодуля, "=", Истина, ТекущаяПозиция) - ТекущийМетод.ПозицияТела;
					ДлинаСверяемойЧасти = ТекущийМетод.ПозицияТела - ТекущийМетод.ПозицияСОписанием + ПозицияВТелеМетода - 1;
