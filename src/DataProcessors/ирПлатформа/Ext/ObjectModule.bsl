			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					ИначеЕсли Истина
						...
						И ИмяОбщегоТипаРодителя = "ТекстовыйДокумент"
					...
					Тогда
						...
						Прервать;
					ИначеЕсли Истина
						И ИмяОбщегоТипаРодителя = "Запрос"
						И ИмяОбщегоТипаСлова = "МенеджерВременныхТаблиц"
					Тогда 
						МенеджерВременныхТаблиц = РодительскаяСтруктураТипа.ДержательМетаданных;
						Если ТипЗнч(МенеджерВременныхТаблиц) = Тип("Структура") И МенеджерВременныхТаблиц.Активен Тогда
					...
					ИначеЕсли Истина
						И СтрокаСлова.ТипСлова = "Метод"
						И ИмяОбщегоТипаРодителя = "Массив[РезультатЗапроса]"
						И ТипЗнч(МетаданныеРодителя) = Тип("ПостроительЗапроса")
						И (Ложь
					...
					Тогда 
						...
						СтруктураТипа.Метаданные = ПустаяТаблицаЗначенийИзПостроителя(СтрокаОбщегоТипа.Метаданные);
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено, ТекущийМетод = Неопределено,
	...
	ВерсияФормата = 8; // Метка91237881041 Каждый раз при изменении структуры увеличиваем на 1
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Тогда
		...
		Иначе
			...
			СтараяДлина = СтрДлина(СтарыйМодуль.Текст);
			ПозицияНачалаСтарая = ирОбщий.СтрНайтиЛкс(СтарыйМодуль.Текст, Символы.ПС, Истина, Мин(Макс(1, ТекущаяПозиция - 1), СтараяДлина));  
			ПозицияКонцаСтарая  = ирОбщий.СтрНайтиЛкс(СтарыйМодуль.Текст, Символы.ПС,       , Мин(ТекущаяПозиция,     СтараяДлина));
		КонецЕсли;
