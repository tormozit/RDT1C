...
//  СловоФильтр       - Строка, Неопределено - для отбора, для кэширования должно быть Неопределено
//  ТипСловаФильтр    - Строка, Неопределено - для отбора
...
//  см. НоваяТаблицаСлов()
//
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Тогда
			...
			ИспользоватьКэширование = СловоФильтр = Неопределено;
		...
		Если ИспользоватьКэширование Тогда
			...
			Тогда
				...
				КлючКэширования = ИмяОбщегоТипаРодителяБезДеталей + ";" + ЯзыкПрограммы + ";" + Конфигурация + ";" + ВычислятьТипы + ";" + ТипСловаФильтр + ";" + НаСервере + ";" + КлиентОбычноеПриложение + ";" 
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			ЛиТипСКвалификаторами = Истина
				И (Ложь
					Или ТипСловаЦикл = "Свойство"
					Или ТипСловаЦикл = "Поле")
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Если Истина
						...
						И ТипЗнч(МетаданныеРодителя) <> Тип("Структура")
					Тогда
						СтруктураТипа.Метаданные = НовыйИмитаторЭлементаФормы();
					...
					Тогда
						...
						Если СоздатьИмитатор Тогда
							СтруктураТипа.Метаданные = НовыйИмитаторЭлементаФормы();
							СтруктураТипа.Метаданные.Тип = РодительскаяСтруктураТипа.ИмяОбщегоТипа;
						Иначе
					...
					Тогда
						...
						Образователь = ПроверитьЦиклическиеСсылкиВстроенногоЯзыка(СтруктураЦиклическая); // Платформа создает колонки только при добавлении первой строки
			...
			КонецЕсли;
			ДобавитьВТаблицуСлов(ТаблицаСлов, СловоЦикл, ТипСловаЦикл, ТаблицаТипов, ТипЗначения, "Предопределенный");
		КонецЦикла;
		Если ИспользоватьКэширование И Не ЛиПолноеКэширование Тогда
			...
			СловаКонтекстаМетаданные(СтруктураТипаГлобальный,, ТипСловаФильтр,, ЯзыкПрограммы,,, ВычислятьТипы, ФлагиКомпиляции,, ТаблицаСлов,,,, БазовоеРасширениеКонфигурации);
...
// Параметры:
//  РодительскаяСтруктураТипа		 - см. НоваяСтруктураТипа() - 
//  СловоФильтр							 - Строка, Неопределено - для отбора;
//  ТипСловаФильтр						 - Строка, Неопределено - для отбора;
...
//  см. НоваяТаблицаСлов()
//
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
				...
				Иначе
					//
				КонецЕсли;
				ДобавитьВТаблицуСлов(ТаблицаСлов, КонечноеСлово, СтрокаСлова.ТипСлова, ТаблицаТипов, "??" + ТипЗначенияСлова, "Метаданные");
			КонецЦикла;
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	Возврат 21; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
КонецФункции
		...
		Иначе
			...
			Тогда
				Для Каждого Колонка Из ТаблицаИсточник.Колонки Цикл
					Если ирОбщий.СвойствоСтруктурыЛкс(СтруктураПриемник, Колонка.Имя) = Неопределено Тогда
