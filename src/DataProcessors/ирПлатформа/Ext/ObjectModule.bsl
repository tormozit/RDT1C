...
Функция МодульМетаданных(Знач ТекстМодуля, Знач ПодробныйРазборТолькоЭкспортных = Истина) Экспорт
	...
	Методы.Колонки.Добавить("ПозицияОпределения");
	Методы.Колонки.Добавить("ПозицияТела");
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			...
			СтрокаМетода.ПозицияОпределения = ДлинаРазделаПеременных + Вхождение.FirstIndex + СтрДлина(ОписаниеМетода);
			...
			СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияОпределения + СтрДлина(СтрокаМетода.Описание + Вхождение.SubMatches(1));
