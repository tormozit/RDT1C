...
Функция ЛитералыЗначенияВстроенногоЯзыка() Экспорт
	Список = Новый СписокЗначений;
...
Функция СоздаваемыеВременныеТаблицыПакетаЗапросов(Знач ТекстПакета, Знач ТолькоТребующиеУничтоженияНаВходе = Ложь, Знач ТекстыЗапросовПакета = Неопределено, Знач ВместеСУничтожениями = Ложь) Экспорт 

	#Если Сервер И Не Сервер Тогда
		RegExp2 = Обработки.ирОболочкаРегВыражение.Создать();
	#КонецЕсли
	СозданныеТаблицы = Новый Массив;
	Если ТекстыЗапросовПакета = Неопределено Тогда
		Если Не ирКэш.ДоступныРегВыраженияЛкс() Тогда
			Если ирКэш.ДоступноСхемаЗапросаЛкс() Тогда
				Схема = Вычислить("Новый СхемаЗапроса");
				#Если Сервер И Не Сервер Тогда
			...
			Возврат СозданныеТаблицы;
		КонецЕсли;
		ТекстыЗапросовПакета = РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(ТекстПакета);
	КонецЕсли;
	...
	ШаблонПОМЕСТИТЬИЗ = "(?:" + шСтрокаЗапроса + ")|(?:" + шРазделитель + "(?:(ПОМЕСТИТЬ|INTO\s+TABLE|INTO)|(УНИЧТОЖИТЬ|DROP|DROP\s+TABLE))" + шРазделитель 
		...
		+ "+(" + шИмяВременнойТаблицы + ")(" + шРазделитель + "|;|')|\s)|.+?";
...
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	Знач Конфигурация = Неопределено, Знач ВычислятьТипы = Истина, Знач ФлагиКомпиляции = Неопределено, Знач ТаблицаСлов = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь) Экспорт 
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если ИспользоватьКэширование Тогда
			...
			КлючКэширования = "" + ЯзыкПрограммы + ";" + Конфигурация + ";" + ВычислятьТипы + ";" + НаСервере + ";" + КлиентОбычноеПриложение + ";" + КлиентУправляемоеПриложение + ";" + РодительскаяСтруктураТипа.ТипЯзыка;
			...
			Если ТаблицаСловИзКэша <> Неопределено Тогда
				...
				КонецЕсли; 
				ТаблицаСлов = ТаблицаСловИзКэша.Скопировать(ОтборСлов);
				Продолжить;
			КонецЕсли; 
		КонецЕсли; 
		Если ТаблицаСлов = Неопределено Тогда
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если ВычислятьТипыЦикл Тогда
				...
				Для Каждого ИмяОбщегоТипаСлова Из МассивТиповЗначения Цикл
					...
					Тогда
						ТаблицаСтруктурТипов = ТаблицаТиповЭлементовКоллекции(РодительскаяСтруктураТипа,, ЯзыкПрограммы);
						Прервать;
					ИначеЕсли Ложь
						Или (Истина
							И СтрокаСлова.ТипСлова = "Метод"
						...
						Или (Истина      
							...
							И (Ложь
								Или (Истина
									И ТипЗнч(СтрокаОбщегоТипа.Метаданные) = Тип("ТаблицаФормы") 
									И (Ложь
										Или СтрокаСлова.Слово = "ТекущиеДанные"
										Или СтрокаСлова.Слово = "ОформленияЯчеекДинамическогоСписка"))
								Или (Истина
									И ТипЗнч(СтрокаОбщегоТипа.Метаданные) = Тип("ТабличноеПоле") 
									И (Ложь
					...
					Тогда 
						...
						Если ДанныеЭлементаФормы <> Неопределено Тогда
							Если ТипЗнч(ДанныеЭлементаФормы) = Тип("НастройкиКомпоновкиДанных") Тогда
								ТаблицаСтруктурТипов = ТаблицаТиповЭлементовКоллекции(СтруктураТипаИзЗначения(ДанныеЭлементаФормы.Структура));
							Иначе
								ТаблицаСтруктурТипов = ТаблицаТиповЭлементовКоллекции(СтруктураТипаИзЗначения(ДанныеЭлементаФормы));
								Если ТаблицаСтруктурТипов[0].ИмяОбщегоТипа = "Неизвестный контекст" Тогда                           
									// Деревья
									#Если Клиент Тогда
									ТаблицаЗначений = ирКлиент.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(МетаданныеРодителя, Новый Массив);
									ТаблицаСтруктурТипов = ТаблицаТиповЭлементовКоллекции(СтруктураТипаИзЗначения(ТаблицаЗначений));
									#КонецЕсли
								КонецЕсли;
							КонецЕсли;
							ТаблицаСтруктурТипов.ЗаполнитьЗначения(СтрокаСлова, "СтрокаОписания");
							Прервать;
						КонецЕсли; 
					КонецЕсли;
					...
					Если КорневойТипРодителя <> Неопределено Тогда 
						...
						ИначеЕсли Ложь
							Или (Истина
								И ИмяОбщегоТипаСлова = "КонстантаМенеджер.<Имя константы>" 
								И СтрокаСлова.Слово = "Получить")
							Или (Истина
								И ИмяОбщегоТипаСлова = "КонстантаМенеджерЗначения.<Имя константы>" 
								И СтрокаСлова.Слово = "Значение")
						Тогда
			...
			КонецЕсли;
			ДобавитьВТаблицуСлов(ТаблицаСлов, СтрокаСлова.Слово, СтрокаСлова.ТипСлова, ТаблицаСтруктурТипов, ТипЗначения, "Предопределенный");
		КонецЦикла;
		Если ИспользоватьКэширование Тогда
			...
			мКэшСловГлобальногоКонтекста[КлючКэширования] = ТаблицаСлов.Скопировать();
		КонецЕсли; 
	КонецЦикла;
	Возврат ТаблицаСлов;
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач _ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Тогда
		...
		Если Не ЛиИндексИдентификатор Тогда
			ТаблицаСтруктурТипов = ТаблицаТиповЭлементовКоллекции(РодительскаяСтруктураТипа, Слово, ЯзыкПрограммы);
			ДобавитьВТаблицуСлов(ТаблицаСлов, "*", "Свойство", ТаблицаСтруктурТипов,, "Метаданные1");
			Возврат ТаблицаСлов;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
			...
			Если ТипЗнч(СтрокаОбщегоТипа.Метаданные) = Тип("COMОбъект") Тогда
				...
				Для Каждого КлючИЗначение Из СтруктураЦикла Цикл
					...
					Для Каждого Свойство Из КоллекцияСвойств Цикл
						...
						СтруктураТипа.Метаданные = РодительскаяСтруктураТипа.Метаданные;
						ДобавитьВТаблицуСлов(ТаблицаСлов, ИмяСвойства, лТипСлова, ТаблицаСтруктурТипов,, "Метаданные");
					КонецЦикла;
				КонецЦикла; 
			КонецЕсли; 
		КонецЕсли;
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			Тогда
				...
				Если Найти(ИмяОбщегоТипаРодителя, "Колон") = 0 Тогда
				...
				Если ЗначениеЗаполнено(Слово) Тогда
					Если ТипЗнч(МетаданныеРодителя) = Тип("Структура") Тогда
						...
						Если СтруктураИмен.Свойство(Слово) Тогда
							СхемаКоллекции.Вставить(Слово, СтруктураИмен[Слово]);
						КонецЕсли;
					Иначе
			...
			ИначеЕсли Истина
				И СтрокаОписанияВида.ИмяКоллекции = "Реквизиты" 
				И ТипЗнч(МетаданныеРодителя) = ирОбщий.ТипУправляемаяФормаЛкс()
			Тогда 
				// 
			Иначе
			...
			Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
					...
					Иначе
						...
						Тогда
							...
							Для Каждого Тип Из МассивТипов Цикл
								...
								Иначе
									Если ТипЗнч(ЗначениеСвойства) = Тип("ОбъектМетаданных") Тогда
							...
							КонецЦикла;
							ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, МассивНакопления);
						Иначе
				...
				Иначе
					//
				КонецЕсли; 
				ДобавитьВТаблицуСлов(ТаблицаСлов, КонечноеСлово, СтрокаВида.ТипСлова, ТаблицаСтруктурТипов,, "Метаданные");
			КонецЦикла;
		КонецЦикла;
		// Модуль
		Если Истина
					...
					Если ИнфоТипа <> Неопределено Тогда
						...
						Для Каждого ИнфоТипа Из МассивИнфоТипа Цикл
							Для Каждого Член Из ИнфоТипа.Members Цикл
								...
								ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
								ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипа);
								ДобавитьВТаблицуСлов(ТаблицаСлов, ИмяЧлена, лТипСлова, ТаблицаСтруктурТипов,, "Метаданные");
							КонецЦикла;
						КонецЦикла;
					КонецЕсли; 
				КонецЕсли; 
	...
	Тогда
		...
		Тогда
			...
			ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
			ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипа);
			ДобавитьВТаблицуСлов(ТаблицаСлов, СтрокаВида.Слово, СтрокаВида.ТипСлова, ТаблицаСтруктурТипов,, "Метаданные");
		КонецЕсли;
	КонецЕсли;
	Возврат ТаблицаСлов;
	...
	Если ТипыЭлементовКоллекции.Количество() > 0 Тогда
		Для Каждого БазовыйТипЭлемента Из ТипыЭлементовКоллекции Цикл
			...
			Если ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса") Тогда 
				...
				Попытка
					ИндексЭлемента = Число(ТекстИндексаЭлемента);
				Исключение
			...
			КонецЕсли;
			ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипаЭлемента);
		КонецЦикла;
	Иначе
		ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, НоваяСтруктураТипа());
	КонецЕсли;
	...
	Тогда
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			Если ВычислятьТипы Тогда
				...
				КонецЕсли;
				ТаблицаСтруктурТипов = ТаблицаСтруктурТиповСловаМодуля(ПеременнаяМодуля, МодульМетаданных);
			КонецЕсли;
			ДобавитьВТаблицуСлов(ВнутренняяТаблицаСлов, ПеременнаяМодуля.Имя, "Свойство", ТаблицаСтруктурТипов,, "Метаданные");
		КонецЦикла;
	КонецЕсли; 
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если ВычислятьТипы Тогда
				...
				КонецЕсли;
				ТаблицаСтруктурТипов = ТаблицаСтруктурТиповСловаМодуля(МетодМодуля, МодульМетаданных);
			КонецЕсли;
...
// Возвращаемое значение:
//    ТаблицаЗначений - 
Функция ТаблицаСтруктурТиповСловаМодуля(СловоМодуля, МодульМетаданных) Экспорт
	Если СловоМодуля.ТаблицаСтруктурТипов <> Неопределено Тогда
		ТаблицаСтруктурТипов = СловоМодуля.ТаблицаСтруктурТипов.Скопировать(); // Мультиметка4529884 против циклической ссылки
		#Если Сервер И Не Сервер Тогда
	...
	Иначе
		...
		КонецЕсли; 
	КонецЕсли; 
	ТаблицаСтруктурТипов.ЗаполнитьЗначения(СловоМодуля, "СтрокаОписания"); // Мультиметка4529884 против циклической ссылки
	СловоМодуля.ИмяМодуля = МодульМетаданных.Имя;
	Возврат ТаблицаСтруктурТипов; 
КонецФункции

Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено) Экспорт 
	
	ВерсияФормата = 2; // Метка91237256881041 Каждый раз при изменении структуры увеличиваем на 1
    СтрокаСозданияРодителя = "";
	ФлагиКомпиляции = Неопределено;
	...
	Если ЗначениеЗаполнено(выхИмяФайлаМодуляБезРасширения) Тогда
		...
		КонецЕсли; 
		МодульМетаданных = МодульМетаданныхИзКэша(выхИмяФайлаМодуляБезРасширения);
		Если Ложь
		...
		Тогда
			...
			Если НовыйТекст <> Неопределено Тогда
				...
				Если СтарыйТекст <> НовыйТекст Тогда
					...
					СоздатьКаталог(ФайлМодуля.Путь); // Нужно для подкаталогов расширений
			...
			Если ОбновитьФайлОписания Тогда
				Если Не ФайлМодуля.Существует() Тогда
					...
					КонецЕсли; 
					МодульМетаданных = МодульМетаданных("", СтруктураТипа, ФайлМодуля.ИмяБезРасширения);
				Иначе 
					Если НовыйТекст = Неопределено Тогда
						ТекстовыйДокумент.Прочитать(ФайлМодуля.ПолноеИмя);
						МодульМетаданных = МодульМетаданных(ТекстовыйДокумент.ПолучитьТекст(), СтруктураТипа, ФайлМодуля.ИмяБезРасширения);
						МодульМетаданных.СтруктураТипа = Неопределено;
						ЗаписатьМодульВКэш(МодульМетаданных);
					Иначе
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, ФайлМодуля.ИмяБезРасширения, Ложь);
			...
			Если МодульМетаданных <> Неопределено Тогда
				...
				Если Ложь
					Или БазовыйТип = "ОбработкаОбъект" 
					Или БазовыйТип = "ОтчетОбъект" 
					Или БазовыйТип = "Форма" 
					Или БазовыйТип = "УправляемаяФорма" 
					Или БазовыйТип = "ФормаКлиентскогоПриложения" 
				Тогда
					// Добавим в переменные реквизиты объекта с изменяемыми метаданными, чтобы уточнять их тип по коду
					МодульВременный = МодульМетаданных("",, "кэш");
					Для Каждого СтрокаСлова Из ТаблицаСловИзСтруктурыТипа(СтруктураТипа,,,, Ложь,, "Свойство",,, МодульВременный) Цикл
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Ложь) Экспорт 
	Если МодульМетаданных <> мПустойМодуль Тогда
		...
		Если ЧерезКопию Тогда
			...
			КопияМодуля.Переменные.ЗаполнитьЗначения(Неопределено, "ТаблицаСтруктурТипов");
			КопияМодуля.Методы = КопияМодуля.Методы.Скопировать();
			КопияМодуля.Методы.ЗаполнитьЗначения(Ложь, "Вычислено");
			КопияМодуля.Методы.ЗаполнитьЗначения(Неопределено, "ТаблицаСтруктурТипов");
...
Функция ИмяФайлаМодуляБезРасширения(Знач СтруктураТипа, ТипМодуля = "", СтрокаСозданияРодителя = "", ФлагиКомпиляции = Неопределено) Экспорт 
	...
	Тогда 
		...
		КонецЕсли;
	ИначеЕсли Ложь
		Или ИмяОбщегоТипа = "ВнешняяОбработка"
		Или ИмяОбщегоТипа = "ВнешнийОтчет"
	Тогда
		МетаданныеРодителя = ТипМодуля;
...
Функция ОбновитьКэшСтруктурыФормыИзБуфера(СлужебныеДанные, ТекстБуфераОбмена) Экспорт 
	...
	Попытка
		XMLСтрока = ирОбщий.СтрокаВнутрВХМЛТелоЛкс(ТекстФайла); 
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		СлужебныеДанные.Вставить("ДатаОбновления", ТекущаяДата() + 365*24*60*60);
		// Обычная форма
		Возврат Ложь;
	КонецПопытки; 
	ДокументDOM = ирОбщий.ТекстВДокументDOMЛкс(XMLСтрока);
	...
	ИмитаторФормы = Новый Структура("ВерсияФормата, Элементы, Параметры, Команды, ДинамическиеСписки, ИменаОбработчиков, Реквизиты",
		ВерсияФорматаСтруктурыФормы(), Новый Структура, Новый Структура, Новый Структура, Новый Структура);
	ИмитаторФормы.ИменаОбработчиков = ИменаОбработчиковИзУзлаФормы(ДокументDOM, ДокументDOM.ПервыйДочерний.ДочерниеУзлы[1].ДочерниеУзлы[19], Разыменователь);
	...
	Пока УзелDOM <> Неопределено Цикл 
		...
		Для НомерРеквизита = 1 По КоличествоРеквизитовКоллекции Цикл
			...
			ОписаниеРеквизита.Тип = ТипЗначенияИзУзлаФормы(УзелДочерний.СледующийСоседний.СледующийСоседний);
		КонецЦикла;
		Тип = ТипЗначенияИзУзлаФормы(УзелDOM.РодительскийУзел.РодительскийУзел.ДочерниеУзлы[5]);
		Если Тип = Новый ОписаниеТипов("ДинамическийСписок") Тогда
	...
	//ВсеТипыЭлементов.Вставить("?", Тип("КнопкаФормы"));
	//ВсеТипыЭлементов.Вставить("?", Тип("ДекорацияФормы"));
	...
	ДополнитьСлужебныеДанныеИзКэшаФормы(ИмитаторФормы, СлужебныеДанные, ТекущаяДата());
КонецФункции

Функция ВерсияФорматаСтруктурыФормы()
	Возврат 1; // Каждый раз при изменении структуры увеличиваем на 1
КонецФункции

Процедура ДополнитьСлужебныеДанныеИзКэшаФормы(Знач ИмитаторФормы, СлужебныеДанные, Знач ДатаОбновления) Экспорт 
...
Функция ТипЗначенияИзУзлаФормы(УзелТипа)
	...
	Если УзелТипа.ДочерниеУзлы.Количество() > 1 Тогда
		...
		Если РодТипа.ТекстовоеСодержимое = """#""" Тогда
...
//  Структура - 
//
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина) Экспорт
	...
	Переменные.Колонки.Добавить("ТаблицаСтруктурТипов"); 
	...
	Для Каждого Вхождение Из Модуль.ВхожденияМетодов Цикл
...
Функция ЗаменитьВозвратыНаПерейтиВТекстеМетода(Знач Текст, Знач ИмяПеременнойРезультата = "Результат", Знач ДобавлятьПереносыСтрок = Ложь, Знач ИмяМетки = "Конец", Знач ВставитьМетку = Ложь) Экспорт
	...
	Иначе
		Разделитель = " ";
	КонецЕсли;
	Текст = РегВыражение.Заменить(Текст, "$1" + ИмяПеременнойРезультата + " = $2;" + Разделитель + "Перейти ~" + ИмяМетки +"$3");
	РегВыражение.Pattern = "((?:^|\n|\r)(?:\t| )*)Возврат(?:\t| )*(\n|\r|;)";
...
Функция ПодготовитьТипРезультатаМетода(СтрокаОпределения, МодульМетаданных, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено) Экспорт 
	
	Если Не СтрокаОпределения.Вычислено Тогда
		Если СтрокаОпределения.ТаблицаСтруктурТипов = Неопределено Тогда
			СтрокаОпределения.ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
		КонецЕсли;
		...
		ИмяПеременной = "хй07003433";
		ТекстДляПоискаОпределения = ЗаменитьВозвратыНаПерейтиВТекстеМетода(ТекстДляПоискаОпределения, ИмяПеременной);
		ПодготовитьТипЗначенияСловаМодуля(ИмяПеременной, кэшКомпонентаПодсказки, СтрокаОпределения, МодульМетаданных, СтрокаОпределения, СтрокаОпределения.ОписаниеРезультата, ТекстДляПоискаОпределения);
	КонецЕсли;
...
Функция ПодготовитьТипЗначенияПеременнойМодуля(СтрокаОпределения, МодульМетаданных, Знач ПолеТекстаМодуля = Неопределено) Экспорт 
	...
	Если Не СтрокаОпределения.Вычислено Тогда
		...
		Если ПолеТекстаМодуля = Неопределено Тогда
			ПолеТекстаМодуля = ирКэш.ПолеТекстаМодуляЛкс(МодульМетаданных.Имя);
		КонецЕсли;
		СтрокаОпределения.Вычислено = Истина; // ставим сразу для защиты от бесконечной рекурсии
		ПодготовитьТипЗначенияСловаМодуля(СтрокаОпределения.Имя, ПолеТекстаМодуля,, МодульМетаданных, СтрокаОпределения, СтрокаОпределения.Описание);
	КонецЕсли;

КонецФункции

Процедура ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач МодульМетаданных, Знач СтрокаОпределения, Знач ОписаниеТипа = "", Знач ТекстДляПоискаОпределения = "")
	
	Если кэшКомпонентаПодсказки = Неопределено Тогда
		кэшКомпонентаПодсказки = ирОбщий.НовыйАнализаторКодаЛкс();
		#Если Сервер И Не Сервер Тогда
			кэшКомпонентаПодсказки = Обработки.ирКлсПолеТекстаПрограммы.Создать();
		#КонецЕсли
		кэшКомпонентаПодсказки.мПлатформа = ЭтотОбъект;
		кэшКомпонентаПодсказки.ИнициироватьНеинтерактивно();
	КонецЕсли; 
	...
	ТаблицаСтруктурТипов = кэшКомпонентаПодсказки.ТаблицаСтруктурТиповИзТекста(СтрокаОпределения.ТипЗначения, ОписаниеТипа);
	ДобавитьВТаблицуСтруктурТипов(СтрокаОпределения.ТаблицаСтруктурТипов, ТаблицаСтруктурТипов);
	...
	Тогда
		ТекстДляПоискаОпределения = кэшКомпонентаПодсказки.ЗалитьКомментарииИСтроковыеЛитералы(ТекстДляПоискаОпределения); // Вырезаем условия
		ТаблицаСтруктурТипов = кэшКомпонентаПодсказки.ВычислитьТипЗначенияВыражения(ИмяПеременной, ТекстДляПоискаОпределения, ТекстДляПоискаОпределения,,,,, МетодМодуля = Неопределено);
		Если ТаблицаСтруктурТипов.Количество() > 0 Тогда
			ДобавитьВТаблицуСтруктурТипов(СтрокаОпределения.ТаблицаСтруктурТипов, ТаблицаСтруктурТипов); 
		Иначе
			СтруктураТипа = НоваяСтруктураТипа();
			СтруктураТипа.ИмяОбщегоТипа = "Произвольный";
			ДобавитьВТаблицуСтруктурТипов(СтрокаОпределения.ТаблицаСтруктурТипов, СтруктураТипа); 
		КонецЕсли; 
		//СтрокаОпределения.ТаблицаСтруктурТипов.ЗаполнитьЗначения(СтрокаОпределения, "СтрокаОписания"); // Мультиметка4529884 Была циклическая ссылка ! Теперь заполняем ссылку позже уже в копии таблицы
	КонецЕсли;

	//ПредставлениеТиповИзКода = ПредставлениеМассиваСтруктурТипов(СтрокаОпределения.ТаблицаСтруктурТипов);
	// Обогащаем текстовое описание типа из таблицы структур типов 
	//Если Найти(СтрокаОпределения.ТипЗначения, "??") > 0 Тогда 
	//	Если СтрокаОпределения.ТипЗначения = "??" Тогда 
	//		СтрокаОпределения.ТипЗначения = ПредставлениеТиповИзКода;
	//	Иначе
	//		СтрокаОпределения.ТипЗначения = СтрЗаменить(СтрокаОпределения.ТипЗначения, "??", ПредставлениеТиповИзКода); // могут появиться дубли
	//	КонецЕсли;
	//Иначе
	//	СтрокаОпределения.ТипЗначения = ПредставлениеТиповИзКода;
	//КонецЕсли;
	СтрокаОпределения.ТипЗначения = ПредставлениеМассиваСтруктурТипов(СтрокаОпределения.ТаблицаСтруктурТипов);
...
//
Функция ДобавитьВТаблицуСлов(Знач ТаблицаСлов, Знач Слово, Знач ТипСлова, Знач ТаблицаСтруктурТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "") Экспорт
	...
	Если НайденныеСтроки.Количество() > 0 Тогда  
		Если ТаблицаСтруктурТипов <> Неопределено Тогда
			ДобавитьВТаблицуСтруктурТипов(НайденныеСтроки[0].ТаблицаСтруктурТипов, ТаблицаСтруктурТипов);
		КонецЕсли;
...
Функция ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ТаблицаСвойств = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь) Экспорт 

	ИнициацияОписанияМетодовИСвойств();
	ТаблицаСвойств = СловаКонтекстаПредопределенные(СтруктураТипа, Слово, ТипСлова,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ФлагиКомпиляции, ТаблицаСвойств, ЭтоЛокальныйКонтекст);
...
//
Процедура ДобавитьВТаблицуСтруктурТипов(ТаблицаПриемник, Знач ОписаниеТипов, Знач ПередаватьДополнительныеТипы = Ложь) Экспорт 
	
	Если ТаблицаПриемник = Неопределено Тогда
		ТаблицаПриемник = НоваяТаблицаСтруктурТипа();
	КонецЕсли;
	...
	КонецЕсли;
	НужноВытеснять = ТаблицаПриемник.Количество() > 0;
		...
		Если НужноВытеснять Тогда 
			...
			Если Найденные.Количество() > 0 Тогда 
				...
				Если Ложь
					...
					Или СтрокаПриемника.Детальность = СтрокаИсточника.Детальность И ТаблицаПриемник.Количество() > 10
				Тогда
					Продолжить;
				КонецЕсли;
...
Функция ОбновитьДетальностьСтруктурыТипа(СтруктураТипа) Экспорт 
	Тип = ТипЗнч(СтруктураТипа.Метаданные); 
	ИмяОбщегоТипа = СтруктураТипа.ИмяОбщегоТипа;
	Если Ложь
		Или ИмяОбщегоТипа = "" // Вроде такого не должно быть
	...
	Тогда 
		Результат = 1;
	ИначеЕсли Тип = Тип("ОбъектМетаданныхКонфигурация") Тогда
		Результат = 2;
	ИначеЕсли Тип = Тип("ОбъектМетаданных") Тогда
		Результат = 3;
	ИначеЕсли Ложь
		Или Тип = Тип("Форма") 
		Или Тип = Тип("УправляемаяФорма")
	Тогда
		Результат = 5;
	ИначеЕсли Тип = Тип("ПостроительЗапроса") Тогда
		Результат = 6;
	Иначе
		Результат = 4;
	КонецЕсли; 
...
Функция НайтиИмяМетодаСтрокиМодуля(Знач ИмяМодуля, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулей = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "", Знач ЧислоСтрокДляПоискаИмениМетода = 1000) Экспорт 
...
Функция НайтиИмяМетодаСтрокиФайлаМодуля(Знач ИмяФайлаИлиПолеТекстаМодуля, Знач НомерСтроки, Знач СодержаниеСтроки = Неопределено, ТекстыМодулей = Неопределено, СтароеИмяФайлаМодуляБезРасширения = "",
	...
	Иначе
		ПолеТекстаМодуля = ирКлиент.ПолеТекстаМодуляЛкс(ИмяФайлаИлиПолеТекстаМодуля);
	КонецЕсли; 
	Если ПолеТекстаМодуля = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПассивнаяФормаПоИмениКэш(Знач ИмяФормы) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если ФайлФормы.Существует() Тогда 
			ОписаниеФормы = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлФормы.ПолноеИмя); // Структура
			Если ОписаниеФормы.ВерсияФормата = ВерсияФорматаСтруктурыФормы() Тогда
				ирКэш.Получить().ДополнитьСлужебныеДанныеИзКэшаФормы(ОписаниеФормы, СлужебныеДанные, ФайлФормы.ПолучитьВремяИзменения());
			КонецЕсли; 
		ИначеЕсли Результат = СлужебныеДанные Тогда 
			СлужебныеДанные.Вставить("Тип", ирОбщий.ТипУправляемаяФормаЛкс());
		КонецЕсли; 
...
//   - 
//
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM, Знач ПутьКРодителю = Неопределено, Знач ЛиКоллекция = Ложь) Экспорт
	...
	Если ПутьКРодителю <> Неопределено Тогда
		...
		Если ТипЗнч(Форма) = ТипУправляемаяФорма Тогда
			...
			Если ПутьКРодителю = "" И ИменаРеквизитов = Неопределено Тогда 
				...
				Если ТипЗнч(ФормаИлиДокументDOM) = Тип("Структура") Тогда
					КандидатыРеквизитов = ФормаИлиДокументDOM.Реквизиты.ВыгрузитьКолонку("Имя");
				Иначе 
					КандидатыРеквизитов = ирОбщий.ЧастыеИменаРеквизитовОбъектаВФормеЛкс();
				КонецЕсли;
				...
				Пока Истина Цикл
					...
					Если ЛюбыеДанныеФормы = Неопределено Тогда
						...
						#КонецЕсли
						ТаблицаСтруктурТипов = НоваяТаблицаСтруктурТипа();
						ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипаИзКонкретногоТипа(ирОбщий.ТипУправляемаяФормаЛкс()));
						ПолеТекстаМодуля.ЗаполнитьТаблицуСлов(ТаблицаСтруктурТипов,,, Ложь,, Ложь);
						ПолеТекстаМодуля.ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(,,,,,,, Истина);
						...
						КандидатыРеквизитов = ПолеТекстаМодуля.ТаблицаСлов.Выгрузить(Новый Структура("Определение", "Статистический")).ВыгрузитьКолонку("НСлово");
						ВредныйКандидат = КандидатыРеквизитов.Найти(НРег("ЭтаФорма"));
						Если ВредныйКандидат <> Неопределено Тогда
							КандидатыРеквизитов.Удалить(ВредныйКандидат);
						КонецЕсли;
						ВредныйКандидат = КандидатыРеквизитов.Найти(НРег("ЭтотОбъект"));
						Если ВредныйКандидат <> Неопределено Тогда
							КандидатыРеквизитов.Удалить(ВредныйКандидат);
						КонецЕсли;
					КонецЕсли;
		...
		Если ДокументДОМ = Неопределено Тогда  
			...
			Тогда
				...
				Если УспехРеквизита Тогда
					Для Каждого СтрокаРеквизита Из ТаблицаРеквизитов Цикл
						ИменаРеквизитов.Вставить(СтрокаРеквизита.Имя, СтрокаРеквизита.Тип);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		Иначе
			...
			Пока УзелDOM <> Неопределено Цикл
				...
				Если ИмяРеквизита <> Неопределено Тогда
					...
					Если УзелТипаЗначения <> Неопределено Тогда
						Для Каждого ИДТипа Из ирОбщий.СтрРазделитьЛкс(УзелТипаЗначения.ТекстовоеСодержимое, " ") Цикл
							Попытка
								Тип = ЗначениеИзСтрокиВнутр("{""T""," + ИДТипа + "}");
							Исключение
								Тип = Неопределено;
...
мТаблицаСтруктурТипа.Колонки.Добавить("Детальность");
