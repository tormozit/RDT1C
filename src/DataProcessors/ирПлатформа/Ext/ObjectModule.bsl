	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			Тогда
				...
				Если Истина
					И ЗначениеЗаполнено(Слово) 
					И МетаданныеРодителя.Свойство(Слово)
					И ТипЗнч(МетаданныеРодителя[Слово]) = Тип("ТаблицаЗначений")
					И МетаданныеРодителя[Слово].Колонки.Найти(ИмяКолонкиФлагаТаблицыТипов) <> Неопределено
					И ТипЗнч(МетаданныеРодителя[Слово][0].СтрокаОписания) = Тип("Структура")
				Тогда
					ОписаниеСвойства = МетаданныеРодителя[Слово][0].СтрокаОписания; // см. ОбработкаОбъект.ирКлсПолеТекстаПрограммы.НовоеОписаниеДинамическогоСвойства
					ОписаниеСвойства = ирОбщий.СкопироватьКоллекциюЛкс(ОписаниеСвойства);
					ОписаниеСвойства.СтрокаОписания = СтрокаВида;
					КоллекцияЗначений = ирОбщий.СкопироватьКоллекциюЛкс(КоллекцияЗначений);
					ТаблицаТиповСлова = КоллекцияЗначений[Слово].Скопировать();
					ТаблицаТиповСлова.ЗаполнитьЗначения(ОписаниеСвойства, "СтрокаОписания");
					КоллекцияЗначений[Слово] = ТаблицаТиповСлова;
				КонецЕсли;
			ИначеЕсли Ложь
				Или (Истина
					И (Ложь
			...
			Тогда
				Если ТипЗнч(МетаданныеРодителя) = Тип("Структура") Тогда 
					...
					Если Истина
						И ЗначениеЗаполнено(Слово) 
						И МетаданныеРодителя.Свойство(Слово)
						И МетаданныеРодителя[Слово] <> Неопределено
						И ТипЗнч(МетаданныеРодителя[Слово][0].СтрокаОписания) = Тип("Структура")
					Тогда
						ОписаниеСвойства = МетаданныеРодителя[Слово][0].СтрокаОписания; // см. ОбработкаОбъект.ирКлсПолеТекстаПрограммы.НовоеОписаниеДинамическогоСвойства
						ОписаниеСвойства = ирОбщий.СкопироватьКоллекциюЛкс(ОписаниеСвойства);
					...
					Тогда 
						...
						Если ОписаниеСвойства <> Неопределено Тогда
							...
							ТаблицаТиповСлова.ЗаполнитьЗначения(ОписаниеСвойства, "СтрокаОписания");
							СхемаКоллекции[Слово] = ТаблицаТиповСлова;
						КонецЕсли;
					Иначе
						КоллекцияЗначений = МетаданныеРодителя;
						Если ОписаниеСвойства <> Неопределено Тогда
