						...
						Если ЗначениеСвойства <> Неопределено Тогда
							...
							Тогда 
								...
								Если Ложь
									...
									Или ТипЗнч(МетаданныеРодителя) = Тип("Структура") И ирОбщий.СвойствоСтруктурыЛкс(МетаданныеРодителя, "Тип") = ирОбщий.ТипУправляемаяФормаЛкс()
								Тогда
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач _ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Знач ТаблицаСлов = Неопределено, Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ЭтоЛокальныйКонтекст = Ложь, Знач ЛиВместеСЛокальнымКонтекстом = Ложь) Экспорт 
	...
	Если Истина
		...
		И РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "Структура" 
		И РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "ФиксированнаяСтруктура"
	Тогда
			...
			Тогда
				...
				Иначе
					...
					Если Истина
						...
						И ДочерниеЭлементы.Найти(Слово) = Неопределено 
					Тогда
						ИмитаторФормы = ирОбщий.СлужебныеДанныеФормыЛкс(МетаданныеРодителя);
					КонецЕсли;
					Если ТипЗнч(ИмитаторФормы) = Тип("Структура") Тогда
						...
						КонецЦикла;       
						Если ЗначениеЗаполнено(Слово) И СхемаКоллекции[Слово] = Неопределено Тогда
	...
	Тогда
		...
		Если Найти(СтрокаВида.Слово, "<Имя предопределенного") = 0 Тогда // Иначе при вычислении результата метода модуля менеджера все локальные переменные будут срабатывать
			СтруктураТипа = НоваяСтруктураТипа();
	...
	Если ЛиНачальныйДиапазонСовпал Тогда
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			...
			Для ИндексМетодаЦикл = ИндексТекущегоМетода + 1 По Методы.Количество() - 1 Цикл
				...
				Если СтрокаМетода.ПозицияСОписанием = Неопределено Тогда
...
Функция ДочерниеСвойстваДанныхФормы(ДанныеФормы, Знач ЛиКоллекция = Неопределено, Знач Форма, Знач ИмяСвойства = "") Экспорт
	Если ЛиКоллекция = Неопределено Тогда
		ЛиКоллекция = ирОбщий.ЛиКоллекцияЛкс(ДанныеФормы);
	КонецЕсли;
	...
	Если Найти(ПутьКРодителю, "root.") = 1 Тогда
		...
		СтруктураИмен = ИменаРеквизитовФормы(Форма, ПутьКРодителю, ЛиКоллекция,, ДокументДОМ).Все;
...
//   - 
//
Функция ИменаРеквизитовФормы(Знач ФормаИлиДокументDOM, Знач ПутьКРодителю = Неопределено, Знач ЛиКоллекция = Ложь, выхИдентификаторыРеквизитов = Неопределено, ДокументДОМ = Неопределено) Экспорт
	...
	Если ПутьКРодителю <> Неопределено Тогда
			...
			Пока Истина Цикл
				Для Каждого ИмяРеквизита Из КандидатыИменРеквизитов Цикл
					...
					Если Истина
						...
						И мМассивТиповВключающихМетаданные.Найти(ТипЗнч(ЗначениеРеквизита)) <> Неопределено 
					Тогда
				...
				Для Каждого ЭлементФормы Из Форма.Элементы Цикл
					...
					Тогда
						...
						Если мМассивТиповВключающихМетаданные.Найти(ТипЗначения) <> Неопределено Тогда  
		...
		Если СлужебныеДанные.Свойство("Реквизиты") Тогда
			ТаблицаРеквизитов = СлужебныеДанные.Реквизиты; // ТаблицаЗначений
...
мМассивТиповВключающихМетаданные.Добавить(Тип("КомандыФормы"));
#Если Клиент Тогда
