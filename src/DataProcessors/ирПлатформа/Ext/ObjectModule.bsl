			...
			Если РезультатПараметров.Количество() > 0 Тогда
				...
				Если ЗначениеЗаполнено(СтрокаМетода.Описание) Тогда
					...
					Если Не ОписанияПараметровЗагружены Тогда
						ОписаниеМетода = ирОбщий.ПервыйФрагментЛкс(СтрокаМетода.Описание, ЗаголовокРазделаВозвращаемоеЗначение()); // Иначе можно найти имя типа результата функции в качестве параметра
...
Функция СлитьМетаданныеСтруктурТипов(Знач ТаблицаПриемник, Знач СтрокаИсточника, Знач СтрокаПриемника) Экспорт
	...
	Иначе
		...
		ИначеЕсли Истина
			И ТипМетаданныхИсточника = Тип("ТаблицаЗначений")
			И СтрокаИсточника.Метаданные.Колонки.Найти(ИмяКолонкиФлагаТаблицыТипов) = Неопределено
