					...
					Тогда 
						...
						Если ДанныеЭлементаФормы <> Неопределено Тогда
							...
							Иначе
								...
								Если ТаблицаТипов[0].ИмяОбщегоТипа = "Неизвестный контекст" Тогда                           
									// Деревья
									#Если Клиент Тогда
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
				...
				Иначе
					//
				КонецЕсли; 
				ДобавитьВТаблицуСлов(ТаблицаСлов, КонечноеСлово, СтрокаВида.ТипСлова, ТаблицаТипов,, "Метаданные");
			КонецЦикла;
		КонецЦикла;
		// Модуль
		Если Истина
		...
		Для Каждого БазовыйТипЭлемента Из ТипыЭлементовКоллекции Цикл
			...
			Если ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса") Тогда 
				...
				Если ИндексЭлемента <> Неопределено Тогда
					...
					#КонецЕсли
					ПостроительЗапроса = ПостроительПакетаЗапросовДоИндекса(ТекстыЗапросов, ИндексЭлемента);
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "", Знач НовыйТекст = Неопределено, Знач ТекущийМетод = Неопределено) Экспорт 
	
	ВерсияФормата = 5; // Метка91237256881041 Каждый раз при изменении структуры увеличиваем на 1
		...
		Тогда
			...
			Если ОбновитьФайлОписания Тогда
				...
				Иначе 
					...
					Иначе
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, выхИмяФайлаМодуляБезРасширения, Ложь, МодульМетаданных, ТекущийМетод);
...
Функция ОбновитьКэшСтруктурыФормыИзБуфера(Знач СлужебныеДанные, Знач ТекстБуфераОбмена, Знач Форма = Неопределено) Экспорт 
	...
	Пока УзелDOM <> Неопределено Цикл 
		...
		Если КоличествоРеквизитовКоллекции > 0 Тогда
			Если Ложь
				Или Тип = Новый ОписаниеТипов("ТаблицаЗначений") 
				Или Тип = Новый ОписаниеТипов("ДеревоЗначений")
			Тогда
				ЗначениеРеквизита = Новый (Тип.Типы()[0]); // ТаблицаЗначений
			Иначе 
			...
			Для НомерРеквизита = 1 По КоличествоРеквизитовКоллекции Цикл
				...
				Если ТипЗнч(ЗначениеРеквизита) = Тип("Структура") Тогда
					ЗначениеРеквизита.Вставить(ОписаниеРеквизита.Имя, ОписаниеРеквизита.Тип);
				Иначе
					ЗначениеРеквизита.Колонки.Добавить(ОписаниеРеквизита.Имя, ОписаниеРеквизита.Тип);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Пока УзелDOM <> Неопределено Цикл 
		...
		Если ирОбщий.СтрНачинаетсяСЛкс(ИмяЭлемента, "Navigator") Тогда
...
Функция ВерсияФорматаСтруктурыФормы()
	Возврат 2; // Каждый раз при изменении структуры увеличиваем на 1
КонецФункции
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	Знач ТекущийМетод = Неопределено) Экспорт
	...
	Если ПустаяСтрока(ТекстМодуля) И ЗначениеЗаполнено(ИмяМодуля) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ОбновлятьТолькоТекущийМетод = Ложь;
	Если Истина
		И СтарыйМодуль <> Неопределено        
		И СтарыйМодуль.Свойство("Текст") //1
		И ТекущийМетод <> Неопределено
		И ирКэш.НомерВерсииПлатформыЛкс() >= 803006 // СтрНайти()
	Тогда
		ЧастьДоМетодаСтарая = Лев(СтарыйМодуль.Текст, ТекущийМетод.ПозицияСОписанием);
		ЧастьДоМетодаНовая = Лев(ТекстМодуля, ТекущийМетод.ПозицияСОписанием);
		Если ЧастьДоМетодаСтарая = ЧастьДоМетодаНовая Тогда
			КонецСтарый = ТекущийМетод.ПозицияСОписанием + ТекущийМетод.ДлинаСОписанием;
			ЧастьПослеМетодаСтарая = Сред(СтарыйМодуль.Текст, КонецСтарый);
			Если ТекущийМетод.ТипЗначения = Неопределено Тогда 
				МаркерКонца = Символы.ПС + "КонецПроцедуры";
			Иначе 
				МаркерКонца = Символы.ПС + "КонецФункции";
			КонецЕсли;
			ПозицияКонцаМетодаНовая = ирОбщий.СтрНайтиЛкс(ТекстМодуля, МаркерКонца,, ТекущийМетод.ПозицияСОписанием);
			КонецНовый = ПозицияКонцаМетодаНовая + СтрДлина(МаркерКонца);
			ЧастьПослеМетодаНовая = Сред(ТекстМодуля, КонецНовый);
			ОбновлятьТолькоТекущийМетод = ЧастьПослеМетодаНовая = ЧастьПослеМетодаСтарая;
			СмещениеПозиции = КонецНовый - КонецСтарый;
		КонецЕсли;
	КонецЕсли;
	Если ОбновлятьТолькоТекущийМетод Тогда
		Модуль = СтарыйМодуль;
		...
		ТекущийМетод.НазначенияТипов = Неопределено;
		ТекущийМетод.ПоискСвойств = Неопределено;
...
Функция НоваяТаблицаМетодовМодуля() Экспорт 
	...
	Методы.Колонки.Добавить("Зависимый", Новый ОписаниеТипов("Булево")); // Результат зависит от вызова другого метода любого модуля
	...
	Методы.Колонки.Добавить("НазначенияТипов");
	Методы.Колонки.Добавить("ПоискСвойств");
...
Функция ПодготовитьТипРезультатаМетода(СтрокаОпределения, ИмяИлиМодуль, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено, Знач СФактическимиПараметрами = Ложь) Экспорт 
	...
	Если Ложь
		...
		//Или СтрокаОпределения.Зависимый // Пока нельзя включать, т.к. нет защиты от косвенной рекурсии
	Тогда
		...
		Если кэшИскательВозврата = Неопределено Тогда
			...
			кэшИскательВозврата.Pattern = Шаблоны.Возврат; //12
...
Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач ИмяИлиМодуль, Знач СтрокаОпределения, Знач ОписаниеТипа = "", Знач ТекстДляПоискаОпределения = "",
	...
	Если СФактическимиПараметрами Тогда
		ТаблицаТипов = кэшКомпонентаПодсказки.ВычислитьТипЗначенияВыражения(ИмяПеременной,,,,,,, МетодМодуля = Неопределено,, Истина, ЭтоПрисвоенноеВыражение);
	Иначе 
...
//
Процедура ДобавитьВТаблицуТипов(ТаблицаПриемник, Знач ОписаниеТипов, Знач ПередаватьДополнительныеТипы = Ложь) Экспорт 
	...
	Для Каждого СтрокаИсточника Из ТаблицаИсточник Цикл
		Если НужноВытеснять И СтрокаИсточника.ИмяОбщегоТипа = "Неизвестный контекст" Тогда
			Продолжить;
		КонецЕсли;
		Попытка
	...
	Если Результат = Неопределено Тогда
		...
		Если ФайлФормы.Существует() Тогда 
			...
			Если ИмитаторФормы.ВерсияФормата = ВерсияФорматаСтруктурыФормы() Тогда
				ирКэш.Получить().ДополнитьСлужебныеДанныеИзКэшаФормы(СлужебныеДанные, ИмитаторФормы, ФайлФормы.ПолучитьВремяИзменения());
			КонецЕсли;  
		КонецЕсли;
			...
			Если ПутьКРодителю = "" И ИменаРеквизитов = Неопределено Тогда 
				...
				Пока Истина Цикл
					...
					Если ЛюбыеДанныеФормы = Неопределено Тогда
						...
						#КонецЕсли
						ТаблицаТипов = НоваяТаблицаТипов();
						ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипаИзКонкретногоТипа(ирОбщий.ТипУправляемаяФормаЛкс()));
