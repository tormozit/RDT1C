	...
	Если ЗначениеЗаполнено(выхИмяМодуля) Тогда
		...
		Тогда
			...
			Если МодульМетаданных = Неопределено И ФайлОписанияМодуля.Существует() Тогда
				МодульМетаданных = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлОписанияМодуля.ПолноеИмя,, "JSON"); // см. МодульМетаданных()
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	ВерсияФормата = 20; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
	...
	Если ИндексВызововМодуля = Неопределено Тогда
		...
		Иначе
			...
			Если Не ОбновитьИндекс Тогда 
				ИндексВызововМодуля = ирОбщий.ЗагрузитьЗначениеИзФайлаЛкс(ФайлИндекса.ПолноеИмя,, "JSON");
