...
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Знач БазовоеРасширениеКонфигурации = "", КлючКэширования = Неопределено, Знач ДляЗаписи = Ложь, Знач ТолькоСоЗначениями = Ложь) Экспорт 
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Тогда
			...
			ИспользоватьКэширование = СловоФильтр = Неопределено И Не ДляЗаписи;
		...
		Если ИспользоватьКэширование Тогда
			...
			Тогда
				...
				КлючКэширования = ИмяОбщегоТипаРодителяБезДеталей + ";" + ЯзыкПрограммы + ";" + Конфигурация + ";" + ВычислятьТипы + ";" + ТипСловаФильтр + ";" + НаСервере + ";" + КлиентОбычноеПриложение + ";" 
					+ КлиентУправляемоеПриложение + ";" + РодительскаяСтруктураТипа.ТипЯзыка + ";" + БазовоеРасширениеКонфигурации + ";" + РодительскаяСтруктураТипа.Конструктор + ";" + ДляЗаписи + ";" + ТолькоСоЗначениями;
			Иначе
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если Ложь
				Или ТолькоСоЗначениями И ПустаяСтрока(СтрокаСлова.ТипЗначения)
			...
			Иначе
				ТипЗначения = ТипЗначенияИндекс;
			КонецЕсли;
			ДобавитьВТаблицуСлов(ТаблицаСлов, СловоЦикл, ТипСловаЦикл, ТаблицаТипов, ТипЗначения, "Предопределенный",,,, Не ЭтоНоваяТаблицаСлов, ТипЗначенияИндекс);
		КонецЦикла;
		Если ИспользоватьКэширование И Не ЛиПолноеКэширование Тогда
			...
			СловаКонтекстаМетаданные(СтруктураТипаГлобальный,, ТипСловаФильтр,, ЯзыкПрограммы,,, ВычислятьТипы, ФлагиКомпиляции,, ТаблицаСлов,,,, БазовоеРасширениеКонфигурации,,,,, ДляЗаписи, ТолькоСоЗначениями);
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач СловоФильтр = Неопределено, Знач ТипСловаФильтр = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
	...
	Знач ДляСвойства = "", Знач НаборыСлов = Неопределено, КлючКэширования = Неопределено, Знач СтрогийФильтр = Ложь, Знач ДляЗаписи = Ложь, Знач ТолькоСоЗначениями = Ложь) Экспорт 
...
Функция ДобавитьВТаблицуСлов(Знач ТаблицаСлов, Знач Слово, Знач ТипСлова = "Свойство", Знач ТаблицаТипов = Неопределено, Знач ТипЗначения = "??", Знач Определение = "", Знач Вытеснять = Истина,
	...
	Если ТипЗначенияИндекс = Неопределено Тогда
...
Функция ТаблицаСловИзСтруктурыТипа(Знач СтруктураТипа, Знач ЯзыкПрограммы = 0, Знач Конфигурация = Неопределено, Знач ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ТипСловаФильтр = Неопределено, Знач ФлагиКомпиляции = Неопределено, Знач Слово = Неопределено, Знач МодульМетаданных = Неопределено,
	...
	Знач СтрогийФильтр = Ложь, Знач ДляЗаписи = Ложь, Знач ТолькоСоЗначениями = Ложь) Экспорт 
	...
	Если Не ВнутриГруппыОбщихМодулей Тогда
		ТаблицаСлов = СловаКонтекстаПредопределенные(СтруктураТипа, Слово, ТипСловаФильтр,, ЯзыкПрограммы, Конфигурация, ВычислятьТипы, ФлагиКомпиляции, ТаблицаСлов, ЭтоЛокальныйКонтекст, НаборыСлов,
			БазовоеРасширениеКонфигурации, КлючКэширования, ДляЗаписи, ТолькоСоЗначениями);
	КонецЕсли;
	Если КлючКэширования <> Истина Тогда
		ТаблицаСлов = СловаКонтекстаМетаданные(СтруктураТипа, Слово, ТипСловаФильтр, ВнутриГруппыОбщихМодулей, ЯзыкПрограммы,, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ФлагиКомпиляции, 
			МодульМетаданных, ТаблицаСлов, ВычислятьТипыМетодовМодулей, ЭтоЛокальныйКонтекст,, БазовоеРасширениеКонфигурации,, НаборыСлов, КлючКэширования, СтрогийФильтр, ДляЗаписи, ТолькоСоЗначениями);
		Если Истина
...
Функция ИмяОбщегоТипаИзТипаЗначенияСМетаданными(Знач КонкрентыйТип, Знач МетаданныеТипа, Знач ЛиНеУчитыватьПодтип)
	...
	Если СтрокаВида <> Неопределено Тогда
		Если ЛиНеУчитыватьПодтип Тогда
			Подтип = "Ссылка"; // В языке запросов. TODO Добавить в ТаблицаОбщихТипов слово ВнешнийИсточникДанныхТаблица.<Имя внешнего источника>.<Имя таблицы внешнего источника данных>
		КонецЕсли;
