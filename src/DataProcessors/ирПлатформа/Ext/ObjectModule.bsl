					...
					Иначе
						...
						Тогда 
							...
							Тогда
								...
								Тогда
									ИмяВложенногоТипа = ирОбщий.ПервыйФрагментЛкс(ирОбщий.ПервыйФрагментЛкс(ЗначениеСвойства.Комментарий, " ")); // Типизация реквизита из свойства "Комментарий"
									Попытка
										ТипВложенный = Тип(ИмяВложенногоТипа);
									Исключение
			...
			Если СВложенными Тогда
				Если Вхождение.SubMatches.Count > ИндексГруппыПараметровВШаблоне Тогда
					...
					Если Найти(ПараметрыВызова, "(") > 0 Тогда
						...
						ДобавитьВхожденияСловаВИндекс(ВхожденияВложенные, РегВыражение, ТаблицаВызовов, Истина, Вхождение.FirstIndex + Найти(Вхождение.Value, ПараметрыВызова) - 1, ИндексГруппыВШаблоне);
					КонецЕсли;
				Иначе 
...
Функция ВыбратьПроцессКонфигуратора(Знач ИдентификаторыОтладчиков = Неопределено) Экспорт 
	...
	Для Каждого СтрокаКонфигуратора Из ТаблицаКонфигураторов Цикл
		...
		КонецЕсли; 
		Если Найти(ПредставлениеПроцесса, Метаданные.Синоним) Тогда
			НачальноеЗначениеВыбора = НовыйЭлемент;
...
Функция КлючСервисаИИ() Экспорт 
	Если мКлючСервисаИИ = Неопределено Тогда
		мКлючСервисаИИ = ирОбщий.ВосстановитьЗначениеЛкс("КлючСервисаИИ");
		Если мКлючСервисаИИ <> Неопределено Тогда
			мКлючСервисаИИ = мКлючСервисаИИ.Получить();
		КонецЕсли;
	КонецЕсли;
	Возврат мКлючСервисаИИ;
КонецФункции 

Функция МодельИИ() Экспорт 
	Если мМодельИИ = Неопределено Тогда
		мМодельИИ = ирОбщий.ВосстановитьЗначениеЛкс("МодельИИ");
	КонецЕсли;
	Возврат мМодельИИ;
КонецФункции

Процедура СброситьКэшИИ() Экспорт
	мМодельИИ = Неопределено;
	мКлючСервисаИИ = Неопределено;
КонецПроцедуры
