...
//  ПостроительЗапроса	 - ПостроительЗапроса - 
//
Процедура ПодготовитьМетаданныеПостроителяЗапроса(Знач ПостроительЗапроса) Экспорт  
...
Функция ПодготовитьТипРезультатаМетода(СтрокаОпределения, ИмяИлиМодуль, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено, Знач СФактическимиПараметрами = Ложь, Знач ДляСвойства = "") Экспорт 
	...
	//Тогда
		...
		ТаблицаТипов = ПодготовитьТипЗначенияСловаМодуля(ИмяПеременной, кэшКомпонентаПодсказки, СтрокаОпределения, ИмяИлиМодуль, СтрокаОпределения, СтрокаОпределения.ОписаниеРезультата,
			ТекстДляПоискаОпределения, СФактическимиПараметрами, Истина, ДляСвойства);
	//КонецЕсли;
	Возврат ТаблицаТипов;
...
Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач ИмяИлиМодуль, Знач СтрокаОпределения, Знач ОписаниеТипа = "", Знач ТекстДляПоискаОпределения = "",
	...
	Если СФактическимиПараметрами Тогда
		...
		ТаблицаТипов = кэшКомпонентаПодсказки.ВычислитьТипЗначенияВыражения(ИмяПеременной,,,,,,, МетодМодуля = Неопределено,, Истина,,, ДопКлючКэша, ДляСвойства);
	Иначе 
		...
		Тогда
			ТаблицаТипов = кэшКомпонентаПодсказки.ВычислитьТипЗначенияВыражения(ИмяПеременной,,,,,,, МетодМодуля = Неопределено,, Истина,,,, ДляСвойства);
			Если ТаблицаТипов.Количество() > 0 Тогда
				ДобавитьВТаблицуТипов(СтрокаОпределения.ТаблицаТипов, ТаблицаТипов); 
			Иначе
				СтруктураТипа = НоваяСтруктураТипа();
