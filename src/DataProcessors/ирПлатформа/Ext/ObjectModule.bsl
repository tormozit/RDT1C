...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
	...
	Знач ТаблицаСлов = Неопределено, Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ЭтоЛокальныйКонтекст = Ложь, Знач ЛиВместеСЛокальнымКонтекстом = Ложь, Знач БазовоеРасширениеКонфигурации = "",
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	ВерсияФормата = 17; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
			...
			Если СВложенными Тогда
				...
				Иначе
					...
					Пока ЧислоТочек > 0 Цикл
						...
						СтрокаВложенная.ДлинаВхождения = СтрДлина(Текст); 
...
Функция ТаблицаСловИзСтруктурыТипа(СтруктураТипа, ЯзыкПрограммы = 0, Конфигурация = Неопределено, ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	Знач ВычислятьТипыМетодовМодулей = Ложь, Знач ТаблицаСвойств = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь, Знач НаборыСлов = Неопределено, Знач БазовоеРасширениеКонфигурации = "") Экспорт 
