...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач _ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			ИмяТипаФлага = ИмяТипаФлагаВнутриСтруктуры();
			ТипФлаг = Тип(ИмяТипаФлага);
			Для Каждого ЭлементКоллекции Из КоллекцияЗначений Цикл
				...
				Если ВычислятьТипыЦикл Тогда
					...
					Иначе
						...
						Если ТипЗнч(ЗначениеСвойства) = Тип("ОписаниеТипов") И ЗначениеСвойства.СодержитТип(ТипФлаг) Тогда
							ОписаниеТиповЗначения = Новый ОписаниеТипов(ЗначениеСвойства,, ИмяТипаФлага);
							ЗначениеСвойства = Неопределено;
						КонецЕсли;
						Если Ложь
							Или ОписаниеТиповЗначения <> Неопределено
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			Если ВычислятьТипы Тогда
				Если ВычислятьТипыМетодовМодулей Тогда
					ПодготовитьТипЗначенияПеременнойМодуля(ПеременнаяМодуля, МодульМетаданных.Имя, КомпонентаПодсказки); 
				КонецЕсли;
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если ВычислятьТипы Тогда
				Если ВычислятьТипыМетодовМодулей Тогда
					ПодготовитьТипРезультатаМетода(МетодМодуля, МодульМетаданных.Имя, ИскательВозврата); 
				КонецЕсли;
		...
		Тогда
			...
			Если ОбновитьФайлОписания Тогда
				...
				Иначе 
					...
					Иначе
						МодульМетаданных = МодульМетаданных(НовыйТекст, СтруктураТипа, выхИмяФайлаМодуляБезРасширения, Ложь, МодульМетаданных);
...
//  Структура -
//
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено) Экспорт
...
Функция ЗаменитьВозвратыНаПерейтиВТекстеМетода(Знач Текст, Знач ИмяПеременнойРезультата = "Результат", Знач ДобавлятьПереносыСтрок = Ложь, Знач ИмяМетки = "Конец", Знач ВставитьМетку = Ложь) Экспорт
	...
	РегВыражение.Pattern = "((?:^|\n|\r)(?:\t| )*)Возврат(?:\t| )+(\s*(?:" + шСтрокаПрограммы + "|[^\r;])+)(\r|;)";
...
Функция ПодготовитьТипРезультатаМетода(СтрокаОпределения, ИмяМодуля, кэшИскательВозврата = Неопределено, кэшКомпонентаПодсказки = Неопределено, Знач СФактическимиПараметрами = Ложь) Экспорт 
	...
	Тогда
		...
		ТаблицаТипов = ПодготовитьТипЗначенияСловаМодуля(ИмяПеременной, кэшКомпонентаПодсказки, СтрокаОпределения, ИмяМодуля, СтрокаОпределения, СтрокаОпределения.ОписаниеРезультата, ТекстДляПоискаОпределения,
			СФактическимиПараметрами);
	КонецЕсли;
	Возврат ТаблицаТипов;
...
Функция ПодготовитьТипЗначенияПеременнойМодуля(СтрокаОпределения, ИмяМодуля, Знач ПолеТекстаМодуля = Неопределено) Экспорт 
	Если Не СтрокаОпределения.Вычислено Тогда
		Если СтрокаОпределения.ТаблицаТипов = Неопределено Тогда
			СтрокаОпределения.ТаблицаТипов = НоваяТаблицаТипов();
		КонецЕсли; 
		СтрокаОпределения.Вычислено = Истина; // ставим сразу для защиты от бесконечной рекурсии
		ПодготовитьТипЗначенияСловаМодуля(СтрокаОпределения.Имя, ПолеТекстаМодуля,, ИмяМодуля, СтрокаОпределения, СтрокаОпределения.Описание);
	КонецЕсли;
КонецФункции

Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, кэшКомпонентаПодсказки, Знач МетодМодуля, Знач ИмяМодуля, Знач СтрокаОпределения, Знач ОписаниеТипа = "", Знач ТекстДляПоискаОпределения = "",
	Знач СФактическимиПараметрами = Ложь)
	...
	Если СФактическимиПараметрами Тогда
		ТаблицаТипов = кэшКомпонентаПодсказки.ВычислитьТипЗначенияВыражения(ИмяПеременной, ТекстДляПоискаОпределения, ТекстДляПоискаОпределения,,,,, МетодМодуля = Неопределено,, Истина);
	Иначе 
...
Функция ИмяТипаФлагаВнутриСтруктуры() Экспорт
	Возврат "Почта";
КонецФункции
