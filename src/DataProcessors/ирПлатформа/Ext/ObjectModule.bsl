...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач _ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Если Истина
		...
		И РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "ФиксированноеСоответствие"
	Тогда
	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Для Каждого СтрокаВида Из НайденныеСтроки Цикл
			...
			ИначеЕсли Истина
				...
				И (Ложь
					Или ТипЗнч(РодительскаяСтруктураТипа.ДержательМетаданных) = Тип("Структура")
					Или ТипЗнч(РодительскаяСтруктураТипа.ДержательМетаданных) = ирОбщий.ТипУправляемаяФормаЛкс())
			Тогда
		...
		Тогда
			...
			Иначе
				...
				Если МодульМетаданныхКомпонента = Неопределено И ИмяОбщегоТипаРодителя <> "Неизвестный контекст" Тогда
					СтруктураТипаМодуля = НоваяСтруктураТипа(ИмяОбщегоТипаРодителя);
					СтруктураТипаМодуля.ДержательМетаданных = РодительскаяСтруктураТипа.ДержательМетаданных;
					СтруктураТипаМодуля.Метаданные = МетаданныеРодителя;
					МодульМетаданныхКомпонента = ПодготовитьМодульМетаданных(СтруктураТипаМодуля);
				КонецЕсли;
...
Функция ИмяФайлаМодуляБезРасширения(Знач СтруктураТипа, ТипМодуля = "", СтрокаСозданияРодителя = "", ФлагиКомпиляции = Неопределено) Экспорт 
	...
	Тогда
		...
		Если ИмяОбщегоТипа = "ОбщийМодуль" Тогда
			...
			Иначе 
				// Возможно добавленный объект метаданных
				ТипМодуля = "";
			КонецЕсли;
		Иначе
...
// Функция - Заменить возвраты на перейти в тексте метода
//
// Параметры:
//  Текст					 - 	 - 
//  ИмяПеременнойРезультата	 - 	 - 
//  ДобавлятьПереносыСтрок	 - 	 - 
//  ИмяМетки				 - 	 - 
//  ВставитьМетку			 - Строка, Неопределено - 
// 
// Возвращаемое значение:
//   - 
//
