	...
	Для Каждого СтрокаОбщегоТипа Из ТаблицаОбщихТиповСтруктурыТипа Цикл
		...
		Тогда
			...
			Иначе
				...
				Если МодульМетаданныхКомпонента = Неопределено И ИмяОбщегоТипаРодителя <> "НеизвестныйКонтекст" Тогда
...
шСтрокаПеременныхБезКомментов, шСтрокаПеременных, шСтрокаПрограммы, шПараметрыМетода, шДирективаИлиАннотация, шБлокКомментов, шМетодЧистый, шМетод1, шМетодСПредКомментом, шСтруктураМодуля, шЭлементОбластиМетодов;
...
Функция СтруктураТипаИзКонкретногоТипа(КонкрентыйТип, ЯзыкПрограммы = 0, ШаблонСтруктурыТипа = Неопределено) Экспорт
	...
	ИначеЕсли КонкрентыйТип = Тип("ОбъектМетаданных") Тогда
		...
		Тогда
			...
			СтруктураТипа.ИмяОбщегоТипа = МаркерОбъектаМетаданных + МассивФрагментов[МассивФрагментов.ВГраница() - 1];
		Иначе
			СтруктураТипа.ИмяОбщегоТипа = "ОбъектМетаданныхСправочник"; // Наиболее типичный вариант. Это лучше чем абстрактный тип без членов.
		КонецЕсли;
	ИначеЕсли Истина
