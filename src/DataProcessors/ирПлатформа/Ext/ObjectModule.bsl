...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	Возврат 26; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
КонецФункции
...
Функция КонтекстНапарника() Экспорт
	...
	КонецЕсли;
	КонтекстНапарника = Новый Структура("Сессия, ИдентификаторКомпьютера, СлужебныеПараметры, Соединение, ОтправленныеОбъекты, Браузер");
	...
	КонтекстНапарника.ОтправленныеОбъекты = Новый Соответствие;
