...
Перем ОткрытыеФормыЧужие Экспорт;
...
Функция СвойстваТипаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			КонецЕсли;
			ДобавитьВТаблицуСлов(ТаблицаСвойств, СтрокаСлова.Слово, СтрокаСлова.ТипСлова, ТаблицаСтруктурТипов, ТипЗначения, "Предопределенный");
		КонецЦикла;
		Если ИспользоватьКэширование Тогда
...
Функция СвойстваТипаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Тогда
		...
		Если Не ЛиИндексИдентификатор Тогда
			...
			ДобавитьВТаблицуСлов(ТаблицаСвойств, "*", "Свойство", ТаблицаСтруктурТипов,, "Метаданные");
			Возврат ТаблицаСвойств;
		КонецЕсли;
	КонецЕсли;
...
Функция ПодготовитьМодульМетаданных(Знач СтруктураТипа, Знач ТипМодуля = "", выхИмяФайлаМодуляБезРасширения = "") Экспорт 
	...
	Если ТипМодуля <> Неопределено Тогда
		...
		Если ТипЗнч(МетаданныеРодителя) = Тип("ОбъектМетаданныхКонфигурация") Тогда
			выхИмяФайлаМодуляБезРасширения = "Конфигурация" + "." + ТипМодуля;
		Иначе
			выхИмяФайлаМодуляБезРасширения = МетаданныеРодителя.ПолноеИмя() + "." + ТипМодуля;
		КонецЕсли; 
		МодульМетаданных = МодулиМетаданных[выхИмяФайлаМодуляБезРасширения];
		Если МодульМетаданных = Неопределено Тогда
			...
			Если ФайлОписанияМодуляСуществует Тогда
				...
				Если Ложь
					...
					Или МодульМетаданных.Методы.Колонки.Найти("ПозицияОпределения") = Неопределено // Проверка версии кэш
				Тогда
					// Старый формат кэша
			...
			КонецЕсли; 
			МодулиМетаданных[выхИмяФайлаМодуляБезРасширения] = МодульМетаданных;
		КонецЕсли; 
	КонецЕсли;
...
Функция ПолучитьРазделыМодуля1С(Знач ТекстМодуля = "") Экспорт
	...
	Для Индекс = 0 По ВхожденияМетодов.Количество() - 1 Цикл
		...
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			ПозицияНачалаПервогоМетода = Вхождение.FirstIndex;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Для Индекс = 1 - ВхожденияМетодов.Количество() По 0 Цикл // Обратный обход
		Вхождение = ВхожденияМетодов[-Индекс];
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			ПозицияКонцаПоследнегоМетода = Вхождение.FirstIndex + Вхождение.Length;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Результат.Вставить("Методы", Сред(ТекстМодуля, ПозицияНачалаПервогоМетода, ПозицияКонцаПоследнегоМетода));
	Результат.Вставить("Программа", Сред(ТекстМодуля, ПозицияКонцаПоследнегоМетода + 1));
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач ПодробныйРазборТолькоЭкспортных = Истина, Знач СохранятьОпределения = Ложь) Экспорт
	...
	Вхождения = RegExp.НайтиВхождения(РазделыМодуля.Переменные);
	Для Каждого Вхождение Из Вхождения Цикл
		...
		КонецЕсли;
	КонецЦикла;
	РазделыМодуля.Переменные = Переменные;
	
	RegExp2.Global = Истина;
	RegExp2.Pattern = Шаблоны.ПараметрыМетода;
	Вхождения = РазделыМодуля.ВхожденияМетодов;
	#Если Сервер И Не Сервер Тогда
	...
	Если СохранятьОпределения Тогда
		Методы.Колонки.Добавить("ОпределениеСОписанием", Новый ОписаниеТипов("Строка"));
	КонецЕсли;
	Методы.Колонки.Добавить("Аннотация", Новый ОписаниеТипов("Строка"));
	Методы.Колонки.Добавить("Параметры");
	Методы.Колонки.Добавить("ПозицияОпределения"); // Начиная с 1
	...
	Если Вхождения.Количество() > 100 Тогда
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если Вхождение.SubMatches(1) <> Неопределено Тогда
			...
			СтрокаМетода.ПозицияОпределения = Макс(1, Вхождение.FirstIndex) + СтрДлина(ОписаниеМетода);
			СтрокаМетода.Описание = ОчиститьТекстМодуляОтПрефиксаКомментария(ОписаниеМетода);
			Если СохранятьОпределения Тогда
				СтрокаМетода.ОпределениеСОписанием = СокрЛ(Вхождение.Value);
			КонецЕсли;
			...
			СтрокаМетода.ПозицияТела = СтрокаМетода.ПозицияОпределения + СтрДлина(Вхождение.SubMatches(1));
			СтрокаМетода.Тело = Вхождение.SubMatches(6); 
			СтрокаМетода.Аннотация = ТекущаяАннотация;
			ТекущаяАннотация = "";
			Если Истина
				И Не ПустаяСтрока(ТекстПараметров) 
				И (Ложь
					Или Не ПодробныйРазборТолькоЭкспортных
					Или ЛиЭкспорт)
			Тогда
				РезультатПараметров = RegExp2.НайтиВхождения(ТекстПараметров);
				Если РезультатПараметров.Количество() > 0 Тогда
					...
					Если ЗначениеЗаполнено(СтрокаМетода.Описание) Тогда
						...
						Если ирОбщий.СтрокиРавныЛкс(СтрокаМетода.Тип, "функция") Тогда
							...
							ВхожденияОписаний = ирОбщий.НайтиРегВыражениеЛкс(СтрокаМетода.Описание, ШаблонРезультата, "ТипЗначения, Описание", Ложь,, Ложь, Ложь);
		...
		ИначеЕсли Вхождение.SubMatches(8) <> Неопределено Тогда
			НоваяАннотация = СокрЛП(НРег(Вхождение.SubMatches(8)));
			Если Лев(НоваяАннотация, 1) = "&" Тогда
				ТекущаяАннотация = НоваяАннотация;
			КонецЕсли;
	...
	РазделыМодуля.Методы = Методы;
	РазделыМодуля.Удалить("ВхожденияМетодов"); 
...
Функция ОчиститьТекстМодуляОтПрефиксаКомментария(Знач ТекстМодуля)
	...
	Результат = Сред(RegExp.Заменить(ТекстМодуля, Символы.ПС), 2); // Трезаем первый перенос строки
	Возврат Результат;
...
//   - 
//
Функция ТаблицаПараметровМетода(Знач СтрокаОписания) Экспорт 
...
Функция РедактироватьЗапрос(Текст, Параметры = Неопределено, ТипЗапроса = "Построитель", ПараметрыADO = Неопределено, ПараметрыWMI = Неопределено) Экспорт
