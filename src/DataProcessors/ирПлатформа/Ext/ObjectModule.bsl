			...
			Иначе
				...
				Если МодульМетаданныхКомпонента <> Неопределено Тогда
					...
					Если Истина
						...
						И ЗначениеЗаполнено(РодительскаяСтруктураТипа.ДержательМетаданных) 
					Тогда
						// Для расширения модуля добавляем базовый модуль
						СтруктураТипаМодуля = НоваяСтруктураТипа(ИмяОбщегоТипаРодителя);
						СтруктураТипаМодуля.ДержательМетаданных = Неопределено; 
						СтруктураТипаМодуля.Метаданные = МетаданныеРодителя;
						МодульМетаданныхБазоый = ПодготовитьМодульМетаданных(СтруктураТипаМодуля);
						Если МодульМетаданныхБазоый <> Неопределено Тогда
							ДобавитьВТаблицуСловЭлементыМодуляМетаданных(СтрокаОбщегоТипа, Слово, ТипСлова, ТаблицаСлов, МодульМетаданныхБазоый, ВычислятьТипыМетодовМодулей, ВычислятьТипы, Не ЭтоЛокальныйКонтекст, ФлагиКомпиляции);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаписатьМодульВКэш(Знач МодульМетаданных, Знач ЧерезКопию = Ложь) Экспорт 
	Если МодульМетаданных <> мПустойМодуль Тогда
		...
		Если ЧерезКопию Тогда
			...
			КопияМодуля.Методы.ЗаполнитьЗначения(Неопределено, "КэшПоиска"); 
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Для Каждого Вхождение Из ВхожденияМетодов Цикл
		...
		Если Заголовок <> Неопределено Тогда
			...
			СтрокаМетода.Тело = Сред(Тело, 2); // Сдвиг добавлен после вынужденного переноса символа "переноса строки" из заголовка в тело
