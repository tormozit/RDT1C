	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если ИспользоватьКэширование Тогда
			...
			КлючКэширования = "" + ЯзыкПрограммы + ";" + Конфигурация + ";" + ВычислятьТипы + ";" + ТипСлова + ";" + НаСервере + ";" + КлиентОбычноеПриложение + ";" + КлиентУправляемоеПриложение
				+ ";" + РодительскаяСтруктураТипа.ТипЯзыка + ";" + БазовоеРасширениеКонфигурации + ";" + РодительскаяСтруктураТипа.Конструктор;
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
	...
	Если ВнутренняяТаблицаСлов = Неопределено Тогда
		ВнутренняяТаблицаСлов = НоваяТаблицаСлов();
	КонецЕсли;
	Если Истина
	...
	Тогда
		...
		Тогда
			Если ТаблицаИзКэша <> Неопределено Тогда
				Если ТаблицаИзКэша.Количество() > ВнутренняяТаблицаСлов.Количество() Тогда
					ТаблицаИзКэша = ТаблицаИзКэша.Скопировать();
					ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ВнутренняяТаблицаСлов, ТаблицаИзКэша);
					ВнутренняяТаблицаСлов = ТаблицаИзКэша;
				Иначе
					ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаИзКэша, ВнутренняяТаблицаСлов);
				КонецЕсли;
	...
	Тогда
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			...
			КонецЕсли;
			ДобавитьВТаблицуСловЭлементМодуляМетаданных(ВнутренняяТаблицаСлов, ПеременнаяМодуля.Имя, ВычислятьТипы, ТаблицаИзКэша, ТаблицаТипов, ПеременнаяМодуля.ТипЗначения, ТипСловаЦикл);
		КонецЦикла;
	КонецЕсли; 
	Если Истина
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			КонецЕсли;
			ДобавитьВТаблицуСловЭлементМодуляМетаданных(ВнутренняяТаблицаСлов, ИмяМетода, ВычислятьТипы, ТаблицаИзКэша, ТаблицаТипов, МетодМодуля.ТипЗначения, ТипСловаЦикл);
		КонецЦикла;
		Если Индикатор <> Неопределено Тогда
	...
	Если КлючКэша <> Неопределено Тогда
		МодульМетаданных.ТаблицыСлов[КлючКэша] = ВнутренняяТаблицаСлов.Скопировать();
	КонецЕсли;
	Возврат ВнутренняяТаблицаСлов;
...
Функция ДобавитьВТаблицуСловЭлементМодуляМетаданных(Знач ТаблицаСлов, Знач Слово, Знач ВычислятьТипы, Знач ТаблицаИзКэша, Знач ТаблицаТипов, Знач ТипЗначения, Знач ТипСлова) Экспорт
	Если Слово <> "<>" Тогда
		...
		Тогда
			...
			ТаблицаИзКэша.НайтиСтроки(КлючПоиска)[0].ТипЗначения = ТипЗначения;
		Иначе
			Если Не ирОбщий.СтрНачинаетсяСЛкс(ТипЗначения, "??") Тогда
				ТипЗначения = "??" + ?(ПустаяСтрока(ТипЗначения), "", "," + ТипЗначения);
			КонецЕсли;
		...
		Тогда
			...
			Если ФайлОписанияМодуля.Существует() Тогда
				...
				Иначе
					...
					МодульМетаданных.Вставить("ТаблицыСлов", Новый Соответствие);
				КонецЕсли; 
			КонецЕсли; 
...
Функция ВерсияФорматаКэшаМодулей() Экспорт
	ВерсияФормата = 16; // Мультиметка91237881041 Каждый раз при изменении структуры увеличиваем на 1
...
Функция НумероватьСловаВИндексе(ТаблицаВызовов, СтруктураСлов) Экспорт
	...
	НачалиСНуля = ПервыйСвободныйНомерСлова = 0;
	СтарыйНомерСлова = 0;
	//ТаблицаВызовов.Колонки.Добавить("НомерСлова", Новый ОписаниеТипов("Число")); // Вынес наружу для анализатора кода
	Для Каждого Слово Из МассивСлов Цикл
		Если НачалиСНуля Или Не СтруктураСлов.Свойство(Слово, СтарыйНомерСлова) Тогда
...
Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, Знач ПолеТекстаМодуляКэш, Знач МетодМодуля, Знач ИмяИлиМодуль, Знач СтрокаОпределения, Знач ОписаниеТипа = "",
	...
	Иначе 
		Если Истина
			И Не (СтрокаОпределения.Вычислено 
				И Не ЗначениеЗаполнено(ДляСвойства) // Мультиметка2323399173
				)
			И СтрокаОпределения.Имя <> "<>"
		Тогда
			Если Ложь
				Или Не ЗначениеЗаполнено(ДляСвойства) 
				//Или мРежимПроверкиМодуля
			Тогда
