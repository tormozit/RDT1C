...
Функция ДобавитьВременныеТаблицыПараметровВТекстЗапроса(ТекстЗапроса, Знач Параметры) Экспорт
	...
	Для Каждого КлючИЗначение Из Параметры Цикл
		ЗначениеПараметра = КлючИЗначение.Значение; // ТаблицаЗначений
		Если Истина
...
Функция ДобавитьВТаблицуСловЭлементыМодуляМетаданных(Знач СтруктураТипаМини, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутренняяТаблицаСлов = Неопределено,
		...
		Для Каждого ПеременнаяМодуля Из ПодходящиеПеременные Цикл
			Если ВычислятьТипы Тогда
				Если ВычислятьТипыМетодовМодулей Тогда
					ПодготовитьТипЗначенияПеременнойМодуля(ПеременнаяМодуля, МодульМетаданных, КомпонентаПодсказки, ДляСвойства); 
				КонецЕсли;
	...
	Тогда
		...
		Для Каждого МетодМодуля Из ПодходящиеМетоды Цикл
			...
			Если ВычислятьТипы Тогда
				Если ВычислятьТипыМетодовМодулей Тогда
					ПодготовитьТипРезультатаМетода(МетодМодуля, МодульМетаданных,,, ДляСвойства); 
				КонецЕсли;
...
Функция НепрямыеВызовыСловаВМодуле(Знач ИмяМодуля, Знач ТекстовыйДокумент, Знач Файл, Знач ПолеТекстаПрограммы, Знач ИскатьСлово, Знач ЛиМетод = Истина) Экспорт
	...
	Если ИндексВызововМодуля = Неопределено Тогда
		...
		Если ОбновитьИндекс Тогда
			...
			ИндексВызововМодуля = ИндексПоискаВызововСловаВМодуле(ТекстМодуля, ШаблонВнешнегоВызова,, ВерсияФорматаИндекса);
...
Функция ИндексПоискаВызововСловаВМодуле(Знач ТекстМодуля, Знач ШаблонВнешнегоВызова, Знач ШаблонОповещения = "", Знач ВерсияФорматаИндекса = 0) Экспорт
	_РежимОтладки = Ложь;
	...
	Вхождения = РегВыражение.НайтиВхождения(ТекстМодуля,, Истина);
	...
	ДобавитьВызовыСловаВИндекс(ТаблицаВызовов, СтруктураСлов);
	Если ЗначениеЗаполнено(ШаблонОповещения) Тогда
		...
		КонецЦикла;
		ДобавитьВызовыСловаВИндекс(ТаблицаОповещений, СтруктураСлов);
	КонецЕсли;
...
// Возвращаемое значение:
//    Структура - 
Функция ДобавитьВызовыСловаВИндекс(ТаблицаВызовов, СтруктураСлов) Экспорт
	_РежимОтладки = Ложь;
...
Функция МодульМетаданных(Знач ТекстМодуля, Знач СтруктураТипа = Неопределено, Знач ИмяМодуля = "", Знач ПодготовитьОписанияРезультатовМетодов = Истина, Знач СтарыйМодуль = Неопределено,
	...
	Если ЛиНачальныйДиапазонСовпал Тогда
		...
		Если ЛиКонечныйДиапазонСовпал Тогда
			Если ТекущийМетод = Неопределено Тогда
				...
				Модуль.Программа = Сред(ТекстМодуля, ПозицияНачалаСтарая);
	...
	Модуль.Вставить("ПрямыеВызовыМетодов");
...
Функция ПодготовитьТипЗначенияПеременнойМодуля(СтрокаОпределения, ИмяИлиМодуль, Знач ПолеТекстаМодуляКэш = Неопределено, Знач ДляСвойства = "") Экспорт 
	...
	КонецЕсли;
	ПолеТекстаМодуляКэш = ПолеТекстаМодуля(ИмяИлиМодуль, ПолеТекстаМодуляКэш);
	ПодготовитьТипЗначенияСловаМодуля(СтрокаОпределения.Имя, ПолеТекстаМодуляКэш,, ИмяИлиМодуль, СтрокаОпределения, СтрокаОпределения.Описание,,, ДляСвойства);
	Если СтрокаОпределения.ТаблицаТипов.Количество() > 1 Тогда
		ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюВКолонкеЛкс(СтрокаОпределения.ТаблицаТипов, "ИмяОбщегоТипа", "Произвольный"); // Нужно для проверок выражений
	КонецЕсли;
КонецФункции
...
Функция ПодготовитьТипЗначенияСловаМодуля(Знач ИмяПеременной, Знач ПолеТекстаМодуляКэш, Знач МетодМодуля, Знач ИмяИлиМодуль, Знач СтрокаОпределения, Знач ОписаниеТипа = "",
	...
	Иначе 
		Если Не СтрокаОпределения.Вычислено И СтрокаОпределения.Имя <> "<>" Тогда
			Если Ложь
			...
			Если Истина
				...
				И СтрокаОпределения.ТипЗначения <> "?"
			Тогда
				ТаблицаТипов = ПолеТекстаМодуляКэш.ТаблицаТиповИзТекста(СтрокаОпределения.ТипЗначения, ОписаниеТипа,, ДляСвойства);  
			...
			Если Ложь
				...
				Или (Истина
					...
					И ЛиДетальностьТиповДостаточна(ТаблицаТипов, 4)) // Мультиметка481662548
			Тогда
...
Функция ТипыЭлементовКоллекции(Знач СтруктураТипаКоллекции, Знач ЯзыкПрограммы = 0, выхНужнаПроверкаТипов = Истина) Экспорт
	...
	ИначеЕсли Истина
		...
		И ТипЗнч(СтруктураТипаКоллекции.Метаданные) = Тип("Форма")
	Тогда 
