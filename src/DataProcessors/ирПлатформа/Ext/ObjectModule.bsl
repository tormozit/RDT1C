...
//  Слово       - Строка, Неопределено - для отбора, для кэширования должно быть Неопределено
...
Функция СловаКонтекстаПредопределенные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВиртуальнаяТаблица = Неопределено, Знач ЯзыкПрограммы = 0,
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Если Ложь
			...
			Или ИмяОбщегоТипаРодителяБезДеталей = "ТаблицаФормы"
...
Функция СловаКонтекстаМетаданные(Знач РодительскаяСтруктураТипа, Знач Слово = Неопределено, Знач ТипСлова = Неопределено, Знач ВнутриГруппыОбщихМодулей = Ложь, Знач ЯзыкПрограммы = 0,
	...
	Знач ДляСвойства = "", Знач НаборыСлов = Неопределено, КлючКэширования = Неопределено, Знач СтрогийФильтр = Ложь) Экспорт 
	...
	Если Слово <> Неопределено И ТекущийИндекс <> Неопределено Тогда 
		...
		Тогда 
			...
			Иначе
				ТаблицаТипов = ДобавитьВТаблицуТипов(, НоваяСтруктураТипа("Неопределено"));
			КонецЕсли;
...
Функция ИмяМодуляИзСтруктурыТипа(Знач СтруктураТипа, ТипМодуля = "", СтрокаСозданияРодителя = "", ФлагиКомпиляции = Неопределено) Экспорт 
	...
	ИначеЕсли ирОбщий.ЛиИмяТипаФормыЛкс(ИмяОбщегоТипа, Истина) Тогда 
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(МетаданныеРодителя) Тогда
			ТипМодуля = "Модуль";
			Если ТипЗнч(МетаданныеРодителя) = Тип("Строка") Тогда
				МетаданныеРодителя = МетаданныеРодителя + ".Форма";
			КонецЕсли;
...
Функция ТаблицаСловИзСтруктурыТипа(Знач СтруктураТипа, Знач ЯзыкПрограммы = 0, Знач Конфигурация = Неопределено, Знач ВнешниеФункцииКомпоновкиДанных = Истина, Знач ВычислятьТипы = Истина,
	...
	Знач ВычислятьТипыМетодовМодулей = Неопределено, Знач ТаблицаСлов = Неопределено, Знач ЭтоЛокальныйКонтекст = Ложь, Знач НаборыСлов = Неопределено, Знач БазовоеРасширениеКонфигурации = "",
	Знач СтрогийФильтр = Ложь) Экспорт 
	...
	Если КлючКэширования <> Истина Тогда
		ТаблицаСлов = СловаКонтекстаМетаданные(СтруктураТипа, Слово, ТипСлова, ВнутриГруппыОбщихМодулей, ЯзыкПрограммы,, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ФлагиКомпиляции, 
			МодульМетаданных, ТаблицаСлов, ВычислятьТипыМетодовМодулей, ЭтоЛокальныйКонтекст,, БазовоеРасширениеКонфигурации,, НаборыСлов, КлючКэширования, СтрогийФильтр);
		Если Истина
			И Слово = Неопределено
