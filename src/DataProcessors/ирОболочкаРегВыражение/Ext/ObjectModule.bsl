...
Функция Вычислитель()
	Если Вычислитель = Неопределено Тогда
		...
		Иначе
			...
			Вычислитель.ВызыватьИсключения = Истина; //@skip-check unknown-method-property
		КонецЕсли; 
	КонецЕсли; 
	// Ускорение
...
Функция НоваяТаблицаВхождений() Экспорт 
	...
	ТаблицаВхождений.Колонки.Добавить("Подгруппы"); // Массив из Строка, ОбработкаОбъект.ирОболочкаРегВхождение
	//! ТаблицаВхождений.Колонки.Добавить("Подгруппа0");
...
Функция ОбработатьТекстОшибки(Знач ТекстОшибки, Знач Выражение) Экспорт
	Если ТекущийДвижок = ВсеДвижки().VBScript Тогда
		...
		Если КодОшибкиВБСкрипт.Количество() > 0 Тогда
			КодОшибкиВБСкрипт = КодОшибкиВБСкрипт[0].Подгруппа0;
