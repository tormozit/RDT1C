...
Процедура ИнициализироватьИнформациюОСобытиях()
	...
	Для Каждого СыройТипСобытия Из ТаблицаТиповСобытий Цикл
		...
		Если ЗначениеЗаполнено(МинимальнаяВерсияПлатформы) Тогда
			...
			Если ирКэш.НомерВерсииПлатформыЛкс() < МинимальнаяВерсияПлатформы Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли; 
...
Процедура УстановитьФлажкиДляСобытийДоступа()
	...
	Если НастройкиДоступа <> Неопределено Тогда
		//! мНастройки.Вставить("Событие_Доступ_Доступ"); 
	...
	Если НастройкиОтказаВДоступе <> Неопределено Тогда
		//! мНастройки.Вставить("Событие_Доступ_ОтказВДоступе");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		Ответ = Вопрос(НСтр("ru = 'Настройки в форме были изменены, но не были применены. Уверены что хотите выйти?'"), РежимДиалогаВопрос.ДаНет, 60);
