...
Процедура ЗагрузитьИндекс(ОпределятьКнигуПоОткрытойСтранице = Ложь) Экспорт
	...
	Если ОпределятьКнигуПоОткрытойСтранице Тогда
		ФайлИндекса = ПолучитьАрхивСинтаксПомощникаПоПутиКЭлементу(ПутьКЭлементу, 2, , АрхивСинтаксПомощника);
		// АрхивСинтаксПомощника теперь изменился
	Иначе
		ФайлИндекса = АрхивСинтаксПомощникаПоИмени(2, АрхивСинтаксПомощника);
	КонецЕсли; 
	Если ТекущийФайлИндекса = ФайлИндекса Тогда
		Возврат;
	КонецЕсли;
	лТаблицаИндекса = ПрочитатьИндексАрхиваСинтаксПомощника(АрхивСинтаксПомощника);
	Если лТаблицаИндекса = Неопределено Тогда
		...
		Если АрхивСинтаксПомощника = "shcntx_ru" Тогда
			...
			Если МассивФайлов.Количество() > 0 Тогда
				...
				Для Каждого ФайлЧастиИндекса Из МассивФайлов Цикл
					...
					РезультатПоиска = RegExp.НайтиВхождения(ТекстФайла);
		...
		ЗаписатьИндексАрхиваСинтаксПомощника(АрхивСинтаксПомощника, лТаблицаИндекса);
	КонецЕсли;
...
Процедура ЗагрузитьСодержание(ОпределятьКнигуПоОткрытойСтранице = Ложь) Экспорт
	...
	Если ОпределятьКнигуПоОткрытойСтранице Тогда
		ФайлСодержания = ПолучитьАрхивСинтаксПомощникаПоПутиКЭлементу(ПутьКЭлементу, 1, , АрхивСинтаксПомощника);
		// АрхивСинтаксПомощника теперь изменился
	Иначе
		ФайлСодержания = АрхивСинтаксПомощникаПоИмени(1, АрхивСинтаксПомощника);
	КонецЕсли; 
	Если ТекущийФайлСодержания = ФайлСодержания Тогда
		Возврат;
	КонецЕсли;
	лСодержание = СодержанияАрхивовСправки[АрхивСинтаксПомощника];
	Если лСодержание = Неопределено Тогда
		...
		СодержанияАрхивовСправки[АрхивСинтаксПомощника] = лСодержание;
	КонецЕсли;
...
Процедура ВыбратьИскомуюСтроку(СтрокаДереваТиповСлова, БезусловнаяАктивизацияРезультатов = Истина) Экспорт 
	...
	ОткрытьСтраницуСтрокиПоиска();
	ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(ЭлементыФормы.ИскомоеСлово, ЭтаФорма); // Чтобы в историю добавлялось при открытии с готовым указателем на страницу
...
Функция ИмяАрхиваСинтаксПомощникаПоЯзыкуПрограммы(ЯзыкПрограммы) Экспорт 
...
Процедура ОткрытьСтраницуСтрокиПоиска()
	...
	ирОбщий.ПрисвоитьЕслиНеРавноЛкс(АрхивСинтаксПомощника, ИмяАрхиваСинтаксПомощникаПоЯзыкуПрограммы(ЯзыкПрограммы), ЛиСменилсяВыбранныйАрхив);
	...
	Тогда
		...
		Тогда
			СтруктураКлюча.Слово = "<Новый>";
			СтруктураКлюча.Вставить("ВариантСинтаксиса", СтрокаОписания.Слово);
		КонецЕсли; 
...
Процедура КоманднаяПанельХтмлСписокИзмененийПлатформы(Кнопка)
	...
	ИзмененияПлатформы.Колонки.Добавить("Раздел", Новый ОписаниеТипов("Строка"));
	ИзмененияПлатформы.Колонки.Добавить("Функциональность", Новый ОписаниеТипов("Строка"));
	ИзмененияПлатформы.Колонки.Добавить("Версия", Новый ОписаниеТипов("Строка"));
	ИзмененияПлатформы.Колонки.Добавить("КакСтало", Новый ОписаниеТипов("Строка"), "Как стало");
	ИзмененияПлатформы.Колонки.Добавить("КакБыло", Новый ОписаниеТипов("Строка"), "Как было");
	ИзмененияПлатформы.Колонки.Добавить("РезультатИзменения", Новый ОписаниеТипов("Строка"), "Результат изменения");
	ИзмененияПлатформы.Колонки.Добавить("Примечание", Новый ОписаниеТипов("Строка"));
	...
	Для Каждого Узел Из ДокументHTML.Тело.ДочерниеУзлы Цикл
		Если Узел.ИмяУзла = "table" Тогда
			...
			Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
				...
				Иначе
					...
					Иначе
						Сообщить(Строка(КоличествоКолонок) + " - " + СтрокаТаблицы.ИмяУзла + " / "
		...
		КонецЕсли;
	КонецЦикла;
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(ИзмененияПлатформы,, Ложь, "Изменения платформы",,,, "ИзмененияПлатформы");
КонецПроцедуры

Процедура КоманднаяПанельХтмлТабличноеОписаниеПлатформы(Кнопка)
	
	ОписаниеПлатформы = ирОбщий.ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ОписаниеПлатформы");
...
Процедура КоманднаяПанельХтмлСсылкиНаТип(Кнопка)
	...
	Если УзлыСтраницы.length > 0 Тогда
		ПоискСсылокНаТип = ирОбщий.ПервыйФрагментЛкс(УзлыСтраницы.Item(0).innerText, " (");
	Иначе
		ПоискСсылокНаТип = ирОбщий.ПервыйФрагментЛкс(ДОМ.body.childNodes.Item(0).innerText, " ");
	КонецЕсли;
	ОписаниеПлатформы = ирОбщий.ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ОписаниеПлатформы",,, ПоискСсылокНаТип);
	ОписаниеПлатформы.ПоискСсылокНаТип = ПоискСсылокНаТип;
	ОписаниеПлатформы.Открыть();
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирСинтаксПомощник.Форма.Форма");
...
Для Каждого Файл Из Файлы Цикл
	Если Файл.Размер() > 20000 Тогда // отсекаем мелкий мусор
		СтрокаКниги = мКниги.Найти(Файл.ИмяБезРасширения, "ИмяАрхива");
		Если СтрокаКниги <> Неопределено Тогда
			ПредставлениеЭлемента = СтрокаКниги.Представление;
		//ИначеЕсли Файл.ИмяБезРасширения = "shcntx_ru" Тогда
		//	ПредставлениеЭлемента = "Объектная модель";
		//ИначеЕсли Файл.ИмяБезРасширения = "shlang_ru" Тогда
		//	ПредставлениеЭлемента = "Встроенный язык 1";
		//ИначеЕсли Файл.ИмяБезРасширения = "shclang_ru" Тогда
		//	ПредставлениеЭлемента = "Встроенный язык 2";
		//ИначеЕсли Файл.ИмяБезРасширения = "debug_ru" Тогда
		//	ЭлементСписка.Представление = "Отладка";
		//ИначеЕсли Файл.ИмяБезРасширения = "config_ru" Тогда
		//	ЭлементСписка.Представление = "Обновление конфигурации";
		//ИначеЕсли Файл.ИмяБезРасширения = "devtool_ru" Тогда
		//	ЭлементСписка.Представление = "Разработка конфигурации";
		Иначе
			Продолжить;
		КонецЕсли; 
		ЭлементыФормы.АрхивСинтаксПомощника.СписокВыбора.Добавить(Файл.ИмяБезРасширения, ПредставлениеЭлемента);
	КонецЕсли; 
КонецЦикла;
