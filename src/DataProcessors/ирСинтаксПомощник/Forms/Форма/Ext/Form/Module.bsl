...
Процедура ОбновитьРезультатыПоиска(БезусловнаяАктивизацияРезультатов = Истина)
	...
	ЯзыкПрограммы = ПолучитьЯзыкПрограммыПоИмениАрхиваСинтаксПомощника();
	ТаблицаСтруктурВозможныхТиповКонтекста = ирКлиент.НайтиВозможныеСтрокиОписанияСловаВСинтаксПомощникеЛкс(ИскомоеСлово, ЯзыкПрограммы, ПоискСУчетомТипаСлова);
...
Процедура КоманднаяПанельХтмлСсылкиНаТип(Кнопка)
	...
	КонецЕсли;
	ОписаниеПлатформы = ирКлиент.ПолучитьФормуЛкс("Обработка.ирПлатформа.Форма.ОписаниеПлатформы",,, ПоискСсылокНаТип);
