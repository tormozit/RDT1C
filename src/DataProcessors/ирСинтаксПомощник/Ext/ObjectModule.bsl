...
Функция ЗагрузитьОписаниеМетода(Знач ТипКонтекста, Знач ИмяМетода, Знач ОписаниеХТМЛ, Знач ТипСлова = "Метод", Знач ЯзыкПрограммы = 0, Знач ТаблицаПараметров = Неопределено, Знач РабочийКаталог = "",
	...
	РегВыражение.Pattern = шКлюч + "(?:Доступность|Использование в версии):" + шКонецКлюча ;
		...
		Для Каждого ВхождениеПараметра Из ВхожденияПараметра Цикл
			...
			Если ТаблицаПараметров = мПлатформа.ТаблицаПараметров Тогда
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					Если ЗначениеЗаполнено(НоваяСтрокаПараметра.ТипЗначения) Тогда
						КромеСвойств = "ТипЗначения";
					КонецЕсли;
				КонецЕсли; 
			КонецЕсли; 
			...
			Если ТекстПоискаТипов <> Неопределено Тогда
				...
				КонецЦикла;
				СтруктураСтроки.ТипЗначения = ИмяТипаИзПредставления(ИзвлечьИзФрагментаHTMLОбычныйТекст(Сред(СтрокаТипаЗначения, 3)));
				Если Истина
					И ТипСлова <> "Таблица" 
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПараметра, СтруктураСтроки,, КромеСвойств);
...
Функция ИзвлечьИзФрагментаHTMLОбычныйТекст(Знач Текст, Знач УдалитьКрайнююТочку = Истина) Экспорт 
	...
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "\{", "<"); // Мультиметка251114_070814
	Возврат Текст;

КонецФункции

Процедура ОбрезатьОписание(Текст, Знач ФорматХТМЛ = Ложь)
	...
	Текст = РегВыражение.Заменить(Текст, Символы.ПС + "\{b>$1\{/b>: "); // Мультиметка251114_070814
КонецПроцедуры
...
Функция ИмяТипаИзПредставления(Знач Представление) Экспорт
