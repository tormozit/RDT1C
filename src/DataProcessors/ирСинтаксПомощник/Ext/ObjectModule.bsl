...
Функция ЗагрузитьОписаниеМетода(Знач ТипКонтекста, Знач ИмяМетода, Знач ОписаниеHTML, Знач ТипСлова = "Метод", Знач ЯзыкПрограммы = 0, Знач ТаблицаПараметров = Неопределено, Знач РабочийКаталог = "",
	...
	ВычислительРегВыражений.Pattern = шКлюч + "Доступность:" + шКонецКлюча ;
	ВхожденияДоступности = ВычислительРегВыражений.НайтиВхождения(ОписаниеHTML);
	ОписаниеДоступности = "";
	Если ВхожденияДоступности.Количество() > 0 Тогда
		ОписаниеДоступности = Сред(ОписаниеHTML, ВхожденияДоступности[0].FirstIndex + 1);
...
Функция ЗагрузитьОписаниеСвойства(Знач СтрокаОписания) Экспорт 
	...
	ВычислительРегВыражений.Pattern = шТегКлюч + ирОбщий.ТекстДляРегВыраженияЛкс(ирОбщий.КодироватьТекстВXMLЛкс(СтрокаОписания.Слово)) + "\s*\([^<>\)]+\)" + шКонецТегаКлюч
		+ "(?:<div class=""__SINCE_SHOW_STYLE__[^/]*/p>)?";
	Вхождения = ВычислительРегВыражений.НайтиВхождения(ТекстДокумента);
	Если Вхождения.Количество() > 0 Тогда
		ТекстДокумента = СокрЛ(Сред(ТекстДокумента, Вхождения[0].FirstIndex + Вхождения[0].Length + 1));
	КонецЕсли;
	МаркерЗначения = "<p class=""V8SH_chapter"">Значения</p>";
	Если Найти(ТекстДокумента, МаркерЗначения) > 0 Тогда
		ТекстДокумента = СтрЗаменить(ТекстДокумента, МаркерЗначения, СтрЗаменить(МаркерЗначения, "</p>", ":</p>"));
		ВычислительРегВыражений.Pattern = " \(\w+\)(</a><br><[ap])";
		ТекстДокумента = ВычислительРегВыражений.Заменить(ТекстДокумента, ",$1");
	КонецЕсли;
	ОбрезатьОписание(ТекстДокумента);
	ТекстДокумента = ИзвлечьИзФрагментаHTMLОбычныйТекст(ТекстДокумента, Ложь);
	Если Найти(СтрокаОписания.Слово, "<") > 0 Тогда
...
Функция ИзвлечьИзФрагментаHTMLОбычныйТекст(Знач Текст, Знач УдалитьКрайнююТочку = Истина) Экспорт 
	...
	ВычислительРегВыражений.Pattern = "(?:<br>|</p>)";
...
Процедура ОбрезатьОписание(Текст)
	...
	ВычислительРегВыражений.Pattern = "<p class=""?V8SH_chapter""?>Пример:</p>\s*<table[\s\S]*?</table>";
	Текст = ВычислительРегВыражений.Заменить(Текст, "");
	// Все заголовки выносим на новую строку с префиксом "> "
	ВычислительРегВыражений.Pattern = "<p class=""?V8SH_chapter""?>([^<]+)" + шКонецТегаКлюч + "<p[^>]*>";
	Текст = ВычислительРегВыражений.Заменить(Текст, Символы.ПС + "> $1 ");
КонецПроцедуры
...
ВычислительРегВыражений2 = ирОбщий.НовоеРегВыражениеЛкс();
