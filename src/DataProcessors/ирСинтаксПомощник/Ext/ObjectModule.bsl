...
Функция ЗагрузитьОписаниеСвойства(Знач СтрокаОписания) Экспорт 
	...
	ВычислительРегВыражений.Pattern = ирОбщий.ТекстДляРегВыраженияЛкс(">" + СтрокаОписания.Слово + " (") + мПлатформа.шИмя + ирОбщий.ТекстДляРегВыраженияЛкс(")</p>");
	Вхождения = ВычислительРегВыражений.НайтиВхождения(ТекстДокумента);
	Если Вхождения.Количество() > 0 Тогда
		ТекстДокумента = Сред(ТекстДокумента, Вхождения[0].FirstIndex + Вхождения[0].Length + 1);
	КонецЕсли;
	СтрокаОписания.Описание = ИзвлечьИзФрагментаHTMLОбычныйТекст(ТекстДокумента, Ложь);
	Возврат СтрокаОписания.Описание;      
КонецФункции
