...
Функция КлючСсылающегосяОбъекта(Знач ТекущаяСтрока)
	Возврат ирКлиент.КлючОбъектаСтрокиВерсииДанныхЛкс(ТекущаяСтрока);
	...
	//	КлючОбъекта = ЗначениеИзСтрокиВнутр(ТекущаяСтрока.ДанныеСтрока);
	//КонецЕсли;
	//Возврат КлючОбъекта;
КонецФункции
		...
		Для Каждого ОписаниеТаблицы Из ТаблицаВсехТаблицБД Цикл
			...
			Для Каждого ПолеТаблицыБД Из ПоляТаблицыБД Цикл
				...
				Для Каждого ТипКолонки Из Типы Цикл
					...
					КонецЕсли;
					ОбработатьШагСвязи(ОписаниеТаблицы.ПолноеИмя, ИменаСвойствТаблицы, КэшПутейКНачалу, ПутьТекущейТаблицыКНачалу, ТипСсылкиТаблицыЦикл, ТипСсылкиНачала, ТипСсылкиКонца, ДлинаЦикл, ИмяТипаКолонки,
						ИмяПоляБД, СвязиКТаблицеБД, ИзбранныеСвязи, ТипКолонки, СостояниеОбновленияФормы, "ПутьКОсновномуОбъекту");
					ОбработатьШагСвязи(ОписаниеТаблицы.ПолноеИмя, ИменаСвойствТаблицы, КэшПутейККонцу, ПутьТекущейТаблицыККонцу, ТипСсылкиТаблицыЦикл, ТипСсылкиКонца, ТипСсылкиНачала, ДлинаЦикл, ИмяТипаКолонки,
...
Процедура ОбработатьШагСвязи(ИмяРодительскойТаблицы, ИменаСвойствТаблицы, КэшПутей, ТекущаяТаблица, ТипСсылкиТаблицыЦикл, ТипСсылкиК, ТипСсылкиОт, ДлинаГлобальная, ИмяТипаКолонки, ИмяПоляБД, СвязиКТаблицеБД, 
	...
	КонецЕсли;
	Если Истина
		И ТипСсылкиТаблицыЦикл = ТипСсылкиОт 
...
Функция СоздатьЗапросСвязиДляТаблицы(Знач СтрокаСвязи = Неопределено, Знач ТолькоКоличество = Ложь) Экспорт
	...
	Если СтрокаСвязи.ПутьКСвязанномуОбъекту = "" Тогда
		...
		Запрос.Текст = ТекстВЫБРАТЬ + "Т.Ссылка)
			...
			|ГДЕ
			|	" + ЧастьЗапроса.ПоследнийПсевдоним + " = &Ссылка";
	ИначеЕсли СтрокаСвязи.ПутьКОсновномуОбъекту = "" Тогда
		...
		Запрос.Текст = ТекстВЫБРАТЬ + ЧастьЗапроса.ПоследнийПсевдоним + ") КАК Ссылка
			|ИЗ
			|	" + ЧастьЗапроса.ТекстИз + "
			|ГДЕ
			|	Т.Ссылка = &Ссылка
			|	И НЕ " + ЧастьЗапроса.ПоследнийПсевдоним + " ЕСТЬ NULL";
	Иначе
		...
		КонецЕсли;
		Запрос.Текст = ТекстВЫБРАТЬ + ЧастьЗапросаИсходящая.ПоследнийПсевдоним + ") КАК Ссылка
			|ИЗ
			|	" + ТекстИз + "
			|ГДЕ
			|	" + ЧастьЗапросаВходящая.ПоследнийПсевдоним + " = &Ссылка
			|	И НЕ " + ЧастьЗапросаИсходящая.ПоследнийПсевдоним + " ЕСТЬ NULL";
	КонецЕсли;
	Запрос.УстановитьПараметр("Ссылка", ВыбОбъект);
	Возврат Запрос;
КонецФункции
...
Функция ПостроитьЧастьЗапросаИзПутиСвязи(ИмяОсновнойТаблицы, ПутьСтрокой, ПсевдонимОсновнойТаблицы = "Т")
	...
	ПредыдущийПсевдоним = ПсевдонимОсновнойТаблицы;
	НомерТЧ = 1;
	Для каждого ЭлементПути Из РазобранныйПуть Цикл
		Если ЭлементПути.Тип = "ТабличнаяЧасть" Тогда
			...
			ТекстИз = ТекстИз + "
				...
				|	ПО " + ПредыдущийПсевдоним + ".Ссылка = " + ПсевдонимТЧ + ".Ссылка";
			ПредыдущийПсевдоним = ПсевдонимТЧ;
		ИначеЕсли ЭлементПути.Тип = "Поле" Тогда
			ПредыдущийПсевдоним = ПредыдущийПсевдоним + "." + ЭлементПути.Имя;
		КонецЕсли;
	КонецЦикла;
	Возврат Новый Структура("ТекстИз, ПоследнийПсевдоним", ТекстИз, ПредыдущийПсевдоним);
КонецФункции
...
Процедура КП_СвязиКТаблицеБДОбновитьКоличество(Кнопка)
	...
	Для Каждого СтрокаСвязи Из СвязиКТаблицеБД Цикл
		...
		СтрокаСвязи.КоличествоОтобрано = Запрос.Выполнить().Выгрузить()[0].Ссылка;
