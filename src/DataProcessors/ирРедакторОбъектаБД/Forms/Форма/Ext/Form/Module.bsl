			...
			Если СписокСвойств.Количество() > 0 Тогда
				...
				Попытка
					...
					Для Каждого СтрокаРеквизита Из РеквизитыТаблица Цикл
						...
						КонецЕсли; 
					КонецЦикла;
					СтруктураНабораЗаписей = ирОбщий.ОбъектБДПоКлючуЛкс("РегистрСведений.ДополнительныеСведения", Запрос.Параметры); 
...
Процедура ОсновныеДействияФормыВыгрузитьВXML(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(СериализованныйОбъект, ПредставлениеОбъектаБД(), "XML");
...
Процедура КлсКомандаТаблицаНажатие(Кнопка)
	ирОбщий.УниверсальнаяКомандаФормыЛкс(ЭтаФорма, Кнопка, ТекущееТабличноеПоле());
КонецПроцедуры
