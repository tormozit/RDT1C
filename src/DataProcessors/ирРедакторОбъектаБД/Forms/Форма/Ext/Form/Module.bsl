...
Перем мПоследнийМоментНачалаЧтенияДопПолей;

Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт
	выхИменаСвойств = "Реквизит.АвтозапускПодсчетаСсылок, Реквизит.АвтозапускПоискаСсылок, Реквизит.ИмяСиноним, Реквизит.ИскатьВложенныеДоЧислаСтрок, Реквизит.КомпоновщикДопПолей, Реквизит.ПорядокРеквизитов, Реквизит.СвязиИПараметрыВыбора, Реквизит.СчитыватьПослеЗаписи, Форма.ВнешняяНавигационнаяСсылка";
...
Функция ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта, ДанныеИзБД = Истина, СсылкаИзменилась = Истина)
	...
	ЭлементыФормы.СчитыватьПослеЗаписи.Видимость = фОбъект.ЭтоНаборЗаписей;
	...
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		ирКлиент.ТабличноеПолеСОтборомПросмотраУстановитьДанныеЛкс(ЭтаФорма, ЭлементыФормы.РеквизитыТаблица, Ложь);
	...
	Если фОбъект.ЭтоСсылочныйОбъект Или фОбъект.мРежимМенеджераЗаписи Или фОбъект.ЭтоКонстанта Тогда
		...
		РеквизитыТаблицаИспользоватьОтборПриИзменении();
			...
			Тогда
				...
				Если ВыполнитьНачальноеЗаполнениеИзбранного Тогда
					...
					СтрокаСвязи.Избранное = Истина
						...
						И Не СтрокаСвязи.ТипТаблицы = "Очередь"
...
Процедура ПриОткрытии()
	...
	//! РеквизитыТаблицаОтобранное.Колонки.Добавить("РасширенноеЗначение");
	//! РеквизитыТаблицаОтобранное.Колонки.Добавить("ЗначенияФункОпций");
...
Функция КнопкаЗаписатьНажатие(Элемент = Неопределено, РежимЗаписи = "", ОбратимаяЗапись = Неопределено)
	...
	Иначе
		...
		Если Не ОбратимаяЗапись Тогда
			...
			КонецЕсли;
			Если Ложь
				Или ЗначениеЗаполнено(НоваяСсылка)
				Или ЭтоНаборЗаписей И СчитыватьПослеЗаписи
			Тогда
				ПеречитатьДанные(,, Ложь);
				ЭтаФорма.Модифицированность = Ложь;
			КонецЕсли;
...
Процедура ЛиЧитатьПисатьДвиженияПриИзменении(Элемент = Неопределено) Экспорт
...
Процедура РеквизитыТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирКлиент.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, КнопкаИдентификаторы, "ЗначениеРеквизита",
			Новый Структура("ЗначениеРеквизита", "РасширенноеЗначение"), Истина,, "ЗначениеРеквизита",,, РеквизитыСтрокаПоиска);
...
Функция ТаблицаЗначенийТабличнойЧасти(ИмяТЧ)
	Возврат ЭлементыФормы.ПанельТабличныеЧасти.Страницы["Страница" + мПрефиксИмениТабличногоПоляТЧ + ИмяТЧ].Значение.ТаблицаЗначений; // ТаблицаЗначений
КонецФункции
...
Функция ТаблицаЗначенийДвижения(Знач ПолноеИмяРегистра)
	...
	Возврат ЭлементыФормы.ПанельДвижения.Страницы["Страница" + ИмяТабличногоПоляРегистра].Значение.ТаблицаЗначений; // ТаблицаЗначений
КонецФункции
...
Процедура ОсновныеДействияФормыСкопировать(Кнопка)
	Если фОбъект.ЭтоСсылочныйОбъект Тогда
		ФормаРедактора = ирКлиент.ОткрытьОбъектВРедактореОбъектаБДЛкс(ПолучитьИзмененныйОбъект().Методы.Скопировать(),, Новый УникальныйИдентификатор);
		Если Истина
			И Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(фОбъект.ВыбОбъект))
			И ЛиЧитатьПисатьДвижения
		Тогда
			Ответ = Вопрос("Хотите скопировать движения документа?", РежимДиалогаВопрос.ДаНет);
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ФормаРедактора.ЛиЧитатьПисатьДвижения = Истина;
				ФормаРедактора.ЛиЧитатьПисатьДвиженияПриИзменении();
				Для Каждого СтрокаРегистраИсточник Из СтраницыДвижения Цикл
					СтрокаРегистраПриемник = ФормаРедактора.СтраницыДвижения.Найти(СтрокаРегистраИсточник.Имя, "Имя");
					ЗаполнитьЗначенияСвойств(СтрокаРегистраПриемник, СтрокаРегистраИсточник);
					ИмяТабличногоПоляРегистра = ПолучитьИмяТаблицыФормыРегистра(СтрокаРегистраПриемник.ПолноеИмя);
					ФормаРедактора.ЭлементыФормы.ПанельДвижения.Страницы["Страница" + ИмяТабличногоПоляРегистра].Значение.ТаблицаЗначений = ТаблицаЗначенийДвижения(СтрокаРегистраПриемник.ПолноеИмя).Скопировать();
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнительныеПоляПрочитатьПоля(Кнопка = Неопределено)
	мПоследнийМоментНачалаЧтенияДопПолей = ТекущаяДата();
...
Процедура ПослеЧтенияДопПолей(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	Если Ложь
		...
		Или РезультатЗадания.МоментНачала < мПоследнийМоментНачалаЧтенияДопПолей
	Тогда
		Возврат;
	КонецЕсли;
	...
	Для ДлинаЦикл = 1 По МаксДлинаСвязи Цикл
		...
		Для Каждого ОписаниеТаблицы Из ТаблицаВсехТаблицБД Цикл
			...
			Если Ложь
				...
				Или ОписаниеТаблицы.Тип = "Очередь"
			Тогда
				Продолжить;
			КонецЕсли;
...
Процедура РеквизитыТаблицаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ирКлиент.ТабличноеПолеСОтборомПросмотраПриОкончанииРедактированияЛкс(ЭтаФорма, Элемент);
КонецПроцедуры

Процедура РеквизитыТаблицаИспользоватьОтборПриИзменении(Элемент = Неопределено)
	ЭлементОтбора = ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(РеквизитыТаблицаКомпоновщик.Настройки, "ЗначениеРеквизита",, ВидСравненияКомпоновкиДанных.Заполнено);
	ЭлементОтбора.Использование = РеквизитыТаблицаИспользоватьОтбор;
	ирКлиент.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.РеквизитыТаблица);
КонецПроцедуры
...
ЭтаФорма.РеквизитыТаблица = РеквизитыТаблицаОтобранное;
...
фОбъект.СчитыватьПослеЗаписи = Истина; 
// РеквизитыСтрокаПоиска - - специально отвязано от РеквизитыТаблица чтобы в фильтре не применялось, т.к. там будут ошибки из-за недоступных в запросе типов 
