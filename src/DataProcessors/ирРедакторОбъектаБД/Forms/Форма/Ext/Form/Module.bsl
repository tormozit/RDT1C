...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено, Знач ЗапретитьЗаполнение = Неопределено)
	...
	Тогда
		...
		Иначе
			...
			НоваяСсылка = "";
		КонецЕсли;
		Для Каждого ОбъектМД Из Метаданные.КритерииОтбора Цикл
			#Если Сервер И Не Сервер Тогда
				ОбъектМД = Метаданные.КритерииОтбора.ДокументыПоКонтрагенту;
			#КонецЕсли
			Если ОбъектМД.Тип.СодержитТип(ТипЗнч(НоваяСсылка)) Тогда
				СтрокаКритерия = ТаблицаКритерииОтбора.Добавить();
				СтрокаКритерия.Имя = ОбъектМД.Имя;
				СтрокаКритерия.Представление = ОбъектМД.Представление();
...
Процедура КП_СсылкиНаОбъектОбновить(Кнопка = Неопределено)
	...
	Если ЭлементыФормы.ПанельТипПоискаСсылок.ТекущаяСтраница = ЭлементыФормы.ПанельТипПоискаСсылок.Страницы.ТаблицаКритерииОтбора Тогда
		...
		РезультатЗадания.Вставить("ТаблицаСсылок", СсылкиНаОбъект.ВыгрузитьКолонки());
		Запрос = Новый Запрос;
...
Процедура КритерииОтбораПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	ирКлиент.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
КонецПроцедуры
...
Процедура КонтекстныеМенюКолонкаБД(Кнопка)
