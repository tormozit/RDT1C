...
Процедура ОбновитьРазмерыДинамическогоТабличногоПоля(Знач ПанельТаблиц)
	...
	Если ЗначениеЗаполнено(ИмяСтраницы) Тогда
		ОбновитьРазмерыТабличногоПоляСтраницы(ПанельТаблиц, ЭлементыФормы[Сред(ИмяСтраницы, СтрДлина("Страница") + 1)]);
	КонецЕсли;
	...
	Если ЭлементыФормы.ПанельДвижения.Видимость Тогда
		...
		Если ТабличноеПолеДвижения.Колонки.Количество() = 0 Тогда
			...
			Если Не ирОбщий.ЛиКорневойТипПоследовательностиЛкс(ирОбщий.ТипТаблицыБДЛкс(ТекущаяСтрока.ПолноеИмя)) Тогда
				ПоляТаблицыБД = ирОбщий.ПоляТаблицыМДЛкс(МетаРегистр.ПолноеИмя());
			Иначе
...
Процедура НастроитьКолонкиТабличногоПоляДвиженийИлиТЧ(Знач ТабличноеПолеНабораДвиженийИлиТЧ)
	...
	#Если Сервер И Не Сервер Тогда
		НаборЗаписейПриВыводеСтроки();
		НаборЗаписейПриАктивизацииСтроки();
		НаборЗаписейПриИзмененииФлажка();
	#КонецЕсли
	ТабличноеПолеНабораДвиженийИлиТЧ.УстановитьДействие("ПриВыводеСтроки", Новый Действие("НаборЗаписейПриВыводеСтроки"));
	ТабличноеПолеНабораДвиженийИлиТЧ.УстановитьДействие("ПриАктивизацииСтроки", Новый Действие("НаборЗаписейПриАктивизацииСтроки"));
	ТабличноеПолеНабораДвиженийИлиТЧ.УстановитьДействие("ПриИзмененииФлажка", Новый Действие("НаборЗаписейПриИзмененииФлажка"));
...
Процедура ОсновныеДействияФормыПоказатьРазличияБД(Кнопка)
	...
	Если ТекстПосле <> Неопределено Тогда
		ирОбщий.СравнитьЗначенияВФормеЛкс(ТекстДо, ТекстПосле, , "Старое", "Новое", , Ложь);
	КонецЕсли; 
	...
	КонецЕсли; 
	ирОбщий.СравнитьЗначенияВФормеЛкс(СтруктураОбъектаДо.Методы, СтруктураОбъектаПосле.Методы,,,,,, ТекущийРеквизит);
...
Процедура КлсКомандаТаблицаНажатие(Кнопка)
	...
	Если ИсточникДействий = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ирОбщий.УниверсальнаяКомандаФормыЛкс(ЭтаФорма, Кнопка, ИсточникДействий);
КонецПроцедуры
...
Функция ИмяТаблицыБДТабличногоПоля(Знач ТабличноеПоле, выхЭтоТЧ = Ложь, Старое = Ложь)
	...
	Если ЗначениеЗаполнено(ИмяДвижения) Тогда
		ИмяТаблицыБД = ИмяТаблицыБДТекущегоРегистраДвижений();
	КонецЕсли;
...
Процедура ОсновныеДействияФормыУстановитьЗначениеВКолонке(Кнопка)
	...
	КонецЕсли;
	ирОбщий.ОткрытьМенеджерТабличногоПоляЛкс(ТабличноеПоле, ЭтаФорма, "Обработка", МетаданныеВыбора);
КонецПроцедуры
		...
		Если ЗначениеЗаполнено(ИмяТаблицыБДТабличногоПоля) Тогда
			...
			Если СтрокаПоля <> Неопределено Тогда
				...
				Если ирОбщий.СтрНачинаетсяСЛкс(ТабличноеПоле.Имя, мПрефиксИмениТабличногоПоляТЧ) Тогда
					...
					СтруктураТЧ = Новый Структура(Сред(ТабличноеПоле.Имя, СтрДлина(мПрефиксИмениТабличногоПоляТЧ + 1)), ТабличноеПоле.ТекущаяСтрока);
				Иначе
					ОсновныеДанные = ТабличноеПоле.ТекущаяСтрока;
					СтруктураТЧ = Неопределено;
				КонецЕсли; 
				СтруктураОтбора = ирОбщий.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(ОсновныеДанные, МетаРеквизит, СтруктураТЧ);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
...
Процедура ТаблицаОтбораЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если фОбъект.СвязиИПараметрыВыбора Тогда
		...
		КонецЦикла;
		СтруктураОтбора = ирОбщий.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(ЗначенияРеквизитов, МетаРеквизит);
	КонецЕсли; 
...
Функция ИмяТаблицыБДТекущегоРегистраДвижений()
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
	ИмяТаблицыБДРегистра = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ТекущаяСтрока.ПолноеИмя,, Ложь);
