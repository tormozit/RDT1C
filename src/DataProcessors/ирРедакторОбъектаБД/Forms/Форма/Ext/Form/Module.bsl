...
Процедура НастроитьЭлементыФормы(ЭтаФорма)
	...
	Если ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.Страницы.СтраницаСтраницыПанельДвижения Тогда
		ПодсказкаКнопки = "Записать все таблицы движений";
	Иначе
...
Функция ПоместитьДанныеОбъектаВФорму(СтруктураОбъекта, ДанныеИзБД = Истина, СсылкаИзменилась = Истина) 
	...
	Иначе // Это набор записей
		...
		ИмяНередактируемойКолонки = "МоментВремени";
		Если Истина
			И НаборЗаписей.Колонки.Найти(ИмяНередактируемойКолонки) <> Неопределено 
			И ирОбщий.ПоляТаблицыБДЛкс(ИмяОсновнойТаблицы).Найти(ИмяНередактируемойКолонки, "Имя") = Неопределено
		Тогда
			// https://www.hostedredmine.com/issues/946859
			НаборЗаписей.Колонки.Удалить(ИмяНередактируемойКолонки);
		КонецЕсли;
		...
		ирОбщий.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ЭлементыФормы.НаборЗаписей);
...
Процедура ПриАктивизацииСтрокиСтраницыДвижений(СброситьОтборПросмотра = Истина)
	...
	Если ЭлементыФормы.ПанельДвижения.Видимость Тогда
		...
		Если ТабличноеПолеДвижения.Колонки.Количество() = 0 Тогда
			...
			ОбновитьЗаголовкиКолонокДинамическойТаблицы(ИмяТаблицыБДРегистра, ТабличноеПолеДвижения.Имя);
		КонецЕсли; 
		...
		ирОбщий.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ТабличноеПолеДвижения);
		Если СброситьОтборПросмотра Тогда
			ЭтаФорма.ДвижениеИспользоватьОтбор = Ложь;
		КонецЕсли; 
		ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ТабличноеПолеДвижения);
	КонецЕсли; 
...
Процедура ПриАктивизацииСтрокиСтраницыТабличныхЧастей(СброситьОтборПросмотра = Истина)
	...
	Если ЭлементыФормы.ПанельТабличныеЧасти.Видимость Тогда
		...
		Если ТабличноеПолеТабличнойЧасти.Колонки.Количество() = 0 Тогда
			...
			ОбновитьЗаголовкиКолонокДинамическойТаблицы(ИмяОсновнойТаблицы + "." + ТекущаяСтрока.Имя, ТабличноеПолеТабличнойЧасти.Имя);
		КонецЕсли;
		...
		ирОбщий.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ТабличноеПолеТабличнойЧасти);
		Если СброситьОтборПросмотра Тогда
			ЭтаФорма.ТабличнаяЧастьИспользоватьОтбор = Ложь;
		КонецЕсли; 
		ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ТабличноеПолеТабличнойЧасти);
	КонецЕсли; 
...
Процедура НастроитьКолонкиТабличногоПоляДвиженийИлиТЧ(Знач ТабличноеПолеНабораДвиженийИлиТЧ)
	...
	КонецЕсли; 
	Если Ложь
		Или ТабличноеПолеНабораДвиженийИлиТЧ = ЭлементыФормы.НаборЗаписей
		Или ирОбщий.СтрНачинаетсяСЛкс(ТабличноеПолеНабораДвиженийИлиТЧ.Имя, мПрефиксИмениТабличногоПоляДвижения) 
	Тогда
		КолонкаАктивность = ТабличноеПолеНабораДвиженийИлиТЧ.Колонки.Найти("Активность");
		Если КолонкаАктивность <> Неопределено Тогда
			КолонкаАктивность.ТолькоПросмотр = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура РеквизитыТаблицаЗначениеРеквизитаНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	УспехВыбора = ирОбщий.ПолеВводаКолонкиЗначенияРеквизита_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.РеквизитыТаблица, ИмяОсновнойТаблицы, "ИмяРеквизита", "ЗначениеРеквизита",
		СвязиИПараметрыВыбора, СтандартнаяОбработка, ВыбОбъект);
...
Процедура СвязанныеДанныеОбновитьКоличество(Кнопка = Неопределено)
	...
	#КонецЕсли
	ирОбщий.ВыполнитьЗапросЛкс(Запрос.Текст, Запрос.Параметры, ЭтаФорма, ЭлементыФормы.КП_СвязанныеКолонки.Кнопки.ОбновитьКоличество, "ОбновитьКоличествоЗавершение");
...
Процедура ОбновитьКоличествоЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		ТаблицаРезультата = РезультатЗадания[0].Выгрузить();
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено)
	...
	ЭтаФорма.фОбъект.ТипОсновнойТаблицы = ирОбщий.ТипТаблицыБДЛкс(ИмяОсновнойТаблицы);
...
Процедура ОсновныеДействияФормыИзменитьИдентификатор(Кнопка)
	...
	//ОригиналОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(ИмяОсновнойТаблицы, фОбъект.ВыбОбъект);
	//КопияОбъекта = ирОбщий.ОбъектБДПоКлючуЛкс(ИмяОсновнойТаблицы, фОбъект.ВыбОбъект);
...
Процедура НаборЗаписейПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
	...
	Кнопки = ЭлементыФормы.ОсновныеДействияФормы.Кнопки.ПодменюТекущаяТаблица.Кнопки;
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, Кнопки.Идентификаторы);
	ирОбщий.ПрисвоитьЕслиНеРавноЛкс(Элемент.Подвал, Кнопки.ПоказыватьИтоги.Пометка);
	Если Истина
	...
	КонецЕсли; 
	ОформитьЯчейкиПоМетаданным(Элемент, ИмяТаблицыБД, ОформлениеСтроки, Кнопки.Идентификаторы);
...
Процедура КоманднаяПанельНабораДвиженийПереключитьАктивность(Кнопка)
	...
	Тогда
		...
		КонецЦикла;
	КонецЕсли; 
	ТаблицаСРедакторомСтрокиПриАктивизацииСтроки(ТабличноеПоле);
...
Процедура ОсновныеДействияФормыОткрытьЗагрузкуДанныхВТабличноеПоле(Кнопка)
	...
	КонецЕсли;
	ирОбщий.ЗагрузитьДанныеВТабличноеПолеЛкс(ЭтаФорма, ТабличноеПоле, Ложь, МетаданныеКолонок,, ПолучитьОбъект().Данные); 
	ОбновитьЗаголовкиСтраниц();
КонецПроцедуры
...
Процедура ПоискСсылокЗавершитьВФорме(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт 
	...
	Тогда
		...
		#Если Сервер И Не Сервер Тогда
			ОбработкаОбъект = Обработки.ирРедакторОбъектаБД.Создать();
		#КонецЕсли
		ОбработкаОбъект.ОбработатьТаблицуНайденныхСсылок(РезультатЗадания.ТаблицаСсылок, РезультатЗадания.СсылкиНа, фОбъект.СсылкиНаОбъект);
...
Процедура ОбновитьОтборСтрокСпискаТабличныхЧастей(НовоеИспользованиеОтбора)
	...
	Кнопка = ЭлементыФормы.КП_СтраницыТабличныхЧастей.Кнопки.ТабличныеЧастиСкрыватьПустые;
	Кнопка.Пометка = НовоеИспользованиеОтбора;
...
Процедура ТаблицаСРедакторомСтрокиПриАктивизацииСтроки(Элемент)
...
Процедура КП_СвязанныеКолонкиУстановитьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.СвязанныеДанные,, Истина);

КонецПроцедуры

Процедура КП_СвязанныеКолонкиСнятьПометки(Кнопка)
	
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.СвязанныеДанные,, Ложь);
...
Процедура НаборЗаписейПриАктивизацииСтроки(Элемент)
	...
	ирОбщий.ТабличноеПолеПриАктивизацииСтрокиЛкс(ЭтаФорма, Элемент);
	ТаблицаСРедакторомСтрокиПриАктивизацииСтроки(Элемент);
...
Процедура НастройкаКолонокТаблицы(Кнопка)
	...
	ФормаНастроек.ОбработкаОбъектСлужебная.НастройкиКолонок.Загрузить(мНастройкаКолонок.НастройкиКолонок.Выгрузить());
	ФормаНастроек.СсылкаОбъекта = КлючОбъекта();
	мНастройкаКолонок.ПолноеИмяТаблицы = ПолноеИмяМетаданных;
	ПоляТаблицы = ирКэш.ПоляТаблицыБДЛкс(ПолноеИмяМетаданных);
	#Если Сервер И Не Сервер Тогда
	...
	Если ТабличноеПоле.ТекущаяКолонка <> Неопределено Тогда
		ФормаНастроек.ПараметрИмяТекущейКолонки = ТабличноеПоле.ТекущаяКолонка.Имя;
	КонецЕсли; 
...
Процедура ОбработатьВыборНастроекКолонок(Знач ВыбранноеЗначение)
	...
	#КонецЕсли
	ТабличноеПоле = ТекущееТабличноеПоле();
	СтарыеНастройки = мНастройкаКолонок.НастройкиКолонок.Выгрузить();
	мНастройкаКолонок.ПрименитьНастройкиКолонокКТабличномуПолю(ТабличноеПоле, ВыбранноеЗначение);
	Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение.Свойство("Сохранить") И ВыбранноеЗначение.Сохранить Тогда
		СохранитьНастройкиТаблицы(ТабличноеПоле);
	Иначе
		мНастройкаКолонок.НастройкиКолонок.Загрузить(СтарыеНастройки);
	КонецЕсли; 
...
Процедура КоманднаяПанельТабличнаяЧастьКолонкиБД(Кнопка)
	
	ТекущаяСтрока = ЭлементыФормы.СтраницыПанельТабличныеЧасти.ТекущаяСтрока;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ПолноеИмяТаблицы = фОбъект.мПолноеИмяМД + "." + ЭлементыФормы.СтраницыПанельТабличныеЧасти.ТекущаяСтрока.Имя;
	НоваяФорма = ирОбщий.ПолучитьФормуЛкс("Обработка.ирКолонкиБД.Форма",,, ПолноеИмяТаблицы);
	ТабличноеПолеТЧ = ЭлементыФормы[ЭлементыФормы.СтраницыПанельТабличныеЧасти.ТекущаяСтрока.ИмяСтраницы];
	Если ТабличноеПолеТЧ.ТекущаяКолонка <> Неопределено Тогда
		НоваяФорма.ПараметрТекущаяСтрока = ПолноеИмяТаблицы + "." + ТабличноеПолеТЧ.ТекущаяКолонка.Имя;
	КонецЕсли;
	НоваяФорма.Открыть();
