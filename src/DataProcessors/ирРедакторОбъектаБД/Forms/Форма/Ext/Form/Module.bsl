...
Функция ДобавитьСтрокуТаблицыРеквизиты(ЗначениеРеквизита, ИмяРеквизита, Представление = "", ОписаниеТипов = Неопределено, ТолькоПросмотр = Ложь, ДанныеИзБД = Истина, МетаРеквизит = Неопределено)
	...
	Тогда
		...
		РолиПоля = ирОбщий.РольПоляБДЛкс(МетаРеквизит);
...
Функция УстановитьТаблицуБД(Знач НовоеИмяОсновнойТаблицы) Экспорт 
	...
	Если ИмяОсновнойТаблицы <> НовоеИмяОсновнойТаблицы Тогда
		//ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(ЭлементыФормы.ИмяОсновнойТаблицы, НовоеИмяОсновнойТаблицы);
...
Процедура ОбновитьТаблицуСвязанныхКолонок(Принудительно = Ложь)
	...
	Если Ложь
		...
		Или (Истина
			...
			И Не ирОбщий.ПолучитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельОсновная.ТекущаяСтраница)) 
	Тогда
...
Процедура ОбновитьСвязанныйДинамическийСписок()
	...
	Если Элемент.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	мСвязанныйДинамическийСписок.УстановитьОбъектМетаданных(Элемент.ТекущаяСтрока.ПолноеИмяТаблицы);
	...
	Для Каждого ПолеТаблицы Из ПоляТаблицы Цикл
		...
		Если КолонкаТП <> Неопределено Тогда
			Если ЗначениеЗаполнено(КолонкаТП.Данные) Тогда
				...
				КонецЕсли;
				ирОбщий.ДобавитьОписаниеТиповВПодсказкуШапкиКолонкиЛкс(КолонкаТП, ПолеТаблицы.ТипЗначения, ПолеТаблицы.Метаданные, ДобавлятьРоль);
			КонецЕсли; 
		Иначе
...
Процедура ПолеВводаКолонки_ПриИзменении(Элемент)
	...
	ТабличноеПоле = ТекущееТабличноеПоле();
	ирОбщий.ТабличноеПолеСОтборомПросмотраПриОкончанииРедактированияЛкс(ЭтаФорма, ТабличноеПоле);
...
Процедура РеквизитыТаблицаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Колонка = ЭлементыФормы.РеквизитыТаблица.Колонки.ЗначениеРеквизита Тогда
		...
		СтрокаПоиска = СтрокаПоискаВРеквизите();
		Если ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка, ВыбраннаяСтрока.РасширенноеЗначение, СтрокуРазрешеноРедактировать,,, 
	...
	Тогда 
		ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка,,,,, ВыбраннаяСтрока.ПредставлениеРеквизита);
...
Процедура ОткрытьДинамичекийСписокСвязаннойТаблицы(ИспользоватьДинамическийСписокИР = Неопределено, СвязанныйДинамическийСписокИР = Ложь)
	...
	Иначе
		Если СвязанныйДинамическийСписокИР Тогда
			мСвязанныйДинамическийСписок = ирОбщий.ФормаДинамическогоСпискаЛкс(,, "Связанный");
