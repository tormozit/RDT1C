...
Процедура ПеречитатьДанные(Знач СохранятьИдентификаторСсылки = Ложь, ТолькоМакет = Ложь, СсылкаИзменилась = Истина)
	...
	Если ЗначениеЗаполнено(ВыбОбъект) Тогда
		ирКлиент.НачатьОтслеживаниеЧужойФормыЛкс(ЭтаФорма, Истина);
	КонецЕсли;
...
Процедура НастроитьТабличноеПолеНабораЗаписей()
	...
	#Если Сервер И Не Сервер Тогда
		ПолеРегистратора = Обработки.ирТипПолеБД.Создать();
	#КонецЕсли
	Если Истина
...
Функция ЗаписатьОбъект(Знач РежимЗаписи, ОбратимаяЗапись = Ложь)
	...
	Попытка
		ирОбщий.ЗаписатьОбъектЛкс(СтруктураОбъекта.Методы, , РежимЗаписи,,,,,, Истина, АдресСообщений);
		Если фОбъект.ЭтоСсылочныйОбъект Тогда
	...
	Исключение
		...
		#КонецЕсли
		ОписаниеОшибки = ОписаниеОшибки();
		ЗаголовокПоля = ирОбщий.СтрокаМеждуМаркерамиЛкс(ОписаниеОшибки, """", """", Ложь);
		...
		Для Каждого Сообщение Из Сообщения Цикл
			...
			Если ЗначениеЗаполнено(ИмяПоля) Тогда
				...
				Если Найти(ИмяПоля, ".") > 0 Тогда
					...
					ТабличноеПоле = ЭлементыФормы[мПрефиксИмениТабличногоПоляТЧ + ИмяТЧ];
					ИндексСтроки = Число(ирОбщий.СтрокаМеждуМаркерамиЛкс(ИмяПоля, "[", "]"));
...
Процедура РеквизитыТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ПоискВОбъекте.НайтиСтроки(КлючПоиска).Количество() > 0 Тогда
		ОтметкаНезаполненного = Не ЗначениеЗаполнено(ДанныеСтроки.ЗначениеРеквизита);
		//Если Не ОтметкаНезаполненного Тогда
			ОформлениеСтроки.Ячейки.ЗначениеРеквизита.ЦветФона = ирОбщий.ЦветФонаОшибкиЛкс();
		//КонецЕсли;
	Иначе
...
Процедура РеквизитыТаблицаЗначениеРеквизитаОкончаниеВводаТекста(Элемент, Текст, Значение, СтандартнаяОбработка)
	...
	ирКлиент.ПолеВвода_ОкончаниеВводаТекстаЛкс(Элемент, Текст, Значение, СтандартнаяОбработка, ЭлементыФормы.РеквизитыТаблица.ТекущаяСтрока.ЗначениеРеквизита);
...
Процедура НаборЗаписейПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
	...
	КонецЕсли; 
	ОшибкиЗаполнения = ПоискВОбъекте.НайтиСтроки(Новый Структура("ТаблицаОбъекта, НомерСтрокиТаблицы", ИмяТЧ, Элемент.Значение.Индекс(ДанныеСтроки) + 1));
	ЦветФонаОшибки = ирОбщий.ЦветФонаОшибкиЛкс();
	Для Каждого ОшибкаЗаполнения Из ОшибкиЗаполнения Цикл
		ОформлениеСтроки.Ячейки[ОшибкаЗаполнения.Реквизит].ЦветФона = ЦветФонаОшибки;
...
Процедура НайденныеСсылкиВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ТекущиеДанные = ирКлиент.ДанныеСтрокиТабличногоПоляЛкс(Элемент);
...
Процедура ОткрытьДинамичекийСписокСвязаннойТаблицы(ИспользоватьДинамическийСписокИР = Неопределено, СвязанныйДинамическийСписокИР = Ложь)
	...
	Иначе
		Если СвязанныйДинамическийСписокИР Тогда
			мСвязанныйДинамическийСписок = ирКлиент.ФормаДинамическогоСпискаЛкс(,, "Связанный");
...
Процедура КП_СвязанныеКолонкиМетаданныеКолонкиБД(Кнопка)
	...
	ПолеТаблицы = ПоляТаблицыБД.Найти(ТекущаяСтрока.ИмяКолонки, "Имя");
	Если ПолеТаблицы.Метаданные <> Неопределено Тогда
		#Если Сервер И Не Сервер Тогда
			ПолеТаблицы = Обработки.ирТипПолеБД.Создать();
		#КонецЕсли
		ирОбщий.ИсследоватьЛкс(ПолеТаблицы.Метаданные);
	КонецЕсли; 
...
Процедура КоманднаяПанельТекущаяТаблицыКолонкиБД(Кнопка)
	...
	Если Не ЗначениеЗаполнено(ИмяТаблицыБДТабличногоПоля) Тогда
		Возврат;
	КонецЕсли;
	НоваяФорма = ирКлиент.ПолучитьФормуЛкс("Обработка.ирКолонкиБД.Форма",,, ИмяТаблицыБДТабличногоПоля);
