...
Процедура ДополнительныеПоляПрочитатьПоля(Кнопка = Неопределено)
	...
	мСтрокаПорядкаНайденныхСсылок = ирКлиент.ПрочитатьДопПоляСсылающихсяОбъектовЛкс(ЭлементыФормы.НайденныеСсылки, КомпоновщикДопПолей,,, ЭтаФорма, Кнопка, "ПослеЧтенияДопПолей"); 
...
Процедура ПослеЧтенияДопПолей(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	...
	Для Каждого СтрокаРезультата Из ТаблицаДанных Цикл
		СтрокаСсылки = СсылкиНаОбъект.Найти(СтрокаРезультата.Данные, "Данные");
		ЗаполнитьЗначенияСвойств(СтрокаСсылки, СтрокаРезультата);
	КонецЦикла;
	Если СостояниеЗадания <> Неопределено Тогда // Только в асинх режиме
		СсылкиНаОбъект.Сортировать(мСтрокаПорядкаНайденныхСсылок);
	КонецЕсли;
КонецПроцедуры
