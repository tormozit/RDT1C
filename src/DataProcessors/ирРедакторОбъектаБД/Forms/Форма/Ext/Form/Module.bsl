...
Процедура ЗагрузитьОбъектПоКлючу(Знач КлючИлиОбъект) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ИмяОсновнойТаблицы) Тогда
		Возврат;
	КонецЕсли; 
	Если Не ТаблицаИзменилась Или Не фОбъект.ЭтоСсылочныйОбъект Тогда
...
Процедура НайтиВОбъектеНажатие(Элемент = Неопределено)
	...
	ТекстИскомойСсылки = ТекстИскомойСсылки();
	ТекстыСсылок = Новый Массив;
	Если ЗначениеЗаполнено(ТекстИскомойСсылки) Тогда
		ТекстыСсылок.Добавить(ТекстИскомойСсылки);
	КонецЕсли;
	Если Ложь
...
Функция ТекстИскомойСсылки()
	
	ТекстСсылки = "";
	Если ирОбщий.ЛиТипСсылкиБДЛкс(ТипЗнч(фОбъект.ИскомоеЗначение), Ложь) Тогда
		ТекстСсылки = ирОбщий.ТекстПоискаСсылкиВXMLЛкс(фОбъект.ИскомоеЗначение);
	КонецЕсли;
	Возврат ТекстСсылки;
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент = Неопределено, Знач ЗапретитьЗаполнение = Неопределено)
	...
	мФормаУпрДляПодключаемыхКоманд = ирКлиент.ДобавитьКнопкиКомандБСПЛкс(ЭлементыФормы.ОсновныеДействияФормы, ИмяОсновнойТаблицы, ЭтоСсылочныйОбъект);
	Если Ложь
