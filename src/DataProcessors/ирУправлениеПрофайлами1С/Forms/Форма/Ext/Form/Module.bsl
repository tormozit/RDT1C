...
Процедура ПриОткрытии()
	...
	Если ПараметрПометитьНастройкиКонфигуратора Тогда
		КПФайлыВсеНастройкиКонфигуратора();
	КонецЕсли;
...
Процедура УстановитьПараметрыВыбраннойБазы(Знач НастройкиБазыНаКлиенте = Неопределено)
	...
	НастройкиЭтойБазыНаКлиенте = ирКэш.НастройкиБазыНаКлиентеЛкс();
	Если НастройкиБазыНаКлиенте = Неопределено Тогда    
		НастройкиБазыНаКлиенте = НастройкиЭтойБазыНаКлиенте;
	КонецЕсли; 
	Если НастройкиБазыНаКлиенте = Неопределено Тогда
	...
	ВыбранаТекущаяБаза = Истина
		И НастройкиЭтойБазыНаКлиенте <> Неопределено
		И НастройкиЭтойБазыНаКлиенте.ID = НастройкиБазыНаКлиенте.ID;
...
Процедура УстановитьПараметрыПользователя1С(ВыбранаТекущаяБаза = Неопределено)
	...
	Если ВыбранаТекущаяБаза = Истина Или База = ирОбщий.КлючБазыВСпискеПользователяЛкс() Тогда
		...
		ЭлементыФормы.Пользователь1С.ТолькоПросмотр = Ложь;
	Иначе
		...
		ЭлементыФормы.Пользователь1С.ТолькоПросмотр = Истина;
	КонецЕсли;
...
Процедура ОбновитьТаблицуФайлов()
	...
	СтрокаФайла.Описание = "Настройки конфигуратора. Примеры. Настройки окон синтакс-помощника, табло, локальных переменных. Настройки текстового редактора. Настройки приложений сравнения/объединения текстов.";
	СтрокаФайла = Файлы.Добавить();
	СтрокаФайла.ЛогическийПуть = "$Roaming\$1Cv";
	...
	СтрокаФайла.Описание = "Настройки конфигуратора. Примеры. Параметры подключения к хранилищу конфигурации. Список последних вычисленных выражений.";
	СтрокаФайла = Файлы.Добавить();
	СтрокаФайла.ЛогическийПуть = "$Roaming\$1Cv\$База\$Пользователь1С";
...
Процедура КПФайлыВсеНастройкиКонфигуратора(Кнопка = Неопределено)
