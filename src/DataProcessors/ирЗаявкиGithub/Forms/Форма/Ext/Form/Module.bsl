...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Реквизит.Репозиторий, Реквизит.КаталогКэша";
...
Процедура ДействияФормыЗагрузить(Кнопка)
	...
	КонецЕсли;
	ЧислоЗаявок = ирОбщий.ОбъектИзТекстаЖСОНЛкс(Ответ.ПолучитьТелоКакСтроку()).open_issues_count;
...
Процедура ЗагрузитьПорцию()
	...
	Иначе
		...
		ПорцияЗаявок = ирОбщий.ОбъектИзТекстаЖСОНЛкс(ТелоОтвета, Истина);
		ирОбщий.ОбработатьИндикаторЛкс(мИндикатор, мИндикатор.Счетчик + ПорцияЗаявок.Количество());
	КонецЕсли;
	Если ПорцияЗаявок.Количество() Тогда
		Для Каждого ЗаявкаЖСОН Из ПорцияЗаявок Цикл
			Номер = ЗаявкаЖСОН["number"];
			...
			СтрокаТЗ.Заголовок = ЗаявкаЖСОН["title"];
			СтрокаТЗ.Статус = ЗаявкаЖСОН["state"];
			СтрокаТЗ.ДатаСоздания = XMLЗначение(Тип("Дата"), ЗаявкаЖСОН["created_at"]);
			СтрокаТЗ.ДатаОбновления = XMLЗначение(Тип("Дата"), ЗаявкаЖСОН["updated_at"]);
			Если ЗаявкаЖСОН["closed_at"] <> Неопределено Тогда
				СтрокаТЗ.ДатаЗакрытия = XMLЗначение(Тип("Дата"), ЗаявкаЖСОН["closed_at"]);
			КонецЕсли;
			Если ЗаявкаЖСОН["user"] <> Неопределено Тогда
				СтрокаТЗ.Автор = ЗаявкаЖСОН["user"]["login"];
			КонецЕсли;
			Метки = "";
			Если ЗаявкаЖСОН["labels"] <> Неопределено Тогда
				Для Каждого Метка Из ЗаявкаЖСОН["labels"] Цикл
					Метки = Метки + ?(ПустаяСтрока(Метки), "", ", ") + Метка["name"];
				КонецЦикла;
			КонецЕсли;
			СтрокаТЗ.Метки = Метки;
			СтрокаТЗ.ЧислоКомментариев = ЗаявкаЖСОН["comments"];
			СтрокаТЗ.Адрес = ЗаявкаЖСОН["html_url"];
			Если ЗаявкаЖСОН["body"] <> Неопределено Тогда
				СтрокаТЗ.Описание = ЗаявкаЖСОН["body"];
			КонецЕсли;
		КонецЦикла;
...
Процедура ОстановитьЗагрузку() Экспорт
	...
	ТаблицаЗаявок.Колонки.Удалить(ТаблицаЗаявок.Колонки.ДанныеПоиска1);
	ТаблицаЗаявок.Колонки.Удалить(ТаблицаЗаявок.Колонки.ДанныеПоиска2);
...
Процедура ЗаполнитьДанныеПоиска() Экспорт
	Для Сч=1 По 2 Цикл
		...
		Для Каждого ЗаявкиЭлемент Из Заявки.НайтиСтроки(Новый Структура(ИмяКолонки, "")) Цикл
			ЗаявкиЭлемент[ИмяКолонки] = ЗаявкиЭлемент.Заголовок + " " + ЗаявкиЭлемент.Описание;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПрименитьФильтрПоПодстрокеБезСохранения()
	СловаПоиска = ирОбщий.РазделитьСтрокуПоискаНаСловаПоискаЛкс(ЗаявкиСтрокаПоиска);
	...
	КонецЕсли;
	ТабличноеПоле = ЭлементыФормы.Заявки;
