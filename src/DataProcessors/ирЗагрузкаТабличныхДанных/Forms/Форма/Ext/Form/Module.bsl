...
Процедура ЗаполнитьСоответствиеТЗИИ(Кнопка)
	...
	#КонецЕсли
	ирОбщий.ВыполнитьЗаданиеФормыЛкс("ирОбщий.ОтправитьЗапросИИЛкс", ПараметрыЗадания, ЭтаФорма, "ЗаполнитьТЗИИ",, Кнопка, "ЗаполнитьТЗИИКонец", мПлатформа.ЛиЗапросыИИНаСервере(), БлокируемыеЭлементы);
КонецПроцедуры
...
Процедура ЗаполнитьТЗИИКонец(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	Если Ложь
		Или СостояниеЗадания = Неопределено
	...
	РезультатИИ = ирОбщий.ОбъектИзСтрокиJSONЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(РезультатЗадания.Ответ, "```json", "```")); // Соответствие
	КолонкиТабличногоДокумента = ПолучитьКолонкиТабличногоДокумента();
...
Процедура ЗаполнитьСоответствиеБДИИ(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.ОтправитьЗапросИИЛкс(); 
	#КонецЕсли
	ирОбщий.ВыполнитьЗаданиеФормыЛкс("ирОбщий.ОтправитьЗапросИИЛкс", ПараметрыЗадания, ЭтаФорма, "ЗаполнитьБДИИ",, Кнопка, "ЗаполнитьБДИИКонец", мПлатформа.ЛиЗапросыИИНаСервере(), БлокируемыеЭлементы);
КонецПроцедуры
...
//   СостояниеЗадания -  - 
//   РезультатЗадания - см. ирОбщий.ОтправитьЗапросИИЛкс - 
Процедура ЗаполнитьБДИИКонец(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	Если Ложь
		Или СостояниеЗадания = Неопределено
		Или СостояниеЗадания = СостояниеФоновогоЗадания.ЗавершеноАварийно 
	Тогда
