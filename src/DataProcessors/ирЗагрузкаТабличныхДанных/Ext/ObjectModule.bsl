...
Функция КонвертироватьДанные(Параметры = Неопределено) Экспорт
	...
	Иначе
		...
		ЭтотОбъект.ТаблицаЗначений = ирОбщий.СузитьТипыКолонокТаблицыБезПотериДанныхЛкс(ТаблицаЗначений,,,,, Истина, СопоставлениеКолонокБД.ВыгрузитьКолонку("ИмяКолонкиИсточника"));
		СообщитьИтогиКонвертации(КоличествоОшибокКонвертации);
	КонецЕсли;
...
//
Процедура ОбновитьКолонкиТаблицыЗначений(ОчиститьСуществующие = Ложь, ИзБД = Истина, СообщитьОДобавленииКолонокБД = Ложь) Экспорт 
	...
	Если ИзБД Тогда
		Если ЗначениеЗаполнено(ПолноеИмяТаблицы) Тогда
			...
			Для каждого СтрокаСопоставленияБД Из СопоставлениеКолонокБД Цикл
				СтараяКолонкаТЗ = ТаблицаЗначений.Колонки.Найти(СтрокаСопоставленияБД.ИмяКолонкиПриемника);
				Если Ложь
				...
				КонецЕсли; 
				НоваяКолонкаТЗ = ТаблицаЗначений.Колонки.Добавить(СтрокаСопоставленияБД.ИмяКолонкиПриемника, СтрокаСопоставленияБД.ОписаниеТиповПриемника, СтрокаСопоставленияБД.СинонимКолонкиПриемника);
				Если СтараяКолонкаТЗ <> Неопределено Тогда
	...
	Иначе
		...
		Для НомерКолонки = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
			...
			Иначе
				...
				КонецЦикла;
				Для НомерСтроки = СтруктураПозицийТД.НомерСтрокиДанных По Мин(СтруктураПозицийТД.НомерПоследнейСтрокиДанных, СтруктураПозицийТД.НомерСтрокиДанных + 10000 / ТабличныйДокумент.ШиринаТаблицы) Цикл
					#Если Клиент Тогда
						...
						Если ЗначениеЗаполнено(ТекстЯчейки) Тогда
							Для Каждого ПримитивныйТип Из ПримитивныеТипыПоПриоритету Цикл
								...
								ИначеЕсли ПримитивныйТип = "Число" Тогда
									ОшибкиКонвертации = "";
									РезультатКонвертации = ирОбщий.СтрокаВЧислоЛкс(ТекстЯчейки,, ОшибкиКонвертации);
									Если ЗначениеЗаполнено(ОшибкиКонвертации) Тогда
										РезультатКонвертации = Неопределено;
									КонецЕсли; 
								Иначе
			...
			Если МаксДлинаСтроки > 0 Тогда
				// Теперь квалификаторы вычисляются после конвертации
				...
				ТаблицаЗначений.Колонки.Добавить(ИмяКолонки, Новый ОписаниеТипов(ИменаТипов), СинонимКолонки, ШиринаКолонки);
			КонецЕсли;
		КонецЦикла; 
...
Функция ПолучитьКолонкиТабличногоДокумента(СтруктураПозицийТД = Неопределено, ТолькоВыделенныеСтрокиТД = Ложь) Экспорт 
	...
	Для Счетчик = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		СтрокаНомераКолонки = Формат(Счетчик, "ЧЦ=3; ЧВН=");
		СинонимКолонки = "";
		Если ЗначениеЗаполнено(СтруктураПозицийТД.НомерСтрокиЗаголовков) Тогда
...
Функция ПолучитьВозможныеЗначения(Знач Колонка, Знач Представление, выхПримечание, Знач ТекущиеДанные, КэшПоиска = Неопределено)
	...
	Если Не ПустаяСтрока(Колонка.СвязьПоТипу)  Тогда
		Если ТипЗНЧ(Колонка.СвязьПоТипу) = Тип("Строка") Тогда
			Если Не ТекущиеДанные.Свойство(Колонка.СвязьПоТипу, СвязьПоТипу) Тогда 
				выхПримечание = выхПримечание + Символы.ПС  + ирОбщий.СтрШаблонИменЛкс("Колонка %1 связи по типу не найдена",, Колонка.СвязьПоТипу);
			КонецЕсли;
		Иначе
			...
			Иначе
				...
				ИначеЕсли ОбъектМД <> Неопределено Тогда
					...
					Иначе
						...
						КлючКэша = ОбъектМД.ПолноеИмя() + "." + Представление;
						...
						Если НайденныеЗначения = Неопределено Тогда
							...
							Тогда
								...
								Если ТипЗНЧ(Колонка.СвязьПоВладельцу) = Тип("Строка") Тогда
									Если Не ТекущиеДанные.Свойство(Колонка.СвязьПоВладельцу, СвязьПоВладельцу) Тогда 
										выхПримечание = выхПримечание + Символы.ПС  + ирОбщий.СтрШаблонИменЛкс("Колонка %1 связи по владельцу не найдена",, Колонка.СвязьПоВладельцу);
									КонецЕсли;
								Иначе
...
Процедура ВосстановитьИзменяемыеСвойстваСтрокиСопоставленияБД(Знач ЗагружаемыйРеквизит, Знач КопияСопоставления)
	...
	Если СтараяСтрока <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ЗагружаемыйРеквизит, СтараяСтрока, "ИзИсточника, ПолеПоиска, Пометка, Значение, ИмяКолонкиИсточника");
...
Процедура СопоставитьКолонкуБД(Знач СтрокаСопоставления, Знач КолонкаИсточника = Неопределено, РазрешитьМенятьПометку = Истина) Экспорт 
	...
	Тогда
		...
		СтрокаСопоставления.ИзИсточника = КолонкаИсточника <> Неопределено;
