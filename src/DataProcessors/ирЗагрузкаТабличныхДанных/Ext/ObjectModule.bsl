...
Процедура ВыбратьЗначениеВЯчейке(СоздатьОбъект = Ложь) Экспорт
	...
	Если мВыбратьЗначение(Значение, ТекущаяКолонка.ОписаниеТипов, СвязьПоТипу, ЭлементСвязиПоТипу, СвязьПоВладельцу, ТекущаяКолонка.ИмяКолонкиПриемника = "Родитель", СоздатьОбъект, ТекущаяКолонка.ИскатьПо) Тогда
		...
		Если ТекущаяОбласть.Параметр = Истина Тогда
			ТекущаяОбласть.Параметр = Ложь;
		КонецЕсли;
		...
		Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.ПрЛкс(_РежимОтладки, 1, 1)
			...
			Для каждого СтрокаСопоставления Из ПомеченныеКолонки Цикл
				...
				Если Истина
					...
					И (Ложь
						Или ОбластьЯчейки.Параметр = Истина 
						Или ЗначениеЗаполнено(Примечание)) 
				Тогда
					// Используем флаг "Параметр" для обозначения наличия установленного нами примечания
					...
					ОбластьЯчейки.Параметр = ЗначениеЗаполнено(Примечание);
				КонецЕсли; 
			КонецЦикла;
		Иначе
				...
				ИначеЕсли ОбъектМД <> Неопределено Тогда
					...
					Иначе
						...
						Если НайденныеЗначения = Неопределено Тогда
							НайденныеЗначения = НайтиСсылкиСправочникаПоПредставлению(выхПримечание, ОбъектМД, ОписаниеКолонки, Представление, РазрешитьСозданиеОбъектов, СтруктураОтбора);
...
Функция НайтиСсылкиСправочникаПоПредставлению(выхПримечание, Знач ОбъектМД, Знач ОписаниеКолонки, Знач ПредставлениеСсылки, Знач РазрешитьСозданиеОбъектов, Знач СтруктураОтбора)
