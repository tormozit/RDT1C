...
Функция ВычислитьЗначениеЯчейки(Знач АлгоритмОбъект, Знач ТекущиеДанные, Знач ТекстЯчейки, Знач ТекстыЯчеек, Знач Результат, Знач ОписаниеОшибки = "", СтандартнаяОбработка = Истина, Знач КэшПоиска = Неопределено)
	
	ТекстЯчейки = СокрЛП(ТекстЯчейки);
	Попытка
		мПлатформа.ВыполнитьМетодАлгоритма(АлгоритмОбъект, 0, Результат, ТекстЯчейки, ТекстыЯчеек, ТекущиеДанные, ОписаниеОшибки, СтандартнаяОбработка, КэшПоиска);
	Исключение
...
Функция КонвертироватьДанные(Параметры = Неопределено) Экспорт
	...
	Иначе
		Если КоличествоЭлементов > 10000 И ирКэш.ЛиОтладкаРазрешенаЛкс() Тогда
			ирОбщий.СообщитьЛкс("Возможно включено разрешение отладки и тогда оно замедляет конвертацию значений.");
		КонецЕсли;
		Успех = Истина;
		...
		Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
			...
			Для каждого СтрокаСопоставления Из ПомеченныеКолонки Цикл
				...
				Если ЗначениеЗаполнено(СтрокаСопоставления.Алгоритм) Тогда 
					Попытка
						АлгоритмОбъект = ирОбщий.ДесериализоватьАлгоритмОбъектЛкс(СтрокаСопоставления.Алгоритм, мАлгоритмКонвертацииЗначенияПараметры);
						#Если Сервер И Не Сервер Тогда
					...
					КонецПопытки;
					Вычисление = ВычислитьЗначениеЯчейки(АлгоритмОбъект, РезультатыЯчеек, ТекстЯчейки, ТекстыЯчеек, РезультатЯчейки, Примечание, СтандартнаяОбработка, КэшПоиска);
				...
				Тогда
					Если СтрокаСопоставления.ОбрезатьКрайниеПробелы Тогда
						ТекстЯчейки = СокрЛП(ТекстЯчейки);
					КонецЕсли; 
					Если СтрокаСопоставления.ЗаменитьОпасныеНепечатныеСимволы Тогда
						ТекстЯчейки = ирОбщий.ЗаменитьОпасныеНепечатныеСимволыЛкс(ТекстЯчейки, Ложь, РегВыражение);
					КонецЕсли; 
					Примечание = "";  
					Расшифровка = Неопределено;
					Если Не СтрокаСопоставления.ИгнорироватьРасшифровку И ОбластьЯчейки <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Примечание) Тогда
					...
					Если ВыводитьОшибкиКонвертацииСразу Тогда
						ПредставлениеКраткое = ирОбщий.ПредставлениеСОбрезкойДлиныЛкс(ОригинальныйТекстЯчейки, 50);
					...
					Иначе
						...
						Тогда
							...
							Если Истина
								...
								И ирОбщий.ЛиСодержитОпасныеНепечатныеСимволыЛкс(Представление)
							Тогда
								выхПримечание = выхПримечание + Символы.ПС + НСтр("ru='В искомом значении обнаружены опасные непечатные символы. Рекомендую флажок ""Заменить опасные непечатные символы""'");
							КонецЕсли;
						КонецЕсли; 
					КонецЕсли; 
				Иначе
...
// Параметры:
//   ОписаниеКолонки -  - 
//   ОбъектМД - ОбъектМетаданныхСправочник, ОбъектМетаданныхЗадача, ОбъектМетаданныхПланСчетов - 
//   ЗначенииеОтбора -  - 
// Возвращаемое значение:
//   Строка -
Функция ИмяПоляПоискаСсылки(Знач ОписаниеКолонки, Знач ОбъектМД, ЗначенииеОтбора = Неопределено)
	...
	Если ПолеПоиска = Неопределено Тогда
		...
		Если Ложь
			Или СтрокаОсновногоПредставления = "AsCode" 
			Или СтрокаОсновногоПредставления = "ВВидеКода" 
		Тогда
			ИскатьПо = "Код";
		ИначеЕсли Ложь
			Или СтрокаОсновногоПредставления = "AsDescription" 
			Или СтрокаОсновногоПредставления = "ВВидеНаименования" 
		Тогда
			ИскатьПо = "Наименование";
		ИначеЕсли Ложь
			Или СтрокаОсновногоПредставления = "AsNumber" 
			Или СтрокаОсновногоПредставления = "ВВидеНомера" 
		Тогда
			ИскатьПо = "Номер";
		Иначе
