...
Функция КонвертироватьДанные(Параметры = Неопределено) Экспорт
	...
	Иначе
		Если КоличествоЭлементов > 10000 И ирКэш.ЛиОтладкаРазрешенаЛкс() Тогда
			ирОбщий.СообщитьЛкс("Возможно включено разрешение отладки. Оно сильно замедляет конвертацию значений.");
		КонецЕсли;
		Успех = Истина;
...
Процедура ВычислитьЗависимостиКонвертации()
	...
	Для Каждого СтрокаСопоставления Из СопоставлениеКолонокТЗ Цикл
		...
		СтрокаСопоставления.СодержитСсылки = Ложь
			Или СтрокаСопоставления.ОписаниеТипов.Типы().Количество() = 0
			Или ирОбщий.ЛиОписаниеТиповПростогоСсылочногоТипаЛкс(СтрокаСопоставления.ОписаниеТипов,, Ложь);
	КонецЦикла;
