...
Функция ДобавитьСтрокуОшибкиЛкс(ИмяОперации, Знач ИнформацияОбОшибке = Неопределено, ГлубинаОшибки = 0, Контекст = "КлиентскоеПриложение") Экспорт
	...
	Для Каждого ПолеТаблицыБД Из ПоляТаблицыБД Цикл
		...
		//	КонецЕсли;
			...
			Тогда
				...
				СтруктураОтбора.Вставить("Родитель", Новый(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(ОбъектМД)));
			КонецЕсли;
			Если Истина
				И ИмяПоля = "Родитель" 
