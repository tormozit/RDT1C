...
Функция ДобавитьСтрокуОшибкиЛкс(Знач ИмяОперации, Знач ИнформацияОбОшибке = Неопределено, Знач ГлубинаОшибки = 0, Знач Контекст = "КлиентскоеПриложение", Знач ОбъектБД = Неопределено) Экспорт
	...
	Для Каждого СтрокаТипаМетаданных Из ТипыМетаданных Цикл
		...
		Для Каждого МетаОбъект Из КоллекцияМетаОбъектов Цикл
					...
					Если ПравоДоступа("Изменение", МетаОбъект) Тогда
						...
						Исключение
							...
							Если Не ЕстьИгнориуемыеСигнатуры(ОписаниеОшибки()) Тогда 
								ДобавитьСтрокуОшибкиЛкс(ИмяОперации, ИнформацияОбОшибке(), 1,, СтруктураОбъекта.Методы);
							КонецЕсли; 
							НачатьТранзакцию();
						КонецПопытки;
					КонецЕсли; 
			...
			Исключение
				...
				ДобавитьСтрокуОшибкиЛкс(ИмяОперации, ИнформацияОбОшибке(),,, СтруктураОбъекта.Методы);
			КонецПопытки;
