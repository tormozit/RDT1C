...
Процедура ТестироватьПолеВвода(ПолеВвода, Знач ТолькоПросмотр) Экспорт
	...
	Тогда
		//ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(ПолеВвода, Неопределено);
		...
		//Если НепустоеЗначение <> Неопределено Тогда
			...
			Иначе
				ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(ПолеВвода, НепустоеЗначение);
			КонецЕсли; 
		//КонецЕсли; 
	КонецЕсли; 
	...
	Тогда
		...
		//Если НепустоеЗначение <> Неопределено Тогда
			...
			Если ДействиеНачалоВыбора <> Неопределено Тогда
				// Здесь часто ограничивается множество выбора и потому высока вероятность установить некорректное с точки зрения смысла значение
			Иначе
				ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(ПолеФормы, НепустоеЗначение);
			КонецЕсли; 
		//КонецЕсли; 
	КонецЕсли; 
...
Функция ГлобальныйТестФорм() Экспорт
	...
	Счетчик = 0;
	ПараметрыСозданияФормы = ирОбщий.ПараметрыБыстрогоСозданияФормыЛкс();
					...
					Если Форма <> Неопределено Тогда
						...
						Если ЭтоФормаСсылочногоОбъекта Тогда
							...
							Иначе
								...
								КонецЕсли; 
								Форма = ирОбщий.ПолучитьФормуЛкс(МетаФорма.ПолноеИмя(), ПараметрыСозданияФормы, , КлючВременнойФормы);
							КонецЕсли; 
						КонецЕсли;
					КонецЕсли; 
					Если Форма = Неопределено Тогда
