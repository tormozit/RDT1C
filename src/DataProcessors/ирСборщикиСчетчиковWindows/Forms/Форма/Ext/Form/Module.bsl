...
Процедура ПосмотретьГрафики(Знач ВыбраннаяСтрока, РежимПросмотра = 0)
	...
	Если РежимПросмотра = 1 Тогда
		...
		ТаблицаСтроковыхЗначений = ирОбщий.ТаблицаИзТекстаСРазделителемЛкс(Текст.ПолучитьТекст(), Разделитель, Истина, Истина);
		...
		ПолеНабораПоказателяЭталон = СхемаКомпоновки.НаборыДанных.Таблица.Поля.Найти("Показатель");
		Для Каждого Колонка Из ТаблицаТипизированая.Колонки Цикл
			Если ТипЗнч(ТаблицаТипизированая[0][Колонка.Имя]) <> Тип("Число") Тогда 
				Продолжить;
			КонецЕсли; 
			ПолеНабораПоказателя = СхемаКомпоновки.НаборыДанных.Таблица.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
		...
		КонецЦикла;
		СхемаКомпоновки.НаборыДанных.Таблица.Поля.Удалить(ПолеНабораПоказателяЭталон);
