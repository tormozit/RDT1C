...
Процедура ПриОткрытии()
	...
	КонецЕсли;
	СписокВыбораКолонки = ЭлементыФормы.КолонкаНомераВерсии.СписокВыбора;
	Для Каждого Колонка Из СтрокиДляОбработки.Колонки Цикл
		Если Колонка.ТипЗначения.СодержитТип(Тип("Число")) И Колонка.ТипЗначения.КвалификаторыЧисла.РазрядностьДробнойЧасти = 0 Тогда
			СписокВыбораКолонки.Добавить(Колонка.Имя, Колонка.Заголовок);
		КонецЕсли;
	КонецЦикла;
	СписокВыбораКолонки.СортироватьПоЗначению();
	Если СписокВыбораКолонки.НайтиПоЗначению("НомерВерсии") <> Неопределено Тогда
		ЭтаФорма.КолонкаНомераВерсии = "НомерВерсии";
	ИначеЕсли СписокВыбораКолонки.Количество() = 0 Тогда
		ЭтаФорма.КолонкаНомераВерсии = Неопределено;
	КонецЕсли;
...
Процедура вВыполнитьОбработку(Кнопка = Неопределено) Экспорт 
	...
	Тогда
		Если Не ЗначениеЗаполнено(КолонкаНомераВерсии) Тогда
			ирОбщий.СообщитьЛкс("Нужно указать колонку с номером версии");
			Возврат;
		КонецЕсли;
	КонецЕсли; 
	...
	#КонецЕсли
	НастройкаВыполнения.Вставить("КолонкаНомераВерсии", КолонкаНомераВерсии);
...
ирКлиент.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПодборИОбработкаОбъектов.Форма.ЗагрузитьИзВерсии");
мИспользоватьНастройки = Ложь;

//Реквизиты настройки и значения по умолчанию.
мНастройка = Новый Структура("КолонкаНомераВерсии");
