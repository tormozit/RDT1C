...
Процедура ЗагрузитьДоступныеОбработки()
...
Процедура НастроитьЭлементыФормы()
	...
	Если Доступность Тогда
		ЭлементыФормы.ОбластьПоиска.Картинка = ирОбщий.КартинкаКорневогоТипаМДЛкс(мИскомыйОбъект.ТипТаблицы);
	Иначе
	...
	ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(ЭтаФорма.Заголовок,, ирОбщий.ПредставлениеЗначенияСОграничениемДлиныЛкс(ОбластьПоиска, 50), ": ");
...
Процедура НастроитьСлужебныеКолонкиТабличногоПоляВыборки()
	...
	ирОбщий.НастроитьКолонкуКартинкиЛкс(КолонкаТП);
	СтрокиДляОбработки.Колонки[КолонкаТП.Данные].Заголовок = КолонкаТП.ТекстШапки;
...
Процедура ЗаполнитьСтроки() Экспорт
	...
	Если Не МноготабличнаяВыборка Тогда
		ДобавленВСписок = ирОбщий.ДобавитьОтборКомпоновкиВИсториюТаблицыБДЛкс(ЭтаФорма, ОбластьПоиска, Компоновщик.Настройки, СтарыйКомпоновщик.Настройки);
...
Процедура ЗагрузитьОтобранныеСтроки(ПерваяПорция = Истина)
	...
	Если ПерваяПорция Тогда
		ирОбщий.ЗагрузитьВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, мРезультатЗапроса, мВыборкаРезультата, ЭлементыФормы.КПСтрокиДляОбработки,,, СсылкаНаБуфернуюТаблицу);
	Иначе
		ирОбщий.ЗагрузитьВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, мРезультатЗапроса, мВыборкаРезультата, ЭлементыФормы.КПСтрокиДляОбработки,, 0, СсылкаНаБуфернуюТаблицу);
	КонецЕсли; 
...
Функция ПолучитьЗапросВыборки(выхНастройкаКомпоновки = Неопределено)
	...
	//	КонецЦикла;
	//КонецЦикла;
	Запрос = ирОбщий.ЗапросИзКомпоновкиЛкс(мСхемаКомпоновки, выхНастройкаКомпоновки, Истина,,, мСхемаКолонок, Не БезАвтоупорядочивания);
	#Если Сервер И Не Сервер Тогда
		Запрос = Новый Запрос;
	#КонецЕсли
...
Функция ПолучитьИсполняемуюКомпоновку(выхНастройкаКомпоновки = Неопределено, выхСхемаКомпоновки = Неопределено, ТолькоВычислитьКоличество = Ложь)
...
//
Функция ПолучитьЗапрос(ПредставленияПолей = Неопределено)
	...
	Если ПодключатьСвязанныеТаблицы И ирОбщий.ЛиКорневойТипСсылкиЛкс(мИскомыйОбъект.ТипТаблицы) Тогда
		...
		Если ирОбщий.ЛиКорневойТипДокументаЛкс(мИскомыйОбъект.ТипТаблицы) Тогда
			Для Каждого МетаРегистр Из мИскомыйОбъект.Метаобъект.Движения Цикл
				ДобавитьДоступныйОтборПоКоличествуСтрокПодчиненнойТаблицы(МетаРегистр.Имя, "Регистратор", МетаРегистр.ПолноеИмя(), ТекстОбщееКоличествоДвижений, ТекстСоединенияПодчиненныхТаблиц, "Регистр");
			КонецЦикла;
		КонецЕсли; 
		Если ирОбщий.ЛиКорневойТипСсылочногоОбъектаБДЛкс(мИскомыйОбъект.ТипТаблицы) Тогда
			...
			Для Каждого ОписаниеТЧ Из СтруктураТЧ Цикл
				ДобавитьДоступныйОтборПоКоличествуСтрокПодчиненнойТаблицы(ОписаниеТЧ.Ключ, "Ссылка", ПолноеИмяМД + "." + ОписаниеТЧ.Ключ, ТекстОбщееКоличествоСтрокТЧ, ТекстСоединенияПодчиненныхТаблиц, "ТЧ");
			КонецЦикла;
		КонецЕсли; 
		...
		Для Каждого МетаРегистр Из Метаданные.РегистрыСведений Цикл
			...
			Тогда
				...
				Если МетаРегистр.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
					...
					СПараметрами = Истина;
				КонецЕсли; 
				ДобавитьВедомыйРегистр(МетаРегистр, МетаРегистр.Измерения[0].Имя, ПолноеИмяРегистра, ТекстЗапроса, ТекстСоединенияПодчиненныхТаблиц, ПредставлениеРегистра, ПредставленияПолей, СПараметрами);
			КонецЕсли; 
		КонецЦикла;
		Для Каждого МетаРегистр Из Метаданные.РегистрыНакопления Цикл
			...
			Тогда
				...
				ДобавитьВедомыйРегистр(МетаРегистр, МетаРегистр.Измерения[0].Имя, ПолноеИмяРегистра, ТекстЗапроса, ТекстСоединенияПодчиненныхТаблиц, ПредставлениеРегистра, ПредставленияПолей, Истина);
			КонецЕсли;                                                                                                                                         
		КонецЦикла;
...
Процедура ДобавитьДоступныйОтборПоКоличествуСтрокПодчиненнойТаблицы(Знач КраткоеИмяПодчиненнойТаблицы, Знач ПолеСсылкиВПодчиненнойТаблице, Знач ПолноеИмяПодчиненнойТаблицы,
	ТекстОбщееКоличествоДвижений, ТекстСоединенияПодчиненныхТаблиц, Знач ПрефиксИмениПоля)
	...
	|	{ ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ _Регистр_." + ПолеСсылкиВПодчиненнойТаблице + ", КОЛИЧЕСТВО(*) КАК " + мИмяПоляКоличествоДвиженийСкрытое + "
	...
	КонецЕсли; 
	ВыражениеКоличества = "ЕСТЬNULL(" + ПсевдонимТаблицы + "." + мИмяПоляКоличествоДвиженийСкрытое + ", 0)";
...
Процедура ДобавитьВедомыйРегистр(Знач ОбъектМД, Знач ПолеСсылкиВПодчиненнойТаблице, Знач ПолноеИмяПодчиненнойТаблицы, ТекстВыбораПолей, ТекстСоединенияПодчиненныхТаблиц, Знач ПредставлениеРегистра,
	Знач ПредставленияПолей, Знач СПараметрами = Ложь)
	...
	Если СПараметрами Тогда
		ПараметрыТаблицы = "(, {" + ПолеСсылкиВПодчиненнойТаблице + ".* КАК Ссылка})";
	КонецЕсли;
	ПсевдонимТаблицы = ОбъектМД.Имя + "Т";      
	ТекстСоединенияПодчиненныхТаблиц = ТекстСоединенияПодчиненныхТаблиц + Символы.ПС + " 
	|	{ ЛЕВОЕ СОЕДИНЕНИЕ " + ПолноеИмяПодчиненнойТаблицы + ПараметрыТаблицы + " КАК " + ПсевдонимТаблицы + "
	|	ПО " + ПсевдонимТаблицы + "." + ПолеСсылкиВПодчиненнойТаблице + " = Т.Ссылка}";   
	Для Каждого ПолеТаблицы Из ирКэш.ПоляТаблицыБДЛкс(ПолноеИмяПодчиненнойТаблицы) Цикл
		Если ПолеСсылкиВПодчиненнойТаблице = ПолеТаблицы.Имя Тогда
			Продолжить;
		КонецЕсли;
		ИмяПоля = ОбъектМД.Имя + "_" + ПолеТаблицы.Имя;
...
Функция НастроитьКомпоновщик(СохранятьТекущиеНастройки = Истина, ЗагружатьТекущиеНастройки = Ложь, НастройкиНабораТаблицБД = Неопределено, ИмяПоляКоличестваСтрок = "")
	...
	Если МноготабличнаяВыборка Тогда
		...
		Тогда
			ВариантИсточников = 2;
		КонецЕсли; 
		ТекстЗапроса = ирОбщий.ТекстЗапросаПоВыбраннымТаблицамЛкс(ИменаОбъектовМД, ВариантИсточников, ПервыеNКаждойТаблицы, ПодключатьСвязанныеТаблицы, мИмяКолонкиПолногоИмениТаблицы, ИмяПоляКоличестваСтрок);
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапроса;
	...
	Иначе
		...
		Запрос = ПолучитьЗапрос(ПредставленияПолей);
		#Если Сервер И Не Сервер Тогда
			Запрос = Новый Запрос;
		#КонецЕсли
	...
	Если ПодключатьСвязанныеТаблицы И ирОбщий.ЛиКорневойТипСсылкиЛкс(мИскомыйОбъект.ТипТаблицы) Тогда
		ПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
		ПолеНабора.ПутьКДанным = мИмяПоляКоличествоДвиженийСкрытое;
		ПолеНабора.Поле = ПолеНабора.ПутьКДанным;
		ирОбщий.УстановитьОграниченияИспользованияПоляНабораДанныхСхемыКомпоновкиЛкс(ПолеНабора, Истина);
		Для Каждого КлючИЗначение Из мИменаКоличестваПодчиненных Цикл
			ПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
			ПолеНабора.ПутьКДанным = "КоличествоПодчиненных." + КлючИЗначение.Ключ;
			ПолеНабора.Заголовок = "Количество подчиненных." + КлючИЗначение.Значение;
	...
	Если МноготабличнаяВыборка Тогда
		...
		Если ЗначениеЗаполнено(ИмяПоляКоличестваСтрок) Тогда
			...
			ПолеНабора.Заголовок = "Количество строк";
...
Процедура ДобавитьПредставленияПолей(Знач КоллекцияДоступныхПолей, Знач ПредставленияПолей, Знач ПредставленияПапок)
	...
	Для Каждого ДоступноеПоле Из КоллекцияДоступныхПолей Цикл
		Если Ложь
			...
			Или "" + ДоступноеПоле.Поле = мИмяПоляКоличествоДвиженийСкрытое
...
Процедура ПриОткрытии()
	...
	Если ПараметрМассивСсылок <> Неопределено Тогда
		ЗагрузитьСтрокиДляОбработки(ирОбщий.РазличныеЗначенияМассиваЛкс(ПараметрМассивСсылок),, ПараметрВыбранныеПоля, Истина);
...
Процедура КПСтрокиДляОбработкиУстановитьФлажки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭтаФорма, ЭлементыФормы.СтрокиДляОбработки, мИмяКолонкиПометки, Истина);

КонецПроцедуры

Процедура КПСтрокиДляОбработкиСнятьФлажки(Кнопка)

	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭтаФорма, ЭлементыФормы.СтрокиДляОбработки, мИмяКолонкиПометки, Ложь);
...
Процедура ДоступныеОбработкиВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		Если Не ОбработкаРазрешенаДляТаблицы(ВыбраннаяСтрока.Родитель) Тогда
			...
			Возврат;
		КонецЕсли;
		РедакторНастройки = ПолучитьФорму(ВыбраннаяСтрока.Родитель.ИмяФормы, ЭтаФорма);
	КонецЕсли;
	Если Истина
...
Функция ОбработкаРазрешенаДляТаблицы(Знач СтрокаПроверки)
	...
	Тогда
		Возврат Истина;
	ИначеЕсли Истина
		И ирОбщий.ЛиКорневойТипПеречисленияЛкс(мИскомыйОбъект.ТипТаблицы)
...
Функция УстановитьОбластьПоиска(пОбластьПоиска = Null, НастройкиНабораТаблицБД = Неопределено) Экспорт
	...
	ЭлементыФормы.ПодключатьСвязанныеТаблицы.Доступность = мИскомыйОбъект <> Неопределено И ирОбщий.ЛиКорневойТипСсылкиЛкс(мИскомыйОбъект.ТипТаблицы);
...
Процедура ЗагрузитьСтрокиДляОбработки(Знач Массив, Знач ОчиститьТекущийСписок = Истина, Знач ВыбранныеПоля = Неопределено, Знач АктивироватьСтраницуОбработки = Ложь) Экспорт
	...
	Если ТипЗнч(ОбластьПоиска) = Тип("Строка") И ЗначениеЗаполнено(ОбластьПоиска) Тогда
		...
		Если ПоляСМетаданными <> Неопределено Тогда
			Для Каждого ОформлениеСтроки Из ОформленияСтрок Цикл
				...
				ирОбщий.ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, СтрокаПоля);
			КонецЦикла;
		КонецЕсли; 
	КонецЕсли; 
...
Процедура КПСтрокиДляОбработкиОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ОтборИзменен = ирОбщий.ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(ЭлементыФормы.СтрокиДляОбработки, Ложь, Компоновщик.Настройки.Отбор, Истина);
...
Процедура КПСтрокиДляОбработкиОтборПоЗначению(Кнопка)
	...
	ОтборИзменен = ирОбщий.ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(ЭлементыФормы.СтрокиДляОбработки, Истина, Компоновщик.Настройки.Отбор, Истина);
	Если ОтборИзменен Тогда
		ПослеИзмененияОтбораНаСтраницеСтрокиДляОбработки();
	КонецЕсли; 
...
Процедура ОбновитьКоллекциюРезультата(ЗадаватьВопросПередПеречитыванием = Ложь) Экспорт 
	...
	//Если КолонкиДляОбновления <> "" Тогда
		...
		Иначе
			...
			КонецЕсли; 
			ИмяВременнойТаблицы = ирОбщий.ИмяВременнойТаблицыДляТаблицыБДЛкс(ирКэш.ИмяТаблицыИзМетаданныхЛкс(мИскомыйОбъект.МетаОбъект.ПолноеИмя()),  "ТаблицаКлючей");
			//ТекстГДЕ = "";
...
Процедура ПоляБитыхСсылокВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка = ЭлементыФормы.ПоляБитыхСсылок.Колонки.ОписаниеТипов Тогда
		ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка,, Ложь,,, ЭлементыФормы.ПоляБитыхСсылок.ТекущаяСтрока.Заголовок);
	КонецЕсли; 
...
Процедура УстановитьЗначениеОтбора(Знач НовоеЗначение, УстановитьЗначениеПоляВвода = Истина)
	...
	Если УстановитьЗначениеПоляВвода Тогда
		ТабличноеПолеОтбора.Колонки.ПравоеЗначениеДляКраткогоОтображенияЭлемента.ЭлементУправления.Значение = НовоеЗначение;
	КонецЕсли; 
	Попытка
...
Процедура ОтборКомпоновкиЗначениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	
	Если ЛиЗапрешеноЗапоминатьЗначенияОтбора() Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ПолеВводаСИсториейВыбора_НачалоВыбораИзСпискаЛкс(ЭлементыФормы.КомпоновщикОтбор.Колонки.ПравоеЗначениеДляКраткогоОтображенияЭлемента.ЭлементУправления,
		ирОбщий.КлючИсторииВыбораЗначенияОтбораЛкс(ОбластьПоиска, ЭлементыФормы.КомпоновщикОтбор.ТекущаяСтрока));
	
КонецПроцедуры

Процедура ОтборКомпоновкиЗначениеПриИзменении(Элемент)
...
Процедура ДобавитьЗначениеОтбораВИсторию()
	...
	ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(ЭлементыФормы.КомпоновщикОтбор.ТекущаяСтрока, ОбластьПоиска);
...
Процедура ВключитьКнопкуСпискаВыбораПравогоЗначенияОтбора()
	...
	ЭлементыФормы.КомпоновщикОтбор.Колонки.ПравоеЗначениеДляКраткогоОтображенияЭлемента.ЭлементУправления.КнопкаСпискаВыбора = Истина;
...
Процедура ОбновитьПодменюПоследнихОтборов()
	...
	Если ТипЗнч(ОбластьПоиска) = Тип("Строка") Тогда
		ирОбщий.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КПОтбор.Кнопки.ПоследниеОтборы, ОбластьПоиска, Новый Действие("ПоследниеОтборыНажатие"), "Отборы");
	Иначе
...
Функция ПоследниеОтборыНажатие(Кнопка) Экспорт
	...
	НастройкаКомпоновки = ирОбщий.ВыбранныйЭлементПоследнихЗначенийЛкс(ЭтаФорма, ОбластьПоиска, Кнопка, "Отборы", Истина);
	#Если Сервер И Не Сервер Тогда
		НастройкаКомпоновки = Новый НастройкиКомпоновкиДанных;
	#КонецЕсли
	ирОбщий.СкопироватьОтборЛюбойЛкс(ЭлементыФормы.КомпоновщикОтбор.Значение, НастройкаКомпоновки.Отбор);
	ирОбщий.ОбновитьТабличноеПолеДереваПослеУстановкиДанныхЛкс(ЭтаФорма, ЭлементыФормы.КомпоновщикОтбор);
...
Процедура КомпоновщикОтборПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
КонецПроцедуры
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПодборИОбработкаОбъектов.Форма.ПодборИОбработка");

мПлатформа = ирКэш.Получить();
#Если Сервер И Не Сервер Тогда
	ПриПолученииДанныхДоступныхПолей();
	ОтборКомпоновкиЗначениеНачалоВыбора();
	ОтборКомпоновкиЗначениеНачалоВыбораИзСписка();
	ОтборКомпоновкиЗначениеПриИзменении();
#КонецЕсли
...
ЭлементыФормы.КомпоновщикОтбор.Колонки.ПравоеЗначениеДляКраткогоОтображенияЭлемента.ЭлементУправления.УстановитьДействие("НачалоВыбораИзСписка", Новый Действие("ОтборКомпоновкиЗначениеНачалоВыбораИзСписка"));
ЭлементыФормы.КомпоновщикОтбор.Колонки.ПравоеЗначениеДляКраткогоОтображенияЭлемента.ЭлементУправления.УстановитьДействие("ПриИзменении", Новый Действие("ОтборКомпоновкиЗначениеПриИзменении"));
