...
Процедура ОтборКомпоновкиЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.КомпоновщикОтбор, СтандартнаяОбработка,, Истина,, "ПравоеЗначение");
...
Процедура КПОтборУстановитьПолеВПравомЗначении(Кнопка)
	...
	ирКлиент.УстановитьПолеВПравомЗначенииТабличПоляОтбораЛкс(ЭлементыФормы.КомпоновщикОтбор, ЭлементыФормы.КомпоновщикДоступныеПоляОтбора);
...
Процедура КПОтборНайтиВСписке(Кнопка)
	...
	ирКлиент.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭтаФорма, ЭлементыФормы.КомпоновщикДоступныеПоляОтбора, ЭлементыФормы.КомпоновщикОтбор);
	
КонецПроцедуры

Процедура КПВыборНайтиВСписке(Кнопка)
	
	ирКлиент.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭтаФорма, ЭлементыФормы.КомпоновщикДоступныеПоляОтбора, ЭлементыФормы.КомпоновщикВыбор);
...
Процедура КП_ДоступныеПоляПерейтиКОпределению(Кнопка)
	...
	Если ДоступноеПоле = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ирКлиент.ОткрытьРедакторОбъектаБДЛкс(ПолноеИмяПервойТаблицыБД(), "" + ДоступноеПоле.Поле);
...
Процедура ОтборКомпоновкиЗначениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Если ЛиЗапрешеноЗапоминатьЗначенияОтбора() Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ПолеВводаСИсториейВыбора_ОбновитьСписокЛкс(ЭлементыФормы.КомпоновщикОтбор.Колонки.ПравоеЗначениеДляКраткогоОтображенияЭлемента.ЭлементУправления,
