...
Функция НастроитьКомпоновщик(СохранятьТекущиеНастройки = Истина, ЗагружатьТекущиеНастройки = Ложь, НастройкиНабораТаблицБД = Неопределено, ИмяПоляКоличестваСтрок = "")
	...
	Если МноготабличнаяВыборка Тогда
		...
		Иначе
			Запрос = Новый Запрос;
			Запрос.Текст = ирОбщий.ТекстЗапросаПоВыбраннымТаблицамЛкс(ИменаОбъектовМД, ВариантИсточников, ПервыеNКаждойТаблицы, ПодключатьСвязанныеТаблицы, мИмяКолонкиПолногоИмениТаблицы, ИмяПоляКоличестваСтрок);
		КонецЕсли;
	Иначе
