...
Функция ПолучитьЗапрос()
	...
	Если ПодключатьПоляКоличестваПодчиненных Тогда
	...
	Если ПодключатьПоляНесогласованности И Не МноготабличнаяВыборка Тогда 
...
Процедура ДобавитьДоступныйОтборПоКоличествуСтрокПодчиненнойТаблицы(Знач КраткоеИмяПодчиненнойТаблицы, Знач ПолеСсылкиВПодчиненнойТаблице, Знач ПолноеИмяПодчиненнойТаблицы,
	...
	ПолныйПсевдонимТаблицы = ПрефиксИмениПоля + "_" + КраткоеИмяПодчиненнойТаблицы + "Т";
	ИмяПоля = "КоличествоСтрок_" + ПрефиксИмениПоля + "_" + КраткоеИмяПодчиненнойТаблицы;
	ТекстУсловияСоединенияПодчиненныхТаблиц = ТекстУсловияСоединенияПодчиненныхТаблиц + Символы.ПС + " 
	|	{ ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ _Регистр_." + ПолеСсылкиВПодчиненнойТаблице + ", КОЛИЧЕСТВО(*) КАК " + мИмяПоляКоличествоДвиженийСкрытое + "
	|	ИЗ " + ПолноеИмяПодчиненнойТаблицы + " КАК _Регистр_ СГРУППИРОВАТЬ ПО _Регистр_." + ПолеСсылкиВПодчиненнойТаблице + ") КАК " + ПолныйПсевдонимТаблицы + "
	|	ПО " + ПолныйПсевдонимТаблицы + "." + ПолеСсылкиВПодчиненнойТаблице + " = Т.Ссылка}";
	Если ТекстОбщееКоличествоДвижений <> "" Тогда
		ТекстОбщееКоличествоДвижений = ТекстОбщееКоличествоДвижений + " + ";
	КонецЕсли; 
	ВыражениеКоличества = "ЕСТЬNULL(" + ПолныйПсевдонимТаблицы + "." + мИмяПоляКоличествоДвиженийСкрытое + ", 0)";
...
Функция НастроитьКомпоновщик(СохранятьТекущиеНастройки = Истина, ЗагружатьТекущиеНастройки = Ложь, НастройкиНабораТаблицБД = Неопределено, ИмяПоляКоличестваСтрок = "")
	...
	Если МноготабличнаяВыборка Тогда
		...
		Тогда
			ВариантИсточников = 2;
		КонецЕсли; 
		ТекстЗапроса = ирОбщий.ТекстЗапросаПоВыбраннымТаблицамЛкс(ИменаОбъектовМД, ВариантИсточников, ПервыеNКаждойТаблицы, ПодключатьПоляКоличестваПодчиненных, мИмяКолонкиПолногоИмениТаблицы, ИмяПоляКоличестваСтрок);
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапроса;
	...
	#КонецЕсли
	СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
	...
	Если ПодключатьПоляКоличестваПодчиненных Тогда
		СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
		СлужебноеПолеНабора.ПутьКДанным = мИмяПоляКоличествоДвиженийСкрытое;
		СлужебноеПолеНабора.Поле = СлужебноеПолеНабора.ПутьКДанным;
		ирОбщий.УстановитьОграниченияИспользованияПоляНабораДанныхСхемыКомпоновкиЛкс(СлужебноеПолеНабора, Истина);
		Для Каждого КлючИЗначение Из мИменаКоличестваПодчиненных Цикл
			СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
			СлужебноеПолеНабора.ПутьКДанным = "КоличествоПодчиненных." + КлючИЗначение.Ключ;
			СлужебноеПолеНабора.Заголовок = "Количество подчиненных." + КлючИЗначение.Значение;
			СлужебноеПолеНабора.Поле = КлючИЗначение.Ключ;
		КонецЦикла;
	КонецЕсли;
	Если ПодключатьПоляНесогласованности Тогда
		Для Каждого КлючИЗначение Из мИменаПроверкиСогласованности Цикл
			СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
			СлужебноеПолеНабора.ПутьКДанным = "Несогласовано." + КлючИЗначение.Ключ;
			СлужебноеПолеНабора.Заголовок = "Несогласовано." + КлючИЗначение.Значение;
			СлужебноеПолеНабора.Поле = КлючИЗначение.Ключ;
		КонецЦикла;
	КонецЕсли;
	...
	КонецЕсли;
	Если МноготабличнаяВыборка Тогда
		СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Найти(мИмяКолонкиПолногоИмениТаблицы);
		...
		Если ЗначениеЗаполнено(ИмяПоляКоличестваСтрок) Тогда
			СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Найти(ИмяПоляКоличестваСтрок);
			Если СлужебноеПолеНабора = Неопределено Тогда
				СлужебноеПолеНабора = СхемаКомпоновки.НаборыДанных[0].Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
				СлужебноеПолеНабора.ПутьКДанным = ИмяПоляКоличестваСтрок;
			КонецЕсли; 
			СлужебноеПолеНабора.Поле = ИмяПоляКоличестваСтрок;
			СлужебноеПолеНабора.Заголовок = "Количество строк";
