...
Процедура вВыполнитьОбработку(Кнопка = Неопределено) Экспорт 
	...
	БлокируемыеЭлементыФормы.Добавить(ЭлементыФормы.Элемент1); //@skip-check unknown-method-property
	НастройкаВыполнения = ПолучитьНастройкуЛкс();
	#Если Сервер И Не Сервер Тогда
		НастройкаВыполнения = Новый Структура;
	#КонецЕсли
	НастройкаВыполнения.Вставить("Параметр1", 0);
	ВыполнитьЗаданиеГрупповойОбработки(ЭтаФорма, Кнопка, БлокируемыеЭлементыФормы, НастройкаВыполнения);
...
ирКлиент.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирПодборИОбработкаОбъектов.Форма.ШаблонОбработки");
