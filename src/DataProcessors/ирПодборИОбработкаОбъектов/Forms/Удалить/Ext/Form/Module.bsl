...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		КонецЕсли;
		ЭлементыФормы.УдалитьЧерезСУБД.Доступность = Ложь
...
Процедура УдалитьЧерезСУБДНажатие(Элемент)
	...
	Иначе
		ЧислоОбъектов = "Х";
	КонецЕсли;
	УдалятьСЗависимыми = Не ирКэш.ПараметрыЗаписиОбъектовЛкс().ОтключатьКонтрольЗаписи;
...
Процедура УдалитьОбъектыЧерезСУБД(Знач Запрос, Знач ИмяГлавнойТаблицы, Знач ИмяГлавнойВременнойТаблицы = "", Знач ИмяТаблицыКлючей = "", Знач ТекстСУБД = "", выхОтключенныеИтоги)
	...
	ИмяТипаТаблицыКлючей = ирОбщий.ПервыйФрагментЛкс(РезультатПоиска.Подгруппа0, ",");
	ИмяСсылкиТаблицыКлючей = РезультатПоиска.Подгруппа1;
	ТекстСУБД = ирОбщий.ЗаменитьРегВыражениеЛкс(ТекстСУБД, "INSERT\s+INTO\s+#\w+", "INSERT INTO " + ИмяТаблицыКлючейСУБД);
	ОпределениеПолейТаблицыКлючей = Новый Массив;
	//Если ЗначениеЗаполнено(ИмяТипаТаблицыКлючей) Тогда
	//	ОпределениеПолейТаблицыКлючей.Добавить(ИмяТипаТаблицыКлючей + " BINARY(16)");
	//КонецЕсли;
	ОпределениеПолейТаблицыКлючей.Добавить(ИмяСсылкиТаблицыКлючей + " BINARY(16)");
	ТекстСУБД = "CREATE TABLE " + ИмяТаблицыКлючейСУБД + " (" + ирОбщий.СтрСоединитьЛкс(ОпределениеПолейТаблицыКлючей) + ");" + Символы.ПС + ТекстСУБД;
	...
	Если УдалятьСЗависимыми Тогда
		...
		Если ирОбщий.ЛиКорневойТипДокументаЛкс(ИмяГлавнойТаблицы) Тогда
			...
			Для Каждого РегистрМД Из ПодчиненныеРегистры Цикл
				...
				УдалитьСтрокиТаблицыСУБДПорциями(Запрос, РегистрМД.ПолноеИмя(), ИмяТаблицыКлючей, ИмяТаблицыКлючейСУБД,, выхОтключенныеИтоги);
			КонецЦикла;
			ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
		КонецЕсли;
	...
	Если ирОбщий.ЛиКорневойТипДокументаЛкс(ИмяГлавнойТаблицы) Тогда
		...
		Для Каждого МетаЖурнал Из Метаданные.ЖурналыДокументов Цикл
			...
			Если Не МетаЖурнал.РегистрируемыеДокументы.Содержит(ГлавныйОбъектМД) Тогда
				Продолжить;
			КонецЕсли;
			УдалитьСтрокиТаблицыСУБДПорциями(Запрос, МетаЖурнал.ПолноеИмя(), ИмяТаблицыКлючей, ИмяТаблицыКлючейСУБД);
		КонецЦикла;
		ирОбщий.ОсвободитьИндикаторПроцессаЛкс(); 
	КонецЕсли;
	...
	КонецЦикла;
	ирОбщий.ОсвободитьИндикаторПроцессаЛкс(); 
	УдалитьСтрокиТаблицыСУБДПорциями(Запрос, ИмяГлавнойТаблицы, ИмяТаблицыКлючей, ИмяТаблицыКлючейСУБД);
...
Процедура УдалитьСтрокиТаблицыСУБДПорциями(Знач Запрос, Знач ИмяТаблицы, Знач ИмяТаблицыКлючей, Знач ИмяТаблицыКлючейСУБД, Знач ИмяПоляСсылки = "", ОтключенныеИтоги = Неопределено)
	
	Если ирОбщий.ЛиКорневойТипРегистраБДЛкс(ирОбщий.ТипТаблицыБДЛкс(ИмяТаблицы)) Тогда
		Если Не ЗначениеЗаполнено(ИмяПоляСсылки) Тогда
			ИмяПоляСсылки = "Регистратор";
		КонецЕсли;
		МенеджерРегистра = ирОбщий.ПолучитьМенеджерЛкс(ИмяТаблицы);
		#Если Сервер И Не Сервер Тогда
			МенеджерРегистра = РегистрыНакопления.РозничнаяВыручка;
	...
	ТекстЗапроса = "ИЗ " + ИмяТаблицы + " КАК Т
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ " + ИмяТаблицыКлючей + " КАК КлючиСтрокТ
		|ПО Т." + ИмяПоляСсылки + " = КлючиСтрокТ.Ссылка";
	Если МенеджерРегистра <> Неопределено Тогда
		...
		Если ПоляТаблицы.Найти(СловоПериод, "Имя") <> Неопределено Тогда 
			...
			Исключение
				МаксПериодИтогов = Неопределено;
			КонецПопытки;
			Запрос.Текст = "ВЫБРАТЬ МИНИМУМ(Период) " + ТекстЗапроса;
	...
	Если ирОбщий.ЛиКорневойТипРегистраБухгалтерииЛкс(ИмяТаблицы) Тогда
		МассивТаблиц.Добавить(ИмяТаблицы + ".Субконто");
	КонецЕсли;
	...
	Для Каждого ИмяЧастиТаблицы Из МассивТаблиц Цикл
		...
		ТекстСУБД = ирОбщий.ЗаменитьРегВыражениеЛкс(ТекстСУБД, "(FROM\s+[\w\.]+\s+(?:AS\s+)?\w+)", "$1 with (tablockx)"); 
