...
Функция ВыполнитьЗаданиеГрупповойОбработки(Знач ФормаОбработки, Знач Кнопка = Неопределено, Знач БлокируемыеЭлементыФормы = Неопределено, Знач НастройкаОбработки = Неопределено,
	...
	КонецЕсли; 
	НастройкаОбработки.Вставить("ПообъектныеТранзакции", ПообъектныеТранзакции);
	Если ТолькоКлиент Тогда
		ПараметрыЗадания.Вставить("ВыполнятьНаСервере", Ложь);
		НастройкаОбработки.Вставить("ПообъектныеТранзакции", Ложь);
	КонецЕсли;
...
Функция ВыполнитьГрупповуюОбработку(Параметры) Экспорт
	...
	Если Не Отказ Тогда
		...
		Попытка
			...
			СтруктураКлючаСтроки = ирОбщий.СтруктураКлючаТаблицыБДЛкс(?(МноготабличнаяВыборка, ОбластьПоиска[0].Значение, ОбластьПоиска), Истина,,, Ложь);
...
Функция ОбработатьЭлементыОбъекта(ПараметрыОбработкиОбъекта) Экспорт 
	...
	ПрименятьПообъектныеТранзакции = Ложь
		Или ПараметрыОбработки.ПообъектныеТранзакции
		Или (Истина
				...
				Для Каждого СтрокаДляОбработки Из КопияСтрокОбъекта Цикл
					...
					Иначе
						...
						Если НайденныеСтроки.Количество() = 0 Тогда
							ВызватьИсключение "Строка таблицы по ключу " + ирОбщий.РасширенноеПредставлениеЗначенияЛкс(КлючСтроки) + " не найдена в объекте БД";
						КонецЕсли;
