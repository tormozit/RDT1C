...
Функция ВыполнитьЗаданиеГрупповойОбработки(Знач ФормаОбработки, Знач Кнопка = Неопределено, Знач БлокируемыеЭлементыФормы = Неопределено, Знач НастройкаОбработки = Неопределено,
	...
	КонецЕсли;  
	ДоступнаяОбработка = ФормаОбработки.ВладелецФормы.ДоступныеОбработки.Строки.Найти(ПараметрыЗадания.ИмяОбработки, "ИмяФормы");
	НастройкаОбработки.Вставить("ТранзакцияОбъекта", ДоступнаяОбработка.ТранзакцияОбъекта);
	...
	РезультатЗадания = ирОбщий.ВыполнитьЗаданиеФормыЛкс("ВыполнитьГрупповуюОбработку", ПараметрыЗадания, ФормаОбработки, "ОбработкаОбъектов", "Обработка объектов " + ПараметрыЗадания.ИмяОбработки,
		Кнопка, "ВыполнитьОбработкуЗавершение", ФормаОбработки.Открыта(), БлокируемыеЭлементыФормы, Истина,, Истина, ПрефиксыОповещений, Истина);
			...
			Если ДинамическаяВыборка Тогда
				...
				Если Не ЕстьКлючевойНомерСтроки Тогда
					...
					Пока ВыборкаКлючей.Следующий() Цикл
						...
						ДобавитьОбъектВОчередьОбработки(ТипТаблицы, СтруктураКлючаОбъектаДоп, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, КопияСтрокОбъекта, ПараметрыОбработки,
...
Процедура ДобавитьОбъектВОчередьОбработки(ТипТаблицы, СтруктураКлючаОбъектаСПометкой, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, КопияСтрокОбъекта, ПараметрыОбработки,
		ЭтоПервыйОбъектБД, ЭтоПоследнийОбъектБД, ИмяОбъектаОбработкиОбъекта, ИмяМетодаОбработкиОбъекта, СтрокиРезультатовОбъекта = Неопределено, СтруктураПотоков) 
	ПараметрыОбработкиОбъекта = НовыеПараметрыОбработкиОбъекта(ТипТаблицы, СтруктураКлючаОбъектаСПометкой, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, КопияСтрокОбъекта, ПараметрыОбработки
		, ЭтоПервыйОбъектБД, ЭтоПоследнийОбъектБД, ИмяОбъектаОбработкиОбъекта, ИмяМетодаОбработкиОбъекта);
...
Функция НовыеПараметрыОбработкиОбъекта(ТипТаблицы, СтруктураКлючаОбъектаСПометкой, СтруктураКлючаОбъекта, СтруктураКлючаСтроки, КопияСтрокОбъекта, ПараметрыОбработки,
		ЭтоПервыйОбъектБД, ЭтоПоследнийОбъектБД, ИмяОбъектаОбработкиОбъекта, ИмяМетодаОбработкиОбъекта) Экспорт
...
Функция ОбработатьЭлементыОбъекта(ПараметрыОбработкиОбъекта) Экспорт 
	...
	Если ПараметрыОбработки.ТранзакцияОбъекта Тогда
	...
	Попытка
		...
		Тогда
			Если РежимОбходаДанных = "КлючиОбъектов" Тогда
				...
				СсылкаОбъекта = СтруктураКлючаОбъекта[ИмяПоляСсылка]; // ЛюбаяСсылка
				ОбъектДляОбработки.Вставить("Методы", СсылкаОбъекта);
				ОбъектДляОбработки.Вставить("Данные", СсылкаОбъекта);
			Иначе
		...
		ИначеЕсли ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(ТипТаблицы) Тогда
			...
			Если РежимОбходаДанных = "Строки" Тогда
				...
				КоллекцияСтрок = ОбъектДляОбработки.Данные[ИмяТЧ]; // ТаблицаЗначений
		...
		ИначеЕсли ирОбщий.ЛиМетаданныеРегистраЛкс(ОбъектМДОбъекта) Тогда
			...
			Иначе
				Если Истина
					...
					И ирОбщий.ЛиКорневойТипРегистраБухгалтерииЛкс(ТипТаблицы) 
				Тогда
...
//.
// Параметры:
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ШаблонОбработки").ПолучитьНастройкуЛкс() - 
//   Отказ -  - 
...
Процедура ИзменитьДобавитьСтроку_ОбработатьОбъект(Знач Объект, Знач КоллекцияСтрок = Неопределено, Знач ОбъектБДДанные = Неопределено, Знач ПараметрыОбработки = Неопределено, Знач ОбъектБДМетоды = Неопределено, выхПринудительнаяЗапись = Ложь) Экспорт
	...
	Для каждого СтрокаРеквизита из ПараметрыОбработки.РеквизитыДляСохранения Цикл
		...
		Тогда
			...
			#Если Сервер И Не Сервер Тогда
				ДопРеквизит = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПустаяСсылка(); // - БСП //@skip-check unknown-method-property
			#КонецЕсли
...
//.
// Параметры:
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ИзменитьРегистрациюНаУзле").ПолучитьНастройкуЛкс() - 
//   Отказ -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("Удалить").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ПровестиДокументы").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ОтменитьПроведениеДокументов").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ОчиститьДвиженияДокументов").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ИзменитьПометкуНаУдаление").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("Записать").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ЗаписатьВерсию").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ЗагрузитьИзВерсии").ПолучитьНастройкуЛкс() - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
...
//.
// Параметры:
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ПолучитьФорму("ВыгрузкаВФайл").ПолучитьНастройкуЛкс() - 
//   Отказ -  - 
Функция ВыгрузкаВФайл_ПередОбработкойВсех(ПараметрыОбработки, Отказ) Экспорт
...
//   КоллекцияСтрок -  - 
//   ОбъектБДДанные -  - 
//   ПараметрыОбработки - см. ОбщиеПараметрыЭлементов, см. ВыгрузкаВФайл_ПередОбработкойВсех - 
//   ОбъектБДМетоды -  - 
//   выхПринудительнаяЗапись -  - 
