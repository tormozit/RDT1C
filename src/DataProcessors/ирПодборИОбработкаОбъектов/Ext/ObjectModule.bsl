...
Функция ВыполнитьЗаданиеГрупповойОбработки(Знач ЭтаФорма, Знач Кнопка = Неопределено, Знач БлокируемыеЭлементыФормы = Неопределено, Знач НастройкаВыполнения = Неопределено, РазрешитьМногопоточность = Истина) Экспорт 
	...
	ПараметрыЗадания.Вставить("ИмяОбработки", ирОбщий.ПоследнийФрагментЛкс(ирОбщий.ИмяФормыИзФормыЛкс(ЭтаФорма)));
	...
	РезультатЗадания = ирОбщий.ВыполнитьЗаданиеФормыЛкс("ВыполнитьГрупповуюОбработку", ПараметрыЗадания, ЭтаФорма, "ОбработкаОбъектов", "Обработка объектов " + ПараметрыЗадания.ИмяОбработки,
		Кнопка, "ВыполнитьОбработкуЗавершение",, БлокируемыеЭлементыФормы, Истина,, Истина);
		...
		Иначе
			...
			Если Не ирОбщий.ЛиКорневойТипПеречисленияЛкс(ТипТаблицы) Тогда
				...
				Если ирОбщий.ЛиКорневойТипСсылкиЛкс(ТипТаблицы, Истина) Тогда
					ЭтоНовыйОбъект = ОбъектДляОбработки.Методы.ЭтоНовый();  // https://www.hostedredmine.com/issues/937634
				КонецЕсли; 
				Попытка
