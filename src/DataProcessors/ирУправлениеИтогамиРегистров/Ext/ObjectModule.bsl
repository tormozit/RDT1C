			...
			Иначе
				Если СтрокаТаблицы.ТипРегистра <> "РегистрСведений" Тогда
					...
					ИначеЕсли Команда = "УстановитьГраницыИтогов" Тогда
						...
						Тогда
							РегистрМенеджер.УстановитьПериодРассчитанныхИтогов(ПериодИтогов.МаксимальнаяДата); // - устарело, но совместимо с 8.2, @skip-check unknown-method-property
					...
					ИначеЕсли Команда = "ПерестроитьИЗаполнитьАгрегаты" Тогда
						Если СтрокаТаблицы.АгрегатыИтоги <> "Агрегаты" Тогда
							Продолжить;
						КонецЕсли;
						РегистрМенеджер.ПерестроитьИспользованиеАгрегатов();
					ИначеЕсли Команда = "ПолучитьОптимальныеАгрегаты" Тогда
						Если СтрокаТаблицы.АгрегатыИтоги <> "Агрегаты" Тогда
							Продолжить;
						КонецЕсли;
						РегистрМенеджер.ОпределитьОптимальныеАгрегаты();
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Исключение
