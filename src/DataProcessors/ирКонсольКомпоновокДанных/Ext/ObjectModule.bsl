...
Перем мКлючИсточника Экспорт; 
Перем мСтарыйТекстФайла Экспорт; 
...
Функция РеквизитыДляСервера(Параметры) Экспорт
	Результат = ирОбщий.ВыгрузитьРеквизитыОбработкиЛкс(ЭтотОбъект,, Истина);
	Возврат Результат;
КонецФункции
...
Функция ВывестиРезультат(Параметры) Экспорт
	...
	ПоляИтогаТаблицыВКоллекцию = Параметры.ПоляИтогаТаблицыВКоллекцию;
	ЭлементыРезультата = Неопределено;
	РезультатВывода = ирОбщий.СкомпоноватьОтчетВКонсолиЛкс(КоллекцияВывода, МакетКомпоновкиДанных, СтруктураВнешниеНаборыДанных, Автофиксация, Параметры.МодальныйРежим, ЛиОтладка, АдресДанныхРасшифровки,
			ВыполнятьНаСервере И Не Предпросмотр, ЭлементыРезультата, ВыполнятьПредварительныйЗапрос, МенеджерВременныхТаблиц, ДлительностьВыполненияМакета, ПоляИтогаТаблицыВКоллекцию);
	Результат = РезультатВывода(Параметры, АдресДанныхРасшифровки, КоллекцияВывода, Предпросмотр, РезультатВывода, ЭлементыРезультата);
	Возврат Результат;
КонецФункции

Функция РезультатВывода(Параметры, Знач АдресДанныхРасшифровки, Знач КоллекцияВывода, Знач Предпросмотр, Знач РезультатВывода, Знач ЭлементыРезультата) Экспорт
	...
	Результат.Вставить("ИмяЭУВывода", Параметры.ИмяЭУВывода);
	Результат.Вставить("ЭлементыРезультата", ЭлементыРезультата);
	Возврат Результат;
КонецФункции
...
Функция ОткрытьПоТаблицеЗначений(Знач ТаблицаЗначений, Знач НастройкаКомпоновки = Неопределено) Экспорт
	мСхемаКомпоновкиДанных = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицамЗначенийЛкс(Новый Структура("Основной", ТаблицаЗначений),,, Истина);
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(мСхемаКомпоновкиДанных));
...
Функция ОткрытьПоЗапросу(Знач Запрос, Отбор = Неопределено) Экспорт
	мСхемаКомпоновкиДанных = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросуЛкс(Запрос);
	Попытка
		КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(мСхемаКомпоновкиДанных));
	Исключение
...
Процедура ОткрытьПоТабличномуПолю(Знач ТабличноеПоле, Знач СхемаКомпоновки = Неопределено, Знач НастройкаКомпоновки = Неопределено, Знач ВнешниеНаборыДанных = Неопределено) Экспорт
	...
	Если СхемаКомпоновки = Неопределено Тогда
		мСхемаКомпоновкиДанных = ирОбщий.СоздатьСхемуКомпоновкиПоТаблицамЗначенийЛкс(ВнешниеНаборыДанных,,,,,, Истина);
	Иначе
...
Функция ОткрытьДляОтладки(Знач СхемаИлиМакетКомпоновки = Неопределено, Знач Настройки = Неопределено, Знач ВнешниеНаборыДанных = Неопределено,
		Модально = Ложь, АктивироватьСтраницуНастроек = Неопределено, СразуВыполнитьКомпоновку = Ложь, МенеджерВременныхТаблиц = Неопределено, Знач ИмяРедактируемойСхемы = "") Экспорт
	мИмяРедактируемойСхемы = ИмяРедактируемойСхемы;
	Форма = ЭтотОбъект.ПолучитьФорму();
...
Процедура ПредложитьЗаменуЗапросовПустышек(Схема) Экспорт
	Если ирКэш.ЛиВКонфигурацииЕстьМеханизмЗаменыЗапросовПустышекЛкс() Тогда
		Если Найти(ирОбщий.ОбъектВСтрокуXMLЛкс(Схема), "ПОМЕСТИТЬ Представления_") > 0 Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				Схема = ЗаменитьЗапросыПустышки(Схема);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Функция ЗаменитьЗапросыПустышки(Знач Схема) Экспорт
	АдресСхемы = ПоместитьВоВременноеХранилище(Схема); // Пришлось завернуть, т.к. в обычном приложении схему компоновки нельзя передать параметром
	АдресСхемы = ирСервер.ЗаменитьЗапросыПустышкиЛкс(АдресСхемы);
	НоваяСхема = ПолучитьИзВременногоХранилища(АдресСхемы);
	Возврат НоваяСхема;
КонецФункции
...
Функция РедактироватьСтруктуруСхемы(Знач ВладелецФормы, СтруктураСхемы, Модально = Ложь) Экспорт
	...
	КонецЕсли;
	Если СтруктураСхемы.Свойство("КлючИсточника") Тогда
		мКлючИсточника = СтруктураСхемы.КлючИсточника;
	КонецЕсли;
	Если СтруктураСхемы.Свойство("СтарыйТекстФайла") Тогда
		мСтарыйТекстФайла = СтруктураСхемы.СтарыйТекстФайла; 
	КонецЕсли;
...
Функция НоваяТаблицаВнешниеНаборыДанных() Экспорт
...
Функция НоваяТаблицаНастройкиОтчета() Экспорт
...
Процедура ОформитьЯчейкуСИменемМакета(Знач ЯчейкаТаблицыОбластиКомпоновкиДанных, Знач ИмяОбласти)
	#Если Сервер И Не Сервер Тогда
		...
		ЯчейкаТаблицыОбластиКомпоновкиДанных = ЯчейкаТаблицыОбластиКомпоновкиДанных.Добавить(0).Ячейки[0];
	#КонецЕсли
	ПолеОбластиКомпоновкиДанных = ЯчейкаТаблицыОбластиКомпоновкиДанных.Элементы.Добавить(Тип("ПолеОбластиКомпоновкиДанных"));
...
ДеревоОтчетов.Колонки.Добавить("СхемаКомпоновкиДанных"); // СхемаКомпоновкиДанных  
...
ДеревоОтчетов.Колонки.Добавить("Настройки"); // см. НоваяТаблицаНастройкиОтчета
ДеревоОтчетов.Колонки.Добавить("НастройкаДляЗагрузки");
ДеревоОтчетов.Колонки.Добавить("СохранятьНастройкиАвтоматически", Новый ОписаниеТипов("Булево"));
ДеревоОтчетов.Колонки.Добавить("События");
ДеревоОтчетов.Колонки.Добавить("ВнешниеНаборыДанных"); // см. НоваяТаблицаВнешниеНаборыДанных
ДеревоОтчетов.Колонки.Добавить("НомерыСтрокЗапросов"); 
