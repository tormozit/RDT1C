...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено, Предпросмотр = Ложь, СтруктураВнешниеНаборыДанных = Неопределено, Знач РазрашитьЗаполнятьРасшифровки = Истина)
	...
	Попытка
		МакетКомпоновкиДанных = ирОбщий.СобратьМакетКомпоновкиЛкс(ВременнаяСхемаКомпоновки, ВременныеНастройкиКомпоновки, СтруктураВнешниеНаборыДанных,,, ПроверятьДоступностьПолей,,
			РазрашитьЗаполнятьРасшифровки И Не НеЗаполнятьРасшифровки, ДанныеРасшифровкиЛ, МакетОформления, Тип(ИмяТипаГенератора), ВстраиватьЗначенияРесурсов);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
...
Процедура КоманднаяПанельРеультатаКопия(Кнопка)
	...
	ирОбщий.ПолеТабличногоДокументаВосстановитьОформлениеТекущихСтрокЛкс(ЭтаФорма, ПолеТабличногоДокумента);
					...
					Иначе
						...
						Если Построитель <> Неопределено Тогда
							Для Каждого ДоступноеПоле Из Построитель.ДоступныеПоля Цикл
								Если КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ДоступноеПоле.ПутьКДанным)) = Неопределено Тогда
									мОтключенныеПоля.Добавить(ДоступноеПоле.ПутьКДанным);
								КонецЕсли; 
							КонецЦикла;
						КонецЕсли;
					КонецЕсли;
...
Процедура КоманднаяПанельСтруктураПроверитьНастройки(Кнопка)
	...
	Если ЕстьНекорректныеЭлементы Тогда
		...
		Если Ответ = КодВозвратаДиалога.ОК Тогда
			ирОбщий.Сравнить2ЗначенияВФормеЛкс(СтрокаИсправленныхНастроек, СтрокаТекущихНастроек, Ложь, "Исправленные", "Текущие", , Ложь,,, "XML");
		КонецЕсли;
	Иначе
...
Процедура ОформитьТекущиеСтроки()
	ирОбщий.ПолеТабличногоДокументаОформитьТекущиеСтрокиЛкс(ЭтаФорма, ЭлементыФормы.РезультатТабличныйДокумент);
КонецПроцедуры
...
Процедура КП_МакетКомпоновкиДанныхСравнить(Кнопка)
	...
	Если ЭлементыФормы.ПанельМакетКомпоновки.ТекущаяСтраница = ЭлементыФормы.ПанельМакетКомпоновки.Страницы.XML Тогда
		ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.МакетКомпоновкиXML);
	Иначе
		ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.МакетаКомпоновкиПлоский);
	КонецЕсли; 
...
Процедура КоманднаяПанель1СравнитьСРезультирующим(Кнопка)
	...
	Если МакетКомпоновкиДанных <> Неопределено Тогда
		...
		Для Каждого ИмяНабора Из КоординатыСтроки Цикл
			...
			КонецЕсли; 
		КонецЦикла;
		ирОбщий.Сравнить2ЗначенияВФормеЛкс(ЭлементыФормы.ДеревоНаборовДанныхЗапросов.ТекущаяСтрока.НаборДанных.Запрос, НаборМакета.Запрос,, "Запрос схемы", "Запрос макета",,,,, "ЯзыкЗапросов");
	КонецЕсли;
