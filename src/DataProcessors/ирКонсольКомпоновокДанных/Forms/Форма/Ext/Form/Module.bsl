...
//                 <продолжение описания значения>.
//
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено, Предпросмотр = Ложь, СтруктураВнешниеНаборыДанных = Неопределено, Знач РазрешитьЗаполнятьРасшифровки = Истина,
	...
	Если ЛиОтладка Тогда
		ирОбщий.СообщитьСУчетомМодальностиЛкс("Компоновка макета в " + ИмяТипаГенератора + " - " 
			+ Строка(ирОбщий.ТекущееВремяВМиллисекундахЛкс() - НачалоКомпоновкиМакета) + " мс", МодальныйРежим);
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	Если мМенеджерВременныхТаблиц <> Неопределено Тогда
		ПериодОчисткиМенеджераВременныхТаблиц = ирОбщий.ПериодОчисткиМенеджераВременныхТаблицЛкс();
		#Если Сервер И Не Сервер Тогда
	...
	КонецЕсли;
	ДопСвойства = ирОбщий.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.РезультатТабличныйДокумент);
...
Процедура КоманднаяПанельСтруктураXML(Кнопка)
	...
	ирОбщий.ОткрытьТекстЛкс(ирОбщий.ОбъектВСтрокуXMLЛкс(КомпоновщикНастроек.Настройки), "Настройки компоновки " + ирОбщий.ПоследнийФрагментЛкс(ТекущаяДата(), " "), "XML");
...
Процедура КоманднаяПанельСтруктураЗагрузитьНастройки(Кнопка)
	...
	Файл = ирОбщий.ВыбратьФайлЛкс(, "xml", "Настройки компоновки");
