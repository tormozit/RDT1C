...
Процедура КоманднаяПанель1СравнитьСРезультирующим(Кнопка)
	...
	Если МакетКомпоновкиДанных <> Неопределено Тогда
		...
		Если ТекущаяСтрока.Родитель = Неопределено И ТекущаяСтрока.Строки.Количество() > 0 Тогда
		...
		Для Каждого ИмяНабора Из КоординатыСтроки Цикл
			...
			КонецЕсли; 
		КонецЦикла;
		ирОбщий.Сравнить2ЗначенияВФормеЛкс(ТекущаяСтрока.НаборДанных.Запрос, НаборМакета.Запрос,, "Запрос схемы", "Запрос макета",,,,, "ЯзыкЗапросов");
	КонецЕсли;
