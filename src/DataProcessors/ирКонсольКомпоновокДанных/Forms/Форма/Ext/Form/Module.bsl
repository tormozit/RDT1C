...
Процедура СоздатьФайл(Отладка = Ложь) Экспорт 
	...
	Если ЗначениеЗаполнено(мИмяРедактируемойСхемы) Тогда
...
Функция СохранитьКомпоновкиВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, Знач ИмяФайла = Неопределено, СброситьМодифицированность = Неопределено, Знач ЭтоФайлВосстановления = Ложь)
	...
	Тогда
		СтруктураПараметров = Новый Структура("СхемаКомпоновки, КомпоновщикНастроек, КлючИсточника, СтарыйТекстФайла", 
			мТекущаяСтрока.СхемаКомпоновкиДанных, КомпоновщикНастроек, мКлючИсточника, мСтарыйТекстФайла);
		...
		КонецЕсли;
		Если Истина
			И ЗначениеЗаполнено(мКлючИсточника) 
			И мПлатформа.мСвязанныйКонфигуратор <> Неопределено
		Тогда
			НовыйТекстСхемы = ирОбщий.ОбъектВСтрокуXMLЛкс(мТекущаяСтрока.СхемаКомпоновкиДанных);
			ЭтаФорма.Модифицированность = Ложь;
			Закрыть();
			мПлатформа.АктивизироватьОкноКонфигуратора(мКлючИсточника, НовыйТекстСхемы, мСтарыйТекстФайла);
			Возврат Истина;
		ИначеЕсли ВладелецФормы = Неопределено Тогда
			...
			ТекстовыйДокумент.УстановитьТекст(НовыйТекстСхемы);
...
Процедура КоманднаяПанельРезультатаКопия(Кнопка)
	...
	ирКлиент.ОткрытьТабличныйДокументИзПоляРезультатаКомпоновкиЛкс(ПолеТабличногоДокумента, ДанныеРасшифровки, "ирКонсольКомпоновки.Результат");
...
Процедура ПриОткрытии()
	...
	Тогда
		...
		Если мСхемаКомпоновкиДанных <> Неопределено Тогда
			...
			мСхемаКомпоновкиДанных = Неопределено; // Актуальная схема только в строке дерева
		КонецЕсли; 
...
Процедура _ЗапроситьОткрытиеКонсолиЗапросовПриОткрытии()
	...
	Если мМакетКомпоновкиДанных <> Неопределено Тогда
		...
		КонецЕсли; 
	Иначе
		Для Каждого КлючИЗначение Из ирОбщий.ВсеНаборыДанныхЗапросовКомпоновкиЛкс(мТекущаяСтрока.СхемаКомпоновкиДанных.НаборыДанных) Цикл
			НаборДанных = КлючИЗначение.Значение.НаборДанных;
