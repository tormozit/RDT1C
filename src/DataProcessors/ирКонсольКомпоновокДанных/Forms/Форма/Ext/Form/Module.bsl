...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено, Предпросмотр = Ложь, СтруктураВнешниеНаборыДанных = Неопределено)
	...
	Попытка
		МакетКомпоновкиДанных = ирОбщий.СобратьМакетКомпоновкиЛкс(ВременнаяСхемаКомпоновки, ВременныеНастройкиКомпоновки, СтруктураВнешниеНаборыДанных,,, ПроверятьДоступностьПолей,,
			Не НеЗаполнятьРасшифровки, ДанныеРасшифровкиЛ, СвойМакетОформления, Тип(ИмяТипаГенератора), ВстраиватьЗначенияРесурсов);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
...
Функция СкомпоноватьРезультатМой(Знач МакетКомпоновкиДанных = Неопределено, Знач СтруктураВнешниеНаборыДанных = Неопределено, Предпросмотр = Ложь, РазрешитьАсинхронно = Ложь)
	...
	ПараметрыЗадания.Вставить("ВыполнятьПредварительныйЗапрос", ирОбщий.ВосстановитьЗначениеЛкс("ир_ВыполнятьПредварительныйЗапрос"));
