...
Процедура МетаданныеВыборПриИзменении(Элемент = Неопределено)
	...
	ОписаниеМетода = мПлатформа.ОписаниеМетодаМодуля(Метод);
	Если Истина
		И ОписаниеМетода <> Неопределено
		И ЗначениеЗаполнено(ОписаниеМетода.Параметры)
	Тогда
		НомерПараметра = 1;
		Для Каждого ОписаниеПараметра Из мПлатформа.ПараметрыМетодаМодуля(ОписаниеМетода) Цикл
...
Процедура МетодОткрытие(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.ОткрытьМетодМодуляПоИмениЛкс(Метод);
