...
Процедура ОбновитьСписокРегламентныхЗаданийСПараметрами(ТекущийИдентификатор = Неопределено)
	...
	Если Истина
		...
		И ирОбщий.ОбщийМодульЛкс("БлокировкаРаботыСВнешнимиРесурсами") <> Неопределено
	Тогда
		ЗависимостиЗаданийБСП = Вычислить("РегламентныеЗаданияСлужебный").РегламентныеЗаданияЗависимыеОтФункциональныхОпций();
	...
	Для Каждого Регламентное из Регламентные Цикл
		...
		Если ЗависимостиЗаданийБСП <> Неопределено Тогда
			ОтборЗависимостиБСП.Вставить("РегламентноеЗадание", Регламентное.Метаданные);
...
Процедура ВыполнитьРегламентноеЗаданиеВЭтомСеансе(НаСервере = Ложь)
	...
	Если Истина
		...
		И ирОбщий.ОбщийМодульЛкс("БлокировкаРаботыСВнешнимиРесурсами").РаботаСВнешнимиРесурсамиЗаблокирована()
	Тогда
		//ИмяПользователя = РегламентноеЗадание.ИмяПользователя;
		ИмяПользователя = ИмяПользователя();
		ОбщегоНазначенияМодуль = ирОбщий.ОбщийМодульЛкс("ОбщегоНазначения");
		СтароеРазрешениеПоВнешнимРесурсам = ОбщегоНазначенияМодуль.ХранилищеСистемныхНастроекЗагрузить("РегламентныеЗадания", РегламентноеЗадание.Метаданные.ИмяМетода,,, ИмяПользователя) = Истина;
		Если Не СтароеРазрешениеПоВнешнимРесурсам Тогда
			...
			ИначеЕсли Ответ = КодВозвратаДиалога.Да Тогда
				ОбщегоНазначенияМодуль.ХранилищеСистемныхНастроекСохранить("РегламентныеЗадания", РегламентноеЗадание.Метаданные.ИмяМетода, Истина,, ИмяПользователя);
	...
	Исключение
		Если ВосстановитьРазрешениеПоВнешнимРесурсамБСП Тогда
			ОбщегоНазначенияМодуль.ХранилищеСистемныхНастроекСохранить("РегламентныеЗадания", РегламентноеЗадание.Метаданные.ИмяМетода, Ложь,, ИмяПользователя);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки;
	Если ВосстановитьРазрешениеПоВнешнимРесурсамБСП Тогда
...
Процедура СписокРегламентныхЗаданийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.РаботаетСВнешнимиРесурсамиБСП Тогда
		Если ирОбщий.ОбщийМодульЛкс("БлокировкаРаботыСВнешнимиРесурсами").РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
