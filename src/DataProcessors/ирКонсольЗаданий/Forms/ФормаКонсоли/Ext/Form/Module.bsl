...
Процедура ОбновитьАктивныеФоновыеЗадания() Экспорт
	...
	//	Возврат;
	//КонецЕсли;
	ДоступнаСтрогаяСвязь = ирКэш.ДоступноСтрогаяСвязьСеансаСФоновымЗаданиемЛкс();
...
Процедура ОбновитьСписокРегламентныхЗаданийСПараметрами(ТекущийИдентификатор = Неопределено)
	...
	ЗависимостиЗаданийБСП = Неопределено;
	Если ирКэш.ДоступнаБлокировкаРаботыСВнешнимиРесурсамиБСП() Тогда
		ЗависимостиЗаданийБСП = ирСервер.РегламентныеЗаданияЗависимыеБСП();
		ОтборЗависимостиБСП = Новый Структура;
		ОтборЗависимостиБСП.Вставить("РаботаетСВнешнимиРесурсами", Истина);
		ЭлементыФормы.СписокРегламентныхЗаданий.Колонки.РаботаетСВнешнимиРесурсамиБСП.Видимость = Истина;
	Иначе
		ЭлементыФормы.СписокРегламентныхЗаданий.Колонки.РаботаетСВнешнимиРесурсамиБСП.Видимость = Ложь;
	КонецЕсли;
	Для Каждого Регламентное из Регламентные Цикл
		...
		Если Истина
			И ЗависимостиЗаданийБСП <> Неопределено
			И Регламентное.Метаданные <> Неопределено
		Тогда
			ОтборЗависимостиБСП.Вставить("РегламентноеЗадание", Регламентное.Метаданные.ПолноеИмя());
			НоваяСтрока.РаботаетСВнешнимиРесурсамиБСП = ЗависимостиЗаданийБСП.НайтиСтроки(ОтборЗависимостиБСП).Количество();
		КонецЕсли;
	...
	КонецЕсли;
	Если КлючиВыделенныхСтрок <> Неопределено Тогда
...
Функция ОбновитьСписокФоновыхЗаданий(ВызовВнутриОбновленияРегламентныхЗаданий = Ложь, ДляСбораСтатистики = Ложь, Перезапустить = Ложь)
	...
	#Если Сервер И Не Сервер Тогда
		ФоновыеЗаданияПодробно(ПараметрыЗадания);
...
Процедура ПриОткрытии()
	...
	ИмяСобытия = ИмяСобытияПереключенияЗаданияДляЖурналаРегистрации();
	Кнопки = ЭлементыФормы.КПРегламентныеЗадания.Кнопки;
	ирКлиент.ВставитьЗначениеПараметраВПодсказкуКнопкиЛкс(Кнопки.Включить, ИмяСобытия);
	ирКлиент.ВставитьЗначениеПараметраВПодсказкуКнопкиЛкс(Кнопки.Выключить, ИмяСобытия);
	...
	Если ПараметрАктивироватьСписокМетодов Тогда
...
Процедура ПриОткрытииОтложенно()
	Если ПараметрОтключитьВсеЗадания Тогда
		КПРегламентныеЗаданияВыключитьВсе();
	КонецЕсли;
КонецПроцедуры

Процедура РегламентныеЗаданияОбновитьПодменюПоследниеВыбранные()
	ирКлиент.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КПРегламентныеЗадания.Кнопки.ПоследниеВыбранные, ЭлементыФормы.СписокРегламентныхЗаданий, Новый Действие("РегламентныеПоследниеВыбранныеНажатие"));
КонецПроцедуры
...
Процедура УстановитьОтборФоновыхЗаданий(Кнопка)
	...
	РезультатВыбора = Диалог.ОткрытьМодально();
	ирКлиент.ОткрытьИЗакрытьПустуюФормуЛкс();
	Если РезультатВыбора <> Неопределено Тогда
		ЭтаФорма.ОтборФоновыхЗаданий = РезультатВыбора;
...
Процедура УстановитьОтборРегламентныхЗаданий(Кнопка)
	...
	ирКлиент.ОткрытьИЗакрытьПустуюФормуЛкс();
	Если РезультатВыбора <> Неопределено Тогда
		ЭтаФорма.ОтборРегламентныхЗаданий = РезультатВыбора;
...
Процедура ВыполнитьРегламентноеЗаданиеВЭтомСеансе(НаСервере = Ложь)
	...
	ВосстановитьРазрешениеПоВнешнимРесурсамБСП = Ложь;
	Если Истина
		И ТекущиеДанные.РаботаетСВнешнимиРесурсамиБСП
		И ирСервер.ЛиРаботаСВнешнимиРесурсамиЗаблокированаБСП()
	Тогда
		//ИмяПользователя = РегламентноеЗадание.ИмяПользователя;
		ИмяПользователя = ИмяПользователя();
		СтароеРазрешениеПоВнешнимРесурсам = ХранилищеСистемныхНастроек.Загрузить("РегламентныеЗадания", РегламентноеЗадание.Метаданные.ИмяМетода,, ИмяПользователя) = Истина;
		Если Не СтароеРазрешениеПоВнешнимРесурсам Тогда
			Ответ = Вопрос("Задание обращается к внешним ресурсам, работа с которыми заблокирована. Запустить с обходом блокировки?", РежимДиалогаВопрос.ДаНетОтмена);
			Если Ответ = КодВозвратаДиалога.Отмена Тогда
				Возврат;
			ИначеЕсли Ответ = КодВозвратаДиалога.Да Тогда
				ХранилищеСистемныхНастроек.Сохранить("РегламентныеЗадания", РегламентноеЗадание.Метаданные.ИмяМетода, Истина,, ИмяПользователя);
				ВосстановитьРазрешениеПоВнешнимРесурсамБСП = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Попытка
		...
		КонецЕсли;
	Исключение
		Если ВосстановитьРазрешениеПоВнешнимРесурсамБСП Тогда
			ХранилищеСистемныхНастроек.Сохранить("РегламентныеЗадания", РегламентноеЗадание.Метаданные.ИмяМетода, Ложь,, ИмяПользователя);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки;
	Если ВосстановитьРазрешениеПоВнешнимРесурсамБСП Тогда
...
Процедура СписокРегламентныхЗаданийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	КонецЕсли;
	Если ДанныеСтроки.РаботаетСВнешнимиРесурсамиБСП Тогда
		Если ирСервер.ЛиРаботаСВнешнимиРесурсамиЗаблокированаБСП() Тогда
			ОформлениеСтроки.Ячейки.РаботаетСВнешнимиРесурсамиБСП.ЦветФона = ирОбщий.ЦветФонаОшибкиЛкс();
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура КоманднаяПанельМетодыПерейтиКРегламентномуЗаданию(Кнопка)
	...
	ирКлиент.НайтиСтрокуТабличногоПоляЛкс(ЭтаФорма, ЭлементыФормы.СписокРегламентныхЗаданий, "МетаданныеИмя", ТекущаяСтрока.Метаданные);
КонецПроцедуры
...
Процедура КПРегламентныеЗаданияПерейтиКМетоду(Кнопка)
	...
	ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.Страницы.Методы;
	ирКлиент.НайтиСтрокуТабличногоПоляЛкс(ЭтаФорма, ЭлементыФормы.Методы, "ИмяМетода", ИмяМетода);
КонецПроцедуры
...
Процедура КПРегламентныеЗаданияПерейтиКОпределениюМетода(Кнопка)
	Если ЭлементыФормы.СписокРегламентныхЗаданий.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ОткрытьМетодМодуляПоИмениЛкс(ЭлементыФормы.СписокРегламентныхЗаданий.ТекущаяСтрока.ИмяМетода);
КонецПроцедуры
...
Процедура КоманднаяПанельМетодыПерейтиКОпределениюМетода(Кнопка)
	Если ЭлементыФормы.Методы.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ОткрытьМетодМодуляПоИмениЛкс(ЭлементыФормы.Методы.ТекущаяСтрока.ИмяМетода);
КонецПроцедуры
...
Процедура КоманднаяПанельФоновыеЗаданияПерейтиКОпределениюМетода(Кнопка)
	Если ЭлементыФормы.СписокФоновыхЗаданий.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ОткрытьМетодМодуляПоИмениЛкс(ЭлементыФормы.СписокФоновыхЗаданий.ТекущаяСтрока.ИмяМетода);
КонецПроцедуры
...
Процедура КПРегламентныеЗаданияВыключитьВсе(Кнопка = Неопределено)
