...
Процедура ЗаполнитьНастройкиКолонокГруппы(Знач ЭтаФорма, Знач ГруппаКолонок)
	...
	Для Каждого КолонкаТП Из ирОбщий.ВсеПодчиненныеЭлементыФормыЛкс(ГруппаКолонок) Цикл
		...
		ОписаниеКолонки.Заголовок = СтрЗаменить(ОписаниеКолонки.Заголовок, Символы.ПС, "");
		Если Прав(ОписаниеКолонки.Заголовок, 1) = "…" Тогда
			ОписаниеКолонки.Заголовок = ирОбщий.ПервыйФрагментЛкс(КолонкаТП.Подсказка, Символы.ПС);
		КонецЕсли;
