...
Функция ЭлементОтбораПоляБыстрогоОтбора(Знач ЭтаФорма, Знач ПодчиненныйЭлемент)
	...
	ИндексЭлемента = Число(ирОбщий.НайтиРегулярноеВыражениеЛкс(ПодчиненныйЭлемент.Имя, "\d+")[0].ТекстВхождения);
...
Процедура Подключаемый_ВыполнитьКомандуНаСервере(ПараметрыВыполнения) Экспорт 
