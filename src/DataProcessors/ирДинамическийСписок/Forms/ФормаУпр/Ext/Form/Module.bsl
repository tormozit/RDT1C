...
Процедура ОбновитьПоказательНевидимыхКолонок()
	...
	ЧислоНевидимых = ирОбщий.ОтобратьЭлементыКоллекцииЛкс(фОбъект.НастройкиКолонок, Новый Структура("Видимость", Ложь)).Количество();
	Если ЗначениеЗаполнено(ЧислоНевидимых) Тогда
		ЧислоНевидимых = "-" + ЧислоНевидимых;
	Иначе
		ЧислоНевидимых = "*";
	КонецЕсли; 
	ирОбщий.ОбновитьТекстПослеМаркераВСтрокеЛкс(Элементы.ФормаНастроитьКолонки.Заголовок,, ЧислоНевидимых, ":");
