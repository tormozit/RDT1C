...
Процедура УстановитьОбъектМетаданных(Знач НовоеПолноеИмяТаблицы = Неопределено, Знач СохранитьНастройкиТаблицы = Ложь) Экспорт
	...
	Элементы.ФормаРедакторОбъектаБД.Доступность = ТипТаблицыБД <> "Изменения" И ТипТаблицыБД <> "ВиртуальнаяТаблица"; 
	Элементы.ПодменюРедакторОбъектаБД.Доступность = Элементы.ФормаРедакторОбъектаБД.Доступность; 
	...
	КонецЕсли;
	ОбновитьБыстрыеОтборы();
	ОбновитьПараметрыВывода(ЭтаФорма);
	Если РежимВыбора Тогда
		ЭтаФорма.КлючСохраненияПоложенияОкна = "Выбор;" + XMLСтрока(ирОбщий.ЛиКорневойТипПеречисленияЛкс(ТипТаблицыБД));
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбновитьБыстрыеОтборы()
		...
		Иначе
			...
			ИначеЕсли ирОбщий.СтрКончаетсяНаЛкс(ПодчиненныйЭлемент.Имя, СловоЗначение) Тогда
				...
				Если ЭлементОтбора <> Неопределено Тогда
					...
					ирОбщий.ПолеВводаСИсториейВыбора_НачалоВыбораИзСпискаЛкс(ПодчиненныйЭлемент, КлючИсторииВыбораЭлементаОтбора(ЭтаФорма, ЭлементОтбора));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ЭлементОтбораПоляБыстрогоОтбора(Знач ЭтаФорма, Знач ПодчиненныйЭлемент)
	...
	ЭлементыОтбора = ЭтаФорма.ДинамическийСписок.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Элементы;
	...
	ИндексЭлемента = ИндексЭлемента - 4;
...
Процедура ПолеБыстрогоОтбораПриИзменении(Элемент)
	...
	Если ЭлементОтбора <> Неопределено Тогда
		ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(ЭлементОтбора, фОбъект.ПолноеИмяТаблицы);
		ирОбщий.ПолеВводаСИсториейВыбора_НачалоВыбораИзСпискаЛкс(Элемент, КлючИсторииВыбораЭлементаОтбора(ЭтаФорма, ЭлементОтбора)); // Для ссылочных этот список пользователь не увидит - платформа его перекроет
	КонецЕсли;
...
Процедура ПолеБыстрогоОтбораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЭлементОтбора <> Неопределено Тогда 
		...
		СтруктураОтбора = ирОбщий.СтруктураОтбораЗначенийДоступногоПоляКомпоновкиЛкс(ДоступноеПолеОтбора);
		...
		КонецЕсли;
		Если ирОбщий.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(Элемент, СтандартнаяОбработка, ЭлементОтбора.ПравоеЗначение, СтруктураОтбора) Тогда 
...
Процедура ПолеБыстрогоОтбораАвтоПодбор(Элемент, Текст)
	...
	Тогда
		//ирОбщий.ПромежуточноеОбновлениеСтроковогоЗначенияПоляВводаЛкс(ЭтаФорма, Элемент, Текст); // вроде бы нет пользы
		Попытка
...
// На клиенте запрещено изменять состав кнопок
&НаСервере
...
Функция ПараметрыВыбораОбъектаМетаданных()
	Возврат ирОбщий.ПараметрыВыбораОбъектаМетаданныхЛкс(Истина, Истина, Истина, Истина,, Истина, Истина, Истина,,, Истина,,, Истина,,,,,, Истина);
КонецФункции
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Элементы.КомпоновщикНастроекОтбор82.Видимость = Ложь;
	КонецЕсли;
...
Процедура ДинамическийСписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	ПолноеИмяТаблицыБД = ПараметрыВывода.ИмяТаблицы;
	...
	Для Каждого СтрокаСписка Из Строки Цикл
		...
		Если Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных("ИдентификаторСсылкиЛкс")) <> Неопределено Тогда
			...
			Если ЯчейкаИдентификатора <> Неопределено Тогда
				ИдентификаторСсылки = ирОбщий.СтроковыйИдентификаторСсылкиЛкс(ирОбщий.КлючСтрокиТаблицыБДИзСтрокиТаблицыЗначенийЛкс(ПолноеИмяТаблицыБД, ДанныеСтроки));
...
Процедура ПроверитьЗапомнитьОтбор()
	...
	ОтборИзменен = Ложь;
	ДобавленВСписок = ирОбщий.ДобавитьОтборКомпоновкиВИсториюТаблицыБДЛкс(ЭтаФорма, фОбъект.ПолноеИмяТаблицы, АктивнаяНастройка, СтараяНастройка, ОтборИзменен);
...
Процедура НовоеОкно(Команда)
	...
	ирОбщий.ОткрытьФормуСпискаЛкс(фОбъект.ПолноеИмяТаблицы,, Истина,,,, Элементы.ДинамическийСписок.ТекущаяСтрока,, ПользовательскийОтбор(),, Новый УникальныйИдентификатор);
...
Процедура ПерейтиВФормуСписка(Знач ИспользоватьДинамическийСписокИР)
	...
	Форма = ирОбщий.ОткрытьФормуСпискаЛкс(фОбъект.ПолноеИмяТаблицы, ДинамическийСписок.Отбор, ИспользоватьДинамическийСписокИР, ВладелецФормы, РежимВыбора, МножественныйВыбор,
...
Процедура СброситьНастройкиСписка(Команда)
	...
	//ФормаНового = ирОбщий.ФормаДинамическогоСпискаЛкс(Новый Структура("КлючСохраненияПоложенияОкна", ТекущаяДата()),, КлючУникальности);
...
Функция ПоследниеОтборыНажатие(Кнопка) Экспорт
	...
	ирОбщий.ОбновитьТабличноеПолеДереваПослеУстановкиДанныхЛкс(ЭтаФорма, Элементы.КомпоновщикНастроекПользовательскийОтбор);
...
Процедура ДинамическийСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ирОбщий.ОписаниеТаблицыБДЛкс(фОбъект.ПолноеИмяТаблицы).Тип = "ВиртуальнаяТаблица" Тогда 
		...
		ирОбщий.ОткрытьФормуСпискаЛкс(ПолноеИмяМД,,,,,,,, ОтборИзмерений);
	Иначе
...
&НаКлиенте
// В частности полезно звать после программного изменения ширины колонок, из-за чего происходит горизонтальная прокрутка влево (баг платформы 8.3.23- #TP-552)
Процедура ПереустановитьТекущуюКолонку()
	...
	Элементы.ДинамическийСписок.ТекущийЭлемент = Элементы.ДинамическийСписок.ПодчиненныеЭлементы[0]; 
	Элементы.ДинамическийСписок.ТекущийЭлемент = НоваяТекущаяКолонка;
...
Процедура ДинамическийСписокПриАктивизацииСтроки(Элемент = Неопределено)
	...
	Если мСвязанныйРедакторОбъектаБД <> Неопределено И мСвязанныйРедакторОбъектаБД.Открыта() Тогда
		ирОбщий.УстановитьПрикреплениеФормыВУправляемомПриложенииЛкс(мСвязанныйРедакторОбъектаБД);
		ОткрытьСвязанныйРедакторОбъектаБДСтроки();
	КонецЕсли;
...
Процедура ОбновитьИтоги()
	...
	Иначе
		...
		ЗапросСписка.Параметры.Вставить("Ключи", ирОбщий.ТаблицаКлючейВыделенныхСтрокДинСпискаЛкс(ТабличноеПоле, фОбъект.ПолноеИмяТаблицы));
		...
		КонецЦикла;
		#Если Сервер И Не Сервер Тогда
			ирОбщий.ТабличноеПолеОбновитьТекстыПодваловЛкс();
			ирОбщий.ДанныеСтрокДинамическогоСпискаЛкс();
		#КонецЕсли
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьСвязанныйРедакторОбъектаБДСтроки()
	...
	ирОбщий.ОткрытьТекущуюСтрокуТабличногоПоляТаблицыБДВРедактореОбъектаБДЛкс(Элементы.ДинамическийСписок, фОбъект.ПолноеИмяТаблицы,, Истина, мСвязанныйРедакторОбъектаБД,, Ложь);
...
Процедура ВнешнееСобытие(Источник, Событие, Данные) Экспорт
	...
	Если Истина
		И ТекущийЭлемент = Элементы.ДинамическийСписок
		И Найти(Данные, ирКэш.КодыКлавишЛкс()["CTRL+A"]) = 1 
	Тогда
...
Процедура ОбновитьПараметрыВывода(ЭтаФорма)
	...
	#Если Клиент Тогда
		//ТекущееЗначение = ирОбщий.ТабличноеПолеЗначениеТекущейЯчейкиЛкс(Элементы.ДинамическийСписок); // Мультиметика901926764 Слишком тяжелое обновление всех строк. Поэтому фичу окраски текущего значения грязно отключил.
	#КонецЕсли
...
Процедура КомпоновщикНастроекПользовательскиеНастройкиЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)       
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, Элементы.КомпоновщикНастроекПользовательскиеНастройки, СтандартнаяОбработка, Значение, Истина,, "Значение");
КонецПроцедуры
...
Процедура КомпоновщикНастроекПользовательскиеНастройкиОтборПравоеЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, Элементы.КомпоновщикНастроекПользовательскийОтбор, СтандартнаяОбработка,, Истина,, "ПравоеЗначение");
КонецПроцедуры
...
Процедура КомпоновщикНастроекПользовательскиеНастройкиОтборПриАктивизацииСтроки(Элемент)
	ПодключитьОбработчикОжидания("ВосстановитьВыделеннуюСтрокуОтбора", 0.1, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ВосстановитьВыделеннуюСтрокуОтбора()
	
	// Антибаг платформы. Текущая строка пропадает из выделенных
	ТаблицаФормы = Элементы.КомпоновщикНастроекПользовательскийОтбор;
	Если Истина
...
Процедура ШиринаКолонок(Команда)
	...
	Для Каждого Колонка Из Элементы.ДинамическийСписок.ПодчиненныеЭлементы Цикл
		Если Не Колонка.РастягиватьПоГоризонтали Тогда
			Продолжить;
		КонецЕсли;
		Колонка.Ширина = Колонка.Ширина + ДобавитьСимволов;
	КонецЦикла;
	УстановитьТекущуюКолонку();
...
Процедура СвязанныеКолонкиБД(Команда)
	ирОбщий.ОткрытьРедакторОбъектаБДЛкс(фОбъект.ПолноеИмяТаблицы,,, Элементы.ДинамическийСписок.ТекущаяСтрока,,, Ложь,,,, Истина);
КонецПроцедуры
