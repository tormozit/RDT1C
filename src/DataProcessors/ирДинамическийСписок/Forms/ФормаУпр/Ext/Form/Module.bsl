...
Функция ОбновитьЗапросБД()
	...
	КонецЕсли; 
	СоединенныеРегистры = ирОбщий.ВыгрузитьСвойствоЭлементовКоллекцииЛкс(СоединяемыеРегистры.НайтиСтроки(Новый Структура("Соединить", Истина)), "ПолноеИмя");
...
Процедура ЗаполнитьНастройкиКолонокГруппы(Знач ГруппаКолонок)
	...
	Для Каждого КолонкаТП Из ГруппаКолонок.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(КолонкаТП) = Тип("ГруппаФормы") Тогда
			ЗаполнитьНастройкиКолонокГруппы(КолонкаТП);
			Продолжить;
		КонецЕсли;
...
Процедура ПрименитьНастройкиКолонок()
	...
	Для Счетчик = 1 По НачальноеКоличество Цикл
		...
		КолонкаТП = Элементы.Найти(Элементы.ДинамическийСписок.Имя + ОписаниеКолонки.Имя);
		Если КолонкаТП <> Неопределено Тогда
			//КолонкиТП.Сдвинуть(КолонкаТП, -КолонкиТП.Индекс(КолонкаТП));
			Элементы.Переместить(КолонкаТП, КолонкаТП.Родитель, КолонкаТП.Родитель.ПодчиненныеЭлементы[0]);
...
Процедура ОтборБезЗначенияВТекущейКолонке(Команда)
	...
	НазначитьПользовательскийИдентификатор = Ложь; // https://www.hostedredmine.com/issues/947618
	ирОбщий.ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(Элементы.ДинамическийСписок,,,,,, НазначитьПользовательскийИдентификатор);
	ПроверитьЗапомнитьОтбор();
...
Процедура ОтборПоЗначенияВТекущейКолонке(Команда)
	...
	АктуализироватьПутьКДаннымТекущегоПоля();
	НазначитьПользовательскийИдентификатор = Ложь; // https://www.hostedredmine.com/issues/947618
	ирОбщий.ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(Элементы.ДинамическийСписок, Истина,,,,, НазначитьПользовательскийИдентификатор);
	ПроверитьЗапомнитьОтбор();
