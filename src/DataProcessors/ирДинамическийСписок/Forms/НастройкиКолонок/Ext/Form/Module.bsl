...
Процедура ПриОткрытии()
	...
	мПоляТаблицыИсточника = Неопределено;
	Если ЗначениеЗаполнено(ПолноеИмяТаблицы) Тогда
		мПоляТаблицыИсточника = ирКэш.ПоляТаблицыБДЛкс(ПолноеИмяТаблицы, Истина);
		#Если Сервер И Не Сервер Тогда
			мПоляТаблицыИсточника = Новый ТаблицаЗначений;
		#КонецЕсли
	ИначеЕсли СвязанноеТабличноеПоле <> Неопределено Тогда 
		...
		Если ТаблицаЗначений <> Неопределено Тогда
			мПоляТаблицыИсточника = ТаблицаЗначений.Колонки;
		КонецЕсли;
	КонецЕсли;
	Если мПоляТаблицыИсточника <> Неопределено Тогда 
		Для Каждого СтрокаНастройкиКолонки Из ОбработкаОбъектСлужебная.НастройкиКолонок Цикл
			...
			КонецЕсли;
			ДоступноеПоле = мПоляТаблицыИсточника.Найти(ИмяПоляТаблицы);
...
Процедура НастройкиКолонокВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		...
		ДействияФормыАктивизироватьКолонку();
	ИначеЕсли Колонка = ЭлементыФормы.НастройкиКолонок.Колонки.ТипЗначения Тогда
		Если мПоляТаблицыИсточника <> Неопределено Тогда
			ДоступноеПоле = мПоляТаблицыИсточника.Найти(ВыбраннаяСтрока.Имя);
			Если ДоступноеПоле <> Неопределено Тогда
				ирКлиент.ОткрытьЗначениеЛкс(ДоступноеПоле.ТипЗначения, Ложь, СтандартнаяОбработка);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли; 
