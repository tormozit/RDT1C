...
Функция УстановитьОбъектМетаданных(Знач НовоеПолноеИмяТаблицы = Неопределено, Знач СохранитьНастройкиТаблицы = Ложь) Экспорт
	...
	мФормаУпрДляПодключаемыхКоманд = ирКлиент.ДобавитьКнопкиКомандБСПЛкс(ЭлементыФормы.КП_Список, ПолноеИмяТаблицы);
	...
	Тогда
		//! ОсновнойЭУ.Значение = 0; // СправочникСписок
...
Процедура ПриОткрытии()
	...
	#Если Сервер И Не Сервер Тогда
		УстановитьНовыйПриемОбъектаОтложено();
	#КонецЕсли
	ПодключитьОбработчикОжидания("УстановитьНовыйПриемОбъектаОтложено", 0.1, Истина);
...
Функция КлючСтрокиИзСсылки(Ссылка)
	...
	ИначеЕсли ирОбщий.ЛиСсылкаНаОбъектБДЛкс(Ссылка, Ложь) Тогда 
		...
		ДанныеСписка = ЭлементыФормы.ДинамическийСписок.Значение; // ТаблицаЗначений
...
Процедура ОбновлениеОтображения()
	...
	//ЧислоНевидимых = ирОбщий.ОтобратьКоллекциюЛкс(ЭлементыФормы.ДинамическийСписок.Колонки, Новый Структура("Видимость", Ложь)).Количество();
	#Если Сервер И Не Сервер Тогда
		Э = ЭлементыФормы.ДинамическийСписок.Колонки[0];
	#КонецЕсли
	ЧислоНевидимых = ирОбщий.ОтобратьКоллекциюЛкс(ЭлементыФормы.ДинамическийСписок.Колонки, "Не Э.Видимость ").Количество();
...
Функция ПоследниеОтборыНажатие(Кнопка) Экспорт
	...
	НастройкаКомпоновки = ирКлиент.ВыбранныйЭлементПоследнихЗначенийЛкс(ЭтаФорма, фОбъект.ПолноеИмяТаблицы, Кнопка, "Отборы");
	#Если Сервер И Не Сервер Тогда
...
Процедура УстановитьНовыйПриемОбъектаОтложено()
...
#Если Сервер И Не Сервер Тогда
	ЭлементыФормы.ДинамическийСписок.ТипЗначения = Новый ОписаниеТипов("СправочникСписок.ирАлгоритмы");   
#КонецЕсли
