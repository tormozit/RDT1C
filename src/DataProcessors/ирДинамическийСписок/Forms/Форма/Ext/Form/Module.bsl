...
Функция УстановитьОбъектМетаданных(НовоеПолноеИмяТаблицы = Неопределено) Экспорт
	...
	Исключение
	КонецПопытки;
	ЭлементыФормы.КП_Список.Кнопки.ДобавитьСтрокуЧерезРедакторОбъектаБД.Доступность = ЭтоСсылкаБД;
...
Процедура ЗагрузитьНастройкиТаблицы()
	...
	КонецЕсли;
	Попытка
		ЭлементыФормы.ДинамическийСписок.ИерархическийПросмотр = СохраненныеНастройки.ИерархическийПросмотр;
	Исключение
	КонецПопытки;
