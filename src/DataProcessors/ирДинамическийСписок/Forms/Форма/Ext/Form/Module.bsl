...
Процедура КП_СписокВыбрать(Кнопка)
	...
	Массив = ирОбщий.ВыделенныеСтрокиТабличногоПоляЛкс(ЭлементыФормы.ДинамическийСписок, Ложь);
	ДинамическийСписокВыбор(ТабличноеПоле, Массив, ТабличноеПоле.ТекущаяКолонка, Истина);
...
Процедура ПолеВводаКолонкиСписка_НачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если СвязиИПараметрыВыбора Тогда
		...
		СтруктураОтбора = ирОбщий.СтруктураОтбораПоСвязямИПараметрамВыбораЛкс(ТабличноеПоле.ТекущиеДанные, МетаРеквизит);
	КонецЕсли; 
