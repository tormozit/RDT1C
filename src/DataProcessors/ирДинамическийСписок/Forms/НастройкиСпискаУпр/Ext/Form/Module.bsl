	...
	Иначе
		...
		Если ПараметрАктивироватьГруппу = "Отбор" Тогда
			...
			Если ПараметрАктивироватьЭлемент <> Неопределено Тогда
				...
				ТаблицаФормы = ПользовательскиеНастройки.ПодчиненныеЭлементы[БазовыйИндекс + 0].ПодчиненныеЭлементы[1]; // Для английского встроенного языка
...
Процедура ОтладитьКомпоновку(Команда)
	...
	НастройкиКомпоновки = КомпоновщикРезультата.ПолучитьНастройки();
	Для Каждого ЭлементыЭлемент Из НастройкиКомпоновки.ПараметрыДанных.Элементы Цикл
		ПараметрСхемы = Схема.Параметры.Добавить();
		ПараметрСхемы.Имя = "" + ЭлементыЭлемент.Параметр;
	КонецЦикла;
	ирОбщий.ОтладитьЛкс(Схема,, НастройкиКомпоновки);
