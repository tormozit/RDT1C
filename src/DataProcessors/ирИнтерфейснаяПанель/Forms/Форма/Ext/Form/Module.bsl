...
Процедура ПриОткрытии()
	...
	КонецЕсли;
	//СвернутьЧастоеОтложенно(); // Так будут ломаться привязки положения при следующем открытии
...
Процедура ПоискПоМетаданным()
	...
	Для каждого стрКорень из дзВсеОбъекты.Строки цикл
		...
		Для каждого стрОбъект из стрКорень.Строки цикл
			...
			Тогда
				ПредставлениеОбъекта = ФильтруемоеПредставлениеСтрокиДерева(стрОбъект);
				ПремияФильтра = ирОбщий.НайтиФильтрВСловеЛкс(Ложь, ирОбщий.СтрСоединитьЛкс(СловаФильтра.Слова, " "), ПредставлениеОбъекта, НРег(ПредставлениеОбъекта), Истина);
				//Если ПремияФильтра = 0 Тогда
				//	Продолжить; // Нельзя пропускать, т.к. у этого фильтра строгий порядок слов
				//КонецЕсли;
...
Функция ФильтруемоеПредставлениеСтрокиДерева(СтрокаДерева) Экспорт
	Возврат СтрокаДерева.Имя + " " + СтрокаДерева.Представление;
КонецФункции
