...
Процедура ОткрытиеФормы(ГрупповаяКоманда = "", ТабличноеПоле = Неопределено)
	...
	Для Каждого ТекущаяСтрока Из ТабличноеПоле.ВыделенныеСтроки Цикл
		...
		Иначе
			НоваяФорма.Открыть();
			ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ФильтрИмен;
		КонецЕсли;
...
Процедура ПрочитатьНастройки() Экспорт 
	...
	Если Не ирКэш.ДоступныРегВыраженияЛкс() Тогда
		стНастройки.ИспользоватьРегулярныеВыражения = Ложь;
	КонецЕсли; 
	...
	КонецЕсли;
	Если НЕ стНастройки.Свойство("ИспользоватьДинамическийСписокИР") Тогда
		стНастройки.Вставить("ИспользоватьДинамическийСписокИР", Истина);
	КонецЕсли;
		...
		Для каждого стрСтатистика из СтатистикаИспользования цикл
			...
			Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(стПоиск.Имя, Ложь) Тогда
				...
				Если СтатическаяВетка.Фильтровать Тогда 
					...
					Если НЕ ?(стНастройки.ИспользоватьРегулярныеВыражения, 
...
Процедура ВыводСтандартныхИнструментов(ДеревоОбъектов = Неопределено, СтрокаПоиска = "")
	...
	СтрокаКорня = ?(ДеревоОбъектов = Неопределено, ДеревоИнтерфейса, ДеревоОбъектов).Строки.Найти(ПредставлениеКорняКорня);
		...
		Если НЕ РезультатЗапросаПоНастройкам.Пустой() Тогда
			...
			Пока ВыборкаНастроек.Следующий() Цикл
				...
				Если СтрокаКорняОтчетов = Неопределено Тогда
					СтрокДерева = ?(ДеревоОбъектов = Неопределено, ДеревоИнтерфейса, ДеревоОбъектов).Строки.Добавить();
...
Процедура КомандыПанелиВыгрузитьМетаданные(Кнопка)
	...
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстСпискаОбъектов = "";
	Для Каждого ТекущаяСтрока Из ЭлементыФормы.ДеревоИнтерфейса.ВыделенныеСтроки Цикл
