...
Процедура СгенерироватьКомандыКонтекстногоМенюПоСтроке(Знач ТабличноеПоле, Знач ТекущаяСтрока)
	...
	ИначеЕсли ЗначениеЗаполнено(ВидОбъекта) И ЗначениеЗаполнено(ИмяОбъекта) Тогда
		...
		Если Истина 
			И мПлатформа.ЛиЕстьСловоТипаКонтекстаЛкс("ОбъектМетаданных" + ВидОбъекта, "Макеты")
			И ОбъектМД.Макеты.Количество() 
		Тогда
...
Процедура ПриОткрытии()
	...
	КартинкаСтрок = ПолучитьМакет("Картинки").Рисунки[0].Картинка;
...
Функция НайтиАктивироватьСтрокуОбъектаИзПараметраФормы()
	...
	Если Истина
		...
		И СтрЧислоВхождений(ПараметрИмяОбъектаМетаданных, ".") > 1
	Тогда
		ПервыеФрагменты = ирОбщий.ПервыеФрагментыЛкс(ПараметрИмяОбъектаМетаданных);
		НайденнаяСтрока = Дерево.Строки.Найти(ПервыеФрагменты, "ПолноеИмяОбъекта", Истина);
	КонецЕсли;
...
Функция ПрефиксПолногоИмениМД(Знач Единственное)
	Если ирОбщий.ИмяТипаМДМножественноеИзЕдинЛкс(Единственное) = Неопределено Тогда
		ПрефиксПолногоИмени = "";
	Иначе
		ПрефиксПолногоИмени = ирОбщий.ПеревестиСтроку(Единственное) + ".";
	КонецЕсли;
...
Процедура ДобавитьОбъект(СтрокиВетви, Знач ОбъектМД, Знач ВидОбъекта, Знач ИндексКартинки, ЛиВнешнийИсточник = Ложь)

	#Если Сервер И Не Сервер Тогда
		ОбъектМД = Метаданные.Справочники.Валюты;
	#КонецЕсли
	стрСтрока = СтрокиВетви.Добавить();
	Если ЛиВнешнийИсточник Тогда
...
// Параметры:
//   ИмяЕдинственное -  - 
//   ИндексКартинкиВетви -  - см. ПолучитьМакет("Картинки").Рисунки[0].Картинка, не совместимо с ОбщаяКартинка.ирДеревоМетаданных
//   ИндексКартинкаОбъекта -  - см. ПолучитьМакет("Картинки").Рисунки[0].Картинка, не совместимо с ОбщаяКартинка.ирДеревоМетаданных
//   ЛиВнешнийИсточник -  - 
//   ЕстьДоступ -  - 
//   КоллекцияМД -  - 
//   Представление -  - 
Процедура ДобавитьВетвьМетаданных(Знач ИмяЕдинственное, Знач ИндексКартинкиВетви = 0, Знач ИндексКартинкаОбъекта = 0, Знач ЛиВнешнийИсточник = Ложь, Знач ЕстьДоступ = Истина, 
	...
	//ОписаниеТипа = ирОбщий.ОписаниеТипаМетаданныхЛкс(ирОбщий.ПервыйФрагментЛкс(ИмяЕдинственное));
	...
	//стрВетвь.ИндексКартинки = ОписаниеТипа.ИндексКартинкиМножественное;
	стрВетвь.ИндексКартинки = ИндексКартинкиВетви;
	стрВетвь.ЕстьДоступ = ЕстьДоступ;
	СтрокиВетви = стрВетвь.Строки;
	Для каждого ОбъектМД из КоллекцияМД цикл
		ДобавитьОбъект(СтрокиВетви, ОбъектМД, ИмяЕдинственное, 
			//ОписаниеТипа.ИндексКартинкиЕдинственное
			ИндексКартинкаОбъекта
			, ЛиВнешнийИсточник);
	КонецЦикла;
...
Процедура ЗаполнитьДеревоВсехОбъектов() Экспорт
	...
	Если ирКэш.НомерВерсииПлатформыЛкс() >= 805004 Тогда
		ДобавитьВетвьМетаданных("Очередь", 66, 66);
	КонецЕсли;
	...
	Если ирКэш.ДоступноВнешниеИсточникДанныхЛкс() Тогда 
		Для Каждого МетаВнешнийИсточникДанных Из Метаданные.ВнешниеИсточникиДанных Цикл
			ИмяВетви = "ВнешнийИсточникДанных." + МетаВнешнийИсточникДанных.Имя;
...
Процедура ВыводСтандартныхИнструментов(ДеревоОбъектов = Неопределено, СтрокаПоиска = "")
	...
	Если СтрокаКорня = Неопределено Тогда
		...
		СтрокаКорня.ИндексКартинки = 6; // - Форма
	...
	Для Каждого СтрокаИнструмента Из ТаблицаСтандартныхИнструментов Цикл
		Если ЛиОбъектПроходитФильтры(СтрокаИнструмента.Представление, СтрокаПоиска,,, СловаФильтра) Тогда
			...
			СтрокДерева.ИндексКартинки = 7; // - Формы
...
Процедура КомандыПанелиЗагрузитьМетаданные(Кнопка = Неопределено, Знач ЧерезЗеркалоБазы = Неопределено) Экспорт
	ЧерезЗеркалоБазы = ЧерезЗеркалоБазы = Истина; // при вызове через нажатие кнопки значение по умолчанию не подставляется
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
...
Процедура КлсКомандаТаблицаНажатие(Кнопка)
	ИсточникДействий = ТекущийЭлемент;
	Если ТипЗнч(ИсточникДействий) = Тип("ТабличноеПоле") Тогда
		ирКлиент.УниверсальнаяКомандаФормыЛкс(ЭтаФорма, Кнопка, ИсточникДействий);
	КонецЕсли;
КонецПроцедуры
