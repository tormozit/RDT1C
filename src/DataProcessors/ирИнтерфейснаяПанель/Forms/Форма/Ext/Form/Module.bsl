...
Процедура ОткрытиеФормы(ГрупповаяКоманда = "", ТабличноеПоле = Неопределено)
	...
	Для Каждого ТекущаяСтрока Из ТабличноеПоле.ВыделенныеСтроки Цикл
		...
		Если ТипЗнч(Команда) = Тип("КнопкаКоманднойПанели") Тогда
			...
			Попытка
				...
				Тогда
					...
					ИначеЕсли Кнопка.Имя = "КолонкиБД" Тогда
						НоваяФорма = ирКлиент.ПолучитьФормуЛкс("Обработка.ирКолонкиБД.Форма",,, ОбъектМД.ПолноеИмя());
			...
			КонецПопытки;
		Иначе
			Если ирОбщий.СтрКончаетсяНаЛкс(Команда, ".ФормаСписка") Тогда
		...
		Если СтруктураТекущейСтроки <> Неопределено Тогда 
			Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ИмяОбъекта, Ложь) Тогда
				ирКлиент.НачатьОтслеживаниеФормыСсылкиЛкс(НоваяФорма);
			Иначе
...
Процедура ОповеститьСвязанныеФормы(Знач ТабличноеПоле = Неопределено)
	...
	Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда 
		...
		Если ОбъектМД <> Неопределено Тогда
			...
			ирКлиент.ОповеститьФормыПодсистемыЛкс("ТаблицаБД", ирКэш.ИмяТаблицыИзМетаданныхЛкс(ОбъектМД.ПолноеИмя()), ЭтаФорма);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ЗаполнитьТабличныеПоляСтатистики(РежимЗаполненияИзбранного = Неопределено, Знач ОбновленнаяСтрока = Неопределено, Знач УстановитьТекущуюСтроку = Неопределено)
	...
	Если ОбновленнаяСтрока = Неопределено Тогда
		...
		Если СтатистикаИспользования.Найти("Справочники", "Вид") <> Неопределено Тогда
			// Старый формат конвретируем в новый
		...
		Иначе
			...
			Если СтатическаяВетка.Имя = "Последнее" Тогда
				...
				Для Индекс = МаксЭлементовСтатистики + 1 - ВыборкаСтатистики.Количество() По 0 Цикл // Обратный обход
					...
					Если стрСтатистика.КоличествоИспользований < 10 И Не стрСтатистика.Избранное Тогда
			...
			Для каждого стрСтатистика из ВыборкаСтатистики цикл
				...
				Тогда
					КлючТекущейСтроки = ирОбщий.СкопироватьУниверсальнуюКоллекциюЛкс(стПоиск);
				КонецЕсли; 
...
Процедура ЗаполнитьДеревоИнтерфейса() Экспорт
	...
	Если ЧислоДинамическихСтрок > 0 Тогда
		Если ЧислоДинамическихСтрок <= 30 Тогда
			ирКлиент.ТабличноеПолеДеревоЗначений_РазвернутьВсеСтрокиЛкс(элДерево, ИндексНачальнойСтроки);
...
Процедура ЗаполнитьДеревоВсехОбъектов() Экспорт
	...
	Если ирКэш.ДоступныВнешниеИсточникДанныхЛкс() Тогда 
		Для Каждого МетаВнешнийИсточникДанных Из Метаданные.ВнешниеИсточникиДанных Цикл
			...
			ВеткиМетаданных.Добавить(ИмяВетви);
...
Процедура ПоискПоМетаданным()
	...
	Для каждого стрКорень из дзВсеОбъекты.Строки цикл
		...
		Для каждого стрОбъект из стрКорень.Строки цикл
			Если НЕ ЛиОбъектПроходитФильтры(стрОбъект,, ВеткиМетаданных.НайтиПоЗначению(стрКорень.Имя) <> Неопределено, ОбъектыВыбранныхПодсистем, СловаФильтра, ПрерватьРодителя) тогда
...
Функция ЛиОбъектПроходитФильтры(СтрокаДерева, Знач СтрокаПоиска = "", ПроверятьПодсистему = Ложь, ОбъектыВыбранныхПодсистем = Неопределено, КэшСловаФильтра = Неопределено,
...
Процедура КомандыПанелиИсследовательОбъектов(Кнопка)
	...
	ирОбщий.ИсследоватьЛкс(ОбъектМДСтрокиСписка(),,,,,,,, ЭтаФорма);
