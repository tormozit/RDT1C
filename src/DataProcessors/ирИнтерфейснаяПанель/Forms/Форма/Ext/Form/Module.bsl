...
Процедура КП_ИзбранноеВыгрузитьСписок(Кнопка)
	...
	Для Каждого ВыделеннаяСтрока Из ирОбщий.ВыделенныеИлиВсеСтрокиТабличногоПоляЛкс(ЭлементыФормы.Избранное) Цикл
		КлючПоиска = Новый Структура("Имя, Вид");
		ЗаполнитьЗначенияСвойств(КлючПоиска, ВыделеннаяСтрока); 
		Найденные = ДеревоИнтерфейса.Строки.НайтиСтроки(КлючПоиска, Истина);
		Если Найденные.Количество() > 0 Тогда
			МассивИмен.Добавить(Найденные[0].ПолноеИмяОбъекта);
		КонецЕсли;
	КонецЦикла; 
...
Процедура КП_ИзбранноеЗагрузитьСписок(Кнопка)
	...
	Если РезультатФормы <> Неопределено Тогда
		...
		Для Каждого ИмяОбъекта Из ИменаОбъектов Цикл
			КлючПоиска = Новый Структура("ПолноеИмяОбъекта", ИмяОбъекта);
			Найденные = ДеревоИнтерфейса.Строки.НайтиСтроки(КлючПоиска, Истина);
			Если Найденные.Количество() > 0 Тогда
				ДобавитьСтрокуВСтатическуюВетку(Найденные[0], Истина);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
