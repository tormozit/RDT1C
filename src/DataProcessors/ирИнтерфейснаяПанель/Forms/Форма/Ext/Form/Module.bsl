...
Процедура ПриОткрытии()
	...
	КонецЕсли;
	ирОбщий.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.Дерево).МенеджерПоиска = ирОбщий.СоздатьМенеджерПоискаВТабличномПолеЛкс(,,,, Истина); // Для отключения раскраски
			...
			Попытка
				...
				Тогда
					...
					ИначеЕсли Кнопка.Имя = "ДинамическийСписок" Тогда
						НоваяФорма = ирОбщий.ПолучитьФормуСпискаЛкс(СтрокаПолногоИмени,, Истина);
					...
					ИначеЕсли Кнопка.Имя = "СвязанныйДинамическийСписок" Тогда
						НоваяФорма = ирОбщий.ФормаДинамическогоСпискаЛкс(,, "Связанный");
...
Процедура ПоискПоМетаданным()
	...
	Для каждого стрКорень из дзВсеОбъекты.Строки цикл
		...
		Для каждого стрОбъект из стрКорень.Строки цикл
			Если НЕ ОбъектПроходитФильтры(стрОбъект,, ВеткиМетаданных.НайтиПоЗначению(стрКорень.Представление) <> Неопределено, ОбъектыВыбранныхПодсистем, СловаФильтра, ПрерватьРодителя) тогда
...
Процедура ВыводСодержимогоКаталогов(ДеревоОбъектов = Неопределено, СтрокаПоиска="")
	...
	Для каждого Каталог Из мНастройкиФормы.Каталоги Цикл
		...
		Для каждого НайденныйФайл Из МассивНайденныхФайлов Цикл
			...
			Если Не ОбъектПроходитФильтры(НайденныйФайл.ИмяБезРасширения, СтрокаПоиска,,, СловаФильтра, ПрерватьРодителя) Тогда
				Если ПрерватьРодителя Тогда
					Прервать;
				КонецЕсли;
				Продолжить;
...
Функция ОбъектПроходитФильтры(СтрокаДерева, Знач СтрокаПоиска = "", ПроверятьПодсистему = Ложь, ОбъектыВыбранныхПодсистем = Неопределено, КэшСловаФильтра = Неопределено,
	выхПрерватьРодителя = Ложь) Экспорт 
		...
		Если КэшСловаФильтра.СловоРодителя <> "" Тогда
			Если Ложь
				...
				Или (Истина
					...
					И Не ПроверитьУсловие(СтрокаДерева.Родитель.Представление, КэшСловаФильтра.СловоРодителя)) // Проверка регулярным выражением
				Или (Истина
					И Не мНастройкиФормы.ПоискЧерезРегулярныеВыражения 
					И Найти(НРег(СтрокаДерева.Родитель.Представление), КэшСловаФильтра.СловоРодителя) = 0)
			Тогда
	...
	Тогда
		...
		Если ирКэш.НомерИзданияПлатформыЛкс() >= "82" Тогда
			ОбъектМД = ОбъектМДВетви(СтрокаДерева);
...
Процедура КомандыПанелиГлобальныеСочетанияКлавиш(Кнопка)
	...
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(ТаблицаЗначений,, Ложь, ЗаголовокТаблицы,,,, ЗаголовокТаблицы);
КонецПроцедуры

Процедура ДеревоСтрокаПоискаОчистка(Элемент, СтандартнаяОбработка)
	
	Если Элемент.Значение = "" Тогда
		ирОбщий.ТабличноеПолеДеревоЗначений_СвернутьВсеСтрокиЛкс(ЭлементыФормы.Дерево);
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
