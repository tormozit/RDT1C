...
Процедура ПриОткрытии()
	...
	КонецЕсли;
	Если мНастройкиФормы.ТекущееИзбранное <> Неопределено Тогда
		ЭлементыФормы.Избранное.ТекущаяСтрока = Избранное[мНастройкиФормы.ТекущееИзбранное];
	КонецЕсли;
	ирОбщий.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.Дерево).МенеджерПоиска = ирОбщий.СоздатьМенеджерПоискаВТабличномПолеЛкс(); // Для отключения раскраски
	...
	Для Каждого ТекущаяСтрока Из ТабличноеПоле.ВыделенныеСтроки Цикл
			...
			Попытка
				...
				Тогда
					...
					ИначеЕсли Кнопка.Имя = "ПодпискиНаСобытия" Тогда
						НоваяФорма = ирОбщий.ПолучитьФормуЛкс("Отчет.ирПодпискиНаСобытия.Форма",,, СтрокаПолногоИмени);
						НоваяФорма.ОбъектМетаданных = СтрокаПолногоИмени;
					...
					ИначеЕсли Кнопка.Имя = "СтруктураХранения" Тогда
						НоваяФорма = ирОбщий.ФормаСтруктурыХраненияТаблицыБДЛкс();
						НоваяФорма.ПараметрИмяТаблицы = ирКэш.ИмяТаблицыИзМетаданныхЛкс(ОбъектМД.ПолноеИмя());
		...
		Иначе
			Если Найти(Команда + "!", ".ФормаСписка!") > 0 Тогда
				...
				КонецЕсли;
				Попытка
					НоваяФорма = ирОбщий.ПолучитьФормуСпискаЛкс(ИмяМД,, ИспользоватьДинамическийСписокИРСейчас);
				Исключение
					ирОбщий.СообщитьЛкс(ОписаниеОшибки(), СтатусСообщения.Внимание);
					Продолжить;
				КонецПопытки;
...
Процедура СохранитьНастройки() Экспорт
	...
	мНастройкиФормы.РазвернутыеСтроки = СохранитьРазвернутыеВетви();
...
Процедура ПрочитатьНастройки() Экспорт 
	...
	Если НЕ мНастройкиФормы.Свойство("СтатистикаИспользования") тогда
		мНастройкиФормы.Вставить("СтатистикаИспользования");
		тзСтатистикаИспользования = Новый ТаблицаЗначений;
		...
		мНастройкиФормы.СтатистикаИспользования = тзСтатистикаИспользования;
	КонецЕсли;	
	...
	КонецЕсли;
	Если НЕ мНастройкиФормы.Свойство("ИсторияКоманд") Тогда
		мНастройкиФормы.Вставить("ИсторияКоманд", ИсторияКоманд);
	КонецЕсли;
	Если НЕ мНастройкиФормы.Свойство("ИспользоватьРегулярныеВыражения") Тогда
		мНастройкиФормы.Вставить("ИспользоватьРегулярныеВыражения", ИспользоватьРегулярныеВыражения);
	КонецЕсли;
	Если Не ирКэш.ДоступныРегВыраженияЛкс() Тогда
		мНастройкиФормы.ИспользоватьРегулярныеВыражения = Ложь;
	КонецЕсли; 
	Если мНастройкиФормы.ИспользоватьРегулярныеВыражения Тогда
	...
	Для каждого СтатическаяВетка Из МассивСтатическихВеток Цикл
		...
		Для каждого стрСтатистика из СтатистикаИспользования цикл
			...
			Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(стПоиск.Имя, Ложь) Тогда
				...
				Если СтатическаяВетка.Фильтровать Тогда 
					ПредставлениеСсылки = Строка(СсылкаПоИД);
					Если НЕ ?(мНастройкиФормы.ИспользоватьРегулярныеВыражения, 
			...
			Иначе
				Если стрСтатистика.СтрокаДерева = Неопределено Тогда
					...
					Если мСтроки.Количество() = 0 тогда
						...
						мСтроки = Дерево.Строки.НайтиСтроки(стПоиск, Истина);
						Если мСтроки.Количество() > 0 тогда
							стрОбъект = мСтроки[0];
						Иначе
							стрОбъект = Null;
...
Процедура ВыводСтандартныхИнструментов(ДеревоОбъектов = Неопределено, СтрокаПоиска = "")
	...
	Для Каждого СтрокаИнструмента Из ТаблицаСтандартныхИнструментов Цикл
		Если ОбъектПроходитФильтры(СтрокаИнструмента.Представление, СтрокаПоиска,,, СловаФильтра) Тогда
...
Процедура ВыводСохраненныхНастроекОтчетов(ДеревоОбъектов = Неопределено, СтрокаПоиска = "")
	...
	Если ЕстьУниверсальныйОтчет И Есть_РС_СохраненныеНастройки Тогда
		...
		Если НЕ РезультатЗапросаПоНастройкам.Пустой() Тогда
			...
			СтрокаКорняОтчетов = ?(ДеревоОбъектов = Неопределено, Дерево, ДеревоОбъектов).Строки.Найти("Отчеты");
			СловаФильтра = Неопределено;
			...
			Пока ВыборкаНастроек.Следующий() Цикл
				...
				Если СтрокаКорняОтчетов = Неопределено Тогда
					...
					СтрокДерева.Имя = СтрокДерева.Представление;
...
Функция ПроверитьУсловие(СтрокаПоиска, ШаблонПоиска, ТолькоПроверка = Истина, ШаблонЗамены = "", ИтоговаяСтрока = "") Экспорт 
	...
	Попытка
		...
		Найдено = RegExp.Проверить(СтрокаПоиска); 
	Исключение
		//ирОбщий.СообщитьЛкс(ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
	...
	Если Не Найдено Или ТолькоПроверка тогда 
		...
		возврат Найдено ; 
	Иначе 
		ИтоговаяСтрока = RegExp.Заменить(СтрокаПоиска, ШаблонЗамены); 
		возврат Найдено; 
	КонецЕсли; 
...
Функция ОбъектПроходитФильтры(стрОбъект, Знач СтрокаПоиска = "", ПроверятьПодсистему = Ложь, ОбъектыВыбранныхПодсистем = Неопределено, КэшСловаФильтра = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Если ТипЗнч(СтрОбъект) = Тип("СтрокаДереваЗначений") Тогда
			...
			Представление = стрОбъект.Имя + " " + стрОбъект.Представление;
		Иначе
			// В случае передачи имени файла
			Представление = стрОбъект;
		КонецЕсли;
		Если КэшСловаФильтра = Неопределено Тогда
			КэшСловаФильтра = ирОбщий.РазделитьСтрокуПоискаНаСловаПоискаЛкс(СтрокаПоиска);
		КонецЕсли; 
		Для Каждого СтрокаПоиска Из КэшСловаФильтра Цикл
			...
			Если НЕ ПустаяСтрока(СокрЛП(СтрокаПоиска)) Тогда
				Если Ложь
					Или (Истина
						И мНастройкиФормы.ИспользоватьРегулярныеВыражения 
						И Не ПроверитьУсловие(Представление, СтрокаПоиска)) // Проверка регулярным выражением
					Или (Истина
						И Не мНастройкиФормы.ИспользоватьРегулярныеВыражения 
...
Функция УниверсальныйОтчетПоРегиструНакопления(ИмяРегистра)
	...
	Иначе
		...
		ФормаОтчета = ирОбщий.ПолучитьФормуЛкс("Отчет.УниверсальныйОтчет.Форма", ПараметрыФормы,, ОбъектМД.ПолноеИмя());
	КонецЕсли;
	Возврат ФормаОтчета;
...
Функция ОткрытьФункциональныеОпцииОбъекта(ПолноеИмяМД)
	...
	КонецЕсли;
	ФормаПросмотра = ирОбщий.ОткрытьЗначенияФункциональныхОпцийЛкс(ЗначенияФункОпций, ПолноеИмяМД);
...
Процедура ПрименитьРежимИмяСиноним()
	...
	КонецЦикла;
	ирОбщий.ТабличноеПолеОбновитьКолонкиИмяСинонимЛкс(ЭлементыФормы.Дерево, ЭтотОбъект.флИмяСиноним);
...
Процедура ЗаполнитьКоличествоСтрок()
	...
	Если ЭлементыФормы.Дерево.Колонки.КоличествоСтрок.Видимость Тогда
		ирОбщий.ЗаполнитьКоличествоСтрокТаблицВДеревеМетаданныхЛкс(Дерево);
	КонецЕсли;
...
Процедура КомандыПанелиИсследовательОбъектов(Кнопка)
	...
	Форма.Открыть();
	ДеревоИнтерфейсаПриАктивизацииСтроки(ЭлементыФормы.Дерево);
...
Процедура КомандыПанелиВыгрузитьМетаданные(Кнопка)
	...
	Если Не ЗначениеЗаполнено(ТекстСпискаОбъектов) Тогда
		...
		Возврат;
	КонецЕсли;
	КаталогВыгрузкиКонфигурации = ирОбщий.ВыбратьКаталогВФормеЛкс(КаталогВыгрузкиКонфигурации,, "Укажите каталог выгрузки файлов метаданных");
	...
	КонецЕсли;
	ИмяФайлаСпискаОбъектов = ПолучитьИмяВременногоФайла("txt");
	//ИмяФайлаСпискаОбъектов = Вычислить("КаталогДокументов()") + "\СписокОбъектовКонфигурации.lst"; // В 8.2 нет
	ТекстовыйДокумент.УстановитьТекст(ТекстСпискаОбъектов);
	ТекстовыйДокумент.Записать(ИмяФайлаСпискаОбъектов);
	ТекстЛога = "";
	Успех = ирОбщий.ВыполнитьКомандуКонфигуратораЛкс("/DumpConfigToFiles """ + КаталогВыгрузкиКонфигурации + """ -listFile """ + ИмяФайлаСпискаОбъектов + """ -Format Plain",
		СтрокаСоединенияИнформационнойБазы(), ТекстЛога,,,,,,,, Истина,, Истина);
...
Процедура КП_ИзбранноеВыгрузитьСписок(Кнопка)
	...
	Для Каждого ВыделеннаяСтрока Из ирОбщий.ВыделенныеИлиВсеСтрокиТабличногоПоляЛкс(ЭлементыФормы.Избранное) Цикл
		...
		Иначе
			МассивИмен.Добавить(ОбъектМДВетви(ВыделеннаяСтрока).ПолноеИмя());
		КонецЕсли;
	КонецЦикла; 
	ирОбщий.ОткрытьТекстЛкс(ирОбщий.СтрСоединитьЛкс(МассивИмен, Символы.ПС), "Список избранного",,,,, Истина);
...
Процедура КП_ИзбранноеЗагрузитьСписок(Кнопка)
	...
	КонецЦикла;
	Форма = ирОбщий.ПолучитьФормуТекстаЛкс(ирОбщий.СтрСоединитьЛкс(ИменаОбъектов, Символы.ПС), "Введите имена объектов по одному на строке");
	...
	Если РезультатФормы <> Неопределено Тогда
		...
		Для Каждого ИмяОбъекта Из ИменаОбъектов Цикл
			...
			Если ЗначениеЗаполнено(ЗначениеСсылки) Тогда
				ирОбщий.ДобавитьСсылкуВИзбранноеЛкс(ЗначениеСсылки);
			Иначе
...
Процедура КомандыПанелиТаблицаТиповМетаданных(Кнопка)
	 ирОбщий.ОткрытьТаблицуЗначенийЛкс(ирКэш.ТипыМетаОбъектов(),, Ложь, "Типы метаданных");
КонецПроцедуры

Процедура КомандыПанелиТаблицаВсехТаблицБД(Кнопка)
	 ирОбщий.ОткрытьТаблицуЗначенийЛкс(ирКэш.ТаблицаВсехТаблицБДЛкс(),, Ложь, "Таблицы БД");
КонецПроцедуры
...
Процедура КомандыПанелиВсеСочетанияКлавиш(Кнопка)
	...
	ирОбщий.ОткрытьТаблицуЗначенийЛкс(ВсеСочетанияКлавиш,, Ложь, ЗаголовокТаблицы,,,, ЗаголовокТаблицы);
	
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирИнтерфейснаяПанель.Форма.Форма");
