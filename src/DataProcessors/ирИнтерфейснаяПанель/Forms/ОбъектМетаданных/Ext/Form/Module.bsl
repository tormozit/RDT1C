...
Процедура ДействияФормыНайтиВИнтерфейснойПанели(Кнопка)
	ирКлиент.ОткрытьОбъектМетаданныхЛкс(ПолноеИмя, Истина);
КонецПроцедуры
...
Процедура ДействияФормыКолонкаБД(Кнопка)
	ОписаниеТаблицы = ирОбщий.ОписаниеТаблицыБДЛкс(Родитель);
	ПоляТаблицыБД = ирОбщий.ПоляТаблицыМДЛкс(Родитель);
	Если ПоляТаблицыБД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ПолеБД = ПоляТаблицыБД.Найти(мОбъектМД, "Метаданные");
	Если ПолеБД = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ОткрытьКолонкуБДЛкс(ОписаниеТаблицы.ПолноеИмя, ПолеБД.Имя);
КонецПроцедуры
