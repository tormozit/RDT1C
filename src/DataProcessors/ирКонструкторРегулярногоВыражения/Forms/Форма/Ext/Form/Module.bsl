...
Процедура КПВыражениеПерейтиКОпределению(Кнопка)
	...
	Результат = ирОбщий.НайтиРегулярноеВыражениеЛкс(НачалоСтроки, мЛеваяСкобкаПараметра + "(" + шИмя + ")?$",, Ложь,,,,, РегВычислитель);
	#Если Сервер И Не Сервер Тогда
	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	...
	Результат = ирОбщий.НайтиРегулярноеВыражениеЛкс(КонецСтроки, "^(" + шИмя + ")?" + мПраваяСкобкаПараметра,, Ложь,,,,, РегВычислитель);
	#Если Сервер И Не Сервер Тогда
	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	Если Результат.Количество() = 0 Тогда
...
Процедура ПерейтиКПараметру(Знач ТекущееИмя, Знач ВыделитьФрагментВВыражении = "")
	...
	Если ЗначениеЗаполнено(ВыделитьФрагментВВыражении) Тогда
		ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ЭлементыФормы.ПолеТекстаВыражения, ВыделитьФрагментВВыражении);
	КонецЕсли; 
...
Процедура ДействияФормыПараметрыИзБуфераОбмена(Кнопка)
	...
	Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстРасширенный, "(\+|\-)\s*" + мЛеваяСкобкаПараметра + "(" + шИмя + ")" + мПраваяСкобкаПараметра + "\s*=\s*(.*)\r",,,,,,, РегВычислитель);
	#Если Сервер И Не Сервер Тогда
	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	...
	Иначе
		...
		Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, ПрисвоениеСтроковогоЗначения,,,,,,, РегВычислитель);
		#Если Сервер И Не Сервер Тогда
		    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
		#КонецЕсли
		Если Вхождения.Количество() > 0 Тогда
			...
			Для каждого Вхождение Из Вхождения Цикл
				...
				СтроковоеВыражение1С = "" + ЛитералСтрока1С + "|\s*\+\s*|" + Идентификатор + "";
				// {ОписаниеРегулярногоВыражения.Конец}
				Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ПрисвоенноеЗначение, СтроковоеВыражение1С,,,,,,, РегВычислитель);
				ВыражениеПараметра = "";
				Для каждого Вхождение Из Вхождения Цикл
...
Функция ВычислитьВхождения()
	...
	Если ИменаПодгруппВместоИндексов Тогда
		СтрокаИменаПодгрупп = ирОбщий.СтрСоединитьЛкс(ИменаПодгрупп());
	КонецЕсли; 
	Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, РазвернутоеВыражениеПараметра, СтрокаИменаПодгрупп,, ИгнорироватьРегистр, Многострочный, Ложь,, РегВычислитель);
	#Если Сервер И Не Сервер Тогда
		Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	...
	Для Каждого Вхождение Из Вхождения Цикл
		Если ПараметрыСоСписками.Количество() > 0 Тогда 
			...
			Если Вхождения.Колонки.Найти(СтрокаПараметраСоСписком.Имя) = Неопределено Тогда
				Вхождения.Колонки.Добавить(СтрокаПараметраСоСписком.Имя);
			КонецЕсли; 
			ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.ТекстВхождения, РазвернутоеВыражениеВложенное,,, ИгнорироватьРегистр, Многострочный, Ложь,, РегВычислитель);
			#Если Сервер И Не Сервер Тогда
