...
Процедура ПриОткрытии()
	...
	мИсторияФайлов = ирОбщий.ВосстановитьЗначениеЛкс("ирКонструкторРегулярногоВыражения_мИсторияФайлов");
	...
	#КонецЕсли
	ЭлементыФормы.ДвижокВычислителя.СписокВыбора = RegExp.ДоступныеДвижки();
	ЭтаФорма.ДвижокВычислителя = RegExp.ТекущийДвижок();
...
Функция СтрокаБезФорматирования(Знач Текст)
	...
	RegExp.Pattern = "\n\s*";
	Результат = RegExp.Заменить(Текст, "");
	Возврат Результат;
...
Процедура КПВыражениеСгенерироватьПрограммныйКод(Кнопка)
	...
	|Многострочный = " + ?(Многострочный, "ИСТИНА", "ЛОЖЬ") + ";
	|ИгнорироватьРегистр = " + ?(ИгнорироватьРегистр, "ИСТИНА", "ЛОЖЬ") + ";
	|Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстГдеИскать, " + НачальнаяСтрока.Имя + ", """ + СтрокаИменаПодгрупп + """,, ИгнорироватьРегистр, Многострочный, Ложь);
	|#Если Сервер И Не Сервер Тогда
	|	Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	|#КонецЕсли";
	...
	Если СтрокаПараметраСоСписком <> Неопределено Тогда
		ПрограммныйКод = ПрограммныйКод + 
		"	ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.ТекстВхождения, " + СтрокаПараметраСоСписком.Имя + ", """ + ирОбщий.СтрСоединитьЛкс(ИменаВложенныхПодгрупп) + """,, ИгнорироватьРегистр, Многострочный, Ложь);
		|		#Если Сервер И Не Сервер Тогда
		|		    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
		|		#КонецЕсли
...
Процедура ОткрытьВыражениеВRegexBuddy(НачатьОтладку = Ложь)
	...
	Если мОлеСервер = Неопределено Тогда
		...
		//КонецЕсли; 
		ирОбщий.ТекстВБуферОбменаОСЛкс(ТекстДляБуфераОбмена);
...
Процедура КПВыражениеВставитьВыражение1С(Кнопка)
	...
	Текст = ирОбщий.ТекстИзБуфераОбменаОСЛкс();
...
Функция ОбновитьСтрокуПараметра(Знач СтрокаРодителя, Счетчик = Неопределено, ПодгруппыЛокальные = Неопределено, РазвернутыйСчетчик = 0)
	...
	Пока Счетчик <= СтрДлина(ЗначениеПараметра) Цикл
		...
		ИначеЕсли ТекущийСимвол = мЛеваяСкобкаПараметра Тогда
			ИмяДочернегоПараметра = ирОбщий.СтрокаМеждуМаркерамиЛкс(Сред(ЗначениеПараметра, Счетчик), , мПраваяСкобкаПараметра, Ложь);
...
Процедура ОбновитьПроверочныйТекст(РазрешитьАвтовыделение = Истина)
	...
	РаскрашенныйТекст = ирОбщий.РазметитьТекстРезультатамиПоискаЛкс(Текст, ВхожденияПодгруппы, ИндексПодгруппы, ПереносСлов);
...
Функция ПрижатьВлево(Знач Строка) 
	...
	Результат = Строка;
	Найдено = RegExpLocal.НайтиВхождения(Строка);
...
Функция ПроверочныйТекст()
	...
	Текст = ирОбщий.ТекстИзПоляДокументаHTMLЛкс(ЭлементыФормы.ПроверочныйТекст);
	Возврат Текст;
...
Функция УстановитьПроверочныйТекст(Текст)
	...
	ДокументHtml = ЭлементыФормы.ПроверочныйТекст.Документ;
	ДокументHtml.body.textContent = Текст;
	Возврат Текст;
...
Процедура ОткрытьФайл(РежимОбъединения = Ложь)
	...
	ИмяФайла = ирОбщий.ВыбратьФайлЛкс(, мРасширениеФайла, мОписаниеРасширенияФайла);
	Если ИмяФайла = Неопределено Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ЗагрузитьИзФайла(ИмяФайла = "")
	...
	ЗагружаемыеДанные = ирОбщий.ПрочитатьЗначениеИзФайлаСКонтролемПотерьЛкс(ИмяФайла);
...
Процедура ОбновлениеОтображения()
	...
	//ПостроительТабличногоПоля = ирОбщий.ПостроительТабличногоПоляСОтборомКлиентаЛкс(ЭлементыФормы.Варианты);
	//#Если Сервер И Не Сервер Тогда
...
Процедура КПВыражениеПерейтиКОпределению(Кнопка)
	...
	Результат = ирОбщий.НайтиРегулярноеВыражениеЛкс(НачалоСтроки, мЛеваяСкобкаПараметра + "(" + шИмя + ")?$",, Ложь,,,,, RegExp);
	#Если Сервер И Не Сервер Тогда
	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	...
	Результат = ирОбщий.НайтиРегулярноеВыражениеЛкс(КонецСтроки, "^(" + шИмя + ")?" + мПраваяСкобкаПараметра,, Ложь,,,,, RegExp);
	#Если Сервер И Не Сервер Тогда
	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	Если Результат.Количество() = 0 Тогда
...
Процедура ПерейтиКПараметру(Знач ТекущееИмя, Знач ВыделитьФрагментВВыражении = "")
	...
	Если ЗначениеЗаполнено(ВыделитьФрагментВВыражении) Тогда
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ЭлементыФормы.ПолеТекстаВыражения, ВыделитьФрагментВВыражении);
	КонецЕсли; 
...
Функция ПолучитьВыделениеВПроверочномТексте()
	...
	ВыделениеВТексте = ирОбщий.ПолучитьВыделениеВДокументеHTMLЛкс(ЭлементыФормы.ПроверочныйТекст);
...
Процедура УстановитьВыделениеВПроверочномТексте(Знач ВыделениеВТексте, Знач РодительскийУзел = Неопределено)
	...
	ирОбщий.УстановитьВыделениеВДокументеHTMLЛкс(ЭлементыФормы.ПроверочныйТекст, ВыделениеВТексте, РодительскийУзел);
...
Процедура ВыделитьГруппуВПроверочномТексте(Знач ВыбраннаяГруппа)
	...
	ИндексПодгруппы = ЭлементыФормы.Подгруппы.ТекущаяСтрока.Индекс;
	ирОбщий.ВыделитьГруппуВДокументеHTMLЛкс(ЭлементыФормы.ПроверочныйТекст, ВыбраннаяГруппа, ИндексПодгруппы);
...
Процедура ДействияФормыПараметрыИзБуфераОбмена(Кнопка)
	...
	// {Шаблон.Конец}
	ТекстРасширенный = ирОбщий.СтрокаМеждуМаркерамиЛкс(Текст, "{Шаблон.Начало}", "{Шаблон.Конец}", Ложь);
	Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ТекстРасширенный, "(\+|\-)\s*" + мЛеваяСкобкаПараметра + "(" + шИмя + ")" + мПраваяСкобкаПараметра + "\s*=\s*(.*)\r",,,,,,, RegExp);
	#Если Сервер И Не Сервер Тогда
	    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	...
	Иначе
		...
		Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, ПрисвоениеСтроковогоЗначения,,,,,,, RegExp);
		#Если Сервер И Не Сервер Тогда
		    Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
		#КонецЕсли
		Если Вхождения.Количество() > 0 Тогда
			...
			Для каждого Вхождение Из Вхождения Цикл
				...
				СтроковоеВыражение1С = "" + ЛитералСтрока1С + "|\s*\+\s*|" + Идентификатор + "";
				// {ОписаниеРегулярногоВыражения.Конец}
				Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(ПрисвоенноеЗначение, СтроковоеВыражение1С,,,,,,, RegExp);
				ВыражениеПараметра = "";
				Для каждого Вхождение Из Вхождения Цикл
...
Процедура КП_ПроверочныйТекстОткрытьТаблицуРезультатов(Кнопка)
	...
	Если Вхождения = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ОткрытьЗначениеЛкс(Вхождения,,,, Ложь);
...
Функция ВычислитьВхождения()
	...
	Если ИменаПодгруппВместоИндексов Тогда
		СтрокаИменаПодгрупп = ирОбщий.СтрСоединитьЛкс(ИменаПодгрупп());
	КонецЕсли; 
	Вхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Текст, РазвернутоеВыражениеПараметра, СтрокаИменаПодгрупп,, ИгнорироватьРегистр, Многострочный, Ложь,, RegExp);
	#Если Сервер И Не Сервер Тогда
		Вхождения = Обработки.ирПлатформа.Создать().ВхожденияРегВыражения;
	#КонецЕсли
	...
	Для Каждого Вхождение Из Вхождения Цикл
		Если ПараметрыСоСписками.Количество() > 0 Тогда 
			...
			Если Вхождения.Колонки.Найти(СтрокаПараметраСоСписком.Имя) = Неопределено Тогда
				Вхождения.Колонки.Добавить(СтрокаПараметраСоСписком.Имя);
			КонецЕсли; 
			ВложенныеВхождения = ирОбщий.НайтиРегулярноеВыражениеЛкс(Вхождение.ТекстВхождения, РазвернутоеВыражениеВложенное,,, ИгнорироватьРегистр, Многострочный, Ложь,, RegExp);
			#Если Сервер И Не Сервер Тогда
...
Процедура ДействияФормыПрименить(Кнопка)
	...
	ирОбщий.ПрименитьИзмененияИЗакрытьФормуЛкс(ЭтаФорма, РазвернутоеВыражение);
...
Процедура КПВыражениеСравнить(Кнопка)
	...
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.ПолеТекстаВыражения);
...
Процедура ВхожденияПодгруппыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КП_Вхождения.Кнопки.Идентификаторы);
...
Процедура СохранитьИмяФайла()
	...
	Если ЗначениеЗаполнено(мИмяОткрытогоФайла) Тогда
		...
		ирОбщий.СохранитьЗначениеЛкс("ирКонструкторРегулярногоВыражения_мИсторияФайлов", мИсторияФайлов);
		ОбновитьПодменюИсторииФайлов();
	КонецЕсли; 
...
Процедура БиблиотекаВычислителяПриИзменении(Элемент)
	...
	RegExp.УстановитьДвижок(Элемент.Значение);
	RegExpLocal.УстановитьДвижок(Элемент.Значение);
	ОбновитьПроверочныйТекст();
	
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКонструкторРегулярногоВыражения.Форма.Форма");
мПлатформа = ирКэш.Получить();
шИмя = мПлатформа.шИмя;
RegExp = ирОбщий.НовыйВычислительРегВыражений();
RegExpLocal = ирОбщий.НовыйВычислительРегВыражений();
