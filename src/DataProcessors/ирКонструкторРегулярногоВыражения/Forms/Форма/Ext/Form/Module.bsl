...
Процедура КПВыражениеСгенерироватьПрограммныйКод(Кнопка)
	...
	Для Счетчик = СтрокиПараметров.Найти(НачальнаяСтрока) + 1 По СтрокиПараметров.Количество() Цикл
		...
		Если ЗначениеЗаполнено(СтрокаРодителя.Комментарий) Тогда
			ПрограммныйКод = ПрограммныйКод + Сред(ирОбщий.ДобавитьТекстВТекстЛкс(, СтрокаРодителя.Комментарий, "// ", Истина), 2) + Символы.ПС;
		КонецЕсли; 
...
Процедура ОбновитьВыражениеПараметра(Знач РазрешитьИзменятьСвернутость = Истина)
	...
	КонецЕсли; 
	Если РазрешитьИзменятьСвернутость Тогда
		ИзменитьСвернутостьРазвернутоеВыражение(РазвернутоеВыражение <> ПолеТекстаВыражения);
	КонецЕсли;
...
Процедура ОбновитьПроверочныйТекст(РазрешитьАвтовыделение = Истина, СообщатьОшибки = Ложь)
	...
	КонецЦикла;
	Если РазвернутоеВыражение <> "" Тогда
		Вхождения = ВычислитьВхождения(СообщатьОшибки);
		Если Вхождения = Неопределено Тогда
			Возврат;
		КонецЕсли; 
		Если ИндексТекущейГруппы = -1 Тогда
			Для ИндексКолонки = РегВычислитель.НоваяТаблицаВхождений().Колонки.Количество() По Вхождения.Колонки.Количество() - 1 Цикл
		...
		Для каждого Вхождение Из Вхождения Цикл
			...
			Длина = Вхождение.ДлинаВхождения;
			Конец = Начало + Длина;
			ТекстВхождения = Вхождение.ТекстВхождения;
			...
			НайденнаяГруппа.Значение = ТекстВхождения;
			...
			Для ИндексКолонки = 0 По Вхождение.Группы.Количество() - 1 Цикл
				...
				Если ИндексКолонки = ИндексТекущейГруппы Тогда
					//НайденнаяГруппа.ПозицияГруппы = Начало + Найти(ТекстВхождения, ЗначениеГруппы) - 1;
					//НайденнаяГруппа.ДлинаГруппы = СтрДлина(ЗначениеГруппы);
					НайденнаяГруппа.ЗначениеГруппы = ЗначениеГруппы;
				КонецЕсли;
			КонецЦикла;
			Если ИндексТекущейГруппы = -1 Тогда
				НайденнаяГруппа.ЗначениеГруппы = ТекстВхождения;
			КонецЕсли; 
		КонецЦикла;
	...
	ВыделениеВТексте = мПолеТекстаПоиска.ПолучитьВыделениеВДокументеHTML();
	мПолеТекстаПоиска.РазметитьТекстРезультатамиПоискаИУстановитьВПоле(, ВхожденияГруппы, ИндексГруппы, ПереносСлов);
	...
	Если Автовыделение И РазрешитьАвтовыделение Тогда
		...
		Иначе
			ТекстовыйДиапазон = ТелоДокумента.createTextRange();
...
Процедура ПолеТекстаВыраженияАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
	...
	ОбновитьВыражениеПараметра(Ложь);
...
Процедура ЗагрузитьИзФайла(ИмяФайла = "")
	...
	Иначе
		...
		Если ПересечениеПараметров.Количество() > 0 Тогда
			ирОбщий.СообщитьЛкс("Объединение параетров невозможно, т.к. есть конфликты имен: " + ирОбщий.РасширенноеПредставлениеЛкс(ПересечениеПараметров));
			Возврат;
		КонецЕсли; 
	КонецЕсли; 
	Попытка
...
Процедура ПредыдущееВхождениеНажатие(Элемент = Неопределено, ОбновитьРазметкуТекста = Ложь)
	...
	КонецЕсли; 
	Вхождение = мПолеТекстаПоиска.ПредыдущееВхождениеНажатие(ЭлементыФормы.Группы.ТекущаяСтрока.Индекс);
	Если Вхождение <> Неопределено Тогда
		ЭлементыФормы.ВхожденияГруппы.ТекущаяСтрока = Вхождение;
	КонецЕсли;
КонецПроцедуры
...
Процедура СледующееВхождениеНажатие(Элемент = Неопределено, ОбновитьРазметкуТекста = Ложь)
	Если ОбновитьРазметкуТекста <> Ложь Тогда
		ОбновитьПроверочныйТекст(Ложь);
	КонецЕсли; 
	Вхождение = мПолеТекстаПоиска.СледующееВхождениеНажатие(ЭлементыФормы.Группы.ТекущаяСтрока.Индекс);
	Если Вхождение <> Неопределено Тогда
		ЭлементыФормы.ВхожденияГруппы.ТекущаяСтрока = Вхождение;
	КонецЕсли;
КонецПроцедуры
...
Функция ВычислитьВхождения(СообщатьОшибки = Ложь)
	...
	Если РазвернутоеВыражениеПараметра = "" Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ВхожденияГруппыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		мПолеТекстаПоиска.ВыделитьРезультатПоиска(ВыбраннаяСтрока, ЭлементыФормы.Группы.ТекущаяСтрока.Индекс);
	Иначе
...
Процедура ШаблонЗаменыНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка);
...
Процедура КПВыражениеОткрытьСхему(Кнопка)
	...
	//ПортВременный = 443;
	//ТаймаутЗапроса = 1;
	//СоединениеHTTP = ирОбщий.СоединениеHTTPЛкс(СерверЗапроса, ПортВременный,,, ИнтернетПрокси, ТаймаутЗапроса, Истина);
	//АдресРесурсаСПараметрами = "/?regex=" + РазвернутоеВыражениеДляЗапросаКСайту;
	//ЗапросHTTP = ирОбщий.ЗапросHTTPЛкс(АдресРесурсаСПараметрами);
...
Процедура ИзменитьСвернутостьРазвернутоеВыражение(Знач Видимость, Знач ПередЗакрытием = Ложь)
	
	ирКлиент.ИзменитьСвернутостьЛкс(ЭтаФорма, Видимость, ЭлементыФормы.РазвернутоеВыражение, ЭтаФорма.ЭлементыФормы.Разделитель5, ЭлементыФормы.ПанельВыражение, "низ",, Ложь);
...
ПереносСлов = Истина;
мСтруктураВосстановления = ирКлиент.НоваяСтруктураВосстановленияКонсолиЛкс("irRegexpConstructor");
