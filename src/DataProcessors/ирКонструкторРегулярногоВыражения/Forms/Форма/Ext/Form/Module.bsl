...
Функция ИменаПодгрупп()
	...
	ИменаПодгрупп = ирОбщий.СвернутьМассивЛкс(ИменаПодгрупп); // Редко могут быть дубли, если имена параметров Я1, Я10, Я11
...
Процедура КПВыражениеЭкранироватьТекст(Кнопка)
	...
	КонецЕсли; 
	ТекстВставки = ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(ВыделенныйТекст);
	ЗаменитьИВыделитьВыделенныйТекстВыражения(ТекстВставки);
