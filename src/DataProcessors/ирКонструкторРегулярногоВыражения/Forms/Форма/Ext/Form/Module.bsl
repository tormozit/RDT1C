...
Процедура ПредыдущееВхождениеНажатие(Элемент = Неопределено, ОбновитьРазметкуТекста = Ложь)
	...
	Для Каждого Вхождение Из ВхожденияПодгруппы Цикл
		Если Вхождение.ПозицияПодгруппы = Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		Если Вхождение.ПозицияПодгруппы < ВыделениеВТексте.Начало Тогда
			ВыбраннаяГруппа = Вхождение;
		Иначе
			ИнвертироватьПоиск = Вхождение.ПозицияПодгруппы > ВыделениеВТексте.Начало + 2; // 2 - Крайний перенос строки 
			Прервать;
		КонецЕсли; 
	КонецЦикла;
...
Процедура СледующееВхождениеНажатие(Элемент = Неопределено, ОбновитьРазметкуТекста = Ложь)
	...
	Для Каждого Вхождение Из ВхожденияПодгруппы Цикл
		Если Вхождение.ПозицияПодгруппы = Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		ИнвертироватьПоиск = Вхождение.ПозицияПодгруппы < ВыделениеВТексте.Начало;
		Если Истина
			И Вхождение.ПозицияПодгруппы >= ВыделениеВТексте.Начало
			И Вхождение.ПозицияПодгруппы + Вхождение.ДлинаПодгруппы > ВыделениеВТексте.Конец + 2 // 2 - Крайний перенос строки 
		Тогда
			ВыбраннаяГруппа = Вхождение;
			Прервать;
		КонецЕсли; 
