...
Процедура СохранитьИмяФайла()
	ирОбщий.СохранитьЗначениеЛкс(Метаданные().Имя + "_ИмяФайла", ИмяФайла);
КонецПроцедуры

Процедура СохранитьТекущийЗапрос()
	ПоследнийЗапрос = ПолучитьПутьСтроки(ТекущаяСтрока);
	ирОбщий.СохранитьЗначениеЛкс("КонсольОтчетов_ПоследнийЗапрос", ПоследнийЗапрос);
КонецПроцедуры

Процедура ВосстановитьТекущийЗапрос()
	ПоследнийЗапрос = ирОбщий.ВосстановитьЗначениеЛкс("КонсольОтчетов_ПоследнийЗапрос");
...
Процедура ВосстановитьИмяФайла()
	ИмяФайла = ирОбщий.ВосстановитьЗначениеЛкс(Метаданные().Имя + "_ИмяФайла");
...
Процедура СкопироватьПараметрыСтроки(ИсходнаяСтрока, КонечнаяСрока)
	...
	КонечнаяСрока.ПредставленияДляИмен = ирОбщий.КопияОбъектаЛкс(ИсходнаяСтрока.ПредставленияДляИмен);
	КонечнаяСрока.ФорматыДляИмен = ирОбщий.КопияОбъектаЛкс(ИсходнаяСтрока.ФорматыДляИмен);
	...
	КонечнаяСрока.Макет = ирОбщий.КопияОбъектаЛкс(ИсходнаяСтрока.Макет);
	КонечнаяСрока.НастройкиПостроителя = ирОбщий.КопияОбъектаЛкс(ИсходнаяСтрока.НастройкиПостроителя);
...
Функция ЗагрузитьЗапросыИзФайла(ПриЗапуске, ОчиститьПередЗагрузкой = Истина, ИмяФайлаДляЧтения = "")
	Если Не ЗначениеЗаполнено(ИмяФайлаДляЧтения) Тогда
		ИмяФайлаДляЧтения = ИмяФайла;
	ИначеЕсли ОчиститьПередЗагрузкой Тогда
		ИмяФайла = ИмяФайлаДляЧтения;
	КонецЕсли;
	Попытка
		ПолученноеЗначение = ирКлиент.ПрочитатьЗначениеИзФайлаСКонтролемПотерьЛкс(ИмяФайлаДляЧтения);
...
Функция ПолучитьТекстНастроекПостроителя()
	...
	Для Каждого Параметр Из ПостроительОтчетов.Параметры Цикл
		...
		Тогда
			ПредставлениеПараметра = ирОбщий.ПредставлениеИзИдентификатораЛкс(Параметр.Ключ);
		Иначе
...
Функция ВыполнитьЗапрос(Куда)
	...
	Если СтраницаДляВывода = 0 Тогда
		...
		Если ЛиМинимальнаяШирина Тогда
			ирКлиент.УстановитьАвтоширинуКолонокТабличногоДокументаЛкс(ЭлементыФормы.РезультатТабДокСвод, Истина, Ложь, 1);
		КонецЕсли;
	...
	Если ЛиЗамерВремени Тогда
		ирОбщий.СообщитьСУчетомМодальностиЛкс("Время формирования результата - " + Строка(ирОбщий.ТекущееВремяВМиллисекундахЛкс() - НачалоВыполнения)
				+ " мс", МодальныйРежим);
	КонецЕсли;
	Возврат Истина;
КонецФункции
		...
		Если ОтработыватьРасшифровкуПоУмолчанию И ТекущаяСтрока.ОтчетРасшифровки <> Неопределено Тогда
			...
			Если ВыбранныйОтчет <> Неопределено Тогда
				Если ВыбранныйОтчет = ТекущаяСтрока Тогда
					...
					ирКлиент.УстановитьОтборПостроителяПриРасшифровкеЛкс(ПостроительОтчетов, Расшифровка);
				Иначе
...
Процедура ОткрытьФайлОтчетов(ОчиститьПередЗагрузкой = Истина)
	Если СохранитьЗапросыВФайл(Истина) Тогда
		...
		Длг.Расширение = "rcf";
		Если Длг.Выбрать() Тогда
			ИмяФайлаДляЧтения = Длг.ПолноеИмяФайла;
			ЗагрузитьЗапросыИзФайла(Ложь, ОчиститьПередЗагрузкой, ИмяФайлаДляЧтения);
			Если ОчиститьПередЗагрузкой Тогда
				ОчиститьРезультаты();
				ТекущаяСтрока = НеОпределено;
			КонецЕсли;
		...
		Если ТипЗнч(ТекущаяСтрока.НастройкиПостроителя) = Тип("ТаблицаЗначений") Тогда
			Для Каждого Тн ИЗ ТекущаяСтрока.НастройкиПостроителя Цикл
				...
				(ПоследняяОткрытаяНастройка = Неопределено И Тн.Имя = ТекущаяСтрока.НастройкаДляЗагрузки) Тогда
					...
					Тн.Макет = ирОбщий.КопияОбъектаЛкс(ФиксированныйМакет);
...
Процедура КоманднаяПанельРеультатаКопия(Кнопка)
	...
	ИначеЕсли ЭлементыФормы.ПанельРезультатов.Страницы.Индекс(ЭлементыФормы.ПанельРезультатов.ТекущаяСтраница) = 1 Тогда
		...
		НоваяДиаграмма = Рисунок.Объект; // Диаграмма
		...
		НоваяДиаграмма.ОбластьПостроения.ОтображатьШкалу = ИсходнаяДиаграмма.ОбластьПостроения.ОтображатьШкалу; //@skip-check unknown-method-property // совместимость 8.2
		НоваяДиаграмма.ОбластьПостроения.ЦветШкалы = ИсходнаяДиаграмма.ОбластьПостроения.ЦветШкалы; //@skip-check unknown-method-property // совместимость 8.2
		НоваяДиаграмма.ОбластьПостроения.ЛинииШкалы = ИсходнаяДиаграмма.ОбластьПостроения.ЛинииШкалы; //@skip-check unknown-method-property // совместимость 8.2
		НоваяДиаграмма.ОбластьПостроения.ВертикальныеМетки = ИсходнаяДиаграмма.ОбластьПостроения.ВертикальныеМетки; //@skip-check unknown-method-property // совместимость 8.2
		...
		НоваяДиаграмма.ОтображатьЗаголовок = ИсходнаяДиаграмма.ОтображатьЗаголовок; //@skip-check unknown-method-property // совместимость 8.2
		НоваяДиаграмма.ОтображатьЛегенду = ИсходнаяДиаграмма.ОтображатьЛегенду; //@skip-check unknown-method-property // совместимость 8.2
			...
			Для Каждого Тн ИЗ ТекущаяСтрока.НастройкиПостроителя Цикл
				...
				Если Найдено Тогда
					...
					Если ФормаСохраненияНастроек.НаименованиеНастройки = Тн.Имя Тогда
						...
						ДС.Макет = ирОбщий.КопияОбъектаЛкс(ФиксированныйМакет);
					...
					Иначе
						...
						ДС.НастройкиПостроителя = ирОбщий.КопияОбъектаЛкс(Тн.НастройкиПостроителя);
						ДС.Макет = ирОбщий.КопияОбъектаЛкс(Тн.Макет);
...
Процедура ДеревоЗапросовПередНачаломИзменения(Элемент, Отказ)
	...
	ФормаРедактирования.ПредставленияДляИмен = ирОбщий.КопияОбъектаЛкс(Элемент.ТекущаяСтрока.ПредставленияДляИмен);
	ФормаРедактирования.ФорматыДляИмен = ирОбщий.КопияОбъектаЛкс(Элемент.ТекущаяСтрока.ФорматыДляИмен);
	ФормаРедактирования.Параметры = ирОбщий.КопияОбъектаЛкс(Элемент.ТекущаяСтрока.ПараметрыЗапроса);
...
Процедура ПриОткрытии()
	...
	Если ПостроительОтчетов.Текст <> "" Тогда
		...
		Для Каждого ЭлементПараметра Из ПостроительОтчетов.Параметры Цикл
			...
			СтрокаПараметра.ПредставлениеПараметра = ирОбщий.ПредставлениеИзИдентификатораЛкс(ЭлементПараметра.Ключ);
			СтрокаПараметра.ТипЗначения = Новый ОписаниеТипов(ирОбщий.ЗначенияВМассивЛкс(ТипЗнч(ЭлементПараметра.Значение)));
	...
	ДопСвойства = ирКлиент.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.РезультатТабДокСвод);
	ДопСвойства.КнопкаОтображенияПодвала = ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Автосумма;
КонецПроцедуры
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	...
	ИначеЕсли ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		Если ЗначениеВыбора.Свойство("РедактированиеЗапроса") Тогда
			...
			Элемент.ТекущаяСтрока.ПараметрыЗапроса = ирОбщий.КопияОбъектаЛкс(ФормаРедактирования.Параметры);
			Элемент.ТекущаяСтрока.ПредставленияДляИмен = ирОбщий.КопияОбъектаЛкс(ФормаРедактирования.ПредставленияДляИмен);
			Элемент.ТекущаяСтрока.ФорматыДляИмен = ирОбщий.КопияОбъектаЛкс(ФормаРедактирования.ФорматыДляИмен);
		...
		ИначеЕсли ЗначениеВыбора.Свойство("РедактированиеНовогоЗапроса") Тогда
			...
			НоваяСтрока.ПараметрыЗапроса = ирОбщий.КопияОбъектаЛкс(ФормаРедактирования.Параметры);
			НоваяСтрока.ПредставленияДляИмен = ирОбщий.КопияОбъектаЛкс(ФормаРедактирования.ПредставленияДляИмен);
			НоваяСтрока.ФорматыДляИмен = ирОбщий.КопияОбъектаЛкс(ФормаРедактирования.ФорматыДляИмен);
			...
			Если ЗначениеВыбора.Копирование Тогда
				...
				НоваяСтрока.Макет = ирОбщий.КопияОбъектаЛкс(СтрокаОригинал.Макет);
				НоваяСтрока.НастройкиПостроителя = ирОбщий.КопияОбъектаЛкс(СтрокаОригинал.НастройкиПостроителя);
...
Процедура КоманднаяПанельФормыСравнить(Кнопка)
	ирКлиент.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.РезультатТабДокСвод);
КонецПроцедуры
...
Процедура ДеревоЗапросовНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	СохранитьТекущиеНастройкиВСтроку();
	ирКлиент.ДеревоКонсолиНачалоПеретаскиванияЛкс(Элемент, ПараметрыПеретаскивания, Выполнение, "СтрокаДереваПостроителей");
КонецПроцедуры

Процедура ДеревоЗапросовПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	ирКлиент.ДеревоКонсолиПроверкаПеретаскиванияЛкс(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка, "СтрокаДереваПостроителей");
КонецПроцедуры

Процедура ДеревоЗапросовПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	ирКлиент.ДеревоКонсолиПеретаскиваниеЛкс(ЭтаФорма, Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка, "СтрокаДереваПостроителей", "Запрос");
КонецПроцедуры
...
Процедура ОтборЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	Если Не ЗначениеЗаполнено(ЭлементыФормы.Отбор.ТекущаяСтрока.Имя) Тогда
		Возврат;
	КонецЕсли;
	ирКлиент.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.Отбор, СтандартнаяОбработка,, Истина);
КонецПроцедуры

Процедура РезультатТабДокСводПриАктивизацииОбласти(Элемент)
	ирКлиент.ПолеТабличногоДокументаПриАктивизацииОбластиЛкс(ЭтаФорма, Элемент);
КонецПроцедуры

ирКлиент.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКонсольПостроителейОтчетов.Форма.Форма");
