...
Процедура СобытияПриИзмененииФлажка(Элемент, Колонка)
	...
	ирОбщий.ТабличноеПолеПриИзмененииФлажкаЛкс(ЭтаФорма, Элемент, Колонка, ЭлементыФормы.КоманднаяПанельСобытия.Кнопки.ТолькоПомеченные);
...
Процедура ТабличноеПолеСписокСобытийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура РедактированиеУсловийСобытияСвойствоНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	ДоступныеСвойства = ирОбщий.ПостроительТабличногоПоляСОтборомКлиентаЛкс(ЭлементыФормы.ДоступныеСвойства).Результат.Выгрузить();
	#Если Сервер И Не Сервер Тогда
	...
	Если ЭлементСписка <> Неопределено Тогда
		...
		Если РезультатВыбора <> Неопределено Тогда
			ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(Элемент, Свойства.Найти(РезультатВыбора.Значение, "НИмя").Имя);
		КонецЕсли; 
	КонецЕсли; 
	
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирНастройкаТехножурнала.Форма.НастройкаКаталога");
