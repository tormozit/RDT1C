	...
	Если ТаблицаСсылающихсяОбъектов.Количество() > 0 Тогда
		...
		Попытка
			Для Каждого ОписаниеОбъекта Из СсылающиесяОбъекты Цикл
				...
				#Если Сервер И Не Сервер Тогда
					ЗаменитьСсылкиВОбъектеБД(ПараметрыОбработкиОбъекта);
				#КонецЕсли
			КонецЦикла; 
	...
	Если ирОбщий.ЛиТипОбъектаБДЛкс(ирОбщий.ТипОбъектаБДЛкс(СтруктураОбъектБД)) Тогда
		...
		Если ОбъектИзменен Тогда
			...
			Тогда 
				...
				Если СтруктураОбъектБД.Данные[ирОбщий.ПеревестиСтроку("Проведен")] Тогда
					СтрокаДляДокумента = ИзмененныеПроведенныеДокументы.Добавить();
					СтрокаДляДокумента.ДатаДокумента = СтруктураОбъектБД.Данные[ирОбщий.ПеревестиСтроку("Дата")];
					СтрокаДляДокумента.ТипДокумента = ОбъектМД.Имя;
					СтрокаДляДокумента.Документ = СтруктураОбъектБД.Данные[ирОбщий.ПеревестиСтроку("Ссылка")];
				КонецЕсли;
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли;
...
Функция ОбработатьНеправильныйОбъектПослеЗамены(УдаляемаяСсылка, НепосредственноеУдаление = Ложь, СсылкаДляЗаменыДанных = Неопределено) Экспорт 
	...
	Если Не ОбъектУдалили Тогда
		ИмяИмяПредопределенныхДанных = ирОбщий.ПеревестиСтроку("ИмяПредопределенныхДанных");
		...
		ЛиКорневойТипСПредопределенными = ирОбщий.ЛиКорневойТипОбъектаСПредопределеннымЛкс(КорневойТип) И Не ирОбщий.РежимСовместимостиМеньше8_3_4Лкс();
		НачатьТранзакцию();
		Попытка
			...
			Если СсылкаДляЗаменыДанных <> Неопределено Тогда
				...
				Если СтандартнаяОбработка Тогда
					...
					Если Истина
						И ЛиКорневойТипСПредопределенными
						И (Ложь
							Или Не НепосредственноеУдаление
							Или (Истина
								И НепосредственноеУдаление
								И Не ЗначениеЗаполнено(СтруктураПравильногоОбъекта.Данные[ИмяИмяПредопределенныхДанных])))
					Тогда
						СтруктураПравильногоОбъекта.Данные[ИмяИмяПредопределенныхДанных] = СтруктураПравильногоОбъектаСНеправильнымиДанными.Данные[ИмяИмяПредопределенныхДанных];
					КонецЕсли;
					СтруктураПравильногоОбъекта.Данные[ИмяПометкаУдаления] = Ложь;
			...
			КонецЕсли; 
			Если ЛиКорневойТипСПредопределенными Тогда
				ИмяПредопределенного = СтруктураНеправильногоОбъекта.Данные[ИмяИмяПредопределенныхДанных];
				Если Истина
					И СтруктураНеправильногоОбъекта.Данные[ИмяПредопределенный]
					И ирОбщий.ПолучитьМенеджерЛкс(УдаляемаяСсылка)[ИмяПредопределенного] <> УдаляемаяСсылка
				Тогда
					СтруктураНеправильногоОбъекта.Данные[ИмяИмяПредопределенныхДанных] = "";
				КонецЕсли;
			КонецЕсли;
