...
Функция ОбработатьНеправильныйОбъектПослеЗамены(УдаляемаяСсылка, НепосредственноеУдаление = Ложь, СсылкаДляЗаменыДанных = Неопределено) Экспорт 
	...
	Если Не ОбъектУдалили Тогда
		...
		Попытка
			...
			КонецЕсли; 
			ИмяКлючИндекс = СтруктураНеправильногоОбъекта.Данные[ИмяИмяПредопределенныхДанных];
			Если Истина
				И ирОбщий.ЛиКорневойТипОбъектаСПредопределеннымЛкс(КорневойТип)
				И СтруктураНеправильногоОбъекта.Данные[ИмяПредопределенный]
				И ирОбщий.ПолучитьМенеджерЛкс(УдаляемаяСсылка)[ИмяКлючИндекс] <> УдаляемаяСсылка
			Тогда
