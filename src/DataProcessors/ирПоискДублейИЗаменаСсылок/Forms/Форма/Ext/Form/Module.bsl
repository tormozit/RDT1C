...
Функция ПолучитьЗапросПоискаДублей(ЗапросСОтборомОбщим, выхТекстИдексов = "", выхПоискПоПохожимСловам = Неопределено)
	...
	Для Каждого ПолеСравнения Из мПоляСравнения Цикл
		...
		Если КомпоновщикСравнения.Настройки.ДоступныеПоляПорядка.НайтиПоле(Новый ПолеКомпоновкиДанных(ПервыйФрагмент)).Папка Тогда
			...
			КонецЕсли;
			ПоляТЧ.Добавить(ирОбщий.СтрокаБезПервогоФрагментаЛкс(ПолноеИмяПоля));
