	...
	Для Каждого ПолеСравнения Из мПоляСравнения Цикл
		...
		Если ЗначениеЗаполнено(ИмяДопРеквизита) Тогда
			...
			Для Каждого ХарактеристикиЭлемент Из ОбъектМД.Характеристики Цикл
				ТипХарактеристики = ирОбщий.ПоляТаблицыМДЛкс(ХарактеристикиЭлемент.ЗначенияХарактеристик).Найти(ХарактеристикиЭлемент.ПолеКлюча.Имя, "Имя").ТипЗначения.Типы()[0];
				СвойствоСсылка = ПланыВидовХарактеристик[Метаданные.НайтиПоТипу(ТипХарактеристики).Имя].НайтиПоНаименованию(ИмяДопРеквизита, Истина);
			...
			Если Не ЗначениеЗаполнено(ВыражениеПоля) Тогда
				...
				ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс(НСтр("ru='Поле ""%1"" пропущено, т.к. для него не найдено характеристики с именем %2'"), ПолноеИмяПоля));
				Продолжить;
			КонецЕсли;
