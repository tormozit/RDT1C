...
Процедура ОтборПарЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирКлиент.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.ОтборПар, СтандартнаяОбработка,, Истина,, "ПравоеЗначение");
...
Процедура КП_ДоступныеПоляПерейтиКОпределению(Кнопка)
	...
	Если ДоступноеПоле <> Неопределено Тогда
		...
		Если ПоляТаблицы.Найти(ИмяПоля, "Имя") <> Неопределено Тогда
			ирКлиент.ОткрытьРедакторОбъектаБДЛкс(ПолноеИмяТаблицыБД, ИмяПоля);
		КонецЕсли; 
	КонецЕсли; 
