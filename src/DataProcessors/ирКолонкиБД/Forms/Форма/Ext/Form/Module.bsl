...
Процедура ДействияФормыЗаполнитьТаблицу(Кнопка = Неопределено, РазрешитьАсинхронно = Истина)
	...
	ирОбщий.ВыполнитьЗаданиеФормыЛкс("ЗаполнитьТаблицуКолонок", ПараметрыЗадания, ЭтаФорма,,,
		ЭлементыФормы.ДействияФормы.Кнопки.ЗаполнитьТаблицу, "ЗаполнитьТаблицуКолонокЗавершение", РазрешитьАсинхронно <> Ложь И КлючУникальности = Неопределено И КолонкиБД.Количество() = 0,, Истина);
КонецПроцедуры
...
Процедура КолонкиБДПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Ячейка.ОтображатьКартинку = Истина;
	Ячейка.ИндексКартинки = ирОбщий.ИндексКартинкиТипаЗначенияБДЛкс(ОписаниеПоля.ТипЗначения);
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКолонкиБД.Форма.Форма");
