...
//   ICatalogCollection {COMAdmin.COMAdminCatalog}, ICOMAdminCatalog {COMAdmin.COMAdminCatalog}, ICOMAdminCatalog2 {COMAdmin.COMAdminCatalog}
Функция НовыйCatalogCollection() Экспорт
КонецФункции
...
Функция ВыполнитьРегистрацию() Экспорт
	...
	ПриложенияСистемы = КаталогПриложений.GetCollection("Applications"); // см. НовыйCatalogCollection
		...
		Для Каждого СтрокаПриложения Из Приложения Цикл
			...
			Если СтрокаПриложения.Создать Тогда
				...
				Если ЗначениеЗаполнено(ИмяФайла) Тогда
					...
					Если Не СтрокаПриложения.Добавленный Тогда
						Для Каждого СтрокаКонкурентногоПриложения Из Приложения.НайтиСтроки(Новый Структура("ИзданиеПлатформы, x64", СтрокаПриложения.ИзданиеПлатформы, СтрокаПриложения.x64)) Цикл
							Если Не ЗначениеЗаполнено(СтрокаКонкурентногоПриложения.Идентификатор) Тогда
								Продолжить;
							КонецЕсли;
							Компоненты = ПриложенияСистемы.GetCollection("Components", СтрокаКонкурентногоПриложения.Идентификатор); // см. НовыйCatalogCollection
							Компоненты.Populate();
					...
					Иначе
						СтрокаБазовогоПриложения = Приложения.НайтиСтроки(Новый Структура("Добавленный, ИзданиеПлатформы, x64", Ложь, СтрокаПриложения.ИзданиеПлатформы, СтрокаПриложения.x64))[0];
						Компоненты = ПриложенияСистемы.GetCollection("Components", СтрокаБазовогоПриложения.Идентификатор); // см. НовыйCatalogCollection
						Компоненты.Populate();
					...
					КонецЕсли;
					Роли = ПриложенияСистемы.GetCollection("Roles", ПриложениеОбъект.Key); // см. НовыйCatalogCollection
					Роли.Populate();
					Роль = Неопределено;
					...
					Если Роль = Неопределено Тогда
						...
						Роли.SaveChanges();
					КонецЕсли;
					ПользователиИБ = Роли.GetCollection("UsersInRole", Роль.Key); // см. НовыйCatalogCollection
...
Функция ИмяСлужебногоПользователяОСAuthenticatedUsers() Экспорт
	...
	ПользователиОС = СлужбаWMI.ExecQuery("SELECT Name
			...
			|WHERE SID = 'S-1-5-11'"); //@skip-check unknown-method-property
...
Процедура ОбновитьТаблицуКлассов(ЗаполнятьТолькоВнешниеСоединения = Ложь) Экспорт
	...
	Если ирКэш.Это64битнаяОСЛкс() Тогда
		...
		Компоненты = КаталогПриложений.GetCollection("WOWLegacyServers"); // см. НовыйCatalogCollection
		ЗаполнитьКлассыИзКоллекции(Компоненты, Ложь);
	КонецЕсли;
	Компоненты = КаталогПриложений.GetCollection("InprocServers"); // см. НовыйCatalogCollection
	ЗаполнитьКлассыИзКоллекции(Компоненты, ирКэш.Это64битнаяОСЛкс());
	Компоненты = КаталогПриложений.GetCollection("LegacyServers"); // см. НовыйCatalogCollection
	ЗаполнитьКлассыИзКоллекции(Компоненты, ирКэш.Это64битнаяОСЛкс());
	...
	Для Каждого Приложение Из ПриложенияСистемы Цикл
		...
		Компоненты = ПриложенияСистемы.GetCollection("Components", Приложение.Key); // см. НовыйCatalogCollection
		Попытка
			Компоненты.Populate();
		Исключение
		...
		КонецЕсли;
		Если ирКэш.Это64битнаяОСЛкс() Тогда
			Компоненты = ПриложенияСистемы.GetCollection("LegacyComponents", Приложение.Key); // см. НовыйCatalogCollection
			Компоненты.Populate();
			Для Каждого Компонента Из Компоненты Цикл
				ИмяКласса = Компонента.Value("ProgID");
				Если Истина
...
Функция НайтиЗаполнитьСтрокуПриложения(ИмяКласса, Приложение, x64, ПриложенияСистемы)
	...
	Роли = ПриложенияСистемы.GetCollection("Roles", Приложение.Key); // см. НовыйCatalogCollection
	Роли.Populate();
	Для Каждого Роль Из Роли Цикл
		ПользователиКомпоненты = Роли.GetCollection("UsersInRole", Роль.Key); // см. НовыйCatalogCollection
