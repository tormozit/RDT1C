...
Процедура КоманднаяПанельКлассыПроверитьРаботуОбъектов(Кнопка)
	...
	Для Каждого ИмяКласса Из ИменаКлассов Цикл
		...
		СтрокаПриложения = Приложения.НайтиСтроки(Новый Структура("ИмяКласса, Включено, ОтдельнаяАктивация", ИмяКласса, Истина, Истина));
		Если СтрокаПриложения.Количество() Тогда
			ОтборКлассов.x64 = СтрокаПриложения[0].x64;
		КонецЕсли; 
		...
		Попытка
			ПроверочныйОбъект = Новый COMОбъект(СтрокаКласса.ИмяКласса); // AutomationСервер
