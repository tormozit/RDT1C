...
Процедура ДействияФормыЗагрузить(Кнопка)
	Если мФайлРезультата = Неопределено Тогда
		...
		+"" + КомандаСистемы + " | Out-File -FilePath \""" + мФайлРезультата.ПолноеИмя + "\"" -Encoding " + КодировкаТекста.UTF8 + " -Append -NoNewline;}"; /// Почему то работает без конечной двойной кавычки
...
Процедура ОбновитьПоискВТекстеФайла(Знач ПереносСлов = Ложь)
	...
	Если ЗначениеЗаполнено(ЧтоИскать) Тогда
		...
		ТекстХтмл = мПолеТекстаПоиска.РазметитьТекстРезультатамиПоиска(ТекстИзмененийФайла, РезультатыПоиска, мИндексГруппыПодсветки, Истина);
	Иначе
		ТекстХтмл = ТекстИзмененийФайла;
...
Функция НомерСтрокиВМодуле(Знач ЛиПредыдущаяВерсия = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТекстНомераСтроки) Тогда
		НачалоБлока = Сред(ТекстаПоиска, ПозицияКонцаЗаголовкаБлока, НачальнаяПозицияВыделения - ПозицияКонцаЗаголовкаБлока - 1);
...
Функция СсылкаНаСтрокуАктуальнойВерсии(ИмяМодуля = Неопределено) Экспорт
	...
	Если СтрокаФайла = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	НомерСтрокиМодуля = Мин(1, НомерСтрокиВМодуле());
