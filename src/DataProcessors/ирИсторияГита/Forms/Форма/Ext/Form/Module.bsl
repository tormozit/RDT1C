...
Процедура ЗагрузитьФайлыКоммита() Экспорт
	...
	КонецЕсли;
	ТекстИзмененийКоммита = ирОбщий.ВыполнитьКомандуОСЛкс(НачалоКомандыГита() + "show -m " + ПараметрыВыводаИзмененийДляКоманды() + ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит + ПараметрГитаМаскаФайла(),,,,, КодировкаТекста.UTF8);
