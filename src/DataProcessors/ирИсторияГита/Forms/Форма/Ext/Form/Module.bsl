...
Процедура ЗагрузитьИзмененияФайла() Экспорт
	...
	КонецЕсли;
	КомандаВыводаСКонцамиСтрок = НачалоКомандыГита() + "diff --ws-error-highlight=all " + ПараметрыВыводаИзмененийДляКоманды() + " " + ПараметрыКоммитаДляКоманды()
		+ " -- " + ЭлементыФормы.ФайлыКоммита.ТекущаяСтрока.Файл; // Для диагностики
