...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Реквизит.ПапкаГита, Форма.ЧтоИскать, Форма.МаскаИмениФайла, Форма.ЧислоСтрокКонтекста, Форма.Ветка, Форма.ФильтрПодсистем, Форма.КромеВетки, Форма.ШаблонКомментария, Форма.СортироватьПоСтатусу";
...
Функция ТекстВерсииФайла(Знач ЛиПредыдущаяВерсия = Ложь) Экспорт
	...
	Если ЛиПредыдущаяВерсия Тогда
		...
		ЛогическоеВыражение = Ложь
			Или Родители.Количество() = 1
			Или Родители.Найти("Родитель", "Роль") = Неопределено;
		Команда = Команда + ?(ЛогическоеВыражение, Родители[0].Коммит, Родители.Найти(ЭлементыФормы.ФайлыКоммита.ТекущаяСтрока.НомерРодителя, "Номер").Коммит);
	Иначе
		Если ПустаяСтрока(ЭлементыФормы.Коммиты.ТекущаяСтрока.Коммит) Тогда
