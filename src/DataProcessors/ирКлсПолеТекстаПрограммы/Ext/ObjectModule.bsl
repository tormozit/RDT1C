...
Функция ФорматироватьТекстВстроенногоЯзыка(Знач МожноВесьТекст = Истина) Экспорт 
	...
	ВыделенныйТекст = ВыделенныйТекст();
	НовыйТекст = СтроитьДеревоПрограммныхСкобок(ВыделенныйТекст, НачальнаяСтрока);
...
// Возвращаемое значение:
//    Строка - 
Функция СтроитьДеревоПрограммныхСкобок(Знач ВыделенныйТекст, Знач НачальнаяСтрока = 1, Знач СобратьРодителейПозиции = 0, выхРодителиПозиции = Неопределено) Экспорт
	...
	Если СтроитьКарту Тогда
		...
		Если выхРодителиПозиции = Неопределено Тогда
			...
			выхРодителиПозиции.Колонки.Добавить("ПозицияНачала");
			выхРодителиПозиции.Колонки.Добавить("ПозицияКонца");
		КонецЕсли;
	Иначе 
...
Функция ОпределениеАктивногоМетодаСОписанием(СмещениеНомераМетода = 0) Экспорт 
	Если мМетодМодуля <> Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		Результат = мМодульМетаданных.Программа;
	КонецЕсли; 
	Возврат Результат;
КонецФункции
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0, Знач ДопКлючКэша = "", Знач ДляСвойства = "", СобиратьОшибки = Ложь, ВсеПрисвоения = Неопределено) Экспорт
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			Если мРекурсивныйПуть.ДопКлючКэша <> Неопределено И Не СобиратьВсеПрисвоения Тогда 
				Если мМетодМодуля <> Неопределено Тогда 
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ПроверятьТипЧерезЕсли И Не ЛиУстановкаЗначения Тогда 
								...
								КонецЕсли;
							КонецЕсли; 
							Если ЛиПеременнаяМодуля И Не ЛиКорневойВызов И Не СобиратьВсеПрисвоения Тогда
								// Для ускорения
								Перейти ~Возврат;
							КонецЕсли;
...
Процедура ОткрытьПоискВызововМетода(Знач ИмяМетода = Неопределено, Знач ИскатьНепрямые = Истина, Знач Автозапуск = Истина) Экспорт 
