...
Процедура ПриНажатииКлавишиАвтодополнение(Источник, Событие, Данные, Символ) Экспорт
	...
	Если Истина
		...
		И Найти(Данные, КодыКлавиш["Enter"]) = 1 
		И мКонечнаяСтрока > 1
		И ирОбщий.СтрНачинаетсяСЛкс(СокрЛ(ПолеТекста.ПолучитьСтроку(мКонечнаяСтрока - 1)), "//")
		И Не ирОбщий.СтрНачинаетсяСЛкс(СокрЛ(ПолеТекста.ПолучитьСтроку(мКонечнаяСтрока)), "//")
...
Функция ВычислитьТипПараметраМетодаМодуля(Знач ПараметрыМетода, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Если мМетодМодуля.ВызывающиеМодули <> Неопределено Тогда
			Для Каждого ИмяВызывающегоМодуля Из мМетодМодуля.ВызывающиеМодули Цикл
		...
		Для Каждого ВызывающийМодуль Из ВызывающиеМодули Цикл
			Если ТипЗнч(ВызывающийМодуль) = Тип("Строка") Тогда
				Если Найти(ВызывающийМодуль, ".Форма.") > 0 Тогда   
					СтруктураТипаМодуля = ВычислитьСловоФормыПоПолномуИмени(ВызывающийМодуль);
				Иначе
					СтруктураТипаМодуля = Неопределено;
					Если Найти(ВызывающийМодуль, ".Команда.") > 0 Тогда   
						ТаблицаТиповМодуля = ТаблицаТиповИзТекста("см. " + ВызывающийМодуль);
					ИначеЕсли Найти(ВызывающийМодуль, "Объект.") > 0  Тогда
						ТаблицаТиповМодуля = ТаблицаТиповИзТекста(ВызывающийМодуль);
					Иначе
						ТаблицаТиповМодуля = ВычислитьТипЗначенияВыражения(ВызывающийМодуль,,,,,, Ложь);
					КонецЕсли;
...
Функция ВычислитьСловоФормыПоПолномуИмени(Знач ИмяТипа, Знач ДляСвойства = "")
	...
	Если ЗначениеЗаполнено(ЧастиИмени.ПутьКЭлементу) Тогда
		...
		КонецЕсли;
	Иначе
		ТаблицаТипов = ВычислитьПолучитьФорму(ВычислитьТипЗначенияВыражения("",,,,,, Ложь), ирОбщий.ЗначенияВМассивЛкс(Выражение));
	КонецЕсли;
	Если ТаблицаТипов.Количество() > 0 Тогда
			...
			Если Заголовки.Количество() > 0 Тогда
				...
				Для Каждого Заголовок Из Заголовки Цикл
					...
					Если Найти(ТекстОписанияМетода, ЗаголовокНрег) = 0 Тогда
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, Заголовок, "Конструкция",, "", "Локальный",,, Истина);
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Если мКонечнаяСтрока > 1 Тогда
		...
		Если мЯзыкПрограммы = 0 Тогда
			...
			КонецЕсли;
		Иначе
			Если Лев(ПредшествующаяСтрока, 1) = "|" Тогда
				ПредшествующаяСтрока = СокрЛ(Сред(ПредшествующаяСтрока, 2));
			КонецЕсли;
			Если Ложь
				Или Прав(ПредшествующаяСтрока, 1) = "," 
				Или ирОбщий.СтрНачинаетсяСЛкс(ПредшествующаяСтрока, "И")
			Тогда
	...
	Тогда
		...
		Если ПоднятьНеопределено Тогда 
			...
			ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора,, 1);
		КонецЕсли;
	КонецЕсли;
	Если Истина
		И ПеременнаяПредСтроки <> Неопределено 
	...
	Иначе
		...
		Если ПеречислимыеСвойстваМД.Найти(ИмяТаблицыБД, "Слово") <> Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(мРодительскийКонтекст) И УровеньСвойства = 0 Тогда
				...
				СтрокаСлова.Рейтинг = 2 * КонстантаРейтингСовпаденияТипа;
		...
		Если мСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа" Тогда
			Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
				...
				Тогда     
					мСтрокаЛучшегоСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, ИмяТипа, "Конструкция",, "Тип", "Локальный", Ложь);
		...
		Иначе 
			...
			Тогда
				...
				Иначе
					// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
					Для Каждого СтрокаСлова Из ТаблицаСлов Цикл  						Если Найти(СтрокаСлова.ТипЗначения, мИмяОжидаемогоТипа) > 0 Тогда  							СтрокаСлова.Рейтинг = 2 * КонстантаРейтингСовпаденияТипа;  						КонецЕсли;  					КонецЦикла;  
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "", Знач МножительРейтинга = 2) Экспорт
	...
	Если КлючПоиска.Свойство("ТипЗначения") Тогда
		Если КлючПоиска.Количество() > 1 Тогда
			...
			//! КлючПоискаБезТипа.Вставить("ТипЗначения");
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			Тогда
				...
				ТипыСовместимы = Ложь
					Или (Истина
						И ирОбщий.СтрНачинаетсяСЛкс(Найденные[0].ТипЗначения, "РасширениеФормы")
						И ирОбщий.СтрНачинаетсяСЛкс(КлючПоиска.ТипЗначения, "Форма"))
					И ирОбщий.СтрНайтиЛкс(ирОбщий.ПервыйФрагментЛкс(Найденные[0].ТипЗначения, "(") + ",", КлючПоиска.ТипЗначения + ","); 
			...
			КонецЕсли;
			мСтрокаЛучшегоСлова.Рейтинг = мСтрокаЛучшегоСлова.Рейтинг + КонстантаРейтингСовпаденияТипа * МножительРейтинга;
			СловоНайдено = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
