						...
						Если ТаблицаТиповОбрамляющегоВызова.Количество() > 0 Тогда
							...
							Иначе
								...
								Если ТипЗнч(СтрокаМетода) = Тип("СтрокаТаблицыЗначений") Тогда
									...
									Тогда
										мТаблицаСловЛитерала = ВозможныеТекстовыеЗначенияПараметраИзДругогоМетода(СтрокаМетода,, Ложь);
									Иначе
	...
	Тогда 
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
			...
			Иначе
				...
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияПараметраИзДругогоМетода(СтрокаОписанияМетода, ЛиНаборЗначенийПолный, ИмяТипаСтруктуры, ДобавлятьНизкоВероятные);
			КонецЕсли;
			Если мЭтоТекстовыйЛитерал Тогда
				ВыбраннаяСтруктураТипа.ТипЯзыка = ИмяПараметра;
			КонецЕсли;
...
Функция ПроверитьВыраженияУровня(Знач Вхождения, Знач ПроверятьВсеЛитералы = Ложь, Знач ТабличноеПоле = Неопределено, Знач Форма = Неопределено, Знач Индикатор = Неопределено,
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если ВыражениеЛитерала <> Неопределено Тогда 
			...
			Тогда
				...
				Если Истина
					И НетТочки
					И ирОбщий.ЛиИмяПеременнойЛкс(СловаЛитерала[0]) 
				Тогда
	...
	КонецЦикла;
	Если Этап = 1 Тогда
		ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("Проверка текущего метода (%1) завершена. Найдено %2 возможных ошибок.", ИмяТекущегоМетода(), ОшибкиМодуля.Количество()),,, Истина);
	КонецЕсли;
КонецФункции
