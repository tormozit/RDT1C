...
//
Функция НайтиПоказатьСловоВТексте(СтрокаПоиска, Знач СловоЦеликом = Истина, ПолеТекста = Неопределено, УстановитьФокус = Ложь, ИскатьСНачала = Ложь) Экспорт 
	
	Если ПолеТекста = Неопределено Тогда
		ПолеТекста = ЭтотОбъект.ПолеТекста;
	КонецЕсли;
	Если СловоЦеликом = "Переменная" Тогда
		СловоЦеликом = ШаблонПоискаПеременной();
	КонецЕсли;
	Результат = ирКлиент.НайтиПоказатьФрагментВПолеТекстаЛкс(ФормаВладелец, ПолеТекста, СтрокаПоиска, СловоЦеликом, ИскатьСНачала);
...
Функция ШаблонПоискаПеременной(ЛиМетод = Ложь) Экспорт 
...
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, КакВызовМетода = Ложь, НомерСтроки = 0, НомерКолонки = 0) Экспорт 
	...
	РегВыражение.Pattern = СтрокаШаблона;
	Результат = РегВыражение.НайтиВхождения(НачалоКонтекста);
	Если Результат.Количество() > 0 Тогда
		ПервоеВхождение = Результат[0];
...
Функция НайтиОпределениеСлова(Слово, ПоследнееОпределение = Неопределено, ПоследнееВхождение = Неопределено) Экспорт 
	...
	Иначе
		...
		РегВыражение.Pattern = шПредИмя + ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(Слово) + "(" + шПрисваивание + "|" + ШаблонКоллекции + ")";
		РезультатТекста = РегВыражение.НайтиВхождения(мТекстДляПоискаОпределения);
		Если РезультатТекста.Количество() > 0 Тогда
	...
	Для Каждого РодительскаяСтрутураТипа Из МассивРодительскихСтрутурТипа Цикл
		...
		Если РодительскаяСтрутураТипа.ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
			...
			мПлатформа.СвойстваТипаМетаданные(РодительскаяСтрутураТипа,
				ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, ЯзыкПрограммы, ТекущийИндекс,,, ЛиСерверныйКонтекст, МодульМетаданных, ТаблицаСвойствТипа);
			Если ТаблицаСвойствТипа.Количество() > 0 Тогда
				Для Каждого СтрокаСлова Из ТаблицаСвойствТипа Цикл
...
Функция НайтиВыбранныеТаблицыВТекстеЗапроса(Знач СтрокаЗапросаПакета, Знач НачальнаяПозицияЗапроса) Экспорт 
	#Если Сервер И Не Сервер Тогда
		СтрокаЗапросаПакета = Новый ДеревоЗначений;
	#КонецЕсли
	СтрокаЗапросаПакета.СложныйАнализВыполнен = Истина;
	//Шаблон = шИЗ;
	Шаблон = "((?:ИЗ|FROM)" + шРазделитель + "+" + шОписаниеТаблицы + "[\s\S]*?)(?:$|\sОБЪЕДИНИТЬ(?:\s+ВСЕ)?\s+|\sUNION(?:\s+ALL)?\s+)";
	ВхожденияИЗ = ирОбщий.НайтиРегулярноеВыражениеЛкс(СтрокаЗапросаПакета.Текст, Шаблон);
	...
	Для Каждого ВхождениеИЗ Из ВхожденияИЗ Цикл
		Если ВхожденияИЗ.Количество() > 1 Тогда 
			...
			СлужебноеПолеТекста.УстановитьГраницыВыделения(НачальнаяПозицияЧасти, КонечнаяПозицияЧасти);
			ТекстИсточника = СлужебноеПолеТекста.ВыделенныйТекст;
			...
			ЗаполнитьСтрокуУпрощеннойСтруктурыЗапроса(СтрокаЧастиОбъединения);
		Иначе
			СтрокаЧастиОбъединения = СтрокаЗапросаПакета;
		КонецЕсли;
		Найденные = ирОбщий.НайтиРегулярноеВыражениеЛкс(ВхождениеИЗ.ТекстВхождения, шОписаниеТаблицыСЗахватом);
		Для Каждого Найденное Из Найденные Цикл
			...
			КонецЕсли; 
			СлужебноеПолеТекста.УстановитьГраницыВыделения(НачальнаяПозиция, КонечнаяПозиция);
			ТекстИсточника = СлужебноеПолеТекста.ВыделенныйТекст;
			...
			ЗаполнитьСтрокуУпрощеннойСтруктурыЗапроса(СтрокаИсточника);
		КонецЦикла;
	КонецЦикла;
КонецФункции

Процедура ЗаполнитьСтрокуУпрощеннойСтруктурыЗапроса(Знач СтрокаДерева)
	...
	лКонечнаяКолонка = 0;
	СлужебноеПолеТекста.ПолучитьГраницыВыделения(лНачальнаяСтрока, лНачальнаяКолонка, лКонечнаяСтрока, лКонечнаяКолонка);
	СтрокаДерева.Текст = СлужебноеПолеТекста.ВыделенныйТекст;
...
Процедура ПоказатьПозициюМодуля(Знач ТекущаяСтрока, Знач ПолноеИмяФайлаМодуля, Знач ПолеТекстаФрагмента = Неопределено, Знач РасширениеФайлаМодуля = "txt", Знач ТекстФайла = "", Знач ЧислоПредСтрок = 1) Экспорт 
	...
	//Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСтрокиМодуля) Тогда
		...
		//ТекущаяСтрока.ВызывающийМетод = мПлатформа.НайтиИмяМетодаСтрокиФайлаМодуля(ТекущаяСтрока.КраткоеИмяФайла, ТекущаяСтрока.НомерСтрокиМодуля);
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	ТаблицаСловТЗ.Индексы.Добавить("НСлово");
	РегВыражение.Pattern = "(?:" + шСтрокаПрограммы + "|" + шКомментарий + "|(?:Новый|New)\s+" + шИмя + "|" + Шаблон + ")";
	Результат = РегВыражение.НайтиВхождения(" " + ТекстБезКомментариевИОпасныхСтрок);
	Для Каждого Match Из Результат Цикл
