...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
			...
			Тогда 
				...
				Если ВыбраннаяСтруктураТипа.ТипЯзыка = "" Тогда
					Для Каждого ОписаниеТаблицы Из ДоступныеТаблицы.НайтиСтроки(Новый Структура("Тип, Схема", "ДоступнаяВременнаяТаблица", "")) Цикл
						...
						СтрокаСлова.ТипЗначенияИндекс = ирОбщий.ПервыйФрагментЛкс(СтрокаСлова.ТипЗначения, "(");
					КонецЦикла;
					СтрокаСлова = ТаблицаСлов.Добавить();
				...
				Тогда
					Для Каждого СтрокаСлова Из ТаблицаЛокальногоКонтекста("Свойство") Цикл
						...
						//Если ЗаполнятьТипы Тогда // Если откладывать вычисление типа, то потом через метод УточнитьТипЗначенияВСтрокеТаблицыСлов он не вычислится
							...
							НоваяСтрока.ТипЗначенияИндекс = ирОбщий.ПервыйФрагментЛкс(НоваяСтрока.ТипЗначения, "(");
						//КонецЕсли; 
					КонецЦикла;
				КонецЕсли;
	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		...
		Если мЯзыкПрограммы = 1 И мЭтоЛокальныйКонтекстТаблицыСлов И Не мЭтоОбъявлениеПсевдонима Тогда
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЕСТЬ NULL", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЛЕВОЕ СОЕДИНЕНИЕ", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ПРАВОЕ СОЕДИНЕНИЕ", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ВНУТРЕННЕЕ СОЕДИНЕНИЕ", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ВНЕШНЕЕ СОЕДИНЕНИЕ", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "СГРУППИРОВАТЬ ПО", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "УПОРЯДОЧИТЬ ПО", "Конструкция",,,,,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ОБЪЕДИНИТЬ ВСЕ", "Конструкция",,,,,, Истина);
		КонецЕсли;
		Если Истина
			И мЯзыкПрограммы = 0 
			...
			Если Заголовки.Количество() > 0 Тогда
				...
				Для Каждого Заголовок Из Заголовки Цикл
					...
					Если Найти(ТекстОписанияМетода, ЗаголовокНрег) = 0 Тогда
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, Заголовок, "Конструкция",,,,,, Истина);
		...
		Тогда
			...
			Если Не ОтделятьБольшиеНаборыСлов Тогда
				мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Для Каждого ", "Конструкция",,,,,, Истина);
			КонецЕсли;
		КонецЕсли;
		...
		Тогда 
			Если ТипКонтекстаСтатистики <> "Локальный" Тогда
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Если мКонечнаяСтрока > 1 Тогда
		...
		Тогда
			СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, СловоГенераторКодаЗаполненияСвойств(), "Конструкция",,,, Ложь);
		...
		Тогда
			СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, СловоГенераторКодаСопоставитьСвойства(), "Конструкция",,,, Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Если Не РазрешитьМногоТипов Тогда
		...
		Если ИменаТипов.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(ТаблицаОжидаемыхТипов) Тогда
				...
				Если ЛучшийТип <> Неопределено Тогда
					ЛиСистемноеСлово = ЛиСистемноеСлово(ЛучшийТип, Истина);
	...
	Если Не ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
		...
		Тогда
			УстановитьРейтингСлову(ПеременнаяПредСтроки, 2*ШагРейтингаСлова); // Опасно, т.е. атрибут "Определение" будет некорректным если слова нет в списке
	...
	Иначе
		...
		Иначе 
			...
			Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
				//Если ЛиМногоСлов И ЛиПримитивныйТип(ИмяТипа) Тогда
				//	Продолжить;
				//КонецЕсли;
				ЗапретитьГлобальныйКонтекст = Ложь
				...
				ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора, ирОбщий.ПоследнийФрагментЛкс(ВыражениеПриемник),, ПоследнееСловоПриемника, ЗапретитьГлобальныйКонтекст);
			...
			Тогда
				...
				Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
					...
					Тогда
						Продолжить;
					КонецЕсли;
					СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Новый " + ИмяТипа, "Конструкция",,,, Ложь);
					Если Ложь
			...
			Если Вхождения.Количество() > 0 Тогда
				...
				Если ТаблицаТиповКоллекции.Количество() > 0 Тогда
					...
					Если ирОбщий.СтрКончаетсяНаЛкс(СтруктураТипаКоллекции.ИмяОбщегоТипа, "Соответствие") Тогда
						ТаблицаТиповЭлементов = мПлатформа.ТипыЭлементовКоллекции(СтруктураТипаКоллекции);
						Если ЗначениеЗаполнено(ТаблицаТиповЭлементов) Тогда
...
// Возвращаемое значение:
//   , Строка  - 
Функция ИмяПараметраВызоваМетода(СтрокаМетода, Знач СТипомСтрока = Истина, выхТаблицаТипов = Неопределено, Знач СтруктураТипаМетода = Неопределено, Знач ВариантСинтаксиса = Неопределено, 
		...
		Если ПараметрыМетодаМодуля <> Неопределено И ПараметрыМетодаМодуля.Количество() >= мНомерПараметра Тогда
			...
			Если Не СТипомСтрока Тогда 
				Если РассчитатьТипПараметра И СтрокаПараметра.ТаблицаТипов = Неопределено Тогда
					ЗагрузитьМетодМодуля(СтрокаМетода);
					ИнициироватьРекурсивныйПуть();
					выхТаблицаТипов = ВычислитьТипПараметраМетодаМодуля(ПараметрыМетодаМодуля, СтрокаПараметра);
				Иначе
...
Функция ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, ТаблицаТиповКонтекста = Неопределено, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
	#КонецЕсли
	мЗамещающийФрагмент = ВставитьВыбранноеСловоГенератор(СтрокаРезультата);
	ВызватьАвтодополнениеСнова = Ложь;
	Если ПустаяСтрока(мЗамещающийФрагмент) Тогда
		СтрокаНачала = "";           
...
Функция ВставитьВыбранноеСловоГенератор(Знач СтрокаРезультата, Знач ДляАдаптера = Ложь) Экспорт
	Результат = "";
	РодительИСлово = РодительИСловоСтрокиТаблицыСлов(СтрокаРезультата);
	Слово = РодительИСлово.Слово; 
	СтруктураТипаКонтекста = РодительИСлово.СтруктураТипаКонтекста;
	Если Слово = СловоГенераторКодаЗаполненияСвойств() Тогда
		СловаРодителя = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаКонтекста,,,, Ложь,, "Свойство");
		МассивВставки = Новый Массив;
		НепечатноеНачалоСтроки = ирОбщий.НепечатноеНачалоСтрокиЛкс(мТекущаяСтрокаНачало);
		СловаРодителя.Сортировать("Слово");
		НачалоВставляемыхСтрок = НепечатноеНачалоСтроки + мРодительскийКонтекст + ".";
		Для Каждого СтрокаТаблицы Из СловаРодителя Цикл
			КонецСтроки = ";";
			Если МассивВставки.Количество() = 0 Тогда
				Строка = ""; 
				Если ДляАдаптера Тогда
					КонецСтроки = "<!>" + КонецСтроки;
				КонецЕсли;
			Иначе
	...
	ИначеЕсли Слово = СловоГенераторКодаСопоставитьСвойства() Тогда
		...
		ИмяЛевогоОбъекта1 = ирОбщий.ПервыйФрагментЛкс(ОписаниеОжидаемогоТипа.ВыражениеПриемник);
		ТаблицаТиповЛевая = ВычислитьТипЗначенияВыражения(ИмяЛевогоОбъекта1);
		СловаЛевые = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповЛевая[0], мЯзыкПрограммы,,,,, "Свойство");
		...
		НепечатноеНачалоСтроки = ирОбщий.НепечатноеНачалоСтрокиЛкс(мТекущаяСтрокаНачало);
		Если мЭтоТекстовыйЛитерал Тогда
			НепечатноеНачалоСтроки = НепечатноеНачалоСтроки + "|";
		КонецЕсли;
		ОсновнойКонецСтроки = "";
		ПрефиксВставки = "";
		Если Ложь
		...
		Тогда
			...
			НепечатноеНачалоСтроки = НепечатноеНачалоСтроки + Символы.Таб + "И ";
		...
		Для Каждого СтрокаСловаЛевого Из СловаЛевые Цикл
			...
			Если МассивВставки.Количество() = 0 Тогда
				...
				КонецЕсли;
			Иначе
				Строка = НепечатноеНачалоСтроки + ИмяЛевогоОбъекта1 + "." + СтрокаСловаЛевого.Слово + " = " + мРодительскийКонтекст + ".";
			КонецЕсли;
			ПодобранноеСлово = "";
			Если мСтрокаЛучшегоСлова <> Неопределено Тогда
...
// Вызывается из адаптера. Отладчик Visual Studio Code внутри не останавливается.
Функция Адаптер_ПриВыбореСтрокиАвтодополнения(Знач Слово, Знач ЛиМетод, Знач КлючНабораСлов, Знач Шаблон) Экспорт 
	СтрокаТаблицыСлов = СтрокаСловаИзСловаИНабора(Слово, ЛиМетод, КлючНабораСлов);
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		Шаблон = ВставитьВыбранноеСловоГенератор(СтрокаТаблицыСлов, Истина);
	КонецЕсли;
	Возврат Шаблон;
КонецФункции
	...
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		...
		Если ЛиОтдельноеОписаниеАктивно Тогда 
			...
			Если ЗначениеЗаполнено(ДетальноеОписаниеДляСписка) Тогда
				...
				Если Истина
					И мКэшОжидаемогоТипа <> Неопределено 
					И ЗначениеЗаполнено(мКэшОжидаемогоТипа.ТаблицаОжидаемыхТипов)
				Тогда
					СтруктураОписания.Добавить("> Ожидаемый тип: " + мПлатформа.ПредставлениеМассиваСтруктурТипов(мКэшОжидаемогоТипа.ТаблицаОжидаемыхТипов,,, Ложь));
				КонецЕсли;
				МетаСвойства = "";
				Если ЛучшийТип <> Неопределено Тогда
					МетаданныеТипа = ЛучшийТип.Метаданные;
