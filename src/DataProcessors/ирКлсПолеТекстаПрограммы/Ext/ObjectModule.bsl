...
Функция СоздатьОписаниеМетода(ИмяПараметра) Экспорт 
	...
	КомменатрийМетода = Сред(мОригинальныйТекст, мЗаменяемыйДиапазон.Начало, мЗаменяемыйДиапазон.Конец - мЗаменяемыйДиапазон.Начало);
	// Исправляем описания параметров
	КомменатрийМетода = ирОбщий.ЗаменитьРегВыражениеЛкс(КомменатрийМетода, "-[ \t]*-(\s*[" + шБуква + "])", "-$1", Ложь);
	// Исправляем описания возвращаемого значения
	КомменатрийМетода = СтрЗаменить(КомменатрийМетода, " - .", ".");
	мЗамещающийФрагмент = КомменатрийМетода;
...
Функция ПрямоеИмяМодуляИзПолного(Знач ИмяМодуля) Экспорт
	Фрагменты = ирОбщий.СтрРазделитьЛкс(ИмяМодуля, ".");
	Если Фрагменты[0] = "ОбщийМодуль" Тогда
		Фрагменты.Удалить(0);
	КонецЕсли;
	Если Истина
	...
	Тогда
		...
		Фрагменты.Удалить(Фрагменты.ВГраница());
	КонецЕсли;
	Если Фрагменты[Фрагменты.ВГраница()] = "МодульОбъекта" Тогда
		Фрагменты.Удалить(Фрагменты.ВГраница());
		Фрагменты[0] = Фрагменты[0] + "Объект";
	КонецЕсли;
	Если Фрагменты[Фрагменты.ВГраница()] = "МодульМенеджера" Тогда
		Фрагменты.Удалить(Фрагменты.ВГраница());
		Фрагменты[0] = ирОбщий.МножественноеИмяМДЛкс(Фрагменты[0]);
	КонецЕсли;
	Если Ложь
		Или Фрагменты[Фрагменты.ВГраница()] = "Модуль" 
		Или Фрагменты[Фрагменты.ВГраница()] = "МодульКоманды" 
	Тогда
		Фрагменты.Удалить(Фрагменты.ВГраница());
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
			...
			Если ТаблицаСвойствТипа.Количество() > 0 Тогда
				...
				Для Каждого СтрокаСлова Из ТаблицаСвойствТипа Цикл
					...
					КонецЕсли; 
				КонецЦикла;
				// Перенес в см. ОбработкаОбъект.ирПлатформа.СловаКонтекстаПредопределенные
				//Пока Истина
		...
		Если мМетодМодуля.ВызывающиеМодули <> Неопределено Тогда
			Для Каждого ЛюбоеИмяМодуля Из мМетодМодуля.ВызывающиеМодули Цикл
				...
				Если ТаблицаТиповМодуля.Количество() > 0 Тогда
					...
					ВызывающиеМодули.Добавить(мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипаМодуля));
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		...
		Тогда
			...
			СтрокаСлова = ТаблицаСлов.Найти(НРег("Строка"), "НСлово");
			Если СтрокаСлова = Неопределено Тогда
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Если мКонечнаяСтрока > 1 И мЯзыкПрограммы = 0 Тогда
		ПредшествующаяСтрока = СлужебноеПолеТекстаДолгое.ПолучитьСтроку(мКонечнаяСтрока - 1);
		Если Прав(СокрП(ПредшествующаяСтрока), 1) = ";" Тогда
			// При вводе новой инструкции даем приоритет переменной, с которой начинается предшествующая инструкция
			мРегВыражение.Pattern = мПлатформа.шПустоеНачалоСтроки + "(" + шИмя + ")\s*[=.]";
	...
	КонецЕсли;
	Если ПеременнаяПредСтроки <> Неопределено Тогда
