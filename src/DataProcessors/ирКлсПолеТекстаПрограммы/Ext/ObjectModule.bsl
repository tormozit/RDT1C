...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
		...
		Тогда
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				Если мЯзыкПрограммы <> 1 Тогда
					...
					Если ТипСлова = "Метод" Тогда
						ТаблицаТиповСловаМодуля = мПлатформа.ПодготовитьТипРезультатаМетода(СтрокаСлова, мМодульМетаданных, ПолеТекстаПрограммыВызова,, ДляСвойства); 
					Иначе
						выхЛиПеременнаяМодуля = Истина; 
	...
	Тогда
		ТаблицаТипов.Колонки.Метаданные.Заголовок = ФлагОбрезкиГлубины();
	КонецЕсли;
	Если Истина
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда
				...
				// Мультиметка2323399173
				КлючВыраженияСПозицией = "0;" + БазовыйКлючВыражения; // Полный расчет
				КлючВыраженияСПозициейИСвойством = "1;" + БазовыйКлючВыражения + ";" + ирОбщий.ПервыеФрагментыЛкс(ДляСвойства,, 4); // Обрезанный по ширине или глубине расчет
				...
				Если ТаблицаТиповКэш <> Неопределено Тогда
					...
					//Иначе 
						ТаблицаТипов = ТаблицаТиповКэш; // Попадание в кэш выражени
						Перейти ~Возврат;
					//КонецЕсли;
				КонецЕсли;
			...
			Тогда
				...
				Иначе
					...
					Тогда
						...
						КонецЕсли;
					ИначеЕсли ТипСлова = "Метод" Тогда
						ДляСвойстваВверх = "";
					КонецЕсли;
						...
						Пока ИндексВхождения > 0 Цикл 
									...
									Если ЗначениеЗаполнено(ИмяТипа) Тогда
										...
										ИначеЕсли ТипНового <> Неопределено Тогда
											...
											Тогда                                                
												ПервыйФактПараметр = ПоследнееВхождение.SubMatches(6);
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПервыйФактПараметр) Тогда
													ТаблицаТиповПараметра = ВычислитьТипЗначенияВыражения(ПервыйФактПараметр,,,,,, Ложь,,, Истина, Позиция0ВМетодеОт, ПоследнееВхождение.FirstIndex);
													ИменаСвойств = "";
							...
							Иначе
								...
								Для Каждого СтруктураТипаКоллекции Из ТаблицаТиповКоллекции Цикл
									...
									КонецЕсли;
								КонецЦикла;
								Если ЗначениеЗаполнено(ТаблицаТиповКоллекции.Колонки.Метаданные.Заголовок) Тогда
...
Функция ЛиТаблицаТиповКэшНеГотова(ТаблицаТиповКэш, ДляСвойства) Экспорт
	...
	Если Истина
		И Не Результат 
		И ЗначениеЗаполнено(ДляСвойства)
		И ДляСвойства <> ИмяФиктивногоСвойства() // Мультиметка2323399173
...
Функция СобратьСвойстваСтруктуры(Знач Выражение, Знач Структура = Неопределено, Знач ГлобальныйПоискВМодуле = Ложь, Знач ТолькоПрисвоение = Ложь, Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0,
	Знач ДляСвойства = "", Знач ДляСвойстваВниз = "", выхОбрезано = Ложь)
		...
		Если Вхождения.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из СвойстваЧерезТочкуИзТекста Цикл
				...
				Иначе
					...
					КонецЕсли;
				КонецЕсли;
				Если ТаблицаТиповЗначения.Колонки.Метаданные.Заголовок = ФлагОбрезкиГлубины() Тогда
					выхОбрезано = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	Если ПредшествующийСимвол = "," Или ПредшествующийСимвол = "(" Тогда
		...
		Если ЗначениеЗаполнено(мВызовМетода) Тогда
			ИмяОтключенияЛишнихВычислений = ИмяОтключенияЛишнихВычислений();
			ТаблицаТиповВызова = ВычислитьТипЗначенияВыражения(мВызовМетода + ИмяОтключенияЛишнихВычислений + ")",,,, мЭтоКонструктор); // ИмяФиктивногоСвойства() нельзя передавать, т.к. не найдутся параметры запроса
		...
		Для Каждого Вхождение Из ВхожденияОбъектов Цикл
			...
			Если ТаблицаТипов.Количество() > 0 Тогда
				...
				Тогда
					Продолжить;
				КонецЕсли;
				ТаблицаСловОбъекта = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипа,,,,,, "Свойство",,,,,,,, БазовоеРасширениеКонфигурации); 
