...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	// БСП
	ДобавитьПравилоВычисленияФункции("ОбщийМодуль", "ВычислитьОбщийМодульБСП", "ОбщийМодуль.ОбщегоНазначения"); // Добавил чтобы без ограничений на количество динамических вызовов считалось
...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	Знач ПолныйАнализСоставаТипов = Истина, выхЛиПеременнаяМодуля = Ложь, Знач ПозицияВМетоде = 0) Экспорт 
					...
					Если ПараметрыМетода <> Неопределено И ТипСлова = "Свойство" Тогда
						...
						Если СтрокаПараметраМетода <> Неопределено Тогда
							...
							Если СтрокаПараметраМетода.ТаблицаТипов = Неопределено Тогда
								ВычислитьТипПараметраМетода(ПараметрыМетода, ПройденныеВызовы, СтрокаПараметраМетода, ТаблицаТипов);
							КонецЕсли; 
...
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ТаблицаТипов, ТипВыхода = Неопределено)
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0) Экспорт
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда 
			...
			Тогда 
				...
				КонецЕсли;
				КлючВыраженияСПозицией = XMLСтрока(Позиция0ВМетодеОт) + ";" + XMLСтрока(Позиция0ВМетодеДо) + ";" + Лев(Выражение, 200);
			...
			Тогда
				...
				Иначе 
					ТаблицаСтруктурРодителей = ВычислитьТипЗначенияВыражения(РодительскийКонтекст, ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь,,,,, Позиция0ВМетодеДо);
				...
				МассивДочернихСтруктурТипа = ВычислитьТипДочернегоЭлемента(ТаблицаСтруктурРодителей, ТекущееСлово, ТипСлова, ТекущийИндекс, ТекущиеАргументы, ПредшествующийТекст, ПолныйАнализСоставаТипов,, Позиция0ВМетодеДо);
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущееСлово) И ТипСлова = "Свойство" Тогда  
				...
				Иначе
					...
					Иначе 
						...
						КонецЕсли;
						ШаблонПоиска = ШаблонНазначенияТипаВыражению(ШаблонПоиска);
			...
			Тогда     
				Если Выражение = мПлатформа.ИмяПеременнойВозвращаемогоЗначения() Тогда 
					ЭтоВиртПрисвоение = Истина;
				КонецЕсли;
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						СобратьТипыСвойствСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ТекстДляПоискаОпределения, ПредшествующийТекст, ГлобальныйПоискВМодуле, ЭтоВиртПрисвоение, Позиция0ВМетодеДо);
					...
					Тогда
						...
						КонецЕсли;
						ШаблонПоиска = шПредИмя + Выражение + "\s*\.\s*Колонки\s*.\s*Добавить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
						Вхождения = НайтиВхожденияРегВыраженияКэш(ШаблонПоиска, ГлобальныйПоискВМодуле);
						Для Каждого ВхождениеСвойства Из Вхождения Цикл
							Если ЛиВхождениеВЗапрешенномДиапазоне(ВхождениеСвойства, Позиция0ВМетодеОт, Позиция0ВМетодеДо) Тогда
								Прервать;
							...
							Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда
								Попытка
									СтруктураТипаЦикл.Метаданные.Колонки.Добавить(ТекстовоеЗначение);
								Исключение
									Попытка
										СтруктураТипаЦикл.Метаданные.Вставить(ТекстовоеЗначение);
									Исключение
										Пустышка = 0;
									КонецПопытки;
								КонецПопытки; 
...
Функция ЛиВхождениеВЗапрешенномДиапазоне(Знач ПоследнееВхождение, Знач ПозицияВМетодеОт = 0, Знач ПозицияВМетодеДо = 0)
	Результат = Ложь
		...
		Или ПозицияВМетодеДо > 0 И ПоследнееВхождение.FirstIndex + ПоследнееВхождение.Length > ПозицияВМетодеДо;
	Возврат Результат;
КонецФункции
...
// БСП
Функция ВычислитьОбщийМодульБСП(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);

	//ИмяМодуля = ВычислитьЗначениеВыражения(ПервыйПараметр, ПредшествующийТекст,, ПозицияВМетоде); // Долго и редко полезно
...
Функция ТаблицаСловВJSON(КлючНабораСлов = "") Экспорт 
	...
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл  			Строка.ЛиРез = Строка.ТипЗначения = "";  			Строка.ЛиМетод = Строка.ТипСлова = "Метод";  			Строка.ЛиТочный = Строка.Определение <> "Статистический";  		КонецЦикла;  
	КонецЕсли;
