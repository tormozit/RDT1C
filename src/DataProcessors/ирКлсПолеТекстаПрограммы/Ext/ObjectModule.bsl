...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	...
	Для Каждого РодительскаяСтруктураТипа Из РодительскиеСтрутурыТипа Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если РодительскиеСтрутурыТипа.Колонки.Метаданные.Заголовок <> "" Тогда
	...
	Если РазрешитьАнализИмениТипа Тогда
		...
		Иначе
			...
			Если мЭтоТекстовыйЛитерал Тогда
				...
				ТаблицаТипов = ВычислитьТипЗначенияВыражения(мВызовМетода + ИмяОтключенияЛишнихВычислений + ")", ТекстДляПоискаОпределения, ПредшествующийТекст,, мЭтоКонструктор, ПолныйАнализСоставаТипов, Ложь,,,,, Позиция0ВМетодеДо);
				Возврат ТаблицаТипов;
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда
				...
				КлючВыраженияСПозициейИСвойством = "1;" + БазовыйКлючВыражения + ";" + ирОбщий.ПервыеФрагментыЛкс(ДляСвойства,, 4); // Случай обрезаемых метаданных по глубине или ширине 
			...
			Тогда
				...
				Иначе
					...
					ИначеЕсли Истина
						И ТипСлова = "Метод" 
						И ДляСвойстваВверх <> ИмяФиктивногоСвойства()
					Тогда
						ДляСвойстваВверх = "";
					КонецЕсли;
					Если ЗначениеЗаполнено(ТекущийИндекс) Тогда
						ДляСвойстваВверх = ДляСвойстваВверх + "."; // TODO передавать конкретное имя когда оно есть
					КонецЕсли;
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ПрисвоенныйТип <> Неопределено Тогда 
								...
								КонецЕсли;
							КонецЕсли;
							ЭтоПрисвоение = Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 2));
							...
							Если ЭтоПрисвоение Тогда
								...
								Если СловоНовый = Нрег("Новый") Или СловоНовый = Нрег("New") Тогда
									Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 5)) Тогда 
										ИмяТипа = ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 5);
									ИначеЕсли Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 6)) Тогда 
									...
									Если ЗначениеЗаполнено(ИмяТипа) Тогда
										...
										Если Истина
											...
											И Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 6)) 
										Тогда
											ПолноеИмяОсновногоКлассаCOM = ВычислитьЗначениеВыражения(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 6),,, Позиция0ВМетодеОт, ПоследнееВхождение.FirstIndex);
									...
									КонецЕсли; 
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипа);
								Иначе //Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 8)) Тогда ;
							...
							Иначе
								...
								ТаблицаТиповКоллекции = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст,,,,, ПолныйАнализСоставаТипов, Ложь,,,,, ПозицияВхождения0,, ДляСвойства);
			...
			Иначе
				...
				Если Найти(УказательТипа, "Объект.") > 0 И СтрЧислоВхождений(УказательТипа, ".") > 1 Тогда
					...
					Если ОписаниеТиповОбъекта <> Неопределено Тогда
						...
						КонецЕсли;
						НоваяТаблицаТипов = ВычислитьТипДочернегоЭлемента(ТаблицаТиповРодителя, Слово, "Метод",,,,,,,, ДляСвойстваПуть);
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, _2 = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Тогда
		...
		Если Результат.Количество() > 0 Тогда
			...
			Фрагменты = ирОбщий.СтрРазделитьЛкс(ПервоеВхождение);
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	Тогда
		...
		КонецЕсли;
	Иначе
		Если мИмяОжидаемогоТипа = "Булево" Тогда
			СтрокаСлова = ТаблицаСлов.Найти("Истина", "Слово");
			Если СтрокаСлова <> Неопределено Тогда
