...
Процедура РазобратьКонтекстСтроки(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Ложь) Экспорт
	...
	Если ЛиСправаОтРавенства Тогда
		СтрокаШаблона = СтрокаШаблона + "(" + шРазделитель + "*(?:=|<>)" + шРазделитель + "*" + СтрокаШаблона + ")";
	КонецЕсли;
...
Функция ВычислитьТипПараметраМетодаМодуля(Знач ПараметрыМетода, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Если ПрямыеВызовыМетодов = Неопределено Тогда
			ПрямыеВызовыМетодов = мПлатформа.ИндексПоискаВызововСловаВМодуле(мОригинальныйТекст, ШаблонПрямогоВызоваСлова("(" + шИмя + ")",, Истина));
...
Функция МожноПрименитьПрисвоениеТипаИзЕсли(Знач ТекстДляПоискаОпределения, Знач Позиция0ВМетодеДо, Знач ПоследнееВхождение, Знач ПозицияВхождения0)
	...
	Тогда 
		...
		Если Ложь
			Или Истина
				И ПозицияКонецЕсли = 0
				И (Ложь
					Или ПозицияИначе = 0
					Или ПозицияИначе > ПозицияЕсли)
			Или Истина
				И ПозицияИначе = 0 
				И (Ложь
			...
			Или Истина
				И ПозицияКонецЕсли > ПозицияИначе
				И ПозицияИначе > ПозицияЕсли
		Тогда 
		...
		Если ЭтоИмяТипа Тогда
			...
			Если Фрагменты.Количество() > 1 Тогда
				...
				Если Найти(ТипКоллекции, " ") > 0 Тогда
					Возврат ТаблицаТипов;
				КонецЕсли;
				ТипыЭлементовКоллекции = ТаблицаТиповИзТекста(Фрагменты[1], Описание,, ДляСвойстваПуть);
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, _2 = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Тогда
		...
		Если ТаблицаОжидаемыхТипов <> Неопределено Тогда
			...
			КонецЕсли;
			Если Истина
				И ТаблицаОжидаемыхТипов.Количество() = 1 
				И (Ложь
					Или Не ОтделятьБольшиеНаборыСлов
					Или ТаблицаОжидаемыхТипов[0].Детальность > 2)
			Тогда
				...
				Если ПеречислимыеСвойстваМД.Найти(мИмяОжидаемогоТипа, "Слово") = Неопределено Тогда
					...
					ОбозначитьСловоСОжидаемымТиповВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора);
				Иначе
					...
					ИначеЕсли УровеньСвойства = 1 Тогда
						мСтрокаЛучшегоСлова = ТаблицаСлов.Найти(мИмяОжидаемогоТипа, "Слово");
					КонецЕсли;
...
Процедура ОбозначитьСловоСОжидаемымТиповВТаблицеСлов(КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора) Экспорт
	КонстантаРейтингСовпаденияТипа = КонстантаРейтингСовпаденияТипа();
...
Функция ЗаполнитьТаблицуСловДляТекстовогоЛитерала(ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен = Истина, ИмяПараметра = "") Экспорт
	...
	Тогда
		...
		// Имена свойств
		Если Истина
			И ВыбраннаяСтруктураТипа.ИмяОбщегоТипа <> "" // Исключаем общее описание значений элементов структуры (Метаданные = ТаблицаЗначений)
			И (Ложь
	...
	Тогда
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
			...
			Иначе
				...
				Если Истина
					И ФормаВладелец <> Неопределено 
					И СтараяТекущаяПозиция <> мПозицияВТексте 
					И АнализаторКодаМодуля = ЭтотОбъект
				Тогда
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен) Экспорт
	...
	БазовоеРасширениеКонфигурации = БазовоеРасширениеКонфигурации();
	Если Не КонтекстТекстовогоЛитералаИзвестен Тогда
		ВхожденияОбъектов = НайтиВхожденияРегВыраженияКэш("(" + шОбъектноеВыражение + ")\[\s*" + ИмяПараметра + "\s*\]",,, "ОбъектСВКвадратнымиСкобками;" + ИмяПараметра);
	...
	Если Не КонтекстТекстовогоЛитералаИзвестен Тогда
		// Мультиметка00193361 Вероятно дублирование
		ШаблонПоиска = шПредИнструкция + "ЗаполнитьЗначенияСвойств\s*\(\s*(" + шОбъектноеВыражение + ")\s*,\s*(" + шОбъектноеВыражение + ")\s*,\s*" + ИмяПараметра + "(?=[\s,\)])";
...
Функция ПроверитьВыражения(Знач ТабличноеПоле = Неопределено, Форма = Неопределено, Знач ПолныйПерерасчет = Ложь, Знач ПроверятьВсеЛитералы = Ложь) Экспорт
	...
	Для Этап = 1 По 2 Цикл
		...
		КонецЕсли;
		Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(Вхождения.Количество(), "Глубокая проверка " + ДобавкаПредставления);
		...
		Если ВыражениеЛитерала <> Неопределено Тогда 
			...
			Тогда
				...
				Тогда
					...
					ИмяВызываемогоМетода = мВызовМетода;
					СтрокаОписания = ТаблицаТипов[0].СтрокаОписания;
					РекурсивныйПуть = мРекурсивныйПуть;
					мРекурсивныйПуть = Неопределено;
					мПозицияВТексте = 0; // Не поддерживаем его актуальность. Поэтому обнуляем чтобы не делалось попытки восстановить позицию в ВычислитьВозможныеТекстовыеЗначенияПараметра()
					...
					Тогда
						...
						СтрокаОшибки.ВыражениеРодитель = ИмяВызываемогоМетода;
						Если СтрокаОписания <> Неопределено Тогда
			...
			Тогда
				ТекстЛитерала = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ВыражениеЛитерала);
				Если ирОбщий.ЛиТекстЯзыкаЗапросовЛкс(ТекстЛитерала) Тогда
					мНачальнаяПозиция0ВложенногоТекста = ПозицияВхождения0ВМодуле;
					...
					Для Каждого ВхождениеЗапроса Из ВхожденияЗапроса Цикл
						...
						//мТекстДляПоискаОпределения = мТекстБлока; 
						ПозицияВхождения0ВБлоке = ПозицияВхождения0ВБлоке - мПозицияТекстаДляПоискаОпределения + мНачальнаяПозиция0ВложенногоТекста + 2;
...
Функция ТаблицаСловВJSON(КлючНабораСлов = "") Экспорт 
	...
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
		...
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл 
			...
			Строка.Приоритет = ?(Строка.Рейтинг = РейтингСовпаденияТипа, 1, 0);
		КонецЦикла; 
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл  			Строка.ЛиРез = Строка.ТипЗначения <> "";  			Строка.ЛиМетод = Строка.ТипСлова = "Метод";  			Строка.ЛиТочный = Строка.Определение <> "Статистический";  			Строка.Тип = Лев(Строка.ТипЗначения, 200);  			Строка.Приоритет = ?(Строка.Рейтинг = РейтингСовпаденияТипа, 1, 0);  		КонецЦикла;  
	КонецЕсли;
