...
Процедура ПриНажатииКлавишиАвтодополнение(Источник, Событие, Данные, Символ) Экспорт
	...
	Тогда 
		...
		//Если Символ = "=" И Последние2СимволаРавны Тогда
...
Функция НовыеКоординатыВыражения(Знач Выражение, Знач Позиция0ВМетодеДо) Экспорт 
	...
	СтруктураСтрокиМодуля.Вставить("Позиция0ВТексте", мПозицияТекстаДляПоискаОпределения + Позиция0ВМетодеДо);
		...
		Если ЭтоИмяТипа Тогда
			...
			Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
				...
				Иначе 
					...
					Тогда
						Если ТипЗнч(СтруктураТипа.Метаданные) <> Тип("Структура") Тогда
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Иначе
		...
		Если мЭтоТекстовыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
			...
			СтарыйМетод = мМетодМодуля;
			КоллекцияЭлементовМД = ЗаполнитьТаблицуВозможныхЗначенийЛитерала(ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен,, ДобавлятьНизкоВероятные);
	...
	Если мЯзыкПрограммы = 1 И мЭтоЛокальныйКонтекстТаблицыСлов Тогда
	...
	Иначе
		...
		Иначе 
			...
			Если Истина
				...
				И (Ложь
					Или ЗначениеЗаполнено(мВызовМетода)
...
Функция РассчитатьОжидаемыйТипВыражения(ЭтоСсылочныйЛитерал = Неопределено) Экспорт
	...
	Тогда
		...
		ИначеЕсли ПредшествующийТокен = ";" Тогда
			...
			Вхождения = ирОбщий.НайтиРегВыражениеЛкс(Символы.ПС + ТекстовыйДокумент.ПолучитьСтроку(ТекстовыйДокумент.КоличествоСтрок()), мПлатформа.шПустоеНачалоСтроки + "(" + шИмя + ")\s*[=.]");
			Если Вхождения.Количество() Тогда
				ИмяПеременной = Вхождения[0].Подгруппа0;
...
Функция РассчитатьИмяОжидаемогоТипа(Знач ШаблонРазбора, Знач НачалоПредТекста, выхТаблицаОжидаемыхТипов, Знач ДлинаПредТекста) Экспорт
	...
	Если Вхождения.Количество() > 0 Тогда
		...
		Если мИмяОжидаемогоТипа = Неопределено Тогда
			...
			выхТаблицаОжидаемыхТипов = ВычислитьТипЗначенияВыражения(ВыражениеПриемник,,,,,,,,,,, мПозицияВБлоке + Смещение,, ДляСвойства);
			мИмяОжидаемогоТипа = мПлатформа.ПредставлениеМассиваСтруктурТипов(выхТаблицаОжидаемыхТипов);
			Если СтарыеТипы <> Неопределено Тогда
				СтарыеТипы[ВыражениеПриемник] = мИмяОжидаемогоТипа; // Жертвуем свежестью ради скорости
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
...
//
Функция ЗаполнитьТаблицуВозможныхЗначенийЛитерала(Знач ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	...
	Тогда 
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
					...
					Для Каждого СтруктураТипаСсылки Из ТаблицаТиповСсылки Цикл
						...
						Если ТипЗнч(СтруктураТипаСсылки.Метаданные) = Тип("ОбъектМетаданных") Тогда
							...
							Если ЗначениеЗаполнено(ИмяТаблицыБД) Тогда
								...
								АнализаторКода.ЗаполнитьТаблицуСлов(,,, Ложь);
								мТаблицаСловЛитерала = мПлатформа.НоваяТаблицаСлов();
			...
			Иначе
				...
				КонтекстТекстовогоЛитералаИзвестен = Неопределено;
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(СтрокаОписанияМетода, КонтекстТекстовогоЛитералаИзвестен, ИмяТипаСтруктуры, ДобавлятьНизкоВероятные);
			КонецЕсли;
			Если мЭтоТекстовыйЛитерал Тогда
				ВыбраннаяСтруктураТипа.ТипЯзыка = ИмяПараметра;
			КонецЕсли;
	...
	ИначеЕсли ИмяТипаСтруктуры = "" Тогда
		...
		Если Прав(ВыражениеСлеваОтРавно, 1) = "=" Тогда
			...
			Если Истина
				...
				И мРодительскийКонтекст = ""
			Тогда
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ВыражениеСлеваОтРавно, КонтекстТекстовогоЛитералаИзвестен,, Истина, ДобавлятьНизкоВероятные);
...
//.
// Параметры:
//    СтрокаОписанияМетода - СтрокаТаблицыЗначений - 
//    КонтекстТекстовогоЛитералаИзвестен - Булево								  -  
//    ИмяТипаСтруктуры	 - Строка - если пустое, то непосредственно литерал
// Возвращаемое значение:
//    Массив, ТаблицаЗначений - 
Функция ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(Знач СтрокаОписанияМетода, КонтекстТекстовогоЛитералаИзвестен = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь)
	...
	ИмяПараметра = ИмяПараметраВызоваМетода(СтрокаОписанияМетода);
	КонтекстТекстовогоЛитералаИзвестен = Ложь;
	Если СтрокаОписанияМетода.ИмяМодуля <> мМодульМетаданных.Имя Тогда
		АнализаторКодаМодуля = мПлатформа.ПолеТекстаМодуля(СтрокаОписанияМетода.ИмяМодуля);
	...
	КоллекцияЭлементовМД = АнализаторКодаМодуля.ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен, ИмяТипаСтруктуры,, ДобавлятьНизкоВероятные);
...
//  РезультатГотов	 - Булево - 
//  ИмяТипаСтруктуры	 - Строка - если пустое, то непосредственно литерал
// 
// Возвращаемое значение:
//   - 
//
Функция ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(Знач ИмяПеременной, РезультатГотов = Ложь, Знач ИмяТипаСтруктуры = "", Знач ЛиСобиратьПрисвоения = Ложь, Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
		...
		Тогда
			...
			Для Каждого Вхождение Из ВхожденияОбъектов Цикл
				...
				Если ЕстьСовпадение И ЗначениеЗаполнено(ВыражениеИсточник) Тогда
					ТаблицаТипов = ВычислитьТипЗначенияВыражения(ВыражениеИсточник,,,,,, Ложь,,,,, ПозицияВхождения0, Неопределено);
					Если ТаблицаТипов.Количество() > 0 Тогда
						ЛучшийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТипов);
	...
	ИначеЕсли ИмяТипаСтруктуры = "Массив" Тогда
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Если Не ирОбщий.СтрокиРавныЛкс(Вхождение.SubMatches(1), ИмяПеременной) Тогда
				Продолжить;
			КонецЕсли;
			НовыеЗначенияПараметра = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(Вхождение.SubMatches(0), РезультатГотов,,, ДобавлятьНизкоВероятные);
	...
	Пока Истина Цикл
		...
		Для Каждого Вхождение Из ВхожденияВызовов Цикл
			...
			Если ТаблицаТипов.Количество() > 0 Тогда
				...
				НовыеЗначенияПараметра = ЗаполнитьТаблицуВозможныхЗначенийЛитерала(ТаблицаТипов[0], РезультатГотов, ИмяТипаСтруктуры, ДобавлятьНизкоВероятные);
				Если Истина
					И ИмяТипаСтруктуры = ""
