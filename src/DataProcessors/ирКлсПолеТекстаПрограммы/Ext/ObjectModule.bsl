...
Функция ЗапомнитьДляСравнения(Знач РазрешитьВопросы = Истина, Знач Текст = "") Экспорт
	...
	КонецЕсли; 
	Возврат ирКлиент.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ФормаВладелец, ПолеТекста, ВариантСинтаксисаСравнителя, РазрешитьВопросы, Текст);
КонецФункции
				...
				Если ЗначениеЗаполнено(ТекущийИндекс) Тогда
					...
					Тогда
						...
						Для Каждого СтрокаСлова Из КопияКомпоненты.ТаблицаСлов Цикл
							...
							Если Истина
								И ТаблицаСтруктурТипа.Количество() > 0 
								И ТаблицаСтруктурТипа[0].ИмяОбщегоТипа = "Массив" 
								И ТипЗнч(ТаблицаСтруктурТипа[0].Метаданные) = Тип("ТаблицаЗначений")
							Тогда
								ЗначениеПеременной = ТаблицаСтруктурТипа[0].Метаданные[0].Метаданные;
								Если ТипЗнч(ЗначениеПеременной) = Тип("ПостроительЗапроса") Тогда
									// Перенес из Мультиметка6091573442
				...
				Иначе
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							КонецЕсли;
							СчитатьГлубинуВеток(ТекстДляПоискаОпределения, ГлубинаВеток, Позиция0ВхожденияВМетоде, ПоследнееВхождение);
							Если Ложь
								Или (Истина
			...
			Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
				...
				Если Фрагменты.Количество() > 1 Тогда
					...
					//Если Прав(СтрокаОписания, 1) = ":" Тогда // Отключил проверку, т.к. описание свойства может быть многострочным, а тут проверяется только первая строка
						ТипыЭлементовКоллекции = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(Фрагменты[1], "]"), Описание, ТекстДоВхождения, ДляСвойстваПуть, СчетчикСтрок, ЛиВложенный, Истина, Позиция0ВМетоде);
					//КонецЕсли; 
							...
							Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяСвойства) Тогда 
								...
								Если ФрагментыОписания.Количество() > 1 Тогда
									...
									Иначе
										...
										Исключение
											МетаданныеСвойства = ТаблицаТиповИзТекста(ФрагментыОписания[1],, ТекстДоВхождения, ДляСвойстваВниз);  
										КонецПопытки;
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	Знач Сортировать = Истина, Знач ДобавлятьНизкоВероятные = Ложь, Знач ОтделятьБольшиеНаборыСлов = Ложь, Знач ФильтроватьСлово = Неопределено) Экспорт
		...
		Попытка
			Для Каждого СтруктураКонтекста Из мТаблицаТиповКонтекста Цикл
				ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ЭтоГруппаОбщихМодулей,,
					?(мЭтоТекстовыйЛитерал И СтруктураКонтекста.ТипЯзыка <> "ИмяТипа", Неопределено, мФлагиКомпиляции), ФильтроватьСлово, МодульМетаданныхКонтекста(СтруктураКонтекста),, ВыгрузкаТаблицыСлов,
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	ИмяТипаВложенногоТипа = ОписаниеОжидаемогоТипа.ИмяТипаВложенногоТипа;
	Если Истина
		...
		Иначе 
			...
			Тогда
				...
				Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
					...
					СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
				КонецЦикла;
				Если мИмяОжидаемогоТипа = "Тип" И ЗначениеЗаполнено(ИмяТипаВложенногоТипа) Тогда
					СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Тип(""" + ИмяТипаВложенногоТипа + """)", "Конструкция",, "Тип", "Локальный", Ложь);
					СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
					СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
				КонецЕсли;
				Если Истина
...
Функция РассчитатьОжидаемыйТипВыражения(ЭтоСсылочныйЛитерал = Неопределено) Экспорт
	...
	Тогда
		...
		Тогда
			...
			ВыражениеПриемник = РассчитатьИмяОжидаемогоТипа(ШаблонРазбора, ПредшествующийТекст, ДлинаПредТекста, ТаблицаОжидаемыхТипов, ИмяТипаВложенногоТипа);
		ИначеЕсли Истина
			И ПредшествующийТокен = """" 
		...
		ИначеЕсли мЯзыкПрограммы = 1 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		мКэшОжидаемогоТипа = КэшОжидаемогоТипа(ТаблицаОжидаемыхТипов, ВыражениеПриемник, РазрешитьМногоТипов, ПредшествующийТокен, ИмяТипаВложенногоТипа);
	Иначе
...
Функция РассчитатьИмяОжидаемогоТипа(Знач ШаблонРазбора, Знач НачалоПредТекста, Знач ДлинаПредТекста, выхТаблицаОжидаемыхТипов, выхИмяТипаВложенногоТипа) Экспорт
			...
			Если ЛиВложенныйТип Тогда
				...
				Для Каждого СтрокаТаблицы Из ТаблицаТипов Цикл
					...
					Тогда
						Продолжить;
					КонецЕсли;
					выхИмяТипаВложенногоТипа = ИмяТипа; 
...
// Возвращаемое значение:
//    Структура - 
Функция КэшОжидаемогоТипа(Знач ТаблицаОжидаемыхТипов, Знач ВыражениеПриемник, Знач РазрешитьМногоТипов, Знач ПредшествующийТокен, Знач ИмяТипаВложенногоТипа) Экспорт
	Результат = Новый Структура;
	...
	Результат.Вставить("ИмяТипаВложенногоТипа", ИмяТипаВложенногоТипа);
	Возврат Результат;
КонецФункции
...
Функция ЗаполнитьТаблицуВозможныхЗначенийЛитерала(Знач ВыбраннаяСтруктураТипа, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь,
	...
	Тогда
		...
		Тогда
			...
			Если СтруктураТипаПараметра.Количество() > 0 Тогда
				мТаблицаСловЛитерала = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаПараметра[0],,,,,, "Свойство",, ФильтроватьСлово,,,,,, БазовоеРасширениеКонфигурации());
			КонецЕсли;
		ИначеЕсли Истина
			...
			Тогда
				...
				Если СтруктураТипаПараметра.Количество() > 0 Тогда
					...
					Если ЗначениеЗаполнено(СтруктураТипаЭлемента) Тогда
						БазовоеРасширениеКонфигурации = БазовоеРасширениеКонфигурации();
						КоллекцияЭлементовМД = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаЭлемента[0],,,,,,,, ФильтроватьСлово,,,,,, БазовоеРасширениеКонфигурации);
	...
	Тогда
		...
		Если ЛучшийТип <> Неопределено Тогда
			КоллекцияЭлементовМД = мПлатформа.ТаблицаСловИзСтруктурыТипа(ЛучшийТип,,,,,, "Свойство",, ФильтроватьСлово).ВыгрузитьКолонку("Слово");
		КонецЕсли;
	ИначеЕсли Истина
			...
			Для Каждого Вхождение Из ВхожденияОбъектов Цикл
				...
				Для Каждого СтруктураТипа Из ТаблицаТипов Цикл
					...
					Тогда
						Продолжить;
					КонецЕсли;
					ТаблицаСловОбъекта = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипа,,,,,, "Свойство",, ФильтроватьСлово,,, ТаблицаСловОбъекта,,, БазовоеРасширениеКонфигурации, Истина);
				КонецЦикла;
				...
				Для Счетчик = 0 По 1 Цикл
					...
					Если ТаблицаТипов.Количество() > 0 Тогда
						...
						Для СчетчикТипа = 1 По Мин(5, ТаблицаТипов.Количество()) Цикл // для ВсеЭлементыФормы нужно 5
							...
							ТаблицаСловОбъекта = мПлатформа.ТаблицаСловИзСтруктурыТипа(ЛучшийТип,,,,,, "Свойство",, ФильтроватьСлово,,, ТаблицаСловОбъекта,,, БазовоеРасширениеКонфигурации, Истина);
						КонецЦикла;
				...
				Если ТаблицаТипов.Количество() > 0 Тогда
					...
					ИначеЕсли ТипЗнч(МетаданныеТипа) = Тип("ТаблицаФормы") Тогда
						...
						Если ТаблицаТиповДочерняя.Количество() > 0 Тогда
							Результат = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповДочерняя[0],,,, Ложь,, "Свойство",, ФильтроватьСлово,,,,,,, Истина);
						КонецЕсли;
					КонецЕсли;
		...
		Тогда
			...
			Для Каждого Вхождение Из ВхожденияОбъектов Цикл
				...
				Если ЕстьСовпадение И ЗначениеЗаполнено(ВыражениеИсточник) Тогда
					...
					Если ТаблицаТипов.Количество() > 0 Тогда
						ЛучшийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТипов);
						Результат = мПлатформа.ТаблицаСловИзСтруктурыТипа(ЛучшийТип,,,,,, "Свойство",, ФильтроватьСлово,,,,,, БазовоеРасширениеКонфигурации, Истина);
