...
Перем мКорневаяТаблицаСтруктурТипов Экспорт;
...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	ДобавитьПравилоВычисленияФункции("НайтиЭлементКоллекцииЛкс", "ВычислитьНайтиЭлементКоллекцииЛкс", "ОбщийМодуль.ирОбщий");
...
Функция ВыполнитьКоманду(Знач Команда, Знач КодКлавиши = "", ОбновитьКонтекст = Истина)
	...
	Попытка
		...
		ИначеЕсли Команда = КнопкиМакета0.ПоискОбщегоМетода.Имя Тогда
...
Функция ВыделитьМетод(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	Для Каждого СтрокаПараметра Из ФормаНовогоМетода.ТаблицаПараметров Цикл
		...
		Тогда
			...
			ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(СтрокаПараметра.Имя, ТекстДо + ТекстНовогоМетода);
			СтрокаПараметра.ТипЗначения = мПлатформа.ПредставлениеМассиваСтруктурТипов(ТаблицаСтруктурТипов);
		КонецЕсли; 
	КонецЦикла;
...
Функция НайтиПоказатьСправкуПоСтруктуреТипа(Знач ПутьКСлову, Знач СтруктураТипа, ВключатьПутьКОписаниюТипаЗначения = Ложь, выхФормаВыбора = Неопределено, ВладелецФормы = Неопределено,
	Знач ТаблицаСтруктурТиповКонтекста = Неопределено) Экспорт 
	...
	ИначеЕсли мЯзыкПрограммы = 0 Тогда 
		...
		Если ТаблицаСтруктурТипов = Неопределено Тогда
			...
			Если СтрокаПеременной <> Неопределено Тогда
				...
				Возврат Истина;
			КонецЕсли;
			//ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(мКонтекст, мТекстДляПоискаОпределения, мПредшествующийТекст, Истина); // Так для ПолучитьФорму("Справочник.Мо<>й") будет вычислять форму
			ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(мКонтекст, мТекстДляПоискаОпределения, мПредшествующийТекст, Ложь);
			Если Истина
				И Прав(мКонтекст, 1) <> "("
				И ТаблицаСтруктурТипов[0].Детальность = 0
				И ТаблицаСтруктурТипов[0].СтрокаОписания = Неопределено
			Тогда
				ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(мКонтекст + "(", мТекстДляПоискаОпределения, мПредшествующийТекст, Ложь);
			КонецЕсли;
		КонецЕсли;
...
Функция СсылкаСтрокиМодуля(Знач НомерСтроки = 0) Экспорт 
	...
	Если мМодульМетаданных.СтруктураТипа <> Неопределено Тогда
		ИмяРасширения = мМодульМетаданных.СтруктураТипа.ДержательМетаданных;
		Если ТипЗнч(ИмяРасширения) <> Тип("Строка") Тогда
			ИмяРасширения = "";
		КонецЕсли;
	КонецЕсли;
	Результат = ирОбщий.СсылкаСтрокиМодуляЛкс(мИмяМодуля, мНачальнаяСтрока, ИмяМетода, СмещениеСтрокиМетода, ПолеТекста.ПолучитьСтроку(мНачальнаяСтрока), ИмяРасширения);
	Возврат Результат;
КонецФункции
...
Функция ВычислитьТипДочернегоЭлемента(МассивРодительскихСтрутурТипа, ТекущееСлово, ТипСлова, ТекущийИндекс = Неопределено, ТекущиеАргументы = "", ПредшествующийТекст = "",
	...
	Для Каждого РодительскаяСтруктураТипа Из МассивРодительскихСтрутурТипа Цикл
		...
		Если РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
					...
					Если ПараметрыМетода <> Неопределено И ТипСлова = "Свойство" Тогда
						...
						Если СтрокаПараметраМетода <> Неопределено Тогда
							Если мМодульМетаданных.СтруктураТипа <> Неопределено Тогда
									...
									Если СтрокаОбработчика <> Неопределено Тогда 
										...
										Если ЭлементФормы <> Неопределено Тогда
											Если ирОбщий.СтрокиРавныЛкс(СтрокаПараметраМетода.Имя, "Элемент") Тогда 
												...
												КонецЕсли;
												мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтрукутраТипаЭлемента);
												Прервать;
											ИначеЕсли Истина                         
											...
											ИначеЕсли Истина                         
												...
												И ТипЗнч(ФормаМодуля) = Тип("Форма")
											Тогда
												мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, мПлатформа.СтруктураТипаИзКонкретногоТипа(Тип(СтрокаПараметраМетода.ТипЗначения),, Новый Структура("Метаданные", ЭлементФормы)));
												Прервать;
											КонецЕсли;
										КонецЕсли;
									КонецЕсли; 
								...
								Тогда
									...
									Для Каждого Шаблон Из ОбъектМД.ШаблоныURL Цикл
										Если ирОбщий.НайтиЭлементКоллекцииЛкс(Шаблон.Методы, "Обработчик", мМетодМодуля.Имя) <> Неопределено Тогда 
											мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, мПлатформа.СтруктураТипаИзКонкретногоТипа(Тип("HTTPЗапрос")));
										КонецЕсли;
									КонецЦикла;
								КонецЕсли;
								...
								Тогда
									Если ПараметрыМетода.Индекс(СтрокаПараметраМетода) = 0 Тогда
										мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, мМодульМетаданных.СтруктураТипа.Метаданные.ТипПараметраКоманды);
									КонецЕсли;
							...
							Если ЗначениеЗаполнено(СтрокаПараметраМетода.ТипЗначения) Тогда
								ТаблицаСтруктурТиповНазначенных = ТаблицаСтруктурТиповИзТекста(СтрокаПараметраМетода.ТипЗначения, СтрокаПараметраМетода.Описание);
								мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, ТаблицаСтруктурТиповНазначенных); 
							КонецЕсли;
							ШаблонПараметра = "(" + шВыражениеПрограммы + ")?" + шРазделитель + "*(?:$|,|\))";
							...
							Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
								...
								Тогда
									...
									Продолжить;
								КонецЕсли;
								ФактическиеПараметры = Сред(ВхождениеВызова.ТекстВхождения, Найти(ВхождениеВызова.ТекстВхождения, "(") + 1);
								Если ИндексПараметра > СтрЧислоВхождений(ФактическиеПараметры, ",") Тогда
									Продолжить;
								КонецЕсли;
								ВхожденияПараметров = ирОбщий.НайтиРегВыражениеЛкс(ФактическиеПараметры, ШаблонПараметра,,,,,, Истина, мРегВыражение,, ВхожденияПараметров); // Передаем РегВыражение для ускорения
								#Если Сервер И Не Сервер Тогда
								...
								Если ВхожденияПараметров.Количество() > ИндексПараметра Тогда
									...
									ИначеЕсли ЛиПрисвоениеБулевойКонстанты(ФактическийПараметр) Тогда
										мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, мПлатформа.НоваяСтруктураТипа("Булево"));
									Иначе
										...
										Если ПройденныеВызовы[КлючВызова] = Неопределено Тогда
											...
											Если ТаблицаСтруктурТиповИсточник[0].Детальность > 0 Тогда
												мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, ТаблицаСтруктурТиповИсточник);
												Если ТаблицаСтруктурТиповИсточник[ТаблицаСтруктурТиповИсточник.Количество() - 1].Детальность > 1 Тогда
			...
			//	И МассивРодительскихСтрутурТипа = мКорневаяТаблицаСтруктурТипов
...
Функция ВычислитьПрисвоенияСвойствуМодуля(Знач ИмяСвойства, Знач ИскатьПоВсемуМодулюПриНеуспехе = Ложь) Экспорт 
	Если мМодульМетаданных.ТипМодуля = "Форма" Тогда
		ИмяСамоСвойстваКонтекста = "ЭтаФорма";
	Иначе
		ИмяСамоСвойстваКонтекста = "ЭтотОбъект";
	КонецЕсли;
	ШаблонСвойства = "(?:" + ИмяСамоСвойстваКонтекста + "\.)?" + ИмяСвойства;
	ТекстДляПоискаОпределения = ЗалитьКомментарииИСтроковыеЛитералы(мМодульМетаданных.Программа); // Вырезаем условия
	ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(ШаблонСвойства, ТекстДляПоискаОпределения, ТекстДляПоискаОпределения);
	Если Истина
		И ТаблицаСтруктурТипов.Количество() = 1
		И ТаблицаСтруктурТипов[0].Детальность = 0
		И ИскатьПоВсемуМодулюПриНеуспехе
	Тогда
		ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(мОригинальныйТекст, ШаблонНазначенияТипаВыражению(ШаблонСвойства),,,,,, Истина);
	КонецЕсли;  
	Возврат ТаблицаСтруктурТипов;
КонецФункции
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	Знач ЭтоВызовКонструктора = Ложь, ПолныйАнализ = Истина, Знач КорневойВызов = Истина, Знач ГлобальныйПоиск = Ложь) Экспорт
	...
	Если РазрешитьАнализИмениТипа Тогда
		...
		Иначе
			Если мЭтоСтроковыйЛитерал Тогда
				...
				ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(мВызовМетода + ИмяОтключенияЛишнихВычислений + ")", ТекстДляПоискаОпределения, ПредшествующийТекст,, мЭтоКонструктор, ПолныйАнализ, Ложь);
				Возврат ТаблицаСтруктурТипов;
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли; 
	...
	Иначе
		...
		Если Вхождения.Количество() > 0 Тогда 
			...
			Если ТаблицаСтруктурРодителей.Количество() = 0 Тогда
				Если ПустаяСтрока(РодительскийКонтекст) И мКорневаяТаблицаСтруктурТипов <> Неопределено Тогда
					ТаблицаСтруктурРодителей = мКорневаяТаблицаСтруктурТипов; 
				Иначе 
					...
					Если ПустаяСтрока(РодительскийКонтекст) И мКорневаяТаблицаСтруктурТипов = Неопределено Тогда
						мКорневаяТаблицаСтруктурТипов = ТаблицаСтруктурРодителей;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущееСлово) Тогда 
				...
				Тогда
					// Считаем что тип уже достаточно точно вычислен
				Иначе
					// Обогощаем присвоенными типами
					ШаблонПриемника = Выражение;
					Если Лев(Выражение, 1) <> "(" Тогда
						ШаблонПриемника = ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(ШаблонПриемника);
					КонецЕсли;
					мРегВыражение.Global = Истина;
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Тогда
								...
								ТаблицаСтруктурТипов = ТаблицаСтруктурТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ПрисвоенныйТип, "-"),, ТекстДоВхождения);
							...
							Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(1)) Тогда
								...
								ИначеЕсли Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(6)) Тогда 
									ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
									МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения, РазрешитьАнализИмениТипа,, ПолныйАнализ, Ложь);
									мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, МассивСтруктурПрисвоенныхТипов);
							...
							Иначе
								...
								ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
								ТаблицаСтруктурТиповКоллекции = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения,,, ПолныйАнализ, Ложь);
								СтруктураТипаКоллекции = ТаблицаСтруктурТиповКоллекции[0];
								Если ЗначениеЗаполнено(СтруктураТипаКоллекции.ИмяОбщегоТипа) Тогда
									МассивСтруктурПрисвоенныхТипов = мПлатформа.СловаКонтекстаМетаданные(СтруктураТипаКоллекции, "[0]",,, мЯзыкПрограммы, "0",,, мФлагиКомпиляции)[0].ТаблицаСтруктурТипов;
									мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, МассивСтруктурПрисвоенныхТипов);
								КонецЕсли;
							КонецЕсли;
							Если Истина
								И ТаблицаСтруктурТипов.Количество() > 0
								И ТаблицаСтруктурТипов[ТаблицаСтруктурТипов.Количество() - 1].Детальность > 0
							Тогда 
								Прервать;
							КонецЕсли;
						КонецЦикла;
			...
			Если ТипСлова = "Свойство" И Не СтруктураТипа.Конструктор Тогда
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаСтруктурТипов Цикл
					...
					Тогда
						...
						РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляЗаполненияМетаданных, Истина);
						Если РезультатСвойств.Количество() > 0 Тогда
							...
							Иначе
								ТекстовоеЗначение = ВычислитьТекстовоеЗначение(ПрисвоенноеВыражение, Лев(ТекстДляЗаполненияМетаданных, ВхождениеСвойства.FirstIndex));
							КонецЕсли;
							СтруктураТипаЦикл.Метаданные = Новый Структура;
							Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда
								Попытка
									СтруктураТипаЦикл.Метаданные = Новый Структура(ТекстовоеЗначение);
								Исключение
								КонецПопытки;
							КонецЕсли;
						ИначеЕсли Истина
						...
						Тогда
							...
							КонецЕсли;
						КонецЕсли;
						мРегВыражение.Pattern = Выражение + "\s*\.\s*Вставить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
						мРегВыражение.Global = Истина;
					...
					Тогда
						ТекстЗапроса = ВычислитьТекстовоеЗначение(Выражение + ".Текст", ТекстДляЗаполненияМетаданных);
...
// Возвращаемое значение:
//    Булево - 
Функция ЛиПрисвоениеБулевойКонстанты(Знач ПрисвоенноеВыражение) Экспорт
	Результат = Ложь 
...
// Возвращаемое значение:
//    Строка - 
Функция ШаблонНазначенияТипаВыражению(ШаблонПриемника) Экспорт
	...
	Результат = шПредИмя + ШаблонПриемника 
		//+ "(" + шПрисваивание + ";?|" + ШаблонКоллекции + ")(\s*//" + шНазначениеТипа + ")?";
		+ "(" + шПрисваивание + ";?|" + ШаблонКоллекции + ")(?:\s*//([^\n]+))?";
	Возврат Результат;
КонецФункции
...
// Возвращаемое значение:
//    Строка - 
Функция ВычислитьТекстовоеЗначение(ВыражениеТекста, Знач ТекстДляЗаполненияМетаданных) Экспорт
	мРегВыражение.Global = Ложь;
	мРегВыражение.Pattern = ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(ВыражениеТекста) + "\s*=\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
	мРегВыражение.Global = Истина;
	РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляЗаполненияМетаданных, Истина);
	Если РезультатСвойств.Количество() > 0 Тогда
		ВхождениеСвойства = РезультатСвойств[0];
		...
		Если Лев(ПрисвоенноеВыражение, 1) = """" Тогда
			Попытка
				ТекстЗапроса = Вычислить(ПрисвоенноеВыражение);
			Исключение
				ТекстЗапроса = "";
			КонецПопытки;
		Иначе
			ТекстЗапроса = ВычислитьТекстовоеЗначение(ПрисвоенноеВыражение, Лев(ТекстДляЗаполненияМетаданных, ВхождениеСвойства.FirstIndex));
		КонецЕсли;
	КонецЕсли;
...
// Возвращаемое значение:
//    ТаблицаЗначений - 
Функция ТаблицаСтруктурТиповИзТекста(Знач ТекстТипа, Знач Описание = "", Знач ТекстДоВхождения = "") Экспорт
	...
	Иначе
		...
		Если ЭтоИмяТипа Тогда
			...
			Если Фрагменты.Количество() > 1 Тогда
				...
				ТипыЭлементовКоллекции = ТаблицаСтруктурТиповИзТекста(Фрагменты[1], Описание);
			...
			Иначе
				...
				МетаданныеИзОписания = Новый ТаблицаЗначений;
				Для Счетчик = 2 По ТекстОписания.КоличествоСтрок() Цикл
					...
					Если Лев(СтрокаОписания, 1) = "*" Тогда
						...
						КонецПопытки;
						Попытка
							МетаданныеИзОписания.Колонки.Добавить(ИмяСвойства, ОписаниеТипов);
						Исключение
						КонецПопытки;
					КонецЕсли;
				КонецЦикла;
				...
				Для Каждого ИмяТипа Из ФрагментыЗапятой Цикл
					...
					Если Тип <> Неопределено Тогда
						...
						Если ТипЗнч(СтруктураТипа.Метаданные) = Тип("ОбъектМетаданныхКонфигурация") Тогда
							СтруктураТипа.ИмяОбщегоТипа = ИмяТипа;
						КонецЕсли;
						Если МетаданныеИзОписания.Колонки.Количество() > 0 Тогда
							Если Ложь
								Или ИмяТипа = "Структура"
								Или ИмяТипа = "ФиксированнаяСтруктура"
							Тогда 
								МетаданныеТипа = ирОбщий.УстановитьМетаданныеКоллекцииЛкс(МетаданныеИзОписания.Колонки, Новый Структура);
							Иначе
								МетаданныеТипа = МетаданныеИзОписания;
							КонецЕсли;
		...
		Иначе
			...
			Если Найти(ТекстТипа, "(") = 0 И Найти(ТекстТипа, ".") > 0 Тогда
				...
				Если ирОбщий.СтрНачинаетсяСЛкс(ТекстТипа, КорневоеСловоБД) Тогда
					мЯзыкПрограммы = 1;
					СтруктураТипа = ВычислитьТипЗначенияВыражения(Сред(ТекстТипа, СтрДлина(КорневоеСловоБД) + 1))[0].Квалификаторы;
					мЯзыкПрограммы = 0;
			...
			Если СтруктураТипа = Неопределено Тогда 
				Если Найти(ТекстТипа, "(") = 0 Тогда 
					ТекстТипа = ТекстТипа + "(";
				КонецЕсли;
				ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(ТекстТипа, ТекстДоВхождения, ТекстДоВхождения,,,, Ложь);
			Иначе
				мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, СтруктураТипа);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Если ТаблицаСтруктурТипов.Количество() = 0 Тогда
		мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, мПлатформа.НоваяСтруктураТипа());
	КонецЕсли;
	Возврат ТаблицаСтруктурТипов;
КонецФункции
...
Функция ЗаполнитьТаблицуСлов(ТаблицаСтруктурТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Если мЭтоСтроковыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
		...
		Тогда
			...
			Тогда
				КоллекцияЭлементовМД = ирОбщий.ВыгрузитьСвойствоКоллекцииЛкс(ирОбщий.СобытияЭлементаФормыЛкс(ВыбраннаяСтруктураТипа.Метаданные), "Слово");
			ИначеЕсли Истина
		...
		Тогда
			...
			Если ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
				...
				ВнутренняяТаблицаСлов = мПлатформа.СловаКонтекстаМетаданные(выхСтруктураТипаКонтекста,, "Метод",,,,,, Новый Структура("Сервер", Истина));
	...
	// Имена типов внутри комментария и текстовых литерала
	Если Истина
...
Функция КопияКомпоненты()
	...
	КопияКопоненты.мКорневаяТаблицаСтруктурТипов = мКорневаяТаблицаСтруктурТипов;
...
Функция ВычислитьПолучитьФорму(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	Если Лев(ПервыйПараметр, 1) = """" Тогда
		ИмяФормы = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ПервыйПараметр);
	ИначеЕсли ПустаяСтрока(ПервыйПараметр) Тогда 
		// Основная форма
	Иначе
...
Функция ВычислитьНайтиЭлементКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	|" + ИмяПеременной + "=" + МассивПараметров[0]);
	Возврат Результат;
		...
		Если ЛиОтдельноеОписаниеАктивно И ТаблицаСтруктурТипов <> Неопределено Тогда 
			...
			Тогда
				Если мСтруктураТипаКонтекста.Конструктор Тогда
					...
					ТаблицаСтруктурТипов = мПлатформа.СловаКонтекстаПредопределенные(СтруктураТипа, СтрокаТаблицыСлов.Слово, "Конструктор",, ЯзыкПрограммы, Конфигурация)[0].ТаблицаСтруктурТипов;
				КонецЕсли; 
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, Знач ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	Если ВключатьКонструкции Тогда
		Если ЯзыкПрограммы = 0 Тогда
			...
			ЛитералыЗначения = мПлатформа.ЛитералыЗначенияВстроенногоЯзыка();
		ИначеЕсли ЯзыкПрограммы = 1 Тогда
...
Функция УстановитьТекст(Знач Текст, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	ОчиститьТаблицуСловЛокальногоКонтекста();
	мФлагиКомпиляции = мПлатформа.НовыеФлагиКомпиляции();
	мКорневаяТаблицаСтруктурТипов = Неопределено;
...
Функция ИмяТипаКонтекста(ТаблицаСтруктурТипов) Экспорт 
	Возврат мПлатформа.ИмяТипаИзСтруктурыТипа(ТаблицаСтруктурТипов[0]);
КонецФункции
