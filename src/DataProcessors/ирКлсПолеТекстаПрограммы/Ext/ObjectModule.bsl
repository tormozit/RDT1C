			...
			Если СтрокаОписанияМетода.ТипКонтекста = "ПолеКомпоновкиДанных" Тогда
				...
				ИначеЕсли ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
					Если ирОбщий.СтрКончаетсяНаЛкс(ВыражениеСлеваОтРавно, ".Поле") Тогда
						КоллекцияЭлементовМД = ирОбщий.ВыгрузитьСвойствоЛкс(ВыбраннаяСтруктураТипа.Метаданные.Элементы);
					Иначе
	...
	Иначе
		...
		Если Прав(ВыражениеСлеваОтРавно, 1) = "=" Тогда
			...
			Если ирОбщий.СтрНачинаетсяСЛкс(ирОбщий.ПоследнийФрагментЛкс(ВыражениеСлеваОтРавно), "ПутьКДанным") Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если Не КонтекстТекстовогоЛитералаИзвестен И Не мПлатформа.мРежимПроверкиМодуля Тогда
...
Функция СписокРеквизитовУправляемойФормы(Знач Форма) Экспорт
	РеквизитыФормы = мПлатформа.ИменаРеквизитовФормы(Форма);
	КоллекцияЭлементовМД = ирОбщий.ВыгрузитьСвойствоЛкс(РеквизитыФормы.Все);
	//СхемаФормы = Новый СхемаКомпоновкиДанных;
	//НаборДанных = СхемаФормы.НаборыДанных.Добавить(Тип("НаборДанныхОбъектСхемыКомпоновкиДанных"));
	...
	//	ПолеНабора.ПутьКДанным = ИмяРеквизита;  
	//КонецЦикла;
	//КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	//КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаФормы));
	//КоллекцияЭлементовМД = ирОбщий.ВыгрузитьСвойствоЛкс(КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.Элементы, "Поле");
	Возврат КоллекцияЭлементовМД;
КонецФункции
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен, Знач ЛиСобиратьПрисвоения = Ложь) Экспорт
	ИмяШаблон = ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(ИмяПараметра);
...
Процедура ДобавитьПростыеСлова(Знач КоллекцияЭлементовМД, Знач ЛиПриоритетно = Ложь)
		...
		Если ЛиОтдельноеОписаниеАктивно И ТаблицаТипов <> Неопределено Тогда 
			...
			Тогда
				...
				Если ТаблицаТипов.Количество() > 0 Тогда
					...
					СтруктураПодсказкиМетода = СтруктураПодсказкиМетода(ТаблицаТипов[0], Ложь, Истина, ЕстьПараметры);
					...
					КонецЕсли; 
				КонецЕсли;
			ИначеЕсли ЛиСистемноеСлово(ТаблицаТипов[0]) Тогда
				СинтаксПомощник = мПлатформа.СинтаксПомощник();
				ОписаниеСлова = СинтаксПомощник.ЗагрузитьОписаниеСвойства(ТаблицаТипов[0].СтрокаОписания);
				Если ЗначениеЗаполнено(ОписаниеСлова) Тогда
					СтруктураОписания.Добавить("> Описание: " + ОписаниеСлова);
				КонецЕсли; 
			КонецЕсли; 
