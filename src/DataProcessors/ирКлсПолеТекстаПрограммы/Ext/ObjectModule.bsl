	...
	Если СтруктураТипа.СтрокаОписания <> Неопределено Тогда 
		...
		Если ТаблицаВладелец.Колонки.Найти("ЛиЭкспорт") <> Неопределено Тогда
			...
			Если СтрокаСловаМодуля.Имя <> "<>" Тогда
				...
				Если Найти(мТекущееСлово, "(") > 0 Или мЭтоТекстовыйЛитерал Тогда
...
Процедура ОткрытьАвтодополнение(Знач КодКлавиши = "", Знач Модально = Ложь) Экспорт 
	...
	Пока Истина Цикл
		...
		ПолеТекстаПрограммы = ЭтотОбъект; 
		// адаптер Мультиметка5265263814
		Если Ложь
			Или Не Успешно
			Или (Истина
				И ЛиАвтовызов
				И Не ЕстьЛучшееСлово 
				И ПолеТекстаПрограммы.ТаблицаСлов.Количество() > 50
				И (Ложь
					...
					Или Прав(ТекущаяСтрокаНачало, 1) = ","
					Или (Истина
						И ПолеТекстаПрограммы.мРодительскийКонтекст = ""
						И Прав(СокрП(ТекущаяСтрокаНачало), 1) = ".")
					Или (Истина
						И ПолеТекстаПрограммы.мРодительскийКонтекст = ""
						И ПолеТекстаПрограммы.мЭтоТекстовыйЛитерал
						И Не ирОбщий.СтрКончаетсяНаЛкс(ТекущаяСтрокаНачало, """" + ПолеТекстаПрограммы.мКонтекст))))
		Тогда
			Возврат;
		КонецЕсли;
		//Если Ложь 
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Иначе
		...
		Если мТаблицаСловЛитерала <> Неопределено Тогда
			ДобавитьПростыеСлова(мТаблицаСловЛитерала);
		КонецЕсли;
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	ИменаТипов = ирОбщий.СтрРазделитьЛкс(мИмяОжидаемогоТипа, ",", Истина, Ложь);
	ирОбщий.УдалитьЗначениеИзМассиваЛкс(ИменаТипов, "Неопределено");
	ирОбщий.УдалитьЗначениеИзМассиваЛкс(ИменаТипов, "Произвольный");
	...
	Иначе
		...
		Если мСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа" Тогда
			Для Каждого ИмяТипа Из ирОбщий.СтрРазделитьЛкс(мИмяОжидаемогоТипа, ",", Истина) Цикл
		...
		Иначе
			Если Истина
				...
				И Не ирОбщий.СтрокиРавныЛкс(ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(ВыражениеПриемник), мРодительскийКонтекст)
			Тогда
				КлючПоиска = Новый Структура;
				КлючПоиска.Вставить("НСлово", НРег(ирОбщий.ПоследнийФрагментЛкс(ВыражениеПриемник))); // НСлово может отсутствовать
...
Процедура ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора) Экспорт
	...
	Если КлючПоискаБезТипа.Количество() > 1 Тогда
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Найденные = НаборСлов.НайтиСтроки(КлючПоискаБезТипа);
		Если Истина
			И Найденные.Количество() = 0 
			И КлючПоиска.Свойство("ТипЗначения") 
			И Найти(КлючПоиска.ТипЗначения, ".") = 0
		Тогда
			КлючПоискаПеречисления = ирОбщий.СкопироватьКоллекциюЛкс(КлючПоиска);
			КлючПоискаПеречисления.ТипЗначения = "Перечисление" + КлючПоиска.ТипЗначения;
		...
		Если Найденные.Количество() > 0 Тогда
			Если КлючПоиска.Количество() > 1 Тогда
				...
				Если Не ирОбщий.СтрНайтиЛкс(Найденные[0].ТипЗначения + ",", КлючПоиска.ТипЗначения + ",") Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		...
		Если СтрокаОписанияМетода.ТипСлова <> "Свойство" Тогда
			...
			Если СтрокаОписанияМетода.ТипКонтекста = "ПолеКомпоновкиДанных" Тогда
				...
				Если КомпоновщикНастроек <> Неопределено Тогда
					КоллекцияЭлементовМД = КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.Элементы;
				ИначеЕсли ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = ирОбщий.ТипУправляемаяФормаЛкс() Тогда
					...
					Если ирОбщий.СтрКончаетсяНаЛкс(ВыражениеСлеваОтРавно, ".Поле") Тогда
						...
						КонецЦикла;
					Иначе
						КоллекцияЭлементовМД = СписокРеквизитовУправляемойФормы(ВыбраннаяСтруктураТипа.Метаданные);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяПеременнойСтруктуры) Тогда
				...
				Для Каждого Вхождение Из Вхождения Цикл
					...
					Если ТаблицаТипов.Количество() > 0 Тогда
						мТаблицаСловЛитерала = мПлатформа.СловаКонтекстаМетаданные(ТаблицаТипов[0],, "Свойство");
					КонецЕсли;
						...
						Если ТаблицаТипов.Количество() > 0 Тогда
							...
							ИначеЕсли ТипЗнч(МетаданныеТипа) = Тип("ТаблицаФормы") Тогда
								...
								Если ТаблицаТиповДочерняя.Количество() > 0 Тогда
									мТаблицаСловЛитерала = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповДочерняя[0],,,, Ложь,, "Свойство");
								КонецЕсли;
							КонецЕсли;
						КонецЕсли;
						Перейти ~КонецПоискаСтруктур;
		...
		Если Прав(ВыражениеСлеваОтРавно, 1) = "=" Тогда
			...
			Если ирОбщий.СтрНачинаетсяСЛкс(ирОбщий.ПоследнийФрагментЛкс(ВыражениеСлеваОтРавно), "ПутьКДанным") Тогда
				...
				Тогда
					...
					Тогда
						КоллекцияЭлементовМД = СписокРеквизитовУправляемойФормы(ТаблицаТиповСлева[0].ДержательМетаданных);
						КонтекстТекстовогоЛитералаИзвестен = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Тогда
				КоллекцияЭлементовМД = ВычислитьВозможныеТекстовыеЗначенияПараметра(ВыражениеСлеваОтРавно, КонтекстТекстовогоЛитералаИзвестен, Истина);
				ДобавитьПростыеСлова(КоллекцияЭлементовМД, Истина);
...
Функция СписокРеквизитовУправляемойФормы(Знач Форма) Экспорт
	КоллекцияЭлементовМД = Новый ТаблицаЗначений;
	Для Каждого КлючИЗначение Из мПлатформа.ИменаРеквизитовФормы(Форма).Все Цикл
...
//   - 
//
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен, Знач ЛиСобиратьПрисвоения = Ложь) Экспорт
	ИмяШаблон = ирОбщий.ТекстДляРегВыраженияЛкс(ИмяПараметра);
	...
	Тогда
		ВхожденияОбъектов = НайтиВхожденияРегВыраженияКэш("(" + шОбъектноеВыражение + ")\[" + ИмяШаблон + "\]",,, "ОбъектСВКвадратнымиСкобками;" + ИмяПараметра);
...
Процедура ДобавитьПростыеСлова(Знач КоллекцияЭлементовМД, Знач ЛиПриоритетно = Ложь, Знач Определение = "Метаданные")
	...
	Если КоллекцияЭлементовМД = мТаблицаСловЛитерала Тогда
		...
		ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(КоллекцияЭлементовМД, ТаблицаСлов);
	Иначе
...
//  Строка - 
//
Функция ИмяПараметраВызоваМетода(СтрокаОписания, Знач СТипомСтрока = Истина, выхТаблицаТипов = Неопределено, Знач СтруктураТипаМетода = Неопределено) Экспорт
	ИмяПараметра = "";
		...
		Для Каждого СтрокаПараметра Из СтрокиПараметров Цикл
			...
			Тогда
				...
				Если Не СТипомСтрока Тогда
					...
					КонецЕсли;
					выхТаблицаТипов = ТаблицаТиповИзТекста(ИмяТипаЗначения);
				КонецЕсли;
				Прервать;
			КонецЕсли; 
		КонецЦикла;
	...
	Если НужноУточнитьТип Или ВернутьСтруктуруТипа Тогда
		...
		Иначе 
			...
			Если ЗначениеЗаполнено(РодительИСлово.Родитель) Тогда
				ТипыКонтекста = ирОбщий.РазличныеЗначенияМассиваЛкс(РодительИСлово.СтруктураТипаКонтекста);
			Иначе
...
Функция ВычислитьПоместитьВоВременноеХранилище(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	...
	ТаблицаТипов[0].Детальность = Неопределено;
	Возврат ТаблицаТипов;
		...
		Если ЛиОтдельноеОписаниеАктивно И ТаблицаТипов <> Неопределено Тогда 
			...
			Тогда
				...
				Если ЛучшийТип.СтрокаОписания <> Неопределено Тогда
					...
					Иначе
						СтрокаПеременной = ЛучшийТип.СтрокаОписания; // см. мПлатформа.НоваяТаблицаМетодовМодуля()[0]
						Если Истина
							И ТипЗнч(СтрокаПеременной) = Тип("СтрокаТаблицыЗначений")
							И СтрокаПеременной.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено
						Тогда
							Если ЗначениеЗаполнено(СтрокаПеременной.ТипЗначения) Тогда
								СтруктураОписания.Добавить("> Тип: " + СтрокаПеременной.ТипЗначения);
							КонецЕсли;
