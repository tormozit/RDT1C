...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Тогда
		...
		Для Каждого Директива Из ДирективыКомпиляцииТипаМодуля Цикл
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "&" + Директива, "Конструкция",, "", "Предопределенный",,, Истина);
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "") Экспорт
	...
	Если КлючПоиска.Свойство("ТипЗначения") Тогда
		...
		КонецЕсли;
		ИмяТипа = КлючПоиска.ТипЗначения;
...
Функция СобратьКомментарийМетода(ОписаниеМетода = "", Знач СтрокаРезультата = Неопределено, ЗначенияПараметров = Неопределено) Экспорт 
	...
	Для Каждого Параметр Из Параметры Цикл
		...
		КонецЕсли;
		ирОбщий.ДобавитьМногострочнуюСтрокуВТекстЛкс(ТипЗначения, Комментарий, "//");
		КомментарийМетода.Добавить(ПрефиксОписанияПараметра + Параметр.Имя + " - " + ТипЗначения);
