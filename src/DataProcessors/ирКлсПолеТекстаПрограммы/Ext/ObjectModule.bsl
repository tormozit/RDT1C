...
Функция ЗагрузитьМетодМодуляПоПозиции(Знач ПозицияВТексте) Экспорт 
	...
	Если мМодульМетаданных <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если НовыйМетодМодуля = мМетодМодуля Тогда
		//Возврат Ложь;
		Возврат НовыйМетодМодуля = Неопределено;
	КонецЕсли;
		...
		Если ВхожденияПараметров.Количество() > ИндексПараметра Тогда
			...
			Иначе
				...
				Если ПройденныеВызовы[КлючВызова] = Неопределено Тогда
					...
					ИмяВиртуальнойПеременной = "ИсходящийПараметрЗщшм4е";
					ТаблицаТиповИсточник = ПолеТекстаПрограммыВызова.ВычислитьТипЗначенияВыражения(ИмяВиртуальнойПеременной, 
						" " + ПолеТекстаПрограммыВызова.мТекстДляПоискаОпределения + ";" + Символы.ПС + ИмяВиртуальнойПеременной + "=" + ФактическийПараметр + ";",
						ПолеТекстаПрограммыВызова.мПредшествующийТекст,,,, Ложь,,,, Истина, ВхождениеПараметра.ПозицияВхождения - 1 - НачальнаяПозицияБлока);
...
Функция ВычислитьТипФункцииСФактПараметрами(Знач МетодМодуля, Знач ТекущиеАргументы, Знач ПредшествующийТекст, Знач ПолныйАнализСоставаТипов, Знач ПараметрыИсходящегоВызова,
	...
	Для Каждого КлючИЗначение Из МетодМодуля.КлючевыеПараметры Цикл 
		...
		Если Не ЗначениеЗаполнено(ВыражениеПараметра) Тогда
			Продолжить;
		Иначе 
			ИмяПеременной = "Переменная99а836";
			ПредшествующийТекст = ПредшествующийТекст + Символы.ПС + ИмяПеременной + "=" + ВыражениеПараметра; // Так нужно делать для выражений вида "Ф = Новый Структура" TODO
			ЗначениеПараметра = ВычислитьТипЗначенияВыражения(ИмяПеременной, ПредшествующийТекст,,,, ПолныйАнализСоставаТипов, Ложь,,, Истина, Истина);
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Знач ТолькоПрисвоение = Ложь, Знач ПозицияВМетодеДляКэша = 0) Экспорт
	...
	Если ЛиКорневойВызов Тогда
		...
		мРекурсивныйПуть.Вставить("Выражения", Новый Структура);
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда 
			...
			Если Истина
				...
				И мМетодМодуля <> Неопределено 
			Тогда 
				ВыраженияМетода = Неопределено;
				Если Не мРекурсивныйПуть.Выражения.Свойство(мМетодМодуля.Имя, ВыраженияМетода) Тогда 
					ВыраженияМетода = Новый Соответствие;
					мРекурсивныйПуть.Выражения.Вставить(мМетодМодуля.Имя, ВыраженияМетода);
				КонецЕсли;
				КлючВыражения = XMLСтрока(ПозицияВМетодеДляКэша) + ";" + Выражение;
				ТаблицаТиповКэш = ВыраженияМетода[КлючВыражения];
			...
			Тогда
				...
				Если ЛиПеременнаяМодуля Тогда 
					КлючВыражения = "";
					Если Не (ГлобальныйПоискВМодуле И ЛиКорневойВызов) Тогда
						Возврат ТаблицаТипов;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ЭтоПрисвоение Тогда
								...
								Иначе//Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(7)) Тогда 
									ТекстДоВхождения = Сред(ТекстДляПоискаОпределения, НачальнаяПозицияБлока, ПозицияВхождения0); // МультиМетка8294218
									МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов,
							...
							Иначе
								...
								ТекстДоВхождения = Сред(ТекстДляПоискаОпределения, НачальнаяПозицияБлока, ПозицияВхождения0); // МультиМетка8294218
								ТаблицаТиповКоллекции = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения,,, ПолныйАнализСоставаТипов, Ложь,,,,, ПозицияВхождения0);
			...
			Тогда
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						СобратьТипыСвойствСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ТекстДляПоискаОпределения, ПредшествующийТекст, ГлобальныйПоискВМодуле);
					...
					Тогда
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							ТаблицаТиповЭлементов = ВычислитьТипЗначенияВыражения(ВхождениеСвойства.SubMatches(0), ТекстДоВхождения, ПредшествующийТекст, РазрешитьАнализИмениТипа,
								ЭтоВызовКонструктора, ПолныйАнализСоставаТипов, Ложь, ГлобальныйПоискВМодуле);
					...
					Тогда
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							Иначе
								ТекстДоВхождения = ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, ВхождениеСвойства, ТекстДляПоискаОпределения);
								ТекстовоеЗначение = ВычислитьЗначениеВыражения(ПрисвоенноеВыражение, ТекстДоВхождения);
							КонецЕсли;
			...
			Тогда 
				...
				КонецЕсли;
			КонецЕсли;
			Если ЗначениеЗаполнено(КлючВыражения) Тогда
				ВыраженияМетода.Вставить(КлючВыражения, ТаблицаТипов);
			КонецЕсли;
		КонецЕсли; 
	КонецЕсли;
	Если ТаблицаТипов.Количество() = 0 Тогда
...
Функция СобратьТипыСвойствСтруктуры(Знач Выражение, Структура = Неопределено, Знач ТекстДляПоискаОпределения, Знач ПредшествующийТекст, Знач ГлобальныйПоискВМодуле = Ложь)
	мРегВыражение.Global = Ложь;
	...
	Для Каждого Вхождение Из РезультатСвойств Цикл
		ВыражениеИмени = Вхождение.SubMatches(0);
		ТекстДоВхождения = Неопределено;
		Если Лев(ВыражениеИмени, 1) = """" Тогда
			ТекстовоеЗначение = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ВыражениеИмени);
		Иначе
			ТекстДоВхождения = ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, Вхождение, ТекстДляПоискаОпределения);
			ТекстовоеЗначение = ВычислитьЗначениеВыражения(ВыражениеИмени, ТекстДоВхождения);
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда 
			ВыражениеЗначения = Вхождение.SubMatches(1);
			Если ТекстДоВхождения = Неопределено Тогда
				ТекстДоВхождения = ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, Вхождение, ТекстДляПоискаОпределения);
			КонецЕсли;
			ТаблицаТиповЗначения = ВычислитьТипЗначенияВыражения(ВыражениеЗначения, ТекстДоВхождения, ПредшествующийТекст,,,, Ложь);
			...
			Если Не ЛиДетальностьТиповДостаточна(ТаблицаТиповЗначения, МинДетальность) Тогда
				СтруктураУточнения.Вставить(ТекстовоеЗначение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если Не ГлобальныйПоискВМодуле Тогда // Для сокращения редко полезных затратных вычислений
		Для Каждого КлючИЗначение Из СтруктураУточнения Цикл
			...
			ТаблицаТиповЗначения = ВычислитьТипЗначенияВыражения(ВыражениеЗначения, ТекстДляПоискаОпределения, ПредшествующийТекст,,,, Ложь, ГлобальныйПоискВМодуле, Истина);
...
// Возвращаемое значение:
//    Строка - 
Функция ВычислитьЗначениеВыражения(Выражение, Знач ПредшествующийТекст, Знач ГлобальныйПоискВМодуле = Ложь)
	Если ПустаяСтрока(Выражение) Тогда
		Возврат "";
	КонецЕсли;
	ТаблицаТипов = ВычислитьТипЗначенияВыражения(Выражение, ПредшествующийТекст, ПредшествующийТекст,,,, Ложь, ГлобальныйПоискВМодуле, Найти(Выражение, "(") = 0, Истина);
	СтруктураТипа = ТаблицаТипов[0];
...
Функция ВычислитьНайтиЭлементКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ИмяПеременной = "йихкооя294";
	Результат = ВычислитьТипЗначенияВыражения(ИмяПеременной + "[0]", ПредшествующийТекст + ";
	|" + ИмяПеременной + "=" + МассивПараметров[0],,,,, Ложь,,,, Истина);
	Возврат Результат;
...
Функция ВычислитьОтобратьКоллекциюЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	КонецЕсли;
	ИмяПеременной = "йихкооя294";
	ТаблицаТиповЭлементов = ВычислитьТипЗначенияВыражения(ИмяПеременной + "[0]", ПредшествующийТекст + ";
	|" + ИмяПеременной + "=" + МассивПараметров[0],,,,, Ложь,,,, Истина);  
