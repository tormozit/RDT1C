...
//
Функция ОткрытьКонструкторМетода(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	Перем НачСтрока, НачКолонка, КонСтрока, КонКолонка;
	...
	Если мМетодМодуля <> Неопределено И Не ЗначениеЗаполнено(ВыделенныйТекст()) Тогда 
		...
		ВыделенныйТекст = ВыделенныйТекст();
		ДлинаПустоты = СтрДлина(ВыделенныйТекст) - СтрДлина(СокрЛ(ВыделенныйТекст));
		ФормаНовогоМетода.ПолноеОпределение = Лев(ВыделенныйТекст, ДлинаПустоты) + ФормаНовогоМетода.ПолноеОпределение; 
	...
	Иначе
		...
		Если мСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа" Тогда
			Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
				...
				Если Истина
					...
					И Не ЗначениеЗаполнено(мРодительскийКонтекст) 
				Тогда
					мСтрокаЛучшегоСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, мИмяОжидаемогоТипа, "Конструкция",, "Тип", "Локальный", Ложь);
		...
		Иначе 
			...
			Тогда
				...
				Тогда
					СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЗНАЧЕНИЕ(" + мИмяОжидаемогоТипа + ".ПустаяСсылка)", "Конструкция",, мИмяОжидаемогоТипа, "Локальный", Ложь);
					СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
					СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
				КонецЕсли;
				Если мЯзыкПрограммы = 1 Тогда
					...
					Если ЗначениеЗаполнено(Вхождения) Тогда
						...
						Для Индекс = 1 - ПоследняяВыбраннаяТаблица.Количество() По 0 Цикл // Обратный обход
							...
							ИмяПравойТаблицы = ПоследнееВхождение.Группа1;
							...
							ТаблицаДочернихСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповВыбраннойТаблицы[0], мЯзыкПрограммы);
							...
							Для Каждого СтрокаДочерняя Из НайденныеДочерние Цикл
								СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, ИмяПравойТаблицы + "." + СтрокаДочерняя.Слово, "Конструкция",, мИмяОжидаемогоТипа, "Локальный", Ложь);
								СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
								СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
							КонецЦикла;
						КонецЦикла;
...
Функция ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "") Экспорт
...
Функция СобратьКомментарийМетода(Знач ОписаниеМетода = "", Знач СтрокаРезультата = Неопределено, Знач ЗначенияПараметров = Неопределено, Знач ШиринаТекста = 180) Экспорт 
...
Функция СобратьКомментарийПараметра(Знач Параметр, Знач ШиринаТекста, Знач ПрефиксОписанияПараметра, Знач ПрефиксОписанияПараметраПродолжение, Знач ДобавлятьИмя = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(Комментарий) Тогда
		...
		ирОбщий.ДобавитьМногострочнуюСтрокуВТекстЛкс(Результат, Комментарий, ПрефиксОписанияПараметраПродолжение, СНовойСтроки);
	КонецЕсли;
	Возврат Результат;
КонецФункции
