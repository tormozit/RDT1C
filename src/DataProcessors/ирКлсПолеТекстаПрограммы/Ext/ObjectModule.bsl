...
Функция НачалоЧастиОбъединенияЗапроса(Знач ТекстДляПоискаОпределения, Знач Позиция0ВМетодеДо) Экспорт
	...
	ВхожденияЧастей = мРегВыражение.НайтиВхождения(Лев(ТекстДляПоискаОпределения, Позиция0ВМетодеДо + 1),, Истина);
	НачалоЧасти = 0;
...
Функция ЗаполнитьТаблицуВозможныхЗначенийЛитерала(Знач ВыбраннаяСтруктураТипа, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	...
	Тогда
		...
		Тогда
			мТаблицаСловЛитерала = МетаданныеОбъекта.Поля;
		ИначеЕсли Истина
			И СтрокаОписанияМетода.Слово = "Найти"
...
Функция ОткрытьДеревоСвязейПоля(Знач ЧтоИскать = Неопределено, Знач Автозапуск = Истина) Экспорт 
	...
	Если ирОбщий.ЛиИмяПеременнойЛкс(ТекущееВыражение) Тогда
		ПодготовитьПакетЗапросов(мПакетЗапросов);
	...
	Если Не ВыражениеПодходит Тогда
		ирОбщий.СообщитьЛкс("Команду нужно вызывать на поле временной таблицы");
		Возврат Ложь;
	КонецЕсли;
...
Функция ВычислитьТаблицаЗначенийВыгрузитьКолонку(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	ТаблицаТипов.ЗагрузитьКолонку(ТаблицаТипов.ВыгрузитьКолонку("Метаданные"), "ДержательМетаданных");
...
// см. ВычислитьРезультатМетодаШаблон 
Функция ВычислитьТаблицаЗначенийВМассивБСП(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() < 1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	СтруктураТипа = мПлатформа.НоваяСтруктураТипа("Массив");
			...
			Тогда
				...
				Если ЛучшийТип.СтрокаОписания <> Неопределено Тогда
					...
					Тогда 
						...
						Если НайденныеПоляБД = Неопределено Тогда
							// Виртуальное поле
						Иначе
...
Процедура УстановитьТекст(Знач Текст = Неопределено, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач ИмяМодуляСжатое = Неопределено,
	...
	Иначе 
		...
		Если ОбъектМД = Неопределено Тогда 
			...
			Иначе
				//СтруктураТипа.Метаданные = ИмяМодуля; // Так в модуле приложения будет пустым список подсказки после Справочники.
