...
Перем шПостИмяНепрямое Экспорт;
...
Функция ЗагрузитьЗапросПакетаМодуляПоПозиции(Знач ТекстПакетаЗапросов, НомерПервойСтрокиБлока = Неопределено, Позиция0ВЛитерале = Неопределено) Экспорт
	...
	Если Позиция0ВЛитерале <> Неопределено И мИндексЗапросаПакета <> Неопределено Тогда
		...
		мПозицияТекстаДляПоискаОпределения = мПакетЗапросов.ПозицииЗапросов[мИндексЗапросаПакета];
		Если Истина
...
//.
// Параметры:
//    ВыражениеЛитерала - Строка - 
Процедура РазобратьКонтекстЗапросаВТекстовомЛитерале(Знач ВыражениеЛитерала, Знач ТекстЛитерала) Экспорт
	...
	мПакетЗапросов.ТекстыЗапросов = мПлатформа.РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(ТекстЛитерала, мПозицияВТексте - мНачальнаяПозиция0ВложенногоТекста, мПакетЗапросов.ПозицииЗапросов);
КонецПроцедуры
	...
	ИначеЕсли мЯзыкПрограммы = 0 Тогда 
		...
		Если ТаблицаТипов <> Неопределено Тогда
			Для Каждого СтруктураТипа Из ТаблицаТипов Цикл
				ДобавитьПереходыИзСтруктурыТипаВСписокВыбора(СтруктураТипа, Результат, ТаблицаТиповРодителя[0]);
			КонецЦикла;
		КонецЕсли;
		Если Ложь
...
// Параметры:
//    МетодМодуля - ? - 
Процедура ПерейтиКОпределениюЛокальногоМетода(МетодМодуля, Знач ИмяПараметра = "", Знач СмещениеСтрокиМетода = 0, Знач НомерКолонки = 0) Экспорт
	Если МетодМодуля.ПозицияТела = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПредставлениеЯзыка() Экспорт 
...
Функция ШаблонПрямогоВызоваСлова(Знач ИмяВызова, Знач ТипСлова = "Метод") Экспорт 
	...
	Если ТипСлова = "Свойство" Тогда
		ШаблонВызова = ШаблонВызова + шПостИмяНепрямое; 
	Иначе 
...
Функция МодульМетаданныхКонтекста(Знач РодительскаяСтрутураТипа)
	...
	Если Ложь
		Или РодительскаяСтрутураТипа.ИмяОбщегоТипа = "Локальный" 
		Или ЗначениеЗаполнено(РодительскаяСтрутураТипа.ТипЯзыка) 
	Тогда
	...
	Если СобиратьОшибки Тогда 
		...
		Тогда 
			...
			Тогда 
				...
				ОшибкаМодуля.Язык = ПредставлениеЯзыка();
			...
			Иначе 
				...
				Для Счетчик = 2 По ТекстОписания.КоличествоСтрок() Цикл
					...
					Если Истина
						...
						И Не Лев(СтрокаОписания, 2) = "**" // TODO добавить обработку второго уровня вложенности
					Тогда
...
Функция ОткрытьПоискВызововСлова(Знач Слово = Неопределено, Знач ИскатьНепрямые = Истина, Знач Автозапуск = Истина) Экспорт 
	...
	Если Не ЗначениеЗаполнено(Слово) Тогда
		Возврат Ложь;
	КонецЕсли;
	ТаблицаТиповКонтекста = ВычислитьТипЗначенияВыражения(Слово, " " + мТекстДляПоискаОпределения, мПредшествующийТекст, Истина);
	Если ТаблицаТиповКонтекста.Количество() > 0 Тогда
		СтруктураТипаКонтекста = ТаблицаТиповКонтекста[0];
	Иначе 
		СтруктураТипаКонтекста = мПлатформа.НоваяСтруктураТипа();
	...
	Иначе
		Если СтруктураТипаРодителя <> Неопределено Тогда 
			...
			//СтруктураТипаКонтекста = Неопределено;
		Иначе
			Если ирОбщий.МножественноеИмяМДЛкс(ирОбщий.ПервыйФрагментЛкс(Слово)) <> Неопределено Тогда
				ПолноеИмяСлова = ирОбщий.ПервыеФрагментыЛкс(Слово,, Макс(2, СтрЧислоВхождений(Слово, ".") - 1)) + ".";
			КонецЕсли;
			Если ИскатьНепрямые = Неопределено Тогда
				Если Истина
		...
		КонецЕсли;
	КонецЕсли;
	ПолноеИмяСлова = ПолноеИмяСлова + ирОбщий.ПоследнийФрагментЛкс(Слово);
	ПоискВызововМетода = ПолучитьФорму("СсылкиНаСлово",, ПолноеИмяСлова);
...
Процедура ПроверитьВыраженияПослеТочки(ТабличноеПоле = Неопределено) Экспорт
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если ВыражениеЛитерала <> Неопределено Тогда 
			...
			Тогда
				...
				Если ирОбщий.ЛиТекстЯзыкаЗапросовЛкс(ТекстЛитерала) Тогда
					...
					Для Каждого ВхождениеЗапроса Из ВхожденияЗапроса Цикл
						...
						ЗагрузитьЗапросПакетаМодуляПоПозиции(ВыражениеЛитерала,, ПозицияВхождения0);
						мТекстДляПоискаОпределения = мТекстБлока;
			...
			Продолжить;
		КонецЕсли;
		Выражение = Вхождение.SubMatches(2);
		Если Выражение = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		мЭтоТекстовыйЛитерал = Ложь; 
		...
		//РазобратьТекущийКонтекст(,,,,, Истина, ПозицияВхождения0); // Много накладных расходов
...
шВызовМетодаПрограммы = "(?:" + шПредИмяПрямое + "(Новый|New)\s+)?(" + шОбъектноеВыражение + "?)"
