...
Процедура ОбновитьСтатистикуВыбораВСтрокеСлова(Знач СтрокаСлова)
	...
	//	Возврат;
	//КонецЕсли;
	ТипВладелец = Неопределено; // см. ОбработкаОбъект.ирПлатформа.НоваяСтруктураТипа
	ТаблицаТиповСлова = УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаСлова, Истина,, ТипВладелец);
...
Функция ТипКонтекстаСтатистики(Знач СтруктураТипаКонтекста = Неопределено) Экспорт
	...
	ИначеЕсли Истина
		...
		И Найти(КонкретныйТипКонтекста, ".") > 0
	Тогда
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(Знач ТекущаяСтрока, Знач ВернутьСтруктуруТипа = Ложь, Знач НужноУточнитьТип = Ложь, выхТипВладелец = Неопределено) Экспорт 
	#Если Сервер И Не Сервер Тогда
	...
	Если НужноУточнитьТип Или ВернутьСтруктуруТипа Тогда
		#Если Сервер И Не Сервер Тогда
			мПлатформа = Обработки.ирПлатформа.Создать();
		#КонецЕсли
		выхТипВладелец = мСтруктураТипаКонтекста;
		...
		Иначе 
			...
			Для Каждого СтрокаТипаКонтекста Из ТипыКонтекста Цикл
				...
				Если ОписанияСлов.Количество() > 0 Тогда
					...
					выхТипВладелец = СтрокаТипаКонтекста;
					Прервать;
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли; 
		Если ЗначениеЗаполнено(ТаблицаТипов) Тогда
			...
			Если ТипЗнч(СтрокаОписания) = Тип("СтрокаТаблицыЗначений") Тогда
				...
				Если Истина
					И мЭтоЛокальныйКонтекстТаблицыСлов
					И КолонкиТаблицы.Найти("Слово") <> Неопределено
					И СтрокаОписания.ТипКонтекста = "Глобальный"
				Тогда
					выхТипВладелец = НоваяСтруктураТипа("Глобальный");
				ИначеЕсли Истина
					И ирОбщий.ЛиИмяТипаФормыЛкс(выхТипВладелец.ИмяОбщегоТипа, Истина, Ложь)
					И КолонкиТаблицы.Найти("ИмяМодуля") <> Неопределено
					И Найти(СтрокаОписания.ИмяМодуля, "МодульОбъекта")
				Тогда  
					выхТипВладелец = мПлатформа.МодульМетаданныхИзКэша(СтрокаОписания.ИмяМодуля).СтруктураТипа;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли; 
...
Функция ОписаниеТекущегоСловаАвтодополнения(Знач Слово, Знач ЛиМетод = Неопределено, Знач КлючНабораСлов = "", Знач ЛиОтдельноеОписаниеАктивно = Истина, Знач ТолькоДляАвтооткрытия = Ложь) Экспорт 
	...
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		ТипВладелец = Неопределено; // см. ОбработкаОбъект.ирПлатформа.НоваяСтруктураТипа
		ТаблицаТипов = УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаТаблицыСлов, ЛиОтдельноеОписаниеАктивно,, ТипВладелец);
			...
			Если ЛучшийТип <> Неопределено Тогда  
				...
				Тогда
					Если Истина
						...
						И СтрокаТаблицыСлов.ТипЗначения <> СтрокаОписания.ТипЗначения
				...
				Тогда
					СтрокаПараметра = СтрокаПараметраМетодаМодуля(СтрокаОписания.Имя);
