...
Перем мПозицииЗапросовПакета Экспорт;
...
Процедура РазобратьТекущийКонтекст(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Неопределено, Знач НомерСтроки = 0, Знач НомерКолонки = 0,
	...
	Иначе
		мПозицииЗапросовПакета = Новый Массив;
		мЗапросыПакета = ирОбщий.РазбитьГрубоТекстПакетногоЗапросаНаЗапросыЛкс(ТекстПакетаЗапросов, мПозицияВТексте, мПозицииЗапросовПакета);
		#Если Сервер И Не Сервер Тогда
			мЗапросыПакета = Новый Массив;
		#КонецЕсли
		Для ИндексЗапроса = 0 По мЗапросыПакета.ВГраница() Цикл
			мТекстБлока = мЗапросыПакета[ИндексЗапроса];
			мПозицияТекстаДляПоискаОпределения = мПозицииЗапросовПакета[ИндексЗапроса];
...
Функция ДобавитьПереходыИзСтруктурыТипаВСписокВыбора(Знач СтруктураТипа, Знач СписокВыбора = Неопределено) 
	...
	Если ТипЗнч(СтруктураТипа.ДержательМетаданных) = Тип("Структура") Тогда 
		КоординатыТекстаЗапроса = СтруктураТипа.ДержательМетаданных; // Структура
		АнализаторКода = КопияКомпоненты();
		АнализаторКода.мЯзыкПрограммы = 1; 
		АнализаторКода.мМодульМетаданных = СтруктураТипа.ДержательМетаданных.Модуль;
		...
		Если КоординатыТекстаЗапроса.Свойство("ИндексВПакете") Тогда
			ИндексВПакете = КоординатыТекстаЗапроса.ИндексВПакете;
			ПозицииЗапросов = Новый Массив;
			ЗапросыПакета = мПлатформа.РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(АнализаторКода.мТекстДляПоискаОпределения,, ПозицииЗапросов);
			НачальнаяПозицияПоиска = НачальнаяПозицияПоиска + ПозицииЗапросов[ИндексВПакете];
			АнализаторКода.мТекстДляПоискаОпределения = Сред(АнализаторКода.мТекстБлока, НачальнаяПозицияПоиска, СтрДлина(ЗапросыПакета[ИндексВПакете]));
		КонецЕсли;
		ПоследнееВхождение = Неопределено;
		...
		Если ПоследнееВхождение <> Неопределено Тогда
			...
			Иначе 
				...
				ИмяМетода = "";
			КонецЕсли;
			КлючПерехода = ирОбщий.СсылкаСтрокиМодуляЛкс(СтруктураТипа.ДержательМетаданных.Модуль.Имя,, ИмяМетода, НомерСтрокиМетода,, НомерКолонкиМетода);
	...
	Тогда
		КлючПерехода = ирОбщий.ПолноеИмяФормыЛкс(СтруктураТипа.Метаданные);
		Если СписокВыбора.НайтиПоЗначению(КлючПерехода) = Неопределено Тогда
	...
	Если мЯзыкПрограммы = 1 Тогда 
		...
		Если ФрагментыСлова.Количество() < 3 Тогда
			...
			Если РезультатТекста.Количество() > 0 Тогда
				...
				КонецЕсли;
				ИндексЗапроса = мСтруктураПакетаЗапросов.Найти(НРег(ПоследнееОпределение));
				Если ИндексЗапроса <> Неопределено Тогда
					Если ФрагментыСлова.Количество() = 2 Тогда
						ЗагрузитьЗапросПакетаПоИндексу(ИндексЗапроса);
					...
					Иначе 
						...
						Если Истина
							// Если найдено имя доступной таблицы
...
Процедура ЗагрузитьЗапросПакетаПоИндексу(Знач ИндексЗапроса) Экспорт
	мТекстБлока = мЗапросыПакета[ИндексЗапроса];
	мТекстДляПоискаОпределения = мТекстБлока;
	мПозицияТекстаДляПоискаОпределения = мНачальнаяПозиция0ВложенногоТекста + мПозицииЗапросовПакета[ИндексЗапроса];
КонецПроцедуры
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	ИначеЕсли Лев(Выражение, 1) = """" Тогда
		...
		Если ВычислитьЗначение Тогда 
			...
			Если СтрДлина(СтруктураТипа.Метаданные) > 50 Тогда
				ЗаполнитьПозициюТекстовогоЛитерала(СтруктураТипа, Выражение, Позиция0ВМетодеДо);
			КонецЕсли;
		КонецЕсли;
		...
		Если ВычислениеРазрешено Тогда 
			...
			Тогда
				...
				Если ТаблицаСтруктурРодителей.Количество() > 0 Тогда
					...
					ВычислитьТипДочернегоЭлемента(ТаблицаСтруктурРодителей, ТекущееСлово, ТипСлова, ТекущийИндекс, ТекущиеАргументы, ПредшествующийТекст, ПолныйАнализСоставаТипов, ЛиПеременнаяМодуля,
										...
										ИначеЕсли ТипНового <> Неопределено Тогда
											...
											Тогда 
												...
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПервыйФактПараметр) Тогда
													ТекстЗапроса = ПервыйФактПараметр;
													ЗаполнитьПозициюТекстовогоЛитерала(СтруктураТипа, ТекстЗапроса, ПоследнееВхождение.FirstIndex + Найти(ПоследнееВхождение.Value, """"));
								...
								Иначе //Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(8)) Тогда ;
									...
									Если ПрименитьТип Тогда 
										...
										Иначе                   
											...
											СмещениеПозиции = 0;
										КонецЕсли;
										МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст,,,,, ПолныйАнализСоставаТипов,
											Ложь,,, ВычислитьЗначение,, ПозицияВхождения0 + СмещениеПозиции);
										мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, МассивСтруктурПрисвоенныхТипов);
									КонецЕсли;
								КонецЕсли;
							Иначе
			...
			Тогда     
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						КоординатыТекста = Новый Структура;
						ТекстЗапроса = ВычислитьЗначениеВыражения(Выражение + ".Текст", ТекстДляПоискаОпределения, ГлобальныйПоискВМодуле, Позиция0ВМетодеОт, Позиция0ВМетодеДо, КоординатыТекста);
						Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
							ПостроительЗапроса = ПостроительЗапросаИзТекстаДляТипа(ТекстЗапроса);
							СтруктураТипаЦикл.Метаданные = ПостроительЗапроса;
							СтруктураТипаЦикл.ДержательМетаданных = КоординатыТекста;
							ирОбщий.СкопироватьКоллекциюЛкс(СобратьТипыСвойствСтруктуры(Выражение + ".Параметры",,,, Позиция0ВМетодеОт, Позиция0ВМетодеДо), ПостроительЗапроса.Параметры);
						КонецЕсли; 
						мПлатформа.ОбновитьДетальностьСтруктурыТипа(СтруктураТипаЦикл);
					КонецЕсли; 
				КонецЦикла;
...
Процедура ЗаполнитьПозициюТекстовогоЛитерала(Знач СтруктураТипа, Знач Выражение, Знач Позиция0ВМетодеДо) Экспорт
	...
	СтруктураТипа.ДержательМетаданных = СтруктураСтрокиМодуля;
КонецПроцедуры
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьПрочитатьЗначениеJSON(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьФабрикаXDTOПрочитатьJSON(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьФабрикаXDTOПрочитатьXML(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
Функция УстановитьТекст(Знач Текст = Неопределено, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	КонецЕсли; 
	мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипа, ТипМодуля,, Текст, мМетодМодуля, мПозицияВТексте);
	мФлагиКомпиляции = ирОбщий.СкопироватьКоллекциюЛкс(мМодульМетаданных.ФлагиКомпиляции);
...
Процедура ОбновитьМодульМетаданных(Знач Текст = Неопределено) 
	...
	Если мМодульМетаданных <> Неопределено И мМодульМетаданных.СтруктураТипа <> Неопределено Тогда
		мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(мМодульМетаданных.СтруктураТипа, ирОбщий.ПоследнийФрагментЛкс(мИмяМодуля),, Текст, мМетодМодуля, мПозицияВТексте);
	Иначе
		мМодульМетаданных = мПлатформа.МодульМетаданных(Текст,, "Динамический.Модуль");
		УстановитьФлагиКомпиляцииМодуля();
	КонецЕсли;           
	Если Истина
