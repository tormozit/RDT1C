							...
							Если СледРекурсии = Неопределено Тогда
								...
								Если ПрисвоенныйКонтекст <> Выражение Тогда
									ТаблицаПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДляПоискаОпределения,
										Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1),,, ПолныйАнализСоставаТипов, Ложь,,,,, ПозицияВхождения0,,, Истина);
...
Функция ЛиТаблицаТиповКэшНеГотова(ТаблицаТиповКэш, ДляСвойства) Экспорт
	...
	Тогда 
		...
		ТаблицаТиповКэш[0].Метаданные.Свойство(ирОбщий.ПервыйФрагментЛкс(ДляСвойства), ТаблицаТиповСвойства);
...
Функция ЗаполнитьТаблицуСловДляТекстовогоЛитерала(ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен = Истина, ИмяПараметра = "") Экспорт
	...
	Если Истина
		...
		И СтрокаОписанияМетода.ТипСлова = "Метод"
	Тогда
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен) Экспорт
	...
	Для Каждого Вхождение Из ВхожденияВызовов Цикл
		...
		Если ТаблицаТипов.Количество() > 0 Тогда
			...
			КонецЕсли;
			ПозицияВТексте = мПозицияТекстаДляПоискаОпределения + ПозицияВхождения0 + ПозицияСсылки;
	...
	//Если ЗначениеЗаполнено(СтараяТекущаяПозиция) Тогда
