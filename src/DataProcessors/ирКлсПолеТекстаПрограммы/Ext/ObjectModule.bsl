...
Функция ОткрытьРедакторСтроковогоЛитерала() Экспорт
	...
	Если СмещениеКолонки > 0 Тогда
		ПараметрГраницыВыделения.НачальнаяКолонка = ПараметрГраницыВыделения.НачальнаяКолонка - СмещениеКолонки;
		ПараметрГраницыВыделения.КонечнаяКолонка = ПараметрГраницыВыделения.КонечнаяКолонка - СмещениеКолонки;
	КонецЕсли;
								...
								Иначе //Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(8)) Тогда ;
									...
									Если ПрименитьТип Тогда 
										...
										Иначе
											...
											СмещениеПозиции = 0;
										КонецЕсли;
										ТаблицаПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст,,,,, ПолныйАнализСоставаТипов, Ложь,,, ВычислитьЗначение,, ПозицияВхождения0 + СмещениеПозиции);
										мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, ТаблицаПрисвоенныхТипов);
									КонецЕсли;
								КонецЕсли;
							Иначе
