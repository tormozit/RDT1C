...
Функция ОткрытьКонструкторМетода(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	Если мМетодМодуля <> Неопределено И Не ЗначениеЗаполнено(ВыделенныйТекст()) Тогда 
		...
		Если Лев(ВыделенныйТекст(), 2) = Символы.ПС + Символы.ПС Тогда
			// На случай аннотации, которая захватывает предшествуюшую пустую строку
			ФормаНовогоМетода.ПолноеОпределение = Символы.ПС + Символы.ПС + ФормаНовогоМетода.ПолноеОпределение; 
		КонецЕсли;
