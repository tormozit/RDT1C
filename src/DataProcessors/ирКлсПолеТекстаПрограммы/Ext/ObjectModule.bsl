...
// Разбирает вызов метода и выделяет в тексте заданный параметр
//
// Параметры:
//  Нет.
//
...
Функция ЗагрузитьЗапросПакетаМодуляПоПозиции(Знач ТекстПакетаЗапросов = "", НомерПервойСтрокиБлока = Неопределено, Позиция0ВЛитерале = Неопределено) Экспорт
	...
	Если мПакетЗапросов.ТекстыЗапросов = Неопределено Тогда
		...
		мПакетЗапросов.ТекстыЗапросов = мПлатформа.РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(ТекстПакетаЗапросов, мПозицияВТексте - мНачальнаяПозиция0ВложенногоТекста, мПакетЗапросов.ПозицииЗапросов);
	КонецЕсли;
...
Процедура РазобратьКонтекстЗапросаВТекстовомЛитерале(Знач ВыражениеЛитерала, Знач ТекстЛитерала) Экспорт
	...
	мПакетЗапросов.ПозицииЗапросов = Новый Массив;
	мПакетЗапросов.ТекстыЗапросов = мПлатформа.РазбитьГрубоТекстПакетногоЗапросаНаЗапросы(ВыражениеЛитерала, мПозицияВТексте - мНачальнаяПозиция0ВложенногоТекста, мПакетЗапросов.ПозицииЗапросов);
КонецПроцедуры
...
Функция СсылкаСтрокиМодуля(Знач НомерСтроки = 0, Знач Расширенная = Истина) Экспорт 
	ИмяМетода = "";
	Если Не ЗначениеЗаполнено(НомерСтроки) Тогда
	...
	КонецЕсли;      
	Результат = ирОбщий.СсылкаСтрокиМодуляЛкс(мИмяМодуля, мНачальнаяСтрока, ИмяМетода, СмещениеСтрокиМетода, ПолеТекста.ПолучитьСтроку(мНачальнаяСтрока),,, Расширенная);
	Возврат Результат;
КонецФункции
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			//	ТипыВыраженийМетода.Вставить(КлючВыраженияСПозицией, ТаблицаТипов); // Зачем дублируем вставку?
			//КонецЕсли;
		КонецЕсли; 
	КонецЕсли;
	Если ТаблицаТипов.Количество() = 0 Тогда
	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			...
			Если СтруктураТипаЦикл.ИмяОбщегоТипа = "Запрос" Тогда
				...
				ТаблицаТиповМенеджера[0].Метаданные = ТаблицаТипов[0].ДержательМетаданных;
