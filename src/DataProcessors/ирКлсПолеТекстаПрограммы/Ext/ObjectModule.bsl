...
Функция ПредставлениеЗначенияВЯзыкеЗапросов(Знач Значение, Знач ВыводитьСообщениеВместоИсключения = Истина, Знач ПараметрыДиалектаSQL = Неопределено, Знач Английский1С = Ложь) Экспорт 
	...
	Если Результат = "" Тогда
		ТекстОшибки = ирОбщий.СтрШаблонЛкс("Литеральное представление значения ""%1"" в языке запросов не предусмотрено", ирОбщий.РасширенноеПредставлениеЗначенияЛкс(Значение));
		Если ВыводитьСообщениеВместоИсключения Тогда
			ирОбщий.СообщитьЛкс(ТекстОшибки);
		Иначе
...
Функция СловоЯзыкаЗапросов(Ключ, ПараметрыДиалектаSQL = Неопределено, Английский1С = Ложь) Экспорт
		...
		ИначеЕсли Подгруппа3 <> Неопределено Тогда 
			...
			Если Истина
				...
				И (Ложь
					...
					Или мРодительскийКонтекст = "")
			Тогда
				// Вызова из консоли кода для заполнения выходных параметров. Текст алгоритма синтаксически корректен.
