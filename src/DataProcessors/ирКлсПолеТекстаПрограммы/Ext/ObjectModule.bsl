	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			...
			Если СтруктураТипаЦикл.Метаданные.Количество() = 0 Тогда
				СтруктураТипаЦикл.Метаданные = Неопределено; // чтобы детальность не стала 5
			КонецЕсли;
...
Функция ЛиТаблицаТиповКэшНеГотова(ТаблицаТиповКэш, ДляСвойства) Экспорт
	...
	Тогда 
		ТаблиицаТиповСвойства = Неопределено;
		ТаблицаТиповКэш[0].Метаданные.Свойство(ирОбщий.ПервыйФрагментЛкс(ДляСвойства), ТаблиицаТиповСвойства);
		Результат = ТаблиицаТиповСвойства = Неопределено; // При наличии свойство со значением Неопределено возможны повторные расчеты
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Функция ИнициироватьРекурсивныйПуть(Знач Выражение = "", Знач ДопКлючКэша = "") Экспорт
	...
	РекурсивныйПуть.Вставить("ПозицииЗапроса", Новый Массив); // для языка запросов защита от циклической рекурсии
...
Функция ПроверитьВыраженияПослеТочки(Знач ТабличноеПоле = Неопределено, Форма = Неопределено, Знач ПолныйПерерасчет = Ложь) Экспорт
	...
	Для Этап = 1 По 2 Цикл
		...
		Счетчик = 0; 
		Для Каждого Вхождение Из Вхождения Цикл
			Если Этап = 1 И ЛиВхождениеВЗапрешенномДиапазоне(Вхождение, мПозицияТекстаДляПоискаОпределения, мПозицияТекстаДляПоискаОпределения + СтрДлина(мТекстБлока)) Тогда 
				Продолжить;
			КонецЕсли;
			Счетчик = Счетчик + 1;
			...
			Если ВыражениеЛитерала <> Неопределено Тогда 
				...
				Тогда
					...
					Если ирОбщий.ЛиТекстЯзыкаЗапросовЛкс(ТекстЛитерала) Тогда
						...
						Для Каждого ВхождениеЗапроса Из ВхожденияЗапроса Цикл
							...
							ПозицияВхождения0 = ПозицияВхождения0 + мНачальнаяПозиция0ВложенногоТекста - мПозицияТекстаДляПоискаОпределения;
							мТекстДляПоискаОпределения = мТекстБлока;
				...
				ИначеЕсли Истина
					...
					И Найти(ВыражениеЛитерала, ".") > 0
					И СтрДлина(ВыражениеЛитерала) > 8
				Тогда
					мЭтоТекстовыйЛитерал = Истина; 
					ПозицияВхождения0 = ПозицияВхождения0 - мПозицияТекстаДляПоискаОпределения + 1;
					ТекстЛитерала = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ВыражениеЛитерала);
					ДобавитьВозможнуюОшибкуПослеТочки(ВложенныеВыраженияШаблон, ТекстЛитерала, ПозицияВхождения0 + 1);
				КонецЕсли; 
			Иначе
				мЭтоТекстовыйЛитерал = Ложь; 
				мЯзыкПрограммы = 0;
	...
	Для Каждого Метод Из ирОбщий.РазличныеЗначенияКолонкиТаблицыЛкс(ОшибкиМодуля, "Метод") Цикл 
		мМодульМетаданных.Методы.Найти(Метод, "Имя").ЕстьОшибки = Истина;
	КонецЦикла;
	Если Форма <> Неопределено Тогда
		Форма.КоличествоНайдено = ОшибкиМодуля.Количество();
	Иначе
...
Процедура ДобавитьВозможнуюОшибкуПослеТочки(Знач ВложенныеВыраженияШаблон, Знач НайденноеВыражение, Знач ПозицияВхождения0ВБлоке) Экспорт
	...
	мРекурсивныйПуть.Вставить("РежимПроверки");
	мРекурсивныйПуть.ПозицииЗапроса.Очистить();
	мРекурсивныйПуть.СчетчикиВызовов.Очистить();
	ВычислитьТипЗначенияВыражения(НайденноеВыражение, мТекстДляПоискаОпределения,, Истина,,, Ложь,,,,, ПозицияВхождения0ВБлоке,,, Истина);
