...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	// БСП
	//ДобавитьПравилоВычисленияФункции("ОбщийМодуль", "_ВычислитьОбщийМодуль", "ОбщийМодуль.ОбщегоНазначения");
...
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ТаблицаТипов, ТипВыхода = Неопределено) Экспорт
	...
	Если Истина 
		...
		И (Ложь
			Или ТаблицаТипов.Количество() = 0
			// Наверное можно еще учесть количество различных типов
			Или ТаблицаТипов[ТаблицаТипов.Количество() - 1].Детальность < 4)
	Тогда
...
// БСП
Функция _ВычислитьОбщийМодуль(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
