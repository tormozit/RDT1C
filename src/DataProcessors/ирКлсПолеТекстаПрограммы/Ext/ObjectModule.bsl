	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			...
			Если НовыеМетаданныеТаблицы.Количество() > 0 И НовыеМетаданныеТаблицы <> МетаданныеТаблицы Тогда
				...
				Тогда
					//! МетаданныеТаблицы = 0; // ТаблицаЗначений
					Для Каждого Колонка Из МетаданныеТаблицы.Колонки Цикл
						Если Не НовыеМетаданныеТаблицы.Свойство(Колонка.Имя) Тогда
							НовыеМетаданныеТаблицы.Вставить(Колонка.Имя, мПлатформа.ТаблицаТиповИзОписанияТипов(Колонка.ТипЗначения));
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
...
Функция ЗаполнитьТаблицуСловДляТекстовогоЛитерала(ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен = Истина) Экспорт
	...
	Тогда
		...
		ИначеЕсли Истина
			...
			И (Ложь
				Или ИмяПараметра = "Колонки"
				Или ИмяПараметра = "Ключ" 
				Или ИмяПараметра = "Ключи") 
		Тогда 
