...
Перем мФормаАвтодополнение Экспорт; // см. ФормаАвтодополнение()
Перем мФормаВызовМетода Экспорт; // см. ФормаВызовМетода()
...
Перем мМодульМетаданных Экспорт;  // см. ирПлатформа.ПодготовитьМодульМетаданных()
...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	ДобавитьПравилоВычисленияФункции("ЗначениеРеквизитаОбъекта", "ВычислитьЗначениеРеквизитаОбъектаБСП", "ОбщийМодуль.ОбщегоНазначения");
...
Процедура ПриНажатииКлавишиАвтодополнение(Источник, Событие, Данные, Символ) Экспорт
	...
	Тогда 
		...
		Если Символ = "=" И Последние2Символа <> "==" Тогда
			...
			ЕстьПодходящееПрисвоение = мСтрокаЛучшегоСлова <> Неопределено;
		КонецЕсли;
		Если Ложь
			Или Истина
				И Не БылаОткрыта 
...
Функция ТекущийВызовМетода(Знач НомерПараметраДляВыделения = 0) Экспорт 
	...
	Если мКонечнаяСтрока > 1 Тогда
		...
		Если мНомерПервойСтрокиТелаМетода <> Неопределено Тогда
	...
	КонецЕсли;
	ТекстГдеИскать = ТекстДоКурсора + ирОбщий.СтрокаБезКонцаЛкс(КонецВыражения) + ",";
...
Процедура РазобратьКонтекстСтроки(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Ложь) Экспорт
	...
	Если ЛиСправаОтРавенства Тогда
		СтрокаШаблона = СтрокаШаблона + "(" + шРазделитель + "*(?==|<>)[=<> \t]*(?=[=<> \t])" + СтрокаШаблона + ")";  // Такой сложный прием нужен для для обработки беспробельной операции, например "А=Б"
	КонецЕсли;
...
Функция ТелоМетода(МетодМодуля, выхНомерПервойСтрокиТела = 1, Знач БезВозвратов = Ложь, _выхНачальнаяПозиция = 0) Экспорт 
...
Функция РазобратьТекстЗапроса(ТекстЗапроса, СокращенноеДерево = Ложь, ОбновлятьСостояние = Истина, Знач пПолеТекста = Неопределено, СообщатьОПропущенныхТерминалах = Ложь, выхТаблицаКомментариев = Неопределено,
	...
	//! мПарсер = Новый COMОбъект("GoldParserForNet.Parser");
...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	...
	Для Каждого РодительскаяСтруктураТипа Из РодительскиеСтрутурыТипа Цикл
				...
				Если ПараметрыМетода <> Неопределено Тогда
					...
					Если СтрокаПараметраМетода <> Неопределено Тогда
						...
						Тогда
							...
							КонецЕсли;
						КонецЕсли;
						ВычислитьТипПараметраМетодаМодуля(ПараметрыМетода, СтрокаПараметраМетода, ДляСвойства); 
		...
		Тогда
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				Если мЯзыкПрограммы <> 1 Тогда
					...
					Иначе
						выхЛиПеременнаяМодуля = Истина; 
						мПлатформа.ПодготовитьТипЗначенияПеременнойМодуля(СтрокаСлова, мМодульМетаданных, ПолеТекстаПрограммыВызова, ДляСвойства); 
					КонецЕсли;  
				КонецЕсли;
		...
		КонецЕсли;
		ТаблицаСвойствТипа.Очистить();
		Если Не ЭтоЛокальныйКонтекст Или Не ЗапретВычисленияВМодуле Тогда
			мПлатформа.СловаКонтекстаМетаданные(РодительскаяСтруктураТипа, ТекущееСлово, ТипСлова,, мЯзыкПрограммы, ТекущийИндекс,,, мФлагиКомпиляции,
				МодульМетаданныхКонтекста(РодительскаяСтруктураТипа), ТаблицаСвойствТипа,, ЭтоЛокальныйКонтекст, ЛиВместеСЛокальнымКонтекстом, БазовоеРасширениеКонфигурации, ДляСвойства);
		Иначе 
...
Функция ВычислитьТипПараметраМетодаМодуля(Знач ПараметрыМетода, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Тогда
		...
		КонецЕсли;
	Иначе
		Возврат СтрокаПараметраМетода.ТаблицаТипов;
	КонецЕсли;
	//Если мРекурсивныйПуть = Неопределено Тогда
	//	ИнициироватьРекурсивныйПуть();
	//КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТипЗначения) Тогда 
		...
		Если мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТипов, 4, Истина, ЛучшийТип) Тогда 
			Возврат ТаблицаТипов;
		КонецЕсли;
		Если ТаблицаТипов.Количество() > 0 Тогда
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Если ПрямыеВызовыМетодов = Неопределено Тогда
			...
			Если Ложь
				...
				//Или мФлагиКомпиляции.КлиентУправляемоеПриложение 
				//Или мФлагиКомпиляции.КлиентОбычноеПриложение 
			Тогда
			...
			КонецЕсли;
			ПрямыеВызовыМетодов = мПлатформа.ИндексПоискаВызововСловаВМодуле(мОригинальныйТекст, ШаблонПрямогоВызоваСлова("(" + шИмя + ")",, Истина), ШаблонОповещения);
		...
		Тогда
			Для Каждого ВхождениеВызова Из ПрямыеВызовыМетодов.Оповещения Цикл
				Если ПолеТекстаПрограммыВызова = Неопределено Тогда
					ПолеТекстаПрограммыВызова = КопияКомпоненты();
				КонецЕсли;
				НачальнаяПозицияБлока = ПолеТекстаПрограммыВызова.ЗагрузитьМетодМодуляПоПозицииИРазобратьКонтекст(ВхождениеВызова.ПозицияВхождения);
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда
				...
				КонецЕсли;
				БазовйКлючВыражения = мРекурсивныйПуть.ДопКлючКэша + ";" + ГлобальныйПоискВМодулеВход + ";" + XMLСтрока(Позиция0ВМетодеДо) + ";" + Лев(Выражение, 200); 
				КлючВыраженияСПозицией = "0;" + БазовйКлючВыражения; // Полный расчет
				...
				Тогда
					КлючВыраженияСПозицией = "1;" + БазовйКлючВыражения; // Частичный расчет
					ТаблицаТиповКэш = ТипыВыраженийМетода[КлючВыраженияСПозицией]; // см. мПлатформа.НоваяТаблицаТипов()
				ИначеЕсли ТаблицаТиповКэш <> Неопределено Тогда  
					ДляСвойства = ""; // Чтобы кэш считался готовым всегда
				КонецЕсли;
				...
				Иначе
					...
					Тогда
						Если ЗначениеЗаполнено(ДляСвойстваВверх) Тогда
							ДляСвойстваВверх = ТекущееСлово + "." + ДляСвойстваВверх;
						Иначе
							...
							Если ЭтоПрисвоение Тогда
										...
										ИначеЕсли ТипНового <> Неопределено Тогда
											...
											Тогда 
												...
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПервыйФактПараметр) Тогда 
													...
													ТекстПакета = ВычислитьЗначениеВыражения(ПервыйФактПараметр,,, Позиция0ВМетодеОт, ПоследнееВхождение.FirstIndex, КоординатыТекста);
											...
											Тогда                                                
												ПервыйФактПараметр = ПоследнееВхождение.SubMatches(6);
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПервыйФактПараметр) Тогда
													ТаблицаТиповПараметра = ВычислитьТипЗначенияВыражения(ПервыйФактПараметр,,,,,,,,, Истина, Позиция0ВМетодеОт, ПоследнееВхождение.FirstIndex);
													ИменаСвойств = "";
													ЛучшийТип = Неопределено; // см. мПлатформа.НоваяСтруктураТипа()
													Если мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповПараметра,, Истина, ЛучшийТип) Тогда
								...
								Иначе //Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(8)) Тогда ;
									...
									Если ПрименитьТип Тогда 
										...
										Если ТаблицаПрисвоенныхТипов.Количество() > 0 Тогда
											Если Истина
												И ирОбщий.СтрНачинаетсяСЛкс(ТаблицаПрисвоенныхТипов[0].ИмяОбщегоТипа, "Обещание", Истина) 
												И (Ложь
													// TODO переделать на подгруппу
													Или ирОбщий.СтрНайтиЛкс(ПоследнееВхождение.Value, " Ждать ") > 0 
													Или ирОбщий.СтрНайтиЛкс(ПоследнееВхождение.Value, " ждать ") > 0)
											Тогда
												ТаблицаПрисвоенныхТипов[0].ИмяОбщегоТипа = ирОбщий.ТекстМеждуМаркерамиЛкс(ТаблицаПрисвоенныхТипов[0].ИмяОбщегоТипа, "[", "]", Ложь);
											КонецЕсли;
											СтрокаОписания = Неопределено;
											Если ТаблицаТипов.Количество() > 0 Тогда
												СтрокаОписания = ТаблицаТипов[0].СтрокаОписания;
											КонецЕсли;
											ТаблицаПрисвоенныхТипов.ЗаполнитьЗначения(СтрокаОписания, "СтрокаОписания"); 
	...
	Если СобиратьОшибки Тогда 
		Если ЗначениеЗаполнено(ТекущиеАргументы) И ТекущиеАргументы <> "()" Тогда
			Если мРекурсивныйПуть.ВложенныеВыражения.Найти(ТекущиеАргументы, "Выражение") = Неопределено Тогда
				...
				СтрокаВыражения.ВызовМетода = РодительскийКонтекст + "." + ТекущееСлово;
				СтрокаВыражения.Позиция0ВБлоке = Позиция0ВМетодеДо + СтрДлина(СтрокаВыражения.ВызовМетода + "(");
			Иначе 
				Пустышка = 0;
			КонецЕсли;
		КонецЕсли;
				...
				Или (Истина
					...
					И (Ложь
						...
						Или Истина
							...
							И ирОбщий.ЛиФормаИлиИмитаторЛкс(ЛучшийТип.Метаданные)
						))
				Или (Истина
					И ТаблицаТипов.Количество() = 1
					И ТипЗнч(ЛучшийТип.СтрокаОписания) = Тип("СтрокаТаблицыЗначений")
...
Функция ИмяОтключенияЛишнихВычислений() Экспорт
	...
	Возврат ИмяОтключенияЛишнихВычислений;
КонецФункции
...
Функция ДобавитьОшибкуМодуля(Позиция0ВБлоке) Экспорт
...
Процедура СобратьДинамическиеСвойстваОбъекта(Знач Выражение, Знач ТаблицаТипов, ТекстДляПоискаОпределения = "", Знач ЭтоВызовКонструктора = Ложь, Знач ПолныйАнализСоставаТипов = Истина, Знач ГлобальныйПоискВМодуле = Ложь,
	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			СобратьСвойстваСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ГлобальныйПоискВМодуле, ЛиВиртПрисвоение, Позиция0ВМетодеОт, Позиция0ВМетодеДо, ДляСвойстваПуть);
			Если СтруктураТипаЦикл.Метаданные.Количество() = 0 Тогда
		...
		Тогда
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				Если ирОбщий.ЛиИмяПеременнойЛкс(ТекстовоеЗначение) Тогда
					...
					Если Истина
						И ЗначениеЗаполнено(ДляСвойства) 
						И ДляСвойства <> "Индексы" // TODO грязно, перенести на более ранний этап
						И ДляСвойства <> "Колонки"
						И ДляСвойства <> "Строки"
					...
					Если ЗначениеЗаполнено(ПрисвоенныйТип) Тогда
						...
						Если ЗначениеЗаполнено(ТипЗначения) Тогда
							ТаблицаТиповКолонки = ТаблицаТиповИзТекста(ТипЗначения,,, ДляСвойстваВниз);
						КонецЕсли;
					КонецЕсли;
					Если Не мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповКолонки) Тогда
						...
						КонецЕсли;
					КонецЕсли;
					Если Не ЗначениеЗаполнено(ТаблицаТиповКолонки) Тогда
						ТаблицаТиповКолонки = Неопределено;
					КонецЕсли;
							...
							Для Каждого КлючИЗначение Из ПостроительЗапроса.Параметры Цикл
								...
								Тогда 
									Если ЗначениеПараметра.Количество() > 0 Тогда 
										НовоеЗначениеПараметра = ЗначениеПараметра[0].Метаданные;
										Если Ложь
											Или ТипЗнч(НовоеЗначениеПараметра) = Тип("ОбъектМетаданных")
											Или ТипЗнч(НовоеЗначениеПараметра) = Тип("ОбъектМетаданныхКонфигурация")
										Тогда
											//ЗначениеПараметра = Неопределено;
										Иначе
											ЗначениеПараметра = НовоеЗначениеПараметра;
										КонецЕсли;
									Иначе 
										//ЗначениеПараметра = Неопределено;
									КонецЕсли;
	...
	Тогда
		...
		Для Каждого СтрокаТипа Из ТаблицаТиповКэш Цикл
			Если ТипЗнч(СтрокаТипа.Метаданные) = Тип("Структура") Тогда 
				...
				СтрокаТипа.Метаданные.Свойство(ирОбщий.ПервыйФрагментЛкс(ДляСвойства), ТаблицаТиповСвойства);
...
Процедура ДобавитьЕдинственныйТип(Знач ТаблицаТипов, СтруктураТипа, Знач ЗапретитьЗаменуТипа = Ложь) Экспорт
	Если ТаблицаТипов.Количество() > 0 Тогда 
		...
		ИначеЕсли СтруктураТипа.ИмяОбщегоТипа = "ОбъектМетаданных" Тогда
			Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
				Если ирОбщий.СтрНачинаетсяСЛкс(СтрокаТипа.ИмяОбщегоТипа, "ОбъектМетаданных", Истина ) > 0 Тогда
					НайденныйТип.Добавить(СтрокаТипа);
					Прервать;
				КонецЕсли;
			КонецЦикла;
	...
	Если ТаблицаТипов.Количество() = 0 Тогда
		Если СтруктураТипа.ИмяОбщегоТипа = "ОбъектМетаданных" Тогда
			СтруктураТипа.ИмяОбщегоТипа = "ОбъектМетаданныхСправочник"; 
		КонецЕсли;
		мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипа);
	КонецЕсли;
КонецПроцедуры
...
Функция МожноПрименитьПрисвоениеТипаИзЕсли(Знач ТекстДляПоискаОпределения, Знач Позиция0ВМетодеДо, Знач ПоследнееВхождение, Знач ПозицияВхождения0)
	...
	Если Ложь
		...
		Или (Истина
			...
			И Найти(ТекстПоискаТогда, "Или ") = 0)
	Тогда
		ШаблонПоиска = мПлатформа.шПустоеНачалоСтроки + "(Если|ИначеЕсли|Иначе|КонецЕсли)(?=\s|/|;|$)";

		//ВхожденияЕсли = НайтиВхожденияРегВыраженияКэш(ШаблонПоиска,,, "Условия"); // 3 мс в СловаКонтекстаМетаданные() для МетаданныеРодителя.
		мРегВыражение.Global = Истина;
		мРегВыражение.Pattern = ШаблонПоиска;
		ВхожденияЕсли = мРегВыражение.НайтиВхождения(ТекстПоискаЕсли);
		
		Глубина = 0;
		Для Каждого ВхождениеЕсли Из ВхожденияЕсли Цикл
			// 5 мс в СловаКонтекстаМетаданные() для МетаданныеРодителя.
			//Если ЛиВхождениеВЗапрешенномДиапазоне(ВхождениеЕсли, ПозицияВхождения0, Позиция0ВМетодеДо) Тогда
			//	Продолжить;
			//КонецЕсли;
			Токен = НРег(ВхождениеЕсли.SubMatches(0));
			Если Токен = "если" Тогда
				Глубина = Глубина + 1;
			ИначеЕсли Токен = "конецесли" Тогда
				Глубина = Глубина - 1;
...
Функция НайтиВхожденияРегВыраженияКэш(Знач ШаблонПоиска, Знач ГлобальныйПоискВМодуле = Ложь, выхТекстДляПоискаОпределения = "", Знач КлючКэшаПоиска = "")
	...
	Иначе
		// TODO добавить кэш для программы модуля
...
Функция СобратьСвойстваСтруктуры(Знач Выражение, Структура = Неопределено, Знач ГлобальныйПоискВМодуле = Ложь, Знач ТолькоПрисвоение = Ложь, Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0,
	Знач ДляСвойстваПуть = "")
	...
	КонецЕсли;
	МинДетальность = 4;
	ШаблонПоиска = шПредИнструкция + Выражение + "\s*\.\s*Вставить\s*\(\s*(" + шЛитералПрограммы + "|" + шИмяСТочками + ")(?:\s*,\s*((?:" + шЛитералПрограммы + "|" + шСкобкиЛенивые + "|[^\)""])*))?" 
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если ирОбщий.ЛиИмяПеременнойЛкс(ТекстовоеЗначение) Тогда 
			...
			Тогда
				Продолжить;
			КонецЕсли;
			ТаблицаТиповЗначения = ВычислитьТипыПоВхождению(Вхождение, ГлобальныйПоискВМодуле, ДляСвойстваВниз);
		...
		ИначеЕсли Истина
			И Структура.Количество() = 0 
			И ТаблицаТиповВсехЭлементов = Неопределено 
			И Найти(Вхождение.Value, ",") > 0
		Тогда
			ТаблицаТиповВсехЭлементов = ВычислитьТипыПоВхождению(Вхождение, ГлобальныйПоискВМодуле, ДляСвойстваВниз);
		КонецЕсли;
	КонецЦикла;
	Если Истина
	...
	Тогда 
		...
		ШаблонПоиска = шПредИмяПрямое + ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(Выражение + ".") + "(" + шИмя + ")(?![\(\d" + шБуква + "])(\s*=[^\n]*//)?";
		...
		КонецЕсли;
	КонецЕсли;
	Если Структура.Количество() = 0 И ТаблицаТиповВсехЭлементов <> Неопределено Тогда
		Структура = ТаблицаТиповВсехЭлементов;
	КонецЕсли;
	Возврат Структура;
КонецФункции
...
// Возвращаемое значение:
//    ТаблицаЗначений, Неопределено - 
Функция ВычислитьТипыПоВхождению(Знач Вхождение, Знач ГлобальныйПоискВМодуле = Ложь, Знач ДляСвойстваВниз = "") Экспорт
	СтруктураМоноТипа = Неопределено;
	ТаблицаТиповЗначения = Неопределено;
	ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, Вхождение);
		...
		Если ЭтоИмяТипа Тогда
			...
			Если Фрагменты.Количество() > 1 Тогда
				...
				Если ТипыЭлементовКоллекции.Количество() > 0 И Не ЭтоТипСоответствие Тогда
					ИмяОбщеготТипа = БазовыйТип + "[" + мПлатформа.ПредставлениеМассиваСтруктурТипов(ТипыЭлементовКоллекции) + "]";
				Иначе
					ИмяОбщеготТипа = БазовыйТип;
				КонецЕсли;
				...
				Если Найти(УказательТипа, "Объект.") > 0 И СтрЧислоВхождений(УказательТипа, ".") > 1 Тогда
					...
					Если ОписаниеТиповОбъекта <> Неопределено Тогда
						...
						Иначе 
							ТипСлова = "Свойство";
						КонецЕсли;
						НоваяТаблицаТипов = ВычислитьТипДочернегоЭлемента(ТаблицаТиповРодителя, Слово, ТипСлова,,,,,,,, ДляСвойстваПуть);
					КонецЕсли;
				КонецЕсли;
				Если НоваяТаблицаТипов.Количество() = 0 Тогда
					НоваяТаблицаТипов = ВычислитьТипЗначенияВыражения(УказательТипа, ТекстДоВхождения, ТекстДоВхождения,,,, Ложь,,,,,,, ДляСвойстваПуть);
				КонецЕсли;
				...
				Иначе 
					...
					Если ПараметрыМетода <> Неопределено Тогда
						...
						Если СтрокаПараметра <> Неопределено Тогда
							...
							НоваяТаблицаТипов = ТаблицаТиповИзПараметраМетодаМодуля(СтрокаМетода, СтрокаПараметра);
...
// Возвращаемое значение:
//    ТаблицаЗначений, Неопределено - 
Функция ТаблицаТиповИзПараметраМетодаМодуля(Знач СтрокаМетода, Знач СтрокаПараметра) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаПараметра.ТипЗначения) Тогда
		...
		НоваяТаблицаТипов = АнализаторМодуля.ТаблицаТиповИзТекста(СтрокаПараметра.ТипЗначения, СтрокаПараметра.Описание); // TODO защита от бесконечной рекурсии
	Иначе 
...
Функция ЧастиПолногоИмениЭлементаФормы(Знач ИмяТипа) Экспорт
	...
	ИначеЕсли ФрагментыТочки.Количество() > 2 И ФрагментыТочки[2] = "Форма" Тогда 
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	КонецЕсли;
	ПредшествующийСимвол = Прав(СокрП(мПредшествующийТекст), 1);
	Если ПредшествующийСимвол = "," Или ПредшествующийСимвол = "(" Тогда
		ТекущийВызовМетода();
		Если ЗначениеЗаполнено(мВызовМетода) Тогда
			ИмяОтключенияЛишнихВычислений = ИмяОтключенияЛишнихВычислений();
			ТаблицаТиповВызова = ВычислитьТипЗначенияВыражения(мВызовМетода + ИмяОтключенияЛишнихВычислений + ")",,,, мЭтоКонструктор);
			...
			КонецЕсли;
		КонецЕсли; 
	ИначеЕсли ПредшествующийСимвол = "=" Или ПредшествующийСимвол = ">" Тогда
	...
	Если Ложь
		...
		Или (Истина
			...
			И ТаблицаОжидаемыхТипов[0].Детальность < 3)
	Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Истина
		...
		И (Ложь
			...
			Или СтрокаОписания.Слово = "ТекущаяСтраница" И СтрокаОписания.ТипКонтекста = "ГруппаФормы"
	...
	Тогда
		...
		КонецЕсли;
	Иначе
		Если НаборыСлов[КлючОсновногоНабора] = Неопределено Тогда
	...
	Тогда
		...
		Если СтрокаОписанияМетода.ТипСлова <> "Свойство" Тогда
			...
			Тогда 
				...
				ЛучшийТип = Неопределено; // см. мПлатформа.НоваяСтруктураТипа()
				Если Истина
					И мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповОбрамляющегоВызова, 4, Истина, ЛучшийТип) 
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен) Экспорт
	...
	Тогда
		...
		Для Каждого Вхождение Из ВхожденияОбъектов Цикл
			...
			Если ТаблицаТипов.Количество() > 0 Тогда
				ТаблицаСловОбъекта = мПлатформа.СловаКонтекстаМетаданные(ТаблицаТипов[0],,,,,, "Метод",,,,,,,, БазовоеРасширениеКонфигурации); 
				ЗначенияПараметра = ТаблицаСловОбъекта.ВыгрузитьКолонку("Слово");
				Если ЗначенияПараметра.Количество() > 0 Тогда
					КонтекстТекстовогоЛитералаИзвестен = Истина;
					Прервать;
	...
	ВхожденияВызовов = НайтиВхожденияРегВыраженияКэш("((" + шОбъектноеВыражение + ")\s*\(\s*(?:(?:" + шПростоеВыражениеПрограммы + "\s*,\s*)*?))" + ИмяПараметра + "[\s,\)]",,,
		"ПараметрМетода;" + ИмяПараметра);
	Для Каждого Вхождение Из ВхожденияВызовов Цикл
		ПозицияВхождения0 = Вхождение.FirstIndex;
		ВыражениеВызоваМетода = Вхождение.SubMatches(1); 
...
//.
// Параметры:
//    СтруктураТипа - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа()  -  
// Возвращаемое значение:
//    ОбъектМетаданныхСправочник, Неопределено - 
Функция ОбъектМДИзСтруктурыТипаМодуля(Знач СтруктураТипа) Экспорт
	ОбъектМД = СтруктураТипа.Метаданные; // ОбъектМетаданныхСправочник
	Если ТипЗнч(ОбъектМД) = Тип("Форма") Тогда
...
Функция ВычислитьТаблицаЗначенийВыгрузитьКолонку(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	...
	ИначеЕсли ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("ТаблицаЗначений") Тогда
		Колонка = РодительскаяСтруктураТипа.Метаданные.Колонки.Найти(ИмяКолонки);
...
//
Функция ПроверитьВыражения(Знач ТабличноеПоле = Неопределено, Форма = Неопределено, Знач ПолныйПерерасчет = Ложь, Знач ПроверятьВсеЛитералы = Ложь) Экспорт
	...
	Вхождения = НайтиВхожденияРегВыраженияКэш(шПроверяемоеВыражение, Истина);
	Для Этап = 1 По 2 Цикл
		...
		ПроверитьВыраженияУровня(Вхождения, ПроверятьВсеЛитералы, ТабличноеПоле, Форма, ДлинаДобавкиОкрестности, Индикатор, КонечнаяПозицияТекущегоМетода, НачальнаяПозицияТекущегоМетода,
...
//    Этап - Число - 
Функция ПроверитьВыраженияУровня(Знач Вхождения, Знач ПроверятьВсеЛитералы = Ложь, Знач ТабличноеПоле = Неопределено, Знач Форма = Неопределено, Знач ДлинаДобавкиОкрестности = Неопределено
, Знач Индикатор = Неопределено, Знач КонечнаяПозицияТекущегоМетода = Неопределено, Знач НачальнаяПозицияТекущегоМетода = Неопределено
, Знач СтруктураОбновленияТабличногПоля = Неопределено, Знач Этап = 0, Знач ВложенныеВыраженияШаблон, Знач СмещениеВБлоке = 0, Знач НачалоБлока = Неопределено) Экспорт

	ИнструкцияПодавленияПроверки = ИнструкцияПодавленияПроверки(); 
	Счетчик = 0; 
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если ВыражениеЛитерала <> Неопределено Тогда 
			...
			Тогда
				...
				Тогда
					...
					РекурсивныйПуть = мРекурсивныйПуть;
					мРекурсивныйПуть = Неопределено;
					мПозицияВТексте = ПозицияВхождения0ВМодуле + 1;
					мПозицияВБлоке = ПозицияВхождения0ВБлоке;
					
					//ИмяПараметра = "";
					//КоллекцияЭлементовМД = ЗаполнитьТаблицуСловДляТекстовогоЛитерала(ТаблицаТипов[0],, ИмяПараметра);
					//ТаблицаСлов.Очистить();
					//ДобавитьПростыеСлова(КоллекцияЭлементовМД);
					//ЗаполнитьСловаВНижнемРегистре();
					//
					ЗаполнитьТаблицуСлов(ТаблицаТипов,,, Ложь, Ложь);
		...
		Иначе
			Если Этап > 0 Тогда
				...
				Если Ложь
					Или Найти(ирОбщий.ПервыйФрагментЛкс(ПостФрагмент, Символы.ПС), ИнструкцияПодавленияПроверки) > 0
					Или (Истина
...
Функция ИнструкцияПодавленияПроверки(Знач Чистая = Ложь) Экспорт
	Результат = "@skip-check unknown-method-property";
	Если Чистая Тогда
		Возврат Результат;
	Иначе 
		Возврат "//" + Результат;
...
Функция ДобавитьВозможнуюОшибкуПослеТочки(Знач ВложенныеВыраженияШаблон, Знач НайденноеВыражение, Знач Позиция0ВБлоке, Знач ПроверятьВсеЛитералы, Знач НачалоБлока) Экспорт
	мРекурсивныйПуть.ВложенныеВыражения = ВложенныеВыраженияШаблон.СкопироватьКолонки();
...
Функция ПроверитьПрограммныйКод(СообщатьОбУспешнойПроверке = Ложь, ТекстДляПроверки = Неопределено, Контекст = Неопределено) Экспорт
	...
	Если СообщатьОбУспешнойПроверке И Успех И ЯзыкПрограммы = 0 Тогда
		#Если Клиент Тогда
		ПроверитьВыражения(,, Истина, Истина);
		#КонецЕсли
	КонецЕсли;
	Возврат Успех;
...
Функция ИмяТипаКонтекста(_ТаблицаТипов = Неопределено) Экспорт 
	...
	Если мСтруктураТипаКонтекста <> Неопределено Тогда
		...
		ИначеЕсли мАгрегатныеТипыКонтекста.Количество() > 1 Тогда
			...
			Результат = "" + XMLСтрока(мАгрегатныеТипыКонтекста.Количество()) + ": " + ПредставлениеМассиваСтруктурТипов(мТаблицаТиповКонтекста);
		Иначе
			Результат = мПлатформа.ИмяТипаИзСтруктурыТипа(мСтруктураТипаКонтекста);
		КонецЕсли;
	Иначе 
...
шПроверяемоеВыражение = "(([+=]\s*)?" + шЛитералПрограммы + "(?:\s*[+=;])?)|" + шКомментарий + "|(" + шПредИмяПрямое + ")(" + шИмяСкобкиЛенивые + "?" + шИндекс + "?(?:\." + шИмяСкобкиЛенивые + "?" + шИндекс + "?)+)";
