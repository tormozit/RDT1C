...
Процедура ПриНажатииКлавишиАвтодополнение(Источник, Событие, Данные, Символ) Экспорт
	...
	Тогда 
		...
		Если Истина
			...
			И СтарыйКонтекст <> мРодительскийКонтекст 
...
Процедура ОбновитьФормуАвтодополнения(Данные) Экспорт
	...
	БылЗаполненТекущееСлово = ЗначениеЗаполнено(ФормаАвтодополнение.ТекущееСлово);
	...
	ОбязательноОткрыть = Найти(Данные, КодыКлавиш["CTRL+Space"]) = 1;
	Если Ложь
		Или Не БылЗаполненТекущееСлово
		Или ОбязательноОткрыть И Не БылЗаполненФильтр
		Или Не ОбязательноОткрыть И БылЗаполненФильтр
	Тогда
		ФормаАвтодополнение.ЭлементыФормы.ПолеОтбораПоПодстроке.Значение = мНачалоСлова;
	Иначе
		ФормаАвтодополнение.ЭлементыФормы.ПолеОтбораПоПодстроке.Значение = "";
	КонецЕсли;
...
Функция СтроитьДеревоПрограммныхСкобок(Знач ВыделенныйТекст = "", Знач НачальнаяСтрока = 1, Знач ПозицияДляСбораРодителей = 0, выхРодителиПозиции = Неопределено, Знач БазовыйУровень = Неопределено,
	...
	Для СчетчикСтроки = 1 По ТекстДокумент.КоличествоСтрок() Цикл
		...
		СтрокаТекстаДоКомментария = ЗалитьКомментарииИСтроковыеЛитералы(СтрокаБезКраев,, Истина,, Истина);
		СтрокаБезКраевИЛитералов = СокрЛП(СтрокаТекстаДоКомментария);
		Если Не ЗначениеЗаполнено(СтрокаБезКраев) Тогда
			СерияПустыхСтрок = СерияПустыхСтрок + 1;
			Если СжиматьПустыеСтроки И СерияПустыхСтрок > 1 Тогда
				ТекстДокумент.УдалитьСтроку(СчетчикСтроки);
				Если мНачальнаяСтрока > СчетчикСтроки Тогда
					мНачальнаяСтрока = мНачальнаяСтрока - 1;
				КонецЕсли;
		...
		Иначе
			СерияПустыхСтрок = 0;
		КонецЕсли;
		Если СжиматьВнутренниеПустотыВСтроке И Лев(СтрокаБезКраев, 1) <> "|" Тогда
		...
		СловоНайдено = НайтиКлючевыеСловаФорматирования(СтрокаТекстаДоКомментария, ГруппыКлючевыхСлов, УровеньИнструкции, УровеньВыражения, УровеньНачалаУсловия, СмещениеСледующегоУровня, ВхожденияКэш, РегВыражениеКэш);
		...
		Если ПустаяСтрока(СтрокаТекстаДоКомментария) Тогда
			Продолжить;
		КонецЕсли;
		Если Прав(СтрокаБезКраевИЛитералов, 1) = ";" Тогда 
...
Функция НайтиКлючевыеСловаФорматирования(Знач СтрокаТекстаДоКомментария, Знач ГруппыКлючевыхСлов, УровеньИнструкции = 0, УровеньВыражения = 0,  УровеньНачалаУсловия = 0, 
	СмещениеСледующегоУровня = Неопределено, ВхожденияКэш = Неопределено, РегВыражениеКэш = Неопределено) Экспорт
					...
					Иначе
						...
						Если ЧислоКонцовУсловия > 0 Тогда
							...
							Если Ложь
								...
								Или ирОбщий.НайтиРегВыражениеЛкс(СтрокаТекстаДоКомментария, шПредИмяПрямое + ШаблонКонцаКонструкции + "(?=//|\s*;|\s*$)",,,,,,, РегВыражениеКэш,, ВхожденияКэш).Количество() = 0 
							Тогда
								СмещениеСледующегоУровня = 1;
							КонецЕсли;
						КонецЕсли;
...
// вызывается из адаптера
Функция СоздатьОписаниеМетода(ИмяПараметра) Экспорт 
	...
	// Исправляем описания параметров
	КомментарийМетода = ирОбщий.ЗаменитьРегВыражениеЛкс(КомментарийМетода, "-[ \t]*-(\s*[" + шБуква + "])", "-$1", Ложь);
	// Исправляем описания возвращаемого значения
	КомментарийМетода = СтрЗаменить(КомментарийМетода, " - .", ".");
	мЗамещающийФрагмент = КомментарийМетода;
	Если ЗначениеЗаполнено(ИмяПараметра) Тогда
		ПерейтиКОпределениюЛокальногоМетода(мМетодМодуля, ИмяПараметра,,, Истина);
	КонецЕсли;
...
Функция УказатьВызывающийМодуль(ИмяМодуля) Экспорт 
	...
	Тогда
		...
		мЗаменяемыйДиапазон.Конец = мМетодМодуля.ПозицияСОписанием + мМетодМодуля.ДлинаСОписанием - мМетодМодуля.ДлинаОпределения; 
		КомментарийМетода = Сред(мОригинальныйТекст, мЗаменяемыйДиапазон.Начало, мЗаменяемыйДиапазон.Конец - мЗаменяемыйДиапазон.Начало);
		ЗаголовокРазделаВызывающиеМодули = ЗаголовокРазделаВызывающиеМодули();
...
Функция ВыделитьМетод(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	Если ФормаВладелец = Неопределено Тогда
		...
		СформироватьЗамещающийФрагментИзМетодов();
...
Процедура СформироватьЗамещемыйФрагментИзМетода() Экспорт
	мЗаменяемыйДиапазон = НовыйЗаменяемыйДиапазон();
	Если мМетодМодуля <> Неопределено Тогда
...
Функция ТекстИмитаторовВременныхТаблиц(Знач МенеджерВременныхТаблиц = Неопределено, Знач ТекстОсновногоЗапроса = "") Экспорт
	...
	КонецЕсли;
	ТекстИмитаторов = ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(ТекстыЗапросов, "Представление"), ";" + Символы.ПС);
	...
	Если мЯзыкПрограммы = 0 Тогда 
		...
		Тогда
			...
			Если Истина
				...
				//И СтрЧислоВхождений(мКонтекст, ".") = 1 // Так отсекаем методы менеджеров
		...
		Если Не ЛиПредопределенное Тогда
			...
			Тогда
				...
				мПлатформа.СловаКонтекстаМетаданные(РодительскаяСтруктураТипа, ТекущееСлово, ТипСлова,, мЯзыкПрограммы, ТекущийИндекс,,, мФлагиКомпиляции,
					Модуль, ТаблицаСвойствТипа, ВычислятьТипыМетодовМодулей, ЭтоЛокальныйКонтекст, ЛиВместеСЛокальнымКонтекстом, БазовоеРасширениеКонфигурации, ДляСвойства);
			Иначе 
...
Функция ВычислитьТипПараметраМетодаМодуля(Знач ПараметрыМетода, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
		...
		Тогда
			...
			Иначе
				...
				Если ЭлементФормы <> Неопределено Тогда
					...
					Тогда 
						ТаблицаДочернихСлов = мПлатформа.СловаКонтекстаПредопределенные(мПлатформа.СтруктураТипаИзЗначения(ЭлементФормы,, ШаблонСтруктурыТипа), "ТекущиеДанные", "Свойство");
	...
	Если ПустаяСтрока(ТипЗначения) Тогда
		ТипЗначения = ирОбщий.ТекстМеждуМаркерамиЛкс(СтрокаПараметраМетода.Описание, "-", Символы.ПС);
	КонецЕсли;
	Если ЗначениеЗаполнено(ТипЗначения) Тогда 
		ТипЗначения = СокрЛП(ТипЗначения);
		ТаблицаТиповНазначенных = ТаблицаТиповИзТекста(СтрокаПараметраМетода.ТипЗначения, СтрокаПараметраМетода.Описание,, ДляСвойства);
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Если мМетодМодуля.ВызывающиеМодули <> Неопределено Тогда
			Для Каждого ЛюбоеИмяМодуля Из мМетодМодуля.ВызывающиеМодули Цикл
				Если Ложь
					Или Найти(ЛюбоеИмяМодуля, ".Форма.") > 0
					Или Найти(ЛюбоеИмяМодуля, ".Команда.") > 0 
				Тогда
					ТаблицаТиповМодуля = ТаблицаТиповИзТекста("см. " + ЛюбоеИмяМодуля);
				ИначеЕсли Найти(ЛюбоеИмяМодуля, "Объект.") > 0  Тогда
					ТаблицаТиповМодуля = ТаблицаТиповИзТекста(ЛюбоеИмяМодуля);
				Иначе
					ТаблицаТиповМодуля = ВычислитьТипЗначенияВыражения(ЛюбоеИмяМодуля,,,,,, Ложь);
				КонецЕсли;
				Если ТаблицаТиповМодуля.Количество() > 0 Тогда
					...
					Если СтруктураТипаМодуля <> Неопределено Тогда
						...
						Если Модуль <> Неопределено Тогда
							ВызывающиеМодули.Добавить(Модуль);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		ВызывающиеМодули.Добавить(мМодульМетаданных);
		СчетчикВызовов = 0;
		МинДетальность = 3;
		МаксЧислоВызовов = 3;
		ШаблонПараметра = "(" + шВыражениеПрограммы + ")?" + шРазделитель + "*(?:$|,|\))";
		...
		Если Истина
			И мМетодМодуля.ЛиЭкспорт
			И ирОбщий.СтрКончаетсяНаЛкс(мИмяМодуля, "МодульОбъекта") 
		Тогда
			НачалоИмениМодуляОбъекта = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(мИмяМодуля);
			ТаблицаТиповМодуляФормы = ТаблицаТиповОсновнойФормыОбъекта(мМодульМетаданных.СтруктураТипа);
			Если ТаблицаТиповМодуляФормы.Количество() > 0 Тогда
				СтруктураТипаМодуля = ТаблицаТиповМодуляФормы[0];
				Если Истина
					И ирОбщий.ЛиИмяТипаФормыЛкс(СтруктураТипаМодуля.ИмяОбщегоТипа, Истина, Ложь)
					И СтруктураТипаМодуля.Метаданные <> мМодульМетаданных.СтруктураТипа.Метаданные
				Тогда
					МодульОсновнойФормы = мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипаМодуля);
					Если Истина
						И МодульОсновнойФормы <> Неопределено
						И ВызывающиеМодули.Найти(МодульОсновнойФормы) = Неопределено 
					Тогда
						ВызывающиеМодули.Добавить(МодульОсновнойФормы);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		ПрямоеИмяМодуля = ПрямоеИмяМодуляИзПолного(мИмяМодуля);
		Для Каждого ВызывающийМодуль Из ВызывающиеМодули Цикл
			ПолеТекстаПрограммыВызова = Неопределено;
			Если ВызывающийМодуль <> мМодульМетаданных Тогда
				ПолеТекстаПрограммыВызова = мПлатформа.ПолеТекстаМодуля(ВызывающийМодуль); // Загружаем текст модуля
			КонецЕсли;
			Если Ложь
				Или ВызывающийМодуль = мМодульМетаданных 
				Или ирОбщий.СтрНайтиЛкс("." + ВызывающийМодуль.Имя + ".", "." + НачалоИмениМодуляОбъекта + ".",,,, Ложь) > 0 // Мультиметка240720_160801 TODO переделать на анализ основного объекта формы
			Тогда
				ПрямыеВызовыМетодов = ВызывающийМодуль.ПрямыеВызовыМетодов;
				Если ПрямыеВызовыМетодов = Неопределено Тогда
					ПрямыеВызовыМетодов = мПлатформа.НовыйИндексПоискаВызововСловаВМодуле(ВызывающийМодуль);
					ЗаполнитьИндексПоискаВызововМетодовВМодуле(ПрямыеВызовыМетодов, ВызывающийМодуль.Текст);
					ВызывающийМодуль.ПрямыеВызовыМетодов = ПрямыеВызовыМетодов;
					...
					КонецЕсли;
				Иначе
					мПлатформа.ВыполнитьОтложенноеОбновлениеВызововСловМодуля(ВызывающийМодуль);
				КонецЕсли;
			Иначе
				ПрямыеВызовыМетодов = мПлатформа.НовыйИндексПоискаВызововСловаВМодуле(ВызывающийМодуль);
				ШаблонОповещения = "";
				...
				КонецЕсли;
				ШаблонВнешнегоВызова = ШаблонПрямогоВызоваСлова(ПрямоеИмяМодуля + "\.(" + мМетодМодуля.Имя + ")",, Истина); 
				мПлатформа.ЗаполнитьИндексПоискаВызововСловаВМодуле(ПрямыеВызовыМетодов, ВызывающийМодуль.Текст, ШаблонВнешнегоВызова, ШаблонОповещения);
				ПрямыеВызовыМетодов.Вызовы.Колонки.Добавить("ПараметрыВызова"); // Строка, Массив 
				ПрямыеВызовыМетодов.Вызовы.Колонки.Добавить("ТекстВхождения"); // Строка               
				ПрямыеВызовыМетодов.Вызовы.Колонки.Добавить("Метод"); // Строка       
				Если ПрямыеВызовыМетодов.Оповещения <> Неопределено Тогда
				...
				Для Счетчик = 0 По ВГраница Цикл
					...
					Если ПараметрыВызова = Неопределено Тогда
						Если ВхождениеВызова.ТекстВхождения = Неопределено Тогда
							ВхождениеВызова.ТекстВхождения = Сред(ВызывающийМодуль.Текст, ВхождениеВызова.ПозицияВхождения + 1, ВхождениеВызова.ДлинаВхождения);
						КонецЕсли;
...
~Выход: 
	...
	Если ЛиКлючевойПараметр Тогда
		Если Не ЗначениеЗаполнено(мМетодМодуля.ТипЗначения) Тогда
			мМетодМодуля.ТипЗначения = "Произвольный"; // чтобы последующая проверка типа результата метода не удалила ключевые параметры см. мПлатформа.ПодготовитьТипЗначенияПеременнойМодуля()
		КонецЕсли;
	ИначеЕсли Истина
...
Функция ЗаполнитьОписанияПараметровИзСобытия(СтрокаСобытия, Знач СтрокаПараметраМетода, Знач ПараметрыСобытия = Неопределено) Экспорт
	Результат = Неопределено;
	Если Истина
		И мМетодМодуля.Параметры.Количество() > 0 
		И (Ложь
			Или Не ЗначениеЗаполнено(мМетодМодуля.Параметры[0].ТипЗначения)
			Или СтрокаПараметраМетода.ТаблицаТипов.Количество() = 0)
	Тогда
		...
		Если ВычислениеРазрешено Тогда
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Иначе
								...
								ПозицияВхождения0 = Макс(1, ПозицияВхождения0); // Если это не делать, то будет циклическая рекурсия в ВычислитьНеизменяющаяСтроковаяФункция при обработке ОбработкаОбъект.ирПлатформа.РазмаскироватьДирективыПрепроцессора
							КонецЕсли;
			...
			Тогда     
				Если ЛиПеременнаяРезультата Тогда 
					ЛиВиртПрисвоение = Истина;
				КонецЕсли;
				СобратьДинамическиеСвойстваОбъекта(Выражение, ТаблицаТипов, ТекстДляПоискаОпределения, ЭтоВызовКонструктора, ПолныйАнализСоставаТипов, ГлобальныйПоискВМодуле, Позиция0ВМетодеОт,
...
//.
// Параметры:
//    ЛучшийТип - СтрокаТаблицыЗначений, Неопределено - 
// Возвращаемое значение:
//    Булево - 
Функция ЛиСистемноеСлово(Знач ЛучшийТип) Экспорт
	ЛиСистемноеСлово = Истина
		И ЛучшийТип <> Неопределено
		И ТипЗнч(ЛучшийТип.СтрокаОписания) = Тип("СтрокаТаблицыЗначений")
		И ЛучшийТип.СтрокаОписания.Владелец().Колонки.Найти("Слово") <> Неопределено;
	Возврат ЛиСистемноеСлово;
КонецФункции
...
Процедура СобратьДинамическиеСвойстваОбъекта(Знач Выражение, Знач ТаблицаТипов, ТекстДляПоискаОпределения = "", Знач ЭтоВызовКонструктора = Ложь, Знач ПолныйАнализСоставаТипов = Истина, Знач ГлобальныйПоискВМодуле = Ложь,
	Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0, Знач ДляСвойстваПуть = "", Знач ЛиВиртПрисвоение = Ложь, Знач ЛиПеременнаяМодуля = Ложь) 
	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			...
			Если Истина
				И ТипЗнч(СтруктураТипаЦикл.Метаданные) = Тип("Структура") 
				И СтруктураТипаЦикл.Метаданные.Свойство("Ключ")
				И СтруктураТипаЦикл.Метаданные.Свойство("Значение")
			Тогда 
				ТаблицаТиповЗначенияВсехЭлементов = СтруктураТипаЦикл.Метаданные.Значение;
				ТаблицаТиповКлючаВсехЭлементов = СтруктураТипаЦикл.Метаданные.Ключ;
			Иначе 
				СтруктураТипаЦикл.Метаданные = Новый Структура("Ключ, Значение");
			КонецЕсли;
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				КонецЕсли;
			КонецЦикла;
			СтруктураТипаЦикл.Метаданные.Значение = ТаблицаТиповЗначенияВсехЭлементов;
			СтруктураТипаЦикл.Метаданные.Ключ = ТаблицаТиповКлючаВсехЭлементов;
			мПлатформа.ОбновитьДетальностьСтруктурыТипа(СтруктураТипаЦикл);
		ИначеЕсли Ложь
		...
		Тогда
			...
			ШаблонПоиска = шПредИмяПрямое + ШаблонВыражения + "\s*\.\s*Колонки\s*.\s*(?:Добавить\s*\(|Вставить\s*\((?:" + шИмя + "|\d+),|" + шИмя + "\.Имя\s*=)\s*" 
	...
	Если ТаблицаТипов.Количество() > 0 Тогда 
		...
		Если СтруктураТипа.ИмяОбщегоТипа = "COMОбъект" Тогда 
			Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
				...
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(СтруктураТипа.ИмяОбщегоТипа, "Форма") Тогда
			Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
				Если ирОбщий.СтрНачинаетсяСЛкс(СтрокаТипа.ИмяОбщегоТипа, "РасширениеФормыКлиентского", Истина ) > 0 Тогда
					Если СтруктураТипа.ИмяОбщегоТипа = "ФормаКлиентскогоПриложения" Тогда
						НайденныйТип.Добавить(СтрокаТипа);
					КонецЕсли; 
				ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(СтрокаТипа.ИмяОбщегоТипа, "РасширениеФормы", Истина ) > 0 Тогда
					Если СтруктураТипа.ИмяОбщегоТипа = "Форма" Тогда
						НайденныйТип.Добавить(СтрокаТипа);
					КонецЕсли; 
				КонецЕсли;
			КонецЦикла;
...
// Возвращаемое значение:
//    ТаблицаЗначений - 
Функция ТаблицаТиповИзТекста(Знач ТекстТипа, Знач Описание = "", Знач ТекстДоВхождения = "", Знач ДляСвойстваПуть = "", СчетчикСтрок = 1, Знач ЛиВложенный = Ложь, Знач ЛиВнутриКоллекции = Ложь) Экспорт
	ТаблицаТипов = мПлатформа.НоваяТаблицаТипов();
		...
		Если ЭтоИмяТипа Тогда
			...
			Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
				...
				Если Фрагменты.Количество() > 1 Тогда
					...
					КонецЕсли;
					//Если Прав(СтрокаОписания, 1) = ":" Тогда
						ТипыЭлементовКоллекции = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(Фрагменты[1], "]"), Описание, ТекстДоВхождения, ДляСвойстваПуть, СчетчикСтрок,, Истина);
					//КонецЕсли; 
					...
					Если ирОбщий.СтрКончаетсяНаЛкс(БазовыйТип, "Массив") Тогда
						...
						СтруктураТипа.Метаданные = ТипыЭлементовКоллекции;
					Иначе
				...
				Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
					СтрокаОписания = СокрЛП(ТекстОписания.ПолучитьСтроку(СчетчикСтрок));
					Если Лев(СтрокаОписания, 1) = "-" Тогда
						Прервать;
					ИначеЕсли Лев(СтрокаОписания, ДлинаПрефикса + 1) = ПрефиксУровня + "*" Тогда
						СчетчикСтрок = СчетчикСтрок + 1;
						Продолжить;
					ИначеЕсли Лев(СтрокаОписания, ДлинаПрефикса) = ПрефиксУровня Тогда
						СчетчикСтрок = СчетчикСтрок + 1;
						ФрагментыОписания = ирОбщий.СтрРазделитьЛкс(Сред(СтрокаОписания, ДлинаПрефикса + 1), "-", Истина);
						...
						Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяСвойства) Тогда 
							...
							Если ФрагментыОписания.Количество() > 1 Тогда
								Если Прав(СтрокаОписания, 1) = ":" Тогда
									СчетчикСтрок = СчетчикСтрок - 1;
									МетаданныеСвойства = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ФрагментыОписания[1], ":"), Описание, ТекстДоВхождения, ДляСвойстваВниз, СчетчикСтрок, Истина);
								Иначе
									Попытка 
									...
									КонецПопытки;
								КонецЕсли;
								Если МетаданныеСвойства.Колонки.Метаданные.Заголовок <> "" Тогда
									ОписаниеСвойствОбрезано = Истина;
								КонецЕсли;
							КонецЕсли;
		...
		Иначе
			...
			Тогда
				...
				ИначеЕсли ИмяМножественное <> Неопределено Тогда
					СтруктураТипа = ВычислитьСловоФормыПоПолномуИмени(УказательТипа, ДляСвойстваПуть);
			...
			Иначе
				...
				Если Найти(УказательТипа, "Объект.") > 0 И СтрЧислоВхождений(УказательТипа, ".") > 1 Тогда
					...
					Если ОписаниеТиповОбъекта <> Неопределено Тогда
						...
						ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюВКолонкеЛкс(НоваяТаблицаТипов, "Детальность", 0);
...
Функция ВычислитьСловоФормыПоПолномуИмени(Знач ИмяТипа, Знач ДляСвойства = "")
	...
	Если ТаблицаТипов.Количество() > 0 Тогда
		Если ТаблицаТипов[0].Детальность < 2 Тогда // Опасно
			ТаблицаТипов = ВычислитьТипЗначенияВыражения(Выражение + "(",,,,,, Ложь,,,, Истина,,, ДляСвойства);
		КонецЕсли;
	КонецЕсли;
	Если ТаблицаТипов.Количество() > 0 Тогда
...
Функция ТаблицаТиповТекущегоВыражения(ВзятьЛевоеОтРавенства = Ложь, БезКонструкторов = Ложь, ПолныйАнализСоставаТипов = Истина, Знач ЛиCOMВызов = Ложь, Знач ЛиДляРодителя = Истина) Экспорт
	...
	Если мЯзыкПрограммы = 0 И ЗначениеЗаполнено(мТекстДляПоискаОпределения) Тогда
		Позиция0ВМетодеДо = СтрДлина(мТекстДляПоискаОпределения);  // для А = А.Б
	Иначе
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	ИначеЕсли Истина
		И мЯзыкПрограммы = 0 
		И ирОбщий.СтрКончаетсяНаЛкс(ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(мТекущаяСтрокаНачало, " ", Ложь), "Для каждого")
	Тогда
		мРегВыражение.Pattern = "(?:" + шИмя + ")?\s+Из\s+(" + шОбъектноеВыражение + ")\s+Цикл$";
		Результат = мРегВыражение.НайтиВхождения(мТекущаяСтрокаКонец);
		Если Результат.Количество() > 0 Тогда
			ПервоеВхождение = Результат[0].SubMatches(0); 
			ТаблицаТиповКоллекции = ВычислитьТипЗначенияВыражения(ПервоеВхождение);
			Если ТаблицаТиповКоллекции.Количество() > 0 Тогда
				...
				Если ТаблицаТиповЭлемента.Количество() > 0 Тогда
					ИмяТипаЭлемента = мПлатформа.ИмяТипаИзСтруктурыТипа(ТаблицаТиповЭлемента[0]);
					ИмяПеременной = ирОбщий.ПервыйФрагментЛкс(ИмяТипаЭлемента);
					Если ирОбщий.СтрКончаетсяНаЛкс(ИмяПеременной, "Значений") Тогда
						ИмяПеременной = ирОбщий.ПервыйФрагментЛкс(ИмяПеременной, "Значений");
					КонецЕсли;
					НоваяСтрока = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, ИмяПеременной, "Свойство",, ИмяТипаЭлемента, "Локальный");
					НоваяСтрока.Рейтинг = КонстантаРейтингСовпаденияТипа;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		мСтруктураТипаКонтекста.ИмяОбщегоТипа = "";
	Иначе
		...
		Если мЭтоТекстовыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
			...
			СтарыйМетод = мМетодМодуля;
			КоллекцияЭлементовМД = ЗаполнитьТаблицуВозможныхЗначенийЛитерала(ВыбраннаяСтруктураТипа, ЛиНаборЗначенийПолный,, ДобавлятьНизкоВероятные);
		...
		Попытка
			...
			Тогда
				...
				Если мСтруктураТипаКонтекста.ИмяОбщегоТипа = "ВнешнийИсточникДанных.<Имя внешнего источника>.ВременнаяТаблица" Тогда
					Для Каждого СтрокаВременнойТаблицы Из ДоступныеТаблицы.НайтиСтроки(Новый Структура("Тип", "ВременнаяТаблица")) Цикл
						мПлатформа.ДобавитьВТаблицуСлов(ВыгрузкаТаблицыСлов, СтрокаВременнойТаблицы.Имя, "Свойство",, "ДоступнаяВременнаяТаблица", "Локальный");
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			Если Истина
			...
			Тогда 
				...
				Если ПараметрыМетодаМодуля <> Неопределено Тогда
					Для Каждого СтрокаПараметра Из ПараметрыМетодаМодуля Цикл 
						...
						мПлатформа.ДобавитьВТаблицуСлов(ВыгрузкаТаблицыСлов, СтрокаПараметра.Имя, "Свойство",, ТипЗначения, "Локальный",, мМетодМодуля.СтарыеТипы);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;  
			Если Истина
			...
			Тогда
				...
				Для Каждого ЭлементСписка Из мПакетЗапросов.Структура Цикл 
					Если ЗначениеЗаполнено(ЭлементСписка.Представление) Тогда
						мПлатформа.ДобавитьВТаблицуСлов(ВыгрузкаТаблицыСлов, ЭлементСписка.Представление, "Свойство",, "ДоступнаяВременнаяТаблица", "Локальный", Ложь);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;  
			Если Истина
	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		ПрименитьОжидаемыйТипВТаблицеСлов(ОтделятьБольшиеНаборыСлов); 
		Если мЯзыкПрограммы = 1 И мЭтоЛокальныйКонтекстТаблицыСлов И Не мЭтоОбъявлениеПсевдонима Тогда
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЕСТЬ NULL", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЛЕВОЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ПРАВОЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ВНУТРЕННЕЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ВНЕШНЕЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "СГРУППИРОВАТЬ ПО", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "УПОРЯДОЧИТЬ ПО", "Конструкция",, "", "Локальный",,, Истина);
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ОБЪЕДИНИТЬ ВСЕ", "Конструкция",, "", "Локальный",,, Истина);
		КонецЕсли;
		Если Истина                                                    
			И мЭтоЛокальныйКонтекстТаблицыСлов
			И мЯзыкПрограммы = 0
		...
		Тогда
			// Подавляем обычно бесполезную функцию, но при этом стоящую выше многих полезных переменных, имеющих то же начало
			СтрокаСлова = ТаблицаСлов.НайтиСтроки(Новый Структура("НСлово, ТипСлова", НРег("Строка"), "Метод"));
			Если СтрокаСлова.Количество() Тогда
				СтрокаСлова = СтрокаСлова[0];
			Иначе
				СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Строка", "Метод",, "Строка", "Предопределенный", Ложь,, Истина);
			КонецЕсли;
			СтрокаСлова.Рейтинг = - КонстантаРейтингСовпаденияТипа;
			Если Не ОтделятьБольшиеНаборыСлов Тогда
				мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Для Каждого ", "Конструкция",, "", "Локальный",,, Истина);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	НеУспех = Истина 
		...
		И (Ложь
			Или мНаборыСлов <> Неопределено И мНаборыСлов.Количество() > 0
			Или ТаблицаСлов.Количество() > 50)
		И (Ложь
			Или Прав(ТекущаяСтрокаНачало, 1) = "="
			Или Прав(ТекущаяСтрокаНачало, 1) = " "
			Или Прав(ТекущаяСтрокаНачало, 1) = "("
			Или Прав(ТекущаяСтрокаНачало, 1) = ","
			Или (Истина
			...
			Или (Истина
				И мРодительскийКонтекст = ""
				И мЭтоТекстовыйЛитерал
				И Не ирОбщий.СтрКончаетсяНаЛкс(ТекущаяСтрокаНачало, """" + мКонтекст)));
	Возврат Не НеУспех;
КонецФункции

Процедура ПодготовитьТаблицуТиповКЗаполнениюТаблицыСлов(Знач ТаблицаТипов)
...
Процедура ЗаполнитьТипыЗначенийСлов(Знач ВнутренняяТаблицаСлов, Знач ПринудительноВсе = Ложь) Экспорт
	...
	Для Каждого НаборСлов Из НаборыСлов Цикл
		...
		Если ПринудительноВсе Или СловаДляУточненияТипа.Количество() < МаксЧислоСловДляЗаполненияТипов() Тогда
			Для Каждого ВнутренняяСтрокаСлова Из СловаДляУточненияТипа Цикл
				ОбновитьТипЗначенияИзТаблицыТипов(ВнутренняяСтрокаСлова,, Ложь);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Тогда 
		...
		Для Каждого КорневоеСловоБД Из КорневыеСловаБД Цикл
			...
			ИначеЕсли ирОбщий.СтрокиРавныЛкс(ПервоеСловоРодителя, КорневоеСловоБД) Тогда
				...
				мТаблицаСловЛитерала = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаИменБД, 1, Конфигурация, ВнешниеФункцииКомпоновкиДанных,,,, мФлагиКомпиляции,,,,,,, БазовоеРасширениеКонфигурации());
				Если Не ЗначениеЗаполнено(РодительскийКонтекст) Тогда
					ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюВКолонкеЛкс(мТаблицаСловЛитерала, "ТипЗначения", "");
					ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюВКолонкеЛкс(мТаблицаСловЛитерала, "ТипСлова", "Метод");
				КонецЕсли;
				мЯзыкПрограммы = 0;
				мКорневаяТаблицаТипов = Неопределено;
			КонецЕсли;
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Если мКонечнаяСтрока > 1 И мЯзыкПрограммы = 0 Тогда
		ПредшествующаяСтрока = СокрП(ирОбщий.ПервыйФрагментЛкс(СлужебноеПолеТекстаДолгое.ПолучитьСтроку(мКонечнаяСтрока - 1), "//"));
		Если Ложь
			Или Прав(ПредшествующаяСтрока, 1) = ";" 
			Или ирОбщий.СтрКончаетсяНаЛкс(ПредшествующаяСтрока, "цикл")
		Тогда
			// При вводе новой инструкции даем приоритет переменной, с которой начинается предшествующая инструкция
			мРегВыражение.Pattern = мПлатформа.шПустоеНачалоСтроки + "(?:Для\s+Каждого\s+)?(" + шИмя + ")\s*(?:=|\.|из)";
			Вхождения = мРегВыражение.НайтиВхождения(Символы.ПС + ПредшествующаяСтрока);
	...
	Если Не РазрешитьМногоТипов Тогда
		...
		ИменаТипов = ирОбщий.СкопироватьКоллекциюЛкс(ВсеИменаТипов);
		ирОбщий.УдалитьЗначениеИзМассиваЛкс(ИменаТипов, "Неопределено");
		Если ИменаТипов.Количество() > 1 Тогда
			// Нужно для конструктора ИсточникДоступныхНастроекКомпоновкиДанных
			ирОбщий.УдалитьЗначениеИзМассиваЛкс(ИменаТипов, "Строка");
		КонецЕсли;
		Если ИменаТипов.Количество() > 1 Тогда
			ирОбщий.УдалитьЗначениеИзМассиваЛкс(ИменаТипов, "Массив");
		КонецЕсли;
		Если ИменаТипов.Количество() > 1 Тогда
			ирОбщий.УдалитьЗначениеИзМассиваЛкс(ИменаТипов, "СписокЗначений");
		КонецЕсли;
		Если ИменаТипов.Количество() > 0 Тогда
			Если ЗначениеЗаполнено(ТаблицаОжидаемыхТипов) Тогда
				ЛучшийТип = ТаблицаОжидаемыхТипов.Найти(ИменаТипов[0], "ИмяОбщегоТипа");
				Если ЛучшийТип <> Неопределено Тогда
					СтрокаОписания = ЛучшийТип.СтрокаОписания;
					Попытка
				...
				КонецЕсли;
			КонецЕсли; 
			мИмяОжидаемогоТипа = ИменаТипов[0]; 
			ЛиСистемноеСлово = Истина
				И СтрокаОписания <> Неопределено
				И СтрокаОписания.Владелец().Колонки.Найти("ТипКонтекста") <> Неопределено;
			Если Ложь 
				Или (Истина
					И ИменаТипов.Количество() > 1 
					И (Ложь 
		...
		КонецЕсли;
	КонецЕсли;
	//Если ПустаяСтрока(мИмяОжидаемогоТипа) Тогда
	//	ИменаТипов.Очистить();
	//КонецЕсли;
	КонстантаРейтингСовпаденияТипа = КонстантаРейтингСовпаденияТипа();
	КлючОсновногоНабора = "";
	...
	Тогда
		КлючПоиска = Новый Структура;
		КлючПоиска.Вставить("ТипСлова", "Свойство");
		КлючПоиска.Вставить("Слово", ПеременнаяПредСтроки); // В закэшированных наборах НСлово может отсутствовать, например Глобальный-ЗначениеВЗапросе
	...
	Иначе
		...
		Если ПеречислимыеСвойстваМД.Найти(ИмяТаблицыБД, "Слово") <> Неопределено Тогда
			...
			Если Не ЗначениеЗаполнено(мРодительскийКонтекст) И УровеньСвойства = 0 Тогда
				...
				//ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора);
				СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Метаданные.СвойстваОбъектов." + ИмяТаблицыБД, "Конструкция",,, "Локальный", Ложь);
				СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
		...
		Иначе 
			...
			Если Истина
				...
				И (Ложь
					...
					Или (Истина
						...
						И (Ложь
							Или ЗначениеЗаполнено(РодительПриемника)
							Или (Истина
			...
			Тогда
				...
				ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора);
			КонецЕсли;
			ЛиПримитивныйТип = Ложь
				Или мИмяОжидаемогоТипа = "Булево"
				Или мИмяОжидаемогоТипа = "Строка"
				Или мИмяОжидаемогоТипа = "Число"
				Или мИмяОжидаемогоТипа = "Дата"
				Или мИмяОжидаемогоТипа = "Null";
			Если Истина
			...
			Тогда
				КлючПоиска = Новый Структура;
				КлючПоиска.Вставить("ТипЗначения", мИмяОжидаемогоТипа);
				ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора, ирОбщий.ПоследнийФрагментЛкс(ВыражениеПриемник));
				Если ПустаяСтрока(мРодительскийКонтекст) Тогда
					КлючПоиска = Новый Структура("НСлово, ЯзыкПрограммы, ЕстьКонструктор");
					КлючПоиска.ЯзыкПрограммы = 0;
					КлючПоиска.ЕстьКонструктор = Истина;
					Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
						КлючПоиска.НСлово = НРег(ИмяТипа);
						Если мПлатформа.ТаблицаОбщихТипов.НайтиСтроки(КлючПоиска).Количество() = 0 Тогда
							Продолжить;
						КонецЕсли;
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Новый " + ИмяТипа, "Конструкция",,, "Локальный", Ложь);
						Если Ложь
						...
						СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
					КонецЦикла;
					Если мИмяОжидаемогоТипа = "Тип" Тогда
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Тип(""" + ВыражениеПриемник + """)", "Конструкция",, "Тип", "Локальный", Ложь);
						СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
						СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
					КонецЕсли;
					Если Истина
						И ТаблицаОжидаемыхТипов <> Неопределено
						И ТаблицаОжидаемыхТипов.Количество()
						И ТипЗнч(ТаблицаОжидаемыхТипов[0].СтрокаОписания) = Тип("СтрокаТаблицыЗначений")
						И ТаблицаОжидаемыхТипов[0].СтрокаОписания.Владелец().Колонки.Найти("ТелоБезВозвратов") <> Неопределено
					Тогда
						// Функция-конструктор
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, ПолноеИмяМетода(ТаблицаОжидаемыхТипов[0].СтрокаОписания, Истина), "Метод",,, "Метаданные", Ложь);
						СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
						СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
					КонецЕсли;
				КонецЕсли;
...
Функция РассчитатьОжидаемыйТипВыражения(ЭтоСсылочныйЛитерал = Неопределено) Экспорт
	мСтрокаЛучшегоСлова = Неопределено;
	мИмяОжидаемогоТипа = "";
	ВыражениеПриемник = "";
	Если Ложь
	...
	Если Ложь
		...
		Или мПредшествующийТекст <> мКэшОжидаемогоТипа.мПредшествующийТекст
	Тогда
		ДлинаПредТекста = 200;
		...
		Если ПредшествующийТокен = "," Или ПредшествующийТокен = "(" Тогда
			...
			Если ЗначениеЗаполнено(мВызовМетода) Тогда
				...
				ДляСвойства = ""; // иначе не найдутся параметры запроса // TODO придумать как ускорить
				//КонецЕсли;
				...
				Если ТаблицаТиповВызова.Количество() > 0 Тогда
					ТаблицаОжидаемыхТипов = ТаблицаОжидаемыхТиповПараметраМетода(ТаблицаТиповВызова[0], ВыражениеПриемник, СтруктураВызова.ВыражениеПриемник);
				КонецЕсли;
			КонецЕсли; 
		ИначеЕсли Ложь
			Или ПредшествующийТокен = "=" 
		...
		Тогда
			...
			ВыражениеПриемник = РассчитатьИмяОжидаемогоТипа(ШаблонРазбора, ПредшествующийТекст, ТаблицаОжидаемыхТипов, ДлинаПредТекста);
		ИначеЕсли Истина
			И ПредшествующийТокен = """" 
			И ирОбщий.СтрокиРавныЛкс(мВызовМетода, "Тип(")
		Тогда
			ШаблонРазбора = "\sТипЗнч\((" + шПростоеВыражениеПрограммы + ")\)\s*(?:=|<>)\s*" + ирОбщий.ТекстДляРегВыраженияЛкс(мВызовМетода + """") + "$";
			ВыражениеПриемник = РассчитатьИмяОжидаемогоТипа(ШаблонРазбора, ПредшествующийТекст, ТаблицаОжидаемыхТипов, ДлинаПредТекста);
		...
		ИначеЕсли мЯзыкПрограммы = 1 Тогда
			...
			КонецЕсли;
			Если ирОбщий.ЛиТаблицаБДСуществуетЛкс(Выражение) Тогда
				мИмяОжидаемогоТипа = "КАК";
			КонецЕсли;
		КонецЕсли;
		мКэшОжидаемогоТипа = КэшОжидаемогоТипа(ТаблицаОжидаемыхТипов, ВыражениеПриемник, РазрешитьМногоТипов, ПредшествующийТокен);
	Иначе
...
Функция ТаблицаОжидаемыхТиповПараметраМетода(Знач СтруктураТипаМетода, ВыражениеПриемник = Неопределено, Знач ПеременнаяКонструктора = "", Знач ВариантСинтаксиса = Неопределено) Экспорт
	СтрокаОписанияМетода = СтруктураТипаМетода.СтрокаОписания;
	ТаблицаОжидаемыхТипов = Неопределено;
	ВыражениеПриемник = ИмяПараметраВызоваМетода(СтрокаОписанияМетода, Ложь, ТаблицаОжидаемыхТипов, СтруктураТипаМетода, ВариантСинтаксиса);
	...
	Тогда
		...
		Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяКлюча) Тогда
			...
			Если ЗначениеЗаполнено(Переменная) Тогда
				ТаблицаВозможныхСлов = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(Переменная,, "Структура"); // см. мПлатформа.НоваяТаблицаСлов() - для ускорения передать фильтр по слову
				Если Истина
	...
	ИначеЕсли Истина
		...
		И мНомерПараметра > 2
	Тогда
		ВыражениеВызоваМетода = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(мФактическиеПараметры[1]) + "(" + ИмяОтключенияЛишнихВычислений() + ")";
...
Функция РассчитатьИмяОжидаемогоТипа(Знач ШаблонРазбора, Знач НачалоПредТекста, выхТаблицаОжидаемыхТипов, Знач ДлинаПредТекста) Экспорт
	...
	Если Вхождения.Количество() > 0 Тогда
		...
		Если ирОбщий.ЛиИмяПеременнойЛкс(ВыражениеПриемник) Тогда
			...
			ИмяСтарогоТипа = СтарыеТипы[ВыражениеПриемник];
		...
		Если мИмяОжидаемогоТипа = Неопределено Тогда
			...
			Иначе
				ДляСвойства = "";
			КонецЕсли;
			Смещение = Вхождения[0].FirstIndex + Найти(Вхождения[0].Value, ВыражениеПриемник) - 2 - ДлинаПредТекста;
			ТаблицаТипов = ВычислитьТипЗначенияВыражения(ВыражениеПриемник,,,,,,,,,,, мПозицияВБлоке + Смещение,, ДляСвойства);
			ИмяТипа = мПлатформа.ПредставлениеМассиваСтруктурТипов(ТаблицаТипов);
			Если СтарыеТипы <> Неопределено Тогда
				СтарыеТипы[ВыражениеПриемник] = ИмяТипа; // Жертвуем свежестью ради скорости
			КонецЕсли; 
			Если ЛиВложенныйТип Тогда
				мИмяОжидаемогоТипа = "Тип";
				ВыражениеПриемник = "";
				Для Каждого СтрокаТаблицы Из ТаблицаТипов Цикл
					ИмяТипа = СтрокаТаблицы.ИмяОбщегоТипа;
					Если Ложь
						Или ИмяТипа = "?"
...
// Возвращаемое значение:
//    Структура - 
Функция КэшОжидаемогоТипа(Знач ТаблицаОжидаемыхТипов, Знач ВыражениеПриемник, Знач РазрешитьМногоТипов, Знач ПредшествующийТокен) Экспорт
	Результат = Новый Структура;
	...
	Результат.Вставить("мПредшествующийТекст", мПредшествующийТекст);
	Возврат Результат;
КонецФункции
...
Процедура ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "") Экспорт
	...
	Если КлючПоиска.Свойство("ТипЗначения") Тогда
		Если КлючПоиска.Количество() > 1 Тогда
			ВариантыИмениТипа.Добавить("*");
			КлючПоискаБезТипа.Удалить("ТипЗначения");
		КонецЕсли;
		ВариантыИмениТипа.Добавить(КлючПоиска.ТипЗначения);
		ВариантыИмениТипа.Добавить("??, " + КлючПоиска.ТипЗначения);
		ВариантыИмениТипа.Добавить("??, " + КлючПоиска.ТипЗначения + ", Неопределено");
	Иначе
		ВариантыИмениТипа.Добавить("");
	КонецЕсли;
	КонстантаРейтингСовпаденияТипа = КонстантаРейтингСовпаденияТипа();
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			Если Истина
				И КлючПоиска.Свойство("ТипЗначения")
				И КлючПоиска.Количество() > 1 
			Тогда
				УточнитьТипЗначенияВСтрокеТаблицыСлов(мСтруктураТипаКонтекста, Найденные[0]);
				Если Не ирОбщий.СтрНайтиЛкс(ирОбщий.ПервыйФрагментЛкс(Найденные[0].ТипЗначения, "(") + ",", КлючПоиска.ТипЗначения + ",") Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			мСтрокаЛучшегоСлова = Найденные[0];
			Если КлючИЗначение.Ключ <> КлючОсновногоНабора Тогда
				КлючПоиска1 = Новый Структура("НСлово, ТипСлова");
				КлючПоиска1.НСлово = НРег(мСтрокаЛучшегоСлова.Слово);
				КлючПоиска1.ТипСлова = мСтрокаЛучшегоСлова.ТипСлова;
			...
			КонецЕсли;
			мСтрокаЛучшегоСлова.Рейтинг = мСтрокаЛучшегоСлова.Рейтинг + КонстантаРейтингСовпаденияТипа;
			Прервать;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
//
Функция ЗаполнитьТаблицуВозможныхЗначенийЛитерала(Знач ВыбраннаяСтруктураТипа, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	...
	Тогда
		...
		Если СтрокаОписанияМетода.ТипСлова <> "Свойство" Тогда
						...
						Если ТаблицаТиповОбрамляющегоВызова.Количество() > 0 Тогда
							...
							Иначе
								...
								Если ТипЗнч(СтрокаМетода) = Тип("СтрокаТаблицыЗначений") Тогда
									...
									Тогда
										мТаблицаСловЛитерала = ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(СтрокаМетода,, Ложь);
									Иначе
			...
			Если ЗначениеЗаполнено(ИмяПеременнойСтруктуры) Тогда
				ТаблицаСловЛитерала = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ИмяПеременнойСтруктуры,, СтрокаОписанияМетода.ТипКонтекста);
				Если ТаблицаСловЛитерала <> Неопределено Тогда
					мТаблицаСловЛитерала = ТаблицаСловЛитерала;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		ИначеЕсли Истина
			...
			И ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("ОбъектМетаданных")
		Тогда  
			//КоллекцияЭлементовМД = ирОбщий.ОтобратьКоллекциюЛкс(ВыбраннаяСтруктураТипа.Метаданные.Реквизиты, "Э.Индексирование<>П1",, Метаданные.СвойстваОбъектов.Индексирование.НеИндексировать);
		...
		Тогда
			КоллекцияЭлементовМД = мПлатформа.ДочерниеСвойстваДанныхФормы(ВыбраннаяСтруктураТипа.Метаданные, Истина, ВыбраннаяСтруктураТипа.ДержательМетаданных);
		ИначеЕсли Истина
			И ИмяПараметра = "ИмяРеквизита"
			И (Ложь
		...
		Тогда
			КоллекцияЭлементовМД = мПлатформа.ИменаРеквизитовФормы(ВыбраннаяСтруктураТипа.Метаданные, "").Все;
		ИначеЕсли Истина
		...
		Тогда
			Если ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса") Тогда
				ТекстЗапроса = ТекстИмитаторовВременныхТаблиц(ВыбраннаяСтруктураТипа.ДержательМетаданных, ВыбраннаяСтруктураТипа.Метаданные.Текст);
				КоллекцияЭлементовМД = мПлатформа.ПараметрыИзТекстаЗапроса(ТекстЗапроса, Истина);
				Для Каждого КлючИЗначение Из ВыбраннаяСтруктураТипа.Метаданные.Параметры Цикл
					Если Истина
		...
		Тогда
			...
			Если СтруктураТипаПараметра.Количество() > 0 Тогда
				мТаблицаСловЛитерала = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаПараметра[0],,,,,, "Свойство",,,,,,,, БазовоеРасширениеКонфигурации());
			КонецЕсли;
		ИначеЕсли Истина
		...
		Тогда
			...
			КоллекцияЭлементовМД = КомпоновщикНастроек.Настройки.ПараметрыДанных.ДоступныеПараметры.Элементы;
		ИначеЕсли Истина
			И (Ложь
		...
		Тогда
			мТаблицаСловЛитерала = ВыбраннаяСтруктураТипа.Метаданные.Поля;
		ИначеЕсли Истина
			И СтрокаОписанияМетода.Слово = "Найти"
			И ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("КоллекцияОбъектовМетаданных")
			И Не мПлатформа.мРежимПроверкиМодуля
		Тогда
			КоллекцияЭлементовМД = ВыбраннаяСтруктураТипа.Метаданные;
		ИначеЕсли Истина
			И СтрокаОписанияМетода.Слово = "Найти"
	...
	Тогда 
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
					...
					Для Каждого СтруктураТипаСсылки Из ТаблицаТиповСсылки Цикл
						...
						Если ТипЗнч(СтруктураТипаСсылки.Метаданные) = Тип("ОбъектМетаданных") Тогда
							...
							Если ЗначениеЗаполнено(ИмяТаблицыБД) Тогда
								// Мультиметка81792365
								АнализаторКода = ирОбщий.НовыйАнализаторКодаЛкс();
								АнализаторКода.ИнициироватьНеинтерактивно(1);
								...
								АнализаторКода.ЗаполнитьТаблицуСлов(, Ложь,, Ложь, Ложь);
								мТаблицаСловЛитерала = мПлатформа.НоваяТаблицаСлов();
			...
			Иначе
				...
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(СтрокаОписанияМетода, ЛиНаборЗначенийПолный, ИмяТипаСтруктуры, ДобавлятьНизкоВероятные);
			КонецЕсли;
			Если мЭтоТекстовыйЛитерал Тогда
				ВыбраннаяСтруктураТипа.ТипЯзыка = ИмяПараметра;
			КонецЕсли;
	...
	ИначеЕсли ИмяТипаСтруктуры = "" Тогда
		...
		Если Прав(ВыражениеСлеваОтРавно, 1) = "=" Тогда
			...
			Если Истина
				...
				И мРодительскийКонтекст = ""
			Тогда
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ВыражениеСлеваОтРавно, ЛиНаборЗначенийПолный,, Истина, ДобавлятьНизкоВероятные);
				ДобавитьПростыеСловаВТаблицу(КоллекцияЭлементовМД, Истина);
				КоллекцияЭлементовМД = Новый Массив;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Тогда
		ЛиНаборЗначенийПолный = Истина;
		ТаблицаТиповКолонок = ВычислитьТипДочернегоЭлемента(ВыбраннаяСтруктураТипа.Метаданные[0].ИсточникДанных, "Колонки", "Свойство");
		ЛучшийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТиповКолонок, 4);
		Если ЛучшийТип <> Неопределено Тогда
			КоллекцияЭлементовМД = мПлатформа.ТаблицаСловИзСтруктурыТипа(ЛучшийТип,,,,,, "Свойство").ВыгрузитьКолонку("Слово");
		КонецЕсли;
	ИначеЕсли Истина
...
// Возвращаемое значение:
//    Массив, ТаблицаЗначений - 
Функция ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(Знач СтрокаМетода, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь)
	...
	ЛиКорневойВызов = Ложь;
	ВызывающийМетод = мМетодМодуля;
	Если мРекурсивныйПуть = Неопределено Тогда
		ЛиКорневойВызов = Истина;
	...
	//ИндексПараметра = -1;
	//Для Каждого СтрокаПараметра Из мПлатформа.ПараметрыМетодаМодуля(СтрокаМетода) Цикл
	//	Если мФактическиеПараметры.Количество() = ИндексПараметра + 1 Тогда
	//		Прервать;
	//	КонецЕсли;
	//	ИндексПараметра = ИндексПараметра + 1;
	...
	//		И ИмяПараметра <> СтрокаПараметра.Имя 
	//	Тогда
	//		СтрокаМетода.КлючевыеПараметры.Вставить(СтрокаПараметра.Имя, ВычислитьТипЗначенияВыражения(ВыражениеПараметра,,,,,, Ложь,,,,, мПозицияВБлоке));
	//	КонецЕсли;
	//КонецЦикла;
	//мРекурсивныйПуть.ДопКлючКэша = ДопКлючКэшаДляВычисленийВМетоде(СтрокаМетода);
	//
	// Теперь сначала вычисляем оптимистичный вариант и одновременно собираем влияющие параметры
	...
	КоллекцияЭлементовМД = АнализаторКодаМодуля.ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ИмяПараметра, ЛиНаборЗначенийПолный, ИмяТипаСтруктуры,, ДобавлятьНизкоВероятные);
	
	//
	// Если при вычислении обнаружены зависимости от параметров, то повторяем вычисление после расчета типов этих параметров
	Если СтрокаМетода.КлючевыеПараметры.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПараметра Из мПлатформа.ПараметрыМетодаМодуля(СтрокаМетода) Цикл
			...
			КонецЕсли;
			ИндексПараметра = ИндексПараметра + 1;
			Если Не СтрокаМетода.КлючевыеПараметры.Свойство(СтрокаПараметра.Имя) Тогда
				Продолжить;
			КонецЕсли; 
			ВыражениеПараметра = мФактическиеПараметры[ИндексПараметра];
			Если Истина
			...
			Тогда
				...
				Если мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповПараметра) Тогда
					СтрокаМетода.КлючевыеПараметры[СтрокаПараметра.Имя] = ТаблицаТиповПараметра;
	...
	Если ЛиКорневойВызов Тогда
		...
		//Если мРекурсивныйПуть <> Неопределено Тогда // Эта проверка маскирует некорректную передачу флага ЛиКорневойВызов обычно в ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода
			мРекурсивныйПуть.ДопКлючКэша = ДопКлючКэшаДляВычисленийВМетоде();
		//КонецЕсли;
	КонецЕсли;
	Возврат КоллекцияЭлементовМД;
КонецФункции
...
Функция СписокРеквизитовУправляемойФормы(Знач Форма, Знач ПутьКРодителю = "") Экспорт
	...
	Для Каждого КлючИЗначение Из мПлатформа.ИменаРеквизитовФормы(Форма, ПутьКРодителю, ЛиДляКоллекции).Все Цикл
		КоллекцияЭлементовМД.Колонки.Добавить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
	КоллекцияЭлементовМД = КоллекцияЭлементовМД.Колонки;
	Возврат КоллекцияЭлементовМД;
КонецФункции
...
//   - 
//
Функция ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(Знач ИмяПеременной, ЛиНаборЗначенийПолный = Ложь, Знач ИмяТипаСтруктуры = "", Знач ЛиСобиратьПрисвоения = Ложь, Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	...
	Если ИмяТипаСтруктуры = "" Тогда
		...
		Если Истина
			И Не ЛиНаборЗначенийПолный 
			И Найти(мТекстБлока, "[" + ИмяПеременной + "]") > 0 
		Тогда
			ВхожденияОбъектов = НайтиВхожденияРегВыраженияКэш("(" + шОбъектноеВыражение + ")\[" + ИмяШаблон + "\]",,, "ОбъектСВКвадратнымиСкобками;" + ИмяПеременной);
		...
		Если Истина
			...
			И (Ложь
				...
				Или Найти(мТекстБлока, ИмяПеременной + " =") > 0)
		Тогда
			ШаблонПоиска = шПредИмяПрямое + "(?:(?:Если|ИначеЕсли|И|Или|If|ElseIf|And|Or)\s|\(|,)";
	...
	ИначеЕсли ИмяТипаСтруктуры = "Структура" Тогда 
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Если ТаблицаТипов.Количество() > 0 Тогда
				Результат = мПлатформа.СловаКонтекстаМетаданные(ТаблицаТипов[0],, "Свойство");
			КонецЕсли;
			Возврат Результат;
		КонецЦикла;
				...
				Если ТаблицаТипов.Количество() > 0 Тогда
					...
					ИначеЕсли ТипЗнч(МетаданныеТипа) = Тип("ТаблицаФормы") Тогда
						...
						Если ТаблицаТиповДочерняя.Количество() > 0 Тогда
							Результат = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповДочерняя[0],,,, Ложь,, "Свойство");
						КонецЕсли;
					КонецЕсли;
		...
		Тогда
			...
			Для Каждого Вхождение Из ВхожденияОбъектов Цикл
				...
				Если ЕстьСовпадение И ЗначениеЗаполнено(ВыражениеИсточник) Тогда
					...
					Если ТаблицаТипов.Количество() > 0 Тогда
						ЛучшийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТипов);
						Результат = мПлатформа.ТаблицаСловИзСтруктурыТипа(ЛучшийТип,,,,,, "Свойство",,,,,,,, БазовоеРасширениеКонфигурации);
	...
	ИначеЕсли ИмяТипаСтруктуры = "Массив" Тогда
			...
			Если СтрокаПараметра <> Неопределено Тогда
				...
				Тогда
					...
					ИначеЕсли ирОбщий.ЛиФормаИлиИмитаторЛкс(мМодульМетаданных.СтруктураТипа.Метаданные) Тогда
						РеквизитыФормы = СписокРеквизитовУправляемойФормы(мМодульМетаданных.СтруктураТипа.Метаданные);
						Результат = мПлатформа.НоваяТаблицаСлов();
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Если Не ирОбщий.СтрокиРавныЛкс(Вхождение.SubMatches(1), ИмяПеременной) Тогда
				Продолжить;
			КонецЕсли;
			НовыеЗначенияПараметра = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(Вхождение.SubMatches(0), ЛиНаборЗначенийПолный,,, ДобавлятьНизкоВероятные);
			Если НовыеЗначенияПараметра.Количество() > 0 Тогда
				Результат = мПлатформа.НоваяТаблицаСлов();
				Для Каждого Слово Из НовыеЗначенияПараметра Цикл
					мПлатформа.ДобавитьВТаблицуСлов(Результат, Слово);
				КонецЦикла;
				Возврат Результат;
			КонецЕсли;
		КонецЦикла;
	...
	Пока Истина Цикл
		...
		ВхожденияВызовов = НайтиВхожденияРегВыраженияКэш("[^" + мПлатформа.шБукваЦифра + "]((Новый\s+)?(?!(?:ТипЗнч|НРег|ВРег|Лев|Прав|Сред|СокрЛ|СокрП|СокрЛП|Найти|СтрНайти|СтрДлина|СтрРазделить)\s*\()(" 
		...
		Для Каждого Вхождение Из ВхожденияВызовов Цикл
			...
			Если Вхождение.SubMatches(11) <> Неопределено Тогда
				Если ПустаяСтрока(ИмяПереннойПодмены) Тогда
			...
			Если ТаблицаТипов.Количество() > 0 Тогда
				...
				НовыеЗначенияПараметра = ЗаполнитьТаблицуВозможныхЗначенийЛитерала(ТаблицаТипов[0], ЛиНаборЗначенийПолный, ИмяТипаСтруктуры, ДобавлятьНизкоВероятные);
				Если Истина
					И ИмяТипаСтруктуры = "" 
	...
	Иначе
		...
		Для Каждого ЭлементКоллекцииМД Из КоллекцияЭлементовМД Цикл
			...
			Если ЛиПриоритетно Тогда
				СтрокаСлова.Рейтинг = РейтингСовпаденияТипа;
			КонецЕсли;
			Если ЗначениеЗаполнено(ИмяОписанияТипа) Тогда
				...
				СтрокаСловаСТипом.ТаблицаТипов = мПлатформа.ТаблицаТиповИзОписанияТипов(ЭлементКоллекцииМД[ИмяОписанияТипа]);
				Если ЗаполнятьТип Тогда
...
//  Строка - 
//
Функция ИмяПараметраВызоваМетода(СтрокаМетода, Знач СТипомСтрока = Истина, выхТаблицаТипов = Неопределено, Знач СтруктураТипаМетода = Неопределено, Знач ВариантСинтаксиса = Неопределено) Экспорт
	ИмяПараметра = "";
	Если СтрокаМетода = Неопределено Тогда
		Возврат ИмяПараметра;
	КонецЕсли;
	Если СтрокаМетода.Владелец().Колонки.Найти("ТелоБезВозвратов") <> Неопределено Тогда  
		...
		Если ПараметрыМетодаМодуля <> Неопределено И ПараметрыМетодаМодуля.Количество() >= мНомерПараметра Тогда
			...
			Если Не СТипомСтрока Тогда 
				выхТаблицаТипов = ТаблицаТиповИзПараметраМетодаМодуля(СтрокаМетода, СтрокаПараметра);
			КонецЕсли;
		КонецЕсли;
	Иначе 
		// Системный метод
...
Функция ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, ТаблицаТиповКонтекста = Неопределено, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	Тогда
		...
		Если ПараметрЗакрытияПодсказки = Истина Тогда
			ТаблицаМетодовМодуля = мПлатформа.СловаКонтекстаМетаданные(СтруктураТипаКонтекста, Слово, "Метод",,,,, Истина, мФлагиКомпиляции, МодульМетаданныхКонтекста(СтруктураТипаКонтекста),,,
				мЭтоЛокальныйКонтекстТаблицыСлов,, БазовоеРасширениеКонфигурации());
			Если Истина
	...
	Тогда
		СтрокаДобавка = СтрокаДобавка + " ";
		СтруктураТипаКонтекста = НоваяСтруктураТипа();
		СтруктураТипаКонтекста.ИмяОбщегоТипа = "";
		СтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа";
		СтруктураТипаКонтекста.Конструктор = Истина;
		ТаблицаТиповКонтекста = мПлатформа.ДобавитьВТаблицуТипов(, СтруктураТипаКонтекста);
	КонецЕсли;
	...
	ВосстановитьГраницыВыделенияПослеКоманды();
	Возврат Истина
		И ТаблицаТиповКонтекста <> Неопределено
		И ТаблицаТиповКонтекста.Количество() > 0;
КонецФункции
...
Функция ЛиСжиматьВнутренниеПустотыВСтроке() Экспорт
	Результат = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".СжиматьВнутренниеПустотыВСтроке");
	Возврат Результат;
КонецФункции

Функция ЛиСжиматьПустыеСтроки() Экспорт
	Результат = ирОбщий.ВосстановитьЗначениеЛкс(ИмяКласса + ".СжиматьПустыеСтроки");
	Возврат Результат;
КонецФункции
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаТипаКонтекста, Знач ТекущаяСтрока, Знач ВернутьСтруктуруТипа = Ложь, Знач НужноУточнитьТип = Истина) Экспорт 
	#Если Сервер И Не Сервер Тогда
	...
	Если НужноУточнитьТип Или ВернутьСтруктуруТипа Тогда
		...
		Иначе 
			...
			Для Каждого СтрокаТипаКонтекста Из ТипыКонтекста Цикл
				ОписанияСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтрокаТипаКонтекста, мЯзыкПрограммы, Конфигурация,, Истина,, ТекущаяСтрока.ТипСлова, мФлагиКомпиляции, РодительИСлово.Слово,
...
//  ТаблицаТипов	 - см. ОбработкаОбъект.ирПлатформа.НоваяТаблицаТипов() - 
Функция ВычислитьРезультатМетодаШаблон(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
КонецФункции
...
// см. ВычислитьРезультатМетодаШаблон 
Функция ВычислитьПрочитатьJSON(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() > 0 Тогда
...
// см. ВычислитьРезультатМетодаШаблон 
Функция ВычислитьБлокировкаДанныхДобавить(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = МассивПараметров[0];
...
Функция ВычислитьОтобратьКоллекциюЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	...
	СтруктураТипа.Метаданные = ТаблицаТиповЭлементов[0].Метаданные;
	Результат = мПлатформа.ДобавитьВТаблицуТипов(, СтруктураТипа);
	Возврат Результат;
...
// см. ВычислитьРезультатМетодаШаблон 
Функция ВычислитьНеизменяющаяСтроковаяФункция(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
// см. ВычислитьРезультатМетодаШаблон 
Функция ВычислитьОператорВыбора(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() < 2 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Результат = Неопределено;
...
Функция ОписаниеТекущегоСловаАвтодополнения(Знач Слово, Знач ЛиМетод = Неопределено, Знач КлючНабораСлов = "", Знач ЛиОтдельноеОписаниеАктивно = Истина, Знач ТолькоДляАвтооткрытия = Ложь) Экспорт 
	...
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		СтрокаТипаКонтекста = мСтруктураТипаКонтекста;
		ТаблицаТипов = УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаТипаКонтекста, СтрокаТаблицыСлов, ЛиОтдельноеОписаниеАктивно);
		...
		Если ЛиОтдельноеОписаниеАктивно И ТаблицаТипов <> Неопределено Тогда 
			...
			Если ЗначениеЗаполнено(ДетальноеОписаниеДляСписка) Тогда
				...
				Если ЛучшийТип <> Неопределено Тогда
					...
					Если Истина
						И ТипЗнч(МетаданныеТипа) = Тип("Структура") 
						И ирОбщий.СвойствоСтруктурыЛкс(МетаданныеТипа, "Тип") <> ирОбщий.ТипУправляемаяФормаЛкс() // имитатор формы
					Тогда 
						СписокСвойств = Новый СписокЗначений;
						СписокСвойств.ЗагрузитьЗначения(ирОбщий.ВыгрузитьСвойствоЛкс(МетаданныеТипа));
						СписокСвойств.СортироватьПоЗначению();
						СтруктураОписания.Добавить("> Свойства*: " + ирОбщий.СтрСоединитьЛкс(СписокСвойств));
					ИначеЕсли ТипЗнч(МетаданныеТипа) = Тип("ТаблицаЗначений") Тогда 
						СтруктураОписания.Добавить("> Элемент*: " + ПредставлениеМассиваСтруктурТипов(МетаданныеТипа));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли; 
			Если ЛучшийТип <> Неопределено Тогда  
				СтрокаСловаМодуля = ЛучшийТип.СтрокаОписания; // см. мПлатформа.НоваяТаблицаМетодовМодуля()[0]
				Если Истина
					И ТипЗнч(СтрокаСловаМодуля) = Тип("СтрокаТаблицыЗначений")
					И СтрокаСловаМодуля.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено
				Тогда
					Если Истина
						И ЗначениеЗаполнено(СтрокаСловаМодуля.ТипЗначения) 
						И СтрокаТаблицыСлов.ТипЗначения <> СтрокаСловаМодуля.ТипЗначения
						И Не ирОбщий.СтрНачинаетсяСЛкс(СтрокаСловаМодуля.ТипЗначения, "??")
					Тогда
						СтруктураОписания.Добавить("> Тип: " + СтрокаСловаМодуля.ТипЗначения);
					КонецЕсли;
					Если Истина
						И мПлатформа.ЛиИмяМодуляФормы(СтрокаСловаМодуля.ИмяМодуля) 
						И СтрокаСловаМодуля.ПозицияОпределения > 0
						И СтрокаТаблицыСлов.ТипСлова = "Метод" 
					Тогда
						ИмяОбщегоТипа = мПлатформа.МодульМетаданныхИзКэша(СтрокаСловаМодуля.ИмяМодуля).СтруктураТипа.ИмяОбщегоТипа;
						Если ирОбщий.ЛиИмяТипаФормыЛкс(ИмяОбщегоТипа) Тогда
							ДоступностьМассив = Новый Массив;
							ДоступностьСервер = "";
							Если СтрокаСловаМодуля.БезКонтекста Тогда
								ДоступностьСервер = "СерверБезКонтекста";
							ИначеЕсли СтрокаСловаМодуля.Сервер Тогда
								ДоступностьСервер = "Сервер";
							КонецЕсли;
							Если ЗначениеЗаполнено(ДоступностьСервер) Тогда
								ДоступностьМассив.Добавить(ДоступностьСервер);
							КонецЕсли;
							Если СтрокаСловаМодуля.Клиент Тогда
				...
				ИначеЕсли Истина
					И ТипЗнч(СтрокаСловаМодуля) = Тип("СтрокаТаблицыЗначений")
					И СтрокаСловаМодуля.Владелец().Колонки.Найти("Знач") <> Неопределено
				Тогда
					СтрокаПараметра = СтрокаПараметраМетодаМодуля(СтрокаСловаМодуля.Имя);
				...
				ИначеЕсли ЛучшийТип.ИмяОбщегоТипа = "ОбщийМодуль" Тогда
			...
			Если Истина
				...
				И (Ложь
					...
					Или (Истина
						И СтрокаТаблицыСлов.ТипСлова <> "Метод" 
						И СтрокаТаблицыСлов.ТипСлова <> "Событие"))
			Тогда
				Если Ложь
					Или СтрокаТипаКонтекста.Конструктор 
					Или СтрокаТипаКонтекста.ТипЯзыка = "ИмяТипа"
				Тогда
					Найденные = мПлатформа.ТаблицаОбщихТипов.НайтиСтроки(Новый Структура("ЯзыкПрограммы, Слово", 0, СтрокаТаблицыСлов.Слово));
					Если Найденные.Количество() > 0 Тогда
						ЛучшийТип = мПлатформа.НоваяСтруктураТипа();
						ЛучшийТип.ИмяОбщегоТипа = СтрокаТаблицыСлов.Слово;
						ЛучшийТип.ТипЯзыка = "ИмяТипа";
						ЛучшийТип.СтрокаОписания = Найденные[0];
					КонецЕсли;
				КонецЕсли;
				Если ЛучшийТип.СтрокаОписания <> Неопределено Тогда
					Если Истина
						И мЯзыкПрограммы = 1 
						И ТипЗнч(СтрокаТипаКонтекста.Метаданные) = Тип("ОбъектМетаданных")
						И СтрокаТаблицыСлов.ТипСлова = "Поле"
					Тогда 
						ИмяПоля = СтрокаТаблицыСлов.Слово;
						...
						Если ИндексыХранения.Количество() Тогда
							СтруктураОписания.Добавить("> Индексы: " + ирОбщий.СтрСоединитьЛкс(ирОбщий.ВыгрузитьСвойствоЛкс(ИндексыХранения, "Представление")));
						КонецЕсли;
						Если Истина
						...
						КонецЕсли;
					КонецЕсли;
					Если ЛиСистемноеСлово(ЛучшийТип) Тогда  
					...
					Иначе
						СтрокаПеременной = ЛучшийТип.СтрокаОписания; // см. мПлатформа.НоваяТаблицаПеременныхМодуля()[0]
						Если Истина
							И ТипЗнч(СтрокаПеременной) = Тип("СтрокаТаблицыЗначений")
							И СтрокаПеременной.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено
						Тогда
							СтруктураОписания.Добавить("> Переменная модуля: " + ?(СтрокаПеременной.ЛиЭкспорт, "Экспорт", "Внутр"));
			...
			Если Ложь
				...
				Или СтрокаТаблицыСлов.ТипСлова = "Событие" 
			Тогда
				Если СтрокаТипаКонтекста.Конструктор Тогда
					СтруктураТипа = мПлатформа.НоваяСтруктураТипа();
				...
				Если ЛучшийТип <> Неопределено Тогда
					...
					Если СтруктураПодсказкиМетода.Количество() > 0 Тогда
						СтруктураОписания.Добавить("> Параметры: " + СтруктураПодсказкиМетода[0].label);
						Если СтруктураПодсказкиМетода.Количество() > 1 Тогда
							СтруктураОписания.Добавить("> Варианты: " + СтруктураПодсказкиМетода.Количество());
						КонецЕсли; 
					КонецЕсли; 
					Если СтрокаТаблицыСлов.ТипСлова = "Метод" Тогда 
...
Функция ПроверитьВыраженияУровня(Знач Вхождения, Знач ПроверятьВсеЛитералы = Ложь, Знач ТабличноеПоле = Неопределено, Знач Форма = Неопределено, Знач Индикатор = Неопределено,
		...
		Если ВыражениеЛитерала <> Неопределено Тогда 
			...
			Тогда
				...
				Тогда
					...
					ЗаполнитьТаблицуСлов(ТаблицаТипов.Скопировать(), Ложь,, Ложь, Ложь);
					мРекурсивныйПуть = РекурсивныйПуть;
					Если Истина
						И ТаблицаСлов.Количество() > 0
	...
	КонецЦикла;
	Если Этап = 1 Тогда
		ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("Проверка текущего метода (%1) завершена", ИмяТекущегоМетода()),,, Истина);
	КонецЕсли;
КонецФункции
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, Знач ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	ИначеЕсли мЯзыкПрограммы = 1 Тогда
		ВключаяРусские = ТипЗнч(Конфигурация) = Тип("Неопределено");
...
Функция ТаблицаСловДляАдаптера(Знач КлючНабораСлов) Экспорт
	...
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
		...
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл 
			...
			Строка.Приоритет = Цел(Строка.Рейтинг/РейтингСовпаденияТипа) * 100;
		КонецЦикла; 
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл  			Строка.ЛиРез = Строка.ТипЗначения <> "";  			Строка.ЛиМетод = Строка.ТипСлова = "Метод";  			Строка.ЛиТочный = Строка.Определение <> "Статистический";  			Строка.Тип = Лев(Строка.ТипЗначения, 200);  			Строка.Приоритет = Цел(Строка.Рейтинг/РейтингСовпаденияТипа) * 100;  		КонецЦикла;  
	КонецЕсли;
