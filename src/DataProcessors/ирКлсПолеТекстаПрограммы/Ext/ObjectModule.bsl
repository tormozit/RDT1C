...
Функция ВычислитьТипПараметраМетодаМодуля(Знач ПараметрыМетода, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Тогда
			ИмяМодуляОбъекта = ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(мИмяМодуля);
		...
		Для Каждого ВызывающийМодуль Из ВызывающиеМодули Цикл
			...
			Если Ложь
				...
				Или ирОбщий.СтрНайтиЛкс("." + ВызывающийМодуль.Имя + ".", "." + ИмяМодуляОбъекта + ".",,,, Ложь) > 0 // Мультиметка240720_160801 TODO переделать на анализ основного объекта формы
			Тогда
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		...
		Тогда
			...
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Строка", "Метод",, "Строка", "Предопределенный", Ложь,, Истина).Рейтинг = - КонстантаРейтингСовпаденияТипа;
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Если ТаблицаОжидаемыхТипов = Неопределено И мИмяОжидаемогоТипа = "" И ВыражениеПриемник = "" Тогда
		Возврат;
	КонецЕсли; 
	Если Истина
		//И мСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа"
...
Функция РассчитатьОжидаемыйТипВыражения(ЭтоСсылочныйЛитерал = Неопределено) Экспорт
	...
	Тогда
		...
		Тогда
			...
			Вхождения = мРегВыражение.НайтиВхождения(Символы.ПС + ТекстовыйДокумент.ПолучитьСтроку(ТекстовыйДокумент.КоличествоСтрок()));
...
Функция СписокРеквизитовУправляемойФормы(Знач Форма, Знач ПутьКРодителю = "") Экспорт
	КоллекцияЭлементовМД = Новый ТаблицаЗначений;
	Для Каждого КлючИЗначение Из мПлатформа.ИменаРеквизитовФормы(Форма, ПутьКРодителю).Все Цикл
		...
		Если Найти(мТекстБлока, "(" + ИмяПеременной) > 0 Тогда // Для ускорения. Экономит 3мс на больших текстах
			...
			Для Каждого Вхождение Из Вхождения Цикл
				...
				Если ТаблицаТипов.Количество() > 0 Тогда
					...
					МетаданныеТипа = ЛучшийТипИзТаблицыТипов(ТаблицаТипов).Метаданные;
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, Знач ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	НаборыСлов = НаборыСлов();
	Для Каждого НаборСлов Из НаборыСлов Цикл
		ирОбщий.ОбновитьКопиюКолонкиВНижнемРегистреЛкс(НаборСлов, "Слово");
	КонецЦикла;
