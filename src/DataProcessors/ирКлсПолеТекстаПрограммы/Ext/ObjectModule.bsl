...
Функция НайтиОпределениеСлова(Слово, ПоследнееОпределение = Неопределено, ПоследнееВхождение = Неопределено) Экспорт 
	...
	Если мЯзыкПрограммы = 1 Тогда
		мРегВыражение.Global = Ложь;
		мРегВыражение.Pattern = СтрЗаменить(шПоискОписанияТаблицы, "#Идентификатор#", ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(Слово));
		...
		Если РезультатТекста.Количество() > 0 Тогда
			...
			Тогда
				...
				мРегВыражение.Pattern = СтрЗаменить(ШаблонСозданияТаблицы, "#Идентификатор#", ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(Слово));
	...
	Иначе
		мРегВыражение.Global = Истина;
		ЭкранированноеСлово = ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ирОбщий.ПервыйФрагментЛкс(Слово, "("));
...
Функция ШаблонВызоваМетода(Знач ИмяВызова) Экспорт 
	...
	Иначе
		...
		Начало = "(?:[^" + шБуква + "\d\.]|^)";
	КонецЕсли;
	ШаблонВызова = Начало + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ИмяВызова) + "\s*\(((?:(?:" + шВыражениеПрограммы + ")?" + шРазделитель + "*,)*" + шВыражениеПрограммы + "?" + шРазделитель + "*)\)";
	...
	Если РазрешитьАнализИмениТипа Тогда
		...
		Иначе
			Если мЭтоСтроковыйЛитерал Тогда
				...
				Результат = ВычислитьТипЗначенияВыражения(мВызовМетода + ИмяОтключенияЛишнихВычислений + ")", ТекстДляПоискаОпределения, ПредшествующийТекст,, мЭтоКонструктор, ПолныйАнализ, Ложь);
				Возврат Результат;
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли; 
			...
			Если мЯзыкПрограммы = 1 Тогда
				Если РодительскийКонтекст = "" Тогда
					Если ЗначениеЗаполнено(ТекущееСлово) Тогда
						мРегВыражение.Global = Ложь;
						мРегВыражение.Pattern = СтрЗаменить(шПоискОписанияТаблицы, "#Идентификатор#", ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ТекущееСлово));
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущееСлово) Тогда 
				...
				Иначе
					...
					Если Лев(Выражение, 1) <> "(" Тогда
						ШаблонПриемника = ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ШаблонПриемника);
					КонецЕсли;
					мРегВыражение.Global = Истина;
					...
					Если РезультатТекста.Количество() > 0 Тогда
						СтруктураТипа = мПлатформа.НоваяСтруктураТипа();
						ИндексВхождения = РезультатТекста.Количество();
						Пока ИндексВхождения > 0 Цикл 
							...
							Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(1)) Тогда
								...
								ИначеЕсли Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(6)) Тогда 
									ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
									ТаблицаСтруктурТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения, РазрешитьАнализИмениТипа,, ПолныйАнализ, Ложь);
							...
							Иначе
								...
								ТаблицаСтруктурТипов = мПлатформа.СловаКонтекстаМетаданные(ТаблицаСтруктурТиповКоллекции[0], "[0]",,, мЯзыкПрограммы, "0",,, мФлагиКомпиляции)[0].ТаблицаСтруктурТипов;
								СтруктураТипа = ТаблицаСтруктурТипов[0];
							КонецЕсли;
							Если Истина
...
Функция ВычислитьТекстовоеЗначение(ВыражениеТекста, Знач ТекстДляЗаполненияМетаданных) Экспорт
	мРегВыражение.Global = Ложь;
	мРегВыражение.Pattern = ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(ВыражениеТекста) + "\s*=\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
	мРегВыражение.Global = Истина;
	РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляЗаполненияМетаданных, Истина);
	Если РезультатСвойств.Количество() > 0 Тогда
		ВхождениеСвойства = РезультатСвойств[0];
...
Процедура ОткрытьПоискВызововМетода(Знач ИмяМетода = Неопределено, Знач ИскатьНепрямые = Неопределено) Экспорт 
	...
	Если ИмяМетода = Неопределено Тогда
		КончитьОбработкуКоманды();
		РазобратьТекущийКонтекст(,, ЛиВЗаголовкеКонкретногоМетода());
		Если ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
...
Процедура ВыполнитьШаблонТекста()
	...
	Иначе
		...
		мРегВыражение.Pattern = "([^\s" + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(СтрокаРазделителейШаблоновТекста) + "]*)$";
		Результат = мРегВыражение.НайтиВхождения(мТекущаяСтрокаНачало);
		Если Результат.Количество() > 0 Тогда
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, Знач ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	Иначе
		Если ТолькоСвойства Тогда
			Шаблон = "(ъъъъ)";
		Иначе
			Шаблон = "(?:[^." + шБуква + "\d]|^)\s*(?:" + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=\s*\())";
		КонецЕсли;
		Шаблон = Шаблон + "|(?:[^." + шБуква + "\d]|^)\s*(?:" + ирОбщий.ПодготовитьТекстДляРегулярногоВыраженияЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=[^\(?:" + шБуква + "\d]|$))";
...
Процедура УстановитьТекст(Знач Текст, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	ОчиститьТаблицуСловЛокальногоКонтекста();
	мФлагиКомпиляции = мПлатформа.НовыеФлагиКомпиляцииМодуля();
	...
	Иначе
		...
		ИначеЕсли ирОбщий.СтрокиРавныЛкс(Фрагменты[0], "ОбщаяФорма") Тогда
			...
			ОбъектМД = Метаданные.ОбщиеФормы[Фрагменты[1]];
		Иначе
