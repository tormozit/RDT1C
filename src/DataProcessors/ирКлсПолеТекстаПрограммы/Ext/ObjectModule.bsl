...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	ДобавитьПравилоВычисленияФункции("МакетПечатнойФормы", "ВычислитьМакетПечатнойФормыБСП", "ОбщийМодуль.УправлениеПечатью"); // Добавил чтобы без ограничений на количество динамических вызовов считалось
...
Функция ВыделитьТекстовыйЛитерал(Знач ПолеТекстаЛ = Неопределено, выхНачальнаяПозиция = 0, выхКонечнаяПозиция = 0, Знач РазбиратьКонтекст = Истина) Экспорт 
	...
	КонецЦикла;
	Если Успех Тогда
		ТекстСтроковогоЛитерала = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ПолеТекстаЛ.ВыделенныйТекст());
	Иначе
	...
	Если мЯзыкПрограммы = 1 Тогда
		...
		Если РезультатТекста.Количество() > 0 Тогда
			...
			Тогда
				...
				ТекстБезКомментариевИОпасныхСтрок = ЗалитьКомментарииИСтроковыеЛитералы(мТекстДляПоискаОпределения,,, Истина);
...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	Знач ПолныйАнализСоставаТипов = Истина, выхЛиПеременнаяМодуля = Ложь, Знач ПозицияВМетоде = 0, Знач ВычислитьЗначение = Ложь) Экспорт 
			...
			Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
				...
				Если ТипСлова = "Метод" Тогда
					...
					Если ПравилоВычисленияФункции <> Неопределено Тогда 
						...
						КонецЕсли;
						//Попытка
							ТаблицаТиповВычисленная = Вычислить(ПравилоВычисленияФункции.Правило + "(РодительскаяСтруктураТипа, ПараметрыИсходящегоВызова, ТекущееСлово, ПредшествующийТекст, ПозицияВМетоде)");
					...
					Если ПараметрыМетода <> Неопределено И ТипСлова = "Свойство" Тогда
						...
						Если СтрокаПараметраМетода <> Неопределено Тогда
							...
							Если СтрокаПараметраМетода.ТаблицаТипов = Неопределено Тогда
								ВычислитьТипПараметраМетода(ПараметрыМетода, ПройденныеВызовы, СтрокаПараметраМетода, ТаблицаТипов, ВычислитьЗначение);
							КонецЕсли; 
...
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ТаблицаТипов, Знач ВычислитьЗначение = Ложь)
	...
	КонецЦикла;
	Если Истина 
		// Для надо отключить проверку детальности ирОбщий.НайтиЭлементКоллекцииЛкс
		//И (Ложь
		//	Или ВычислитьЗначение 
		//	Или Не ДетальностьТиповДостаточна)
		И мРекурсивныйПуть.ВычислениеРезультатаМетода = мМетодМодуля.Имя 
	Тогда       
...
Функция ВычислитьТипФункцииСФактПараметрами(Знач МетодМодуля, Знач ТекущиеАргументы, Знач ПредшествующийТекст, Знач ПолныйАнализСоставаТипов, Знач ПараметрыИсходящегоВызова,
	...
	Для Каждого КлючИЗначение Из МетодМодуля.КлючевыеПараметры Цикл 
		...
		Если Не ЗначениеЗаполнено(ВыражениеПараметра) Тогда
			Продолжить;
		Иначе 
			ЗначениеПараметра = ВычислитьТипЗначенияВыражения(ВыражениеПараметра, ПредшествующийТекст,,,, ПолныйАнализСоставаТипов, Ложь,,, Истина);
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0, Знач ДопКлючКэша = "") Экспорт
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда 
			...
			Тогда 
				...
				КонецЕсли;
				КлючВыраженияСПозицией = мРекурсивныйПуть.ДопКлючКэша + ";" + XMLСтрока(Позиция0ВМетодеОт) + ";" + XMLСтрока(Позиция0ВМетодеДо) + ";" + Лев(Выражение, 200);
			...
			Тогда
				...
				МассивДочернихСтруктурТипа = ВычислитьТипДочернегоЭлемента(ТаблицаСтруктурРодителей, ТекущееСлово, ТипСлова, ТекущийИндекс, ТекущиеАргументы, ПредшествующийТекст, ПолныйАнализСоставаТипов,,
					Позиция0ВМетодеДо, ВычислитьЗначение);
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Тогда
								...
								КонецЕсли;
							КонецЕсли;                    
							Если ПроверятьТипЧерезЕсли И Не ЛиТипЧерезЕслиНазначен Тогда
								ПрисвоенныйТипЕсли = ПоследнееВхождение.SubMatches(23);
								Если Истина
									И ПрисвоенныйТипЕсли <> Неопределено 
									//И Найти(ПоследнееВхождение.SubMatches(23), "Не ") = 0 // Часть условия до проверки типа
									//И Найти(ПоследнееВхождение.Value, "Или ") = 0 // Часть условия до проверки типа
								Тогда
									НачальнаяПозицияЕсли = ПозицияВхождения0 + ПоследнееВхождение.Length;
									ТекстПоискаЕсли = Сред(ТекстДляПоискаОпределения, НачальнаяПозицияЕсли, Позиция0ВМетодеДо - НачальнаяПозицияЕсли);
									...
									Если ПозицияТогда > 0 Тогда 
										...
										//	ПозицияИначе = 0;
										//КонецЕсли;
										Если ПозицияИначе = 0 И ЧислоЕсли >= ЧислоКонецЕсли Тогда
											Тип = Неопределено;
										...
										ИначеЕсли ТипНового <> Неопределено Тогда
											...
											Тогда 
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(5)) Тогда
													ТекстЗапроса = ПоследнееВхождение.SubMatches(5);
													ТекстЗапроса = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ТекстЗапроса);
													СтруктураТипа.Метаданные = ПостроительЗапросаИзТекстаДляТипа(ТекстЗапроса);
												КонецЕсли;
											ИначеЕсли ТипНового = Тип("ОписаниеТипов") Тогда
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(5)) Тогда
													ИменаТипов = ВычислитьЗначениеВыражения(ПоследнееВхождение.SubMatches(5),,, ПоследнееВхождение.FirstIndex);
			...
			Тогда     
				Если ЛиПеременнаяРезультата Тогда 
					ЛиВиртПрисвоение = Истина;
				КонецЕсли;
				Если ЗначениеЗаполнено(Позиция0ВМетодеДо) Тогда
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						СобратьТипыСвойствСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ТекстДляПоискаОпределения, ПредшествующийТекст, ГлобальныйПоискВМодуле, ЛиВиртПрисвоение, Позиция0ВМетодеДо);
					...
					Тогда
						...
						Для Каждого ВхождениеСвойства Из Вхождения Цикл
							...
							ТаблицаТиповЭлементов = ВычислитьТипЗначенияВыражения(ВхождениеСвойства.SubMatches(0),,, РазрешитьАнализИмениТипа,
					...
					Тогда
						...
						КонецЕсли;
						ШаблонПоиска = шПредИмя + Выражение + "\s*\.\s*Колонки\s*.\s*Добавить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")\s*(?:,\s*" + шПростоеВыражениеПрограммы + ")?";
						Вхождения = НайтиВхожденияРегВыраженияКэш(ШаблонПоиска, ГлобальныйПоискВМодуле);
						Для Каждого ВхождениеСвойства Из Вхождения Цикл
							Если ЛиВхождениеВЗапрешенномДиапазоне(ВхождениеСвойства, Позиция0ВМетодеОт, Позиция0ВМетодеДо) Тогда
								Прервать;
							...
							Иначе
								ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, ВхождениеСвойства);
								ТекстовоеЗначение = ВычислитьЗначениеВыражения(ПрисвоенноеВыражение,,, ВхождениеСвойства.FirstIndex);
							КонецЕсли;
							Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда
								ОписаниеТипов = ВычислитьЗначениеВыражения(ВхождениеСвойства.SubMatches(1),,, ВхождениеСвойства.FirstIndex);
								Если Не ЗначениеЗаполнено(ОписаниеТипов) Тогда
									ОписаниеТипов = Неопределено;
								КонецЕсли;
								Попытка
									СтруктураТипаЦикл.Метаданные.Колонки.Добавить(ТекстовоеЗначение, ОписаниеТипов);
								Исключение
									...
									КонецЕсли;
									Попытка
										СтруктураТипаЦикл.Метаданные.Вставить(ТекстовоеЗначение, ТаблицаТиповКолонки);
									Исключение
										Пустышка = 0;
									КонецПопытки;
								КонецПопытки; 
					...
					Тогда
						...
						Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
							ПостроительЗапроса = ПостроительЗапросаИзТекстаДляТипа(ТекстЗапроса);
...
// Возвращаемое значение:
//    ПостроительЗапроса - 
Функция ПостроительЗапросаИзТекстаДляТипа(Знач ТекстЗапроса) Экспорт
...
Функция ЛиВхождениеВЗапрешенномДиапазоне(Знач ПоследнееВхождение, Знач ПозицияВМетодеОт = 0, Знач ПозицияВМетодеДо = 0)
	Результат = Ложь
		...
		Или ПозицияВМетодеДо > 0 И ПоследнееВхождение.FirstIndex + ПоследнееВхождение.Length > ПозицияВМетодеДо + 1;
	Возврат Результат;
КонецФункции
...
Функция СобратьТипыСвойствСтруктуры(Знач Выражение, Структура = Неопределено, Знач ТекстДляПоискаОпределения, Знач ПредшествующийТекст, Знач ГлобальныйПоискВМодуле = Ложь, Знач ТолькоПрисвоение = Ложь,
	Знач ПозицияВМетодеДо = 0)
	
	мРегВыражение.Global = Ложь; 
	мРегВыражение.Pattern = шПредИмя + Выражение + "\s*=\s*Новый\s+Структура\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
	...
	Для Каждого Вхождение Из Вхождения Цикл
		Если ЛиВхождениеВЗапрешенномДиапазоне(Вхождение,, ПозицияВМетодеДо) Тогда
			Прервать;
		КонецЕсли;
		...
		Если Вхождения.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из СвойстваЧерезТочкуИзТекста Цикл
				...
				Если ПозицияВМетодеОт = Неопределено Тогда
					ПозицияВМетодеОт = 0;
				КонецЕсли;
				ТаблицаТиповЗначения = ВычислитьТипЗначенияВыражения(ВыражениеЗначения,,,,,, Ложь, ГлобальныйПоискВМодуле, Истина,, ПозицияВМетодеОт, ПозицияВМетодеДо);
				Если ТаблицаТиповЗначения.Количество() = 0 Тогда
...
Функция ЛиБесполезноеПрисвоениеПеременной(Знач ПрисвоенноеВыражение) Экспорт
	Результат = Ложь 
		...
		Или Не ЗначениеЗаполнено(ПрисвоенноеВыражение) 
	;
	Возврат Результат;
КонецФункции    
...
Функция ШаблонНазначенияТипаВыражению(Знач ШаблонПриемника, Знач ПроверятьТипЧерезЕсли = Ложь) Экспорт
	...
	Если ПроверятьТипЧерезЕсли Тогда
		шУсловиеСПроверкойТипа = шПредИмя + "(?:Если|ИначеЕсли|И)\s+(?:ТипЗнч|TypeOf)\(" + ШаблонПриемника + "\)\s*=\s*((?:Тип|Type)\s*\(\s*""" + шИмя + """\s*\))";
		...
		Тогда
			...
			Тогда 
				...
				ТаблицаТиповОбрамляющегоВызова = ВычислитьТипЗначенияВыражения(мВызовМетода, мТекстДляПоискаОпределения, мПредшествующийТекст,,,,,,,,, 
					мПозицияВТексте - мПозицияТекстаДляПоискаОпределения - 1);
				ПоследнийТип = ТаблицаТиповОбрамляющегоВызова[ТаблицаТиповОбрамляющегоВызова.Количество() - 1];
				Если ПоследнийТип.Детальность > 3 Тогда
					ВыбраннаяСтруктураТипа.Метаданные = ПоследнийТип.Метаданные;
				КонецЕсли;
			КонецЕсли;
...
Функция ВычислитьПолучитьФорму(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьПолучитьМакет(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьПолучитьОбласть(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьРеквизитФормыВЗначение(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если Истина
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьКоллекцияДобавить(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли; 
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьНайтиЭлементКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьОтобратьКоллекциюЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьСоздатьОбъектПоИмениМетаданныхЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьВычислить(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьОбщийМодуль(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = МассивПараметров[0];
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьМакетПечатнойФормыБСП(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = МассивПараметров[0];
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьЗначенияРеквизитовОбъектаБСП(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	...
	Исключение
		Возврат Неопределено;
	КонецПопытки;
	ТипыСсылки = ВычислитьТипЗначенияВыражения(МассивПараметров[0], ПредшествующийТекст,,,,,,,,,, ПозицияВМетоде);
...
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
Функция ВычислитьСтрокаТаблицыЗначенийВСтруктуруБСП(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0) 
	Если МассивПараметров.Количество() < 1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ТаблицаТипов = ВычислитьТипЗначенияВыражения(МассивПараметров[0], ПредшествующийТекст,,,,,,,,,, ПозицияВМетоде);
...
Функция ОписаниеТекущегоСловаАвтодополнения(Знач Слово, Знач ЛиМетод, Знач КлючНабораСлов = "") Экспорт 
	
	ТолькоПолезноеОписание = Истина;
	ЛиОтдельноеОписаниеАктивно = Истина;
