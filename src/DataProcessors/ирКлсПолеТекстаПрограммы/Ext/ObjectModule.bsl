...
Функция СтроитьДеревоПрограммныхСкобок(Знач ВыделенныйТекст = "", Знач НачальнаяСтрока = 1, Знач ПозицияДляСбораРодителей = 0, выхРодителиПозиции = Неопределено, Знач БазовыйУровень = Неопределено,
	...
	Для СчетчикСтроки = 1 По ТекстДокумент.КоличествоСтрок() Цикл
		...
		СтрокаБезКраев = СокрЛП(СтрокаТекстаДоКомментария);
		СмещениеУровня = 0;
		...
		Если ПустаяСтрока(СтрокаТекстаДоКомментария) Тогда
			Продолжить;
		КонецЕсли;
		Если Прав(СтрокаБезКраев, 1) = ";" Тогда 
		...
		Иначе
			...
			КонецЕсли;
			НовыйУровеньВыражения = НовыйУровеньВыражения + СтрЧислоВхождений(СтрокаБезКраев, "(") - СтрЧислоВхождений(СтрокаБезКраев, ")");
		КонецЕсли;
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Тогда
			...
			Если ТаблицаТиповМодуляФормы.Количество() > 0 Тогда
				...
				Если ирОбщий.ЛиИмяТипаФормыЛкс(СтруктураТипаМодуля.ИмяОбщегоТипа, Истина, Ложь) Тогда
...
Функция ВычислитьТипФункцииСФактПараметрами(Знач МетодМодуля, Знач ТекущиеАргументы, Знач ПредшествующийТекст, Знач ПолныйАнализСоставаТипов, Знач ПараметрыИсходящегоВызова,
	...
	Для Каждого КлючИЗначение Из МетодМодуля.КлючевыеПараметры Цикл 
		...
		Если Не ЗначениеЗаполнено(ВыражениеПараметра) Тогда
			Продолжить;
		КонецЕсли;
		ТаблицаТиповПараметра = ВычислитьТипЗначенияВыражения(ВыражениеПараметра, ПредшествующийТекст,,,, ПолныйАнализСоставаТипов, Ложь,,, Истина); // ДляСвойства здесь вредно передавать
		...
		Если ирОбщий.ПервыйФрагментЛкс(СтруктураТипаЦикл.ИмяОбщегоТипа, "[") = "Массив" Тогда
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				Для Каждого СтруктураТипаКоллекции Из ТаблицаТиповЭлемента Цикл
					...
					КонецЕсли;
				КонецЦикла;
				СтруктураТипаЦикл.Метаданные = ТаблицаТиповЭлемента;
		...
		Если ЭтоИмяТипа Тогда
			...
			Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
				...
				Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
					...
					КонецЕсли;
					Если Истина
						И Лев(СтрокаОписания, ДлинаПрефикса) = ПрефиксУровня
						И Лев(СтрокаОписания, ДлинаПрефикса + 1) <> ПрефиксУровня + "*"
					Тогда
						СчетчикСтрок = СчетчикСтрок + 1;
...
Функция ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(Знач СтрокаМетода, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь)
	...
	Если СтрокаМетода.КлючевыеПараметры.Количество() > 0 Тогда
		...
		Если ЕстьВлияющиеФактПараметры Тогда
			...
			Если мМетодМодуля.КлючевыеПараметры <> Неопределено Тогда
...
Функция ВычислитьПолучитьФорму(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	ТаблицаТипов.ЗаполнитьЗначения(, "Метаданные");
