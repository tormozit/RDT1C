								...
								Иначе
									...
									Если ПрименитьТип Тогда 
										...
										Если ТаблицаПрисвоенныхТипов.Количество() > 0 Тогда
											...
											КонецЦикла; 
											Если ГлубинаВеток.Необязательная = Неопределено И ЧислоПроверенныхВариантов = 1 Тогда
							...
							Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяСвойства) Тогда 
								...
								Если ФрагментыОписания.Количество() > 1 Тогда
									...
									Иначе
										...
										КонецПопытки;
										Если Не ЗначениеЗаполнено(МетаданныеСвойства) Тогда
											мПлатформа.ДобавитьВТаблицуТипов(МетаданныеСвойства, НоваяСтруктураТипа("Произвольный"));
										КонецЕсли;
										...
										МетаданныеСвойства.ЗаполнитьЗначения(ОписаниеДинамическогоСвойства, "СтрокаОписания");
									КонецЕсли;
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	//Для Индекс = 0 По ВсеИменаТипов.ВГраница() Цикл
	//	Если Найти(ВсеИменаТипов[Индекс], "[") > 0 Тогда
	//		ВсеИменаТипов[Индекс] = ирОбщий.ПервыйФрагментЛкс(мИмяОжидаемогоТипа, "[");
	//	КонецЕсли;
	//КонецЦикла;
	...
	Иначе
		...
		Иначе 
			...
			Тогда
				...
				ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора);
			КонецЕсли;
			МаксТиповПроверять = 2;
			ЧислоПроверенныхТипов = 0;
			Для Каждого ИмяТипа Из ВсеИменаТипов Цикл
				Если Не ЛиПримитивныйТип(ИмяТипа) Тогда
					КлючПоиска = Новый Структура;
					КлючПоиска.Вставить("ТипЗначения", ИмяТипа);
			...
			Тогда
				...
				Если мИмяОжидаемогоТипа = "Тип" Тогда
					Если мИмяОжидаемогоТипа <> ВыражениеПриемник Тогда
		...
		Если мИмяОжидаемогоТипа = Неопределено Тогда
			...
			Если ЛиВложенныйТип Тогда
				...
				Для Каждого СтрокаТаблицы Из ТаблицаТипов Цикл
					...
					Прервать; // TODO сделать возврат массива имен вложенных типов
				КонецЦикла;
			Иначе
...
Функция ИзвлечьВложенныеТипыИзТаблицыСлов(Знач ТаблицаСловОбъекта)
	...
	ЕстьВложенноеОписаниеТипов = Истина
		И ТаблицаСловОбъекта.Количество() > 0
		И ЗначениеЗаполнено(ИмяСвойстваОписанияТипаИзОбъекта(ТаблицаСловОбъекта[0].ТаблицаТипов[0].Метаданные));
...
// Возвращаемое значение:
//    Строка - 
Функция ИмяСвойстваОписанияТипаИзОбъекта(Знач ЭлементКоллекцииМД, ИмяКлюча = Неопределено) Экспорт
	Попытка
...
Функция ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, ТаблицаТиповКонтекста = Неопределено, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	ИначеЕсли Истина
		...
		И Слово = "Тип"
	Тогда
		СтрокаДобавка = СтрЗаменить(СтрокаДобавка, "()", "("""")");
	...
	ИначеЕсли Истина
		...
		И мЯзыкПрограммы = 1
	Тогда
		СтрокаДобавка = СтрЗаменить(СтрокаДобавка, "()", "(1,1,1)");
	ИначеЕсли Истина
		И СтрокаРезультата.ТипСлова = "Конструкция"
		И Слово = "Новый"
	Тогда
