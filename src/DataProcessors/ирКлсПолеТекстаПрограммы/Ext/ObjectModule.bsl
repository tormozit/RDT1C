		...
		Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
			...
			Если ТаблицаСвойствТипа.Количество() > 0 Тогда
				...
				Для Каждого СтрокаСлова Из ТаблицаСвойствТипа Цикл
					...
					//	Продолжить;
					//КонецЕсли;
					мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, СтрокаСлова.ТаблицаТипов,, РазрешитьВытеснять);
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда
				...
				Иначе
					...
					Тогда
						Если ДляСвойства = "." Тогда
							ДляСвойстваВверх = ТекущееСлово + ".";
						ИначеЕсли ЗначениеЗаполнено(ДляСвойства) Тогда
			...
			Тогда  
				...
				Иначе
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						ГлубинаВетки = 0;
						ГлубинаАльтернативнойВетки = -1;
						ГлубинаБратскойВетки = Неопределено;
						ЧислоПроверенныхВариантов = 0;
						ПредКонецВхождения = Позиция0ВМетодеДо;
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							ТекстПроверкиВеток = Сред(ТекстДляПоискаОпределения, КонецВхождения, ПредКонецВхождения - КонецВхождения + 2);
							ПредКонецВхождения = КонецВхождения; 
							мРегВыражение.Global = Истина;
							мРегВыражение.Pattern = мПлатформа.шПустоеНачалоСтроки + "(Если|ИначеЕсли|Иначе|КонецЕсли)(?=\s|/|;|$)";
							...
							Для ИндексВхожденияЕсли = -ВхожденияЕсли.ВГраница() По 0 Цикл
								...
								Если Токен = "если" Тогда
									ГлубинаВетки = ГлубинаВетки + 1;
									Если ГлубинаВетки > ГлубинаАльтернативнойВетки Тогда
										ГлубинаАльтернативнойВетки = -1;
									КонецЕсли;
									Если ГлубинаБратскойВетки <> Неопределено И ГлубинаВетки > ГлубинаБратскойВетки Тогда
										ГлубинаБратскойВетки = Неопределено;
									КонецЕсли;
								ИначеЕсли Токен = "конецесли" Тогда
									ГлубинаВетки = ГлубинаВетки - 1;
									Если ГлубинаБратскойВетки = Неопределено Тогда
										ГлубинаБратскойВетки = ГлубинаВетки;
									КонецЕсли;
								ИначеЕсли ГлубинаВетки > ГлубинаАльтернативнойВетки Тогда
									ГлубинаАльтернативнойВетки = ГлубинаВетки;
								КонецЕсли;
							КонецЦикла;
							Если Ложь
								Или (Истина
									И ГлубинаАльтернативнойВетки >= 0 
									И ГлубинаБратскойВетки <> ГлубинаАльтернативнойВетки)
								Или (Истина
									// Нужно для СкопироватьКоллекциюЛкс()
									И ЭтоФактическийТип 
									И ГлубинаБратскойВетки <> Неопределено // для вычисления результата РодительЭлементаУправляемойФормыЛкс()
									)
							Тогда
								Продолжить;
							КонецЕсли; 
							...
							Если ПроверятьТипЧерезЕсли И Не ЛиУстановкаЗначения Тогда 
								...
								Если ПрисвоенныйТипЕсли <> Неопределено Тогда
									...
									Если Не ЛиТипЧерезЕслиНазначен Тогда
										ПрименитьТип = МожноПрименитьПрисвоениеТипаИзЕсли(ТекстПроверкиВеток, ГлубинаБратскойВетки);
								...
								Иначе //Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 8)) Тогда ;
									...
									Если ПрименитьТип Тогда 
										...
										Если ТаблицаПрисвоенныхТипов.Количество() > 0 Тогда
											...
											//КонецЕсли;
											//ТаблицаПрисвоенныхТипов.ЗаполнитьЗначения(СтрокаОписания, "СтрокаОписания"); // Так получаются сочетания, которые трактуются проверкой как ошибка
							...
							Если Ложь
								...
								Или Истина
									И ЧислоПроверенныхВариантов >= ?(ГлубинаБратскойВетки = Неопределено, 1, 2) // для СтруктураКлючаТаблицыБДЛкс нужно 2
...
Процедура ПроверитьВозможныеОшибкиВыражения(Знач ТаблицаТиповОтРодителя, Знач Позиция0ВМетодеДо, Знач ТаблицаТипов, Знач ЛиВиртПрисвоение, Знач РодительскийКонтекст, Знач ТаблицаТиповРодителя, Знач ТекущееСлово,
	...
	Если ЗначениеЗаполнено(ТекущиеАргументы) Тогда 
		...
		Если Истина
			И ТекущиеАргументы <> "()"
			И мРекурсивныйПуть.ВложенныеВыражения.Найти(ТекущиеАргументы, "Выражение") = Неопределено 
		Тогда
...
Процедура СобратьДинамическиеСвойстваОбъекта(Знач Выражение, Знач ТаблицаТипов, ТекстДляПоискаОпределения = "", Знач ЭтоВызовКонструктора = Ложь, Знач ПолныйАнализСоставаТипов = Истина, Знач ГлобальныйПоискВМодуле = Ложь,
	...
	КонецЕсли;
	Если Прав(ДляСвойстваПуть, 1) = "." Тогда
		// Обращение по индексу
		ДляСвойстваПутьБезТочки = иробщий.СтрокаБезКонцаЛкс(ДляСвойстваПуть);
	КонецЕсли;
	Если ЗначениеЗаполнено(ДляСвойстваПутьБезТочки) Тогда
		ДляСвойства = ирОбщий.ПервыйФрагментЛкс(ДляСвойстваПутьБезТочки);
		ДляСвойстваВниз = ирОбщий.СтрокаБезПервогоФрагментаЛкс(ДляСвойстваПутьБезТочки);
	Иначе
		ДляСвойства = "";
		ДляСвойстваВниз = ИмяФиктивногоСвойства(); 
	КонецЕсли;
	ШаблонВыражения = ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(Выражение);
	ДляСвойстваСтруктурыПуть = ДляСвойстваПуть;
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Если ирОбщий.ПервыйФрагментЛкс(СтруктураТипаЦикл.ИмяОбщегоТипа, "[") = "Массив" Тогда
			// Отдельная логика с передачей конечной точки нужна для обращения по индексу к результату ТМС5 ПодготовитьНастройкиСовместимостейПоТипуКонтроля
			Если ДляСвойстваПуть = ДляСвойстваПутьБезТочки Тогда
				ДляСвойстваМассива = ДляСвойства;
				ДляСвойстваМассиваВниз = ДляСвойстваВниз; 
			ИначеЕсли ЗначениеЗаполнено(ДляСвойстваПуть) Тогда
				ДляСвойстваМассива = ирОбщий.ПервыйФрагментЛкс(ДляСвойстваПуть);
				ДляСвойстваМассиваВниз = ирОбщий.СтрокаБезПервогоФрагментаЛкс(ДляСвойстваПуть);
			Иначе
				ДляСвойстваМассива = "";
				ДляСвойстваМассиваВниз = ИмяФиктивногоСвойства(); 
			КонецЕсли;
			Если ДляСвойстваМассива = ИмяФиктивногоСвойства() Тогда
				ТаблицаТипов.Колонки.Метаданные.Заголовок = ФлагОбрезкиГлубины();
				Продолжить;
			КонецЕсли;
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				ТаблицаТиповЭлемента = ВычислитьТипЗначенияВыражения(ВхождениеСвойства.SubMatches(0),,,,
					ЭтоВызовКонструктора, ПолныйАнализСоставаТипов, Ложь, ГлобальныйПоискВМодуле,,,, ВхождениеСвойства.FirstIndex,, ДляСвойстваМассиваВниз);
...
// Возвращаемое значение:
//    Булево - 
Функция МожноПрименитьПрисвоениеТипаИзЕсли(Знач ТекстПроверкиВетвлений, Знач ГлубинаБратскойВетки)
	Если ГлубинаБратскойВетки <> Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
