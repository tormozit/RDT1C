...
Функция ПолноеИмяМетода(Знач СтрокаМетода = Неопределено, Знач РазрешитьЛокальное = Ложь) Экспорт 
	...
	КонецЕсли;
	Фрагменты = ирОбщий.СтрРазделитьЛкс(СтрокаМетода.ИмяМодуля, ".");
	...
	Тогда
		Фрагменты.Удалить(Фрагменты.ВГраница());
	КонецЕсли;
	Фрагменты.Добавить(СтрокаМетода.Имя);
...
Функция ВычислитьТипПараметраМетодаМодуля(Знач ПараметрыМетода, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Если Истина
			И мМетодМодуля.ВызывающиеМодули <> Неопределено 
		...
		Для Каждого ВызывающийМодуль Из ВызывающиеМодули Цикл
			...
			Иначе
				...
				ПрямыеВызовыМетодов = мПлатформа.НовыйИндексПоискаВызововСловаВМодуле(ВызывающийМодуль);
				ИмяОбщегоМодуля = мМодульМетаданных.СтруктураТипа.Метаданные.Имя;
				ШаблонОповещения = "";
				Если ПрямыеВызовыМетодов.Оповещения <> Неопределено Тогда
					// Добавляет 7мс на каждые 10к строк
					ШаблонОповещения = шПредИмяПрямое + "Новый\s+ОписаниеОповещения\s*\(\s*""(" + шИмя + ")""\s*,\s*(?:"+ ИмяОбщегоМодуля +")\s*,\s*(" + шПростоеВыражениеПрограммы + ")";
				КонецЕсли;
				ШаблонВнешнегоВызова = ШаблонПрямогоВызоваСлова(ИмяОбщегоМодуля + "\.(" + мМетодМодуля.Имя + ")",, Истина); 
					...
					Тогда
						...
						Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяСвойства) Тогда 
							...
							Если ФрагментыОписания.Количество() > 1 Тогда
								Если Прав(СтрокаОписания, 1) = ":" Тогда
									МетаданныеСвойства = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ФрагментыОписания[1], ":"), Описание, ТекстДоВхождения, ДляСвойстваВниз, СчетчикСтрок - 1, Истина);
								Иначе
									Попытка 
...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Иначе
		...
		Иначе 
			...
			КонецЕсли;
			// TODO
			//Если ИмяТипа = "Тип" Тогда
			//	СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Тип(""" + ИмяТипа + """)", "Конструкция",,, "Локальный", Ложь);
			//	СтрокаСлова.Рейтинг = КонстантаРейтингСовпаденияТипа;
			//	СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
			Если Истина
				И ЗначениеЗаполнено(мИмяОжидаемогоТипа)
...
Функция РассчитатьОжидаемыйТипВыражения(ЭтоСсылочныйЛитерал = Неопределено) Экспорт
	...
	Тогда
		...
		Тогда
			...
			//ВыражениеПриемник = АнализаторКода.мКонтекст;
		ИначеЕсли Истина
			И ПредшествующийТокен = """" 
...
Функция ТаблицаОжидаемыхТиповПараметраМетода(Знач СтруктураТипаМетода, ВыражениеПриемник = Неопределено, Знач ПеременнаяКонструктора = "") Экспорт
	СтрокаОписанияМетода = СтруктураТипаМетода.СтрокаОписания;
	ТаблицаОжидаемыхТипов = Неопределено;
	ВыражениеПриемник = ИмяПараметраВызоваМетода(СтрокаОписанияМетода, Ложь, ТаблицаОжидаемыхТипов, СтруктураТипаМетода);
	...
	Тогда
		...
		Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяПараметраЗапроса) Тогда
			...
			ОписанияПараматров = мПлатформа.ПараметрыИзТекстаЗапроса(ПостроительЗапроса.Текст, Истина);
			ОписанияПараматров.Свойство(ИмяПараметраЗапроса, ТаблицаОжидаемыхТипов);
		КонецЕсли;
	ИначеЕсли Истина
		И ЛиСистемныйМетод
...
Функция ЗаполнитьТаблицуВозможныхЗначенийЛитерала(Знач ВыбраннаяСтруктураТипа, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
					...
					Иначе
						...
						Если ЗначениеЗаполнено(ВыражениеСлеваОтРавно) Тогда
							Если Найти(ВыражениеСлеваОтРавно, ".") = 0 Тогда
								...
								КонецЕсли;
							Иначе
								ТаблицаТипов = ВычислитьТипЗначенияВыражения(ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(ВыражениеСлеваОтРавно));
								Если ТаблицаТипов.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
			...
			Тогда
				...
				Если СтруктураТипаПараметра.Количество() > 0 Тогда
					...
					мТаблицаТиповКонтекста = ВычислитьТипЗначенияВыражения(мРодительскийКонтекст,,, Истина);
	...
	ИначеЕсли ИмяТипаСтруктуры = "" Тогда
		...
		Если Прав(ВыражениеСлеваОтРавно, 1) = "=" Тогда
			ВыражениеСлеваОтРавно = СокрЛП(ирОбщий.ПервыйФрагментЛкс(мТекущаяСтрокаНачало, "="));
			Если ирОбщий.СтрНачинаетсяСЛкс(ирОбщий.ПоследнийФрагментЛкс(ВыражениеСлеваОтРавно), "ПутьКДанным") Тогда
		...
		Для Каждого ЭлементКоллекцииМД Из КоллекцияЭлементовМД Цикл
			...
			Если ЗначениеЗаполнено(ИмяОписанияТипа) Тогда
				...
				Если ЗаполнятьТип Тогда
					ОбновитьТипЗначенияИзТаблицыТипов(СтрокаСловаСТипом);
...
//  Строка - 
//
Функция ИмяПараметраВызоваМетода(СтрокаМетода, Знач СТипомСтрока = Истина, выхТаблицаТипов = Неопределено, Знач СтруктураТипаМетода = Неопределено) Экспорт
	ИмяПараметра = "";
	Если СтрокаМетода = Неопределено Тогда
		Возврат ИмяПараметра;
	КонецЕсли;
