							...
							Иначе
								...
								Для Каждого СтруктураТипаЭлемента Из ТаблицаТиповКоллекции Цикл
									Если ЗначениеЗаполнено(СтруктураТипаЭлемента.ИмяОбщегоТипа) Тогда
										ЛиУстановкаЗначения = Истина;
										ТаблицаПрисвоенныхТипов = мПлатформа.СловаКонтекстаМетаданные(СтруктураТипаЭлемента, "[0]",,, мЯзыкПрограммы, "0",,, мФлагиКомпиляции)[0].ТаблицаТипов;
										мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, ТаблицаПрисвоенныхТипов);
									КонецЕсли;
								КонецЦикла;
							КонецЕсли;
...
Функция ДатаОбновленияСтруктурыФормыМодуля() Экспорт
	...
	Если мМодульМетаданных <> Неопределено Тогда
		...
		Если СтруктураТипа <> Неопределено И СтруктураТипа.ИмяОбщегоТипа = "ФормаКлиентскогоПриложения" Тогда
