...
Перем мЛиТаблицаСловСортированаПоРейтингу; // Булево
...
Процедура ОбновитьФормуАвтодополнения(Данные = "") Экспорт
	...
	КонецЕсли;
	//Если ОбязательноОткрыть Тогда
	//	мЛиТаблицаСловСортированаПоРейтингу = Не мЛиТаблицаСловСортированаПоРейтингу;
	//	СортироватьТаблицуСлов();
	//КонецЕсли;
	ФормаАвтодополнение.ЛиОбработкаСобытия = Ложь; 
	ФормаАвтодополнение.ПриИзмененииОтбора();
КонецПроцедуры

Процедура СортироватьТаблицуСлов() Экспорт
	//Если мЛиТаблицаСловСортированаПоРейтингу Тогда
	//	ТаблицаСлов.Сортировать("Рейтинг Убыв, НСлово, ТипСлова, Определение, ТипЗначения, Частота");
	//Иначе
			...
			Тогда
				...
				Если ПрямыеВызовыМетодов = Неопределено Тогда
					Если Истина
						...
						И ирОбщий.НайтиРегВыражениеЛкс(ТекстМодуля, мМетодМодуля.Имя + "[""\(]",, Ложь,,,, Истина, мРегВыражение).Количество() = 0 
					Тогда
						Продолжить;
					КонецЕсли;
...
Процедура ОткрытьАвтодополнение(Знач КодКлавиши = "", Знач Модально = Ложь) Экспорт 
	...
	Пока Истина Цикл
		...
		Иначе
			...
			//ОбновитьФормуАвтодополнения(0); // Нужно для наложения отбора при вставке с автооткрытием, например после "Перейти ~"
		КонецЕсли;
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	ЛиПодробноеПредставлениеТипов = Ложь;
	МассивДляПроверкиДублей = Новый Массив;
	Для Каждого СтруктураТипаКонтекста Из ТаблицаТиповКонтекста Цикл
		...
		//КонецЕсли;
		ПредставлениеКонкретногоТипа = ПредставлениеКонкретногоТипа + мПлатформа.ИмяТипаИзСтруктурыТипа(СтруктураТипаКонтекста, ЛиПодробноеПредставлениеТипов);
		Если МассивДляПроверкиДублей.Найти(ПредставлениеКонкретногоТипа) = Неопределено Тогда
	...
	ИначеЕсли мАгрегатныеТипыКонтекста.Количество() = 0 Тогда 
		...
		мКонкретныйТипКонтекста = мПлатформа.ИмяТипаИзСтруктурыТипа(СтруктураТипаКонтекста, ЛиПодробноеПредставлениеТипов);
	Иначе
			...
			Тогда 
				...
				Если ПараметрыМетодаМодуля <> Неопределено Тогда
					Для Каждого СтрокаПараметра Из ПараметрыМетодаМодуля Цикл 
						...
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, СтрокаПараметра.Имя, "Свойство",, ТипЗначения, "Локальный",, мМетодМодуля.СтарыеТипы); 
	...
	Если РазрешитьОткрытиеОкон Или ВычислятьТипы Или ДобавлятьСтатистическиеСлова Тогда
		Если ДобавлятьСтатистическиеСлова Тогда
			ВключатьКонструкции = Не мЭтоТекстовыйЛитерал И Не ЗначениеЗаполнено(мРодительскийКонтекст) И мЯзыкПрограммы = 0;
	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		...
		Тогда
			...
			Для Каждого ИмяТипа Из ЧастыеТипы Цикл
				УстановитьРейтингСлову(ИмяТипа, ШагРейтингаСлова, "Свойство");
			КонецЦикла;
		КонецЕсли;
		Если Истина                                                    
			И мЭтоЛокальныйКонтекстТаблицыСлов
		...
		Тогда
			УстановитьРейтингСлову("Строка", -ШагРейтингаСлова, "Метод", "Строка"); // Подавляем обычно бесполезную функцию, но при этом стоящую выше многих полезных переменных, имеющих то же начало
			УстановитьРейтингСлову("ТипЗнч", ШагРейтингаСлова, "Метод", "Тип");
...
Процедура ЗагрузитьСтатистикуВыбораВТаблицуСлов(Знач ТипКонтекстаСтатистики) Экспорт
	...
	Если СуммаВсехСлов > 0 Тогда
		...
		Если ЗначениеЗаполнено(мНаборыСлов) Тогда
			...
			КонецЦикла;
		КонецЕсли; 
		ПремируемаяДоляСлова = Макс(0.1, Мин(0.5, 10/ОбщееЧислоСлов)); // Должна быть в 10 раз больше чем равномерная, но в пределах [0.1;0.5]
		ШагРейтингаСлова = ШагРейтингаСлова();
		Для Каждого СтрокаРейтинга Из СтатистикаПоКонтексту Цикл
				...
				Для Каждого КлючИЗначение Из мНаборыСлов Цикл
					Для Каждого СтрокаСлова Из КлючИЗначение.Значение.НайтиСтроки(КлючПоискаСлова) Цикл
						Если Ложь
							...
							Или Не СтрокаРейтинга.ЭтоМетод И СтрокаСлова.ТипСлова <> "Метод" 
						Тогда
							СтрокаСлова = УстановитьРейтингСлову(СтрокаСлова.Слово, ШагРейтингаСлова, СтрокаСлова.ТипСлова, СтрокаСлова.ТипЗначения);
							СловоНайдено = Истина;
			...
			Если Истина
				И СловоНайдено
				И мНаборыСлов = Неопределено
				И (Ложь
					Или мСтрокаЛучшегоСлова = Неопределено 
					Или мСтрокаЛучшегоСлова.Рейтинг < СтрокаСлова.Рейтинг)
			Тогда
...
Функция УстановитьРейтингСлову(Знач Слово, Знач НовыйРейтинг = 0, Знач ТипСлова = "Свойство", Знач ТипЗначения = "") Экспорт
	...
	Иначе
		...
		Иначе 
			...
			Тогда
				...
				ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, НаборыСлов, КлючОсновногоНабора,, ?(ирОбщий.СтрокиРавныЛкс(ВыражениеПриемник, "СтандартнаяОбработка"), 4, 3));
			...
			Тогда
				...
				Тогда
					Для Каждого ИмяТипа Из ИменаВложенныхТипов Цикл
						СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Тип(""" + ИмяТипа + """)", "Конструкция",, "Тип", "Локальный", Ложь);
						СтрокаСлова.Рейтинг = 2 * ШагРейтингаСлова;
						СтрокаСлова.НСлово = НРег(СтрокаСлова.Слово);
					КонецЦикла;
				КонецЕсли;
			...
			Тогда
				...
				Иначе
					// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
					Для Каждого СтрокаСлова Из ТаблицаСлов Цикл  						Если Найти(СтрокаСлова.ТипЗначения, мИмяОжидаемогоТипа) > 0 Тогда  							СтрокаСлова.Рейтинг = 3 * ШагРейтингаСлова;  						КонецЕсли;  					КонецЦикла;  
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция РассчитатьОжидаемыйТипВыражения(ЭтоСсылочныйЛитерал = Неопределено) Экспорт
	...
	Тогда
		...
		ИначеЕсли мЯзыкПрограммы = 1 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		мКэшОжидаемогоТипа = КэшОжидаемогоТипа(ТаблицаОжидаемыхТипов, ВыражениеПриемник, РазрешитьМногоТипов, ПредшествующийТокен, ИменаВложенныхТипов);
	Иначе
	...
	Если Вхождения.Количество() > 0 Тогда
		...
		Если мИмяОжидаемогоТипа = Неопределено Тогда
			...
			Если Истина
				...
				И мМетодМодуля <> Неопределено
			Тогда
				ТаблицаТипов = мПлатформа.ПодготовитьТипРезультатаМетода(мМетодМодуля, мМодульМетаданных,,, ИмяОтключенияЛишнихВычислений());
			Иначе
...
//.
// Параметры:
//    ТаблицаОжидаемыхТипов - см. ОбработкаОбъект.ирПлатформа.НоваяТаблицаТипов - 
//    ВыражениеПриемник - Строка - 
//    ИменаВложенныхТипов - Массив из Строка, Неопределено - 
// Возвращаемое значение:
//    Структура - 
Функция КэшОжидаемогоТипа(Знач ТаблицаОжидаемыхТипов, Знач ВыражениеПриемник, Знач РазрешитьМногоТипов, Знач ПредшествующийТокен, Знач ИменаВложенныхТипов) Экспорт
	Результат = Новый Структура;
...
Функция ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "", Знач МножительРейтинга = 3) Экспорт
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			Тогда
				...
				ТипыСовместимы = Ложь
					Или ирОбщий.СтрНайтиЛкс(ирОбщий.ПервыйФрагментЛкс(Найденные[0].ТипЗначения, "(") + ",", КлючПоиска.ТипЗначения + ",")
					Или (Истина
			...
			КонецЕсли;
			СтрокаЛучшегоСлова.Рейтинг = СтрокаЛучшегоСлова.Рейтинг + ШагРейтингаСлова * МножительРейтинга;
			Если Ложь
				Или мСтрокаЛучшегоСлова = Неопределено 
				Или мСтрокаЛучшегоСлова.Рейтинг < СтрокаЛучшегоСлова.Рейтинг
			Тогда
				мСтрокаЛучшегоСлова = СтрокаЛучшегоСлова;
			КонецЕсли;
			СловоНайдено = Истина;
			Прервать;
		КонецЕсли;
...
Функция ВозможныеТекстовыеЗначенияПараметраИзДругогоМетода(Знач СтрокаМетода, ЛиНаборЗначенийПолный = Истина, Знач ИмяТипаСтруктуры = "", Знач ДобавлятьНизкоВероятные = Ложь,
	...
	Если ЗначениеЗаполнено(СтрокаМетода.КлючевыеПараметры) Тогда
...
Процедура ОбновитьСтатистикуВыбораВСтрокеСлова(Знач СтрокаСлова)
	...
	Тогда
		Возврат;
	КонецЕсли;
	//Если ТипЗнч(СтрокаСлова) = Тип("СтрокаТаблицыЗначений") Тогда
	//	ЧислоСлов = СтрокаСлова.Владелец().Количество();
	//Иначе
	//	ЧислоСлов = ТаблицаСлов.Количество();
	//КонецЕсли;
	//Если ЧислоСлов < 5 Тогда
	//	Возврат;
	//КонецЕсли;
	ЭтоСловоГлобальногоКонтекста = Ложь;
	Если мЭтоЛокальныйКонтекстТаблицыСлов Тогда
		ТаблицаТипов = УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаСлова, Истина);
		ЛучшийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТипов, 1);
		ЭтоСловоГлобальногоКонтекста = Истина
			И ЛиСистемноеСлово(ЛучшийТип)
			И ЛучшийТип.СтрокаОписания.ТипКонтекста = "Глобальный";
	КонецЕсли;
	ТипКонтекстаСтатистики = ТипКонтекстаСтатистики(ЭтоСловоГлобальногоКонтекста);
...
//.
// Параметры:
//   РазрешитьГлобальныйКонтекст - Булево - 
// Возвращаемое значение:
//   Строка, Неопределено
Функция ТипКонтекстаСтатистики(Знач РазрешитьГлобальныйКонтекст = Истина) Экспорт
	Если Истина
		//И мИмяМодуля = мПлатформа.ИмяДинамическогоМодуля()
		И мЭтоЛокальныйКонтекстТаблицыСлов 
		И РазрешитьГлобальныйКонтекст
	Тогда
		ТипКонтекстаСтатистики = "Глобальный";
	ИначеЕсли Ложь
		Или мСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа"
		Или мСтруктураТипаКонтекста.ТипЯзыка = "ЗначениеВЗапросе"
		Или мСтруктураТипаКонтекста.ТипЯзыка = "ИмяПредопределенногоЗначения"
	Тогда
		ТипКонтекстаСтатистики = мРодительскийКонтекст;
		Если Не ЗначениеЗаполнено(ТипКонтекстаСтатистики) Тогда
			ТипКонтекстаСтатистики = "<" + мСтруктураТипаКонтекста.ТипЯзыка + ">";
		КонецЕсли; 
	ИначеЕсли Истина
		И Не мЭтоТекстовыйЛитерал
		//И мЯзыкПрограммы = 0 // Отключил, т.к. выше исключаются все неподходящие случаи через ТипЯзыка 
		И (Ложь
			Или мКонкретныйТипКонтекста = "ОбщийМодуль"
			Или мКонкретныйТипКонтекста = "Форма"
			Или мКонкретныйТипКонтекста = "ФормаКлиентскогоПриложения"
			Или Найти(мКонкретныйТипКонтекста, ".") > 0)
	Тогда
		ТипКонтекстаСтатистики = ИмяТипаКонтекста(); 
	ИначеЕсли Ложь
		Или ТаблицаСлов.Найти("Метаданные", "Определение") = Неопределено
		Или Не мЭтоТекстовыйЛитерал И мСтруктураТипаКонтекста.Метаданные = Неопределено // для СправочникиМенеджер
		//И мКонкретныйТипКонтекста <> "Структура"
		//И мКонкретныйТипКонтекста <> "СтрокаТаблицыЗначений"
		//И мКонкретныйТипКонтекста <> "СтрокаДереваЗначений"
		//И мКонкретныйТипКонтекста <> "ВыборкаИзРезультатаЗапроса"
		//И мКонкретныйТипКонтекста <> "ЭлементыФормы"
		//И мКонкретныйТипКонтекста <> "ВсеЭлементыФормы"
		//И мКонкретныйТипКонтекста <> "ДанныеФормыСтруктура"
		//И мКонкретныйТипКонтекста <> "ДанныеФормыСтруктураСКоллекцией"
		//И мКонкретныйТипКонтекста <> "ДанныеФормыЭлементДерева"
		//И мКонкретныйТипКонтекста <> "ДанныеФормыЭлементКоллекции"
	Тогда
		ТипКонтекстаСтатистики = мКонкретныйТипКонтекста;
	КонецЕсли;
	Возврат ТипКонтекстаСтатистики;
КонецФункции 

// Вызывается из адаптера. Отладчик Visual Studio Code внутри не останавливается.
Процедура ОбновитьСтатистикуВыбораСлова(Знач Слово, Знач ЛиМетод, Знач КлючНабораСлов) Экспорт 
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(Знач ТекущаяСтрока, Знач ВернутьСтруктуруТипа = Ложь, Знач НужноУточнитьТип = Ложь) Экспорт 
	#Если Сервер И Не Сервер Тогда
...
Функция ВычислитьТипЗнч(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено, Знач ДляСвойства = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ТаблицаТиповВложенная = ВычислитьТипЗначенияВыражения(МассивПараметров[0], ПредшествующийТекст, ПредшествующийТекст,,,, Ложь,,,,, ПозицияВМетоде,, ИмяФиктивногоСвойства());
...
Функция ОписаниеТекущегоСловаАвтодополнения(Знач Слово, Знач ЛиМетод = Неопределено, Знач КлючНабораСлов = "", Знач ЛиОтдельноеОписаниеАктивно = Истина, Знач ТолькоДляАвтооткрытия = Ложь) Экспорт 
	...
	ЧислоРасширений = 0;
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		ТаблицаТипов = УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаТаблицыСлов, ЛиОтдельноеОписаниеАктивно);
		...
		Если ЛиОтдельноеОписаниеАктивно И ТаблицаТипов <> Неопределено Тогда 
			...
			Если ЛучшийТип <> Неопределено Тогда  
				...
				Тогда
					...
					Тогда
						...
						КонецЕсли;
						//СтруктураОписания.Добавить("> Метод модуля: " + ?(СтрокаСловаМодуля.ЛиЭкспорт, "Экспорт", "Внутр"));
					КонецЕсли;
				ИначеЕсли Истина
					И ТипЗнч(СтрокаСловаМодуля) = Тип("СтрокаТаблицыЗначений")
			...
			КонецЕсли;
			Если Истина
				И ЛиСистемноеСлово(ЛучшийТип, Истина)
				И ЗначениеЗаполнено(ЛучшийТип.СтрокаОписания.ТипКонтекста)
				И ЛучшийТип.СтрокаОписания.ТипКонтекста <> мКонкретныйТипКонтекста
			Тогда  
				СтруктураОписания.Добавить("> Владелец: " + ЛучшийТип.СтрокаОписания.ТипКонтекста);
			ИначеЕсли Истина
				И ЛучшийТип <> Неопределено
				И ирОбщий.ЛиИмяТипаФормыЛкс(мКонкретныйТипКонтекста, Истина, Ложь)
				И ЛучшийТип.СтрокаОписания.Владелец().Колонки.Найти("ИмяМодуля") <> Неопределено
				И Найти(ЛучшийТип.СтрокаОписания.ИмяМодуля, "МодульОбъекта")
			Тогда  
				СтруктураОписания.Добавить("> Владелец: " + "МодульОбъекта");
			КонецЕсли;
			Если Истина
				И ЛучшийТип <> Неопределено
				И (Ложь
		...
		Если ВыражениеЛитерала <> Неопределено Тогда 
			...
			Тогда
				...
				Тогда
					...
					Если ТаблицаТипов[0].ТипЯзыка = "ИмяТипа" Тогда
						СтрокаСлова = ТаблицаСлов.Добавить();
		...
		Тогда
			Если Не ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
				Если ЗначениеЗаполнено(мТекстДляПоискаОпределения) Тогда
					ТекстДляАнализа = мТекстДляПоискаОпределения; // Так не увидим метки дальше по тексту. Поэтому будем искать их отдельно в остатке текста в конце этого метода
				КонецЕсли;
			Иначе
