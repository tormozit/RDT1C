...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	ДобавитьПравилоВычисленияФункции("ЗаписатьЗначениеJSON", "ВычислитьПоместитьВоВременноеХранилище", "Локальный");
...
Процедура ЗаполнитьТипыЗначенийСлов(Знач ВнутренняяТаблицаСлов, Знач ПринудительноВсе = Ложь) Экспорт
	СловаДляУточненияТипа = ВнутренняяТаблицаСлов.НайтиСтроки(Новый Структура("МожноУточнитьТип", Истина));
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	Если Истина
		И мСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа"
		И ТаблицаОжидаемыхТипов.Количество() = 0
		И ирОбщий.ЛиИмяПеременнойЛкс(ВыражениеСлеваОтРавно)
	Тогда
		...
		КлючПоиска.НСлово = ВыражениеСлеваОтРавно;
		Если ирОбщий.СтрНачинаетсяСЛкс(ВыражениеСлеваОтРавно, "сз", Истина) Тогда
			КлючПоиска.НСлово = "СписокЗначений";
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ВыражениеСлеваОтРавно, "тз", Истина) Тогда
			КлючПоиска.НСлово = "ТаблицаЗначений";
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ВыражениеСлеваОтРавно, "дз", Истина) Тогда
			КлючПоиска.НСлово = "ДеревоЗначений";
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ВыражениеСлеваОтРавно, "мс", Истина) Тогда
			КлючПоиска.НСлово = "Массив";
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ВыражениеСлеваОтРавно, "стк", Истина) Тогда
			КлючПоиска.НСлово = "Структура";
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(ВыражениеСлеваОтРавно, "стт", Истина) Тогда
	...
	Если Не ЗначениеЗаполнено(мИмяОжидаемогоТипа) Тогда
		...
		КонецЕсли;
		Если ТаблицаОжидаемыхТипов.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
...
Процедура ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора) Экспорт
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Найденные = НаборСлов.НайтиСтроки(КлючПоиска);
		Если Истина
...
~КонецПоискаСтруктур:
		...
		Тогда
			Если ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса") Тогда
				КоллекцияЭлементовМД = мПлатформа.ПараметрыИзТекстаЗапроса(ВыбраннаяСтруктураТипа.Метаданные.Текст);
			КонецЕсли;
		ИначеЕсли Истина
		...
		ИначеЕсли Истина
			...
			И ИмяПараметра = "Имя"
		Тогда
			КоллекцияЭлементовМД = Метаданные.ФункциональныеОпции[0];
		ИначеЕсли Ложь
		...
		Тогда
			...
			Если ИмяОбщегоТипа = "РасширениеЭлементовУправленияРасположенныхВФорме" Тогда
				ИмяОбщегоТипа = "";
			КонецЕсли;
			мТаблицаСловЛитерала = ирОбщий.СобытияЭлементаФормыЛкс(ЭлементФормыСобытия, ИмяОбщегоТипа);
		ИначеЕсли Истина
			И ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса")
			И (Ложь
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен, Знач ЛиСобиратьПрисвоения = Ложь) Экспорт
	...
	ВхожденияВызовов = НайтиВхожденияРегВыраженияКэш("((Новый\s+)?(" + шОбъектноеВыражение + ")\s*\(\s*(?:(?:" + шПростоеВыражениеПрограммы + "\s*,\s*)*?))" + ИмяШаблон + "\s*[,\)]",,,
...
Процедура ДобавитьПростыеСлова(Знач КоллекцияЭлементовМД, Знач ЛиПриоритетно = Ложь, Знач Определение = "Метаданные")
	...
	Иначе
		ИмяОписанияТипа = "";
		ИмяКлюча = "Имя";
		Если ТипЗнч(КоллекцияЭлементовМД) = Тип("Структура") Тогда
			ИмяКлюча = "Ключ";
			ИмяОписанияТипа = "Значение";
		ИначеЕсли КоллекцияЭлементовМД.Количество() > 0 Тогда
			ЭлементКоллекцииМД = КоллекцияЭлементовМД[0];
			Попытка
	...
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		...
		Если ЛиОтдельноеОписаниеАктивно И ТаблицаТипов <> Неопределено Тогда 
			...
			Тогда
				...
				КонецЕсли; 
				ЛушчийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТипов, 1);
				Если ЛушчийТип <> Неопределено Тогда
					//ПараметрыМетода = мПлатформа.ПараметрыМетодаПлатформы(ЛушчийТип);
					СтруктураПодсказкиМетода = СтруктураПодсказкиМетода(ЛушчийТип, ТолькоДляАвтооткрытия, Истина, ЕстьПараметры);
					...
					Если СтрокаТаблицыСлов.ТипСлова = "Метод" Тогда 
						СтрокаМетода = ЛушчийТип.СтрокаОписания;
						Если Истина
					...
					КонецЕсли; 
				КонецЕсли;
			ИначеЕсли ТаблицаТипов.Количество() > 0 Тогда
				ЛушчийТип = ЛучшийТипИзТаблицыТипов(ТаблицаТипов, 1);
				Если ЛиСистемноеСлово(ЛушчийТип) Тогда
					СинтаксПомощник = мПлатформа.СинтаксПомощник();
					ОписаниеСлова = СинтаксПомощник.ЗагрузитьОписаниеСвойства(ЛушчийТип.СтрокаОписания);
					Если ЗначениеЗаполнено(ОписаниеСлова) Тогда
						СтруктураОписания.Добавить("> Описание: " + ОписаниеСлова);
					КонецЕсли; 
				КонецЕсли;
