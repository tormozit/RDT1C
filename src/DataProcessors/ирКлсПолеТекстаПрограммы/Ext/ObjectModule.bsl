...
Процедура ПриНажатииКлавишиАвтодополнение(Источник, Событие, Данные, Символ) Экспорт
	...
	Тогда 
		...
		Если Ложь
			...
			//	И ЯзыкПрограммы = 0
			//	И (Ложь
			//		Или ирОбщий.СтрКончаетсяНаЛкс(СокрП(мТекущаяСтрокаНачало), "Новый") И не ЛиВнутриКомментария 
			//		Или ирОбщий.СтрКончаетсяНаЛкс(СокрП(мТекущаяСтрокаНачало), "см.") И ЛиВнутриКомментария
			//		Или ирОбщий.СтрКончаетсяНаЛкс(СокрП(мТекущаяСтрокаНачало), "Массив из") И ЛиВнутриКомментария
			//		))
			//Или (Истина
			//	И Символ = " " 
		...
		Тогда
			...
			ирКлиент.ПодключитьОбработчикОжиданияОповещенияЛкс(ирКлиент.ОписаниеОповещенияЛкс("ОткрытьАвтодополнениеПассивно", ЭтотОбъект), Найти(Данные, КодыКлавиш["CTRL+Space"]) = 1);
			СтарыйКонтекст = мРодительскийКонтекст;
		КонецЕсли;
	КонецЕсли;
	Если Ложь
...
Процедура ОбновитьФормуАвтодополнения(Данные) Экспорт
	...
	Если Ложь
		...
		Или Найти(Данные, КодыКлавиш["CTRL+Space"]) = 1 И Не БылЗаполненФильтр
		Или Найти(Данные, КодыКлавиш["CTRL+Space"]) <> 1 И БылЗаполненФильтр
	Тогда
...
Функция ТекущийВызовМетода(Знач НомерПараметраДляВыделения = 0) Экспорт 
	...
	Если мКонечнаяСтрока > 1 Тогда
		...
		Если НомерПараметраДляВыделения > 0 Тогда
			...
			НачальнаяПозиция = 1 + СтрДлина(СлужебноеПолеТекстаДолгое.ВыделенныйТекст) + СтрДлина(ПредшествующийТекст);
		КонецЕсли;
	КонецЕсли;
	Если ЯзыкПрограммы = 0 Тогда
		...
		Для Индекс = 1 - Вхождения.Количество() По 0 Цикл // Обратный обход
			...
			Если Не ирОбщий.ЛиВнутриТекстовогоЛитералаЛкс(Вхождение.Value) Тогда
				...
				НачальнаяПозиция = НачальнаяПозиция - СтрДлина(ПредшествующийТекст);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОткрытьПодсказкуПоВызовуМетодаПассивно(Знач ФормаПодсказкаПоПараметрам, Знач Автообновление = Истина)
	...
	Если ФормаПодсказкаПоПараметрам.Автообновление <> Автообновление Тогда
		// УП
		ФормаПодсказкаПоПараметрам.Автообновление = Автообновление;
	КонецЕсли;
...
Функция ПолноеИмяМетода() Экспорт
	ИмяМетода = мИмяМодуля;
	...
	Тогда
		Фрагменты.Удалить(Фрагменты.ВГраница());
	КонецЕсли;
	Фрагменты.Добавить(мМетодМодуля.Имя);
...
Функция ДобавитьПереходНаСловоМодуляВСписокВыбора(Знач СтрокаСловаМодуля, Знач СписокВыбора, Знач ИмяРасширения = "") Экспорт
	...
	Если СписокВыбора.НайтиПоЗначению(КлючПерехода) = Неопределено Тогда
		Если Представление = "Метод" Тогда
			...
			КонецЕсли;
		Иначе 
			Если СтрокаСловаМодуля.ПозицияОпределения > 0 Тогда
				СписокВыбора.Добавить(КлючПерехода, Представление + " объявление",, Картинка);
			КонецЕсли;
...
// Возвращаемое значение:
//    ТаблицаЗначений - 
Функция ТаблицаТиповИзТекста(Знач ТекстТипа, Знач Описание = "", Знач ТекстДоВхождения = "", Знач ДляСвойстваПуть = "", СчетчикСтрок = 1, Знач ЛиВложенный = Ложь) Экспорт
	ТаблицаТипов = мПлатформа.НоваяТаблицаТипов();
...
Процедура ОткрытьАвтодополнение(Знач КодКлавиши = "", Знач Модально = Ложь) Экспорт 
	...
	Пока Истина Цикл
		...
		Успешно = ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста,, ЕстьЛучшееСлово, Модально,, Не ЛиАвтовызов Или ЛиВнутриКомментария());
	...
	Если ирКэш.ЛиСеансТолстогоКлиентаУПЛкс() Тогда
		Если Не Форма.СвободныйРежим Тогда
			...
			Если ирОбщий.СлужебныеДанныеФормыЛкс(Форма).ИмяФормы = "Обработка.ирКлсПолеТекстаПрограммы.Форма.ВызовМетода" Тогда
				// Первый раз оно почему то не становится поверх окна
				Форма.Закрыть();
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Иначе
		...
		Если мЭтоТекстовыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
			...
			СтарыйМетод = мМетодМодуля;
			КоллекцияЭлементовМД = ЗаполнитьТаблицуВозможныхЗначенийЛитерала(ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен, Истина, ДобавлятьНизкоВероятные);
	...
	Если мЯзыкПрограммы = 1 И мЭтоЛокальныйКонтекстТаблицыСлов Тогда
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЕСТЬ NULL", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ЛЕВОЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ПРАВОЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ВНУТРЕННЕЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ВНЕШНЕЕ СОЕДИНЕНИЕ", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "СГРУППИРОВАТЬ ПО", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "УПОРЯДОЧИТЬ ПО", "Конструкция",, "", "Локальный");
		мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "ОБЪЕДИНИТЬ ВСЕ", "Конструкция",, "", "Локальный");
	КонецЕсли;
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	Тогда
		...
		Если ПредшествующийТокен = "," Или ПредшествующийТокен = "(" Тогда
			...
			Если ЗначениеЗаполнено(мВызовМетода) Тогда
				...
				Если ТаблицаТиповВызова.Количество() > 0 Тогда
					...
					ВыражениеПриемник = ИмяПараметраВызоваМетода(СтрокаОписанияМетода, Ложь, ТаблицаОжидаемыхТипов, ТаблицаТиповВызова[0]);
					...
					Тогда
						...
						Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяПараметраЗапроса) Тогда
							...
							ПостроительЗапроса = ТаблицаТиповВызова[0].Метаданные; // ПостроительЗапроса
							ПостроительЗапроса.Параметры.Свойство(ИмяПараметраЗапроса, ТаблицаОжидаемыхТипов);
						КонецЕсли;
					ИначеЕсли Истина
						И ЛиСистемныйМетод
					...
					Тогда
						...
						Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяКлюча) Тогда
							...
							ТаблицаВозможныхСлов = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ирОбщий.ПервыйФрагментЛкс(мВызовМетода),, Ложь); // см. мПлатформа.НоваяТаблицаСлов() - для ускорения передать фильтр по слову
							Если Истина
		...
		мКэшОжидаемогоТипа = КэшОжидаемогоТипа(ТаблицаОжидаемыхТипов, ВыражениеПриемник, РазрешитьМногоТипов);
	Иначе
		ВыражениеПриемник = мКэшОжидаемогоТипа.ВыражениеПриемник;
		...
		РазрешитьМногоТипов = мКэшОжидаемогоТипа.РазрешитьМногоТипов;
...
// Возвращаемое значение:
//    Структура - 
Функция КэшОжидаемогоТипа(Знач ТаблицаОжидаемыхТипов, Знач ВыражениеПриемник, Знач РазрешитьМногоТипов) Экспорт
	Результат = Новый Структура;
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Для Каждого ВариантИмениТипа Из ВариантыИмениТипа Цикл
			...
			Если Истина
				...
				И Найденные[0].НСлово = НРег(КромеСлова)
			Тогда
				Найденные.Удалить(0);
			КонецЕсли;
...
//  КонтекстТекстовогоЛитералаИзвестен	 - Булево								 - 
//  ЛиДляЛитерала	 - Булево - иначе для структуры 
// 
// Возвращаемое значение:
//   Массив, ТаблицаЗначений - 
//
Функция ЗаполнитьТаблицуВозможныхЗначенийЛитерала(Знач ВыбраннаяСтруктураТипа, КонтекстТекстовогоЛитералаИзвестен = Истина, Знач ЛиДляЛитерала = Истина, Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	Если ЛиДляЛитерала Тогда
		КоллекцияЭлементовМД = Новый Массив;
	КонецЕсли;
	КонтекстТекстовогоЛитералаИзвестен = Истина;
	...
	Тогда
		...
		Если СтрокаОписанияМетода.ТипСлова <> "Свойство" Тогда
			...
			Если ЗначениеЗаполнено(ИмяПеременнойСтруктуры) Тогда
				мТаблицаСловЛитерала = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ИмяПеременнойСтруктуры,, Ложь);
			КонецЕсли;
		КонецЕсли;
	...
	Тогда 
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
			...
			Иначе
				...
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(СтрокаОписанияМетода, КонтекстТекстовогоЛитералаИзвестен, ЛиДляЛитерала, ДобавлятьНизкоВероятные);
			КонецЕсли;
			Если мЭтоТекстовыйЛитерал Тогда
				ВыбраннаяСтруктураТипа.ТипЯзыка = ИмяПараметра;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ЛиДляЛитерала Тогда
		КонтекстТекстовогоЛитералаИзвестен = Ложь;
		...
		Если Прав(ВыражениеСлеваОтРавно, 1) = "=" Тогда
			...
			Если Истина
				...
				И мРодительскийКонтекст = ""
			Тогда
				КоллекцияЭлементовМД = ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ВыражениеСлеваОтРавно, КонтекстТекстовогоЛитералаИзвестен, Истина, Истина, ДобавлятьНизкоВероятные);
...
// Возвращаемое значение:
//    Массив, ТаблицаЗначений - 
Функция ВозможныеТекстовыеЗначенияВыраженияИзДругогоМетода(Знач СтрокаОписанияМетода, КонтекстТекстовогоЛитералаИзвестен = Истина, Знач ЛиДляЛитерала = Истина, Знач ДобавлятьНизкоВероятные = Ложь)
	...
	КоллекцияЭлементовМД = АнализаторКодаМодуля.ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен, ЛиДляЛитерала,, ДобавлятьНизкоВероятные);
...
//   - 
//
Функция ВозможныеТекстовыеЗначенияВыраженияИзТекущегоМетода(Знач ИмяПеременной, РезультатГотов = Ложь, Знач ЛиДляЛитерала = Истина, Знач ЛиСобиратьПрисвоения = Ложь, Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	...
	Пока Истина Цикл
		...
		Для Каждого Вхождение Из ВхожденияВызовов Цикл
			...
			Если ТаблицаТипов.Количество() > 0 Тогда
				...
				НовыеЗначенияПараметра = ЗаполнитьТаблицуВозможныхЗначенийЛитерала(ТаблицаТипов[0], РезультатГотов, ЛиДляЛитерала, ДобавлятьНизкоВероятные);
				Если Истина
					И ЛиДляЛитерала
		...
		Для Каждого ЭлементКоллекцииМД Из КоллекцияЭлементовМД Цикл
			...
			Если ЗначениеЗаполнено(ИмяОписанияТипа) Тогда
				...
				Если ЗаполнятьТип Тогда
					ОбновитьТипЗначенияИзТаблицыТипов(СтрокаСловаСТипом, СтрокаСловаСТипом.ТаблицаТипов);
...
Функция ОписаниеТекущегоСловаАвтодополнения(Знач Слово, Знач ЛиМетод = Неопределено, Знач КлючНабораСлов = "", Знач ЛиОтдельноеОписаниеАктивно = Истина, Знач ТолькоДляАвтооткрытия = Ложь) Экспорт 
	...
	Если СтрокаТаблицыСлов <> Неопределено Тогда
		...
		Если ЛучшийТип <> Неопределено Тогда  
			...
			Тогда
				СтрокаПараметра = ЛучшийТип.СтрокаОписания; // см. мПлатформа.НоваяТаблицаПараметровМетодаМодуля()[0]
...
//
Процедура ОбновитьТипЗначенияИзТаблицыТипов(Знач НоваяСтрока, Знач ТаблицаТипов, Знач ЗаменитьСуществующий = Истина, Знач ЛиФинальныйРасчет = Ложь) Экспорт 
	...
	Если Лев(СтарыйТипЗначения, 2) = "??" Тогда
		...
		Если Не ЛиФинальныйРасчет Тогда 
			Если ЗначениеЗаполнено(НовыйТипЗначения) Тогда
...
Функция ТаблицаСловДляАдаптера(Знач КлючНабораСлов) Экспорт
	...
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
		...
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл 
			...
			Строка.Приоритет = ?(Строка.Рейтинг < РейтингСовпаденияТипа, 0, Цел(Строка.Рейтинг/РейтингСовпаденияТипа) * 100);
		КонецЦикла; 
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл  			Строка.ЛиРез = Строка.ТипЗначения <> "";  			Строка.ЛиМетод = Строка.ТипСлова = "Метод";  			Строка.ЛиТочный = Строка.Определение <> "Статистический";  			Строка.Тип = Лев(Строка.ТипЗначения, 200);  			Строка.Приоритет = ?(Строка.Рейтинг < РейтингСовпаденияТипа, 0, Цел(Строка.Рейтинг/РейтингСовпаденияТипа) * 100);  		КонецЦикла;  
	КонецЕсли;
