...
Функция ТаблицаТиповТекущегоВыражения(ЛиСправаОтРавенства = Ложь, БезКонструкторов = Ложь, ПолныйАнализСоставаТипов = Истина, Знач ЛиCOMВызов = Ложь) Экспорт
			...
			Тогда
				...
				Иначе
					Для Каждого СтрокаТипаМетаОбъекта Из ирКэш.ТипыМетаОбъектов(Истина, Ложь) Цикл
						НадоДобавить = Не ЗначениеЗаполнено(ИмяКорневогоТипаМД) Или ИмяКорневогоТипаМД = СтрокаТипаМетаОбъекта.Единственное;
						Если Не НадоДобавить И мМодульМетаданных.СтруктураТипа <> Неопределено Тогда
							НадоДобавить = Ложь
								Или Истина
