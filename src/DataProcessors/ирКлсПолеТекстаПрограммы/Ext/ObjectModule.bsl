...
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Если Не ирОбщий.СтрокиРавныЛкс(ТипЗначения, "Произвольный") Тогда
		...
		Если ВхожденияВызова.Колонки.Найти("ПараметрыВызова") = Неопределено Тогда
			ВхожденияВызова.Колонки.Добавить("ПараметрыВызова"); // Строка, Массив
		КонецЕсли;
				...
				Иначе
					...
					Иначе
						...
						Если Истина
							...
							И (Ложь
								Или ПолеТекстаПрограммыВызова.мМетодМодуля.ДлинаОпределения > 20000 // Для Обработка.ирКлсПолеТекстаПрограммы.Форма.КонструкторЗапроса.ЗаполнитьПоТокену() нужно 20000
						...
						КонецЕсли;
					КонецЕсли;
					КлючВызова = ИмяВызывающегоМетода + "-" + мМетодМодуля.Имя + "-" + СтрокаПараметраМетода.Имя;
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда 
				...
				БазовйКлючВыражения = мРекурсивныйПуть.ДопКлючКэша + ";" + XMLСтрока(Позиция0ВМетодеДо) + ";" + Лев(Выражение, 200); 
	...
	Если СобиратьОшибки Тогда 
		...
		Если Истина
			...
			И Не ирОбщий.СтрНачинаетсяСЛкс(ЛучшийТипРодителя.ИмяОбщегоТипа, "ВнешняяОбработкаОбъект")
			И Не ирОбщий.СтрНачинаетсяСЛкс(ЛучшийТипРодителя.ИмяОбщегоТипа, "ВнешнийОтчетОбъект")
			И (Ложь
		...
		Тогда 
			...
			Если Ложь
				...
				Или (Истина
					И (Ложь
						Или Найти(ЛучшийТип.ИмяОбщегоТипа, "<") > 0
						Или (Истина 
...
Процедура СобратьДинамическиеСвойстваОбъекта(Знач Выражение, Знач ТаблицаТипов, ТекстДляПоискаОпределения = "", Знач ЭтоВызовКонструктора = Ложь, Знач ПолныйАнализСоставаТипов = Истина, Знач ГлобальныйПоискВМодуле = Ложь,
	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			СобратьСвойстваСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ГлобальныйПоискВМодуле, ЛиВиртПрисвоение, Позиция0ВМетодеОт, Позиция0ВМетодеДо, ДляСвойства);
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				Если ирОбщий.ЛиИмяПеременнойЛкс(ТекстовоеЗначение) Тогда
					...
					Если ЗначениеЗаполнено(ПрисвоенныйТип) Тогда
						...
						Если ЗначениеЗаполнено(ТипЗначения) Тогда
							ТаблицаТиповКолонки = ТаблицаТиповИзТекста(ТипЗначения);
						КонецЕсли;
					КонецЕсли;
	...
	Для Каждого УказательТипа Из ФрагментыЗапятой Цикл 
		...
		Если ЭтоИмяТипа Тогда
			...
			Если ПозицияРазделителя > 0 Тогда
				Фрагменты.Добавить(Лев(ТекстБезВопроса, ПозицияРазделителя));
				Фрагменты.Добавить(Сред(ТекстБезВопроса, ПозицияРазделителя + СтрДлина(МаркерКоллекции) + 1));
			Иначе 
