	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		...
		Тогда
			...
			Для Каждого ИмяТипа Из ЧастыеТипы Цикл
				УстановитьРейтингСлову(ИмяТипа, КонстантаРейтингСовпаденияТипа, "Свойство");
			КонецЦикла;
		КонецЕсли;
		Если Истина                                                    
			И мЭтоЛокальныйКонтекстТаблицыСлов
		...
		Тогда
			...
			УстановитьРейтингСлову("Строка", -КонстантаРейтингСовпаденияТипа, "Метод", "Строка");
...
Функция УстановитьРейтингСлову(Знач Слово, Знач НовыйРейтинг = 0, Знач ТипСлова = "Свойство", Знач ТипЗначения = "") Экспорт
	СтрокаСлова = ТаблицаСлов.НайтиСтроки(Новый Структура("НСлово, ТипСлова", НРег(Слово), "Свойство"));
...
Функция ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "", Знач МножительРейтинга = 2) Экспорт
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			КонецЕсли;
			СтрокаЛучшегоСлова.Рейтинг = СтрокаЛучшегоСлова.Рейтинг + КонстантаРейтингСовпаденияТипа * МножительРейтинга;
			Если Ложь
		...
		Если Найти(мТекстБлока, "(" + ИмяПеременной) > 0 Тогда // Для ускорения. Экономит 3мс на больших текстах
			...
			Для Каждого Вхождение Из Вхождения Цикл
				...
				Если ТаблицаТипов.Количество() > 0 Тогда
					...
					Тогда
						КоллекцияЭлементовМД = мПлатформа.ДочерниеСвойстваДанныхФормы(МетаданныеТипа, Истина, ТаблицаТипов[0].ДержательМетаданных);
					ИначеЕсли ЛучшийТип.ИмяОбщегоТипа = "МенеджерИсторииДанных" Тогда
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(СтрокаТипаКонтекста, Знач ТекущаяСтрока, Знач ВернутьСтруктуруТипа = Ложь, Знач НужноУточнитьТип = Ложь) Экспорт 
	#Если Сервер И Не Сервер Тогда
