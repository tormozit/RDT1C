...
Процедура НайтиВызовыМетода()
	...
	РазобратьТекущийКонтекст();
	Если ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
		ИскомоеСлово = мРодительскийКонтекст; 
	Иначе
		ИскомоеСлово = мКонтекст;
	КонецЕсли;
	ТаблицаСтруктурТиповКонтекста = ОпределитьТипЗначенияКонтекста(ИскомоеСлово, " " + мТекстДляПоискаОпределения, мПредшествующийТекст, Истина);
	СтруктураТипаКонтекста = ТаблицаСтруктурТиповКонтекста[0];
	ПолноеИмяМетода = "";
	...
	Иначе
		Если Прав(мТекущееСлово, 1) <> "(" Тогда
			Возврат;
		КонецЕсли;
		Если СтруктураТипаКонтекста.ИмяОбщегоТипа = "ОбщийМодуль" Тогда 
			ПолноеИмяМетода = СтруктураТипаКонтекста.Метаданные.Имя + ".";
			ПараметрИскатьНепрямые = Ложь;
			СтруктураТипаКонтекста = Неопределено;
		ИначеЕсли Истина
