...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	// БСП
	//ДобавитьПравилоВычисленияФункции("ОбщийМодуль", "ВычислитьОбщийМодуль", "ОбщийМодуль.ОбщегоНазначения");
...
Функция ВыполнитьКоманду(Знач Команда, Знач КодКлавиши = "", ОбновитьКонтекст = Истина)
	...
	Попытка
		...
		ИначеЕсли Команда = КнопкиМакета0.НайтиВызовыМетода.Имя Тогда
...
//  ПозицияВТексте					 - Число - передается для ускорения
//
...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	Знач ПолныйАнализСоставаТипов = Истина) Экспорт 
	...
	Тогда
		...
		Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
			ТаблицаСтруктурТипа = ВычислитьТипЗначенияВыражения(СтрокаСлова.Слово, ПредшествующийТекст, ПредшествующийТекст,,,, Ложь,,, Истина);
					...
					Если ПараметрыМетода <> Неопределено И ТипСлова = "Свойство" Тогда
						...
						Если СтрокаПараметраМетода <> Неопределено Тогда
							...
							Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
								...
								Если ВхожденияПараметров.Количество() > ИндексПараметра Тогда
									...
									Иначе
										...
										ПолеТекстаПрограммыВызова.РазобратьТекущийКонтекст(,,,,,, ВхождениеВызова.ПозицияВхождения + 1);
										...
										Если ПройденныеВызовы[КлючВызова] = Неопределено Тогда
											...
											Если ТаблицаТиповИсточник[0].Детальность > 0 Тогда
												мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, ТаблицаТиповИсточник);
												Если ТаблицаТиповИсточник[ТаблицаТиповИсточник.Количество() - 1].Детальность > 2 Тогда
							...
							Если Ложь 
								...
								Или ТаблицаТипов[ТаблицаТипов.Количество() - 1].Детальность = 0 
							Тогда
								ДобавитьКлючевойПараметрМетодаМодуля(СтрокаПараметраМетода.Имя);
							КонецЕсли;
							Продолжить; ///////
						КонецЕсли;
					КонецЕсли;
					Если мМодульМетаданных <> Неопределено Тогда
						...
						Если НайденныеСтроки.Количество() > 0 Тогда
							...
							Если мЯзыкПрограммы <> 1 Тогда
								ПолеТекстаПрограммыВызова = КопияКомпоненты();
								Если ТипСлова = "Метод" Тогда
									мПлатформа.ПодготовитьТипРезультатаМетода(СтрокаСлова, мМодульМетаданных.Имя,, ПолеТекстаПрограммыВызова); 
								Иначе
									мПлатформа.ПодготовитьТипЗначенияПеременнойМодуля(СтрокаСлова, мМодульМетаданных.Имя, ПолеТекстаПрограммыВызова); 
								КонецЕсли;  
							КонецЕсли;
...
Функция ВычислитьТипФункцииСФактПараметрами(Знач МетодМодуля, Знач ТекущиеАргументы, Знач ПредшествующийТекст, Знач ПолныйАнализСоставаТипов, Знач ПараметрыИсходящегоВызова,
	...
	Для Каждого КлючИЗначение Из МетодМодуля.КлючевыеПараметры Цикл 
		...
		Иначе 
			...
			ПредшествующийТекст = ПредшествующийТекст + Символы.ПС + ИмяПеременной + "=" + ВыражениеПараметра; // Так нужно делать для выражений вида "Ф = Новый Структура"
			ЗначениеПараметра = ВычислитьТипЗначенияВыражения(ИмяПеременной, ПредшествующийТекст,,,, ПолныйАнализСоставаТипов, Ложь,,, Истина);
...
Функция ШаблонВызоваМетода(Знач ИмяВызова) Экспорт 
	...
	Иначе
		...
		Начало = "(?:[^" + шБуква + "\d\.]|^)";
	КонецЕсли;
	ШаблонВызова = Начало + ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(ИмяВызова) + "\s*\(((?:(?:" + шВыражениеПрограммы + ")?" + шРазделитель + "*,)*" + шВыражениеПрограммы + "?" + шРазделитель + "*)\)";
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	Знач ЭтоВызовКонструктора = Ложь, ПолныйАнализСоставаТипов = Истина, Знач КорневойВызов = Истина, Знач ГлобальныйПоиск = Ложь, Знач ТолькоПрисвоения = Ложь, Знач ВычислитьЗначение = Ложь) Экспорт
	...
	Иначе                        
		...
		Тогда
			...
			ВычислениеРазрешено = Ложь;
		КонецЕсли;
		Если Не ВычислениеРазрешено Тогда 
			Если ВычислитьЗначение И Лев(Выражение, 1) = """" Тогда
				СтруктураТипа.ИмяОбщегоТипа = "Строка";
				СтруктураТипа.Метаданные = ВычислитьЗначениеВыражения(Выражение, ПредшествующийТекст);
			КонецЕсли;
		Иначе
			ТаблицаСтруктурРодителей = новый Массив;
			Если Не ТолькоПрисвоения Или Найти(Выражение, ".") = 0 Тогда
				Если Истина
				...
				Иначе 
					ТаблицаСтруктурРодителей = ВычислитьТипЗначенияВыражения(РодительскийКонтекст, ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь);
					...
					КонецЕсли;
				КонецЕсли;
				МассивДочернихСтруктурТипа = ВычислитьТипДочернегоЭлемента(ТаблицаСтруктурРодителей, ТекущееСлово, ТипСлова, ТекущийИндекс, ТекущиеАргументы, ПредшествующийТекст, ПолныйАнализСоставаТипов);
							...
							Если СледРекурсии = Неопределено Тогда
								...
								Если ПрисвоенныйКонтекст <> Выражение Тогда
									МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДляПоискаОпределения,
										Лев(ТекстДляПоискаОпределения, ПоследнееВхождение.FirstIndex - 1), РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь);
				...
				Иначе
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если Не ГлобальныйПоиск Тогда 
								// В рамках текущего метода
							Иначе
								Если ЗагрузитьМетодМодуляПоПозиции(ПозицияВхождения0) Тогда 
									СлужебноеПолеТекстаДолгое.УстановитьГраницыВыделения(ПозицияВхождения0, ПозицияВхождения0);
									НачальнаяСтрока = 0;
									НачальнаяКолонка = 0;
									СлужебноеПолеТекстаДолгое.ПолучитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка,,);
									КончитьОбработкуКоманды();
									РазобратьТекущийКонтекст(,,, НачальнаяСтрока, НачальнаяКолонка);
								КонецЕсли;
							КонецЕсли;
							ПрисвоенныйТип = ПоследнееВхождение.SubMatches(21);
							Если Истина
							...
							Тогда
								...
								ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
							...
							Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(1)) Тогда
								...
								Если Истина
									И ВычислитьЗначение 
									И Вхождения.Количество() = 1
								Тогда
									ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
									СтруктураТипа.Метаданные = ВычислитьЗначениеВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения);
								КонецЕсли;
								Если Ложь
								...
								Тогда 
									Продолжить;
								ИначеЕсли ЛиПрисвоениеБулевойКонстанты(ПрисвоенныйКонтекст) Тогда
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, мПлатформа.НоваяСтруктураТипа("Булево"));
									Продолжить;
								КонецЕсли;
								...
								ИначеЕсли Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(6)) Тогда 
									ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
									МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь);
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, МассивСтруктурПрисвоенныхТипов);
							...
							Иначе
								...
								ТекстДоВхождения = Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1);
								ТаблицаТиповКоллекции = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДоВхождения, ТекстДоВхождения,,, ПолныйАнализСоставаТипов, Ложь);
								СтруктураТипаКоллекции = ТаблицаТиповКоллекции[0];
								Если ЗначениеЗаполнено(СтруктураТипаКоллекции.ИмяОбщегоТипа) Тогда
									МассивСтруктурПрисвоенныхТипов = мПлатформа.СловаКонтекстаМетаданные(СтруктураТипаКоллекции, "[0]",,, мЯзыкПрограммы, "0",,, мФлагиКомпиляции)[0].ТаблицаТипов;
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, МассивСтруктурПрисвоенныхТипов);
								КонецЕсли;
							КонецЕсли;
							Если Истина
								И ТаблицаТипов.Количество() > 0
								И ТаблицаТипов[ТаблицаТипов.Количество() - 1].Детальность > 1
							Тогда 
								Прервать;
							КонецЕсли;
						КонецЦикла;
			...
			Если ТипСлова = "Свойство" И Не СтруктураТипа.Конструктор Тогда
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						СобратьЗначенияСвойствСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ТекстДляПоискаОпределения, ПредшествующийТекст);
					...
					Тогда
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							КонецЕсли;
							Счетчик = Счетчик + 1;
							ТаблицаТиповЭлементов = ВычислитьТипЗначенияВыражения(ВхождениеСвойства.SubMatches(0), ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа,
								ЭтоВызовКонструктора, ПолныйАнализСоставаТипов, Ложь);
					...
					Тогда
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							Иначе
								ТекстовоеЗначение = ВычислитьЗначениеВыражения(ПрисвоенноеВыражение, Лев(ТекстДляПоискаОпределения, ВхождениеСвойства.FirstIndex));
							КонецЕсли;
					...
					Тогда
						ТекстЗапроса = ВычислитьЗначениеВыражения(Выражение + ".Текст", ТекстДляПоискаОпределения);
						Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
							...
							ирОбщий.СкопироватьКоллекциюЛкс(СобратьЗначенияСвойствСтруктуры(Выражение + ".Параметры",, ТекстДляПоискаОпределения, ПредшествующийТекст), ПостроительЗапроса.Параметры);
						КонецЕсли; 
						мПлатформа.ОбновитьДетальностьСтруктурыТипа(СтруктураТипаЦикл);
					КонецЕсли; 
				КонецЦикла;
...
Функция СобратьЗначенияСвойствСтруктуры(Знач Выражение, Структура = Неопределено, Знач ТекстДляПоискаОпределения, Знач ПредшествующийТекст) Экспорт
	мРегВыражение.Global = Ложь;
	...
	Если РезультатСвойств.Количество() > 0 Тогда
		ВхождениеСвойства = РезультатСвойств[0];
		ПрисвоенноеВыражение = ВхождениеСвойства.SubMatches(0);
		Если Лев(ПрисвоенноеВыражение, 1) = """" Тогда
			ТекстовоеЗначение = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ПрисвоенноеВыражение);
		Иначе
	...
	Тогда
		...
		КонецЕсли;
	КонецЕсли;
	МинимальнаяДетальность = 4;
	мРегВыражение.Pattern = Выражение + "\s*\.\s*Вставить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")(?:\s*,\s*([^\n\)]+)?)";
	мРегВыражение.Global = Истина;
	РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляПоискаОпределения);
	Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
		ВыражениеИмени = ВхождениеСвойства.SubMatches(0);
		Если Лев(ВыражениеИмени, 1) = """" Тогда
			ТекстовоеЗначение = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ВыражениеИмени);
		Иначе
			ТекстовоеЗначение = ВычислитьЗначениеВыражения(ВыражениеИмени, Лев(ТекстДляПоискаОпределения, ВхождениеСвойства.FirstIndex));
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда 
			ВыражениеЗначения = ВхождениеСвойства.SubMatches(1);
			ТаблицаТиповЗначения = ВычислитьТипЗначенияВыражения(ВыражениеЗначения, ТекстДляПоискаОпределения, ПредшествующийТекст);
			ЗначениеСвойства = Неопределено;
			Если ТаблицаТиповЗначения[0].Детальность >= МинимальнаяДетальность Тогда
				ЗначениеСвойства = ТаблицаТиповЗначения[0].Метаданные;
			КонецЕсли;
			Структура.Вставить(ТекстовоеЗначение, ЗначениеСвойства);
		КонецЕсли;
	КонецЦикла;
	Для Каждого КлючИЗначение Из ирОбщий.ОтобратьКоллекциюЛкс(Структура, Новый Структура("Значение")) Цикл
		ВыражениеЗначения = Выражение + "." + КлючИЗначение.Ключ;
		ТаблицаТиповЗначения = ВычислитьТипЗначенияВыражения(ВыражениеЗначения, ТекстДляПоискаОпределения, ПредшествующийТекст,,,,,, Истина);
		ЗначениеСвойства = Неопределено;
		Если ТаблицаТиповЗначения[0].Детальность >= МинимальнаяДетальность Тогда
			ЗначениеСвойства = ТаблицаТиповЗначения[0].Метаданные;
			Структура.Вставить(КлючИЗначение.Ключ, ЗначениеСвойства);
		КонецЕсли;
	КонецЦикла;
	Возврат Структура;
КонецФункции
...
Функция ШаблонНазначенияТипаВыражению(Знач ШаблонПриемника) Экспорт
	...
	Результат = шПредИмя + ШаблонПриемника 
		//+ "(" + шПрисвоение + ";?|" + ШаблонКоллекции + ")(\s*//" + шНазначениеТипа + ")?";
		+ "(" + шПрисвоение + ";?|" + ШаблонКоллекции + ")(?:\s*//([^\n]+))?";
	Возврат Результат;
КонецФункции
...
// Возвращаемое значение:
//    Строка - 
Функция ВычислитьЗначениеВыражения(Выражение, Знач ПредшествующийТекст)
	...
	Если Лев(Выражение, 1) = """" Тогда 
		Если Найти(Выражение, Символы.ПС) = 0 Тогда
			// Для ускорения
			Результат = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(Выражение);
		Иначе 
			Попытка
		...
		КонецЕсли;
	Иначе
		// опасность циклической рекурсии
		ТаблицаТипов = ВычислитьТипЗначенияВыражения(Выражение, ПредшествующийТекст, ПредшествующийТекст,,,, Ложь,, Найти(Выражение, "(") = 0, Истина);
		СтруктураТипа = ТаблицаТипов[0];
	...
	Тогда
		...
		Если ПравилоВычисленияФункции = Неопределено Тогда
			ПараметрыМетода = мПлатформа.ПараметрыМетодаМодуля(мМетодМодуля);
			Если ПараметрыМетода <> Неопределено Тогда
				СтрокаПараметра = ПараметрыМетода.Найти(НРег(Выражение), "НИмя");
				Если СтрокаПараметра <> Неопределено Тогда
					ДобавитьКлючевойПараметрМетодаМодуля(СтрокаПараметра.Имя);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// Параметры:
//    ИмяПараметра - ? - 
Процедура ДобавитьКлючевойПараметрМетодаМодуля(Знач ИмяПараметра) Экспорт
	...
	КонецЕсли; 
	КлючевыеПараметры.Вставить(ИмяПараметра);
КонецПроцедуры
...
Функция ТаблицаТиповИзТекста(Знач ТекстТипа, Знач Описание = "", Знач ТекстДоВхождения = "") Экспорт
	...
	Иначе
		...
		Если ЭтоИмяТипа Тогда
			Фрагменты = ирОбщий.СтрРазделитьЛкс(ТекстТипа, " из ", Истина);
			...
			Иначе
				...
				ОписаниеСвойств = Неопределено;
				Если Найти(ТекстТипа, "Структура") > 0 Тогда
					ОписаниеСвойств = Новый Структура;
				КонецЕсли; 
				ИмяТипаФлага = мПлатформа.ИмяТипаФлагаВнутриСтруктуры();
					...
					Если Лев(СтрокаОписания, 1) = "*" Тогда
						...
						Попытка
							...
							Если ОписаниеСвойств <> Неопределено Тогда
								МетаданныеСвойства = Новый ОписаниеТипов(ОписаниеТипов, ИмяТипаФлага);
							КонецЕсли;
						Исключение
							ОписаниеТипов = Неопределено;
							Если ОписаниеСвойств <> Неопределено Тогда
								МетаданныеСвойства = ТаблицаТиповИзТекста(ФрагментыОписания[1])[0].Метаданные;
							КонецЕсли;
						КонецПопытки;
						Попытка
							ОписаниеКолонок.Колонки.Добавить(ИмяСвойства, ОписаниеТипов);
						Исключение
						КонецПопытки;
						Попытка
				...
				Для Каждого ИмяТипа Из ФрагментыЗапятой Цикл
					...
					Если Тип <> Неопределено Тогда
						...
						Если ТипЗнч(СтруктураТипа.Метаданные) = Тип("ОбъектМетаданныхКонфигурация") Тогда
							СтруктураТипа.ИмяОбщегоТипа = ИмяТипа;
						КонецЕсли;
						Если ОписаниеКолонок.Колонки.Количество() > 0 Тогда
							Если Ложь
								Или ИмяТипа = "Структура"
								Или ИмяТипа = "ФиксированнаяСтруктура"
							Тогда 
								МетаданныеТипа = ОписаниеСвойств;
							Иначе
								МетаданныеТипа = ОписаниеКолонок;
							КонецЕсли;
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	Знач _УдалитьФункциюНовый = Ложь, Знач Сортировать = Истина, Знач ДобавлятьНизкоВероятные = Ложь) Экспорт
	...
	Попытка
		ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(выхСтруктураТипаКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы,,,
			?(мЭтоСтроковыйЛитерал, Неопределено, мФлагиКомпиляции),, МодульМетаданныхКонтекста(выхСтруктураТипаКонтекста),,, ДобавлятьЛокальныйКонтекст);
...
Процедура ОткрытьПоискВызововМетода(Знач ИмяМетода = Неопределено, Знач ИскатьНепрямые = Неопределено) Экспорт 
...
Функция ВычислитьНайтиЭлементКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	|" + ИмяПеременной + "=" + МассивПараметров[0],,,,, Ложь);
	Возврат Результат;
...
Функция ВычислитьОтобратьКоллекциюЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	|" + ИмяПеременной + "=" + МассивПараметров[0],,,,, Ложь); 
	Результат = мПлатформа.НоваяСтруктураТипа("Массив[" + мПлатформа.ПредставлениеМассиваСтруктурТипов(ТаблицаТиповЭлементов) + "]");
	Возврат Результат;
...
Функция ВычислитьОбщийМодуль(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
	ИмяМодуля = ВычислитьЗначениеВыражения(ПервыйПараметр, ПредшествующийТекст);
...
Функция УстановитьТекст(Знач Текст, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	КонецЕсли; 
	мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипа, ТипМодуля,, Текст);
	мФлагиКомпиляции = ирОбщий.СкопироватьКоллекциюЛкс(мМодульМетаданных.ФлагиКомпиляции);
...
Процедура УстановитьСлужебныеДанныеФормы(Знач Текст) Экспорт
	...
	мПлатформа.ОбновитьКэшСтруктурыФормыИзБуфера(СлужебныеДанные, Текст);
КонецПроцедуры
...
Процедура ОбновитьМодульМетаданных(Знач Текст = Неопределено) 
	...
	Если мМодульМетаданных <> Неопределено И мМодульМетаданных.СтруктураТипа <> Неопределено Тогда
		мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(мМодульМетаданных.СтруктураТипа, ирОбщий.ПоследнийФрагментЛкс(мИмяМодуля),, Текст);
	Иначе
...
Функция ТаблицаСловВJSON() Экспорт 
	
	ВыгрузкаТаблицы = ТаблицаСлов.Выгрузить(, "ТипСлова,Слово,ТипЗначения");
