		...
		Иначе
			...
			Если мЭтоТекстовыйЛитерал Тогда
				...
				ТаблицаТипов = ВычислитьТипЗначенияВыражения(мВызовМетода + ИмяОтключенияЛишнихВычислений + ")", ТекстДляПоискаОпределения, ПредшествующийТекст,, мЭтоКонструктор, ПолныйАнализСоставаТипов,
					Ложь,,,,, Позиция0ВМетодеДо,, ДляСвойства); // ИмяФиктивногоСвойства() нельзя всегда передавать, т.к. Запрос.УстановитьПараметр("") не будет собирать данные
				Возврат ТаблицаТипов;
			КонецЕсли; 
		КонецЕсли;
	КонецЕсли;
								...
								Иначе //Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(ЧислоГруппВШаблонеВыражения + 8)) Тогда ;
									...
									Если ПрименитьТип Тогда 
										...
										Если ТаблицаПрисвоенныхТипов.Количество() > 0 Тогда
											...
											КонецЦикла;
											мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, ТаблицаПрисвоенныхТипов,,,, Ложь);
										КонецЕсли;
									КонецЕсли;
								КонецЕсли;
							Иначе
...
Функция СобратьСвойстваСтруктуры(Знач Выражение, Знач Структура = Неопределено, Знач ГлобальныйПоискВМодуле = Ложь, Знач ТолькоПрисвоение = Ложь, Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0,
	...
	МинДетальность = 4;
	ШаблонПоиска = шПредИнструкция + Выражение + "\s*\.\s*(Вставить|Свойство)\s*\(\s*(" + шЛитералПрограммы + "|" + шИмяСТочками + ")(?:\s*,\s*((?:" + шЛитералПрограммы + "|" + шСкобкиЛенивые + "|[^\)""])*))?" 
...
Процедура РассчитатьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов)
	...
	КонецЕсли;
	Если мПредшествующийТекст <> мСтарыйПредшествующийТекст Тогда
		ПредшествующийТокен = Прав(СокрП(мПредшествующийТекст), 2);
		Если ПредшествующийТокен <> "<>" Тогда
			ПредшествующийТокен = Прав(ПредшествующийТокен, 1);
		КонецЕсли;
		Если ПредшествующийТокен = "," Или ПредшествующийТокен = "(" Тогда
			...
			Если ЗначениеЗаполнено(мВызовМетода) Тогда
				...
				КонецЕсли;
			КонецЕсли; 
		ИначеЕсли ПредшествующийТокен = "=" Или ПредшествующийТокен = "<>" Тогда
...
Функция ВычислитьСкопироватьКолонки(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	...
	Если ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("ТаблицаЗначений") Тогда
		СтрукутураТипа.Метаданные = РодительскаяСтруктураТипа.Метаданные.СкопироватьКолонки(ИменаКолонок);
	Иначе
		Если ЗначениеЗаполнено(ИменаКолонок) Тогда
			...
			Исключение
				Возврат ТаблицаТипов;
			КонецПопытки;
			ЗаполнитьЗначенияСвойств(СтрукутураТипа.Метаданные, РодительскаяСтруктураТипа.Метаданные);
		ИначеЕсли ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("Структура") Тогда
			СтрукутураТипа.Метаданные = ирОбщий.СкопироватьКоллекциюЛкс(РодительскаяСтруктураТипа.Метаданные);
		КонецЕсли;
	КонецЕсли;
