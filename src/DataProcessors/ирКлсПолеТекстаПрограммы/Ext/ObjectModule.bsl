...
Процедура РазобратьКонтекстЗапросаВТекстовомЛитерале(Знач ВыражениеЛитерала, Знач ТекстЛитерала) Экспорт
	...
	Если Вхождения.Количество() > 0 Тогда
		...
		ТаблицаТиповЗапроса = КопияАнализатора.ВычислитьТипЗначенияВыражения(ПеременнаяЗапроса,,,,,,,,,,, КопияАнализатора.мПозицияВБлоке + 1); 
		Если Истина
	...
	Для Каждого РодительскаяСтруктураТипа Из РодительскиеСтрутурыТипа Цикл
		...
		Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
			...
			Если ТаблицаСвойствТипа.Количество() > 0 Тогда
				...
				Пока Истина
					И ТаблицаСвойствТипа.Количество() > 0
...
Функция ОткрытьПоискВМодулях(Знач ЧтоИскать = Неопределено, Знач Автозапуск = Истина) Экспорт 
	...
	Если ЧтоИскать = Неопределено Тогда
		...
		Если Ложь
			...
			//Или ЧтоИскать = ирОбщий.ПоследнийФрагментЛкс(ТекущееОбъектноеВыражение())
		Тогда
...
//
Процедура ПроверитьВыраженияПослеТочки(Знач ТабличноеПоле = Неопределено, Форма = Неопределено) Экспорт
