...
Функция ДобавитьПереходыИзСтруктурыТипаВСписокВыбора(Знач СтруктураТипа, Знач СписокВыбора = Неопределено) 
	...
	Если ТипЗнч(СтруктураТипа.ДержательМетаданных) = Тип("Структура") Тогда 
		Если СтруктураТипа.ДержательМетаданных.Тип = Тип("МенеджерВременныхТаблиц") Тогда 
			...
			ПакетЗапросов = МенеджерВременныхТаблиц.ПакетыЗапросов[МенеджерВременныхТаблиц.ПакетыЗапросов.ВГраница()]; // см. НовыйПакетЗапросов()
		Иначе
			// Колонка результата запроса
			ПакетЗапросов = СтруктураТипа.ДержательМетаданных;
		КонецЕсли;
		КоординатыТекстаЗапроса = ПакетЗапросов.КоординатыТекста; // см. НовыеКоординатыВыражения()
		...
		Если ВычислениеРазрешено Тогда
			...
			Если мРекурсивныйПуть.ДопКлючКэша <> Неопределено Тогда 
				Если мМетодМодуля <> Неопределено Тогда 
					...
					КонецЕсли;
				Иначе
					ТипыВыраженийМетода = мРекурсивныйПуть.ТипыВыраженийПрограммы; // TODO перенести в мМодульМетаданных
				КонецЕсли;
...
Функция УстановитьТекст(Знач Текст = Неопределено, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	КонецЕсли;
	мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипа, ТипМодуля,, Текст, мМетодМодуля, НовоеКонецВыделения);
