...
//
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм",
	пКонфигурация = Неопределено, Знач пПолеТекстовогоДокумента = Неопределено) Экспорт
...
Процедура РазобратьКонтекстСтроки(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Ложь) Экспорт
	...
	Тогда
		СтрокаШаблона = "(?:" + шРазделитель + "+(КАК|AS|(?:ПОМЕСТИТЬ|INTO(?:" + шРазделитель + "+TABLE)?))" + шРазделитель + "*)?" + СтрокаШаблона + шПрефиксаПараметра + "(?:" + шИмя + ")?";
	Иначе
	...
	Если Ложь
		...
		Или мЯзыкПрограммы = 2
	Тогда
		СтрокаШаблона = шПрефиксаПараметра + "(?:" + шИмя + ")?"; 
	Иначе
		СтрокаШаблона = шИмя;
	КонецЕсли;
...
Функция ВыделитьМетод(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	НевидимоеПолеТекста = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
	#Если Сервер И Не Сервер Тогда
		НевидимоеПолеТекста = Обработки.ирКлсПолеТекстаПрограммы.Создать();
		ПараметрыВходаАлгоритма = Новый Структура;
	#КонецЕсли
	НевидимоеПолеТекста.ИнициироватьНеинтерактивно();
	НевидимоеПолеТекста.мМодульМетаданных = мМодульМетаданных;
	НевидимоеПолеТекста.ЭтоМодуль = Истина;
	НомераПервойСтрокиТела = 1;
	НевидимоеПолеТекста.УстановитьТекст(ТелоАктивногоМетода(НомераПервойСтрокиТела));
	//НевидимоеПолеТекста.мМодульМетаданных = мМодульМетаданных;
	СмещениеНомеровСтрок = НомераПервойСтрокиТела - 1;
	Если Не ЗначениеЗаполнено(ВыделенныйТекст()) Тогда
		ПолеТекста.УстановитьГраницыВыделения(мПерваяСтрокаТелаМетодаМодуля, 1, мПоследняяСтрокаТелаМетодаМодуля + 1, 1);
	КонецЕсли;  
	...
	Если КонСтрока - СмещениеНомеровСтрок < 1 Или НачСтрока - СмещениеНомеровСтрок < 1 Тогда
		Возврат Ложь;
	КонецЕсли;
	НевидимоеПолеТекста.ПолеТекста.УстановитьГраницыВыделения(НачСтрока - СмещениеНомеровСтрок, НачКолонка, КонСтрока - СмещениеНомеровСтрок, КонКолонка);
	...
	//ФормаВладелец.Модифицированность = Истина;
	НевидимоеПолеТекста.ПолеТекста.УстановитьГраницыВыделения(1, 1, НачСтрока - СмещениеНомеровСтрок, НачКолонка);
	ТекстДо = НевидимоеПолеТекста.ПолеТекста.ВыделенныйТекст();
	НевидимоеПолеТекста.ПолеТекста.УстановитьГраницыВыделения(КонСтрока - СмещениеНомеровСтрок, КонКолонка, Макс(1, НевидимоеПолеТекста.ПолеТекста.КоличествоСтрок()), 300);
	ТекстПосле = НевидимоеПолеТекста.ПолеТекста.ВыделенныйТекст();
	НевидимоеПолеТекста.ПолеТекста.УстановитьГраницыВыделения(НачСтрока - СмещениеНомеровСтрок, НачКолонка, КонСтрока - СмещениеНомеровСтрок, КонКолонка);
	ФормаНовогоМетода = НевидимоеПолеТекста.ПолучитьФорму("ВыделениеМетода", ФормаВладелец);
	...
	ПараметрыИзАлгоритма(ТекстДо + ТекстНовогоМетода + ТекстПосле, Не ЛиСерверныйКонтекст, НевидимоеПолеТекста, ЛиКорректныйФрагмент,, Истина);
	...
	Если Не ЛиКорректныйФрагмент Тогда
		...
		КонецЕсли; 
	КонецЕсли; 
	НевидимоеПолеТекста.УстановитьТекст(ТекстДо);
	...
	НевидимоеПолеТекста.УстановитьТекст(ТекстНовогоМетода);
	...
	Для Каждого СтрокаПеременной Из СтрокиЛокальныхПеременных Цикл
		Если ЛиСерверныйКонтекст И МетаОбщиеМодули.Найти(СтрокаПеременной.НСлово) <> Неопределено Тогда
			Продолжить;
		КонецЕсли; 
	...
	ПараметрыСквозныеАлгоритма = ПараметрыИзАлгоритма(ТекстДоПереименованный + ТекстНовогоМетода + ТекстПосле, Не ЛиСерверныйКонтекст, НевидимоеПолеТекста, ЛиКорректныйФрагмент,
	...
	ПараметрыВыходаАлгоритма = ПараметрыИзАлгоритма(ТекстДоПереименованный + ЗаглушкаТекстаМетода + ТекстПосле, Не ЛиСерверныйКонтекст, НевидимоеПолеТекста,, ИмяПеременнойЗаглушки);
	#Если Сервер И Не Сервер Тогда
	...
	ПараметрыВыходаЛожные = ПараметрыИзАлгоритма(ТекстДо + ТекстНовогоМетода + ТекстПосле, Не ЛиСерверныйКонтекст, НевидимоеПолеТекста);
	#Если Сервер И Не Сервер Тогда
	...
	Для Каждого СтрокаПараметра Из ФормаНовогоМетода.ТаблицаПараметров Цикл
		...
		Если ПараметрМетода = Неопределено Тогда
			СтрокаПараметра.СвойствоМодуля = ТаблицаЛокальногоКонтекста().Найти(НРег(СтрокаПараметра.НИмя), "нИмя") <> Неопределено; 
	...
	Если мМетодМодуля <> Неопределено Тогда
		ПолеТекста.ВставитьСтроку(мПоследняяСтрокаТелаМетодаМодуля + 2, Символы.ПС + ФормаНовогоМетода.ПолноеОпределение);
	КонецЕсли;
	ПолеТекста.УстановитьВыделениеДвумерное(СтруктураВыделения);
	ВыделенныйТекст(ФормаНовогоМетода.ТекстВставки);
	Если мМетодМодуля = Неопределено Тогда
		ПолеТекста.ВставитьСтроку(мПерваяСтрокаТелаМетодаМодуля, Символы.ПС + ФормаНовогоМетода.ПолноеОпределение);
	КонецЕсли;
	Если ФормаВладелец = Неопределено Тогда
...
Функция ЗаменитьВозвратыНаПерейтиВТекстеМетода(Знач Текст = Неопределено, Знач ИмяПеременнойРезультата = "Результат", Знач ДобавлятьПереносыСтрок = Ложь) Экспорт
	
	Если Текст = Неопределено Тогда
		Текст = ПолеТекста.ПолучитьТекст();
	КонецЕсли;
	Результат = мПлатформа.ЗаменитьВозвратыНаПерейтиВТекстеМетода(Текст, ИмяПеременнойРезультата, ДобавлятьПереносыСтрок);
	Возврат Результат;
...
Функция ПараметрыИзАлгоритма(Знач ТекстДляПроверки, Знач ЭтоКлиент, Знач НевидимоеПолеПрограммы, выхКорректныйФрагмент = Истина, ИгнорироватьПеременную = "", ПоказыватьСтрокуОшибки = Ложь) Экспорт 
	...
	ТекстДляПроверки = мПлатформа.ЗаменитьВозвратыНаПерейтиВТекстеМетода(ТекстДляПроверки) + ";" + Символы.ПС + "~Конец:";
...
Процедура РазобратьТекущийКонтекст(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Неопределено, Знач НомерСтроки = 0, Знач НомерКолонки = 0,
	...
	Если мЯзыкПрограммы = 0 Тогда
		...
		Если мМетодМодуля <> Неопределено Тогда 
			ТекстБлока = мМетодМодуля.Тело;
			мПозицияТекстаДляПоискаОпределения = мМетодМодуля.ПозицияТела;
		Иначе 
			ТекстБлока = мМодульМетаданных.Программа;
		...
		мПерваяСтрокаТелаМетодаМодуля = СтрЧислоСтрок(Лев(мОригинальныйТекст, мПозицияТекстаДляПоискаОпределения) + "й"); 
		мПоследняяСтрокаТелаМетодаМодуля = мПерваяСтрокаТелаМетодаМодуля + ЧислоСтрокВБлоке - 1;
		НомерТекущейСтрокиВБлоке = мКонечнаяСтрока - мПерваяСтрокаТелаМетодаМодуля + 1;
	Иначе
	...
	мТекстБезКомментариевИОпасныхСтрок = ЗалитьКомментарииИСтроковыеЛитералы(ТекстБлока);
	Если НомерТекущейСтрокиВБлоке > 1 Тогда
		мПолеТекстаВременное.УстановитьТекст(ТекстБлока);                              
...
Функция ТелоАктивногоМетода(выхНомераПервойСтрокиТела = 1) Экспорт 
	выхНомераПервойСтрокиТела = мПерваяСтрокаТелаМетодаМодуля;
	Если мМетодМодуля <> Неопределено Тогда
...
Функция ПерейтиКОпределению(НомерСтроки = 0, НомерКолонки = 0, РазрешитьДиалоги = Истина) Экспорт 
	...
	ИначеЕсли мЯзыкПрограммы = 0 Тогда 
		Если мЭтоСтроковыйЛитерал Тогда
			СтрокаОписанияМетода = ТаблицаСтруктурТиповТекущегоВыражения()[0].СтрокаОписания;
			ИмяПараметра = ИмяПараметраВызоваМетода(СтрокаОписанияМетода);
			ИмяМетодаСсылки = мФактическиеПараметры[мНомерПараметра - 1];
			Если Ложь
				Или ИмяПараметра = "ИмяПроцедуры"
				Или ИмяПараметра = "Действие"
			Тогда
				ИмяМетодаСсылки = ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(ИмяМетодаСсылки);
				МетодМодуля = мМодульМетаданных.Методы.Найти(НРег(ИмяМетодаСсылки), "НИмя");
				Если МетодМодуля <> Неопределено Тогда
					ПерейтиКОпределениюЛокальногоМетода(МетодМодуля);
					Возврат Истина;
				КонецЕсли;
			ИначеЕсли СтрокаОписанияМетода.Слово = "Выполнить" И СтрЧислоВхождений(мФактическиеПараметры[0], ".") = 1 Тогда 
				ИмяМетодаСсылки = ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(ИмяМетодаСсылки);
				Фрагменты = ирОбщий.СтрРазделитьЛкс(ИмяМетодаСсылки);
				Если Истина
				...
				Тогда 
					...
					КонецЕсли;
				КонецЕсли;
				ирКлиент.ПерейтиКОпределениюМетодаВКонфигуратореЛкс(ИмяМетодаСсылки);
				Возврат Неопределено;
			КонецЕсли;
		Иначе
			...
			Если СтрокаПеременной <> Неопределено Тогда
				ПолеТекста.УстановитьГраницыВыделения(СтрокаПеременной.Позиция, СтрокаПеременной.Позиция,,,, ФормаВладелец);
				ВыделитьДиапазонОдномерныйДляПользователя(СтрокаПеременной.Позиция, СтрокаПеременной.Позиция + СтрДлина("Перем " + СтрокаПеременной.Имя));
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
...
Функция ОткрытьОпределениеСтруктурыТипа(Знач СтруктураТипа, Знач ЭтоОбщийТип = Ложь, РазрешитьДиалоги = Истина)  Экспорт 
	...
	Если СтруктураТипа.СтрокаОписания <> Неопределено Тогда 
		...
		#КонецЕсли
		Если ТаблицаВладелец.Колонки.Найти("ЛиЭкспорт") <> Неопределено  Тогда
			ИмяХраненияВыбора = ИмяКласса + ".РежимОткрытияМодулей";
			...
			КонецЕсли;
			ИмяМодуля = СтруктураТипа.СтрокаОписания.ИмяМодуля;
			ИмяМетода = СтруктураТипа.СтрокаОписания.Имя;
			...
			Если СписокВыбора.Количество() = 0 Тогда
				Возврат Ложь;
			КонецЕсли;
			Если РазрешитьДиалоги И ВариантОткрытьВКонфигураторе <> Неопределено Тогда
			...
			Иначе
				ПолеТекстаНовое = ПолеТекстаМодуляКонфигурации(ИмяМодуля);
				ПолеТекстаНовое.ПерейтиКОпределениюЛокальногоМетода(СтруктураТипа.СтрокаОписания);
			КонецЕсли; 
			Возврат Истина;
		ИначеЕсли Истина
...
Функция ПолеТекстаМодуляКонфигурации(ИмяМодуля, ФормаТекста = Неопределено) Экспорт
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ФайлМодуля = мПлатформа.ФайлМодуляИзКраткогоИмени(ИмяМодуля);
	ТекстовыйДокумент.Прочитать(ФайлМодуля.ПолноеИмя); 
	ФормаТекста = ирКлиент.ОткрытьТекстЛкс("", ИмяМодуля + " " + ФайлМодуля.ПолучитьВремяИзменения(), "ВстроенныйЯзык", Истина, ИмяМодуля);
	ПолеТекстаНовое = ФормаТекста.ПолеТекстаПрограммы.ВстроенныйЯзык;
	#Если Сервер И Не Сервер Тогда
	...
	Если мЯзыкПрограммы = 1 Тогда
		...
		Если РезультатТекста.Количество() > 0 Тогда
			...
			Тогда
				...
				РезультатТекста = мРегВыражение.НайтиВхождения(мТекстБезКомментариевИОпасныхСтрок);
				Если РезультатТекста.Количество() > 0 Тогда
	...
	Для Каждого РодительскаяСтруктураТипа Из МассивРодительскихСтрутурТипа Цикл
		...
		Если РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
			...
			Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
				...
				Если Истина
					И РодительскаяСтруктураТипа.ИмяОбщегоТипа = "Локальный"
					//И (Ложь
				...
				мПлатформа.СвойстваТипаПредопределенные(РодительскаяСтруктураТипа, ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, мЯзыкПрограммы, Конфигурация,,, ТаблицаСвойствТипа,
					РодительскаяСтруктураТипа.ИмяОбщегоТипа = "Локальный");
			...
			мПлатформа.СвойстваТипаМетаданные(РодительскаяСтруктураТипа,
				ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, мЯзыкПрограммы, ТекущийИндекс,,, ЛиСерверныйКонтекст, МодульМетаданныхКонтекста(РодительскаяСтруктураТипа), ТаблицаСвойствТипа);
			Если ТаблицаСвойствТипа.Количество() > 0 Тогда
				Для Каждого СтрокаСлова Из ТаблицаСвойствТипа Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущееСлово) Тогда 
				...
				Если РезультатТекста.Количество() > 0 Тогда
					...
					Иначе
						...
						ТаблицаСтруктурТипов = мПлатформа.СвойстваТипаМетаданные(ТаблицаСтруктурТиповКоллекции[0], "[0]",,, мЯзыкПрограммы, "0")[0].ТаблицаСтруктурТипов;
					КонецЕсли;
					Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ТекущийИндекс) Тогда
...
Процедура ОткрытьАвтодополнение(Знач КодКлавиши = "", Знач Модально = Ложь) Экспорт 
	...
	Пока Истина Цикл
		Успешно = ЗаполнитьТаблицуСлов(ТаблицаСтруктурТиповКонтекста, мКонкретныйТипКонтекста, мСтруктураТипаКонтекста, Модально);
...
Функция ЗаполнитьТаблицуСлов(ТаблицаСтруктурТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, РазрешитьОткрытиеОкон = Истина,
	Знач УдалитьФункциюНовый = Ложь, Знач Сортировать = Истина) Экспорт
	...
	Если мЭтоСтроковыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
		...
		Тогда 
			...
			//	Или (Истина
			//		И СтрокаОписания.Слово = "Выполнить" 
			//		И СтрокаОписания.ТипКонтекста = "МенеджерФоновыхЗаданий" 
			//		И ИмяПараметра = "ИмяМетода")
			//	// БлокировкаДанных.Добавить("")
			//	Или (Истина
			//		И СтрокаОписания.Слово = "Добавить" 
			//		И СтрокаОписания.ТипКонтекста = "БлокировкаДанных" 
			//		И ИмяПараметра = "ПространствоБлокировки")
			//	Или (Истина
			//		И СтрокаОписания.Слово = "УстановитьЗначение" 
			//		И СтрокаОписания.ТипКонтекста = "ЭлементБлокировкиДанных" 
		...
		ИначеЕсли Истина
			И ТипЗнч(СтрокаОписанияМетода) = Тип("СтрокаТаблицыЗначений")
			И СтрокаОписанияМетода.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено
			И мПлатформа.ПараметрыМетодаМодуля(СтрокаОписанияМетода).Количество() > мНомерПараметра
		Тогда 
			ИмяПараметра = мПлатформа.ПараметрыМетодаМодуля(СтрокаОписанияМетода)[мНомерПараметра - 1].Имя;
			Если Истина
	...
	Если выхСтруктураТипаКонтекста.ТипЯзыка <> "ИмяТипа" Тогда 
		ПозицияМаркера = Найти(мТекущаяСтрокаНачало, "//");
		Если Истина
			И ПозицияМаркера > 0 
			И Не ирОбщий.ЛиВнутриТекстовогоЛитералаЛкс(Лев(мТекущаяСтрокаНачало, ПозицияМаркера))
		Тогда  
			Если Истина
			...
			Тогда 
				...
				выхСтруктураТипаКонтекста.ТипЯзыка = "ИмяТипа";
			ИначеЕсли Не ЗначениеЗаполнено(мРодительскийКонтекст) Тогда 
				СтруктурТипаИменТипов = мПлатформа.НоваяСтруктураТипа("");
				СтруктурТипаИменТипов.ТипЯзыка = "ИмяТипа";
				СтруктурТипаИменТипов.Метаданные = Метаданные;
				ТаблицаИменТипов = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктурТипаИменТипов, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных,,,, ЛиСерверныйКонтекст);
	...
	Попытка
		ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(выхСтруктураТипаКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы,,,
			ЛиСерверныйКонтекст,, МодульМетаданныхКонтекста(выхСтруктураТипаКонтекста),,, мЭтоЛокальныйКонтекстТаблицыСлов);
	...
	Если РазрешитьОткрытиеОкон Или ВычислятьТипы Или ДобавлятьЛокальныйКонтекст Тогда
		...
		Если ДобавлятьЛокальныйКонтекст Тогда
			ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(Ложь, мЭтоЛокальныйКонтекстТаблицыСлов, Истина, Ложь);
			//Если Истина
...
Функция КопияКомпоненты()
	...
	КопияКопоненты = ирОбщий.СоздатьОбъектПоИмениМетаданныхЛкс("Обработка.ирКлсПолеТекстаПрограммы");
	#Если Сервер И Не Сервер Тогда
		КопияКопоненты = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	...
	КопияКопоненты.мТекстБезКомментариевИОпасныхСтрок = мТекстБезКомментариевИОпасныхСтрок;
...
Процедура ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, ТаблицаСтруктурТиповКонтекста = Неопределено, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	Если Не мЭтоСтроковыйЛитерал И СтрокаРезультата.ТипСлова = "Метод" Тогда
		...
		Если ПараметрЗакрытияПодсказки = Истина Тогда
			ТаблицаМетодовМодуля = мПлатформа.СвойстваТипаМетаданные(мСтруктураТипаКонтекста, СтрокаРезультата.Слово, "Метод",,,,,,, МодульМетаданныхКонтекста(мСтруктураТипаКонтекста));
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(Знач СтруктураТипаКонтекста, Знач ТекущаяСтрока, Знач ВернутьСтруктуруТипа = Ложь, Знач НужноУточнитьТип = Истина) Экспорт 
	...
	Если НужноУточнитьТип Или ВернутьСтруктуруТипа Тогда
		...
		Иначе                                   
			ОписанияСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаКонтекста, ЯзыкПрограммы, Конфигурация,, Истина,, ТекущаяСтрока.ТипСлова, ЛиСерверныйКонтекст, ТекущаяСтрока.Слово,
...
Процедура НайтиВызовыМетода() Экспорт 
	...
	КонецЕсли;
	ТаблицаСтруктурТиповКонтекста = ВычислитьТипЗначенияВыражения(мКонтекст, " " + мТекстДляПоискаОпределения, мПредшествующийТекст, Истина);
	СтруктураТипаКонтекста = ТаблицаСтруктурТиповКонтекста[0];
	ПолноеИмяМетода = "";
	...
	ПоискВызововМетода.ПараметрИскатьНепрямые = ПараметрИскатьНепрямые;
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, Знач ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	Если ЗначениеЗаполнено(ТекстДляАнализа) Или ОбновитьТекстДляАнализа Тогда
		мТекстБезКомментариевИОпасныхСтрок = ЗалитьКомментарииИСтроковыеЛитералы(ТекстДляАнализа,, Истина, Ложь);
	КонецЕсли; 
	...
	Если ВключатьГлобальныйКонтекст Тогда
		...
		ТаблицаГлобальногоКонтекста = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипа,, Конфигурация,, Ложь);
	...
	КонецЕсли;
	ВременныйТекст = мРегВыражение.Заменить(мТекстБезКомментариевИОпасныхСтрок, "");
	...
	Для Каждого Match Из Результат Цикл
		...
		ИначеЕсли Подгруппа3 <> Неопределено Тогда 
			...
			Тогда 
				...
				КонтекстВыполненияПроверки = ?(ЛиСерверныйКонтекст, ирСервер, ирОбщий);
...
//   - 
//
Функция УстановитьТекст(Знач Текст, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач ИмяМодуля = "") Экспорт 
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создать();
	#КонецЕсли
...
// Процедура - Установить имя модуля
//
// Параметры:
//  ИмяМодуля	 - 	 - 
// 
// Возвращаемое значение:
//   Структура - СтруктураТипа
//
Процедура УстановитьИмяМодуля(Знач ИмяМодуля) Экспорт
	...
	Если Фрагменты.Количество() = 1 Тогда
		ТипМодуля = ИмяМодуля;
	ИначеЕсли ирОбщий.СтрокиРавныЛкс(Фрагменты[0], "ОбщийМодуль") Тогда
		ИмяОбщегоТипа = "ОбщийМодуль";
		ОбъектМД = Метаданные.ОбщиеМодули[Фрагменты[1]];
	Иначе
		...
		Иначе
			ИмяОбщегоТипа = "Форма";
			Фрагменты.Вставить(Фрагменты.ВГраница(), "Форма");
		КонецЕсли;
		ОбъектМД = Метаданные.НайтиПоПолномуИмени(ирОбщий.СтрСоединитьЛкс(Фрагменты, "."));
	КонецЕсли; 
	...
	КонецЕсли;      
	мМодульМетаданных = мПлатформа.МодульМетаданных(ПолеТекста.ПолучитьТекст(), СтруктураТипа);
...
мРегВыражение = ирОбщий.НовыйВычислительРегВыражений();
