					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Иначе
								...
								МассивСтруктурПрисвоенныхТипов = мПлатформа.СловаКонтекстаМетаданные(ТаблицаСтруктурТиповКоллекции[0], "[0]",,, мЯзыкПрограммы, "0",,, мФлагиКомпиляции)[0].ТаблицаСтруктурТипов;
								мПлатформа.ДобавитьВТаблицуСтруктурТипов(ТаблицаСтруктурТипов, МассивСтруктурПрисвоенныхТипов);
							КонецЕсли;
							Если Истина
					...
					Тогда
						...
						Если РезультатСвойств.Количество() > 0 Тогда
							...
							Если Лев(ПрисвоенноеВыражение, 1) = """" Тогда
								ТекстовоеЗначение = ирОбщий.ТекстИзВыраженияВстроенногоЯзыкаЛкс(ПрисвоенноеВыражение);
							Иначе
		...
		Если ОбъектМД <> Неопределено Тогда
			Если ИмяОбщегоТипа = "Форма" Тогда
				...
				Иначе
					ОбъектМД = Форма;
				КонецЕсли;
				Если ТипЗнч(ОбъектМД) <> ТипЗнч(Метаданные) Тогда
