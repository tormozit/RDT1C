	...
	Иначе
		...
		Если ЗапросыПакета.Количество() > 0 Тогда
			Для ИндексЗапроса = 0 По ЗапросыПакета.ВГраница() Цикл
				...
				ЧислоСтрокВЗапросе = СтрЧислоСтрок(ЗапросПакета + "й") - 1;
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	Тогда
		Шаблон = "(?:(?:[^." + шБуква + "]|^)\s*(" + шИмя + ")(?=\s*\())"
				 + "|(?:[^." + шБуква + "]|^)\s*(" + шИмя + ")(?=(?:\.(?:" + шИмя + ")?)+|[^\(?:\." + шБуква + "\d]|$)"
				 + "|(?:(" + шПараметрЗапроса + "))";
		КоличествоПодгрупп = 3;
	Иначе
		Шаблон = "(?:[^." + шБуква + "]|^)\s*(?:" + ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=\s*\())"
			  + "|(?:[^." + шБуква + "]|^)\s*(?:" + ирОбщий.ПреобразоватьТекстДляРегулярныхВыраженийЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=[^\(?:" + шБуква + "\d]|$))";
