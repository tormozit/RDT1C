	...
	Если БылаОткрыта Тогда 
		...
		Тогда
			...
			Если Истина
				...
				И ФормаАвтодополнение.ЭлементыФормы.ТаблицаСлов.ТекущаяСтрока.Слово = мКонтекст
				И (Ложь
					Или мКонтекст = "Тогда"
					Или мКонтекст = "Иначе"
					Или мКонтекст = "Цикл"
...
Процедура ФорматироватьТекстЯзыкаЗапросов()
...
Функция ФорматироватьТекстВстроенногоЯзыка(Знач МожноВесьТекст = Истина) Экспорт 
	...
	Если ПустаяСтрока(ВыделенныйТекст) Тогда
		...
		ВыделитьВесьТекст();
		НачальнаяСтрока = 1;
		КонечнаяСтрока = ПолеТекста.КоличествоСтрок();
	КонецЕсли; 
	УстановитьГраницыВыделения(НачальнаяСтрока, 1, КонечнаяСтрока + 1, 1); // Выделяем полные строки
	ВыделенныйТекст = ВыделенныйТекст();
	...
	Для СчетчикСтроки = 1 По ТекстДокумент.КоличествоСтрок() Цикл
		...
		СтрокаТекстаДоКомментария = ЗалитьКомментарииИСтроковыеЛитералы(СтрокаТекста,, Истина);
		...
		Если СчетчикСтроки > 1 Тогда
			...
			КонецЕсли;
			ТекстДокумент.ЗаменитьСтроку(СчетчикСтроки, ирОбщий.СтрокаПовторомЛкс(Символы.Таб, БазовыйУровень + УровеньИнструкции + СмещениеУровня) + СокрЛ(СтрокаТекста));
		Иначе
		...
		КонецЕсли;
		Если Не СловоНайдено Тогда 
			СтрокаБезЛитералов = СокрЛП(ЗалитьКомментарииИСтроковыеЛитералы(СтрокаТекстаДоКомментария,, Истина));
	...
	ПолеТекста.ВставитьТекст(НовыйТекст); 
	Возврат Истина;
		...
		Иначе 
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ФрагментыСлова[1]) Тогда
					ЧислоКонцовУсловия = ирОбщий.НайтиРегВыражениеЛкс(СтрокаТекстаДоКомментария, шПредИмя + ФрагментыСлова[1] + "(?=//|\s|$)",,,,,,,,, ВхожденияКэш).Количество();
				Иначе 
...
Функция ВыделитьМетод(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	Для Каждого СтрокаПараметра Из ФормаНовогоМетода.ТаблицаПараметров Цикл
		...
		Если Истина
			...
			И ирОбщий.НайтиРегВыражениеЛкс(ТекстПосле, шПредИмя + СтрокаПараметра.Имя + "[^" + шБуква + "\d]",, Ложь).Количество() > 0 
		Тогда
			СтрокаПараметра.Выход = Истина;
		КонецЕсли;
	...
	Тогда
		...
		Если ирОбщий.ЛиТекстЯзыкаЗапросовЛкс(ТекстЛитерала) Тогда
			...
			Если Вхождения.Количество() > 0 Тогда
				...
				ТаблицаТиповЗапроса = КопияАнализатора.ВычислитьТипЗначенияВыражения(ПеременнаяЗапроса,,,,,,,,,,, КопияАнализатора.мПозицияВБлоке + СтрДлина(ВыражениеЛитерала) + 2); 
				Если Истина
...
Функция ЗагрузитьМетодМодуля(Знач НовыйМетодМодуля) Экспорт 
	...
	Если НовыйМетодМодуля = мМетодМодуля И НовыйМетодМодуля <> Неопределено Тогда
		мТекстБлока = мМетодМодуля.Тело; // На случай, если вызвано после подмены возвратов в теле
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли мЯзыкПрограммы = 0 Тогда 
		...
		Тогда
			...
			Если ирОбщий.ЛиИмяПеременнойЛкс(мКонтекст) Тогда
				...
				Если МетодМодуля <> Неопределено И МетодМодуля.ПозицияТела <> Неопределено Тогда
...
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач СсылкаСтрокиМодуля, Знач ТолькоЛокально = Истина) Экспорт
	...
	Если СтруктураСсылки.Модуль = мМодульМетаданных.Имя Тогда
		...
		Иначе
			...
			Если ПозицияБлока = Неопределено Тогда
				Возврат Неопределено;
			КонецЕсли;
			ПерейтиКОпределениюЛокальногоМетода(СтрокаМетода, СтруктураСсылки.ПараметрМетода, СтруктураСсылки.СмещениеСтрокиМетода);
		КонецЕсли;
		Возврат Истина;
	ИначеЕсли Не ТолькоЛокально Тогда 
		Если ЗначениеЗаполнено(СтруктураСсылки.Метод) Тогда
			...
			ирКлиент.ОткрытьМетодМодуляПоОпределениюЛкс(СтрокаМетода, СтруктураСсылки.ПараметрМетода,, СтруктураСсылки.СмещениеСтрокиМетода);
		КонецЕсли;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Функция ДобавитьПереходыИзСтруктурыТипаВСписокВыбора(Знач СтруктураТипа, Знач СписокВыбора = Неопределено) 
	...
	Если СтруктураТипа.СтрокаОписания <> Неопределено Тогда 
		...
		Если ТаблицаВладелец.Колонки.Найти("ЛиЭкспорт") <> Неопределено Тогда
			...
			Если СтрокаМетода.Имя = "<>" И Не ПустаяСтрока(мРодительскийКонтекст) Тогда
		...
		Тогда    
			...
			Если ЗначениеЗаполнено(ИмяТипа) Тогда
				ОбъектМД = СтруктураТипа.Метаданные[ПрефиксИмениКоллекции + ирОбщий.МножественноеИмяМДЛкс(ИмяТипа)].Найти(мКонтекст);
...
Процедура ДобавитьТипДанныхВСписокПереходов(Знач СписокВыбора, Знач КлючПерехода) Экспорт
	Если СписокВыбора.НайтиПоЗначению(КлючПерехода) = Неопределено Тогда
		СписокВыбора.Добавить(КлючПерехода, "я Тип " + КлючПерехода,, ирКэш.КартинкаПоИмениЛкс("ирМетаданные"));
	КонецЕсли;
КонецПроцедуры
...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	...
	Для Каждого РодительскаяСтруктураТипа Из РодительскиеСтрутурыТипа Цикл
		...
		КонецЕсли;
		ТаблицаСвойствТипа.Очистить();
		мПлатформа.СловаКонтекстаМетаданные(РодительскаяСтруктураТипа, ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, мЯзыкПрограммы, ТекущийИндекс,,, мФлагиКомпиляции,
			МодульМетаданныхКонтекста(РодительскаяСтруктураТипа), ТаблицаСвойствТипа,, ЭтоЛокальныйКонтекст, ЛиВместеСЛокальнымКонтекстом);
		Если ТаблицаСвойствТипа.Количество() > 0 Тогда
			Для Каждого СтрокаСлова Из ТаблицаСвойствТипа Цикл
				Если Истина
...
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ДляСвойства = "")
	...
	Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
		...
		Если ИндексПараметра > СтрЧислоВхождений(ПараметрыВызова, ",") Тогда
			ЛиПодходящее = Ложь;
		КонецЕсли;
	...
	Если мРекурсивныйПуть.ВычислениеРезультатаМетода = мМетодМодуля.Имя Тогда
		ЛиКлючевойПараметр = мРекурсивныйПуть.ДобавлятьКлючевыеПараметры; // Для Вычислить()
...
Функция РазобратьФактическиеПараметрыВызова(ТекущиеАргументы) Экспорт
	...
	КонецЕсли;
	Результат = мРегВыражение.НайтиВхождения(Сред(ТекущиеАргументы, 2, СтрДлина(ТекущиеАргументы) - 2) + ",");
	МассивПараметров = Новый Массив;
	Для Каждого Вхождение Из Результат Цикл
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0, Знач ДопКлючКэша = "", Знач ДляСвойства = "", СобиратьОшибки = Ложь) Экспорт
		...
		Если ВычислениеРазрешено Тогда
			...
			Если мРекурсивныйПуть.ДопКлючКэша <> Неопределено Тогда 
				...
				КлючВыраженияСПозицией = мРекурсивныйПуть.ДопКлючКэша + ";" 
					//+ XMLСтрока(Позиция0ВМетодеОт) + ";" 
					+ XMLСтрока(Позиция0ВМетодеДо) + ";" + Лев(Выражение, 200);
			...
			Тогда
				...
				Иначе
					Если ДляСвойстваВверх = Неопределено Тогда
						ДляСвойстваВверх = ТекущееСлово;
					КонецЕсли;
					ТаблицаТиповРодителя = ВычислитьТипЗначенияВыражения(РодительскийКонтекст, ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь,,,,,
						Позиция0ВМетодеДо,, ДляСвойстваВверх, СобиратьОшибки);
				...
				Иначе
					...
					Иначе 
						...
						Тогда
							...
							ИначеЕсли ирОбщий.ЛиИмяТипаФормыЛкс(мМодульМетаданных.СтруктураТипа.ИмяОбщегоТипа) Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
										...
										Если ПрименитьТип Тогда 
											...
											Если ТипЗнч(Тип) = Тип("Тип") Тогда 
												...
												Если Тип = Тип("ОбъектМетаданных") Тогда
													СтруктураЕдинственногоТипа.ИмяОбщегоТипа = "ОбъектМетаданных: Справочник"; // Наиболее типичный вариант. Это лучше чем абстрактный тип без свойств.
												КонецЕсли;
												ДобавитьЕдинственныйТип(ТаблицаТипов, СтруктураЕдинственногоТипа);
											КонецЕсли;
											...
											Тогда                                                
												...
												Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПервыйФактПараметр) Тогда
													...
													Если ЗначениеЗаполнено(ИменаСвойств) Тогда
														...
														ПараметрыКонструктора = СокрП(ирОбщий.СтрокаБезКонцаЛкс(ПараметрыКонструктора, 1));
							...
							Если Ложь
								...
								Или Истина
									...
									И мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТипов, 4) // если делать меньше 3, то будет давать артефакты частичное обновление модуля, т.к. там не очищаем колонку ТипЗначения
							Тогда 
								Прервать;
							КонецЕсли;
						КонецЦикла;
			...
			Тогда     
				Если ЛиПеременнаяРезультата Тогда 
					ЛиВиртПрисвоение = Истина;
				КонецЕсли;
				ВычислитьСвойстваОбъекта(Выражение, ТаблицаТипов, ТекстДляПоискаОпределения, ЭтоВызовКонструктора, ПолныйАнализСоставаТипов, ГлобальныйПоискВМодуле, Позиция0ВМетодеОт, Позиция0ВМетодеДо, ДляСвойства, ЛиВиртПрисвоение);
			КонецЕсли; 
			Если Истина 
				И мЯзыкПрограммы = 1
...
~Возврат:
	Если СобиратьОшибки Тогда 
		...
		Если Истина
			...
			И мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповРодителя, 2, ЛучшийТипРодителя)
		...
		Тогда 
			...
			ЛиДетальностьТиповДостаточна = мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТипов, 1, ЛучшийТип);
			Если Ложь
				Или Не ЛиДетальностьТиповДостаточна
				Или (Истина
					И Найти(ЛучшийТип.ИмяОбщегоТипа, "<") > 0
					И ТипЗнч(ЛучшийТип.Метаданные) = Тип("КоллекцияОбъектовМетаданных"))
				//Или (Истина
			...
			Тогда 
				...
				Если мМетодМодуля <> Неопределено Тогда
					...
					ПозицияТела = мМетодМодуля.ПозицияТела;
				Иначе 
					ОшибкаМодуля.Метод = мПлатформа.ИмяМетодаИнициация();
					ПозицияТела = мМодульМетаданных.ПозицияПрограммы;
				КонецЕсли;
				ОшибкаМодуля.ВыражениеРодитель = РодительскийКонтекст;
				ОшибкаМодуля.Слово = ТекущееСлово;
				ОшибкаМодуля.ПозицияВМетоде = Позиция0ВМетодеДо + 1;
				ОшибкаМодуля.Позиция = ОшибкаМодуля.ПозицияВМетоде + ПозицияТела + СтрДлина(РодительскийКонтекст + ".");
				...
				КонецЕсли;
				ОшибкаМодуля.НомерСтрокиМетода = НомерСтрокиИзПозиции(ОшибкаМодуля.ПозицияВМетоде);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВычислитьСвойстваОбъекта(Знач Выражение, Знач ТаблицаТипов, ТекстДляПоискаОпределения = "", Знач ЭтоВызовКонструктора = Ложь, Знач ПолныйАнализСоставаТипов = Истина, Знач ГлобальныйПоискВМодуле = Ложь,
	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				Если мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповЭлемента, 4, ЛучшийТип) Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		Тогда
			...
			КонецЕсли;
			ШаблонПоиска = шПредИнструкция + Выражение + "\s*\.\s*Колонки\s*.\s*(?:Добавить\s*\(\s*|Вставить\s*\((?:" + шИмя + "|\d+),\s*)"
		...
		Тогда
			...
			Если СтруктураТипаЦикл.ИмяОбщегоТипа = "Запрос" Тогда
				...
				ТаблицаТиповМенеджера = ВычислитьТипЗначенияВыражения(Выражение + ".МенеджерВременныхТаблиц",,,,,, ЛиНовыйКэшВыражений, ГлобальныйПоискВМодуле,, Истина, Позиция0ВМетодеОт,
				...
				Если мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповМенеджера, 2, ЛучшийТип) Тогда 
			...
			Пока Истина Цикл            
				...
				Если ЗначениеЗаполнено(ТекстПакета) Тогда
					...
					Если Не ТекстВЭтомБлокеУстановлен Тогда
						...
						ирОбщий.СкопироватьКоллекциюЛкс(ВычислитьСвойстваСтруктуры(Выражение + ".Параметры",,,, ПозицияПоискаТекстаОт, ПозицияПоискаТекстаДо), ПостроительЗапроса.Параметры);
		...
		ИначеЕсли СтруктураТипаЦикл.ИмяОбщегоТипа = "МенеджерВременныхТаблиц" Тогда
			ШаблонПоиска = шПредИнструкция + Выражение + "\s*\.\s*докДобавить\(\s*(" + шПростоеВыражениеПрограммы + ")\s*\)";
...
Процедура ИнициироватьРекурсивныйПуть(Знач Выражение = "", Знач ДопКлючКэша = "") Экспорт
	...
	мРекурсивныйПуть.Вставить("Позиции", Новый Массив); // для языка запросов защита от циклической рекурсии
...
Функция ЛиВхождениеВЗапрешенномДиапазоне(Знач ПоследнееВхождение, Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0, Знач ЛиКорневойВызов = Ложь)
	...
	Тогда 
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Если ТаблицаТиповЗначения.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
			ТаблицаСловИсточника = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповЗначения[0],,,,,, "Свойство");
		...
		Если Вхождения.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из СвойстваЧерезТочкуИзТекста Цикл
				...
				Тогда
					...
					ВычислитьСвойстваОбъекта(ВыражениеЗначения, ТаблицаТиповСоздания,,,, ГлобальныйПоискВМодуле, Позиция0ВМетодеОт, Позиция0ВМетодеДо);
					Продолжить;
				КонецЕсли;
...
Функция ТаблицаТиповИзТекста(Знач ТекстТипа, Знач Описание = "", Знач ТекстДоВхождения = "") Экспорт
	...
	Для Каждого УказательТипа Из ФрагментыЗапятой Цикл 
			...
			Иначе 
				...
				Иначе 
					...
					Если Тип <> Неопределено Тогда
						...
						КонецЕсли;
					Иначе
						ОбщиеТипы = мПлатформа.ТаблицаОбщихТипов.НайтиСтроки(Новый Структура("БазовыйТип", УказательТипа));
		...
		Иначе
			СтруктураТипа = Неопределено;
			Если Найти(УказательТипа, "(") = 0 И Найти(УказательТипа, ".") > 0 Тогда
				...
				ИначеЕсли ИмяМножественное <> Неопределено Тогда
					СтруктураТипа = ВычислитьСвойствоФормыПоПолномуИмени(УказательТипа);
					Если СтруктураТипа = Неопределено Тогда 
			...
			КонецЕсли;
			Если СтруктураТипа = Неопределено Тогда 
				ТаблицаТипов = ВычислитьТипЗначенияВыражения(УказательТипа, ТекстДоВхождения, ТекстДоВхождения,,,, Ложь);
				Если ТаблицаТипов.Количество() = 0 И Найти(УказательТипа, "(") = 0 Тогда
					ТаблицаТипов = ТаблицаТиповИзПолногоИмениМетодаИлиПараметра(УказательТипа);
				КонецЕсли;
			Иначе
				мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипа);
			КонецЕсли;
				...
				Если ПараметрыМетода <> Неопределено Тогда
					...
					Если СтрокаПараметра <> Неопределено Тогда
						...
						Если СтрокаПараметра.ТаблицаТипов = Неопределено Тогда
							ТаблицаТипов = ТаблицаТиповИзТекста(СтрокаПараметра.ТипЗначения, СтрокаПараметра.Описание); // TODO защита от бесконечной рекурсии
						Иначе 
							ТаблицаТипов = СтрокаПараметра.ТаблицаТипов;
						КонецЕсли;
						ТаблицаТипов = ТаблицаТипов.Скопировать(); // Чтобы не было циклической ссылки
						Если ТаблицаТипов.Количество() = 0 Тогда
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, _2 = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Иначе
		...
		Если мАгрегатныеТипыКонтекста.Количество() > 1 И РазрешитьОткрытиеОкон Тогда
	...
	Если Ложь
		...
		Или (Истина
			И ирОбщий.ЛиВнутриКомментарияЛкс(мТекущаяСтрокаНачало) 
	...
	Тогда
		...
		Если ЗначениеЗаполнено(мРодительскийКонтекст) Тогда
			...
			Если ЛиИмяОбщегоМодуля Тогда
				...
				ВнутренняяТаблицаСлов = мПлатформа.СловаКонтекстаМетаданные(мСтруктураТипаКонтекста,, "Метод",,,,,, мПлатформа.НовыеФлагиКомпиляции(, Ложь, Ложь));
	...
	Иначе 
		ДобавлятьЛокальныйКонтекст = Истина
			...
			;
		Попытка
			ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(мСтруктураТипаКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы,,,
				?(мЭтоТекстовыйЛитерал И мСтруктураТипаКонтекста.ТипЯзыка <> "ИмяТипа", Неопределено, мФлагиКомпиляции),, МодульМетаданныхКонтекста(мСтруктураТипаКонтекста),,, ДобавлятьЛокальныйКонтекст, мНаборыСлов);
		...
		ДобавлятьСтатистическиеСлова = Ложь
			...
			Или мСтруктураТипаКонтекста.ИмяОбщегоТипа = "Неизвестный контекст"
	...
	Тогда
		...
		Если СтрокаОписанияМетода.ТипСлова <> "Свойство" Тогда
			...
			Тогда 
				...
				ЛучшийТип = Неопределено;
				Если мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповОбрамляющегоВызова, 4, ЛучшийТип) Тогда
		...
		Если ЗначениеЗаполнено(ИмяПараметра) Тогда 
			...
			Тогда
				...
				Если СтруктураТипаПараметра.Количество() > 0 Тогда
					...
					Если СтруктураТипаЭлемента.Количество() > 0 Тогда
						КоллекцияЭлементовМД = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаЭлемента[0]).Скопировать(Новый Структура("ТипСлова", "Свойство")).ВыгрузитьКолонку("Слово");
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли Ложь
			...
			Иначе
				...
				Если ВыбраннаяСтруктураТипа.СтрокаОписания.ИмяМодуля <> мМодульМетаданных.Имя Тогда
					АнализаторКодаМодуля = ирКэш.ПолеТекстаМодуляЛкс(ВыбраннаяСтруктураТипа.СтрокаОписания.ИмяМодуля).КопияКомпоненты();
				Иначе 
				...
				Для Каждого СтрокаПараметра Из мПлатформа.ПараметрыМетодаМодуля(СтрокаОписанияМетода) Цикл
					...
					Если ЗначениеЗаполнено(ВыражениеПараметра) Тогда
						СтрокаОписанияМетода.КлючевыеПараметры.Вставить(СтрокаПараметра.Имя, ВычислитьТипЗначенияВыражения(ВыражениеПараметра,,,,,, Истина,,,,, мПозицияВБлоке, Неопределено));
					КонецЕсли;
					ИндексПараметра = ИндексПараметра + 1;
				КонецЦикла;
				ИмяПараметра = ИмяПараметраВызоваМетода(СтрокаОписанияМетода);
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен) Экспорт
	...
	Для Каждого Вхождение Из ВхожденияОбъектов Цикл
		...
		Если ТаблицаТипов.Количество() > 0 Тогда
			ТаблицаСловОбъекта = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТипов[0],,,,,, "Свойство"); 
...
Функция ЛиПодробнаяСтруктураТипаУпрФормы(РодительскаяСтруктураТипа) Экспорт
	Результат = Истина
		...
		И (Ложь
			Или ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = ирОбщий.ТипУправляемаяФормаЛкс()
			Или ТипЗнч(РодительскаяСтруктураТипа.Метаданные) = Тип("Структура"));
	Возврат Результат;
КонецФункции
...
Функция УстановитьТекст(Знач Текст = Неопределено, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	мПлатформа.ПроверитьОбновитьКэш();
	Если Истина
		И Не ЗначениеЗаполнено(СжатоеИмяМодуля) 
		И (Ложь
...
//
Процедура ПроверитьВыраженияПослеТочки(ТабличноеПоле = Неопределено) Экспорт
	...
	ИнициироватьРекурсивныйПуть(); 
	Если мМетодМодуля <> Неопределено Тогда
		ПараметрМодуль = мМетодМодуля.Имя;
	КонецЕсли;
	Счетчик = 0;  
	СтруктураОбновленияТабличногПоля = ирКлиент.СтрукутраПервогоОбновленияТабличногоПоляЛкс();
	Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(Вхождения.Количество(), "Глубокая проверка");
	Для Каждого Вхождение Из Вхождения Цикл
		Счетчик = Счетчик + 1;
		Выражение = Вхождение.SubMatches(1);
		Если Выражение = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ирОбщий.ОбработатьИндикаторЛкс(Индикатор, Счетчик);
		ПозицияВхождения0 = Вхождение.FirstIndex + СтрДлина(Вхождение.SubMatches(0));
		НачальнаяПозицияБлока = ЗагрузитьМетодМодуляПоПозицииИРазобратьКонтекст(Истина, ПозицияВхождения0);
		ПозицияВхождения0 = ПозицияВхождения0 - НачальнаяПозицияБлока;
	...
	Если ОшибкиМодуля.Количество() > 0 Тогда
		ОткрытьПроверкуМодуля(ПараметрМодуль);
	КонецЕсли;
КонецПроцедуры
...
//    ПозицияВхождения0 - Число - 
//    ТаблицаРезультата - ТаблицаЗначений - 
Процедура ДобавитьВозможнуюОшибкуПослеТочки(Знач ВложенныеВыраженияШаблон, Знач НайденноеВыражение, Знач ПозицияВхождения0) Экспорт
	...
	ВычислитьТипЗначенияВыражения(НайденноеВыражение,,,,,, Ложь,,,,, ПозицияВхождения0,,, Истина);
	Для Каждого ВложенноеВыражение Из мРекурсивныйПуть.ВложенныеВыражения Цикл
		...
		Для Каждого ВложенноеВхождение Из ВложенныеВхождения Цикл
			Выражение = ВложенноеВхождение.SubMatches(1);
			Если Выражение = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ПозицияВложеннногоВхождения0 = СмещениеПозиции - 1 + ВложенноеВхождение.FirstIndex + СтрДлина(ВложенноеВхождение.SubMatches(0));
...
шВыражениеСТочкой = шСтрокаПрограммы + "|" + шКомментарий + "|(" + шПредИмя + ")(" + шИмяСкобкиЛенивые + "?" + шИндекс + "?(?:\." + шИмяСкобкиЛенивые + "?" + шИндекс + "?)+)";
...
шВызовМетодаПрограммы = "(?:" + шПредИмя + "(Новый|New)\s+)?(" + шОбъектноеВыражение + "?)"
