...
Перем мЗапросыПакета;
Перем мСтруктураПакетаЗапросов;
...
Процедура ИнициироватьНеинтерактивно(пЯзыкПрограммы = 0, пМетодВыполнения = "", пКонтекстВыполнения = Неопределено, пТипТекста = "Алгоритм", пКонфигурация = Неопределено,
	...
	ДобавитьПравилоВычисленияФункции("ПолучитьФормуЛкс", "ВычислитьПолучитьФорму", "ОбщийМодуль.ирКлиент");
	...
	#Если Сервер И Не Сервер Тогда
		...
		ирОбщий.ВычислитьНайтиЭлементКоллекцииЛкс();
		ирОбщий.ВычислитьОтобратьКоллекциюЛкс();
	#КонецЕсли
	ДобавитьПравилоВычисленияФункции("Добавить", "ВычислитьКоллекцияДобавить", "*");
	//ДобавитьПравилоВычисленияФункции("НайтиЭлементКоллекцииЛкс", "ВычислитьНайтиЭлементКоллекцииЛкс", "ОбщийМодуль.ирОбщий"); // Теперь автоматически вычисляется
		...
		Если Ложь
			Или Истина
				...
				И (Ложь
					...
					Или ирОбщий.СтрНачинаетсяСЛкс(Данные, "1") // Зажат шифт   
					)
...
Процедура РазобратьТекущийКонтекст(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Неопределено, Знач НомерСтроки = 0, Знач НомерКолонки = 0,
	...
	мЗапросыПакета = Неопределено;
	мСтруктураПакетаЗапросов = Неопределено;
	...
	Если Истина
		...
		И (Ложь
			Или ирОбщий.ЛиВнутриНепервойСтрокиТекстовогоЛитералаЛкс(мТекущаяСтрокаНачало) // Первые строки литералов игнорируем для ускорения
	...
	Иначе
		...
		мЗапросыПакета = ирОбщий.РазбитьГрубоТекстПакетногоЗапросаНаЗапросыЛкс(ТекстПакетаЗапросов, мПозицияВТексте, ПозицииЗапросовПакета);
		#Если Сервер И Не Сервер Тогда
			мЗапросыПакета = Новый Массив;
		#КонецЕсли
		Для ИндексЗапроса = 0 По мЗапросыПакета.ВГраница() Цикл
			мТекстБлока = мЗапросыПакета[ИндексЗапроса];
...
//
Функция ЗагрузитьМетодМодуляПоПозиции(Знач ПозицияВТексте) Экспорт 
	...
	Иначе
		...
		Для Каждого СтрокаМетода Из мМодульМетаданных.Методы Цикл  			Если Истина  				И СтрокаМетода.ПозицияСОписанием <> Неопределено  				И СтрокаМетода.ПозицияСОписанием <= ПозицияВТексте  				И СтрокаМетода.ПозицияСОписанием + СтрокаМетода.ДлинаСОписанием >= ПозицияВТексте  			Тогда  				НовыйМетодМодуля = СтрокаМетода;  				Прервать;  			КонецЕсли;  		КонецЦикла;  
	КонецЕсли;
	Если НовыйМетодМодуля = мМетодМодуля И НовыйМетодМодуля <> Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	ЗагрузитьМетодМодуля(НовыйМетодМодуля);
	Возврат Истина;

КонецФункции

Процедура ЗагрузитьМетодМодуля(Знач НовыйМетодМодуля) Экспорт 
...
Функция ПоляДоступнойТаблицы(Знач СтрокаДоступнойТаблицы, Знач ПараметрыТаблицы = Неопределено, Знач ДиалектSQL = "", Знач ПородившийЗапрос = Неопределено) Экспорт 
	...
	ИначеЕсли СтрокаДоступнойТаблицы.Тип = "Параметр" Тогда
		...
		Если Истина
			И КонтекстВыполнения <> Неопределено
...
// Параметры:
//    МетодМодуля - ? - 
Процедура ПерейтиКОпределениюЛокальногоМетода(МетодМодуля, Знач ИмяПараметра = "") Экспорт
	РазобратьТекущийКонтекст(,,,,, Истина);
	Если Не ЗначениеЗаполнено(ИмяПараметра) Тогда
		мКонтекст = МетодМодуля.Имя + "(";
		ПерейтиКОпределению();
	Иначе
		Если ЛиВЗаголовкеКонкретногоМетода(МетодМодуля) Тогда
...
// Возвращаемое значение:
//    ? - 
Функция ЛиВЗаголовкеКонкретногоМетода(МетодМодуля = Неопределено) Экспорт
	Если МетодМодуля = Неопределено Тогда
		МетодМодуля = мМетодМодуля;
	КонецЕсли;
	Результат = Истина
...
Функция ОткрытьОпределениеСтруктурыТипа(Знач СтруктураТипа, Знач ЭтоОбщийТип = Ложь, РазрешитьДиалоги = Истина, Знач ИмяПараметраМетода = "")  Экспорт 
	...
	Если СтруктураТипа.СтрокаОписания <> Неопределено Тогда 
		...
		Если ТаблицаВладелец.Колонки.Найти("ЛиЭкспорт") <> Неопределено Тогда
			...
			КонецЕсли;
			Возврат ирКлиент.ОткрытьМетодМодуляПоОпределениюЛкс(СтрокаМетода, РазрешитьДиалоги, ИмяПараметраМетода, ЭтотОбъект);
		ИначеЕсли Истина
	...
	Тогда
		...
		Если РазрешитьДиалоги Тогда
			ирКлиент.ПассивнаяФормаПоИмениЛкс(ИмяФормы).Открыть();
			Возврат Истина;
		КонецЕсли;
		Возврат ИмяФормы;
	КонецЕсли;
...
Функция НайтиОпределениеСлова(Слово, ПоследнееОпределение = Неопределено, ПоследнееВхождение = Неопределено) Экспорт 
	...
	Если мЯзыкПрограммы = 1 Тогда
		мРегВыражение.Global = Ложь;
		мРегВыражение.Pattern = СтрЗаменить(шПоискОписанияТаблицы, "#Идентификатор#", ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(Слово));
		...
		Если РезультатТекста.Количество() > 0 Тогда
			...
			Тогда
				...
				мРегВыражение.Pattern = СтрЗаменить(ШаблонСозданияТаблицы, "#Идентификатор#", ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(Слово));
	...
	Иначе
		...
		Иначе
			//шПрисвоение = "(" + шРазделитель + "*=" + шРазделитель + "*((Новый|New)" + шРазделитель + "+(" + шИмя + ")|(" + шИмяСкобки + "?" + шИндекс 
...
Функция ВычислитьТипДочернегоЭлемента(Знач РодительскиеСтрутурыТипа, Знач ТекущееСлово, Знач ТипСлова, Знач ТекущийИндекс = Неопределено, Знач ТекущиеАргументы = "", Знач ПредшествующийТекст = "",
	Знач ПолныйАнализСоставаТипов = Истина, выхЛиПеременнаяМодуля = Ложь, Знач ПозицияВМетодеДляКэша = 0) Экспорт 
	...
	Если Истина
		...
		И РодительскиеСтрутурыТипа[0].ИмяОбщегоТипа = "Массив[РезультатЗапроса]" 
	Тогда
		ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(, Ложь,,,, ТекущееСлово,, Истина);
		ПеременныеВыражения = Новый Структура;
		Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
			ТаблицаСтруктурТипа = ВычислитьТипЗначенияВыражения(СтрокаСлова.Слово, ПредшествующийТекст, ПредшествующийТекст,,,, Ложь,,, Истина,, ПозицияВМетодеДляКэша);
	...
	Для Каждого РодительскаяСтруктураТипа Из РодительскиеСтрутурыТипа Цикл
		...
		Если РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
			...
			Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
				...
				Если ТипСлова = "Метод" Тогда
					...
					Если ПравилоВычисленияФункции <> Неопределено Тогда 
						...
						КонецЕсли;
						//Попытка
							ТаблицаТиповВычисленная = Вычислить(ПравилоВычисленияФункции.Правило + "(РодительскаяСтруктураТипа, ПараметрыИсходящегоВызова, ТекущееСлово, ПредшествующийТекст)");
				...
				Если Истина
					...
					//И (Ложь // Отключено для доступа к ручной таблице локального контекста
					...
					Если ПараметрыМетода <> Неопределено И ТипСлова = "Свойство" Тогда
						...
						Если СтрокаПараметраМетода <> Неопределено Тогда
							...
							Если СтрокаПараметраМетода.ТаблицаТипов = Неопределено Тогда
								СтрокаПараметраМетода.ТаблицаТипов = ВычислитьТипПараметраМетода(ПараметрыМетода, ПройденныеВызовы, СтрокаПараметраМетода, ТаблицаТипов);
							КонецЕсли; 
				...
				Если мЯзыкПрограммы = 1 Тогда
					...
					Если ЭтоЛокальныйКонтекст Тогда 
						Если мСтруктураПакетаЗапросов = Неопределено Тогда
							мСтруктураПакетаЗапросов = мПлатформа.СоздаваемыеВременныеТаблицыПакетаЗапросов(мЗапросыПакета);
						КонецЕсли;
						ИндексЗапроса = мСтруктураПакетаЗапросов.Найти(НРег(ТекущееСлово));
						Если ИндексЗапроса <> Неопределено Тогда
							ПостроительЗапроса = мПлатформа.ПостроительПакетаЗапросовДоИндекса(мЗапросыПакета, ИндексЗапроса);
			...
			//Если Истина 
			//	И ЯзыкПрограммы = 0
			//	И мФлагиКомпиляции.БезКонтекста
			//	И МассивРодительскихСтрутурТипа = мКорневаяТаблицаТипов
			//	И РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "Глобальный"
			//Тогда
			//	Продолжить;
			//КонецЕсли;
			Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
	...
	Если Истина 
		...
		И ТаблицаТипов[0].СтрокаОписания.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено
		И ТаблицаТипов[0].СтрокаОписания <> мМетодМодуля
	Тогда 
		мМетодМодуля.Зависимый = Истина;
	КонецЕсли;
...
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ТаблицаТипов, ТипВыхода = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаПараметраМетода.ТипЗначения) Тогда
		...
		Если ЛиДетальностьТиповДостаточна(ТаблицаТипов) Тогда
			Возврат ТаблицаТипов;
		КонецЕсли;
	КонецЕсли;
	...
	#КонецЕсли
	СчетчикВызовов = 0;
	ДетальностьТиповДостаточна = Неопределено;
	Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
		...
		Если ВхожденияПараметров.Количество() > ИндексПараметра Тогда
			...
			Если ЛиБесполезноеПрисвоениеПеременной(ФактическийПараметр) Тогда
				Продолжить;
			ИначеЕсли ЛиПрисвоениеБулевойКонстанты(ФактическийПараметр) Тогда
			...
			Иначе
				...
				Если ПройденныеВызовы[КлючВызова] = Неопределено Тогда
					...
					ТаблицаТиповИсточник = ПолеТекстаПрограммыВызова.ВычислитьТипЗначенияВыражения(ФактическийПараметр,,,,,, Ложь,,,, Истина, ВхождениеВызова.ПозицияВхождения - 1 - НачальнаяПозицияБлока);
					Если ТаблицаТиповИсточник[0].Детальность > 0 Тогда
						мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, ТаблицаТиповИсточник);
	...
	КонецЦикла;
	Если Истина
		И ДетальностьТиповДостаточна = Неопределено
		И мРекурсивныйПуть.ВычислениеРезультатаМетода = мМетодМодуля.Имя 
	Тогда       
		ЛиКлючевойПараметр = Не ЛиДетальностьТиповДостаточна(ТаблицаТипов, МинДетальность);
...
// Возвращаемое значение:
//    Булево - 
Функция ЛиДетальностьТиповДостаточна(ТаблицаТипов, Знач МинДетальность = 3) Экспорт
	Если ТаблицаТипов.Количество() = 0 Тогда
		Результат = Ложь;
	Иначе
		ПоследнийТип = ТаблицаТипов[ТаблицаТипов.Количество() - 1];
		Результат = Ложь
			Или ПоследнийТип.ИмяОбщегоТипа = "Строка" 
			Или ПоследнийТип.ИмяОбщегоТипа = "Число"
			//Или ПоследнийТип.ИмяОбщегоТипа = "Булево"
			Или ПоследнийТип.Детальность >= МинДетальность;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Функция ВычислитьТипФункцииСФактПараметрами(Знач МетодМодуля, Знач ТекущиеАргументы, Знач ПредшествующийТекст, Знач ПолныйАнализСоставаТипов, Знач ПараметрыИсходящегоВызова,
	...
	Для Каждого КлючИЗначение Из МетодМодуля.КлючевыеПараметры Цикл 
		...
		Если Не ЗначениеЗаполнено(ВыражениеПараметра) Тогда
			Продолжить;
		Иначе 
			ЗначениеПараметра = ВычислитьТипЗначенияВыражения(ВыражениеПараметра, ПредшествующийТекст,,,, ПолныйАнализСоставаТипов, Ложь,,, Истина, Истина);
			Если ЗначениеПараметра[0].Детальность = 0 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		ФактическиеПараметры.Вставить(КлючИЗначение.Ключ, ЗначениеПараметра);
	КонецЦикла;
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Знач ЭтоПрисвоенноеВыражение = Ложь, Знач ПозицияВМетодеДляКэша = 0) Экспорт
	...
	ИначеЕсли Лев(Выражение, 1) = """" Тогда
		...
		Если ВычислитьЗначение Тогда 
			Если Найти(Выражение, Символы.ПС) = 0 Тогда
				// Для ускорения
				СтруктураТипа.Метаданные = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(Выражение);
			Иначе 
				Попытка
					СтруктураТипа.Метаданные = Вычислить(Выражение);
				Исключение
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе                        
		...
		Иначе
			Если ирОбщий.ЛиИмяПеременнойЛкс(Выражение) Тогда
				// Для ускорения
				ТипСлова = "Свойство";
				ВычислениеРазрешено = Истина;
				РодительскийКонтекст = "";
			Иначе 
				...
				Если Вхождения.Количество() > 0 Тогда 
					ТипСлова = "Свойство";
					ВычислениеРазрешено = Истина;
					РодительскийКонтекст = Сред(Вхождения[0].SubMatches(0), 2);
					ТекущееСлово = Вхождения[0].SubMatches(8);
					ТекущийИндекс = Вхождения[0].SubMatches(10);
					ТекущиеАргументы = Вхождения[0].SubMatches(9);
					Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ТекущийИндекс) Тогда
						ТекущееСлово = Сред(ТекущийИндекс, 2, СтрДлина(ТекущийИндекс) - 2);
					КонецЕсли;
				ИначеЕсли ЭтоПрисвоенноеВыражение Тогда
					ВычислениеРазрешено = Истина;
					ЭтоВиртПрисвоение = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли; 
		Если мЯзыкПрограммы = 1 Тогда
		...
		Если Истина
			И ТипСлова = "Метод"
			И мРекурсивныйПуть.ВычислениеРезультатаМетода = ТекущееСлово
		Тогда
			// Защита от прямой циклической рекурсии
			ВычислениеРазрешено = Ложь;
		КонецЕсли;
		Если ВычислениеРазрешено Тогда 
			Если Не ЗначениеЗаполнено(ТекстДляПоискаОпределения) Тогда
				ТекстДляПоискаОпределения = Лев(мТекстБлока, ПозицияВМетодеДляКэша);
			КонецЕсли;
			...
			Если Истина
				...
				И мМетодМодуля <> Неопределено 
			Тогда 
				ТипыВыражений = мМетодМодуля.ТипыВыражений;
				Если ТипыВыражений = Неопределено Тогда 
					ТипыВыражений = Новый Соответствие;
					мМетодМодуля.ТипыВыражений = ТипыВыражений;
				КонецЕсли;
				КлючВыраженияСПозицией = XMLСтрока(ПозицияВМетодеДляКэша) + ";" + Лев(Выражение, 200);
				ТаблицаТиповКэш = ТипыВыражений[КлючВыраженияСПозицией];
				Если ТаблицаТиповКэш <> Неопределено Тогда
					Возврат ТаблицаТиповКэш;
				КонецЕсли;
				//Если ЗначениеЗаполнено(ПозицияВМетодеДляКэша) Тогда
					// Почему то это ускоряет вычисления
					мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипа);
					ТипыВыражений.Вставить(КлючВыраженияСПозицией, ТаблицаТипов);
				//КонецЕсли;
			КонецЕсли;
			ТаблицаСтруктурРодителей = новый Массив;
			Если Истина
				И Не ЭтоВиртПрисвоение  /////////////////////
				И (Ложь
			...
			Тогда
				...
				Тогда
					ТаблицаСтруктурРодителей = мКорневаяТаблицаТипов; 
				Иначе 
					ТаблицаСтруктурРодителей = ВычислитьТипЗначенияВыражения(РодительскийКонтекст, ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь,,,,, ПозицияВМетодеДляКэша);
					Если ПустаяСтрока(РодительскийКонтекст) И мКорневаяТаблицаТипов = Неопределено Тогда
						мКорневаяТаблицаТипов = ТаблицаСтруктурРодителей;
					КонецЕсли;
				КонецЕсли;
				ЛиПеременнаяМодуля = Ложь;
				МассивДочернихСтруктурТипа = ВычислитьТипДочернегоЭлемента(ТаблицаСтруктурРодителей, ТекущееСлово, ТипСлова, ТекущийИндекс, ТекущиеАргументы, ПредшествующийТекст, ПолныйАнализСоставаТипов, ЛиПеременнаяМодуля, ПозицияВМетодеДляКэша);
				мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, МассивДочернихСтруктурТипа);
				Если ЛиПеременнаяМодуля Тогда 
					КлючВыраженияСПозицией = "";
					Если Ложь
						Или Не ГлобальныйПоискВМодуле 
							...
							Если СледРекурсии = Неопределено Тогда
								...
								Если ПрисвоенныйКонтекст <> Выражение Тогда
									МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст, ТекстДляПоискаОпределения,
										Лев(ТекстДляПоискаОпределения, ПозицияВхождения0 - 1), РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь,,,,, ПозицияВхождения0);
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущееСлово) Тогда  
				Если ТаблицаСтруктурРодителей.Количество() > 0 Тогда
					МетаданныеРодителя = ТаблицаСтруктурРодителей[0].Метаданные;
				Иначе 
					МетаданныеРодителя = Неопределено;
				КонецЕсли;
				Если Истина
					И МетаданныеРодителя <> Неопределено
					И МассивДочернихСтруктурТипа.Количество() = 1
					И (Ложь
						Или МассивДочернихСтруктурТипа[0].ИмяОбщегоТипа = "ОбщийМодуль"
				...
				Иначе
					...
					Если ЭтоВиртПрисвоение Тогда
						ИмяВиртПеременной = "ВиртВыражение294";
						мРегВыражение.Pattern = ШаблонНазначенияТипаВыражению(ИмяВиртПеременной);
					...
					Иначе 
						...
						Тогда 
							НазначенияТиповМетода = мМетодМодуля.НазначенияТипов;
							Если НазначенияТиповМетода = Неопределено Тогда 
								НазначенияТиповМетода = Новый Соответствие;
								мМетодМодуля.НазначенияТипов = НазначенияТиповМетода;
							КонецЕсли;
							КлючВыраженияБезПозиции = НРег(Лев(Выражение, 200));
							Вхождения = НазначенияТиповМетода[КлючВыраженияБезПозиции];
						КонецЕсли;
						Если Вхождения = Неопределено Тогда  
							ШаблонПриемника = ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(Выражение);
							Если Истина
								И ГлобальныйПоискВМодуле 
								И мМодульМетаданных <> Неопределено
								И мМодульМетаданных.Свойство("ТипМодуля") // Пока отсутствует у динамического модуля
							Тогда
								Если мМодульМетаданных.ТипМодуля = "Форма" Тогда
									ИмяСамоСвойствоКонтекста = "ЭтаФорма";
								Иначе
									ИмяСамоСвойствоКонтекста = "ЭтотОбъект";
								КонецЕсли;
								ШаблонПриемника = "(?:" + ИмяСамоСвойствоКонтекста + "\.)?" + ШаблонПриемника;
							КонецЕсли;
							мРегВыражение.Global = Истина;
							мРегВыражение.Pattern = ШаблонНазначенияТипаВыражению(ШаблонПриемника);
							Вхождения = мРегВыражение.НайтиВхождения(ТекстДляПоискаОпределения);
							Если КлючВыраженияБезПозиции <> Неопределено Тогда
								НазначенияТиповМетода[КлючВыраженияБезПозиции] = Вхождения;
							КонецЕсли; 
						Иначе 
							Пустышка = 0;
						КонецЕсли;
					...
					Если Вхождения.Количество() > 0 Тогда
						СтруктураТипа = мПлатформа.НоваяСтруктураТипа();
						Если Не ЗначениеЗаполнено(ПозицияВМетодеДляКэша) Тогда
							ПозицияВМетодеДляКэша = СтрДлина(ТекстДляПоискаОпределения);
						КонецЕсли;
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ПоследнееВхождение.FirstIndex > ПозицияВМетодеДляКэша Тогда
								Продолжить;
							КонецЕсли;
							ЭтоПрисвоение = Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(1));
							Если ЭтоПрисвоение Тогда
								ПрисвоенныйКонтекст = ПоследнееВхождение.SubMatches(2);
								Если Ложь
									Или ЛиБесполезноеПрисвоениеПеременной(ПрисвоенныйКонтекст) 
									Или ирОбщий.СтрокиРавныЛкс(Выражение, ПрисвоенныйКонтекст)
								Тогда 
									Продолжить;
								КонецЕсли;
							КонецЕсли;
							...
							ПрисвоенныйТип = ПоследнееВхождение.SubMatches(22);
							Если Истина
							...
							Тогда
								...
								ТаблицаТипов = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ПрисвоенныйТип, "-"),, ТекстДоВхождения);
								СтруктураТипа = ТаблицаТипов[0];
								Если СтруктураТипа.Детальность > 0 Тогда
									Прервать;
								КонецЕсли;
							КонецЕсли;
							Если ЭтоПрисвоение Тогда
								Если ЛиПрисвоениеБулевойКонстанты(ПрисвоенныйКонтекст) Тогда
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, мПлатформа.НоваяСтруктураТипа("Булево"));
									Продолжить;
								КонецЕсли;
								...
								Если СловоНовый = Нрег("Новый") Или СловоНовый = Нрег("New") Тогда
									...
									КонецЕсли; 
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, СтруктураТипа);
								Иначе//Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(7)) Тогда 
									МассивСтруктурПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ПрисвоенныйКонтекст,,, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов,
										Ложь,,, ВычислитьЗначение,, ПозицияВхождения0);
									мПлатформа.ДобавитьВТаблицуТипов(ТаблицаТипов, МассивСтруктурПрисвоенныхТипов);
								КонецЕсли;
							Иначе
								// Это обход коллекции
								ПрисвоенныйКонтекст = ПоследнееВхождение.SubMatches(16);
			...
			Если Истина
				...
				//И Не ТолькоПрисвоение   // Отключил, чтобы результат мПлатформа.НоваяСтруктураТипа() считался
			Тогда
				//Если ЗначениеЗаполнено(ПозицияВМетодеДляКэша) Тогда
				//	ТекстДляПоискаОпределения = Лев(ТекстДляПоискаОпределения, ПозицияВМетодеДляКэша); // Отрезает полезные свойства структур
				//КонецЕсли; 
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						СобратьТипыСвойствСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ТекстДляПоискаОпределения, ПредшествующийТекст, ГлобальныйПоискВМодуле, ЭтоВиртПрисвоение, ПозицияВМетодеДляКэша);
					...
					Тогда
						...
						мРегВыражение.Global = Истина;
						РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляПоискаОпределения);
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							КонецЕсли;
							Счетчик = Счетчик + 1;
							ТекстДоВхождения = ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, ВхождениеСвойства, ТекстДляПоискаОпределения);
							ТаблицаТиповЭлементов = ВычислитьТипЗначенияВыражения(ВхождениеСвойства.SubMatches(0), ТекстДоВхождения, ПредшествующийТекст, РазрешитьАнализИмениТипа,
					...
					Тогда
						...
						КонецЕсли;
						мРегВыражение.Pattern = Выражение + "\s*\.\s*Колонки\s*.\s*Добавить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
						мРегВыражение.Global = Истина;
						РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляПоискаОпределения);
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							Иначе
								ТекстДоВхождения = ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, ВхождениеСвойства, ТекстДляПоискаОпределения);
								ТекстовоеЗначение = ВычислитьЗначениеВыражения(ПрисвоенноеВыражение, ТекстДоВхождения,, ВхождениеСвойства.FirstIndex);
							КонецЕсли;
							Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда
								Попытка
									Колонки.Добавить(ТекстовоеЗначение);
								Исключение
									Пустышка = 0;
								КонецПопытки; 
							КонецЕсли;
					...
					Тогда
						ТекстЗапроса = ВычислитьЗначениеВыражения(Выражение + ".Текст", ТекстДляПоискаОпределения, ГлобальныйПоискВМодуле);
						Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
							...
							ирОбщий.СкопироватьКоллекциюЛкс(СобратьТипыСвойствСтруктуры(Выражение + ".Параметры",, ТекстДляПоискаОпределения, ПредшествующийТекст), ПостроительЗапроса.Параметры);
						КонецЕсли; 
						мПлатформа.ОбновитьДетальностьСтруктурыТипа(СтруктураТипаЦикл);
					КонецЕсли; 
				КонецЦикла;
...
Функция СобратьТипыСвойствСтруктуры(Знач Выражение, Структура = Неопределено, Знач ТекстДляПоискаОпределения, Знач ПредшествующийТекст, Знач ГлобальныйПоискВМодуле = Ложь, Знач ТолькоПрисвоение = Ложь,
	Знач ПозицияВМетоде = 0)
	
	мРегВыражение.Global = Ложь; 
	мРегВыражение.Pattern = Выражение + "\s*=\s*Новый\s+Структура\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")";
	РезультатСвойств = мРегВыражение.НайтиВхождения(ТекстДляПоискаОпределения, Истина);
	Если РезультатСвойств.Количество() > 0 Тогда
		Вхождение = РезультатСвойств[0];
		ПрисвоенноеВыражение = Вхождение.SubMatches(0);
		Если Лев(ПрисвоенноеВыражение, 1) = """" Тогда
			ТекстовоеЗначение = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ПрисвоенноеВыражение);
		Иначе
			ТекстДоВхождения = ЗагрузитьМетодМодуляПоВхождениюИРазобратьКонтекст(ГлобальныйПоискВМодуле, Вхождение, ТекстДляПоискаОпределения);
			ТекстовоеЗначение = ВычислитьЗначениеВыражения(ПрисвоенноеВыражение,,, Вхождение.FirstIndex);
		КонецЕсли;
	...
	Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ТолькоПрисвоение Или Выражение = мПлатформа.ИмяПеременнойВозвращаемогоЗначения() Тогда
		Возврат Структура;
	КонецЕсли;
	МинДетальность = 4;
	СтруктураУточнения = ирОбщий.СкопироватьКоллекциюЛкс(Структура);
	Вхождения = Неопределено;
	Если Истина
		И мМетодМодуля <> Неопределено 
		И Не ГлобальныйПоискВМодуле 
	Тогда 
		КэшПоискСвойств = мМетодМодуля.ПоискСвойств;
		Если КэшПоискСвойств = Неопределено Тогда 
			КэшПоискСвойств = Новый Соответствие;
			мМетодМодуля.ПоискСвойств = КэшПоискСвойств;
		КонецЕсли;
		КлючВыраженияБезПозиции = НРег(Лев(Выражение, 200));
		Вхождения = КэшПоискСвойств[КлючВыраженияБезПозиции];
		ТекстДляПоискаОпределения = мТекстБлока;
	КонецЕсли;
	Если Вхождения = Неопределено Тогда
		мРегВыражение.Pattern = Выражение + "\s*\.\s*Вставить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")(?:\s*,\s*((?:" + шСтрокаПрограммы + "|" + шНачалоСкобок + "|[^\)""])*))?";
		мРегВыражение.Global = Истина;
		Вхождения = мРегВыражение.НайтиВхождения(ТекстДляПоискаОпределения);
		Если КлючВыраженияБезПозиции <> Неопределено Тогда
			КэшПоискСвойств[КлючВыраженияБезПозиции] = Вхождения;
		КонецЕсли;
	КонецЕсли;
	Для Каждого Вхождение Из Вхождения Цикл
		Если ПозицияВМетоде > 0 И Вхождение.FirstIndex > ПозицияВМетоде Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстовоеЗначение) Тогда 
			...
			Если Не ЛиДетальностьТиповДостаточна(ТаблицаТиповЗначения, МинДетальность) Тогда
				СтруктураУточнения.Вставить(ТекстовоеЗначение, Вхождение.FirstIndex);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если Не ГлобальныйПоискВМодуле Тогда // Для сокращения редко полезных затратных вычислений
		Для Каждого КлючИЗначение Из СтруктураУточнения Цикл
			ВыражениеЗначения = Выражение + "." + КлючИЗначение.Ключ;
			ПозицияВМетодеЦикл = КлючИЗначение.Значение;
			Если ТипЗнч(ПозицияВМетодеЦикл) <> Тип("Число") Тогда
				ПозицияВМетодеЦикл = ПозицияВМетоде;
			КонецЕсли;
			ТаблицаТиповЗначения = ВычислитьТипЗначенияВыражения(ВыражениеЗначения,,,,,, Ложь, ГлобальныйПоискВМодуле, Истина,,, ПозицияВМетодеЦикл);
...
Функция ЛиБесполезноеПрисвоениеПеременной(Знач ПрисвоенноеВыражение) Экспорт
	Результат = Ложь 
		...
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "Неопределено")
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "Undefined") 
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "Null") 
		Или Не ЗначениеЗаполнено(ПрисвоенноеВыражение) // пустое присвоение https://www.hostedredmine.com/issues/947747
	;
	Возврат Результат;
КонецФункции    
...
// Возвращаемое значение:
//    Булево - 
Функция ЛиПрисвоениеБулевойКонстанты(Знач ПрисвоенноеВыражение, выхЗначение = Неопределено) Экспорт
	Если Ложь 
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "Истина") 
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "True") 
	Тогда 
		выхЗначение = Истина;
		Результат = Истина;
	ИначеЕсли Ложь 
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "Ложь") 
		Или ирОбщий.СтрокиРавныЛкс(ПрисвоенноеВыражение, "False") 
	Тогда 
		выхЗначение = Ложь;
		Результат = Истина;
	Иначе 
...
// Возвращаемое значение:
//    Строка - 
Функция ШаблонНазначенияТипаВыражению(Знач ШаблонПриемника) Экспорт
	ШаблонКоллекции = "(" + шРазделитель + "+Из" + шРазделитель + "+(" + шИмяСкобки + "?" + шИндекс + "?" + "(\." + шИмяСкобки + "?" + шИндекс + "?)*))\s*(?:Цикл|Do)?";
	Результат = шПредИмя + ШаблонПриемника 
...
// Возвращаемое значение:
//    Строка - 
Функция ВычислитьЗначениеВыражения(Выражение, Знач ПредшествующийТекст, Знач ГлобальныйПоискВМодуле = Ложь, Знач ПозицияВМетоде = 0)
	Если ПустаяСтрока(Выражение) Тогда
		Возврат "";
	КонецЕсли;
	ТаблицаТипов = ВычислитьТипЗначенияВыражения(Выражение, ПредшествующийТекст, ПредшествующийТекст,,,, Ложь, ГлобальныйПоискВМодуле, Найти(Выражение, "(") = 0, Истина,, ПозицияВМетоде);
	СтруктураТипа = ТаблицаТипов[0];
...
Функция ВычислитьСвойствоФормыПоПолномуИмени(Знач ИмяТипа)
	...
	"Форма = ПолучитьФорму(""" + ЧастиИмени.ИмяФормы + """);
	|Результат = Форма";
	Если ЗначениеЗаполнено(ЧастиИмени.ПутьКЭлементу) Тогда
		ТекстДляПоискаОпределения = ТекстДляПоискаОпределения + "." + ЧастиИмени.ПутьКЭлементу;
	КонецЕсли;
	выхСтруктураТипаКонтекста = ВычислитьТипЗначенияВыражения("Результат", ТекстДляПоискаОпределения)[0];
...
Функция ТаблицаТиповТекущегоВыражения(ЛиСправаОтРавенства = Ложь, БезКонструкторов = Ложь, ПолныйАнализСоставаТипов = Истина, Знач НомерСтроки = 0, Знач НомерКолонки = 0) Экспорт
	...
	КонецЕсли;
	Попытка
		ТаблицаТиповКонтекста = ВычислитьТипЗначенияВыражения(лКонтекст, " " + мТекстДляПоискаОпределения, мПредшествующийТекст, Истина,, ПолныйАнализСоставаТипов);
	Исключение
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	МассивДляПроверкиДублей = Новый Массив;
	ТаблицаТиповКонтекста.Сортировать("Детальность Убыв");
	...
	КонецЦикла; 
	Если ЛиВЗаголовкеКонкретногоМетода() Тогда
	...
	Если мЭтоСтроковыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
		...
		Тогда
			...
			Тогда
				КоллекцияЭлементовМД = ОтобратьМетодыДляСпискаСлов(СтрокаОписанияМетода.ТипКонтекста = "ОписаниеОповещения");
			ИначеЕсли Ложь
				Или ИмяПараметра = "ИмяСобытия"
			Тогда
				КоллекцияЭлементовМД = ирОбщий.ВыгрузитьСвойствоЛкс(ирОбщий.СобытияЭлементаФормыЛкс(ВыбраннаяСтруктураТипа.Метаданные), "Слово");
			ИначеЕсли Истина
		...
		ДобавитьКоллекциюМетаданныхВТаблицуСлов(КоллекцияЭлементовМД);
	ИначеЕсли Истина
	...
	// Имена типов внутри комментария и текстового литерала
	Если Истина
	...
	Тогда  
		...
		Тогда 
			...
			СтрокаСлова.ТипСлова = "Свойство";
		ИначеЕсли ирОбщий.СтрокиРавныЛкс(ирОбщий.ПервыйФрагментЛкс(мРодительскийКонтекст), КорневоеСловоБД) Тогда
			мЯзыкПрограммы = 1;
	...
	Попытка
		ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(выхСтруктураТипаКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы,,,
			?(мЭтоСтроковыйЛитерал И выхСтруктураТипаКонтекста.ТипЯзыка <> "ИмяТипа", Неопределено, мФлагиКомпиляции),, МодульМетаданныхКонтекста(выхСтруктураТипаКонтекста),,, ДобавлятьЛокальныйКонтекст, мНаборыСлов);
		...
		КонецЕсли;  
		Если Истина
			И Не мЭтоСтроковыйЛитерал
			И мЭтоЛокальныйКонтекстТаблицыСлов
			И мМодульМетаданных <> Неопределено
			И ирОбщий.СтрКончаетсяНаЛкс(мИмяМодуля, ".Форма")
			И (Ложь
				Или мМетодМодуля = Неопределено
		...
		Тогда
			...
			Для Каждого Директива Из ДирективыКомпиляцииТипаМодуля Цикл
				мПлатформа.ДобавитьВТаблицуСлов(ВыгрузкаТаблицыСлов, "&" + Директива, "Конструкция",, "Null", "Предопределенный");
			КонецЦикла;
		КонецЕсли;
...
Функция ОтобратьМетодыДляСпискаСлов(ТолькоЭкспортные) Экспорт
...
Функция КопияКомпоненты() Экспорт 
	...
	КопияКопоненты.мСлужебнаяФорма = мСлужебнаяФорма;
	...
	КопияКопоненты.мКорневаяТаблицаТипов = мКорневаяТаблицаТипов;
	...
	КопияКопоненты.мФлагиКомпиляции = мФлагиКомпиляции;
	//КопияКопоненты.мКонечнаяСтрока = мКонечнаяСтрока;
	//КопияКопоненты.мКонечнаяКолонка = мКонечнаяКолонка;
	//КопияКопоненты.мНачальнаяСтрока = мНачальнаяСтрока;
	//КопияКопоненты.мНачальнаяКолонка = мНачальнаяКолонка;
...
Процедура ОткрытьПоискВызововМетода(Знач ИмяМетода = Неопределено, Знач ИскатьНепрямые = Неопределено, Знач Автозапуск = Истина) Экспорт 
	...
	Если ИмяМетода = Неопределено Тогда
		...
		КонецЕсли;
		ИмяМетода = мКонтекст;
		Если Прав(мКонтекст, 1) <> "(" И ЛиВЗаголовкеКонкретногоМетода() Тогда
			ИмяМетода = мМетодМодуля.Имя + "(";
...
// Функция - Вычислить получить форму
//
// Параметры:
...
Функция ВычислитьПолучитьФорму(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	...
	ИмяФормы = ВычислитьЗначениеВыражения(ПервыйПараметр, ПредшествующийТекст);
	...
	Если ФормаИлиИмитатор <> Неопределено Тогда 
		...
		КонецЕсли;
		Результат = мПлатформа.НоваяТаблицаТипов();
		мПлатформа.ДобавитьВТаблицуТипов(Результат, СтруктураТипа);
...
// Функция - Вычислить реквизит формы в значение
//
// Параметры:
//  РодительскаяСтруктураТипа	 - см. Обработки.ирПлатформа.Создать().НоваяСтруктураТипа() - 
//  МассивПараметров			 - 	 - 
...
//   - 
//
Функция ВычислитьРеквизитФормыВЗначение(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если Истина
	...
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
	ИмяРеквизита = ВычислитьЗначениеВыражения(ПервыйПараметр, ПредшествующийТекст);
	...
	Тогда 
		СтруктураТипа = мПлатформа.СтруктураТипаИзКонкретногоТипа(СлужебныеДанные.Реквизиты.Найти(ИмяРеквизита, "НИмя").Тип.Типы()[0]);
	КонецЕсли;
	Результат = мПлатформа.НоваяТаблицаТипов();
	мПлатформа.ДобавитьВТаблицуТипов(Результат, СтруктураТипа);
	Возврат Результат;
...
Функция ВычислитьНайтиЭлементКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВычислитьОтобратьКоллекциюЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Результат[0].ИмяОбщегоТипа = "Массив[" + мПлатформа.ПредставлениеМассиваСтруктурТипов(ТаблицаТиповЭлементов) + "]";
	Возврат Результат;

КонецФункции

Функция ВычислитьСоздатьОбъектПоИмениМетаданныхЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
	Текст = ВычислитьЗначениеВыражения(ПервыйПараметр, ПредшествующийТекст);
	Попытка
	...
	КонецПопытки;
	Результат = мПлатформа.НоваяТаблицаТипов();
	мПлатформа.ДобавитьВТаблицуТипов(Результат, ОписаниеТипов);
	Возврат Результат;

КонецФункции

Функция ВычислитьВычислить(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);
...
// БСП
Функция ВычислитьОбщийМодуль(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "") 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = НРег(МассивПараметров[0]);

	//ИмяМодуля = ВычислитьЗначениеВыражения(ПервыйПараметр, ПредшествующийТекст); // Долго и редко полезно
	Если Лев(ПервыйПараметр, 1) = """" Тогда 
		ИмяМодуля = ирОбщий.ТекстИзВстроенногоЯзыкаЛкс(ПервыйПараметр);
	КонецЕсли;
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, Знач ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	Иначе
		Если ТолькоСвойства Тогда
			Шаблон = "(ъъъъ)";
		Иначе
			Шаблон = "(?:[^." + шБуква + "\d]|^)\s*(?:" + ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=\s*\())";
		КонецЕсли;
		Шаблон = Шаблон + "|(?:[^." + шБуква + "\d]|^)\s*(?:" + ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(мРодительскийКонтекст) + "\.(" + шИмя + ")(?=[^\(?:" + шБуква + "\d]|$))";
...
// 
Функция УстановитьТекст(Знач Текст, Знач Активировать = Ложь, Знач НачальныйТекстДляСравнения = Неопределено, Знач СохранитьГраницыВыделения = Ложь, Знач СжатоеИмяМодуля = Неопределено,
	...
	КонецЕсли; 
	мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипа, ТипМодуля,, Текст, мМетодМодуля);
	мФлагиКомпиляции = ирОбщий.СкопироватьКоллекциюЛкс(мМодульМетаданных.ФлагиКомпиляции);
	Если ТипЗнч(СтруктураТипа.ДержательМетаданных) = Тип("Строка") Тогда
		ИмяМодуля = СтруктураТипа.ДержательМетаданных + " " + СтруктураТипа.ДержательМетаданных;
	КонецЕсли;
...
Процедура ОбновитьМодульМетаданных(Знач Текст = Неопределено) 
	...
	Если мМодульМетаданных <> Неопределено И мМодульМетаданных.СтруктураТипа <> Неопределено Тогда
		мМодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(мМодульМетаданных.СтруктураТипа, ирОбщий.ПоследнийФрагментЛкс(мИмяМодуля),, Текст, мМетодМодуля);
	Иначе
...
Функция ТаблицаСловВJSON(КлючНабораСлов = "") Экспорт 
	...
	ИменаКолонок = "ТипСлова,Слово,ТипЗначения";
	...
	Иначе
		// Однострочный код использован для ускорения при разрешенной отладке. Выше расположен оригинал. Выполняйте изменения синхронно в обоих вариантах. Преобразовано консолью кода из подсистемы "Инструменты разработчика"
		Для Каждого Строка Из ВыгрузкаТаблицы Цикл  		Строка.ЛиРез = Строка.ТипЗначения = "";  		Строка.ЛиМетод = Строка.ТипСлова = "Метод";  КонецЦикла;  
	КонецЕсли;
...
шПрисвоение = "(" + шРазделитель + "*=" + шРазделитель 
	+ "*((новый|New)(?=[\s\(])" // Изменение для поддержки ProgID типа Forms.TextBox.1
		+ "(?:" + шРазделитель + "+(" + шИмя + "))?" + шРазделитель + "*(?:\((?:""(" + шИмя+ "(?:\.[" + шБуква + "\d]+)*)"")?(?:" + шСкобки + "|[^\(\)]+)*\))?"
