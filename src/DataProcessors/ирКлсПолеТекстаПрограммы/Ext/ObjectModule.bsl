...
Процедура ПрименитьОжидаемыйТипВТаблицеСлов(Знач ОтделятьБольшиеНаборыСлов = Ложь)
	...
	Если мКонечнаяСтрока > 1 Тогда
		...
		Тогда
			...
			ОтступТекущейСтроки = ирОбщий.НепечатноеНачалоСтрокиЛкс(мТекущаяСтрокаНачало);
			ДлинаТекущегоОтступа = СтрДлина(ОтступТекущейСтроки);
			Для Счетчик = 1 По 100 Цикл
				...
				Если СтрДлина(ОтступСтроки) < ДлинаТекущегоОтступа Тогда
					Прервать;
				КонецЕсли;
				Если ОтступСтроки = ОтступТекущейСтроки Тогда
