...
Функция НайтиПоказатьСловоВТексте(СтрокаПоиска, Знач СловоЦеликомИлиШаблон = Истина, ПолеТекста = Неопределено, УстановитьФокус = Ложь, ИскатьСНачала = Ложь, Знач ВТекущемБлоке = Ложь,
	...
	Если мМетодМодуля <> Неопределено И ВТекущемБлоке Тогда
		...
		КонецОбласти = мМетодМодуля.ПозицияТела + мМетодМодуля.ДлинаТела;
	Иначе
...
Функция ВыделитьМетод(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	АнализаторКода.УстановитьТекст(ТелоМетода(, НомераПервойСтрокиТела));
...
Функция ТелоМетода(МетодМодуля = Неопределено, выхНомераПервойСтрокиТела = 1, Знач БезВозвратов = Ложь) Экспорт 
	...
	Если МетодМодуля <> Неопределено Тогда  
		...
		ИначеЕсли БезВозвратов Тогда
			...
			Если Результат = Неопределено Тогда
				...
				Результат = Сред(мОригинальныйТекст, МетодМодуля.ПозицияТела, МетодМодуля.ДлинаТела);
	...
	Если Не ЗначениеЗаполнено(ИмяПараметра) Тогда
		...
		Если ПерейтиКОпределению() <> Неопределено Тогда 
			Если ЗначениеЗаполнено(СмещениеСтрокиМетода) Тогда
				...
				ВыделитьСтрокуВПолеТекста(НомерПервойСтрокиОпределенияМетода + СмещениеСтрокиМетода);
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Функция ДобавитьПереходыИзСтруктурыТипаВСписокВыбора(Знач СтруктураТипа, Знач СписокВыбора = Неопределено) 
	...
	Если СтруктураТипа.СтрокаОписания <> Неопределено Тогда 
		...
		Если ТаблицаВладелец.Колонки.Найти("ЛиЭкспорт") <> Неопределено Тогда
			...
			Если СтрокаМетода.Имя <> "<>" Тогда
				ДобавитьПереходНаМетодВСписокВыбора(СтрокаМетода, СписокВыбора);
				Для Каждого КлючИЗначение Из ирКэш.РасширенияКонфигурацииЛкс(Ложь) Цикл
					...
					ПолеТекстаМодуля = ирКэш.ПолеТекстаМодуляЛкс(ИмяРасширенияМодуля);
					...
					Если РасширениеМетода <> Неопределено Тогда
						...
						ДобавитьПереходНаМетодВСписокВыбора(РасширениеМетода, СписокВыбора, Расширение.Имя);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
	...
	Если ТипЗнч(СтруктураТипа.Метаданные) = Тип("ОбъектМетаданных") Тогда
		...
		ДобавитьТипДанныхВСписокПереходов(СписокВыбора, КлючПерехода);
	КонецЕсли;
...
//  Строка - 
//
Функция ДобавитьПереходНаМетодВСписокВыбора(Знач СтрокаМетода, Знач СписокВыбора, Знач ИмяРасширения = "") Экспорт
...
Функция НомерСтрокиИзПозиции(ПозицияВМодуле) Экспорт
	Если ПозицияВМодуле = Неопределено Тогда
		Возврат 1;
	КонецЕсли;
	СлужебноеПолеТекстаДолгое.УстановитьГраницыВыделения(ПозицияВМодуле, ПозицияВМодуле);
					...
					Если Вхождения.Количество() > 0 Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ПроверятьТипЧерезЕсли И Не ЛиУстановкаЗначения Тогда 
										...
										Если ПрименитьТип Тогда 
											...
											Если ТипЗнч(Тип) = Тип("Тип") Тогда 
												...
												Если Тип = Тип("ОбъектМетаданных") Тогда
													СтруктураМоноТипа.ИмяОбщегоТипа = "ОбъектМетаданных: Справочник"; // Наиболее типичный вариант. Это лучше чем абстрактный тип без свойств.
												КонецЕсли;
												ДобавитьЕдинственныйТип(ТаблицаТипов, СтруктураМоноТипа);
											КонецЕсли;
								...
								КонецЕсли;
							КонецЕсли; 
							Если ЛиПеременнаяМодуля И Не ЛиКорневойВызов Тогда
								// Для ускорения
								Перейти ~Возврат;
							КонецЕсли;
...
~Возврат:
	Если ЛиКорневойВызов Тогда
		Если Истина
			...
			И мМетодМодуля.КлючевыеПараметры <> Неопределено 
			И мРекурсивныйПуть.ДопКлючКэша = Неопределено // 22.12.2023 защита от циклической рекурсии при вычислении результата Дерево_НайтиПоПутиСтрокойЛкс
		Тогда
	...
	Для Каждого Вхождение Из Вхождения Цикл
		...
		Если ирОбщий.ЛиИмяПеременнойЛкс(ТекстовоеЗначение) Тогда 
			...
			Если ЗначениеЗаполнено(ПрисвоенныйТип) Тогда
				ТаблицаТиповИзКомментария = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ПрисвоенныйТип, "-"));
				Если ТаблицаТиповИзКомментария.Количество() > 0 Тогда
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, _2 = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Иначе
		...
		Если Истина
			...
			И мТаблицаТиповКонтекста.Количество() > 1 
			И мКонкретныйТипКонтекста = "ОбщийМодуль"
		Тогда
		...
		Попытка
			Для Каждого СтруктураКонтекста Из СтруктурыКонтекста Цикл
				ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ЭтоГруппаОбщихМодулей,,
					...
					ДобавлятьЛокальныйКонтекст, мНаборыСлов);
			КонецЦикла;
	...
	Тогда
		...
		Тогда
			...
			Иначе
				...
				КоллекцияЭлементовМД = ОтобратьМетодыДляСпискаСлов(Модуль, ТолькоЭкспотные);
			КонецЕсли;
		ИначеЕсли Ложь
