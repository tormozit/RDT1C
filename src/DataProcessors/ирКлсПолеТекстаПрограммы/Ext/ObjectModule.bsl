...
Функция НайтиПоказатьСловоВТексте(СтрокаПоиска, Знач СловоЦеликом = Истина, ПолеТекста = Неопределено, УстановитьФокус = Ложь, ИскатьСНачала = Ложь) Экспорт 
	...
	КонецЕсли;
	Результат = ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(ФормаВладелец, ПолеТекста, СтрокаПоиска, СловоЦеликом, ИскатьСНачала);
...
Процедура УстановитьГраницыВыделения() Экспорт 
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
	#КонецЕсли
	//Структура = ПолеТекста.ДвухмерноеВыделение();
	//Если Ложь
