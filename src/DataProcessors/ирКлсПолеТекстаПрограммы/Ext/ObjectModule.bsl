...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, Знач ПрименитьОжидаемыйТип = Истина, выхЕстьЛучшееСлово = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Иначе
		...
		ЛиВнутриВыражения = Истина
			...
			И ЗначениеЗаполнено(ирОбщий.ПервыйФрагментЛкс("" + мТекущаяСтрокаНачало + мТекущаяСтрокаКонец, мКонтекст));
		Попытка
			Для Каждого СтруктураКонтекста Из мТаблицаТиповКонтекста Цикл
				ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураКонтекста, мЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы, ЭтоГруппаОбщихМодулей,,
					...
					ДобавлятьЛокальныйКонтекст, мНаборыСлов, БазовоеРасширениеКонфигурации,,, ЛиВнутриВыражения); //  Мультметка481922915 ДобавлятьЛокальныйКонтекст Или ЛиВнутриКомментария 
			КонецЦикла;
	...
	Если ПрименитьОжидаемыйТип <> Ложь Тогда
		...
		Тогда
			...
			Если ВариантыИмениПеременной <> Неопределено Тогда
				...
				Для Каждого Имя Из ВариантыИмениПеременной Цикл
					СтрокаСлова = мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, Имя + " =", "Конструкция",,,,,, Истина);
		...
		Тогда
			...
			мПлатформа.ДобавитьВТаблицуСлов(ТаблицаСлов, "Дата(1,1,1)", "Конструкция",,,,,, Истина);
...
Функция ОбозначитьСловоСОжидаемымТипомВТаблицеСлов(Знач КлючПоиска, Знач НаборыСлов, Знач КлючОсновногоНабора, Знач КромеСлова = "", Знач МножительРейтинга = 3, Знач НечеткийПоискПоСлову = "",
	...
	Для Каждого КлючИЗначение Из НаборыСлов Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			КонецЕсли;
			УстановитьРейтингВСтрокеСлова(СтрокаЛучшегоСлова, МножительРейтинга);
			СловоНайдено = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьРейтингВСтрокеСлова(Знач СтрокаЛучшегоСлова, Знач МножительРейтинга, Знач ВключатьАвтоОткрытие = Истина) Экспорт
	НовыйРетинг = СтрокаЛучшегоСлова.Рейтинг + МножительРейтинга * ШагРейтингаСлова();
	Если МножительРейтинга < 0 Или НовыйРетинг > СтрокаЛучшегоСлова.Рейтинг Тогда
		СтрокаЛучшегоСлова.Рейтинг = НовыйРетинг;
	КонецЕсли;
	Если Ложь
		Или мСтрокаЛучшегоСлова = Неопределено И ВключатьАвтоОткрытие И МножительРейтинга > 0
		Или мСтрокаЛучшегоСлова <> Неопределено И мСтрокаЛучшегоСлова.Рейтинг < СтрокаЛучшегоСлова.Рейтинг
	Тогда
...
Функция ВставитьВыбранноеСловоГенератор(Знач СтрокаРезультата, Знач ДляАдаптера = Ложь, ВызватьАвтодополнениеСнова = Ложь, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	Если Слово = СловоГенераторКодаЗаполненияСвойств() Тогда
		СловаРодителя = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаКонтекста,,,, Ложь,, "Свойство",,,,,,,,,, Истина);
	...
	ИначеЕсли Слово = СловоГенераторКодаСопоставитьСвойства() Тогда
		...
		СловаЛевые = мПлатформа.ТаблицаСловИзСтруктурыТипа(ТаблицаТиповЛевая[0], мЯзыкПрограммы,,,,, "Свойство",,,,,,,,,, Не ЛиЛогическоеВыражение);
		Если СловаЛевые.Количество() Тогда
			...
			Для Каждого СтрокаСловаЛевого Из СловаЛевые Цикл
				...
				Если МассивВставки.Количество() = 0 Тогда
					...
					КонецСтроки = УказательКаретки + КонецСтроки;
				Иначе
					Строка = НепечатноеНачалоСтроки + ИмяЛевогоОбъекта + "." + СтрокаСловаЛевого.Слово + " = " + мРодительскийКонтекст + ".";
				КонецЕсли;
				ПодобранноеСлово = "";
				Если мСтрокаЛучшегоСлова <> Неопределено Тогда
	...
	Иначе
		...
		Тогда
			...
			Если ПараметрЗакрытияПодсказки = Истина Тогда
				ТаблицаМетодовМодуля = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипаКонтекста, мЯзыкПрограммы,,, Истина,, "Метод", мФлагиКомпиляции, Слово, МодульМетаданныхКонтекста(СтруктураТипаКонтекста),,,
...
Функция Адаптер_ПриВыбореСтрокиАвтодополнения(Знач Слово, Знач ЛиМетод, Знач КлючНабораСлов, Знач Шаблон) Экспорт 
	...
	Тогда 
		СтрокаТаблицыСлов = СтрокаСловаИзСловаИНабора(Слово, ЛиМетод, КлючНабораСлов);
		Если СтрокаТаблицыСлов <> Неопределено Тогда
			МойШаблон = ВставитьВыбранноеСловоГенератор(СтрокаТаблицыСлов, Истина);
		КонецЕсли;
	КонецЕсли;
	Возврат МойШаблон;
КонецФункции
...
#КонецЕсли
// Тут инициируем только внутренние переменные. А экспортные - в ИнициироватьНеинтерактивно
