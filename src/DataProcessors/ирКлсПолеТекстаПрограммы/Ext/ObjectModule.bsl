...
Функция НайтиЗапросВременнойТаблицы(Знач ТекущееСлово, Знач МенеджерВременныхТаблиц = Неопределено, выхПакетЗапросовТекущий = Неопределено, выхИндексЗапроса = Неопределено) Экспорт
	...
	КонецЕсли;
	Если выхИндексЗапроса <> Неопределено Тогда
		ТекстыЗапросов = выхПакетЗапросовТекущий.ТекстыЗапросов; 
		...
		ТекстЗапроса = ТекстыЗапросов[ТекстыЗапросов.ВГраница()];
		Если Найти(ТекстЗапроса, "ИНДЕКСИРОВАТЬ ПО") = 0 Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ " + ТекущееСлово, "");
			ТекстыЗапросов[ТекстыЗапросов.ВГраница()] = ТекстЗапроса;
		КонецЕсли;
		
		ПостроительЗапроса = мПлатформа.ПостроительПакетаЗапросовДоИндекса(ТекстыЗапросов, выхИндексЗапроса);
		Если Истина
...
Функция ВычислитьТипЗначенияВыражения(Знач Выражение = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	...
	Иначе                        
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда
				...
				Если ТаблицаТиповКэш = Неопределено И ЗначениеЗаполнено(ДляСвойства) Тогда
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ПрисвоенныйТип <> Неопределено Тогда 
								Если Лев(ПрисвоенныйТип, 1) = "@" Тогда 
									Если мПлатформа.мРежимПроверкиМодуля = Истина Тогда 
								...
								Тогда
									...
									Если ТаблицаТиповИзКомментария.Количество() > 0 Тогда
										...
										Тогда
											СтруктураМоноТипа = 0; // см. мПлатформа.НоваяСтруктураТипа()
											мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповИзКомментария,, Истина, СтруктураМоноТипа);
											ДобавитьЕдинственныйТип(ТаблицаТипов, СтруктураМоноТипа);
										Иначе 
	...
	Если СобиратьОшибки Тогда 
		...
		Тогда 
			...
			Тогда 
				СобиратьОшибки = Ложь;
				ПозицияНачалаМетода = Неопределено;
				ОшибкаМодуля = ДобавитьОшибкуМодуля(ПозицияНачалаМетода);
...
Функция ДобавитьОшибкуМодуля(ПозицияНачалаМетода = Неопределено) Экспорт
...
Процедура СобратьДинамическиеСвойстваОбъекта(Знач Выражение, Знач ТаблицаТипов, ТекстДляПоискаОпределения = "", Знач ЭтоВызовКонструктора = Ложь, Знач ПолныйАнализСоставаТипов = Истина, Знач ГлобальныйПоискВМодуле = Ложь,
	...
	Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
		...
		Тогда
			...
			ШаблонПоиска = шПредИмяПрямое + Выражение + "\s*\.\s*Колонки\s*.\s*(?:Добавить\s*\(|Вставить\s*\((?:" + шИмя + "|\d+),)\s*" 
				+ "(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")\s*(?:,\s*(" + шПростоеВыражениеПрограммы + ")|,|\))" 
				+ "(?:\s*,\s*(?:" + шСтрокаПрограммы + "|" + шСкобкиЛенивые + "|[^\)""])*)?(?:\)\s*)?" + шНазначениеТипаПриПрисвоении;
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				КонецЕсли;
			КонецЦикла; 
			Если НовыеМетаданныеТаблицы.Количество() > 0 И НовыеМетаданныеТаблицы <> МетаданныеТаблицы Тогда
				Если Ложь
		...
		Тогда
			...
			ИскомоеВыражение = Выражение + "\.Текст";
			ШаблонПоискаТекста = ШаблонНазначенияТипаВыражению(ИскомоеВыражение);
			ВхожденияТекста = НайтиВхожденияРегВыраженияКэш(ШаблонПоискаТекста, ГлобальныйПоискВМодуле, ТекстДляПоискаОпределения, "НазначениеТипа;" + ИскомоеВыражение);
			Для Каждого ВхождениеТекста Из ВхожденияТекста Цикл
				...
				КонецЕсли;
				КоординатыТекста = Неопределено; // см. НовыеКоординатыВыражения()
				ТекстПакета = ВычислитьЗначениеВыражения(ПрисвоенноеВыражение, ТекстДляПоискаОпределения, ГлобальныйПоискВМодуле, ПозицияПоискаТекстаОт, ПозицияПоискаТекстаДо, КоординатыТекста);
				Если ЗначениеЗаполнено(ТекстПакета) Тогда
					ПакетЗапросов = НовыйПакетЗапросов(КоординатыТекста.Выражение, КоординатыТекста);
					МенеджерВременныхТаблиц.ПакетыЗапросов.Добавить(ПакетЗапросов); 
...
Процедура СобратьОперацииМенеджераВременныхТаблиц(Знач МенеджерВременныхТаблиц, Знач ВыражениеЗапроса, Знач Позиция0ВМетодеОт, Знач Позиция0ВМетодеДо, Знач ДляСвойстваПуть,
	...
	ЛиНовыйКэшВыражений = ирОбщий.СтрокиРавныЛкс(ДляСвойстваПуть, "МенеджерВременныхТаблиц");
				...
				Для Счетчик = 2 По ТекстОписания.КоличествоСтрок() Цикл
					...
					Тогда
						...
						Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяСвойства) Тогда 
							Если ЗначениеЗаполнено(ДляСвойства) И Не ирОбщий.СтрокиРавныЛкс(ДляСвойства, ИмяСвойства) Тогда
								Продолжить;
							КонецЕсли;
...
Функция ВычислитьВозможныеТекстовыеЗначенияПараметра(Знач ИмяПараметра, КонтекстТекстовогоЛитералаИзвестен) Экспорт
	...
	Если Не КонтекстТекстовогоЛитералаИзвестен Тогда
		...
		КонецЦикла;
	КонецЕсли;
	ШаблонПоиска = шПредИмяПрямое + "(?:(?:Если|ИначеЕсли|И|Или|If|ElseIf|And|Or)\s|\(|,)\s*" + ИмяПараметра + "\s*=\s*(" + шСтрокаПрограммы + ")";
...
//  Строка - 
//
Функция ИмяПараметраВызоваМетода(СтрокаОписания, Знач СТипомСтрока = Истина) Экспорт
	ИмяПараметра = "";
		...
		Для Каждого Вхождение Из Вхождения Цикл
			...
			Если ВыражениеЛитерала <> Неопределено Тогда 
				...
				Тогда
					...
					Тогда
						ПолеТекста.УстановитьГраницыВыделения(ПозицияВхождения0 + 2, ПозицияВхождения0 + 2);
						//КончитьОбработкуКоманды();
						//РазобратьТекущийКонтекст(,, Истина,,,, ПозицияВхождения0 + 2);
						ПолучитьГраницыВыделения();
						ПрочитатьНачалоИКонецТекущейСтроки("");
						РазобратьКонтекстСтроки(,, Истина);
					КонецЕсли;
					ПозицияВхождения0 = ПозицияВхождения0 - (мПозицияТекстаДляПоискаОпределения - 0);
					ТаблицаТипов = ДобавитьВозможнуюОшибкуПослеТочки(ВложенныеВыраженияШаблон, ТекстЛитерала, ПозицияВхождения0 + 1);
					Если Истина
						И мНомерПараметра > 0
						И НетТочки 
						И ТаблицаТипов.Количество() > 0
					Тогда
						...
						Тогда
							СтрокаОшибки = ДобавитьОшибкуМодуля();
							...
							СтрокаОшибки.Позиция = Вхождение.FirstIndex + 2;
							СтрокаОшибки.ПозицияВМетоде = ПозицияВхождения0 - 3;
				...
				Тогда
					...
					Если ирОбщий.ЛиТекстЯзыкаЗапросовЛкс(ТекстЛитерала) Тогда
						...
						Для Каждого ВхождениеЗапроса Из ВхожденияЗапроса Цикл
							...
							ПозицияВхождения0 = ПозицияВхождения0 + мНачальнаяПозиция0ВложенногоТекста - мПозицияТекстаДляПоискаОпределения + 2;
							мТекстДляПоискаОпределения = мТекстБлока;
...
Функция ДобавитьВозможнуюОшибкуПослеТочки(Знач ВложенныеВыраженияШаблон, Знач НайденноеВыражение, Знач ПозицияВхождения0ВБлоке) Экспорт
	...
	ТаблицаТипов = ВычислитьТипЗначенияВыражения(НайденноеВыражение, мТекстДляПоискаОпределения,, Истина,,, Ложь,,,,, ПозицияВхождения0ВБлоке,,, Истина);
...
шВыражениеСТочкой = "((?:[+=]\s*)?" + шСтрокаПрограммы + "(?:\s*[+=;])?)|" + шКомментарий + "|(" + шПредИмяПрямое + ")(" + шИмяСкобкиЛенивые + "?" + шИндекс + "?(?:\." + шИмяСкобкиЛенивые + "?" + шИндекс + "?)+)";
