...
Функция ФорматироватьТекстВстроенногоЯзыка(Знач МожноВесьТекст = Истина) Экспорт 
	...
	ПерваяСтрока = ТекстДокумент.ПолучитьСтроку(1);
	...
	Если НачальнаяСтрока > 1 Тогда 
		СтрокаТекста = ПолеТекста.ПолучитьСтроку(НачальнаяСтрока - 1);
		СтрокаТекстаДоКомментария = ирОбщий.ПервыйФрагментЛкс(СтрокаТекста, "//");
		СловоНайдено = НайтиКлючевыеСловаФорматирования(СтрокаТекстаДоКомментария, ГруппыКлючевыхСлов,,,,, ВхожденияКэш);
		СтрокаТекстаБезЛитералов = СокрЛП(ЗалитьКомментарииИСтроковыеЛитералы(СтрокаТекстаДоКомментария,, Истина));
		Если Истина  
			И Не СловоНайдено
			И ЗначениеЗаполнено(СтрокаТекстаДоКомментария) 
			И Прав(СтрокаТекстаБезЛитералов, 1) <> ";" 
		Тогда 
			УровеньИнструкции = -1;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
		...
		Если ВхожденияПараметров.Количество() > ИндексПараметра Тогда
			...
			Иначе
				...
				Иначе
					Если Истина
						И ВхожденияВызова.Индекс(ВхождениеВызова) < ВхожденияВызова.Количество() - 1 
		...
		Если ВычислениеРазрешено Тогда
			...
			Тогда
				...
				Иначе 
					ТаблицаТиповРодителя = ВычислитьТипЗначенияВыражения(РодительскийКонтекст, ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа,, ПолныйАнализСоставаТипов, Ложь,,,,,
						Позиция0ВМетодеДо,, ТекущееСлово, СобиратьОшибки);
						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если Ложь
								...
								Или Истина
									...
									И ЧислоПроверенныхВариантов >= 2 // Нужно для СтруктураКлючаТаблицыБДЛкс
			...
			Тогда     
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						СобратьТипыСвойствСтруктуры(Выражение, СтруктураТипаЦикл.Метаданные, ГлобальныйПоискВМодуле, ЛиВиртПрисвоение, Позиция0ВМетодеОт, Позиция0ВМетодеДо, ДляСвойства);
...
Функция ВычислитьРеквизитФормыВЗначение(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если Не ирОбщий.ЛиИмяТипаФормыЛкс(РодительскаяСтруктураТипа.ИмяОбщегоТипа) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Тогда 
		ОписаниеРеквизита = СлужебныеДанные.Реквизиты.Найти(ИмяРеквизита, "НИмя");
		...
		Иначе 
			СтруктураТипа = мПлатформа.СтруктураТипаИзКонкретногоТипа(ОписаниеРеквизита.ОписаниеТипов.Типы()[0]);
		КонецЕсли;
	КонецЕсли;
	Результат = мПлатформа.ДобавитьВТаблицуТипов(, СтруктураТипа);
	Возврат Результат;
...
#Если Сервер И Не Сервер Тогда
	...
	ФормаВладелец = ОткрытьФорму(); // Форма
