						...
						Пока ИндексВхождения > 0 Цикл 
							...
							Если ПрисвоенныйТип <> Неопределено Тогда 
								...
								Тогда
									...
									ТаблицаТиповИзКомментария = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ПрисвоенныйТип, "-"),, ТекстДоВхождения, ДляСвойства,,,, ПозицияВхождения0);
													...
													Если ЗначениеЗаполнено(ИменаСвойств) Тогда
														...
														Для ИндексСвойства = 0 По Мин(ИменаСвойств.ВГраница(), ВыраженияЗначений.Количество() - 2) Цикл 
															...
															Тогда
																Продолжить;
															КонецЕсли;
															ТаблицаПрисвоенныхТипов = ВычислитьТипЗначенияВыражения(ВыражениеЗначения,,,,, ПолныйАнализСоставаТипов, Ложь,,,,, ПозицияВхождения0,, ДляСвойстваВниз);
		...
		Если ирОбщий.ПервыйФрагментЛкс(СтруктураТипаЦикл.ИмяОбщегоТипа, "[") = "Массив" Тогда
			...
			Если Найти(СтруктураТипаЦикл.ИмяОбщегоТипа, "[") > 0 Тогда
				...
				Если ИмяТипаЭлемента = "РезультатЗапроса" Тогда
					Продолжить;
				КонецЕсли;
				СтруктураТипаЦикл.Метаданные = мПлатформа.ДобавитьВТаблицуТипов(СтруктураТипаЦикл.Метаданные, мПлатформа.НоваяСтруктураТипа(ИмяТипаЭлемента));
			КонецЕсли;
			...
			Для Каждого ВхождениеСвойства Из Вхождения Цикл
				...
				Если ирОбщий.ЛиИмяПеременнойЛкс(ТекстовоеЗначение) Тогда
					...
					Если Не мПлатформа.ЛиДетальностьТиповДостаточна(ТаблицаТиповКолонки) Тогда
						ОписаниеТипов = ВычислитьЗначениеВыражения(ВхождениеСвойства.SubMatches(1),,, Позиция0ВМетодеОт, ПозицияСвойства);
						Если ЗначениеЗаполнено(ОписаниеТипов) Тогда
							...
							Если ЗначениеЗаполнено(ТаблицаТиповКолонки) Тогда
								ТаблицаТиповКолонки.ЗаполнитьЗначения(НовоеОписаниеДинамическогоСвойства(, ПозицияСвойства + 10), "СтрокаОписания");
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					Если ЗначениеЗаполнено(ТаблицаТиповКолонки) Тогда
		...
		Если Вхождения.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из СвойстваЧерезТочкуИзТекста Цикл
				...
				Если Истина
					...
					И (Ложь
						Или ТипЗнч(ЗначениеСвойства) <> Тип("ТаблицаЗначений")
						Или ЗначениеСвойства.Колонки.Найти(ИмяКолонкиФлагаТаблицыТипов) = Неопределено)
				Тогда
					Продолжить;
				КонецЕсли;
				Если Истина
...
Функция ВычислитьТипыЭлементаСтруктурыПоВхождению(Знач Вхождение, Знач ГлобальныйПоискВМодуле = Ложь, Знач ДляСвойстваВниз = "") Экспорт
	...
	Если Не ЛиДетальностьТиповДостаточна Тогда
		...
		ИначеЕсли ВыражениеЗначения <> Неопределено Тогда 
			...
			Иначе
				ТаблицаТиповЗначения.ЗаполнитьЗначения(НовоеОписаниеДинамическогоСвойства(, ПозицияСвойства + 10), "СтрокаОписания");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
			...
			Пока СчетчикСтрок <= ТекстОписания.КоличествоСтрок() Цикл
				...
				Если Фрагменты.Количество() > 1 Тогда
					...
					//Если Прав(СтрокаОписания, 1) = ":" Тогда
						ТипыЭлементовКоллекции = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(Фрагменты[1], "]"), Описание, ТекстДоВхождения, ДляСвойстваПуть, СчетчикСтрок,, Истина, Позиция0ВМетоде);
					//КонецЕсли; 
					...
					ИначеЕсли Лев(СтрокаОписания, ДлинаПрефикса) = ПрефиксУровня Тогда
						...
						Если ирОбщий.ЛиИмяПеременнойЛкс(ИмяСвойства) Тогда 
							...
							Если ФрагментыОписания.Количество() > 1 Тогда
								Если Прав(СтрокаОписания, 1) = ":" Тогда
									СчетчикСтрок = СчетчикСтрок - 1;
									МетаданныеСвойства = ТаблицаТиповИзТекста(ирОбщий.ПервыйФрагментЛкс(ФрагментыОписания[1], ":"), Описание, ТекстДоВхождения, ДляСвойстваВниз, СчетчикСтрок, Истина,, Позиция0ВМетоде);
								Иначе
									Попытка 
									...
									КонецПопытки;
									Если ЗначениеЗаполнено(МетаданныеСвойства) Тогда
										ОписаниеСвойства = Неопределено;
										Если ФрагментыОписания.Количество() > 2 Тогда
											ОписаниеСвойства = ФрагментыОписания[2];
										КонецЕсли;
										МетаданныеСвойства.ЗаполнитьЗначения(НовоеОписаниеДинамическогоСвойства(ОписаниеСвойства, Позиция0ВМетоде), "СтрокаОписания");
									КонецЕсли;
								КонецЕсли;
...
Функция НовоеОписаниеДинамическогоСвойства(Знач Описание, Знач Позиция0ВМетоде = 0) Экспорт
	Результат = Новый Структура("Описание, Координаты, СтрокаОписания", Описание, НовыеКоординатыВыражения(, Позиция0ВМетоде));
	Возврат Результат;
КонецФункции
