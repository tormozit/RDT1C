...
Функция ТекущийВызовМетода()
	...
	СлужебноеПолеТекста.УстановитьТекст(мОригинальныйТекст);
	ПредшествующийТекст = "";
	СледующийТекст = "";
	...
	МаксНомерСтроки = Мин(СлужебноеПолеТекста.КоличествоСтрок(), мКонечнаяСтрока + МаксЧислоСтрокАнализировать);
	Если Истина
		И МаксНомерСтроки > 0
...
//  НомерКолонки		 - 	 - 
//
Процедура РазобратьТекущийКонтекст(Знач ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь, Знач КакВызовМетода = Неопределено, Знач НомерСтроки = 0, Знач НомерКолонки = 0) Экспорт 
	...
	Если ОригинальныйТекстИзменен Тогда
		мТекстБезКомментариевИОпасныхСтрок = ЗалитьКомментарииИСтроковыеЛитералы(мОригинальныйТекст);
	КонецЕсли;
	...
	Если мКонечнаяСтрока > 1 Тогда
		...
		СлужебноеПолеТекста.УстановитьГраницыВыделения(1, 1, мКонечнаяСтрока - 1, 333);
		мПредшествующийТекст = СлужебноеПолеТекста.ВыделенныйТекст;
		СлужебноеПолеТекста.УстановитьГраницыВыделения(1, 1, мКонечнаяСтрока, мКонечнаяКолонка);
		ПозицияВТексте = СтрДлина(СлужебноеПолеТекста.ВыделенныйТекст);
		СлужебноеПолеТекста.УстановитьТекст(мТекстБезКомментариевИОпасныхСтрок);
		СлужебноеПолеТекста.УстановитьГраницыВыделения(1, 1, мКонечнаяСтрока - 1, 333);
	...
	Если ЯзыкПрограммы = 0 Тогда
		...
		КонецЕсли;
		Если мМодульМетаданных.Методы.Количество() = 0 Тогда
		...
		Иначе
			СлужебноеПолеТекста.УстановитьТекст(мОригинальныйТекст);
			ЗагрузитьМетодМодуляПоПозиции(ПозицияВТексте); // устанавливает мМетодМодуля
			Если мМетодМодуля <> Неопределено Тогда 
				мПозицияТекстаДляПоискаОпределения = мМетодМодуля.ПозицияТела + 1;
				ТекстБлока = мМетодМодуля.Тело;
				ЧислоСтрокВБлоке = ирОбщий.СтрЧислоСтрокЛкс(ТекстБлока);
				мПерваяСтрокаТелаМетодаМодуля = СтрЧислоСтрок(Лев(мОригинальныйТекст, мМетодМодуля.ПозицияТела + 1) + "й"); 
				мПоследняяСтрокаТелаМетодаМодуля = мПерваяСтрокаТелаМетодаМодуля + ЧислоСтрокВБлоке - 1;
				НомерПервойСтрокиБлока = мПоследняяСтрокаТелаМетодаМодуля; // Для единообразия с ветокой языка запросов
				Если мМетодМодуля.Параметры <> Неопределено Тогда
	...
	Иначе
		...
		ЗапросыПакета = ирОбщий.РазбитьГрубоТекстПакетногоЗапросаНаЗапросыЛкс(мОригинальныйТекст, СтрДлина(мПредшествующийТекст), ПозицииЗапросовПакета);
		#Если Сервер И Не Сервер Тогда
	...
	Если НомерТекущейСтрокиВБлоке > 1 Тогда
		мТекстДляПоискаОпределения = ТекстБлока;
		мТекстБезКомментариевИОпасныхСтрок = ТекстБлока;
...
Процедура ЗагрузитьМетодМодуляПоПозиции(Знач ПозицияВТексте) Экспорт 
	...
	Для Каждого СтрокаМетода Из мМодульМетаданных.Методы Цикл
		Если СтрокаМетода.ПозицияТела + 1 >= ПозицияВТексте Тогда
			Прервать;
		КонецЕсли; 
...
Функция РедактироватьВКонсолиЗапросов() Экспорт
	...
	Текст = ПолеТекста.ПолучитьТекст();
	Если Истина
		И мПлатформа.РедактироватьЗапрос(Текст) <> Неопределено 
...
Функция ТекущаяПозицияВТекстеОдномерная()
	...
	ДлинаТекстаДо = СтрДлина(мПредшествующийТекст); // Раньше тут добавлялась 1 якобы для срабатывания на однострочном литерале
	Если мМетодМодуля <> Неопределено Тогда
...
Процедура ПерейтиКОпределению(НомерСтроки = 0, НомерКолонки = 0, ТолькоТипы = Ложь) Экспорт 
	...
	ИначеЕсли ЯзыкПрограммы = 0 Тогда 
		Если мМетодМодуля <> Неопределено Тогда
			СтрокаПараметра = мМетодМодуля.Параметры.Найти(НРег(мКонтекст), "НИмя");
			Если СтрокаПараметра <> Неопределено Тогда
				мКонтекст = мМетодМодуля.Имя + "(";
				ПерейтиКОпределению();
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Функция НайтиОпределениеСлова(Слово, ПоследнееОпределение = Неопределено, ПоследнееВхождение = Неопределено) Экспорт 
	...
	Если ЯзыкПрограммы = 1 Тогда
		...
		Если РезультатТекста.Количество() > 0 Тогда
			...
			Тогда
				...
				РезультатТекста = РегВыражение.НайтиВхождения(мТекстБезКомментариевИОпасныхСтрок);
				Если РезультатТекста.Количество() > 0 Тогда
	...
	Иначе
		...
		Если ирОбщий.СтрКончаетсяНаЛкс(Слово, "(") Тогда
			...
			РегВыражение.Pattern = ирОбщий.СтрЗаменитьЛкс(мПлатформа.ШаблоныДляАнализаВстроенногоЯзыка().ОписаниеМетодаЧистое, шИмя, ЭкранированноеСлово);
			РезультатТекста = РегВыражение.НайтиВхождения(мОригинальныйТекст);
		Иначе
...
Функция ОпределитьТипДочернегоКонтекста(МассивРодительскихСтрутурТипа, ТекущееСлово, ТипСлова, ТекущийИндекс = Неопределено, ТекущиеАргументы = "", ПредшествующийТекст = "",
	ПолныйАнализ = Истина, Знач ЗапрещенныеМетоды = Неопределено)
		...
		Если РодительскаяСтруктураТипа.ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
			...
			Если Не ЗначениеЗаполнено(ТекущийИндекс) Тогда
				...
				Тогда
					...
					Если НайденныеСтроки.Количество() > 0 Тогда
						ТаблицаСтруктурТиповИсточник = НайденныеСтроки[0].ТаблицаСтруктурТипов; 
						Если мМетодМодуля <> Неопределено Тогда
							СтрокаПараметраМетода = мМетодМодуля.Параметры.Найти(НРег(ТекущееСлово), "НИмя");
							Если СтрокаПараметраМетода <> Неопределено Тогда
								Если ЗапрещенныеМетоды = Неопределено Тогда
									ЗапрещенныеМетоды = Новый Структура;
								КонецЕсли;
								ЗапрещенныеМетоды.Вставить(мМетодМодуля.Имя);
								ШаблонПараметра = "(" + шВыражениеПрограммы + ")?" + шРазделитель + "*(?:$|,|\))";
								ИндексПараметра = мМетодМодуля.Параметры.Индекс(СтрокаПараметраМетода);
								ВхожденияВызова = ирОбщий.НайтиРегВыражениеЛкс(мОригинальныйТекст, ШаблонВызоваМетода(мМетодМодуля.Имя),,,,,, Истина); // Без подгрупп для ускорения
								#Если Сервер И Не Сервер Тогда
								...
								Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
									...
									Если ВхожденияПараметров.Количество() > ИндексПараметра Тогда
										...
										ПолеТекстаПрограммыВызова.РазобратьТекущийКонтекст(,,, ЧислоСтрок + 1, 1);
										Если Не ЗапрещенныеМетоды.Свойство(ПолеТекстаПрограммыВызова.мМетодМодуля.Имя) Тогда
											ИмяВиртуальнойПеременной = "Зщшмеухя";
											ТаблицаСтруктурТиповИсточник = ПолеТекстаПрограммыВызова.ОпределитьТипЗначенияКонтекста(ИмяВиртуальнойПеременной, 
												" " + ПолеТекстаПрограммыВызова.мТекстДляПоискаОпределения + ";" + Символы.ПС + ИмяВиртуальнойПеременной + "=" + ФактическийПараметр + ";",
												ПолеТекстаПрограммыВызова.мПредшествующийТекст,,,, ЗапрещенныеМетоды);
											Если ТаблицаСтруктурТиповИсточник[0].ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
												//ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ТаблицаСтруктурТиповИсточник, ТаблицаСтруктурТипов); 
				...
				мПлатформа.СвойстваТипаПредопределенные(РодительскаяСтруктураТипа, ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, ЯзыкПрограммы, Конфигурация,,, ТаблицаСвойствТипа);
			...
			мПлатформа.СвойстваТипаМетаданные(РодительскаяСтруктураТипа,
				ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, ЯзыкПрограммы, ТекущийИндекс,,, ЛиСерверныйКонтекст, МодульМетаданныхКонтекста(РодительскаяСтруктураТипа), ТаблицаСвойствТипа);
			Если ТаблицаСвойствТипа.Количество() > 0 Тогда
				Для Каждого СтрокаСлова Из ТаблицаСвойствТипа Цикл
...
Функция ОпределитьТипЗначенияКонтекста(Знач Контекст = "", Знач ТекстДляПоискаОпределения = "", Знач ПредшествующийТекст = "", Знач РазрешитьАнализИмениТипа = Ложь,
	Знач ЭтоВызовКонструктора = Ложь, ПолныйАнализ = Истина, Знач ЗапрещенныеМетоды = Неопределено) Экспорт
	...
	Иначе
		...
		Если Результат.Количество() > 0 Тогда 
			...
			Иначе
				...
				Если РезультатТекста.Количество() > 0 Тогда
					...
					Иначе
						...
						ТаблицаСтруктурТипов = мПлатформа.СвойстваТипаМетаданные(ТаблицаСтруктурТиповКоллекции[0], "[0]",,, ЯзыкПрограммы, "0")[0].ТаблицаСтруктурТипов;
					КонецЕсли;
					Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ТекущийИндекс) Тогда
			...
			КонецЕсли;
			МассивДочернихСтруктурТипа = ОпределитьТипДочернегоКонтекста(МассивРодительскихСтрутурТипа, ТекущееСлово, ТипСлова, ТекущийИндекс, ТекущиеАргументы, ПредшествующийТекст, ПолныйАнализ, ЗапрещенныеМетоды);
			...
			Если ТипСлова = "Свойство" И Не СтруктураТипа.Конструктор Тогда
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаСтруктурТипов Цикл
					...
					Тогда
						ШаблонУстановкаТекста = Контекст + "\s*\.\s*Добавить\s*\(\s*(.*?)\s*\)";
						РегВыражение.Pattern = ШаблонУстановкаТекста;
						РегВыражение.Global = Истина;
						РезультатСвойств = РегВыражение.НайтиВхождения(ТекстДляЗаполненияМетаданных);
						...
						Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
							...
							КонецЕсли;
							Счетчик = Счетчик + 1;
							ТаблицаСтруктурТиповЭлементов = ОпределитьТипЗначенияКонтекста(ВхождениеСвойства.SubMatches(0), ТекстДляПоискаОпределения, ПредшествующийТекст, РазрешитьАнализИмениТипа, ЭтоВызовКонструктора, ПолныйАнализ);
...
Функция ЗаполнитьТаблицуСлов(ТаблицаСтруктурТиповКонтекста = Неопределено, выхКонкретныйТип = Неопределено, выхСтруктураТипаКонтекста = Неопределено, РазрешитьОткрытиеОкон = Истина,
	...
	Если мЭтоСтроковыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда 
		...
		Тогда 
			...
			ИначеЕсли Истина
				И ВыбраннаяСтруктураТипа.СтрокаОписания.Слово = "УстановитьПараметр" 
				И ИмяПараметра = "Имя"
	...
	Тогда 
		...
		#Если Сервер И Не Сервер Тогда
			ПолеТекстаПрограммыВызова = Обработки.ирКлсПолеТекстаПрограммы.Создать();
		#КонецЕсли
		ПолеТекстаПрограммыВызова.РазобратьТекущийКонтекст(,, Истина, мНачальнаяСтрока, мНачальнаяКолонка - СтрДлина(мВызовМетода + мНачалоСлова));
		Если Истина
			И ПолеТекстаПрограммыВызова.мИмяМетодаВызова = "Добавить" 
			И ПолеТекстаПрограммыВызова.мНомерПараметра = 1
		Тогда 
			ТаблицаСтруктурТипов = ПолеТекстаПрограммыВызова.ОпределитьТипЗначенияКонтекста(ПолеТекстаПрограммыВызова.мВызовМетода, ПолеТекстаПрограммыВызова.мТекстДляПоискаОпределения);
	...
	Попытка
		ВыгрузкаТаблицыСлов = мПлатформа.ТаблицаСловИзСтруктурыТипа(выхСтруктураТипаКонтекста, ЯзыкПрограммы, Конфигурация, ВнешниеФункцииКомпоновкиДанных, ВычислятьТипы,,,
...
Функция КопияКомпоненты()
	...
	#КонецЕсли
	КопияКопоненты.ИнициализироватьНеинтерактивно(ЯзыкПрограммы);
	КопияКопоненты.мМодульМетаданных = мМодульМетаданных;
	КопияКопоненты.ПолеТекста = ПолеТекста;
	КопияКопоненты.мСтарыйОригинальныйТекст = мОригинальныйТекст;
...
Процедура ВставитьВыбранноеСловоАвтодополнения(СтрокаРезультата, КонкретныйТип, СтруктураТипаКонтекста, ТаблицаСтруктурТиповКонтекста = Неопределено, ПараметрЗакрытияПодсказки = Истина) Экспорт 
	...
	Если Не мЭтоСтроковыйЛитерал И СтрокаРезультата.ТипСлова = "Метод" Тогда
		...
		Если ПараметрЗакрытияПодсказки = Истина Тогда
			ТаблицаМетодовМодуля = мПлатформа.СвойстваТипаМетаданные(СтруктураТипаКонтекста, СтрокаРезультата.Слово, "Метод",,,,,,, МодульМетаданныхКонтекста(СтруктураТипаКонтекста));
...
Функция УточнитьТипЗначенияВСтрокеТаблицыСлов(Знач СтруктураТипаКонтекста, Знач ТекущаяСтрока, Знач ВернутьСтруктуруТипа = Ложь) Экспорт 
	...
	Если НужноУточнитьТип Или ВернутьСтруктуруТипа Тогда
		...
		Если ТекущаяСтрока.Определение = "Статистический" Тогда
			ТаблицаСтруктурТипов = ОпределитьТипЗначенияКонтекста(ТекущаяСтрока.Слово, мТекстДляПоискаОпределения);
		Иначе
...
Процедура ЗаполнитьЛокальныеСвойстваИМетодыПоТексту(ОчиститьТаблицуСлов = Истина, ВключатьКонструкции = Истина, УчитыватьРодительскийКонтекст = Ложь, ОбновитьТекстДляАнализа = Истина,
	...
	Если ВключатьГлобальныйКонтекст Тогда
		...
		ТаблицаГлобальногоКонтекста = мПлатформа.ТаблицаСловИзСтруктурыТипа(СтруктураТипа);
	...
	КонецЕсли;
	ВременныйТекст = РегВыражение.Заменить(мТекстБезКомментариевИОпасныхСтрок, "");
		...
		ИначеЕсли Подгруппа3 <> Неопределено Тогда 
			...
			Если Истина
				...
				И (Ложь
					...
					Или мРодительскийКонтекст = "")
			Тогда 
				// Подмена контекста нужна, чтобы избежать бесконечной рекурсии через вызов КонсольКода.ЗаполнитьПараметрыВыхода в режиме внешней обработки
