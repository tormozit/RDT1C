...
Функция ВыделитьМетод(Знач Параметры = Неопределено, Знач ИмяВременногоМодуля = "") Экспорт 
	...
	ВыходныеПараметрыМатеринскогоМетода = ирОбщий.ВыгрузитьСвойствоЛкс(мПлатформа.ПараметрыМетодаМодуля(мМетодМодуля),, Новый Структура("Знач", Ложь));
...
Функция ТаблицаТиповИзТекста(Знач ТекстТипа, Знач Описание = "", Знач ТекстДоВхождения = "") Экспорт
	...
	Иначе
		...
		Если ЭтоИмяТипа Тогда
			Фрагменты = ирОбщий.СтрРазделитьЛкс(СтрЗаменить(ТекстТипа, "??,", ""), " из ", Истина,,,, Истина);
					...
					Иначе 
						...
						Иначе
							...
							Иначе
								...
								Тогда
									СтруктураТипа = ТаблицаТиповИзТекста(ИмяТипа, Описание, ТекстДоВхождения);
								Иначе 
									Продолжить;
								КонецЕсли;
							КонецЕсли;
		...
		Иначе
			...
			Если СтруктураТипа = Неопределено Тогда 
				...
				Если ТаблицаТипов.Количество() = 0 И Найти(ТекстТипа, "(") = 0 Тогда
					ТекстТипа = ТекстТипа + "(";
					ТаблицаТипов = ВычислитьТипЗначенияВыражения(ТекстТипа, ТекстДоВхождения, ТекстДоВхождения,,,, Ложь);
				КонецЕсли;
			Иначе
