				...
				Если ПараметрыМетода <> Неопределено Тогда
					...
					Если СтрокаПараметраМетода <> Неопределено Тогда
						...
						Если СтрокаПараметраМетода.ТаблицаТипов = Неопределено Тогда
							ВычислитьТипПараметраМетода(ПараметрыМетода, мРекурсивныйПуть.ВходящиеВызовы, СтрокаПараметраМетода, ТаблицаТипов, ДляСвойства);
						КонецЕсли; 
...
//    ТипВыхода - Строка - Служебный параметр для перехода после вызова метода
Функция ВычислитьТипПараметраМетода(Знач ПараметрыМетода, Знач ПройденныеВызовы, Знач СтрокаПараметраМетода, Знач ТаблицаТипов, Знач ДляСвойства = "")
				...
				Для Каждого СтруктураТипаЦикл Из ТаблицаТипов Цикл
					...
					Тогда
						...
						Для Каждого ВхождениеСвойства Из Вхождения Цикл
							...
							Для Каждого СтруктураТипаКоллекции Из ТаблицаТиповЭлементов Цикл
...
Функция СобратьТипыСвойствСтруктуры(Знач Выражение, Структура = Неопределено, Знач ГлобальныйПоискВМодуле = Ложь, Знач ТолькоПрисвоение = Ложь, Знач Позиция0ВМетодеОт = 0, Знач Позиция0ВМетодеДо = 0,
	...
	НашлиСоздание = Ложь;
	ШаблонПоиска = шПредИмя + Выражение + "\s*=\s*Новый\s+Структура\s*\(\s*(" + шИмяСТочками + ")"; // Теперь шСтрокаПрограммы разбираем сразу, а тут только редкий случай
	Вхождения = НайтиВхожденияРегВыраженияКэш(ШаблонПоиска, ГлобальныйПоискВМодуле);
	ИндексВхождения = Вхождения.Количество();
	Пока ИндексВхождения > 0 Цикл 
		ИндексВхождения = ИндексВхождения - 1;
	...
	КонецЕсли;
	МинДетальность = 4;
	ШаблонПоиска = шПредИмя + Выражение + "\s*\.\s*Вставить\s*\(\s*(" + шСтрокаПрограммы + "|" + шИмяСТочками + ")(?:\s*,\s*((?:" + шСтрокаПрограммы + "|" + шСкобкиЛенивые + "|[^\)""])*))?";
	Вхождения = НайтиВхожденияРегВыраженияКэш(ШаблонПоиска, ГлобальныйПоискВМодуле);
	...
	Тогда 
		...
		ШаблонПоиска = шПредИмя + "ЗаполнитьЗначенияСвойств\s*\(\s*" + ирОбщий.ПодготовитьТекстДляРегВыраженияЛкс(Выражение) + "\s*,\s*(" + шПростоеВыражениеПрограммы + ")(?=\s*[,\)])";
...
Функция ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста = Неопределено, _1 = Неопределено, _2 = Неопределено, Знач РазрешитьОткрытиеОкон = Истина,
	...
	Если мЭтоТекстовыйЛитерал И ВыбраннаяСтруктураТипа.ТипЯзыка <> "ИмяТипа" Тогда
		...
		Тогда
			...
			ИначеЕсли ТипЗнч(ВыбраннаяСтруктураТипа.Метаданные) = Тип("ПостроительЗапроса") Тогда
	...
	Тогда  
		...
		ИначеЕсли Истина
			И ЗначениеЗаполнено(мРодительскийКонтекст)
			И мПлатформа.ТаблицаОбщихТипов.НайтиСтроки(Новый Структура("БазовыйТип, ЯзыкПрограммы", ирОбщий.ПервыйФрагментЛкс(мРодительскийКонтекст), мЯзыкПрограммы)).Количество() > 0
		Тогда 
...
Функция ВычислитьКоллекцияДобавить(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	...
	Если МассивПараметров.ВГраница() < ИндексПараметраТипа Тогда
		Возврат Неопределено;
	КонецЕсли;
	ПервыйПараметр = МассивПараметров[ИндексПараметраТипа];
...
Функция ВычислитьНайтиЭлементКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ТипыКоллекции = ВычислитьТипЗначенияВыражения(МассивПараметров[0],,,,,, Ложь,,,, Истина);
...
Функция ВычислитьСкопироватьКолонкиКоллекцииЛкс(Знач РодительскаяСтруктураТипа, Знач МассивПараметров, Знач Слово = "", Знач ПредшествующийТекст = "", Знач ПозицияВМетоде = 0, Знач ТаблицаТипов = Неопределено) 
	Если МассивПараметров.Количество() < 1 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ТипыКоллекцииИсточника = ВычислитьТипЗначенияВыражения(МассивПараметров[0],,,,,, Ложь,,,, Истина);  
	Если ТипыКоллекцииИсточника.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	КоллекцияИсточник = ТипыКоллекцииИсточника[0].Метаданные;
	Попытка
		КоллекцияИсточник = ирОбщий.СкопироватьКоллекциюЛкс(КоллекцияИсточник);
	Исключение
		Возврат Неопределено;
	КонецПопытки;
	Если МассивПараметров.Количество() > 1 Тогда
		ТипыКоллекцииПриемника = ВычислитьТипЗначенияВыражения(МассивПараметров[1],,,,,, Ложь,,,, Истина);  
