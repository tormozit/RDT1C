	...
	Для Каждого ЭлементСписка Из РасширенияМодуля Цикл
		...
		Для Каждого МетодМодуля Из ТаблицаМетодов Цикл
			...
			Если ЗначениеЗаполнено(МетодМодуля.АдресГруппировки) Тогда
				СтрокаМетода.ПутьГруппировки = ирОбщий.Дерево_ПутьСтрокойЛкс(ирОбщий.Дерево_НайтиПоПутиСтрокойЛкс(МодульМетаданных.ОбластиГруппировки, "", МетодМодуля.АдресГруппировки), "Имя",, "/");
			КонецЕсли;
			Если ЗначениеЗаполнено(МетодМодуля.АдресКомпиляции) Тогда
				СтрокаМетода.ПутьКомпиляции = ирОбщий.Дерево_ПутьСтрокойЛкс(ирОбщий.Дерево_НайтиПоПутиСтрокойЛкс(МодульМетаданных.ОбластиКомпиляции, "", МетодМодуля.АдресКомпиляции), "Условие",, "/");
			КонецЕсли;
