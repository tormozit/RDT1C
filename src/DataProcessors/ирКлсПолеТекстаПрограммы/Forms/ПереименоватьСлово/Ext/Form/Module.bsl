...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	ЗаполнитьТаблицуСлов(ТаблицаТиповКонтекста,,, Ложь, Ложь); 
	Если ЛиМетод Тогда
	...
	Если Не ЛиМетод Тогда
		...
		СтрокаОбласти.ПозицияНачала = 0; // На входе позиции отсчитываются от 0, а на выходе уже от 1
		СтрокаОбласти.ПозицияКонца = СтрДлина(ОпределениеАктивногоМетодаСОписанием()) - 1;
		...
		Для Каждого СтрокаОбласти Из Области Цикл
			...
			Для Каждого Вхождение Из Вхождения Цикл
				Если Истина
					И Вхождение.ПозицияВхождения > СтрокаОбласти.ПозицияНачала
					И Вхождение.ПозицияВхождения < СтрокаОбласти.ПозицияКонца
				Тогда 
				...
				Если Истина
					...
					И Вхождение.ПозицияВхождения > СтрокаОбласти.ПозицияКонца
					И Вхождение.ПозицияВхождения < Области[0].ПозицияКонца
				Тогда 
					ПодходящееПрисвоение = НайтиПрисвоениеДляВхождения(Вхождение, ВсеПрисвоения);
					Если ПодходящееПрисвоение = Неопределено Тогда
...
Процедура ОсновныеДействияФормыПрименитьИЗакрыть(Кнопка)
	...
	ТекущаяОбласть = ЭлементыФормы.Области.ТекущаяСтрока;
	НачалоОбластиВМодуле = мПозицияТекстаДляПоискаОпределения - мСмещениеНачалаТела + 1;
	КонецОбластиВМодуле = мПозицияТекстаДляПоискаОпределения - мСмещениеНачалаТела + СтрДлина(мТекстБлока) + 1;
	Если НайтиПоказатьСловоВТексте(НовоеСлово, ШаблонПоискаСлова(НовоеСлово),, Истина, Истина,, НачалоОбластиВМодуле, КонецОбластиВМодуле) Тогда 
	...
	Если ФормаВладелец = Неопределено Тогда 
		...
		мЗаменяемыйДиапазон = Новый Структура("Начало, Конец", 1, СтрДлина(мОригинальныйТекст));
		...
		ИначеЕсли ИндексМетода <> Неопределено Тогда
			...
			Начало = СтрокаМетода.ПозицияСОписанием;
			Конец = СтрокаМетода.ПозицияСОписанием + СтрокаМетода.ДлинаСОписанием;
