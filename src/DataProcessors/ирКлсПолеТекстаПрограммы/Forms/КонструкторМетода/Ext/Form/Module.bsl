...
Процедура ДействияФормыЗаполнитьИИ(Кнопка)
	Если мМетодМодуля = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ПараметрыЗаполнения = ПолучитьФорму("ПараметрыЗаполненияМетодаИИ").ОткрытьМодально();
	Если ПараметрыЗаполнения = Неопределено Тогда
		Возврат;
	КонецЕсли;
	РезультатИИ = ГенерироватьОписаниеМетодаИИ(ОбновитьПолноеОпределение(), ШиринаГенерируемогоТекста, ПараметрыЗаполнения);
	Если РезультатИИ <> Неопределено Тогда
		Если ПустаяСтрока(Имя) Или ПараметрыЗаполнения.ЗаменятьИмяМетода Тогда
			ЭтаФорма.Имя = РезультатИИ.name;
		КонецЕсли;
		Если Истина
		...
		КонецЦикла;
	КонецЕсли;
	Если ПараметрыЗаполнения.КомментарииВКоде Тогда
...
Процедура ДействияФормыСоветоватьИИ(Кнопка)
	Если мМетодМодуля = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Результат = СоветоватьУлучшенияМетодаИИ(ОбновитьПолноеОпределение(), ШиринаГенерируемогоТекста);
