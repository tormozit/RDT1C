...
Процедура ДействияФормыЗаполнитьИИ(Кнопка)
	Если Кнопка.Картинка <> ирКэш.КартинкаПоИмениЛкс("ирОстановить") Тогда
		...
		Если ПараметрыЗаполненияМетода.ПередаватьКонтекст Тогда
			СтруктураКонтекстаИИ = Новый Соответствие;
			Если Не ЧистыйКонтекст Тогда
				...
				СтруктураКонтекстаИИ.Вставить(" домашний модуль " + мИмяМодуля, СловаМодуля);
			КонецЕсли;
			ТаблицаИменТипов = СловаМодуля.Скопировать(, "ТипЗначения");
			КопияПараметров = ТаблицаПараметров.Скопировать(, "ТипЗначения");
			ирОбщий.УдалитьСтрокиТаблицыИлиДереваПоЗначениюЛкс(КопияПараметров, "ТипЗначения", "");
			ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(КопияПараметров, ТаблицаИменТипов);
			...
			ВхожденияСловИРазделителей = ирОбщий.НайтиРегВыражениеЛкс(" " + Описание, "([" + мПлатформа.шБукваЦифра + "]+)|\s+|(\S)");
			ПредСлово = ""; 
			Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ВхожденияИменСТочками.Количество(), "Подбор типов из описания");
			Для Каждого Вхождение Из ВхожденияСловИРазделителей Цикл
				ирОбщий.ОбработатьИндикаторЛкс(Индикатор);
				Если Вхождение.Группы[1] <> Неопределено Тогда
					ПредСлово = "";
				ИначеЕсли Вхождение.Группы[0] <> Неопределено Тогда
					ТекСлово = ТРег(Вхождение.Группы[0]);
					Если СтрДлина(ТекСлово) < 3 Тогда
						Продолжить;
					КонецЕсли;
					Если Не ПустаяСтрока(ПредСлово) Тогда
						ИмяТипа = ирОбщий.ИмяТипаИзИмениПеременнойЛкс(ПредСлово + ТекСлово,,,, Истина);
						Если Найти(ИмяТипа, ".") И Найти(ИмяТипа, "ОпределяемыйТип.") = 0 Тогда
							ТаблицаИменТипов.Добавить().ТипЗначения = ИмяТипа;
						КонецЕсли;
					КонецЕсли;
					ПредСлово = ТекСлово;
				КонецЕсли;
			...
			Для Каждого КлючИЗначение Из ИменаМетаТипов Цикл
				...
				Если ирОбщий.ЛиКорневойТипПеречисленияЛкс(ирОбщий.ПервыйФрагментЛкс(ПолноеИмяМД)) Тогда
					ПроверитьДобавитьВКонтекстИИЗначенияПеречисления(ОбъектМД, СтруктураКонтекстаИИ);
				Иначе
					СтруктураКонтекстаИИ.Вставить(ПолноеИмяМД, ПоляТаблицы);
					Для Каждого ПолеТаблицы Из ПоляТаблицы Цикл
						Если ПолеТаблицы.ТипЗначения = Новый ОписаниеТипов("ТаблицаЗначений") Тогда
							...
							СтруктураКонтекстаИИ.Вставить(ПолноеИмяТЧ, ПоляТаблицыТЧ);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЦикла;
			КонтекстМетода = Новый Массив;
			Для Каждого КлючИЗначение Из СтруктураКонтекстаИИ Цикл
...
Процедура ДействияФормыСоветоватьИИ(Кнопка)
	...
	ПараметрыЗапроса.Вставить("Текст", ТекстЗапроса);
	ОткрытьЧатИИ(ПараметрыЗапроса);
КонецПроцедуры
