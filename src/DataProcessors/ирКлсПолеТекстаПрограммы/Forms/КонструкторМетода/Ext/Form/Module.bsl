...
Процедура ОбновлениеОтображения(Знач ЗаполнятьТипы = Неопределено) Экспорт
	...
	Если СтрокаДома <> Неопределено И СтрокаДома.Вход Тогда
		...
		|" + ирОбщий.СтрСоединитьЛкс(ТаблицаПараметров.Скопировать(Новый Структура("ЧерезСтруктуру", Истина)).ВыгрузитьКолонку("Имя"), Символы.ПС,,, ТекстСмещения + ИмяПараметраДом + ".Вставить(""%1"", %1);") + "
		|";
	КонецЕсли;
...
Процедура ИмяПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Имя) Тогда
		ЭтаФорма.Имя = ирОбщий.ИдентификаторИзПредставленияЛкс(Имя);
	КонецЕсли;
...
Процедура ДействияФормыЗаполнитьИИ(Кнопка)
	Если мМетодМодуля = Неопределено Тогда
		Возврат;
	КонецЕсли;
	//СтруктураОписания = ГенерироватьОписаниеМетодаИИ(мОригинальныйТекст, мМетодМодуля.Имя);
	СтруктураОписания = ГенерироватьОписаниеМетодаИИ(ОбновитьПолноеОпределение(),, ШиринаГенерируемогоТекста);
	Если СтруктураОписания <> Неопределено Тогда
		Если ПустаяСтрока(Описание) Тогда
			ЭтаФорма.Описание = СтруктураОписания.description;
		КонецЕсли;
		СтрокаРезультата = ТаблицаПараметров.Найти(Истина, "ЭтоРезультат");
		Если Истина
			И СтрокаРезультата <> Неопределено
			И ПустаяСтрока(СтрокаРезультата.Комментарий)
		Тогда
			СтрокаРезультата.Комментарий = СтруктураОписания.return_descr;
		КонецЕсли;
		Для Каждого СтруктураПараметра Из СтруктураОписания.parameters Цикл
			СтрокаПараметра = ТаблицаПараметров.Найти(НРег(СтруктураПараметра.name), "НИмя");
			Если СтрокаПараметра = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если ПустаяСтрока(СтрокаПараметра.Комментарий) Тогда
