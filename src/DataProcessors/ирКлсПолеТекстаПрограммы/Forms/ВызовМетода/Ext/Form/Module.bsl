	...
	Если ВКОбщая <> Неопределено И Не ЛиОткрытаИзКонфигуратора Тогда
		...
		Если ВводДоступен() Тогда
			Если ВКОбщая <> Null Тогда
				...
				ВКОбщая.ПереместитьОкноВПозициюКаретки(РазрешитьВыходЗаГраницыЭкрана, ирОбщий.ЛиЕстьМодальныеГруппыЛкс() Или ирКэш.ЛиСеансТолстогоКлиентаУПЛкс());
			КонецЕсли;
		...
		Тогда
			...
			Тогда
				...
				ИначеЕсли СтрокаМетода.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено Тогда
					...
					ОбновитьТипЗначенияИзТаблицыТипов(СтрокаМетода, СтрокаМетода.ТаблицаТипов, Ложь, Истина);
...
Процедура ЗагрузитьВариантМетода(НовыйТекущийВариант = Неопределено) Экспорт 
	...
	Если ЛиСвойствоИЗначение() Тогда
		...
		мНомерПараметра = 2; 
		ТаблицаОжидаемыхТипов = ТаблицаОжидаемыхТиповПараметраМетода(СтруктураТипаКонтекста, ИмяСвойстваДляЗначения);
		...
		КонецЕсли;
		мНомерПараметра = НомерПараметра;
	КонецЕсли;
	УстановитьТекущийПараметр();
...
Процедура ТаблицаПараметровПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	Если Не ЛиСвойствоИЗначение() Тогда
		НомерПараметра = мНомерПараметра;
		мНомерПараметра = ТекущаяСтрока.Владелец().Индекс(ТекущаяСтрока) + 1;
		ТаблицаОжидаемыхТипов = ТаблицаОжидаемыхТиповПараметраМетода(СтруктураТипаКонтекста, ИмяСвойстваДляЗначения,, ТекущийВариант);
