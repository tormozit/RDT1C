...
Функция ТаблицаПараметровВнутр() Экспорт
	Возврат ЗначениеВСтрокуВнутр(ТаблицаПараметров);
КонецФункции
...
Процедура ОбновитьИлиЗакрытьФорму(Знач ЭтоВызовЧерезОжидание = Ложь, Знач Необязательное = Ложь, Знач Принудительно = Ложь) Экспорт 
	...
	Если Ложь 
		...
		Или Не Открыта() 
	Тогда
		СтрокаМетода = СтруктураТипаКонтекста.СтрокаОписания; // MemberInfo {TLI.TLIApplication}, см. мПлатформа.ТаблицаКонтекстов[0], см. мПлатформа.НоваяТаблицаМетодовМодуля()[0]
		Попытка
		...
		Если Открыта() Тогда
			ЭлементыФормы.КоманднаяПанель1.Кнопки.Подменю.Кнопки.УказатьВызывающийМодуль.Доступность = Истина
				...
				И (Ложь
					...
					Или ирОбщий.СтрокиРавныЛкс(мВызовМетода, ПрямоеИмяМодуляИзПолного(СтрокаМетода.ИмяМодуля, Истина) + "." + ИмяМетода + "("));
		КонецЕсли;
		Если Истина
...
Процедура ЗагрузитьВариантМетода(НовыйТекущийВариант = Неопределено) Экспорт 
	...
	//Если Не ЛиТекущийВариантУстановленВручную Тогда
		...
		Тогда
			НайденныйВариантСинтаксиса = ПодобратьВариантСинтаксисаМетодаПоТипамПараметров(СтрокиПараметров);
			Если НайденныйВариантСинтаксиса <> Неопределено Тогда
				ЭтаФорма.ТекущийВариант = НайденныйВариантСинтаксиса;
			КонецЕсли;
		КонецЕсли;
		ЭтаФорма.ТекущийВариант = мПлатформа.ПодобратьВариантСинтаксисаМетодаПоЧислуПараметров(ТаблицаВариантов, КоличествоФактПараметров, ТекущийВариант, ЛиТекущийВариантУстановленВручную, НомерВарианта);
	...
	Возврат Ложь
		...
		Или (Истина
			...
			И (Ложь
				...
				Или ТекущийВариант = "По ключам и значениям"));
КонецФункции
