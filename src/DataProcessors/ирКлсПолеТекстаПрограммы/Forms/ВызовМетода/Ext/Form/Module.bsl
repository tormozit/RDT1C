...
Процедура ОбновлениеОтображения()
	...
	Если ВКОбщая <> Неопределено Тогда
		...
		Если ВводДоступен() Тогда
			...
			//ВосстановитьФокусВводаОткрытиемПустышкиЛкс(); // В данном случае не помогает
			//ирОбщий.ОтправитьНажатияКлавишЛкс("%"); // Так будет переключение ScrollLock
...
Процедура ЗагрузитьВариантМетода(НовыйТекущийВариант = Неопределено) Экспорт 
	...
	//КонецЕсли; 
	Если Истина
		И Не ЗначениеЗаполнено(ОписаниеМетода) 
		И ЗначениеЗаполнено(СтрокаОписания.ПутьКОписанию)
	Тогда                  
		СтрокиПараметров = мПлатформа.ТаблицаПараметровМетода(СтрокаОписания, ОтборПараметров);
	КонецЕсли; 
	Попытка
...
Процедура ПодключитьОбработчикОжиданияОбновления(Задержка = 1)
	...
	Если Не Открыта() И Не МодальныйРежим Тогда
		Возврат;
	КонецЕсли;
	#Если Сервер И Не Сервер Тогда
