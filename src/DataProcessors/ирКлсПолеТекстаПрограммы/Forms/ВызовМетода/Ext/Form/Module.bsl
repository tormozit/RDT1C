...
Процедура ПриОткрытии()
	...
	ПриОткрытии = Истина;
	ЭтаФорма.Автообновление = Автообновление И ФормаВладелец <> Неопределено;
...
Процедура ОбновитьИлиЗакрытьФорму(ЭтоВызовЧерезОжидание = Ложь, Необязательное = Ложь) Экспорт 
	...
	Иначе
		...
		КонецЕсли; 
		ТаблицаСтруктурТиповКонтекста = ВычислитьТипЗначенияВыражения(мВызовМетода, " " + мТекстДляПоискаОпределения, мПредшествующийТекст,, мЭтоКонструктор);
		мВызовМетода = "";                                    
	...
	Тогда
		...
		Если Истина
			...
			И Не (Истина 
				...
				И СтараяСтрокаОписания.ПозицияОпределения = СтрокаОписания.ПозицияОпределения)
		Тогда
			// Продублировано в ТурбоКонф!
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) Экспорт 
	...
	Если Истина
		...
		И Параметр.СтрокаОписания.Владелец().Колонки.Найти("ЛиЭкспорт") <> Неопределено
	Тогда  
