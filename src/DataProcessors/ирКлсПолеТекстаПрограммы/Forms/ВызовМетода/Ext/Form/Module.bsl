	...
	Тогда
		...
		Если Открыта() Тогда
			ЭлементыФормы.КоманднаяПанель1.Кнопки.Подменю.Кнопки.УказатьВызывающийМодуль.Доступность = Истина
				...
				И ирОбщий.СтрокиРавныЛкс(мВызовМетода, ПрямоеИмяМодуляИзПолного(СтруктураТипаКонтекста.СтрокаОписания.ИмяМодуля, Истина) + "." + ИмяМетода + "(");
		КонецЕсли;
		Если Истина
...
Процедура КоманднаяПанель1ДобавитьСквознойПараметр(Кнопка)
	...
	ТекстВставки = "";
	Если мПлатформа.ПараметрыМетодаМодуля(мМетодМодуля).Количество() > 0 Тогда
		ТекстВставки = ", ";
	КонецЕсли;
