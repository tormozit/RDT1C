...
Процедура ОбновитьДанные() Экспорт
	...
	АнализаторМодуля = ирОбщий.НовыйАнализаторКодаЛкс();
	АнализаторМодуля.ИнициироватьНеинтерактивно();
	АнализаторМодуля.ЗагрузитьМодульМетаданных(МодульМетаданных);
	Для Каждого МетодМодуля Из МетодыМодуля Цикл
		ВхожденияВызова = ирОбщий.НайтиРегВыражениеЛкс(АнализаторМодуля.ТелоМетода(МетодМодуля), ШаблонВызова,,,,,,,,, ВхожденияВызова); // Без подгрупп для ускорения
