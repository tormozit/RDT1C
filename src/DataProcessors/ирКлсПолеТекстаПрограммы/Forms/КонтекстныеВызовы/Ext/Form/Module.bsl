...
Процедура ОбновитьДанные() Экспорт
	...
	Для Каждого МетодМодуля Из МетодыМодуля Цикл
		ВхожденияВызова = ирОбщий.НайтиРегВыражениеЛкс(МетодМодуля.Тело, ШаблонВызова,,,,,,,,, ВхожденияВызова); // Без подгрупп для ускорения
		#Если Сервер И Не Сервер Тогда
...
Процедура ВызовыПриАктивизацииСтроки(Элемент)
	...
	ирКлиент.ПоказатьПозициюМодуляЛкс(ТекущаяСтрока, ИмяФайлаМодуля, ЭлементыФормы.ПолеТекстаВызова,, ТекстФайла);
