...
Процедура НайтиВходящие(Знач СтрокаРодителя)
	...
	СтрокаРодителя.ВызывающихМетодов = 0;
	...
	Если Не ПустаяСтрока(СтрокаРодителя.Модуль) Тогда
		...
		КонецЕсли;
		Маска = "";
		ТолькоЛокальныеВрем = ТолькоЛокальные Или Не СтрокаМетодаРодителя.ЛиЭкспорт;
		Если ТолькоЛокальныеВрем Тогда
		...
		Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ФайлыМодулей.Количество(), "Входящие");
		...
		Для Каждого Файл Из ФайлыМодулей Цикл
			...
			КонецЕсли;
			Если ирОбщий.ОбработатьИндикаторЛкс(Индикатор,, "Найдено " + ВходящиеВызовыМетода.Количество()) Тогда
				// Индикация в дереве невозможна
			КонецЕсли;
			...
			Для ЭтапВнутренний = НачальныйСчетчик По 2 Цикл
				...
				КонецЦикла;
			КонецЦикла;
			ДобавитьВходящиеВызовыИзМодуля(СтрокаРодителя, СтрокаМетодаРодителя, ВходящиеВызовыМетода);
		КонецЦикла;
...
Функция ДобавитьВходящиеВызовыИзМодуля(Знач СтрокаРодителя, Знач СтрокаМетодаРодителя, Знач ВходящиеВызовыМетода) Экспорт
	Если ВходящиеВызовыМетода.Количество() Тогда
		...
		Для Каждого СтрокаПары Из СвернутыеВызовы Цикл
			...
			СтрокаПары.Внешний = СтрокаПары.Модуль <> СтрокаМетодаРодителя.ИмяМодуля;
		...
		СтрокаРодителя.ВызывающихМетодов = СтрокаРодителя.ВызывающихМетодов + СвернутыеВызовы.Количество();
		ВходящиеВызовыМетода.Очистить();
		ЭлементыФормы.ДеревоВызовов.ОбновитьСтроки();
	КонецЕсли;
...
Процедура НайтиИсходящие(Знач СтрокаРодителя)
	...
	Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(Вхождения.Количество(), "Исходящие");
	Для Каждого ВхождениеВызова Из Вхождения Цикл
...
Процедура УточнитьСтрокуРезультата(Знач ТекущаяСтрока, Знач Групповое = Истина) Экспорт
	...
	Иначе
		...
		Если ПустаяСтрока(ТекущаяСтрока.Метод) Тогда
			ТекущаяСтрока.Метод = "<>";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВызовыСловаПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Иначе
		Возврат;
	КонецЕсли;
	ЭлементыФормы.ДеревоВызовов.ТекущаяСтрока = Строка;
	...
	Если Не ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		ЗаполнитьДочерниеСтроки();
	КонецЕсли;
	БылоПрерывание = Ложь;
КонецПроцедуры

Процедура ЗаполнитьДочерниеСтроки() Экспорт
	Строка = ЭлементыФормы.ДеревоВызовов.ТекущаяСтрока;
	Строка.Строки.Очистить();
	ЭтаФорма.Обновить();
	ЭтаФорма.ЭлементыФормы.ДеревоВызовов.ОбновитьСтроки();
	ЭтаФорма.Обновить();
	Если Ложь
		Или Не Строка.Выход
...
Процедура ЗавершитьРазвертывание()
	...
	Если ирКэш.ЛиПлатформаWindowsЛкс() Тогда
		// запускаем долгую операцию через нажатие кнопки чтобы пользователь мог прервать https://partners.v8.1c.ru/forum/t/1356801/m/1356801
		ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.КнопкаОбработчикаОжидания;
...
Процедура ТолькоЛокальныеПриИзменении(Элемент)
	ОбновитьДанные();
КонецПроцедуры
...
Процедура КнопкаОбработчикаОжиданияНажатие(Элемент)
	ЭтаФорма.ТекущийЭлемент = ЭлементыФормы.ДеревоВызовов;
	ЗаполнитьДочерниеСтроки();
КонецПроцедуры
...
ДеревоВызовов.Колонки.Добавить("Места", Новый ОписаниеТипов("ТаблицаЗначений")); // см. ДобавитьНайденныеВходящиеВызовы
