...
Процедура ПриОткрытии()
	...
	ЭтаФорма.ОтображатьКонцыСтрок = мИсторияГита.ОтображатьКонцыСтрок;
	ЭтаФорма.ЧислоСтрокКонтекста = мИсторияГита.ЧислоСтрокКонтекста;
	...
	ЭтаФорма.ЭлементыФормы.НадписьПоследнийКоммит.Заголовок = мИсторияГита.ДатаПоследнегоКоммита();
...
Процедура ДействияФормыЗаменитьМодуль(Кнопка)
	Для Каждого Строка Из ЭлементыФормы.ИзмененныеМодули.ВыделенныеСтроки Цикл
		УстановитьВерсиюГитаВФайлКэшаМодулей(Строка);
	КонецЦикла;
КонецПроцедуры
...
Процедура ИзмененныеМодулиПриАктивизацииСтроки(Элемент = Неопределено)
	...
	Если ЗначениеЗаполнено(Элемент.ТекущаяСтрока.ДатаИзмененияКэша) Тогда
		...
		мИсторияГита.ТекстИзмененийФайла = ирОбщий.ВыполнитьКомандуОСЛкс(мИсторияГита.НачалоКомандыГита() + "diff --no-index --no-prefix " + мИсторияГита.ПараметрыВыводаИзмененийДляКоманды()
	...
	Если Истина
		...
		И мМодульМетаданных.Имя = Элемент.ТекущаяСтрока.Модуль
