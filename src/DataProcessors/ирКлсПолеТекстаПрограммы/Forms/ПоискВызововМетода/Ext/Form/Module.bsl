...
Процедура ВызовыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			...
			ПолеТекстаЛ = ФормаТекста.ПолеТекста();
		КонецЕсли;
		ПолеТекстаЛ.УстановитьГраницыВыделения(ВыбраннаяСтрока.Позиция, ВыбраннаяСтрока.Позиция + СтрДлина(ВыбраннаяСтрока.Текст),,, Истина, ФормаТекста);
	КонецЕсли;
...
Процедура ДействияФормыНайти(Кнопка = Неопределено)
	...
	КонецЕсли;
	Если ИскатьНепрямые Или мМодульМетаданных = Неопределено Тогда
		...
		Для Счетчик = 1 По 2 Цикл
			...
			ИначеЕсли Счетчик = 2 Тогда 
				Если Ложь
					...
					Или Найти(ПолноеИмяФайла, "." + ИмяРодногоМодуля + ".") > 0 
				Тогда
