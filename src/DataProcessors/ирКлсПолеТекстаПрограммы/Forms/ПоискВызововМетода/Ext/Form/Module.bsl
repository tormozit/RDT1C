...
Функция ТекстФайлаСтроки(Знач ВыбраннаяСтрока = Неопределено)
	...
	ТекстовыйДокумент.Прочитать(мПлатформа.ПапкаКэшаМодулей.ПолноеИмя + "\" + ВыбраннаяСтрока.КраткоеИмяФайла);
...
Процедура ДействияФормыНайти(Кнопка = Неопределено)
	...
	ВызовыМетода.Очистить();
	ФайлыМодулей = НайтиФайлы(мПлатформа.ПапкаКэшаМодулей.ПолноеИмя, "*.txt", Истина);
	...
	Для Каждого Файл Из ФайлыМодулей Цикл
		...
		Для Счетчик = 1 По 2 Цикл
			...
			Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
				...
				СтрокаВызова.КраткоеИмяФайла = Файл.Имя;
				...
				Если ЗначениеЗаполнено(МаксЧислоРезультатов) И ВызовыМетода.Количество() >= МаксЧислоРезультатов Тогда
					...
					Перейти ~КонецЦикла;
				КонецЕсли; 
				Если Не ПерваяПорцияОтображена И ВызовыМетода.Количество() >= 50 Тогда
...
Процедура ВызовыПриАктивизацииСтроки(Элемент)
	...
	//Если Не ЗначениеЗаполнено(ТекущаяСтрока.НомерСтрокиФайла) Тогда
		ПолеТекстаВызова = ирКлиент.ОболочкаПоляТекстаЛкс(СлужебноеПолеТекста);
		#Если Сервер И Не Сервер Тогда
			ПолеТекстаВызова = Обработки.ирОболочкаПолеТекста.Создать();
		#КонецЕсли
		ПолеТекстаВызова.УстановитьТекст(ТекстФайлаСтроки());
		ПолеТекстаВызова.УстановитьГраницыВыделения(ТекущаяСтрока.Позиция + 1, ТекущаяСтрока.Позиция + СтрДлина(ТекущаяСтрока.Текст));
		ТекущаяСтрока.НомерСтрокиФайла = ПолеТекстаВызова.ВыделениеДвумерное().НачальнаяСтрока;
		Файл = Новый файл(СтрЗаменить(ТекущаяСтрока.КраткоеИмяФайла, ".Форма.Модуль.txt", ".Форма.txt"));
		ТекущаяСтрока.Ссылка = "{" + Файл.ИмяБезРасширения + "(" + XMLСтрока(ТекущаяСтрока.НомерСтрокиФайла) + ")}";
		ВыделениеДвумерное = ПолеТекстаВызова.ВыделениеДвумерное();
		...
		КонецЕсли;
		ПолеТекстаВызова.УстановитьГраницыВыделения(НачальнаяСтрока, 1, ВыделениеДвумерное.КонечнаяСтрока, 100000);
		ЭлементыФормы.ПолеТекстаВызова.УстановитьТекст(ПолеТекстаВызова.ВыделенныйТекст()); 
		ЭлементыФормы.ПолеТекстаВызова.УстановитьГраницыВыделения(НачальнаяСтрокаВнутр, 1, НачальнаяСтрокаВнутр, 1);
	//КонецЕсли;
