...
Процедура ВызовыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ПоказатьПозициюМодуля(ТекущаяСтрока, мПлатформа.ПапкаКэшаМодулей.ПолноеИмя + "\" + ТекущаяСтрока.КраткоеИмяФайла + "." + РасширениеФайлаМодуля, ЭлементыФормы.ПолеТекстаВызова, РасширениеФайлаМодуля);
