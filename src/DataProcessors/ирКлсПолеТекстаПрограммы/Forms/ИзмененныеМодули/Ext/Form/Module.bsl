...
Процедура ОбновитьДанные() Экспорт
	...
	ФайлыМодулей = НайтиФайлы(мПлатформа.ПапкаКэшаМодулей.ПолноеИмя, "*." + РасширениеФайлаМодуля);
	...
	Для Каждого Файл Из ФайлыМодулей Цикл
		...
		Если СтарыйТекст <> ТекстовыйДокументНовый.ПолучитьТекст() Тогда
			...
			Строка.ДатаИзмененияКэша = Файл.ПолучитьВремяИзменения();
			Строка.ДатаИзмененияГита = ИсторияГита.ДатаПоследнегоКоммита(ФайлГита.ПолноеИмя);
