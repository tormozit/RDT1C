...
Процедура ИзмененныеМодулиВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		...
		Если мМодульМетаданных.Имя = ВыбраннаяСтрока.Модуль Тогда
			ПараметрНомерСтроки = мНачальнаяСтрока;
		КонецЕсли;
		ирКлиент.Сравнить2ЗначенияВФормеЛкс(ТекстовыйДокументСтарый, ТекстовыйДокументНовый,, "Гит (старый)", "Кэш (новый)",,, ВыбраннаяСтрока.Модуль,, "ВстроенныйЯзык", ВыбраннаяСтрока.Модуль,,,, ПараметрНомерСтроки);
	КонецЕсли;
КонецПроцедуры
