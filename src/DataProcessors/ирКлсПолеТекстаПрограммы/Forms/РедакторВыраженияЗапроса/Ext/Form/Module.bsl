...
Процедура КоманднаяПанельТекстаОбновитьЗапросы(Кнопка = Неопределено)
	...
	НачальныйТокен = РазобратьТекстЗапроса(ТекстПроверочногоЗапроса,,, СлужебноеПолеТекста,,,,, Ложь); // Здесь важно получать полное, а не сокращенное дерево, т.к. нужно ЕстьАгрегаты
	Запросы.Очистить();
