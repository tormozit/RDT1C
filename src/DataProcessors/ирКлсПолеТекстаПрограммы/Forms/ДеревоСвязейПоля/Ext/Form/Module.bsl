...
Процедура НайтиВходящие(Знач СтрокаРодителя)
	...
	ВходящиеВызовыПоля = ВызовыТекущегоПоля.СкопироватьКолонки();
	...
	Для Каждого СтрокаДерева Из СтрокаРодителя.Строки Цикл
		...
		Пока Родитель <> Неопределено Цикл
			Если СтрокаДерева.ИндексЗапроса = Родитель.ИндексЗапроса Тогда
...
Процедура УточнитьСтрокуРезультата(Знач СтрокаМеста) Экспорт
	ЗагрузитьЗапросПакетаПоИндексу(ПакетЗапросов, ЭлементыФормы.ДеревоВызовов.ТекущаяСтрока.ИндексЗапроса);
...
Процедура ВызовыТекущегоПоляВыбор(Элемент = Неопределено, ВыбраннаяСтрока = Неопределено, Колонка = Неопределено, СтандартнаяОбработка = Истина)
	...
	Если ВыбраннаяСтрока = Неопределено Тогда
		Если СтрокаЗапроса.ИндексЗапроса >= 0 Тогда
			...
			НайтиОпределениеСлова(СтрокаЗапроса.ИмяПоля,, ПоследнееВхождение);
