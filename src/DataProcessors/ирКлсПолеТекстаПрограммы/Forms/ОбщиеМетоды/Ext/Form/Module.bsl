...
Процедура ДобавитьМетодыМодуля(Знач СтруктураТипа, СтрокаМетаТипа, ТипМодуляКонфигурации = "")
	...
	Для Каждого МетодМодуля Из МетодыМодуля Цикл
		...
		Если МетодМодуля.Параметры <> Неопределено Тогда
			СтрокаМетода.ПараметрыСтрокой = ирОбщий.СтрСоединитьЛкс(МетодМодуля.Параметры.ВыгрузитьКолонку("Имя"));
			СтрокаМетода.КоличествоПараметров = МетодМодуля.Параметры.Количество();
		КонецЕсли; 
	КонецЦикла;
	Если ИндикаторМодуля <> Неопределено Тогда
		ирОбщий.ОсвободитьИндикаторПроцессаЛкс();
...
Процедура ОбщиеМетодыПриАктивизацииСтроки(Элемент)
	...
	МодульМетаданных = мПлатформа.ПодготовитьМодульМетаданных(СтруктураТипа,, ИмяФайлаМодуляБезРасширения);
	...
	ПоказатьПозициюМодуля(ТекущаяСтрока, мПлатформа.ПапкаКэшаМодулей.ПолноеИмя + "\" + ИмяФайлаМодуляБезРасширения + "." + РасширениеФайлаМодуля,, РасширениеФайлаМодуля);
