...
Процедура ДействияФормыНайти(Кнопка = Неопределено)
	...
	Для Каждого Файл Из ФайлыМодулей Цикл
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		Если ирОбщий.ОбработатьИндикаторЛкс(Индикатор,, Модуль) Тогда 
		...
		Для Счетчик = НачальныйСчетчик По 2 Цикл
			Если Счетчик = 1 Тогда 
				НайденныеВызовы = мПлатформа.НепрямыеВызовыСловаВМодуле(Модуль, ТекстовыйДокумент, Файл, ЭтотОбъект, ирОбщий.ПоследнийФрагментЛкс(ИскатьСлово), ТипСлова <> ПеречислениеТипСлова.Свойство);
...
Процедура ВызовыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяСтрока.ТипРодителя = "??" Тогда
		...
		Если РежимПоиска = "Ссылки" И ТипСлова <> ПеречислениеТипСлова.Конструктор Тогда
			ТекущаяСтрока.ВыражениеРодитель = ирОбщий.ПервыйФрагментЛкс(ТекущаяСтрока.Текст, "." + ИскатьСлово, Ложь);
...
Процедура УточнитьСтрокуРезультата(Знач ТекущаяСтрока, Знач Групповое = Истина) Экспорт
	...
	Иначе
		Если Не Групповое Тогда
			ПолеФрагмента = ЭлементыФормы.ПолеТекстаВызова;
		КонецЕсли;
