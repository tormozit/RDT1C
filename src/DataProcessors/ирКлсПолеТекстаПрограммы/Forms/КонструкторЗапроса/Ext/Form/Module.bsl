...
Функция ОбработатьРасширениеЯзыкаЗапросов()
	...
	Если ТекстПакета <> "" Тогда
		...
		КонецЕсли; 
		СлужебноеПолеТекста.УстановитьТекст(ТекстПакета);
...
Функция ПолучитьПредставлениеСравненияНаЯзыке(ПредставлениеЛевогоПоля, Знач ВидСравнения, ДоступноеПолеИлиОписаниеТипов = Неопределено, Отступ = "") Экспорт
	...
	ДоступныВидыСравненияПодобноШаблон = ирКэш.ДоступныВидыСравненияПодобноШаблонЛкс();
	...
	Тогда
		...
		Для Каждого Тип Из ТипыПоля Цикл
			...
			Тогда
				Продолжить;
			КонецЕсли; 
			СписокВыражений.Добавить(ирОбщий.ПустоеЗначениеПоТипуНаЯзыкеЗапросовЛкс(Тип, ОписаниеТипов));
		КонецЦикла;
		...
		Тогда
			Если Найти(Результат, ".") > 0 Тогда
				...
				Тогда
					...
					КонецЕсли;
					Если СтруктураПолей.Режим = "ПоискПолейМетаданных" Тогда 
	...
	Иначе
		...
		Тогда
			...
			Если ТипЛогическойГруппы = "И" Тогда
				ЛогическийОператор = ПолучитьСловоЯзыкаЗапросов("AND");
			Иначе
				ЛогическийОператор = ПолучитьСловоЯзыкаЗапросов("OR");
			КонецЕсли; 
		...
		ИначеЕсли ИмяПравила = "<EmbeddedQuery>" Тогда
			...
			Если ВнутреннийТокен.Name = "EmbeddedRoot" Тогда
				...
				Результат = "(" + Символы.ПС + Символы.Таб + СобратьТекстПакета(Пакет, Символы.Таб, ВложеннаяСтруктураПолей) + ")";
			Иначе
		...
		Иначе
			...
			Тогда
				...
				Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) <> Тип("ПолеКомпоновкиДанных") Тогда
					СтрокаПравогоЗначения = ирОбщий.ПредставлениеЗначенияВЯзыкеЗапросовЛкс(ЭлементОтбора.ПравоеЗначение);
				...
				Тогда
					...
					КонецЕсли; 
				ИначеЕсли Истина
					И ирКэш.ДоступныВидыСравненияПодобноШаблонЛкс()
					И (Ложь
...
Функция ДобавитьЗначениеВПараметры(ЗначениеПараметра, Знач XMLТип = Неопределено, Знач БазовоеИмяПараметра = "")
	...
	Если СтрокаПараметра = Неопределено Тогда
		...
		КонецЕсли; 
		ИмяПараметра = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(Параметры, БазовоеИмяПараметра, "НИмя",, ПолучитьСловоЯзыкаЗапросов("Parameter"),, Истина);
		СтрокаПараметра = Параметры.Добавить();
...
Функция РазобратьВыражениеЗапроса(ВыражениеЗапроса, ЗаменятьПустоеНаNULL = Истина) Экспорт
	...
	НачальныйТокен = РазобратьТекстЗапроса(ТекстДляРазбора, , Ложь, СлужебноеПолеТекста);   // Здесь важно получать полное, а не сокращенное дерево
	Если НачальныйТокен = Неопределено Тогда
...
Функция УстановитьВыражениеДляРазбораВСлужебноеПоле(ВыражениеЗапроса, Знач ЗаменятьПустоеНаNULL = Истина)
	...
	СлужебноеПолеТекста.УстановитьТекст(ТекстДляРазбора);
		...
		Тогда
			...
			Иначе
				...
				Если Ложь
					...
					Или ПравоеЗначение = Неопределено 
				Тогда
					ЗаполнитьЭлементОтбораПроизвольногоВыражения(ЭлементОтбора, Токен, , ЧастьОбъединения, ЗапросПакета);
				ИначеЕсли ИмяПравила = "<CompareExpr>" Тогда
				...
				ИначеЕсли ИмяПравила = "<CheckLIKE>" Тогда
					...
					ИначеЕсли ирКэш.НомерВерсииПлатформыЛкс() >= 803005 Тогда
...
Функция ПолучитьОпределениеТаблицы(ИмяИлиСтрокаВыбраннойТаблицы, ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "")
	...
	ИначеЕсли СтрокаВыбраннойТаблицы.ВложенныйПакет <> Неопределено Тогда
		...
		Исключение
			...
			КонецЕсли; 
		КонецПопытки;
		Результат = "(" + Символы.ПС + Смещение + СокрЛ(лТекстПакета) + ")";
	Иначе
	...
	Тогда
		Результат = Результат + " " + ПолучитьСловоЯзыкаЗапросов("AS") + " " + ИмяСНеобходимымиСкобками(СтрокаВыбраннойТаблицы.Имя);
	КонецЕсли; 
...
Функция СобратьТекстЗапросаПакета(ЗапросПакета = Неопределено, Смещение = "", СтруктураПолей = Неопределено) Экспорт 
	...
	Если ЗапросПакета.ТипЗапроса = 0 Тогда
		...
		Для Каждого СтрокаПоля Из ЗапросПакета.ГруппирующиеПоляИтогов Цикл
			...
			Тогда
				ТекстГруппирующиеПоля = ТекстГруппирующиеПоля + " " + ПолучитьСловоЯзыкаЗапросов("AS") + " " + ИмяСНеобходимымиСкобками(СтрокаПоля.Имя);
			КонецЕсли; 
			...
			ИначеЕсли ирОбщий.СтрокиРавныЛкс(СтрокаПоля.ТипИтогов, "ТолькоИерархия") Тогда
				ТекстГруппирующиеПоля = ТекстГруппирующиеПоля + " " + ПолучитьСловоЯзыкаЗапросов("ONLY") + " " + ПолучитьСловоЯзыкаЗапросов("HIERARCHYON");
			КонецЕсли; 
		КонецЦикла;
		ТекстИтоговыеПоля = "";
		Для Каждого СтрокаПоля Из ЗапросПакета.ГруппируемыеПоляИтогов Цикл
			...
			Если Истина
				...
				И ЗначениеЗаполнено(СтрокаПоля.Имя) 
			Тогда
				ТекстИтоговыеПоля = ТекстИтоговыеПоля + " " + ПолучитьСловоЯзыкаЗапросов("AS") + " " + ИмяСНеобходимымиСкобками(СтрокаПоля.Имя);
			КонецЕсли; 
		КонецЦикла;
		Если Ложь
	...
	Если ЗапросПакета.ОпцияПервые Тогда
		Результат = Результат + Символы.ПС + Смещение + ПолучитьСловоЯзыкаЗапросов("LIMIT") + " " + XMLСтрока(ЗапросПакета.ПервыеКоличество);
	КонецЕсли; 
	Если Истина
		И ПараметрыДиалектаSQL.Это1С
...
Функция СобратьТекстПредложенияПостроителя(Знач ЗапросПакета, Знач ТаблицаПолейПостроителя, Знач ТабличноеПолеПостроителя = Неопределено, Знач ЧастьОбъединения = Неопределено, Знач Смещение = "")
	...
	Для Каждого ПолеПостроителя Из ТаблицаПолейПостроителя Цикл
		...
		Тогда
			ТекстПостроителя = ТекстПостроителя + " " + ПолучитьСловоЯзыкаЗапросов("AS") + " " + ИмяСНеобходимымиСкобками(ПолеПостроителя.Имя);
		КонецЕсли;
	КонецЦикла;
...
Процедура ВыбранныеПоляПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Если Истина
		И (Ложь
			Или ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") 
			Или ТипЗнч(ЗначениеПеретаскивания) = Тип("ВыделенныеСтрокиТабличногоПоля"))
		И РазрешеноПеретаскиваниеВПоля(ЗначениеПеретаскивания[0]) 
	Тогда
		СтандартнаяОбработка = Ложь;
		Для Каждого СтрокаПеретаскивания Из ЗначениеПеретаскивания Цикл
			Если ТипЗнч(СтрокаПеретаскивания) = Тип("СтрокаТаблицыЗначений") Тогда
				Если Элемент.Значение.Индекс(СтрокаПеретаскивания) > -1 Тогда
...
Функция ДобавитьДоступноеПолеВТаблицуВыражений(СтрокаДоступногоПоля, ТабличноеПолеИлиТаблицаВыражений, ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено,
	...
	Иначе
		...
		Иначе
			ТаблицаВыражений = ТабличноеПолеИлиТаблицаВыражений;
		КонецЕсли; 
		Если Ложь
			Или ТипЗнч(СтрокаДоступногоПоля) = Тип("ДоступноеПолеКомпоновкиДанных") 
			Или ТипЗнч(СтрокаДоступногоПоля) = Тип("ДоступноеПолеОтбораКомпоновкиДанных")
...
Функция ДобавитьДоступноеПолеВВыбранныеПоля(СтрокаДоступногоПоля, Автополе = Ложь, ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено,
	...
	Если СтрокаВыбранногоПоля = Неопределено Тогда
		...
		СтрокаВыбранногоПоля.Имя = ПолучитьАвтоПсевдонимПоля(СтрокаВыбранногоПоля, ЧастьОбъединения);
...
Функция ПолучитьАвтоПсевдонимПоля(СтрокаПоля, _ЧастьОбъединения = Неопределено, СчетчикБезымянныхПолей = Неопределено)
	...
	Если Не Автополе Тогда
		...
		КонецЕсли; 
		ИмяПоля = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(СтрокаПоля.Владелец().Скопировать(ОтборКонкурентов, "НИмя"), ИмяПоля, "НИмя",,,, Истина);
	КонецЕсли; 
	Возврат ИмяПоля;
...
Процедура ОбновитьКомпоновщикПроверкиУсловияСвязи()
	...
	КомпоновщикПроверкиУсловияСвязи.Инициализировать(мТекущаяСтрокаЧастиОбъединения.ИсточникНастроек);
...
Процедура ПересобратьВременныйТекстЧасти()
	...
	ПолеТекстаЧасти = ирОбщий.ОболочкаПоляТекстаЛкс(ЭлементыФормы.ПолеТекстаЧасти);
	#Если Сервер И Не Сервер Тогда
...
Процедура ПриОткрытии()
	...
	Если ЗначениеЗаполнено(ПараметрТекстПакета) Тогда
		...
		#КонецЕсли
		Компонента.ПолеТекста.УстановитьТекст(ПараметрТекстПакета);
...
Функция ДоступныеТаблицыПоследниеВыбранныеНажатие(Кнопка) Экспорт
	...
	ирОбщий.ПоследниеВыбранныеНажатиеЛкс(ЭтаФорма, ЭлементыФормы.ДоступныеТаблицы, "ПолноеИмя", Кнопка,, КлючХраненияПоследнихВыбранных());
...
Процедура ДоступныеТаблицыПоследниеВыбранныеДобавить(Знач ТекущиеДанные = Неопределено)
	...
	КонецЕсли; 
	ирОбщий.ПоследниеВыбранныеДобавитьЛкс(ЭтаФорма, ТекущиеДанные.ПолноеИмя,, ЭлементыФормы.ДоступныеТаблицы,,, КлючХраненияПоследнихВыбранных());
	ДоступныеТаблицыОбновитьПодменюПоследниеВыбранные();
...
Процедура ЗаполнитьДоступныеТаблицы1С()
	...
	КлючТекущейСтроки = ирОбщий.ТабличноеПолеКлючСтрокиЛкс(ЭлементыФормы.ДоступныеТаблицы, "ПолноеИмя");
	...
	ирОбщий.ТабличноеПолеВосстановитьТекущуюСтрокуЛкс(ЭлементыФормы.ДоступныеТаблицы, КлючТекущейСтроки);
	ЗаполнитьСписокВыбораТиповТаблиц();
...
Процедура ЗаполнитьСписокВыбораТиповТаблиц()
	...
	Для Каждого ЭлементСпискаВыбора Из СписокВыбора Цикл
		ЭлементСпискаВыбора.Картинка = ирОбщий.КартинкаКорневогоТипаМДЛкс(ЭлементСпискаВыбора.Значение);
	КонецЦикла;
...
Функция ПоляВыбраннойТаблицы(ИмяИлиСтрокаВыбраннойТаблицы, ЧастьОбъединения = Неопределено)
	...
	ИначеЕсли СтрокаДоступнойТаблицы <> Неопределено Тогда
		ПоляТаблицы = ПоляДоступнойТаблицы(СтрокаДоступнойТаблицы, ВыбраннаяТаблица.Параметры);
	Иначе
...
Функция ДоступнаяТаблицаПоИмениВыбранной(Знач ПолноеИмя)
...
Функция ПоляДоступнойТаблицы(СтрокаДоступнойТаблицы, ПараметрыТаблицы = Неопределено)
...
Функция СкорректироватьИмяВыбраннойТаблицы(Знач ВыбраннаяТаблица, ПринудительноАвтоимя = Ложь, РазрешитьПереименованиеДублей = Истина)
	...
	КонецЕсли;
	НовоеИмя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ВыбраннаяТаблица.Владелец(), ВыбраннаяТаблица,, мПараметрыДиалектаSQL.Это1С,, 0);
			...
			Для Каждого ЗаменаПолей Из ТаблицаЗамен Цикл
				...
				Если ЭтоПереименованиеПараметра Тогда
					Для Каждого ГруппируемоеПолеИтогов Из ЗапросПакета.ГруппируемыеПоляИтогов Цикл
						ОбновитьВыражениеЗапроса(ГруппируемоеПолеИтогов, ЭлементыФормы.ГруппируемыеПоляИтогов,, ЧастьОбъединения, СтароеИмя, НовоеИмя,,,,, СтруктураПолейЦикла);
					КонецЦикла;
					Для Каждого ГруппирующееПолеИтогов Из ЗапросПакета.ГруппирующиеПоляИтогов Цикл
						ОбновитьВыражениеЗапроса(ГруппирующееПолеИтогов, ЭлементыФормы.ГруппирующиеПоляИтогов,, ЧастьОбъединения, СтароеИмя, НовоеИмя,,,,, СтруктураПолейЦикла);
					КонецЦикла;
				КонецЕсли;
				//ОбновитьПоляОбъединенияЗапроса();
...
Процедура СвязиТаблицПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Условие <> Неопределено Тогда
		...
		ОбновитьВыраженияОтбора(ДанныеСтроки.Условие.Отбор,,,,,, 2); // Очистка COM-объектов перед сериализацией
...
Процедура ВыбранныеТаблицыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	//	Элемент.ТекущиеДанные.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(Элемент.Значение, Элемент.ТекущиеДанные,,, ПолучитьСловоЯзыкаЗапросов("Table") + "1");
...
Процедура ПараметрыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования Тогда
		НовоеИмяПараметра = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(Элемент.Значение, Элемент.ТекущиеДанные,,, ПолучитьСловоЯзыкаЗапросов("Parameter") + "1");
...
Функция ПолучитьПолноеИмяДоступногоПоля(ДоступноеПоле)
	...
	#КонецЕсли
	Возврат ЗначениеОтбораНаЯзыкеЗапросов(ДоступноеПоле.Поле);
...
Процедура ДоступныеТаблицыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(ПоляДоступнойТаблицы(ТекущаяСтрока), ПоляДоступнойТаблицы);
	...
	Для Каждого СтрокаПоля Из ПоляДоступнойТаблицы Цикл
		...
		Если МетаданныеПоля <> Неопределено Тогда
			...
			Исключение
				...
				Продолжить;
			КонецПопытки;
			СтрокаПоля.Описание = ПодсказкаПоля;
		КонецЕсли; 
	КонецЦикла;
...
Процедура ДиалектSQLОчистка(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ирОбщий.ИнтерактивноЗаписатьВЭлементУправленияЛкс(Элемент, мДиалектSQL);
...
Процедура ОбновитьВсеВыраженияЧастиОбъединения(Знач ЗапросПакета, Знач ЧастьОбъединения, Знач РежимОбновленияВыражений = 1, СтруктураПолей = Неопределено)
	...
	Для Каждого ВыбраннаяТаблица Из ЧастьОбъединения.ВыбранныеТаблицы Цикл
		...
		Если СтруктураПолей.Режим = "ЗаполнениеНастроекКомпоновки" Тогда
			...
			Для каждого ПараметрТаблицы Из ВыбраннаяТаблица.Параметры Цикл
				Если ЭтоПараметрПериод1С(ПараметрТаблицы) Тогда
				...
				КонецЕсли; 
				КомпоновщикПараметра = КомпоновщикОтбораВПараметре(ПараметрТаблицы, ВыбраннаяТаблица.ПолноеИмя);
				Если КомпоновщикПараметра <> Неопределено Тогда
					#Если Сервер И Не Сервер Тогда
						КомпоновщикПараметра = Новый КомпоновщикНастроекКомпоновкиДанных;
					#КонецЕсли
					ПараметрТаблицы.ТекстРасширения = "";
					ТаблицаВыражений = ПоляОтбораПостроителя.СкопироватьКолонки();
					Для Каждого ДоступноеПоле Из КомпоновщикПараметра.Настройки.Отбор.ДоступныеПоляОтбора.Элементы Цикл
						Если ДоступноеПоле.Поле = Новый ПолеКомпоновкиДанных("ПараметрыДанных") Тогда
							Продолжить;
						КонецЕсли; 
						ДобавитьДоступноеПолеВТаблицуВыражений(ДоступноеПоле, ТаблицаВыражений, ЗапросПакета, ЧастьОбъединения,, "Определение");
					КонецЦикла;
					ПараметрТаблицы.ТекстРасширения = СобратьТекстПредложенияПостроителя(ЗапросПакета, ТаблицаВыражений,, ЧастьОбъединения);
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли; 
		Для каждого ПараметрТаблицы Из ВыбраннаяТаблица.Параметры Цикл
...
Функция ЛиТерминалЯзыка(Знач ИмяПоля, МожетБытьПсевдонимом = Ложь)
	...
	Результат = Ложь
		Или мТерминалыЯзыкаЗапросов.НайтиСтроки(ОтборАнглийский).Количество() > 0
		Или мТерминалыЯзыкаЗапросов.НайтиСтроки(ОтборРусский).Количество() > 0;
	Возврат Результат;
...
Функция ОбновитьВыражениеЗапроса(СтрокаТаблицыВыражений, ТабличноеПоле = Неопределено, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	...
	Иначе
		...
		Попытка
			...
			СлужебноеПолеТекстаСтарое = СлужебноеПолеТекста;
			СлужебноеПолеТекста = мПлатформа.СлужебноеПолеТекста2;
			...
			//КонецЕсли; 
			СлужебноеПолеТекста = СлужебноеПолеТекстаСтарое; 
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
...
Процедура ПересобратьВременныйПолныйТекст()
	...
	ПолеТекстаЗапроса = ирОбщий.ОболочкаПоляТекстаЛкс(ЭлементыФормы.ПолеТекстаЗапроса);
	#Если Сервер И Не Сервер Тогда
...
Процедура ПанельОсновнаяПриСменеСтраницы(Элемент = Неопределено, ТекущаяСтраница =  Неопределено)
	...
	Если мСтараяСтраницаПанелиОсновная = ЭлементыФормы.ПанельОсновная.Страницы.ПоляОбъединения Тогда
		Для Каждого ЧастьОбъединения Из мТекущаяСтрокаЗапросыПакета.ЧастиОбъединения Цикл
			ПозицияЦикл = 0;
...
Процедура ЗагрузитьСвязьТаблиц()
				...
				Если ГруппаДоступныхПолей <> Неопределено Тогда
					...
					Для Каждого ДоступноеПолеЛевое Из ГруппаДоступныхПолей.Элементы Цикл
						Для Каждого ДоступноеПолеПравое Из ДоступноеПолеТаблицыРодителя.Элементы Цикл
							ДобавитьСвязь = Ложь;
							ЛиИменаПолейСовпадают = ирОбщий.ПоследнийФрагментЛкс(ДоступноеПолеЛевое.Поле) = ирОбщий.ПоследнийФрагментЛкс(ДоступноеПолеПравое.Поле);
							Если Истина
								И ЛиИменаПолейСовпадают
								И ирОбщий.ЛиОписанияТиповПересекаютсяЛкс(ДоступноеПолеЛевое, ДоступноеПолеПравое)
								И ДоступноеПолеЛевое.ТипЗначения.Типы().Количество() = 1 И ДоступноеПолеПравое.ТипЗначения.Типы().Количество() = 1
							Тогда 
								ОписаниеЛевогоПоля = ОписаниеДоступногоПоля(ДоступноеПолеЛевое);
								Если Найти(ирОбщий.ПолноеИмяМДПоляТаблицыЛкс(ОписаниеЛевогоПоля), ".Измерение.") > 0 Тогда 
									ДобавитьСвязь = Истина;
								Иначе
									ОписаниеПравогоПоля = ОписаниеДоступногоПоля(ДоступноеПолеПравое);
							...
							Если Истина
								И Не ДобавитьСвязь
								И ирОбщий.ЛиОписанияТиповПересекаютсяЛкс(ДоступноеПолеЛевое, ДоступноеПолеПравое, Истина)
								И (Ложь
...
Процедура ЗагрузитьВыражениеУсловия()
	...
	Если ТипЗнч(мТекущаяСтрокаУсловия) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
		МожноСобиратьВыражение = Ложь
			Или ЗначениеЗаполнено(ирОбщий.ПредставлениеЗначенияВЯзыкеЗапросовЛкс(мТекущаяСтрокаУсловия.ПравоеЗначение))
...
Процедура СохранитьЗапросПакета(РазрешитьДобавлениеАвтополей = Истина)
	...
	КонецЕсли; 
	мТекущаяСтрокаЗапросыПакета.ОпцияПервые = ЗапросОпцияПервые;
	мТекущаяСтрокаЗапросыПакета.ПервыеКоличество = ЗапросПервыеКоличество;
	...
	Для Каждого ИмяТабличногоПоля Из ТабличныеПоля Цикл
		...
		Если ТабличноеПоле.ТекущаяСтрока <> Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(ЗначениеПоля) = Тип("ДоступныеПоляКомпоновкиДанных") Тогда 
				ИндексСтроки = ТабличноеПоле.ТекущаяСтрока.Поле;
			Иначе
...
Процедура ЧастиОбъединенияПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Если Копирование Тогда
		ЧастьОбъединения = СкопироватьЧастьОбъединения(мТекущаяСтрокаЗапросыПакета,, ПозицияНовойЧасти);
	Иначе
...
Процедура ДобавитьЗапрос(Знач Копирование, Позиция = Неопределено, Имя = "") Экспорт 
	...
	Если ТипЗапроса = 1 Тогда
		ПородившийЗапрос = мТекущаяСтрокаЗапросыПакета.ИД;
	КонецЕсли; 
	ЭлементыФормы.ЗапросыПакета.ТекущаяСтрока = ЗапросПакета;
	Если Не Копирование Тогда
...
Процедура ЗагрузитьЗапросПакета()
	...
	Если мТекущаяСтрокаЗапросыПакета <> Неопределено Тогда
		...
		ЭтаФорма.ЗапросОпцияПервые = мТекущаяСтрокаЗапросыПакета.ОпцияПервые;
		ЭтаФорма.ЗапросПервыеКоличество = мТекущаяСтрокаЗапросыПакета.ПервыеКоличество;
		...
		Если Не мТекущаяСтрокаЗапросыПакета.НормализацияВыполнена Тогда
			// https://www.hostedredmine.com/issues/918599
			ОбновитьВсеВыраженияЗапроса(мТекущаяСтрокаЗапросыПакета, 0);
			мТекущаяСтрокаЗапросыПакета.НормализацияВыполнена = Истина;
		КонецЕсли; 
...
Процедура ОбновитьТабличныеПоляПриПроверкеПеретаскивания()
	...
	Если ЭлементыФормы.ПанельЧасти.ТекущаяСтраница = ЭлементыФормы.ПанельЧасти.Страницы.ДеревоУсловия Тогда
		ирОбщий.ОбновитьТабличноеПолеДереваПослеУстановкиДанныхЛкс(ЭтаФорма, ЭлементыФормы.ДеревоУсловия);
		ЭтаФорма.Обновить();
	ИначеЕсли ЭлементыФормы.ПанельЧасти.ТекущаяСтраница = ЭлементыФормы.ПанельЧасти.Страницы.СвязиТаблиц Тогда
		ирОбщий.ОбновитьТабличноеПолеДереваПослеУстановкиДанныхЛкс(ЭтаФорма, ЭлементыФормы.СвязиТаблиц);
		ЭтаФорма.Обновить();
	КонецЕсли;
...
Процедура ДоступныеТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ОформлениеСтроки.Ячейки.Имя.ИндексКартинки = ирОбщий.ИндексКартинкиТипаТаблицыБДЛкс(ДанныеСтроки.Тип);
...
Функция ПолучитьИндексКартинкиВыбраннойТаблицы(ДанныеСтроки, выхСтрокаДоступнойТаблицы = Неопределено)
	...
	КонецЕсли; 
	Если ИндексКартинки = Неопределено Тогда
		ИндексКартинки = ирОбщий.ИндексКартинкиТипаТаблицыБДЛкс(Тип);
	КонецЕсли;
	Возврат ИндексКартинки;
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент)
	...
	Если Истина
		И ПараметрыДиалектаSQL.Это1С 
		И (Ложь
			Или мТекущаяСтрокаЗапросыПакета.ТипЗапроса <> 0
	...
	Если мТекущаяСтрокаЗапросыПакета.ТипЗапроса = 0 Тогда
		мТекущаяСтрокаЗапросыПакета.Имя = ИмяОсновнойТаблицы;
		мТекущаяСтрокаЗапросыПакета.РучноеИмя = ЗначениеЗаполнено(ИмяОсновнойТаблицы);
		ОбновитьНаименованиеЗапроса();
	Иначе
		ОбновитьНаименованиеЗапроса(мТекущаяСтрокаЗапросыПакета);
...
Процедура ОбновитьТаблицуПараметровВыбраннойТаблицы(Знач ВыбраннаяТаблица = Неопределено)
	...
	Если ДоступнаяТаблица <> Неопределено Тогда
		...
		ТаблицаСтруктурТиповКонтекста = ОпределитьТипЗначенияКонтекста(ПолноеИмяТаблицы);
...
Процедура ДоступныеПоляОбъединенияПриАктивизацииСтроки(Элемент)
	...
	Если СтрокаДляОтвязки <> Неопределено Тогда 
		...
		Иначе
			...
			ВыбранноеПоле.Имя = "";
			АвтоПсевдонимОтвязанногоПоля = ПолучитьАвтоПсевдонимПоля(ВыбранноеПоле, ВыбраннаяЧасть);
...
Процедура ПриИзмененииИмениПоляОбъединения(ПолеОбъединения = Неопределено)
	...
	Если ПолеОбъединения = Неопределено Тогда
		ПолеОбъединения = ЭлементыФормы.ПоляОбъединения.ТекущаяСтрока;
	КонецЕсли;
	ПолеОбъединения.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ПоляОбъединения, ПолеОбъединения,, Ложь,
...
Процедура ПоляОбъединенияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	КонецЕсли; 
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
	ИндексКартинкиНеопределено = ирОбщий.ИндексКартинкиТипаЗначенияБДЛкс(Новый ОписаниеТипов);
	Для Каждого ЧастьОбъединения Из ЧастиОбъединения Цикл
		...
		Если ЗначениеЗаполнено(ИДПоля) Тогда
			ВыбранноеПоле = ЧастьОбъединения.ВыбранныеПоля.Найти(ИДПоля, "ИД");
			Если ВыбранноеПоле <> Неопределено Тогда
				ИндексКартинки = ирОбщий.ИндексКартинкиТипаЗначенияБДЛкс(ВыбранноеПоле.ТипЗначения);
...
Процедура ПереключитьРежимПроизвольногоВыраженияОтбора(ЭлементОтбора)
	...
	Если ТекущееЗначениеФлажка Тогда
		...
		Иначе
			Ответ = Вопрос("Преобразование к простому условию не может быть выполнено. Заменить на пустое простое условие?", РежимДиалогаВопрос.ОКОтмена,, КодВозвратаДиалога.Отмена);
...
Процедура КПДоступныеТаблицыОтборБезЗначенияВТекущейКолонке(Кнопка)
	...
	ирОбщий.ТабличноеПолеОтборДляЗначенияВТекущейКолонкеЛкс(ЭлементыФормы.ДоступныеТаблицы);
...
Функция КомпоновщикОтбораВПараметре(Знач ВыбраннаяСтрока, Знач ПолноеИмяТаблицы, выхПолноеИмяТаблицыПараметра = "")
	...
	Если ЭтоПараметрОтбор1С(ВыбраннаяСтрока) Тогда
		...
		Тогда
			...
			Иначе
				...
				//КонецЕсли;
			КонецЕсли; 
			ЗапросИмитатор = ирОбщий.ПолучитьЗапросИмитаторКоллекцииПолейЛкс(ДоступныеПоля.Колонки);
			СхемаПараметра = ирОбщий.СоздатьСхемуКомпоновкиПоЗапросу(ЗапросИмитатор);
...
Процедура ВыбранныеТаблицыПередУдалением(Элемент, Отказ)
	...
	Если Ответ <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	Для Каждого ВыделеннаяСтрока Из ирОбщий.ВыделенныеСтрокиТабличногоПоляЛкс(ЭлементыФормы.ВыбранныеТаблицы) Цикл
...
//Процедура ДоступныеПоляВыбораПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
//	
...
Функция ВыбранныеПоляИмяПриИзменении(Элемент = Неопределено, Знач СтрокаВыбранногоПоля = Неопределено, Знач ЧастьОбъединения = Неопределено, РазрешитьОбрезку = Неопределено)
	...
	Если СтрокаВыбранногоПоля.Автополе Тогда
		НовоеИмя = ПолучитьАвтоПсевдонимПоля(СтрокаВыбранногоПоля, ЧастьОбъединения);
	...
	Если Не СтрокаВыбранногоПоля.Автополе Тогда 
		Если ЧастьОбъединения = мТекущаяСтрокаЧастиОбъединения Тогда
...
Процедура ГруппировкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
    ...
    ирОбщий.МассивЗначенийПеретаскиванияЛкс(ПараметрыПеретаскивания, МассивЗначений, ТипЗначенияПеретаскивания);
...
Процедура СвязиТаблицПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Тогда
		...
		Тогда
			...
			ирОбщий.СкопироватьЭлементыКомпоновкиЛкс(Строка.Условие.Отбор, Новый Структура("Элементы", МассивЗначений));
			ЭлементыФормы.СвязиТаблиц.ОбновитьСтроки();
			ЗагрузитьСвязьТаблиц();
		КонецЕсли;
	КонецЕсли; 
	...
	Иначе
		...
		Иначе
			Если ДанныеСтроки.Родитель <> Неопределено Тогда
				...
				ирОбщий.ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, СтрокаПоля, СтрокаДоступнойТаблицы,, ЭтоИндекс, ПолеИспользовано);
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли; 
...
Функция ОписаниеДоступногоПоля(Знач ДоступноеПоле, выхСтрокаДоступнойТаблицы = Неопределено)
	...
	Если СтрокаВыбраннойТаблицы <> Неопределено Тогда
		...
		Если выхСтрокаДоступнойТаблицы <> Неопределено Тогда
			ПоляТаблицы = ПоляДоступнойТаблицы(выхСтрокаДоступнойТаблицы);
...
Процедура ПоляДоступнойТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ЭлементыФормы.ДоступныеТаблицы.ТекущаяСтрока <> Неопределено Тогда
		ирОбщий.ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.ДоступныеТаблицы.ТекущаяСтрока, "Имя");
	КонецЕсли; 
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
...
Процедура КПВыбранныеПоляДекларироватьТипы(Кнопка = Неопределено, ЧастьОбъединения = Неопределено)
	...
	Для Каждого СтрокаВыбранногоПоля Из ЧастьОбъединения.ВыбранныеПоля Цикл
		...
		Для Каждого ТипПоля Из СтрокаВыбранногоПоля.ТипЗначения.Типы() Цикл
			...
			|	КОГДА ЛОЖЬ
			|		ТОГДА " + ирОбщий.ПустоеЗначениеПоТипуНаЯзыкеЗапросовЛкс(ТипПоля, СтрокаВыбранногоПоля.ТипЗначения);
		КонецЦикла;
		НовоеОпределение = НовоеОпределение + "
		|КОНЕЦ";
...
Процедура ДобавитьПолеЧастиОбъединенияВГруппирующиеПоля(Знач ВыбраннаяСтрока)
	...
	ДобавитьДоступноеПолеВТаблицуВыражений(ВыбраннаяСтрока, ЭлементыФормы.ГруппирующиеПоляИтогов,,,,,, Новый Структура("ТипИтогов", "Элементы"),, "");
	ОбновитьРолиИтогов();
...
Процедура ЗаменитьПравоеЗначениеЭлементаОтбораПараметром(Знач ВыбраннаяСтрока, XMLТип = Неопределено)
	...
	Иначе
		...
		Иначе
			ИмяПараметра = ДобавитьЗначениеВПараметры(ВыбраннаяСтрока.ПравоеЗначение,, БазовоеИмяПараметра);
		КонецЕсли; 
	КонецЕсли; 
...
Функция ВлияющиеНаВыбранноеПолеПоляМетаданных(Знач ИмяПоля) Экспорт 
	...
	Если ПолеОбъединения = Неопределено Тогда
		Возврат Результат;
	КонецЕсли; 
	СтруктураОбработкиПолей = НоваяСтруктураОбработкиПолей("ПоискПолейМетаданных");
	СтруктураОбработкиПолей.РазрешитьПроверкуСуществованияТаблиц = Истина;
	ПоляДляПровеки = Новый СписокЗначений;
	ПоляДляПровеки.Добавить(мТекущаяСтрокаЗапросыПакета, ИмяПоля);
				...
				Для Каждого СтрокаПоля Из СтруктураОбработкиПолей.СоответствиеПолей Цикл
					...
					Иначе
						...
						Тогда 
							//
						Иначе
							ПолноеИмяПоля = ДоступнаяТаблица.ПолноеИмя + "." + КраткоеИмяПоля;
					...
					#КонецЕсли
					ИмяДочернегоПоля = "";
					Пока ДоступноеПоле.Родитель <> Неопределено Цикл
						Для Каждого Тип Из ДоступноеПоле.ТипЗначения.Типы() Цикл
							Если Не ирОбщий.ЛиТипСсылкиБДЛкс(Тип) Тогда
								Продолжить;
							КонецЕсли; 
							ПолноеИмяМД = Метаданные.НайтиПоТипу(Тип).ПолноеИмя();
							ПолноеИмяМД = ПолноеИмяМД + "." + ИмяДочернегоПоля; // Пустое ИмяДочернегоПоля тоже подходит
							Если Результат.НайтиПоЗначению(ПолноеИмяМД) = Неопределено Тогда
								Результат.Добавить(ПолноеИмяМД);
							КонецЕсли; 
						КонецЦикла;
...
Процедура ЧастиОбъединенияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	...
	Тогда
		Если ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Копирование Тогда
			...
			СкопироватьЧастьОбъединения(мТекущаяСтрокаЗапросыПакета, ПараметрыПеретаскивания.Значение, ПозицияВставки);
		КонецЕсли; 
		Возврат;
	КонецЕсли; 
...
Процедура ВыбранныеТаблицыВсеПоляПриИзменении(Элемент)
	Если Элемент.Значение Тогда
		ДобавитьВсеПоляТаблицыВВыбранныеПоля(ЭлементыФормы.ВыбранныеТаблицы.ТекущаяСтрока.Имя,,,, Истина);
	Иначе
...
Процедура ВыбранныеПоляНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	//ПараметрыПеретаскивания.Значение = ирОбщий.ВыделенныеСтрокиТабличногоПоляЛкс(Элемент); // Так платформа будет делать копию таблицы
...
Процедура ВыбранныеПоляОпределениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Если ТекущееПолеОбъединения <> Неопределено Тогда
		...
		Если ТипЗначения.Типы().Количество() < 5 Тогда
			Для Каждого Тип Из ТипЗначения.Типы() Цикл
...
Функция ТекущееПолеОбъединения(Знач ВыбранноеПоле = Неопределено)
	...
	КонецЕсли;
	Результат = ПоляОбъединения.Найти(ВыбранноеПоле.ИД, "_" + мТекущаяСтрокаЧастиОбъединения.Номер);
	Возврат Результат;
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКлсПолеТекстаПрограммы.Форма.КонструкторЗапроса");
...
ирОбщий.ПодключитьОбработчикиСобытийДоступныхПолейКомпоновкиЛкс(ЭлементыФормы.ДоступныеПоляОбъединения);
