	...
	Иначе 
		...
		Тогда
			...
			Тогда
				Возврат Неопределено;
			КонецЕсли; 
			ВыражениеПоля = ПолучитьПолноеИмяДоступногоПоля(СтрокаДоступногоПоля, ТипЗначенияПоля);
		Иначе
...
Функция ДобавитьДоступноеПолеВВыбранныеПоля(Знач СтрокаДоступногоПоля, Знач Автополе = Ложь, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	...
	КонецЕсли; 
	ТипЗначенияПоля = Неопределено;
	ОпределениеПоля = ПолучитьПолноеИмяДоступногоПоля(СтрокаДоступногоПоля, ТипЗначенияПоля);
...
Функция ПолучитьПолноеИмяДоступногоПоля(Знач ДоступноеПоле, выхТипЗначенияПоля = Ложь)
