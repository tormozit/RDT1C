...
Процедура ОбновлениеОтображения()
	...
	Если ПараметрыДиалектаSQL <> Неопределено Тогда
		...
		ЭлементыФормы.ПанельОсновная.Страницы.Итоги.Доступность = ТипЗапроса < 2 И ЛиКорневойЗапрос();
...
Функция ПереименоватьТаблицуИлиПараметр(Пакет = Неопределено, Знач СтароеИмяТаблицыИлиПараметра = "", Знач НовоеИмяТаблицыИлиПараметра = "", Знач СтароеИмяПоля = "", Знач НовоеИмяПоля = "",
	Знач ГлобальныйОбход = Истина, Знач ЗапросПакета = Неопределено) Экспорт 
	...
	Для Каждого СтрокаПоля Из ПоляДоступнойТаблицы Цикл
		...
		Если МетаданныеПоля <> Неопределено Тогда
			...
			Исключение
				Продолжить;
			КонецПопытки;
			СтрокаПоля.Описание = СтрокаПоля.Описание + Символы.ПС + ПодсказкаПоля;
		КонецЕсли; 
	КонецЦикла;
	ПоляДоступнойТаблицыОбновитьВидимостьКолонкиИспользовано();
			...
			Если ДоступноеПолеТаблицыРодителя <> Неопределено Тогда
				...
				Если ГруппаДоступныхПолей <> Неопределено Тогда
					...
					Для Каждого ДоступноеПолеЛевое Из ГруппаДоступныхПолей.Элементы Цикл
						...
						КраткоеИмяЛевое = ирОбщий.ПоследнийФрагментЛкс(ДоступноеПолеЛевое.Поле);
						Для Каждого ДоступноеПолеПравое Из ДоступноеПолеТаблицыРодителя.Элементы Цикл
							ДобавитьСвязь = Ложь;   
							КраткоеИмяПравое = ирОбщий.ПоследнийФрагментЛкс(ДоступноеПолеПравое.Поле);
							...
							Тогда 
								...
								КонецЕсли; 
							КонецЕсли; 
							ЛиПростыеТипы = ДоступноеПолеЛевое.ТипЗначения.Типы().Количество() = 1 И ДоступноеПолеПравое.ТипЗначения.Типы().Количество() = 1;
							Если Истина
								И Не ДобавитьСвязь
							...
							Если ДобавитьСвязь Тогда
								Если КраткоеИмяПравое = "Ссылка" Или ЛиИменаПолейСовпадают Тогда
									ВозможныеПары.Вставить(0, ДоступноеПолеПравое.Поле);
								Иначе
									ВозможныеПары.Добавить(ДоступноеПолеПравое.Поле);
								КонецЕсли;
							КонецЕсли; 
						КонецЦикла;
						Счетчик = 0;
						Для Каждого ПравоеПоле Из ВозможныеПары Цикл
							Счетчик = Счетчик + 1;
							Если Ложь
								Или Счетчик = 3
								Или (Истина
									...
									И ирОбщий.ПоследнийФрагментЛкс(ПравоеПоле) <> "Ссылка")
							Тогда
								Прервать;
							КонецЕсли;
							ЭлементОтбора = ирОбщий.НайтиДобавитьЭлементОтбораКомпоновкиЛкс(НоваяНастройкаКомпоновки, ДоступноеПолеЛевое.Поле, ПравоеПоле,,, Ложь);
...
Процедура ИмяОсновнойТаблицыПриИзменении(Элемент)
	...
	Иначе
		ОбновитьНаименованиеЗапроса(мТекущаяСтрокаЗапросыПакета);
		ПереименоватьТаблицуИлиПараметр(, мТекущееИмяОсновнойТаблицы, ИмяОсновнойТаблицы,,,, мТекущаяСтрокаЗапросыПакета);
	КонецЕсли; 
...
Процедура ВыбранныеПоляПередУдалением(Элемент, Отказ)
	...
	Если Ложь
		...
		Или мТекущаяСтрокаЗапросыПакета.ТипЗапроса = 1 И ЗапросыПакета.Количество() > мТекущаяСтрокаЗапросыПакета.Индекс + 1 
	Тогда 
		...
		КоличествоУдалено = 0;
		Если ЛиКорневойЗапрос() Тогда
			ИмяВнешнейТаблицы = мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблиц;
			СтрокаЗапросыПакета = мТекущаяСтрокаЗапросыПакета;
		Иначе 
			ИмяВнешнейТаблицы = ВладелецФормы.ЭлементыФормы.ВыбранныеТаблицы.ТекущаяСтрока.Имя;
			СтрокаЗапросыПакета = ВладелецФормы.ЭлементыФормы.ЗапросыПакета.ТекущаяСтрока;
		КонецЕсли;
		...
		Для Каждого СтрокаПоля Из Кандидаты Цикл
			...
			Если ПроверятьСсылки Тогда   
				Если ЛиКорневойЗапрос() Тогда
					КоличествоПоПолю = ПереименоватьТаблицуИлиПараметр(, ИмяВнешнейТаблицы, ИмяВнешнейТаблицы, СтрокаПоля.Имя, СтрокаПоля.Имя, Истина, СтрокаЗапросыПакета);
				Иначе
					КоличествоПоПолю = ВладелецФормы.ПереименоватьТаблицуИлиПараметр(, ИмяВнешнейТаблицы, ИмяВнешнейТаблицы, СтрокаПоля.Имя, СтрокаПоля.Имя, Ложь);
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоУдалено > 0 Тогда
			Если ЛиКорневойЗапрос() Тогда 
				ЗаказатьРасширеннуюПроверкуЕслиЗапросСоздаетВременнуюТаблицу(СтрокаЗапросыПакета);
			Иначе
				...
				Если СтрокаПоляОбъединенияПоИмени <> СтрокаПоляОбъединенияПоИД Тогда 
					...
					Если СтрокаПоляОбъединенияПоИмени = Неопределено Тогда
						Если СтрокаПоляОбъединенияПоИД.Монополе Тогда
							...
							Если ТипЗапроса = 1 Тогда // Создание временной таблицы
								ПереименоватьТаблицуИлиПараметр(, ИмяОсновнойТаблицы, ИмяОсновнойТаблицы, СтароеИмя, НовоеИмя, Истина, мТекущаяСтрокаЗапросыПакета);
							КонецЕсли; 
						Иначе
							// Разделение полей 
							СтрокаНовогоПсевдонима = ПоляОбъединения.Добавить();
...
Процедура КПДеревоУсловияНайтиВСписке(Кнопка)
	...
	ирКлиент.ПоказатьДоступноеПолеЭлементаНастроекКомпоновкиЛкс(ЭтаФорма, ЭлементыФормы.ДоступныеПоляУсловия, ЭлементыФормы.ДеревоУсловия);
...
Функция ПередУдалениемЗапросаСозданияВременнойТаблицы()
	...
	Если мТекущаяСтрокаЗапросыПакета.ТипЗапроса = 1 И ЗапросыПакета.Количество() > мТекущаяСтрокаЗапросыПакета.Индекс + 1  Тогда 
		КоличествоОбнаружено = ПереименоватьТаблицуИлиПараметр(, мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблицы, мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблицы,,,, мТекущаяСтрокаЗапросыПакета);
		Если КоличествоОбнаружено > 0 Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ПереименоватьТаблицуИлиПараметр(, мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблицы,,,,, мТекущаяСтрокаЗапросыПакета);
			Иначе
...
Процедура ЗапомнитьИсточникПерехода()
	...
	КонецЕсли; 
	Если мТекущаяСтрокаЧастиОбъединения <> Неопределено Тогда
		ИндексЧасти = мТекущаяСтрокаЧастиОбъединения.Владелец().Индекс(мТекущаяСтрокаЧастиОбъединения);
	Иначе
		ИндексЧасти = 0;
	КонецЕсли;
	АдресУхода = Новый Структура;
	АдресУхода.Вставить("ИндексЗапроса", ЗапросыПакета.Индекс(мТекущаяСтрокаЗапросыПакета));
...
Процедура ОсновныеДействияФормыВернутьсяИзПерехода(Кнопка)
	Если мИсторияПереходов = Неопределено Или мИсторияПереходов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	АдресВозврата = ирОбщий.ОбъектИзСтрокиXMLЛкс(мИсторияПереходов[0]);
	...
	Если ЧастиОбъединения.Количество() > ИндексЧасти Тогда
		Если ЭлементыФормы.ЧастиОбъединения.ТекущаяСтрока <> ЧастиОбъединения[ИндексЧасти] Тогда
			ЭлементыФормы.ЧастиОбъединения.ТекущаяСтрока = ЧастиОбъединения[ИндексЧасти];
			мИсторияПереходов.Удалить(0);
		КонецЕсли;
