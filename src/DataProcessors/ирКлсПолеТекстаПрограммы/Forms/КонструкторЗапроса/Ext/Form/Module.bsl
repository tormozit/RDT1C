...
Функция ОбновитьКомпоновщикЧастиОбъединения(ЧастьОбъединения = Неопределено, Принудительно = Истина)
	...
	Для Каждого СтрокаДоступнойТаблицы Из ДоступныеПоляВыбора.Строки Цикл
		...
		Для Каждого СтрокаДоступногоПоля Из СтрокаДоступнойТаблицы.Строки Цикл
			...
			Если Истина
				И ПолеНабора.Поле = "Ссылка" 
				И (Ложь
					Или ирОбщий.ЛиКорневойТипСсылкиЛкс(СтрокаДоступнойТаблицы.ТипТаблицы) 
					Или ирОбщий.ЛиТипВложеннойТаблицыБДЛкс(СтрокаДоступнойТаблицы.ТипТаблицы))
			Тогда
				ПолеНабора.Заголовок = СтрЗаменить(ПолеНабора.Заголовок, ".Ссылка", "._Ссылка");
			КонецЕсли;
...
Процедура ПоляДоступнойТаблицыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ЭлементыФормы.ДоступныеТаблицы.ТекущаяСтрока <> Неопределено Тогда
		ирКлиент.ОформитьСтрокуДоступногоПоляЛкс(ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.ДоступныеТаблицы.ТекущаяСтрока, "Имя");
	КонецЕсли; 
	ирКлиент.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки);
...
Процедура ПереключитьОтборыЗапроса(НовоеИспользование) Экспорт 
	...
	ПеренестиОдносторонниеУсловияСоединенияВОтбор();
	Для Каждого ЧастьОбъединения Из ЧастиОбъединения Цикл
		ЭлементыФормы.ЧастиОбъединения.ТекущаяСтрока = ЧастьОбъединения;
...
Процедура ПеренестиОдносторонниеУсловияСоединенияВОтбор() Экспорт 
	...
	Для Каждого СвязьТаблиц Из ирОбщий.ВсеСтрокиДереваЗначенийЛкс(СвязиТаблиц) Цикл
		...
		Если ЕстьЭлементРавенстваПолей Тогда
			ирОбщий.СкопироватьЭлементыКомпоновкиЛкс(КомпоновщикЧасти.Настройки.Отбор, УсловиеСвязи.Отбор.Элементы, Ложь, ЭлементыПереноса);
