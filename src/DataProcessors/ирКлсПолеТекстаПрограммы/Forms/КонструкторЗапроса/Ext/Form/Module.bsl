...
Функция ОбновитьКомпоновщикЧастиОбъединения(ЧастьОбъединения = Неопределено, Принудительно = Истина)
	...
	Если Параметры.Количество() > 0 Тогда
		...
		КонецЦикла;
	КонецЕсли; 
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоДеревуПолейЛкс(ДоступныеПоляВыбора, ПоказыватьИндексы, ПараметрыДиалектаSQL.МногоТаблиц, Не ЭтоВыборкаИзТаблицыПараметра(), МаркерПоляФункцииТипЗначения());
...
Процедура ЗаполнитьИИКонец(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено) Экспорт
	...
	РезультатИИ = ирОбщий.ОбъектИзСтрокиJSONЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(РезультатЗадания.Ответ, "```json", "```")); // см. ПрочитатьJSON(ОбразецРезультатаИИЖСОН())
...
Процедура ДействияФормыСоветоватьИИ(Кнопка)
	ТекстЗапроса = ТекстЗапросаИИСоветовПоУлучшениюКода(СобратьПолныйТекст());
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Текст", ТекстЗапроса);
	#Если Сервер И Не Сервер Тогда
		ОтправитьЗапросИИ(); 
	#КонецЕсли
	Результат = ирОбщий.ВыполнитьЗаданиеФормыЛкс("ОтправитьЗапросИИ", ПараметрыЗадания, ЭтаФорма, "СоветоватьИИ",, Кнопка, "СоветоватьИИКонец", мПлатформа.ЛиЗапросыИИНаСервере());
КонецПроцедуры
