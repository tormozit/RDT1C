...
Процедура ДобавитьТекстПредложенияПостроителя(ОбщийТекст, ПредложениеПостроителя, ТаблицаПолейПостроителя, ТабличноеПолеПостроителя, ЗапросПакета,
	ЧастьОбъединения = Неопределено, Смещение = "", Знач РазрешитьНормализациюИмен = Истина)
...
Функция ПолучитьОбновитьВыражениеПоляПостроителя(ПолеПостроителя, ТабличноеПоле = Неопределено, Знач ЗапросПакета, Знач ЧастьОбъединения = Неопределено, Знач РазрешитьНормализациюИмен = Истина)
	...
	ТекстВыражения = ОбновитьВыражениеЗапроса(ПолеПостроителя, ТабличноеПоле, ЗапросПакета, ЧастьОбъединения,,,,, РазрешитьНормализациюИмен, 0);
...
Функция СобратьТекстЧастиОбъединения(ЗапросПакета = Неопределено, ЧастьОбъединения = Неопределено, Смещение = "", Знач РазвернутьАвтополя = Ложь, Знач СтруктураПолей = Неопределено,
	...
	ДобавитьТекстПредложенияПостроителя(Результат, СловоЯзыкаЗапросовВФорме("SELECT"),
		ЧастьОбъединения.ПоляВыбораПостроителя, ЭлементыФормы.ПоляВыбораПостроителя, ЗапросПакета, ЧастьОбъединения, Смещение, Не РежимКомпоновкиДанных);
...
Процедура ДоступныеТаблицыОбновитьПодменюПоследниеВыбранные()
	...
	ирКлиент.ПоследниеВыбранныеЗаполнитьПодменюЛкс(ЭтаФорма, ЭлементыФормы.КПДоступныеТаблицы.Кнопки.ПоследниеВыбранные, ЭлементыФормы.ДоступныеТаблицы,,, КлючХраненияПоследнихВыбранных());
