				...
				Для ИндексВыбранногоПоля = 0 По ЧастьОбъединения.ВыбранныеПоля.Количество() - 1 Цикл
					...
					Иначе
						...
						Если ЗначениеЗаполнено(ВыбранноеПоле.Имя) И Не ирОбщий.СтрокиРавныЛкс(ВыбранноеПоле.Имя, НовоеИмя) Тогда
							...
							ирОбщий.СообщитьЛкс(ирОбщий.СтрШаблонЛкс("В запросе[%1] переименовано поле части[%3] объединения %4->%2. Старое имя в комментарии.", ЗапросПакета.Имя, ВыбранноеПоле.Имя, 
...
Процедура ДобавитьПробелПослеТокена(Текст, ИмяПредыдущегоТокенаВниз)
	...
	// Однострочный код здесь дал бы 20% ускорение, поэтому смысла не имеет
			...
			Для Каждого СтрокаЗапроса Из ЗапросыПакета Цикл
				...
				Для Каждого ВыбранноеПоле Из СтрокаЗапроса.ЧастиОбъединения[0].ВыбранныеПоля Цикл
					Если ирОбщий.СтрНайтиЛкс(ВыбранноеПоле.Определение, ВыбраннаяТаблица.Имя + ".") = 0 Тогда
						Продолжить;
					КонецЕсли;
					ВычислительРегВыражений.Pattern = "(?:\s|\(|,|^)" + ВыбраннаяТаблица.Имя + "\s*\.\s*(" + мПлатформа.шИмя + ")\s*" ;
					Вхождения = ВычислительРегВыражений.НайтиВхождения(ВыбранноеПоле.Определение);
...
Функция СобратьТекстПредложенияПостроителя(Знач ЗапросПакета, Знач ТаблицаПолейПостроителя, Знач ТабличноеПолеПостроителя = Неопределено, Знач ЧастьОбъединения = Неопределено, Знач Смещение = "")
	...
	Для Каждого ПолеПостроителя Из ТаблицаПолейПостроителя Цикл
		...
		Тогда
			ТекстПостроителя = ТекстПостроителя + " " + СловоЯзыкаЗапросовВФорме("AS") + " " + ИмяСНеобходимымиСкобками(ПолеПостроителя.Имя);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьДоступноеПолеСДочернимиВВыбранныеПоля(СтрокаДоступногоПоля, Автополе = Ложь, ЗапросПакета = Неопределено,
	...
	Иначе
		...
		//КонецЕсли; 
		// Одно поле
		ДобавитьДоступноеПолеВВыбранныеПоля(СтрокаДоступногоПоля, Автополе, ЗапросПакета, ЧастьОбъединения, СдвигИндексаПослеДобавления, КонтролироватьУникальность, ОбновитьПоляОбъединения);
	КонецЕсли; 

КонецПроцедуры

Процедура ДобавитьВсеПоляВыбраннойТаблицыВВыбранныеПоля(Знач СтрокаДоступногоПоля, Знач ЧастьОбъединения = Неопределено, Знач СдвигИндексаПослеДобавления = 0, Знач Автополе = Истина,
	...
	КонецЕсли;
	Для Каждого лСтрокаДоступногоПоля Из СтрокаДоступногоПоля.Элементы Цикл
		ДобавитьДоступноеПолеВВыбранныеПоля(лСтрокаДоступногоПоля, Автополе, ЗапросПакета, ЧастьОбъединения, СдвигИндексаПослеДобавления, КонтролироватьУникальность, Ложь);
	КонецЦикла;
...
Функция ДобавитьДоступноеПолеВВыбранныеПоля(Знач СтрокаДоступногоПоля, Знач Автополе = Ложь, Знач ЗапросПакета = Неопределено, Знач ЧастьОбъединения = Неопределено,
	Знач СдвигИндексаПослеДобавления = 0, Знач КонтролироватьУникальность = Ложь, Знач ОбновитьПоляОбъединения = Истина)
...
Процедура СвязиТаблицПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Условие <> Неопределено Тогда
		...
		НайденныеЭлементы = ирОбщий.ОтобратьЭлементыКоллекцииЛкс(НайденныеЭлементы, Новый Структура("ЛевоеЗначение"));
	...
	Если Токен.Kind = 0 Тогда
		...
		Если ИмяПравила = "<Field>" Тогда
			...
			Если Найти(Выражение, "(") > 0 Тогда
				Возврат Результат;
			КонецЕсли; 
			Результат = ПолучитьПолноеИмяПоляВВыражении(Выражение, ЗапросПакета, ЧастьОбъединения);
...
Процедура СохранитьЧастьОбъединения(РазрешитьДобавлениеАвтополей = Истина)
	...
	мТекущаяСтрокаЧастиОбъединения.ОпцияАвтоупорядочиваниеОтладки = ОпцияАвтоупорядочиваниеОтладки;
...
Процедура ЗагрузитьЧастьОбъединения()
	...
	Если мТекущаяСтрокаЧастиОбъединения <> Неопределено Тогда
		...
		ЭтаФорма.ОпцияАвтоупорядочиваниеОтладки = мТекущаяСтрокаЧастиОбъединения.ОпцияАвтоупорядочиваниеОтладки;
		//ОбновитьДоступныеВременныеТаблицы();
		...
		ЭтаФорма.ПоляПорядкаОтладки = мТекущаяСтрокаЧастиОбъединения.ПоляПорядкаОтладки;
...
Процедура ПриИзмененииИмениПоляОбъединения(ПолеОбъединения = Неопределено)
	...
	ПолеОбъединения.Имя = ирОбщий.АвтоУникальноеИмяВКоллекцииЛкс(ПоляОбъединения, ПолеОбъединения,, Ложь,
		СловоЯзыкаЗапросовВФорме("Field") + "1");
...
Процедура КПТекстСравнить(Кнопка)
	...
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.ПолеТекстаЗапроса);
...
ирОбщий.ПодключитьОбработчикиСобытийДоступныхПолейКомпоновкиЛкс(ЭлементыФормы.ДоступныеВыбранныеПоляПорядкаОтладки);
...
ЧастиОбъединения.Колонки.Добавить("ПоляПорядкаОтладки");
