...
Процедура ДобавитьВсеПоляВыбраннойТаблицыВВыбранныеПоля(Знач Родитель, Знач ЧастьОбъединения = Неопределено, Знач СдвигИндексаПослеДобавления = 0, Знач Автополе = Истина,
	...
	Если ЧастьОбъединения = Неопределено Тогда
		...
		ЧастьОбъединения = мТекущаяСтрокаЧастиОбъединения;
	КонецЕсли;
	Для Каждого Потомок Из Родитель.Элементы Цикл
		Если Потомок.Папка Тогда
			Продолжить;
		КонецЕсли;
		ДобавитьДоступноеПолеВВыбранныеПоля(Потомок, Автополе, ЗапросПакета, ЧастьОбъединения, СдвигИндексаПослеДобавления, КонтролироватьУникальность, Ложь);
	КонецЦикла;
