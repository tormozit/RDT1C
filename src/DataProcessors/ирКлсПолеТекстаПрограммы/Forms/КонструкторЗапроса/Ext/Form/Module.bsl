...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если ЭкземплярКомпоненты <> Неопределено Тогда
		Если Ложь
			Или Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ЭкземплярКомпоненты, "КонструкторЗапросовИР") 
			Или Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ЭкземплярКомпоненты, "КонструкторЗапросов1С")
		Тогда
		...
		Тогда
			...
			Иначе
				...
				ИначеЕсли ИмяПравила = "<CheckLIKE>" Тогда
					...
					ИначеЕсли ирКэш.ДоступныВидыСравненияПодобноШаблонЛкс() Тогда
...
Процедура ДиалектSQLОчистка(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ирОбщий.ИнтерактивноЗаписатьВПолеВводаЛкс(Элемент, мДиалектSQL);
	...
	Если ЭтоПараметрОтбор1С(ВыбраннаяСтрока) Тогда
		...
		Тогда
			...
			Иначе
				...
				//КонецЕсли;
			КонецЕсли; 
			ЗапросИмитатор = ирОбщий.ЗапросИмитаторКоллекцииПолейЛкс(ДоступныеПоля.Колонки);
...
Процедура УдалитьВыбранноеПоле(ВыбранноеПоле, Знач ЗапросПакета = Неопределено)
...
Процедура ЗаказатьРасширеннуюПроверкуЕслиЗапросСоздаетВременнуюТаблицу(Знач ЗапросСозданияТаблицы)
	...
	Тогда
		ЭтаФорма.РасширеннаяПроверка = Истина;
		Для Индекс = ЗапросыПакета.Индекс(ЗапросСозданияТаблицы) + 1 По ЗапросыПакета.Количество() - 1 Цикл
			ЗапросПакетаЦикл = ЗапросыПакета[Индекс];
...
Процедура ВыбранныеПоляПередУдалением(Элемент, Отказ)
	...
	Если мТекущаяСтрокаЗапросыПакета.ТипЗапроса = 1 И ЗапросыПакета.Количество() > мТекущаяСтрокаЗапросыПакета.Индекс + 1 Тогда 
		...
		Если КоличествоУдалено > 0 Тогда
			ЗаказатьРасширеннуюПроверкуЕслиЗапросСоздаетВременнуюТаблицу(мТекущаяСтрокаЗапросыПакета);
		КонецЕсли;
	Иначе
		Для Каждого СтрокаПоля Из Кандидаты Цикл
			УдалитьВыбранноеПоле(СтрокаПоля);
