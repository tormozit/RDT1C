...
Функция РасшифровкаЗначенияДляТекстаЗапроса(ЗначениеПоля, Знач НоваяФорма) Экспорт 
	...
	Иначе
		РасшифровкаПоля = ирОбщий.ПредставлениеСОграничениемДлиныЛкс(ЗначениеПоля);
...
Функция ОбновитьКомпоновщикЧастиОбъединения(ЧастьОбъединения = Неопределено, Принудительно = Истина)
	...
	Если Параметры.Количество() > 0 Тогда
		...
		КонецЦикла;
	КонецЕсли; 
	СхемаКомпоновки = ирОбщий.СоздатьСхемуКомпоновкиПоДеревуПолейЛкс(ДоступныеПоляВыбора, ПоказыватьИндексы, ПараметрыДиалектаSQL.МногоТаблиц, ЭтоВыборкаИзТаблицыПараметра(), МаркерПоляФункцииТипЗначения());
...
Процедура ОсновныеДействияФормыЗаполнитьИИ(Кнопка)
	...
	РезультатИИ = ГенерироватьИменаЗапросаИИ(СобратьПолныйТекст(), ИндексыЗапросовПакета);
	Для Каждого РезультатЭлемент Из РезультатИИ Цикл
		ЗапросПакета = ЗапросыПакета[РезультатЭлемент.index];
		Если ЗапросПакета.ТипЗапроса = 1 Тогда
...
Функция ГенерироватьИменаЗапросаИИ(Знач ТекстЗапроса, Знач ИндексыЗапросовПакета) Экспорт
	ТекстПромта = ТекстЗапросаГенерироватьИменаЗапросаИИ(ТекстЗапроса, ИндексыЗапросовПакета);
	ОтветМоделиСтр = ОтправитьЗапросИИ(Новый Структура("Текст", ТекстПромта));
	Если ПустаяСтрока(ОтветМоделиСтр) Тогда
		Возврат Неопределено; 
	КонецЕсли;
	Возврат ирОбщий.ОбъектИзСтрокиJSONЛкс(ирОбщий.ТекстМеждуМаркерамиЛкс(ОтветМоделиСтр, "```json", "```")); // см. ПрочитатьJSON(ОбразецРезультатаИИЖСОН())
КонецФункции 
...
Процедура ДействияФормыСоветоватьИИ(Кнопка)
	Результат = СоветоватьУлучшенияМетодаИИ(СобратьПолныйТекст());
