...
Процедура КПДеревоУсловияСвязиМаксДата(Кнопка)
	...
	ОтобраныВсеИзмеренияПравойТаблицы = ЛиВСвязиОтобраныВсеИзмеренияПравойТаблицы(, ЛеваяТаблица, ПраваяТаблица, ПолеДатыЛевойТаблицы, ПолеДатыПравойТаблицы, ЭлементОтбораДат);
	Если ЭлементОтбораДат = Неопределено Тогда
		Предупреждение("В условии связи не найден элемент, связывающий даты.");
		Возврат;
	КонецЕсли;
	Если ОтобраныВсеИзмеренияПравойТаблицы Тогда
	...
	СтрокаСвязиПравая.Условие = ирОбщий.КопияОбъектаЛкс(ЭлементыФормы.СвязиТаблиц.ТекущаяСтрока.Условие, Истина);
	...
	Для Каждого ЭлементОтбора Из КомпоновщикУсловияСвязи.Настройки.Отбор.Элементы Цикл
		Если Ложь
			Или ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных")
