...
Функция ПередУдалениемЗапросаСозданияВременнойТаблицы()
	...
	Если мТекущаяСтрокаЗапросыПакета.ТипЗапроса = 1 И ЗапросыПакета.Количество() > мТекущаяСтрокаЗапросыПакета.Индекс + 1  Тогда 
		КоличествоОбнаружено = ПереименоватьТаблицуИлиПараметр(, мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблицы, мТекущаяСтрокаЗапросыПакета.ИмяОсновнойТаблицы,,,, мТекущаяСтрокаЗапросыПакета).КоличествоОбнаружено;
