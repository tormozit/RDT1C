	...
	Иначе
		Если Открыта() Тогда
			...
			ИначеЕсли ВариантАвтоВызова = 2 Тогда
				ОткрытьАвтодополнение(ирКэш.КодыКлавишЛкс()["ctrl+space"],, Истина); // Так будет обязательное открытие. Вредно внутри ненадежного текстового литерала и комментария
				ОбновитьФормуАвтодополнения();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
