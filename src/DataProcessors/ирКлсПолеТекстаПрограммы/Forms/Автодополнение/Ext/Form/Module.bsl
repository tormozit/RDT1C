...
Функция ЗапуститьЗапросНапарникаВФорме()
	...
	Если Ложь
		...
		Или СтрокаСловаНапарника.Слово <> СтатусИИСпит()
	Тогда
		Возврат Ложь;
	КонецЕсли;
	КонтекстНапарника = мПлатформа.КонтекстНапарника();
	КонтекстНапарника.Браузер = ЭлементыФормы.СлужебноеПолеХТМЛ.Документ.defaultView;
	СоединениеНапарника();
	СтрокаСловаНапарника.Слово = "ИИ думает...";
	...
	ЗапроситьНапарникаАсинх(ЗапросНапарника);
	Возврат Истина;
КонецФункции 

Процедура СлужебноеПолеХТМЛonclick(Элемент, pEvtObj)
	ОбработатьОтветНапарника(Элемент.Документ.getElementById("responseContainer").innerText);
...
ЭлементыФормы.СлужебноеПолеХТМЛ.УстановитьТекст(мПлатформа.ПолучитьМакет("ФоновыйHTTPЗапрос").ПолучитьТекст());
