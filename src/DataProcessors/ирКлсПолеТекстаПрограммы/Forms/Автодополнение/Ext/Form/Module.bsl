...
Функция ЗапуститьЗапросНапарникаВФорме()
	...
	КонтекстНапарника = мПлатформа.КонтекстНапарника();
	КонтекстНапарника.ФоновыйЗапросХттп = мФоновыйЗапросХттп;
	...
	Попытка
		ЗапроситьНапарникаАсинх(ЗапросНапарника, Истина);
	Исключение
...
Процедура СлужебноеПолеХТМЛonclick(Элемент, pEvtObj) Экспорт
	...
	КонецЕсли;
	Отказ = Ложь;
	Результат = ОбработатьОтветНапарника(ЧастьОтвета,, ЛиПерваяЧасть, Отказ);
	Если Отказ Тогда
		мФоновыйЗапросХттп.ОтменитьЗапрос();
