...
Процедура ОбновитьОписаниеСлова() Экспорт
	...
	Если СтруктураОписания <> Неопределено Тогда
		Если ТекущаяСтрока.ТипЗначения = "ИИ" Тогда
			ТекстХТМЛ = мПлатформа.ПолучитьМакет("ПросмотрMarkdownHTML").ПолучитьТекст();
			ТекстХТМЛ = СтрЗаменить(ТекстХТМЛ, "%ТекстJS%", ""); 
...
Процедура ОписаниеДокументСформирован(Элемент)
	...
	КонецЕсли;
	Если ТекущаяСтрока.ТипЗначения = "ИИ" Тогда 
		Элемент.Документ.defaultView.setText(ТекущаяСтрока.Слово, "9pt");
	КонецЕсли;
КонецПроцедуры
