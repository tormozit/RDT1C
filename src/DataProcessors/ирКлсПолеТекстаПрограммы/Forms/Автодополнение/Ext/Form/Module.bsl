...
Процедура НайтиПодходящиеСлова(Знач СброситьТекущуюСтроку = Истина)
	...
	Если ЭлементыФормы.ДействияФормы.Кнопки.ПереключитьОтборИСортировку.Пометка Тогда
		...
		Если СброситьТекущуюСтроку И ПодходящиеСлова.Количество() Тогда
			ИндексЛучшего = 0;
			Если Истина
				И ЛиАвтодополнениеНапарника()
				И ПустаяСтрока(ТекущееСлово)
...
Процедура ПриОткрытии()
	...
	Если ЛиАвтодополнениеНапарника() Тогда
		...
		СтрокаСлова.Рейтинг = 10 * ШагРейтингаСлова();
		Если ПродолжатьСловоНапарника И ЗначениеЗаполнено(мКонтекстНапарника.ОстатокСлова) Тогда
			СтрокаСлова.Слово = мКонтекстНапарника.ОстатокСлова;
		Иначе
			СтрокаСлова.Слово = СтатусИИСпит();
			ПодключитьОбработчикОжидания("ЗапуститьЗапросНапарникаВФорме", 2, Истина);
		КонецЕсли;
		мКонтекстНапарника.ОстатокСлова = "";
	КонецЕсли;
КонецПроцедуры 
...
Функция ЗапуститьЗапросНапарникаВФорме()
	...
	ЗапроситьНапарника(ЭлементыФормы.СлужебноеПолеХТМЛ.Документ.defaultView);
	Возврат Истина;
КонецФункции 

Процедура СлужебноеПолеХТМЛonclick(Элемент, pEvtObj)
	ОбработатьОтветНапарникаНаКлиенте(Элемент.Документ.getElementById("responseContainer").innerText);
