...
Процедура СлужебноеПолеХТМЛonclick(Элемент, pEvtObj) Экспорт
	...
	Если Истина
		И Не ЛиПерваяЧасть 
		И Не мФоновыйЗапросХттп.ЛиПотокЗавершен()
	Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЛиАвтодополнениеНапарника() Тогда
		...
		Если Истина
			...
			И (Ложь
				...
				Или ТекущееСлово <> "" И Не ирОбщий.СтрНачинаетсяСЛкс(СтрокаСловаНапарника.Слово, ТекущееСлово))
		Тогда
			СтрокаСловаНапарника.Слово = СтатусИИСпит();
			ПодключитьОбработчикОжидания("ЗапуститьЗапросНапарникаВФорме", 1, Истина);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
