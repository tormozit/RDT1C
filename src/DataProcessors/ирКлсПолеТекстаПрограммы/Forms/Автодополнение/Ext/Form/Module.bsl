...
// В описании этой функции локально термин "слово" означает часть элемента списка от заглавной буквы до следующей заглавной
...
// В определении функции термин "слово" означает весь элемент списка.
//
// Параметры:
...
Функция НайтиФильтрВСлове(Знач Строка, Знач ВыделитьВхождения = Ложь)
	...
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
		...
		//СловаФильтра = ирОбщий.СтрРазделитьЛкс(ирОбщий.ПредставлениеИзИдентификатораЛкс(ТекущееСлово), " ");
		//Если Ложь
		//	Или СловаФильтра.Количество() > 1
		//	Или НРег(Лев(ТекущееСлово, 1)) = Лев(ТекущееСлово, 1)
		//Тогда
		//	Для Каждого СловоФильтра Из СловаФильтра Цикл
		//		ПозицияВхождения = Найти(СловоСтроки, ТРег(СловоФильтра));
		...
		Если ПозицияВхождения > 0 Тогда
			...
			ИначеЕсли ПозицияВхождения = 2 И Не ирОбщий.ЛиБукваЛкс(Лев(СловоСтроки, 1)) Тогда
				ПремияФильтра = 3; // Полное совпадение в начале после первой небуквы
			Иначе
				ПервыйСимвол = Сред(СловоСтроки, ПозицияВхождения, 1);
				Если ВРег(ПервыйСимвол) = ПервыйСимвол Тогда
					ПремияФильтра = 3; // полное совпадение внутри с начала подслова
			...
			Для ПозицияВФильтре = 1 По ДлинаФильтра Цикл
				...
				Пока ПозицияВСлове <= ДлинаСлова Цикл
					...
					Если СимволСлова <> СимволФильтра Тогда
						...
						НоваяПозицияСлова = Найти(Сред(СловоСтроки, ПозицияВСлове + 1), ТРег(ФрагментФильтра)); // 
						Если НоваяПозицияСлова = 0 И ФрагментФильтра <> СимволФильтра Тогда
							НоваяПозицияСлова = Найти(Сред(СловоСтроки, ПозицияВСлове + 1), ВРег(СимволФильтра));
						КонецЕсли;
						...
						ПропущенныеФрагменты.Добавить(ПропущенныйФрагмент);
				...
				ПозицияВСлове = ПозицияВСлове + 1;
			КонецЦикла;
			Если ПремияФильтра > 1 Тогда
				БылиПропускиЗаглавныхБукв = Ложь;
				Для Каждого РезультатЭлемент Из ПропущенныеФрагменты Цикл
					Если НРег(РезультатЭлемент) <> РезультатЭлемент Тогда
						БылиПропускиЗаглавныхБукв = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если Не БылиПропускиЗаглавныхБукв Тогда
					ПремияФильтра = ПремияФильтра + 1;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		СловоСтроки = Строка.Слово;  		Если Не ЗначениеЗаполнено(ТекущееСлово) Тогда  			Строка.ПремияФильтра = 1;  			Возврат СловоСтроки;  		КонецЕсли;  		ВыделениеРезультатовПоиска = мПлатформа.ВыделениеРезультатовПоиска;  		Фильтр = НРег(ТекущееСлово);  		ДлинаФильтра = СтрДлина(Фильтр);  		ДлинаСлова = СтрДлина(СловоСтроки);  		Результат = Новый Массив;  		НСловоСтроки = Строка.НСлово;  		ПозицияВхождения = Найти(НСловоСтроки, Фильтр);                          		Если ПозицияВхождения > 0 Тогда  			ПредСовпадение = Истина;  			ПозицияВСлове = ПозицияВхождения + ДлинаФильтра;  			НайденныйФрагмент = Сред(СловоСтроки, ПозицияВхождения, ДлинаФильтра);  			Если ПозицияВхождения = 1 Тогда  				ПремияФильтра = 4;  			ИначеЕсли ПозицияВхождения = 2 И Не ирОбщий.ЛиБукваЛкс(Лев(СловоСтроки, 1)) Тогда  				ПремияФильтра = 3;  			Иначе  				ПервыйСимвол = Сред(СловоСтроки, ПозицияВхождения, 1);  				Если ВРег(ПервыйСимвол) = ПервыйСимвол Тогда  					ПремияФильтра = 3;  				ИначеЕсли НРег(НайденныйФрагмент) = НайденныйФрагмент Тогда  					ПремияФильтра = 2;  				Иначе  					ПремияФильтра = 1;  				КонецЕсли;  			КонецЕсли;  			Если ПозицияВхождения > 1 Тогда  				Результат.Добавить(Лев(СловоСтроки, ПозицияВхождения - 1));  			КонецЕсли;  			Результат.Добавить(Лев(ВыделениеРезультатовПоиска, 1));  			Результат.Добавить(НайденныйФрагмент);  		Иначе  			ПремияФильтра = 1;  			ПозицияВСлове = 1;  			ПредСовпадение = Ложь;  			ПревышениеДлиныСлова = ДлинаСлова - ДлинаФильтра;  			ПозицияПервогоСовпадения = 0;  			ПропущенныеФрагменты = Новый Массив;  			Для ПозицияВФильтре = 1 По ДлинаФильтра Цикл  				СимволФильтра = Сред(Фильтр, ПозицияВФильтре, 1);  				ФрагментФильтра = Сред(Фильтр, ПозицияВФильтре, 2);  				Пока ПозицияВСлове <= ДлинаСлова Цикл  					СимволСлова = Сред(НСловоСтроки, ПозицияВСлове, 1);  					Если СимволСлова <> СимволФильтра Тогда  						Если ПредСовпадение Тогда  							Если ВыделитьВхождения Тогда  								Результат.Добавить(Прав(ВыделениеРезультатовПоиска, 1));  							КонецЕсли;  							Если ПремияФильтра = 1 И ПозицияПервогоСовпадения = 1 Тогда  								ПремияФильтра = 2;  							КонецЕсли;  						КонецЕсли;  						ПредСовпадение = Ложь;      						НоваяПозицияСлова = Найти(Сред(СловоСтроки, ПозицияВСлове + 1), ТРег(ФрагментФильтра));  						Если НоваяПозицияСлова = 0 И ФрагментФильтра <> СимволФильтра Тогда  							НоваяПозицияСлова = Найти(Сред(СловоСтроки, ПозицияВСлове + 1), ВРег(СимволФильтра));  						КонецЕсли;  						Если НоваяПозицияСлова = 0 Тогда  							ПозицияВСлове = ДлинаСлова + 1;  							Прервать;  						КонецЕсли;  						ПропущенныйФрагмент = Сред(СловоСтроки, ПозицияВСлове, НоваяПозицияСлова);  						ПропущенныеФрагменты.Добавить(ПропущенныйФрагмент);  						ПозицияВСлове = ПозицияВСлове + НоваяПозицияСлова;  					Иначе  						ПропущенныйФрагмент = "";  					КонецЕсли;  					ПодходитСимвол = ПредСовпадение Или ПозицияВСлове = 1;  					Если Не ПодходитСимвол Тогда  						СимволСлова = Сред(СловоСтроки, ПозицияВСлове, 1);  						ПодходитСимвол = ВРег(СимволСлова) = СимволСлова;  					КонецЕсли;  					Если ПодходитСимвол Тогда  						Если ВыделитьВхождения Тогда  							Результат.Добавить(ПропущенныйФрагмент);  							Если Не ПредСовпадение Тогда  								Результат.Добавить(Лев(ВыделениеРезультатовПоиска, 1));  							КонецЕсли;  							Результат.Добавить(Сред(СловоСтроки, ПозицияВСлове, 1));  						КонецЕсли;  						ПредСовпадение = Истина;  						Если ПозицияПервогоСовпадения = 0 Тогда  							ПозицияПервогоСовпадения = ПозицияВСлове;  						КонецЕсли;  						Прервать;  					КонецЕсли;  					Если ВыделитьВхождения Тогда  						Результат.Добавить(ПропущенныйФрагмент + Сред(СловоСтроки, ПозицияВСлове, 1));  					КонецЕсли;  					ПозицияВСлове = ПозицияВСлове + 1;  					ПредСовпадение = Ложь;  					Если ПозицияВСлове > ПревышениеДлиныСлова + ПозицияВФильтре Тогда  						Прервать;  					КонецЕсли;  				КонецЦикла;  				Если ПозицияВСлове > ПревышениеДлиныСлова + ПозицияВФильтре Тогда  					Если Ложь  						Или ПозицияПервогоСовпадения = 0  						Или ПозицияПервогоСовпадения + 1 > ПревышениеДлиныСлова + ПозицияВФильтре  					Тогда  						ПремияФильтра = 0;  						Прервать;  					КонецЕсли;  					ПремияФильтра = 1;  					ПозицияВФильтре = 0;  					ПозицияВСлове = ПозицияПервогоСовпадения + 1;  					Если ВыделитьВхождения Тогда  						Результат.Очистить();  						Результат.Добавить(Лев(СловоСтроки, ПозицияВСлове));  					КонецЕсли;  					ПозицияПервогоСовпадения = 0;  				КонецЕсли;  				ПозицияВСлове = ПозицияВСлове + 1;  			КонецЦикла;  			Если ПремияФильтра > 1 Тогда  				БылиПропускиЗаглавныхБукв = Ложь;  				Для Каждого РезультатЭлемент Из ПропущенныеФрагменты Цикл  					Если НРег(РезультатЭлемент) <> РезультатЭлемент Тогда  						БылиПропускиЗаглавныхБукв = Истина;  						Прервать;  					КонецЕсли;  				КонецЦикла;  				Если Не БылиПропускиЗаглавныхБукв Тогда  					ПремияФильтра = ПремияФильтра + 1;  				КонецЕсли;  			КонецЕсли;  		КонецЕсли;  		Если ВыделитьВхождения И ПремияФильтра > 0 И ПредСовпадение Тогда  			Результат.Добавить(Прав(ВыделениеРезультатовПоиска, 1));  			Результат.Добавить(Сред(СловоСтроки, ПозицияВСлове));  		КонецЕсли;  		Если ВыделитьВхождения Тогда  			Возврат ирОбщий.СтрСоединитьЛкс(Результат, "");  		Иначе  			Строка.ПремияФильтра = ПремияФильтра;  		КонецЕсли;  
	КонецЕсли;
КонецФункции
