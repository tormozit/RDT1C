...
Процедура ВызовыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Иначе
		Если ВыбраннаяСтрока.Модуль = "<МодульКонсолиКода>" Тогда 
...
Процедура ДействияФормыНайти(Кнопка = Неопределено)
	...
	#Если Сервер И Не Сервер Тогда
		мПлатформа = Обработки.ирПлатформа.Создатьц();
	#КонецЕсли
	...
	Для Каждого Файл Из ФайлыМодулей Цикл
		...
		Для Счетчик = 1 По 2 Цикл
			...
			Для Каждого ВхождениеВызова Из ВхожденияВызова Цикл
				...
				ПозицияСкобки = Найти(ВхождениеВызова.ТекстВхождения, "(");      
				Если ПозицияСкобки > 0 Тогда 
					ТекстВызова = Сред(ВхождениеВызова.ТекстВхождения, ПозицияСкобки + 1);
				...
				Если ЗначениеЗаполнено(МаксЧислоРезультатов) И ВызовыМетода.Количество() >= МаксЧислоРезультатов Тогда
					...
					Перейти ~КонецЦикла;
				КонецЕсли; 
				Если Не ПерваяПорцияОтображена Тогда
...
Процедура УточнитьСтрокуРезультата(ТекущаяСтрока) Экспорт
	Если ТекущаяСтрока.Модуль = "<МодульКонсолиКода>" Тогда
