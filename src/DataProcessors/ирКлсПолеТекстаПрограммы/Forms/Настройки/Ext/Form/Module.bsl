...
Процедура ОсновныеДействияФормыОК(Кнопка = Неопределено)
	...
	ЭтотОбъект.мКонтекстНапарника = Неопределено;
	...
	мПлатформа.МаксОжиданиеИИ = МаксОжиданиеИИ;
	мПлатформа.ЛиЗапросыИИНаСервере = ЛиЗапросыИИНаСервере;
...
Процедура ПриОткрытии()
	...
	СписокЗначений = ЭлементыФормы.МаксОжиданиеИИ.СписокВыбора;
	СписокЗначений.Добавить(60);
	СписокЗначений.Добавить(120);
	СписокЗначений.Добавить(300);
...
Процедура ПрочитатьОбновитьНастройки() Экспорт
	...
	ЭтаФорма.МаксОжиданиеИИ = мПлатформа.МаксОжиданиеИИ();
	ЭтаФорма.ЛиЗапросыИИНаСервере = мПлатформа.ЛиЗапросыИИНаСервере();
...
Процедура ПроверитьИИНажатие(Элемент)
	ирОбщий.ОтправитьЗапросИИЛкс(Новый Структура("текст", "привет"),, Истина, КлючСервисаИИ, СерверИИ, МодельИИ);
КонецПроцедуры
...
Процедура ПроверитьНапарникаНажатие(Элемент)
	КопияКонтекстНапарника = КонтекстНапарника();
	КопияКонтекстНапарника.Сессия = "";
	Если СоединениеНапарника(КопияКонтекстНапарника, КлючСервисаНапарник) <> Неопределено Тогда
