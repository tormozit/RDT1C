...
Процедура ОбновитьОтбор()
	...
	ирОбщий.ТабличноеПолеСДаннымиПоискаУстановитьОтборПоПодстрокеЛкс(ЭтаФорма, ЭлементыФормы.Типы, ПодстрокаПоиска, КолонкиПоиска);
...
Процедура КППоляПерейтиКОпределению(Кнопка)
	...
	Если Фрагменты.Количество() = 2 Тогда
		...
		Фрагменты.Удалить(0);
	КонецЕсли;
	ирОбщий.ОткрытьРедакторОбъектаБДЛкс(ПолноеИмяМД, Фрагменты[0]);
