	...
	Для Каждого СтрокаТаблицыМетаОбъектов Из СтрокиМетаОбъектов Цикл
		...
		Тогда 
			...
			Для Каждого ОбъектМД Из КоллекцияМетаданных Цикл
				...
				СтрокаТипаДанных.ОтобраноВерсий = "?";
				...
				Если СтрокаТипаДанных.ЕстьДоступ И ВычислятьКоличествоОтобрано Тогда
					ОтборВерсий.Вставить("Метаданные", ОбъектМД);
					СтрокаТипаДанных.ОтобраноВерсий = ИсторияДанныхМоя.ВыбратьВерсии(ОтборВерсий,,, МаксКоличествоВерсий).Количество();
				КонецЕсли; 
			КонецЦикла;
...
Процедура ВычислитьРазмерыТаблиц() Экспорт 
	...
	ТаблицыТаблица = Типы.Выгрузить();
	ТаблицыТаблица.Индексы.Добавить("ПолноеИмяМД");
	Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
		ПолноеИмяМД = ИдентификаторыОбъектовМД.Найти(СтрокаРезультата.MetadataId, "Идентификатор").ПолноеИмя;
		СтрокаТаблицы = ТаблицыТаблица.Найти(ПолноеИмяМД, "ПолноеИмяМД");
		...
		КонецЕсли; 
		СтрокаТаблицы.РазмерВерсийГотовыхКБ = СтрокаРезультата.ReadySizeKb;
		СтрокаТаблицы.РазмерВерсийСырыхКБ = СтрокаРезультата.RawSizeKb;
		СтрокаТаблицы.ЧислоВерсийГотовых = СтрокаРезультата.ReadyCount;
		СтрокаТаблицы.ЧислоВерсийСырых = СтрокаРезультата.RawCount;
	КонецЦикла;
	Типы.Загрузить(ТаблицыТаблица);
