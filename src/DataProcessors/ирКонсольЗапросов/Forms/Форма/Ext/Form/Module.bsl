...
Процедура ПриПолученииДанныхДоступныхПолей(Элемент, ОформленияСтрок)
	ирКлиент.ПриПолученииДанныхТабличногоПоляКомпоновкиЛкс(ЭтаФорма, Элемент, ОформленияСтрок, ЭлементыФормы.ВыборКомпоновки);
КонецПроцедуры
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапросов <> Ложь Тогда
		...
		ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапросов, Текст, "ТекстЗапроса, Запрос", АктивизироватьСтрокуДереваЗапросов <> Ложь,, Кнопки, ИскатьСНачала <> Ложь);
	КонецЕсли; 
...
Процедура ПрименитьСтрокуПоискаКСтруктуреЗапроса(Знач Текст = Неопределено, Знач АктивироватьСтроку = Ложь, Знач ИскатьСНачала = Ложь)
	...
	Если Текст = Неопределено Тогда
		Текст = ДеревоЗапросовСтрокаПоиска;
	КонецЕсли;
	МенеджерПоиска = ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапроса, Текст, "Текст", АктивироватьСтроку,,, ИскатьСНачала <> Ложь);
