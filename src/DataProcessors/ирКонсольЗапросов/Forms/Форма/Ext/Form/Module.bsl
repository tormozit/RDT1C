...
Процедура УстановитьГраницыВыделенияНаНачалоЭлементаСтруктуры(Знач ТекущаяСтрока = Неопределено)
	...
	Если СмещениеНачальнойСтроки > 0 Тогда
		НачальнаяКолонка = 1;
	КонецЕсли;
	ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(НачальнаяСтрока, НачальнаяКолонка, СтрокаДанных.КонечнаяСтрока, СтрокаДанных.КонечнаяКолонка);
...
Функция РазобратьТекстЗапроса(Знач ТекстЗапроса = Неопределено, Знач СтроитьДеревоЗапроса = Истина, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрокаДляОтображенияОшибок = 0,
	...
	Иначе
		...
		ИменаСоздаваемыхТаблиц = ирОбщий.СоздаваемыеВременныеТаблицыПакетаЛкс(ТекстЗапроса,, ТекстыЭлементов);
		ИндексЭлемента = 0;
...
Процедура ЗаполнитьСтрокуДереваЗапросаУпрощенно(Знач СтрокаДерева, ПредшествующийТекст = "", ИскатьИсточники = Ложь)
	...
	СтрокаКоординат.НачальнаяСтрока = СтрЧислоСтрок(ПредшествующийТекст + "й"); // Добавка нужна, чтобы учитывалась конечная строка, если она пустая https://partners.v8.1c.ru/forum/topic/472131
	...
	СтрокаКоординат.КонечнаяСтрока = СтрокаКоординат.НачальнаяСтрока + СтрЧислоСтрок(СтрокаДерева.Текст + "й") - 1;
...
Процедура ЗагрузитьВременнуюТаблицуРезультатаПакета()
	...
	Если СтрокаВременнойТаблицы <> Неопределено Тогда
		ЭлементыФормы.ВременныеТаблицы.ТекущаяСтрока = СтрокаВременнойТаблицы;
		мРезультатЗапроса = ВыполнитьЗапросИсточникаДанных("SELECT * FROM " + ИмяСозданнойВременнойТаблицы,, Ложь);
		Если мРезультатЗапроса <> Неопределено Тогда
			мРезультатЗапроса = мРезультатЗапроса[0];
		КонецЕсли;
		ирКлиент.УстановитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельРезультата.Страницы.РезультатКоллекция, Ложь);
		ЗагрузитьРезультатЗапроса(, Истина, Истина);
	КонецЕсли;
