...
Функция КлсПолеТекстаПрограммыОбновитьКонтекст(Знач Компонента = Неопределено, Знач Кнопка = Неопределено) Экспорт 
	...
	Если Компонента = ПолеТекстаПрограммы.КодОбработкиРезультата Тогда
		...
		КонецЕсли; 
		Компонента.ДобавитьСловоЛокальногоКонтекста("мРезультатЗапроса", , ТипЗначенияПеременной, мРезультатЗапроса,,, ТаблицаСтруктурТипов);
	...
	ИначеЕсли Компонента = ПолеТекстаПрограммы.КодПередВыполнениемЗапроса Тогда
	...
	ИначеЕсли Компонента = ПолеТекстаПрограммы.КодОбработкиСтрокиРезультата Тогда
		...
		КонецЕсли; 
		Компонента.ДобавитьСловоЛокальногоКонтекста("СтрокаРезультата", , ТипЗначенияПеременной, МетаданныеСлова,,, ТаблицаСтруктурТипов);
	...
	ИначеЕсли Компонента = ПолеТекстаПрограммы.ТекстЗапроса Тогда
...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	КомпонентаТекстаЗапроса = ПолеТекстаПрограммы.ТекстЗапроса;
	#Если Сервер И Не Сервер Тогда
	    КомпонентаТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
		Компонента = КомпонентаТекстаЗапроса;
	#КонецЕсли
	...
	Если Кнопка = ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		...
		ЛиВыделенВесьТекст = НовыйТекстЗапроса = КомпонентаТекстаЗапроса.ПолеТекста.ПолучитьТекст();
		...
		Если ЛиВыделенВесьТекст Тогда
			...
			КонецЦикла;
		Иначе
			СтрокаТекстаЗапроса = "|" + ирОбщий.УдалитьВнешниеСкобкиВыраженияЛкс(СтрокаТекстаЗапроса, """", Символы.ПС + "|""");
		КонецЕсли; 
		Если СтрДлина(КомпонентаТекстаЗапроса.ПолеТекста.ВыделенныйТекст()) = 0 Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
		ирОбщий.ТекстВБуферОбменаОСЛкс(СтрокаТекстаЗапроса);
		Возврат;
	КонецЕсли;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные) Экспорт
	...
	Если Истина
		И ЭкземплярКомпоненты = ПолеТекстаЗапроса
...
Процедура ПроверитьВключитьРедактированиеУзлаСтруктурыЗапроса()
	...
	Если мТекущаяСтрокаДереваЗапроса.Текст <> ПолеТекстаЗапроса.ПолеТекста.ПолучитьТекст() Тогда
		КоманднаяПанельТекстЗапросаИзменитьИлиСохранитьУзелДерева(, Истина);
	КонецЕсли;
...
Функция ПроверитьПерейтиКСтрокеЗапросаСоздания(Знач КомпонентаТекстаЗапроса, Знач ОпределениеСлова, Знач ФрагментыВыражения)
	...
	Если СтрокаЗапросаСоздания <> Неопределено Тогда
		...
		Если ФрагментыВыражения.Количество() = 2 Тогда
			КомпонентаТекстаЗапроса.НайтиПоказатьСловоВТексте("КАК " + ФрагментыВыражения[1],,,, Истина);
		КонецЕсли; 
...
Функция ПолучитьТекущуюКомпонентуКонтекстнойПодсказки(Кнопка = Неопределено)
	...
	Иначе
		Если Истина
			...
			И ПолеТекстаПрограммы.Свойство(ТекущийЭлемент.Имя) 
		Тогда
...
Процедура СохранитьПараметрыСтрокиДереваЗапросов(СтрокаДереваЗапросов = Неопределено, ПараметрыЗапроса = Неопределено)
	
	Если СтрокаДереваЗапросов = Неопределено Тогда
		СтрокаДереваЗапросов = мТекущаяСтрокаДереваЗапросов;
	КонецЕсли; 
	Если ПараметрыЗапроса = Неопределено Тогда
		ПараметрыЗапроса = Параметры;
	КонецЕсли; 
	СтрокаДереваЗапросов.ПараметрыЗапроса = ирОбщий.СузитьТипыКолонокТаблицыБезПотериДанныхЛкс(ПараметрыЗапроса.Скопировать()); // Сужаем типы, чтобы огромный список типов колонки "Значение" не засорял память и файл
...
Функция СохранитьВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь, Знач ИмяФайла = Неопределено, СброситьМодифицированность = Неопределено, Знач ЭтоФайлВосстановления = Ложь)
	...
	Тогда
		...
		//Исключение
		//	ирОбщий.ПоказатьОшибкуВТекстеПрограммыЛкс(ПолеТекстаЗапроса.ПолеТекста,,,, МодальныйРежим, ИнформацияОбОшибке());
		//	Возврат;
		//КонецПопытки;
		...
		Для Каждого СтрокаКопииДерева Из ВсеСтрокиДерева Цикл
			...
			Для Каждого СтрокаПараметра Из СтрокаКопииДерева.ПараметрыЗапроса Цикл
				Если ТипЗнч(СтрокаПараметра.Значение) = Тип("ТаблицаЗначений") Тогда
					...
					КонецЕсли;
					Если Истина
						И ЭтоФайлВосстановления 
						И СтрокаПараметра.Значение.Количество() > МаксКоличествоСтрокВТЗ 
					Тогда
...
Процедура ЗавершитьЗагрузкуРезультата() Экспорт
	...
	Иначе
		...
		ирОбщий.ТабличноеПолеСОтборомПросмотраОбновитьПредставлениеЛкс(ЭтаФорма, ЭлементыФормы.РезультатКоллекция);
	КонецЕсли; 
			...
			Если Не ирОбщий.ПолучитьГотовностьДанныхСтраницыЛкс(ЭтаФорма, ЭлементыФормы.ПанельРезультата.Страницы.РезультатКоллекция) Тогда
				...
				Если СпособВыгрузки = 1 Тогда
					Если ТипЗнч(мРезультатЗапроса) = Тип("РезультатЗапроса") Тогда
						...
						ирОбщий.ЗагрузитьВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, мРезультатЗапроса, мВыборкаРезультата, ЭлементыФормы.КоманднаяПанельКоллекция, ОбработчикЗавершенияЗагрузки,
...
Процедура РезультатКоллекцияПослеДобавленияКолонок()
	...
	ирОбщий.ТабличноеПолеСОтборомПросмотраОбновитьКомпоновщикЛкс(ЭтаФорма, ЭлементыФормы.РезультатКоллекция);
...
Процедура ОткрытьФайл(РежимОбъединения = Ложь)
	...
	Если РежимОбъединения Или СохранитьВФайл(Истина) Тогда
		РезультатВыбора = ирОбщий.ВыбратьФайлЛкс(, "sel", "Файл консоли запросов", мИмяОткрытогоФайла,,, "Выберите файл со списком запросов");
...
Процедура ЗапроситьОчисткуМенеджераВременныхТаблиц() Экспорт 
	...
	Если Открыта() Тогда
		Если ВременныеТаблицы.Количество() > 0 Тогда
			...
			ирОбщий.ПериодОчисткиМенеджераВременныхТаблицЛкс(ПоУмолчаниюОчищать);
...
Функция УстановитьПараметрыЗапроса(ОбъектЗапроса = Неопределено, ДобавитьВыборкуИзВременнойТаблицы = Истина)
	...
	Тогда
		ОбновитьТекстВОбъектеЗапроса(ОбъектЗапроса);
...
Процедура ОбновитьТекстВОбъектеЗапроса(ОбъектЗапроса)
	
	Если ОбъектЗапроса = Неопределено Тогда
		ОбъектЗапроса = мОбъектЗапроса;
	КонецЕсли;
	//мОбъектЗапроса.Текст = ПолучитьТекстЗапроса(Истина); // Закомментировано по https://www.hostedredmine.com/issues/955030
...
Процедура ВычислитьПараметрыЗапроса(ОбъектЗапроса = Неопределено, ВычислятьПараметрыСКД = Истина)
	...
	Для каждого СтрокаПараметра Из Параметры Цикл
		Если Не ирОбщий.ЛиИмяПеременнойЛкс(СтрокаПараметра.ИмяПараметра) Тогда
			Продолжить;
		КонецЕсли; 
		ЗагрузитьЗначениеПараметраИзФайла(СтрокаПараметра, ВычислятьПараметрыСКД);
		ЗначениеПараметра = СтрокаПараметра.Значение;
	...
	КонецЦикла;
	Если Истина
		И ВычислятьПараметрыСКД 
		И мТекущаяСтрокаДереваЗапросов.ТипЗапроса = "Компоновка"
	Тогда
		ПолучитьРезультирующийЗапрос(ОбъектЗапроса);
	КонецЕсли; 
...
Функция ВыполнитьЗапрос(Кнопка = Неопределено, Знач пЗагружатьЭлементов = Неопределено, РежимВременнойТаблицы = Неопределено, СохранитьДляОтложеннойОтладки = Неопределено,
	...
	Если РежимВременнойТаблицы <> Истина Тогда
		...
		//Если ЛиТекущийЗапросПолный Тогда 
		    ДобавитьПараметрыВКомпонентуПередВыполнениемЗапроса(ПолеТекстаПрограммы.КодПередВыполнениемЗапроса);
	...
	Иначе
		...
		Исключение
			...
			КонецЕсли; 
			ирОбщий.ПоказатьОшибкуВТекстеПрограммыЛкс(ПолеТекстаЗапроса.ПолеТекста, , , Истина, МодальныйРежим, ИнформацияОбОшибке(),,, ЭтаФорма);
			Возврат Ложь;
		КонецПопытки;
...
Процедура ЗаполнитьОписаниеРезультата(Знач ТекстЗапроса, Знач МассивСоздаваемыхВременныхТаблиц = Неопределено, Знач РежимВременнойТаблицы = Ложь)
	
	Если МассивСоздаваемыхВременныхТаблиц = Неопределено Тогда
		МассивСоздаваемыхВременныхТаблиц = Новый Массив;
	КонецЕсли; 
...
Процедура ЗагрузитьРезультатПакета(ОбновитьРазмерРезультата = Ложь) Экспорт 
	...
	Если Истина
		...
		И ДеревоЗапроса.Строки[0].Строки.Количество() = мРезультатПакета.Количество() 
...
Процедура ЗагрузитьДанныеТекущейСтрокиДереваЗапросов()
	...
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(ДеревоЗапросовСтрокаПоиска) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(, ПолеТекстаЗапроса.ПолеТекста, ДеревоЗапросовСтрокаПоиска, Ложь);
		КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	ирОбщий.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПолеТекстаПрограммы.ТекстЗапроса, "КонструкторЗапросовИР").Доступность = ирКэш.ЛиПлатформаWindowsЛкс();
	НастроитьКолонкиТабличногоПоляРезультата();
	...
	Если мРежимРедактораЗапроса Тогда
		ПолеТекстаПрограммы.ТекстЗапроса.РежимКомпоновкиДанных = (мТекущаяСтрокаДереваЗапросов.ТипЗапроса = "Компоновка");
	КонецЕсли;
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	...
	Если ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		Если ЗначениеВыбора.Свойство("Формула") Тогда
			ирОбщий.ИнтерактивноЗаписатьВКолонкуТабличногоПоляЛкс(ЭлементыФормы.Параметры, ЭлементыФормы.Параметры.Колонки.Выражение, ЗначениеВыбора.Формула);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьОбработкуСтрокРезультата(ТекстДляВыполнения)
	...
	Попытка
		...
		Если ТолькоВыделенные Тогда
			...
			СтрокиДляОбработки = ирОбщий.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ЭлементыФормы.РезультатКоллекция, МассивСтрок);
...
Процедура КнопкаВыполнитьОбработкуТаблицыРезультата(Элемент)
	...
	Если ТипЗнч(РезультатКоллекция) = Тип("ДеревоЗначений") Тогда 
		...
		Возврат;
	КонецЕсли; 
	КодОбработкиСтрокиРезультата = ПолеТекстаПрограммы.КодОбработкиСтрокиРезультата;
		#Если Сервер И Не Сервер Тогда
...
Процедура КнопкаВыполнитьОбработкуРезультата(Элемент)
	...
	Если мРезультатЗапроса = Неопределено Тогда
		...
		Возврат;
	КонецЕсли; 
	КодОбработкиРезультата = ПолеТекстаПрограммы.КодОбработкиРезультата;
		#Если Сервер И Не Сервер Тогда
...
Функция ВыполнитьЗапросИсточникаДанных(ТекстЗапроса, пЗагружатьЭлементов = 0, ВыводитьСообщения = Истина, СохранитьДляОтложеннойОтладки = Ложь, выхРезультатПакета = Неопределено,
	...
	Если выхРезультатПакета <> Неопределено Тогда
		выхРезультатПоИменам = мПлатформа.СтруктураРезультатаПакетногоЗапроса(ТекстЗапроса, ПолеТекстаПрограммы.ТекстЗапроса.мПараметрыДиалектаSQL.СтрочныйКомментарий);
	КонецЕсли; 
	Возврат Результат;
	...
	Иначе
		...
		Исключение
			...
			Если СообщитьОшибку Тогда
				ирОбщий.ПоказатьОшибкуВТекстеПрограммыЛкс(ПолеТекстаЗапроса.ПолеТекста, СтартоваяСтрока, , Истина, МодальныйРежим, выхИнформацияОбОшибке,,, ЭтаФорма);
			КонецЕсли; 
		КонецПопытки;
...
Процедура ПараметрыЗначениеПараметраНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ирОбщий.ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ЭтаФорма, ЭлементыФормы.Параметры, СтандартнаяОбработка, , ИспользоватьОграничениеТипа) Тогда 
		ПривестиЗначениеПараметраКОграничениюТипов();
...
Процедура ПоказатьТекстПодзапроса()
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(ТекстЗапроса,, ТекстЗапроса);
	КонецЕсли; 
...
Процедура ДеревоЗапросаПриАктивизацииСтроки(Элемент = Неопределено) Экспорт 
	...
	ПолеТекстаЗапроса.УстановитьФокус();
	ПоказатьТекстПодзапроса();
	мТекущаяСтрокаДереваЗапроса = ТекущаяСтрока;
	Если мРедактируемыйУзел = мТекущаяСтрокаДереваЗапроса Тогда
		ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(мРедактируемыйТекстУзла,, мТекущаяСтрокаДереваЗапроса.Текст);
	КонецЕсли; 
...
Процедура ПерейтиКЗависимомуЗапросуНажатие(Кнопка)
	...
	ПолеТекстаПрограммы.ТекстЗапроса.НайтиПоказатьСловоВТексте(ИмяВременнойТаблицы);
...
Процедура ЗаблокироватьРедактированиеТекстаЗапроса(ТолькоПросмотрДерева)
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ОбъектКомпоненты = ирОбщий.ПрефиксИменЭлементовЭкземпляраКомпонентыЛкс(ПолеТекстаПрограммы.ТекстЗапроса);
...
Процедура ДеревоЗапросаВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		Если ЭтоСтрокаСозданияВременнойТаблицы() Тогда
			...
			ВыполнитьЗапрос(,, Истина);
		ИначеЕсли ВыбраннаяСтрока.ТипУзла = "<TableWithName>" Тогда
		...
		ИначеЕсли ВыбраннаяСтрока.АдресРезультата <> Неопределено Тогда
			...
			мВыборкаРезультата = Неопределено;
			ЗаполнитьОписаниеРезультата(ВыбраннаяСтрока.Текст);
			ЭлементыФормы.ЗапросРезультата.УстановитьТекст(мОписаниеРезультата.ТекстЗапроса);
			ЗагрузитьРезультатЗапроса();
...
Функция КоманднаяПанельТекстЗапросаСохранитьУзел(НовыйРежимСворачивания = Неопределено)
	...
	Если Не ПроверитьТекстПодзапроса() Тогда 
		...
		//	ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(мРедактируемыйУзел.Текст);
		//КонецЕсли;
		Если Ответ = КодВозвратаДиалога.Отмена Тогда 
			Возврат Ложь;
		КонецЕсли;
	...
	Если РазобратьТекстЗапроса(,, мСлужебноеПолеТекстовогоДокумента) <> Истина Тогда
		...
		ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(мСлужебноеПолеТекстовогоДокумента.ПолучитьТекст());
	...
	Если НоваяТекущаяСтрокаДереваЗапроса <> ДеревоЗапроса Тогда 
		...
		КонецЦикла;
	КонецЕсли;
	СтруктураДвумерногоВыделения = ПолеТекстаЗапроса.ПолеТекста.СтруктураДвумерногоВыделения();
	...
	ИначеЕсли мРедактируемыйУзел <> Неопределено Тогда 
		...
		ОбновитьКнопкуИзменитьСохранитьУзел();
	КонецЕсли;
	ПолеТекстаЗапроса.ПолеТекста.УстановитьДвумерноеВыделениеСтруктурой(СтруктураДвумерногоВыделения);
	Если мРедактируемыйУзел = Неопределено Тогда
		СохранитьФайлДляВосстановления();
	КонецЕсли; 
	Если ЗначениеЗаполнено(ДеревоЗапросовСтрокаПоиска) Тогда
...
Процедура КоманднаяПанельФормыРежимДереваЗапроса(Кнопка = Неопределено) Экспорт 
	...
	Если Не НовыйРежимДереваЗапроса Тогда 
		Если ДеревоЗапроса.Строки.Количество() > 0 Тогда 
			//ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(РазвернутьВложенныеЗапросы(ДеревоЗапроса.Строки[0], мТекстЗапросаДерева));
			СохранитьДанныеТекущейСтрокиДереваЗапросов();
			УстановитьТекстПоляТекстаЗапроса(мТекущаяСтрокаДереваЗапросов.ТекстЗапроса);
...
Функция РазобратьТекстЗапроса(Знач ТекстЗапроса = Неопределено, Знач СтроитьДеревоЗапроса = Истина, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрокаДляОтображенияОшибок = 0)
	...
	Если ЛиПарсерЗапросаДоступен() Тогда
		НачальныйТокен = ПолеТекстаПрограммы.ТекстЗапроса.РазобратьТекстЗапроса(ТекстЗапроса, Истина,,,,,, СтартоваяСтрокаДляОтображенияОшибок);
		Если Истина
...
Функция ПостроитьНисходящееЗапросы(Знач СтрокаДерева, Знач ТокенСверху, Знач ЭтоВыборТаблицы = Ложь, Знач ПсевдонимСверху = Неопределено, Знач ЭтоЗапросСОсновнойТаблицейСверху = Ложь,
	...
	Тогда
		Если Не ЗначениеЗаполнено(СтрокаВниз.Имя) Тогда 
			...
			Тогда
				...
				ПолеТекстаПрограммы.ТекстЗапроса.РазобратьКомментарий(ТекстКомментария, ИмяЗапросаИзКомментария, ИмяЧастиОбъединенияИзКомментария);
			...
			Тогда
				...
				ТекстКомментария = ПолучитьТекстМеждуТокенами(ТокенКомментария.BeginNoise, ТокенКомментария);
				ПолеТекстаПрограммы.ТекстЗапроса.РазобратьКомментарий(ТекстКомментария, ИмяЗапросаИзКомментария);
				Если ЗначениеЗаполнено(ИмяЗапросаИзКомментария) Тогда
	...
	Если Истина
		...
		И РодительскоеПравило = "<TableName>"
	Тогда 
		//ПсевдонимВверх = ПолеТекстаПрограммы.ТекстЗапроса.ПолучитьТекстИзДанныхТокена(Данные); 
		ПсевдонимВверх = ПолеТекстаПрограммы.ТекстЗапроса.ПолучитьТекстИзТокена(ТокенСверху); 
	Иначе
...
Функция ОбработатьСтрокуДереваЗапроса(СтрокаДерева, НачальнаяСтрока = 1, НачальнаяКолонка = 1)
	...
	Для Каждого ДинамическоеИмя Из МассивДинамическихИмен Цикл
		...
		Для Счетчик = 0 По ВГраница Цикл
			...
			Тогда
				БазовоеИмя = БазовоеИмя + (СтрокаДерева.Родитель.Строки.Количество() - СтрокаДерева.Родитель.Строки.Индекс(СтрокаДерева)) + "_";
			КонецЕсли; 
...
Процедура ДеревоЗапросаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ИначеЕсли ДанныеСтроки.ТипУзла = "<TableWithName>" Тогда 
		...
		Ячейка.ИндексКартинки = ирОбщий.ИндексКартинкиТипаТаблицыБДЛкс(ТипТаблицы);
	КонецЕсли;
...
Функция ПолучитьПостроительЗапросовДляИтогов(СообщатьОбОшибке = Ложь, ТекстЗапроса = "")
	...
	Исключение
		...
		Если СообщатьОбОшибке Тогда
		    ...
		    ирОбщий.ПоказатьОшибкуВТекстеПрограммыЛкс(ПолеТекстаЗапроса.ПолеТекста, , , Истина, МодальныйРежим,
		        ИнформацияОбОшибке);
		КонецЕсли; 
		Возврат Неопределено;
	КонецПопытки; 
...
Функция ПроверитьДобавитьВыборкуИзВременнойТаблицыВТекстЗапроса(выхТекстЗапроса, ДобавитьВыборкуИзВременнойТаблицы = Истина)
	...
	Тогда
		...
		КонецЕсли;
		Если МассивСоздаваемыхВременныхТаблиц.Количество() = 1 Тогда
...
Функция СтрокаПорядкаОтладки(выхТекстЗапроса)
	...
	СтрокаПорядкаОтладки = ирОбщий.СтрокаМеждуМаркерамиЛкс(выхТекстЗапроса, ПолеТекстаЗапроса.мПараметрыДиалектаSQL.СтрочныйКомментарий + ПолеТекстаЗапроса.мМаркерПорядкаОтладки, Символы.ПС, Ложь,,, Истина);
...
Функция ПолучитьРезультирующийЗапрос(Запрос, Знач ТекстЗапроса = "", ИспользоватьНастройкиРасширений = Ложь)
	...
	Если мТекущаяСтрокаДереваЗапросов.ТипЗапроса = "Компоновка" Тогда 
		...
		Попытка
			#Если Сервер И Не Сервер Тогда
				ирОбщий.ОтЛкс(мСхемаКомпоновки, ВременныеНастройки);
			#КонецЕсли
		...
		Если КомпоновкаУдалась Тогда
			Если МакетКомпоновкиДанных.НаборыДанных.Количество() > 0 Тогда
				...
				Для Каждого лПараметр Из МакетКомпоновкиДанных.ЗначенияПараметров Цикл
					...
					Если СтрокаПараметра = Неопределено Тогда
						Запрос.УстановитьПараметр(лПараметр.Имя, лПараметр.Значение);
					КонецЕсли; 
				КонецЦикла;
			Иначе
			...
			Если ВыраженияКомпоновки.Количество() > 0 Тогда
				...
				Если ЗначенияВычисляемыхПараметров <> Неопределено Тогда
					...
					Для Каждого КолонкаПараметра Из ЗначенияВычисляемыхПараметров.Колонки Цикл
						...
						Запрос.УстановитьПараметр(ИмяПараметра, СтрокаПараметра.Значение);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли; 
		КонецЕсли;
			...
			Если ЗапросИзПостроителя.Текст <> "" Тогда
				Для Каждого лПараметр Из ЗапросИзПостроителя.Параметры Цикл
					Если Параметры.Найти(НРег(лПараметр.Ключ), "НИмяПараметра") <> Неопределено Тогда
						Продолжить;
					КонецЕсли;
					Запрос.УстановитьПараметр(лПараметр.Ключ, лПараметр.Значение);
				КонецЦикла;
				Если ИспользоватьНастройкиРасширений = Истина Тогда
	...
	Если Запрос <> Неопределено Тогда
		Запрос.Текст = мПлатформа.РазмаскироватьВременныеТаблицы(ТекстЗапроса, мМассивВременныхТаблицКомпоновщика);
	КонецЕсли; 
...
Процедура МенюДереваЗапросаПеренестиВНовыйЗапрос(Кнопка)
	...
	Если мВременныеТаблицыПакета[Нрег(ИмяВременнойТаблицы)] <> Неопределено Тогда
		...
		Если Ответ = КодВозвратаДиалога.Нет Тогда
			...
			Если СтрокаДереваЗапроса <> Неопределено Тогда
				ирОбщий.Сравнить2ЗначенияВФормеЛкс(мТекущаяСтрокаДереваЗапроса.Текст, СтрокаДереваЗапроса.Текст,, "Новый", "Существующий");
			КонецЕсли; 
		КонецЕсли;
		Если Ответ <> КодВозвратаДиалога.Да Тогда
			Возврат;
	...
	Если мТекущаяСтрокаДереваЗапроса.ТипУзла = "<InUnion>" Тогда
		Если ПолеТекстаПрограммы.ТекстЗапроса.ЭтоРусскийВариантТекстаЗапроса(ТекстЗапроса) Тогда
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ТекстЗапросаПоместить = ПолеТекстаПрограммы.ТекстЗапроса.ИзменитьОператорПоместитьЗапроса(ТекстЗапросаПоместить, ИмяВременнойТаблицы);
	...
	Иначе
		...
		ТекстЗапросаПоместить = ПолеТекстаПрограммы.ТекстЗапроса.ИзменитьОператорПоместитьЗапроса(НоваяСтрокаЗапроса.ТекстЗапроса, ИмяВременнойТаблицы);
...
Процедура МенюДереваЗапросаВстроитьВРодительскийЗапрос(Кнопка)
	...
	ПроверкаУспешна = ПолеТекстаПрограммы.ТекстЗапроса.ШаблонЗапросаДопускаетВстройкуВРодительскийЗапрос(ПроверочныйТекст);
...
Процедура РезультатКоллекцияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, мКнопкаРежимаОтображенияРезультата);
...
Процедура ПараметрыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Тогда
		Если ирОбщий.ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ЭтаФорма, Элемент, СтандартнаяОбработка,,,,, ВыбраннаяСтрока.ИмяПараметра) Тогда 
	...
	Если ЭтоПеретаскиваниеСтрокПараметров(ЗначениеПеретаскивания) Тогда
		...
		Для Каждого СтрокаПараметра Из ЗначениеПеретаскивания Цикл
			Если ЭлементыФормы.ДеревоЗапросов.ТекущаяСтрока <> Строка Тогда
				...
				ирОбщий.ЗагрузитьВТаблицуЗначенийЛкс(Строка.ПараметрыЗапроса, ПараметрыЗапроса);
			Иначе
				ПараметрыЗапроса = Параметры;
			КонецЕсли; 
...
Процедура КоманднаяПанельТекстЗапросаРезультатаСравнитьТекст(Кнопка)
	...
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.ЗапросРезультата);
...
Процедура КоманднаяПанельТекстЗапросаОбъединениеСсылочныхТаблиц(Кнопка)
	...
	Если Не ПустаяСтрока(Текст) Тогда
		ирОбщий.УстановитьТекстСОткатомЛкс(ПолеТекстаЗапроса.ПолеТекста, Текст);
	КонецЕсли; 
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаПолучитьТекст(Кнопка)
	...
	ирОбщий.ОперироватьСтруктуройЛкс(ТекстМодуля,, СтруктураНатуральныхПараметров);
    ирОбщий.ТекстВБуферОбменаОСЛкс(ТекстМодуля);
...
Функция ВыполнитьОбработчикПередВыполнениемЗапроса()
	...
	Результат = ПолеТекстаПрограммы.КодПередВыполнениемЗапроса.ВыполнитьПрограммныйКод();
...
Процедура ПараметрыВыражениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли ТекущиеДанные.ЭтоВыражение = "СКД" Тогда
		Форма = ирОбщий.ПолучитьФормуЛкс("Обработка.ирКонсольКомпоновокДанных.Форма.РедакторВыраженияКомпоновки", , Элемент);
...
Процедура ОбновитьКнопкуРегулированияЗначенияПараметра()
	...
	ЭлементыФормы.Параметры.Колонки.Значение.ЭлементУправления.КнопкаРегулирования = ТипЗнч(ЭлементыФормы.Параметры.ТекущиеДанные.Значение) = Тип("Дата");
...
Процедура КоманднаяПанельКоллекцияЗагрузитьПолностью(Кнопка)
	...
	Если СпособВыгрузки = 1 Тогда
		#Если Сервер И Не Сервер Тогда
			ЗавершитьЗагрузкуРезультата();
		#КонецЕсли
		ирОбщий.ЗагрузитьВыборкуВТабличноеПолеПервуюПорциюЛкс(ЭтаФорма, мРезультатЗапроса, мВыборкаРезультата, ЭлементыФормы.КоманднаяПанельКоллекция, "ЗавершитьЗагрузкуРезультата", 0, СсылкаНаБуфернуюТаблицу);
	Иначе
...
Процедура КоманднаяПанельТекстЗапросаПерейтиКРодительскомуУзлу(Кнопка)
	...
	Иначе
		ПолеТекстаПрограммы.ТекстЗапроса.НайтиПоказатьСловоВТексте(ДочерняяСтрока.ИмяСРодителем);
	КонецЕсли; 
...
Процедура РезультатТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	Если Автосумма Тогда
		ТекстКнопки = ирОбщий.ПолеТабличногоДокументаПолучитьПредставлениеСуммыВыделенныхЯчеекЛкс(Элемент);
	Иначе
		ТекстКнопки = "";
	КонецЕсли;
...
Процедура КоманднаяПанельТабличныйДокументСравнить(Кнопка)
	...
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.РезультатТабличныйДокумент);
...
Процедура СвойстваСтрокиРезультатаПриПолученииДанных(Элемент, ОформленияСтрок)
	...
	Для Каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		...
		ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(ЭтаФорма, Элемент, ОформлениеСтроки, ДанныеСтроки, КнопкаРежимаОтображения, "ПредставлениеЗначения",,,, "ПредставлениеЗначения");
	КонецЦикла;
...
Процедура РезультатКоллекцияПриАктивизацииСтроки(Элемент = Неопределено)
	...
	ирОбщий.ТабличноеПолеПриАктивизацииСтрокиЛкс(ЭтаФорма, Элемент,, Не ПоказыватьСвойства);
...
Функция мПолучитьЗапросWMI(ПринудительноИзКонструктора = Ложь)
	...
	Иначе
		...
		ТекстГДЕ = "";
		ТекстПорядок = "";
		ФормаКонструктораЗапроса = ПолеТекстаПрограммы.ТекстЗапроса.ПолучитьФорму("КонструкторЗапроса");
		...
		//Тогда
			ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(Результат);
		//КонецЕсли; 
	КонецЕсли;
	Возврат Результат;
...
Функция ЗаполнитьВыборкиИтоговИзЗапроса(СообщатьОбОшибке = Ложь)
	...
	Если RegExp.Проверить(ТекстЗапроса) Тогда // грязно
		...
		Если ПостроительЗапроса = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли; 
		ТекстБезКОмментариев = ПолеТекстаПрограммы.ТекстЗапроса.ЗалитьКомментарииИСтроковыеЛитералы(ПостроительЗапроса.Текст);
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаЗаписьНаСервере(Кнопка)
	...
	ПолеТекста = ЭлементыФормы.КодОбработкиСтрокиРезультата.ПолеТекста;
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
...
Процедура КоманднаяПанельОбработкиРезультатаЗаписьНаСервере(Кнопка)
	...
	ПолеТекста = ЭлементыФормы.КодОбработкиРезультата.ПолеТекста;
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
		ирОбщий.ЗаписатьОбъектЛкс();
	#КонецЕсли
...
Процедура КП_ТаблицаПакетаНайтиЗапросВДереве(Кнопка = Неопределено)
	...
	Иначе
		...
		Фрагменты = ирОбщий.РазбитьГрубоТекстПакетногоЗапросаНаЗапросыЛкс(ПолеТекстаЗапроса.ПолеТекста.ПолучитьТекст(),, ПозицииЗапросов);
		Если Фрагменты.Количество() > ИндексЗапроса Тогда
			НачальнаяПозиция = ПозицииЗапросов[ИндексЗапроса] + 1;
			...
			ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(НачальнаяПозиция, НачальнаяПозиция,,, Истина);
		КонецЕсли; 
...
Процедура КП_СвойстваСтрокиРезультатаСравнить(Кнопка)
	...
	ирОбщий.ЗапомнитьСодержимоеЭлементаФормыДляСравненияЛкс(ЭтаФорма, ЭлементыФормы.СвойстваСтрокиРезультата);
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	АктивизироватьСтрокуДереваЗапросов = Неопределено, АктивизироватьСтрокуДереваЗапроса = Неопределено, Текст = Неопределено)
	...
	Если ОбновитьПоДеревуЗапросов <> Ложь Тогда
		...
		ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапросов, Текст, "ТекстЗапроса, Запрос", АктивизироватьСтрокуДереваЗапросов <> Ложь,, Кнопки);
	КонецЕсли; 
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		АктивироватьПолеТекста = Элемент <> Неопределено;
		ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапроса, Текст, "Текст", АктивизироватьСтрокуДереваЗапроса <> Ложь);
		ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(?(АктивироватьПолеТекста, ЭтаФорма, Неопределено), ПолеТекстаЗапроса.ПолеТекста, Текст, Ложь);
	КонецЕсли; 
...
Процедура ВпередНажатие(Элемент)
	...
	ирОбщий.СледующееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапросов);
	
КонецПроцедуры

Процедура НазадНажатие(Элемент)
	
	ирОбщий.ПредыдущееВхождениеСтрокиПоискаВТабличномПолеЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапросов);
...
Процедура ЗаменитьЛитераломОбращенияКПараметру(Знач СтрокаПараметра)
	...
	Если ЗначениеЗаполнено(ТекстЗамены) Тогда
		...
		Если КоличествоЗначений > 1 Тогда
			...
			СловоЦеликом = Ложь;
		КонецЕсли; 
		Пока КомпонентаТекстаЗапроса.НайтиПоказатьСловоВТексте(ИскомаяСтрока, СловоЦеликом) Цикл
			КомпонентаТекстаЗапроса.ПолеТекста.ВыделенныйТекст(ТекстЗамены);
		КонецЦикла; 
		Если мРежимДереваЗапроса Тогда
	...
	Если Ответ = КодВозвратаДиалога.Нет Или Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если КонструкторЗапроса.ЗагрузитьТекстВКонструктор(ТекстЗапроса, КонструкторЗапроса) Тогда 
			...
			Если Не КонструкторЗапроса.ОпцияПервые Тогда
				...
				КонструкторЗапроса.ПервыеКоличество = 500000;
			КонецЕсли; 
	...
	Иначе
		...
		Если ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеПоля, Ложь) Тогда
			...
			Если СтрокаПараметра <> Неопределено Тогда
				...
				РасшифровкаПоля = ПолеТекстаЗапроса.ПрефиксПараметра() + СтрокаПараметра.ИмяПараметра;
			КонецЕсли; 
		КонецЕсли; 
	КонецЕсли;
...
Процедура РезультатКоллекцияПриАктивизацииКолонки(Элемент)
	...
	Если Элемент.ТекущаяКолонка = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	ВсеСтрокиОбновлены = ирОбщий.ТабличноеПолеПриАктивацииКолонкиЛкс(ЭтаФорма, Элемент);
...
Процедура КоманднаяПанельПараметрыОчистить(Кнопка)
	...
	ПрефиксПараметра = ПолеТекстаЗапроса.ПрефиксПараметра();
	Для Каждого СтрокаПараметра Из Параметры Цикл
		ВхожденияТекстаЗапроса = ирОбщий.НайтиРегулярноеВыражениеЛкс(ПолныйТекстЗапроса, ирОбщий.ШаблонПоискаСловаЛкс(ПрефиксПараметра + СтрокаПараметра.ИмяПараметра));
		...
		Для Каждого СтрокаВычисляемогоПараметра Из Параметры.НайтиСтроки(Новый Структура("ЭтоВыражение", Истина)) Цикл
			ВхожденияВычисляемыхПараметров = ирОбщий.НайтиРегулярноеВыражениеЛкс(СтрокаВычисляемогоПараметра.Выражение, ирОбщий.ШаблонПоискаСловаЛкс("Параметры." + СтрокаПараметра.ИмяПараметра));
...
Процедура КлсКомандаНажатиеИдентификаторы(Кнопка)
	...
	ирОбщий.УниверсальнаяКомандаФормыЛкс(ЭтаФорма, Кнопка, ЭлементыФормы.РезультатКоллекция);
	ЭлементыФормы.СвойстваСтрокиРезультата.ОбновитьСтроки();
...
Процедура РезультатКоллекцияИспользоватьОтборПриИзменении(Элемент = Неопределено)
	Если ТипЗнч(ЭлементыФормы.РезультатКоллекция.Значение) = Тип("ТаблицаЗначений") Тогда
		ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.РезультатКоллекция,,, Элемент = Неопределено);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПараметрыКомпоновкиПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура ПараметрыИмяФайлаНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ТекущаяСтрока = ЭлементыФормы.Параметры.ТекущаяСтрока;
	РезультатВыбора = ирОбщий.ВыбратьФайлЛкс(, "VT_",, ТекущаяСтрока.ИмяФайла);
	Если РезультатВыбора <> Неопределено Тогда
...
Процедура ПараметрыВыражениеПриИзменении(Элемент)
	...
	Тогда
		Возврат;
	КонецЕсли;
	ирОбщий.ПолеВводаСИсториейВыбора_ПриИзмененииЛкс(ЭлементыФормы.Параметры.Колонки.Выражение, ЭтаФорма,,, ЭлементыФормы.Параметры.Имя + СтрокаПараметра.ЭтоВыражение);
КонецПроцедуры

Процедура ПараметрыВыражениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	СтрокаПараметра = ЭлементыФормы.Параметры.ТекущаяСтрока;
	ирОбщий.ПолеВводаСИсториейВыбора_НачалоВыбораИзСпискаЛкс(ЭлементыФормы.Параметры.Колонки.Выражение, ЭтаФорма, ЭлементыФормы.Параметры.Имя + СтрокаПараметра.ЭтоВыражение);
КонецПроцедуры
...
ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирКонсольЗапросов.Форма.Форма");
#Если Сервер И Не Сервер Тогда
...
мСлужебноеПолеТекстовогоДокумента = ирКэш.Получить().СлужебноеПолеТекста;
