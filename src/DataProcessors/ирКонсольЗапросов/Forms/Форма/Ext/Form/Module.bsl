	...
	Тогда
		Если Не ЗначениеЗаполнено(СтрокаВниз.Имя) Тогда 
			...
			ИначеЕсли Ложь
				...
				Или РодительскоеПравило = "<AddUnion>"
...
Функция ПроверитьДобавитьВыборкуИзВременнойТаблицыВТекстЗапроса(выхТекстЗапроса, ДобавитьВыборкуИзВременнойТаблицы = Истина)
	...
	Тогда
		...
		Если ЗначениеЗаполнено(ЭлементСписка.Представление) Тогда
			...
			|SELECT * FROM " + ирОбщий.ПоследнийФрагментЛкс(ЭлементСписка.Представление, "*");
		КонецЕсли;
	...
	Если мСтруктурноеРедактирование И ЭлементыФормы.ДеревоЗапроса.ТекущаяСтрока = ДеревоЗапроса.Строки[0] Тогда 
		...
		Для Каждого ЭлементСписка Из СоздаваемыеВременныеТаблицы Цикл
			Если Не ЗначениеЗаполнено(ЭлементСписка.Представление) Тогда
				Продолжить;
			КонецЕсли; 
			мРегВыражение.Pattern = "(\s|;|^)(?:УНИЧТОЖИТЬ|DROP(?:\s+TABLE)?)\s+(" + ЭлементСписка.Представление + ")(\s*(?:;|$))";
			...
			Если Вхождения.Количество() = 1 Тогда
				...
				мРегВыражение2.Pattern = "(ПОМЕСТИТЬ|INTO\s+TABLE|INTO)\s+" + ЭлементСписка.Представление + "\s+";
