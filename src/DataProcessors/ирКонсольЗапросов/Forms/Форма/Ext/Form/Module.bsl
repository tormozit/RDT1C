...
Процедура ВычислитьПараметрыЗапроса(ОбъектЗапроса = Неопределено)
	...
	Для Каждого СтрокаВыраженияСКД Из Параметры.Скопировать(Новый Структура("ЭтоВыражение", "СКД"), "ИмяПараметра, Выражение") Цикл
		...
		НовыйПараметр.Выражение = СтрЗаменить(НовыйПараметр.Выражение, ПолеТекстаЗапроса.ПрефиксПараметра(), "Параметры.");
	КонецЦикла;
...
Процедура ПриОткрытии()
	...
	Если мКоллекцияДляЗаполнения <> Неопределено Тогда
		...
		ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(ТекстЗапроса);
	КонецЕсли; 
...
Функция КоманднаяПанельТекстЗапросаСохранитьУзел(НовыйРежимСворачивания = Неопределено)
	...
	Если НоваяТекущаяСтрокаДереваЗапроса <> ДеревоЗапроса Тогда 
		...
		КонецЦикла;
	КонецЕсли;
	СтруктураВыделения = ирОбщий.ПолеТекста_ПолучитьДиапазонВыделенияЛкс(ПолеТекстаЗапроса.ПолеТекста);
	...
	ИначеЕсли мРедактируемыйУзел <> Неопределено Тогда 
		...
		ОбновитьКнопкуИзменитьСохранитьУзел();
	КонецЕсли;
	ирОбщий.ПолеТекста_УстановитьДиапазонВыделенияЛкс(ПолеТекстаЗапроса.ПолеТекста, СтруктураВыделения);
	Если мРедактируемыйУзел = Неопределено Тогда
		СохранитьФайлДляВосстановления();
	КонецЕсли; 
	Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
...
Функция РазобратьТекстЗапроса(ТекстЗапроса = Неопределено, СтроитьДеревоЗапроса = Истина, ПолеТекста = Неопределено, СтартоваяСтрокаДляОтображенияОшибок = 0)
...
Функция ЗаполнитьВыборкиИтоговИзЗапроса(СообщатьОбОшибке = Ложь)
	...
	Если RegExp.Test(ТекстЗапроса) Тогда // грязно
		...
		Если RegExp.Test(ТекстБезКОмментариев) Тогда // грязно
...
Процедура КоманднаяПанельКоллекцияИдентификаторы(Кнопка)
...
Процедура РедакторHTMLonclick(ДанныеСобытия) Экспорт 
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_KEY_BINDING_2082" Тогда // CTRL+D
			СочетаниеКлавиш_CTRL_D = Новый СочетаниеКлавиш(Клавиша.D,, Истина);
			Если ЭлементыФормы.КоманднаяПанельФормы.Кнопки.СверткаРезультаты.СочетаниеКлавиш <> СочетаниеКлавиш_CTRL_D Тогда
				ирОбщий.СообщитьЛкс("Нарушено сопоставление сочетания клавиш """ + ирОбщий.ПредставлениеСочетанияКлавишЛкс(СочетаниеКлавиш_CTRL_D) + """");
			Иначе
				КоманднаяПанельФормыСверткаРезультаты();
			КонецЕсли; 
