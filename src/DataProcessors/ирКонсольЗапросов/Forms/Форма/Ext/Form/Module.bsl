...
Функция КлсПолеТекстаПрограммыОбновитьКонтекст(Знач Компонента = Неопределено, Знач Кнопка = Неопределено) Экспорт 
	...
	ИначеЕсли Компонента = ПоляТекстовПрограммы.ТекстЗапроса Тогда
...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	КомпонентаТекстаЗапроса = ПоляТекстовПрограммы.ТекстЗапроса;
	#Если Сервер И Не Сервер Тогда
...
Процедура ПриОткрытии()
	...
	ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПоляТекстовПрограммы.ТекстЗапроса, "КонструкторЗапросовИР").Доступность = ирКэш.ЛиПлатформаWindowsЛкс();
	НастроитьКолонкиТабличногоПоляРезультата();
	...
	Если мРежимРедактораЗапроса Тогда
		ПоляТекстовПрограммы.ТекстЗапроса.РежимКомпоновкиДанных = (мТекущаяСтрокаДереваЗапросов.ТипЗапроса = "Компоновка");
	КонецЕсли;
...
Функция ВыполнитьЗапросИсточникаДанных(ТекстЗапроса, пЗагружатьЭлементов = 0, ВыводитьСообщения = Истина, СохранитьДляОтложеннойОтладки = Ложь, выхРезультатПакета = Неопределено,
	...
	Если выхРезультатПакета <> Неопределено Тогда
		выхРезультатПоИменам = мПлатформа.СтруктураРезультатаПакетногоЗапроса(ТекстЗапроса, ПоляТекстовПрограммы.ТекстЗапроса.мПараметрыДиалектаSQL.СтрочныйКомментарий);
	КонецЕсли; 
	Возврат Результат;
...
Функция ВыполнитьЛокальноКодПередВыполнениемЗапроса(ТекстДляВыполнения, ЛиСинтаксическийКонтроль = Ложь) Экспорт
	...
	Если НаСервере И ПолучитьОбъектЗапроса() = мОбъектЗапроса Тогда
		#Если Сервер И Не Сервер Тогда
			мОбъектЗапроса = Новый Запрос;
		#КонецЕсли
		...
		#КонецЕсли
		мОбъектЗапроса.МенеджерВременныхТаблиц = Неопределено;
		#Если Сервер И Не Сервер Тогда
		...
		ТекстДляВыполнения = 
			...
			|мОбъектЗапроса = Новый Запрос;
			|мОбъектЗапроса.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
			|ирОбщий.СкопироватьУниверсальнуюКоллекциюЛкс(Параметры, мОбъектЗапроса.Параметры);
			|" + ТекстДляВыполнения + "
			|_П1 = ирОбщий.СтруктураОбъектаДляОтладкиЛкс(мОбъектЗапроса);";
		СнимокЗапроса = Неопределено;
		ирСервер.ВыполнитьАлгоритм(ТекстДляВыполнения,,, ПолучитьСтруктуруПараметров(), СнимокЗапроса);
		НовыйЗапрос = ирОбщий.ЗапросИзСтруктурыЛкс(СнимокЗапроса.Объект,, МенеджерВременныхТаблиц);
...
Процедура ПерейтиКЗависимомуЗапросуНажатие(Кнопка)
	...
	ПоляТекстовПрограммы.ТекстЗапроса.НайтиПоказатьСловоВТексте(ИмяВременнойТаблицы);
...
Процедура ЗаблокироватьРедактированиеТекстаЗапроса(ТолькоПросмотрДерева)
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ОбъектКомпоненты = ирКлиент.ПрефиксИменЭлементовЭкземпляраКомпонентыЛкс(ПоляТекстовПрограммы.ТекстЗапроса);
...
Функция ПостроитьНисходящееЗапросы(Знач СтрокаДерева, Знач ТокенСверху, Знач ЭтоВыборТаблицы = Ложь, Знач ПсевдонимСверху = Неопределено, Знач ЭтоЗапросСОсновнойТаблицейСверху = Ложь,
	...
	Тогда
		Если Не ЗначениеЗаполнено(СтрокаВниз.Имя) Тогда 
			...
			Тогда
				...
				ПоляТекстовПрограммы.ТекстЗапроса.РазобратьКомментарий(ТекстКомментария, ИмяЗапросаИзКомментария, ИмяЧастиОбъединенияИзКомментария);
			...
			Тогда
				...
				ТекстКомментария = ПолучитьТекстМеждуТокенами(ТокенКомментария.BeginNoise, ТокенКомментария);
				ПоляТекстовПрограммы.ТекстЗапроса.РазобратьКомментарий(ТекстКомментария, ИмяЗапросаИзКомментария);
				Если ЗначениеЗаполнено(ИмяЗапросаИзКомментария) Тогда
	...
	Тогда 
		...
		ПсевдонимВверх = ПоляТекстовПрограммы.ТекстЗапроса.ПолучитьТекстИзТокена(ТокенСверху); 
	Иначе
...
Процедура МенюДереваЗапросаПеренестиВНовыйЗапрос(Кнопка)
	...
	Если мТекущаяСтрокаДереваЗапроса.ТипУзла = "<InUnion>" Тогда
		Если ПоляТекстовПрограммы.ТекстЗапроса.ЭтоРусскийВариантТекстаЗапроса(ТекстЗапроса) Тогда
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ТекстЗапросаПоместить = ПоляТекстовПрограммы.ТекстЗапроса.ИзменитьОператорПоместитьЗапроса(ТекстЗапросаПоместить, ИмяВременнойТаблицы);
	...
	Иначе
		...
		ТекстЗапросаПоместить = ПоляТекстовПрограммы.ТекстЗапроса.ИзменитьОператорПоместитьЗапроса(НоваяСтрокаЗапроса.ТекстЗапроса, ИмяВременнойТаблицы);
...
Процедура МенюДереваЗапросаПреобразоватьВПодзапрос(Кнопка)
			...
			Если СтрокаДерева.ТипУзла = "<TableWithName>" Тогда 
				...
				Если СтрокаЗапросаСоздания <> Неопределено Тогда
					мСлужебноеПолеТекстовогоДокумента.УстановитьГраницыВыделения(
						...
						СтрокаДанных.КонечнаяСтрока,
						СтрокаДанных.КонечнаяКолонка);
					ОпределениеТаблицы = ПоляТекстовПрограммы.ТекстЗапроса.ИзменитьОператорПоместитьЗапроса(СтрокаЗапросаСоздания.Текст);
	...
	Иначе
		...
		КонецЕсли; 
		ЭтоРусскийЯзык = ПоляТекстовПрограммы.ТекстЗапроса.ЭтоРусскийВариантТекстаЗапроса(ТекстДляОпределенияЯзыка);
...
Процедура МенюДереваЗапросаВстроитьВРодительскийЗапрос(Кнопка)
	...
	ПроверкаУспешна = ПоляТекстовПрограммы.ТекстЗапроса.ШаблонЗапросаДопускаетВстройкуВРодительскийЗапрос(ПроверочныйТекст);
...
Процедура КоманднаяПанельТекстЗапросаПерейтиКРодительскомуУзлу(Кнопка)
	...
	Иначе
		ПоляТекстовПрограммы.ТекстЗапроса.НайтиПоказатьСловоВТексте(ДочерняяСтрока.ИмяСРодителем);
	КонецЕсли; 
...
Функция мПолучитьЗапросWMI(ПринудительноИзКонструктора = Ложь)
	...
	Иначе
		...
		ТекстГДЕ = "";
		ТекстПорядок = "";
		ФормаКонструктораЗапроса = ПоляТекстовПрограммы.ТекстЗапроса.ПолучитьФорму("КонструкторЗапроса");
...
Функция ЗаполнитьВыборкиИтоговИзЗапроса(СообщатьОбОшибке = Ложь)
	...
	Если RegExp.Проверить(ТекстЗапроса) Тогда // грязно
		...
		Если ПостроительЗапроса = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли; 
		ТекстБезКОмментариев = ПоляТекстовПрограммы.ТекстЗапроса.ЗалитьКомментарииИСтроковыеЛитералы(ПостроительЗапроса.Текст);
...
Процедура УстановитьРежимВыполненияКодаНаСервере(НовыйРежим = Неопределено)
	...
	#КонецЕсли
	КодПередВыполнениемЗапроса.ЛиСерверныйКонтекст = НаСервере;
	КодОбработкиСтрокиРезультата = ПоляТекстовПрограммы.КодОбработкиСтрокиРезультата;
	#Если Сервер И Не Сервер Тогда
	    КодОбработкиСтрокиРезультата = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
