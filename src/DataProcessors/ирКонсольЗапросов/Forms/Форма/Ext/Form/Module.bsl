...
Перем мРазрешитьУстановкуПозицииИзСтруктурыПросмотра;

Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	выхИменаСвойств = "Реквизит.ВключатьДеревоЗапросаПриОтладке, Реквизит.МаксСтрокДляСложнойВедомойСтруктуры";
КонецФункции
...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "Проверить") Тогда
	...
	Если мСтруктурноеРедактирование Тогда
		Развернули = Ложь;
		Если Ложь
			Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КонструкторЗапросовИР")
			Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КонструкторЗапросов1С")
		Тогда
			...
			КонецЕсли;
		ИначеЕсли Ложь
			Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаВесьТекст") 
			Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") 
		Тогда
			Если мРежимСворачивания Тогда 
				Если Не КоманднаяПанельТекстЗапросаСворачивать() Тогда 
					Возврат;
			...
			КонецЕсли;
		КонецЕсли;
		Если Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "Форматировать") Тогда
		...
		КонецЕсли; 
	КонецЕсли;
	Если Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		...
		Если СтрДлина(КомпонентаТекстаЗапроса.ПолеТекста.ВыделенныйТекст()) = 0 Тогда
			...
			Если Не ирКэш.ЛиПортативныйРежимЛкс() Тогда
				ПрограммныйКод = ПрограммныйКод + "// ирОбщий.ОтЛкс(" + ИмяПеременнойЗапроса + ") - для отладки запроса в точке останова" + Символы.ПС;
			КонецЕсли; 
		КонецЕсли; 
		ирКлиент.ТекстВБуферОбменаОСЛкс(ПрограммныйКод);
		Возврат;
	КонецЕсли;
	Если Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "ПерейтиКОпределению") Тогда
...
Процедура ВнешнееСобытие(Источник, Событие, Данные) Экспорт
	...
	Если ЭкземплярКомпоненты = ПолеТекстаЗапроса Тогда 
		Если мСтруктурноеРедактирование Тогда 
			Если мРедактируемыйУзел = Неопределено Тогда
				ПроверитьВключитьРедактированиеУзлаСтруктурыЗапроса();
			КонецЕсли;
		Иначе
			ПодключитьОбработчикОжидания("ОбновитьСтруктуруПросмотра", 0.5, Истина);
		КонецЕсли;
	КонецЕсли; 
	
КонецПроцедуры

Процедура ОбновитьСтруктуруПросмотра()
	Если Ложь
	...
	Если ВводДоступенЛкс() Тогда
		Если Истина
			...
			И (ЭлементыФормы.ДеревоЗапросов.Высота < 120 Или ЭлементыФормы.ДеревоЗапроса.Свертка <> РежимСверткиЭлементаУправления.Нет)
		Тогда
		...
		Иначе
			...
			КонецЕсли;
			мРазрешитьУстановкуПозицииИзСтруктурыПросмотра = Ложь;
			Если мСтарыйТекстЗапроса <> ПолеТекстаЗапроса.ПолеТекста.ПолучитьТекст() Тогда 
				РазобратьТекстЗапроса(,,,, Ложь);
				мСтарыйТекстЗапроса = ПолеТекстаЗапроса.ПолеТекста.ПолучитьТекст();
			Иначе
				КоманднаяПанельТекстЗапросаНайтиВДеревеЗапроса();
			КонецЕсли;
			мРазрешитьУстановкуПозицииИзСтруктурыПросмотра = Истина;
		КонецЕсли;
	КонецЕсли;
	ПодключитьОбработчикОжидания("ОбновитьСтруктуруПросмотра", 1, Истина);
КонецПроцедуры
...
Процедура ЗапомнитьИсточникПерехода()
...
Функция ТекущаяКомпонентаПоляТекста(Кнопка = Неопределено)
	...
	Если Ложь
		...
		Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КонструкторЗапросовИР") 
		Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаВесьТекст") 
		Или Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "ВставитьИзБуфераОбменаВесьТекст") 
	Тогда
...
Процедура ОткрытьФайл(РежимОбъединения = Ложь)
	...
	Если РежимОбъединения Или СохранитьВФайл(Истина) Тогда
		РезультатВыбора = ирКлиент.ВыбратьФайлЛкс(, "sel", "Файл консоли запросов", мИмяОткрытогоФайла,,, "Выберите файл со списком запросов");
...
Функция УстановитьПараметрыЗапроса(ОбъектЗапроса = Неопределено, ДобавитьВыборкуИзВременнойТаблицы = Истина, ИспользоватьТекущийПодзапрос = Истина)
	...
	Тогда
		...
		Если Не ПроверитьКорректностьТекстаЗапросаИЗаполнитьПараметры(,,, ОбъектЗапроса.Текст, ДобавитьВыборкуИзВременнойТаблицы,, Ложь) Тогда 
...
Процедура ЗагрузитьДанныеТекущейСтрокиДереваЗапросов()
	...
	ЭлементыФормы.МенюДереваЗапроса.Доступность = мЗапросЗагруженВКонсоль;
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(ДеревоЗапросовСтрокаПоиска) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(, ПолеТекстаЗапроса.ПолеТекста, ДеревоЗапросовСтрокаПоиска, Ложь);
		КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(ПолеТекстаПрограммы.ТекстЗапроса, "КонструкторЗапросовИР").Доступность = ирКэш.ЛиПлатформаWindowsЛкс();
	НастроитьКолонкиТабличногоПоляРезультата();
...
Процедура ДеревоЗапросаПриАктивизацииСтроки(Элемент = Неопределено) Экспорт 
	...
	ПолеТекстаЗапроса.УстановитьФокус(Истина); // Нужно дать полю текста фокус для обхода ошибки платформы с прокруткой
	ЕстьРезультатПоиска = Ложь;
	Если Не мСтруктурноеРедактирование Тогда 
		Если мРазрешитьУстановкуПозицииИзСтруктурыПросмотра Тогда
			УстановитьГраницыВыделенияНаНачалоЭлементаСтруктуры(ТекущаяСтрока);
			НайденныеСтроки = ирКлиент.ДопСвойстваЭлементаФормыЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапроса).МенеджерПоиска.НайденныеСтроки;
			Если НайденныеСтроки.Найти(мТекущаяСтрокаДереваЗапроса) <> Неопределено Тогда
				ЕстьРезультатПоиска = ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаЗапроса.ПолеТекста, ДеревоЗапросовСтрокаПоиска, Ложь);
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Процедура УстановитьГраницыВыделенияНаНачалоЭлементаСтруктуры(Знач ТекущаяСтрока = Неопределено)
	...
	КонецЕсли;
	ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка, СтрокаДанных.КонечнаяСтрока, СтрокаДанных.КонечнаяКолонка);
	//ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(СтрокаДанных.КонечнаяСтрока, СтрокаДанных.КонечнаяКолонка, СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка,, ЭтаФорма);
	ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка, СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка + 1,, ЭтаФорма);
...
Процедура ЗаблокироватьРедактированиеТекстаЗапроса(ТолькоПросмотрДерева)
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	ОбъектКомпоненты = ирКлиент.ПрефиксИменЭлементовЭкземпляраКомпонентыЛкс(ПолеТекстаПрограммы.ТекстЗапроса);
...
Функция КоманднаяПанельТекстЗапросаСохранитьУзел(НовыйРежимСворачивания = Неопределено)
	...
	Если НоваяТекущаяСтрокаДереваЗапроса <> ДеревоЗапроса Тогда 
		...
		КонецЦикла;
	КонецЕсли;
	ДвухмерноеВыделение = ПолеТекстаЗапроса.ПолеТекста.ДвухмерноеВыделение();
	ДеревоЗапросаПриАктивизацииСтроки();
	ЭлементыФормы.ДеревоЗапроса.УстановитьДействие("ПриАктивизацииСтроки", СтарыйОбработчик);
	Если НовыйРежимСворачивания = Неопределено Тогда
		ЗаблокироватьРедактированиеТекстаЗапроса(Истина);
	ИначеЕсли мРедактируемыйУзел <> Неопределено Тогда 
		...
		ОбновитьКнопкуИзменитьСохранитьУзел();
	КонецЕсли;
	ПолеТекстаЗапроса.ПолеТекста.УстановитьДвухмерноеВыделение(ДвухмерноеВыделение);
	Если мРедактируемыйУзел = Неопределено Тогда
		СохранитьФайлДляВосстановления();
	КонецЕсли; 
	Если ЗначениеЗаполнено(ДеревоЗапросовСтрокаПоиска) Тогда
...
Процедура КоманднаяПанельФормыРежимДереваЗапроса(Кнопка = Неопределено) Экспорт 
	...
	ЭлементыФормы.КоманднаяПанельТекстЗапроса.Кнопки.ПерейтиКРодительскомуУзлу.Доступность = мСтруктурноеРедактирование;
	ЭлементыФормы.КоманднаяПанельТекстЗапроса.Кнопки.НайтиВДеревеЗапроса.Доступность = мСтруктурноеРедактирование;
...
Функция РазобратьТекстЗапроса(Знач ТекстЗапроса = Неопределено, Знач СтроитьДеревоЗапроса = Истина, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрокаДляОтображенияОшибок = 0, Знач ПолныйРазбор = Истина)
	...
	Если ПолныйРазбор И ЛиПарсерЗапросаДоступен() Тогда
	...
	Иначе
		...
		Если ТекстыЭлементов.Количество() = 1 Тогда
			ИскатьИсточники = СтрЧислоВхождений(ТекстЗапроса, Символы.ПС) < МаксСтрокДляСложнойВедомойСтруктуры;
		...
		Иначе
			...
			НакопленныйТекст = "";  
			ИскатьИсточники = КореньДерева.СтрокаДанных.КонечнаяСтрока < МаксСтрокДляСложнойВедомойСтруктуры;
...
Процедура ДобавитьСтрокуЗапросаВДеревоЗапроса(Знач ИменаСоздаваемыхТаблиц, Знач ИндексЭлемента, Знач ИскатьИсточники, Знач КореньДерева, Знач ТекстыЭлементов, НакопленныйТекст = "")
	...
	Если ИменаСоздаваемыхТаблиц[ИндексЭлемента] <> Неопределено Тогда
		...
		СтрокаДерева.Имя = "Создать_" + СтрЗаменить(ИменаСоздаваемыхТаблиц[ИндексЭлемента], ".", "_");
	Иначе
...
Процедура ЗаполнитьСтрокуДереваЗапросаУпрощенно(Знач СтрокаДерева, ПредшествующийТекст = "", ИскатьИсточники = Ложь)
	...
	//СтрокаКоординат.НачальнаяСтрока = СтрЧислоСтрок(ПредшествующийТекст); // Не учитывает конечную строку, если она пустая https://partners.v8.1c.ru/forum/topic/472131
	...
	//СтрокаКоординат.КонечнаяСтрока = СтрЧислоСтрок(ПредшествующийТекст); // Не учитывает конечную строку, если она пустая https://partners.v8.1c.ru/forum/topic/472131
	//СтрокаКоординат.КонечнаяСтрока = СтрЧислоВхождений(ПредшествующийТекст, Символы.ПС) + 1; // Неоптимально
	...
	Если ИскатьИсточники Тогда
		ПолеТекстаЗапроса.НайтиВыбранныеТаблицыВТекстеЗапроса(СтрокаДерева, НачальнаяПозицияЗапроса);
	КонецЕсли;
...
Функция ОбновитьСтатусВременойТаблицы(Знач ИмяВременнойТаблицы, НесуществующиеТаблицы = Неопределено)
	...
	Если НесуществующиеТаблицы = Неопределено Тогда
		НесуществующиеТаблицы = Новый Структура;
	КонецЕсли;
	Если Не НесуществующиеТаблицы.Свойство(ИмяВременнойТаблицы) Тогда
		...
		Иначе
			...
			КонецЕсли; 
			НесуществующиеТаблицы.Вставить(ИмяВременнойТаблицы);
		КонецЕсли;
	КонецЕсли;
	Возврат Результат;
...
Функция ПолучитьРезультирующийЗапрос(ЗапросПриемник, Знач ТекстЗапроса = "", ИспользоватьНастройкиРасширений = Ложь)
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаЗаписьНаСервере(Кнопка)
	...
	ПолеТекста = ПолеТекстаПрограммы.КодОбработкиСтрокиРезультата.ПолеТекста;
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
...
Процедура КоманднаяПанельОбработкиРезультатаЗаписьНаСервере(Кнопка)
	...
	ПолеТекста = ПолеТекстаПрограммы.КодОбработкиРезультата.ПолеТекста;
	#Если Сервер И Не Сервер Тогда
		ПолеТекста = Обработки.ирОболочкаПолеТекста.Создать();
		ирОбщий.ЗаписатьОбъектЛкс();
	#КонецЕсли
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапросов <> Ложь Тогда
		...
		ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапросов, Текст, "ТекстЗапроса, Запрос", АктивизироватьСтрокуДереваЗапросов <> Ложь,, Кнопки, ИскатьСНачала = Истина);
	КонецЕсли; 
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		...
		Если ЗначениеЗаполнено(ДеревоЗапросовСтрокаПоиска) Тогда
			ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(?(АктивироватьПолеТекста, ЭтаФорма, Неопределено), ПолеТекстаЗапроса.ПолеТекста, Текст, Ложь, ИскатьСНачала <> Ложь);
		КонецЕсли;
	КонецЕсли; 
...
Процедура КоманднаяПанельКоллекцияРасшифроватьСтроку(Кнопка = Неопределено)
	...
	Если ЕстьГруппировка Тогда
		...
		Иначе
			...
			Сообщение = "Выведенные строки не удолетворяют отбору. Используй команду ""Отладить строку"" на них.";
...
Процедура РезультатКоллекцияИспользоватьОтборПриИзменении(Элемент = Неопределено)
	Если ТипЗнч(ЭлементыФормы.РезультатКоллекция.Значение) = Тип("ТаблицаЗначений") Тогда
		ирКлиент.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.РезультатКоллекция,,, Элемент = Неопределено);
	КонецЕсли;
КонецПроцедуры
