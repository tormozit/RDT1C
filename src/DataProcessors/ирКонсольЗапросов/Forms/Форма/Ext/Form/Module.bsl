...
Процедура ДеревоЗапросаПриАктивизацииСтроки(Элемент = Неопределено) Экспорт 
	...
	ПолеТекстаЗапроса.УстановитьФокус(Истина);
	...
	Если Не мСтруктурноеРедактирование Тогда 
		Если мРазрешитьУстановкуПозицииИзСтруктурыПросмотра Тогда
			...
			КонецЕсли;
			ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка, СтрокаДанных.КонечнаяСтрока, СтрокаДанных.КонечнаяКолонка,, ЭтаФорма);
			...
			ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка, СтрокаДанных.НачальнаяСтрока, СтрокаДанных.НачальнаяКолонка,, ЭтаФорма);
			...
			Если НайденныеСтроки.Найти(мТекущаяСтрокаДереваЗапроса) <> Неопределено Тогда
				ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаЗапроса.ПолеТекста, ДеревоЗапросовСтрокаПоиска, Ложь);
			КонецЕсли;
		КонецЕсли;
		Возврат;
	КонецЕсли;
