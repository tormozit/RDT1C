...
Процедура ДеревоЗапросаПриАктивизацииСтроки(Элемент = Неопределено) Экспорт 
	...
	Если мРазрешитьАктивациюРедактора Тогда
	...
	Если Не мСтруктурноеРедактирование Тогда 
		...
		Тогда
			...
			ПолеТекстаЗапроса.НайтиВыбранныеТаблицыВТекстеЗапроса(ТекущаяСтрока, ПозицияНачалаУзла);
	...
	Иначе
		...
		Если мРедактируемыйУзел = ТекущаяСтрока Тогда
			ПолеТекстаЗапроса.ПолеТекста.УстановитьТекст(мРедактируемыйТекстУзла,, ТекущаяСтрока.Текст);
		КонецЕсли;
...
Функция РазобратьТекстЗапроса(Знач ТекстЗапроса = Неопределено, Знач СтроитьДеревоЗапроса = Истина, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрокаДляОтображенияОшибок = 0,
	...
	Иначе
		...
		Если ТекстыЭлементов.Количество() = 1 Тогда
			ИскатьИсточники = Не (РазрешитьПростойРазборДляВедомой И СтрЧислоВхождений(ТекстЗапроса, Символы.ПС) > МаксСтрокДляСложнойВедомойСтруктуры);
