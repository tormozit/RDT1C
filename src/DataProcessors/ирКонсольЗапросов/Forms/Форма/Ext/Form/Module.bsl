...
Процедура ЗаполнитьОписаниеРезультата(Знач ТекстЗапроса, Знач МассивСоздаваемыхВременныхТаблиц = Неопределено, Знач РежимВременнойТаблицы = Ложь, Знач УстановитьТекст = Истина)
	...
	Если РежимВременнойТаблицы <> Истина Тогда
		...
		мОписаниеРезультата.ПараметрыЗапроса = ирОбщий.СкопироватьКоллекциюЛкс(мТекущаяСтрокаДереваЗапросов.ПараметрыЗапроса);
...
Функция РазобратьТекстЗапроса(Знач ТекстЗапроса = Неопределено, Знач СтроитьДеревоЗапроса = Истина, Знач ПолеТекста = Неопределено, Знач СтартоваяСтрокаДляОтображенияОшибок = 0,
	...
	Иначе
		...
		ИменаСоздаваемыхТаблиц = ирОбщий.СоздаваемыеВременныеТаблицыПакетаЗапросовЛкс(ТекстЗапроса,, ТекстыЭлементов, Истина);
		ИндексЭлемента = 0;
...
Процедура ДобавитьСтрокуЗапросаВДеревоЗапроса(Знач ИменаСоздаваемыхТаблиц, Знач ИндексЭлемента, Знач ИскатьИсточники, Знач КореньДерева, Знач ТекстыЭлементов, НакопленныйТекст = "")
	...
	ИмяОсновнойТаблицы = ИменаСоздаваемыхТаблиц[ИндексЭлемента];
	Если ИмяОсновнойТаблицы <> Неопределено Тогда   
		...
		КонецЕсли;
		СтрокаДерева.НИмяОсновнойТаблицы = НРег(ИмяОсновнойТаблицы);
...
Функция ПроверитьДобавитьВыборкуИзВременнойТаблицыВТекстЗапроса(выхТекстЗапроса, ДобавитьВыборкуИзВременнойТаблицы = Истина)
	...
	#Если Сервер И Не Сервер Тогда
		ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
	#КонецЕсли
	МассивСоздаваемыхВременныхТаблиц = ирОбщий.СоздаваемыеВременныеТаблицыПакетаЗапросовЛкс(выхТекстЗапроса);
	Если Истина
...
Функция ПолучитьРезультирующийЗапрос(ЗапросПриемник, Знач ТекстЗапроса = "", ИспользоватьНастройкиРасширений = Ложь)
	...
	КонецЕсли; 
	МассивСоздаваемыхВременныхТаблиц = ирОбщий.СоздаваемыеВременныеТаблицыПакетаЗапросовЛкс(ТекстЗапроса, Истина);
	Возврат МассивСоздаваемыхВременныхТаблиц;
...
Процедура ОбновитьКнопкиПоляЗначенияПараметра()
	...
	ПолеВвожа.КнопкаСпискаВыбора = ирОбщий.ЛиТипЗначенияДопускаетРегулированиеВПолеВводаЛкс(ТипЗначения);
...
Процедура КоманднаяПанельКоллекцияРасшифроватьСтроку(Кнопка = Неопределено)
	...
	Если Ответ = КодВозвратаДиалога.Нет Или Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если КонструкторЗапроса.ЗагрузитьТекстВКонструктор(ТекстЗапроса, КонструкторЗапроса) Тогда 
			...
			мРезультатЗапроса = ВыполнитьЗапросИсточникаДанных(ТекстЗапросаБезОтбора,,,, мРезультатПакета);
