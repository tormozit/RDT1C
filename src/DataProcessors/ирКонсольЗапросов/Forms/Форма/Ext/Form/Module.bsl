...
Процедура ЗаменитьЛитераломОбращенияКПараметру(Знач СтрокаПараметра)
	...
	Если ЗначениеЗаполнено(ТекстЗамены) Тогда
		...
		Если КоличествоЗначений > 1 Тогда
			...
			СловоЦеликом = Ложь;
		КонецЕсли; 
		Пока КомпонентаТекстаЗапроса.НайтиПоказатьСловоВТексте(ИскомаяСтрока, СловоЦеликом,,, Истина) Цикл
		...
		Иначе
			Если КомпонентаТекстаЗапроса.НайтиПоказатьСловоВТексте("&" + СтрокаПараметра.ИмяПараметра,,,, Истина) Тогда
