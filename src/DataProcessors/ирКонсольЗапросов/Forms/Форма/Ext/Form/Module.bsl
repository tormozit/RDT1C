...
Процедура ЗагрузитьДанныеТекущейСтрокиДереваЗапросов()
	...
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПоискаВДереве) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаЗапроса.ПолеТекста, СтрокаПоискаВДереве, Ложь);
		КонецЕсли; 
	...
	ПолеТекстаЗапроса.ПолеТекста.ТолькоПросмотр(Истина
		...
		И (Ложь
			Или (Истина
				...
				И (Ложь
					Или мРедактируемыйУзел = Неопределено 
					Или мРедактируемыйУзел <> мТекущаяСтрокаДереваЗапроса))
			Или (Истина
...
Процедура ПоместитьТаблицуЗначенийВоВременнуюТаблицуИнтерактивно(Знач ТаблицаЗначений, Знач ИзПараметра = Ложь, Знач ИмяТаблицы = "")
	...
	Если ИзПараметра Или ФормаПараметровПомещения.СоздатьЗапросСозданияТаблицы Тогда
		...
		Если мРежимДереваЗапроса Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
		ПолеТекстаЗапроса.ПолеТекста.УстановитьГраницыВыделения(1, 1);
		Если Не ПустаяСтрока(ПолеТекстаЗапроса.ПолеТекста.ПолучитьТекст()) Тогда
			ТекстЗапросаПоместить = ТекстЗапросаПоместить + ";";
		КонецЕсли; 
		ирОбщий.ПолеТекстаУстановитьВставитьТекстИПереносСтрокиЛкс(ПолеТекстаЗапроса.ПолеТекста, ТекстЗапросаПоместить);
	Иначе
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаЗаписьНаСервере(Кнопка)
	...
	ирОбщий.ПолеТекстаУстановитьВставитьТекстИПереносСтрокиЛкс(ЭлементыФормы.КодОбработкиСтрокиРезультата, "ирОбщий.ЗаписатьОбъектЛкс(Объект, Истина);");
	
КонецПроцедуры

Процедура КоманднаяПанельОбработкиРезультатаЗаписьНаСервере(Кнопка)
	
	ирОбщий.ПолеТекстаУстановитьВставитьТекстИПереносСтрокиЛкс(ЭлементыФормы.КодОбработкиРезультата, "ирОбщий.ЗаписатьОбъектЛкс(Объект, Истина);");
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапросов <> Ложь Тогда
		ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭлементыФормы.ДеревоЗапросов, Текст, "ТекстЗапроса, Запрос", мСтруктураПоискаВДеревеЗапросов, АктивизироватьСтрокуДереваЗапросов <> Ложь);
	КонецЕсли; 
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		ирОбщий.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭлементыФормы.ДеревоЗапроса, Текст, "Текст", мСтруктураПоискаВДеревеЗапроса, АктивизироватьСтрокуДереваЗапроса <> Ложь);
...
Процедура КоманднаяПанельОбработкаСтрокиРезультатаСообщитьСразу(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.СообщитьЛкс();
	#КонецЕсли
	ирОбщий.ПолеТекстаУстановитьВставитьТекстИПереносСтрокиЛкс(ЭлементыФормы.КодОбработкиСтрокиРезультата, "ирОбщий.СообщитьЛкс();");
...
Процедура КоманднаяПанельОбработкиРезультатаСообщитьСразу(Кнопка)
	...
	#Если Сервер И Не Сервер Тогда
		ирОбщий.СообщитьЛкс();
	#КонецЕсли
	ирОбщий.ПолеТекстаУстановитьВставитьТекстИПереносСтрокиЛкс(ЭлементыФормы.КодОбработкиРезультата, "ирОбщий.СообщитьЛкс();");
