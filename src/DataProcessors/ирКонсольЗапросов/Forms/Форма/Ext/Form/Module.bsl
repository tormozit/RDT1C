...
Процедура КлсПолеТекстаПрограммыНажатие(Кнопка)
	...
	Если Кнопка = ирКлиент.КнопкаКоманднойПанелиЭкземпляраКомпонентыЛкс(КомпонентаТекстаЗапроса, "КопироватьВБуферОбменаТекстВВидеКода") Тогда
		...
		Если СтрДлина(КомпонентаТекстаЗапроса.ПолеТекста.ВыделенныйТекст()) = 0 Тогда
			...
			КонецЕсли; 
		КонецЕсли; 
		ирКлиент.ТекстВБуферОбменаОСЛкс(СтрокаТекстаЗапроса);
		Возврат;
	КонецЕсли;
...
Процедура КоманднаяПанельФормыРежимДереваЗапроса(Кнопка = Неопределено) Экспорт 
	...
	Если мТекущаяСтрокаДереваЗапросов.ТипЗапроса <> "Папка" Тогда
		...
		Иначе
			...
			мРедактируемыйУзел = Неопределено;
		КонецЕсли;
		ирКлиент.ИзменитьСвернутостьЛкс(ЭтаФорма, НовыйРежимДереваЗапроса, ЭлементыФормы.ДеревоЗапроса, ЭтаФорма.ЭлементыФормы.гРазделительДеревоЗапросов, ЭтаФорма.Панель, "низ");
	КонецЕсли; 
	мРежимДереваЗапроса = НовыйРежимДереваЗапроса;
...
Процедура ПриПолученииДанныхДоступныхПолей(Элемент, ОформленияСтрок)
	...
	ирКлиент.ПриПолученииДанныхДоступныхПолейКомпоновкиЛкс(ЭтаФорма, Элемент, ОформленияСтрок, ЭлементыФормы.ВыборКомпоновки);
...
Процедура СтрокаПоискаПриИзменении(Элемент = Неопределено, ОбновитьПоДеревуЗапросов = Неопределено, ОбновитьПоДеревуЗапроса = Неопределено,
	...
	Если ОбновитьПоДеревуЗапросов <> Ложь Тогда
		...
		ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапросов, Текст, "ТекстЗапроса, Запрос", АктивизироватьСтрокуДереваЗапросов <> Ложь,, Кнопки);
	КонецЕсли; 
	Если ОбновитьПоДеревуЗапроса <> Ложь Тогда
		АктивироватьПолеТекста = Элемент <> Неопределено;
		ирКлиент.ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ЭтаФорма, ЭлементыФормы.ДеревоЗапроса, Текст, "Текст", АктивизироватьСтрокуДереваЗапроса <> Ложь);
		ирКлиент.НайтиПоказатьСтрокуВПолеТекстаЛкс(?(АктивироватьПолеТекста, ЭтаФорма, Неопределено), ПолеТекстаЗапроса.ПолеТекста, Текст, Ложь);
	КонецЕсли; 
