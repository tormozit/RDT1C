	...
	Если мТекущаяСтрокаДереваЗапросов.ТипЗапроса = "Компоновка" Тогда 
		...
		Если КомпоновкаУдалась Тогда
			Если МакетКомпоновкиДанных.НаборыДанных.Количество() > 0 Тогда
				...
				КонецЦикла;
			Иначе
				// Такое происходит если нет ни одного запроса на выборку или выбранных полей нет
				ирОбщий.СообщитьЛкс("Нет ни одного запроса на выборку или нет выбранных полей компоновки");
			КонецЕсли;
