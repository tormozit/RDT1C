...
Процедура ЗагрузитьДанныеТекущейСтрокиДереваЗапросов()
	...
	Если мЗапросЗагруженВКонсоль Тогда
		...
		Если ЗначениеЗаполнено(ДеревоЗапросовСтрокаПоиска) Тогда
			ВосстановитьТекущуюСтрокуВТекстеЗапроса = Не ирОбщий.НайтиПоказатьСтрокуВПолеТекстаЛкс(ЭтаФорма, ПолеТекстаЗапроса.ПолеТекста, ДеревоЗапросовСтрокаПоиска, Ложь);
		КонецЕсли; 
