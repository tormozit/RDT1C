...
Функция ПроверитьДобавитьВыборкуИзВременнойТаблицыВТекстЗапроса(выхТекстЗапроса, ДобавитьВыборкуИзВременнойТаблицы = Истина)
	...
	Если Истина
		...
		И ПолеТекстаПрограммы.ТекстЗапроса.мПараметрыДиалектаSQL.Пакет
	Тогда
		ИмяВременнойТаблицы = МассивСоздаваемыхВременныхТаблиц[МассивСоздаваемыхВременныхТаблиц.ВГраница()];
		Если ИмяВременнойТаблицы <> Неопределено Тогда
...
Функция ПолучитьРезультирующийЗапрос(Запрос, Знач ТекстЗапроса = "", ИспользоватьНастройкиРасширений = Ложь)
	...
	Если мТекущаяСтрокаДереваЗапросов.ТипЗапроса = "Компоновка" Тогда 
		...
		Попытка
			...
			#КонецЕсли
			МакетКомпоновкиДанных = ирОбщий.СобратьМакетКомпоновкиЛкс(мСхемаКомпоновки, Компоновщик.Настройки,,,, Ложь);
		...
		Если КомпоновкаУдалась Тогда
			...
			Если ВыраженияКомпоновки.Количество() > 0 Тогда
				...
				Для Каждого СтрокаПараметра Из ВыраженияКомпоновки Цикл
					...
					ирОбщий.НайтиДобавитьЭлементНастроекКомпоновкиПоПолюЛкс(НастройкаКомпоновки.Выбор, Новый ПолеКомпоновкиДанных("ПараметрыДанных." + СтрокаПараметра.ИмяПараметра));
				КонецЦикла;
				Попытка
					ЗначенияВычисляемыхПараметров = ирОбщий.СкомпоноватьВКоллекциюЗначенийПоСхемеЛкс(мСхемаКомпоновки, НастройкаКомпоновки);
				Исключение
...
Процедура РезультатКоллекцияИспользоватьОтборПриИзменении(Элемент = Неопределено)
	ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(ЭтаФорма, ЭлементыФормы.РезультатКоллекция);
КонецПроцедуры
