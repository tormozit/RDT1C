...
Функция ОбновитьКонтекстПодсказкиИПолучитьСтруктуруПараметров(ЭкземплярКомпоненты)
	...
	#КонецЕсли
	Если ЭкземплярКомпоненты = ПолеТекстаПрограммы.КодОбработкиРезультата Тогда
	...
	ИначеЕсли ЭкземплярКомпоненты = ПолеТекстаПрограммы.КодПередВыполнениемЗапроса Тогда
	...
	ИначеЕсли ЭкземплярКомпоненты = ПолеТекстаПрограммы.КодОбработкиСтрокиРезультата Тогда
	...
	ИначеЕсли ЭкземплярКомпоненты = ПолеТекстаПрограммы.ТекстЗапроса Тогда
		ВычислитьПараметрыЗапроса(, Ложь);
		ОбновитьПараметрыКонтекстнойПодсказкиТекстаЗапроса();
		ПрефиксПараметра = ЭкземплярКомпоненты.ПрефиксПараметра();
		//КонецЕсли;
		...
		//Если Лев(Кнопка.Имя, СтрДлина(МаркерЭкземпляра)) = МаркерЭкземпляра Тогда
			...
			Для Каждого СтрокаПараметра Из Параметры Цикл
				ЭкземплярКомпоненты.ДобавитьСловоЛокальногоКонтекста(ПрефиксПараметра + СтрокаПараметра.ИмяПараметра,, ирОбщий.ОписаниеТиповИзТипаЛкс(ТипЗнч(СтрокаПараметра.Значение)));
				Если ТипЗнч(СтрокаПараметра.Значение) = Тип("ТаблицаЗначений") Тогда
					ирОбщий.ДобавитьДоступнуюТаблицуБДЛкс(ЭкземплярКомпоненты.ДоступныеТаблицы, ПрефиксПараметра + СтрокаПараметра.ИмяПараметра, , "Параметр", СтрокаПараметра.ИмяПараметра);
				КонецЕсли; 
			КонецЦикла;
		//КонецЕсли; 
	...
	Если Событие <> Неопределено Тогда
		...
		ИначеЕсли Событие.event = "EVENT_BEFORE_SHOW_SUGGEST" Тогда
			#Если Сервер И Не Сервер Тогда
				ПолеТекстаЗапроса = Обработки.ирКлсПолеТекстаПрограммы.Создать();
			#КонецЕсли
			ОбновитьКонтекстПодсказкиИПолучитьСтруктуруПараметров(ПолеТекстаЗапроса);
