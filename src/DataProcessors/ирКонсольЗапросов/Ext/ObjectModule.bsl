...
Функция РедактироватьНаборДанныхСхемыКомпоновкиДанных(Знач ВладелецФормы, Знач НаборДанных, Знач Схема, Знач НастройкаКомпоновки = Неопределено, Знач ИмяПоляКомпоновки = "",
	Знач НомерСтроки = Неопределено) Экспорт
	...
	мСтрокаЗапросаПараметра.НачальнаяСтрока = НомерСтроки;
	мСтрокаЗапросаПараметра.НачальнаяКолонка = 1;
	мСтрокаЗапросаПараметра.КонечнаяСтрока = НомерСтроки;
	мСтрокаЗапросаПараметра.КонечнаяКолонка = 1;
...
Функция ПодключитьИсточникADO(ИсточникДанных, выхСоединениеADO = Неопределено, стОпции = Неопределено, выхСписокОшибок = Неопределено, выхСтрокаСоединенияДляСообщения = "", Асинхронно = Ложь, выхСоединениеФакт = "") Экспорт
	Перем Параметры;
	Перем Результат;
	...
	Если Параметры.ТипИсточникаДанных=0 Тогда
		...
		Иначе
			...
			ИначеЕсли (ПутьДоступен = Ложь) И (Лев(Нрег(Путь),7)<>Нрег("<Пусто>")) Тогда
				выхСписокОшибок.Добавить("Ошибка доступности файла с данными: "+СтрИнфо);
			КонецЕсли;
		КонецЕсли; 
	ИначеЕсли Параметры.ТипИсточникаДанных=1 Тогда
		...
		Если НЕ ПутьДоступен Тогда
			выхСписокОшибок.Добавить("Ошибка доступности каталога файловой базы: "+СтрИнфо);
		КонецЕсли; 
	ИначеЕсли Параметры.ТипИсточникаДанных = 2 Тогда
		...
		Если ПустаяСтрока(Параметры.БазаСервер) И НЕ ПустаяСтрока(Параметры.БазаИмя) Тогда
			выхСписокОшибок.Добавить("Не указано имя сервера для клиент-серверной базы");
		ИначеЕсли НЕ ПустаяСтрока(Параметры.БазаСервер) И ПустаяСтрока(Параметры.БазаИмя) Тогда
			выхСписокОшибок.Добавить("Не указано имя базы на сервере для клиент-серверной базы");
		ИначеЕсли ПустаяСтрока(Параметры.БазаСервер) И ПустаяСтрока(Параметры.БазаИмя) Тогда
			выхСписокОшибок.Добавить("Не указано имя сервера и имя базы на сервере для клиент-серверной базы");
		КонецЕсли; 
	ИначеЕсли Параметры.ТипИсточникаДанных=3 Тогда
		// ресурс интернета
		Если ПустаяСтрока(Путь) Тогда
			выхСписокОшибок.Добавить("Не задан путь (URL) к ресурсу интернета базы данных");
		КонецЕсли; 
    Иначе
		выхСписокОшибок.Добавить("Не предусмотренный тип источника данных: "+ Параметры.ТипИсточникаДанных);
	КонецЕсли;
	
	Если ПроверятьПользователя И Не Параметры.АутентификацияОС Тогда
		Если ПустаяСтрока(Параметры.Пользователь) Тогда
			выхСписокОшибок.Добавить("Не указано имя пользователя базы");
		КонецЕсли; 
	КонецЕсли; 
	
	//Если ВыбратьПуть=Истина И выхСписокОшибок.Количество()=0 Тогда
	//	#Если Клиент Тогда
	...
	//			выхСписокОшибок.Добавить("Выбор файла с данными для подключения к источнику данных отменен пользователем");
	//		КонецЕсли;
	//	#Иначе 
	//		выхСписокОшибок.Добавить("Не выбран файл с данными для подключения к источнику данных");
	//	#КонецЕсли	
	//КонецЕсли;
	...
	Если выхСписокОшибок.Количество() > 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		...
		|";
		выхСписокОшибок.Добавить(СтрЕррор);
		Результат = -1;
	КонецПопытки;
	...
	Иначе
		...
		Если Результат <> 1  Тогда
			...
			КонецЕсли; 
			выхСписокОшибок.Добавить(ЕррорТекст);
		КонецЕсли; 
	КонецЕсли;
...
ДеревоЗапросов.Колонки.Добавить("КопироватьМенеджерНаСервер", Новый ОписаниеТипов("Булево"));
