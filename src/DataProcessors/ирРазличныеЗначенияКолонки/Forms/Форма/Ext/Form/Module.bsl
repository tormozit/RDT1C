...
Функция ОбновитьДанные(РежимОтладки = Ложь, ИзменятьФлагУчитыватьОтбор = Ложь)
	...
	Иначе
		...
		Запрос = ирОбщий.ЗапросДинамическогоСпискаЛкс(ИсточникДействий, ПолноеИмяТаблицыБД, УчитываяОтбор, КорневоеПолеКолонки, Компоновщик);
...
Процедура ДействияФормыСнятьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭтаФорма, ЭлементыФормы.Значения, , Ложь);
...
Процедура ДействияФормыУстановитьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭтаФорма, ЭлементыФормы.Значения, , Истина);
...
Процедура ПриОткрытии()
	...
	ирОбщий.СоздатьМенеджерСохраненияНастроекФормыЛкс(ЭтаФорма, ЭлементыФормы.ПредставлениеТипаЗначения.Видимость);
	НастроитьЭлементыФормы();
КонецПроцедуры
...
Процедура ИмяТипаЗначенияОткрытие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ирОбщий.ПредложитьЗакрытьМодальнуюФормуЛкс(ЭтаФорма);
	ирОбщий.ОткрытьОбъектМетаданныхЛкс(Метаданные.НайтиПоТипу(Тип(ИмяТипаЗначения)));
...
Процедура ДействияФормыСортироватьПоУбыванию(Кнопка)
	...
	СтрокаСортировки = ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.Значения, Ложь, Найти(ИмяТипаЗначения, ".") > 0);
...
Процедура ДействияФормыСортироватьПоВозрастанию(Кнопка)
	ЗначенияРасширенные.ЗагрузитьКолонку(ЭлементыФормы.Значения.Значение.ВыгрузитьКолонку("Пометка"), "Пометка");
	СтрокаСортировки = ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.Значения, Истина, Найти(ИмяТипаЗначения, ".") > 0);
	Если ЗначениеЗаполнено(СтрокаСортировки) Тогда
		ЗначенияРасширенные.Сортировать(СтрокаСортировки);
	КонецЕсли;
КонецПроцедуры
