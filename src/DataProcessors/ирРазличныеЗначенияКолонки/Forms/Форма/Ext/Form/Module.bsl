...
Функция ОбновитьДанные(РежимОтладки = Ложь, ИзменятьФлагУчитыватьОтбор = Ложь)
	...
	Иначе
		...
		Если ВыгрузкаРезультата = Неопределено Тогда
			...
			ВыгрузкаРезультата = ирОбщий.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ИсточникДействий,,,, ?(УчитываяОформление, СтруктураПолученияТекстовЯчеек, Неопределено)); // Отбор здесь не учитываем, т.к. по текущей колонке мы его отключаем
			#Если Сервер И Не Сервер Тогда
