...
Функция СохраняемаяНастройкаФормы(выхНаименование, выхИменаСвойств) Экспорт 
	...
	Если Истина
		...
		И Не ЭлементыФормы.ИмяТипаЗначения.Видимость
	Тогда
...
Функция ОбновитьДанные(РежимОтладки = Ложь, ИзменятьФлагУчитыватьОтбор = Ложь)
	...
	Иначе
		...
		Если ТипИсточникаДанных = "ТаблицаЗначений" Тогда 
			...
			Тогда
				КомпоновкаТП = ирОбщий.КомпоновкаТабличногоПоляЛкс(Форма, ИсточникДействий);
		...
		Если ВыгрузкаРезультата = Неопределено Тогда
			...
			ВыгрузкаРезультата = ирОбщий.ТаблицаИлиДеревоЗначенийИзТаблицыФормыСКоллекциейЛкс(ИсточникДействий,, УчитываяОтбор,, ?(УчитываяОформление, СтруктураПолученияТекстовЯчеек, Неопределено)); 
			#Если Сервер И Не Сервер Тогда
	...
	Иначе
		...
		#КонецЕсли
		Если ПоляТаблицыБД.Найти(КорневоеПолеКолонки, "Имя") <> Неопределено Тогда
			Запрос = Новый Запрос;
			ТекстИсточникаДанных = ПолноеИмяТаблицыБД;
			МассивТекстовЗапросов = Неопределено;
			...
			КонецЦикла;
		Иначе
			СхемаИНастройки = ирОбщий.ИсполняемыеСхемаИНастройкиСпискаУправляемойФормаЛкс(ирОбщий.РодительЭлементаУправляемойФормыЛкс(ВладелецФормы));
			...
			#Если Сервер И Не Сервер Тогда
				Запрос = Новый Запрос;
			#КонецЕсли
			МассивТекстовЗапросов = мПлатформа.РазбитьГрубоТекстПакетногоЗапросаНаТекстыЗапросов(Запрос.Текст);
		...
		КонецЕсли; 
		ЭтаФорма.ОтборИсточникаБезТекущейКолонки = ирОбщий.ПредставлениеОтбораЛкс(НастройкаКомпоновки.Отбор);
		Если Не УчитываяОтбор Или ТекстИсточникаДанных <> ПолноеИмяТаблицыБД Тогда
			НастройкаКомпоновки.Отбор.Элементы.Очистить();
		КонецЕсли;
	...
	ЭтаФорма.ИмяТипаЗначения = СтрЗаменить(ИмяТипаЗначения, "Ссылка" + ".", "."); // Здесь переводить фрагмент "Ссылка" не нужно
...
Функция ОсновныеДействияФормыПрименить(Кнопка = Неопределено)
	...
	ИначеЕсли ЭтоБулеваНеограниченнаяСтроковаяКолонка() И СписокЭлементовДляОтбора.Количество() > 0 Тогда
	...
	КомпоновкаТП = ирОбщий.КомпоновкаТабличногоПоляЛкс(Форма, ИсточникДействий);
	Если КомпоновкаТП <> Неопределено Тогда
		ирОбщий.ТабличноеПолеСОтборомПросмотраОтобратьЛкс(Форма, ИсточникДействий,,, Истина);
	КонецЕсли; 
	Возврат Истина;
...
Процедура ДействияФормыСнятьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.Значения, , Ложь);
...
Процедура ДействияФормыОткрытьТип(Кнопка)
	...
	КонецЕсли; 
	ирОбщий.ПредложитьЗакрытьМодальнуюФормуЛкс(ЭтаФорма);
	ирОбщий.ОткрытьЗначениеЛкс(Тип(ирОбщий.ПервыйФрагментЛкс(ЭлементыФормы.Значения.ТекущаяСтрока.ИмяТипаЗначения, "(")), Ложь);
...
Процедура ПрименитьФильтрПоПодстрокеБезСохранения()
	...
	Если Найти(ИмяТипаЗначения, ".") > 0 Тогда 
		ТипЗначения = Тип(ирОбщий.ИмяТипаИзПолногоИмениМДЛкс(ИмяТипаЗначения));
...
Процедура ДействияФормыУстановитьПометки(Кнопка)
	...
	ирОбщий.ИзменитьПометкиВыделенныхИлиОтобранныхСтрокЛкс(ЭлементыФормы.Значения, , Истина);
...
Процедура ПриОткрытии()
	...
	ирОбщий.СоздатьМенеджерСохраненияНастроекФормыЛкс(ЭтаФорма, ЭлементыФормы.ИмяТипаЗначения.Видимость);
	НастроитьЭлементыФормы();
КонецПроцедуры
...
Процедура ДействияФормыСортироватьПоУбыванию(Кнопка)
	...
	ЗначенияРасширенные.Сортировать(ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.Значения, Ложь));
КонецПроцедуры

Процедура ДействияФормыСортироватьПоВозрастанию(Кнопка)
	ЗначенияРасширенные.ЗагрузитьКолонку(ЭлементыФормы.Значения.Значение.ВыгрузитьКолонку("Пометка"), "Пометка");
	ЗначенияРасширенные.Сортировать(ирОбщий.ТабличноеПолеСортироватьЛкс(ЭтаФорма, ЭлементыФормы.Значения, Истина));
КонецПроцедуры

Процедура КлсКомандаТаблицаНажатие(Кнопка)
	ирОбщий.УниверсальнаяКомандаФормыЛкс(ЭтаФорма, Кнопка, ЭлементыФормы.Значения);
КонецПроцедуры

ирОбщий.ИнициироватьФормуЛкс(ЭтаФорма, "Обработка.ирРазличныеЗначенияКолонки.Форма.Форма");
