	...
	Если _РежимОтладки Тогда // Можно менять на Истина в точке останова, например условием ирОбщий.Пр(_РежимОтладки, 1, 1)
		...
		Для Каждого СтрокаПоля Из ПоляТаблицыБД Цикл
			...
			Если СтрокаПоля.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений")) Тогда
				ТабличнаяЧасть = Объект[СтрокаПоля.Имя]; // ТабличнаяЧасть
...
Процедура ЗагрузитьСнимок(Снимок, ТолькоДанные = Ложь) Экспорт
	...
	Если Данные <> Неопределено Тогда
		Данные.Очистить();
		ирОбщий.СкопироватьКоллекциюЛкс(СтруктураОбъекта.Данные, Данные);
	Иначе
...
Функция ОбъектБД(ВосстановитьДанные = Истина) Экспорт
	...
	Результат = ирОбщий.ОбъектБДПоКлючуЛкс(ПолноеИмяТаблицы, КлючОбъекта,, Ложь, Ложь,,,, Не ВосстановитьДанные).Данные; // ДокументОбъект, СправочникОбъект, БизнесПроцессОбъект
	//КонецЕсли;
	...
	Если ВосстановитьДанные Тогда
		...
		Для Каждого КлючИЗначение Из Данные Цикл
			...
			Попытка
				Результат[ИмяРеквизита] = Данные[ИмяРеквизита];
			Исключение
				// Реквизит группы для элемента или наоборот
			КонецПопытки;
		КонецЦикла;
...
Функция ЭтоНовый() Экспорт
	СсылкаДанных = Данные[ирОбщий.ПеревестиСтроку("Ссылка")]; // ЛюбаяСсылка - почему не используем ЭтотОбъект.Ссылка?
	Результат = СсылкаДанных.Пустая(); 
	Возврат Результат;
КонецФункции
...
Функция ЗаменитьИдентификаторОбъекта(ИдентификаторСсылки) Экспорт
	ЭтотОбъект.Ссылка = Новый (ТипЗнч(Ссылка)); // Теперь это новый объект
	ЭтотОбъект._СсылкаНового = Новый (ТипЗнч(Ссылка), ирОбщий.ЗначенияВМассивЛкс(ИдентификаторСсылки));
КонецФункции
...
Процедура УстановитьНовыйНомер(ПрефиксНомера) Экспорт
	...
	Иначе
		ОбъектБД = ОбъектБД();
		ОбъектБД.УстановитьНовыйНомер(ПрефиксНомера);
		Конструктор(ОбъектБД);
	КонецЕсли;
КонецПроцедуры
