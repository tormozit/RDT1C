...
Процедура ОбновитьСлужбуПоСтроке(СтрокаСлужбы)
	...
	Иначе
		...
		ИначеЕсли ирОбщий.СтрНачинаетсяСЛкс(СтрокаСлужбы.ИмяПользователя, "NT AUTHORITY\") Тогда
			Команда = Команда + " obj= """ + СтрокаСлужбы.ИмяПользователя + """";
		КонецЕсли; 
	КонецЕсли; 
	Результат = ирОбщий.ВыполнитьКомандуОСЛкс(Команда,,, Истина);
	...
	Тогда
		...
		// Передаем полные имена файлов, т.к. powershell не наследует текущий каталог через cmd.exe
		КомандаСистемыЗапускаСкрипта = ирОбщий.КомандаСистемыЗапускаСкриптаPowerShellЛкс("""" + ФайлСкрипта.ПолноеИмя + """ -FolderListFile """ + ФайлПараметров.ПолноеИмя + """");
		РезультатУстановкиПрав = ирОбщий.ВыполнитьКомандуОСЛкс(КомандаСистемыЗапускаСкрипта,,, Истина, "Предоставление прав на файлы"); // Может выполняться долго
...
Процедура ОбновитьСтрокуЗапускаСлужбы(Знач СтрокаСлужбы) Экспорт 
	...
	Если СтрокаСлужбы.ТипСлужбы = мТипыСлужб.АгентСервера.ТипСлужбы Тогда
		...
		ИначеЕсли СтрокаСлужбы.РежимОтладки = "http" Тогда
			...
			КонецЕсли; 
			Если ЗначениеЗаполнено(СтрокаСлужбы.СерверОтладкиПорт) Или СтрокаСлужбы.Порт <> 1540 Тогда
				СерверОтладкиПорт = СтрокаСлужбы.СерверОтладкиПорт;
				Если Не ЗначениеЗаполнено(СерверОтладкиПорт) Тогда
					СерверОтладкиПорт = СтрокаСлужбы.Порт + 100;
				КонецЕсли;
				СтрокаЗапускаНовая = СтрокаЗапускаНовая + " -debugServerPort " + XMLСтрока(СерверОтладкиПорт);
			КонецЕсли; 
...
Функция СоединениеADO(Знач СерверMSSQL, ИмяБД = "master") Экспорт 
	...
	Если Не КонсольЗапросов.ПодключитьИсточникADO(ИсточникДанных, СоединениеADO,, ОшибкиПодключения) Тогда  
		СоединениеADO = Неопределено;
