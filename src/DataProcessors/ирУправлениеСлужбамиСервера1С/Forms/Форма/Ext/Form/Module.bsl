...
Процедура СлужбыАгентовСерверов1ССборкаПлатформыНоваяНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	НовыйКаталог = ирКлиент.ВыбратьКаталогВФормеЛкс(Элемент.Значение);
	Если ЗначениеЗаполнено(НовыйКаталог) Тогда
		ирКлиент.ИнтерактивноЗаписатьВПолеВводаЛкс(Элемент, НовыйКаталог);
	КонецЕсли; 
...
Процедура ГлавнаяКоманднаяПанельВыгрузитьБД(Кнопка)
	...
	//ПараметрыСУБД = ирОбщий.ПараметрыСоединенияADOЭтойБДЛкс(); 
	//СтрокаКоманды = """" + ИсполняемыйФайл.ПолноеИмя + """ """ + Команда + """ ";
	//Если ирКэш.ЛиФайловаяБазаЛкс() Тогда
	//	СтрокаКоманды = СтрокаКоманды + "--db-path=" + НСтр(СтрокаСоединенияИнформационнойБазы(), "file");
	//Иначе
	...
	ирОбщий.ВыполнитьКомандуАвтоСервераЭтойБазыЛкс(Команда,,, ИсполняемыйФайл.ПолноеИмя);
