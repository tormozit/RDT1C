...
Процедура КластерыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	КаталогКластера = ЭлементыФормы.СлужбыАгентовСерверов.ТекущаяСтрока.КаталогКонфигурации + "\reg_" + ТекущаяСтрока.Кластер;
	ФормаКластера = ПолучитьФорму("СписокБазКластера", ЭтаФорма, КаталогКластера);
	ФормаКластера.КластерИмя = ТекущаяСтрока.Кластер;
	ФормаКластера.КаталогКластера = КаталогКластера;
	ФормаКластера.ПараметрАгентСервера = ЭлементыФормы.СлужбыАгентовСерверов.ТекущаяСтрока;
