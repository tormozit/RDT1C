...
Процедура ПриОткрытии()
	...
	//ЭтаФорма.x64 = ирОбщий.СтрКончаетсяНаЛкс(ПараметрАгентСервера.СборкаПлатформыАктивная, "64");
	ОбновитьСписок();
...
Процедура ЗаполнитьТаблицуБазИзФайловКластера(КаталогКластера, ВычислятьРазмеры = Истина) Экспорт 
	...
	ФайлКонфигурацииКластера = ФайлКонфигурацииКластера(КаталогКластера);
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.Прочитать(ФайлКонфигурацииКластера.ПолноеИмя);
	...
	Пока 1 = 1 Цикл
		...
		ОписаниеБазы.РазрешенаВыдачаЛицензий = Число(ДочерниеУзлы[12].ТекстовоеСодержимое) > 0;
		Попытка
...
Функция ФайлКонфигурацииКластера(Знач КаталогКластера) Экспорт
	...
	Если Не ФайлКонфигурацииКластера.Существует() Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Возврат ФайлКонфигурацииКластера;
КонецФункции
...
Процедура ДействияФормыОчиститьЖурнал(Кнопка)
	Ответ = Вопрос("Вы уверены что хотите очистить журнал регистрации выделенных баз (все файлы должны быть свободны)?", РежимДиалогаВопрос.ОКОтмена);
...
Процедура ДействияФормыОчиститьИндексПоиска(Кнопка)
	Ответ = Вопрос("Вы уверены что хотите очистить индекс полнотекстового поиска выделенных баз (все файлы должны быть свободны)?", РежимДиалогаВопрос.ОКОтмена);
	Если Ответ <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	Индикатор = ирОбщий.ПолучитьИндикаторПроцессаЛкс(ЭлементыФормы.БазыКластера.ВыделенныеСтроки.Количество());
...
Процедура КаталогКластераПриИзменении(Элемент)
	ОбновитьСписок();
КонецПроцедуры
...
Процедура ДействияФормыОчиститьСеансовыеДанные(Кнопка)
	НужноЗапуститьСлужбу = Ложь;
	Если ПараметрАгентСервера.Выполняется Тогда
		Ответ = Вопрос("Служба агента сервера будет перезапущена. Продолжить?", РежимДиалогаВопрос.ОКОтмена);
		Если Ответ <> КодВозвратаДиалога.ОК Тогда
			Возврат;
		КонецЕсли;
		ИзменитьСостояниеСлужбы(ПараметрАгентСервера, "stop");
		НужноЗапуститьСлужбу = Истина;
	КонецЕсли;
	Для Каждого Файл Из НайтиФайлы(КаталогКластера, "snccntx*") Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если НужноЗапуститьСлужбу Тогда
		ИзменитьСостояниеСлужбы(ПараметрАгентСервера, "start");
	КонецЕсли;
КонецПроцедуры

Процедура ДействияФормыЗаблокироватьРегламентныеЗадания(Кнопка)
	Файл = ФайлКонфигурацииКластера(КаталогКластера);
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.Прочитать(Файл.ПолноеИмя);
	СтарыйТекст = ТекстовыйДокумент.ПолучитьТекст();
	Шаблон = "(\})(,\d)(,\d,"")";
	Вхождения = ирОбщий.НайтиРегВыражениеЛкс(СтарыйТекст, Шаблон,,,,, Ложь);
	ЧислоНезаблокированных = Вхождения.НайтиСтроки(Новый Структура("Группа1", ",0")).Количество();
	Если ЧислоНезаблокированных = 0 Тогда
		ирОбщий.СообщитьЛкс("Регламентные задания всех баз уже заблокированы");
		Возврат;
	КонецЕсли;
	Если Ложь
		Или Вхождения.НайтиСтроки(Новый Структура("Группа1", ",1")).Количество() <> БазыКластера.НайтиСтроки(Новый Структура("ЗапретРегламентныхЗаданий", Истина)).Количество()
		Или ЧислоНезаблокированных <> БазыКластера.НайтиСтроки(Новый Структура("ЗапретРегламентныхЗаданий", Ложь)).Количество()
	Тогда
		ирОбщий.СообщитьЛкс("Неподдерживаемый формат файла");
		Возврат;
	КонецЕсли;
	Ответ = Вопрос("Вы уверены что хотите заблокировать регламентные задания ВСЕХ баз кластера?", РежимДиалогаВопрос.ОКОтмена);
	Если Ответ <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	НужноЗапуститьСлужбу = Ложь;
	Если ПараметрАгентСервера.Выполняется Тогда
		...
		НужноЗапуститьСлужбу = Истина;
	КонецЕсли;
	НовыйТекст = ирОбщий.ЗаменитьРегВыражениеЛкс(СтарыйТекст, Шаблон, "$1,1$3", Ложь);
	//РезультатСравнения = ирКлиент.СравнитьЗначенияВФормеЧерезXMLЛкс(СтарыйТекст, НовыйТекст, Истина, "Старый", "Новый",,, "Подтвердите изменения",,,,, Истина);
	//Если РезультатСравнения = Неопределено Тогда
	//	Возврат;
	//КонецЕсли;
	ИмяРезервногоФайла = СтрЗаменить(Файл.ПолноеИмя, "lst.lst", "lsto.lst");
	ТекстовыйДокумент.Записать(ИмяРезервногоФайла);
	ирОбщий.СообщитьЛкс("Сохранен резервный файл кластера " + ИмяРезервногоФайла);
