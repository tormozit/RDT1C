...
Процедура ОткрытьФормуНастроек()
	...
	Иначе
		...
		КонецЕсли; 
		Если Не ФормаНастроек.Открыта() Тогда
			ирОбщий.СообщитьЛкс();
			Возврат;
		КонецЕсли; 
	...
	Если ирКэш.ЛиПортативныйРежимЛкс() Тогда
		...
		Если Структура.Ответ Тогда
			...
			Если Ответ = КодВозвратаДиалога.ОК Тогда
				...
				ирОбщий.ОповеститьФормыПодсистемыЛкс("ЗакрытьВсеФормыИнструментовРазработчика", Структура); // Второй проход нужен для двух шагового закрытия форм, например КонсольКода+ВызовМетода
