...
Процедура ирОтладить(Команда)
	...
	#Иначе
		...
		ОтчетОбъект = ЭтаФорма.Отчет;  //@skip-check unknown-method-property
		ирОбщий.ОтладитьЛкс(СхемаКомпоновки,, ОтчетОбъект.КомпоновщикНастроек.ПолучитьНастройки());
	#КонецЕсли
...
Функция ПолучитьАдресСхемы()
	...
	Иначе
		ОтчетОбъект = РеквизитФормыВЗначение("Отчет"); // ОтчетОбъект
