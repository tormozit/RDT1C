...
Процедура ПередЗаписью(Отказ)
	...
	КонецЕсли;
	Если Не Отказ Тогда
		ТекстАлгоритма = ПолеТекстаПрограммы.ПолеТекстовогоДокумента.ПолучитьТекст();
	КонецЕсли;
...
Процедура ПриИзмененииДанных()
	...
	ПолеТекстаПрограммы.ПолеТекстовогоДокумента.УстановитьТекст(ТекстАлгоритма);
...
Функция ПоказатьОшибкуВыполнения(ИнформацияОбОшибке, Знач ТекстСообщения = "", РежимВыполненияАлгоритма = 0, СтартоваяСтрока = 0) Экспорт
	...
	Если ВыполнятьАлгоритмыЧерезВнешниеОбработки Тогда 
		ОбновитьСвязи();
	КонецЕсли;
	ТекущийЭлемент = ПолеТекстаПрограммы.ПолеТекстовогоДокумента;
	Если Ложь
	...
	ТекстИстиннойОшибки = ирОбщий.ПоказатьОшибкуВЗапросеИлиПрограммномКодеЛкс(ПолеТекстаПрограммы.ПолеТекстовогоДокумента,
...
Процедура ОбновитьСвязи()
	...
	КонецЕсли;
	ПолеТекстаПрограммы.ПолеТекстовогоДокумента.ЦветРамки = НовыйЦветРамки;
...
Процедура КоманднаяПанельТекстАлгоритмаОбновитьИзФайла(Кнопка)
	...
	Если ТекстАлгоритмаИзФайла <> Неопределено Тогда
		...
		ПолеТекстаПрограммы.ПолеТекстовогоДокумента.УстановитьТекст(ТекстАлгоритмаИзФайла);
