...
Процедура ДействияФормыСформировать(Кнопка = Неопределено) Экспорт 
	...
	Если Ложь
		...
		Или (Истина
			...
			И Роль <> РольСтарый) 
	Тогда
		ТаблицаПрав = ирОбщий.ВычислитьПраваДоступаЛкс(ВычислятьФункциональныеОпции, ИменаРолей, ИспользоватьНаборПолей, НаборПолейТаблица.Выгрузить(), ОбъектМетаданных, ПолеОбъекта, Пользователь,
			ЭтаФорма, ЭлементыФормы.ДействияФормы.Кнопки.Сформировать, "СформироватьЗавершение");
	Иначе
		ТаблицаПрав = ТаблицаПравСтарая;
