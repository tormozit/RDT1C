...
Процедура ДействияФормыСформировать(Кнопка = Неопределено) Экспорт 
	...
	Если Ложь
		...
		Или (Истина
			...
			И Роль <> РольСтарый) 
	Тогда
		Результат = ирОбщий.ВычислитьПраваДоступаЛкс(ВычислятьФункциональныеОпции, ИменаРолей, ИспользоватьНаборПолей, НаборПолейТаблица.Выгрузить(), ОбъектМетаданных, ПолеОбъекта, Пользователь,
			ФункциональныеОпцииПолей.ВыгрузитьКолонки(), ЭтаФорма, ЭлементыФормы.ДействияФормы.Кнопки.Сформировать, "СформироватьЗавершение");
		Если Результат <> Неопределено Тогда
			ТаблицаПрав = Результат.ТаблицаПрав;
			ФункциональныеОпцииПолей.Загрузить(Результат.ФункциональныеОпцииПолей);
		КонецЕсли; 
	Иначе
		Результат = Истина;
	КонецЕсли;
...
Процедура СформироватьЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
...
Процедура НаборПолейНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	//СтандартнаяОбработка = Ложь;
	//ФормаВыбора = ирОбщий.ПолучитьФормуВыбораОбъектаМетаданныхЛкс(,, НаборПолей, Истина, Истина, Истина, Истина, Истина, Истина, Истина,, Истина,,, Истина);
	//РезультатВыбора = ФормаВыбора.ОткрытьМодально();
	//Если РезультатВыбора <> Неопределено Тогда
	//	НаборПолей.Загрузить(РезультатВыбора);
	//КонецЕсли; 
