...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено)
	...
	ОписаниеОшибки = "";
	Успех = ирОбщий.ОбработатьСобытиеЛкс(События, Метаданные().Макеты.АлгоритмПередВыполнением.Имя, ОписаниеОшибки, ВременнаяСхемаКомпоновки, ВременныеНастройкиКомпоновки);
	Если Не Успех Тогда
