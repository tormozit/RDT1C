...
Функция ВыполнитьОтчет()
	...
	ИначеЕсли СтраницаРезультата = ЭлементыФормы.ПанельРезультатов.Страницы.РезультатТабличныйДокумент Тогда
		...
		ирОбщий.ВывестиРезультатКомпоновкиСАвтофиксациейСтрокЛкс(ЭлементыФормы.РезультатТабличныйДокумент, ПроцессорКомпоновкиДанных,
			ДанныеРасшифровкиКонсоли.Элементы, , , Автофиксация, мЭлементыРезультата);
...
Процедура СохранитьТекущиеНастройкиВСтроку()
	...
	Тогда
		ирОбщий.ПрисвоитьЕслиНеРавноЛкс(мТекущаяСтрока.Автофиксация, Истина, ЭтаФорма.Модифицированность);
		Для Каждого Тн ИЗ мТекущаяСтрока.Настройки Цикл
			Если Ложь
			...
			Тогда
				ТекущаяНастройкаКомпоновки = КомпоновщикНастроек.ПолучитьНастройки();
...
Процедура РезультатТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Для каждого ЗначениеПоля Из ДанныеРасшифровкиКонсоли.Элементы[Расшифровка].ПолучитьПоля() Цикл
		Если Не ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеПоля.Значение) Тогда
			Продолжить;
		КонецЕсли; 
		ДополнительныеПунктыМеню.Добавить(Новый Структура("Ссылка, ОткрытьВРедактореОбъекта"), "Открыть """ + ЗначениеПоля.Поле + " = " + ЗначениеПоля.Значение + """",
			, ирОбщий.ПолучитьОбщуюКартинкуЛкс("ирРедактироватьОбъектБД"));
	КонецЦикла;
	ВыбранноеДействие = Неопределено;
	НастройкиРасшифровки = ОбработкаРасшифровки.Выполнить(Расшифровка, ВыбранноеДействие,, ДополнительныеПунктыМеню);
	Если Истина
		И ТипЗнч(ВыбранноеДействие) = Тип("Структура")
		И ВыбранноеДействие.Свойство("ОткрытьВРедактореОбъекта")
	Тогда
		ирОбщий.ОткрытьОбъектВРедактореОбъектаБДЛкс(ВыбранноеДействие.Ссылка);
	КонецЕсли; 
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	...
	ИначеЕсли ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		Для Каждого СтрокаПараметра Из ЗначениеВыбора.Параметры Цикл
			...
			ИначеЕсли СтрокаПараметра.ЭтоВыражение = Истина Тогда
...
Процедура РезультатТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельТаблицаЗначений.Кнопки.Идентификаторы.Пометка);
...
Процедура РезультатДеревоПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирОбщий.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельДеревоЗначений.Кнопки.Идентификаторы.Пометка);
