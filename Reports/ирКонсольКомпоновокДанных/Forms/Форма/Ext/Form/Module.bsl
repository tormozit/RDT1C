...
Функция ВыполнитьОтчет()
	...
	Если КомпоновщикНастроек.Настройки.Структура.Количество() = 0 Тогда
		...
		Сообщить("Добавлена группировка ""Детальные записи"", т.к. структура компоновки была пустая");
	КонецЕсли; 
...
Процедура ОткрытьКонструкторСхемы(СтрокаОтчета)
	...
	Если СтрокаОтчета.СхемаКомпоновкиДанных <> Неопределено Тогда
		...
		КопияСхемы = ирОбщий.ПолучитьКопиюОбъектаЛкс(СтрокаОтчета.СхемаКомпоновкиДанных, Истина);
...
Процедура РедактрироватьЗапросСхемы(Знач ВыбраннаяСтрока)
	...
	КонсольЗапросов.РедактироватьНаборДанныхСхемыКомпоновкиДанных(ЭтаФорма, ВыбраннаяСтрока.НаборДанных, мТекущаяСтрока.СхемаКомпоновкиДанных);
...
Процедура КоманднаяПанельФормыНоваяКонсоль(Кнопка)
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ПутьВДереве = ирОбщий.ПолучитьСтрокуПутиВДеревеЛкс(ЭлементыФормы.ДеревоОтчетов.ТекущаяСтрока, "");
		КопияДерева = ДеревоОтчетов.Скопировать();
...
Функция НайтиДобавитьПростуюГруппировкуКомпоновки(Знач ТабличноеПоле, Знач ПолеКомпоновки = Неопределено, Знач Строка = Неопределено)
	...
	Пока ГруппировкиКомпоновки.Количество() > 0 Цикл
...
Процедура КоманднаяПанель1СравнитьСРезультирующим(Кнопка)
	...
	Если МакетКомпоновкиДанных <> Неопределено Тогда
		...
		КоординатыСтроки = ирОбщий.ПолучитьКоординатыСтрокиДереваЛкс(ЭлементыФормы.ДеревоНаборовДанныхЗапросов.ТекущаяСтрока, "Имя");
...
Процедура КоманднаяПанель1Добавить(Кнопка)
	...
	НовыйНаборДанных.Имя = ирОбщий.ПолучитьАвтоУникальноеИмяВКоллекцииЛкс(РодительНового.Строки, "Запрос");
