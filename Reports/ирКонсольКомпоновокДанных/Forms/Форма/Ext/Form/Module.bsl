...
//Перем мРедактируемаяКонструкторомСтрока;
...
Процедура ОбновитьПодменюИсторииФайлов()
	...
	Если Не мРежимРедактора Тогда
		...
		ирНеглобальный.ОбновитьПодменюИсторииФайловЛкс(ИсторияФайлов, Кнопки);
	КонецЕсли; 
...
Процедура СохранитьИмяФайла()
	...
	Если ЗначениеЗаполнено(мИмяФайла) Тогда
		ирНеглобальный.ДобавитьВИсториюЭлементЛкс(ИсторияФайлов, мИмяФайла);
...
Функция СохранитьЗапросыВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь)
	...
	Иначе
		Если ПустаяСтрока(мИмяФайла) или ЗапрашиватьИмяФайла Тогда
			...
			Длг.Фильтр = ирНеглобальный.ПолучитьСтрокуФильтраДляВыбораФайлаЛкс("dcf", "Файл консоли компоновки");
			Длг.Расширение = "dcf";
		...
		Если НЕ ЗначениеВФайл(мИмяФайла, ДеревоОтчетов) Тогда
...
Процедура ЗагрузитьНастройки(СтрокаНастроек)
	...
	ЛксОбновитьЗначенияНедоступныхПараметровИзСхемы(КомпоновщикНастроек, мТекущаяСтрока.СхемаКомпоновкиДанных);
...
Процедура ИнициализироватьКомпоновщикНастроек(Строка)
	...
	Если Не НастройкиЗагружены Тогда
		...
		КонецЕсли;
		ЛксОбновитьЗначенияНедоступныхПараметровИзСхемы(КомпоновщикНастроек, СхемаКомпоновкиДанных);
...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено)
	...
	КонецЕсли;
	//МакетКомпоновкиДанных = ЛксПолучитьМакетКомпоновкиДанныхСВременнымиТаблицами(СхемаКомпоновкиДанныхКонсоли,
	...
	Если ЛиОтладка Тогда
		ЛксСообщитьСУчетомМодальности("Компоновка макета в " + ИмяТипаГенератора + " - " 
...
Функция ВыполнитьОтчет()
	...
	Если ЛиОтладка Тогда
		...
		Для Каждого ЭлементНаборДанныхМакета Из СтруктураНаборовДанныхЗапросовМакета Цикл
			...
			ЛксВыполнитьЗамеритьЗапрос(Запрос, ЛиОтладка, "Запрос - " + НаборДанных.Имя)
		КонецЦикла;
	КонецЕсли;
	СтраницаРезультата = ЭлементыФормы.ПанельРезультатов.ТекущаяСтраница;
	...
	ИначеЕсли СтраницаРезультата = ЭлементыФормы.ПанельРезультатов.Страницы.РезультатТабличныйДокумент Тогда
		ЛксВывестиРезультатКомпоновкиСАвтофиксациейСтрок(ЭлементыФормы.РезультатТабличныйДокумент, ПроцессорКомпоновкиДанных,
	...
	Если ЛиОтладка Тогда
		ЛксСообщитьСУчетомМодальности("Формирование результата - " + Строка(ирКэш.Получить().ПолучитьТекущееВремяВМиллисекундах() - НачалоВывода)
			+ " мс", МодальныйРежим);
	КонецЕсли;
...
Процедура ПоказатьСвернутьНастройки(Видимость = Истина)
	...
	ЛксИзменитьСвернутость(Видимость, ЭлементыФормы.ПанельВсехНастроек, ЭлементыФормы.РазделительГоризонтальныйПодНастройками, ЭтаФорма.Панель, "верх");
...
Процедура СкопироватьПараметрыСтроки(ИсходнаяСтрока, КонечнаяСрока)
	...
	КонечнаяСрока.СхемаКомпоновкиДанных = ЛксПолучитьКопиюОбъекта(ИсходнаяСтрока.СхемаКомпоновкиДанных);
	КонечнаяСрока.Настройки = ЛксПолучитьКопиюОбъекта(ИсходнаяСтрока.Настройки);
...
Процедура ДеревоОтчетовПередНачаломИзменения(Элемент, Отказ)
	...
	Отказ = Истина;
	Если мТекущаяСтрока <> Неопределено Тогда
		//мРедактируемаяКонструкторомСтрока = мТекущаяСтрока;
		КонструкторСхемы = мКонструкторыСхем[мТекущаяСтрока];
		Если КонструкторСхемы = Неопределено Тогда
			...
			Если мТекущаяСтрока.СхемаКомпоновкиДанных <> Неопределено Тогда
				...
				КопияСхемы = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(мТекущаяСтрока.СхемаКомпоновкиДанных));
				КонструкторСхемы.УстановитьСхему(КопияСхемы);
			КонецЕсли;
			мКонструкторыСхем[мТекущаяСтрока] = КонструкторСхемы;
		КонецЕсли; 
...
Процедура ДеревоОтчетовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель)
	...
	КонецЕсли;
	Если мТекущаяСтрока <> Неопределено Тогда
		//мРедактируемаяКонструкторомСтрока = НоваяСтрока;
		КонструкторСхемы = Новый КонструкторСхемыКомпоновкиДанных;
		мКонструкторыСхем[мТекущаяСтрока] = КонструкторСхемы;
		КонструкторСхемы.УстановитьСхему(мТекущаяСтрока.СхемаКомпоновкиДанных);
...
Процедура РезультатТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если НастройкиРасшифровки <> Неопределено Тогда
		...
		ЛксОбновитьЗначенияНедоступныхПараметровИзСхемы(КомпоновщикНастроек, СхемаКомпоновкиДанныхКонсоли);
		Если ВыполнитьОтчет() Тогда
		    ЗаписатьИсторию();
			ОбновитьКнопкиИстории();
		КонецЕсли;
...
Процедура ПриОткрытии()
	...
	Если СхемаКомпоновкиДанных <> Неопределено Тогда
		...
		ГлавнаяСтрока.СхемаКомпоновкиДанных = ЛксПолучитьКопиюОбъекта(СхемаКомпоновкиДанных);
	Иначе
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	...
	ИначеЕсли ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		ТекущаяСхемаКомпоновкиДанных = ЭлементыФормы.ДеревоОтчетов.ТекущаяСтрока.СхемаКомпоновкиДанных;
		...
		КонецЦикла;
		КонструкторСхемы = Новый КонструкторСхемыКомпоновкиДанных;
		КонструкторСхемы.УстановитьСхему(ТекущаяСхемаКомпоновкиДанных);
		КонструкторСхемы.Редактировать(ЭтаФорма);
...
Процедура ЗаполнитьСтрокиДереваНаборовДанныхЗапросов(СтрокиДерева, НаборыДанных)
	...
	Для Каждого НаборДанных Из НаборыДанных Цикл
		...
		НоваяСтрока.ТипНабораДанных = Новый ОписаниеТипов(ЛксБыстрыйМассив(ТипЗнч(НаборДанных)));
...
Процедура ДеревоНаборовДанныхЗапросовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	КонецЕсли;
	//мРедактируемаяКонструкторомСтрока = мТекущаяСтрока;
	Обработки.ирКонсольЗапросов.Создать().РедактироватьНаборДанныхСхемыКомпоновкиДанных(ЭтаФорма,
...
Процедура ОбновитьТаблицыСхемы()
	...
	Если мТекущаяСтрока <> Неопределено Тогда
		Если мТекущаяСтрока.СхемаКомпоновкиДанных <> Неопределено Тогда
			...
			ЗаполнитьСтрокиДереваНаборовДанныхЗапросов(КорневаяСтрока.Строки, лСхемаКомпоновкиДанных.НаборыДанных);
			ЭлементыФормы.ДеревоНаборовДанныхЗапросов.Развернуть(ДеревоНаборовДанныхЗапросов.Строки[0], Истина);
			ирНеглобальный.ДополнитьСтруктуруВнешихНаборовДанныхПустышкамиЛкс(лСхемаКомпоновкиДанных, мВнешниеНаборыДанных);
...
Процедура ДеревоНаборовДанныхЗапросовПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если ДанныеСтроки.ТипНабораДанных.Типы().Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 
	ТипНабораДанных = ДанныеСтроки.ТипНабораДанных.Типы()[0];
...
Процедура КоманднаяПанельФормыИсполняемыеЗапросы(Кнопка)
	...
	МакетКомпоновкиДанных = ПолучитьМакетКомпоновки();
	Если МакетКомпоновкиДанных <> Неопределено Тогда
		Обработки.ирКонсольЗапросов.Создать().ОткрытьПоМакетуКомпоновки(МакетКомпоновкиДанных, , СхемаКомпоновкиДанныхКонсоли);
	КонецЕсли; 
...
Процедура КоманднаяПанельФормыНаборыДанныхОбъекты(Кнопка)
	...
	Исследовать(мВнешниеНаборыДанных, Ложь);
...
Процедура КоманднаяПанельФормыСравнить(Кнопка)
	...
	ЛксСравнитьСодержимоеЭлементаУправления(СравнительТабличныхДокументов, ЭлементыФормы.РезультатТабличныйДокумент);
...
Процедура РезультатТаблицаПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ирНеглобальный.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельТаблицаЗначений.Кнопки.Пустые.Пометка);
...
Процедура СвернутьНажатие(Элемент)
	...
	ЛксДеревоЗначенийСвернуть(ЭлементыФормы.ДеревоЗначений, истина);
	
КонецПроцедуры

Процедура РазвернутьНажатие(Элемент)
	
	ЛксДеревоЗначенийСвернуть(ЭлементыФормы.ДеревоЗначений, Ложь);
	
КонецПроцедуры

Процедура РезультатДеревоПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	
	ирНеглобальный.ТабличноеПолеПриВыводеСтрокиЛкс(Элемент, ОформлениеСтроки, ДанныеСтроки, ЭлементыФормы.КоманднаяПанельДеревоЗначений.Кнопки.Пустые.Пометка);
	
КонецПроцедуры

Процедура КоманднаяПанельТаблицаШиринаКолонок(Кнопка)
	
	ЛксВвестиИУстановитьШиринуКолонокТабличногоПоля(ЭлементыФормы.ТаблицаЗначений);
	
КонецПроцедуры

Процедура КоманднаяПанельТаблицаСжатьКолонки(Кнопка)
	
	ЛксСжатьКолонкиТабличногоПоля(ЭлементыФормы.ТаблицаЗначений);
	
КонецПроцедуры

Процедура КоманднаяПанельДеревоСжатьКолонки(Кнопка)
	
	ЛксСжатьКолонкиТабличногоПоля(ЭлементыФормы.ДеревоЗначений);
	
КонецПроцедуры

Процедура КоманднаяПанельДеревоШиринаКолонок(Кнопка)
	
	ЛксВвестиИУстановитьШиринуКолонокТабличногоПоля(ЭлементыФормы.ДеревоЗначений);
...
Процедура КоманднаяПанельТабличныйДокументМакетКомпоновки(Кнопка)
	...
	МакетКомпоновкиДанных = ПолучитьМакетКомпоновки();
	Если МакетКомпоновкиДанных <> Неопределено Тогда
		Исследовать(МакетКомпоновкиДанных);
	КонецЕсли; 
...
Процедура ПараметрыДанныхПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	//ОформлениеСтроки.Ячейки.Значение.УстановитьТекст(ЛксПолучитьРасширенноеПредставлениеЗначения(ДанныеСтроки.Значение));
...
Процедура КоманднаяПанель9Исследовать(Кнопка)
	...
	Обработки.ирИсследовательОбъектов.Создать().ИсследоватьОбъект(ЭтаФорма.ЭлементыФормы.ПараметрыДанных.Значение);
...
Процедура КоманднаяПанельФормыСхемаКомпоновки(Кнопка)
	...
	Исследовать(СхемаКомпоновкиДанных, Ложь);
	
КонецПроцедуры

Процедура КоманднаяПанельФормыНастройкиКомпоновки(Кнопка)
	
	Исследовать(КомпоновщикНастроек.Настройки, Ложь);
	
КонецПроцедуры

Процедура КоманднаяПанельТаблицаЗначенийКонсольОбработки(Кнопка)
	
	ирНеглобальный.ОткрытьОбъектыИзВыделенныхЯчеекВПодбореИОбработкеОбъектов(ЭтаФорма.ЭлементыФормы.ТаблицаЗначений);
	
КонецПроцедуры

Процедура ДеревоОтчетовПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	
	ЛксДеревоКонсолиПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка, "СтрокаДереваКомпоновок");
...
Процедура ДеревоОтчетовНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	СохранитьТекущиеНастройкиВСтроку();
	ЛксДеревоКонсолиНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение, "СтрокаДереваКомпоновок");
	
КонецПроцедуры

Процедура ДеревоОтчетовПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка)
	
	ЛксДеревоКонсолиПеретаскивание(ЭтаФорма, Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Колонка, "СтрокаДереваКомпоновок");
	
КонецПроцедуры

Процедура КоманднаяПанельДеревоЗаполнитьГруппыДублейДляЗамены(Кнопка)
	
	ЛксОткрытьФормуЗаменыСсылокИзТабличногоПоля(ЭтаФорма.ЭлементыФормы.ДеревоЗначений);
	
КонецПроцедуры

Процедура КоманднаяПанельТаблицаЗаполнитьГруппуДублейДляЗамены(Кнопка)
	
	ЛксОткрытьФормуЗаменыСсылокИзТабличногоПоля(ЭтаФорма.ЭлементыФормы.ТаблицаЗначений);
...
Процедура ПараметрыДанныхЗначениеОкончаниеВводаТекста(Элемент, Текст, Значение, СтандартнаяОбработка)
	...
	ирНеглобальный.ПолеВвода_ОкончаниеВводаТекстаЛкс(Элемент, Текст, Значение, СтандартнаяОбработка, ЭлементыФормы.ПараметрыДанных.ТекущаяСтрока.Значение);
...
Процедура ПараметрыДанныхЗначениеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ирНеглобальный.ПолеВводаРасширенногоЗначения_НачалоВыбораЛкс(ТабличноеПоле, СтандартнаяОбработка, , Истина);
...
Процедура КоманднаяПанельТаблицаЗначенийРедакторОбъектаБД(Кнопка)
	...
	ирНеглобальный.ОткрытьСсылкуЯчейкиВРедактореОбъектаБДЛкс(ЭлементыФормы.ТаблицаЗначений);
	
КонецПроцедуры

Процедура КоманднаяПанельДеревоЗначенийРедакторОбъектаБД(Кнопка)
	
	ирНеглобальный.ОткрытьСсылкуЯчейкиВРедактореОбъектаБДЛкс(ЭлементыФормы.ДеревоЗначений);

КонецПроцедуры

Процедура КоманднаяПанельТаблицаЗначенийОткрытьТаблицу(Кнопка)

	ирНеглобальный.ОткрытьФормуПроизвольногоЗначенияЛкс(ТаблицаЗначений, Ложь);
	
КонецПроцедуры

Процедура КоманднаяПанельДеревоЗначенийОткрытьТаблицу(Кнопка)
	
	ирНеглобальный.ОткрытьФормуПроизвольногоЗначенияЛкс(ДеревоЗначений, Ложь);
...
Процедура РесурсыСхемыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	//мРедактируемаяКонструкторомСтрока = мТекущаяСтрока;
	//лСхемаКомпоновкиДанных = мРедактируемаяКонструкторомСтрока.СхемаКомпоновкиДанных;
	Форма = ПолучитьФорму("КонструкторВыраженияКомпоновки", ЭтаФорма);
	СхемаКомпоновкиДанныхКонсоли = ПолучитьСхемуКомпоновкиДанных();
...
ЛксПодключитьОбработчикиСобытийДоступныхПолейКомпоновки(ЭлементыФормы.ДоступныеПоляОтбора);
ЛксПодключитьОбработчикиСобытийДоступныхПолейКомпоновки(ЭлементыФормы.ДоступныеПоляПолейГруппировок);
