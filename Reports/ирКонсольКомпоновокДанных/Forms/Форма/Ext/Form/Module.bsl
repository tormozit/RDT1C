...
Функция ПолучитьСхемуКомпоновки()
	...
	Если мТекущаяСтрока <> Неопределено Тогда
		Возврат ирОбщий.ПолучитьКопиюОбъектаЛкс(мТекущаяСтрока.СхемаКомпоновкиДанных);
	Иначе 
...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено)
	...
	ВременнаяСхемаКомпоновки = ПолучитьСхемуКомпоновки();
	Если ВременнаяСхемаКомпоновки = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	ВременныеНастройкиКомпоновки = ПолучитьНастройкиКомпоновки();
	ОписаниеОшибки = "";
	Успех = ирОбщий.ОбработатьСобытиеЛкс(События, Метаданные().Макеты.АлгоритмПередВыполнением.Имя, ОписаниеОшибки, ВременнаяСхемаКомпоновки, ВременныеНастройкиКомпоновки, мВнешниеНаборыДанных);
	Если Не Успех Тогда
...
Функция ВыполнитьОтчет()
	...
	Результат = ирОбщий.СкомпоноватьОтчетВКонсолиЛкс(КоллекцияВывода, МакетКомпоновкиДанных, мВнешниеНаборыДанных, Автофиксация, МодальныйРежим, ЛиОтладка, АдресДанныхРасшифровки, НаСервере);
...
Процедура СобытияАлгоритмНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Для Каждого СтрокаПараметра Из ТекущаяСтрокаСобытия.Параметры Цикл
		Если ирОбщий.СтрокиРавныЛкс(СтрокаПараметра.Имя, "СхемаКомпоновки") Тогда
			СтрокаПараметра.Значение = ПолучитьСхемуКомпоновки();
		...
		ИначеЕсли ирОбщий.СтрокиРавныЛкс(СтрокаПараметра.Имя, "ВнешниеНаборыДанных") Тогда
			СтрокаПараметра.Значение = мВнешниеНаборыДанных;
		КонецЕсли; 
	КонецЦикла;
