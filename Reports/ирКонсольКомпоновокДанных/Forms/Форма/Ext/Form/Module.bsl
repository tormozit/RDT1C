...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено)
	...
	//	КомпоновщикНастроек.Настройки, мВнешниеНаборыДанных, ДанныеРасшифровки, ЛиОтладка, СвойМакетОформления);
	...
	МакетКомпоновкиДанных = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанныхКонсоли, КомпоновщикНастроек.Настройки,
		ДанныеРасшифровки, СвойМакетОформления, Тип(ИмяТипаГенератора), ПроверятьДоступностьПолей);
	Если ЛиОтладка Тогда
...
Процедура КоманднаяПанельРеультатаКопия(Кнопка)
	...
	ЗаполнитьЗначенияСвойств(ТабДок, ЭлементыФормы.РезультатТабличныйДокумент); 
	ТабДок.ИмяСохраненияПоложенияОкна = "ирКонсольКомпоновки.Результат";
	Результат = ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ТабДок,,,, Ложь);
...
Процедура РезультатТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Для каждого ЗначениеПоля Из ДанныеРасшифровки.Элементы[Расшифровка].ПолучитьПоля() Цикл
		Если Не ирОбщий.ЛиСсылкаНаОбъектБДЛкс(ЗначениеПоля.Значение) Тогда
			Продолжить;
		КонецЕсли; 
		ДополнительныеПунктыМеню.Добавить(Новый Структура("Ссылка, ОткрытьВРедактореОбъекта", ЗначениеПоля.Значение), "Открыть """ + ЗначениеПоля.Поле + " = " + ЗначениеПоля.Значение + """",
...
Процедура КоманднаяПанельСтруктураСохранитьКакНастройкуПоУмолчанию(Кнопка)
	...
	СхемаXDTO.settingsVariant[ВыбранныйВариант.Значение].settings = НастройкиXDTO;
...
Процедура КоманднаяПанельТаблицаСравнить(Кнопка)
	...
	ирОбщий.СравнитьСодержимоеЭлементаУправленияЛкс(СравнительТабличныхДокументов, ЭлементыФормы.ТаблицаЗначений);
...
Процедура КоманднаяПанель4РедактироватьСписок(Кнопка)
...
Процедура КоманднаяПанельТабличныйДокументПередатьВПодборИОбработкуОбъектов(Кнопка)
	...
	ТаблицаЗначений = ирОбщий.ПолучитьТаблицуКлючейИзСкомпонованногоТабличногоДокументаЛкс(ЭлементыФормы.РезультатТабличныйДокумент, ДанныеРасшифровки);
