...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено, Предпросмотр = Ложь)
	...
	Исключение
		...
		Если Не Предпросмотр И ЗначениеЗаполнено(ПолеИзОшибки) Тогда
			НайденныйЭлемент = ирОбщий.НайтиЭлементНастроекКомпоновкиПоПолюИерархическиЛкс(КомпоновщикНастроек.Настройки, Новый ПолеКомпоновкиДанных(ПолеИзОшибки), Истина);
