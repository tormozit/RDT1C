...
Процедура ОбновитьТаблицыСхемы()
	...
	ВнешниеНаборыДанныхКопия = ВнешниеНаборыДанных.Скопировать();
	...
	Если мТекущаяСтрока <> Неопределено Тогда
		Если мТекущаяСтрока.СхемаКомпоновкиДанных <> Неопределено Тогда
			...
			ирОбщий.ДополнитьСтруктуруВнешихНаборовДанныхПустышкамиЛкс(лСхемаКомпоновкиДанных, СтруктураВнешниеНаборыДанных);
			Для Каждого КлючИЗначение Из СтруктураВнешниеНаборыДанных Цикл
				НоваяСтрока = ВнешниеНаборыДанных.Добавить();
				НоваяСтрока.Имя = КлючИЗначение.Ключ;
				СтараяСтрокаНабора = ВнешниеНаборыДанныхКопия.Найти(КлючИЗначение.Ключ, "Имя");
				Если СтараяСтрокаНабора <> Неопределено Тогда
					НоваяСтрока.Значение = СтараяСтрокаНабора.Значение;
				Иначе
					НоваяСтрока.Значение = КлючИЗначение.Значение;
				КонецЕсли; 
			КонецЦикла;
