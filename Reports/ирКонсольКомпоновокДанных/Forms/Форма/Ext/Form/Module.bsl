...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено)
	...
	//	КомпоновщикНастроек.ПолучитьНастройки(), мВнешниеНаборыДанных, ДанныеРасшифровки, ЛиОтладка, СвойМакетОформления);
	Если ИмяТипаГенератора = Неопределено Тогда 
		...
		КонецЕсли;
	КонецЕсли;
	МакетКомпоновкиДанных = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанныхКонсоли, КомпоновщикНастроек.ПолучитьНастройки(),
...
Процедура КоманднаяПанельРеультатаКопия(Кнопка)
	...
	Результат = ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ТабличныйДокумент,,,, Ложь);
