...
Функция ПолучитьМакетКомпоновки(ИмяТипаГенератора = Неопределено)
	...
	Если ИмяТипаГенератора = Неопределено Тогда 
		...
		КонецЕсли;
	КонецЕсли;
	МакетКомпоновкиДанных = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанныхКонсоли, ПолучитьНастройкиКомпоновки(),
		ДанныеРасшифровкиЛ, СвойМакетОформления, Тип(ИмяТипаГенератора), ПроверятьДоступностьПолей);
...
Процедура ПараметрыДанныхПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если мСхемаКомпоновкиДанных.Параметры.Найти("" + ДанныеСтроки.Параметр).ОграничениеИспользования Тогда
...
Процедура КоманднаяПанельТаблицаЗначенийОткрытьТаблицу(Кнопка)
	...
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ТаблицаЗначений, Ложь,,,,, ЭлементыФормы.ТаблицаЗначений);
	
КонецПроцедуры

Процедура КоманднаяПанельДеревоЗначенийОткрытьТаблицу(Кнопка)
	
	ирОбщий.ОткрытьФормуПроизвольногоЗначенияЛкс(ДеревоЗначений, Ложь,,,,, ЭлементыФормы.ДеревоЗначений);
