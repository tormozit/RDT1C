...
Функция СхемаКомпоновки() Экспорт 
	...
	Для Каждого ПолеНабора Из Схема.НаборыДанных[0].Поля Цикл
		...
		Если СтрокаСлова = Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		СтрокиЗначений = мПлатформа.ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", СтрокаСлова.ТипЗначения, "Свойство"));
		Если СтрокиЗначений.Количество() > 0 Тогда
			Попытка
				Пустышка = Вычислить("Метаданные.СвойстваОбъектов." + СтрокаСлова.ТипЗначения);
			Исключение
				Продолжить;
			КонецПопытки;
		Иначе
			СтрокиЗначений = мПлатформа.ТаблицаКонтекстов.НайтиСтроки(Новый Структура("ТипКонтекста, ТипСлова", "Перечисление" + СтрокаСлова.ТипЗначения, "Свойство"));
			Если СтрокиЗначений.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли; 
		КонецЕсли; 
