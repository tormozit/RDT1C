...
Процедура ДействияФормыСформировать(Кнопка = Неопределено) Экспорт 
	...
	Тогда
		Результат = ирОбщий.СобратьОбъектыМДОдногоТипаЛкс(мОбъекты, ТипОбъектов, РасширенныеПредставления, мКолонкиРасширенногоПредставления,
			ЭтаФорма, ЭлементыФормы.ДействияФормы.Кнопки.Сформировать, "СобратьОбъектыМДЗавершение");
	Иначе
		Результат = Истина;
	КонецЕсли; 
...
Процедура СобратьОбъектыМДЗавершение(СостояниеЗадания = Неопределено, РезультатЗадания = Неопределено, ПараметрыЗадания = Неопределено) Экспорт 
	
	Если Ложь
		Или СостояниеЗадания = Неопределено
		Или СостояниеЗадания = СостояниеФоновогоЗадания.Завершено 
	Тогда
		мОбъекты = РезультатЗадания;
		СкомпоноватьРезультатПоГотовымТаблицам();
	КонецЕсли; 
...
Процедура НастроитьКомпоновщик(ЗаполнитьНастройки = Истина)
	...
	Если ЗаполнитьНастройки Тогда
		...
		Если Не ирОбщий.ЭтоКорневойТипМетаданныхЛкс(ТипОбъектов) Тогда 
